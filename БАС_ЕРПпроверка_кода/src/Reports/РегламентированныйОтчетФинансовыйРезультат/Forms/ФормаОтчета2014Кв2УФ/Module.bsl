...
//		Показатель2 = Показатель2 + ОБ2(счетД,счетК);
...
//---------------------------------------------------------------
    ...
    Для каждого элД  Из СписокСчетовДт Цикл
	  ...
	  Для каждого элК  Из СписокСчетовКт Цикл
		...
		КонецЕсли; 
		Инд = Инд+1;	
//		Показатель1 = Показатель1 + ОБ(счетД,счетК);
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	_ДобавитьСтрокуРасшифровки( ЭтаФорма,"_П23003", "Показник ""Податок на прибуток від звичайної діяльності"", графа ""За звітний період""",	"+", "!ДО",П23003, Счет98,,НачалоГода (мДатаНачалаПериодаОтчета), мДатаКонцаПериодаОтчета);
	
	П23004 = ДО2(мБухИтоги22, Счет98);
	СтруктураДанныхФинРез["П23004"] = П23004;
	_ДобавитьСтрокуРасшифровки( ЭтаФорма,"_П23004", "Показник ""Податок на прибуток від звичайної діяльності"", графа ""За попередній період""",	"+", "!ДО",П23004, Счет98,,НачалоГода (ДобавитьМесяц(мДатаНачалаПериодаОтчета,-12)), ДобавитьМесяц(мДатаКонцаПериодаОтчета, -12));
	...
	Для каждого Элемент Из СтруктураВход Цикл
		...
		Если ИмяПанели  = "ФинРез" Тогда
			...
			Если Лев(ИмяСтарое, 1) = "П" Тогда
				// для J
				Структура.Вставить("R"+Сред(ИмяСтарое, 2,4) + "G" + Прав(ИмяСтарое, 1), Элемент.Значение);
				
				// для S
				Структура.Вставить(?(Прав(ИмяСтарое, 1) = "3", "A","B")+Сред(ИмяСтарое, 2,4) , Элемент.Значение);
