...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(СписокЗаказов) Тогда
		ТекстСообщения = НСтр("ru='Непосредственное открытие отчета ""Состояние выполнения заказа на производство"" не предусмотрено. 
            ...
            |;uk='Безпосереднє відкриття звіту ""Стан виконання замовлення на виробництво"" не передбачено. 
            |Для відкриття звіту можна скористатися командою ""Стан виконання"" в формах документів.'");
...
Процедура СостояниеЗаказовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если СписокЭтапов.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Этапы не выполнены. Расшифровка невыполненных этапов невозможна.';uk='Етапи не виконані. Розшифровка невиконаних етапів неможлива.'");
...
Функция ОбновитьДанныеНаСервере(СохранитьПараметрыОтбора = Ложь, ОбновитьСвойстваЭлементовФормы = Ложь)
	...
	Если ЗапуститьВыполнениеВФоне Тогда
		...
		НаименованиеЗадания = НСтр("ru='Получение состояния выполнения заказов на производство';uk='Отримання стану виконання замовлень на виробництво'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Не рассчитана';uk='Не розрахована'"));
...
Процедура УстановитьЗаголовок(ЗаказПолностьюВыполнен = Ложь)
	...
	Если ОтборЗаказНаПроизводство.Количество() = 1 Тогда
		ТекстЗаголовок = НСтр("ru='Состояние выполнения заказа %1';uk='Стан виконання замовлення %1'");
		...
		Если ЗаказПолностьюВыполнен Тогда
			ДопТекст = ДопТекст + " " + НСтр("ru='(заказ полностью выполнен)';uk='(замовлення повністю виконаний)'");
		КонецЕсли;
	Иначе
		ТекстЗаголовок = НСтр("ru='Состояние выполнения заказов %1';uk='Стан виконання замовлень %1'");
		Если ЗаказПолностьюВыполнен Тогда
			ДопТекст = НСтр("ru='(все заказы выполнены)';uk='(всі замовлення виконано)'");
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьСвойстваЭлементовФормы()
	...
	Если ЦенаВключаетНДС Тогда
		ШаблонПредставления = НСтр("ru='%1 (с НДС)';uk='%1 (з ПДВ)'");
	Иначе
		ШаблонПредставления = НСтр("ru='%1 (без НДС)';uk='%1 (без ПДВ)'");
	КонецЕсли;
	...
	Элементы.ГруппаКолонокЗатраты.Заголовок = НСтр("ru='Затраты';uk='Витрати'") + ", " + ЗаголовокКолонкиЗатрат;
