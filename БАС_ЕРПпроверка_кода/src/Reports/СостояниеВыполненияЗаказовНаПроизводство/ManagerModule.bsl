...
Процедура СостояниеВыполненияЗаказовНаПроизводство(Параметры, АдресХранилища) Экспорт
	...
	СостояниеЗаказов.Колонки.Добавить("ЗатратыПлан", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2, ДопустимыйЗнак.Неотрицательный)));
	СостояниеЗаказов.Колонки.Добавить("ЗатратыФакт", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2, ДопустимыйЗнак.Неотрицательный)));
	...
	|	ЗаказНаПроизводствоЭтапы.Подразделение КАК ПодразделениеИзготовитель,
 	|	&ЕдиницаИзмеренияЭтапа КАК ЕдиницаИзмерения,
	...
	|	ЗаказПереработчикуПродукция.Склад,
	...
	Запрос.УстановитьПараметр("ЕдиницаИзмеренияЭтапа", НСтр("ru='единиц/партий';uk='одиниць/партій'"));
	...
	Пока ВыборкаПродукция.Следующий() Цикл
		...
		СтрокаЗаказаПродукция.ПредставлениеЗаказа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru='Заказ № %1 от %2 (строка %3), дата потребности %4';uk='Замовлення № %1 від %2 (рядок %3), дата потреби %4'"),
		...
		Если НЕ ВыборкаПродукция.ГрафикРассчитан И НЕ ВыборкаПродукция.ВсеЗапланированоПроизводством Тогда
			СтрокаЗаказаПродукция.ПредставлениеЗаказа = СтрокаЗаказаПродукция.ПредставлениеЗаказа + ". " + НСтр("ru='График не рассчитан.';uk='Графік не розрахований.'");
			Продолжить;
		КонецЕсли;
		...
		Если СтрокаПродукция.Осталось = 0 Тогда
			...
			СтрокаЗаказаПродукция.ПредставлениеЗаказа = СтрокаЗаказаПродукция.ПредставлениеЗаказа + ". " + НСтр("ru='Выполнено.';uk='Виконано.'");
...
Функция ЗатратыПоЗаказам(Параметры)
	...
	Если Параметры.ВалютаЗатрат = 1 Тогда
		...
		Коэффициенты = РаботаСКурсамивалютУТ.ПолучитьКоэффициентыПересчетаВалюты(ВалютаПлановойСтоимости,
			...
			Параметры.ТекущаяДатаСеанса);
		
		КоэфПересчетаПлан = Коэффициенты.КоэффициентПересчетаВВалютуУпр;
		КоэфПересчетаФакт = 1;
	...
	ИначеЕсли Параметры.ВалютаЗатрат = 2 Тогда
		...
		КоэфПересчетаПлан = 1;
		КоэфПересчетаФакт = Коэффициенты.КоэффициентПересчетаВВалютуВзаиморасчетов;
	...
	ИначеЕсли Параметры.ВалютаЗатрат = 3 Тогда
		...
		КоэфПересчетаПлан = Коэффициенты.КоэффициентПересчетаВВалютуРегл;
		КоэфПересчетаФакт = 1;
	...
	|	СУММА(РесурсныеСпецификацииВыходныеИзделия.Количество) КАК КоличествоВыходныхИзделийСпецификации,
	...
	|				И ЗаказНаПроизводствоПродукция.Характеристика = РесурсныеСпецификацииВыходныеИзделия.Характеристика
	...
	|	ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки, 1) КАК Цена,
	...
	|		КОГДА &ЦенаВключаетНДС
	|			ТОГДА СтоимостьЗатрат.Стоимость + СтоимостьЗатрат.СтоимостьДопРасходы
	|		ИНАЧЕ СтоимостьЗатрат.СтоимостьБезНДС + СтоимостьЗатрат.СтоимостьДопРасходыБезНДС
	...
	|		ПО ПартииНЗП.АналитикаУчетаНоменклатуры = Аналитика.КлючАналитики
	...
	|				ИЛИ НЕ Константы.УчитыватьСебестоимостьТоваровПоВидамЗапасов)
