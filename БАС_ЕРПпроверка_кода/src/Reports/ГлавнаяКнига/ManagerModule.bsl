...
Процедура СформироватьОтчет(Знач ПараметрыОтчета, АдресХранилища) Экспорт
	...
	ДокументРезультат.ИмяПараметровПечати = "ГлавнаяКнига";
	
	УправлениеКолонтитулами.УстановитьКолонтитулы(ДокументРезультат, НСтр("ru='Главная книга';uk='Головна книга'"));
...
Функция СформироватьЗаголовок(ПараметрыОтчета)
	...
	ТекстПроСписокПоказателей = НСтр("ru='Выводимые данные: сумма';uk='Виведені дані: сума'");
	ТекстПроИтоги = "";
	ТекстПроИтоги = Сред(ТекстПроИтоги, 3);

	Если Не ПустаяСтрока(ТекстПроИтоги) Тогда
		ЗаголовокОтчета.Параметры.ТекстПроИтоги = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Детализация по %1';uk='Деталізація за %1'"), ТекстПроИтоги);
	КонецЕсли;
	
	Если ПараметрыОтчета.ВыводитьЕдиницуИзмерения Тогда
		ЗаголовокОтчета.Параметры.ТекстПроИтоги = СокрЛП("" + ЗаголовокОтчета.Параметры.ТекстПроИтоги + Символы.ПС + НСтр("ru='Единица измерения: гривна';uk='Одиниця виміру: гривня'"));
	КонецЕсли;

	ЗаголовокОтчета.Параметры.Заголовок = НСтр("ru='Главная книга';uk='Головна книга'");
...
Процедура ВывестиШапкуТаблицы(ПараметрыОтчета, СтруктураПараметров, Выборка, ШиринаВыводимогоРаздела)
	...
	НачалоЗаголовка.Параметры.ОписаниеСчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Главная книга. Счет %1 ""%2""';uk='Головна книга. Рахунок %1 ""%2""'"),
		Выборка.СчетПредставление,
		Выборка.СчетНаименование);
	Если ЗначениеЗаполнено(ПараметрыОтчета.Периодичность) Тогда
	...
	Если СтруктураПараметров.РазвернутоеСальдо Тогда
		...
		Иначе	
			Если ЗначениеЗаполнено(СтруктураПараметров.СтрокаРазвернутогоСальдо.ПоСубконто) Тогда
				...
				ДанныеСчета = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(СтруктураПараметров.Счет);
		...
		Запрос.УстановитьПараметр("Организация", ПараметрыОтчета.Организация);
		Запрос.УстановитьПараметр("Субсчета",    БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(СтруктураПараметров.Счет));
...
Функция ПолучитьСписокРасшифровок(ПараметрыРасшифровки, ОтчетПоПроводкам = Истина)
	...
	Если ПараметрыРасшифровки <> Неопределено Тогда		
		...
		СписокРасшифровки.Добавить(КопияРасшифровки, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
														НСтр("ru='Обороты счета %1';uk='Обороти рахунку %1'"), СтрокаСчета));
		
		Если ОтчетПоПроводкам Тогда
			КопияРасшифровки = СоздатьКопиюСоответствияСтруктуры(ПараметрыРасшифровки);
			КопияРасшифровки.Вставить("ИмяОбъекта", "ОтчетПоПроводкам");
			СписокРасшифровки.Добавить(КопияРасшифровки, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
															НСтр("ru='Отчет по проводкам %1';uk='Звіт по проведках %1'"), СтрокаСчета));
		КонецЕсли;		
	Иначе		
