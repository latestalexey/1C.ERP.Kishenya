...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2012Кв1УФ";
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/01/2012";
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = НСтр("ru='Отчет Ф4 - ФСС з ТВП';uk='Звіт Ф4 - ФСС з ТВП'");
...
Процедура ЗаполнитьДатуПодписи(Форма)
	...
	Если ЧастиДатыПодписи.Свойство("ПоследняяЦифраГода") Тогда
		ЗаполнитьПоказательНаСервере("ГодДаты",   ЧастиДатыПодписи.ПоследняяЦифраГода, Форма, "Уведомление");
	Иначе
		ЗаполнитьПоказательНаСервере("ГодДаты", Прав(Формат(Форма.ДатаПодписи, "ДФ=yy"), 1), Форма, "Уведомление");
	Конецесли;
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("NSTRAX", Организация.НаименованиеПолное, Форма, "Уведомление");

	СписокПоказателей = Новый СписокЗначений;
	СписокПоказателей.Добавить("","КодПоЕДРПОУ");
	СписокПоказателей.Добавить("","КодЗКГНГ");
	СписокПоказателей.Добавить("","НомерФССУтратаТрудосп");
	СписокПоказателей.Добавить("","НомерРегистрации");	
	...
	OKPO         		  = Сведения.КодПоЕДРПОУ;
	НомерФССУтратаТрудосп = Сведения.НомерФССУтратаТрудосп;
	...
	ЗаполнитьПоказательНаСервере("OKPO",   OKPO, 							Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("CSTRAX",   НомерФССУтратаТрудосп, 		Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("ОргБухгалтер", СокрЛП(ФИОГлавБуха), 		Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("ОргДиректор",  СокрЛП(ФИОДиректора), 		Форма, "Уведомление");
	...
	КонецЕсли;
	ЗаполнитьПоказательНаСервере("РС", 	 ДопСведенияОбОрганизации.НомерСчета, Форма, "Уведомление");	
	ЗаполнитьПоказательНаСервере("Банк", ДопСведенияОбОрганизации.Банк, 	  Форма, "Уведомление");	
	ЗаполнитьПоказательНаСервере("МФО",  ДопСведенияОбОрганизации.МФО, 	 	  Форма, "Уведомление");	
	Попытка
		БанковскийСчетДляРасчетовСФСС = Организация.БанковскийСчетДляРасчетовСФСС;
		ЗаполнитьПоказательНаСервере("РС", 	 БанковскийСчетДляРасчетовСФСС.НомерСчета,	Форма, "Уведомление");	
		ЗаполнитьПоказательНаСервере("Банк", БанковскийСчетДляРасчетовСФСС.Банк,		Форма, "Уведомление");	
		ЗаполнитьПоказательНаСервере("МФО",  БанковскийСчетДляРасчетовСФСС.Банк.Код,	Форма, "Уведомление");	
	Исключение
	КонецПопытки;
	
	Если ЗначениеЗаполнено(Сведения.ДатаРегистрации)
		И Сведения.ДатаРегистрации >= '2011-01-01' Тогда
		ЗаполнитьПоказательНаСервере("DERZHREESTR",  Сведения.НомерРегистрации, Форма, "Уведомление");	
	КонецЕсли;
	...
	ЗаполнитьПоказательНаСервере("ПериодОтчета",  "за  " +  СтрПериодОтчета, Форма, "Уведомление");	
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	S254 = 		ПолучитьПоказательНаСервере("S214", Форма, "Уведомление")											 
			+	ПолучитьПоказательНаСервере("S224", Форма, "Уведомление")											 
			+	ПолучитьПоказательНаСервере("S234", Форма, "Уведомление")											 
			+	ПолучитьПоказательНаСервере("S244", Форма, "Уведомление");											 
	ЗаполнитьПоказательНаСервере("S254", S254, Форма, "Уведомление");												 
												 
	S284 = 		ПолучитьПоказательНаСервере("S254", Форма, "Уведомление")											 
			+	ПолучитьПоказательНаСервере("S264", Форма, "Уведомление")											 
			+	ПолучитьПоказательНаСервере("S274", Форма, "Уведомление");											 
	ЗаполнитьПоказательНаСервере("S284", S284, Форма, "Уведомление");												 
												 
	S256 = 		ПолучитьПоказательНаСервере("S216", Форма, "Уведомление")											 
			+	ПолучитьПоказательНаСервере("S226", Форма, "Уведомление")											 
			+	ПолучитьПоказательНаСервере("S236", Форма, "Уведомление");											 
	ЗаполнитьПоказательНаСервере("S256", S256, Форма, "Уведомление");												 
												 
	S257 = 		ПолучитьПоказательНаСервере("S217", Форма, "Уведомление")											 
			+	ПолучитьПоказательНаСервере("S227", Форма, "Уведомление")											 
			+	ПолучитьПоказательНаСервере("S237", Форма, "Уведомление");											 
	ЗаполнитьПоказательНаСервере("S257", S257, Форма, "Уведомление");												 
	
												 
	S121 = ПолучитьПоказательНаСервере("S284", Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("S121", S121, Форма, "Уведомление");												 
												 
	//Всього
	//На початок періоду
	S109 = 		ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S101", Форма, "Уведомление"))											 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S102", Форма, "Уведомление"))											 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S103", Форма, "Уведомление"))											 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S104", Форма, "Уведомление"))											 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S105", Форма, "Уведомление"))											 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S106", Форма, "Уведомление"))											 
			-	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S108", Форма, "Уведомление")); 
	ЗаполнитьПоказательНаСервере("S109", S109, Форма, "Уведомление");												 
												   
	S110 = 		ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S112", Форма, "Уведомление"))											 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S113", Форма, "Уведомление"))											 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S121", Форма, "Уведомление"))											 
			-	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S106", Форма, "Уведомление")); 
	ЗаполнитьПоказательНаСервере("S110", S110, Форма, "Уведомление");												 
												   
	
	S122 = 		ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S112", Форма, "Уведомление"))											 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S113", Форма, "Уведомление"))											 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S114", Форма, "Уведомление"))											 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S121", Форма, "Уведомление")); 
	ЗаполнитьПоказательНаСервере("S122", S122, Форма, "Уведомление");												 
												   
	
	S123 = 		ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S101", Форма, "Уведомление"))											 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S102", Форма, "Уведомление"))											 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S103", Форма, "Уведомление"))											 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S104", Форма, "Уведомление")) 
			+	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S105", Форма, "Уведомление")) 
			-	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S108", Форма, "Уведомление"))
			-	ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("S114", Форма, "Уведомление")); 
	ЗаполнитьПоказательНаСервере("S123", S123, Форма, "Уведомление");												 
...
Процедура РассчитатьСуммыЗадолженностей()
	...
	мБухИтогиОст.РассчитатьИтоги("Хозрасчетный", Итоги, "Сумма", "Счет", НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета), СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета,,"65,66,378",,,, "Организация", СтруктураРеквизитовФормы.Организация);
	
	//задолженность страховщика на начало года
	СтруктураДанныхУведомление.Вставить("S101",  мБухИтогиОст.ПолучитьИтог("СуммаНачальныйОстатокКт", "Счет",СчетФСС) 
										+ мБухИтогиОст.ПолучитьИтог("СуммаНачальныйОстатокКт", "Счет",СчетФСС378));
	
	//задолженность фонда на начало года
	СтруктураДанныхУведомление.Вставить("S112",  мБухИтогиОст.ПолучитьИтог("СуммаНачальныйОстатокДт", "Счет",СчетФСС) 
...
Процедура РассчитатьЗачтенныеПеречисленныеИПрочиеСуммы()	
	...
	мБухИтоги.РассчитатьИтоги("Хозрасчетный", Итоги, "Сумма", "Период,Счет,Субконто1,Субконто2,КорСчет", НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета), СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета,"Месяц","652,378",МассивСубконто,,, "Организация", СтруктураРеквизитовФормы.Организация);
	
	//доначислено взносов самостоятельно
	НужнаяСтатья = СтрокиДекларации.ФССУтрТрудосп_ДоначислСамост_Взнос;
	S103_ошибки_самост = мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС, НужнаяСтатья)
	...
	S103_ошибки_пеня = мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС, НужнаяСтатья)
					  +мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС378, НужнаяСтатья);
										
	СтруктураДанныхУведомление.Вставить("S103", S103_ошибки_самост + S103_ошибки_пеня);
										
	//доначислено взносов по актам
	НужнаяСтатья = СтрокиДекларации.ФССУтрТрудосп_ДоначислАкт_Взнос;
	S104_Проверки_акты = мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС, НужнаяСтатья)
		   				+мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС378, НужнаяСтатья);
	
	//штраф
	НужнаяСтатья = СтрокиДекларации.ФССУтрТрудосп_Штраф;
	S104_Проверки_штарфы = мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС, НужнаяСтатья)
		   				  +мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС378, НужнаяСтатья);
	
	//доначислено взносов по актам
	НужнаяСтатья = СтрокиДекларации.ФССУтрТрудосп_ДоначислАкт_Пеня;
	S104_Проверки_Пеня = мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС, НужнаяСтатья)
		   				+мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС378, НужнаяСтатья);
										
	СтруктураДанныхУведомление.Вставить("S104", S104_Проверки_акты + S104_Проверки_штарфы + S104_Проверки_Пеня);									
										
	////////не принято к зачету
	//////НужнаяСтатья = СтрокиДекларации.ФССУтрТрудосп_НеПринятоКЗачет;
	//////СтруктураДанныхУведомление.Вставить("???", мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС, НужнаяСтатья)
	//////									+мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС378, НужнаяСтатья));
	
	//получено от частичной оплаты за путевки
	НужнаяСтатья = СтрокиДекларации.ФССУтрТрудосп_ОплПутев;
	СтруктураДанныхУведомление.Вставить("S105", мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС, НужнаяСтатья)
										+мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС378, НужнаяСтатья));
	...
	СтруктураДанныхУведомление.Вставить("S114", Перечислено);
	
	// заполним строки 15...20 исходя из аналитики по статье налоговых деклараций в проводках по перечислению средств Фонду
	СтруктураПеречислений = Новый Структура();
	СтруктураПеречислений.Вставить("S115", СтрокиДекларации.ФССУтрТрудосп_ДоначислАкт); 
	СтруктураПеречислений.Вставить("S115", СтрокиДекларации.ФССУтрТрудосп_ДоначислАкт_Взнос); 
	
	СтруктураПеречислений.Вставить("S116", СтрокиДекларации.ФССУтрТрудосп_ДоначислСамост); 
	СтруктураПеречислений.Вставить("S116", СтрокиДекларации.ФССУтрТрудосп_ДоначислСамост_Взнос); 
	
	СтруктураПеречислений.Вставить("S117", СтрокиДекларации.ФССУтрТрудосп_ДоначислАкт_Пеня); 
	СтруктураПеречислений.Вставить("S117", СтрокиДекларации.ФССУтрТрудосп_ДоначислСамост_Пеня); 
	СтруктураПеречислений.Вставить("S117", СтрокиДекларации.ФССУтрТрудосп_Пеня); 
	
	СтруктураПеречислений.Вставить("S118", СтрокиДекларации.ФССУтрТрудосп_Штраф); 
	
	СтруктураПеречислений.Вставить("S119", СтрокиДекларации.ФССУтрТрудосп_НеПринятоКЗачет); 
	
	СтруктураПеречислений.Вставить("S120", СтрокиДекларации.ФССУтрТрудосп_ОплПутев); 
	СуммыПеречислений = Новый Структура();
	Для каждого ВидПеречисления Из СтруктураПеречислений Цикл
		
		КодСтроки 	 = ВидПеречисления.Ключ;
		НужнаяСтатья = ВидПеречисления.Значение;
		
		Если НЕ СуммыПеречислений.Свойство(КодСтроки) Тогда
			СуммыПеречислений.Вставить(КодСтроки, 0);
		КонецЕсли;
		
		Перечислено = мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, КорСчет, Субконто1", СчетФСС, 	 СчетРасчСчет1, НужнаяСтатья)
					+ мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, КорСчет, Субконто1", СчетФСС, 	 СчетРасчСчет2, НужнаяСтатья) 
					+ мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, КорСчет, Субконто1", СчетФСС, 	 СчетРасчСчет3, НужнаяСтатья)
					+ мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, КорСчет, Субконто1", СчетФСС, 	 СчетРасчСчет4, НужнаяСтатья)
					+ мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, КорСчет, Субконто1", СчетФСС378, СчетРасчСчет1, НужнаяСтатья)
					+ мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, КорСчет, Субконто1", СчетФСС378, СчетРасчСчет2, НужнаяСтатья) 
					+ мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, КорСчет, Субконто1", СчетФСС378, СчетРасчСчет3, НужнаяСтатья)
					+ мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, КорСчет, Субконто1", СчетФСС378, СчетРасчСчет4, НужнаяСтатья);			
		
	    СуммыПеречислений[КодСтроки] = СуммыПеречислений[КодСтроки] + Перечислено;
		
	КонецЦикла;
	
	Для каждого ВидПеречисления Из СуммыПеречислений Цикл
		СтруктураДанныхУведомление.Вставить(ВидПеречисления.Ключ, ВидПеречисления.Значение);
	КонецЦикла;
	...
	//получено от фонда на начало квартала
	СтруктураДанныхУведомление.Вставить("S106", ПолученоОтФонда);
...
Процедура  РассчитатьСуммыРасходовПоСтрахованию()
	...
	СтруктураДанныхУведомление.Вставить("S214", S214);
	...
	S224 = мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто1,КорСчет", СчетФСС, НужнаяСтатья,СчетЗП)
		 ...
		 + мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто1,КорСчет, ", СчетФСС378, НужнаяСтатья,СчетЗП)
		 + мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто1,КорСчет, ", СчетФСС378, НужнаяСтатья,СчетПрочихНачислений);
	СтруктураДанныхУведомление.Вставить("S224", S224);
	...
	S234 = мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто1,КорСчет", СчетФСС,  НужнаяСтатья,СчетЗП,)
		 ...
		 + мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто1,КорСчет, ", СчетФСС378, НужнаяСтатья,СчетЗП)
		 + мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто1,КорСчет, ", СчетФСС378, НужнаяСтатья,СчетПрочихНачислений);
	СтруктураДанныхУведомление.Вставить("S234", S234);
	...
	S244 = мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто1,КорСчет", СчетФСС, НужнаяСтатья,СчетЗП)
		 ...
		 + мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто1,КорСчет, ", СчетФСС378, НужнаяСтатья,СчетЗП)
		 + мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто1,КорСчет, ", СчетФСС378, НужнаяСтатья,СчетПрочихНачислений);
	СтруктураДанныхУведомление.Вставить("S244", S244);
	...
	S264 = мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто1", 		 СчетФСС,	 НужнаяСтатья)
		 ...
		 + мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто1,КорСчет", СчетФСС378, НужнаяСтатья,СчетПрочихНачислений);
	СтруктураДанныхУведомление.Вставить("S264", S264);
	...
	S274 = мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто1",		 СчетФСС, 	 НужнаяСтатья)
		 ...
		 + мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто1,КорСчет", СчетФСС378, НужнаяСтатья,СчетПрочихНачислений);
	СтруктураДанныхУведомление.Вставить("S274", S274);
