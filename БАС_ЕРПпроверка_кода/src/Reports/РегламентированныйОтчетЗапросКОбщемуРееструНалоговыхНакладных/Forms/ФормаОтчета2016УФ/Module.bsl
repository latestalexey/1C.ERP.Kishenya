...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/09/2016";
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = НСтр("ru='Запрос';uk='Запит'");
...
Процедура ЗаполнитьДатуПодписи(Форма)
	...
	ЗаполнитьПоказательНаСервере("HFILL",  Форма.ДатаПодписи, Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("D_FILL",  Форма.ДатаПодписи, Форма, "Уведомление");
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("HNAME", Организация.НаименованиеПолное, Форма, "Уведомление");
	...
	КодЕДРПОУ   = Строка(Сведения.КодПоЕДРПОУ);
	ИНН         = Строка(Сведения.ИНН);
	НалоговаяИнспекция= Сведения.НалоговаяИнспекция;
	HEMAIL         = Строка(Сведения.Email);
	...
	ЗаполнитьПоказательНаСервере("HTIN",  КодЕДРПОУ, 		Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("HNPDV", ИНН,		 		Форма, "Уведомление");
	
	ЗаполнитьПоказательНаСервере("HEMAIL", HEMAIL, Форма, "Уведомление");
	
	ЗаполнитьПоказательНаСервере("HBOS",  ФИОДиректора, 	Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("HKBOS", ИННФИОДиректора, 	Форма, "Уведомление");
    ...
    Если ЗначениеЗаполнено(НалоговаяИнспекция) Тогда 
		...
		ЗаполнитьПоказательНаСервере("HKSTI", НалоговаяИнспекция.Код, 			Форма, "Уведомление");
		ЗаполнитьПоказательНаСервере("HSTI", НалоговаяИнспекция, 				Форма, "Уведомление");
	...
	НомерЗапроса = ПолучитьСледующийНомерДокументаВПериоде(Организация, мДатаНачалаПериодаОтчета, мДатаКонцаПериодаОтчета);
	
	ЗаполнитьПоказательНаСервере("HNUM",  		НомерЗапроса, Форма, "Уведомление");;
	ЗаполнитьПоказательНаСервере("C_DOC_CNT",  	НомерЗапроса, Форма, "СлужебныеДанные");;
	...
	PERIOD_MONTH = Месяц(мДатаКонцаПериодаОтчета);
	...
	ЗаполнитьПоказательНаСервере("H01",  	"X", Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("H02",  	"",  Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("H03", 	"",  Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("H01G1D", 		 Форма.ДатаПодписи - 24 * 60 * 60, Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("H01G1",  	"X", Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("H01G2",  	"X", Форма, "Уведомление");
...
Функция ПолучитьСледующийНомерДокументаВПериоде(Организация, НачПериода, КонПериода)
	...
	Запрос.УстановитьПараметр("ИсточникОтчета", "РегламентированныйОтчетЗапросКОбщемуРееструНалоговыхНакладных");
	Запрос.УстановитьПараметр("НачГода", НачалоГода(НачПериода));
	Запрос.УстановитьПараметр("КонГода", КонецГода(КонПериода));
...
Процедура ВосстановитьСохраненныеДанные()
	...
	////СписокСохранения.Свойство("ДатаКон",  ДатаКон);
...
Процедура УдалитьСтроку(Команда)
	...
	Если ЗначениеЗаполнено(ПолучитьПоказательНаСервере("H01",ЭтаФорма,"Уведомление")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВставитьДополнительнуюСтроку(Форма, ИмяГруппыБлока = Неопределено, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, НомерПервойСтроки = 1) Экспорт
	...
	Если ЗначениеЗаполнено(ПолучитьПоказательНаСервере("H01",Форма,"Уведомление")) Тогда
		Сообщить(НСтр("ru='В начале установить флажок ""за списком""!';uk= 'Спочатку встановіть відмітку ""за списком""!'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если ИмяЯчейки = "H01" Тогда
		...
		// если устанавливаем флаг "за день", то нужно очистить список налоговых
		Если НЕ ЗначениеЗаполнено(ТабличныйДокумент.Области["H01"].Значение) Тогда
			Если    T1R.Количество() > 1
				ИЛИ ЗначениеЗаполнено(ТабличныйДокумент.Области.T1RXXXXG21_1.Значение) Тогда
			        Сообщить(НСтр("ru='В начале нужно очистить список налоговых документов!';uk= 'Спочатку треба очистити список податкових документів'"));
					Возврат;
			КонецЕсли;
		КонецЕсли;
		...
		ТабличныйДокумент.Области["H01G1D"].Значение = ДатаПодписи - 24 * 60 * 60;;
		ТабличныйДокумент.Области["H01G1"].Значение = "X";
		ТабличныйДокумент.Области["H01G2"].Значение = "X";
	...
	Если ИмяЯчейки = "H02" Тогда
		...
		ТабличныйДокумент.Области["H01G1D"].Значение = "";
		ТабличныйДокумент.Области["H01G1"].Значение = "";
		ТабличныйДокумент.Области["H01G2"].Значение = "";
		ТабличныйДокумент.Области["H04"].Значение = "";
		ТабличныйДокумент.Области["H04G1"].Значение = 0;
	...
	Если ИмяЯчейки = "H04" Тогда
		...
		Если ЗначениеЗаполнено(ТабличныйДокумент.Области[ИмяЯчейки].Значение) Тогда
			ТабличныйДокумент.Области[ИмяЯчейки].Значение	= "";
			ТабличныйДокумент.Области["H04G1"].Значение	= "";
		Иначе
			ТабличныйДокумент.Области[ИмяЯчейки].Значение	= "X";
		КонецЕсли;
	...
	Если Лев(ИмяЯчейки, 3) = "T1R" Тогда
		
		Если ЗначениеЗаполнено(ТабличныйДокумент.Области["H01"].Значение)  Тогда
			Сообщить(НСтр("ru='В начале установить флажок ""за списком""!';uk= 'Спочатку встановіть відмітку ""за списком""!'"));
			СтандартнаяОбработка = Ложь;
			Возврат;
		КонецЕсли;
	...
	Если  ИмяЯчейки = "H01G1" 
		ИЛИ ИмяЯчейки = "H01G2" Тогда
...
Функция ПолучитьДанныеФормыНаСервере();
	...
	ПоказателиОтчета.Уведомление.Вставить("HPAGES", 1);
	ПоказателиОтчета.Уведомление.Вставить("R011G1", ЭтаФорма[СтруктураРеквизитовФормы.мИдГруппы1].Количество());
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Для Сч = 1 По КоличествоСтраниц Цикл
		...
		Если СтруктураРеквизитовФормы.Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
			НовСтрока.ИмяСхемы = "J1300104"; 
		Иначе	
			НовСтрока.ИмяСхемы = "F1300104"; 
		КонецЕсли;
			
		НовСтрока.ПредставлениеСхемы 	 = НСтр("ru='Запрос к ЕРНН';uk= 'Запит до ЄРПН'");
		НовСтрока.ПредставлениеСхемы 	 = НовСтрока.ПредставлениеСхемы  + НСтр("ru=' стр. ';uk=' стор. '") + Формат(Сч, "ЧГ=0");
		
		НовСтрока.НазваниеСтраницыПанели = "Уведомление";
...
Процедура СформироватьСпискиВыбора()
	...
	ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "T1RXXXXG4S", 2, , "Вид документа", КоллекцияСписковВыбора["ВидыВыданныхНН"]);
...
Процедура ЗагрузитьВыписку(Команда)
	...
	// Загружаем файл пользователя на сервер
	...
	Диалог.Показать(Новый ОписаниеОповещения("ЗагрузитьВыпискуЗавершение", ЭтаФорма))
	
КонецПроцедуры	

&НаКлиенте
Процедура ЗагрузитьВыпискуЗавершение(Результат, ДопПараметры) Экспорт
	...
	XML = XML.ПолучитьТекст();
	
	// заполняем макет на сервере данными файла
	Выписка = ЗагрузитьВыпискуНаСервере(XML);
	
	// Выводим макет для показа пользователю.
	Выписка.ТолькоПросмотр = Истина;
	Выписка.Показать(НСтр("ru='Выписка из ЕРНН';uk= 'Витяг з ЄРПН'"));
	
КонецПроцедуры

&НаСервере	
Функция ЗагрузитьВыпискуНаСервере(XML)
	
	// создаем макет. Для ТЧ копиреум строки из макета.  
	Выписка = Новый ТабличныйДокумент;
	Выписка.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Выписка"));

	// получаем значения полей XML, в т.ч. табличные части. Имена в ТЧ должны соответствовать именам на макете
	// одновременно определяем количество строк в двух ТЧ
	КоличествоСтрокT1R = 20; КоличествоСтрокT2R = 30; //тестовые значения
	РеквизитыXML = ПолучитьРеквизитыФайлаНаСервере(Выписка, XML, КоличествоСтрокT1R, КоличествоСтрокT2R);
	
	Если КоличествоСтрокT1R > 1 Тогда
		ОбластьT1R = Выписка.Область("T1R");
		ОбластьПриемник = Выписка.Область(ОбластьT1R.Низ + 1,,ОбластьT1R.Низ + КоличествоСтрокT1R -1);
		Выписка.ВставитьОбласть(ОбластьT1R, ОбластьПриемник, ТипСмещенияТабличногоДокумента.ПоВертикали, Ложь);
	КонецЕсли;
	
	Если КоличествоСтрокT2R > 1 Тогда
		ОбластьT2R = Выписка.Область("T2R");
		ОбластьПриемник = Выписка.Область(ОбластьT2R.Низ + 1,,ОбластьT2R.Низ + КоличествоСтрокT2R - 1);
		Выписка.ВставитьОбласть(ОбластьT2R, ОбластьПриемник, ТипСмещенияТабличногоДокумента.ПоВертикали, Ложь);
	КонецЕсли;
	
	// присвоим имена добавленным областям
	ОбластиТаблицВыписки = Новый Массив();
	Для Каждого ТекОбласть Из Выписка.Области Цикл
		
		ИмяИсхОбласти = ТекОбласть.Имя;
		
		Если  НЕ Лев(ИмяИсхОбласти, 3) = "T1R" 
			И НЕ Лев(ИмяИсхОбласти, 3) = "T2R" Тогда
			Продолжить;
		КонецЕсли;
		
		Если Найти(ИмяИсхОбласти, "XXXX") = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		ОбластиТаблицВыписки.Добавить(ТекОбласть);
		
	КонецЦикла;
	
	Для Каждого ТекОбласть Из ОбластиТаблицВыписки Цикл
		
		ИмяИсхОбласти = ТекОбласть.Имя;
		Если  Лев(ИмяИсхОбласти, 3) = "T1R" Тогда
			ТекСчетчик = КоличествоСтрокT1R;
		Иначе
			ТекСчетчик = КоличествоСтрокT2R;
		КонецЕсли;
		
		Для СчДопСтрок = 1 По ТекСчетчик - 1 Цикл
			НовоеИмя = СтрЗаменить(ИмяИсхОбласти, "_1", "_" + Формат(СчДопСтрок + 1, "ЧГ=0"));
			Выписка.Область(ТекОбласть.Низ + СчДопСтрок, ТекОбласть.Лево, ТекОбласть.Низ + СчДопСтрок, ТекОбласть.Право).Имя = НовоеИмя;		
		КонецЦикла;
	
	КонецЦикла;
		
	// Загружаем на макет значения реквизитов файла ХМЛ.
	Для каждого РеквизитXML Из РеквизитыXML Цикл
		Выписка.Область(РеквизитXML.Ключ).Значение = РеквизитXML.Значение;
	КонецЦикла;
	
	// пронумеровываем строки в каждой ТЧ
	Для каждого Область Из Выписка.Области Цикл
		ИмяОбласти = Область.Имя;
		
		Если Найти(ИмяОбласти, "НомерСтроки") = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		Позиция_ = Найти(ИмяОбласти, "_"); 
		
		Если Позиция_ > 0 Тогда
			НомерСтроки = Сред(ИмяОбласти, Позиция_ + 1);
...
Функция ПолучитьРеквизитыФайлаНаСервере(Выписка, XML, КоличествоСтрокT1R, КоличествоСтрокT2R)
	
	КоличествоСтрокT1R = 0;
	КоличествоСтрокT2R = 0;
		...
		Пока ЧтениеXML.Прочитать() Цикл
				...
				Если Найти(ТекУзел, "XXXX") > 0 Тогда
					...
					Пока ЧтениеXML.ПрочитатьАтрибут()  Цикл
						Если ЧтениеXML.Имя = "ROWNUM" Тогда
							...
							Если Лев(ТекУзел, 3) = "T1R" Тогда
								Если ТекНомерСтроки > КоличествоСтрокT1R Тогда
									КоличествоСтрокT1R = ТекНомерСтроки; 
								КонецЕсли;
							Иначе
								Если ТекНомерСтроки > КоличествоСтрокT2R Тогда
									КоличествоСтрокT2R = ТекНомерСтроки; 
								КонецЕсли;
							КонецЕсли;
			...
			ИначеЕсли ЧтениеXML.ТипУзла = ТипУзлаXML.Текст Тогда
				...
				Если  ПозицияНомераСтроки > 0 Тогда
					...
					Если НЕ Выписка.Области.Найти(ТекУзел + "_1") = Неопределено Тогда
						РеквизитыXML.Вставить(ТекУзел + "_" + Формат(ТекНомерСтроки,"ЧГ=0;"), ПреобразоватьЗначениеXML(ТекЗначение, Выписка.Область(ТекУзел + "_1")));
					КонецЕсли;
									
				Иначе
					
					Если НЕ Выписка.Области.Найти(ТекУзел) = Неопределено Тогда
						РеквизитыXML.Вставить(ТекУзел, ПреобразоватьЗначениеXML(ТекЗначение, Выписка.Область(ТекУзел))); 	
					КонецЕсли;
	...
	Исключение
		Сообщить(НСтр("ru='Выбранный файл повреждени или меет не правильный формат. Просмотр невозможен!';uk= 'Обраний файл пошкоджений, або має невірний формат. Перегляд неможливий!'"));
	КонецПопытки;		

	Возврат РеквизитыXML;                                   
	
КонецФункции

&НаСервере
Функция ПреобразоватьЗначениеXML(Значение, ТекОбласть)
	...
	ИначеЕсли Врег(ИмяОбласти) = "H01"
		  ИЛИ Врег(ИмяОбласти) = "H01G1"
		  ИЛИ Врег(ИмяОбласти) = "H01G2"
		  ИЛИ Врег(ИмяОбласти) = "H02"
		  Тогда
