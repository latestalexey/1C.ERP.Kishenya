...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2018";
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/11/2018";
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	// выгрузка у организации-филиала по данным головной организации
	Если ЗначениеЗаполнено(Организация.ГоловнаяОрганизация) Тогда
	
		СписокПоказателейГоловнойОрганизации = Новый СписокЗначений;
		СписокПоказателейГоловнойОрганизации.Добавить("","КодПоЕДРПОУ");
		СписокПоказателейГоловнойОрганизации.Добавить("","ИНН");
		СписокПоказателейГоловнойОрганизации.Добавить("","НалоговаяИнспекция");
		
		СведенияГоловнаяОрганизация = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(Организация.ГоловнаяОрганизация, ТекДата, СписокПоказателейГоловнойОрганизации);
		
		КодЕДРПОУ          	   = СведенияГоловнаяОрганизация.КодПоЕДРПОУ;
		ИНН                    = СведенияГоловнаяОрганизация.ИНН;
		НалоговаяИнспекция     = СведенияГоловнаяОрганизация.НалоговаяИнспекция;
	
	КонецЕсли;
	
	ЗаполнитьПоказательНаСервере("HBOS", 		ФИОДиректора, 		Форма, "НалоговыйДокумент");
	ЗаполнитьПоказательНаСервере("HKBOS", 		ИННДиректора, 		Форма, "НалоговыйДокумент"); 
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если НЕ СписокСохранения.Свойство("РасчетЦеныТочный", РасчетЦеныТочный) Тогда
		РасчетЦеныТочный = Истина;	
	КонецЕсли;
...
Функция Сохранить(Автосохранение = Ложь, НаимТекРаздела) Экспорт	
	...
	СписокСохранения.Вставить("РасчетЦеныТочный", РасчетЦеныТочный);
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить("РегламентированнаяОтчетность_МенеджерЗвит1С_ВыгрузкаНалоговыхДокументов_РасчетЦеныТочный", ,РасчетЦеныТочный);
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если Лев(ИмяЯчейки,4) =  "RG32" Тогда
...
Процедура Выгрузить(Команда)
	...
	Если НЕ мНалоговыйДокумент_РеквизитыНомерСтрокиННАктуальны Тогда
		ПоказатьПредупреждение(, НСтр("ru='Номера строк налогового документа не актуальны! Выгрузка возможна только при корректно указанных номерах строк!';uk= 'Номери рядків податкового документа не актуальні! Вивантаження можливе тільки при коректно зазначених номерах рядків!'")); 
		Возврат;
	КонецЕсли;
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	Если ТипЗнч(Ячейка) = Тип("Структура") Тогда
		...
		Если Ячейка.Свойство("ИмяЯчейки") Тогда
			...
			ТекИмяЯчейки = СтрЗаменить(ТекИмяЯчейки, "RG11_10", "RG11U10");
...
&НаСервере
Функция ПродажаНижеОбычнойЦены (НалоговыйДокумент)
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "БП" Тогда
		Возврат НалоговыйДокумент.ПродажаНижеОбычнойЦены;
	Иначе
		Возврат Ложь;	
	КонецЕсли;
КонецФункции
...
Процедура ЗаполнитьАвто(Команда)
	...
	Если ПродажаНижеОбычнойЦены(мНалоговыйДокумент) Тогда
		
		ТекстВопроса = НСтр("ru='В указанной налоговой накладной установлен признак продажи по ценам ниже обычных.
							 |Можно заполнить по данным либо обычной налоговой накладной, либо налоговой накладной на превышение обычных цен над  продажными (""/ЗЦ"").
							 |Заполнить данными обычной налоговой накладной?
							 |
							 |- Да:   будут загружены данные обычной налоговой накладной
							 |- Нет:  будут загружены данные по налоговой накладной на сумму превышения
							 |- Отмена: отказ от заполнения
							 |';uk= 'В зазначеній податковій накладній встановлена ознака продажу за цінами, нижче звичайних.
							 |Є можливість заповнити даними або звичайної податкової накладної, або податкової накладної на перевищення звичайних цін над продажними (""/ЗЦ).
							 |Заповнити даними звичайної податкової накладної?
							 |
							 |- Так: будуть завантажено дані звичайної податкової накладної
							 |- Ні:     будуть завантажено дані податкової накладної на суму перевищення
							 |- Відміна: відмова від заповнення
							 |'");
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьАвтоЗавершение", ЭтотОбъект);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНетОтмена, 60, КодВозвратаДиалога.Отмена);
	Иначе
...
&НаКлиенте
Процедура ЗаполнитьАвтоЗавершение(Ответ, ДополнительныеПараметры)  Экспорт
	...
	ПараметрыЗаполненияЗвит1С = Неопределено;
	ЗаполнитьАвтоНаСервере(ПараметрыЗаполненияЗвит1С, ИспользоватьДанныеЗЦ);
	ДозаполнитьНаКлиенте(ПараметрыЗаполненияЗвит1С);
...
&НаСервере
Процедура ЗаполнитьАвтоНаСервере(ПараметрыЗаполненияЗвит1С, ИспользоватьДанныеЗЦ = Ложь)
	
	ОчиститьНаСервере();
	
	СохранитьДанныеРаздела(СтруктураРеквизитовФормы.НаимТекущегоРаздела, Ложь);
	Заполнить(ПараметрыЗаполненияЗвит1С, ИспользоватьДанныеЗЦ);
...
&НаСервере
Процедура Заполнить(ПараметрыЗаполненияЗвит1С, ИспользоватьДанныеЗЦ = Ложь)
	
	
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "БП" Тогда
		мНалоговыйДокумент_РеквизитыНомерСтрокиННАктуальны = мНалоговыйДокумент.РеквизитыНомерСтрокиННАктуальны;
	Иначе
		мНалоговыйДокумент_РеквизитыНомерСтрокиННАктуальны = Истина;
	КонецЕсли;
	...
	//Если ЗначениеЗаполнено(мНалоговыйДокумент.ОсновнаяНалоговаяПриРезервеНомераПриПродажаНижеОбычнойЦены) Тогда
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "БП" И ЗначениеЗаполнено(мНалоговыйДокумент.ОсновнаяНалоговаяПриРезервеНомераПриПродажаНижеОбычнойЦены) Тогда
		НалоговыйДокументОбъект = мНалоговыйДокумент.ОсновнаяНалоговаяПриРезервеНомераПриПродажаНижеОбычнойЦены.ПолучитьОбъект();	
	    ИспользоватьДанныеЗЦ = Истина;
	Иначе	
		НалоговыйДокументОбъект = мНалоговыйДокумент.ПолучитьОбъект();	
	КонецЕсли;
	...
	Попытка
		...
		Иначе
			МассивОбъектов.Добавить(НалоговыйДокументОбъект.Ссылка);	
		КонецЕсли;
	...
	Если ИспользоватьДанныеЗЦ Тогда
		...
		ДанныеТаблицы 	= СтруктураПараметров.НалоговыйДокументЗЦ.R;	
	Иначе	
		ДанныеШапки 	= СтруктураПараметров.НалоговыйДокумент;
		ДанныеТаблицы 	= СтруктураПараметров.НалоговыйДокумент.R;	
	КонецЕсли;
	
	ДанныеШапки.Свойство("R01G1",     СтруктураДанныхНалоговыйДокумент.R01G1);
	...
	ДанныеШапки.Свойство("HFBUY",  СтруктураДанныхНалоговыйДокумент.HFBUY);
	...
	ДанныеШапки.Свойство("HBOS",  СтруктураДанныхНалоговыйДокумент.HBOS);
	ДанныеШапки.Свойство("HKBOS", СтруктураДанныхНалоговыйДокумент.HKBOS);

	ДанныеШапки.Свойство("R003G10S",СтруктураДанныхНалоговыйДокумент.R003G10S);
	ДанныеШапки.Свойство("HTINSEL",  СтруктураДанныхНалоговыйДокумент.HTINSEL);
	ДанныеШапки.Свойство("HTINBUY", СтруктураДанныхНалоговыйДокумент.HTINBUY);
	...
	КолонкиМногострочки = ДанныеМногострочки.Колонки;
	...
	Для каждого СтрокаТЧ Из ДанныеТаблицы Цикл
		...
		Для каждого Колонка Из ДанныеТаблицы.Колонки Цикл
			...
			Если ИмяXSD = "G105_2S" Тогда
				ИмяXSD = "G1052S";	
			КонецЕсли;
			Если ИмяXSD = "G11_10" Тогда
				ИмяXSD = "G11U10";	
			КонецЕсли;			
		...
		Если РасчетЦеныТочный Тогда
			Если  ЗначениеЗаполнено(СтрокаМногострочки.RG5)
				И ЗначениеЗаполнено(СтрокаМногострочки.RG6)
				И ЗначениеЗаполнено(СтрокаМногострочки.RG010) Тогда
				
				СтрокаМногострочки.RG6 = СтрокаМногострочки.RG010 / СтрокаМногострочки.RG5;
...
Функция ПолучитьДанныеФормыНаСервере();
	...
	ПолныйНомер = СокрЛП(ДанныеТП.HNUM1);
	...
	Для каждого ИдМногострочногоРаздела Из мСтруктураМногострочныхРазделов Цикл
		...
		Для каждого Строка Из Таблица Цикл
			...
			// нельзя в названии показателя многострочки использвоать два подчеркивания
			// поэтому на форме имя показателя XSD искажено (удален слеш внутри имени)
			СтруктураДанных.Вставить("RG105_2S_" 	 + НомерСтроки, Строка.RG1052S);
			СтруктураДанных.Вставить("RXXXXG105_2S_" + НомерСтроки, Строка.RG1052S);
			СтруктураДанных.Вставить("RG11_10_" 	 + НомерСтроки, Строка.RG11U10);
			СтруктураДанных.Вставить("RXXXXG11_10_"  + НомерСтроки, Строка.RG11U10);
...
Функция КопияТабличногоДокумента()
	...
	мТабличныйДокумент.МасштабПечати = 90;
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		ИначеЕсли ЧтениеXML.ТипУзла = ТипУзлаXML.Текст Тогда
			...
			Если  ПозицияНомераСтроки > 0 Тогда
				...
				ТекИмяОбласти  = СтрЗаменить(ТекИмяОбласти, "RG11_10", "RG11U10");
...
&НаКлиенте
Процедура РасчетЦеныТочныйПриИзменении(Элемент)
	ПоказатьПредупреждение(,НСтр("ru='Отчет необходимо перезаполнить!';uk= 'Звіт потрібно перезаповнити!'"));
КонецПроцедуры
