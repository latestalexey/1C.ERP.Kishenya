...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураДанныхОтчетОЛьготах	= Новый Структура;
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.ИмяСтраницы              = "ОтчетОЛьготах";
	СтрокаУровня1.Представление            = НСтр("ru='Отчет о суммах полученных льгот в налогообложении';uk='Звіт про суми отриманих пільг в оподатковуванні'");
...
Процедура РазделыОтчетаПриАктивизацииСтроки(Элемент) Экспорт
	...
	Если    Элемент.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "ОтчетОЛьготах" Тогда
...
Процедура СохранитьДанныеРаздела(НаимРаздела, ОчищатьРазделЕслиЕстьМногострочность)
	...
	//
	ИначеЕсли НаимРаздела = "ОтчетОЛьготах"
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	// Многострочные и обычные разделы.
	//
	ИначеЕсли ИмяТекРаздела = "ОтчетОЛьготах"
...
Процедура ЗаполнитьПоказателиЗначениямиПоУмолчанию(НаимРаздела)
	...
	Если НаимРаздела = "ОтчетОЛьготах" Тогда
...
Функция ОпределитьТабличноеПолеПоИдМногострочногоРаздела(ИдГруппы, ИдГрупп) Экспорт
	...
	Если  ИдГруппы = ИдГрупп.мИдГруппы1 Тогда
		...
		ТекТабличноеПоле = "ОтчетОЛьготах";
...
Процедура ЗаполнитьДатуПодписи(Форма)
	...
	ЗаполнитьПоказательНаСервере("HFILL",  Форма.ДатаПодписи, Форма, "ОтчетОЛьготах");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ОтчетОЛьготах", "HFILL_", 8, Формат(Форма.ДатаПодписи,"ДФ=ddMMyyyy"), Ложь);
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("HNAME", Организация.НаименованиеПолное, Форма, "ОтчетОЛьготах");
	...
	ЗаполнитьПоказательНаСервере("HBOS",  ФИОДиректора, Форма, "ОтчетОЛьготах");
	ЗаполнитьПоказательНаСервере("HKBOS", ИННДиректора, Форма, "ОтчетОЛьготах");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ОтчетОЛьготах", "HKBOS_",  10, ИННДиректора, Ложь);	
	
	ЗаполнитьПоказательНаСервере("HBUH",  ФИОГлавБуха,  Форма, "ОтчетОЛьготах");
	ЗаполнитьПоказательНаСервере("HKBUH", ИННГлавБуха,  Форма, "ОтчетОЛьготах");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ОтчетОЛьготах", "HKBUH_",  10, ИННГлавБуха, Ложь);	
	...
	ЗаполнитьПоказательНаСервере("HTIN", КодЕДРПОУ, Форма, "ОтчетОЛьготах");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ОтчетОЛьготах", "HTIN_",  10, КодЕДРПОУ, Ложь);	
	
	ЗаполнитьПоказательНаСервере("HLOC", HLOC, Форма, "ОтчетОЛьготах");
	
	ЗаполнитьПоказательНаСервере("HEMAIL", HEMAIL, Форма, "ОтчетОЛьготах");

	ЗаполнитьПоказательНаСервере("HTEL", Телефон, Форма, "ОтчетОЛьготах");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ОтчетОЛьготах", "HTEL_",  10, Телефон, Ложь);	
	
	ЗаполнитьПоказательНаСервере("HFAX", Факс, Форма, "ОтчетОЛьготах");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ОтчетОЛьготах", "HFAX_",  10, Факс, Ложь);	
	
	ЗаполнитьПоказательНаСервере("HZIP", Индекс, Форма, "ОтчетОЛьготах");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ОтчетОЛьготах", "HZIP_",  5, Индекс, Ложь);	
	...
	Если ЗначениеЗаполнено(НалоговаяИнспекция) Тогда 
		...
		ЗаполнитьПоказательНаСервере("HSTI", НалоговаяИнспекция_Наименование, 	Форма, "ОтчетОЛьготах");
	...
	ЗаполнитьПоказательНаСервере("HZY",  		 Год, Форма, "ОтчетОЛьготах");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ОтчетОЛьготах", "HZY_", 4, Формат(Год, "ЧГ="), Ложь);	
	
	ЗаполнитьПоказательНаСервере("HZKV", Месяц(КонецКвартала(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета))/3, Форма, "ОтчетОЛьготах");
...
Функция ОпределитьИдМногострочногоРазделаПоАктивнойСтранице(ИмяЛиста = Неопределено, ТекущаяСтраница, ИдГрупп)
	...
	Если ИмяТекущейСтраницыПанели = "ОтчетОЛьготах" Тогда
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 4)) = "HKOP" Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
																				 "HKOPFG",  "ОтчетОЛьготах",
																				 "HKOPFG_", "ОтчетОЛьготах",
																				 4, Ложь, Неопределено));
	...
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 4)) = "HINT" Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
																				 "HINTURB",  "ОтчетОЛьготах",
																				 "HINTURB_", "ОтчетОЛьготах",
																				 5, Ложь, Неопределено));
	...
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 5)) = "HMTEL" Тогда
	...
	ИначеЕсли ВРЕГ(Лев(ИмяЯчейки, 5)) = "HKBUH" Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 ...
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
																				 "HKBUH",  "ОтчетОЛьготах",
																				 "HKBUH_", "ОтчетОЛьготах",
																				 10, Ложь, Неопределено));
		
		ПоказатьВводСтроки(ОписаниеОповещения,Значение, НСтр("ru='Введите значение';uk='Введіть значення'"), 10);

	ИначеЕсли  ИмяЯчейки = "HZ" 
...
Процедура ОчиститьНаСервере() Экспорт
	...
	СтруктураДанныхОтчетОЛьготах.Очистить();
...
//&НаСервере
//Функция ПоказателиПредыдущейДекларации(ДатаНачалаПериода,ДатаОкончанияПериода,ИсточникОтчета,ПредставлениеОтчета,ИмяПоля,ПредставлениеПоказателя)
//  перем ПоказателиОтчета;
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Если СтруктураРеквизитовФормы.Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
		НовСтрока.ИмяСхемы = "J0201406"; 
	Иначе	
		НовСтрока.ИмяСхемы = "F0201406"; 
	КонецЕсли;
	
	НовСтрока.ПредставлениеСхемы 	 = НСтр("ru='Отчет о суммах полученных льгот в налогообложении';uk='Звіт про суми отриманих пільг в оподатковуванні'");
	НовСтрока.НазваниеСтраницыПанели = "ОтчетОЛьготах";
