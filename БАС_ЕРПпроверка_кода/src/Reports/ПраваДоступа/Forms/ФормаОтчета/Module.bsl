...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
		...
		Если УправлениеДоступомСлужебный.УпрощенныйИнтерфейсНастройкиПравДоступа() Тогда
			ВызватьИсключение
				НСтр("ru='Чтобы открыть отчет откройте карточку пользователя,
                           ...
                           |;uk='Щоб відкрити звіт відкрийте картку користувача,
                           |перейдіть по посиланню ""Права доступу"", натисніть на кнопку ""Звіт з прав доступу"".'");
		Иначе
			ВызватьИсключение
				НСтр("ru='Чтобы открыть отчет откройте карточку пользователя или группы пользователей,
                           |перейдите по ссылке ""Права доступа"", нажмите на кнопку ""Отчет по правам доступа"".'
                           |;uk='Щоб відкрити звіт відкрийте картку користувача або групи користувачів,
                           |перейдіть по посиланню ""Права доступу"", натисніть на кнопку ""Звіт з прав доступу"".'");
		КонецЕсли;
	КонецЕсли;
	...
	Если Параметры.Пользователь <> Пользователи.АвторизованныйПользователь()
	   И НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		ВызватьИсключение НСтр("ru='Недостаточно прав для просмотра отчета.';uk='Недостатньо прав для перегляду звіту.'");
	КонецЕсли;
...
Процедура ВывестиОтчет(Ссылка)
	...
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
		Свойства.Вставить("ЗаголовокОтчета",             НСтр("ru='Отчет по правам пользователя';uk='Звіт з прав користувача'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   НСтр("ru='Роли пользователя по профилям';uk='Ролі користувача за профілями'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ru='Пользователь: %1';uk='Користувач: %1'"));
		
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		Свойства.Вставить("ЗаголовокОтчета",             НСтр("ru='Отчет по правам внешнего пользователя';uk='Звіт з прав зовнішнього користувача'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   НСтр("ru='Роли внешнего пользователя по профилям';uk='Ролі зовнішнього користувача по профілям'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ru='Внешний пользователь: %1';uk='Зовнішній користувач: %1'"));
		
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
		Свойства.Вставить("ЗаголовокОтчета",             НСтр("ru='Отчет по правам группы пользователей';uk='Звіт з прав групи користувачів'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   НСтр("ru='Роли группы пользователей по профилям';uk='Ролі групи користувачів по профілям'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ru='Группа пользователей: %1';uk='Група користувачів: %1'"));
	Иначе
		Свойства.Вставить("ЗаголовокОтчета",             НСтр("ru='Отчет по правам группы внешних пользователей';uk='Звіт з прав групи зовнішніх користувачів'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   НСтр("ru='Роли группы внешних пользователей по профилям';uk='Ролі групи зовнішніх користувачів по профілям'"));
		Свойства.Вставить("ПредставлениеОбъекта",        НСтр("ru='Группа внешних пользователей: %1';uk='Група зовнішніх користувачів: %1'"));
	КонецЕсли;
	...
	Если НЕ ВыводПравГруппы Тогда
		...
		Если ПользовательИБПрочитан Тогда
			...
			Если НЕ ЗначениеЗаполнено(СвойстваПользователяИБ.ПользовательОС) Тогда
				Область.Параметры.ПользовательОС = НСтр("ru='Не указан';uk='Не зазначений'");
			КонецЕсли;
			Документ.Вывести(Область, 2);
		Иначе
	...
	Если ПодробныеСведенияОПравахДоступа Тогда
		...
		Если ОднаПерсональнаяГруппа Тогда
			Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
				ПредставлениеДоступа = НСтр("ru='Ограничения доступа пользователя';uk='Обмеження доступу користувача'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ПредставлениеДоступа = НСтр("ru='Ограничения доступа внешнего пользователя';uk='Обмеження доступу зовнішнього користувача'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ПредставлениеДоступа = НСтр("ru='Ограничения доступа группы пользователей';uk='Обмеження доступу групи користувачів'");
			Иначе
				ПредставлениеДоступа = НСтр("ru='Ограничения доступа группы внешних пользователей';uk='Обмеження доступу групи зовнішніх користувачів'");
			КонецЕсли;
		Иначе
			Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
				ПредставлениеДоступа = НСтр("ru='Группы доступа пользователя';uk='Групи доступу користувача'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ПредставлениеДоступа = НСтр("ru='Группы доступа внешнего пользователя';uk='Групи доступу зовнішнього користувача'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ПредставлениеДоступа = НСтр("ru='Группы доступа группы пользователей';uk='Групи доступу групи користувачів'");
			Иначе
				ПредставлениеДоступа = НСтр("ru='Группы доступа группы внешних пользователей';uk='Групи доступу групи зовнішніх користувачів'");
			КонецЕсли;
		КонецЕсли;
	...
	Область.Параметры.ПредставлениеГруппировкиПравОбъектов = НСтр("ru='Просмотр объектов';uk='Перегляд об''єктів'");
	Документ.Вывести(Область, 1);
	...
	Для каждого ОписаниеВидаОбъектов Из ПраваОбъектов.Строки Цикл
		...
		Если УпрощенныйИнтерфейс Тогда
			Область.Параметры.ПредставлениеПрофилиИлиГруппыДоступа = НСтр("ru='Профили';uk='Профілі'");
		Иначе
			Область.Параметры.ПредставлениеПрофилиИлиГруппыДоступа = НСтр("ru='Группы доступа';uk='Групи доступу'");
		КонецЕсли;
		...
		Если ПодробныеСведенияОПравахДоступа Тогда
			Область.Параметры.ПредставлениеПрофилиИлиГруппыДоступа = НСтр("ru='(профиль, роли)';uk='(профіль, ролі)'");
		Иначе
		...
		Для каждого ОписаниеОбъекта Из ОписаниеВидаОбъектов.Строки Цикл
			...
			Если ОписаниеОбъекта.ЧтениеБезОграничения Тогда
				Если ОписаниеОбъекта.Просмотр Тогда
					УточнениеПредставленияОбъекта = НСтр("ru='(просмотр, не ограничен)';uk='(перегляд, не обмежений)'");
				Иначе
					УточнениеПредставленияОбъекта = НСтр("ru='(просмотр*, не ограничен)';uk='(перегляд*, не обмежений)'");
				КонецЕсли;
			Иначе
				Если ОписаниеОбъекта.Просмотр Тогда
					УточнениеПредставленияОбъекта = НСтр("ru='(просмотр, ограничен)';uk='(перегляд, обмежений)'");
				Иначе
					УточнениеПредставленияОбъекта = НСтр("ru='(просмотр*, ограничен)';uk='(перегляд*, обмежений)'");
				КонецЕсли;
			КонецЕсли;
	...
	Область = Макет.ПолучитьОбласть("ПраваОбъектовГруппировка");
	Область.Параметры.ПредставлениеГруппировкиПравОбъектов = НСтр("ru='Редактирование объектов';uk='Редагування об''єктів'");
	Документ.Вывести(Область, 1);
		...
		Для каждого ОписаниеОбъекта Из ОписаниеВидаОбъектов.Строки Цикл
			...
			Иначе
				Если ОписаниеОбъекта.Изменение Тогда
					Если ОписаниеОбъекта.ИзменениеБезОграничения Тогда
						Если ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ru='(изменение, не ограничено)';uk='(зміна, не обмежена)'");
						Иначе // НЕ ОписаниеОбъекта.Редактирование
							УточнениеПредставленияОбъекта = НСтр("ru='(изменение*, не ограничено)';uk='(зміна*, не обмежена)'");
						КонецЕсли;
					Иначе
						Если ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = НСтр("ru='(изменение, ограничено)';uk='(зміна, обмежена)'");
						Иначе // НЕ ОписаниеОбъекта.Редактирование
							УточнениеПредставленияОбъекта = НСтр("ru='(изменение*, ограничено)';uk='(зміна*, обмежена)'");
						КонецЕсли;
					КонецЕсли;
				Иначе // НЕ ОписаниеОбъекта.Изменение
					УточнениеПредставленияОбъекта = НСтр("ru='(изменение не доступно)';uk='(зміна не доступна)'");
				КонецЕсли;
			КонецЕсли;
	...
	Для каждого ОписаниеТипаОбъектов Из НастройкиПрав.Строки Цикл
		...
		ЗаголовокДляПодпапок =
			НСтр("ru='Для
                       ...
                       |;uk='Для
                       |підпапок'");
		ПодсказкаДляПодпапок = НСтр("ru='Права не только для текущей папки, но и для ее нижестоящих папок';uk='Права не тільки для поточної папки, але і для її нижчестоящих папок'");
		...
		ЗаголовокНастройкаПолученаОтГруппы = НСтр("ru='Настройка прав получена от группы';uk='Настройка прав отримана від групи'");
		
		Область = Макет.ПолучитьОбласть("НастройкиПравЛегендаСтрокаНаследование");
		Область.Параметры.Подсказка = НСтр("ru='Наследование прав от вышестоящих папок';uk='Спадкування прав від вищестоящих папок'");
		Документ.Вывести(Область, 2);
		...
		ТекстДа  = НСтр("ru='Да';uk='Так'");
		ТекстНет = НСтр("ru='Нет';uk='Ні'");
	...
	Если ОписаниеВидаДоступа.Строки.Количество() = 0 Тогда
		...
		ИначеЕсли ОписаниеВидаДоступа.ВсеРазрешены Тогда
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ru='%1 (без запрещенных, текущий пользователь всегда разрешен)';uk='%1 (без заборонених, поточний користувач завжди дозволений)'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ru='%1 (без запрещенных, текущий внешний пользователь всегда разрешен)';uk='%1 (без заборонених, поточний зовнішній користувач завжди дозволений)'");
			Иначе
				ШаблонПредставленияВидаДоступа = НСтр("ru='%1 (без запрещенных)';uk='%1 (без заборонених)'");
			КонецЕсли;
		Иначе
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ru='%1 (без разрешенных, текущий пользователь всегда разрешен)';uk='%1 (без дозволених, поточний користувач завжди дозволений)'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ru='%1 (без разрешенных, текущий внешний пользователь всегда разрешен)';uk='%1 (без дозволених, поточний зовнішній користувач завжди дозволений)'");
			Иначе
				ШаблонПредставленияВидаДоступа = НСтр("ru='%1 (без разрешенных)';uk='%1 (без дозволених)'");
			КонецЕсли;
		КонецЕсли;
	Иначе
		Если ОписаниеВидаДоступа.ВсеРазрешены Тогда
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ru='%1 (запрещенные, текущий пользователь всегда разрешен):';uk='%1 (заборонені, поточний користувач завжди дозволений):'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ru='%1 (запрещенные, текущий внешний пользователь всегда разрешен):';uk='%1 (заборонені, поточний зовнішній користувач завжди дозволений):'");
			Иначе
				ШаблонПредставленияВидаДоступа = НСтр("ru='%1 (запрещенные):';uk='%1 (заборонені):'");
			КонецЕсли;
		Иначе
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ru='%1 (разрешенные, текущий пользователь всегда разрешен):';uk='%1 (дозволені, поточний користувач завжди дозволений):'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					НСтр("ru='%1 (разрешенные, текущий внешний пользователь всегда разрешен):';uk='%1 (дозволені, поточний зовнішній користувач завжди дозволений):'");
			Иначе
				ШаблонПредставленияВидаДоступа = НСтр("ru='%1 (разрешенные):';uk='%1 (дозволені):'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ПредставлениеРежимаЗапуска(РежимЗапуска)
	...
	Если РежимЗапуска = "Авто" Тогда
		ПредставлениеРежимаЗапуска = НСтр("ru='Авто';uk='Авто'");
		
	ИначеЕсли РежимЗапуска = "ОбычноеПриложение" Тогда
		ПредставлениеРежимаЗапуска = НСтр("ru='Обычное приложение';uk='Звичайний додаток'");
		
	ИначеЕсли РежимЗапуска = "УправляемоеПриложение" Тогда
		ПредставлениеРежимаЗапуска = НСтр("ru='Управляемое приложение';uk='Керована прикладна програма'");
	Иначе
	...
	Пока Индекс >= 0 Цикл
		...
		Если СвойстваВидаДоступа = Неопределено Тогда
			...
			Если МетаданныеВладельцаНастроекПрав = Неопределено Тогда
				Строка.Представление = НСтр("ru='Неизвестный вид доступа';uk='Невідомий вид доступу'");
			Иначе
				Строка.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Настройки прав на %1';uk='Настройки прав на %1'"),
