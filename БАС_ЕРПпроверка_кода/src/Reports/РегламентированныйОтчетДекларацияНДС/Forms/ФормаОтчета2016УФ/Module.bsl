...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	//значения по деклаларциям прошлого периода
	...
	Если ПолучитьданныеЗаПрошлыйПериодИз1С Тогда	
		...
		ПоказателиПрошлойДекларацииПоНДС = ПоказателиПредыдущейДекларации(ДатПрошлогоПериода1, ДатПрошлогоПериода2,"РегламентированныйОтчетДекларацияНДС", "РегламентированныйОтчетУточняющийРасчетДекларацияНДС", НСтр("ru='""Декларация по НДС""';uk= '""Декларація по ПДВ""'"),"Декларация",НСтр("ru='строки 16';uk='рядка 16'"));
		...
		Если СтруктураРеквизитовФормы.СпецРежим > 0 Тогда
			...
			Если НЕ ПоказателиПрошлогоПриложения10 = Неопределено Тогда
				Попытка
					СтруктураДанныхПриложение10.R01111GB.Значение = ПоказателиПрошлогоПриложения10.R0114GB;
					СтруктураДанныхПриложение10.R02111GB.Значение = ПоказателиПрошлогоПриложения10.R0214GB;
					СтруктураДанныхПриложение10.R03111GB.Значение = ПоказателиПрошлогоПриложения10.R0314GB;
				Исключение 
				КонецПопытки
			КонецЕсли;
	...
	Иначе
		...
		Если КонецМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = КонецМесяца('2016-01-01') Тогда
			ИмяПоказателяПредыдущейДекларации = "B2400";	
		КонецЕсли;
		Если СтруктураРеквизитовФормы.СпецРежим = 0 Тогда
	...
	ТекстЗапросаПриложение8 = "ВЫБРАТЬ
	                          ...
	                          |	ЗКП.ДоговорКонтрагента.Владелец.НаименованиеПолное КАК ИмяКонтрагента,
	                          |	ЗКП.ДоговорКонтрагента.Владелец.ИНН КАК ИННКонтрагента,
