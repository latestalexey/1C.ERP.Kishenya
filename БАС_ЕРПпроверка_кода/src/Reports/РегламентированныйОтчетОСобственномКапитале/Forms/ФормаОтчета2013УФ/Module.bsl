...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
   ...
   // Бухгалтерский итоги по счетам
	...
	Если НЕ (РезФинРез = Неопределено)  Тогда
		...
		Иначе	
			////П41007 =  ЗаполнитьПоказательПоДаннымФинОтчетности("4100", "7", "Ф", РезФинРез, "2190", "3") 	
			////		- ЗаполнитьПоказательПоДаннымФинОтчетности("4100", "7", "Ф", РезФинРез, "2195", "3",,"-");
...
&НаСервере
// Функция возвращает дебетовый оборот счета, переданного в качестве параметра
Функция ДО(Счет)
