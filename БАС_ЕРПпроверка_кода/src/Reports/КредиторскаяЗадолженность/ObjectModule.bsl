...
Перем ДатаОтчета, ДатаОстатков, Валюта, ДанныеОтчета, ИспользуетсяОтборПоСегментуПартнеров,
	Календарь, СтрокаСостояниеВзаиморасчетов, СтрокаДолгНеПросрочен, ВариантКлассификацииЗадолженности;
...
Процедура УстановитьОбязательныеНастройки(КомпоновщикНастроек, ПользовательскиеНастройкиМодифицированы)
	...
	СтрокаСостояниеВзаиморасчетов = НСтр("ru='Состояние взаиморасчетов';uk='Стан взаєморозрахунків'");
	СтрокаДолгНеПросрочен = НСтр("ru='Не просрочено';uk='Не прострочено'");
...
Процедура УстановитьВариантКлассификацииЗадолженности(КомпоновщикНастроек, ПользовательскиеНастройкиМодифицированы)
	...
	Если НЕ ЗначениеЗаполнено(ПараметрВариантКлассификацииЗадолженности.Значение) Тогда
		КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ВариантКлассификацииЗадолженности", Справочники.ВариантыКлассификацииЗадолженности.ЭлементПоУмолчанию());
...
Функция ПолучитьТаблицуПлатежей()
	...
	|			ТОГДА Расчеты.КПоступлениюОстаток
	|		ИНАЧЕ 0
	|	КОНЕЦ * ЕСТЬNULL(КурсыВалют.КоэффициентПересчета, 1) КАК КПоступлениюВВалютеОтчета,
	|	АналитикаУчета.Партнер КАК Партнер,
	|	АналитикаУчета.Организация КАК Организация,
	|	АналитикаУчета.Контрагент КАК Контрагент,
	|	АналитикаУчета.Договор КАК Договор, 
