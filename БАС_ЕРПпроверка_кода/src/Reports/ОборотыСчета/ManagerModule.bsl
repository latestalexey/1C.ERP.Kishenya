...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт 
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Обороты счета %1 %2';uk='Обороти рахунку %1 %2'"),
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	Если ОсновнойНабор <> Неопределено Тогда
		...
		СекцияГДЕ = Сред(ОсновнойНабор.Запрос, СтрНайти(ОсновнойНабор.Запрос,"ГДЕ") + 4);
		...
		Для Каждого СтрокаУсловия Из СтрокиСекцииГДЕ Цикл
			...
			Если СтрНайти(СтрокаУсловия, "ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)") > 0 Тогда
