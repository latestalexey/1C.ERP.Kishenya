...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2019Кв1УФ";
...
Процедура ЗаполнитьПриложениеРИ_АМ_ТЦ()
   ...
   // Списание    = Кт оборот 10,11,12,286 - не в корр с 10,11,12,13,286 при условии, что на конец приода нет остатка по этим счетам учета (по Дт)
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.Статья = СтНД.НП15_РИ_Фин_ОЦ										Тогда ЗаполнитьКорректировкуРИ("R0317G3", "+",  Выборка.Сумма, Выборка.Статья)
		...
		Иначе
			Попытка		
				...
				ИначеЕсли Выборка.Статья = СтНД.НП15_РИ_Фин_КиноРасходы								Тогда ЗаполнитьКорректировкуРИ("R03113G3", "+",  Выборка.Сумма, Выборка.Статья)
				ИначеЕсли Выборка.Статья = СтНД.НП15_РИ_Фин_Спорт									Тогда ЗаполнитьКорректировкуРИ("R03114G3", "+",  Выборка.Сумма, Выборка.Статья)
...
Процедура ЗаполнитьПриобретениеИзОффшоров()
	ЗаполнитьПриобретения("R03161G3", СтруктураРеквизитовФормы.Контрагенты_Оффшоры);
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПриобретениеИзОффшоровОргформа()

	ЗаполнитьПриобретения("R03162G3", СтруктураРеквизитовФормы.Контрагенты_ОффшорыОргформа);
...
Процедура ЗаполнитьУбытокПрошлогоГода(ПараметрыЗаполненияЗвит1С)
	...
	Иначе
		...
		Если ГодПериода = 2016 Тогда
			ИмяСхемы1 = ИмяСхемы1 + "15";
			ИмяСхемы2 = ИмяСхемы2 + "03";
		ИначеЕсли ГодПериода = 2017 Тогда 
			ИмяСхемы1 = ИмяСхемы1 + "16";
			ИмяСхемы2 = ИмяСхемы2 + "04";
		Иначе
		    Если ИмяСхемы1 = "J01001" Тогда 
				ИмяСхемы1 = ИмяСхемы1 + "17";
			Иначе
				ИмяСхемы1 = ИмяСхемы1 + "16";
			КонецЕсли;
			ИмяСхемы2 = ИмяСхемы2 + "04";
...
Процедура ЗаполнитьПоказателиПрошогоПериода(ПараметрыЗаполненияЗвит1С)
	...
	Иначе
		...
		Если ГодПериода < 2018 или (ГодПериода = 2019 и Месяц(ДатаПрошлогоКвартала) <= 1) Тогда
			//старая схема
			ИмяСхемы = ИмяСхемы + "16";
		Иначе // с 1 квартала 2019
			ИмяСхемы = ИмяСхемы + "17";
		КонецЕсли;
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Если ЮрЛицо Тогда
		...
		НовСтрока.ИмяСхемы = ?(ГодоваяДекларация, ?(ПроизводительСХ, "J0100516", "J0108104"), "J0100117");
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ГодоваяДекларация, ?(ПроизводительСХ, "J0151816", "J0181804"), "J0111817");
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ГодоваяДекларация, ?(ПроизводительСХ, "J0150316", "J0180304"), "J0110317");
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ГодоваяДекларация, ?(ПроизводительСХ, "J0152816", "J0182804"), "J0112817");
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ГодоваяДекларация, ?(ПроизводительСХ, "J0150616", "J0180604"), "J0110617");
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ГодоваяДекларация, ?(ПроизводительСХ, "J0151316", "J0181304"), "J0111317");
		...
		НовСтрока = тбСхемыXML.Добавить(); 
		НовСтрока.ИмяСхемы = ?(ГодоваяДекларация, ?(ПроизводительСХ, "J0151416", "J0181404"), "J0111417");
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела1.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ГодоваяДекларация, ?(ПроизводительСХ, "J0151516", "J0181504"), "J0111517");
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ГодоваяДекларация, ?(ПроизводительСХ, "J0151616", "J0181604"), "J0111617");
		...
		Если НЕ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("HZU", ЭтаФорма, "Декларация")) Тогда
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ГодоваяДекларация, ?(ПроизводительСХ, "J0151716", "J0181704"), "J0111717");
	...
	Иначе
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0100702";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Декларация о прибыли';uk='Декларація про прибуток'");
		НовСтрока.НазваниеСтраницыПанели ="Декларация";
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0171802";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение PІ';uk='Додаток РI'");
		НовСтрока.НазваниеСтраницыПанели ="ПриложениеРИ";
		НовСтрока.Префикс = "RI";
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0170302";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение ЦП';uk='Додаток ЦП'");
		НовСтрока.НазваниеСтраницыПанели ="ПриложениеЦП";
		НовСтрока.Префикс = "CP";
		
		НовСтрока = тбСхемыXML.Добавить(); 
		НовСтрока.ИмяСхемы = "F0172802";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение ТЦ';uk='Додаток ТЦ'");
		НовСтрока.НазваниеСтраницыПанели ="ПриложениеТЦ";
		НовСтрока.Префикс = "TC";
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0170602";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение АМ';uk='Додаток АМ'");
		НовСтрока.НазваниеСтраницыПанели ="ПриложениеАМ";
		НовСтрока.Префикс = "AM";
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0171302";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение ПЗ';uk='Додаток ПЗ'");
		НовСтрока.НазваниеСтраницыПанели ="ПриложениеПЗ";
		НовСтрока.Префикс = "PZ";
		
		НовСтрока = тбСхемыXML.Добавить(); 
		НовСтрока.ИмяСхемы ="F0171402";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение ЗП';uk='Додаток ЗП'");
		НовСтрока.НазваниеСтраницыПанели ="ПриложениеЗП";
		НовСтрока.Префикс = "ZP";
