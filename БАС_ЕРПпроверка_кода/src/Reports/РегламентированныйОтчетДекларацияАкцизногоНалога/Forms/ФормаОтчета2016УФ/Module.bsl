...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "12/01/2016";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	КонецЦикла;
	ЗаполнитьПоказательНаСервере("R07G1", ?(Форма.D6.Итог("D6R001G6")>0, "Х", ""), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("R07G2", ?(Форма.D6.Итог("D6R001G6")>0, Форма.D6.Количество(), ""), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("R07G3", ?(Форма.D6.Итог("D6R001G6")>0, Форма.D6.Количество(), ""), Форма, "ДекларацияАкциз");
	...
	Если ЭтоУточненка ИЛИ  ЗначениеЗаполнено(ПолучитьПоказательНаСервере("HZMP", Форма, "ДекларацияАкциз")) Тогда
		...
		ЗаполнитьПоказательНаСервере("R09G2", Форма.D8.Количество(), Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R09G3", Форма.D8.Количество(), Форма, "ДекларацияАкциз");
	Иначе
	...
	Если ЗначениеЗаполнено(ПолучитьПоказательНаСервере("D10T1RXXXXG2S_1", Форма, "ДекларацияАкциз")) Тогда
		...
		ЗаполнитьПоказательНаСервере("R011G2", 1,  Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R011G3", 1,  Форма, "ДекларацияАкциз");
	Иначе
...
Процедура  ОпределитьСписокВыгружаемыхСхем(тбСхемыXML);
	...
	Для Каждого стр Из  тбСхемыXML Цикл
		...
		Если Лев(Стр.Префикс,2) = "D10"  Тогда
			стр.фВыгрузка = 0;
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Для каждого Страница Из ТаблицаСтраницыРаздела1 Цикл
		...
		НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0209505", "F0209505");  
		...
		Для  Инд = 1 ПО Страница.ДанныеДопСтрок[0].Значение.D6.Количество() Цикл
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0295605", "F0295605");  
		...
		Для  Инд = 1 ПО Страница.ДанныеДопСтрок[0].Значение.D8.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0295805", "F0295805");  
