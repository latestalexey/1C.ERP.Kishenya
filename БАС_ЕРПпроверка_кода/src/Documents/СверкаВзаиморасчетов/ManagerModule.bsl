...
Функция СформироватьПечатнуюФормуАктСверкиВзаимныхРасчетов(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Пока ДанныеДокумента.Следующий() Цикл
		...
		ТекстЗаголовка = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(ДанныеДокумента, НСтр("ru='Акт сверки взаимных расчетов';uk= 'Акт звірки взаємних розрахунків'", КодЯзыкаПечать), КодЯзыкаПечать);
...
Функция ПредставленияДокументов(КодЯзыкаПечать)
	...
	//++ НЕ УТ
	ВидыДокументовКонтрагента.Вставить("ВыбытиеДенежныхДокументов",				НСтр("ru='Расходный кассовый ордер';uk= 'Видатковий касовий ордер'", КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ЗаказПереработчику", 					НСтр("ru='Заказ';uk= 'Замовлення'", КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ОтчетПереработчика", 					НСтр("ru='Отчет переработчика';uk= 'Звіт переробника'", КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ПоступлениеДенежныхДокументов",			НСтр("ru='Приходный кассовый ордер';uk= 'Прибутковий касовий ордер'", КодЯзыкаПечать));
	//-- НЕ УТ
	//++ НЕ УТКА
	ВидыДокументовКонтрагента.Вставить("ЗаказДавальца", 						НСтр("ru='Заказ';uk= 'Замовлення'", КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ОтчетДавальцу", 						НСтр("ru='Отчет переработчика';uk= 'Звіт переробника'", КодЯзыкаПечать));
	//-- НЕ УТКА
...
Функция ПредставленияВладельцевДокументов(КодЯзыкаПечать)	
	...
	//++ НЕ УТ
	ВидыДокументовКонтрагента.Вставить("ВыбытиеДенежныхДокументов",				НСтр("ru='плательщик';uk= 'платник'", КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ЗаказПереработчику", 					НСтр("ru='от';uk= 'від'", КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ОтчетПереработчика", 					НСтр("ru='от';uk= 'від'", КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ПоступлениеДенежныхДокументов",			НСтр("ru='получатель';uk= 'одержувач'", КодЯзыкаПечать));
	//-- НЕ УТ
	//++ НЕ УТКА
	ВидыДокументовКонтрагента.Вставить("ЗаказДавальца", 						НСтр("ru='от';uk= 'від'", КодЯзыкаПечать));
	ВидыДокументовКонтрагента.Вставить("ОтчетДавальцу", 						НСтр("ru='от';uk= 'від'", КодЯзыкаПечать));
	//-- НЕ УТКА
...
Функция ВывестиДетальныеЗаписи(
	...
	Пока ДетальныеЗаписи.Следующий() Цикл
		...
		Иначе
			...
			Если ПервыйПроход
				...
				ОбластьМакетаНачОстаток.Параметры.НачалоПериодаСверки = Формат(ДанныеДокумента.НачалоПериода, "ДФ='дд ММММ гггг'; Л=" + КодЯзыкаПечать);
			...
			ОбластьМакетаСтрока.Параметры.Дата = Формат(ДетальныеЗаписи.Дата, "ДФ='дд.ММ.гггг'; Л=" + КодЯзыкаПечать);
	...
	ОбластьМакетаКонОстаток.Параметры.КонецПериодаСверки = Формат(ДанныеДокумента.КонецПериода, "ДФ='дд ММММ гггг'; Л=" + КодЯзыкаПечать);
...
Функция ВывестиТаблицуЗадолженности(
	...
	Для Каждого ВалютаВзаиморасчетов Из МассивВалют Цикл
		...
		Если ВыводитьПодвал Тогда
			...
			ОбластьМакетаПодвал.Параметры.КонецПериодаСверки = Формат(ДанныеДокумента.КонецПериода, "ДФ='дд ММММ гггг'; Л="+КодЯзыкаПечать);
