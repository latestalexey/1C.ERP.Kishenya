...
Процедура ЕжегодныеОтпускаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ЕжегодныеОтпускаОписаниеСтажевыхПоказателей" Тогда
		КадровыйУчетРасширенныйКлиент.ОткрытьФормуРедактированияСтажейСотрудника(
			ЭтаФорма, Сотрудник, ДатаСобытия, Новый ФиксированныйМассив(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВидСтажаОтпуска(ТекущиеДанные.ВидЕжегодногоОтпуска))));
	КонецЕсли;
...
Функция ПроверитьЗаполнениеОтпусков()
	...
	Для каждого СтрокаОтпуска Из ЕжегодныеОтпуска Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаОтпуска.ВидЕжегодногоОтпуска) Тогда
			ТекстСообщения = НСтр("ru='Не заполнен вид ежегодного отпуска';uk='Не заповнений вид щорічної відпустки'");
		...
		Если НЕ ЗначениеЗаполнено(СтрокаОтпуска.КоличествоДнейВГод) И НЕ СтрокаОтпуска.ОтпускЗависитОтСтажа Тогда
			ТекстСообщения = НСтр("ru='Не заполнено количество дней в год';uk='Не заповнено кількість днів на рік'");
		...
		Если ЕжегодныеОтпуска.НайтиСтроки(ПараметрыОтбора).Количество() > 1 Тогда
			ТекстСообщения = НСтр("ru='Вид отпуска ""%1"" использован в нескольких строках';uk='Вид відпустки ""%1"" використаний в декількох рядках'");
...
Процедура ЗаполнитьЕжегодныеОтпускаПоПозицииНаСервере()
	...
	Если ЗначениеЗаполнено(ДолжностьПоШтатномуРасписанию) Тогда
		...
		СтруктураПараметров.ДатаСобытия = ДатаСобытия;
		...
		СтруктураПараметров.Должность = ?(ЗначениеЗаполнено(ДолжностьПоШтатномуРасписанию), ДолжностьПоШтатномуРасписанию, Должность);
