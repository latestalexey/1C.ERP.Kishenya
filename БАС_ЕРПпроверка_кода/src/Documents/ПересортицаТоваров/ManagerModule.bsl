...
Функция ТекстЗапросаПроверкиЗаполненияСерий(ПараметрыУказанияСерий) Экспорт
	...
	|		""СерияОприходование"",
	|		ВЫБОР
	|			КОГДА ТаблицаСерий.СерияОприходование = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ,
	|		ЛОЖЬ,
	...
	|	ГДЕ
	|		(ТаблицаСерий.СтатусУказанияСерий <> 0
	|				ИЛИ ТаблицаСерий.СтатусУказанияСерийОприходование <> 0)) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
...
#Область ТекстСписаниеКомиссионныхТоваров // (Дт  :: Кт 0221, 0241)
	...
	|	
	|	ВЫБОР КОГДА Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.МатериалыПринятыеВПереработку)
	|		ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.МатериалыПринятыеВПереработку)
	|		ИНАЧЕ ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТоварыПринятыеНаКомиссию)
	|	КОНЕЦ КАК СчетКт,
	|
	|	ВЫБОР КОГДА Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.МатериалыПринятыеВПереработку)
...
#Область ТекстОприходованиеТоваровПоСебестоимости // (Дт 281 :: Кт 281, 68ОК)
	ТекстОприходованиеТоваровПоСебестоимости = "
	|ВЫБРАТЬ //// Оприходование товаров по себестоимости (Дт 281 :: Кт 281, 68ОК)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
#Область ТекстОприходованиеТоваровПоУказаннойСтоимости // (Дт 281 :: Кт 281, 68ОК)
	ТекстОприходованиеТоваровПоУказаннойСтоимости = "
	|ВЫБРАТЬ ///// Оприходование товаров по указанной стоимости (Дт 281 :: Кт 281, 68ОК)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	|	Операция.Склад.Подразделение КАК ПодразделениеДт,
	|	Строки.КорВидЗапасов.Назначение.НаправлениеДеятельности КАК НаправлениеДеятельностиДт,
	| 	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеДт, 
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДт,
...
#Область ТекстОприходованиеТоваровОтклонениеВСтоимости // (Дт 9X :: Кт 28)
	ТекстОприходованиеТоваровОтклонениеВСтоимости = "
	|ВЫБРАТЬ //// Отклонение в стоимости собственных товаров (Дт 9X :: Кт 28)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
#Область ТекстОприходованиеТоваровОтклонениеВСтоимостиДоходы // (Дт 28, 68ОК :: Кт 7X)
	ТекстОприходованиеТоваровОтклонениеВСтоимостиДоходы = "
	|ВЫБРАТЬ //// Отклонение в стоимости товаров (Дт 28, 68ОК :: Кт 7X)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
#Область ТекстКорректировкиНДСПартийПоСебестоимости // (Дт 28 :: Кт 6435)
	ТекстКорректировкиНДСПартийПоСебестоимости = "
...
#Область ТекстКорректировкиНДСПартийПоУказаннойСтоимости // (Дт 23, 9X :: Кт 6435)
	ТекстКорректировкиНДСПартийПоУказаннойСтоимости = "
	|ВЫБРАТЬ //// Отклонение в стоимости собственных товаров (Дт 23, 9X :: Кт 6435)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
#КонецОбласти 
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКорректировкиНДСПартийПоСебестоимости
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКорректировкиНДСПартийПоУказаннойСтоимости
	+ "";
