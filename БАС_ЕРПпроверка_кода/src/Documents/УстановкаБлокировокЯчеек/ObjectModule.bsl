...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОтборРазмещениеТоваров") Тогда
		...
		Если ЗначенияРеквизитов.Статус <> Перечисления.СтатусыОтборовРазмещенийТоваров.ВыполненоСОшибками Тогда
			ТекстСообщения = НСтр("ru='Ввод установки блокировки возможен только на основании задания на отбор (размещение) в статусе %Выполнено%';uk='Введення установки блокування можливе тільки на підставі завдання на відбір (розміщення) в статусі %Выполнено%'");
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Документы.УстановкаБлокировокЯчеек.ИнициализироватьДанныеДокумента(Ссылка, ДополнительныеСвойства);
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ТипБлокировки = Перечисления.ТипыБлокировокСкладскихЯчеек.Размещение Тогда
			ТекстСообщения = НСтр("ru='Ячейка %Ячейка% не может быть заблокирована с типом блокировки ""%ТипБлокировки%"":
                                       ...
                                       |;uk='Комірка %Ячейка% не може бути заблокована з типом блокування ""%ТипБлокировки%"":
                                       |є невиконані завдання на розміщення товару у комірку'");
		ИначеЕсли Выборка.ТипБлокировки = Перечисления.ТипыБлокировокСкладскихЯчеек.Отбор Тогда
			ТекстСообщения = НСтр("ru='Ячейка %Ячейка% не может быть заблокирована с типом блокировки ""%ТипБлокировки%"":
                                       ...
                                       |;uk='Комірка %Ячейка% не може бути заблокована з типом блокування ""%ТипБлокировки%"":
                                       |є невиконані завдання на відбір товару з комірки'");
		ИначеЕсли Выборка.ТипБлокировки = Перечисления.ТипыБлокировокСкладскихЯчеек.Полная Тогда
			ТекстСообщения = НСтр("ru='Ячейка %Ячейка% не может быть заблокирована с типом блокировки ""%ТипБлокировки%"":
                                       ...
                                       |;uk='Комірка %Ячейка% не може бути заблокована з типом блокування ""%ТипБлокировки%"":
                                       |є невиконані завдання на %Действие%'");
			//
			Если Выборка.КОтборуОстаток <> 0
			     И Выборка.КРазмещениюОстаток <> 0 Тогда
				 ТекстДействия = НСтр("ru='отбор и размещение товара в ячейку';uk='відбір та розміщення товару в комірку'");
			ИначеЕсли Выборка.КОтборуОстаток <> 0 Тогда
				 ТекстДействия = НСтр("ru='отбор товара из ячейки';uk='відбір товару з комірки'");
			ИначеЕсли Выборка.КРазмещениюОстаток <> 0 Тогда
				 ТекстДействия = НСтр("ru='размещение в ячейку';uk='розміщення в клітинку'");
			КонецЕсли;
