...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени("ПроведениеДокументаИзменениеКатегорий");
...
Процедура ПоказателиСотрудниковПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени("ИзменениеЗначенийПоказателейВФормеДокументаИзменениеКатегорий");
...
Процедура Заполнить(Команда)
	...
	ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени("ЗаполнениеДокументаИзменениеКатегорий");
...
Процедура ПодборСотрудников(Команда)
	...
	ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени("ПодборСотрудникаВФормеДокументаИзменениеКатегорий");	
...
Процедура ПоказателиСотрудниковПриОкончанииРедактированияНаСервере(ИдентификаторСтроки)
	...
	ФильтрСотрудников = Документы.ИзменениеКатегорий.ПустойФильтрСотрудников();
...
Процедура ЗаполнитьНачисленияПоказателиСотрудников(СообщатьПользователю = Ложь, ФильтрСотрудников = Неопределено)
	...
	Иначе
		...
		Документы.ИзменениеКатегорий.ЗаполнитьСотрудников(Объект, НачисленияПоказателиСотрудников.Сотрудники);
...
Функция ПлановыеНачисленияПоказателиСотрудников(ФильтрСотрудников = Неопределено)
	...
	НачисленияПоказателиСотрудников = Документы.ИзменениеКатегорий.НачисленияПоказателиСотрудников(ПараметрыПолучения);
...
Функция ПараметрыПолученияНачисленийПоказателейСотрудников()
	...
	ПараметрыПолучения = Документы.ИзменениеКатегорий.ПараметрыПолученияНачисленийПоказателейСотрудников();
...
&НаСервере
Процедура ПоказателиСотрудниковКатегорияПриИзмененииНаСервере()
	Строка = Объект.Сотрудники.НайтиПоИдентификатору(Элементы.ПоказателиСотрудников.ТекущаяСтрока);
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗначениеПроцентовЗаКатегориюСрезПоследних.Значение КАК Значение
	|ИЗ
	|	РегистрСведений.КадроваяИсторияСотрудников.СрезПоследних(
	|			&ДатаИзменения,
	|			Сотрудник = &Сотрудник
	|				И ГоловнаяОрганизация = &Организация
	|				И НЕ ЭтоЗамещение) КАК КадроваяИсторияСотрудниковСрезПоследних
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗначениеПроцентовЗаКатегорию.СрезПоследних(&ДатаИзменения, Категория = &Категория) КАК ЗначениеПроцентовЗаКатегориюСрезПоследних
	|		ПО КадроваяИсторияСотрудниковСрезПоследних.ДолжностьПоШтатномуРасписанию.Должность = ЗначениеПроцентовЗаКатегориюСрезПоследних.Должность";
	
	Запрос.УстановитьПараметр("Сотрудник",Строка.Сотрудник);
	Запрос.УстановитьПараметр("ДатаИзменения",Строка.ДатаИзменения);
	Запрос.УстановитьПараметр("Категория",Строка.Категория);
	...
	Если не Результат.Пустой() Тогда
		...
		Выборка.Следующий();
		стрПоказатель=Объект.Сотрудники.НайтиСтроки(Новый Структура("Сотрудник",Строка.Сотрудник));
		Если стрПоказатель.Количество()>0   Тогда
			стрПоказатель[0]["ПроцентЗаКатегорию"] = Выборка.Значение;
			стрПоказатель[0].ПроцентЗаКатегориюПредставлениеТаблицы = Строка(Выборка.Значение);
			стрПоказатель[0].ПроцентЗаКатегориюИспользуется=Истина;
			Если стрПоказатель[0].ПроцентЗаКатегориюТаблица.Количество()=1 Тогда
				стрПоказатель[0].ПроцентЗаКатегориюТаблица[0].Значение = Выборка.Значение;
			Иначе
				стрПоказатель[0].ПроцентЗаКатегориюТаблица.Очистить();
				новст = стрПоказатель[0].ПроцентЗаКатегориюТаблица.Добавить();
				новст.Значение = Выборка.Значение;
			КонецЕсли;	
		КонецЕсли;
	КонецЕсли;	
КонецПроцедуры
...
&НаКлиенте
Процедура ПоказателиСотрудниковКатегорияПриИзменении(Элемент)
	ПоказателиСотрудниковКатегорияПриИзмененииНаСервере();
КонецПроцедуры
