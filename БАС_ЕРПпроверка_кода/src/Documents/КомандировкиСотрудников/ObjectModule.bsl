...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	 Для каждого Строка Из Сотрудники Цикл
		Если ЗначениеЗаполнено(Строка.ДатаНачала)
	        ...
	        ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Дата начала командировки не может быть меньше даты окончания.';uk='Дата початку відрядження не може бути менша дати закінчення.'"),,"Объект.Сотрудники[" + НомерТекущейСтроки + "].ДатаОкончания",, Отказ);
		КонецЕсли;
	 КонецЦикла;
КонецПроцедуры
...
Процедура ОбновитьСвязаннуюКомандировку(Строка, Отказ)
	...
	Исключение
		НомерТекущейСтроки = Строка.НомерСтроки - 1;
		ТекстСообщения = НСтр("ru='Не удалось обновить командировку %1.';uk='Не вдалося оновити відрядження %1.'");
