...
#Область ТекстНачислениеНДСЗаУслуги // (Дт 7X :: Кт 643 )
	...
	|ВЫБРАТЬ //// НДС налоговые обязательтва неподтвержденные (Дт 7X :: Кт 643 )
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговые обязательства неподтвержденные"" КАК Содержание
	|
	|ИЗ 
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	|	ВЫБОР ТИПЗНАЧЕНИЯ(ТаблицаДокумента.АналитикаДоходов)
	|	КОГДА ТИП(Справочник.ОбъектыЭксплуатации) ТОГДА
	|		ВЫБОР КОГДА ВЫРАЗИТЬ(ТаблицаДокумента.АналитикаДоходов КАК Справочник.ОбъектыЭксплуатации).НаименованиеПолное = """" ТОГДА
	|			ВЫРАЗИТЬ(ТаблицаДокумента.АналитикаДоходов КАК Справочник.ОбъектыЭксплуатации).Наименование
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ(ТаблицаДокумента.АналитикаДоходов КАК Справочник.ОбъектыЭксплуатации).НаименованиеПолное
	|		КОНЕЦ
	|	КОГДА ТИП(Справочник.НематериальныеАктивы) ТОГДА
	|		ВЫБОР КОГДА ВЫРАЗИТЬ(ТаблицаДокумента.АналитикаДоходов КАК Справочник.НематериальныеАктивы).НаименованиеПолное = """" ТОГДА
	|			ВЫРАЗИТЬ(ТаблицаДокумента.АналитикаДоходов КАК Справочник.НематериальныеАктивы).Наименование
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ(ТаблицаДокумента.АналитикаДоходов КАК Справочник.НематериальныеАктивы).НаименованиеПолное
	|		КОНЕЦ
	|	ИНАЧЕ
	//-- НЕ УТ
