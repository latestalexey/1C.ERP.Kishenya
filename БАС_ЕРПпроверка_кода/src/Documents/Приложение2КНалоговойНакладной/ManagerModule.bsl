...
#Область ОбязательстваПоНДС // (Дт 643 :: Кт 641)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговые обязательства: корректировка (Приложение 2)"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.Приложение2КНалоговойНакладной КАК Операция
	|	ПО
...
#Область ОбязательстваПоНДСУсловнаяПродажаНаДоходы // (Дт 641 :: Кт 7X)
	ОбязательстваПоНДСУсловнаяПродажаНаДоходы = "
	|ВЫБРАТЬ //// Налоговые обязательства (Дт 641 :: Кт 7X)
	|	Операция.Ссылка КАК Ссылка,
	|	НДСРеестр.Период КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	...
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиДоходов.ДоходыПриВосстановленииНалоговогоКредита) КАК АналитикаУчетаКт,
	...
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиДоходов.ДоходыПриВосстановленииНалоговогоКредита) КАК СубконтоКт1,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|
	|	0 КАК ВалютнаяСуммаКт,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговые обязательства: доходы корректировка (Приложение 2)"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|   )
	|	И Операция.ВидОперацииВозвратКорректировка = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриложений2КНалоговойНакладной.Возврат)	
	|";
#КонецОбласти
...
#Область ОбязательстваПоНДСУсловнаяПродажа // (Дт 643 :: Кт 641)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговые обязательства: условная продажа корректировка (Приложение 2)"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	
	|ГДЕ
	|	Операция.ВидОперации В (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийНалоговыхДокументов.УсловнаяПродажа), ЗНАЧЕНИЕ(Перечисление.ВидыОперацийНалоговыхДокументов.СводнаяУсловнаяПродажа))
	|	И Операция.ВидОперацииВозвратКорректировка В (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриложений2КНалоговойНакладной.Корректировка), ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПриложений2КНалоговойНакладной.КорректировкаВозврата))
	|";
#КонецОбласти

#Область ПродажеНижеОбычнойЦеныНаДоходы // (Дт 704 :: Кт 641)
	ПродажеНижеОбычнойЦеныНаДоходы = "
	|ВЫБРАТЬ //// Налоговые обязательства (Дт 704 :: Кт 641)
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР
	|		КОГДА Операция.ДатаВключенияВЕдиныйРеестрНалоговыхНакладных <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА Операция.ДатаВключенияВЕдиныйРеестрНалоговыхНакладных
	|		ИНАЧЕ 
	|			Операция.Дата
	|	КОНЕЦ КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	Сумма(Строки.СуммаНДСРегл) КАК Сумма,
	|
	...
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт1, 
	|	Строки.Номенклатура.ГруппаФинансовогоУчета КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|	
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговые обязательства: продажа ниже ОЦ корректировка (Приложение 2)"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.Приложение2КНалоговойНакладной.ТоварыПоДаннымПользователя КАК Строки
	|	ПО 
...
#КонецОбласти
	...
	Возврат ОбязательстваПоНДС
		...
		+ " ОБЪЕДИНИТЬ ВСЕ " + ОбязательстваПоНДСУсловнаяПродажаНаДоходы
		+ " ОБЪЕДИНИТЬ ВСЕ " + ОбязательстваПоНДСУсловнаяПродажа
		+ " ОБЪЕДИНИТЬ ВСЕ " + ПродажеНижеОбычнойЦеныНаДоходы
		;
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ДанныеДокумента.ГодовойПерерасчетПропорциональногоНДС КАК ГодовойПерерасчетПропорциональногоНДС,
	//-- НЕ УТ
	...
	Запрос.УстановитьПараметр("ГодовойПерерасчетПропорциональногоНДС",    Реквизиты.ГодовойПерерасчетПропорциональногоНДС);
	//-- НЕ УТ
...
Функция ТекстЗапросаТаблицаСуммыКорректировокПропорциональногоНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&Период КАК Период,
	|	&Организация КАК Организация,
	|	ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.КорректировкаУсловнойПродажи) КАК ВидУсловнойПродажи,
	|	&СпецРежимНалогообложения КАК СпецРежимНалогообложения,
	|	СУММА(ПоставкаДокумента.СуммаНДСРегл) КАК СуммаКорректировкиНДС
	|ИЗ
	|	Документ.Приложение2КНалоговойНакладной.ТоварыПоДаннымПользователя КАК ПоставкаДокумента
	|ГДЕ
	|	ПоставкаДокумента.Ссылка = &Ссылка
	|	И &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийНалоговыхДокументов.СводнаяУсловнаяПродажа)
	|	И &ГодовойПерерасчетПропорциональногоНДС
	|
	|ИМЕЮЩИЕ
	|	СУММА(ПоставкаДокумента.СуммаНДСРегл) <> 0
	|";
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	// Налоговая накладная (альбомная ориентация)
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "Приложение2КНалоговойНакладнойАльбом";
	КомандаПечати.Представление = НСтр("ru='Приложение 2 к налоговой накладной (альбомная ориентация)';uk='Додаток 2 до податкової накладної (альбомна орієнтація)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Приложение2КНалоговойНакладнойАльбом") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм, 
			"Приложение2КНалоговойНакладнойАльбом", 
			"Приложение 2 к налоговой накладной", 
			СформироватьПечатнуюФормуПриложение2КНалоговойНакладной(МассивОбъектов, ОбъектыПечати, ПараметрыВывода, "Приложение2КНалоговойНакладнойАльбом"
																	// РегламентированнаяОтчетность
																	,ТолькоФормироватьСтруктуруПоказателейПечатнойФормы
																	)
																	// Конец РегламентированнаяОтчетность
...
Функция СформироватьПечатнуюФормуПриложение2КНалоговойНакладной(МассивОбъектов, ОбъектыПечати, ПараметрыВывода, ИмяМакета, ТолькоФормироватьСтруктуруПоказателейПечатнойФормы = Ложь) Экспорт	
	...
	|			ИНАЧЕ Приложение2КНалоговойНакладнойТовары.Упаковка.ЕдиницаИзмерения
	...
	|			ИНАЧЕ Приложение2КНалоговойНакладнойТоварыПоДаннымПользователя.Упаковка.ЕдиницаИзмерения
	|		КОНЕЦ,
	...
	Пока ДанныеПечати.Следующий() цикл
		...
		Если НДСОбщегоНазначенияСервер.ПечатнаяФорма2018(ДанныеПечати.Дата, ДанныеПечати.Организация) Тогда
		...
		Если НомерПоХронологииФормыП2 >= 4 Тогда
			Если НомерПоХронологииФормыП2 = 6 Тогда
...
Процедура ЗаполнитьСтатьюДекларацииНДСНалоговыеОбязательства(ВидОперации, СтатьяДекларацииНДСНалоговыеОбязательства) Экспорт
...
Процедура УстановитьТипПричиныНевыдачиПокупателюПоУмолчанию(Объект) Экспорт
	...
	ИначеЕсли НЕ (Объект.Валюта = Константы.ВалютаРегламентированногоУчета.Получить()) 
		    И НЕ (Объект.ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.РаботыОтНерезидента) Тогда
