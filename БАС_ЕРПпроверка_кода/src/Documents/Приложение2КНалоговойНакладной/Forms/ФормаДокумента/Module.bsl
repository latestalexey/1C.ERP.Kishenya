...
Процедура СтатусРегистрацииВЕРННПриИзменении(Элемент)
	...
	Если Объект.СтатусРегистрацииВЕРНН = ПредопределенноеЗначение("Перечисление.СтатусыРегистрацииВЕРНННалоговыхДокументов.ЗарегистрированВЕРНН") Тогда
...
// ГруппаСтраницаДанные, ГруппаСтраницаДанныеПользователя
	...
	Иначе
//++ НЕ УТ
		МассивТипов.Добавить(Тип("ДокументСсылка.КорректировкаНалоговогоНазначенияОС"));
//-- НЕ УТ
...
Функция ПересчитатьВВалютуРегл(Цена)
	...
	СтруктураКурсовВалюты = РаботаСКурсамиВалют.ПолучитьКурсВалюты(
		Объект.Валюта, 
		Документы.Приложение2КНалоговойНакладной.ПолучитьДатуКурсаВалюты(Объект)
	);
