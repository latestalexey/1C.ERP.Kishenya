...
Процедура ПеренестиСтрокиВДокумент()
	...
	Закрыть();
	ОповеститьОВыборе(Новый Структура("ВыполняемаяОперация, АдресВХранилище, РазвернутьВыпускиБезРаспоряжения",
						"ПодборВыходныхИзделий", АдресВХранилище, РазвернутьТаблицуВыпуска));
...
Процедура СвернутьРазвернутьТЧ(ИмяТЧ, Развернуть = Ложь)
	...
	Если Не Развернуть Тогда
		...
		ТаблицаЗначений = ЭтаФорма[ИмяТЧ].Выгрузить(,КолонкиГруппировок + ", " + КолонкиСуммирования);
	...
	УстановитьНомераСтрок(ЭтаФорма, ИмяТЧ + "Свернутая");
...
Процедура УстановитьВидимость()
	...
	Если (ТипЗнч(Документ) = Тип("ДокументСсылка.РаспределениеПроизводственныхЗатрат")) Тогда
		...
		Элементы.Спецификация.Заголовок = НСтр("ru='Показывать продукцию, выпущенную по спецификации';uk='Показувати продукцію, випущену за специфікацією'");
