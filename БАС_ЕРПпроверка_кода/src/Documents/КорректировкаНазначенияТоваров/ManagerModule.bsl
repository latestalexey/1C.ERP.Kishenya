...
#Область ТекстКорректировкаНаправленийДеятельности // (Дт 28, 02 :: Кт 28, 02)
	ТекстКорректировкаНаправленийДеятельности = "
	|ВЫБРАТЬ //// Корректировка направлений на складах (Дт 28, 02 :: Кт 28, 02)
	|
	...
	|			ИЛИ Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.МатериалыПринятыеВПереработку) ТОГДА
	|		НЕОПРЕДЕЛЕНО
	|	ИНАЧЕ
	...
	|
	|	ВЫБОР
	|		КОГДА Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссию) ТОГДА
	|				ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТоварыПринятыеНаКомиссию) 
	|		КОГДА Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.МатериалыПринятыеВПереработку) ТОГДА
...
#КонецОбласти
	...
	Возврат ТекстКорректировкаНаправленийДеятельности;
...
//++ НЕ УТКА
Функция ВременнаяТаблицаОбеспечениеЗаказовОстаткиБезДавальческойПродукции()
	...
	ТекстЗапроса =
		...
		|	ТаблицаОстатков.Характеристика  КАК Характеристика,
		|	ТаблицаОстатков.Склад           КАК Склад,
		|	ТаблицаОстатков.НаличиеПодЗаказ КАК НаличиеПодЗаказ,
		|	ТаблицаОстатков.КЗаказу         КАК КЗаказу,
		|	ТаблицаОстатков.Потребность     КАК Потребность
		|ПОМЕСТИТЬ ВтОбеспечениеЗаказовОстатки
		|ИЗ
		|	ВтОбеспечениеЗаказовОстаткиДоУдаленияДавальческойПродукции КАК ТаблицаОстатков
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца.Продукция КАК ПродукцияЗаказаДавальца
		|			ПО ПродукцияЗаказаДавальца.Ссылка        = ТаблицаОстатков.Назначение.Заказ
		|			И ПродукцияЗаказаДавальца.Номенклатура   = ТаблицаОстатков.Номенклатура
		|			И ПродукцияЗаказаДавальца.Характеристика = ТаблицаОстатков.Характеристика
		|ГДЕ
		|	ПродукцияЗаказаДавальца.Ссылка ЕСТЬ NULL";
...
Функция ВременнаяТаблицаСкладыОтгрузки()
	...
	ТекстЗапроса =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДвиженияЗаказа.Склад          КАК Склад,
		|	ДвиженияЗаказа.Номенклатура   КАК Номенклатура,
		|	ДвиженияЗаказа.Характеристика КАК Характеристика,
		|	ДвиженияЗаказа.Назначение     КАК Назначение
		|ПОМЕСТИТЬ ВтСкладыОтгрузки
		|ИЗ
		|	ВтОбеспечениеЗаказовОстатки КАК ТаблицаОстатков
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ОбеспечениеЗаказов КАК ДвиженияЗаказа
		|		ПО ДвиженияЗаказа.Регистратор = ТаблицаОстатков.Назначение.Заказ
		|		И ДвиженияЗаказа.Потребность <> 0
		|ГДЕ
		|	ДвиженияЗаказа.Активность
		|	И НЕ ДвиженияЗаказа.Склад ЕСТЬ NULL
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Склад,
		|	Номенклатура,
...
//++ НЕ УТКА
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВТОбеспечениеЗаказовОстатки", "ВТОбеспечениеЗаказовОстаткиДоУдаленияДавальческойПродукции");
//-- НЕ УТКА
...
//++ НЕ УТКА
	ТекстЗапроса = ВременнаяТаблицаОбеспечениеЗаказовОстаткиБезДавальческойПродукции();
	Запрос.Текст = ТекстЗапроса;
	УстановитьПривилегированныйРежим(Истина);
	Запрос.Выполнить();
	УстановитьПривилегированныйРежим(Ложь);
...
Процедура ЗаполнитьПоЗаказу(Заказ, Ссылка, Товары) Экспорт
	...
	ИначеЕсли ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда 
		...
		"
		|ВЫБРАТЬ
		|	ЕСТЬNULL(НазначенияДавальца.Ссылка, ТМ.Ссылка.Назначение) КАК Назначение,
		...
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
		|		Документ.ЗаказНаПроизводство.Продукция КАК ТП
		|	ПО
		|		ТМ.Ссылка = ТП.Ссылка
		|		И ТМ.КлючСвязиПродукция = ТП.КлючСвязи
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		Справочник.Назначения КАК НазначенияДавальца
		|	ПО
		|		НазначенияДавальца.Ссылка = ТП.Назначение
		|		И НазначенияДавальца.Заказ ССЫЛКА Документ.ЗаказДавальца
		|ГДЕ
		|	ТМ.Ссылка = &ДокументОснование
		|	И ЕСТЬNULL(НазначенияДавальца.Ссылка, ТМ.Ссылка.Назначение) <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|";
