...
//++ НЕ УТКА
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья") Тогда
				СписокВыбора.Добавить(Перечисления.ТипыДоговоров.СДавальцем, НСтр("ru='Договор с давальцем';uk='Договір з давальцем'"));
				СписокВыбора.Добавить(Документы.ОтчетДавальцу.ПустаяСсылка(), НСтр("ru='Отчет давальцу';uk='Звіт давальцю'"));
		КонецЕсли;
//-- НЕ УТКА
...
Процедура РассчитатьПредположительнуюСуммуНалоговогоКредита(СтрокаТаблицы)
	...
	Если СтрокаТаблицы.НетНалоговойНакладной И СтрокаТаблицы.ВалютаВзаиморасчетов = ВалютаРегламентированногоУчета Тогда
