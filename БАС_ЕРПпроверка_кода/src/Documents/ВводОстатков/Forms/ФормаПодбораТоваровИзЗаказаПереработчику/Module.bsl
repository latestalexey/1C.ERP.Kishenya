...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, 
	...
	//++ НЕ УТКА
	|		ЕСТЬNULL(ЗаказНаПроизводствоДляДавальца.Назначение,
	...
	|	ПО
	|		ЗаказНаПереработку.Ссылка.ПереработкаПоЗаказу
	|		И ЗаказНаПереработку.Распоряжение = ЗаказНаПроизводство.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Документ.ЗаказНаПроизводство.Продукция КАК ЗаказНаПроизводствоДляДавальца
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|			Справочник.Назначения КАК НазначенияДавальца
	|		ПО
	|			ЗаказНаПроизводствоДляДавальца.Назначение = НазначенияДавальца.Ссылка
	|			И ТИПЗНАЧЕНИЯ(НазначенияДавальца.Заказ) = ТИП(Документ.ЗаказДавальца)
	|
	|	ПО
	|		ЗаказНаПереработку.Распоряжение = ЗаказНаПроизводствоДляДавальца.Ссылка
	|		И ЗаказНаПереработку.КодСтрокиПродукция = ЗаказНаПроизводствоДляДавальца.КодСтроки	
	//-- НЕ УТКА
	|УПОРЯДОЧИТЬ ПО
