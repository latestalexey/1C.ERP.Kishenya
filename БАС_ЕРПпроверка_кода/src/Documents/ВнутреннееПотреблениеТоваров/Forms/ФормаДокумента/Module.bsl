...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство") 
		И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаВПроизводство Тогда
		Элементы.ХозяйственнаяОперация.СписокВыбора.Добавить(Перечисления.ХозяйственныеОперации.ПередачаВПроизводство);
	КонецЕсли;
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаВПроизводство Тогда
		// Такие документы больше используются и открываются только на просмотр
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ТоварыЗаказНаВнутреннееПотребление Тогда
		...
		КонецЕсли;
	//++ НЕ УТ
	ИначеЕсли Поле = Элементы.ТоварыСчетРеглУчетаАктивовПассивов Тогда
		РеглУчетКлиент.ОткрытьНастройкуОтраженияВРеглУчетеАктиваПассиваСтрокиТЧ(ЭтаФорма, "Товары", ВыбраннаяСтрока);
		СтандартнаяОбработка = Ложь;
	//-- НЕ УТ
	КонецЕсли;
...
Функция ПривязатьСтрокиКЗаказамНаВнутреннееПотребление(ОтобратьПоЗаказу = Ложь) Экспорт
	...
	//++ НЕ УТКА
	|			ИЛИ Товары.ЗаказНаВнутреннееПотребление = ЗНАЧЕНИЕ(Документ.ЗаказНаРемонт.ПустаяСсылка)
	//-- НЕ УТКА
	...
	|	Таблица.Ссылка КАК Заказ
	|ИЗ
	|	Документ.ЗаказНаРемонт КАК Таблица
	|ГДЕ
	|	ВЫБОР КОГДА &ОтобратьПоЗаказу ТОГДА
	|				Таблица.Ссылка = &Заказ
	|			ИНАЧЕ Таблица.Подразделение = &Подразделение
	...
	Если РезультатыЗаполнения <> Неопределено Тогда
		...
		Если РезультатыЗаполнения.ЕстьНесобранные Тогда
			...
			ПоказатьОповещениеПользователя(
				НСтр("ru='Количество перезаполнено';uk='Кількість перезаполнено'"),,
...
Процедура УстановитьВидимость(ОчищатьСтатьи = Ложь)
	...
	//++ НЕ УТ
	Элементы.ТоварыГруппаСчетРеглУчета.Видимость = (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваровПоТребованию);
	Элементы.ГруппаКомиссия.Видимость			 = (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаВЭксплуатацию);	
	//-- НЕ УТ
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	//++ НЕ УТ
	ИспользуютсяЗаказы = ИспользуютсяЗаказы ИЛИ ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеРемонтами");
	//-- НЕ УТ
...
Процедура УстановитьУсловноеОформление()
	...
	//++ НЕ УТ
	РеглУчетВызовСервера.УстановитьУсловноеОформлениеСчетаУчетаПриСовместномИспользованииСтатей(ЭтаФорма, "Товары", ТипыСтатей);
	//-- НЕ УТ
	#КонецОбласти
...
Процедура ЗаполнитьСлужебныеРеквизитыТабличнойЧасти()
	...
	//++ НЕ УТ
	РеглУчетВызовСервера.ЗаполнитьПредставлениеСчетаРеглУчетаВТЧ(Объект.Товары);
	//-- НЕ УТ
