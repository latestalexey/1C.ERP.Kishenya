...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаВПроизводство И НЕ ПометкаУдаления Тогда
		ТекстСообщения = НСтр("ru='Запись документа запрещена, т.к. операция ""Передача в производство"" больше не используется.
                                ...
                                |;uk='Запис документа заборонений, оскільки операція ""Передача у виробництво"" більше не використовується.
                                |Передача матеріалів оформляється у формі ""Виробництво - Отримання і повернення матеріалів"".'");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПроверитьЗаполнениеИнвентарныхНомеров(МассивНепроверяемыхРеквизитов, Отказ);
	//-- НЕ УТ
...
//++ НЕ УТКА
	...
	Если Не СтруктураЗаполнения.Свойство("Склад")
		И ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказНаРемонт")
		...
		СтруктураЗаполнения.Вставить("Склад", СкладЗаказаНаРемонт);
		ПараметрыЗаполнения.Вставить("Склад", СкладЗаказаНаРемонт);
		
	КонецЕсли;
	
	// При заполнении на основании заказа на ремонт из рабочего места Внутреннее товародвижение
	// Склад передается из рабочего места, используем его
	// Для заказа на внутреннее потребление наличие или отсутствие отбора по складу не важно
	Если Не ЗначениеЗаполнено(ПараметрыЗаполнения.Склад)
		И СтруктураЗаполнения.Свойство("Склад") Тогда
		ПараметрыЗаполнения.Вставить("Склад", СтруктураЗаполнения.Склад);
	КонецЕсли;
//-- НЕ УТКА
...
//++ НЕ УТ
Процедура ПроверитьЗаполнениеИнвентарныхНомеров(НепроверяемыеРеквизиты, Отказ)
