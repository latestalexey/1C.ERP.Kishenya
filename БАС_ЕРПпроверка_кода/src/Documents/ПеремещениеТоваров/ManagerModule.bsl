...
#Область ТекстПеремещениеКомиссионныхТоваров // (Дт 0221, 0241 :: Кт 0221, 0241)
...
#Область ТекстПеремещениеПродукцииДавальца // (Дт 023 :: Кт 023)
	ТекстПеремещениеПродукцииДавальца = "
	|ВЫБРАТЬ //// Перемещение продукции давальца (Дт 023 :: Кт 023)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Перемещение продукции давальца"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|ГДЕ
	|	Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеремещениеТоваров)
	|	И Строки.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца)
	|";
#КонецОбласти 

#Область ТекстПеремещениеСобственныхТоваров // (Дт 281 :: Кт 281)
...
#КонецОбласти 
	...
	Возврат 
	ТекстПеремещениеКомиссионныхТоваров
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстПеремещениеПродукцииДавальца
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Изменение", Метаданные.Документы.ПеремещениеТоваров) Тогда
