...
#Область ТекстКомитентПродажаКомиссионногоТовара // (Дт  :: Кт 0242)
	...
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТоварыПринятыеНаКомиссиюНаСубкомиссии) КАК СчетКт,   
...
#Область ТекстКомитентВзаиморасчетыПоКомиссионномуТовару // (Дт 36 :: Кт 70)
	...
	|		ВидыЗапасовКомитента.ГруппаФинансовогоУчета
	|	ИНАЧЕ
	...
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	|	Операция.Подразделение КАК ПодразделениеКт,
	|	ЕСТЬNULL(ВидыЗапасовДокумента.Назначение.НаправлениеДеятельности,
	|		ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) КАК НаправлениеДеятельностиКт,
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеКт,
	|
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.АналитикаВидовЗапасов КАК ВидыЗапасовКомитента
	|	ПО
	|		ВидыЗапасовКомитента.КлючАналитики = Строки.ВидЗапасовКомитента
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|		ИНАЧЕ Операция.Комиссионер
	|	КОНЕЦ,
	|	ЕСТЬNULL(ВидыЗапасовДокумента.Назначение.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)),
	|	ВЫБОР
	|		КОГДА &ФормироватьВидыЗапасовПоГруппамФинансовогоУчета
	|			ТОГДА ВидыЗапасовКомитента.ГруппаФинансовогоУчета
	|		ИНАЧЕ Аналитика.Номенклатура.ГруппаФинансовогоУчета
	|	КОНЕЦ,
	|	Операция.Подразделение,
	|	ВЫБОР
	|		КОГДА &ФормироватьВидыЗапасовПоГруппамФинансовогоУчета
	|			ТОГДА Строки.ВидЗапасовКомитента.ГруппаФинансовогоУчета
	|		ИНАЧЕ Аналитика.Номенклатура.ГруппаФинансовогоУчета
	|	КОНЕЦ
	|";
#КонецОбласти
...
#Область ТекстКомитентНДССРеализацииСобственногоТовара // (Дт 70 :: Кт 643)
	...
	|ВЫБРАТЬ  //// НДС с релизации (Дт 70 :: Кт 643)  
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|			ТОГДА ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаНДСРегл, Строки.СуммаНДС)
	|		ИНАЧЕ 0
	|	КОНЕЦ) КАК Сумма,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ВыручкаОтПродажКомиссионера) КАК ВидСчетаДт,
	|	ВЫБОР КОГДА &ФормироватьВидыЗапасовПоГруппамФинансовогоУчета ТОГДА
...
#Область ТекстКомитентЗатратыНаКомиссионноеВознаграждение // (Дт 91-93 :: Кт 377, 685)
	ТекстКомитентЗатратыНаКомиссионноеВознаграждение = "
	|ВЫБРАТЬ //// Затраты на комиссионное вознаграждение (Дт 91-93 :: Кт 377, 685)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|
	|	СУММА(ВЫБОР 
	|		КОГДА НЕ УчетнаяПолитикаКомитента.ПлательщикНДС  
	|			ТОГДА Строки.СуммаВознаграждения
	|		КОГДА Строки.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС)
	|			ТОГДА Строки.СуммаВознаграждения
	|		КОГДА Строки.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НеНДС)
	|			ТОГДА Строки.СуммаВознаграждения
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		УчетнаяПолитикаКомитента КАК УчетнаяПолитикаКомитента
	|	ПО 
	|		Операция.Ссылка = УчетнаяПолитикаКомитента.Ссылка
	|			И (Операция.Организация = УчетнаяПолитикаКомитента.Организация)
	| 
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|		Истина
	|ГДЕ
	|	Операция.Валюта = Константы.ВалютаРегламентированногоУчета
	|
	|СГРУППИРОВАТЬ ПО
	|	Операция.Ссылка,
	|	Операция.Дата,
...
#Область ТекстКомитентНДСНаКомиссионноеВознаграждение // (Дт 644 :: Кт 377, 685)
	...
	|ВЫБРАТЬ //// НДС на комиссионное вознаграждение (Дт 644 :: Кт 377, 685)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
#Область ТекстКомитентУдержатьВознаграждениеОплатаДолга // (Дт 377, 685 Кт 36)
	...
	|	ПО
	|		РасчетыПоДокументам.Ссылка = Расчеты.Ссылка
	|		И РасчетыПоДокументам.Контрагент = Расчеты.Контрагент
	|		И РасчетыПоДокументам.ЗаказКлиента = Расчеты.ЗаказКлиента
...
#Область ТекстКомитентКорректировкаНДСПроданноготовара // (Дт 6431 :: Кт 6432)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговые обязательства: корректировка по цене продажи"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомиссионерНДСВходящийОтПродаж // (Дт 644 :: Кт 704)
	...
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	СУММА(ВЫБОР КОГДА (НЕ Строки.НалоговоеНазначениеСписания.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая)) ТОГДА
	|		ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаНДСРегл, Строки.СуммаНДС)
	|	ИНАЧЕ
	|		0
	|	КОНЕЦ) КАК Сумма,
	|
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговый кредит: отгрузка"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомиссионерКорректировкаНДСПроданногоТовара // (Дт 6442 :: Кт 6441)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговый кредит: корректировка по цене продажи"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#КонецОбласти 
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКомитентКорректировкаНДСПроданногоТовара
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКомиссионерНДСВходящийОтПродаж
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКомиссионерКорректировкаНДСПроданногоТовара
	;
...
Функция ТекстЗапросаВТОтраженияВРеглУчетеУКР() Экспорт
	...
	|		ПО ОтчетПоКомиссииМеждуОрганизациями.Ссылка = ДокументыКОтражению.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаОрганизаций КАК УчетнаяПолитикаОрганизаций
	|		ПО ОтчетПоКомиссииМеждуОрганизациями.Организация = УчетнаяПолитикаОрганизаций.Организация
	|			И ОтчетПоКомиссииМеждуОрганизациями.Дата >= УчетнаяПолитикаОрганизаций.Период
	|
	|СГРУППИРОВАТЬ ПО
	...
	|ПОМЕСТИТЬ УчетнаяПолитикаКомитента
	|ИЗ
