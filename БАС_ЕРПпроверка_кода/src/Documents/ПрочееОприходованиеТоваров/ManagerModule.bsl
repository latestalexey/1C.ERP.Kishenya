...
Процедура ЗаполнитьИменаЭлементовПоХозяйственнойОперации(ХозяйственнаяОперация, МассивВсехЭлементов, МассивЭлементовОперации) Экспорт
	...
	//-- НЕ УТ
	//++ НЕ УТКА
	МассивВсехЭлементов.Добавить("ФормаРегистрБухгалтерииМеждународныйПроводкиДокументаОперативногоУчета");
	//-- НЕ УТКА
	...
	Если ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ВозвратИзЭксплуатации Тогда
		...
		//++ НЕ УТКА
		МассивЭлементовОперации.Добавить("ФормаРегистрБухгалтерииМеждународныйПроводкиДокументаОперативногоУчета");
		//-- НЕ УТКА
	КонецЕсли;
	//-- НЕ УТ
...
#Область ТекстВозвратТоваровНаСкладИзПроизводства // (Дт 23 :: Кт 20, 28 сторно)
...
#Область ТекстВозвратТоваровДавальцаИзПроизводство // (Дт 0221/023 :: Кт 0222)
	ТекстВозвратТоваровДавальцаИзПроизводство = "
...
#Область ТекстПрочееПоступлениеТоваров // (Дт 20, 28 :: Кт 7)
	...
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	|	Операция.Склад.Подразделение КАК ПодразделениеДт,
	|	Строки.НазначениеОтправителя.НаправлениеДеятельности КАК НаправлениеДеятельностиДт,
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеДт,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДт,
	|	
...
#Область ТекстСписаниеКомиссионныхТоваровСторно // Дт  :: Кт 0241)
	ТекстСписаниеКомиссионныхТоваровСторно = "
	|ВЫБРАТЬ //// Списание комиссионных и давальческих товаров (cторно) (Дт  :: Кт 0241)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Списание комиссионных товаров (cторно)"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстСписаниеТоваровНаРасходыСторно // (Дт 23, 9X :: Кт 28, 68ОК)
	ТекстСписаниеТоваровНаРасходыСторно = "
	|ВЫБРАТЬ //// Списание на расходы товаров (cторно) (Дт 23, 9X :: Кт 28, 68ОК)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Списание на расходы товаров (cторно)"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстСписаниеТоваровНаПрочиеАктивыСторно // (Дт 1522 :: Кт 28, 68ОК)
	ТекстСписаниеТоваровНаПрочиеАктивыСторно = "
	|ВЫБРАТЬ //// Списание на прочие активы (cторно) (Дт 1522 :: Кт 28, 68ОК)
	|//Сторнирование происходит в том же месяце, что и списание
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Списание на прочие активы (cторно)"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстСписаниеТоваровДавальцаСторно // (Дт  :: Кт 0221/023)
	ТекстСписаниеТоваровДавальцаСторно = "
	|ВЫБРАТЬ //// Передача товаров давальца в производство (сторно) (Дт  :: Кт 0221/023)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
Функция УстановитьУсловноеОформлениеСписка(Список) Экспорт
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Поступление из производства';uk='Надходження з виробництва'"));
	//-- НЕ УТ
