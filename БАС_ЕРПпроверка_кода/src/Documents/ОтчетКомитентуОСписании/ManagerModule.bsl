...
#Область ТекстЗадолженностьПередКомитентом // (Дт 68ОК :: Кт 63)
	...
	|	ВидыЗапасовТоваров.Назначение.НаправлениеДеятельности КАК НаправлениеДеятельностиДт,
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		Справочник.ВидыЗапасов КАК ВидыЗапасовТоваров
	|	ПО 
	|		(Строки.ВидЗапасов = ВидыЗапасовТоваров.Ссылка)
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
...
#Область ТекстНалоговыйКредитУсловнаяПродажа // (Дт 6441 :: Кт 68ОК, 6435)
	ТекстНалоговыйКредитУсловнаяПродажа = "
	|ВЫБРАТЬ //// Налоговый кредит (Дт 6441 :: Кт 68ОК, 6435)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|		КОГДА НЕ Строки.ВидЗапасов.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая)
	|				И Строки.ЦелевоеНалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая)
...
#КонецОбласти 
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНалоговыйКредитУсловнаяПродажа
	;
