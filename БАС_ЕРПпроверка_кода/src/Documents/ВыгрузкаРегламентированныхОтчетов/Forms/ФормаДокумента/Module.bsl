...
Функция СформироватьИЗаписать(ДокументыОснования, КаталогСохранения = Неопределено, ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно = Истина) Экспорт
	...
	Попытка
		СформироватьТекстыВыгрузкиКлиент(ПервыйДок);
	Исключение
	...
	Попытка
		СохранитьТексты(КаталогСохранения, , , , ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно);
	Исключение
...
Процедура УстановитьСнятьФлажки(Пометка)
	...
	Для Каждого Строка Из Объект.Выгрузки Цикл
		Если НЕ Строка.фВыгрузка = Пометка Тогда
...
Процедура СформироватьТекстыВыгрузки(Команда)
	...
	Если ОшибкаВыгрузки <> Истина Тогда
		ПоказатьПредупреждение(,НСтр("ru='Формирование текстов выгрузки успешно завершено.';uk='Формування текстів вивантаження успішно завершено.'"));
	КонецЕсли;
...
Функция ПреобразоватьТипXML(Значение, Тип, ОписаниеОшибки = "")
	...
	Если ВРег(Тип) = ВРег("DGMonth") 
	 ...
	 ИЛИ ВРег(Тип) = ВРег("DGMonth0") Тогда
		...
		ОписаниеОшибки = "Значение не является целым числом от "+1+" до 12 (номер месяца)";
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGUKTZED")
		...
		ОписаниеОшибки = "Значение не является набором цифр (4, 6, 8или 10-и значным)";
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGspecNom") 
		...
		ОписаниеОшибки = "Значение не является одним из значений: 2,3 или 4 или 7";
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGPNtypr") Тогда
		...
		ОписаниеОшибки = "Значением может быть только одно из 01...17 или 20";		
	...
	ИначеЕсли (ВРег(Тип) = ВРег("DGJ02095Ind5"))
		...
		ОписаниеОшибки =  "Возможно, указан не верный код раздела Декларации по Акцизному налогу";
	...
	Иначе
		...
		ОписаниеОшибки = "Не поддерживаемый XML-тип. Тип значения (" + Тип + ") не может быть преобразован автоматически";
...
Функция ПолучитьСхемуОтчета(ИмяСхемы)
	...
	Если СтруктураСхем.Свойство(ИмяСхемы, Макет) Тогда
		...
		Документы.ВыгрузкаРегламентированныхОтчетов.ПолучитьМакет("common_types").Записать(ВременныйФайл.ИспользуемоеИмяФайла);
		...
		Исключение
			...
			Сообщить("Ошибка в схеме отчета (" + ИмяСхемы + ".XSD)! Выгрузка невозможна!
					 | " + ОписаниеОшибки());
			Возврат СхемаОтчета;		 
		КонецПопытки;
...
Процедура СформироватьТекстыВыгрузкиКлиент(ПервыйДок = Неопределено)
	...
	Для каждого Стр Из Объект.Выгрузки Цикл
		Если НЕ Стр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиГНСУ_XML Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СформироватьТаблицуВыгрузкиНаСервере(АдресВХранилище)
	...
	Исключение
		Сообщить("Данный отчет не поддерживает режим выгрузки!");	
	КонецПопытки
...
Процедура ЗаполнитьИнформациюОбОтчете()
	...
	Если Объект.Основание.Пустая() Тогда
		Элементы.ИнформацияОбОтчете.Заголовок = "Информация об отчете: <отчет не выбран>";
	Иначе	
...
Процедура ЗаполнитьИерархию()
	...
	Для каждого Строка Из ОБъект.Выгрузки Цикл
		...
		Если Позиция_ > 0 Тогда
			...
			// для условного оформления все нечентые индексы делаем отрицательными
