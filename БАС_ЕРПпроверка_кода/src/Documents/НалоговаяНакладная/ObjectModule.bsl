...
// Для табличной части "ТоварыПоДаннымПользователя"
	...
	КонецЕсли;
	ЗаполнитьСуммыРеглВТабличныхЧастях(ВидОперации);
...
Процедура ЗаполнитьДокументНаОснованииЧекаККМ(Знач ДокументОснование, Знач ВидОперации)
	...
	|				И Товары.СтавкаНДС В (ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20), ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7), ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0))
	...
	Для каждого ТекущаяСтрока Из Товары Цикл
		Документы.НалоговаяНакладная.ЗаполнитьСтатьюДекларацииНДСНалоговыеОбязательства(ВидОперации, Контрагент, Договор, ТекущаяСтрока.СтатьяДекларацииНДСНалоговыеОбязательства, ТекущаяСтрока.СтавкаНДС);
...
Процедура ЗаполнитьСуммыРеглВТабличныхЧастях(ВидОперации)
	
	Если ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.ПродажаНижеОбычнойЦены Тогда
		ЗаполнитьСуммыРеглВТабличнойЧастиПродажаНижеОбычнойЦены();
	Иначе
...
Процедура ЗаполнитьСуммыРеглВТабличнойЧастиТовары(ТабличнаяЧасть)	
	...
	СтруктураКурсовВалюты = РаботаСКурсамиВалют.ПолучитьКурсВалюты(Валюта, Дата);
...
Процедура ЗаполнитьСуммыРеглВТабличнойЧастиПродажаНижеОбычнойЦены()
	...
	Для Каждого СтрокаТовары Из ТабличнаяЧасть Цикл	
		...
		СуммаБезНДСГрн = НДСОбщегоНазначенияКлиентСервер.ПересчитатьИзВалютыВВалюту(
					СтрокаТовары.СуммаПревышения,
					Валюта, ВалютаРегУчета,
					СтруктураКурсовВалюты.Курс, 1,
					СтруктураКурсовВалюты.Кратность, 1);
					
		СуммаНДСГрн  = НДСОбщегоНазначенияКлиентСервер.ПересчитатьИзВалютыВВалюту(
					СтрокаТовары.СуммаНДСПревышения,
					Валюта, ВалютаРегУчета,
					СтруктураКурсовВалюты.Курс, 1,
					СтруктураКурсовВалюты.Кратность, 1);
		...
		СтрокаТовары.СуммаБезНДСРегл = СуммаБезНДСГрн;
...
Процедура ЗаполнитьРеквизитыПослеАвтоформирования(ЭтоНовыйОбъект = Истина) Экспорт
	...
	Для Ин=1 по КоличествоСтрок Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТовары.СтатьяДекларацииНДСНалоговыеОбязательства) Тогда
			Документы.НалоговаяНакладная.ЗаполнитьСтатьюДекларацииНДСНалоговыеОбязательства(ВидОперации, Контрагент, Договор, СтрокаТовары.СтатьяДекларацииНДСНалоговыеОбязательства, СтрокаТовары.СтавкаНДС)
		КонецЕсли;
