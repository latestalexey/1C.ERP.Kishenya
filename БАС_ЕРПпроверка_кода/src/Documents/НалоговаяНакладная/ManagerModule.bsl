...
#Область ОбязательстваПоНДС // (Дт 643 :: Кт 641)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговые обязательства: налоговая накладная"" КАК Содержание
	|ИЗ
...
#Область ВычетыИзДоходовНаСуммуНДСПревышенияПриПродажеНижеОбычнойЦены // (Дт 704 :: Кт 641)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговые обязательства: продажа ниже ОЦ"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ОбязательстваПоНДСУсловнаяПродажа // (Дт 643 :: Кт 641)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговые обязательства: условная продажа"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ОбязательстваПоНДСУсловнаяПродажаНаЗатраты // (Дт 9X :: Кт 641)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговые обязательства: затраты"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстЗапросаТаблицаПрочиеРасходы(Запрос, ТекстыЗапроса, Регистры)
    ...
    |	(ДанныеДокумента.СуммаНДСРегл - ДанныеДокумента.СуммаНДСРеглПоРегиструУсловнаяПродажа) КАК СуммаРеглБезНДС,
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Налоговая накладная (альбомная ориентация)';uk='Податкова накладна (альбомна орієнтація)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
...
Функция СформироватьПечатнуюФормуНалоговаяНакладная(МассивОбъектов, ОбъектыПечати, ПараметрыВывода, ИмяМакета, ТолькоФормироватьСтруктуруПоказателейПечатнойФормы = Ложь)
	...
	|			ИНАЧЕ НалоговаяНакладнаяТовары.Упаковка.ЕдиницаИзмерения
	...
	|			ИНАЧЕ НалоговаяНакладнаяТоварыПользователя.Упаковка.ЕдиницаИзмерения
	|		КОНЕЦ КАК ЕдиницаИзмерения,
	...
	Пока ДанныеПечати.Следующий() цикл
		...
		Если НомерПоХронологииФормыНН >= 4 Тогда
			Если НомерПоХронологииФормыНН = 6 Тогда
...
//
Функция ОпределитьКолонкуТаблицы(ВРегВалюте, СтавкаНДС=Неопределено, Дата='00010101')
...
Процедура ЗаполнитьСтатьюДекларацииНДСНалоговыеОбязательства(ВидОперации, Контрагент, ДоговорКонтрагента, СтатьяДекларацииНДСНалоговыеОбязательства, СтавкаНДС) Экспорт
