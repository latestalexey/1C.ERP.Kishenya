		...
		Для каждого СтрокаПоСотруднику Из Сотрудники Цикл
			...
			Если ПроверяемыеРеквизиты.Найти("Сотрудники.ЛицевойСчет") <> Неопределено
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У сотрудника %1 длина номера лицевого счета менее 20 цифр.';uk='У співробітника %1 довжина номера особового рахунку менше 20 цифр.'"),
					СтрокаПоСотруднику.ФизическоеЛицо);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Объект.Сотрудники[%1].%2", СтрокаПоСотруднику.НомерСтроки-1, "ЛицевойСчет"),, Отказ);
			КонецЕсли;
		КонецЦикла;
