...
#Область ТекстВыручкаКомиссионноеВознаграждение // (Дт 36 :: Кт 70)
...
#Область ТекстНДСКомиссионноеВознаграждение // (Дт 70 :: Кт 6432)
...
#Область ТекстЗадолженностьПередКомитентом // (Дт 704 :: Кт 63)
...
#Область ТекстНДСВходящийОтПродаж // (Дт 644 :: Кт 704)
	ТекстНДСВходящийОтПродаж = "
	...
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	СУММА(ВЫБОР КОГДА (НЕ Строки.ЦелевоеНалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая)) ТОГДА
...
#Область ТекстУдержатьВознаграждениеАвансКомитенту // (Дт 37 Кт 36)
...
#Область ТекстУдержатьВознаграждениеОплатаДолгаКомитенту // (Дт 63 Кт 36)
...
#Область ТекстКорректировкаНДСПроданногоТовара // (Дт 6442 :: Кт 6441)
	...
	|
	|	СУММА(ВЫБОР
	|			КОГДА Строки.СтавкаНДСПродажи = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20)
	|				ТОГДА (ВЫРАЗИТЬ(Строки.СуммаПродажи / 6 КАК ЧИСЛО(15, 2))) - Строки.СуммаНДС
	|			КОГДА Строки.СтавкаНДСПродажи = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
	|				ТОГДА (ВЫРАЗИТЬ(Строки.СуммаПродажи * 7 / 107 КАК ЧИСЛО(15, 2))) - Строки.СуммаНДС
	|			КОГДА Строки.СтавкаНДСПродажи = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0)
	...
	|
	|ГДЕ
	|	  НЕ Строки.СтавкаНДСПродажи В (ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС), ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НеНДС)) 
...
#КонецОбласти 
	...
	Возврат ""
		...
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДСВходящийОтПродаж
