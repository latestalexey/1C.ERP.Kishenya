...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
		...
		ЗаполнитьПоЗаказуДавальца(
			ДанныеЗаполнения,
			ДанныеЗаполнения);
		
	//-- НЕ УТКА
	//++ НЕ УТ
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ВыбытиеДенежныхДокументов") Тогда
		
		Документы.ВыбытиеДенежныхДокументов.ЗаполнитьПоОснованию(ЭтотОбъект, ДанныеЗаполнения);
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ДоговорЗаймаСотруднику") Тогда
		...
		ДенежныеСредстваСервер.ЗаполнитьПоДоговоруЗаймаСотруднику(ДанныеЗаполнения, ДанныеЗаполнения, РасшифровкаПлатежа, Истина, "ПоступлениеБезналичныхДенежныхСредств");
	//-- НЕ УТ
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//++ НЕ УТ
	ПроверитьЗаполнениеПоВозвратуНеперечисленнойЗарплаты(МассивНепроверяемыхРеквизитов, Отказ);
	//-- НЕ УТ
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПогашениеЗаймаСотрудником Тогда
		Если ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплатыУТ") Тогда
			ОтражениеЗарплатыВФинансовомУчетеУП.ПроверитьЗаполнениеДокументаПогашенияЗайма(ЭтотОбъект, Отказ);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ПодотчетноеЛицо) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Поле ""Сотрудник"" не заполнено.';uk='Поле ""Співробітник"" не заповнено.'"), , "ПодотчетноеЛицо", "Объект", Отказ);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплатыУТ") Тогда
		...
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратНеперечисленныхДС Тогда
			
			ДенежныеСредстваСервер.ЗарегистрироватьВозвратПоВедомостям(ЭтотОбъект, ДополнительныеСвойства, Отказ);
			
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПогашениеЗаймаСотрудником Тогда
			
			Займы = РасшифровкаПлатежа.Выгрузить(, "ДоговорЗаймаСотруднику, Сумма");
			Займы.Свернуть("ДоговорЗаймаСотруднику", "Сумма");
			Для Каждого Строка Из Займы Цикл
				ЗаймыСотрудникам.ЗарегистрироватьПогашениеЗайма(Движения, Строка.ДоговорЗаймаСотруднику, Строка.Сумма, Дата, Организация, ПодотчетноеЛицо, Ссылка, Отказ);
			КонецЦикла;
...
//++ НЕ УТКА
Процедура ЗаполнитьПоЗаказуДавальца(Знач ДокументОснование, ДанныеЗаполнения)
...
Процедура СформироватьСписокЗависимыхЗаказов()
	...
	|	Документ.ЗаказДавальца КАК ЗаказДавальца
	|ГДЕ
	|	ЗаказДавальца.Ссылка В (ВЫБРАТЬ
	|			РасшифровкаПлатежа.Заказ
	|		ИЗ
	|			Документ.ПоступлениеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК РасшифровкаПлатежа
	|		ГДЕ
...
//++ НЕ УТ
Процедура ПроверитьЗаполнениеПоВозвратуНеперечисленнойЗарплаты(МассивНепроверяемыхРеквизитов, Отказ)
	
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплатыУТ") Тогда
		Возврат;
	КонецЕсли;
	
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратНеперечисленныхДС
		И (ДокументВыдачи = Неопределено Или СуммаДокумента <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументВыдачи, "СуммаДокумента")) Тогда
		
		Текст = НСтр("ru='Сумма возврата не соответствует платежному документу. Возврат должен осуществляться в точном соответствии с исходящим платежным поручением.';uk='Сума повернення не відповідає платіжному документу. Повернення повинні здійснюватися у точній відповідності з вихідним платіжним дорученням.'");
