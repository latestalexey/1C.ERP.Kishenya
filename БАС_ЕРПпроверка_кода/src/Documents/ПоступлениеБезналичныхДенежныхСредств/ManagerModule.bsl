...
#Область ТекстВозвратАвансаОтПоставщика // (Дт 31 :: Кт 63, 371)
	ТекстВозвратАвансаОтПоставщика = "
	|ВЫБРАТЬ //// Возврат аванса от поставщика (Дт 31 :: Кт 63, 371)
	|
...
#Область ТекстВозвратОплатыОтПоставщика // (Дт 31 :: Кт 63)
	ТекстВозвратОплатыОтПоставщика = "
	|ВЫБРАТЬ //// Возврат оплаты от поставщика (Дт 31 :: Кт 63)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
...
#Область ТекстПоступлениеАвансаОтКлиента // (Дт 31 :: Кт 681)
	ТекстПоступлениеАвансаОтКлиента = "
	|ВЫБРАТЬ //// Поступление аванса от клиента (Дт 31 :: Кт 681)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
...
#Область ТекстПоступлениеОплатыОтКлиента // (Дт 31 :: Кт 36)
	ТекстПоступлениеОплатыОтКлиента = "
	|ВЫБРАТЬ //// Поступление оплаты от клиента (Дт 31 :: Кт 36)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
...
#Область ПоступлениеАвансаОтДрОрганизации // (Дт 31 :: Кт 681)
	ТекстПоступлениеАвансаОтДрОрганизации = " 
	|ВЫБРАТЬ //// Поступление аванса от другой организации (Дт 31 :: Кт 681)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
...
#Область ПоступлениеОплатыОтДрОрганизации // (Дт 31 :: Кт 36)
	ТекстПоступлениеОплатыОтДрОрганизации = "
	|ВЫБРАТЬ //// Поступление оплаты от другой организации (Дт 31 :: Кт 36)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
...
#Область ВозвратАвансаОтДрОрганизации // (Дт 31 :: Кт 63, 371)
	ТекстВозвратАвансаОтДрОрганизации = "
	|ВЫБРАТЬ //// Возврат аванса от другой организации (Дт 31 :: Кт 63, 371)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
...
#Область ВозвратОплатыОтДрОрганизации // (Дт 31 :: Кт 63)
	ТекстВозвратОплатыОтДрОрганизации = "
	|ВЫБРАТЬ //// Возврат оплаты от другой организации (Дт 31 :: Кт 63)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
...
#Область ТекстВнутренняяПередачаДС // (Дт 31 :: Кт 683)
	ТекстВнутренняяПередачаДС = "
	|ВЫБРАТЬ // Внутренняя передача ДС(поступление) (Дт 31 :: Кт 683)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
...
#Область ТекстПоступлениеСДругогоСчетаКассы // (Дт 31 :: Кт 333, 334)
	...
	|	Операция.Валюта КАК ВалютаКт,
	|	ВЫБОР КОГДА Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеДенежныхСредствСДругогоСчета) ТОГДА
	|		Операция.БанковскийСчетОтправитель.Подразделение
	|	КОГДА Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзКассыНаРасчетныйСчет) ТОГДА
	|		Операция.КассаОтправитель.Подразделение
	|	КОНЕЦ КАК ПодразделениеКт,
	|	ВЫБОР КОГДА Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеДенежныхСредствСДругогоСчета) ТОГДА
	|		Операция.БанковскийСчетОтправитель.НаправлениеДеятельности
	|	КОГДА Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзКассыНаРасчетныйСчет) ТОГДА
	|		Операция.КассаОтправитель.НаправлениеДеятельности
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Поступление ДС с другого счета"" КАК Содержание
	|
	|ИЗ 
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстПоступлениеОплатыПоЭквайрингу // (Дт 31 :: Кт 3332)
...
#Область ТекстВозвратДепозита // (Дт 31 :: Кт 313, 314)
	ТекстВозвратДепозита = "
	|ВЫБРАТЬ //// Возврат с депозитного счета (Дт 31 :: Кт 313, 314)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
...
#Область ТекстПроцентыПоДепозиту // (Дт 31 :: Кт 36)
	ТекстПроцентыПоДепозиту = "
	|ВЫБРАТЬ //// Поступление начисленных процентов по депозиту (Дт 31 :: Кт 36)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
...
#Область ТекстКапитализацияПроцентовПоДепозиту // (Дт 31 :: Кт 313, 314)
	ТекстКапитализацияПроцентовПоДепозиту = "
	|ВЫБРАТЬ //// Поступление начисленных процентов по депозиту с капитализацией (Дт 31 :: Кт 313, 314)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
...
#Область ТекстВозвратПредоставленногоЗайма // (Дт 31 :: Кт 377)
	ТекстВозвратПредоставленногоЗайма = "
	|ВЫБРАТЬ //// Возврат предоставленного займа (Дт 31 :: Кт 377))
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
...
#Область ТекстПрочиеДоходы // (Дт 31 :: Кт 7X)
	ТекстПрочиеДоходы = "
...
#Область ТекстПрочиеПоступления // (Дт 31 :: Кт XX)
	ТекстПрочиеПоступления = "
	|ВЫБРАТЬ //// Прочие поступления ДС (Дт 31 :: Кт XX)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
	...
	|	Операция.Валюта КАК ВалютаКт,
	|	Строки.Подразделение КАК ПодразделениеКт,
	|	Строки.АналитикаАктивовПассивов КАК НаправлениеДеятельностиКт,
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеКт,
	|
...
#Область ТекстКонвертацияВалюты // (Дт 31 :: Кт 3333, 3342)
	...
	|	ИНАЧЕ
	|		(Операция.СуммаДокумента + Операция.СуммаКомиссии)/Операция.КурсКонвертации
	|	КОНЕЦ КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	0 КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
...
#Область ТекстКонвертацияВалютыПрочиеРасходы // (Дт 9X :: Кт 3333, 3342)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	ВЫБОР КОГДА Строки.СтатьяРасходов = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ЗатратыПриПокупкеИПродажеИностраннойВалюты) ТОГДА
	|		""Отрицательная разница (расходы) от покупки иностранной валюты"" 
	|	ИНАЧЕ
	|		""Комиссионные банка""
...
#Область ТекстПогашениеЗаймаСотрудником // (Дт 31 :: Кт 3773)
	ТекстПогашениеЗаймаСотрудником = "
	|ВЫБРАТЬ //// Погашение займа сотрудником (Дт 31 :: Кт 3773)
	|
	|	Операция.Ссылка КАК Ссылка,
...
#Область ТекстПроцентыКомиссияПоПредоставленномуЗайму // (Дт 31 :: Кт 36)
	ТекстПроцентыКомиссияПоПредоставленномуЗайму = "
	|ВЫБРАТЬ //// Поступление начисленных процентов и комиссии по предоставленному займу (Дт 31 :: Кт 36)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком ТОГДА
	|		Операция.Дата
...
#Область ТекстПоступлениеЗалогаЗаПереданнуюТару // (Дт 31 :: Кт 36)
...
#Область ТекстВозвратОтПодотчетника // (Дт 31 :: Кт 372) 
...
Функция ТекстЗапросаТаблицаПрочиеРасходы(Запрос, ТекстыЗапроса, Регистры)
	...
	|	0 КАК СуммаБезНДС,
	|	-ТаблицаКурсовыхРазниц.СуммаРегл КАК СуммаРегл,
	|	-ТаблицаКурсовыхРазниц.СуммаРегл КАК СуммаРеглБезНДС,
	|	0 КАК НДСРегл,
	...
	|	0 КАК СуммаБезНДС,
	|	ДанныеДокумента.СуммаКомиссии КАК СуммаРегл,
	|	ДанныеДокумента.СуммаКомиссии КАК СуммаРеглБезНДС,
	|	0 КАК НДСРегл,
	|	&НалоговоеНазначениеОрганизации КАК НалоговоеНазначение,
	|	0 КАК ПостояннаяРазница,
	|	0 КАК ВременнаяРазница,
	...
	|	0 КАК СуммаБезНДС,
	|	ДанныеДокумента.СуммаПенсионный КАК СуммаРегл,
	|	ДанныеДокумента.СуммаПенсионный КАК СуммаРеглБезНДС,
	|	0 КАК НДСРегл,
	|	&НалоговоеНазначениеОрганизации КАК НалоговоеНазначение,
	|	0 КАК ПостояннаяРазница,
	|	0 КАК ВременнаяРазница,
...
Функция ТекстЗапросаТаблицаКурсовыхРазниц(Запрос, ТекстыЗапроса)
	...
	|			ТОГДА ВЫРАЗИТЬ((ДанныеДокумента.СуммаДокумента + ДанныеДокумента.СуммаКомиссии) * &КоэффициентПересчетаВВалютуКонвертации КАК ЧИСЛО(15, 2))
	...
	|	ВЫБОР
	|		КОГДА НЕ(&ВалютаКонвертации = &ВалютаРегламентированногоУчета)
	|			ТОГДА ВЫРАЗИТЬ((ДанныеДокумента.СуммаДокумента + ДанныеДокумента.СуммаКомиссии) * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(15, 2))
	...
	|	ВЫБОР
	|		КОГДА НЕ(&ВалютаКонвертации = &ВалютаРегламентированногоУчета)
	|			ТОГДА ВЫРАЗИТЬ((ДанныеДокумента.СуммаДокумента + ДанныеДокумента.СуммаКомиссии) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15, 2))
...
Функция ТекстЗапросаТаблицаВедомостей(Запрос, ТекстыЗапроса, Регистры)
	...
	|ИЗ
	|	Документ.СписаниеБезналичныхДенежныхСредств.ВедомостиНаВыплатуЗарплаты КАК Списание
	|ГДЕ
...
Функция ТекстЗапросаТаблицаРасчетыПоДоговорамКредитовИДепозитов(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА ДанныеДоговора.СтатьяДДСПоступленияВыдачи = ТаблицаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств
	|				ИЛИ ДанныеДоговора.СтатьяДДСОсновногоДолга = ТаблицаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыСуммГрафикаКредитовИДепозитов.ОсновнойДолг)
	|		КОГДА ДанныеДоговора.СтатьяДДСПроцентов = ТаблицаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыСуммГрафикаКредитовИДепозитов.Проценты)
	|		КОГДА ДанныеДоговора.СтатьяДДСКомиссии = ТаблицаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств
