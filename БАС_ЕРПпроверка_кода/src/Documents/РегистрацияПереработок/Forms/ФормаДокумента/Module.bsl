...
Процедура ЗаполнитьПериодСуммированногоУчетаПоПрошлымДокументам()
	ЗаполнитьЗначенияСвойств(Объект, УчетРабочегоВремени.ПериодСуммированногоУчетаПоПрошлымДокументам(Объект.Организация, Объект.Дата));
КонецПроцедуры
...
Процедура РассчитатьСотрудников(МассивСотрудников, Очищать = Ложь)
	...
	Если Очищать Тогда
		...
		Для каждого Сотрудник Из ТаблицаСотрудников Цикл
			Разница = КоличествоЧасовПереработки(Сотрудник);
...
Процедура ДополнитьСверхурочные(ТекущиеДанные)
	...
	Если Разница > 0 Тогда
		ТекущиеДанные.Сверхурочно2 = ТекущиеДанные.Сверхурочно2 + Разница;
	Иначе
...
&НаКлиентеНаСервереБезКонтекста
Функция КоличествоЧасовПереработки(ТекущиеДанные)
	Возврат ТекущиеДанные.ОтработаноЧасов - ТекущиеДанные.НормаЧасов - ТекущиеДанные.ОтработаноЧасовВПраздники;
КонецФункции
