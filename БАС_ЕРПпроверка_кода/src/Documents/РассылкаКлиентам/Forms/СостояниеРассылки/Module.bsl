...
Процедура СформироватьОтчет()
	...
	Иначе
		...
		|	ЕСТЬNULL(СообщениеSMS.Ссылка, НЕОПРЕДЕЛЕНО) КАК Взаимодействие,
		|	ЕСТЬNULL(СообщениеSMSАдресаты.СостояниеСообщения, НЕОПРЕДЕЛЕНО) КАК Состояние,
		|	ЕСТЬNULL(СообщениеSMS.Дата, НЕОПРЕДЕЛЕНО) КАК ДатаДокумента,
		|	ЕСТЬNULL(СообщениеSMS.Номер, НЕОПРЕДЕЛЕНО) КАК НомерДокумента
		|ИЗ
		|	Адресаты КАК АдресатыРассылки
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СообщениеSMS.Адресаты КАК СообщениеSMSАдресаты
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.СообщениеSMS КАК СообщениеSMS
		|			ПО СообщениеSMSАдресаты.Ссылка = СообщениеSMS.Ссылка
		|				И (СообщениеSMS.ВзаимодействиеОснование = &РассылкаКлиентам)
