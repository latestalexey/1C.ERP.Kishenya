...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(Распоряжение) Тогда
			...
			//++ НЕ УТ
				Или СкладскаяОперация = Перечисления.СкладскиеОперации.ПриемкаПродукцииИзПроизводства
			//-- НЕ УТ
				Тогда
	...
	ИначеЕсли Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(ДанныеЗаполнения))
		...
		Иначе
			...
			Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПоставщику")
				...
				//++ НЕ УТ
				Или ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ВозвратМатериаловИзПроизводства")
				Или ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ВыпускПродукции")
				Или ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПереработчику")
				Или ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПоступлениеОтПереработчика")
				Или ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ВозвратСырьяОтПереработчика")
				//-- НЕ УТ
				//++ НЕ УТКА
				Или ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПоступлениеСырьяОтДавальца")
				//-- НЕ УТКА
			...
			ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
				...
				РеквизитСклада = "СкладПоступления";
		...
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПереработчику") Тогда
			ТЧсТоварами = "Продукция";
			//-- НЕ УТ
			//++ НЕ УТКА
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.МаршрутныйЛистПроизводства") Тогда
			ТЧсТоварами = "ВыходныеИзделия";
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
			ТЧсТоварами = "Продукция";
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
			ТЧсТоварами = "Материалы";
			//-- НЕ УТКА
		КонецЕсли;
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПоставщику")
			...
			//-- НЕ УТ
			//++ НЕ УТКА
			Или ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказНаПроизводство")
			Или ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.МаршрутныйЛистПроизводства")
		...
		//++ НЕ УТКА
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПоступлениеСырьяОтДавальца") Тогда
		
			ИсточникНазначение = "ТаблицаТовары.Ссылка.ЗаказДавальца.Назначение";
		...
		//++ НЕ УТ
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПоступлениеОтПереработчика") Тогда
			
			СтруктураПараметров = Новый Структура();
			СтруктураПараметров.Вставить("ЗаказПереработчику"); 
			СтруктураПараметров.Вставить("РаспоряжениеЗаказаПереработчику", "ЗаказПереработчику.Распоряжение");
			СтруктураПараметров.Вставить("ПереработкаПоЗаказу", "ЕСТЬNULL(ЗаказПереработчику.ПереработкаПоЗаказу, ЛОЖЬ)");
			СтруктураПараметров.Вставить("ПоступлениеПоЗаказам");
			СтруктураПараметров = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, СтруктураПараметров);
			
			Если СтруктураПараметров.ПереработкаПоЗаказу Тогда
				Заказ = СтруктураПараметров.РаспоряжениеЗаказаПереработчику;
			Иначе
				Заказ = СтруктураПараметров.ЗаказПереработчику;
			КонецЕсли;
			ПриемПоЗаказам = СтруктураПараметров.ПоступлениеПоЗаказам;
		...
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПереработчику") Тогда
			...
			СтруктураПараметров = Новый Структура();
			СтруктураПараметров.Вставить("Распоряжение", "Распоряжение");
			СтруктураПараметров.Вставить("ПереработкаПоЗаказу", "ПереработкаПоЗаказу");
			СтруктураПараметров = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, СтруктураПараметров);
			
			Заказ = СтруктураПараметров.Распоряжение;
			ПриемПоЗаказам = СтруктураПараметров.ПереработкаПоЗаказу;
		
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ВозвратСырьяОтПереработчика") Тогда
		...
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ВыпускПродукции") Тогда
			...
			СтруктураПараметров = Новый Структура();
			СтруктураПараметров.Вставить("Распоряжение", "Распоряжение.Распоряжение"); 
			СтруктураПараметров.Вставить("ВыпускПоРаспоряжениям");
			СтруктураПараметров = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, СтруктураПараметров);
			
			Заказ = СтруктураПараметров.Распоряжение;
			ПриемПоЗаказам = СтруктураПараметров.ВыпускПоРаспоряжениям;
		
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПоступлениеСырьяОтДавальца") Тогда
			
			СтруктураПараметров = Новый Структура();
			СтруктураПараметров.Вставить("ЗаказДавальца"); 
			СтруктураПараметров.Вставить("ПоступлениеПоЗаказам");
			СтруктураПараметров = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, СтруктураПараметров);
			
			Заказ = СтруктураПараметров.ЗаказДавальца;
			ПриемПоЗаказам = СтруктураПараметров.ПоступлениеПоЗаказам;
			
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.МаршрутныйЛистПроизводства") Тогда
			
			СтруктураПараметров = Новый Структура();
			СтруктураПараметров.Вставить("Распоряжение");
			СтруктураПараметров = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, СтруктураПараметров);
			Заказ = СтруктураПараметров.Распоряжение;
			ПриемПоЗаказам = ЗначениеЗаполнено(СтруктураПараметров.Распоряжение);
			РеквизитКоличество = "КоличествоФакт";
		...
		//++ НЕ УТ
		ИначеЕсли ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ЗаказПереработчику") Тогда
			СтруктураПараметров = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Распоряжение, "Статус");
			
			Если СтруктураПараметров.Статус <> Перечисления.СтатусыЗаказовПереработчикам.КИсполнению
				И СтруктураПараметров.Статус <> Перечисления.СтатусыЗаказовПереработчикам.Закрыт Тогда
				СтрокаИсключения = НСтр("ru='Распоряжение ""%1"" находится в статусе ""%2"".
                |Ввод на основании разрешен только в статусах ""К исполнению"" и ""Закрыт"".'
                |;uk='Розпорядження ""%1"" знаходиться в статусі ""%2"".
                |Введення на підставі дозволене тільки в статусах ""До виконання"" і ""Закрито"".'");
			КонецЕсли;
		//-- НЕ УТ
		//++ НЕ УТКА
		ИначеЕсли ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
			СтруктураПараметров = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Распоряжение, "Статус");
			
			Если СтруктураПараметров.Статус = Перечисления.СтатусыЗаказовДавальцев.НеСогласован Тогда
				СтрокаИсключения = НСтр("ru='Распоряжение ""%1"" находится в статусе ""%2"".
                |Ввод на основании разрешен только в статусах ""Согласован"", ""К производству"", ""К отгрузке"" и ""Закрыт"".'
                |;uk='Розпорядження ""%1"" знаходиться в статусі ""%2"".
                |Введення на підставі дозволене тільки в статусах ""Погоджено"", ""До виробництва"", ""До відвантаження"" і ""Закрито"".'");
			КонецЕсли;
...
Процедура СформироватьСписокЗависимыхЗаказов()
	...
	//++ НЕ УТКА
	|			ИЛИ ПриходныйОрдерНаТоварыТовары.Назначение.Заказ ССЫЛКА Документ.ЗаказДавальца
	//-- НЕ УТКА
