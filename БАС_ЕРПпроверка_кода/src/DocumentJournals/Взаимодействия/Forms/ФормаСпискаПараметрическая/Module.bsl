...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТолькоПочта Тогда
		ЗаголовокУчастникиПочта =  НСтр("ru='Кому, от';uk='Кому, від'");
...
Процедура ДеревоВзаимодействийПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ДеревоВзаимодействий.ВыделенныеСтроки.Количество() > 0 Тогда
		...
		Иначе
			ТекстВопроса = НСтр("ru='Пометить выделенные строки на удаление?';uk='Позначити виділені рядки на вилучення?'");
		КонецЕсли;
	...
	Если Копирование Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			Если ТипЗнч(ТекущиеДанные.Ссылка) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") 
				...
				ПоказатьПредупреждение(, НСтр("ru='Копирование электронных писем запрещено';uk='Копіювання електронних листів заборонено'"));
...
Процедура ОтложитьРассмотрениеВыполнить(Команда)
	...
	ПоказатьВводДаты(ОбработчикОповещенияОЗакрытии, ДатаОтработки, НСтр("ru='Отработать после';uk='Відпрацювати після'"));
...
Процедура УправлениеСтраницамиСервер()
	...
	Если ВВидеДерева Тогда
		...
		Команды.ПереключитьРежимПросмотра.Подсказка = НСтр("ru='Установить режим просмотра в виде списка';uk='Встановити режим перегляду у вигляді списку'");
		Элементы.СтраницыСписокДерево.ТекущаяСтраница = Элементы.СтраницаДерево;
		ЗаполнитьДеревоВзаимодействий();
	Иначе
		Элементы.Список.Период = Интервал;
		Команды.ПереключитьРежимПросмотра.Подсказка = НСтр("ru='Установить режим просмотра в виде дерева';uk='Встановити режим перегляду у вигляді дерева'");
