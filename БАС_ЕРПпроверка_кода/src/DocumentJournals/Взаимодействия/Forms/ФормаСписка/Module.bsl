...
Процедура ПапкиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если ТекущиеДанные.ЕстьПравоНаИзменение = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Недостаточно прав для создания папки.';uk='Недостатньо прав для створення папки.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПапкиПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = НСтр("ru='Удалить папку ""';uk='Видалити папку ""'") + Строка(ТекущиеДанные.Значение) 
	+ НСтр("ru='"" и переместить все ее содержимое в папку ""Удаленные""';uk='"" і перемістити весь її вміст у папку ""Вилучені""'");
...
Процедура Ответить(Команда)
	...
	Если ВыборКорректен(Элементы.Список.Имя, Истина) Тогда
		...
		Если ТипЗнч(ТекущееВзаимодействие) <> Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
			ПоказатьПредупреждение(, НСтр("ru='Команда ""Ответить"" может быть выполнена только для входящего письма.';uk='Команда ""Відповісти"" може бути виконана тільки для вхідного листа.'"));
			Возврат;
		КонецЕсли;
	Иначе
		Возврат;
...
Процедура ОтветитьВсем(Команда)
	...
	Если ВыборКорректен(Элементы.Список.Имя, Истина) Тогда
		ТекущееВзаимодействие = Элементы.Список.ТекущиеДанные.Ссылка;
		Если ТипЗнч(ТекущееВзаимодействие) <> Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
			ПоказатьПредупреждение(, НСтр("ru='Команда ""Ответить всем"" может быть выполнена только для входящего письма.';uk='Команда ""Відповісти всім"" може бути виконана тільки для вхідного листа.'"));
			Возврат;
		КонецЕсли;
	Иначе
		Возврат;
...
Процедура Переслать(Команда)
	...
	Если ВыборКорректен(Элементы.Список.Имя, Истина) Тогда
		...
		Если ТипЗнч(ТекущееВзаимодействие) <> Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") 
			...
			ПоказатьПредупреждение(, НСтр("ru='Команда ""Переслать"" может быть выполнена только для писем.';uk='Команда ""Переслати"" може бути виконана тільки для листів.'"));
			Возврат;
		КонецЕсли;
	Иначе
		Возврат;
...
Процедура ДобавитьВЗакладки(Команда)
	...
	ИмяТекущегоЭлемента = ТекущийЭлемент.Имя;
	Если СтрНачинаетсяС(ИмяТекущегоЭлемента, "Список") И Не ВыборКорректен(ИмяТекущегоЭлемента) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не выбран элемент для добавления в закладки.';uk='Не вибраний елемент для додавання в закладки.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не Результат.ЭлементДобавлен Тогда
		...
		Возврат;
	КонецЕсли;
	ПоказатьОповещениеПользователя(НСтр("ru='Добавлены в закладки:';uk='Додані в закладки:'"),
...
Процедура ОтложитьРассмотрениеВыполнить(Команда)
	...
	ПоказатьВводДаты(ОбработчикОповещенияОЗакрытии, ДатаОтработки, НСтр("ru='Отработать после';uk='Відпрацювати після'"), ЧастиДаты.ДатаВремя);
...
Процедура ПеренестиВПапку(Команда)
	...
	Если ИмяТекущегоЭлемента = "Папки" Тогда
		Если ТипЗнч(ПапкиТекущиеДанные.Значение) = Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты") 
			...
			ПоказатьПредупреждение(, НСтр("ru='Команда не может быть выполнена для данного объекта';uk='Команда не може бути виконана для даного об''єкта'"));
			Возврат;
		ИначеЕсли ПапкиТекущиеДанные.ЕстьПравоНаИзменение = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Недостаточно прав для изменения папок.';uk='Недостатньо прав для зміни папок.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииСтатусаСервер(ОбновлятьПанельНавигации)
	...
	ШаблонЗаголовка = НСтр("ru='Статус: %1';uk='Статус: %1'");
...
Процедура ИзменитьОтборСписок(ИмяТаблицы, ДанныеДляОбработки);
	...
	Если ДанныеДляОбработки.Значение = НСтр("ru='Все';uk='Всі'") Тогда
	...
	Если ИмяТаблицы = "Предметы" Тогда
		...
		ИмяОтбора = НСтр("ru='Предмет';uk='Предмет'");
	...
	ИначеЕсли ИмяТаблицы = "Папки" Тогда
			...
			Если ТипЗнч(ДанныеДляОбработки.Значение) = Тип("СправочникСсылка.ПапкиЭлектронныхПисем") Тогда
				...
				ВидСравненияЭлементаОтбора = ВидСравненияКомпоновкиДанных.Равно;
				ПравоеЗначение             = ДанныеДляОбработки.Значение;
				ИмяОтбора = НСтр("ru='Папка';uk='Папка'");
			...
			Иначе
				...
				ВидСравненияЭлементаОтбора = ВидСравненияКомпоновкиДанных.Равно;
				ПравоеЗначение             = ДанныеДляОбработки.Значение;
				ИмяОтбора = НСтр("ru='Учетная запись';uk='Обліковий запис'");
	...
	ИначеЕсли ИмяТаблицы = "Контакты" Тогда
		...
		ВидСравненияЭлементаОтбора = ВидСравненияКомпоновкиДанных.Равно;
		ПравоеЗначение             = ДанныеДляОбработки.Значение;
		ИмяОтбора = НСтр("ru='Контакт';uk='Контакт'");
		ЗначениеОтбора = ДанныеДляОбработки.Значение;
	...
	ИначеЕсли ИмяТаблицы = "Свойства" Тогда
		...
		Если ТипЗнч(ДанныеДляОбработки.Значение) = Тип("Строка") 
			И ДанныеДляОбработки.Значение = НСтр("ru='Не указан';uk='Не зазначений'") Тогда
			
			ВидСравненияЭлементаОтбора = ВидСравненияКомпоновкиДанных.НеЗаполнено;
			ПравоеЗначение             = "";
			ЗначениеОтбора = НСтр("ru='Не указан';uk='Не зазначений'");
	...
	ИначеЕсли ИмяТаблицы = "Категории" Тогда
		...
		ИмяОтбора      = НСтр("ru='Категория';uk='Категорія'");
	...
	ИначеЕсли ИмяТаблицы = "Закладки" Тогда
		...
		ЗаголовокПанелиНавигации   = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, НСтр("ru='Закладка';uk='Закладка'"), ДанныеДляОбработки.Значение);
...
Процедура УправлениеВидимостьюПриПереключенииПанелиНавигации()
	...
	Если ПанельНавигацииСкрыта Тогда
		...
		КомандаИзменитьОтображениеПанелиНавигации.Подсказка = НСтр("ru='Показать панель навигации';uk='Показати панель навігації'");
		КомандаИзменитьОтображениеПанелиНавигации.Заголовок = НСтр("ru='Показать панель навигации';uk='Показати панель навігації'");
	Иначе
		Элементы.ИзменитьОтображениеПанелиНавигации.Картинка = БиблиотекаКартинок.СтрелкаВлево;
		КомандаИзменитьОтображениеПанелиНавигации.Подсказка = НСтр("ru='Скрыть панель навигации';uk='Сховати панель навігації'");
		КомандаИзменитьОтображениеПанелиНавигации.Заголовок = НСтр("ru='Скрыть панель навигации';uk='Сховати панель навігації'");
	КонецЕсли;
...
Процедура УстановитьЗаголовокВидаПанелиНавигации(ЗначениеОтбора = Неопределено)
	...
	Иначе
		...
		ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаЗакладки Тогда
			...
			Элементы.ВыборВариантаНавигации.Заголовок = НСтр("ru='По закладкам';uk='По закладках'");
			Элементы.УстановитьВариантНавигацииПоЗакладкам.Пометка = Истина;
			
		ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаПредмет Тогда
			
			Элементы.ВыборВариантаНавигации.Заголовок = НСтр("ru='По предметам';uk='По предметах'");
		...
		ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаКонтакт Тогда
			...
			Элементы.ВыборВариантаНавигации.Заголовок = НСтр("ru='По контактам';uk='По контактах'");
			Элементы.УстановитьВариантНавигацииПоКонтакту.Пометка = Истина;
			
		ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаПапки Тогда
			
			Элементы.ВыборВариантаНавигации.Заголовок = НСтр("ru='По папкам';uk='По папках'");
			Элементы.УстановитьВариантНавигацииПоПапкам.Пометка = Истина;
			
		ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаКатегории Тогда
			
			Элементы.ВыборВариантаНавигации.Заголовок = НСтр("ru='По категориям';uk='По категоріях'");
			Элементы.ДопКнопкаВыбораВариантаНавигацииКатегории.Пометка = Истина;
			
		КонецЕсли;
		
		Элементы.ВыборВариантаНавигации.Подсказка = НСтр("ru='Выберите варианта навигации';uk='Виберіть варіанти навігації'");
...
Функция ДобавитьСтрокуВсе(ДанныеФормыКоллекция, НомерКартинки = 0)
	...
	КонецЕсли;
		...
		НоваяСтрока.Значение = НСтр("ru='Все';uk='Всі'");
		НоваяСтрока.Представление = НСтр("ru='Все';uk='Всі'");
	...
	Для Каждого Строка Из РодительскаяСтрока.Строки Цикл
		...
		Если ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Строка.Значение) Тогда
			...
			НоваяСтрока.Представление = ?(ПустаяСтрока(СтрокаДетали.Тема), 
				НСтр("ru='Тема не указана';uk='Тема не вказана'"), СтрокаДетали.Тема) + " " + НСтр("ru='от';uk='від'") + " " 
...
Процедура ПослеЗаполненияПанелиНавигации(УстанавливатьПризнакНеОтрабатыватьАктивизациюПанелиНавигации = Истина)
	...
	ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаСвойства Тогда
		...
		Если ТекущиеДанные = Неопределено Тогда
			Элементы.Свойства.ТекущаяСтрока = НайтиСтрокуВДанныхФормыДерево(Свойства,НСтр("ru='Все';uk='Всі'"),"Значение",Ложь);
			ТекущиеДанные = Свойства.НайтиПоИдентификатору(Элементы.Свойства.ТекущаяСтрока);
		КонецЕсли;
	...
	ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаКатегории Тогда
		...
		Если ТекущиеДанные = Неопределено Тогда
			Элементы.Категории.ТекущаяСтрока = НайтиСтрокуВДанныхФормыКоллекция(Категории,НСтр("ru='Все';uk='Всі'"),"Значение");
			ТекущиеДанные = Категории.НайтиПоИдентификатору(Элементы.Категории.ТекущаяСтрока);
		КонецЕсли;
...
Процедура ДополнитьМенюПанелиНавигации()
	...
	Пока Выборка.Следующий() Цикл
		...
		ЭлементКнопкаПодменю.Заголовок = НСтр("ru='По';uk='По'") + " " + Выборка.Представление;
	...
	Если ТаблицаДопРеквизитовСвойствТипаБулево.Количество() > 0 Тогда
		...
		ЭлементКнопкаПодменю.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
		ЭлементКнопкаПодменю.ИмяКоманды = НоваяКоманда.Имя;
		ЭлементКнопкаПодменю.Заголовок = НСтр("ru='По категориям';uk='По категоріях'");
...
Функция ДобавитьВЗакладкиСервер(Знач ДанныеДляОбработки, ИмяЭлементаФормы)
	...
	Если СтрНачинаетсяС(ИмяЭлементаФормы, "Список") Тогда
		...
		Если СписокВзаимодействий.Количество() = 0 Тогда
			Результат.ТекстСообщенияОшибка = НСтр("ru='Не выбран элемент для добавления в закладки.';uk='Не вибраний елемент для додавання в закладки.'");
			Возврат Результат;
		КонецЕсли;
		...
		НаименованиеЗакладки = ?(ТолькоПочта, НСтр("ru='Избранные письма';uk='Обрані листи'"), НСтр("ru='Избранные взаимодействия';uk='Обрані взаємодії'"));
		Если СписокВзаимодействий.Количество() > 1 Тогда
			Текст = ?(ТолькоПочта, НСтр("ru='Выбранные письма (%1)';uk='Вибрані листи (%1)'"), НСтр("ru='Выбранные взаимодействия (%1)';uk='Вибрані взаємодії (%1)'"));
	...
	Иначе
		...
		Если ДанныеДляОбработки.Значение = НСтр("ru='Все';uk='Всі'") Тогда
			Результат.ТекстСообщенияОшибка = НСтр("ru='Закладку без отбора создать нельзя.';uk='Закладку без відбору створити не можна.'");
			Возврат Результат;
		КонецЕсли;
		...
		Если ИмяЭлементаФормы = "ПредметыПанельНавигации" Тогда
			...
			Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеДляОбработки.Значение) Тогда
				...
				НаименованиеЗакладки       = НСтр("ru='Предмет';uk='Предмет'") + " = " + Строка(ДанныеДляОбработки.Значение); 
				Текст = ?(ТолькоПочта, НСтр("ru='Письма по предмету %1';uk='Листи з предмету %1'"), НСтр("ru='Взаимодействия по предмету %1';uk='Взаємодії за предметом %1'"));
			...
			ИначеЕсли ДанныеДляОбработки.Значение = НСтр("ru='Не указан';uk='Не зазначений'") Тогда
				
				НаименованиеЗакладки       = НСтр("ru='Предмет не указан';uk='Предмет не вказано'");
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Письма без предмета';uk='Листи без предмета'"), НСтр("ru='Взаимодействия без предмета';uk='Взаємодії без предмета'"));
				
			ИначеЕсли ДанныеДляОбработки.Значение = НСтр("ru='Прочие вопросы';uk='Інші питання'") Тогда
				
				НаименованиеЗакладки       = НСтр("ru='Прочие вопросы';uk='Інші питання'");
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Прочие письма';uk='Інші листи'"), НСтр("ru='Прочие взаимодействия';uk='Інші взаємодії'"));
				
			Иначе
				
				НаименованиеЗакладки       = НСтр("ru='Тип предмета';uk='Тип предмета'") + " " + Строка(ДанныеДляОбработки.ОписаниеТипов.Типы()[0]);
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Письма по: %1';uk='Листи по: %1'"), НСтр("ru='Взаимодействия по: %1';uk='Взаємодії за: %1'"));
		...
		ИначеЕсли ИмяЭлементаФормы = "Свойства" Тогда
			...
			Если ТипЗнч(ДанныеДляОбработки.Значение) = Тип("Строка") 
				И ДанныеДляОбработки.Значение = НСтр("ru='Не указан';uk='Не зазначений'") Тогда
				НаименованиеЗакладки       = ТекущееСвойствоПанелиНавигации.Наименование + " " + НСтр("ru='не указан';uk='не зазначений'");
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Письма';uk='Листи'"), НСтр("ru='Взаимодействия';uk='Взаємодії'"));
			Иначе
				...
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, 
				                                   НСтр("ru='Письма с заданным свойством: %1';uk='Листи з заданою властивістю: %1'"), 
				                                   НСтр("ru='Взаимодействия с заданным свойством: %1';uk='Взаємодії з заданою властивістю: %1'"));
		...
		ИначеЕсли ИмяЭлементаФормы = "Категории" Тогда
			...
			НаименованиеЗакладки       = НСтр("ru='Входит в категорию';uk='Входить у категорію'") + " " + Строка(ДанныеДляОбработки.Значение);
			Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Письма из категории: %1';uk='Листи з категорії: %1'"), НСтр("ru='Взаимодействия из категории: %1';uk='Взаємодії з категорії: %1'"));
			Результат.ПредставлениеЭлемента = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Результат.ПредставлениеЭлемента, ДанныеДляОбработки.Значение);
		...
		ИначеЕсли ИмяЭлементаФормы = "Контакты" Тогда
			...
			Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеДляОбработки.Значение) Тогда
				...
				НаименованиеЗакладки       = НСтр("ru='Контакт';uk='Контакт'") + " = " + Строка(ДанныеДляОбработки.Значение); 
				Текст = ?(ТолькоПочта, НСтр("ru='Письма по контакту %1';uk='Листи по контакту %1'"), НСтр("ru='Взаимодействия по контакту %1';uk='Взаємодії за контактом %1'"));
				Результат.ПредставлениеЭлемента = 
			...
			ИначеЕсли ДанныеДляОбработки.Значение = НСтр("ru='Контакт не подобран';uk='Контакт не підібраний'") Тогда
				
				НаименованиеЗакладки       = НСтр("ru='Контакт не указан';uk='Контакт не вказано'");
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Письма без подобранных контактов';uk='Листи без підібраних контактів'"), НСтр("ru='Взаимодействия без подобранных контактов';uk='Взаємодії без підібраних контактів'"));
				
			Иначе
				
				НаименованиеЗакладки       = НСтр("ru='Тип контакта';uk='Тип контакту'") + " " + Строка(ДанныеДляОбработки.ОписаниеТипов.Типы()[0]);
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Письма по контактам: %1';uk='Листи по контактах: %1'"), НСтр("ru='Взаимодействия по контактам: %1';uk='Взаємодії за контактами: %1'"));
				Результат.ПредставлениеЭлемента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Результат.ПредставлениеЭлемента, ДанныеДляОбработки.ОписаниеТипов.Типы()[0]);
		...
		ИначеЕсли ИмяЭлементаФормы = "Папки" Тогда
			...
			НаименованиеЗакладки       = НСтр("ru='В папке';uk='У папці'") + " " + Строка(ДанныеДляОбработки.Значение);
			Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru='Письма в папке: %1';uk='Листи в папці: %1'"), НСтр("ru='Взаимодействия в папке: %1';uk='Взаємодії в папці %1'"));
	...
	Пока Выборка.Следующий() Цикл
		Если ПолучитьXML(КомпоновщикНастроек.ПолучитьНастройки()) =  ПолучитьXML(Выборка.КомпоновщикНастроек.Получить()) Тогда
			Результат.ТекстСообщенияОшибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Закладка с такими настройками уже существует : %1';uk='Закладка з такими настройками вже існує : %1'"),
...
Процедура ВыполнитьПолнотекстовыйПоиск()
	...
	Если ТекстОшибки = Неопределено Тогда
		...
		ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Найдено %1 %2.';uk='Знайдено %1 %2.'"),
			?(ТолькоПочта,НСтр("ru='писем';uk='листів'"), НСтр("ru='взаимодействий';uk='взаємодій'")) + ": ",
		...
		КонецЕсли;
	Иначе
		Если НЕ ТекстОшибки = НСтр("ru='Ничего не найдено';uk='Нічого не знайдене'") Тогда
...
Функция НайтиВзаимодействияПолнотекстовыйПоиск(КоличествоЭлементов)
	...
	Если СписокПоиска.ПолноеКоличество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			...
			ВидСравненияКомпоновкиДанных.Равно,, Истина);
		Элементы.СтрокаПоиска.ЦветФона = ЦветаСтиля.ОшибкаПолнотекстовыйПоискФон;
		Возврат НСтр("ru='Ничего не найдено';uk='Нічого не знайдене'");
	КонецЕсли;
...
Процедура ЗаполнитьОписаниеНайденоПП(Взаимодействие)
	...
	Иначе
		...
		Если ВзаимодействияКлиентСервер.ЯвляетсяПрисоединеннымФайломВзаимодействий(СтрокаТаблицыСОписанием.Значение) Тогда
			ТекстНайдено = НСтр("ru='Найдено в присоединенном файле';uk='Знайдено у приєднаному файлі'");
		Иначе
			ТекстНайдено = НСтр("ru='Найдено в';uk='Знайдено у'");
		КонецЕсли;
