...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ДоступноНачислениеЗарплаты Тогда
		...
		МассивДокументовПоФизическомуЛицу = ЗарплатаКадры.ДокументыПоФизическомуЛицу(ФизическоеЛицоСсылка);
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ЭтаФорма.НачислениеЗарплаты, "МассивДокументов", МассивДокументовПоФизическомуЛицу, Истина);
	КонецЕсли;
	...
	Если ДоступноНачислениеЗарплаты Тогда
		ЗарплатаКадры.ПриСозданииНаСервереФормыСДинамическимСписком(ЭтотОбъект, "НачислениеЗарплаты");
	КонецЕсли; 
...
Процедура ПрочитатьНачисления()
	...
	Если ЗначениеЗаполнено(ДатаУвольнения) Тогда
		...
		Элементы.ДекорацияЗаголовокНачислений.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Начисления действовавшие по %1';uk='Нарахування, що діяли по %1'") + ":",
	...
	ЗарплатаКадрыРасширенный.РедактированиеСоставаНачисленийСотрудниковДействующиеНачисленияВРеквизит(Неопределено, ТаблицаСотрудников, ЭтаФорма, ОписаниеТаблицыВидовРасчета, , Ложь);
		...
		Для каждого СтрокаУдержаний Из Удержания Цикл
			...
			Если Не ЗарплатаКадрыПовтИсп.ПолучитьИнформациюОВидеРасчета(СтрокаУдержаний.Удержание).Рассчитывается Тогда
				...
				Если ДобавленыПоляПоказателей И ЗначениеЗаполнено(СтрокаУдержаний["ТекущееЗначение1"]) Тогда
					...
					СтрокаУдержаний["ПредставлениеПоказателя1"] = НСтр("ru='Фиксированная сумма';uk='Фіксована сума'");
					СтрокаУдержаний["Значение1"] = СтрокаУдержаний["ТекущееЗначение1"];
					СтрокаУдержаний["ТочностьПоказателя1"] = "ЧДЦ=2";
					
				Иначе
					
					СтрокаУдержаний["Представление"] = НСтр("ru='Фиксированная сумма';uk='Фіксована сума'");
			...
			Если СтрокиУдержаний.Количество() > 0 Тогда
				...
				Если СтрокаУдержаний <> Неопределено Тогда
					...
					Если Выборка.СпособРасчета = Перечисления.СпособыРасчетаУдержанияПоИсполнительномуДокументу.Долей Тогда
						ПредставлениеПоказателя = Формат(Выборка.Числитель, "ЧДЦ=") + " / " + Формат(Выборка.Знаменатель, "ЧДЦ=") + " " + НСтр("ru='доля заработка';uk='частка заробітку'");
						ЗначениеПоказателя = 0;
					ИначеЕсли Выборка.СпособРасчета = Перечисления.СпособыРасчетаУдержанияПоИсполнительномуДокументу.Процентом Тогда
						ПредставлениеПоказателя = НСтр("ru='% заработка';uk='% заробітку'");
						ЗначениеПоказателя = Выборка.Процент;
					Иначе
						ПредставлениеПоказателя = НСтр("ru='Фиксированная сумма';uk='Фіксована сума'");
...
Процедура ПоказатьРегистратор(ИмяЭлементаТабличнойЧасти)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ИмяЭлементаТабличнойЧасти = "Удержания" Тогда
			Регистратор = РегистраторУдержаний(ТекущиеДанные.Удержание, ФизическоеЛицоСсылка, ТекущаяОрганизация);
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция РегистраторУдержаний(Удержание, ФизическоеЛицо, Организация)
