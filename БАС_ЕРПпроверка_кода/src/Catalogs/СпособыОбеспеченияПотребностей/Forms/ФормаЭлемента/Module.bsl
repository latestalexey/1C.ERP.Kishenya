...
Процедура ПриЧтенииСозданииНаСервере()
	...
	//++ НЕ УТ
	НастроитьПараметрыВыбораПодразделенияНаСервере();
	//-- НЕ УТ
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	//++ НЕ УТКА
	Если НЕ ЗначениеЗаполнено(Объект.ИсточникОбеспеченияПотребностей)
		И Объект.ТипОбеспечения = Перечисления.ТипыОбеспечения.ПроизводствоНаСтороне 
		И ПереключательОдинПоставщик = 1 Тогда
		ТекстСообщения = НСтр("ru='Поле ""Переработчик"" не заполнено';uk='Поле ""Переробник"" не заповнено'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Объект.Ссылка, "Переработчик",, Отказ); 
	КонецЕсли; 
	//-- НЕ УТКА
...
Процедура ТипОбеспеченияПриИзменении(Элемент)
	...
	//++ НЕ УТ
	ТипОбеспеченияПриИзмененииНаСервере();
	//-- НЕ УТ
...
Процедура РежимИспользованияНесколькоПоставщиковПриИзменении(Элемент)
	...
	Объект.ВидЦеныПоставщика = ПредопределенноеЗначение("Справочник.ВидыЦенПоставщиков.ПустаяСсылка");
	//++ НЕ УТКА
	Переработчик = ПредопределенноеЗначение("Справочник.Партнеры.ПустаяСсылка");
	//-- НЕ УТКА
...
#Область ПрочиеПроцедурыИФункцииУП

&НаСервере
Процедура ТипОбеспеченияПриИзмененииНаСервере()

	// При производстве можно выбрать только Подразделение-диспетчер
	Если Объект.ТипОбеспечения = Перечисления.ТипыОбеспечения.Производство 
		И НЕ Объект.Подразделение.Пустая() Тогда
		
		ЭтоПодразделениеДиспетчер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Подразделение, "ПодразделениеДиспетчер");
		Если НЕ ЭтоПодразделениеДиспетчер Тогда
...
&НаСервере
Процедура НастроитьПараметрыВыбораПодразделенияНаСервере()

	ПараметрыВыбораПодразделения = Новый Массив;
	Если Объект.ТипОбеспечения = Перечисления.ТипыОбеспечения.Производство Тогда
		ПараметрыВыбораПодразделения.Добавить(Новый ПараметрВыбора("Отбор.ПодразделениеДиспетчер", Истина));
	КонецЕсли;
...
Процедура СформироватьЗаголовкиПоясняющихНадписей()
	...
	ИначеЕсли Объект.ТипОбеспечения = ПредопределенноеЗначение("Перечисление.ТипыОбеспечения.Производство") Тогда
		...
		НадписьДатаФормированияЗаказа = НСтр("ru='Дата формирования заказа на производство:';uk='Дата формування замовлення на виробництво:'");
		НадписьДатаБлижайшейПоставки = НСтр("ru='Дата выпуска продукции по заказу:';uk='Дата випуску продукції по замовленню:'");
		НадписьГраницаОбеспечиваемогоПериода = НСтр("ru='Обеспечиваемый период до:';uk='Забезпечуваний період до:'");

	ИначеЕсли Объект.ТипОбеспечения = ПредопределенноеЗначение("Перечисление.ТипыОбеспечения.ПроизводствоНаСтороне") Тогда

		НадписьДатаФормированияЗаказа = НСтр("ru='Дата формирования заказа переработчику:';uk='Дата формування замовлення переробнику:'");
		НадписьДатаБлижайшейПоставки = НСтр("ru='Дата поступления продукции по заказу:';uk='Дата надходження продукції по замовленню:'");
		НадписьГраницаОбеспечиваемогоПериода = НСтр("ru='Обеспечиваемый период до:';uk='Забезпечуваний період до:'");
	//-- НЕ УТ
	КонецЕсли;
...
Процедура АктивизироватьСтраницы(Элементы, ТипОбеспечения, ФормироватьПлановыеЗаказы, ОдинИсточник = Неопределено)
	...
	//++ НЕ УТ
	ИначеЕсли ТипОбеспечения = ПредопределенноеЗначение("Перечисление.ТипыОбеспечения.Производство") Тогда

		Элементы.СтраницыТипОбеспечения.ТекущаяСтраница  = Элементы.СтраницаПроизводство;
		Элементы.СтраницыСрокОбеспечения.ТекущаяСтраница = Элементы.СтраницаСрокПроизводства;
		
	ИначеЕсли ТипОбеспечения = ПредопределенноеЗначение("Перечисление.ТипыОбеспечения.ПроизводствоНаСтороне") Тогда

		Элементы.СтраницыТипОбеспечения.ТекущаяСтраница  = Элементы.СтраницаПроизводствоНаСтороне;
...
Функция ТекстОшибкиГарантированногоСрокаОтгрузки()
	...
	//++ НЕ УТ	
	ИначеЕсли Объект.ТипОбеспечения = ПредопределенноеЗначение("Перечисление.ТипыОбеспечения.Производство") Тогда
		Пояснение = НСтр("ru='Гарантированный срок отгрузки не может быть меньше срока производства. Гарантированный срок отгрузки увеличен.';uk='Гарантований строк відвантаження не може бути менше терміну виробництва. Гарантований термін відвантаження збільшений.'");
	ИначеЕсли Объект.ТипОбеспечения = ПредопределенноеЗначение("Перечисление.ТипыОбеспечения.ПроизводствоНаСтороне") Тогда
		Пояснение = НСтр("ru='Гарантированный срок отгрузки не может быть меньше срока переработки. Гарантированный срок отгрузки увеличен.';uk='Гарантований строк відвантаження не може бути менше строку переробки. Гарантований строк відвантаження збільшений.'");
	//-- НЕ УТ
	Иначе
		Пояснение = "";
	КонецЕсли;
...
Функция ЗаполнитьСписокВыбораТипаОбеспечения()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство") 
		...
		Представление = ?(ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне"), НСтр("ru='Собственное производство';uk='Власне виробництво'"), НСтр("ru='Производство';uk='Виробництво'"));
