...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтрокаДерева = ФинансоваяОтчетностьКлиентСервер.КорневойЭлемент(
														СтрокаДерева, 
														ВидЭлемента("ТаблицаСложная"));
	ФинансоваяОтчетностьКлиентСервер.УстановитьНовогоРодителя(СтрокаДерева, КопияДереваЭлементов, Истина, Ложь);
...
Процедура ПредставлениеВариантаИспользованияЯчейкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ВидПоказателей", ВидПоказателей);
...
Функция ПолучитьОбластьРедактирования(Форма)
	...
	Возврат Новый Структура("Верх, Лево, Низ, Право",
								4 + Форма.ВысотаШапки + 1, 	//первая строка после шапки
								3, 					 		//все строки во второй колонке, значит редактируем с третьей колонки
								4 + Форма.ВысотаМакета, 	//до последней строки
								2 + Форма.ШиринаМакета 	 	//до последней колонки
								);
...
Процедура УправлениеФормой()
	...
	СтрокаВычисление = НСтр("ru='Вычисление';uk='Обчислення'")+":";
	Элементы.Формула.Доступность = Истина;
	Формула = "";
	Если ЭтоСвязанный Тогда
		Формула = НСтр("ru='<расчет смотри в связанном элементе>';uk='<розрахунок дивись у зв''язаному елементі>'");
		
	ИначеЕсли ВидЭлемента = ВидЭлемента("ПроизводныйПоказатель") Тогда
		
		Элементы.НадписьИнфо.Заголовок = СтрокаВычисление;
	...
	ИначеЕсли ВидЭлемента = ВидЭлемента("МонетарныйПоказатель") Тогда
		...
		Если ЕстьНастройки = Истина Тогда
			Формула = Формула + " <" + НСтр("ru='установлены дополнительные отборы показателя';uk='встановлені додаткові відбори показника'") + ">";
		КонецЕсли;
...
Процедура ПоказатьСвойстваОбласти(Область)
	...
	Расшифровка = Область.Расшифровка;
	Если Расшифровка.ВидЭлемента = ВидЭлемента("Измерение") Тогда
		Возврат;
	КонецЕсли;
	Если ОбластьВЗонеРедактирования(ЭтаФорма, Область) Тогда
...
Процедура РедактироватьЭлементОтчета(Область)
	...
	Если Расшифровка.ВидЭлемента = ПредопределенноеЗначение("Перечисление.ВидыЭлементовФинансовогоОтчета.Группа")
		...
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Настройка свойств элемента доступна в виде отчета!';uk='Настройка властивостей елемента доступна у виді звіту!'"));
		Возврат;
	КонецЕсли;
...
Функция КраткоеИмяМонетарногоПоказателя(Счет, ТипИтога, НачальноеСальдо)
	...
	ГраницаПериода = ?(НачальноеСальдо,НСтр("ru='Начальное';uk='Початкове'"),НСтр("ru='Конечное';uk='Кінцеве'")) + " ";
	Если СтрНайти(ИмяИтога, "Оборот") > 0 Тогда
		ГраницаПериода = "";
	КонецЕсли;
	СтрокаИтога = НРег(Лев(ИмяИтога,1)) + Прав(ИмяИтога,СтрДлина(ИмяИтога)-1);
	Возврат ГраницаПериода + СтрокаИтога + " " + НСтр("ru='по счету';uk='по рахунку'") + " " + Строка(Счет);
