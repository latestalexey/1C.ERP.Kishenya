...
Функция ПолучитьРеквизитыОрганизации(Организация, Период = '00010101') Экспорт
	...
	|	УчетныеПолитики.ИнформацияОСтатусеПлательщикаНалогов КАК ИнформацияОСтатусеПлательщикаНалогов
	|ИЗ
	|	Справочник.Организации КАК Организации
...
Функция НалоговоеНазначениеНДС(Организация, Дата) Экспорт
	...
	Запрос = Новый Запрос("
	|ВЫБРАТЬ
	|	СпрУчетнаяПолитика.СистемаНалогообложения КАК СистемаНалогообложения
	|ИЗ
...
Функция ПлательщикНДС(Организация, Дата) Экспорт
	...
	|	) КАК УчетнаяПолитикаОрганизаций
	|ГДЕ
	|	УчетнаяПолитикаОрганизаций.УчетнаяПолитика.СистемаНалогообложения В (
	|		ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.НалогНаПрибыльИНДС),
	|		ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.ЕдиныйНалогИНДС)
	|	)
	|");
...
Функция ПлательщикЕдиногоНалога(Организация, Дата) Экспорт
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ПлательщикЕдиногоНалога = Выборка.Плательщик;
	КонецЕсли;
...
Функция ПлательщикНалогаНаПрибыль(Организация, Дата) Экспорт
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ПлательщикНалогаНаПрибыль = Выборка.Плательщик;
	КонецЕсли;
