...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ru='Не совместимы с текущей версией';uk='Не сумісні з поточною версією'");
		...
		Дело.ЕстьДела      = КоличествоНесовместимыхПрофилейГруппДоступа > 0;
		Дело.Важное        = Истина;
		Дело.Представление = НСтр("ru='Профили групп доступа';uk='Профілі груп доступу'");
		Дело.Количество    = КоличествоНесовместимыхПрофилейГруппДоступа;
		...
		Если МетаданныеРоли = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При проверке поставляемого профиля ""%1""
                           ...
                           |;uk='При перевірці профілю, що поставляє, ""%1""
                           |роль ""%2"" не знайдена в метаданних.'"),
...
Функция ПоставляемыеПрофили()
	...
	ЗаголовокОшибки =
		НСтр("ru='Заданы недопустимые значения в процедуре ПриЗаполненииПоставляемыхПрофилейГруппДоступа
                   ...
                   |
                   |'
                   |;uk='Задані неприпустимі значення у процедурі ПриЗаполненииПоставляемыхПрофилейГруппДоступа
                   |загального модуля УправлениеДоступомПереопределяемый.
                   |
                   |'");
	   ...
	   И НЕ ПараметрыОбновления.ОбновлятьИзмененныеПрофили Тогда
		...
		ВызватьИсключение ЗаголовокОшибки
			+ НСтр("ru='Когда в параметре ПараметрыОбновления свойство
                         ...
                         |;uk='Коли в параметрі ПараметрыОбновления властивість
                         |ОбновлятьИзмененныеПрофили встановлено Хибність,
                         |тоді властивість ЗапретитьИзменениеПрофилей теж
                         |повинна бути встановлена Хибність.'");
	КонецЕсли;
	...
	ОписаниеПрофиляАдминистратор.Наименование  = НСтр("ru='Администратор';uk='Адміністратор'");
	...
	ОписаниеПрофиляАдминистратор.Описание =
		НСтр("ru='Предназначен для:
                   ...
                   |'
                   |;uk='Призначений для:
                   |- настройки параметрів роботи й обслуговування інформаційної системи,
                   |- настройки прав доступу інших користувачів,
                   |- вилучення позначених об''єктів,
                   |- у рідких випадках для внесення змін у конфігурацію.
                   |
                   |Рекомендується не використовувати для ""звичайної"" роботи в інформаційній системі.
                   |'");
	...
	Для Каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
				   ...
				   И Не Метаданные.ОпределяемыеТипы.ВнешнийПользователь.Тип.СодержитТип(Тип) Тогда
					ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='В описании профиля ""%1""
                                   ...
                                   |;uk='В описі профілю ""%1""
                                   |зазначено недопустиме призначення ""%2 (%3)"".
                                   |Очікується призначення, як значення типу ""Тип"" для посилання,
                                   |зазначене в аналізованому типі Пользователь і
                                   |зазначене в аналізованому типі ВнешнийПользователь
                                   |(крім типу СправочникСсылка.Пользователи).'"),
			...
			Если ВсеРоли.Получить(Роль) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В описании профиля ""%1 (%2)""
                               ...
                               |;uk='В описі профілю ""%1 (%2)""
                               |роль ""%3"" не знайдена в метаданих.'"),
					ОписаниеПрофиля.Имя,
					ОписаниеПрофиля.Идентификатор,
					Роль);
			КонецЕсли;
			...
			Если НедоступныеРоли.Получить(Роль) <> Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В описании профиля ""%1 (%2)""
                               ...
                               |;uk='В описі профілю ""%1 (%2)""
                               |вказана роль ""%3"", 
                               |яка не відповідає призначенню профілю:
                               |""%4"".'"),
					ОписаниеПрофиля.Имя,
					ОписаниеПрофиля.Идентификатор,
					Роль,
		...
		Если СвойстваПрофилей.Получить(ОписаниеПрофиля.Идентификатор) <> Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Профиль с идентификатором ""%1"" уже существует.';uk='Профіль із ідентифікатором ""%1"" уже існує.'"),
				ОписаниеПрофиля.Идентификатор);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ОписаниеПрофиля.Имя) Тогда
			Если СвойстваПрофилей.Получить(ОписаниеПрофиля.Имя) <> Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Профиль с именем ""%1"" уже существует.';uk='Профіль із ім''ям ""%1"" уже існує.'"),
					ОписаниеПрофиля.Имя);
			КонецЕсли;
			...
			Если СвойстваВидовДоступа.ПоИменам.Получить(ИмяВидаДоступа) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В описании профиля ""%1""
                               ...
                               |;uk='В описі профілю ""%1""
                               |вказаний неіснуючий вид доступу ""%2"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
					  ОписаниеПрофиля.Идентификатор),
					ИмяВидаДоступа);
			...
			Если Не ВидДоступаСоответствуетНазначениюПрофиля Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В описании профиля ""%1""
                               ...
                               |""%3"".'
                               |;uk='В описі профілю ""%1""
                               |вказаний вид доступу ""%2""
                               |який не відповідає призначенню профілю:
                               |""%3"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
					  ОписаниеПрофиля.Идентификатор),
			   ...
			   И УточнениеВидаДоступа <> "ВначалеВсеРазрешены" Тогда
				...
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В описании профиля ""%1""
                               ...
                               |- ""Предустановленный"".'
                               |;uk='В описі профілю ""%1""
                               |для виду доступу ""%2"" вказано невідоме уточнення ""%3"".
                               |
                               |Допустимі тільки наступні уточнення:
                               |- ""ВначалеВсеЗапрещены"" або """",
                               |- ""ВначалеВсеРазрешены"",
                               |- ""Предустановленный"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
			...
			Если ОбъектМетаданных = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В описании профиля ""%1""
                               ...
                               |;uk='В описі профілю ""%1""
                               |для виду доступу ""%2""
                               |не існує тип вказаного значення доступу
                               |""%3"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
					  ОписаниеПрофиля.Идентификатор),
			...
			Если ПустаяСсылкаЗначенияДоступа = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В описании профиля ""%1""
                               ...
                               |;uk='В описі профілю ""%1""
                               |для виду доступу ""%2""
                               |зазначений не посилальний тип значення доступу
                               |""%3"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
					  ОписаниеПрофиля.Идентификатор),
			 ...
			 ИЛИ СвойстваВидаДоступаПоТипу.Имя <> ВидДоступа Тогда
				...
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В описании профиля ""%1""
                               ...
                               |;uk='В описі профілю ""%1""
                               |вказано значення доступу ""%3""
                               |типу, який не зазначений у властивостях виду доступу ""%2"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
					  ОписаниеПрофиля.Идентификатор),
					ВидДоступа,
			...
			Если ТаблицаЗначенийДоступа.НайтиСтроки(Отбор).Количество() > 0 Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В описании профиля ""%1""
                               ...
                               |;uk='В описі профілю ""%1""
                               |для виду доступу ""%2""
                               |повторно вказано значення доступу
                               |""%3"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
					  ОписаниеПрофиля.Идентификатор),
...
Функция ПредставлениеНазначенияПрофиля(НазначениеПрофиля)
	...
	Если НазначениеПрофиля = "СовместноДляПользователейИВнешнихПользователей" Тогда
		Возврат НСтр("ru='Совместно для пользователей и внешних пользователей';uk='Спільно для користувачів і зовнішніх користувачів'");
		
	ИначеЕсли НазначениеПрофиля = "ДляВнешнихПользователей" Тогда
		Возврат НСтр("ru='Для внешних пользователей';uk='Для зовнішніх користувачів'");
	КонецЕсли;
	
	Возврат НСтр("ru='Для пользователей';uk='Для користувачів'");
			...
			Если МетаданныеРоли = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При обновлении поставляемого профиля ""%1""
                               |роль ""%2"" не найдена в метаданных.'
                               |;uk='При оновленні профілю, що поставляє, ""%1""
                               |роль ""%2"" не знайдена в метаданих.'"),
					СвойстваПрофиля.Наименование,
					Роль);
			КонецЕсли;
