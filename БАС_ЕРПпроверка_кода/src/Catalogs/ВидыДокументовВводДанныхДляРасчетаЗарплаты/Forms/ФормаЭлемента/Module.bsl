...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОрганизацияЗаполняется И Не ЗначениеЗаполнено(Объект.Организация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Поле ""Организация"" не заполнено';uk='Поле ""Організація"" не заповнене'"), , "Объект.Организация", , Отказ);
	КонецЕсли;
	
	Если ПодразделениеЗаполняется И Не ЗначениеЗаполнено(Объект.Подразделение) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Поле ""Подразделение"" не заполнено';uk='Полі ""Підрозділ"" не заповнено'"), , "Объект.Подразделение", , Отказ);
	КонецЕсли;
...
Процедура НаименованиеПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(Объект.ПредставлениеДокумента) 
		Или Объект.ПредставлениеДокумента = БылоНаименование Тогда
...
Процедура СпособПримененияЗначенийПоказателейПриИзменении(Элемент)
	...
	Если Не ДоступностьВводаОперативныхДанных И (ВыбранныеВидыВремени.Количество() > 0 Или Объект.ВидыРабот.Количество() > 0) Тогда
		...
		ТекстВопроса = НСтр("ru='При вводе постоянных показателей ввод оперативных данных (о времени или сдельных работах) невозможен. Выбор видов времени и/или видов работ будет отменен.
                             |Продолжить?'
                             |;uk='При введенні постійних показників введення оперативних даних про часу або відрядних роботах) неможливий. Вибір видів часу і/або видів робіт буде скасовано.
                             |Продовжити?'");
...
Процедура СпособПримененияЗначенийПоказателейПриИзмененииЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	УстановитьДоступностьВводаОперативныхДанных(ДополнительныеПараметры.ДоступностьВводаОперативныхДанных);	
	...
	Если Объект.ВидыРабот.Количество() > 0 Или ВыбранныеВидыВремени.Количество() > 0 Или КоличествоВыбранныхПоказателей(ЭтаФорма) > 0 Тогда
		Если ВыбранныеВидыВремени.Количество() + КоличествоВыбранныхПоказателей(ЭтаФорма) = 0 Тогда
			ТекстВопроса = НСтр("ru='Список выбранных видов работ будет очищен.
                                 |Продолжить?'
                                 |;uk='Список обраних видів робіт буде очищений.
                                 |Продовжити?'");
		Иначе
			ТекстВопроса = НСтр("ru='Такой способ ввода не предусматривает совместный ввод выполненных работ 
                                 ...
                                 |Продолжить?'
                                 |;uk='Такий спосіб введення не передбачає спільний введення виконаних робіт 
                                 |зі значеннями показників та/або даними робочого часу.
                                 |Вибір показників, видів часу буде вичерпано.
                                 |Продовжити?'");
		КонецЕсли;
...
Процедура ВидыРаботЗаполняютсяВДокументеПриИзмененииЗавершение(Ответ, ДополнительныеПараметры) Экспорт 
	...
	Если Ответ = КодВозвратаДиалога.Нет Тогда
		ВидыРаботЗаполняютсяВДокументе = 0;
	...
	УстановитьДоступностьЗаполненияВидовРабот();
...
Процедура ПрочитатьДанныеФормы(ТекущийОбъект = Неопределено)
	...
	ПрочитатьНастройкуИспользованияВидовВремени(ТекущийОбъект);
	ПрочитатьНастройкуИспользованияПоказателей(ТекущийОбъект);
	ПрочитатьНастройкуИспользованияВидовРабот(ТекущийОбъект);
	...
	УстановитьДоступностьВремяСводно(ЭтаФорма);
	УстановитьДоступностьВремяВводитсяЗаМесяц(ЭтаФорма);
	УстановитьДоступностьВводаОперативныхДанных(Объект.СпособПримененияЗначенийПоказателей = Перечисления.СпособыПримененияЗначенийПоказателейРасчетаЗарплаты.Разовое);
	УстановитьЗаголовокСтраницыПоказатели(ЭтаФорма);
	УстановитьЗаголовокСтраницыВидыВремени(ЭтаФорма);
	УстановитьЗаголовокСтраницыВидыРабот(ЭтаФорма);
	УстановитьДоступностьИспользованияПериодаОкончания(ЭтаФорма);
...
&НаСервере
Процедура ПрочитатьНастройкуИспользованияПоказателей(ТекущийОбъект = Неопределено)
	...
	Для Каждого СтрокаПоказателя Из ПоказателиВсе Цикл
		СтрокаПоказателя.Пометка = ТекущийОбъект.Показатели.НайтиСтроки(Новый Структура("Показатель", СтрокаПоказателя.Показатель)).Количество() > 0;
	КонецЦикла;

	ОрганизацияЗаполняется = ЗначениеЗаполнено(ТекущийОбъект.Организация);
	ПодразделениеЗаполняется = ЗначениеЗаполнено(ТекущийОбъект.Подразделение);
...
Процедура ЗаполнитьВыбранныеПоказатели(Форма)
	...
	// Доступность множественности зависит от выбранности показателей по назначению.
	УстановитьДоступностьВыбораНесколькихОбъектов(Форма);
...
&НаСервере
Процедура ПрочитатьНастройкуИспользованияВидовВремени(ТекущийОбъект = Неопределено)
	...
	Для Каждого СтрокаВидовВремени Из ВидыНерабочегоВремени Цикл
		СтрокаВидовВремени.Пометка = ТекущийОбъект.ВидыВремени.НайтиСтроки(Новый Структура("ВидВремени", СтрокаВидовВремени.ВидВремени)).Количество() > 0;
	КонецЦикла;
...
&НаСервере
Процедура ПрочитатьНастройкуИспользованияВидовРабот(ТекущийОбъект = Неопределено)
	
	Если ТекущийОбъект = Неопределено Тогда
		ТекущийОбъект = Объект;
	КонецЕсли;
	
	ВидыРаботЗаполняютсяВДокументе = ТекущийОбъект.ВидыРаботЗаполняютсяВДокументе;
	ВыполненныеРаботыРаспределяютсяПоСотрудникамЧисло = ТекущийОбъект.ВыполненныеРаботыРаспределяютсяПоСотрудникам;
	ВыполненныеРаботыВводятсяСводноЧисло = ТекущийОбъект.ВыполненныеРаботыВводятсяСводно;
...
Процедура ЗаписатьНастройкуИспользованияВидовРабот(ТекущийОбъект)
	...
	ТекущийОбъект.ВидыРаботЗаполняютсяВДокументе = ВидыРаботЗаполняютсяВДокументе;
...
#Область ПроцедурыУправленияСвойствамиЭлементовФормы
...
Процедура УстановитьОтборПоказателей(Форма)
	...
	// Набор отмеченных показателей изменился.
	ЗаполнитьВыбранныеПоказатели(Форма);
...
Процедура УстановитьДоступностьВыбораНесколькихОбъектов(Форма)
	...
	Если Форма.ВыбранныеПоказателиДляСотрудника.Количество() = 0 
		...
		И (Форма.Объект.ВидыРабот.Количество() = 0 И Не Форма.ВидыРаботЗаполняютсяВДокументе) Тогда
	...
	Если Не Форма.Элементы.НесколькоСотрудников.Доступность Тогда
		Если Форма.ВидыРаботЗаполняютсяВДокументе 
...
Процедура УстановитьДоступностьЗаполненияСпискаОбъектов(Форма)
	...
	Форма.Элементы.ЗаполнятьСписокОбъектов.Доступность = 
			Форма.Объект.НесколькоОрганизаций Или Форма.Объект.НесколькоПодразделений 
			Или (Форма.Объект.НесколькоСотрудников И Не Форма.ВидыРаботЗаполняютсяВДокументе);
...
Процедура УстановитьДоступностьВыбораОрганизации(Форма)
	...
	Форма.Элементы.Организация.ОтметкаНезаполненного = Не ЗначениеЗаполнено(Форма.Объект.Организация);
...
Процедура УстановитьДоступностьВыбораПодразделения(Форма)
	...
	Форма.Элементы.Подразделение.ОтметкаНезаполненного = Не ЗначениеЗаполнено(Форма.Объект.Подразделение);
...
Процедура УстановитьДоступностьОтображенияПодразделения(Форма)
	...
	Форма.Элементы.ПоказыватьПодразделение.Доступность = Не Форма.Объект.НесколькоПодразделений И Не Форма.Объект.НесколькоОрганизаций
		И (Форма.ВыбранныеПоказателиДляСотрудника.Количество() > 0 Или Форма.ВыбранныеВидыВремени.Количество() > 0 
			Или Форма.Объект.ВидыРабот.Количество() > 0 Или Форма.ВидыРаботЗаполняютсяВДокументе)
		И Форма.ВыбранныеПоказателиДляПодразделения.Количество() = 0;
	
	Если Не Форма.Элементы.ПоказыватьПодразделение.Доступность Тогда 
		Форма.Объект.ПоказыватьПодразделение = Форма.ВыбранныеПоказателиДляПодразделения.Количество() > 0 
			И Не Форма.Объект.НесколькоПодразделений И Не Форма.Объект.НесколькоОрганизаций;
	КонецЕсли;
...
Процедура УстановитьДоступностьВремяСводно(Форма)
	...
	Форма.Элементы.ВремяВводитсяСводно.Доступность = Не Форма.Объект.ЗначенияПоказателейВводятсяНаРазныеДаты 
		И Форма.Объект.ВидыРабот.Количество() = 0 И Не Форма.ВидыРаботЗаполняютсяВДокументе;
...
&НаСервере
Процедура УстановитьДоступностьВводаОперативныхДанных(ДоступностьВвода)
	
	УстановитьДоступностьВводаВидовВремени(ДоступностьВвода);
	УстановитьДоступностьВводаВидовРабот(ДоступностьВвода);
...
&НаСервере
Процедура УстановитьДоступностьВводаПоказателей(ДоступностьВводаПоказателей)
	
	Элементы.СтраницаПоказателей.Доступность = ДоступностьВводаПоказателей;
	
	Если ДоступностьВводаПоказателей Тогда
		Возврат;
	КонецЕсли;
	
	// Если ввод показателей недоступен - очищаем коллекцию выбранных.
...
&НаСервере
Процедура УстановитьДоступностьЗаполненияВидовРабот()
	
	Если ВидыРаботЗаполняютсяВДокументе = 0 Тогда
		ВыполненныеРаботыРаспределяютсяПоСотрудникамЧисло = 0;
	КонецЕсли;
	
	ДоступностьВводаВидовРабот = ВидыРаботЗаполняютсяВДокументе = 0;
	
	Если ВидыРаботЗаполняютсяВДокументе = 1 Тогда
	...
	УстановитьДоступностьВыбораНесколькихОбъектов(ЭтаФорма);
	УстановитьДоступностьОтображенияПодразделения(ЭтаФорма);
	УстановитьДоступностьВремяСводно(ЭтаФорма);
	УстановитьДоступностьВремяВводитсяЗаМесяц(ЭтаФорма);
	
	// Если выбрано ВидыРаботЗаполняютсяВДокументе, ввод показателей и видов времени уже недоступны.
	УстановитьДоступностьВводаВидовВремени(ДоступностьВводаВидовРабот);
	УстановитьДоступностьВводаПоказателей(ДоступностьВводаВидовРабот);
