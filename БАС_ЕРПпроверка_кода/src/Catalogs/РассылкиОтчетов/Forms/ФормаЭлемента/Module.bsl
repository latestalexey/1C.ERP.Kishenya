...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Автор) Тогда
			...
			ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru='Рассылка отчетов. Открытие формы элемента';uk='Розсилка звітів. Відкриття форми елемента'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Помилка заповнення автора розсилки:
                |Користувач ""%1"" (%2) не може бути автором.'"),
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(Объект.Наименование) Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ru='Не введено наименование';uk='Не введено найменування'");
	...
	Если Объект.ИспользоватьЭлектроннуюПочту И Не Объект.Личная Тогда
		Если Не ЗначениеЗаполнено(ТипПолучателейРассылки) Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ru='Не выбран тип получателей';uk='Не обран тип одержувачів'");
	...
	Если Объект.Подготовлена Тогда
		Если Объект.Отчеты.Количество() = 0 Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ru='Не выбрано ни одного отчета';uk='Не обрано жодного звіту'");
		...
		Если Не ЗначениеЗаполнено(Объект.ПериодичностьРасписания) Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ru='Не выбрана периодичность запуска';uk='Не обрана періодичність запуску'");
		...
		Если Объект.ИспользоватьFTPРесурс Тогда
			Если Не ЗначениеЗаполнено(Объект.FTPСервер)
				...
				Отказ = Истина;
				ТекстСообщения = НСтр("ru='Не введен FTP адрес';uk='Не введена FTP адреса'");
		...
		Если Объект.ИспользоватьСетевойКаталог Тогда
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогWindows) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru='Не введен сетевой каталог Windows';uk='Не введено мережевий каталог Windows'");
			...
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогLinux) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru='Не введен сетевой каталог Linux';uk='Не введено мережевий каталог Linux'");
		...
		Если Объект.ИспользоватьПапку Тогда
			Если Не ЗначениеЗаполнено(Объект.Папка) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru='Не выбрана папка';uk='Не обрана папка'");
		...
		Если Объект.ИспользоватьЭлектроннуюПочту Тогда
			Если Объект.Личная Тогда
				Если Не ЗначениеЗаполнено(Объект.ВидПочтовогоАдресаПолучателей) Тогда
					Отказ = Истина;
					ТекстСообщения = НСтр("ru='Не выбран почтовый адрес';uk='Не обрана поштова адреса'");
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(Объект.ВидПочтовогоАдресаПолучателей) Тогда
					Отказ = Истина;
					ТекстСообщения = НСтр("ru='Не выбран тип почтового адреса получателей';uk='Не вибрано тип поштової адреси одержувачів'");
			...
			Если Не ЗначениеЗаполнено(Объект.УчетнаяЗапись) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru='Не выбрана учетная запись для отправки';uk='Не обран обліковий запис для відправки'");
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если СтрокаОтчеты.СКД Тогда
			...
			Если Не ЗаполненыВсеОбязательныеНастройки Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Для отчета ''%1'' заполнены не все настройки, обязательные для заполнения. Необходимо заполнить все обязательные настройки или снять флажок ''Подготовлена''.';uk='Для звіту ''%1'' заповнені не всі настройки, обов''язкові для заповнення. Необхідно заповнити всі обов''язкові настройки або зняти прапорець ''Підготовлена''.'"),
		...
		Если ПроверитьПерсонализированные Тогда
			...
			Если ОтчетПерсонализирован Тогда
				Отказ = Истина;
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Для отчета ''%1'' задан отбор по получателям рассылки. Необходимо отключить отбор по получателям или изменить вид рассылки на ''Свой отчет для каждого получателя''.';uk='Для звіту ''%1'' заданий відбір по одержувачах розсилки. Необхідно відключити відбір по одержувачах або змінити вид розсилки на ''Свій звіт для кожного одержувача''.'"),
					Строка(СтрокаОтчеты.Отчет));
				Поле = "Отчеты["+ Формат(ТекущийОбъект.Отчеты.Индекс(ОбъектСтрокаОтчеты), "ЧН=0; ЧГ=0") +"].Представление";
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ТекущийОбъект, Поле);
			КонецЕсли;
...
Процедура ТипПолучателейРассылкиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.Получатели.Количество() > 0 Тогда
		...
		СтрокаВопроса = НСтр("ru='Для продолжения необходимо очистить список получателей.';uk='Для продовження необхідно очистити список одержувачів.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Очистить';uk='Очистити'"));
...
Процедура FTPСерверИКаталогНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыПроизвольнойФормы.Вставить("Заголовок", НСтр("ru='<Укажите получателя>';uk='<Вкажіть одержувача>'"));
...
Процедура ПапкаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если Не ПравоИзмененияПапкиИФайлов(ВыбранноеЗначение) Тогда
		СтандартнаяОбработка = Ложь;
		ТекстПредупреждения = НСтр("ru='Недостаточно прав для изменения файлов папки ""%1"".';uk='Недостатньо прав для зміни файлів папки ""%1"".'");
...
Процедура РодительОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если ВыбранноеЗначение = Кэш.ГруппаЛичныхРассылок Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, НСтр("ru='Выбранная группа используется только для личных рассылок по электронной почте';uk='Обрана група використовується тільки для особистих розсилань по електронній пошті'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтчетыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если СтруктураВыбора.СОшибками.Количество > 0 Тогда
		...
		Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
			ТекстСообщения = НСтр("ru='Не удалось добавить отчет из-за ошибок';uk='Не вдалося додати звіт через помилки'");
		ИначеЕсли СтруктураВыбора.Успешно.Количество = 0 Тогда
			ТекстСообщения = НСтр("ru='Не удалось добавить отчеты из-за ошибок';uk='Не вдалося додати звіти через помилки'");
		ИначеЕсли СтруктураВыбора.Выбрано.Количество >= 6 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Добавлено отчетов: %1 из %2,
                |Не добавлено из-за ошибок: %3'
                |;uk='Додане звітів: %1 з %2,
                |Не додане через помилки: %3'"),
		...
		Иначе
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Успешно добавлены отчеты:
                    ...
                    |%2'
                    |;uk='Успішно додані звіти:
                    |%1
                    |
                    |Наступні звіти не вдалося додати через помилки:
                    |%2'"),
		...
		ПараметрыПредупреждения.Заголовок = НСтр("ru='Ошибки добавления отчетов в рассылку';uk='Помилки додавання звітів у розсилку'");
	...
	Иначе
		...
		Если СтруктураВыбора.Успешно.Количество = 0 Тогда
			...
			ТекстОповещения = НСтр("ru='Все выбранные отчеты уже включены в рассылку';uk='Всі обрані звіти вже включені в розсилку'");
		Иначе
			Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
				ЗаголовокОповещения = НСтр("ru='Отчет включен в рассылку';uk='Звіт включений у розсилку'");
			Иначе
				ЗаголовокОповещения = НСтр("ru='Отчеты включены в рассылку';uk='Звіти включені в розсилку'");
			КонецЕсли;
...
Процедура ПользовательскиеНастройкиЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Если Элементы.Отчеты.ТекущиеДанные.ПолноеИмя = "Отчет.МониторЦелевыхПоказателей" Тогда
		ИДПользовательскойНастройки = Элементы.КомпоновщикНастроекПользовательскиеНастройки.ТекущаяСтрока;
		ЗначениеПараметраНастроек = КомпоновщикНастроекКД.ПользовательскиеНастройки.ПолучитьОбъектПоИдентификатору(ИДПользовательскойНастройки);
		Если ЗначениеПараметраНастроек.Параметр = Новый ПараметрКомпоновкиДанных("ВыбранныеВариантыАнализа") Тогда
			СтандартнаяОбработка = Ложь;
			
			ПараметрыНастройки = Новый Структура("НастройкаВыбранныхВариантовАнализа", Истина);
			ПараметрыНастройки.Вставить("ПользовательскиеНастройки", КомпоновщикНастроекКД.ПользовательскиеНастройки);
			ПараметрыНастройки.Вставить("КлючНазначенияИспользования", "Рассылка = " + Объект.Ссылка);
...
Процедура ПолучателиРассылкиНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.ТипПолучателейРассылки) Тогда
		ТекстОшибки = НСтр("ru='Для ввода получателей необходимо выбрать их тип';uk='Для введення одержувачів необхідно вибрати тип'");
...
Процедура КомандаВыполнитьСейчас(Команда)
	Если Не Объект.Подготовлена Тогда
		ПоказатьПредупреждение(, НСтр("ru='Рассылка не подготовлена';uk='Розсилка не підготовлена'"));
		Возврат;
	КонецЕсли;
	ПараметрыЗаписи = Новый Структура;
...
Процедура ИспользоватьПолучателяРассылкиВНастройкеОтчета(Команда)
	...
	Если Не Объект.Персонализирована Тогда
		...
		СообщениеТекст = НСтр("ru='Использовать получателя в параметрах возможно только для вида рассылки ""%1""';uk='Використовувати одержувача в параметрах можливо тільки для виду розсилки ""%1""'");
	...
	Если КоличествоТипов <> 1 И КоличествоТипов <> 2 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Поле ""Получатели"" не заполнено';uk='Поле ""Одержувачі"" не заповнене'"), , "ТипПолучателейРассылки");
		Возврат;
	КонецЕсли;
	...
	Если НайденныеИОМД.Количество() <> 1 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Тип получателей не найден';uk='Тип одержувачів не знайдений'"));
		Возврат;
	КонецЕсли;
	...
	Если СКД Тогда
		...
		Если ИдентификаторНастройки = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru='Выберите настройку отчета';uk='Виберіть настройки звіту'"));
			Возврат;
		КонецЕсли;
		...
		Если СтрокаНастроек = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru='Настройка отчета не найдена';uk='Настройка звіту не знайдена'"));
			Возврат;
		КонецЕсли;
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru='Указывать получателя можно только для параметров и отборов отчетов';uk='Вказувати одержувача можна тільки для параметрів і відборів звітів'"));
			Возврат;
		КонецЕсли;
		...
		Если ЭтоЭлементОтбора Тогда
			...
			Иначе
				...
				КонецЕсли;
				Если ПолеКД = Неопределено Тогда
					ПоказатьПредупреждение(, НСтр("ru='Для настройки отчета не найдено описание доступного поля';uk='Для настройки звіту не знайдений опис доступного поля'"));
					Возврат;
				КонецЕсли;
	...
	Если Не ОписаниеДоступныхТипов.СодержитТип(ОсновнойТипПолучателей) Тогда
        ...
        |;uk='Тип ""%1"" не підходить по типу до обраної настройки.
        |Необхідно вибрати інший тип одержувачів або іншу настройку.'");
...
Процедура ЗаполнитьРасписаниеПоШаблону(Команда)
	...
	СписокВариантов.ПоказатьВыборЭлемента(Обработчик, НСтр("ru='Выберите шаблон расписания';uk='Виберіть шаблон розкладу'"));
КонецПроцедуры
...
Процедура ДобавитьШаблонПолучателя(Команда)
	...
	Если НЕ Объект.Персонализирована Тогда
		СообщениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Использование получателя в тексте шаблона возможно только для вида рассылки ""%1""';uk='Використання одержувача в тексті шаблона можливе тільки для виду розсилки ""%1""'"),
...
Процедура ДобавитьШаблонПоУмолчанию(Команда)
	...
	ИначеЕсли ЗначениеПредмета = ШаблонПоУмолчанию Тогда
		...
		Если ПерезаполняетсяТема Тогда
			ТекстПредупреждения = НСтр("ru='Тема письма уже соответствует шаблону по умолчанию.';uk='Тема листа вже відповідає шаблону за замовчуванням.'");
		Иначе
			ТекстПредупреждения = НСтр("ru='Текст письма уже соответствует шаблону по умолчанию.';uk='Текст листа вже відповідає шаблону за замовчуванням.'");
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстПредупреждения);
	...
	Иначе
		...
		Если ПерезаполняетсяТема Тогда
			ЗаголовокВопроса = НСтр("ru='Добавить в тему письма шаблон по умолчанию';uk='Додати в тему листа шаблон за замовчуванням'");
			ТекстВопроса = НСтр("ru='Заменить тему письма на шаблон по умолчанию?';uk='Замінити тему листа на шаблон за замовчуванням?'");
		Иначе
			ЗаголовокВопроса = НСтр("ru='Добавить в текст письма шаблон по умолчанию';uk='Додати в текст листа шаблон за замовчуванням'");
			ТекстВопроса = НСтр("ru='Заменить текст письма на шаблон по умолчанию?';uk='Замінити текст листа на шаблон за замовчуванням?'");
		КонецЕсли;
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(1, НСтр("ru='Заменить';uk='Замінити'"));
		Кнопки.Добавить(2, НСтр("ru='Добавить';uk='Додати'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ПроверитьРассылкуПослеВыбораПолучателей(РезультатВыбора, ПараметрыДоставки) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		ПараметрыДоставки.Вставить("ШаблонТемы",    НСтр("ru='Тестовое сообщение BAS ERP';uk='Тестове повідомлення BAS ERP'"));
		ПараметрыДоставки.Вставить("ШаблонТекста",  НСтр("ru='Это сообщение отправлено системой рассылок BAS ERP.';uk='Це повідомлення надіслано системою розсилок BAS ERP.'") + Символы.ПС + Кэш.ЗаголовокСистемы);
...
Функция ВыбратьФормат(СсылкаНаОтчет, ОбработчикРезультата)
	...
	Если ЭтоФорматПоУмолчанию Тогда
		ЗаголовокДиалога = НСтр("ru='Выберите форматы по умолчанию';uk='Виберіть формати за замовчуванням'");
	Иначе
		ЗаголовокДиалога = НСтр("ru='Выберите форматы для отчета ""%1""';uk='Виберіть формати для звіту ""%1""'");
...
Процедура ПроверитьРассылку(ПараметрыДоставки)
	...
	Если Не Объект.Подготовлена Или Объект.Ссылка.Пустая() Тогда
		ЗаголовокВопроса = НСтр("ru='Проверка способа доставки';uk='Перевірка способу доставки'");
		Если Не Объект.Подготовлена Тогда
            ...
            |;uk='Перед перевіркою розсилка повинна бути підготовлена.
            |Натисніть ""Продовжити"", щоб установити прапорець ""Підготовлена"" і записати розсилку.'");
		Иначе
			ТекстВопроса = НСтр("ru='Перед проверкой рассылка должна быть записана.
            |Нажмите ""Продолжить"", чтобы записать рассылку.'
            |;uk='Перед перевіркою розсилка повинна бути записана.
            |Натисніть ""Продовжити"", щоб записати розсилку.'");
		КонецЕсли;
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(1, НСтр("ru='Продолжить';uk='Продовжити'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ВидимостьДоступностьКорректность(Форма, Изменения = "")
	...
	Если Изменения = "" Или Изменения = "ВидРассылки" Тогда
		...
		Если Объект.Личная <> ИспользуетсяГруппаЛичныхРассылок Тогда
			УстановитьМодифицированностьФормы(Форма, "Родитель", , 
				НСтр("ru='Группа установлена в соответствии с видом рассылки';uk='Група встановлена відповідно до виду розсилання'"));
	...
	Если Изменения = "" Или Изменения = "Отчеты" Тогда
		...
		Если КоличествоОтчетов > 0 Тогда
			Элементы.СтраницаОтчеты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Отчеты (%1)';uk='Звіти (%1)'"), 
				Формат(КоличествоОтчетов, "ЧН=0; ЧГ="));
		Иначе
			Элементы.СтраницаОтчеты.Заголовок = НСтр("ru='Отчеты';uk='Звіти'") ;
		КонецЕсли;
	КонецЕсли;
	...
	Если Изменения = ""
		...
		СпособыДоставки = "";
		Если Объект.ИспользоватьПапку Тогда
			СпособыДоставки = НСтр("ru='папка';uk='папка'");
		КонецЕсли;
		Если Объект.ИспользоватьСетевойКаталог Тогда
			СпособыДоставки = НСтр("ru='сетевой каталог';uk='мережний каталог'");
		КонецЕсли;
		Если Объект.ИспользоватьFTPРесурс Тогда
			СпособыДоставки = НСтр("ru='FTP';uk='FTP'");
		КонецЕсли;
		Если Объект.ИспользоватьЭлектроннуюПочту И Не Объект.ТолькоУведомить Тогда
			СпособыДоставки = СпособыДоставки + ?(СпособыДоставки = "", НСтр("ru='эл. почта';uk='ел. пошта'"), " "+ НСтр("ru='и эл. почта';uk='і ел. пошта'"));
		КонецЕсли;
		
		Элементы.СтраницаДоставка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Доставка (%1)';uk='Доставка (%1)'"), СпособыДоставки);
	КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ВыполнятьПоРасписанию" Тогда
		Если Объект.ВыполнятьПоРасписанию Тогда
			Элементы.СтраницаРасписание.Заголовок = НСтр("ru='Расписание (активно)';uk='Розклад (активно)'");
		Иначе
			Элементы.СтраницаРасписание.Заголовок = НСтр("ru='Расписание (не активно)';uk='Розклад (не активно)'");
		КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ПериодичностьРасписания" Тогда
		...
		ИначеЕсли ИмяПеречисления = "Ежемесячно" Тогда
			...
			Если Форма.Расписание.ДеньВМесяце >= 0 Тогда
				...
				Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = НСтр("ru='начала';uk='початку'");
			Иначе
				Форма.ДеньВМесяце = -Форма.Расписание.ДеньВМесяце;
				Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = НСтр("ru='конца';uk='кінця'");
			КонецЕсли;
...
Функция ПредставлениеПолучателей(Форма)
	...
	Если Представление.Всего = 0 Тогда
		Представление.Краткое = НСтр("ru='<Укажите получателей>';uk='<Вкажіть одержувачів>'");
	...
	Если ПредставлениеОтключенных.Всего > 0 Тогда
		ШаблонРазделения = НСтр("ru='Кроме';uk='Крім'")+ ": ";
	...
	Если Представление.МаксимумПревышен
		...
		Если ПредставлениеОтключенных.Всего > 0 Тогда
			ШаблонОкончания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='(всего %1, исключая %2)';uk='(всього %1, виключаючи %2)'"),
				Представление.Всего,
				ПредставлениеОтключенных.Всего);
		Иначе
			ШаблонОкончания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='(всего %1)';uk='(усього %1)'"),
				Представление.Всего);
		КонецЕсли;
...
Функция ПредставлениеФорматовПоУмолчанию()
	Возврат НСтр("ru='по умолчанию';uk='по умовчанню'");
КонецФункции
...
Функция ПолучателиУказаны(Получатели)
	Для Каждого СтрокаТаблицы Из Получатели Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ТекстСообщения = НСтр("ru='Не выбрано ни одного получателя';uk='Не обрано жодного одержувача'");
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПолучателиРассылки");
	Возврат Ложь;
КонецФункции
...
Функция ПроверитьСпособДоставки(Рассылка, Знач ПараметрыДоставки)
	...
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru='Рассылка отчетов. Проверка способа доставки';uk='Розсилка звітів. Перевірка способу доставки'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	...
	Если Выполнена Тогда
		РезультатВыполнения.Текст = НСтр("ru='Тест способа доставки успешно пройден.';uk='Тест способу доставки успішно пройдений.'");
	Иначе
		РезультатВыполнения.Текст = НСтр("ru='Тест способа доставки не пройден.';uk='Тест способу доставки не пройдений.'");
...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	// Параметры журнала
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru='Рассылка отчетов. Инициализация отчета';uk='Розсилка звітів. Ініціалізація звіту'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	...
	Если Не СтрокаОтчеты.Инициализирован Тогда
		ПараметрыОтчета.Ошибки = (
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Отчет %1 был удален из-за ошибки инициализации';uk='Звіт %1 був вилучений через помилку ініціалізації'"),
...
Процедура ДобавитьНастройкиОтчетов(ПрисоединяемыеОтчеты)
	...
	Для Каждого СтрокаОтчетыПараметры Из ПрисоединяемыеОтчеты Цикл
		...
		Если Не ЭтоНовый Тогда
			Если Найденные.Количество() > 0 Тогда
				ШаблонСтрокиСообщения = НСтр("ru='Для отчета ""%1"" загружены новые пользовательские настройки.';uk='Для звіту ""%1"" завантажені нові користувацькі настройки.'");
			Иначе
				ШаблонСтрокиСообщения = НСтр("ru='Добавлен отчет ""%1"".';uk='Доданий звіт ""%1"".'");
			КонецЕсли;
...
Процедура ЗаписатьНаКлиенте(Результат, ПараметрыЗаписи) Экспорт
	...
	Если ПараметрыЗаписи.Шаг = 3 И ЖелательноОтключитьАрхивацию() Тогда
		...
		// Вопрос.
		ЗаголовокВопроса = НСтр("ru='Отключить архивацию';uk='Відключити архівацію'");
		ТекстВопроса = НСтр("ru='При публикации отчетов в папку рекомендуется отключать архивацию в ZIP.';uk='При публікації звітів у папку рекомендується відключати архівацію в ZIP.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Отключить архивацию в ZIP';uk='Відключити архівацію в ZIP'"));
		Кнопки.Добавить(КодВозвратаДиалога.Пропустить, НСтр("ru='Продолжить';uk='Продовжити'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ВыполнитьСейчасЗавершение(Получатели, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоФормаЭлемента Тогда
		...
		КонецЕсли;
		ТекстСостояния = НСтр("ru='Выполняется рассылка отчетов';uk='Виконується розсилка звітів'");
	Иначе
		ТекстСостояния = НСтр("ru='Выполняются рассылки отчетов';uk='Виконуються розсилки звітів'");
	КонецЕсли;
	...
	ИначеЕсли Результат.Статус = "Исключение" Тогда
        ...
        |;uk='Не вдалося виконати розсилку звітів.
...
Функция ВыполнитьРассылкиВФоновомЗадании(ПараметрыВызоваСервера, Знач УникальныйИдентификатор)
	...
	Попытка
		РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			НСтр("ru='Рассылки отчетов: Выполнение рассылок в фоне';uk='Розсилки звітів: Виконання розсилок у фоні'"));
...
#Область ПереопределениеПоведения
...
Процедура НастроитьЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	Если Не РезультатЗакрытия = КодВозвратаДиалога.Отмена 
		...
		КомпоновщикНастроекКД.ЗагрузитьПользовательскиеНастройки(РезультатЗакрытия);
