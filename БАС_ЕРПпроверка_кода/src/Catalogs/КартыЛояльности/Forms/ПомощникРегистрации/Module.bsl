...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Партнер) Тогда
		...
		Если Выборка.Следующий() Тогда
			Если Не Выборка.Клиент Тогда
				ВызватьИсключение СтрШаблон(НСтр("ru='Партнер ""%1"" не является клиентом. Регистрация карт лояльности возможна только для партнеров, являющихся клиентами.';uk='Партнер ""%1"" не є клієнтом. Реєстрація карт лояльності можлива тільки для партнерів, які є клієнтами.'"), Партнер);
			КонецЕсли;
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Партнер) И ЗначениеЗаполнено(ЗаменяемаяКарта) Тогда
		...
		Если ДанныеКарты.Статус = Перечисления.СтатусыКартЛояльности.Аннулирована Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru='Карта лояльности %1 уже аннулирована.';uk='Карта лояльності %1 вже анульована.'"), ЗаменяемаяКарта);
		КонецЕсли;
	...
	Если Режим = "РегистрацияНовойКартыЛояльности" Тогда
		...
		Заголовок = НСтр("ru='Регистрация новой карты лояльности';uk='Реєстрація нової карти лояльності'");
	ИначеЕсли Режим = "ЗаменаКартыЛояльности" Тогда
		...
		КонецЕсли;
		Заголовок = НСтр("ru='Замена карты лояльности';uk='Заміна карти лояльності'");
	Иначе
		...
		Заголовок = НСтр("ru='Помошник регистрации карт лояльности';uk='Помошник реєстрації карт лояльності'");
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ОсновнойТипКода) Тогда
		Текст = НСтр("ru='Считайте новую карту лояльности при помощи сканера штрихкода
                           ...
                           |;uk='Зчитайте нову картку лояльності за допомогою сканера штрих-коду
	...
	ИначеЕсли ОсновнойТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.МагнитныйКод") Тогда
		Текст = НСтр("ru='Считайте новую карту лояльности при помощи считывателя
                           ...
                           |;uk='Зчитайте нову картку лояльності за допомогою зчитувача
	...
	ИначеЕсли ОсновнойТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
		Текст = НСтр("ru='Считайте новую карту лояльности при помощи сканера
                           ...
                           |;uk='Зчитайте нову картку лояльності за допомогою сканера
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Не ВыполняетсяЗакрытие и ЗадаватьВопросПриЗакрытии И Модифицированность Тогда
...
Процедура Далее(Команда)
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаСчитываниеЗаменяемойКартыЛояльности Тогда
		...
		Если Не ЗначениеЗаполнено(ЗаменяемаяКарта) Тогда
			...
			ТекстСообщения = НСтр("ru='Поле ""Заменяемая карта"" не заполнено.';uk='Поле ""Замінна карта"" не заповнено.'");
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаСчитываниеНовойКартыЛояльности Тогда
		...
		Если Не ЗначениеЗаполнено(ПервыйКодНовойКарты) Тогда
			...
			Если ТипПервогоКодаНовойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
				ТекстСообщения = НСтр("ru='Поле ""Штрихкод"" не заполнено.';uk='Поле ""Штрихкод"" не заповнено.'");
			Иначе
				ТекстСообщения = НСтр("ru='Поле ""Магнитный код"" не заполнено.';uk='Поле ""Магнітний код"" не заповнено.'");
			КонецЕсли;
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаУточнениеВторогоКодаКартыЛояльности Тогда
		...
		Иначе
			Если ТипВторогоКодаНовойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Поле ""Штрихкод"" не заполнено.';uk='Поле ""Штрихкод"" не заповнено.'"),
					,
					"ШтрихкодНовойКарты");
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Поле ""Магнитный код"" не заполнено.';uk='Поле ""Магнітний код"" не заповнено.'"),
					,
...
Процедура Готово(Команда)
	...
	Если НоваяКартаПерсонизирована Тогда
		Если ЗаполнитьИнформациюОВладельцеКарты = "Сейчас" Тогда
			Если Не ЗначениеЗаполнено(Партнер) Тогда
				...
				ТекстСообщения = НСтр("ru='Поле ""Партнер"" не заполнено.';uk='Поле ""Партнер"" не заповнено.'");
	...
	Если НоваяКартаПерсонизирована И ЗначениеЗаполнено(Партнер) Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru='Зарегистрирована карта лояльности %1 для клиента %2.';uk='Зареєстрована карта лояльності %1 для клієнта %2.'"), НоваяКарта, Партнер);
	Иначе
		ТекстСообщения = СтрШаблон(НСтр("ru='Зарегистрирована карта лояльности %1.';uk='Зареєстрована карта лояльності %1.'"), НоваяКарта);
	КонецЕсли;
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Зарегистрирована карта лояльности';uk='Зареєстрована карта лояльності'"),
...
Процедура ПечатьАнкетыMicrosoftWord(Команда)
	...
	ОписаниеКоманды.Добавить(НоваяКарта);
	ОписаниеКоманды.Добавить(КодЯзыка);
	КартыЛояльностиКлиент.ПечатьАнкетыMicrosoftWord(ОписаниеКоманды);
	
КонецПроцедуры
&НаКлиенте
Процедура ПечатьАнкетыMicrosoftWordUk(Команда)
...
Процедура ПечатьАнкетыOpenOfficeOrgWriter(Команда)
	...
	ОписаниеКоманды.Добавить(НоваяКарта);
	ОписаниеКоманды.Добавить(КодЯзыка);
	КартыЛояльностиКлиент.ПечатьАнкетыOpenOfficeOrgWriter(ОписаниеКоманды);
	
КонецПроцедуры
&НаКлиенте
Процедура ПечатьАнкетыOpenOfficeOrgWriterUk(Команда)
	...
	ИначеЕсли Страница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаУточнениеВторогоКодаКартыЛояльности Тогда
		Если ТипПервогоКодаНовойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.МагнитныйКод") Тогда
			...
			Текст = НСтр("ru='Карта лояльности ""%1"" (Магнитный код: %2) является картой смешанного типа (имеет магнитный и штриховой коды).
                               ...
                               |;uk='Карта лояльності ""%1"" (Магнітний код: %2) є картою змішаного типу (має магнітний та штриховой коди).
                               |Зчитайте за допомогою сканера штрихкоду або вкажіть вручну штрихкод карти.'");
		Иначе
			...
			Текст = НСтр("ru='Карта лояльности ""%1"" (Штрихкод: %2) является картой смешанного типа (имеет магнитный и штриховой коды).
                               ...
                               |;uk='Карта лояльності ""%1"" (Штрихкод: %2) є картою змішаного типу (має магнітний і штриховий коди).
                               |Зчитайте за допомогою зчитувача магнітних карт або вкажіть вручну магнітний код карти.'");
		КонецЕсли;
	...
	ИначеЕсли Страница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаВводИнформацииОВладельцеКарты Тогда
		Если ТипНовойКарты = ПредопределенноеЗначение("Перечисление.ТипыКарт.Смешанная") Тогда
			Текст = НСтр("ru='Карта лояльности ""%1"" (%2: %3, %4: %5) является персонализированной.
                               ...
                               |;uk='Карта лояльності ""%1"" (%2: %3, %4: %5) є персоналізованою.
                               |Для завершення реєстрації уточніть інформацію про власника карти.'");
		...
		Иначе
			Текст = НСтр("ru='Карта лояльности ""%1"" (%2: %3) является персонализированной.
                               |Для завершения регистрации уточните информацию о владельце карты.'
                               |;uk='Карта лояльності ""%1"" (%2: %3) є персоналізованою.
                               |Для завершення реєстрації уточніть інформацію про власника карти.'");
			НадписьВводИнформацииОВладельцеКарты = СтрШаблон(Текст, ВидНовойКарты, ТипПервогоКодаНовойКарты, ПервыйКодНовойКарты);
		КонецЕсли;
		Если ЗначениеЗаполнено(ЗаменяемаяКарта) Тогда
			НадписьВводИнформацииОВладельцеКарты = СтрШаблон(НСтр("ru='Заменяемая карта лояльности ""%1"" будет аннулирована.';uk='Замінна карта лояльності ""%1"" буде анульована.'"), ЗаменяемаяКарта)
...
Функция ОбработатьПервыйКодНовойКартыНаСервере(Данные, ТипКода, Предобработка)
	...
	Для Каждого СтрокаТЧ Из КартыЛояльности.ЗарегистрированныеКартыЛояльности Цикл
		...
		НоваяСтрока.Наименование = Строка(СтрокаТЧ.Ссылка) + ?(ЗначениеЗаполнено(СтрокаТЧ.Партнер) И ЗначениеЗаполнено(СтрокаТЧ.Ссылка), СтрШаблон(НСтр("ru=' Партнер: %1';uk=' Партнер: %1'"), Строка(СтрокаТЧ.Партнер)), "");
	...
	Если НайденныеКартыЛояльностиСчитываниеНовойКартыЛояльности.Количество() = 0 Тогда
		...
		Если ТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru='Штрихкод ""%1"" не соответствует виду карт лояльности ""%2""';uk='Штрих-код ""%1"" не відповідає виду карт лояльності ""%2""'"), ПервыйКодНовойКарты, ТребуемыйВидНовойКарты);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru='Штрихкод ""%1"" не соответствует ни одному из видов карт лояльности.';uk='Штрихкод ""%1"" не відповідає жодному з видів карт лояльності.'"), ПервыйКодНовойКарты);
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru='Магнитный код ""%1"" не соответствует виду карт лояльности ""%2""';uk='Магнітний код ""%1"" не відповідає виду карт лояльності ""%2""'"), ПервыйКодНовойКарты, ТребуемыйВидНовойКарты);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru='Магнитный код ""%1"" не соответствует ни одному из видов карт лояльности.';uk='Магнітний код ""%1"" не відповідає жодному з видів карт лояльності.'"), ПервыйКодНовойКарты);
			КонецЕсли;
		КонецЕсли;
...
Функция ОбработатьВторойКодНовойКартыНаСервере(Данные, Предобработка)
	...
	Для Каждого СтрокаТЧ Из КартыЛояльности.ЗарегистрированныеКартыЛояльности Цикл
		...
		Если ЗначениеЗаполнено(ВидНовойКарты) И ВидНовойКарты = СтрокаТЧ.ВидКарты Тогда
			...
			Если ТипВторогоКодаНовойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
				ТекстСообщения = НСтр("ru='Штрихкод ""%1"" уже зарегистрирован для карты ""%2"".';uk='Штрихкод ""%1"" вже зареєстрований для карти ""%2"".'");
				Поле = "ШтрихкодНовойКарты";
				КодКарты = СтрокаТЧ.Штрихкод;
			Иначе
				ТекстСообщения = НСтр("ru='Магнитный код ""%1"" уже зарегистрирован для карты ""%2"".';uk='Магнітний код ""%1"" вже зареєстрований для карти ""%2"".'");
	...
	Если Не Результат Тогда
		...
		Если ТипВторогоКодаНовойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru='Штрихкод ""%1"" не соответствует виду карт лояльности ""%2""';uk='Штрих-код ""%1"" не відповідає виду карт лояльності ""%2""'"), ВторойКодНовойКарты, ТребуемыйВидНовойКарты);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru='Штрихкод ""%1"" не соответствует ни одному из видов карт лояльности.';uk='Штрихкод ""%1"" не відповідає жодному з видів карт лояльності.'"), ВторойКодНовойКарты);
			КонецЕсли;
			Поле = "ШтрихкодНовойКарты";
		Иначе
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru='Магнитный код ""%1"" не соответствует виду карт лояльности ""%2""';uk='Магнітний код ""%1"" не відповідає виду карт лояльності ""%2""'"), ВторойКодНовойКарты, ТребуемыйВидНовойКарты);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru='Магнитный код ""%1"" не соответствует ни одному из видов карт лояльности.';uk='Магнітний код ""%1"" не відповідає жодному з видів карт лояльності.'"), ВторойКодНовойКарты);
			КонецЕсли;
			Поле = "МагнитныйКодНовойКарты";
		КонецЕсли;
...
Процедура ОбработатьВыборНовойКартыЛояльности(ТекущиеДанные)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Ссылка) Тогда // Карта уже зарегистрирована.
		...
		Если ТипПервогоКодаНовойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			ТекстСообщения = НСтр("ru='Карта ""%1"" со штрихкодом %2 уже зарегистрирована.';uk='Карта ""%1"" зі штрихкодом %2 вже зареєстрована.'");
		Иначе
			ТекстСообщения = НСтр("ru='Карта ""%1"" с магнитным кодом %2 уже зарегистрирована.';uk='Карта ""%1"" з магнітним кодом %2 вже зареєстрована.'");
		КонецЕсли;
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(ТекущиеДанные.Ссылка) // Карта не зарегистрирована, не действует
		И (ТекущиеДанные.СтатусВидаКарты <> ПредопределенноеЗначение("Перечисление.СтатусыВидовКартЛояльности.Действует")
		   ИЛИ (ЗначениеЗаполнено(ТекущиеДанные.ДатаОкончанияДействия) И КонецДня(ТекущиеДанные.ДатаОкончанияДействия) < ТекущаяДата())) Тогда
...
Функция ОбработатьПервыйКодЗаменяемойКартыНаСервере(Данные, ТипКода, Предобработка)
	...
	Если НайденныеКартыЛояльностиСчитываниеЗаменяемойКартыЛояльности.Количество() > 0 Тогда
		...
		Если ТипКодаЗаменяемойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru='Штрихкод ""%1"" не соответствует виду карт лояльности ""%2""';uk='Штрих-код ""%1"" не відповідає виду карт лояльності ""%2""'"), КодЗаменяемойКарты, ТребуемыйВидНовойКарты);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru='Штрихкод ""%1"" не соответствует ни одному из видов карт лояльности.';uk='Штрихкод ""%1"" не відповідає жодному з видів карт лояльності.'"), КодЗаменяемойКарты);
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ТребуемыйВидНовойКарты) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru='Магнитный код ""%1"" не соответствует виду карт лояльности ""%2""';uk='Магнітний код ""%1"" не відповідає виду карт лояльності ""%2""'"), КодЗаменяемойКарты, ТребуемыйВидНовойКарты);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru='Магнитный код ""%1"" не соответствует ни одному из видов карт лояльности.';uk='Магнітний код ""%1"" не відповідає жодному з видів карт лояльності.'"), КодЗаменяемойКарты);
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбработатьВыборЗаменяемойКартыЛояльности(ТекущиеДанные)
	...
	Иначе // Карта не зарегистрирована.
		...
		Если ТипКодаЗаменяемойКарты = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
			ТекстСообщения = НСтр("ru='Карта ""%1"" со штрихкодом %2 еще не зарегистрирована и не может быть заменена.';uk='Карта ""%1"" зі штрихкодом %2 ще не зареєстрована і не може бути замінена.'");
		Иначе
			ТекстСообщения = НСтр("ru='Карта ""%1"" с магнитным кодом %2 еще не зарегистрирована и не может быть заменена.';uk='Карта ""%1"" з магнітним кодом %2 ще не зареєстрована і не може бути замінена.'");
		КонецЕсли;
