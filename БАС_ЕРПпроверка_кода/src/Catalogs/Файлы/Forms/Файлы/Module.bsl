...
Процедура ИмпортПапки(Команда)
	#Если ВебКлиент Тогда
		ТекстПредупреждения = НСтр("ru='В веб-клиенте импорт папок недоступен.
                                         ...
                                         |;uk='У веб-клієнті імпорт папок недоступний.
                                         |Використовуйте команду ""Створити"" у списку файлів.'");
...
Процедура НайтиВыполнить()
	...
	Если СтрокаПоиска = "" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не указан текст для поиска.';uk='Не зазначений текст для пошуку.'"), , "СтрокаПоиска");
		Возврат;
	КонецЕсли;
	...
	Если Результат = "НичегоНеНайдено" Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось найти файл или папку,
                       ...
                       |;uk='Не вдалося знайти файл або папку,
                       |найменування або код яких містить ""%1"".'"),
			СтрокаПоиска);
		ПоказатьПредупреждение(, ТекстПредупреждения);
	Иначе
...
Процедура ПеренестиВПапку(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Заголовок",    НСтр("ru='Выбор папки';uk='Вибір папки'"));
...
Процедура Печать(Команда)
	...
	Если СистемнаяИнфо.ТипПлатформы <> ТипПлатформы.Windows_x86 
	   И СистемнаяИнфо.ТипПлатформы <> ТипПлатформы.Windows_x86_64 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Печать файлов возможна только в Windows.';uk='Друк файлів можлива тільки в Windows.'"));
		Возврат;
	КонецЕсли;
		...
		ИначеЕсли ТипЗначения = Тип("СправочникСсылка.Файлы") Тогда
			...
			Если Действие = ДействиеПеретаскивания.Копирование Тогда
				...
				Если ЗначениеПеретаскивания.Количество() = 1 Тогда
					ОповещениеЗаголовок = НСтр("ru='Файл скопирован.';uk='Файл скопійований.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Файл ""%1""
                                   |скопирован в папку ""%2""'
                                   |;uk=' Файл ""%1""
                                   |скопійовано в папку ""%2""'"),
						ЗначениеПеретаскивания[0],
						Строка(ПапкаДляДобавления));
				Иначе
					ОповещениеЗаголовок = НСтр("ru='Файлы скопированы.';uk='Файли скопійовані.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Файлы (%1 шт.) скопированы в папку ""%2""';uk='Файли (%1 шт.) скопійовані в папку ""%2""'"),
			...
			Иначе
				...
				Если ЗначениеПеретаскивания.Количество() = 1 Тогда
					ОповещениеЗаголовок = НСтр("ru='Файл перенесен.';uk='Файл перенесений.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Файл ""%1""
                                   |перенесен в папку ""%2""'
                                   |;uk='Файл ""%1""
                                   |перенесений в папку ""%2""'"),
						Строка(ЗначениеПеретаскивания[0]),
						Строка(ПапкаДляДобавления));
				Иначе
					ОповещениеЗаголовок = НСтр("ru='Файлы перенесены.';uk='Файли перенесені.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Файлы (%1 шт.) перенесены в папку ""%2""';uk='Файли (%1 шт.) перенесені в папку ""%2""'"),
		...
		ИначеЕсли ТипЗначения = Тип("СправочникСсылка.ПапкиФайлов") Тогда
			...
			Если РодительСменен <> Истина Тогда
				Если НайденоЗацикливание = Истина Тогда
					ПоказатьПредупреждение(, НСтр("ru='Зацикливание уровней.';uk='Зациклення рівнів.'"));
				КонецЕсли;
				Возврат;
			КонецЕсли;
			...
			Если ЗначениеПеретаскивания.Количество() = 1 Тогда
				...
				ОповещениеЗаголовок = НСтр("ru='Папка перенесена.';uk='Папка перенесена.'");
				ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Папка ""%1""
					...
					Строка(ПапкаДляДобавления));
			Иначе
				ОповещениеЗаголовок = НСтр("ru='Папки перенесены.';uk='Папки перенесені.'");
				ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Папки (%1 шт.) перенесены в папку ""%2""';uk='Папки (%1 шт.) перенесені в папку ""%2""'"),
					Строка(ЗначениеПеретаскивания.Количество()),
					Строка(ПапкаДляДобавления));
			КонецЕсли;
			ПоказатьОповещениеПользователя(ОповещениеЗаголовок, , ОповещениеТекст, БиблиотекаКартинок.Информация32);
...
Процедура УстановитьВидимостьПоясненияСинхронизацииФайлов()
	...
	Иначе
		...
		Если Элементы.Папки.ТекущиеДанные.ПапкаСинхронизируется Тогда
			...
			ЧастиСтроки.Добавить(НСтр("ru='Работа с файлами ведется в облачном сервисе';uk='Робота з файлами ведеться в хмарному сервісі'"));
...
Процедура ОбновитьПредпросмотр()
	...
	Иначе
		...
		Элементы.АдресДанныхФайла.ТекстНевыбраннойКартинки = НСтр("ru='Нет данных для предварительного просмотра';uk='Немає даних для попереднього перегляду'");
	КонецЕсли;
