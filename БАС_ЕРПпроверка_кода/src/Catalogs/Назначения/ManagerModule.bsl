...
Процедура СоздатьОбновитьНазначение(Заказ, Назначение, Клиент, Номер, Дата, ПометкаУдаления, НаправлениеДеятельности = Неопределено) Экспорт
	...
	Если ПоЗаказам Тогда
		...
		ПредставлениеНазначения = ПредставлениеНазначения(Номер, Дата, НаправлениеДеятельности, Заказ, Клиент);
			
		Если РеквизитыНазначения.НаправлениеДеятельности <> НаправлениеДеятельности
			...
			НазначениеОбъект.НаправлениеДеятельности = НаправлениеДеятельности;
...
Функция ТекстЗапросаНеобеспеченныхНазначенийОбеспечено(ПараметрыФормированияЗапроса) Экспорт
	...
	//++ НЕ УТКА
	|ГДЕ
	|	НЕ Назначения.Заказ Ссылка Документ.ЗаказДавальца Или &ВключатьЗаказыДавальцев
	//-- НЕ УТКА
	|
	|СГРУППИРОВАТЬ ПО
	|	Назначения.Ссылка,
...
//-- НЕ УТКА
		...
		|					Таблица.Потребность
		|			КОНЕЦ
		|		+ Таблица.НаличиеПодЗаказ      КАК Потребность,
		|
		|		ВЫБОР КОГДА Таблица.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) ТОГДА
		|					-Таблица.КЗаказу
		|				ИНАЧЕ
		|					Таблица.КЗаказу
		|			КОНЕЦ
		|			+ Таблица.НаличиеПодЗаказ  КАК КЗаказу
		|
		|	ИЗ
