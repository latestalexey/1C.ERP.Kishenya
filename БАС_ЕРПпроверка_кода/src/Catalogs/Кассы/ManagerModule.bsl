	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		Если ЗначениеЗаполнено(ОрганизацияПоУмолчанию) Тогда
			...
			Если СправочникОбъект.ВалютаДенежныхСредств = Константы.ВалютаРегламентированногоУчета.Получить() Тогда
				СправочникОбъект.СчетУчета = ПланыСчетов.Хозрасчетный.КассаВНациональнойВалюте;
			Иначе
				СправочникОбъект.СчетУчета = ПланыСчетов.Хозрасчетный.КассаВИностраннойВалюте;
			КонецЕсли;
			//-- НЕ УТ
