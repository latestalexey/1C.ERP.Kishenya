...
Процедура ИзменитьФормулу(Команда)
	...
	ПараметрыФормы.Вставить("ТипСобытийОповещений", Объект.ТипСобытия);
...
Процедура ТипСобытияПриИзмененииНаСервере(ПроверятьИзменениеТипаСобытия)
	...
	Если ЗначениеЗаполнено(Объект.ТипСобытия) Тогда
		...
		ПолноеИмяТипаСобытия = МетаданныеПеречисления.ПолноеИмя() + "." + 
		                     МетаданныеПеречисления.ЗначенияПеречисления.Получить(Перечисления.ТипыСобытийОповещений.Индекс(Объект.ТипСобытия)).Имя;
	Иначе
