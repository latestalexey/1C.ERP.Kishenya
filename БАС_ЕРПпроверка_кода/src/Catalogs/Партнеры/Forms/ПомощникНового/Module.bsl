	...
	Если Не ВидДокумента.Пустая() Тогда
		...
		ПредставлениеДокументаФизЛица = ""
			...
			+ ?(ЗначениеЗаполнено(СерияДокумента),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=', серия: %1';uk=', серія: %1'"), СерияДокумента), "")
			+ ?(ЗначениеЗаполнено(НомерДокумента),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=', № %1';uk=', № %1'"), НомерДокумента), "")
			+ ?(ЗначениеЗаполнено(ДатаВыдачи),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=', выдан: %1 года';uk=', видано: %1 року'"), Формат(ДатаВыдачи,"ДФ='дд ММММ гггг'")), "")
			+ ?(ЗначениеЗаполнено(СрокДействия),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=', действует до: %1 года';uk=', діє до: %1 року'"), Формат(СрокДействия,"ДФ='дд ММММ гггг'")), "")
