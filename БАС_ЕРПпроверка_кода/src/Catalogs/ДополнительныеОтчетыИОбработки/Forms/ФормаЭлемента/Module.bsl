...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДобавления Тогда
		Если ЭтоНовый Тогда
			ВызватьИсключение НСтр("ru='Недостаточно прав доступа для добавления дополнительных отчетов или обработок.';uk='Недостатньо прав доступу для додавання додаткових звітів або обробок.'");
		Иначе
	...
	Если ПоказатьДиалогЗагрузкиИзФайлаПриОткрытии И Не Элементы.ЗагрузитьИзФайла.Видимость Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав для загрузки дополнительных отчетов и обработок';uk='Недостатньо прав для завантаження додаткових звітів й обробок'");
	КонецЕсли;
...
Процедура ВариантыДополнительногоОтчетаПередУдалением(Элемент, Отказ)
	...
	Если НЕ Вариант.Пользовательский Тогда
		ПоказатьПредупреждение(, НСтр("ru='Пометка на удаление предопределенного варианта отчета запрещена.';uk='Позначка на вилучення визначеного варіанта звіту заборонена.'"));
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Пометить ""%1"" на удаление?';uk='Відмітити ""%1"" для вилучення?'"), Вариант.Наименование);
...
Процедура НастроитьВидимость(Команда)
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Для настройки видимости команд печати обработку необходимо записать. Продолжить?';uk='Для настройки видимості команд друку обробку необхідно записати. Продовжити?'");
...
Процедура ПродолжитьЗаписьНаКлиенте(Результат, ЗакрытьПослеЗаписи)  Экспорт
	...
	Если ПараметрыЗаписи.РегистрацияОбработки Тогда
		ОбновитьПовторноИспользуемыеЗначения();
		ТекстОповещения = НСтр("ru='Для применения изменений в открытых окнах необходимо их закрыть и открыть заново.';uk='Для застосування змін у відкритих вікнах необхідно їх закрити і відкрити заново.'");
...
Процедура ОбновлениеИзФайлаЗапуск()
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ПараметрыДиалога.Заголовок = НСтр("ru='Выберите файл внешнего отчета или обработки';uk='Виберіть файл зовнішнього звіту або обробки'");
	ИначеЕсли Объект.Вид = ВидДополнительныйОтчет Или Объект.Вид = ВидОтчет Тогда
		ПараметрыДиалога.ИндексФильтра = 1;
		ПараметрыДиалога.Заголовок = НСтр("ru='Выберите файл внешнего отчета';uk='Виберіть файл зовнішнього звіту'");
	Иначе
		ПараметрыДиалога.ИндексФильтра = 2;
		ПараметрыДиалога.Заголовок = НСтр("ru='Выберите файл внешней обработки';uk='Виберіть файл зовнішньої обробки'");
	КонецЕсли;
...
Процедура ОбновлениеИзФайлаПослеВыбораФайла(ПомещенныеФайлы, ПараметрыРегистрации) Экспорт
	...
	Иначе
		...
		ТекстПредупреждения = НСтр("ru='Расширение файла не соответствует расширению внешнего отчета (ERF) или обработки (EPF).';uk='Розширення файлу не відповідає розширенню зовнішнього звіту (ERF) або обробки (EPF).'");
...
Процедура ОбновлениеИзФайлаМеханикаНаКлиенте(ПараметрыРегистрации)
	...
	Если ПараметрыРегистрации.Успех Тогда
		ОповещениеЗаголовок = ?(ПараметрыРегистрации.ЭтоОтчет, НСтр("ru='Файл внешнего отчета загружен';uk='Файл зовнішнього звіту завантажений'"), НСтр("ru='Файл внешней обработки загружен';uk='Файл зовнішньої обробки завантажений'"));
...
Процедура ОбновлениеИзФайлаПоказатьКонфликты(ПараметрыРегистрации)
	Если ПараметрыРегистрации.КоличествоКонфликтующих > 1 Тогда
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = НСтр("ru='Конфликты при загрузке внешнего отчета';uk='Конфлікти при завантаженні зовнішнього звіту'");
            ...
            |;uk='Внутрішнє ім''я звіту ""[Name]""
            |вже зайнято існуючими додатковими звітами ([Count]): 
            |[List].
            |
            |Оберіть:
            |1. ""[Continue]"" - завантажити новий звіт в режимі налагодження.
            |2. ""[Disable]"" - завантажити новий звіт, відключивши публікацію всіх конфліктуючих звітів.
            |3. ""[Open]"" - відмінити завантаження і показати список конфліктуючих звітів.'");
		Иначе
			ЗаголовокВопроса = НСтр("ru='Конфликты при загрузке внешней обработки';uk='Конфлікти при завантаженні зовнішньої обробки'");
            ...
            |;uk='Внутрішнє ім''я обробки ""[Name]""
            |вже зайнято існуючими додатковими обробками ([Count]):
            |[List].
            |
            |Оберіть:
            |1. ""[Continue]"" - завантажити нову обробку в режимі налагодження.
            |2. ""[Disable]"" - завантажити нову обробку, відключивши публікацію всіх конфліктуючих обробок.
            |3. ""[Open]"" - відмінити завантаження і показати список конфліктуючих обробок.'");
		КонецЕсли;
		ПредставлениеКнопкиОтключить = НСтр("ru='Отключить конфликтующие';uk='Відключити конфліктуючі'");
		ПредставлениеКнопкиОткрыть = НСтр("ru='Отменить и показать список';uk='Скасувати й показати список'");
	Иначе
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = НСтр("ru='Конфликт при загрузке внешнего отчета';uk='Конфлікт при завантаженні зовнішнього звіту'");
			ТекстВопроса = НСтр("ru='Внутреннее имя отчета ""[Name]""
            ...
            |;uk='Внутрішнє ім''я звіту ""[Name]""
            |вже зайнято існуючим додатковим звітом [List].
            |
            |Оберіть:
            |1. ""[Continue]"" - завантажити новий звіт в режимі налагодження.
            |2. ""[Disable]"" - завантажити новий звіт, відключивши публікацію конфліктує звіту.
            |3. ""[Open]"" - відкрити картку звіту, що конфліктує.'");
			ПредставлениеКнопкиОтключить = НСтр("ru='Отключить другой отчет';uk='Відключити інший звіт'");
		Иначе
			ЗаголовокВопроса = НСтр("ru='Конфликт при загрузке внешней обработки';uk='Конфлікт при завантаженні зовнішньої обробки'");
			ТекстВопроса = НСтр("ru='Внутреннее имя обработки ""[Name]""
            ...
            |;uk='Внутрішнє ім''я обробки ""[Name]""
            |вже зайнято існуючої додатковою обробкою [List].
            |
            |Оберіть:
            |1. ""[Continue]"" - завантажити нову обробку у режимі налагодження.
            |2. ""[Disable]"" - завантажити нову обробку, відключивши публікацію конфліктуючої обробки.
            |3. ""[Open]"" - відкрити картку конфліктуючої обробки.'");
			ПредставлениеКнопкиОтключить = НСтр("ru='Отключить другую обработку';uk='Відключити іншу обробку'");
		КонецЕсли;
		ПредставлениеКнопкиОткрыть = НСтр("ru='Отменить и открыть';uk='Скасувати й відкрити'");
	КонецЕсли;
	ПредставлениеКнопкиПродолжить = НСтр("ru='В режиме отладки';uk='У режимі налагодження'");
...
Процедура ОбновлениеИзФайлаРазрешениеКонфликтов(Ответ, ПараметрыРегистрации) Экспорт
	...
	ИначеЕсли Ответ = "ОтменитьИОткрыть" Тогда
		...
		Если ПоказатьСписок Тогда // Форма списка с отбором по конфликтующим.
			...
			ФормаЗаголовок = НСтр("ru='Дополнительные отчеты и обработки с внутреннем именем ""%1""';uk='Додаткові звіти й обробки із внутрішньому ім''ям ""%1""'");
...
Процедура ОткрытьВариант()
	...
	Если НЕ ЗначениеЗаполнено(Вариант.Ссылка) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вариант отчета ""%1"" не зарегистрирован.';uk='Варіант звіту ""%1"" не зареєстрований.'"), Вариант.Наименование);
...
Процедура ИзменитьРегламентноеЗадание(РежимВыбора = Ложь, ИзмененФлажок = Ложь)
	...
	Если ЭлементКоманда.ВариантЗапуска <> ПредопределенноеЗначение("Перечисление.СпособыВызоваДополнительныхОбработок.ВызовСерверногоМетода")
        ...
        |;uk='Команда з варіантом запуску ""%1""
        |не може використовуватися в регламентних завданнях.'");
...
Функция ПредставлениеБыстрогоДоступаПользователей(КоличествоПользователей)
	Если КоличествоПользователей = 0 Тогда
		Возврат НСтр("ru='Нет';uk='Ні'");
	КонецЕсли;
	
	БыстрыйДоступПредставление = СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
		КоличествоПользователей,
		НСтр("ru='пользователь';uk='користувач'") + "," + НСтр("ru='пользователя';uk='користувача'") + "," + НСтр("ru='пользователей';uk='користувачів'"));
...
Процедура ОбновлениеИзФайлаМеханикаНаСервере(ПараметрыРегистрации)
	...
	Иначе
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокОшибки = НСтр("ru='Отчет не подключен';uk='Звіт не підключений'");
		Иначе
			ЗаголовокОшибки = НСтр("ru='Обработка не подключена';uk='Обробка не підключена'");
		КонецЕсли;
...
Функция ПодготовитьПараметрыФормыВыборОбъектовМетаданных()
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Назначение дополнительной обработки';uk='Призначення додаткової обробки'"));
...
Процедура УстановитьВидимостьДоступность(Регистрация = Ложь)
	...
	Если ЭтоОтчет И Объект.ИспользуетХранилищеВариантов Тогда
		...
		Если Регистрация ИЛИ КоличествоВариантов = 0 Тогда
			...
			Элементы.СтраницаВарианты.Заголовок = НСтр("ru='Варианты отчета';uk='Варіанти звіту'");
		Иначе
			...
			Элементы.СтраницаВарианты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Варианты отчета (%1)';uk='Варіанти звіту (%1)'"),
	...
	Иначе
		...
		Элементы.СтраницаРазрешения.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Разрешения (%1)';uk='Дозволу (%1)'"),
		...
		ИначеЕсли ТипЗнч(БезопасныйРежим) = Тип("Строка") Тогда
			...
			Элементы.ДекорацияПерсональныйПрофильБезопасностиНадпись.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Дополнительный отчет или обработка будет подключаться к программе с использованием ""персонального""
                    ...
                    |;uk='Додатковий звіт або обробка буде підключатися до програми з використанням ""персонального""
                    |профілю безпеки %1, в якому будуть дозволені лише такі операції:'"),
				БезопасныйРежим);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 не является корректным режимом подключения для дополнительных отчетов и обработок, требующих разрешений на использование профилей безопасности!';uk='%1 не є коректним режимом для підключення додаткових звітів і обробок, що потребують дозволів на використання профілів безпеці!'"),
				БезопасныйРежим);
		КонецЕсли;
	...
	Если ЭтоГлобальнаяОбработка Тогда
		ПредставлениеНазначения = НСтр("ru='<Не задано>';uk='<Не задано>'");
	...
	Иначе
		Если Объект.Назначение.Количество() = 0 Тогда
			ПредставлениеНазначения = НСтр("ru='Не определено';uk='Не визначено'");
		Иначе
			ПредставлениеНазначения = "";
	...
	Если ЭтоНовый Тогда
		Заголовок = ?(ЭтоОтчет, НСтр("ru='Дополнительный отчет (создание)';uk='Додатковий звіт (створення)'"), НСтр("ru='Дополнительная обработка (создание)';uk='Додаткова обробка (створення)'"));
	Иначе
		Заголовок = Объект.Наименование + " " + ?(ЭтоОтчет, НСтр("ru='(Дополнительный отчет)';uk='(Додатковий звіт)'"), НСтр("ru='(Дополнительная обработка)';uk='(Додаткова обробка)'"));
	КонецЕсли;
	...
	Для Каждого ЭлементКоманда Из Объект.Команды Цикл
			...
			Если Не ПустаяСтрока(ЭлементКоманда.ЗаменяемыеКоманды) Тогда
				...
				Если КоличествоЗаменяемых > 0 Тогда
					Если КоличествоЗаменяемых = 1 Тогда
						ШаблонКомментария = НСтр("ru='Заменяет стандартную команду печати %1';uk='Замінює стандартну команду друку %1'");
					Иначе
						ШаблонКомментария = НСтр("ru='Заменяет стандартные команды печати: %1';uk='Замінює стандартні команди друку: %1'");
					КонецЕсли;
		...
		Иначе
			ЭлементКоманда.РегламентноеЗаданиеПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не применимо для команд с вариантом запуска ""%1""';uk='Не застосовно для команд із варіантом запуску ""%1""'"),
...
Процедура ВариантыДополнительногоОтчетаЗаполнить()
	...
	Исключение
		ТекстОшибки = НСтр("ru='Не удалось получить список вариантов отчета из-за ошибки, возникшей при подключении этого отчета:';uk='Не вдалося одержати список варіантів звіту через помилку, що виникла при підключенні цього звіту:'");
...
Функция НазваниеСтраницыКоманд()
	Если Объект.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма Тогда
		Возврат НСтр("ru='Команды печати';uk='Команди друку'");
	Иначе
		Возврат НСтр("ru='Команды';uk='Команди'");
	КонецЕсли;
КонецФункции
