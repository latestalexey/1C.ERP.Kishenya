	...
	Если НЕ ДополнительныеОтчетыИОбработки.ПроверитьГлобальнаяОбработка(Вид) Тогда
		Если НЕ ИспользоватьДляФормыОбъекта И НЕ ИспользоватьДляФормыСписка 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Необходимо отключить публикацию или выбрать для использования как минимум одну из форм';uk='Необхідно відключити публікацію або вибрати для використання як мінімум одну з форм'")
				,
				,
				,
		...
		Если Конфликтующие.Количество() > 0 Тогда
			...
			Если ПроверкаЭлемента Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                    ...
                    |;uk='Ім''я ""%1"", використовуване даним звітом (обробкою), уже зайнято іншим опублікованим додатковим звітом (обробкою). 
                    |
                    |Для продовження необхідно змінити вид Публікації з ""%2"" на ""%3"" або ""%4"".'"),
					ИмяОбъекта,
			...
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Имя ""%1"", используемое отчетом (обработкой) ""%2"", уже занято другим опубликованным дополнительным отчетом (обработкой).';uk='Ім''я ""%1"", яке використовується звітом (обробкою) ""%2"", вже зайняте іншим опублікованим додатковим звітом (обробкою).'"),
					ИмяОбъекта,
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтоНовый() И НЕ ДополнительныеОтчетыИОбработки.ПравоДобавления(ЭтотОбъект) Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав для добавления дополнительных отчетов или обработок.';uk='Недостатньо прав для додавання додаткових звітів або обробок.'");
	КонецЕсли;
	
	// Предварительные проверки
	Если НЕ ЭтоНовый() И Вид <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "Вид") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Невозможно сменить вид существующего дополнительного отчета или обработки.';uk='Неможливо перемінити вид існуючого додаткового звіту або обробки.'"),,,,
			Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ПриЗаписиОтчета(Отказ)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			ТекстОшибки = НСтр("ru='Ошибка подключения:';uk='Помилка підключення:'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
