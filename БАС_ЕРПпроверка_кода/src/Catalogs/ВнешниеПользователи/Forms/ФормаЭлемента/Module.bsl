	   ...
	   И ПользовательИБРоли.Количество() = 0 Тогда
		...
		Если НЕ ПараметрыЗаписи.Свойство("СПустымСпискомРолей") Тогда
			...
			ПоказатьВопрос(
				...
				,
				,
				НСтр("ru='Запись пользователя информационной базы';uk='Запис користувача інформаційної бази'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль ""%1"" в строке %%1 недоступна для внешних пользователей.';uk='Роль ""%1"" в рядку %%1 недоступна для зовнішніх користувачів.'"), Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
	...
	Если Объект.ПометкаУдаления И ВходВПрограммуРазрешен Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("ru='Чтобы разрешить вход в программу, требуется снять
                       ...
                       |позначку на вилучення із цього зовнішнього користувача.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	ИначеЕсли УровеньДоступа.УправлениеСписком Тогда
		...
		Если ПользовательИБАутентификацияОС <> Ложь Тогда
			УточнениеСвойств.Добавить(НСтр("ru='Аутентификация ОС (включена)';uk='Аутентифікація ОС (включена)'"));
		КонецЕсли;
		...
		Если ПользовательИБПользовательОС <> "" Тогда
			УточнениеСвойств.Добавить(НСтр("ru='Пользователь ОС (указан)';uk='Користувач ОС (зазначений)'"));
		КонецЕсли;
		
		Если ПользовательИБПоказыватьВСпискеВыбора Тогда
			ЕстьРазличияУстранимыеБезАдминистратора = Истина;
			УточнениеСвойств.Добавить(НСтр("ru='Показывать в списке выбора (включено)';uk='Показувати в списку вибору (включене)'"));
		КонецЕсли;
		
		Если ПользовательИБРежимЗапуска <> "Авто" Тогда
			ЕстьРазличияУстранимыеБезАдминистратора = Истина;
			УточнениеСвойств.Добавить(НСтр("ru='Режим запуска (не Авто)';uk='Режим запуску (не Авто)'"));
		КонецЕсли;
