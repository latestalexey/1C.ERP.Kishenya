...
Процедура СтатусОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = ПредопределенноеЗначение("Перечисление.СтатусыВидовКартЛояльности.Закрыт") Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("СтатусОбработкаВыбораЗавершение", ЭтотОбъект,
			...
			НСтр("ru='Действующие карты лояльности будут аннулированы, продолжить?';uk='Діючі карти лояльності будуть анульовані, продовжити?'"), РежимДиалогаВопрос.ДаНет);
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ТипКода = Перечисления.ТипыКодовКарт.МагнитныйКод Тогда
				...
				Если Выборка.ВидКарты <> Объект.Ссылка Тогда
					ТекстСообщения = НСтр("ru='Диапазон магнитных кодов %2 - %3 в строке %1 пересекается с диапазоном магнитных кодов карт ""%5"" %6 - %7 в строке %4';uk='Діапазон магнітних кодів %2 - %3 в рядку %1 перетинається з діапазоном магнітних кодів карт ""%5"" %6 - %7 у рядку %4'");
				Иначе
					Если Выборка.НомерСтроки > СтрокаТЧ.НомерСтроки Тогда
						Продолжить;
					КонецЕсли;
					ТекстСообщения = НСтр("ru='Диапазон магнитных кодов %2 - %3 в строке %1 пересекается с диапазоном магнитных кодов %6 - %7 в строке %4';uk='Діапазон магнітних кодів %2 - %3 в рядку %1 перетинається з діапазоном магнітних кодів %6 - %7 у рядку %4'");
				КонецЕсли;
			...
			Иначе
				...
				Если Выборка.ВидКарты <> Объект.Ссылка Тогда
					ТекстСообщения = НСтр("ru='Диапазон штрихкодов %2 - %3 в строке %1 пересекается с диапазоном штрихкодов карт ""%5"" %6 - %7 в строке %4';uk='Діапазон штрихкодів %2 - %3 в рядку %1 перетинається з діапазоном штрихкодів карт ""%5"" %6 - %7 у рядку %4'");
				Иначе
					Если Выборка.НомерСтроки > СтрокаТЧ.НомерСтроки Тогда
						Продолжить;
					КонецЕсли;
					ТекстСообщения = НСтр("ru='Диапазон штрихкодов %2 - %3 в строке %1 пересекается с диапазоном штрихкодов %6 - %7 в строке %4';uk='Діапазон штрихкодів %2 - %3 в рядку %1 перетинається з діапазоном штрихкодів %6 - %7 у рядку %4'");
				КонецЕсли;
...
Процедура СформироватьОписаниеНастроек()
	...
	Если Объект.Персонализирована И Объект.АвтоматическаяРегистрацияПриПервомСчитывании Тогда
		ОписаниеНастроек = НСтр("ru='Карты данного вида могут быть накопительными
                                      ...
                                      |;uk='Карти цього виду можуть бути накопичувальними
                                      |і активуються при першому зчитуванні в документі продажу або за допомогою помічника реєстрації'");
	ИначеЕсли Объект.Персонализирована И НЕ Объект.АвтоматическаяРегистрацияПриПервомСчитывании Тогда
		ОписаниеНастроек = НСтр("ru='Карты данного вида могут быть накопительными
                                      ...
                                      |;uk='Карти цього виду можуть бути накопичувальними
                                      |і активуються за допомогою помічника реєстрації карт лояльності.'");
	ИначеЕсли Не Объект.Персонализирована И Объект.АвтоматическаяРегистрацияПриПервомСчитывании Тогда
		ОписаниеНастроек = НСтр("ru='Карты данного вида не могут быть накопительными
                                      |и активируются при первом считывании в документе продажи или с помощью помощника регистрации'
                                      |;uk='Карти даного виду не можуть бути накопичувальними
                                      |і активуються при першому зчитуванні в документі продажу або за допомогою помічника реєстрації'");
	ИначеЕсли Не Объект.Персонализирована И Не Объект.АвтоматическаяРегистрацияПриПервомСчитывании Тогда
		ОписаниеНастроек = НСтр("ru='Карты данного вида не могут быть накопительными
                                      |и активируются с помощью помощника регистрации карт лояльности.'
                                      |;uk='Картк цього виду не можуть бути накопичувальними
                                      |і активуються за допомогою помічника реєстрації карт лояльності.'");
	КонецЕсли;
