...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если УдалятьСозданныеСообщения И Объект.СрокХраненияСообщений = 0 Тогда
		ТекстСообщения = НСтр("ru='Не указан срок хранения сообщений.';uk='Не зазначений строк зберігання повідомлень.'");
...
Процедура УправлениеВидимостью()
	...
	Элементы.ГруппаSMSНеНастроены.Видимость           = ПолучитьФункциональнуюОпцию("ИспользоватьПрочиеВзаимодействия") 
...
Функция ТекстПоясненияТипРассылки(Принудительная)
	...
	Если НЕ Принудительная Тогда
		ТекстПояснения = НСтр("ru='Рассылки и оповещения будут отправляться согласно настройкам подписок. Виды контактной информации,
                                     ...
                                     |;uk='Розсилки і оповіщення будуть відправлятися згідно настройок підписок. Види контактної інформації,
                                     |зазначені у цій формі будуть використані як значення за замовчуванням і можуть бути 
                                     |змінені при оформленні підписки.'");
	Иначе
		ТекстПояснения = НСтр("ru='Рассылки и оповещения будут отправляться принудительно. Настройка подписок невозможна. В данной форме
                                     ...
                                     |;uk='Розсилки і оповіщення будуть відправлятися примусово. Настройка підписок неможлива. В даній формі
                                     |необхідно налаштувати, хто буде одержувачем повідомлень і види контактної інформації, згідно
                                     |яким будуть визначатися адреси електронної пошти та номери телефонів адресатів.'");
	КонецЕсли;
...
Процедура УправлениеДоступностью(Форма)
	...
	Если Форма.УчетнаяЗаписьНедоступнаДляОтправки Тогда
...
Процедура УстановитьЗаголовки()
	...
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	            ЭтотОбъект, "ОтправлятьПартнеру", НСтр("ru='на контактную информацию контрагента';uk='на контактну інформацію контрагента'"), ИспользоватьПартнеровКакКонтрагентов);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	            ЭтотОбъект, "ВидКонтактнойИнформацииПартнераДляПисем", НСтр("ru='для контрагента';uk='для контрагента'"), ИспользоватьПартнеровКакКонтрагентов);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	            ЭтотОбъект, "ВидКонтактнойИнформацииПартнераДляSMS", НСтр("ru='для контрагента';uk='для контрагента'"), ИспользоватьПартнеровКакКонтрагентов);
