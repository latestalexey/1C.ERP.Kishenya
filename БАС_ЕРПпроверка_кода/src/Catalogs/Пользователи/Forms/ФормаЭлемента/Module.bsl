			...
			Если Объект.Ссылка.Пустая() Тогда
				ВызватьИсключение
					НСтр("ru='В демонстрационном режиме не поддерживается
                               ...
                               |;uk='У демонстраційному режимі не підтримується
		   ...
		   И ПользовательИБРоли.Количество() = 0 Тогда
			...
			Если НЕ ПараметрыЗаписи.Свойство("СПустымСпискомРолей") Тогда
				...
				ПоказатьВопрос(
					...
					НСтр("ru='Пользователю информационной базы не установлено ни одной роли. Продолжить?';uk='Користувачеві інформаційної бази не встановлене ні однієї ролі. Продовжити?'"),
	...
	Если ТребуетсяЗаписьПользователяИБ(ЭтотОбъект) Тогда
		...
		Если ПараметрыЗаписи.Свойство("ССозданиемПервогоАдминистратора") Тогда
			ТекущийОбъект.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
				НСтр("ru='Первый пользователь информационной базы назначается администратором.';uk='Перший користувач інформаційної бази призначається адміністратором.'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ВходВПрограммуРазрешен
	   ...
	   И СрокДействия <= НачалоДня(ТекущаяДатаСеанса()) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Ограничение должно быть до завтра или более.';uk='Обмеження має бути до завтра або більше.'"),, "ВходВПрограммуРазрешен",, Отказ);
	КонецЕсли;
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНесуществующаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль ""%1"" в строке %%1 не найдена в метаданных.';uk='Роль ""%1"" в рядку %%1 не знайдено метаданих.'"), Строка.Синоним));
			КонецЕсли;
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль ""%1"" недоступна для пользователей.';uk='Роль ""%1"" недоступна для користувачів.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль ""%1"" в строке %%1 недоступна для пользователей.';uk='Роль ""%1"" в рядку %%1 недоступна для користувачів.'"), Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
...
Процедура ВходВПрограммуРазрешенПриИзменении(Элемент)
	...
	Если Объект.ПометкаУдаления И ВходВПрограммуРазрешен Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("ru='Чтобы разрешить вход в программу, требуется снять
                       ...
                       |;uk='Щоб дозволити вхід у програму, потрібно зняти
                       |позначку вилучення із цього користувача.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не УровеньДоступа.ИзменениеРазрешенияНаВход
	   И Не ВходВПрограммуРазрешен Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("ru='После записи вход в программу сможет разрешить только администратор.';uk='Після запису вхід у програму зможе дозволити тільки адміністратор.'"));
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("ru='ПолныеПрава';uk='ПолныеПрава'");
...
Процедура ПроверитьЗаполнениеEmail(Отказ)
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
		НСтр("ru='Не заполнен адрес электронной почты';uk='Не заповнена адреса електронної пошти'"),,
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	ИначеЕсли УровеньДоступа.УправлениеСписком Тогда
		...
		Если ПользовательИБПолноеИмя <> Объект.Наименование Тогда
			...
			УточнениеСвойств.Вставить(0, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Полное имя ""%1""';uk='Повне ім''я ""%1""'"),
		...
		Если УточнениеСвойств.Количество() > 0 Тогда
			...
			Если ПоказатьКомандыУстраненияРазличий Тогда
				Рекомендация = Символы.ПС
					+ НСтр("ru='Нажмите ""Записать"", чтобы устранить различия и не выводить это предупреждение.';uk='Натисніть ""Записати"", щоб усунути розходження й не виводити це попередження.'");
			
			ИначеЕсли Не Пользователи.ЭтоПолноправныйПользователь() Тогда
				Рекомендация = Символы.ПС
					+ НСтр("ru='Обратитесь к администратору, чтобы устранить различия.';uk='Зверніться до адміністратора, щоб усунути розходження.'");
			Иначе
				Рекомендация = "";
			КонецЕсли;
			Элементы.СвойстваПояснениеНесоответствия.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Следующие свойства пользователя информационной базы отличаются от указанных в этой форме:
                           |%1.'
                           |;uk='Наступні властивості користувача інформаційної бази відрізняються від зазначених у цій формі:
                           | %1.'"),
	...
	Если ДействияВФорме.СвойстваЭлемента = "Редактирование" Тогда
		Рекомендация = Символы.ПС
			+ НСтр("ru='Нажмите ""Записать"", чтобы устранить проблему и не выводить это предупреждение.';uk='Натисніть кнопку ""Записати"", щоб вирішити проблему і не виводити це попередження.'");
	...
	Элементы.СопоставлениеПояснениеНесоответствия.Заголовок =
		НСтр("ru='Пользователь информационной базы не найден.';uk='Користувач інформаційної бази не знайдено.'") + Рекомендация;
