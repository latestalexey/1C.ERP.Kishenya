...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого Строка Из ПериодыОтсутствия Цикл
		...
		Если Не ЗначениеЗаполнено(Строка.Начало) Тогда
			ТекстСообщения = НСтр("ru='Дата начала не заполнена.';uk='Дата початку не заповнена.'");
		...
		Если Не ЗначениеЗаполнено(Строка.Окончание) Тогда
			ТекстСообщения = НСтр("ru='Дата окончания не заполнена.';uk='Дата закінчення не заповнена.'");
		...
		Если ЗначениеЗаполнено(ПредыдущееОкончание) Тогда
			Если Строка.Начало <= ПредыдущееОкончание Тогда
				ТекстСообщения = НСтр("ru='Периоды отсутствия пересекаются.';uk='Періоди відсутності перетинаються.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПериодыОтсутствия[" + ПериодыОтсутствия.Индекс(Строка) + "].Начало", , Отказ);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПериодыОтсутствияПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Годы.Найти(Год(ДанныеСтроки.Начало)) = Неопределено Или Годы.Найти(Год(ДанныеСтроки.Окончание)) = Неопределено  Тогда
		ТекстСообщения = НСтр("ru='Введенные даты не относятся к выбранным годам среднего заработка.';uk='Введені дати не відносяться до вибраних років середнього заробітку.'");
	...
	Если МесяцыКорректировки.Найти(НачалоМесяца(ДанныеСтроки.Начало)) = Неопределено
		...
		ТекстСообщения = НСтр("ru='Период относится к месяцу, редактирование данных которого недоступно.';uk='Період відноситься до місяця, редагування даних якого недоступно.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , , , Отказ);
		Возврат;
	КонецЕсли;
	
	Если ДанныеСтроки.Начало > ДанныеСтроки.Окончание  Тогда
		ТекстСообщения = НСтр("ru='Начало позже окончания.';uk='Початок пізніше закінчення.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , , , Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьИнформациюДоступностиПериодов()
	...
	Если МесяцыКорректировки.Количество() = 0 Тогда
		Элементы.ПериодыОтсутствия.Подсказка = НСтр("ru='Редактирование периодов недоступно.';uk='Редагування періодів недоступно.'");
	...
	Элементы.ПериодыОтсутствия.Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Редактирование периодов доступно с %1 по %2.';uk='Редагування періодів доступне з %1 по %2.'"), Формат(МинимальнаяДата, "ДЛФ=D"), Формат(КонецМесяца(МаксимальнаяДата), "ДЛФ=D"));
