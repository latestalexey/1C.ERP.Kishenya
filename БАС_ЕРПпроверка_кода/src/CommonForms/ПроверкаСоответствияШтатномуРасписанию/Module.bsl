...
Процедура НастроитьВидимостьЭлементов()
	...
	Если НесколькоПроблем Тогда
		...
		Если ПроблемыТекущегоДокумента Тогда
			ТекстНадписи = НСтр("ru='Ниже приведен список сотрудников и позиций, которые не соответствуют штатному расписанию.';uk='Нижче наведено список співробітників і позицій, які не відповідають штатним розкладом.'");
			Если ПроверкаСоответствияГрейдам Тогда 
				ТекстНадписи = НСтр("ru='Ниже приведен список сотрудников, начисления которых не соответствуют установленным для грейдов ограничениям.';uk='Нижче наведено список співробітників, нарахування яких не відповідають установленим для грейдів обмежень.'");
			КонецЕсли;	
		Иначе
			ТекстНадписи = НСтр("ru='Ниже приведен список документов, которые не будут соответствовать штатному расписанию. Используйте правую кнопку мыши для редактирования.';uk='Нижче наведено перелік документів, які не будуть відповідати штатним розкладом. Використовуйте праву кнопку миші для редагування.'");
		КонецЕсли;
