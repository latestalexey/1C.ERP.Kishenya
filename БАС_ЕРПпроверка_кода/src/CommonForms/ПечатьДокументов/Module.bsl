...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.ИсточникДанных) Тогда 
		ОбщегоНазначенияКлиентСервер.Проверить(ТипЗнч(Параметры.ПараметрКоманды) = Тип("Массив") Или ОбщегоНазначения.ЗначениеСсылочногоТипа(Параметры.ПараметрКоманды),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Недопустимое значение параметра ПараметрКоманды при вызове метода УправлениеПечатьюКлиент.ВыполнитьКомандуПечати.
                ...
                |Передано: %1'
                |;uk='Неприпустиме значення параметра ПараметрКоманды при виклику методу УправлениеПечатьюКлиент.ВыполнитьКомандуПечати.
                |Очікувалося: Масив, ЛюбаяСсылка.
	...
	Если ЭтоПечатьКомплекта() Тогда
		Элементы.Копий.Заголовок = НСтр("ru='Копий комплекта';uk='Копій комплекту'");
	КонецЕсли;
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("ОбщаяФорма.СохранениеПечатнойФормы") Тогда
		...
		Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение <> КодВозвратаДиалога.Отмена Тогда
			...
			Иначе
				...
				КонецЦикла;
				Состояние(НСтр("ru='Сохранение успешно завершено.';uk='Збереження успішно завершене.'"));
			КонецЕсли;
		КонецЕсли;
...
&НаКлиенте
Процедура КодЯзыкаДляМногоязычныхПечатныхФормПриИзменении(Элемент)
...
Процедура ПерейтиКДокументу(Команда)
	...
	СписокВыбора.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ru='Перейти к печатной форме';uk='Перейти до друкованої форми'"));
...
Функция СформироватьПечатныеФормы(ИменаМакетов, Отказ)
	...
	Если ПустаяСтрока(КодЯзыкаДляМногоязычныхПечатныхФорм) Тогда
		КодЯзыкаДляМногоязычныхПечатныхФорм = Локализация.ПолучитьЯзыкФормированияПечатныхФорм();
	КонецЕсли;
	...
	Если ПараметрыВывода.Свойство("КодЯзыкаДляМногоязычныхПечатныхФорм") Тогда
		ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм = КодЯзыкаДляМногоязычныхПечатныхФорм;
	Иначе
		ПараметрыВывода.Вставить("КодЯзыкаДляМногоязычныхПечатныхФорм",	КодЯзыкаДляМногоязычныхПечатныхФорм);
	КонецЕсли;
	...
	Иначе
		...
		ПечатныеФормы = УправлениеПечатью.СформироватьПечатныеФормы(
			Параметры.ИмяМенеджераПечати, 
			ИменаМакетов,
			Параметры.ПараметрКоманды, 
			Параметры.ПараметрыПечати.ДополнительныеПараметры, 
			ПараметрыВывода,
			ТипыОбъектовПечати
		);
	...
	Элементы.КодЯзыкаДляМногоязычныхПечатныхФорм.Видимость      = (Результат.Найти(Истина, "ЭтоМногоязычнаяПечатнаяФорма") <> Неопределено);
	Элементы.КодЯзыкаДляМногоязычныхПечатныхФорм.ТолькоПросмотр = Не Пользователи.РолиДоступны("ФормированиеПечатныхФормНаВсехДоступныхЯзыках");
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		Если НастройкаПечатнойФормы.Количество <> 1 Тогда
			НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление 
				+ " (" + НастройкаПечатнойФормы.Количество + " " + НСтр("ru='экз.';uk='екз.'") + ")";
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
	...
	Если ЕстьРазрешенныйВывод Тогда
		Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
			...
			Если ПолеТабличногоДокумента.Вывод = ИспользованиеВывода.Запретить Тогда
				НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление + " (" + НСтр("ru='вывод не доступен';uk='виведення не доступне'") + ")";
			ИначеЕсли ПолеТабличногоДокумента.Защита Тогда
				НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление + " (" + НСтр("ru='только печать';uk='тільки друк'") + ")";
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;	
КонецПроцедуры
...
Процедура УстановитьИмяПринтераВПодсказкеКнопкиПечать()
	Если НастройкиПечатныхФорм.Количество() > 0 Тогда
		...
		Если Не ПустаяСтрока(ИмяПринтера) Тогда
			ЭтотОбъект.Команды["Печать"].Подсказка = НСтр("ru='Напечатать на принтере';uk='Надрукувати на принтері'") + " (" + ИмяПринтера + ")";
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьЗаголовокФормы()
	...
	Иначе
		Если ЭтоПечатьКомплекта() Тогда
			Заголовок = НСтр("ru='Печать комплекта';uk='Друк комплекту'");
		ИначеЕсли ТипЗнч(Параметры.ПараметрКоманды) <> Тип("Массив") Или Параметры.ПараметрКоманды.Количество() > 1 Тогда
			Заголовок = НСтр("ru='Печать документов';uk='Друк документів'");
		Иначе
			Заголовок = НСтр("ru='Печать документа';uk='Друк документу'");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьИмяФайлаДляАрхива()
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		...
		Иначе
			Результат = НСтр("ru='Документы';uk='Документи'");
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура СохранитьПечатныеФормыВПапку(СписокФайловВоВременномХранилище, Знач Папка = "")
	...
	Состояние(НСтр("ru='Сохранение успешно завершено';uk='Збереження успішно завершене'"), , НСтр("ru='в папку:';uk='у папку:'") + " " + Папка);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрисоединенныеФайлы") Тогда
		...
		Для Каждого Файл Из ФайлыВоВременномХранилище Цикл
			...
			Результат.Добавить(МодульПрисоединенныеФайлы.ДобавитьПрисоединенныйФайл(
				ПараметрыФайла, Файл.АдресВоВременномХранилище, , НСтр("ru='Печатная форма';uk='Друкована форма'")));
		КонецЦикла;
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Процедура ОткрытьМакетДляРедактирования()
	...
	ОтобразитьСостояниеТекущейПечатнойФормы(НСтр("ru='Макет редактируется';uk='Макет редагується'"));
...
Процедура ПереформироватьПечатнуюФорму(ИмяМакета, ИмяРеквизита)
	...
	Отказ = Ложь;
    Если Параметры.ИменаМакетов = "КомплектДокументов" 
	    И ИмяМакета <> "КомплектДокументов"Тогда
		//Если это обновление одной из форм комплекта 
		//Временно переопределим коллекцию на коллекцию с текущей формой
		АдресКомплектаПечатныхФорм = Параметры.ПараметрыПечати.ДополнительныеПараметры.АдресКомплектаПечатныхФорм;
		...
		Если ТекущийКомплект.Количество() > 0 Тогда
			ТекущийАдресКомплектаПечатныхФорм = ПоместитьВоВременноеХранилище(ТекущийКомплект, ЭтотОбъект.УникальныйИдентификатор);	
			Параметры.ПараметрыПечати.ДополнительныеПараметры.АдресКомплектаПечатныхФорм = ТекущийАдресКомплектаПечатныхФорм;
	        КоллекцияПечатныхФорм = СформироватьПечатныеФормы("КомплектДокументов", Отказ);
			Параметры.ПараметрыПечати.ДополнительныеПараметры.АдресКомплектаПечатныхФорм = АдресКомплектаПечатныхФорм;
			Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
				Если ПечатнаяФорма.ИмяМакета = "КомплектДокументов" Тогда
					ПечатнаяФорма.ИмяМакета = ИмяМакета;
				КонецЕсли;
			КонецЦикла;
		Иначе
			ВызватьИсключение "Печатная форма "+ИмяМакета+" не найдена!";	
		КонецЕсли;
    Иначе
		КоллекцияПечатныхФорм = СформироватьПечатныеФормы(ИмяМакета, Отказ);
	КонецЕсли; 
	Если Отказ Тогда
		ВызватьИсключение НСтр("ru='Печатная форма не была переформирована.';uk='Друкована форма не була переформована.'");
	КонецЕсли;
