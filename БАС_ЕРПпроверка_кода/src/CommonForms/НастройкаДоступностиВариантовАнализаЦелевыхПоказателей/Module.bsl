...
Процедура ЗаписатьНастройкиСервер()
	...
	Для Каждого ВариантАнализа Из ВариантыАнализаСохраненные Цикл 
		
		ОтборПоВариантуАнализа = Новый Структура("ВариантАнализа", ВариантАнализа.ВариантАнализа);
		ЗаписываемыеНастройки = НастройкиДоступностиЗначение.НайтиСтроки(ОтборПоВариантуАнализа);
		
		ВариантАнализаОбъект = ВариантАнализа.ВариантАнализа.ПолучитьОбъект();
...
&НаКлиенте 
Процедура ЗаполнитьВыбранныеВариантыАнализа(ИдентификаторСтроки = Неопределено, ВключаяНедоступные = Ложь, ПользователиПодразделения)
	Если ИдентификаторСтроки = Неопределено Тогда
		// Обработка всех
		КоллекцияВариантовАнализа = ДеревоВариантовАнализа.ПолучитьЭлементы();
	Иначе
	...
	Для каждого ЭлементКоллекцииВариантовАнализа Из КоллекцияВариантовАнализа Цикл
		...
		Если ЭлементКоллекцииВариантовАнализа.ТипСтроки = 0 Тогда
			ЗаполнитьВыбранныеВариантыАнализа(ИдентификаторЭлемента, ВключаяНедоступные, ПользователиПодразделения);
...
Функция ПолучитьВариантыАнализа(ЗНАЧ ПараметрыОтбора = Неопределено, РезультатВДерево = Истина, ВсеВариантыАнализа = Ложь)
	...
	Для Каждого НастройкаДоступностиЗначение Из НастройкиДоступностиЗначение Цикл 
		НастройкаДоступностиЗначение.Подразделение = НастройкаДоступностиЗначение.Пользователь.Подразделение;
	...
	Если ИспользоватьНесохраненныеНастройки Тогда
		...
		|	НесохраненныеНастройкиДоступности.Подразделение КАК Подразделение,
