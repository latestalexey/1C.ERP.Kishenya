...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Валюта") Тогда
		РежимРаботы = 2;
		Заголовок = НСтр("ru='Способ определения валюты';uk='Спосіб визначення валюти'");
	ИначеЕсли Параметры.Свойство("Количество") Тогда
		РежимРаботы = 1;
		Заголовок = НСтр("ru='Способ определения ед. измерения';uk='Спосіб визначення од. виміру'");
	Иначе
		ВызватьИсключение НСтр("ru='Неизвестный режим работы';uk='Невідомий режим роботи'");
	КонецЕсли;
...
Процедура ОбработатьВыбор(ВыбраннаяСтрока)
	...
	Если Не СтрокаПодходитДляВыбора Тогда
		ПоказатьПредупреждение(, НСтр("ru='Строка не может быть выбрана!';uk='Рядок не може бути вибраний!'"));
		Возврат;
	КонецЕсли;
