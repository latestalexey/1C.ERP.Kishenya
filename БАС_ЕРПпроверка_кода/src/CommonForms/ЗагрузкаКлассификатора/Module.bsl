...
Процедура ПрочитатьФайлВДерево(Знач АдресФайла)
	...
	Исключение
		ШаблонСообщения = НСтр("ru='Ошибка при открытии файла данных справочника %1';uk='Помилка при відкритті файлу даних довідника %1'");
	...
	Если НЕ ОшибкаПриОткрытииФайла Тогда
		...
		Исключение
			ШаблонСообщения = НСтр("ru='Файла данных справочника %1 поврежден';uk='Файлу даних довідника %1 пошкоджений'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КраткоеИмяКлассификатора);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			ОшибкаПриОткрытииФайла = Истина;
			ОписаниеОшибки = ИнформацияОбОшибке();
	...
	Пока ВозможностьЧтенияXML(ФайлЧтение) Цикл
		...
		Если ТипXML.ИмяТипа = ИмяТипаXML Тогда
			...
			ДобавитьВДерево(ДеревоПолное, Ссылка, ЭтоГруппа, Родитель, Код, Наименование, ЗначенияДополнительныхПолей);
			Если ЭтоГруппа Тогда
				ДобавитьВДерево(ДеревоГруппОбъект, Ссылка, ЭтоГруппа, Родитель, Код, Наименование, ЗначенияДополнительныхПолей, Ложь);
			КонецЕсли;
			
			// Проверяем, что текущим узлом является КонецЭлемента
			Если ФайлЧтение.ТипУзла <> ТипУзлаXML.КонецЭлемента Тогда
				ТекстСообщения = НСтр("ru='Ошибка чтения XML';uk='Помилка читання XML'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				Возврат;
			КонецЕсли;
			...
			ФайлЧтение.Прочитать();
		Иначе
			ШаблонСообщения = НСтр("ru='Файл данных не является справочником %1';uk='Файл даних не є довідником %1'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КраткоеИмяКлассификатора);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			ОшибкаПриОткрытииФайла = Истина;
			Возврат;
...
//
&НаСервере
Процедура ДобавитьВДерево(Дерево, Ссылка, ЭтоГруппа, Родитель, Код, Наименование, ЗначенияДополнительныхПолей, ХранитьРеквизиты = Истина);
	...
	Если ХранитьРеквизиты Тогда
		...
		Если НЕ ЭтоГруппа ИЛИ ДополнительныеПоляДляГрупп Тогда
			Для Каждого КлючИЗначение Из ЗначенияДополнительныхПолей Цикл
	...
	Для Каждого ТекСтрока Из Строки Цикл
		...
		Если ТекСтрока.Строки.Количество() = 0 
			...
			Для Каждого ТекСтрокаПолногоДерева Из СтрокаПолногоДерева.Строки Цикл
				...
				Если ТекСтрока.Пометка > 0 И 
					(Не ТекСтрокаПолногоДерева.ЭтоГруппа 
	...
	Для Индекс = НачальныйНомер По КонечныйНомер - 1 Цикл
		...
		Если ЗначениеЗаполнено(СтрокаЭлемента.КодРодителя) Тогда
			Если НЕ СтрокаЭлемента.РодительЭтоГруппа Тогда
				ШаблонСообщения = НСтр("ru='Невозможно создать / обновить элемент с кодом %1 т.к. в качестве родителя у него указан элемент (%2)';uk='Неможливо створити / оновити елемент з кодом %1 т. к. в якоссті батьківського у нього вказаний елемент (%2)'");
		...
		Иначе
			...
			// ничего не делаем
			ШаблонСообщения = НСтр("ru='Элемент с кодом %1 невозможно обновить';uk='Елемент з кодом %1 неможливо оновити'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СтрокаЭлемента.Код);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Продолжить;
		КонецЕсли;
		...
		Если Объект <> Неопределено тогда
			...
			Исключение
				ШаблонСообщения = НСтр("ru='Ошибка при записи элемента с кодом %1
                                             |%2'
                                             |;uk='Помилка при запису елемента з кодом %1
                                             |%2'");
...
Процедура ОткрытьИПрочитатьФайлЗавершение(Результат, АдресФайла, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		ШаблонСообщения = НСтр("ru='Выполняется считывание данных классификатора %1 из файла';uk='Виконується зчитування даних класифікатора %1 з файлу'");
...
Процедура ОткрытьИПрочитатьФайл(Команда)
	...
	Попытка
		...
		НачатьПомещениеФайла(Оповещение, АдресФайла,  ИмяФайлаПоУмолчанию, Истина)
	Исключение
		...
		ПоказатьПредупреждение( , ТекстСообщения, , НСтр("ru='Ошибка';uk='Помилка'"));
...
Процедура ЗагрузитьДанные(Команда)
	...
	Если ДеревоГрупп.ПолучитьЭлементы().Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='Не загружены данные из файла';uk='Не завантажені дані з файлу'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ДеревоГрупп");
		Возврат;
	КонецЕсли;
	
	Состояние(НСтр("ru='Выполняется построение списка загружаемых объектов';uk='Виконується побудова списку завантажуваних об''єктів'"));
	
	КоличествоЭлементов = ПостроитьТаблицуЭлементов();
	
	Если КоличествоЭлементов = 0 Тогда
		ТекстСообщения = НСтр("ru='Не выбрано элементов для загрузки';uk='Не вибрано елементів для завантаження'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ДеревоГрупп");
		Возврат;
	КонецЕсли;
	...
	Для Прогресс = 1 По 100 Цикл
		...
		Если НовыйТекущийНомер > ТекущийНомер Тогда
			ШаблонСообщения = НСтр("ru='Выполняется загрузка классификатора %1';uk='Виконується завантаження класифікатора %1'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КраткоеИмяКлассификатора);
	...
	ШаблонСообщения = НСтр("ru='Загрузка классификатора %1 завершена';uk='Завантаження класифікатора %1 завершена'");
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КраткоеИмяКлассификатора);
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ - ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ДеревоГрупп
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru='Загрузка классификатора ЕНАОФ';uk='Завантаження класифікатора ЕНАОФ'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
