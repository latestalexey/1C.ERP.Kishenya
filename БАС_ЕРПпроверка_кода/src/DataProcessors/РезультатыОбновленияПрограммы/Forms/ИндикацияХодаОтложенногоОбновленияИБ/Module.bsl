...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		...
		Элементы.ПодсказкаГдеНайтиЭтуФорму.Заголовок = 
			НСтр("ru='Ход обработки данных версии программы можно также проконтролировать из раздела
                       ...
                       |;uk='Хід обробки дані версії програми можна також проконтролювати з розділі
                       |""Інформація"" на робочому столі, команда ""Опис змін програми"".'");
	...
	Иначе
		ЗаголовокОбновлениеЗавершено = НСтр("ru='Версия программы успешно обновлена на версию %1';uk='Версія програми успішно оновлена на версію %1'");
	...
	Иначе
		...
		СведенияОбОбновлении = ОбновлениеИнформационнойБазыСлужебный.СведенияОбОбновленииИнформационнойБазы();
		ШаблонЗаголовка = НСтр("ru='Дополнительные процедуры обработки данных завершены %1 в %2';uk='Додаткові процедури обробки даних завершені %1 в %2'");
	...
	Элементы.ЗаголовокИнформации.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Выполняются дополнительные процедуры обработки данных на версию %1
            ...
            |;uk='Виконуються додаткові процедури обробки даних на версію %1
            |Робота із цими даними тимчасово обмежена'"), Метаданные.Версия);
...
Функция СообщениеОРезультатахОбновления(СведенияОбОбновлении)
	...
	Если ВсегоОбработчиков = УспешноВыполненоОбработчиков Тогда
		...
		Если ВсегоОбработчиков = 0 Тогда
			...
			ТекстСообщения = "";
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Все процедуры обновления выполнены успешно (%1)';uk='Всі процедури оновлення виконані успішно (%1)'"), УспешноВыполненоОбработчиков);
		КонецЕсли;
		Элементы.КартинкаИнформация1.Картинка = БиблиотекаКартинок.Успешно32;
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не все процедуры удалось выполнить (выполнено %1 из %2)';uk='Не всі процедури вдалося виконати (виконане %1 з %2)'"), 
...
Процедура ОбновитьИнформациюОХодеОбновления(СведенияОбОбновлении, ОбновлениеЗавершено = Ложь)
	...
	Элементы.ИнформацияСтатусОбновления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Выполнено: %1 из %2';uk='Виконано: %1 з %2'"),
...
Процедура ИзменитьРасписаниеПослеУстановкиРасписания(НовоеРасписание, ДополнительныеПараметры) Экспорт
	...
	Если НовоеРасписание <> Неопределено Тогда
		Если НовоеРасписание.ПериодПовтораВТечениеДня = 0 Тогда
			...
			КнопкиВопроса.Добавить("НастроитьРасписание", НСтр("ru='Настроить расписание';uk='Настроїти розклад'"));
			КнопкиВопроса.Добавить("РекомендуемыеНастройки", НСтр("ru='Установить рекомендуемые настройки';uk='Встановити рекомендовані настройки'"));
			...
			ТекстСообщения = НСтр("ru='Дополнительные процедуры обработки данных выполняются небольшими порциями,
                ...
                |;uk='Додаткові процедури обробки даних виконуються невеликими порціями,
                |тому для їхньої коректної роботи необхідно обов''язково вказати інтервал повтору після завершення.
                |
                |Для цього у вікні настройки розкладу необхідно перейти на вкладку ""Денний""
                |і заповнити поле ""Повторювати через"".'");
