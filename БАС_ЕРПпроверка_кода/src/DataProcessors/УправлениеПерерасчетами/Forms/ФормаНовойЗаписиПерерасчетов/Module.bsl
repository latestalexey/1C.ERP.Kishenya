...
Функция ЗаписатьПерерасчет()
	...
	Запись.ФизическоеЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Сотрудник, "ФизическоеЛицо");
...
Процедура ЗаполнитьСписокДокументовНачисления()
	...
	Запрос.Текст =
		...
		|		ПО НачисленияУдержанияПоСотрудникам.Период = ПерерасчетЗарплаты.ПериодДействия
		|			И НачисленияУдержанияПоСотрудникам.Регистратор = ПерерасчетЗарплаты.ДокументНачисления
		|			И НачисленияУдержанияПоСотрудникам.Сотрудник = ПерерасчетЗарплаты.Сотрудник
		|			И НачисленияУдержанияПоСотрудникам.Организация = ПерерасчетЗарплаты.Организация
		|ГДЕ
		...
		|	И ПерерасчетЗарплаты.ДокументНачисления ЕСТЬ NULL 
		|
		|УПОРЯДОЧИТЬ ПО
		|	Регистратор";
