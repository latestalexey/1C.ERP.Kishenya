...
Процедура ВыполнитьОбработкуЧеков(Команда)
	...
	Если МассивВыбранныхСтрок.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Для обработки чеков ККМ необходимо выбрать хотя бы одну кассу ККМ.';uk='Для обробки чеків ККМ необхідно вибрати хоча б одну касу ККМ.'"),
			,
			"ТаблицаКассы");
		Возврат
	КонецЕсли;
	...
	Если ВариантОбработки = 1 Тогда
		ТекстВопроса = НСтр("ru='ВНИМАНИЕ! Все чеки ККМ за выбранный период будут удалены. 
            ...
            |;uk='УВАГА! Всі чеки ККМ за вибраний період будуть вилучені. 
            |Це може зайняти тривалий час. Вилучити чеки ККМ?'");
		СписокКнопок.Добавить("Обработать", НСтр("ru='Удалить';uk='Вилучити'"));
	Иначе
		ТекстВопроса = НСтр("ru='ВНИМАНИЕ! Все чеки ККМ за выбранный период будут заархивированы. 
            ...
            |;uk='УВАГА! Всі чеки ККМ за вибраний період будуть заархівовані. 
            |Це може зайняти тривалий час. Заархівувати чеки ККМ?'");
		СписокКнопок.Добавить("Обработать", НСтр("ru='Заархивировать';uk='Заархівувати'"));
	КонецЕсли;
...
Функция ОбработатьЧекиККМНаСервере()
	...
	Запрос = Новый Запрос(
		...
		|	И КассоваяСмена.КассаККМ В(&КассыККМ)
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		...
		ОбновитьСписокКассККМ();
	Иначе
		НаименованиеЗадания = НСтр("ru='Обработка чеков ККМ';uk='Обробка чеків ККМ'");
...
Процедура ОповеститьПользователяОРезультатахВыполненияЗадания()
	...
	Если НЕ ПараметрыОбработкиЧеков.ОбработкаВыполнена Тогда
		Если СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ИнформационнаяБазаФайловая Тогда
			СообщениеОбОшибкеАрхивация = НСтр("ru='Архивация чеков ККМ завершена с ошибками.';uk='Архівування чеків ККМ завершено з помилками.'");
			СообщениеОбОшибкеУдаление  = НСтр("ru='Удаление чеков ККМ завершено с ошибками.';uk='Вилучення чеків ККМ завершене з помилками.'");
		Иначе
			СообщениеОбОшибкеАрхивация = НСтр("ru='Архивация чеков ККМ завершена с ошибками. См. журнал регистрации.';uk='Архівування чеків ККМ завершено з помилками. См. журнал реєстрації.'");
			СообщениеОбОшибкеУдаление  = НСтр("ru='Удаление чеков ККМ завершено с ошибками. См. журнал регистрации.';uk='Вилучення чеків ККМ завершене з помилками. Див. журнал реєстрації.'");
		КонецЕсли;
		
		Если ВариантОбработки = 0 Тогда
			ПоказатьОповещениеПользователя(НСтр("ru='Архивация завершена.';uk='Архівування завершено.'"),
				,
				СообщениеОбОшибкеАрхивация,
				БиблиотекаКартинок.Информация32);
		ИначеЕсли ВариантОбработки = 1 Тогда
			ПоказатьОповещениеПользователя(НСтр("ru='Удаление завершено.';uk='Вилучення завершене.'"),
				,
	...
	Иначе
		Если ВариантОбработки = 0 Тогда
			ПоказатьОповещениеПользователя(НСтр("ru='Архивация завершена.';uk='Архівування завершено.'"),
				,
				НСтр("ru='Архивация чеков ККМ успешно завершена.';uk='Архівування чеків ККМ успішно завершено.'"),
				БиблиотекаКартинок.Информация32);
		ИначеЕсли ВариантОбработки = 1 Тогда
			ПоказатьОповещениеПользователя(НСтр("ru='Удаление завершено.';uk='Вилучення завершене.'"),
				,
				НСтр("ru='Удаление чеков ККМ успешно завершено.';uk='Вилучення чеків ККМ успішно завершене.'"),
				БиблиотекаКартинок.Информация32);
		КонецЕсли;
	КонецЕсли;
