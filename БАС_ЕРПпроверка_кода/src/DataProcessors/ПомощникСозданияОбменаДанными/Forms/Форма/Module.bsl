...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоПродолжениеНастройкиВПодчиненномУзлеРИБ Тогда
		...
		Элементы.ГруппаПереключателяВыбораРежимаРаботыПомощника.Заголовок = НСтр("ru='Продолжение настройки синхронизации данных с главным узлом';uk='Продовження настройки синхронізації даних з головним вузлом'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ОбщегоНазначенияКлиент.ПредлагатьСоздаватьРезервныеКопии() Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Перед настройкой синхронизации рекомендуется сделать <a href = %1 >резервную копию данных</a>.';uk='Перед настройкою синхронізації рекомендується зробити <a href = %1 >резервну копію даних</a>.'"),
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ДлительнаяОперация Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выполняется создание синхронизации данных.
                                            ...
                                            |;uk='Виконується створення синхронізації даних.
                                            |Робота помічника не може бути завершена.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ЭтоПродолжениеНастройкиВПодчиненномУзлеРИБ Тогда
		ТекстПредупреждения = НСтр("ru='Выполняется настройка подчиненного узла распределенной информационной базы.
                                         ...
                                         |;uk='Виконується настройка підпорядкованого вузла розподіленої інформаційної бази.
                                         |Відмовитися від настройки й використати значення по умовчанню?'");
	...
	ТекстПредупреждения = НСтр("ru='Отменить настройку синхронизации и выйти из помощника?';uk='Скасувати настройку синхронізації й вийти з помічника?'");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗакрытиеФормыСопоставленияОбъектов" Тогда
		...
		Если Отказ Тогда
			ПоказатьПредупреждение(, НСтр("ru='При получении информации статистики возникли ошибки.';uk='При одержанні інформації статистики виникли помилки.'"));
		Иначе
...
Процедура ИмяФайлаНастроекОбменаДаннымиДляЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога.Вставить("Фильтр", НСтр("ru='Файл настроек синхронизации данных (*.xml)';uk='Файл настройок синхронізації даних (*.xml)'") + "|*.xml" );
...
Процедура СохранитьФайлНастроекОбменаДанными(Команда)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='При сохранении файла настроек синхронизации данных возникли ошибки.';uk='При збереженні файлу настройок синхронізації даних виникли помилки.'"));
	...
	Иначе
		...
		#Иначе
			...
			ТекстПредложения = НСтр("ru='Для открытия каталога необходимо необходимо установить расширение работы с файлами.';uk='Для відкриття каталогу необхідно необхідно встановити розширення роботи з файлами.'");
...
Процедура ПослеПроверкиРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Диалог.Заголовок      = НСтр("ru='Укажите имя файла настроек синхронизации данных';uk='Укажіть ім''я файлу настройок синхронізації даних'");
...
Процедура ПроверитьПодключениеCOMЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Если ПустаяСтрока(ТекстСообщения) Тогда
			ТекстСообщения = НСтр("ru='Проверка подключения успешно завершена.';uk='Перевірка підключення успішно завершена.'");
		КонецЕсли;
...
Процедура ПроверитьПодключениеWSЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Если Не Отказ Тогда
			...
			ПоказатьПредупреждение(, НСтр("ru='Подключение успешно установлено.';uk='Підключення успішно встановлене.'"));
...
Процедура ОбработчикОжиданияДлительнойОперации()
	...
	Иначе // Failed, Canceled
		...
		ТекстВопроса = НСтр("ru='При создании синхронизации данных возникли ошибки.
                            ...
                            |;uk='При створенні синхронізації даних виникли помилки.
...
Процедура УстановитьВидимостьНаСервере()
	...
	Если Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы Тогда
		...
		Элементы.ГруппаПереключателяВыбораРежимаРаботыПомощника.Заголовок = НСтр("ru='Создание начального образа подчиненного узла РИБ';uk='Створення початкового образа підпорядкованого вузла РІБ'");
...
Функция ПредставлениеРезультатаТранспортСообщений()
	...
	Если НастройкаОбменаССервисом Тогда
        ...
        |%1'
        |;uk='Параметри підключення до додатка в сервісі:
        |%1'");
	...
	Иначе
		...
		Результат = Строка(Объект.ВидТранспортаСообщенийОбмена)
			+ НСтр("ru=', параметры:';uk=', параметри:'") + Символы.ПС
...
Функция ПолучитьОписаниеНастроекТранспортаОбмена()
	...
	Для Каждого Элемент Из ПредставленияНастроек Цикл
		...
		Если РежимРаботыМастера = "ОбменЧерезВнешнееСоединение" Тогда
			...
			Если Элемент.Ключ = "COMВариантРаботыИнформационнойБазы" Тогда
				...
				ЗначениеНастройки = ?(Объект[Элемент.Ключ] = 0, НСтр("ru='Файловый';uk='Файловий'"), НСтр("ru='Клиент-серверный';uk='Клієнт-Серверний'"));
		...
		ИначеЕсли  Не ТипЗначения(ЗначениеНастройки, "Число")
			...
			ЗначениеНастройки = НСтр("ru='<пусто>';uk='<порожньо>'");
	...
	Если ПустаяСтрока(Результат) Тогда
		...
		Результат = НСтр("ru='Настройки подключения не заданы.';uk='Настройки підключення не задані.'");
...
Процедура ПроверитьПодключение(ВидТранспорта)
	...
	Если Не Отказ Тогда
		...
		Если ВидТранспорта = ПредопределенноеЗначение("Перечисление.ВидыТранспортаСообщенийОбмена.FILE") Тогда
			ТекстПредупреждения = НСтр("ru='Запись данных в указанный каталог разрешена.';uk='Запис даних у вказаний каталог дозволена.'");
		Иначе
			ТекстПредупреждения = НСтр("ru='Подключение успешно установлено.';uk='Підключення успішно встановлене.'");
		КонецЕсли;
...
Процедура ПроверитьКорректностьПараметровПодключенияWS(Отказ)
	...
	Если ПустаяСтрока(Объект.WSURLВебСервиса) Тогда
		...
		НСтрока = НСтр("ru='Укажите адрес приложения в Интернете.';uk='Укажіть адресу додатка в Інтернеті.'");
	...
	ИначеЕсли ПустаяСтрока(Объект.WSИмяПользователя) Тогда
		...
		НСтрока = НСтр("ru='Укажите имя пользователя.';uk='Укажіть ім''я користувача.'");
	...
	ИначеЕсли ПустаяСтрока(Объект.WSПароль) Тогда
		...
		НСтрока = НСтр("ru='Укажите пароль пользователя.';uk='Укажіть пароль користувача.'");
...
Процедура ПроверитьПодключениеWSНаКлиенте(Отказ, РасширеннаяПроверка = Ложь)
	...
	Если Отказ И ПредлагатьПерейтиВЖурналРегистрации Тогда
		...
		ТекстВопроса = НСтр("ru='Ошибка установки подключения.
            |Перейти в журнал регистрации?'
            |;uk='Помилка установки підключення.
            |Перейти в журнал реєстрації?'");
...
Процедура ПроверитьПодключениеWSНаСервере(Отказ, РасширеннаяПроверка, ПредлагатьПерейтиВЖурналРегистрации)
	...
	Если РасширеннаяПроверка Тогда
		...
		Если Не ПараметрыПриемника.ПланОбменаСуществует Тогда
			...
			Сообщение = НСтр("ru='Другая программа не предназначена для синхронизации с текущей.';uk='Інша програма не призначена для синхронізації з поточної.'");
...
Процедура ПроверитьЗаполнениеРеквизитовВФорме(Отказ, ИмяФормыДляПроверки, ПараметрыФормы, ИмяРеквизитаФормы)
	...
	Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо задать обязательные настройки.';uk='Необхідно задати обов''язкові настройки.'"),,, ИмяРеквизитаФормы, Отказ);
...
Процедура ЗавершениеПервогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ)
	...
	Если Не Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы
		...
		НСтрока = НСтр("ru='Сохраните файл с настройками для другой программы';uk='Збережіть файл із настройками для іншої програми'");
	...
	Иначе
		...
		Если ВыполнитьОбменДаннымиСейчас Тогда
			...
			Состояние(НСтр("ru='Выполняется отправка данных...';uk='Виконується відправлення даних...'"));
			...
			Если Отказ Тогда
				...
				ПоказатьПредупреждение(, НСтр("ru='Во время отправки данных возникли ошибки(см. журнал регистрации).';uk=' Під час відправлення даних виникли помилки(див. журнал реєстрації).'"));
...
Процедура ЗавершениеВторогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ, Знач ОткрыватьПослеЗакрытияТекущей)
	...
	Состояние(НСтр("ru='Выполняется создание настройки синхронизации данных';uk='Виконується створення настройки синхронізації даних'"));
	
	НастроитьНовыйОбменДаннымиНаСервере(Отказ, НастройкаОтборовНаУзле, ЗначенияПоУмолчаниюНаУзле);
	
	Если Отказ Тогда
		
		ПоказатьПредупреждение(, НСтр("ru='При создании настройки синхронизации данных возникли ошибки.';uk='При створенні настройки синхронізації даних виникли помилки.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьФормуСопоставления()
	...
	Если Не ТекущиеДанные.ИспользоватьПредварительныйПросмотр Тогда
		ПоказатьПредупреждение(, НСтр("ru='Для этих данных нельзя выполнить сопоставление.';uk=' Для цих даних не можна виконати зіставлення.'"));
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При выполнении обработчика ПриПодключенииККорреспонденту произошла ошибка:%1%2';uk='При виконанні обробника ПриПодключенииККорреспонденту відбулася помилка:%1%2'"),
				Символы.ПС,
...
Функция НадписьДалееFTP()
	...
	Возврат НСтр("ru='Нажмите кнопку ""Далее"" для настройки подключения через FTP-ресурс.';uk='Натисніть кнопку ""Далі"" для настройки підключення через FTP-ресурс.'");
...
Функция НадписьДалееEMAIL()
	...
	Возврат НСтр("ru='Нажмите кнопку ""Далее"" для настройки подключения по почте.';uk='Натисніть кнопку ""Далі"" для настройки підключення поштою.'");
...
Функция НадписьДалееНастройки()
	...
	Возврат НСтр("ru='Нажмите кнопку ""Далее"" для настройки дополнительных параметров синхронизации данных.';uk='Натисніть кнопку ""Далі"" для настройки додаткових параметрів синхронізації даних.'");
...
Функция Подключаемый_СтраницаМастераНастройкаПараметров_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеЭтойБазы) Тогда
		...
		НСтрока = НСтр("ru='Укажите наименование этой программы.';uk='Укажіть найменування цієї програми.'");
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ru='Укажите наименование другой программы.';uk='Укажіть найменування іншої програми.'");
	...
	Если ПустаяСтрока(Объект.ПрефиксИнформационнойБазыПриемника) Тогда
		...
		НСтрока = НСтр("ru='Укажите существующий или желаемый префикс второй информационной базы.';uk='Укажіть існуючий або бажаний префікс другої інформаційної бази.'");
	...
	Если СокрЛП(Объект.ПрефиксИнформационнойБазыИсточника) = СокрЛП(Объект.ПрефиксИнформационнойБазыПриемника) Тогда
		...
		НСтрока = НСтр("ru='Префиксы информационных баз должны быть различными.';uk='Префікси інформаційних баз повинні бути різними.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.ПрефиксИнформационнойБазыПриемника",, Отказ);
...
Функция Подключаемый_СтраницаМастераНастройкаПараметровДляВнешнегоСоединенияПервойБазы_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеЭтойБазы) Тогда
		...
		НСтрока = НСтр("ru='Укажите наименование информационной базы.';uk='Укажіть найменування інформаційної бази.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеЭтойБазы",, Отказ);
		Возврат Неопределено;
	КонецЕсли;
...
Функция Подключаемый_СтраницаМастераНачало_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.ИмяФайлаНастроекСинхронизацииДанныхДляЗагрузки) Тогда
		ТекстОшибки = НСтр("ru='Выберите файл с настройками синхронизации данных';uk='Виберіть файл із настройками синхронізації даних'");
	...
	ТекстПредупреждения = НСтр("ru='Для передачи файла настроек синхронизации данных необходимо установить расширение для веб-клиента платформы.';uk='Для передачі файлу налаштувань синхронізації даних необхідно встановити розширення для веб-клієнта платформи.'");
...
Процедура ЗавершениеВыбораФайлаНастроекОбменаДаннымиДляЗагрузки(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстОшибки) Тогда
		...
		Если ОшибкаЗагрузкиПараметровМастера Тогда
			ТекстОшибки = НСтр("ru='Указан неправильный файл настроек синхронизации данных. Укажите корректный файл.';uk='Зазначено неправильний файл настройок синхронізації даних. Укажіть коректний файл.'");
		Иначе
...
Функция Подключаемый_СтраницаМастераНастройкаПараметров_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Если НЕ    (Объект.ИспользоватьПараметрыТранспортаEMAIL
		...
		НСтрока = НСтр("ru='Не указаны параметры подключения для синхронизации данных.
                        ...
                        |;uk='Не зазначені параметри підключення для синхронізації даних.
                        |Варто настроїти хоча б один варіант підключення.'");
		//
...
Функция Подключаемый_СтраницаМастераИтогНастройкиОбмена_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Если ВариантРаботыМастера = "НастроитьНовыйОбменДанными" Тогда
        ...
        |;uk='%1%2%3Префікс цієї інформаційної бази: %4
        |Префікс другої інформаційної бази: %5'");
	...
	Иначе
		...
		// Представление результата настройки обмена.
		СтрокаСообщения = НСтр("ru='%1%2%3Префикс этой информационной базы: %4';uk='%1%2%3Префікс цієї інформаційної бази: %4'");
...
Функция Подключаемый_СтраницаМастераИтогНастройкиОбмена_ПриОткрытии_ВнешнееСоединение(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Если НастройкаОбменаССервисом Тогда
        ...
        |;uk='%1
        |Настройки для цієї інформаційної бази:
        |========================================================
        |%2%3Префікс інформаційної бази: %4
        |
        |Настройки для прикладної програми, розташованого в сервісі:
        |========================================================
        |%5%6Префікс додатку: %7'");
	...
	Иначе
        ...
        |;uk='%1
        |Параметри синхронізації даних для цієї програми:
        |========================================================
        |%2%3Префікс інформаційної бази: %4
        |
        |Параметри синхронізації даних для іншої програми::
        |========================================================
        |%5%6Префікс інформаційної бази: %7'");
...
Функция Подключаемый_СтраницаМастераОжиданиеАнализаДанныхСозданиеНастройкиОбмена_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Возникли ошибки на этапе создания настройки синхронизации данных.
                    ...
                    |;uk='Виникли помилки на етапі створення настройок синхронізації даних.
                    |Для вирішення проблем скористайтеся журналом реєстрації.'"));
...
Функция Подключаемый_СтраницаМастераОжиданиеАнализаДанныхПолучениеСообщения_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Возникли ошибки на этапе анализа данных.
                    |Для решения проблем воспользуйтесь журналом регистрации.'
                    |;uk='Виникли помилки на етапі аналізу даних.
                    |Для вирішення проблем скористайтеся журналом реєстрації.'"));
...
Функция Подключаемый_СтраницаМастераОжиданиеАнализаДанныхАвтоматическоеСопоставление_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Возникли ошибки на этапе анализа данных.';uk='Виникли помилки на етапі аналізу даних.'"));
...
Функция Подключаемый_СтраницаМастераСопоставлениеДанных_ПриПереходеДалее(Отказ)
	...
	Если Не ВсеДанныеСопоставлены Тогда
		...
		Сообщение = НСтр("ru='Не все данные сопоставлены. Наличие несопоставленных данных
                               ...
                               |Продолжить?'
                               |;uk='Не всі дані зіставлені. Наявність незіставлених даних
                               |може привести до появи однакових елементів довідників (дублів).
                               |Продовжити?'");
...
Процедура СтраницаМастераОжиданиеСинхронизацииСправочниковЗагрузка_ОбработкаДлительнойОперации()
	...
	Попытка
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Загрузка справочников из сообщения обмена';uk='Завантаження довідників із повідомлення обміну'"));
...
Функция Подключаемый_СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузка_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Возникли ошибки на этапе синхронизации справочников.
                    |Для решения проблем воспользуйтесь журналом регистрации.'
                    |;uk='Виникли помилки на етапі синхронізації довідників.
                    |Для вирішення проблем скористайтеся журналом реєстрації.'"));
...
Функция Подключаемый_СтраницаМастераОжиданиеСохраненияНастроек_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Возникли ошибки на этапе сохранения настроек.
                    |Для решения проблем воспользуйтесь журналом регистрации.'
                    |;uk='Виникли помилки на етапі збереження настройок.
                    |Для вирішення проблем скористайтеся журналом реєстрації.'"));
...
Функция Подключаемый_СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузка_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Возникли ошибки на этапе синхронизации данных.
                    |Для решения проблем воспользуйтесь журналом регистрации.'
                    |;uk='Виникли помилки на етапі синхронізації даних.
                    |Для вирішення проблем скористайтеся журналом реєстрації.'"));
...
Процедура ПроверитьНастройкиПараметровУчетаНаСервере(
	...
	Если Не НастройкиПараметровУчетаЗаданы Тогда
		...
		Если ПустаяСтрока(СообщениеОбОшибке) Тогда
			СообщениеОбОшибке = НСтр("ru='Не заданы параметры учета в этой программе.';uk='Не задані параметри обліку в цій програмі.'");
		КонецЕсли;
	...
	Если Не НастройкиПараметровУчетаКорреспондентаЗаданы Тогда
		...
		Если ПустаяСтрока(СообщениеОбОшибкеКорреспондента) Тогда
			СообщениеОбОшибкеКорреспондента = НСтр("ru='Не заданы параметры учета в приложении, расположенном в Интернете.';uk='Не задані параметри обліку в додатку, розташованому в Інтернеті.'");
		КонецЕсли;
...
Процедура ПроверитьЗаданиеНастроекДляПервойБазы(Отказ, ТипСоединения = "ВебСервис")
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ru='Укажите наименование приложения в Интернете.';uk='Укажіть найменування додатка в Інтернеті.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеВторойБазы",, Отказ);
...
Процедура ПроверитьЗаданиеНастроекДляВторойБазы(Отказ, ТипСоединения)
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ru='Укажите наименование программы.';uk='Укажіть найменування програми.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеВторойБазы",, Отказ);
...
Процедура СтраницаМастераНастройкаПараметров_ПриПереходеДалееНаСервере(Отказ)
	...
	Если Не ПланыОбмена[Объект.ИмяПланаОбмена].НайтиПоКоду(ОбменДаннымиСервер.КодУзлаПланаОбменаСтрокой(Объект.ПрефиксИнформационнойБазыПриемника)).Пустая() Тогда
		...
		НСтрока = НСтр("ru='Значение префикса второй информационной базы не уникально.
            ...
            |;uk='Значення префікса другої інформаційної бази не унікально.
            |У системі вже існує синхронізація даних для інформаційної бази (програми) із зазначеним префіксом.
            |Зміните значення префікса або використайте існуючу синхронізацію.'");
		//
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.ПрефиксИнформационнойБазыПриемника",, Отказ);
...
Процедура СтраницаМастераОжиданиеПроверкиПодключенияВнешнегоСоединения_ОбработкаДлительнойОперацииНаСервере(Отказ)
	...
	Если Объект.COMВариантРаботыИнформационнойБазы = 0 Тогда
		...
		Если ПустаяСтрока(Объект.COMКаталогИнформационнойБазы) Тогда
			...
			НСтрока = НСтр("ru='Укажите каталог информационной базы.';uk='Укажіть каталог інформаційної бази.'");
	...
	Иначе
		...
		Если ПустаяСтрока(Объект.COMИмяСервера1СПредприятия) Тогда
			...
			НСтрока = НСтр("ru='Укажите имя кластера серверов.';uk='Укажіть ім''я кластера серверів.'");
		...
		ИначеЕсли ПустаяСтрока(Объект.COMИмяИнформационнойБазыНаСервере1СПредприятия) Тогда
			...
			НСтрока = НСтр("ru='Укажите имя информационной базы.';uk='Укажіть ім''я інформаційної бази.'");
	...
	Если Не ПланОбменаСуществует Тогда
		...
		Сообщение = НСтр("ru='Синхронизация данных с указанной программой не предусмотрена.';uk='Синхронізація даних із зазначеною програмою не передбачена.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
		Возврат;
	...
	Если НРег(СтрокаСоединенияИнформационнойБазы()) = НРег(ВнешнееСоединение.СтрокаСоединенияИнформационнойБазы()) Тогда
		...
		Сообщение = НСтр("ru='Заданы настройки подключения к этой информационной базе.';uk='Задано настройки підключення до цієї інформаційної бази.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
		Возврат;
...
Процедура ПроверитьНаличиеСуществующегоОбменаДаннымиСВторойБазой(Отказ)
	...
	Если Не ПустаяСтрока(КодУзла)
		...
		Сообщение = НСтр("ru='Синхронизация данных между программами уже была настроена ранее.';uk='Синхронізація даних між програмами вже була настроєна раніше.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
...
Процедура ЗапуститьВыгрузкуДанных(Отказ, УзелИнформационнойБазы, ВидТранспортаСообщенийОбмена)
	...
	Попытка
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыЗадания,
			НСтр("ru='Выгрузка данных';uk='Вивантаження даних'"));
		Если Не Результат.ЗаданиеВыполнено Тогда
			ДлительнаяОперация = Истина;
			ИдентификаторДлительнойОперации = Результат.ИдентификаторЗадания;
		КонецЕсли;
...
Процедура ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Если НЕ ДлительнаяОперация ИЛИ Результат.ФоновоеЗаданиеВыполнено Тогда
		...
		ДлительнаяОперация = Ложь;
		Состояние(НСтр("ru='Отправка данных завершена';uk='Відправлення даних завершене'"));
