...
Процедура СообщениеОНеобходимостиПерерасчетаНадписьОбработкаНавигационнойСсылки(Элемент,
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "Подробнее" Тогда
		...
		СтруктураОтбора.Вставить("ПериодДействия", МесяцНачисления);
		
		ПараметрыОткрытия = Новый Структура("Отбор,ГруппироватьПоДокументамНачисления", СтруктураОтбора, Истина);
...
Процедура ОбновитьИнформационныеНадписи()
	...
	Если СуммаКВыплате >= МинимальнаяСуммаНевыплаченнойЗарплаты() Тогда
		НадписьНевыплаченнаяЗарплата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Остаток невыплаченной зарплаты на конец месяца: %1';uk='Залишок невиплаченої зарплати на кінець місяця: %1'"), СуммаКВыплате);
	...
	Элементы.ГруппаСледующийМесяц.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='После %1';uk='Після %1'"), Сред(Формат(МесяцНачисления, "ДЛФ=ДД"), 3));
	УстановитьОтображениеСообщенияОНеобходимостиПерерасчета();
