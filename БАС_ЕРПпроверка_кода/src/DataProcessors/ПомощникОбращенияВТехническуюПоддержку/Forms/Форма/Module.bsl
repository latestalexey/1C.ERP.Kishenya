...
Процедура ОткрытьСайтИТС(Команда)
	...
	ОбщегоНазначенияУТКлиент.ЗапуститьПриложениеБезОбработкиРезультата("http://its.1c.ua/");
...
Процедура Отправить(Команда)
	...
	ИначеЕсли ПараметрыПодготовкиПисьма = 1 Тогда
		...
		Сообщение.Получатели.Добавить("hotline@1c.ua");
...
Процедура СформироватьТекстПисьма()
	...
	ДопТехИнформация = ИнтернетПоддержкаПользователейВызовСервера.ТекстВложенияТехническаяИнформация("", ДопПараметры);
...
Функция ВыполнитьОтправкуПисьма(Объект, Соединение = Неопределено)
	...
	Адресаты.Добавить(Новый Структура("Адрес,Представление", "hotline@1c.ua", "hotline@1c.ua"));
