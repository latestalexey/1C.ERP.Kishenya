...
Процедура Подключаемый_СтраницаПриветствие_ПриПереходеДалее(Отказ)
	...
	Файл.НачатьИнициализацию(ОписаниеОповещения, Объект.ИмяФайлаМодели);
...
Процедура СтраницаПриветствие_ПриПереходеДалее_ПроверкаСуществованияФайла(Существует, ДополнительныеПараметры) Экспорт
	...
	Если НЕ Существует Тогда
		ИзменитьПорядковыйНомерПерехода(-1);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='По указанному пути файл с данными не найден';uk='За вказаним шляхом файл з даними не знайдено'"), , "ИмяФайлаМодели", "ИмяФайлаМодели");
		Возврат;
	КонецЕсли;
	
	Обработчик = Новый ОписаниеОповещения("ВыполнитьЗагрузкуМодели", ЭтаФорма);
	НачатьПомещениеФайла(Обработчик, АдресХранилища, Объект.ИмяФайлаМодели, Ложь, УникальныйИдентификатор);
...
&НаКлиенте
Процедура ВыполнитьЗагрузкуМодели(Результат, Адрес, ВыбранноеИмяФайла, Параметры) Экспорт
...
Функция ЗагрузитьНаСервере()
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		...
		Обработки.ПомощникВыгрузкиЗагрузкиМоделиМеждународногоУчета.ЗагрузитьМодельУчета(ПараметрыЗадания, АдресХранилища);
	...
	Иначе
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
				...
				"Обработки.ПомощникВыгрузкиЗагрузкиМоделиМеждународногоУчета.ЗагрузитьМодельУчета",
				ПараметрыЗадания,
				НаименованиеЗадания);
