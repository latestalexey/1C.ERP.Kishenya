...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТаблицаПериоды = ПолучитьИзВременногоХранилища(Параметры.АдресТаблицыПериодов);
	...
	Если ЗначениеЗаполнено(Параметры.ОтборПоТипуНоменклатуры) Тогда
		...
		Элементы.ОтборНадпись.Видимость = Истина;
	...
	Если ЭтоПланПродажПоКатегориям Тогда
		...
		Элементы.Шаг1Надпись1.Заголовок = НСтр("ru='Необходимо скопировать колонки в таблицу из внешнего файла (Excel. Word и пр.) через буфер обмена. Обязательно должна быть заполнена колонка: ""Товарная категория"".';uk='Необхідно скопіювати колонки у таблицю із зовнішнього файлу (Excel. Word тощо) через буфер обміну. Обов''язково повинна бути заповнена колонка: ""Товарна категорія"".'");
	
	Иначе
	
		Элементы.Шаг1Надпись1.Заголовок = НСтр("ru='Необходимо скопировать колонки в таблицу из внешнего файла (Excel. Word и пр.) через буфер обмена. Обязательно должна быть заполнена одна из колонок: ""Штрихкод"", ""Код"", ""Артикул"" или ""Номенклатура"".';uk='Необхідно скопіювати колонки у таблицю із зовнішнього файлу (Excel. Word тощо) через буфер обміну. Обов''язково повинна бути заповнена одна з колонок: ""Штрихкод"", ""Код"", ""Артикул"" або ""Номенклатура"".'");
	...
	Если ЭтоАдресВременногоХранилища(Параметры.АдресТаблицыТоваров) Тогда
		...
		Для каждого СтруктураТовар Из МассивСтруктурТоваров Цикл
			...
			Если СтруктураТовар.Свойство("ИдентификаторУпаковка") 
				...
				СтруктураТовар.Вставить("Упаковка",       ?(Найти(Строка(Упаковка), "<Объект не найден>")> 0, Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка(), Упаковка));
			КонецЕсли; 
			...
			КонецЕсли; 
			//++ НЕ УТ
			Если СтруктураТовар.Свойство("ИдентификаторСпецификация") 
				И ЗначениеЗаполнено(СтруктураТовар.ИдентификаторСпецификация) Тогда
...
Процедура Далее(Команда)
	...
	Если Не ЗаполненыОбязательныеПоля Тогда
		ПоказатьПредупреждение(,НСтр("ru='Необходимо скопировать колонки в таблицу из внешнего файла.';uk='Необхідно скопіювати колонки у таблицю із зовнішнього файлу.'"));
		Возврат;
	КонецЕсли;
	
	ОчиститьСообщения();
	Состояние(НСтр("ru='Осуществляется сопоставление введенных данных с данными информационной базы. Пожалуйста подождите...';uk='Здійснюється зіставлення введених даних з даними інформаційної бази. Будь ласка, зачекайте...'"),,,БиблиотекаКартинок.Информация32);
...
Процедура ПеренестиТовары(Команда)
	...
	СтруктураВыбора.Вставить("ОбновитьДополнить",      ОбновитьДополнить);
...
Процедура Подключаемый_ПриИзмененииСуммы(Элемент)
	...
	ИмяКолонки = СтрЗаменить(Элемент.Имя, "ТоварыСумма_", "");
...
Процедура РассчитатьИтоговыеПоказатели(Форма, РассчитыватьСтроки = Истина)
	...
	Для каждого СтрокаПериода Из Форма.Периоды Цикл
		...
		Форма["Объект_ТоварыКоличество_" + СтрокаПериода.ИмяКолонки] = 0;
		Форма["Объект_ТоварыСумма_" + СтрокаПериода.ИмяКолонки] = 0;
	...
	Для каждого СтрокаТЧ Из Товары Цикл
		...
		Для каждого СтрокаПериода Из Форма.Периоды Цикл
			...
			Форма["Объект_ТоварыКоличество_" + СтрокаПериода.ИмяКолонки] = Форма["Объект_ТоварыКоличество_" + СтрокаПериода.ИмяКолонки] + СтрокаТЧ["Количество_" + СтрокаПериода.ИмяКолонки];
			Форма["Объект_ТоварыСумма_" + СтрокаПериода.ИмяКолонки] = Форма["Объект_ТоварыСумма_" + СтрокаПериода.ИмяКолонки] + СтрокаТЧ["Сумма_" + СтрокаПериода.ИмяКолонки];
...
Процедура СопоставитьДанныеТабличногоДокументаСДаннымиИБ()
	...
	Пока ЗаполненыОбязательноеПоле Цикл
		...
		Если ЗаполнятьПартнера Тогда
			НоваяСтрокаТовары.Партнер = Партнер;
		КонецЕсли; 
...
Процедура ИнициализироватьСтруктуруПоиска()
	...
	КонецЕсли;
	//++ НЕ УТ
	Если ТипПлана = Перечисления.ТипыПланов.ПланПроизводства И ИспользоватьДляПланированияМатериалов Тогда
		СтруктураПоиска.Вставить("ЗагружаемаяСпецификация", НомерКолонки);
		НомерКолонки = НомерКолонки +1;
	КонецЕсли;
	//-- НЕ УТ
	...
	Иначе
		...
		ИначеЕсли ТипПлана = Перечисления.ТипыПланов.ПланПроизводства Тогда
			СтруктураПоиска.Вставить("ДатаВыпуска", НомерКолонки);
			НомерКолонки = НомерКолонки +1;
		//-- НЕ УТ
...
Процедура НайтиНоменклатуруПоЗагружаемымДанным(СтрокаТЧ)
	...
	НомерСтроки = СтрокаТЧ.КодСтроки;
...
Процедура ВыполнитьПоискПоШтрихКодам()
	...
	Если ЗначениеЗаполнено(ОтборПоТипуНоменклатуры) Тогда
		ШтрихкодыТекстУсловияОтбор = "И ШтрихкодыНоменклатуры.Номенклатура.ТипНоменклатуры В (&ОтборПоТипуНоменклатуры)";
...
//++ НЕ УТ
&НаСервере
Процедура СопоставитьСпецификации()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ДанныеДляСопоставления.ЗагружаемаяСпецификация,
	|	ВЫРАЗИТЬ(ДанныеДляСопоставления.Номенклатура КАК Справочник.Номенклатура) КАК Номенклатура,
	|	ВЫРАЗИТЬ(ДанныеДляСопоставления.Характеристика КАК Справочник.ХарактеристикиНоменклатуры) КАК Характеристика,
	|	ВЫРАЗИТЬ(ДанныеДляСопоставления.КодСтроки КАК ЧИСЛО(15, 0)) КАК КодСтроки
	|ПОМЕСТИТЬ ДанныеДляСопоставления
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(РесурсныеСпецификации.Ссылка) КАК Спецификация,
	|	ДанныеДляСопоставления.КодСтроки,
	|	КОЛИЧЕСТВО(ДанныеДляСопоставления.КодСтроки) КАК Количество
	|ИЗ
	|	ДанныеДляСопоставления КАК ДанныеДляСопоставления
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РесурсныеСпецификации.ВыходныеИзделия КАК РесурсныеСпецификации
	|		ПО (РесурсныеСпецификации.Номенклатура = ДанныеДляСопоставления.Номенклатура)
	|			И (РесурсныеСпецификации.Характеристика = ДанныеДляСопоставления.Характеристика)
	|			И (РесурсныеСпецификации.Ссылка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСпецификаций.Действует))
	|			И (НЕ РесурсныеСпецификации.Ссылка.ПометкаУдаления)
	|			И (&НачалоПроизводства = ДАТАВРЕМЯ(1, 1, 1)
	|				ИЛИ (РесурсныеСпецификации.Ссылка.НачалоДействия = ДАТАВРЕМЯ(1, 1, 1)
	|					ИЛИ РесурсныеСпецификации.Ссылка.НачалоДействия <= &НачалоПроизводства)
	|					И (РесурсныеСпецификации.Ссылка.КонецДействия = ДАТАВРЕМЯ(1, 1, 1)
	|						ИЛИ РесурсныеСпецификации.Ссылка.КонецДействия > &НачалоПроизводства))
	|			И (НЕ РесурсныеСпецификации.Ссылка.ЭтоГруппа)
	|			И (РесурсныеСпецификации.Ссылка.Наименование ПОДОБНО ДанныеДляСопоставления.ЗагружаемаяСпецификация
	|				ИЛИ ДанныеДляСопоставления.ЗагружаемаяСпецификация ПОДОБНО """")
	|ГДЕ
	|	НЕ РесурсныеСпецификации.Ссылка ЕСТЬ NULL 
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеДляСопоставления.КодСтроки
	|
	...
	Запрос.УстановитьПараметр("ДанныеДляСопоставления", Объект.Товары.Выгрузить());
	Запрос.УстановитьПараметр("НачалоПроизводства", НачалоПериода);
	
	РезультатыЗапросов = Запрос.ВыполнитьПакет();
	
	ТаблицаСпецификации = РезультатыЗапросов[1].Выгрузить();
	
	Для каждого СтрокаТаблицы Из Объект.Товары Цикл 
		
		СтрокаСпецификация = ТаблицаСпецификации.Найти(СтрокаТаблицы.КодСтроки, "КодСтроки");
		Если СтрокаСпецификация <> Неопределено Тогда 
			СтрокаТаблицы.Спецификация  = СтрокаСпецификация.Спецификация;
			ПроверитьСопоставлениеДанныхСтрокиЗагрузки(СтрокаТаблицы, ЭтаФорма);
		КонецЕсли;
...
Процедура ИнициализироватьТабличныйДокумент()
	...
	МакетЗаполненияПоВнешнемуИсточнику = Обработки.ЗагрузкаДанныхИзВнешнихФайлов.ПолучитьМакет("МакетЗагрузкиДанныхДляПланов" + СуфиксМакета);
	...
	//++ НЕ УТ
	Если ТипПлана = Перечисления.ТипыПланов.ПланПроизводства И ИспользоватьДляПланированияМатериалов Тогда
		ОбластьТовары = МакетЗаполненияПоВнешнемуИсточнику.ПолучитьОбласть("Спецификация");
		ТабличныйДокумент.Присоединить(ОбластьТовары);
	КонецЕсли;
	//-- НЕ УТ
	Если ЭтоПланПродажПоКатегориям И ИспользоватьРасчетПоСкоростиПродаж Тогда
	...
	Иначе
		...
		//++ НЕ УТ
		ИначеЕсли ТипПлана = Перечисления.ТипыПланов.ПланПроизводства Тогда
			ОбластьТовары = МакетЗаполненияПоВнешнемуИсточнику.ПолучитьОбласть("ДатаВыпуска");
			ТабличныйДокумент.Присоединить(ОбластьТовары);
		//-- НЕ УТ
		ИначеЕсли ЭтоПланСборкиРазборки Тогда
...
Процедура ПроверитьСопоставлениеДанныхСтрокиЗагрузки(СтрокаЗагрузки, Форма)
	...
	//++ НЕ УТ
	Если Форма.ИспользоватьДляПланированияМатериалов
		И Форма.СтруктураПоиска.Свойство("ЗагружаемаяСпецификация") 
		И ЗначениеЗаполнено(СтрокаЗагрузки.ЗагружаемаяСпецификация) 
		И НЕ ЗначениеЗаполнено(СтрокаЗагрузки.Спецификация) Тогда
		СтрокаЗагрузки.ЕстьНесопоставленныеДанные = Истина;
		Возврат;
	КонецЕсли;
	//-- НЕ УТ
...
Процедура УстановитьДоступностьКолонок()
	...
	//++ НЕ УТ
	Элементы.ТоварыГруппаСпецификация.Видимость = (ТипПлана = Перечисления.ТипыПланов.ПланПроизводства) И ИспользоватьДляПланированияМатериалов;
	//-- НЕ УТ
	...
	//++ НЕ УТ
	Элементы.ТоварыДатаВыпуска.Видимость = НЕ КроссТаблица И ТипПлана = Перечисления.ТипыПланов.ПланПроизводства;
	//-- НЕ УТ
