...
Процедура УстановитьДоступностьЭлементовФормы(Форма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"ФормаОК",
		"Доступность",
		ЗначениеЗаполнено(Форма.Сотрудник));
	...
	Попытка
		...
		Если ФормаДлительнойОперации.Открыта() 
			...
			Если ЗаданиеВыполнено(ИдентификаторЗадания) Тогда
				Состояние(НСтр("ru='Процесс пересчета планового ФОТ завершен';uk='Процес перерахунку планового ФОП завершено'"));
...
Функция ВыполнитьПересчетФОТНаСервере()
	...
	НаименованиеЗадания = НСтр("ru='Пересчет планового ФОТ';uk='Перерахунок планового ФОП'");
	
	РезультатРаботыЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		УникальныйИдентификатор,
		"РасчетЗарплатыРасширенный.ПересчитатьФОТПлановыхНачислений",
		ПараметрыВыполнения,
		НаименованиеЗадания);
