...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("АвтоТест") Тогда
		Возврат;
	КонецЕсли;
	// Не показываем при выполнении обновления.
...
Процедура КомментарийОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	...
	Если Элементы.Задачи.ТекущиеДанные <> Неопределено
		...
		СохраненныйКомментарий = СохраненныеКомментарии.НайтиПоЗначению(ЗадачаID);
		Если СохраненныйКомментарий = Неопределено Тогда
			Если Не ПустаяСтрока(Текст) Тогда
				СохраненныеКомментарии.Добавить(ЗадачаID, Текст);
			КонецЕсли;
		Иначе
			СохраненныйКомментарий.Представление = Текст;
		КонецЕсли;
...
Процедура ЗадачиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗадачиВыборЗавершение", ЭтаФорма, ТекущиеДанные);
...
&НаКлиенте
Процедура ЗадачиВыборЗавершение(Результат, СтрокаЗадачи) Экспорт
		...
		Если ДетальныеСведенияЗадачи.ВключенХронометраж Тогда
			...
			Если ДетальныеСведенияЗадачи.ВключенХронометраж Тогда
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Включен хронометраж по ""%1""';uk='Включений хронометраж по ""%1""'"), Строка(СтрокаЗадачи.Задача));
				Состояние(Текст);
			КонецЕсли;
...
Процедура ВыполнитьЗадачуПервая(Команда)
	...
	Если ПроцессТип = "DMBusinessProcessRegistration"
		...
		Для каждого СтрокаПредмета Из СтрокаЗадачи.Предметы Цикл
			Если ИнтеграцияС1СДокументооборотКлиентСервер.ЭтоДокумент(СтрокаПредмета.Тип)
				И НЕ ПредметЗарегистрирован(СтрокаПредмета.Тип, СтрокаПредмета.ID) Тогда
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                    ...
                    |Для реєстрації відкрийте картку документа і вкажіть реєстраційний номер і дату реєстрації.'"),
					СтрокаЗадачи.Предмет));
				Возврат;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Если СтрокаЗадачи.ПроцессТип = "DMBusinessProcessInvitation" Тогда 
		...
		Если РезультатID = "ПринятоОбязательнымиУчастниками" Тогда 
			...
			ТекстВопроса = НСтр("ru='Есть непринявшие приглашения участники.
                ...
                |;uk='Є учасники, які не приняли запрошення.
                |При підтвердженні вони будуть виключені зі списку учасників. Продовжити?'");
...
Процедура ОбработатьФормуСогласноВерсииСервиса()
	...
	Заголовок = НСтр("ru='Документооборот: задачи мне';uk='Документообіг: задачі мені'");
	...
	Иначе
		...
		Если ВерсияСервиса = "0.0.0.0" Тогда
			...
			Элементы.ДекорацияФункционалНеПоддерживается.Заголовок = 
				НСтр("ru='Нет доступа к Документообороту.';uk='Немає доступу до Документообігу.'");
...
Процедура ЗаполнитьПоляЧислаЗадач()
	...
	Заголовок =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Документооборот: задачи мне (Новых: %1 / Просроченных: %2 / Всего: %3)';uk='Документообіг: задачі мені (Нових: %1 / Прострочених: %2 / Всього: %3)'"),
...
Процедура АктуализироватьДетальныеСведения(СтрокаЗадачи)
	...
	Если СтрокаЗадачи <> Неопределено И НЕ СтрокаЗадачи.Группировка Тогда
		...
		Если Не ЗначениеЗаполнено(СтрокаЗадачи.ДатаПолученияДетальныхСведений)
			или ТекущаяДата() - СтрокаЗадачи.ДатаПолученияДетальныхСведений > 5 * 60 Тогда
			...
			КонецЕсли;
			ДетальныеСведенияОЗадачах.Вставить(СтрокаЗадачи.ЗадачаId, ДетальныеСведенияЗадачи);
			СтрокаЗадачи.ДатаПолученияДетальныхСведений = ТекущаяДата();
...
Функция ПолучитьДетальныеСведенияНаСервере(РеквизитыЗадачи, НеобходимоОбновлениеФормы)
	...
	Для каждого СтрокаПредмета из РеквизитыЗадачи.Предметы Цикл
		...
		Если СтрокаПредмета.Тип = "DMFile" Тогда
			...
			КонецЕсли;
			Предмет.Вставить("Картинка", ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьИндексПиктограммыФайла(
					Предмет.Расширение));
		...
		Иначе // объект иного типа, который может иметь приложенные файлы
			...
			Если ЗначениеЗаполнено(Предмет.Ссылка) Тогда
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка при получении представления связанного объекта';uk='Помилка при отриманні представлення зв''язаного об''єкта'"), 
						УровеньЖурналаРегистрации.Ошибка,,Предмет.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				КонецПопытки;
			Иначе
				Предмет.Вставить("Представление", Представление);
			КонецЕсли;
			...
			Для каждого ФайлXDTO из СписокФайлов.files Цикл
				...
				Файл.Вставить("Картинка", 
					ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьИндексПиктограммыФайла(ФайлXDTO.extension));
				Файлы.Добавить(Файл);
			КонецЦикла;
		КонецЕсли;
	...
	Иначе
		ТекстHTML = ТекстHTML + "<p><FONT color=""#C0C0C0"">"
			+ НСтр("ru='У задачи нет описания.';uk='У задачі немає опису.'") + "</FONT></p>";
	КонецЕсли;
...
Процедура ОбновитьПредставлениеЗадачиНаСервере(РеквизитыЗадачи, ДетальныеСведенияЗадачи)
	...
	СохраненныйКомментарий = СохраненныеКомментарии.НайтиПоЗначению(РеквизитыЗадачи.ЗадачаId);
	Если СохраненныйКомментарий = Неопределено Тогда
		Комментарий = "";
	Иначе
		Комментарий = СохраненныйКомментарий.Представление;
	КонецЕсли;
...
Процедура ПереключитьХронометражЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Если ДетальныеСведенияЗадачи.ВключенХронометраж Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Включен хронометраж по ""%1""';uk='Включений хронометраж по ""%1""'"), Строка(ПараметрыОповещения.Задача));
		Состояние(Текст);
	КонецЕсли;
...
&НаСервере
Функция ПредметЗарегистрирован(ПредметТип, ПредметID)
	
	Прокси = ИнтеграцияС1СДокументооборотПовтИсп.ПолучитьПрокси();
	ОбъектыXDTO = ИнтеграцияС1СДокументооборот.ПолучитьОбъект(Прокси, ПредметТип, ПредметID);
	ОбъектXDTO = ОбъектыXDTO.objects[0];
	
	Возврат ЗначениеЗаполнено(ОбъектXDTO.regNumber) И ЗначениеЗаполнено(ОбъектXDTO.regDate);
				...
				Если ЗначениеЗаполнено(ВыборкаТипПотребителя.ТипОбъектаПотребителя) Тогда
					...
					Если Метаданные.Справочники.Содержит(ОбъектМетаданных) Тогда
						Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Элемент справочника %1';uk='Елемент довідника %1'"),
	...
	Иначе
		Если Тип = "DMBusinessProcessApprovalTaskApproval" и ВыполнитьЗадачу Тогда
			...
			Для каждого СтрокаПредмета Из ПредметыЗадачи Цикл
				Если Не ДоступнаМультипредметность Или СтрокаПредмета.РольПредмета = "Основной" Тогда
					ИнтеграцияС1СДокументооборотВызовСервера.ПриИзмененииСостоянияСогласования(, СтрокаПредмета.ID,
						СтрокаПредмета.Тип, НовоеСостояние, Ложь);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		Возврат Истина;
...
Процедура ОбработатьИсключение(ИнформацияОбОшибке)
	...
	ВерсияСервиса = ИнтеграцияС1СДокументооборот.ВерсияСервиса();
	Если ВерсияСервиса = "0.0.0.0" Тогда
		ИмяСобытия = НСтр("ru='Интеграция с Документооборотом';uk='Інтеграція з Документообігом'");
		ОбработатьФормуСогласноВерсииСервиса();
	Иначе
