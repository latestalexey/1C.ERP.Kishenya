...
Функция НачатьОбработкуЗапросов(Знач Запросы, Знач РежимВключения, РежимОтключения, Знач РежимВосстановления, Знач РежимПроверкиПрименения)
	...
	Задание = ФоновыеЗадания.Выполнить("РаботаВБезопасномРежиме.ВыполнитьМетодКонфигурации",
			...
			,
			НСтр("ru='Обработка запросов на использование внешних ресурсов...';uk='Обробка запитів на використання зовнішніх ресурсів...'"));
...
Функция ЗапросыОбработаны(Знач ИдентификаторЗадания)
	...
	Если Задание = Неопределено Тогда
		ВызватьИсключение(НСтр("ru='При обработке запросов произошла ошибка - не найдено задание обработки запросов.';uk='При обробці запитів відбулася помилка - не знайдена завдання обробки запитів.'"));
	КонецЕсли;
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		...
		Иначе
			ВызватьИсключение(НСтр("ru='При обработке запросов произошла ошибка - задание обработки запросов завершилось с неизвестной ошибкой.';uk='При обробці запитів відбулася помилка - завдання обробки запитів завершилося з невідомою помилкою.'"));
		КонецЕсли;
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		ВызватьИсключение(НСтр("ru='При обработке запросов произошла ошибка - задание обработки запросов было отменено администратором.';uk='При обробці запитів відбулася помилка - завдання обробки запитів було скасовано адміністратором.'"));
	Иначе
...
Процедура ОтменитьОбработкуЗапросов(Знач ИдентификаторЗадания)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Настройка разрешений на использование внешних ресурсов.Отмена выполнения фонового задания';uk='Настройка дозволів на використання зовнішніх ресурсів.Скасування виконання фонового завдання'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
