...
Процедура НастройкаПередачиМатериаловВПроизводствоВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЭтоРежимНоменклатурыВЦелом Тогда
		...
		ТекстСообщения = НСтр("ru='Изменение настройки возможно только для конкретной характеристики.';uk='Зміна настройки можлива тільки для конкретної характеристики.'");
		ОчиститьСообщения();
...
Процедура ЗаполнитьНастройкуПередачиМатериалов()
	...
	Запрос.УстановитьПараметр("ОпределяетсяНастройкойДляПодразделения", НСтр("ru='<как для подразделения в целом>';uk='<як для підрозділу в цілому>'"));
	Запрос.УстановитьПараметр("НастраиваетсяИндивидуально", НСтр("ru='<настраивается индивидуально>';uk='<настроюється індивідуально>'"));
...
Процедура ЗаполнитьСкладНаСервере(Склад)
	...
	Для каждого ИдентификаторСтроки Из Элементы.НастройкаПередачиМатериаловВПроизводство.ВыделенныеСтроки Цикл
		...
		ДанныеСтроки.СпособНастройки = НСтр("ru='<индивидуально для материала>';uk='<індивідуально для матеріалу>'");
