...
Функция ТекстЗапросаФормированияТаблицыДанныеРегистров(ВариантПолучениеДанныхИзРегистров, СкладИлиПодразделение = Неопределено) Экспорт
	...
	Если ВариантПолучениеДанныхИзРегистров = "ЗаказНакладнаяСерииИзОстатка" Тогда
		...
		Иначе
			...
			Если Не ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.МатериалыИРаботыВПроизводстве) Тогда
			...
			Иначе
				...
				|		МатериалыИРаботыВПроизводстве.КоличествоОстаток КАК СвободныйОстаток
				|	ИЗ
				...
				|					И Характеристика = &Характеристика
				|					И (&БезНазначения ИЛИ Назначение = &Назначение)
				|					И Подразделение = &Склад) КАК МатериалыИРаботыВПроизводстве
				|
				|	ОБЪЕДИНИТЬ ВСЕ
				|	
				|	ВЫБРАТЬ
				...
				|		ВЫБОР
				|			КОГДА МатериалыИРаботыВПроизводстве.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
				|				ТОГДА -МатериалыИРаботыВПроизводстве.Количество
				|			ИНАЧЕ МатериалыИРаботыВПроизводстве.Количество
				|		КОНЕЦ
				|	ИЗ
				...
				|		И МатериалыИРаботыВПроизводстве.Характеристика = &Характеристика
				|		И (&БезНазначения ИЛИ МатериалыИРаботыВПроизводстве.Назначение = &Назначение) 
				|		И МатериалыИРаботыВПроизводстве.Подразделение = &Склад
				|		И МатериалыИРаботыВПроизводстве.Регистратор = &Регистратор) КАК ДанныеРегистров
				|
				|СГРУППИРОВАТЬ ПО
				|	ДанныеРегистров.Серия";
...
Функция ВариантПолучениеДанныхИзРегистровПоПараметрамФормы(ПараметрыФормы, Распоряжение, ВидНоменклатуры) Экспорт
	
	ПараметрыУказанияСерий = ПараметрыФормы.ПараметрыУказанияСерий;
	
	ПараметрыУчетнойПолитикиСерий = Новый ФиксированнаяСтруктура(ЗначениеНастроекПовтИсп.НастройкиИспользованияСерий(
																											ВидНоменклатуры,
																											ПараметрыФормы.Склад));
	...
	ИначеЕсли Распоряжение <> Неопределено
		...
		Если СкладыСервер.ИспользоватьОрдернуюСхемуПриПоступлении(ПараметрыФормы.Склад,ПараметрыУказанияСерий.Дата)
