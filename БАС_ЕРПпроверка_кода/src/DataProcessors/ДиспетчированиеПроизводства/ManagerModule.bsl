...
Функция ДиспетчированиеМаршрутныхЛистов(СтруктураПараметров) Экспорт
	...
	|				ИЛИ ТрудозатратыКОформлению.Бригада.Подразделение = &Подразделение)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|					КОГДА ТаблицаДокументов.Подразделение.ГрафикРаботы <> ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
	|						ТОГДА ТаблицаДокументов.Подразделение.ГрафикРаботы
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	ДелоРодитель.Представление  = НСтр("ru='Маршрутные листы к формированию';uk='Маршрутні листи до формування'");
...
Функция СформироватьМаршрутныеЛисты(Параметры, АдресХранилища) Экспорт
	...
	Перем СписокЭтапов;
	Перем СписокРаспоряжений;
	Перем ТекущееКоличествоМЛ;
	
	ПараметрыЗаданияСформироватьМаршрутныеЛисты = Параметры.ПараметрыЗаданияСформироватьМаршрутныеЛисты;
	ПараметрыЗаданияСформироватьМаршрутныеЛисты.Свойство("УправлениеМаршрутнымиЛистами", УправлениеМаршрутнымиЛистами);
	ПараметрыЗаданияСформироватьМаршрутныеЛисты.Свойство("СписокЭтапов", СписокЭтапов);
	ПараметрыЗаданияСформироватьМаршрутныеЛисты.Свойство("СписокРаспоряжений", СписокРаспоряжений);
	ПараметрыЗаданияСформироватьМаршрутныеЛисты.Свойство("ТекущееКоличествоМЛ", ТекущееКоличествоМЛ);

	ДанныеДляФормирования = ОперативныйУчетПроизводстваВызовСервера.ДанныеДляФормированияМаршрутныхЛистов(
									ПараметрыЗаданияСформироватьМаршрутныеЛисты.Подразделение,
									СписокРаспоряжений,
									СписокЭтапов,
	...
	Для каждого ДанныеЗаполнения Из ДанныеДляФормирования Цикл
		...
		Если (НЕ ЗначениеЗаполнено(ПараметрыЗаданияСформироватьМаршрутныеЛисты.Подразделение) 
				ИЛИ ДанныеЗаполнения.Подразделение = ПараметрыЗаданияСформироватьМаршрутныеЛисты.Подразделение)
			И (ДанныеЗаполнения.НачалоРаботыКлючевогоРабочегоЦентра <> '000101010000'
					И ДанныеЗаполнения.НачалоРаботыКлючевогоРабочегоЦентра <= ПараметрыЗаданияСформироватьМаршрутныеЛисты.Окончание
				ИЛИ ДанныеЗаполнения.НачалоРаботыКлючевогоРабочегоЦентра = '000101010000'
					И ДанныеЗаполнения.Начало <= ПараметрыЗаданияСформироватьМаршрутныеЛисты.Окончание) Тогда
	...
	Если ТекущееКоличествоМЛ <> Неопределено Тогда
		АвтоОбновление = (ТекущееКоличествоМЛ + КоличествоОбработанных) < 500;
	Иначе
		АвтоОбновление = Ложь;
	КонецЕсли;
	...
	ДанныеДляДиспетчированияПроизводства = ПодготовитьДанныеДляДиспетчированияПроизводства(Параметры.ПараметрыЗаданияПолучитьДанныеДиспетчирования, СтруктураДействий);
		...
		Если СостояниеРаботыРЦ <> Неопределено Тогда
			...
			Для каждого ВыборкаМЛ Из СписокСтрок Цикл
				ПредставлениеДокумента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												...
												ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаМЛ.Номер, Ложь, Истина),
