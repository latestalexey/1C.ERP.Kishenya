...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Заголовок = НСтр("ru='Групповое изменение реквизитов';uk='Групова зміна реквізитів'");
	КонецЕсли;
...
Процедура Изменить(Команда)
	...
	Если НазначениеКнопки = "Изменить" Тогда
		Если КоличествоОбъектовДляОбработки = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Не указаны элементы для изменения';uk='Не вказані елементи для зміни'"));
			Возврат;
		КонецЕсли;
		...
		Иначе
			ТекстВопроса = НСтр("ru='Отбор не задан. Изменить все элементы?';uk='Відбір не заданий. Змінити всі елементи?'");
			ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьИзменениеПроверкаОтбораВыполнена", ЭтотОбъект);
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , , НСтр("ru='Изменение элементов';uk='Зміна елементів'"));
		КонецЕсли;
...
Процедура ВыполнитьИзменениеПроверкаОтбораВыполнена(РезультатВопроса = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЕстьНастроенныеИзменения() Тогда
		ТекстВопроса = НСтр("ru='Изменения не настроены. Выполнить перезапись элементов без изменений?';uk='Зміни не налаштовані. Виконати перезапис елементів без змін?'");
...
Процедура ВыполнитьДействияПриКонтекстномОткрытии()
	...
	ШаблонЗаголовка = НСтр("ru='Изменение выделенных элементов ""%1"" (%2)';uk='Зміна виділених елементів ""%1"" (%2)'");
	...
	Если ТипыОбъектов.Количество() > 1 Тогда
		ПредставлениеТипа = "";
		ШаблонЗаголовка = НСтр("ru='Изменение выделенных элементов (%2)';uk='Зміна виділених елементів (%2)'");
	КонецЕсли;
...
Процедура ВопросПерейтиКРазблокированиюРеквизитов()
	...
	ТекстВопроса = НСтр("ru='Реквизит заблокирован, перейти к разблокированию реквизитов?';uk='Реквізит заблокований, перейти до розблокування реквізитів?'");
	...
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки, , КодВозвратаДиалога.Да, НСтр("ru='Реквизит заблокирован';uk='Реквізит заблокований'"));
...
Процедура УстановитьКнопкиНаВремяИзменения(НачалоИзменения)
	...
	Если НачалоИзменения Тогда
		Элементы.ФормаИзменить.Заголовок = НСтр("ru='Прервать';uk='Перервати'");
	Иначе
		Если ОбъектыКоторыеНеУдалосьИзменить.Количество() > 0 Тогда
			Элементы.ФормаИзменить.Заголовок = НСтр("ru='Повторить изменение';uk='Повторити зміну'");
		Иначе
			Элементы.ФормаИзменить.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ИзменитьОбъекты()
	...
	Если Объект.ИзменятьВТранзакции Тогда
		...
		Если ТОНастройкаПорции = 1 Тогда // обработка одним вызовом
			...
			ПоказатьОповещениеПользователя(НСтр("ru='Изменение выделенных элементов';uk='Зміна виділених елементів'"), ,НСтр("ru='Пожалуйста подождите, обработка может занять некоторое время...';uk='Будь ласка, почекайте, обробка може зайняти деякий час...'"));
	...
	Иначе
		...
		Состояние(НСтр("ru='Обрабатываются элементы...';uk='Обробляються елементи...'"), 0, НСтр("ru='Изменение выделенных элементов';uk='Зміна виділених елементів'"));
...
Процедура ОбработатьРезультатИзменения(РезультатИзменения = Неопределено, ПродолжитьОбработку = Неопределено)
	...
	Пока ПродолжитьОбработку Цикл
		...
		Если Объект.ИзменятьВТранзакции Тогда
			ТекстПредупреждения = НСтр("ru='При изменении элементов обнаружены ошибки - изменения отменены.';uk='При зміні елементів виявлені помилки - зміни скасовані.'");
		...
		ТекстВопроса = НСтр("ru='При изменении элементов (группы элементов) возникли ошибки.
            ...
            |'
            |;uk='При зміні елементів (групи елементів) виникли помилки.
            |Перервати зміну елементів і перейти до перегляду помилок?
            |'");
		Кнопки = Новый СписокЗначений;
		...
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Больше не спрашивать';uk='Більше не запитувати'"));
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьРезультатИзмененияОтветПолучен", ЭтотОбъект, РезультатИзменения);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки, , КодВозвратаДиалога.Прервать, НСтр("ru='Ошибки при изменении элементов';uk='Помилки при зміні елементів'"));
		Возврат;
	КонецЦикла;
	...
	Если ТекущееСостояниеИзменения.ПоказыватьПроцентОбработанных Тогда
		...
		Состояние(НСтр("ru='Обрабатываются элементы...';uk='Обробляються елементи...'"), ТекущийПроцент, НСтр("ru='Изменение выделенных элементов';uk='Зміна виділених елементів'"));
	КонецЕсли;
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	Если ОбработкаЗавершена Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Изменение реквизитов элементов';uk='Зміна реквізитів елементів'"), , 
			ПодставитьПараметрыВСтроку(НСтр("ru='Изменены элементы (%1).';uk='Змінені елементи (%1).'"), ТекущееСостояниеИзменения.КоличествоИзмененных));
	...
	Если ОбработкаЗавершена Тогда
		ШаблонСообщения = НСтр("ru='Изменения выполнены во всех выбранных элементах (%2).';uk='Зміни виконані у всіх вибраних елементах (%2).'");
	Иначе
		Если Объект.ИзменятьВТранзакции Или ТекущееСостояниеИзменения.КоличествоИзмененных = 0 Тогда
			ШаблонСообщения = НСтр("ru='Изменения не выполнены.';uk='Зміни не виконані.'");
		Иначе
			ШаблонСообщения = НСтр("ru='Изменения выполнены частично.
                                        |Изменено: %1; Не удалось изменить: %3'
                                        |;uk='Зміни виконані частково.
                                        |Змінено: %1; Не вдалося змінити: %3'");
		КонецЕсли;
	КонецЕсли;
	...
	Если Объект.ИзменятьВТранзакции И Не ОбработкаЗавершена Тогда
		...
		Если КоличествоПропущенных > 0 И Не ТекущееСостояниеИзменения.ПрерватьИзменение Тогда
			...
			СтрокаТаблицы.Объект = ПодставитьПараметрыВСтроку(НСтр("ru='... и другие элементы (%1)';uk='... і інші елементи (%1)'"), КоличествоПропущенных);
			СтрокаТаблицы.Причина = НСтр("ru='Пропущены, так как не были изменены один или более элементов.';uk='Пропущені, так як не було змінено один або більше елементів.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура НазадСервер()
	...
	ОбъектыКоторыеНеУдалосьИзменить.Очистить();
	Элементы.ФормаНазад.Видимость = Ложь;
	Элементы.ФормаИзменить.Заголовок = НСтр("ru='Изменить реквизиты';uk='Змінити реквізити'");
...
Процедура ПерейтиНаСтраницуВсеГотово()
	...
	Элементы.НадписьУспешноЗавершено.Заголовок = ПодставитьПараметрыВСтроку(
		НСтр("ru='Реквизиты выбранных элементов были изменены.
            ...
            |;uk='Реквізити вибраних елементів були змінені.
            |Усього змінено елементів: %1'"), ТекущееСостояниеИзменения.КоличествоИзмененных);
	Элементы.ФормаИзменить.Заголовок = НСтр("ru='Готово';uk='Готово'");
	Элементы.ФормаНазад.Видимость = Истина;
...
Функция ИзменитьНаСервере(Знач ОстанавливатьИзменениеПриОшибке)
	...
	Иначе
		НаименованиеЗадания = НСтр("ru='Групповое изменение элементов';uk='Групове редагування елементів'");
...
Процедура СформироватьПояснениеПоАвтонумерации()
	...
	ИначеЕсли Автонумерация Тогда
		ПояснениеПоАвтонумерации = НСтр("ru='<Установить автоматически>';uk='<Встановити автоматично>'");
	Иначе
		ПояснениеПоАвтонумерации = НСтр("ru='<Очистить>';uk='<Очистити>'");
	КонецЕсли;
...
Функция ОписанияКолонокТаблицыРеквизитов()
	...
	ОписаниеКолонки.Тип = Новый ОписаниеТипов("Строка");
	ОписаниеКолонки.Представление = НСтр("ru='Реквизит';uk='Реквізит'");
	...
	ОписаниеКолонки.Представление = НСтр("ru='Новое значение';uk='Нове значення'");
	ОписаниеКолонки.ВидПоля = ВидПоляФормы.ПолеВвода;
...
Процедура СформироватьПояснениеОНастроенныхИзменениях()
	...
	Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
		Пояснение = НСтр("ru='реквизит';uk='реквізит'") + " " + Пояснение;
	ИначеЕсли ИзменяемыеРеквизиты.Количество() > 1 Тогда
		Пояснение = НСтр("ru='реквизиты';uk='реквізити'") + " " + Пояснение;
	КонецЕсли;
	
	Если Не ПустаяСтрока(Пояснение) Тогда
		Пояснение = Пояснение + " " + НСтр("ru='в выбранных элементах';uk='в обраних елементах'");
	КонецЕсли;
	...
	Для Каждого ТабличнаяЧасть Из ИзменяемыеТабличныеЧасти Цикл
		...
		Если ИзменяемыеРеквизиты.Количество() > 3 Тогда
			Если Не ПустаяСтрока(Пояснение) Тогда
				Пояснение = Пояснение + ", ";
			КонецЕсли;
			Пояснение = Пояснение + ПодставитьПараметрыВСтроку(НСтр("ru='реквизиты (%1)';uk='реквізити (%1)'"), ИзменяемыеРеквизиты.Количество());
		Иначе
			Для Каждого Реквизит Из ИзменяемыеРеквизиты Цикл
				Если Не ПустаяСтрока(Пояснение) Тогда
					Пояснение = Пояснение + ", ";
				...
				Если ИзменяемыеРеквизиты.Найти(Реквизит) = 0 Тогда
					Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
						Пояснение = Пояснение + НСтр("ru='реквизит';uk='реквізит'") + " ";
					ИначеЕсли ИзменяемыеРеквизиты.Количество() > 1 Тогда
						Пояснение =  Пояснение + НСтр("ru='реквизиты';uk='реквізити'") + " ";
					КонецЕсли;
				КонецЕсли;
				Пояснение = Пояснение + """" + Реквизит + """";
			КонецЦикла;
		КонецЕсли;
		Пояснение = Пояснение + " " 
			+ ПодставитьПараметрыВСтроку(НСтр("ru='в табличной части ""%1""';uk='у табличній частині ""%1""'"), ТабличнаяЧасть.Ключ);
	КонецЦикла;
	
	Если Не ПустаяСтрока(Пояснение) Тогда
		Если ИзменяемыеТабличныеЧасти.Количество() > 0 Тогда
			Если ЕстьОтборПоСтрокам Тогда 
				Пояснение = Пояснение + " " + НСтр("ru='в тех строках выбранных элементов, которые удовлетворяют условиям отбора';uk='в тих рядках вибраних елементів, які задовольняють умовам відбору'")
			Иначе
				Пояснение = Пояснение + " " + НСтр("ru='<b>во всех строках</b> выбранных элементов';uk='<b>у всіх рядках</b> вибраних елементів'")
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьВыбранныеОбъекты Тогда
		Если Не ПустаяСтрока(Пояснение) Тогда
			Пояснение = НСтр("ru='Изменить';uk='Змінити'") + " " + Пояснение + ".";
		Иначе
			Пояснение = НСтр("ru='Выполнить <b>перезапись</b> выбранных элементов.';uk='Виконати <b>перезапис</b> вибраних елементів.'");
		КонецЕсли;
	Иначе
		Пояснение = НСтр("ru='Не выбраны элементы, реквизиты которых необходимо изменить.';uk='Не вибрані елементи, реквізити яких необхідно змінити.'");
	КонецЕсли;
...
Процедура ОбновитьНадписьКоличествоВыбранных()
	...
	Если ЕстьНастроенныеОтборы() Тогда
		...
		ТекстНадписи = ЧислоЦифрамиПредметИсчисленияПрописью(КоличествоВыбранныхОбъектов, НСтр("ru='элемент,элемента,элементов';uk='елемент,елемента,елементів'"));
	Иначе
		ТекстНадписи = НСтр("ru='Все элементы';uk='Всі елементи'");
	КонецЕсли;
...
Процедура УстановитьНастройкуИзменения(Знач Настройка)
	...
	Если ЕстьЗаблокированные Тогда
		ПоказатьПредупреждение(, НСтр("ru='Некоторые реквизиты заблокированы для изменения, изменения не установлены.';uk='Деякі реквізити заблоковані для зміни, зміни не встановлені.'"));
	КонецЕсли;
...
Функция ПараметрыКонструктора(Формула)
	...
	Результат.Вставить("ОперандыЗаголовок", НСтр("ru='Доступные реквизиты';uk='Доступні реквізити'"));
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя)
	...
	ИначеЕсли ВРег(КлассОМ) = "РЕГИСТРРАСЧЕТА" Тогда
		...
		Иначе
			...
			Иначе
				ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ru='Неизвестный тип объекта метаданных ""%1""';uk='Невідомий тип об''єкта метаданих ""%1""'"), ПолноеИмя);
			КонецЕсли;
		КонецЕсли;
...
Функция ВидОбъектаПоТипу(Тип)
	...
	Иначе
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ru='Неверный тип значения параметра (%1)';uk='Невірний тип значення параметра (%1)'"), Строка(Тип));
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты)
	...
	ИначеЕсли ТипЗнч(Реквизиты) = Тип("Массив") Или ТипЗнч(Реквизиты) = Тип("ФиксированныйМассив") Тогда
		...
		КонецЦикла;
	Иначе
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ru='Неверный тип второго параметра Реквизиты: %1';uk='Невірний тип другого параметра Реквізити: %1'"), Строка(ТипЗнч(Реквизиты)));
	КонецЕсли;
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ru='Общий модуль ""%1"" не найден.';uk='Загальний модуль ""%1"" не знайдений.'"), Имя);
	КонецЕсли;
#Иначе
	Модуль = Вычислить(Имя);
...
Функция ПроверитьВерсиюИРежимСовместимостиПлатформы()
	...
	Если Не (Лев(Информация.ВерсияПриложения, 3) = "8.3"
		...
		ВызватьИсключение НСтр("ru='Обработка предназначена для запуска на версии платформы
            | 8.3 с отключенным режимом совместимости или выше'
            |;uk='Обробка призначена для запуску на версії платформи
            |8.3 з відключеним режимом сумісності або вище'");
