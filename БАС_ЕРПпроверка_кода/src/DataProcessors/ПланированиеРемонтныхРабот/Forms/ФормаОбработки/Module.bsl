...
Процедура ГиперссылкаДетализацииРемонта(Элемент)
	...
	Если Элементы.СтраницыДетализацииРемонта.Видимость Тогда
		Элементы.ГиперссылкаДетализацииРемонта.Заголовок = НСтр("ru='Скрыть детали распоряжения на ремонт';uk='Приховати деталі розпорядження на ремонт'");
	Иначе
		Элементы.ГиперссылкаДетализацииРемонта.Заголовок = НСтр("ru='Показать детали распоряжения на ремонт';uk='Показати деталі розпорядження на ремонт'");
	КонецЕсли;
...
Процедура РемонтыКодЗаказаНаРемонтПриИзменении(Элемент)
	...
	Если ТекущиеДанные.КодЗаказаНаРемонт <> 0 Тогда
		...
		Если ТекущиеДанные.КодЗаказаНаРемонт = 1 Тогда
			...
			ЗаполнитьЗначенияСвойств(НовыйЗаказ, ТекущиеДанные, "ОбъектЭксплуатации, ДатаНачала, Длительность, Подразделение");
...
Процедура РемонтыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если ТекущиеДанные.ОбъектЭксплуатации <> ТекущийОбъектЭксплуатацииСпискаВыбораЗаказа Тогда
		...
		Список.Очистить();
		Список.Добавить(1, НСтр("ru='<новый заказ>';uk='<нове замовлення>'"));
...
Процедура ЗаказыНаРемонтДлительностьПриИзменении(Элемент)
	...
	ЗаполнитьРемонтыПоОтбору(
		Новый Структура("КодЗаказаНаРемонт", ТекущиеДанные.КодЗаказаНаРемонт),
		Новый Структура("ДлительностьЗаказаНаРемонт", ТекущиеДанные.Длительность));
...
Процедура ЗаказыНаРемонтПередУдалениемНаСервере()
	...
	СтруктураПустыхПолейРемонта = Новый Структура(
		"Отмечен, КодЗаказаНаРемонт, ПредставлениеЗаказаНаРемонт, ДатаНачалаЗаказаНаРемонт, ДлительностьЗаказаНаРемонт, ПодразделениеЗаказаНаРемонт",
		Ложь, 0, "", Дата(1, 1, 1), 0, Справочники.СтруктураПредприятия.ПустаяСсылка());
...
Процедура РемонтыОбъединитьРемонтыВОдинЗаказ(Команда)
	...
	Для Каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		...
		Если СтрокаРемонта.ЭтоСтрокаВидаРемонта И ОбъектЭксплуатации <> СтрокаРемонта.ОбъектЭксплуатации Тогда
            ...
            |;uk='Виділені розпорядження оформляються на різні об''єкти ремонту,
            |і не можуть бути виконані в рамках одного замовлення на ремонт'"));
			Возврат;
		КонецЕсли;
	КонецЦикла;
	
	СписокВыбора = Новый СписокЗначений;
	СписокВыбора.Добавить(1, НСтр("ru='<новый заказ>';uk='<нове замовлення>'"));
	...
	СписокВыбора.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("РемонтыОбъединитьРемонтыВОдинЗаказЗавершение", ЭтотОбъект, Новый Структура("ВыделенныеСтроки, НайденныеСтроки, ОбъектЭксплуатации", ВыделенныеСтроки, НайденныеСтроки, ОбъектЭксплуатации)), НСтр("ru='Выберите заказ на ремонт';uk='Виберіть замовлення на ремонт'"));
КонецПроцедуры
...
Процедура РемонтыОбъединитьРемонтыВОдинЗаказЗавершение(ВыбранныйЭлемент1, ДополнительныеПараметры) Экспорт
    ...
    Если ВыбранныйЭлемент <> Неопределено Тогда
        ...
        СтруктураЗаполнения = Новый Структура("Отмечен, КодЗаказаНаРемонт, ПредставлениеЗаказаНаРемонт, ДатаНачалаЗаказаНаРемонт, ДлительностьЗаказаНаРемонт, ПодразделениеЗаказаНаРемонт");
        ...
        Если ВыбранныйЭлемент.Значение = 1 Тогда
            ...
            ЗаполнитьЗначенияСвойств(НовыйЗаказ, ТекущиеДанные, "ДатаНачала, Длительность, Подразделение");
...
Процедура ЗаказыНаРемонтЗаполнитьМероприятие(Команда)
	...
	РемонтноеМероприятие = Неопределено;

	
	ОткрытьФорму("Справочник.РемонтныеМероприятия.ФормаВыбора",,,,,, Новый ОписаниеОповещения("ЗаказыНаРемонтЗаполнитьМероприятиеЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Ложь);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не планируется>';uk='<не планується>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не заполнен>';uk='<не заповнений>'"));
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ЗаказыНаРемонтДлительность.Имя);

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ЗаказыНаРемонт.Длительность");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	Элемент.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Истина);
...
Функция ПолучитьПредставлениеЗаказа(Код)
	...
	ПредставлениеЗаказа = НСтр("ru='№ %Код%';uk='№ %Код%'");
	...
	Иначе
		...
		ИначеЕсли СтруктураПерехода.ТекущаяСтраница = Элементы.СтраницаНавигацииЗаказыНаРемонт Тогда
			...
			Иначе
				...
				ТекстВопроса = НСтр("ru='На текущем шаге обработки были созданы документы заказов на ремонт';uk='На поточному кроці обробки були створені документи замовлень на ремонт'");
				ВариантыОтветов = Новый СписокЗначений;
				ВариантыОтветов.Добавить("ПометитьЗаказыНаУдаление", НСтр("ru='Пометить заказы на удаление';uk='Позначити замовлення на вилучення'"));
				ВариантыОтветов.Добавить("ОтменитьПереход", НСтр("ru='Отменить переход назад';uk='Скасувати перехід назад'"));
				ПоказатьВопрос(
...
// Отражает выставленных пользователем простой отбор в полях компоновщика настроек СКД
//
// Параметры:
// 		ИмяПоля - Строка - Имя поля отбора
// 		Значение - Произвольное значение - Значения поля отбора
//
&НаКлиенте
...
Процедура ЗаполнитьРемонтыПоДаннымОбработки()
	...
	СтруктураЗаполнения = Новый Структура("ДатаНачалаЗаказаНаРемонт, ДлительностьЗаказаНаРемонт, ПодразделениеЗаказаНаРемонт");
...
Процедура СоздатьЗаказыНаРемонт(Отказ = Ложь)
    ...
    |;uk='При проведенні документа %Заказ% виявлені помилки - %КоличествоОшибок% шт.
...
Процедура ПроверитьЗаполнениеРемонтов(Отказ, АктуальныеКодыЗаказовНаРемонт, Строки=Неопределено)
	...
	ШаблонОшибки = НСтр("ru='Не заполнено поле ""%Поле%"" в строке таблицы ""Распоряжения на ремонт""';uk='Не заповнено поле ""%Поле%"" у рядку таблиці ""Розпорядження на ремонт""'");
	...
	Для Каждого СтрокаРемонта Из Строки Цикл
		...
		Если СтрокаРемонта.ЭтоСтрокаВидаРемонта Тогда
			...
			Если СтрокаРемонта.Отмечен Тогда
				...
				Иначе
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						СтрЗаменить(ШаблонОшибки, "%Поле%", НСтр("ru='Заказ на ремонт';uk='Замовлення на ремонт'")),,
						"Ремонты",,
						Отказ);
...
Процедура ПроверитьЗаполнениеЗаказовНаРемонт(Отказ, АктуальныеКодыЗаказовНаРемонт)
	...
	ШаблонОшибки = НСтр("ru='Не заполнено поле ""%Поле%""';uk='Не заповнено поле ""%Поле%""'");
	ШаблонАдресаОшибки = НСтр("ru='в строке %НомерСтроки% таблицы ""Заказы на ремонт""';uk='у рядку %НомерСтроки% таблиці ""Замовлення на ремонт""'");
	...
	Для Каждого ЗаказНаРемонт Из Объект.ЗаказыНаРемонт Цикл
		...
		Если АктуальныеКодыЗаказовНаРемонт.Получить(ЗаказНаРемонт.КодЗаказаНаРемонт) <> Неопределено Тогда // Для данного заказа есть ремонты
			...
			Если Не ЗначениеЗаполнено(ЗаказНаРемонт.ДатаНачала) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрЗаменить(ШаблонОшибки, "%Поле%", НСтр("ru='Дата начала';uk='Дата початку'")) + АдресОшибки,,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.ЗаказыНаРемонт", ЗаказНаРемонт.НомерСтроки, "ДатаНачала"),,
					Отказ);
				
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ЗаказНаРемонт.Длительность) Тогда
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрЗаменить(ШаблонОшибки, "%Поле%", НСтр("ru='Длительность';uk='Тривалість'")) + АдресОшибки,,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.ЗаказыНаРемонт", ЗаказНаРемонт.НомерСтроки, "Длительность"),,
					Отказ);
			...
			Если Не ЗначениеЗаполнено(ЗаказНаРемонт.Подразделение) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрЗаменить(ШаблонОшибки, "%Поле%", НСтр("ru='Подразделение';uk='Підрозділ'")) + АдресОшибки,,
