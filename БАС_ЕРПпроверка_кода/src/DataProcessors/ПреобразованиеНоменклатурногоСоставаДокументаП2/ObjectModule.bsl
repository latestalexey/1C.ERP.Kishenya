...
Процедура ПривестиСтрокиКНовымПравилам(ТаблицаНомСостава, ИмяТабЧасти)
	...
	Для каждого СтрокаТЧ Из СтрокиНомСоставаДокумента Цикл
		...
		ИначеЕсли СтрокаТЧ.ПричинаКорректировкиНовая = "Зміна кількості"
			...
			ИзменениеКоличестваПриИзменении(СтрокаТоваровСнятыеПозиции, ИмяТабЧасти,,КэшированныеЗначения);
		...
		ИначеЕсли СтрокаТЧ.ПричинаКорректировкиНовая = "Зміна ціни" Тогда
			...
			Иначе
				...
				Если ЗначениеЗаполнено(СтрокаТЧ.НомерСтрокиНН) Тогда
					...
					Если ЗначениеЗаполнено(СтрокаТЧ.ЦенаБезНДС) Тогда
						ЦенаИзНН = СтрокаТЧ.ЦенаБезНДС;
					Иначе	 
						...
						ЦенаИзНН = СтрокаУказаннаяПользователем.ЦенаБезНДС;
					КонецЕсли;
				...
				СтрокаТоваровСнятыеПозиции.Цена = -ЦенаИзНН; 
				ИзменениеКоличестваПриИзменении(СтрокаТоваровСнятыеПозиции, ИмяТабЧасти, Ложь, КэшированныеЗначения);
				...
				СтрокаТоваровОставшаясяЧасть.Цена = ЦенаИзНН + СтрокаТЧ.Цена;
...
Функция ПолучитьТаблицуНоменклатурыНалоговогоДокументаДляПечати(Шапка)
	...
	Попытка
		...
		Документы.НалоговаяНакладная.Печать(НалоговыйДокументОбъект, Новый Структура("ТолькоФормироватьСтруктуруПоказателейПечатнойФормы"), КоллекцияПечатныхФорм, Неопределено, Неопределено);
		...
		Если ПечатнаяФормаНН.Колонки.Найти("ROWNUM") = Неопределено Тогда
		    ПечатнаяФормаНН.Колонки.Добавить("НомерСтрокиНН", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(4,0)));
		Иначе	
	...
	Если Не РезультатЗапроса.Пустой() Тогда 
		...
		Пока Выборка.Следующий() Цикл
			...
			Попытка 
				...
				Документы.Приложение2КНалоговойНакладной.Печать(Выборка.Ссылка, Новый Структура("ТолькоФормироватьСтруктуруПоказателейПечатнойФормы"), КоллекцияПечатныхФорм, Неопределено, Неопределено);
...
Процедура ФинализироватьТаблицу(РезультирующаяТаблица, ПечатнаяФормаНН)
	...
	Для каждого СтрокаКУдалению Из МассивСтрокКУдалению Цикл
		РезультирующаяТаблица.Удалить(СтрокаКУдалению);
	КонецЦикла;		
...
Процедура ИзменениеКоличестваПриИзменении(ТекущаяСтрока, ИмяТабЧасти, ЭтоИзменениеКоличества = Истина, КэшированныеЗначения)
