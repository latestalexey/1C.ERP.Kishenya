...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НомерДокумента = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ЗаказОбъект.Номер);
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Планирование заказа №%1 от %2';uk='Планування замовлення №%1 від %2'"),
									НомерДокумента,
	...
	Если Дата <> Неопределено Тогда
		...
		Если Модифицированность Тогда
			ТекстВопроса = НСтр("ru='Для перехода к диагностике графика необходимо сохранить результат расчета.
                                        ...
                                        |;uk='Для переходу до діагностики графіка необхідно зберегти результат розрахунку.
                                        |Зберегти та перейти до діагностики?'");
										
			СписокКнопок = Новый СписокЗначений;
			СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Сохранить и перейти к диагностике';uk='Зберегти та перейти до діагностики'"));
...
Процедура КомандаУстановитьМасштаб(Команда)
	...
	Масштаб = СтрЗаменить(Команда.Имя, "Масштаб", "");
...
Процедура ПолучитьИнтервалы(ПараметрыДокумента)
	...
	|	И НЕ Т.Регистратор ССЫЛКА Документ.ЗаказПереработчику
...
Функция СохранитьРезультатРасчетаНаСервере(ТекстОшибки)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru='Не удалось сохранить результат расчета.';uk='Не вдалося зберегти результат розрахунку.'");
		Возврат Ложь;
...
Функция РассчитатьГрафикВыпуска()
	...
	НаименованиеЗадания = НСтр("ru='Расчет графика выпуска продукции';uk='Розрахунок графіка випуску продукції'");
...
Процедура ВывестиДиаграммуГанта(ИзменилсяМасштаб = Ложь)
	...
	Если ИзменилсяМасштаб Тогда
		Элементы.ФормаМасштабАвто.Пометка    = ПустаяСтрока(Масштаб);
		Элементы.ФормаМасштабКрупный.Пометка = (Масштаб = "Крупный");
		Элементы.ФормаМасштабСредний.Пометка = (Масштаб = "Средний");
		Элементы.ФормаМасштабМелкий.Пометка  = (Масштаб = "Мелкий");
	КонецЕсли;
		
	ПланированиеПроизводстваКлиентСервер.ВывестиДиаграммуГанта(ДиаграммаГанта, Элементы.Продукция.ТекущиеДанные, Ошибки, НеразвернутыеТочки,, Масштаб);
