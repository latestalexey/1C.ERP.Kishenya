...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("ru='Универсальный обмен данными в формате XML (%ВерсияОбработки%)';uk='Універсальний обмін даними у форматі XML (%ВерсияОбработки%)'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если СохраненныйРежимЗагрузки
		...
		Если Объект.НастройкаАвтоматическойЗагрузкиДанных = 1 Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Выполнить загрузку данных из файла обмена?';uk='Виконати завантаження даних з файлу обміну?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура КаталогИнформационнойБазыДляПодключенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Заголовок = НСтр("ru='Выберите каталог информационной базы';uk='Виберіть каталог інформаційної бази'");
...
Процедура ИмяФайлаПравилПриИзменении(Элемент)
	...
	Если ПустаяСтрока(ИмяФайлаПравил) Или Не Файл.Существует() Тогда
		СообщитьПользователю(НСтр("ru='Не найден файл правил обмена';uk='Не знайдений файл правил обміну'"), "ИмяФайлаПравил");
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Загрузить правила обмена данными?';uk='Завантажити правила обміну даними?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,НСтр("ru='Файл обмена';uk='Файл обміну'"),, УникальныйИдентификатор);
...
Процедура ПолучитьИнформациюОФайлеОбменаЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Исключение
			...
			СообщитьПользователю(НСтр("ru='Не удалось прочитать файл обмена.';uk='Не вдалося прочитати файл обміну.'"));
			ОчиститьДанныеОФайлеДляЗагрузкиДанных();
...
Процедура УдалениеУдалить(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Удалить выбранные данные в информационной базе?';uk='Вилучити обрані дані в інформаційній базі?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура УдалениеУдалитьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Состояние(НСтр("ru='Выполняется удаление данных. Пожалуйста, подождите...';uk='Виконується вилучення даних. Будь ласка, почекайте...'"));
		УдалитьНаСервере();
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаОбменаВХранилище,НСтр("ru='Файл обмена';uk='Файл обміну'"),, УникальныйИдентификатор);
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если ЭтоLinuxКлиент() И ПрямаяВыгрузка = 1 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Прямое подключение к информационной базе не поддерживается в клиенте под управлением ОС Linux.';uk='Пряме підключення до інформаційної бази не підтримується в клієнті під управлінням ОС Linux.'"));
		Возврат;
	КонецЕсли;
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаПравилВХранилище,НСтр("ru='Файл правил обмена';uk='Файл правил обміну'"),, УникальныйИдентификатор);
...
Процедура ПрочитатьПравилаОбменаЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ru='Выполняется чтение правил обмена. Пожалуйста, подождите...';uk='Виконується читання правил обміну. Будь ласка, почекайте...'"));
...
Функция ИменаФайловПравилИОбменаСовпадают()
	...
	Если ВРег(СокрЛП(ИмяФайлаПравил)) = ВРег(СокрЛП(ИмяФайлаДанных)) Тогда
        ...
        |;uk='Файл правил обміну не може збігатися з файлом даних.
        |Виберіть інший файл для вивантаження даних.'"));
		Возврат Истина;
...
Процедура ЗаполнитьСписокТиповДоступныхДляУдаления()
	...
	СтрокаДерева.Представление = НСтр("ru='Справочники';uk='Довідники'");
	...
	СтрокаДерева = ДеревоДанных.Строки.Добавить();
	СтрокаДерева.Представление = НСтр("ru='Планы видов характеристик';uk='Плани видів характеристик'");
	...
	СтрокаДерева = ДеревоДанных.Строки.Добавить();
	СтрокаДерева.Представление = НСтр("ru='Документы';uk='Документи'");
...
Функция ВыполнитьПодключениеКИБПриемникуНаСервере()
	...
	Если РезультатПодключения <> Неопределено Тогда
		...
		СообщитьПользователю(НСтр("ru='Подключение успешно установлено.';uk='Підключення успішно встановлене.'"));
...
Процедура ОткрытьФайлЗагрузкиНаСервере(АдресФайла)
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() Тогда
			...
			СообщитьПользователю(НСтр("ru='Не найден файл обмена на сервере.';uk='Не знайдений файл обміну на сервері.'"), "ИмяФайлаОбмена");
			Возврат;
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Если Объект.БезопаснаяЗагрузка Тогда
		...
		Иначе
			СообщитьПользователю(НСтр("ru='Не указан файл правил для загрузки данных.';uk='Не вказано файл правил для завантаження даних.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Универсальный обмен данными в формате XML';uk='Універсальний обмін даними у форматі XML'",ОбъектДляСервера.КодОсновногоЯзыка()),
...
Функция ИмяФайлаНаСервереИлиКлиенте(ИмяРеквизита ,Знач АдресФайла, Знач ИмяФайлаДляРасширения = ".xml",
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() И ПроверятьСуществование Тогда
			...
			СообщитьПользователю(НСтр("ru='Указанный файл не существует.';uk='Зазначений файл не існує.'"));
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка И Не Объект.ФлагОшибки Тогда
		...
		ИмяСохраняемогоФайла = ?(Объект.АрхивироватьФайл, НСтр("ru='Файл выгрузки.zip';uk='Файл вивантаження.zip'"),НСтр("ru='Файл выгрузки.xml';uk='Файл вивантаження.xml'"));
...
Функция ВыполнитьВыгрузкуНаСервере()
	...
	Если Не ПрямаяВыгрузка Тогда
		...
		Если ИмяВременногоФайлаДанных = Неопределено Тогда
			...
			СообщитьПользователю(НСтр("ru='Не определен файл данных';uk='Не визначений файл даних'"));
	...
	Если ОбъектДляСервера.ФлагРежимОтладкиОбработчиков Тогда
		...
		Если Не Файл.Существует() Тогда
			...
			СообщитьПользователю(НСтр("ru='Файл внешней обработки отладчиков событий не существует на сервере';uk='Файл зовнішньої обробки налагоджень подій не існує на сервері'"));
			Возврат Неопределено;
		...
		Если Отказ Тогда
			...
			СообщитьПользователю(НСтр("ru='Не удалось выгрузить обработчики событий';uk='Не вдалося вивантажити обробники подій'"));
			Возврат "";
...
Процедура ИзменитьРежимОбработки(РежимРаботы)
	...
	?(РежимРаботы, НСтр("ru='Режим работы (на клиенте)';uk='Режим роботи (на клієнті)'"), НСтр("ru='Режим работы (на сервере)';uk='Режим роботи (на сервері)'"));
...
Процедура ПоместитьФайлПравилЗагрузкиВХранилище()
	...
	НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,
		?(ЧерезХранилище, НСтр("ru='Файл обмена';uk='Файл обміну'"), ИмяФайлаПравилЗагрузки), ЧерезХранилище, УникальныйИдентификатор);
