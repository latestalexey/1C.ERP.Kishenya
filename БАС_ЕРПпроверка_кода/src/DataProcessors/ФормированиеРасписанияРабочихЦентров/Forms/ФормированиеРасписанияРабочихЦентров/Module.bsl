...
Процедура РасписаниеРабочихЦентровПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если СтрокаРасписание.Начало <> '000101010000' 
		...
		ПоказатьПредупреждение(,НСтр("ru='Начало работы должно быть в периоде, на который составляется расписание.';uk='Початок роботи має бути в періоді, на який складається розклад.'"));
		Отказ = Истина;
	...
	ИначеЕсли СтрокаРасписание.Окончание <> '000101010000' 
		...
		ПоказатьПредупреждение(,НСтр("ru='Окончание работы должно быть в периоде, на который составляется расписание.';uk='Закінчення роботи має бути в періоді, на який складається розклад.'"));
		Отказ = Истина;
	
	ИначеЕсли НЕ ПроверитьПериодРасписанияВВеденногоВручную(СтрокаРасписание.РабочийЦентр) Тогда
		
		ПоказатьПредупреждение(,НСтр("ru='Начало работы не может быть позже начала следующих маршрутных листов.';uk='Початок роботи не може бути пізніше початку наступних маршрутних листів.'"));
		Отказ = Истина;
...
Процедура РасписаниеРабочихЦентровВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "РасписаниеРабочихЦентровЕстьПроблема" Тогда
		...
		Если СтрокаРасписание.ЕстьПроблема Тогда
			ПоказатьПредупреждение(, СтрокаРасписание.ПроблемыПриФормированииРасписания,, НСтр("ru='Информация о расписании';uk='Інформація про розклад'"));
		КонецЕсли; 
...
Процедура КомандаУместитьВГрафикРаботы(Команда)
	...
	Если НЕ ТребуетсяРасчетРасписания(РЦ_РабочийЦентр) Тогда
		ПоказатьПредупреждение(,НСтр("ru='Нет маршрутных листов, которые требуется уместить в график работы.';uk='Немає маршрутних листів, які потрібно вмістити в графік роботи.'"),, НСтр("ru='Информация';uk='Інформація'"));
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = НСтр("ru='Изменить объем работ так чтобы маршрутные листы уместились в график?';uk='Змінити обсяг робіт так щоб маршрутні листи вмістилися в графік?'");
	ОписаниеОповещения = Новый ОписаниеОповещения("УместитьВГрафикРаботыЗавершение", ЭтаФорма, РЦ_РабочийЦентр);
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет,,, НСтр("ru='Уместить в график работы';uk='Вмістити в графік роботи'"));
...
Процедура ВыборПериодаИВидаРабочегоЦентра()
	...
	Если ОтборПодразделение.Пустая() Тогда
		ТекстСообщения = НСтр("ru='Необходимо выбрать подразделение.';uk='Необхідно вибрати підрозділ.'");
...
Процедура РазбитьМаршрутныйЛист()
	...
	Если ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ru='Необходимо выбрать маршрутный лист.';uk='Необхідно вибрати маршрутний лист.'"));
		Возврат;
...
Процедура УстановитьСтатус(ЗначениеСтатуса, ПредставлениеСтатуса)
	...
	ТекстВопроса = НСтр("ru='У выделенных в списке документов будет установлен статус ""%1"". Продолжить?';uk='У виділених в списку документів буде встановлено статус ""%1"". Продовжити?'");
		...
		Если МассивСтрокНедоступныхДляИзменения.Количество() = ИдентификаторыСтрокРасписания.Количество() Тогда
			...
			Если МассивСтрокНедоступныхДляИзменения.Количество() = 1 Тогда
				ТекстВопроса = НСтр("ru='Начало выполнения маршрутного листа запланировано в предыдущем интервале.
                                            ...
                                            |Продолжить?'
                                            |;uk='Початок виконання маршрутного листа заплановано в попередньому інтервалі.
                                            |Розклад попереднього інтервалу може стати неактуальним.
                                            |Продовжити?'");
			Иначе	
				ТекстВопроса = НСтр("ru='Начало выполнения маршрутных листов запланировано в предыдущем интервале.
                                            |Расписание предыдущего интервала может стать неактуальным.
                                            |Продолжить?'
                                            |;uk='Початок виконання маршрутних листів заплановано в попередньому інтервалі.
                                            |Розклад попереднього інтервалу може стати неактуальним.
                                            |Продовжити?'");
			КонецЕсли; 
		Иначе
			...
			Если МассивСтрокНедоступныхДляИзменения.Количество() = 1 Тогда
				ТекстВопроса = НСтр("ru='Начало выполнения одного из выбранных маршрутных листов запланировано в предыдущем интервале.
                                            |Расписание предыдущего интервала может стать неактуальным.
                                            |Продолжить?'
                                            |;uk='Початок виконання одного з вибраних маршрутних листів заплановано в попередньому інтервалі.
                                            |Розклад попереднього інтервалу може стати неактуальним.
                                            |Продовжити?'");
			Иначе	
				ТекстВопроса = НСтр("ru='Начало выполнения некоторых выбранных маршрутных листов запланировано в предыдущем интервале.
                                            |Расписание предыдущего интервала может стать неактуальным.
                                            |Продолжить?'
                                            |;uk='Початок виконання деяких вибраних маршрутних листів заплановано в попередньому інтервалі.
                                            |Розклад попереднього інтервалу може стати неактуальним.
                                            |Продовжити?'");
			КонецЕсли; 
...
Процедура ЗаполнитьСтрокуМЛ(НоваяСтрока, ВыборкаДокументов)
	...
	НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаДокументов.Номер, Ложь, Истина);
	...
	НомерЗаказа = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаДокументов.НомерЗаказа);
...
Процедура ВывестиГрафикЗанятостиРабочегоЦентра(ДанныеТекущейОбластиДанных, ВыбранныйПериодРасписания)
				...
				Иначе
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru='Не определен способ нахождения максимальной загрузки.
                                                        ...
                                                        |;uk='Не визначено спосіб знаходження максимального завантаження.
                                                        |Дані завантаження: з %1 по %2
                                                        |Дані періоди: з %3 по %4'"),
	...
	Если ПараллельнаяЗагрузка Тогда
		...
		ОбластьДелениеШкалы.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка, %1';uk='Завантаження, %1'"), ЕдиницаИзмеренияЗагрузки);
...
Процедура РасшифровкаГрафикаЗанятости(Расшифровка, Элемент)
			...
			Если МожноПереместить Тогда
				Если Расшифровка.Количество() = 1 Тогда
					...
					ПредставлениеКодманды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru='Поместить №%1 перед №%2';uk='Помістити №%1 перед №%2'"),
					...
					// Поместить после
					ПредставлениеКодманды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru='Поместить №%1 после №%2';uk='Помістити №%1 після №%2'"),
												МаршрутныйЛистДляПеремещенияСтрокой,
												Расшифровка[0].НомерМаршрутногоЛиста);
				...
				Иначе
					ПредставлениеКодманды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru='Поместить №%1 перед...';uk='Помістити №%1 перед...'"),
												МаршрутныйЛистДляПеремещенияСтрокой);
					СписокМеню.Добавить("ПереместитьПеред", ПредставлениеКодманды);
					
					ПредставлениеКодманды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru='Поместить №%1 после...';uk='Помістити №%1 після...'"),
												МаршрутныйЛистДляПеремещенияСтрокой);
					СписокМеню.Добавить("ПереместитьПосле", ПредставлениеКодманды);
				КонецЕсли; 
			КонецЕсли; 
		...
		ИначеЕсли ДанныеТекущейОбластиДанных <> Неопределено Тогда
			...
			Если (Расшифровка = Неопределено ИЛИ Расшифровка.ВидОбласти <> "Переналадка")
				...
				ТекстЗаврешить = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Завершить %1';uk='Завершити %1'"),
									Формат(НовоеНачало, "ДФ='dd.MM.yyyy ЧЧ:мм'"));
	...
	Если ВыбранаОбластьМЛ Тогда
		СписокМеню.Добавить("Переместить", НСтр("ru='Выбрать для перемещения';uk='Вибрати для переміщення'"));
		СписокМеню.Добавить("Удалить", НСтр("ru='Удалить из расписания';uk='Вилучити з розкладу'"));
		СписокМеню.Добавить("ВвестиПериод", НСтр("ru='Ввести период выполнения';uk='Ввести період виконання'"));
		СписокМеню.Добавить("Открыть", НСтр("ru='Открыть';uk='Відкрити'"));
	КонецЕсли; 
...
Процедура РасшифровкаГрафикаЗанятостиЗавершение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйЭлемент.Значение = "Открыть" Тогда
		...
		ЗаголовокВыбора = НСтр("ru='Выберите маршрутный лист, чтобы открыть его';uk='Виберіть маршрутний лист, щоб відкрити його'");
	...
	ИначеЕсли ВыбранныйЭлемент.Значение = "Переместить" Тогда
		...
		ЗаголовокВыбора = НСтр("ru='Выберите маршрутный лист, который нужно переместить';uk='Виберіть маршрутний лист, який потрібно перемістити'");
		ВыбратьМЛИВыполнитьДействие(ОбработкаВыбора, ДополнительныеПараметры, ЗаголовокВыбора);
	...
	ИначеЕсли ВыбранныйЭлемент.Значение = "Удалить" Тогда
		...
		ЗаголовокВыбора = НСтр("ru='Выберите маршрутный лист, чтобы удалить его из расписания';uk='Виберіть маршрутний лист, щоб видалити його з розкладу'");
		ВыбратьМЛИВыполнитьДействие(ОбработкаВыбора, ДополнительныеПараметры, ЗаголовокВыбора);
	...
	ИначеЕсли ВыбранныйЭлемент.Значение = "ВвестиПериод" Тогда
		...
		ЗаголовокВыбора = НСтр("ru='Выберите маршрутный лист';uk='Виберіть маршрутний лист'");
		ВыбратьМЛИВыполнитьДействие(ОбработкаВыбора, ДополнительныеПараметры, ЗаголовокВыбора);
	...
	ИначеЕсли ВыбранныйЭлемент.Значение = "ПереместитьПеред" Тогда
		...
		ЗаголовокВыбора = НСтр("ru='Выберите перед каким маршрутным листом нужно поместить';uk='Виберіть перед яким маршрутним листом потрібно помістити'");
	...
	ИначеЕсли ВыбранныйЭлемент.Значение = "ПереместитьПосле" Тогда
		...
		ОбработкаВыбора = Новый ОписаниеОповещения("ВыбратьМЛПереместитьВыбранныйМЛЗавершение", ЭтотОбъект, 1);
		ЗаголовокВыбора = НСтр("ru='Выберите после какого маршрутного листа нужно поместить';uk='Виберіть після якого маршрутного листа потрібно помістити'");
		ВыбратьМЛИВыполнитьДействие(ОбработкаВыбора, ДополнительныеПараметры, ЗаголовокВыбора, МаршрутныйЛистДляПеремещения);
...
Процедура ПоказатьПрочиеСведенияВГрафикеЗанятости(Расшифровка)
	...
	Если Расшифровка.ВидОбласти = "НерабочееВремя" Тогда
		...
		// Нерабочий период
		МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Нерабочий период';uk='Неробочий період'"), Новый Шрифт(,, Истина)));
		
	ИначеЕсли Расшифровка.ВидОбласти = "СвободноеВремя" Тогда
		
		// Свободный период
		МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Свободный период';uk='Вільний період'"), Новый Шрифт(,, Истина)));
		Если ПараллельнаяЗагрузка Тогда
			МассивИнформации.Добавить("; ");
			МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Загрузка:';uk='Завантаження:'"), Новый Шрифт(,, Истина)));
			МассивИнформации.Добавить(" ");
			МассивИнформации.Добавить(Формат(Расшифровка.Загрузка, "ЧН=0; ЧГ=0"));
			МассивИнформации.Добавить(" " + ЕдиницаИзмеренияЗагрузки + "; ");
			
			МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Свободная загрузка:';uk='Вільне завантаження:'"), Новый Шрифт(,, Истина)));
			МассивИнформации.Добавить(" ");
	...
	ИначеЕсли Расшифровка.ВидОбласти = "ОтметкаШкалыВремени" Тогда
		...
		ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Период %1 с %2 до %3';uk='Період %1 з %2 до %3'"),
	...
	ИначеЕсли Расшифровка.ВидОбласти = "Переналадка" Тогда
		...
		// Занято с - по -
		МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Переналадка с';uk='Переналадка з'"), Новый Шрифт(,, Истина)));
		МассивИнформации.Добавить(" ");
		МассивИнформации.Добавить(Формат(Расшифровка.Начало, "ДФ=ЧЧ:мм"));
		МассивИнформации.Добавить(" ");
		МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='по';uk='по'"), Новый Шрифт(,, Истина)));
		МассивИнформации.Добавить(" ");
		МассивИнформации.Добавить(Формат(Расшифровка.Окончание, "ДФ=ЧЧ:мм"));
		МассивИнформации.Добавить("; ");
		
		// Вариант наладки
		МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Вариант наладки:';uk='Варіант наладки:'"), Новый Шрифт(,, Истина)));
		МассивИнформации.Добавить(" ");
...
Процедура ПоказатьСведенияОМЛВГрафикеЗанятости(ДанныеРасшифровки, ИндексМЛДляКоторогоНужноПоказатьИнформацию = 0, ЗаполнитьВыбор = Ложь)
	...
	// Этап
	МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Этап:';uk='Етап:'"), Новый Шрифт(,, Истина)));
	МассивИнформации.Добавить(" ");
	МассивИнформации.Добавить(Расшифровка.ЭтапСпецификация);
	МассивИнформации.Добавить("; ");
	
	// Запланировано
	МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Запланировано:';uk='Заплановано:'"), Новый Шрифт(,, Истина)));
	МассивИнформации.Добавить(" ");
	МассивИнформации.Добавить(Формат(Расшифровка.Количество, "ЧГ=0"));
	МассивИнформации.Добавить(" ед/партий; ");
	
	// Объем работ
	МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Объем работ:';uk='Обсяг робіт:'"), Новый Шрифт(,, Истина)));
	МассивИнформации.Добавить(" ");
	...
	КонецЕсли; 
	МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Занято с';uk='Зайнято з'"), Новый Шрифт(,, Истина)));
	МассивИнформации.Добавить(" ");
	...
	// Номенклатура
	МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Номенклатура:';uk='Номенклатура:'"), Новый Шрифт(,, Истина)));
	МассивИнформации.Добавить(" ");
	...
	Если ЗначениеЗаполнено(Расшифровка.НазначениеПредставление) Тогда
		МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Назначение:';uk='Призначення:'"), Новый Шрифт(,, Истина)));
		МассивИнформации.Добавить(" ");
	...
	// Маршрутный лист
	МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Маршрутный лист:';uk='Маршрутний лист:'"), Новый Шрифт(,, Истина)));
	МассивИнформации.Добавить(" ");
...
Процедура ПереместитьВОчередиВГрафике(Направление)
	...
	Если ТипЗнч(Расшифровка) = Тип("Массив") Тогда
		...
		ЗаголовокВыбора = НСтр("ru='Выберите маршрутный лист, который нужно переместить в очереди';uk='Виберіть маршрутний лист, який потрібно перемістити в черзі'");
		ВыбратьМЛИВыполнитьДействие(ОбработкаВыбора, ДополнительныеПараметры, ЗаголовокВыбора);
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не использовался>';uk='<не використовувався>'"));
	#КонецОбласти
	...
	Если НЕ ВыбранныйВидРабочегоЦентра.Пустая() Тогда
		...
		Если ПараллельнаяЗагрузка Тогда
			...
			ЗаголовокЗагрузка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																	НСтр("ru='Загрузка, %1';uk='Завантаження, %1'"),
																	ЕдиницаИзмеренияЗагрузки);
...
Процедура УместитьВГрафикРаботыЗавершение(РезультатВопроса, РабочийЦентр) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		Если УместитьВГрафикРаботы(РабочийЦентр) Тогда
			ПриИзмененииРасписания();
		Иначе
			ПоказатьПредупреждение(, НСтр("ru='Не удалось уместить маршрутные листы в график.';uk='Не вдалося вмістити маршрутні листи в графік.'"),, НСтр("ru='Ошибка';uk='Помилка'"));
		КонецЕсли; 
	КонецЕсли; 
