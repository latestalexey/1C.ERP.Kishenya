...
Процедура ЗаполнитьРейсамиНаДату(Команда)
	...
	ПараметрыФормы.Вставить("Заголовок",НСтр("ru='Выберите дату формирования эаданий на перевозку';uk='Виберіть дату формування эаданий на перевезення'"));
...
Процедура УстановитьОтборЗаданияНаПеревозкуВРаботе()
	...
	ИначеЕсли ЗначениеЗаполнено(Статус) Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ЗаданияНаПеревозкуВРаботе,
			"Статус", Перечисления.СтатусыЗаданийНаПеревозку[Статус], ВидСравненияКомпоновкиДанных.Равно,,Ложь);
	КонецЕсли;
