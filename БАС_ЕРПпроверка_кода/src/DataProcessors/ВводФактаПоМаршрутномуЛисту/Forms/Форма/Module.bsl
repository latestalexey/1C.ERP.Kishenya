...
Процедура ВыходныеИзделияПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ ОтменаРедактирования ИЛИ НЕ НоваяСтрока Тогда
		...
		Если ДанныеСтроки.Количество + ДанныеСтроки.КоличествоОтклонение < 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Отклонение от норматива не должно приводить к отрицательному выпуску.';uk='Відхилення від нормативу не повинно призводити до від''ємного випуску.'"));
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыходныеИзделияПередУдалением(Элемент, Отказ)
	...
	Если ТекущиеДанные.КоличествоУпаковок <> 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Удаление невозможно, т.к. для изделия указан норматив.
                                    ...
                                    |;uk='Вилучення неможливо, т. к. для виробу зазначений норматив.
                                    |Замість вилучення необхідно вказати нульовий фактичний випуск.'"));
		Отказ = Истина;
	КонецЕсли; 
...
Процедура МатериалыИУслугиПередУдалением(Элемент, Отказ)
	...
	Если ТекущиеДанные.КоличествоУпаковок <> 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Удаление невозможно, т.к. для материала указан норматив.
                                    ...
                                    |;uk='Вилучення неможливо, т. к. для матеріалу призначений норматив.
                                    |Замість вилучення необхідно вказати нульовою фактичний витрата.'"));
		Отказ = Истина;
	КонецЕсли; 
	...
	Если ТекущиеДанные.КодСтроки <> 0 Тогда
		...
		Если СписокСтрок.Количество() = 1 Тогда
			ПоказатьПредупреждение(,НСтр("ru='Удаление невозможно, т.к. для вида работ должен быть указан норматив.
                                        ...
                                        |;uk='Вилучення неможливо, т. к. для виду робіт повинен бути зазначений норматив.
                                        |Замість вилучення необхідно вказати нульову фактична кількість.'"));
			Отказ = Истина;
		КонецЕсли; 
	КонецЕсли; 
...
Процедура КомандаЗаполнитьПоМаршрутномуЛисту(Команда)
	...
	ТекстВопроса = НСтр("ru='Заполнить по маршрутному листу?';uk='Заповнити за маршрутним листом?'");
...
&НаСервере
Процедура ЗаполнитьПоМаршрутномуЛистуВызовСервера()
	...
	НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Выборка.Номер, Ложь, Истина);
