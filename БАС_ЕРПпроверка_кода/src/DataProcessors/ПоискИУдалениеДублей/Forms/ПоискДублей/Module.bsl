...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтображениеСостояния.Текст = НСтр("ru='Поиск дублей не выполнялся. 
                                            ...
                                            |;uk='Пошук дублів не виконувався. 
                                            |Введіть умови відбору та порівняння і натисніть ""Знайти дублі"".'");
	...
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ru='Поиск дублей...';uk='Пошук дублів...'");
	...
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ru='Удаление дублей...';uk='Вилучення дублів...'");
	ОтображениеСостояния.Картинка = Элементы.ДлительнаяОперация48.Картинка;
	...
	ОтображениеСостояния.Текст = НСтр("ru='Не обнаружено дублей по указанным параметрам.
                                            ...
                                            |;uk='Не виявлено дублів за вказаними параметрами.
                                            |Змініть умови відбору і порівняння, натисніть ""Знайти дублі""'");
	ОтображениеСостояния.Картинка = Элементы.Предупреждение32.Картинка;
...
Процедура ОбновитьМестаИспользованияКандидата(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.ПолучитьРодителя() = Неопределено Тогда
		...
		Элементы.ОписаниеТекущейГруппыДублей.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для элемента ""%1"" найдено дублей: %2';uk='Для елементу ""%1"" знайдено дублів: %2'"),
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		Элементы.ОписаниеТекущейГруппыДублей.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Элемент ""%1"" не используется';uk='Елемент ""%1"" не використовується'"), 
	...
	Иначе
		Элементы.МестаИспользованияКандидата.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Места использования ""%1"" (%2)';uk='Місця використання ""%1"" (%2)'"), 
			ДанныеСтроки.Наименование,
			ДанныеСтроки.Количество);
...
Процедура ОбновитьМестаИспользованияНеобработанныхДубли(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.ПолучитьРодителя() = Неопределено Тогда
		...
		Элементы.ОписаниеТекущейГруппыДублей1.Заголовок = НСтр("ru='Для просмотра причин выберите проблемный элемент-дубль.';uk='Для перегляду причин оберіть проблемний елемент-дубль.'");
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		Элементы.ОписаниеТекущейГруппыДублей1.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Замена дубля ""%1"" возможна, но была отменена из-за невозможности замены в других местах.';uk='Заміна дубля ""%1"" можлива, але була скасована через неможливість заміни в інших місцях.'"), 
			ДанныеСтроки.Наименование);
		
		Элементы.СтраницыМестаИспользованияНеобработанных.ТекущаяСтраница = Элементы.ОписаниеГруппыНеобработанных;
	Иначе
		Элементы.МестаИспользованияКандидата.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось заменить дубли в некоторых местах (%1)';uk='Не вдалося замінити дублі в деяких місцях (%1)'"), 
			ДанныеСтроки.Количество);
...
Процедура ПриСозданииНаСервереИнициализацияДанных(НастройкиФормы)
	...
	ВсеВариантыСравнения.Добавить("Равно",   НСтр("ru='Совпадает';uk='Збігається'"));
	ВсеВариантыСравнения.Добавить("Подобно", НСтр("ru='Совпадает по похожим словам';uk='Збігається за схожим словами'"));
КонецПроцедуры
...
Процедура УстановитьЦветаИУсловноеОформление()
	...
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='-';uk='-'"));
...
Процедура ОбновитьОписаниеСостоянияНайденныхДублей(Форма)
	...
	Форма.ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Найдено дублей: %2 (среди элементов: %1). Все отмеченные элементы будут помечены на удаление и заменены во всех местах использования на оригиналы (отмечены стрелкой).';uk='Знайдено дублів: %2 (серед елементів: %1). Всі зазначені елементи будуть позначені на вилучення та замінено у всіх місцях використання на оригінали (позначені стрілками).'"),
...
Функция ЗаполнитьРезультатыПоискаДублей(Знач Данные)
	...
	МестаИспользованияКандидата.Очистить();
	Элементы.ОписаниеТекущейГруппыДублей.Заголовок = НСтр("ru='Дублей не найдено';uk='Дублів не знайдено'");
	...
	Если ВсегоНайденоДублей = 0 Тогда
		ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(Элементы.Информация16.Картинка, " ",
			НСтр("ru='Не обнаружено дублей по указанным условиям.';uk='Не виявлено дублів по зазначеним умовам.'"));
	Иначе
		ОбновитьОписаниеСостоянияНайденныхДублей(ЭтотОбъект);
	КонецЕсли;
		...
		Если ВсегоОсновных = 1 Тогда
			...
			Если ПоследнийКандидат = Неопределено Тогда
				ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Все найденные дубли (%1) успешно объединены';uk='Всі знайдені дублі (%1) успішно об''єднані'"),
						ВсегоОбработано));
			Иначе
				...
				ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Все найденные дубли (%1) успешно объединены
                            |в ""%2""'
                            |;uk='Всі знайдені дублі (%1) успішно об''єднані
                            |у ""%2""'"),
...
Процедура ОбновитьОписаниеПравилПоиска()
	...
	Для Каждого Правило Из ПравилаПоиска Цикл
		Если Правило.Правило = "Равно" Тогда
			Сравнение = Правило.ПредставлениеРеквизита + " " + НСтр("ru='совпадает';uk='збігається'");
		ИначеЕсли Правило.Правило = "Подобно" Тогда
			Сравнение = Правило.ПредставлениеРеквизита + " " + НСтр("ru='совпадает по похожим словам';uk='збігається за схожим словами'");
		Иначе
			Продолжить;
		КонецЕсли;
	...
	Если ПустаяСтрока(ТекстПравил) Тогда
		ТекстПравил = НСтр("ru='Правила не заданы';uk='Правила не задані'");
	КонецЕсли;
...
Процедура ОбновитьОписаниеОтбора()
	...
	Если ПустаяСтрока(ПредставлениеПравилОтбора) Тогда
		ПредставлениеПравилОтбора = НСтр("ru='Все элементы';uk='Всі елементи'");
	КонецЕсли;
...
Процедура ИнициализироватьСценарийМастера()
	...
	КнопкиМастера.Далее.Заголовок = НСтр("ru='Найти дубли >';uk='Знайти дублі >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ru='Найти дубли по указанным критериям';uk='Знайти дублі за вказаними критеріями'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ru='Отказаться от поиска и замены дублей';uk='Відмовитися від пошуку і заміни дублів'");
	...
	КнопкиМастера.Отмена.Подсказка = НСтр("ru='Прервать поиск дублей';uk='Перервати пошук дублів'");
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Видимость = Ложь;
	КнопкиМастера.Далее.Заголовок = НСтр("ru='Удалить дубли >';uk='Вилучити дублі >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ru='Удалить дубли';uk='Вилучити дублі'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ru='Отказаться от поиска и замены дублей';uk='Відмовитися від пошуку і заміни дублів'");
	...
	КнопкиМастера.Далее.Видимость = Ложь;
	КнопкиМастера.Отмена.Заголовок = НСтр("ru='Прервать';uk='Перервати'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ru='Прервать удаление дублей';uk='Перервати вилучення дублів'");
	...
	// 4. Успешное удаление
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Заголовок = НСтр("ru='< Новый поиск';uk='< Новий пошук'");
	КнопкиМастера.Назад.Подсказка = НСтр("ru='Начать новый поиск с другими параметрами';uk='Почати новий пошук з іншими параметрами'");
	КнопкиМастера.Далее.Видимость = Ложь;
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Видимость = Ложь;
	КнопкиМастера.Далее.Заголовок = НСтр("ru='Повторить удаление >';uk='Повторити вилучення >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ru='Удалить дубли';uk='Вилучити дублі'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
...
Процедура ШагПоискНеВыполнялсяПередДействиемДалее(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ТекстПредупреждения = НСтр("ru='Необходимо выбрать область поиска дублей';uk='Необхідно вибрати область пошуку дублів'");
	...
	Если Не ИнформацияОЗадании.Завершено Тогда
		Если ПоказатьДиалогПередЗакрытием Тогда
			...
			Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
				ТекстВопроса = НСтр("ru='Прервать поиск дублей и закрыть форму?';uk='Перервати пошук дублів і закрити форму?'");
			ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
				ТекстВопроса = НСтр("ru='Прервать удаление дублей и закрыть форму?';uk='Перервати вилучення дублів і закрити форму?'");
			КонецЕсли;
...
Функция ФоновоеЗаданиеЗапустить()
	...
	Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
		...
		НаименованиеМетода = НСтр("ru='Поиск и удаление дублей: Поиск дублей';uk='Пошук і вилучення дублів: Пошук дублів'");
	...
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
		...
		НаименованиеМетода = НСтр("ru='Поиск и удаление дублей: Удаление дублей';uk='Пошук і вилучення дублів: Вилучення дублів'");
