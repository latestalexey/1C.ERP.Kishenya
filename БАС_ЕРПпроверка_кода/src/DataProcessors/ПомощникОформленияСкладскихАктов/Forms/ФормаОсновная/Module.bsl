...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Элементы.ДекорацияГруппаУправлениеСтраницаПересортицаПорча.Заголовок 
			= НСтр("ru='С целью оптимизации количества списаний и оприходований товаров рекомендуется произвести зачеты по пересортице.';uk='З метою оптимізації кількості списань і оприходований товарів рекомендується провести заліки за пересортицею.'");
		Элементы.ГруппаТоварыКПересортицеКПорче.Заголовок
...
Процедура ЗачестьПоПересортицеАвто(Команда)
	...
	Если Результат = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Среди списываемых и приходуемых товаров нет товаров, по которым возможно провести автоматический зачет.';uk='Серед товарів, що списуються і оприходуються немає тих, по яких можливо провести автоматичний залік.'"));
	КонецЕсли;
...
Процедура КнопкаНазадЗавершение(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ПерейтиНазад(Ложь);
		Если ДополнительныеПараметры.Свойство("УдаляютсяДокументы") Тогда 
...
Процедура УправлениеДекорациямиПоТоварам()
	...
	Иначе
		...
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='товары';uk='товари'"),,,,"отчет"));
		МассивСтрок.Добавить(НСтр("ru=', по которым необходимо оформить складские акты. Для ограничения списка обрабатываемой номенклатуры воспользуйтесь фильтрами:';uk=', за якими необхідно оформити складські акти. Для обмеження списку номенклатури для оброблення скористайтеся фільтрами:'"));		
		ЗаголовокДекорации = Новый ФорматированнаяСтрока(МассивСтрок);
	КонецЕсли;
