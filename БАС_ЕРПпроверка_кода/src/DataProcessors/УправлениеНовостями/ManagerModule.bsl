...
//
// Параметры:
//  ЛентыНовостей - Таблица значений - таблица лент новостей со следующими колонками:
...
Процедура ЗагрузитьФайлыНовостейССервера(ЛентыНовостей, ЛогШаговВыполнения = Неопределено) Экспорт
	...
	Если ТипЗнч(ЛогШаговВыполнения) <> тип("Массив") Тогда
	...
	ТекстСообщения = НСтр("ru='Начало загрузки файлов новостей
        ...
        |'
        |;uk='Початок завантаження файлів новин
	...
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ru='Новости. Загрузка новостей. Загрузка файлов новостей. Начало';uk='Новини. Завантаження новин. Завантаження файлів новин. Початок'"), // ИдентификаторШага
	...
	// Получить список обрабатываемых лент новостей.
	ОбрабатываемыеЛентыНовостей = ЛентыНовостей.СкопироватьКолонки();
	Для каждого ТекущаяСтрока Из ЛентыНовостей Цикл
		Если ((ТекущаяСтрока.Пометка) ИЛИ (ТекущаяСтрока.ОбязательныйКанал)) Тогда
			НоваяСтрока = ОбрабатываемыеЛентыНовостей.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтрока);
		КонецЕсли;
	КонецЦикла;
	ТаблицаХТТПСоединений = ОбрабатываемыеЛентыНовостей.Скопировать(, "Сайт, Протокол, ВариантЛогинаПароля, Логин, Пароль");
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ru='Загрузка: Получение настроек, формирование списка лент новостей';uk='Завантаження: Отримання настройок, формування списку стрічок новин'"),
			ТШаг,
	...
	Для каждого ТекущееХТТПСоединение Из ТаблицаХТТПСоединений Цикл
		...
		Если ВРег(ТекущееХТТПСоединение.Протокол) = ВРег("file") Тогда
			...
			МассивОбрабатываемыхЛентНовостей = ОбрабатываемыеЛентыНовостей.НайтиСтроки(
													Новый Структура("Сайт, Протокол, ВариантЛогинаПароля, Логин, Пароль",
														ТекущееХТТПСоединение.Сайт, // Должно быть пустое
														ТекущееХТТПСоединение.Протокол,
														ТекущееХТТПСоединение.ВариантЛогинаПароля, // Должно быть = БезЛогинаПароля
														ТекущееХТТПСоединение.Логин, // Должно быть пустое
														ТекущееХТТПСоединение.Пароль // Должно быть пустое
													)
												);
			...
			Для каждого ТекущаяЛентаНовостей Из МассивОбрабатываемыхЛентНовостей Цикл
				...
				Попытка
						...
						Если МассивСлов.Количество() = 5 Тогда
							...
							Исключение
								...
								ТекстСообщения = НСтр("ru='Ошибка получения новостей из метаданных %ИмяФайла% для ленты новостей %ЛентаНовостей% по причине
                                    |%ИнформацияОбОшибке%'
                                    |;uk='Помилка отримання новин з метаданих %ИмяФайла% для стрічки новин %ЛентаНовостей% через
                                    |%ИнформацияОбОшибке%'");
								...
								ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
									НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
									НСтр("ru='Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка получения файла из метаданных';uk='Новини. Завантаження новин. Завантаження файлів новин. Помилка отримання файлу метаданих'"), // ИдентификаторШага
							...
							Если ЕстьОшибки = Ложь Тогда
								Попытка
									Если ТипЗнч(ОбъектМетаданныхМакет) = Тип("ОбъектМетаданных") Тогда
										...
										Иначе
											Макет = Неопределено;
											ТекстСообщения = НСтр("ru='Ошибка получения новостей из макета %ИмяФайла% для ленты новостей %ЛентаНовостей% - эти метаданные не обрабатываются';uk='Помилка отримання новин з макета %ИмяФайла% для стрічки новин %ЛентаНовостей% - ці метадані не обробляються'");
											ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ТекущаяЛентаНовостей.ИмяФайла);
											ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяЛентаНовостей.ЛентаНовостей);
											// Запись в журнал регистрации
											ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
											...
											Если ОбъектМетаданныхМакет.ТипМакета = Метаданные.СвойстваОбъектов.ТипМакета.ТекстовыйДокумент Тогда
												...
												Исключение
													...
													ТекстСообщения = НСтр("ru='ЗагрузитьФайлыНовостейССервера (получено из макета): Не удалось удалить временный файл %ИмяВременногоФайла% для ленты новостей %ЛентаНовостей% по причине:
                                                        ...
                                                        |;uk='ЗагрузитьФайлыНовостейССервера (отримано з макета): Не вдалося видалити тимчасовий файл %ИмяВременногоФайла% для стрічки новин %ЛентаНовостей% з причини:
                                                        |%ИнформацияОбОшибке%
                                                        |
                                                        |Рекомендується вручну видалити цей тимчасовий файл.'");
													...
													ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
														НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
														НСтр("ru='Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка удаления временного файла';uk='Новини. Завантаження новин. Завантаження файлів новин. Помилка видалення тимчасового файлу'"), // ИдентификаторШага
												...
												КонецПопытки;
											Иначе
												ТекстСообщения = НСтр("ru='Ошибка получения новостей из макета %ИмяФайла% для ленты новостей %ЛентаНовостей%: возможна загрузка только из текстового макета';uk='Помилка отримання новин з макета %ИмяФайла% для стрічки новин %ЛентаНовостей%: можлива завантаження тільки з текстового макета'");
												ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ТекущаяЛентаНовостей.ИмяФайла);
												ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяЛентаНовостей.ЛентаНовостей);
												// Запись в журнал регистрации
												ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
									...
									КонецЕсли;
								Исключение
									ТекстСообщения = НСтр("ru='Ошибка получения новостей из макета %ИмяФайла% для ленты новостей %ЛентаНовостей% по причине %ИнформацияОбОшибке%';uk='Помилка отримання новин з макета %ИмяФайла% для стрічки новин %ЛентаНовостей% через %ИнформацияОбОшибке%'");
									ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ТекущаяЛентаНовостей.ИмяФайла);
									ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяЛентаНовостей.ЛентаНовостей);
									ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнформацияОбОшибке%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
									// Запись в журнал регистрации
					...
					КонецЕсли;
				Исключение
					ТекстСообщения = НСтр("ru='Ошибка получения файла %ИмяФайла% для ленты новостей %ЛентаНовостей% по причине %ИнформацияОбОшибке%';uk='Помилка отримання файла %ИмяФайла% для стрічки новин %ЛентаНовостей% через %ИнформацияОбОшибке%'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ТекущаяЛентаНовостей.ИмяФайла);
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяЛентаНовостей.ЛентаНовостей);
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнформацияОбОшибке%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					// Запись в журнал регистрации
					ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
						НСтр("ru='Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка получения файла';uk='Новини. Завантаження новин. Завантаження файлів новин. Помилка отримання файлу'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						, // Данные
						ТекстСообщения // Комментарий
				...
				Запись.ВидОбновления = "Получение новостей";
				...
				ТекстШага = НСтр("ru='Загрузка: загрузка файла новостей из ленты новостей %ЛентаНовостей% (протокол %Протокол%, сайт %Сайт%, имя файла %ИмяФайла%).
                    |Успешно? %Успешно%'
                    |;uk='Завантаження: завантаження файлу новин зі стрічки новин %ЛентаНовостей% (протокол %Протокол%, сайт %Сайт%, ім''я файлу %ИмяФайла%).
                    |Успішно? %Успешно%'");
		...
		ИначеЕсли (ВРег(ТекущееХТТПСоединение.Протокол) = ВРег("http"))
			...
			ТекстШага = НСтр("ru='Загрузка: установка http/https соединения (протокол %Протокол%).
                ...
                |;uk='Завантаження: встановлення http/https з''єднання (протокол %Протокол%).
                |Успішно? %СоединениеУстановлено%'");
			...
			Если ЕстьОшибки = Ложь Тогда
				МассивОбрабатываемыхЛентНовостей = ОбрабатываемыеЛентыНовостей.НайтиСтроки(
					Новый Структура("Сайт, Протокол, ВариантЛогинаПароля, Логин, Пароль",
						ТекущееХТТПСоединение.Сайт,
						ТекущееХТТПСоединение.Протокол,
						ТекущееХТТПСоединение.ВариантЛогинаПароля,
						ТекущееХТТПСоединение.Логин,
						ТекущееХТТПСоединение.Пароль
					)
				);
				...
				Для каждого ТекущаяЛентаНовостей Из МассивОбрабатываемыхЛентНовостей Цикл
					...
					Пока (ФайлПолучен <> Истина) И (ПопыткаПолученияФайла < НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокПолученияФайла) Цикл
						Попытка
							...
							Если Найти(лкИмяФайлаНаСервере, "[from]") > 0 Тогда
								Если (ДатаЗагрузкиПоследнихНовостейНаКлиенте > Дата(1,1,1,1,0,0)) Тогда // > 01.01.0001 01:00:00 (именно 1 час, чтобы минус 1 час не вызвал ошибки)
									...
									Если (лкДатаПоследнейПопыткиЗагрузкиНовостей > '00010101')
											И (лкДатаПоследнейПопыткиЗагрузкиНовостей > (ДатаЗагрузкиПоследнихНовостейНаКлиенте + 2*24*60*60)) Тогда
										...
										ДатаЗагрузкиПоследнихНовостейНаКлиенте = лкДатаПоследнейПопыткиЗагрузкиНовостей - 2*24*60*60;
									КонецЕсли;
									...
									лкИмяФайлаНаСервере = СтрЗаменить(лкИмяФайлаНаСервере, "[from]", "from=" + XMLСтрока(НачалоЧаса(ДатаЗагрузкиПоследнихНовостейНаКлиенте - 3600)) + "Z"); // Дата хранится в часовом поясе +0
								Иначе
							...
							// Непосредственно загрузка файла, начало.
								...
								ТекстСообщения = НСтр("ru='Начало попытки %ПопыткаПолученияФайла% / %HTTPСоединениеКоличествоПопытокПолученияФайла% получения файла %ИмяФайла% с сервера %Сайт% для ленты новостей %ЛентаНовостей% во временный файл %ИмяВременногоФайла%.
                                    |(URL %URL%).'
                                    |;uk='Початок спроби %ПопыткаПолученияФайла% / %HTTPСоединениеКоличествоПопытокПолученияФайла% отримання файла %ИмяФайла% з сервера %Сайт% для стрічки новин %ЛентаНовостей% у тимчасовий файл %ИмяВременногоФайла%.
                                    |(URL %URL%).'");
								...
								ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
									НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
									НСтр("ru='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Начало загрузки';uk='Новини. Завантаження новин. Завантаження файлів новин. Отримання файлів з сервера новин. Початок завантаження'"), // ИдентификаторШага
									УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
									, // ОбъектМетаданных
									, // Данные
									ТекстСообщения // Комментарий
								...
								ТекстСообщения = НСтр("ru='При попытке %ПопыткаПолученияФайла% / %HTTPСоединениеКоличествоПопытокПолученияФайла% получения файла %ИмяФайла% с сервера %Сайт% для ленты новостей %ЛентаНовостей% во временный файл получен ответ сервера с кодом %КодОтвета%, заголовки:
                                    ...
                                    |;uk='При спробі %ПопыткаПолученияФайла% / %HTTPСоединениеКоличествоПопытокПолученияФайла% отримання файла %ИмяФайла% з сервера %Сайт% для стрічки новин %ЛентаНовостей% у тимчасовий файл отримано відповідь сервера з кодом %КодОтвета%, заголовки:
                                    |%Заголовки%
                                    |
                                    |(URL %URL%)
                                    |Час завантаження (сек): %ВремяЗагрузки%'");
								ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПопыткаПолученияФайла%", ПопыткаПолученияФайла);
								ТекстСообщения = СтрЗаменить(ТекстСообщения, "%HTTPСоединениеКоличествоПопытокПолученияФайла%", НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокПолученияФайла);
								ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ТекущаяЛентаНовостей.ИмяФайла);
								ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Сайт%", ТекущаяЛентаНовостей.Сайт);
								...
								ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Заголовки%", ОбработкаНовостейКлиентСервер.ПолучитьСтроковоеПредставление(ХттпОтвет.Заголовки, "=", Символы.ПС));
								ТекстСообщения = СтрЗаменить(ТекстСообщения, "%URL%", ТекущееХТТПСоединение.Протокол + "://" + ТекущаяЛентаНовостей.Сайт + "/" + лкИмяФайлаНаСервере);
								...
								ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
									НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
									НСтр("ru='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Конец загрузки';uk='Новини. Завантаження новин. Завантаження файлів новин. Отримання файлів з сервера новин. Кінець завантаження'"), // ИдентификаторШага
									УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
									, // ОбъектМетаданных
									, // Данные
									ТекстСообщения // Комментарий
								...
								Если ВременныйФайл.Существует() Тогда
									ТекстСообщения = НСтр("ru='Успешное завершение попытки %ПопыткаПолученияФайла% / %HTTPСоединениеКоличествоПопытокПолученияФайла% получения файла %ИмяФайла% с сервера %Сайт% для ленты новостей %ЛентаНовостей% во временный файл %ИмяВременногоФайла%.
                                        ...
                                        |Первые строки файла:
                                        |%ПервыеСтрокиФайла%'
                                        |;uk='Успішне завершення спроби %ПопыткаПолученияФайла% / %HTTPСоединениеКоличествоПопытокПолученияФайла% отримання файла %ИмяФайла% з сервера %Сайт% для стрічки новин %ЛентаНовостей% у тимчасовий файл %ИмяВременногоФайла%.
                                        |Отриманий файл розміром %РазмерФайла% байт.
                                        |(URL %URL%)
                                        |
                                        |Перші рядки файла:
                                        |%ПервыеСтрокиФайла%'");
									...
									ТекстСообщения = СтрЗаменить(ТекстСообщения, "%URL%", ТекущееХТТПСоединение.Протокол + "://" + ТекущаяЛентаНовостей.Сайт + "/" + лкИмяФайлаНаСервере);
									ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПервыеСтрокиФайла%", ОбработкаНовостейВызовСервера.ПолучитьСодержимоеТекстовогоФайла(ВременныйФайл.ПолноеИмя, 10));
									// Запись в журнал регистрации
									ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
										НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
										НСтр("ru='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Успешная проверка чтения временного файла после загрузки';uk='Новини. Завантаження новин. Завантаження файлів новин. Отримання файлів з сервера новин. Успішна перевірка читання тимчасового файлу після завантаження'"), // ИдентификаторШага
										УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
										, // ОбъектМетаданных
										, // Данные
										ТекстСообщения // Комментарий
									...
									Исключение
										...
										ТекстСообщения = НСтр("ru='ЗагрузитьФайлыНовостейССервера (получено из интернет): Не удалось удалить временный файл %ИмяВременногоФайла% для ленты новостей %ЛентаНовостей% по причине:
                                            ...
                                            |
                                            |Рекомендуется вручную удалить этот временный файл.'
                                            |;uk='ЗагрузитьФайлыНовостейССервера (отримано з інтернет): Не вдалося видалити тимчасовий файл %ИмяВременногоФайла% для стрічки новин %ЛентаНовостей% з причини:
                                            |%ИнформацияОбОшибке%
                                            |
                                            |Рекомендується вручну видалити цей тимчасовий файл.'");
										ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяВременногоФайла%", СокрЛП(ИмяВременногоФайла));
								...
								Иначе
									ТекстСообщения = НСтр("ru='Неудачное завершение попытки %ПопыткаПолученияФайла% / %HTTPСоединениеКоличествоПопытокПолученияФайла% получения файла %ИмяФайла% с сервера %Сайт% для ленты новостей %ЛентаНовостей% во временный файл %ИмяВременногоФайла% - после попытки загрузки, файла все еще не существует.
                                        |(URL %URL%).'
                                        |;uk='Невдале завершення спроби %ПопыткаПолученияФайла% / %HTTPСоединениеКоличествоПопытокПолученияФайла% отримання файла %ИмяФайла% з сервера %Сайт% для стрічки новин %ЛентаНовостей% у тимчасовий файл %ИмяВременногоФайла% - після спроби завантаження файлу все ще не існує.
                                        |(URL %URL%).'");
									ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПопыткаПолученияФайла%", ПопыткаПолученияФайла);
									ТекстСообщения = СтрЗаменить(ТекстСообщения, "%HTTPСоединениеКоличествоПопытокПолученияФайла%", НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокПолученияФайла);
									ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ТекущаяЛентаНовостей.ИмяФайла);
									...
									ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
										НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
										НСтр("ru='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Ошибка чтения временного файла после загрузки';uk='Новини. Завантаження новин. Завантаження файлів новин. Отримання файлів з сервера новин. Помилка читання тимчасового файлу після завантаження'"), // ИдентификаторШага
										УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
										, // ОбъектМетаданных
										, // Данные
										ТекстСообщения // Комментарий
						...
						Исключение
							...
							ТекстСообщения = НСтр("ru='Попытка %ПопыткаПолученияФайла% / %HTTPСоединениеКоличествоПопытокПолученияФайла%. Ошибка получения файла %ИмяФайла% с сервера %Сайт% для ленты новостей %ЛентаНовостей% во временный файл по причине: %ИнформацияОбОшибке%';uk='Спроба %ПопыткаПолученияФайла% / %HTTPСоединениеКоличествоПопытокПолученияФайла%. Помилка отримання файла %ИмяФайла% з сервера %Сайт% для стрічки новин %ЛентаНовостей% у тимчасовий файл з причини: %ИнформацияОбОшибке%'");
							ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПопыткаПолученияФайла%", ПопыткаПолученияФайла);
							ТекстСообщения = СтрЗаменить(ТекстСообщения, "%HTTPСоединениеКоличествоПопытокПолученияФайла%", НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокПолученияФайла);
							ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ТекущаяЛентаНовостей.ИмяФайла);
							ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Сайт%", ТекущаяЛентаНовостей.Сайт);
							...
							ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
								НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
								НСтр("ru='Новости. Загрузка новостей. Загрузка файлов новостей. Получение файлов с сервера новостей. Ошибка загрузки';uk='Новини. Завантаження новин. Завантаження файлів новин. Отримання файлів з сервера новин. Помилка завантаження'"), // ИдентификаторШага
								УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
								, // ОбъектМетаданных
								, // Данные
								ТекстСообщения // Комментарий
					...
					ТекстШага = НСтр("ru='Загрузка: загрузка файла новостей из ленты новостей %ЛентаНовостей% (протокол %Протокол%, сайт %Сайт%, имя файла %ИмяФайла%, имя файла на сервере %лкИмяФайлаНаСервере%).
                        |Успешно? %Успешно%'
                        |;uk='Завантаження: завантаження файлу новин зі стрічки новин %ЛентаНовостей% (протокол %Протокол%, сайт %Сайт%, ім''я файлу %ИмяФайла%, ім''я файлу на сервері %лкИмяФайлаНаСервере%).
                        |Успішно? %Успешно%'");
					ТекстШага = СтрЗаменить(ТекстШага, "%ЛентаНовостей%", ТекущаяЛентаНовостей.ЛентаНовостей);
					ТекстШага = СтрЗаменить(ТекстШага, "%Протокол%", ТекущаяЛентаНовостей.Протокол);
					ТекстШага = СтрЗаменить(ТекстШага, "%Сайт%", ТекущаяЛентаНовостей.Сайт);
	...
	// Поместить адреса хранилища из ОбрабатываемыеЛентыНовостей в ЛентыНовостей.
	Для каждого ТекущаяЛентаНовостей Из ОбрабатываемыеЛентыНовостей Цикл
		Если НЕ ПустаяСтрока(ТекущаяЛентаНовостей.АдресВХранилище) Тогда
			НайденныеСтроки = ЛентыНовостей.НайтиСтроки(Новый Структура("ЛентаНовостей", ТекущаяЛентаНовостей.ЛентаНовостей));
			Если НайденныеСтроки.Количество() > 0 Тогда
				НайденныеСтроки[0].АдресВХранилище = ТекущаяЛентаНовостей.АдресВХранилище;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ru='Загрузка: помещение ссылок на хранилища в таблицу лент новостей';uk='Завантаження: розміщення посилань на сховища в таблицю стрічок новин'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ru='Загрузка: обработка неактивных лент новостей';uk='Завантаження: обробка неактивних стрічок новин'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
	...
	ТекстСообщения = НСтр("ru='Завершение загрузки файлов новостей
        ...
        |'
        |;uk='Завершення завантаження файлів новин
        |Час початку (мс): %ТНачало%
	...
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка новостей. Загрузка файлов новостей. Конец';uk='Новини. Завантаження новин. Завантаження файлів новин. Кінець'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
...
Процедура ПарсингХМЛФайловНовостей(ЛентыНовостей, ЛогШаговВыполнения = Неопределено) Экспорт
	...
	ТекстСообщения = НСтр("ru='Начало парсинга файлов новостей
        |Время начала (мс): %ТНачало%
        |'
        |;uk='Початок парсинга файлів новин
        |Час початку (мс): %ТНачало%
        |'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Начало';uk='Новини. Завантаження новин. Парсинг файлів новин. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
	...
	Для каждого ТекущаяСтрока Из ЛентыНовостей Цикл
		...
		ТШагЛентаНовостей = ТекущаяУниверсальнаяДатаВМиллисекундах();
		ЕстьОшибки = Ложь;
		СтатусОбработки = НСтр("ru='Начало обработки';uk='Початок обробки'");

		Если (ТекущаяСтрока.Пометка) ИЛИ (ТекущаяСтрока.ОбязательныйКанал) Тогда
			СтатусОбработки = СтатусОбработки + ", " + НСтр("ru='Помечена для обработки';uk='Позначена для обробки'");
			Если НЕ ПустаяСтрока(ТекущаяСтрока.АдресВХранилище) Тогда
				...
				Если Файл.Существует() Тогда
					...
					Исключение
						...
						ТекстСообщения = НСтр("ru='В загруженном файле для ленты новостей %ЛентаНовостей% находится текст не в формате xml. Лента новостей будет пропущена.
                            ...
                            |Первые строки файла:
                            |%ПервыеСтрокиФайла%'
                            |;uk='В завантаженому файлі для стрічки новин %ЛентаНовостей% знаходиться текст не у форматі xml. Стрічка новин буде пропущена.
                            |
                            |Докладно:
                            |%ИнформацияОбОшибке%
                            |
						...
						ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
							НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
							НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка формата файла';uk='Новини. Завантаження новин. Парсинг файлів новин. Помилка формату файлу'"), // ИдентификаторШага
							УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
							, // ОбъектМетаданных
							, // Данные
							ТекстСообщения // Комментарий
						);
						СтатусОбработки = СтатусОбработки + ", " + НСтр("ru='Формат файла не xml, или неправильно сформированный xml';uk='Формат файлу xml, або неправильно сформований xml'");
						// Пропустить ленту новостей
						ЕстьОшибки = Истина;
					КонецПопытки;
					...
					Если ЕстьОшибки = Ложь Тогда
						...
						ИначеЕсли (НРег(ХМЛТип.ИмяТипа) = "rss") Тогда // И (ВРег(ХМЛТип.URIПространстваИмен)=ВРег("http://www.w3.org/2005/RSS_2_0")).
							...
							Если (ОбъектХДТО.Свойства().Получить("version") = Неопределено) Тогда
								ТекстСообщения = НСтр("ru='Невозможно определить версию rss в ленте новостей %ЛентаНовостей% - отсутствует свойство version. Необходимы данные в формате версии 2.0.';uk='Неможливо визначити версію rss стрічці новин %ЛентаНовостей% - відсутня властивість version. Необхідні дані в форматі версії 2.0.'");
								ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяСтрока.ЛентаНовостей);
								// Запись в журнал регистрации
								ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
									НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
									НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка версии rss';uk='Новини. Завантаження новин. Парсинг файлів новин. Помилка версії rss'"), // ИдентификаторШага
									УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
									, // ОбъектМетаданных
									, // Данные
									ТекстСообщения // Комментарий
								);
								СтатусОбработки = СтатусОбработки + ", " + НСтр("ru='Неправильно сформированный xml: отсутствует свойство version для rss';uk='Неправильно сформований xml: відсутня властивість version для rss'");
								// Пропустить ленту новостей
								ЕстьОшибки = Истина;
							Иначе
								лкВерсия = ОбъектХДТО.Получить("version");
								Если (Число(лкВерсия) <> 2.0) Тогда
									ТекстСообщения = НСтр("ru='Неподдерживаемая версия новостей RSS %version% в ленте новостей %ЛентаНовостей%';uk='Непідтримувана версія новин RSS %version% у стрічці новин %ЛентаНовостей%'");
									...
									СтатусОбработки = СтатусОбработки + ", " + НСтр("ru='Неправильно сформированный xml: свойство version для rss не равно 2.0';uk='Неправильно сформований xml: властивість version для rss не одно 2.0'");
									// Пропустить ленту новостей
									ЕстьОшибки = Истина;
								КонецЕсли;
							КонецЕсли;
						Иначе
							...
							ТекстСообщения = НСтр("ru='Неподдерживаемый тип новостей %ТипНовостей% в файле %ИмяФайла% для ленты новостей %ЛентаНовостей%
                                ...
                                |Первые строки файла:
                                |%ПервыеСтрокиФайла%'
                                |;uk='Непідтримуваний тип новин %ТипНовостей% у файлі %ИмяФайла% для стрічки новин %ЛентаНовостей%
                                |
                                |Перші рядки файла:
                                |%ПервыеСтрокиФайла%'");
							...
							СтатусОбработки = СтатусОбработки + ", " + НСтр("ru='Неправильно сформированный xml: ожидаются данные в формате rss (rss.channel) или atom (feed)';uk='Неправильно сформований xml: очікуються дані у форматі rss (rss.channel) або atom (feed)'");
							// Пропустить ленту новостей.
							ЕстьОшибки = Истина;
						КонецЕсли;
					...
					Исключение
						...
						ТекстСообщения = НСтр("ru='ПарсингХМЛФайловНовостей: Не удалось удалить временный файл %ИмяВременногоФайла% для ленты новостей %ЛентаНовостей% по причине:
                            ...
                            |
                            |Рекомендуется вручную удалить этот временный файл.'
                            |;uk='ПарсингХМЛФайловНовостей: Не вдалося видалити тимчасовий файл %ИмяВременногоФайла% для стрічки новин %ЛентаНовостей% з причини:
                            |%ИнформацияОбОшибке%
                            |
                            |Рекомендується вручну видалити цей тимчасовий файл.'");
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяВременногоФайла%", СокрЛП(ИмяВременногоФайла));
						...
						ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
							НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
							НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка удаления временного файла';uk='Новини. Завантаження новин. Парсинг файлів новин. Помилка видалення тимчасового файлу'"), // ИдентификаторШага
							УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
							, // ОбъектМетаданных
							, // Данные
							ТекстСообщения // Комментарий
				...
				Иначе
					ТекстСообщения = НСтр("ru='Отсутствует загруженный файл %ИмяФайла% для ленты новостей %ЛентаНовостей%. Возможно, недостаточно места на диске, нет прав для записи в этот каталог или произошла какая-то другая ошибка.';uk='Відсутній завантажений файл %ИмяФайла% для стрічки новин %ЛентаНовостей%. Можливо, недостатньо місця на диску, немає прав для запису у цей каталог або сталася якась інша помилка.'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", СокрЛП(ИмяВременногоФайла));
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяСтрока.ЛентаНовостей);
					// Запись в журнал регистрации
					ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
						НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка чтения файла';uk='Новини. Завантаження новин. Парсинг файлів новин. Помилка читання файлу'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						, // Данные
						ТекстСообщения // Комментарий
					);
					СтатусОбработки = СтатусОбработки + ", " + НСтр("ru='Не удалось сохранить файл из временного хранилища';uk='Не вдалося зберегти файл з тимчасового сховища'");
					// Пропустить ленту новостей.
					ЕстьОшибки = Истина;
				КонецЕсли;

			Иначе
				ТекстСообщения = НСтр("ru='Не загружен файл новостей для ленты новостей %ЛентаНовостей% (пустой адрес в хранилище после загрузки файла [%ЛентаНовостей.ИмяФайла%] с сайта [%ЛентаНовостей.Сайт%] по протоколу [%ЛентаНовостей.Протокол%]). Лента новостей не обработана.';uk='Не завантажений файл новин для стрічки новин %ЛентаНовостей% (порожня адреса в сховищі після завантаження файлу [%ЛентаНовостей.ИмяФайла%] з сайту [%ЛентаНовостей.Сайт%] за протоколом [%ЛентаНовостей.Протокол%]). Стрічка новин не оброблена.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяСтрока.ЛентаНовостей);
				...
				ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
					НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Ошибка чтения файла из временного хранилища';uk='Новини. Завантаження новин. Парсинг файлів новин. Помилка читання файлу з тимчасового сховища'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.ЛентыНовостей, // ОбъектМетаданных
					ТекущаяСтрока.ЛентаНовостей, // Данные
					ТекстСообщения // Комментарий
				);
				СтатусОбработки = СтатусОбработки + ", " + НСтр("ru='Отсутствует файл для обработки (пустой адрес в хранилище)';uk='Відсутній файл для обробки (порожній адреса в сховищі)'");
			КонецЕсли;

		Иначе
			СтатусОбработки = СтатусОбработки + ", " + НСтр("ru='Не помечена для обработки';uk='Не позначена для обробки'");
		КонецЕсли;

		ТекстШага = НСтр("ru='Парсинг: лента новостей %ЛентаНовостей%.
            |Статус обработки: %СтатусОбработки%.
            |Успешно? %Успешно%'
            |;uk='Парсинг: стрічка новин %ЛентаНовостей%.
            |Статус обробки: %СтатусОбработки%.
            |Успішно? %Успешно%'");
	...
	ТекстСообщения = НСтр("ru='Завершение парсинга файлов новостей
        ...
        |Длительность (мс): %ТДлительность%
        |'
        |;uk='Завершення парсинга файлів новин
        |Час початку (мс): %ТНачало%
        |Час закінчення (мс): %ТКонец%
        |Тривалість (мс): %ТДлительность%
        |'");
	...
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Конец';uk='Новини. Завантаження новин. Парсинг файлів новин. Кінець'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения // Комментарий
	...
	ТекстСообщения = НСтр("ru='Начало вспомогательных мероприятий после парсинга файлов новостей
        |Время начала (мс): %ТНачало%
        |'
        |;uk='Початок допоміжних заходів після парсинга файлів новин
        |Час початку (мс): %ТНачало%
        |'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка новостей. Вспомогательные мероприятия после парсинга файлов новостей. Начало';uk='Новини. Завантаження новин. Допоміжні заходи після парсинга файлів новин. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ru='Парсинг: вспомогательные мероприятия, ПересчитатьНеобработанныеКатегорииНовостей';uk='Парсинг: допоміжні заходи, ПересчитатьНеобработанныеКатегорииновостей'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ru='Парсинг: вспомогательные мероприятия, ОптимизироватьОтборыПоНовостям';uk='Парсинг: допоміжні заходи, ОптимизироватьОтборыПоНовостям'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ru='Парсинг: вспомогательные мероприятия, ПересчитатьОтборыПоНовостям_РедкоМеняющиеся';uk='Парсинг: допоміжні заходи, ПересчитатьОтборыПоНовостям_Редкоменяющиеся'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ru='Парсинг: вспомогательные мероприятия, ПересчитатьОтборыПоНовостям_Общие';uk='Парсинг: допоміжні заходи, ПересчитатьОтборыПоНовостям_Общие'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ru='Парсинг: вспомогательные мероприятия, ОбновлениеПериодическихСвойствНовостей';uk='Парсинг: допоміжні заходи, ОбновлениеПериодическихСвойствНовостей'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ru='Парсинг: вспомогательные мероприятия, ОбновлениеПривязокКМетаданным';uk='Парсинг: допоміжні заходи, ОбновлениеПривязокКМетаданным'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
	...
	ТекстСообщения = НСтр("ru='Завершение вспомогательных мероприятий после парсинга файлов новостей
        ...
        |Длительность (мс): %ТДлительность%
        |'
        |;uk='Завершення допоміжних заходів після парсинга файлів новин
        |Час початку (мс): %ТНачало%
        |Час закінчення (мс): %ТКонец%
        |Тривалість (мс): %ТДлительность%
        |'");
	...
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка новостей. Вспомогательные мероприятия после парсинга файлов новостей. Конец';uk='Новини. Завантаження новин. Допоміжні заходи після парсинга файлів новин. Кінець'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения // Комментарий
	...
	ТекстСообщения = НСтр("ru='Начало настроенных разработчиками мероприятий после парсинга файлов новостей
        |Время начала (мс): %ТНачало%
        |'
        |;uk='Початок налаштованих розробниками заходів після парсинга файлів новин
        |Час початку (мс): %ТНачало%
        |'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка новостей. Настроенные разработчиками мероприятия после парсинга файлов новостей. Начало';uk='Новини. Завантаження новин. Налаштовані розробниками заходи після парсинга файлів новин. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
	...
	ЛогШаговВыполнения.Добавить(
		Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
			НСтр("ru='Парсинг: вспомогательные мероприятия (переопределяемые), ДополнительноОбработатьНовостиПослеПолучения';uk='Парсинг: допоміжні заходи (переопределяемые), ДополнительноОбработатьНовостиПослеполучения'"),
			ТШаг,
			ТекущаяУниверсальнаяДатаВМиллисекундах()));
	...
	ТекстСообщения = НСтр("ru='Завершение настроенных разработчиками мероприятий после парсинга файлов новостей
        ...
        |Длительность (мс): %ТДлительность%
        |'
        |;uk='Завершення налаштованих розробниками заходів після парсинга файлів новин
        |Час початку (мс): %ТНачало%
        |Час закінчення (мс): %ТКонец%
        |Тривалість (мс): %ТДлительность%
        |'");
	...
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка новостей. Настроенные разработчиками мероприятия после парсинга файлов новостей. Конец';uk='Новини. Завантаження новин. Налаштовані розробниками заходи після парсинга файлів новин. Кінець'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения // Комментарий
...
Функция ПолучитьСтандартныеСпискиТребующиеОбновления() Экспорт
	...
	ПроверяемыйСписок = "Список лент новостей";
	...
	Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
	ПроверяемыйСписок = "Список категорий новостей";
	Запись.Список = ПроверяемыйСписок;
	Запись.Прочитать();
	Если НЕ Запись.Выбран() Тогда
		Запись.Список = ПроверяемыйСписок;
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Для каждого ТекущаяСтрока Из ТаблицаОбновляемыхДанных Цикл
			Если ТипЗнч(ТекущаяСтрока.Список) = Тип("ПланВидовХарактеристикСсылка.КатегорииНовостей") Тогда
				ТекстСообщения = НСтр("ru='Загрузка значений категории %Выборка.Список% (%Выборка.Список.Код%)';uk='Завантаження значень категорії %Выборка.Список% (%Выборка.Список.Код%)'");
			...
			Иначе
				Если ТекущаяСтрока.Список = "Список лент новостей" Тогда
					ТекстСообщения = НСтр("ru='Загрузка списка лент новостей';uk='Завантаження списку стрічок новин'");
					ТекущаяСтрока.ПредставлениеДляЗагрузки = ТекстСообщения;
				ИначеЕсли ТекущаяСтрока.Список = "Список категорий новостей" Тогда
					ТекстСообщения = НСтр("ru='Загрузка списка категорий новостей';uk='Завантаження списку категорій новин'");
					ТекущаяСтрока.ПредставлениеДляЗагрузки = ТекстСообщения;
				ИначеЕсли Найти(ВРег(ТекущаяСтрока.Список), ВРег("Значения категории новостей:")) = 1 Тогда
					ТекстСообщения = НСтр("ru='Загрузка значений категории с кодом %Выборка.Список%';uk='Завантаження значень категорії з кодом %Выборка.Список%'");
	...
	ТекстСообщения = НСтр("ru='УправлениеНовостями.ПолучитьСтандартныеСпискиТребующиеОбновления, Таблица обновляемых данных:
        ...
        |;uk='УправлениеНовостями.ПолучитьСтандартныеСпискиТребующиеОбновления, Таблиця оновлюваних даних:
        |%ТаблицаОбновляемыхДанных%'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТаблицаОбновляемыхДанных%", ОбработкаНовостей.ПолучитьПредставлениеОбъектаХДТО(ТаблицаОбновляемыхДанныхХДТО));
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
		НСтр("ru='Новости. Сервис и регламент. ИнформацияОбОбновлении';uk='Новини. Сервіс і регламент. ИнформацияОбОбновлении'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
...
//
// Параметры:
//  Список                 - Строка или ПВХСсылка.КатегорииНовостей;
//  ТекущаяВерсияНаКлиенте - Дата, для определения, какие данные надо загружать;
//  Прокси                 - прокси веб-сервиса. Если не передан, то создается в функции.
//
// Возвращаемое значение:
...
Функция ОбновитьСтандартныйСписокССервера(Список, ТекущаяВерсияНаКлиенте, Прокси = Неопределено) Экспорт
	...
	// ////? Здесь реализовать многократное считывание данных в случае ошибки.
		...
		Если НЕ ПустаяСтрока(СокрЛП(Список.Код)) Тогда
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru='Ошибка обновления списка значений категории новостей (Прокси.getUpdatedCategoryValuesByCode):
                    ...
                    |;uk='Помилка оновлення списку значень категорії новин (Проксі.getUpdatedCategoryValuesByCode):
                    |sCategoryCode = %СокрЛП(Список.Код)%
                    |errCode = %errCode%
                    |errDescription = %errDescription%
                    |%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'");
				...
				ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
					НСтр("ru='Новости. Загрузка классификаторов. Список значений категории. Ошибка';uk='Новини. Завантаження класифікаторів. Список значень категорії. Помилка'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
			...
			Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
				...
				ТекстСообщения = НСтр("ru='Информация об обновлении значений категорий новостей (Прокси.getUpdatedCategoryValuesByCode):
                    ...
                    |Полученные данные:
                    |%Данные%'
                    |;uk='Інформація про оновлення значень категорій новин (Прокси.getUpdatedCategoryValuesByCode):
                    |configName = %configName%
                    |configVersion = %configVersion%
                    |Код категорії: %КодКатегории%
                    |lastSynchronizationDateTime = %lastSynchronizationDateTime%
                    |errCode = %errCode%
                    |errDescription = %errDescription%
                    |Отримані дані:
				...
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%errCode%", errCode);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%errDescription%", errDescription);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Данные%", ОбработкаНовостей.ПолучитьПредставлениеОбъектаХДТО(СписокОбновленныхДанных, Прокси.ФабрикаXDTO));
				ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
					НСтр("ru='Новости. Загрузка классификаторов. Список значений категории. Информация';uk='Новини. Завантаження класифікаторів. Список значень категорії. Інформація'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
							...
							Иначе
								...
								Исключение
									...
									ТекстСообщения = НСтр("ru='Ошибка записи элемента (%Режим%) с уникальным идентификатором %ТекущаяСтрокаХДТО.guid%:
                                        |%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'
                                        |;uk='Помилка запису елемента (%Режим%) з унікальним ідентифікатором %ТекущаяСтрокаХДТО.guid%:
                                        |%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'");
	...
	ИначеЕсли Список = "Список лент новостей" Тогда
		// Обновление списка лент новостей проходит в два этапа:
		// На первом этапе происходит загрузка измененных / добавленных данных;
		// На втором этапе сравнивается список всех лент новостей в ИБ и всех лент новостей на сервере,
		//  далее ленты новостей, отключенные на сервере, помечаются на удаление.
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Ошибка обновления списка лент новостей (Прокси.getUpdatedChannels):
                ...
                |errDescription = %errDescription%
                |%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'
                |;uk='Помилка оновлення списку стрічок новин (Прокси.getUpdatedChannels):
                |errCode = %errCode%
                |errDescription = %errDescription%
                |%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%errCode%", errCode);
			...
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
				НСтр("ru='Новости. Загрузка классификаторов. Список лент новостей. Ошибка';uk='Новини. Завантаження класифікаторів. Список стрічок новин. Помилка'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
		...
		Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
			...
			ТекстСообщения = НСтр("ru='Информация об обновлении списка лент новостей (Прокси.getUpdatedChannels):
                ...
                |Полученные данные:
                |%Данные%'
                |;uk='Інформація про оновлення списку стрічок новин (Прокси.getUpdatedChannels):
                |configName = %configName%
                |configVersion = %configVersion%
                |lastSynchronizationDateTime = %lastSynchronizationDateTime%
                |errCode = %errCode%
			...
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
				НСтр("ru='Новости. Загрузка классификаторов. Список лент новостей. Информация';uk='Новини. Завантаження класифікаторів. Список стрічок новин. Інформація'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
			...
			Если (СписокОбновленныхДанных.Тип().URIПространстваИмен = "http://v8.1c.ru/news1c/wsTypes")
				...
				Если ТипЗнч(СписокОбновленныхДанных.channel) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
					...
					// Обновить данные в регистре сведений "ДатыОбновленияСтандартныхСписковНовостей".
					Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
					Запись.Список = "Список лент новостей";
					Запись.Прочитать();
					Если Запись.Выбран() Тогда
						Если Запись.ТекущаяВерсияНаКлиенте < ДатаСамогоНовогоЭлемента Тогда
							Запись.Список = "Список лент новостей";
		...
		// Этап 2. Отключение лент новостей, отключенных от этой конфигурации (или помеченных на удаление) на сервере.
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Ошибка обновления списка лент новостей (Прокси.getAllChannels):
                ...
                |errDescription = %errDescription%
                |%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'
                |;uk='Помилка оновлення списку стрічок новин (Прокси.getAllChannels):
                |errCode = %errCode%
                |errDescription = %errDescription%
                |%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%errCode%", errCode);
		...
		Если ТипЗнч(СписокВсехДанных) = ТипОбъектXDTO Тогда
			...
			ТекстСообщения = НСтр("ru='Информация об обновлении списка лент новостей (Прокси.getAllChannels):
                ...
                |Полученные данные:
                |%Данные%'
                |;uk='Інформація про оновлення списку стрічок новин (Прокси.getAllChannels):
                |configName = %configName%
                |configVersion = %configVersion%
                |errCode = %errCode%
                |errDescription = %errDescription%
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%errCode%", errCode);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%errDescription%", errDescription);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Данные%", ОбработкаНовостей.ПолучитьПредставлениеОбъектаХДТО(СписокВсехДанных, Прокси.ФабрикаXDTO));
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия.
				НСтр("ru='Новости. Загрузка классификаторов. Список лент новостей. Информация';uk='Новини. Завантаження класифікаторів. Список стрічок новин. Інформація'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			...
			Если (СписокВсехДанных.Тип().URIПространстваИмен = "http://v8.1c.ru/news1c/wsTypes")
				...
				Если ТипЗнч(СписокВсехДанных.channel) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
					...
					Запрос.Текст = "
						...
						|ГДЕ
						|	Спр.ЗагруженоССервера = ИСТИНА // Загруженные с сервера
					...
					Для каждого ТекущаяСтрокаХДТО Из СписокВсехДанных.channel Цикл
						...
						Если НЕ ПустаяСтрока(ЛентаНовостейСсылка.ВерсияДанных) Тогда // Объект существует в ИБ.
							// Найти эту ленту новостей в списке и отметить, что она присутствует на сервере.
					...
					// Если на сервере отключили какую-то ленту новостей. то пометить ее на удаление.
					Для Каждого ТекущийЭлементСписка Из СписокВсехЗагруженныхСерверныхЛентНовостей Цикл
						Если ТекущийЭлементСписка.Пометка = Ложь Тогда
							...
							ТекстСообщения = НСтр("ru='Лента новостей %ЛентаНовостейНаименование% помечена на удаление, т.к. она была отключена на сервере';uk='Стрічка новин %ЛентаНовостейНаименование% помічена на видалення, т.я. вона була відключена на сервері'");
							...
							ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
								...
								НСтр("ru='Новости. Все обновления новостей. Обновление классификаторов. Отключение ленты новостей';uk='Новини. Всі оновлення новин. Оновлення класифікаторів. Відключення стрічки новин'"), // ИдентификаторШага
								УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
								, // ОбъектМетаданных
								, // Данные
								ТекстСообщения // Комментарий
	...
	ИначеЕсли Список = "Список категорий новостей" Тогда
		СписокОбновленныхДанных = Неопределено;
		Попытка
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Ошибка обновления списка категорий новостей (Прокси.getUpdatedCategories):
                ...
                |errDescription = %errDescription%
                |%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'
                |;uk='Помилка оновлення списку категорій новин (Прокси.getUpdatedCategories):
                |errCode = %errCode%
                |errDescription = %errDescription%
                |%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%errCode%", errCode);
			...
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
				НСтр("ru='Новости. Загрузка классификаторов. Список категорий. Ошибка';uk='Новини. Завантаження класифікаторів. Список категорій. Помилка'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
		...
		Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
			...
			ТекстСообщения = НСтр("ru='Информация об обновлении списка категорий новостей (Прокси.getUpdatedCategories):
                ...
                |Полученные данные:
                |%Данные%'
                |;uk='Інформація про оновлення списку категорій новин (Прокси.getUpdatedCategories):
                |configName = %configName%
                |configVersion = %configVersion%
                |lastSynchronizationDateTime = %lastSynchronizationDateTime%
                |errCode = %errCode%
	...
	ИначеЕсли Найти(ВРег(Список), ВРег("Значения категории новостей:")) = 1 Тогда
		...
		Если НЕ ПустаяСтрока(СписокУИН) Тогда
			...
			Если НЕ ПустаяСтрока(Владелец.ВерсияДанных) Тогда
				...
				Исключение
					...
					ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
						НСтр("ru='Новости. Загрузка классификаторов. Список значений категории (по УИН). Ошибка';uk='Новини. Завантаження класифікаторів. Список значень категорії (за УІН). Помилка'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						, // Данные
						ТекстСообщения // Комментарий
				...
				Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
					...
					ТекстСообщения = НСтр("ru='Информация об обновлении значений категорий новостей (Прокси.getUpdatedCategoryValuesByGuid):
                        ...
                        |Полученные данные:
                        |%Данные%'
                        |;uk='Інформація про оновлення значень категорій новин (Прокси.getUpdatedCategoryValuesByGuid):
                        |configName = %configName%
                        |configVersion = %configVersion%
                        |УІН категорії: %СписокУИН%
                        |lastSynchronizationDateTime = %lastSynchronizationDateTime%
                        |errCode = %errCode%
                        |errDescription = %errDescription%
                        |Отримані дані:
					...
					ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
						НСтр("ru='Новости. Загрузка классификаторов. Список значений категории (по УИН). Информация';uk='Новини. Завантаження класифікаторів. Список значень категорії (за УІН). Інформація'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						, // Данные
						ТекстСообщения // Комментарий
...
Функция ОбновитьВсеСтандартныеСпискиССервера() Экспорт
	...
	Для каждого ТекущийОбновляемыйСписок Из ТаблицаОбновляемыхДанных Цикл
		Если ТекущийОбновляемыйСписок = "Список лент новостей" Тогда
...
// Процедура обновляет список обновляемых справочников и их версии с сервера.
//
// Параметры:
//  Нет.
//
Процедура ОбновитьСписокСправочниковИВерсииССервера() Экспорт
	...
	configName    = СокрЛП(Метаданные.Имя);
	configVersion = ОбработкаНовостейКлиентСервер.ПривестиСтрокуКПравильномуФорматуВерсии(СокрЛП(Метаданные.Версия));
	// Возможно, имя конфигурации из метаданных было заменено на какой-то синоним - взять синоним.
	Продукты = ОбработкаНовостейВызовСервера.ПолучитьЗначениеПредопределеннойКатегории("Product");
	Если Продукты.Количество() > 0 Тогда
		Продукт = Продукты[Продукты.Количество()-1];
	...
	ТекстСообщения = НСтр("ru='Получение списка последних обновлений, начало
        ...
        |Время начала (мс): %ТНачало%
        |'
        |;uk='Отримання списку останніх оновлень, початок
        |configName = %configName%
        |configVersion = %configVersion%
        |Час початку (мс): %ТНачало%
        |'");
	...
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка классификаторов. Список обновлений классификаторов. Начало';uk='Новини. Завантаження класифікаторів. Список оновлень класифікаторів. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
	...
	Если ТипЗнч(ДатыОбновленияСписков) = ТипОбъектXDTO Тогда
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДатыОбновленияСписков%", ОбработкаНовостей.ПолучитьПредставлениеОбъектаХДТО(ДатыОбновленияСписков, Прокси.ФабрикаXDTO));
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДатыОбновленияСписков%", НСтр("ru='Получено значение с типом значения, отличным от ОбъектXDTO';uk='Отримано значення з типом значення, відмінним від ОбъектXDTO'"));
	КонецЕсли;
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка классификаторов. Список обновлений классификаторов. Выполнение';uk='Новини. Завантаження класифікаторів. Список оновлень класифікаторів. Виконання'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
		...
		Если ТипЗнч(ДатыОбновленияСписков) = ТипОбъектXDTO Тогда
			Если ТипЗнч(ДатыОбновленияСписков.updatesItem) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
				Для каждого ТекущаяЗапись Из ДатыОбновленияСписков.updatesItem Цикл
					Если (ТекущаяЗапись.listName = "Список категорий новостей")
							ИЛИ (ТекущаяЗапись.listName = "Список лент новостей") Тогда
						Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
						...
						КонецЕсли;
						Запись.Записать(Истина);
					ИначеЕсли ТекущаяЗапись.listName = "Значения категории новостей" Тогда
						...
						Если ТекущаяЗапись.Свойства().Получить("categoryGuid") <> Неопределено Тогда
							Если НЕ ПустаяСтрока(ТекущаяЗапись.categoryGuid) Тогда
								...
								Если НЕ ПустаяСтрока(КатегорияСсылка.ВерсияДанных) Тогда // Не новый объект
									...
									Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
									Запись.Список = "Значения категории новостей:" + ТекущаяЗапись.categoryGuid;
	...
	ТекстСообщения = НСтр("ru='Получение списка последних обновлений, завершение
        ...
        |Длительность (мс): %ТДлительность%
        |'
        |;uk='Отримання списку останніх оновлень, завершення
        |Час початку (мс): %ТНачало%
        |Час закінчення (мс): %ТКонец%
        |Тривалість (мс): %ТДлительность%
        |'");
	...
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка классификаторов. Список обновлений классификаторов. Конец';uk='Новини. Завантаження класифікаторів. Список оновлень класифікаторів. Кінець'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения // Комментарий
	...
	Если СтрДлина(СтрокаДата) >= 11 Тогда
		Если ВРег(Сред(СтрокаДата, 11, 1)) = "T" Тогда // Похоже на xs:dateTime
			Попытка
				...
				// ////? Платформа всегда преобразует дату в текущий часовой пояс, а нам надо сохранять в GMT
...
// заполнение дополнительных действий (обработчиков гиперссылок).
//
Процедура ПарсингХМЛФайлаНовостей(ОбъектХДТО, ФорматНовостей, ЛентаНовостей)
	...
	ТекстСообщения = НСтр("ru='Начало парсинга файла новостей для ленты новостей %ЛентаНовостей%
        |Время начала (мс): %ТНачало%
        |'
        |;uk='Початок парсинга файлу новин для стрічки новин %ЛентаНовостей%
        |Час початку (мс): %ТНачало%
        |'");
	...
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Начало';uk='Новини. Завантаження новин. Парсинг файлів новин. Парсинг файлу новин. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
	...
	Если НРег(ФорматНовостей) = "rss" Тогда
		...
		ТекстСообщения = НСтр("ru='Парсинг файла новостей для ленты новостей %ЛентаНовостей%
            ...
            |;uk='Парсинг файлу новин для стрічки новин %ЛентаНовостей%
            |Визначено формат стрічки новин: %ФорматНовостей%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ЛентаНовостей);
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ФорматНовостей%", НРег(ФорматНовостей));
		// Запись в журнал регистрации
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
			НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Выполнение';uk='Новини. Завантаження новин. Парсинг файлів новин. Парсинг файлу новин. Виконання'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
		...
		Если ТипЗнч(ОбъектХДТО.Свойства().Получить("channel")) = ТипСвойствоXDTO
			...
			// Если даты изменились - сразу запустить обработку по обновлению данных с серверов.
			...
			Если ОбъектХДТО.Свойства().Получить("channel") <> Неопределено Тогда
				...
				Иначе
					...
					// Запись в журнал регистрации
					ТекстСообщения = НСтр("ru='В файле полученном для ленты новостей %ЛентаНовостей% отсутствует поле channel.lastBuildDate. Все новости будут прочитаны и загружены, но это займет больше времени.';uk='В отриманому файлі для стрічки новин %ЛентаНовостей% відсутнє поле channel.lastBuildDate. Усі новини будуть прочитані і завантажені, але це займе більше часу.'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ЛентаНовостей);
					ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
						НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей';uk='Новини. Завантаження новин. Парсинг файлів новин. Парсинг файлу новин'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			...
			Иначе
				// Запись в журнал регистрации
				ТекстСообщения = НСтр("ru='В файле полученном для ленты новостей %ЛентаНовостей% отсутствует обязательное поле channel. Файл не был обработан.';uk='В отриманому файлі для стрічки новин %ЛентаНовостей% відсутнє обов''язкове поле channel. Файл не був оброблений.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ЛентаНовостей);
				ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
					НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Ошибка';uk='Новини. Завантаження новин. Парсинг файлів новин. Парсинг файлу новин. Помилка'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
	...
	ИначеЕсли НРег(ФорматНовостей) = "feed" Тогда
		...
		Иначе // updated - обязательное поле, наверное формат некорректный
			ДатаЗагрузкиПоследнихНовостейВФайле = '00010101';
			// Запись в журнал регистрации
			ТекстСообщения = НСтр("ru='В файле полученном для ленты новостей %ЛентаНовостей% отсутствует поле feed.updated. Все новости будут прочитаны и загружены, но это займет больше времени.';uk='В отриманому файлі для стрічки новин %ЛентаНовостей% відсутнє поле feed.updated. Усі новини будуть прочитані і завантажені, але це займе більше часу.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ЛентаНовостей);
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
				НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Предупреждение';uk='Новини. Завантаження новин. Парсинг файлів новин. Парсинг файлу новин. Попередження'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
				Метаданные.Справочники.Новости, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
	...
	Иначе
		...
		РезультатЗагрузкиНовостей = НСтр("ru='Новости не загружены, т.к. передан необрабатываемый формат ленты новостей.';uk='Новини не завантажені, т. к. переданий необроблювана формат стрічки новин.'");

		ТекстСообщения = НСтр("ru='Парсинг файла новостей для ленты новостей %ЛентаНовостей%
            |Не обрабатываемый формат ленты новостей: %ФорматНовостей%'
            |;uk='Парсинг файлу новин для стрічки новин %ЛентаНовостей%
            |Не опрацьований формат стрічки новин: %ФорматНовостей%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ЛентаНовостей);
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ФорматНовостей%", НРег(ФорматНовостей));
		// Запись в журнал регистрации
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
	...
	ТекстСообщения = НСтр("ru='Завершение парсинга файла новостей для ленты новостей %ЛентаНовостей%.
        ...
        |Длительность (мс): %ТДлительность%
        |'
        |;uk='Завершення парсинга файлу новин для стрічки новин %ЛентаНовостей%.
        |Результат завантаження новин:
        |%РезультатЗагрузкиНовостей%
        |Час початку (мс): %ТНачало%
        |Час закінчення (мс): %ТКонец%
        |Тривалість (мс): %ТДлительность%
	...
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Конец';uk='Новини. Завантаження новин. Парсинг файлів новин. Парсинг файлу новин. Кінець'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения // Комментарий
...
Процедура ЗагрузитьНовость(
	...
	Если НРег(ФорматНовостей) = "rss" Тогда
		...
		Если ЗаписыватьНовость = Истина Тогда
			...
			// Категории могут передаваться как через стандартный "category", так и через специфичный для "category1C" или "categoryVersions1C".
			...
			Попытка
				...
				ТекстСообщения = НСтр("ru='Начало попытки записи новости с данными:
                    ...
                    |Время начала (мс): %ТНачало%
                    |'
                    |;uk='Початок спроби запису новини з даними:
                    |заголовок [%ЗаголовокНовости%], УІН [%УИННовости%], стрічка новин [%ЛентаНовостей%], формат [%ФорматНовостей%], дата публікації [%ДатаПубликации%].
                    |Час початку (мс): %ТНачало%
                    |'");
				...
				ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
					НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Запись новости. Начало';uk='Новини. Завантаження новин. Парсинг файлів новин. Парсинг файлу новин. Парсинг новини. Запис новини. Початок'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
				...
				ТекстСообщения = НСтр("ru='Успешная попытка записи новости с данными:
                    ...
                    |Длительность (мс): %ТДлительность%
                    |'
                    |;uk='Успішна спроба запису новини з даними:
                    |заголовок [%ЗаголовокНовости%], УІН [%УИННовости%], стрічка новин [%ЛентаНовостей%], формат [%ФорматНовостей%], дата публікації [%ДатаПубликации%], УІН в ІБ [%УИН_ИБ%].
                    |Час початку (мс): %ТНачало%
                    |Час закінчення (мс): %ТКонец%
                    |Тривалість (мс): %ТДлительность%
                    |'");
				...
				ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
					НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Запись новости. Конец';uk='Новини. Завантаження новин. Парсинг файлів новин. Парсинг файлу новин. Парсинг новини. Запис новини. Кінець'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru='Неудачная попытка записи новости с данными:
                    ...
                    |;uk='Невдала спроба запису новини з даними:
                    |заголовок [%ЗаголовокНовости%], УІН [%УИННовости%], стрічка новин [%ЛентаНовостей%], формат [%ФорматНовостей%], дата публікації [%ДатаПубликации%],
                    |через: %ИнформацияОбОшибке%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЗаголовокНовости%", ЗаголовокНовости);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%УИННовости%", УИННовости);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДатаПубликации%", ОбъектНовость.ДатаПубликации);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ОбъектНовость.ЛентаНовостей);
				...
				ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
					НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Запись новости. Ошибка';uk='Новини. Завантаження новин. Парсинг файлів новин. Парсинг файлу новин. Парсинг новини. Запис новини. Помилка'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru='Новость пропущена, ее данные:
                ...
                |;uk='Новина пропущена, її дані:
                |заголовок [%ЗаголовокНовости%], УІН [%УИННовости%], стрічка новин [%ЛентаНовостей%], формат [%ФорматНовостей%], дата публікації [%ДатаПубликации%], %ЗначениеПометкаУдаления%, %СуществуетВБазе%.
                |Можливо, новина вже існує в базі з такою ж датою публікації або (при скасуванні публікації) цієї новини немає в базі.'");
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ЛентаНовостей);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ФорматНовостей%", ФорматНовостей);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЗначениеПометкаУдаления%", ?(ЗначениеПометкаУдаления = Истина, НСтр("ru='на сервере публикация отменена';uk='на сервері публікацію скасовано'"), НСтр("ru='опубликована на сервере';uk='опубліковано на сервері'")));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СуществуетВБазе%", ?(СтруктураНовости.Ссылка.Пустая() = Истина, НСтр("ru='в базе еще не создана';uk='в базі ще не створена'"), НСтр("ru='существует в базе';uk='існує в базі'")));
			// Запись в журнал регистрации
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
				НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Запись новости. Новость пропущена';uk='Новини. Завантаження новин. Парсинг файлів новин. Парсинг файлу новин. Парсинг новини. Запис новини. Новина пропущена'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
		...
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ДатаПубликации%", Формат(ДатаПубликации, "ДФ='dd.MM.yyyy HH:mm:ss'"));
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ЗначениеПометкаУдаления%", ЗначениеПометкаУдаления);
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ЗаписыватьНовость%", ?(ЗаписыватьНовость = Истина, НСтр("ru='Будет записана';uk='Буде записана'"), НСтр("ru='Будет пропущена';uk='Буде пропущена'")));
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%СуществуетВБазе%", ?(СтруктураНовости.Ссылка.Пустая() = Истина, НСтр("ru='В базе еще не создана';uk='В базі ще не створена'"), НСтр("ru='Уже существует в базе';uk='Вже існує в базі'")));
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ДатаПубликацииИБ%", ?(СтруктураНовости.Ссылка.Пустая() = Истина, " " + НСтр("ru='(в базе не было)';uk='(в базі не було)'"), " (" + Формат(ДатаПубликацииИБ, "ДФ='dd.MM.yyyy HH:mm:ss'") + ")"));
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ЗначениеПометкаУдаленияИБ%", ?(СтруктураНовости.Ссылка.Пустая() = Истина, " " + НСтр("ru='(в базе не было)';uk='(в базі не було)'"), " (" + ЗначениеПометкаУдаленияИБ + ")"));
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%УИН_ИБ%", ?(СтруктураНовости.Ссылка.Пустая() = Истина, НСтр("ru='(еще не создана)';uk='(ще не створена)'"), СтруктураНовости.УИН_ИБ));
		РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%НавигационнаяСсылка%", ?(СтруктураНовости.Ссылка.Пустая() = Истина, НСтр("ru='(еще не создана)';uk='(ще не створена)'"), ПолучитьНавигационнуюСсылку(СтруктураНовости.Ссылка)));
...
Процедура ЗагрузитьБлокДанныхКатегорий1С_Категория(ДанныеХДТО, ОбъектНовость)
	...
	Если ТипЗнч(ДанныеХДТО) = ТипОбъектXDTO Тогда
		...
		Если ВРег(КатегорияНовостейСтрока) = ВРег("Важность")
				И ВРег(ЗначениеКатегорииНовостейСтрока) = ВРег("Очень важная")
				И ВРег(УсловиеОтбора) = ВРег("eq") Тогда
			ОбъектНовость.Важность = 1; // Перезапишет ОбъектНовость.Важность, если было установлено ранее из тега priority.
		ИначеЕсли ВРег(КатегорияНовостейСтрока) = ВРег("Важность")
				И ВРег(ЗначениеКатегорииНовостейСтрока) = ВРег("Важная")
				И ВРег(УсловиеОтбора) = ВРег("eq") Тогда
			ОбъектНовость.Важность = 2; // Перезапишет ОбъектНовость.Важность, если было установлено ранее из тега priority.
		Иначе
			// Добавить данные
	...
	Если ТипЗнч(ДанныеХДТО) = ТипОбъектXDTO Тогда
		...
		Если ТипЗнч(СвойствоДвоичныеДанные) = ТипСвойствоXDTO Тогда
			...
			Исключение
				Данные = Неопределено;
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ТекстСообщения = НСтр("ru='Не удалось загрузить бинарные данные для новости с УИН %ОбъектНовость.УИННовости% по причине: %ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%';uk='Не вдалося завантажити двійкові дані для новини з УІН %ОбъектНовость.УИННовости% по причині: %ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "ОбъектНовость.УИННовости", ОбъектНовость.УИННовости);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
					НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Чтение новости. Блок бинарных данных';uk='Новини. Завантаження новин. Парсинг файлів новин. Парсинг файлу новин. Парсинг новини. Читання новини. Блок бінарних даних'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
...
Процедура ЗагрузитьДатуПоследнегоОбновленияСтандартногоСписка(Обновление)
	...
	Если ТипЗнч(Обновление) = ТипОбъектXDTO Тогда
		Попытка
			...
			КонецЕсли;
		Исключение
				ТекстСообщения = НСтр("ru='Ошибка загрузки строки обновления списков: %ИнформацияОбОшибке%';uk='Помилка завантаження рядка оновлення списків: %ИнформацияОбОшибке%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнформацияОбОшибке%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				// Запись в журнал регистрации
				ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
					НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Блок дат обновления списков';uk='Новини. Завантаження новин. Парсинг файлів новин. Парсинг файлу новин. Блок дат оновлення списків'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
				...
				Возврат;
		КонецПопытки;
		Если listName = "Список лент новостей" Тогда
			Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
			Запись.Список = listName;
			Запись.Прочитать();
			Запись.ТекущаяВерсияНаСервере = updated;
			Запись.Записать(Истина);
		ИначеЕсли listName = "Список категорий новостей" Тогда
			Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
			Запись.Список = listName;
			Запись.Прочитать();
			Запись.ТекущаяВерсияНаСервере = updated;
			Запись.Записать(Истина);
		ИначеЕсли listName = "Значения категории новостей" Тогда
			...
			Если Значение.Пустая() Тогда
				Запись = РегистрыСведений.ДатыОбновленияСтандартныхСписковНовостей.СоздатьМенеджерЗаписи();
				Запись.Список = "Значения категории новостей:" + categoryGuid;
				Запись.Прочитать();
				Запись.ТекущаяВерсияНаСервере = updated;
				Запись.Записать(Истина);
			Иначе
			...
			Если ТипЗнч(ОбъектХДТО.categoriesRefList.categoryRef) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
				...
				Пока БылиУдаления = Истина Цикл
					...
					Для каждого ТекущаяКатегория Из лкОбъект.ДоступныеКатегорииНовостей Цикл
						...
						Если КатегорияЕстьНаСервере = Ложь Тогда
							ТекстКомментария = НСтр("ru='Из ленты новостей %Наименование% удалена категория %Категория%.';uk='Із стрічки новин %Наименование% вилучено категорію %Категория%.'");
							ТекстКомментария = СтрЗаменить(ТекстКомментария, "%Наименование%", СокрЛП(лкОбъект.Наименование));
							ТекстКомментария = СтрЗаменить(ТекстКомментария, "%Категория%", СокрЛП(ТекущаяКатегория.КатегорияНовостей.Наименование));
							ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
								НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
								НСтр("ru='Новости. Загрузка классификаторов. Список лент новостей. Лента новостей';uk='Новини. Завантаження класифікаторів. Список стрічок новин. Стрічка новин'"), // ИдентификаторШага
								УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
								, // ОбъектМетаданных
								, // Данные
				...
				Для каждого ТекущаяКатегория Из МассивДоступныхКатегорий Цикл
					...
					Если КатегорияЕстьНаКлиенте = Ложь Тогда
						ТекстКомментария = НСтр("ru='В ленте новостей %Наименование% добавлена категория %Категория%.';uk='У стрічці новин %Наименование% додано категорія %Категория%.'");
						ТекстКомментария = СтрЗаменить(ТекстКомментария, "%Наименование%", СокрЛП(лкОбъект.Наименование));
						ТекстКомментария = СтрЗаменить(ТекстКомментария, "%Категория%", СокрЛП(ТекущаяКатегория.Наименование));
						ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
							НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
							НСтр("ru='Новости. Загрузка классификаторов. Список лент новостей. Лента новостей';uk='Новини. Завантаження класифікаторів. Список стрічок новин. Стрічка новин'"), // ИдентификаторШага
							УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
...
Функция ПодключитьсяКВебСервисуОбновленияСтандартныхСписков()
	...
	ТекстСообщения = НСтр("ru='Начало загрузки стандартных списков
        ...
        |;uk='Початок завантаження стандартних списків
        |МестоположениеWSDL = %МестоположениеWSDL%'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%МестоположениеWSDL%", МестоположениеWSDL);
	// Запись в журнал регистрации
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков. Начало';uk='Новини. Завантаження класифікаторів. З''єднання з веб-сервісом оновлення стандартних списків. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
	...
	Попытка
		...
		// ////? При переходе на 2.1.8 заменить ОбщегоНазначения.WSПрокси на ОбщегоНазначения.СоздатьWSПрокси и проверить список параметров.
		// Код 2.1.7.
		Прокси = ОбщегоНазначения.WSПрокси(
			МестоположениеWSDL, // АдресWSDL
			"http://v8.1c.ru/news1c/ws/Information", // URIПространстваИмен,
			"Information", // ИмяСервиса,
			"InformationSoap", // ИмяТочкиПодключения = "",
			"", // ИмяПользователя,
			"", // Пароль,
			Таймаут, // Таймаут = 0,
			Ложь// ДелатьКонтрольныйВызов
		);
		// Код 2.1.8.
		...
		Если ТипЗнч(Прокси) <> Тип("WSПрокси") Тогда
			ТекстСообщения = НСтр("ru='Неудачное подключение к веб-сервису обновления стандартных списков';uk='Невдале підключення до веб-сервісу оновлення стандартних списків'");
			// Запись в журнал регистрации
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
				НСтр("ru='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков. Ошибка';uk='Новини. Завантаження класифікаторів. З''єднання з веб-сервісом оновлення стандартних списків. Помилка'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
		...
		Иначе
			ТекстСообщения = НСтр("ru='Успешное подключение к веб-сервису обновления стандартных списков';uk='Успішне підключення до веб-сервісу оновлення стандартних списків'");
			// Запись в журнал регистрации
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
				НСтр("ru='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков. Конец';uk='Новини. Завантаження класифікаторів. З''єднання з веб-сервісом оновлення стандартних списків. Кінець'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Неудачное подключение к веб-сервису обновления стандартных списков по причине:
            |%ИнформацияОбОшибке%'
            |;uk='Невдале підключення до веб-сервісу оновлення стандартних списків з причини:
            |%ИнформацияОбОшибке%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнформацияОбОшибке%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		// Запись в журнал регистрации
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
