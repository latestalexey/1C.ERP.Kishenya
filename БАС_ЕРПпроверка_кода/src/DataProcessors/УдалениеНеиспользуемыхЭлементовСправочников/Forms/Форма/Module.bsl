...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("АвтоТест") Тогда // Возврат при получении формы для анализа.
		Возврат;
	КонецЕсли;
	МассивИсключаемыхСправочников = ПолучитьМассивИсключаемыхСправочников();
	// заполнение списка справочников
	Для каждого Справочник Из Метаданные.Справочники Цикл
		Если МассивИсключаемыхСправочников.Найти(Справочник) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ДекорацияЖурналРегистрацииНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("СобытиеЖурналаРегистрации", НСтр("ru='ПоискИУдалениеНеиспользуемыхЭлементовСправочников';uk='ПоискИУдалениеНеиспользуемыхЭлементовСправочников'"));
...
Процедура ВыполнитьПоиск(Команда)
	Если НЕ ПроверитьПометкиВСписке() Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Не выбран ни один справочник';uk='Не обраний жоден довідник'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбработчикОжиданияУдаление()
	...
	Элементы.КоманднаяПанель_Поиск.ПодчиненныеЭлементы.ФормаЗакрыть.Заголовок = "Закрыть";
КонецПроцедуры
...
Процедура НайтиИУдалитьНеиспользуемыеЭлементыСправочников(МассивПараметров, Отказ)
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Недостаточно прав для выполнения операции';uk='Недостатньо прав для виконання операції'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	Попытка
	...
	Исключение
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось установить монопольный режим';uk='Не вдалося встановити монопольний режим'"));
		Возврат;
	КонецПопытки;
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось снять монопольный режим';uk='Не вдалося зняти монопольний режим'"));
	КонецПопытки;
...
Процедура УдалитьЭлементСправочника(Ссылка, СчетчикУдаленныхОбъектов)
	...
	ПараметрыЖурнала.Вставить("ГруппаСобытий", НСтр("ru='ПоискИУдалениеНеиспользуемыхЭлементовСправочников';uk='ПоискИУдалениеНеиспользуемыхЭлементовСправочников'"));
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru='Не удалось удалить элемент справочника';uk='Не вдалося вилучити елемент довідника'")+ПредставлениеОбъекта;
	...
	ИмяСобытия = НСтр("ru='Удален:';uk='Вилучений:'") + " " + ПредставлениеОбъекта;
...
Функция ПолучитьМассивИсключаемыхСправочников()
	...
	//++ НЕ УТ
	МассивИсключений.Добавить(МетаСправочники.ВидыДоходовНДФЛ);
	МассивИсключений.Добавить(МетаСправочники.ВидыОбразованияФизическихЛиц);
	МассивИсключений.Добавить(МетаСправочники.ОснованияУвольнения);
	МассивИсключений.Добавить(МетаСправочники.СтепениЗнанияЯзыка);
	МассивИсключений.Добавить(МетаСправочники.СтепениРодстваФизическихЛиц);
	МассивИсключений.Добавить(МетаСправочники.УченыеЗвания);
	МассивИсключений.Добавить(МетаСправочники.УченыеСтепени);
	МассивИсключений.Добавить(МетаСправочники.ЯзыкиНародовМира);
	//-- НЕ УТ
