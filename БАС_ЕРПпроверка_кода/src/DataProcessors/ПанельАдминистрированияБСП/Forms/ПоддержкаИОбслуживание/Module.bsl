...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаОбработкаЗагрузкаКурсовВалют.Видимость = РежимРаботы.Локальный И ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоВалют");
	...
	Элементы.ОбработкаАктивныеПользователиОткрыть.Видимость								  = НЕ РежимРаботы.БазоваяВерсия;
	Элементы.ОбработкаБлокировкаСоединенийСИнформационнойБазойОткрыть.Видимость			  = НЕ РежимРаботы.БазоваяВерсия;
	Элементы.ГруппаОтчетыИОбработки.Видимость											  = НЕ РежимРаботы.БазоваяВерсия;
	Элементы.ГруппаКорректировкаДанных.Видимость						  				  = НЕ РежимРаботы.БазоваяВерсия;
	Элементы.ГруппаВерсионирование.Видимость                 							  = НЕ РежимРаботы.БазоваяВерсия;
	
	Если РежимРаботы.БазоваяВерсия Тогда
		Элементы.ОписаниеРаздела.Заголовок =
			 НСтр("ru='Контроль за состоянием программы, резервное копирование и восстановление, обновление версии.';uk='Контроль за станом програми, резервне копіювання і відновлення, оновлення версії.'");
	КонецЕсли;
...
Процедура ЦентрМониторингаАдресСервисаПриИзменении(Элемент)
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ru='Адрес сервиса';uk='Адреса сервісу'") + " "
			+ ЦентрМониторингаАдресСервиса + " "
			+ НСтр("ru='не является допустимым адресом веб-сервиса для отправки отчетов об использовании программы.';uk='не є допустимою адресою веб-сервісу для відправки звітів про використання програми.'"); 
