...
Функция ДанныеДокументовДляОбменаСБанком(МассивДокументов, ДатаПолученияДанных, ПлатежныйДокумент = Неопределено) Экспорт
	...
	Если ТипДокументов = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыВБанк") Тогда
		Возврат Документы.ВедомостьНаВыплатуЗарплатыВБанк.ДанныеВедомостиНаВыплатуЗарплатыВБанк(МассивДокументов, ДатаПолученияДанных, ПлатежныйДокумент);
	ИначеЕсли ТипДокументов = Тип("ДокументСсылка.ЗаявкаНаОткрытиеЛицевыхСчетовСотрудников") Тогда
		Возврат Документы.ЗаявкаНаОткрытиеЛицевыхСчетовСотрудников.ДанныеОткрытияЛицевыхСчетов(МассивДокументов, ДатаПолученияДанных);
	ИначеЕсли ТипДокументов = Тип("ДокументСсылка.ЗаявкаНаЗакрытиеЛицевыхСчетовСотрудников") Тогда
		Возврат Документы.ЗаявкаНаЗакрытиеЛицевыхСчетовСотрудников.ДанныеЗакрытияЛицевыхСчетов(МассивДокументов, ДатаПолученияДанных);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеПлатежногоДокумента(ПлатежныйДокумент, МассивВедомостей, МассивОшибок) Экспорт
	...
	Если Результат.Количество() > 1 Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В документе %1 должны присутствовать ведомости только из одной организации и зарплатного проекта.';uk='У документі %1 повинні бути відомості тільки з однієї організації і зарплатного проекту.'"), ПлатежныйДокумент);
		МассивОшибок.Добавить(Новый Структура("ТекстСообщения, Ведомость", ТекстОшибки, Неопределено));
	КонецЕсли;
...
Процедура ПрисоединитьФайлОбменаСБанкамиКОбъекту(ВладелецФайла, ДанныеШапкиДокумента, ДанныеСтрокДокумента, СтруктураПараметровДляФормированияФайла) Экспорт
	...
	Если Отказ Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
			"Объект.Номер",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='По документу %1 получено подтверждение из банка. Изменение документа невозможно';uk='За документом %1 отримано підтвердження з банку. Зміна документа неможлива'"), ВладелецФайла),
			ВладелецФайла,
			,
			НСтр("ru='По документам получены подтверждения из банка. Изменение документов невозможно.';uk='За документами отримано підтвердження з банку. Зміна документів неможлива.'"));
		Возврат;
	КонецЕсли;
	...
	ПрисоединенныйФайл = ПрисоединенныеФайлы.ДобавитьПрисоединенныйФайл(ПараметрыФайла, ФайлОбменаСБанками.АдресФайла);
	
	ОписаниеФайла = ПрисоединенныеФайлы.ПолучитьДанныеФайла(ПрисоединенныйФайл, СтруктураПараметровДляФормированияФайла.УникальныйИдентификаторФормы, Истина);
...
Функция ЗагрузитьПодтвержденияИзБанка(МассивОписанийФайлов, ДокументОбъектПодтверждение = Неопределено) Экспорт
	...
	Для каждого ЭлементМассива Из МассивОписанийФайлов Цикл
		...
		Если ДокументПодтверждения <> Неопределено Тогда
			...
			ПрисоединенныйФайл = ПрисоединенныеФайлы.ДобавитьПрисоединенныйФайл(ПараметрыФайла, АдресФайлаВоВременномХранилище);
