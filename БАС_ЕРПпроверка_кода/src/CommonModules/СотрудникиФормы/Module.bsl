...
Функция СообщениеОКонфликтеВидаЗанятостиНовогоСотрудникаССуществующими(Сотрудник, ФизическоеЛицо, Организация, ВидЗанятости, ДатаПриема) Экспорт
	...
	Если ЗначениеЗаполнено(ДатаПриема) И ЗначениеЗаполнено(Организация) И ЗначениеЗаполнено(ВидЗанятости) Тогда
		...
		СписокНеобходимыхДанных = "ГоловнаяОрганизация,Организация,ТабельныйНомер,Должность,ВидЗанятости,ОформленПоТрудовомуДоговору,ДатаПриема,ДатаУвольнения";
		...
		Для Каждого СтрокаТаблицаПоРабочимМестам Из ТаблицаПоРабочимМестам Цикл
			...
			Если НЕ (СтрокаТаблицаПоРабочимМестам.ОформленПоТрудовомуДоговору И СтрокаТаблицаПоРабочимМестам.ДатаПриема <= ДатаПриема
				И (СтрокаТаблицаПоРабочимМестам.ДатаУвольнения > ДатаПриема ИЛИ СтрокаТаблицаПоРабочимМестам.ДатаУвольнения = '00010101000000')) Тогда
				Продолжить;
			КонецЕсли; 
...
Функция СтандартныеОтборыСотрудников() Экспорт
	...
	Возврат СотрудникиФормыВнутренний.СтандартныеОтборыСотрудников();
	
КонецФункции

Процедура УстановитьСтандартныеОтборыСотрудников(КоллекцияОтборов) Экспорт
	
	СтандартныеОтборы = СтандартныеОтборыСотрудников();
	Для каждого СтандартныйОтбор Из СтандартныеОтборы Цикл
		
		Если НЕ КоллекцияОтборов.Свойство(СтандартныйОтбор.Ключ) Тогда
			КоллекцияОтборов.Вставить(СтандартныйОтбор.Ключ, СтандартныйОтбор.Значение);
		КонецЕсли;
...
Процедура ПроверитьСведенияОГражданствеИДокументеУдостоверяющемЛичность(Форма, Отказ) Экспорт
	...
	Если Не РедактированиеПериодическихСведенийКлиентСервер.ЗаполненыЗначенияПоУмолчаниюПоСтруктуре(Форма, "ДокументыФизическихЛиц", СтруктураВедущихОбъектов) Тогда
		Если НЕ ЗначениеЗаполнено(Форма.ДокументыФизическихЛиц.ДатаВыдачи) Тогда
			
			ТекстСообщения = НСтр("ru='Документ: не заполнено поле ""Дата выдачи"".';uk='Документ: не заповнено поле ""Дата видачі"".'");
...
Процедура ДобавитьКомандыПечатиСправочникуСотрудники(КомандыПечати) Экспорт
	...
	СотрудникиФормыВнутренний.ДобавитьКомандыПечатиСправочникуСотрудники(КомандыПечати);
...
Процедура ДобавитьКомандуПечатиПриказаОбУвольненииВMSWord(КомандыПечати) Экспорт
...
Процедура ДобавитьКомандуПечатиПриказаОПереводеВMSWord(КомандыПечати) Экспорт
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.МенеджерПечати = "Обработка.ПечатьКадровыхПриказов";
	КомандаПечати.Идентификатор = "ПФ_MXL_Перевод";
	КомандаПечати.Представление = НСтр("ru='Приказ о переводе (Word)';uk='Наказ про переведення (Word)'");
	КомандаПечати.Порядок = 20;
	КомандаПечати.Картинка = БиблиотекаКартинок.ФорматWord2007;
	КомандаПечати.ФорматСохранения = ТипФайлаТабличногоДокумента.DOCX;
...
Процедура ДобавитьКомандуПечатиПриказаОПриемеВMSWord(КомандыПечати) Экспорт
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.МенеджерПечати = "Обработка.ПечатьКадровыхПриказов";
	КомандаПечати.Идентификатор = "ПФ_MXL_UK_П1";
...
Процедура ДобавитьКомандуПечатиЛичнойКарточкиП2(КомандыПечати) Экспорт
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.МенеджерПечати = "Справочник.Сотрудники";
	КомандаПечати.Идентификатор = "ПФ_MXL_UK_ЛичнаяКарточка";
	КомандаПечати.Представление = НСтр("ru='Личная карточка (П-2)';uk='Особова картка (П-2)'");
...
Процедура УстановитьСпособРасчетаАвансаПоУмолчанию(Форма) Экспорт
	...
	Если Форма.ДоступенПросмотрДанныхДляНачисленияЗарплаты И НЕ Форма.ПолучитьФункциональнуюОпциюФормы("ИспользоватьКадровыйУчет") Тогда
...
Функция ПодобратьСписокФизЛиц(ФизическоеЛицоСсылка, Знач Фамилия, Знач Имя, Знач Отчество, Знач Уточнение = Неопределено) Экспорт
	...
	Если МассивФизлиц.Количество() > 0 Тогда
		...
		Иначе
			...
			СтруктураВозврат.Вставить("ВозможнаПроверкаПоКодПоДРФО", Ложь);
			...
			|ПОМЕСТИТЬ ВТМинимальныеКодПоДРФО
			|ИЗ
			...
			|ВЫБРАТЬ
			|	ВЫБОР
			|		КОГДА МинимальныеКодПоДРФО.КодПоДРФО = """"
			|			ТОГДА ЛОЖЬ
			|		ИНАЧЕ ИСТИНА
			|	КОНЕЦ КАК ВозможнаПроверкаПоКодПоДРФО
			|ИЗ
			|	ВТМинимальныеКодПоДРФО КАК МинимальныеКодПоДРФО
			|";
...
Функция ТекстСообщенияОКонфликтеВидаЗанятости(ВидЗанятости, Организация, КонфликтныйСотрудникОрганизация = Неопределено, КонфликтныйСотрудникВидЗанятости = Неопределено, КонфликтныйСотрудникТабельныйНомер = "", КонфликтныйСотрудникДолжность = Неопределено) Экспорт
	...
	ИначеЕсли ВидЗанятости = Перечисления.ВидыЗанятости.ВнутреннееСовместительство Тогда
		ТекстСообщения = НСтр("ru='Нельзя оформить сотрудника на внутреннее совместительство, так как он не работает в %1.';uk='Не можна оформити працівника на внутрішнє сумісництво, так як він не працює в %1.'");
	КонецЕсли;
...
Функция ПолучитьВидЗанятостиДляНовогоСотрудника(Сотрудник, Организация, ФизическоеЛицо) Экспорт
	...
	СписокНеобходимыхДанных = "ГоловнаяОрганизация,Организация,ВидЗанятости,ОформленПоТрудовомуДоговору,ДатаПриема,ДатаУвольнения";
...
Функция ИзменяемыеПоляФизическогоЛица() Экспорт
	...
	ИзменяемыеПоля.Вставить("КодПоДРФО", "КодПоДРФОИзменен");
...
Функция ПолучитьСведенияОЛьготахНДФЛ(ФизическоеЛицо) Экспорт
	...
	Если Пользователи.РолиДоступны("ЧтениеНалоговИВзносов,ДобавлениеИзменениеНалоговИВзносов") Тогда
		...
		"////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	МАКСИМУМ(ЛьготыПоНДФЛ.МесяцРегистрации) КАК МесяцРегистрации
		|ИЗ
		|	(ВЫБРАТЬ
		...
		|	ГДЕ
		|		ЛьготыНаДетейНДФЛ.ФизическоеЛицо = &ФизическоеЛицо
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		ЛьготыФизическихЛицНДФЛ.Период
		|	ИЗ
		|		РегистрСведений.ЛьготыФизическихЛицНДФЛ КАК ЛьготыФизическихЛицНДФЛ
		|	ГДЕ
		|		ЛьготыФизическихЛицНДФЛ.ФизическоеЛицо = &ФизическоеЛицо) КАК ЛьготыПоНДФЛ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ЛьготыФизическихЛицНДФЛСрезПоследних.Регистратор,
		|	ЛьготыФизическихЛицНДФЛСрезПоследних.Регистратор.Номер КАК Номер,
		|	ЛьготыФизическихЛицНДФЛСрезПоследних.Регистратор.Дата КАК Дата,
		|	ЛьготыФизическихЛицНДФЛСрезПоследних.Льгота,
		|	ЛьготыФизическихЛицНДФЛСрезПоследних.Регистратор.Сотрудник КАК Сотрудник,
		|	ЛьготыФизическихЛицНДФЛСрезПоследних.Регистратор.Организация КАК Организация
		|ИЗ
		|	РегистрСведений.ЛьготыФизическихЛицНДФЛ.СрезПоследних(, ФизическоеЛицо = &ФизическоеЛицо) КАК ЛьготыФизическихЛицНДФЛСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|	ЛьготыНаДетейНДФЛ.Регистратор.Номер КАК Номер,
		|	ЛьготыНаДетейНДФЛ.Регистратор.Дата КАК Дата,
		|	ЛьготыНаДетейНДФЛ.Регистратор.Организация КАК Организация,
		|	ЛьготыНаДетейНДФЛ.Регистратор.Сотрудник КАК Сотрудник,
		...
		|			РегистрСведений.ЛьготыНаДетейНДФЛ КАК ЛьготыНаДетейНДФЛ
		|		ГДЕ
		|			ЛьготыНаДетейНДФЛ.ФизическоеЛицо = &ФизическоеЛицо) КАК ВложенныйЗапрос
		...
		Для ИндексРезультата = 0 По 2 Цикл
...
Процедура СохранитьДанныеДополнительныхФорм(Форма, Отказ) Экспорт
	...
	Если ТипЗнч(Форма.ПрочитанныеДанныеФорм) = Тип("ФиксированноеСоответствие") Тогда
		Для каждого ИмяФормы Из Форма.ПрочитанныеДанныеФорм Цикл
			Форма.СохранитьДанныеДополнительнойФормы(ИмяФормы.Ключ, Отказ); 
		КонецЦикла;
	КонецЕсли;
...
//			РазделМеню
//
Функция РазделМеню(Заголовок, ПунктыМеню, ИсключаемыеПункты = Неопределено) Экспорт
	
	Подменю = ЗарплатаКадры.РазделМеню(Заголовок);
	Подменю.Вставить("ИменаНастроенныхОбъектовМетаданных", Новый Массив);
	
	ПредставленияДоступныхОбъектов = ЗарплатаКадры.ПредставленияДоступныхОбъектовМетаданных(ПунктыМеню, "Добавление", ИсключаемыеПункты);
...
Функция ПунктыМенюНеНастроенныхОбъектов(ОписаниеМеню) Экспорт
	...
	Если ОписаниеМеню.Свойство("ИменаНастроенныхОбъектовМетаданных") Тогда
		ИменаНастроенныхОбъектов = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(ОписаниеМеню.ИменаНастроенныхОбъектовМетаданных);
	Иначе
