...
#КонецОбласти	
	...
	ИначеЕсли ЭтоФормаПодбораВДокументыПродажи Тогда
		...
		Элементы.ОстаткиТоваров.Видимость = Форма.ОтображатьОстатки И (Не Форма.СкрыватьКомандуОстаткиНаСкладах Или ОтображатьОстаткиСклады);
		
		ВидимостьПолейКоличестваТоваров = (Не Форма.СкрыватьКомандуОстаткиНаСкладах
											Или ОтображатьОстаткиСклады);
		...
		ЗаголовокКнопкиЗапрашиватьКоличествоИЦену = НСтр("ru='Запрашивать количество и цену';uk='Запитувати кількість і ціну'");
...
Функция ШаблонТекстаЗапросаспискаХарактеристикПоПараметрамСоглашенияСПоставщиком(ВариантРасчетаЦенПоставщика)
	...
	ИначеЕсли ВариантРасчетаЦенПоставщика = "ЦеныПоСоглашению" Тогда
		...
		|	ВЫРАЗИТЬ({СвободныеОстаткиВНаличии} / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки2, 1) КАК ЧИСЛО(15, 2)) КАК ВНаличииОстаток,
		|	ВЫРАЗИТЬ({ВсеОстаткиДоступно} / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки2, 1) КАК ЧИСЛО(15, 2)) КАК Доступно
...
Процедура ЗаполнитьДеревоЗначенийОтборов(Форма, ДеревоОтборов, ПоХарактеристикам)
	...
	Для Каждого Реквизит из РеквизитыБыстрогоОтбора Цикл
		...
		Иначе
			...
			Если МассивНеиспользуемыхРеквизитов.Найти(Реквизит.ИмяРеквизита) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
