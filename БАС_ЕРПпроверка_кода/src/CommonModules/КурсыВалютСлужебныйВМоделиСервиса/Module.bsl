...
Процедура ЗагрузитьКурсы() Экспорт
	...
	Если Дескрипторы.Descriptor.Количество() < 1 Тогда
		ВызватьИсключение(НСтр("ru='В менеджере сервиса отсутствуют данные вида ""КурсыВалют""';uk='В менеджері сервісу відсутні дані ""КурсыВалют""'"));
	КонецЕсли;
...
Процедура ОбработатьПоставляемыеКурсыЗаДень(Знач Дескриптор, Знач ПутьКФайлу)
	...
	Если ДатаКурсов = "" Тогда
		ВызватьИсключение НСтр("ru='Данные вида ""КурсыВалютЗаДень"" не содержат характеристики ""Дата"". Обновление курсов невозможно.';uk='Дані ""КурсыВалютЗаДень"" не містять характеристики ""Дата"". Оновлення курсів неможливо.'"); 
	КонецЕсли;
...
Процедура РаспространитьКурсыПоОД(Знач ДатаКурсов, Знач ТаблицаКурсов, 
	...
	Для каждого ОбластьДанных Из ОбластиДляОбновления Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые данные.Распространение курсов валют по областям данных';uk='Дані,  що поставляються. Поширення курсів валют по областях даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
