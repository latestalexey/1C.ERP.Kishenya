...
Функция СформироватьДанныеЗаполненияПоступления(МассивСсылок, РеквизитыШапки) Экспорт
	...
	Запрос = Новый Запрос("
		...
		|		&ОписаниеПоступленияСД              КАК ОписаниеПоступления,
		...
		|		ЛОЖЬ                               КАК РассчитыватьДатуВозвратаТарыПоКалендарю,
		|		НЕОПРЕДЕЛЕНО                       КАК КалендарьВозвратаТары,
		|		&ОписаниеПоступленияОП             КАК ОписаниеПоступления,
		...
		|
		|ВЫБРАТЬ
		|	ЗаказДавальца.Ссылка КАК ЗаказПоставщику,
		|	ЗаказДавальца.Статус КАК Статус,
		|	ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовДавальцев.КПроизводству) КАК ДопустимыйСтатус1,
		|	ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовДавальцев.КОтгрузке)     КАК ДопустимыйСтатус2,
		|	ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовДавальцев.Закрыт)        КАК ДопустимыйСтатус3,
		...
		|		  ЗаказДавальца.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовДавальцев.КПроизводству)
		|		И ЗаказДавальца.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовДавальцев.КОтгрузке)
		...
		|
		|ВЫБРАТЬ
		|	ЗаказПереработчику.Ссылка КАК ЗаказПоставщику,
		|	ЗаказПереработчику.Статус КАК Статус,
		|	ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПереработчикам.КИсполнению) КАК ДопустимыйСтатус1,
		|	ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПереработчикам.Закрыт)      КАК ДопустимыйСтатус2,
		|	НЕОПРЕДЕЛЕНО                                                    КАК ДопустимыйСтатус3,
		...
		|		  ЗаказПереработчику.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПереработчикам.КИсполнению)
	...
	//++ НЕ УТ
	Запрос.УстановитьПараметр("ОписаниеПоступленияСД", НСтр("ru='поступления сырья от давальца';uk='надходження сировини від давальця'"));
	Запрос.УстановитьПараметр("ОписаниеПоступленияОП", НСтр("ru='поступления от переработчика';uk='надходження від переробника'"));
	//-- НЕ УТ
	РезультатЗапроса = Запрос.ВыполнитьПакет();
