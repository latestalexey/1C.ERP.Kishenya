	...
	ИначеЕсли ТипЗнч(Контекст.ДанныеСертификата) <> Тип("Строка") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при вызове процедуры ОткрытьСертификат общего модуля ЭлектроннаяПодписьКлиент:
                       ...
                       |;uk='Помилка при виклику процедури ОткрытьСертификат загального модуля ЭлектроннаяПодписьКлиент:
                       |Некоректне значення параметра ДанныеСертификата ""%1"".'"),
...
Процедура СохранитьСертификатПродолжение(Контекст)
	...
	Диалог.Заголовок = НСтр("ru='Выберите файл для сохранения сертификата';uk='Виберіть файл для збереження сертифіката'");
	Диалог.Фильтр    = НСтр("ru='Файлы сертификатов (*.cer)|*.cer|Все файлы (*.*)|*.*';uk='Файли сертифікатів (*.cer)|*.cer|Все файли (*.*)|*.*'");
...
Процедура СохранитьСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Сертификат сохранен в файл:';uk='Сертифікат збережений у файл:'"),,
...
Процедура СохранитьЗапросНаСертификатПослеУстановкиРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
	Диалог.Заголовок = НСтр("ru='Выберите файл для сохранения запроса на сертификат';uk='Оберіть файл для збереження запиту на сертифікат'");
	Диалог.Фильтр    = НСтр("ru='Файлы сертификатов (*.p10)|*.p10|Все файлы (*.*)|*.*';uk='Файли сертифікатів (*.p10)|*.p10|Всі файли (*.*)|*.*'");
	Диалог.МножественныйВыбор = Ложь;
...
Процедура СохранитьЗапросНаСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Иначе
		ЕстьПолученныеФайлы = Истина;
		ПоказатьОповещениеПользователя(НСтр("ru='Запрос на сертификат сохранен в файл:';uk='Запит на сертифікат збережений у файл:'"),,
			ПолученныеФайлы[0].Имя);
	КонецЕсли;
...
Процедура СохранитьПодписьПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Файлы электронных подписей (*.%1)|*.%1|Все файлы (*.*)|*.*';uk='Файли електронних підписів (*. %1)|*. %1|Всі файли (*.*)|*.*'"),
		РасширениеДляФайловПодписи);
	
	ДиалогОткрытияФайла.Фильтр = Фильтр;
	ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
	ДиалогОткрытияФайла.Заголовок = НСтр("ru='Выберите файл для сохранения подписи';uk='Виберіть файл для збереження підпису'");
...
Процедура СохранитьПодписьПослеПолученияФайла(ПолученныеФайлы, Контекст) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Электронная подпись сохранена в файл:';uk='Електронний підпис збережена у файл:'"),,
		ПолученныеФайлы[0].Имя);
	...
	Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Сертификат не найден на компьютере по причине:
                       |%1'
                       |;uk='Сертифікат не знайдений на комп''ютері через:
                       |%1'"),
			ПредставлениеОшибки);
	Иначе
		ТекстОшибки = НСтр("ru='Сертификат не найден на компьютере.';uk='Сертифікат не знайдений на комп''ютері.'");
	КонецЕсли;
...
Процедура ПроверитьПодписьНаКлиентеПослеПроверкиПодписи(Сертификат, Контекст) Экспорт
	...
	Если Сертификат = Неопределено Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение,
			НСтр("ru='Сертификат не найден в данных подписи.';uk='Сертифікат не знайдений в даних підпису.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьСертификатНаКлиентеПослеОшибкиПроверки(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Если Контекст.ОписаниеОшибкиНаСервере <> Неопределено Тогда
		ОписаниеОшибки = Контекст.ОписаниеОшибкиНаСервере + " " + НСтр("ru='(на сервере)';uk='(на сервері)'") + Символы.ПС
			+ ОписаниеОшибки + " " + НСтр("ru='(на клиенте)';uk='(на клієнті)'");
	КонецЕсли;
...
Процедура СоздатьМенеджерКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	ЗаголовокФормы = НСтр("ru='Требуется программа электронной подписи и шифрования';uk='Потрібна програма електронного підпису та шифрування'");
	Операция       = Контекст.Операция;
	
	Если Операция = "Подписание" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось подписать данные по причине:';uk='Не вдалося підписати дані з причини:'");
		
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось проверить подпись по причине:';uk='Не вдалося перевірити підпис з причини:'");
		
	ИначеЕсли Операция = "Шифрование" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось зашифровать данные по причине:';uk='Не вдалося зашифрувати дані з причини:'");
		
	ИначеЕсли Операция = "Расшифровка" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось расшифровать данные по причине:';uk='Не вдалося розшифрувати дані з причини:'");
		
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось проверить сертификат по причине:';uk='Не вдалося перевірити сертифікат з причини:'");
		
	ИначеЕсли Операция = "ПолучениеСертификатов" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось получить сертификаты по причине:';uk='Не вдалося отримати сертифікати з причини:'");
	...
	ИначеЕсли Операция <> "" Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка в функции МенеджерКриптографии.
                       ...
                       |;uk='Помилка у функції МенеджерКриптографии.
                       |Невірне значення параметра Операція ""%1"".'"), Операция);
	Иначе
		ЗаголовокОшибки = НСтр("ru='Не удалось выполнить операцию по причине:';uk='Не вдалося виконати операцію з причини:'");
	КонецЕсли;
	...
	Если Не Подключено Тогда
		ТекстОшибки =
			НСтр("ru='В обозреватель интернет требуется установить расширение
                       ...
                       |;uk='У оглядач інтернет потрібно встановити розширення
                       |для роботи з електронним підписом і шифруванням.'");
...
Процедура СоздатьМенеджерКриптографииПослеЦикла(Контекст)
	...
	Если СвойстваОшибки.Ошибки.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru='Не предусмотрено использование ни одной программы.';uk='Не передбачено використання жодної програми.'");
		СвойстваОшибки.Вставить("Описание", ТекстОшибки);
		...
		Если Не СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ЭтоПолноправныйПользователь Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр("ru='Обратитесь к администратору.';uk='Зверніться до адміністратора.'");
		КонецЕсли;
	...
	Иначе
		Если ЗначениеЗаполнено(Контекст.Программа) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Программа %1 не доступна или не установлена на компьютере.';uk='Програма %1 не доступна або не встановлена на комп''ютері.'"),
				Контекст.Программа);
		Иначе
			ТекстОшибки = НСтр("ru='Ни одна из программ не доступна или не установлена на компьютере.';uk='Жодна з програм не доступна або не встановлена на комп''ютері.'");
		КонецЕсли;
		СвойстваОшибки.Вставить("Описание", ТекстОшибки);
	КонецЕсли;
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПодключенияРасширения(Подключено, Контекст) Экспорт
	...
	Диалог.Заголовок = НСтр("ru='Выберите файл сертификата (только для шифрования)';uk='Оберіть файл сертифіката (тільки для шифрування)'");
	Диалог.Фильтр = НСтр("ru='Сертификат X.509 (*.cer;*.crt)|*.cer;*.crt|Все файлы(*.*)|*.*';uk='Сертифікат X. 509 (*.cer;*.crt)|*.cer;*.crt|Всі файли(*.*)|*.*'");
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПомещенияФайла(Адрес, Контекст)
	...
	Если Форма = Неопределено Тогда
		ПоказатьПредупреждение(,
			НСтр("ru='Файл сертификата должен быть в формате DER X.509, операция прервана.';uk='Файл сертифіката повинен бути у форматі DER X. 509, операція перервана.'"));
		Возврат;
	КонецЕсли;
	
	Если Не Форма.Открыта() Тогда
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Открыть", НСтр("ru='Открыть';uk='Відкрити'"));
		...
		ПоказатьВопрос(
			...
			НСтр("ru='Сертификат уже добавлен.';uk='Сертифікат вже доданий.'"), Кнопки);
	КонецЕсли;
	...
	Если ТипЗнч(ОшибкаНаКлиенте) <> Тип("Структура") Тогда
		ВызватьИсключение
			НСтр("ru='Для процедуры ПоказатьОшибкуОбращенияКПрограмме
                       ...
                       |;uk='Для процедури ПоказатьОшибкуОбращенияКПрограмме
                       |вказано некоректний тип параметра ОшибкаНаКлиенте.'");
	КонецЕсли;
	...
	Если ТипЗнч(ОшибкаНаСервере) <> Тип("Структура") Тогда
		ВызватьИсключение
			НСтр("ru='Для процедуры ПоказатьОшибкуОбращенияКПрограмме
                       ...
                       |;uk='Для процедури ПоказатьОшибкуОбращенияКПрограмме
                       |вказано некоректний тип параметра ОшибкаНаСервере.'");
	КонецЕсли;
...
Процедура НачалоВыбораСертификатаПриУстановленномОтборе(Форма) Экспорт
	...
	Текст = НСтр("ru='Сертификаты, которые могут быть использованы для этой операции ограничены.';uk='Сертифікати, які можуть бути використані для цієї операції обмежені.'");
	...
	Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
		Заголовок = НСтр("ru='Следующие разрешенные сертификаты доступны для выбора:';uk='Такі дозволені сертифікати доступні для вибору:'");
	...
	Если ЗначениеЗаполнено(НедоступныеСертификаты) Тогда
		Если ЭлектроннаяПодписьКлиентСервер.ОбщиеНастройки().СоздаватьЭлектронныеПодписиНаСервере Тогда
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ru='Следующие разрешенные сертификаты не найдены ни на компьютере, ни на сервере:';uk='Такі дозволені сертифікати не знайдено ні на комп''ютері, ні на сервері:'");
			Иначе
				Заголовок = НСтр("ru='Ни один из следующих разрешенных сертификатов не найден ни на компьютере, ни на сервере:';uk='Жлден з наступних дозволених сертифікатів не знайдено ні на комп''ютері, ні на сервері:'");
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ru='Следующие разрешенные сертификаты не найдены на компьютере:';uk='Такі дозволені сертифікати не знайдені на комп''ютері:'");
			Иначе
				Заголовок = НСтр("ru='Ни один из следующих разрешенных сертификатов не найден на компьютере:';uk='Жоден з наступних дозволених сертифікатів не знайдено на комп''ютері:'");
			КонецЕсли;
		КонецЕсли;
...
Процедура ДобавитьПодписьИзФайлаПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если Контекст.ПроверитьМенеджерКриптографииНаКлиенте
	   И ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru='Требуется программа электронной подписи и шифрования';uk='Потрібна програма електронного підпису та шифрування'"),
...
Процедура СохранитьДанныеВместеСПодписью(ОписаниеДанных, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры = Новый Структура;
	СерверныеПараметры.Вставить("ЗаголовокДанных",     НСтр("ru='Данные';uk='Дані'"));
...
Процедура СохранитьДанныеВместеСПодписьюПослеСохраненияФайлаДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru='При записи файла возникла ошибка:';uk='При записі файлу виникла помилка:'") + Символы.ПС + Результат.ОписаниеОшибки);
		
		ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru='Не удалось сохранить подписи вместе с файлом';uk='Не вдалося зберегти підписи разом з файлом'"), "", Ошибка, Новый Структура);
		Возврат;
...
Процедура СохранитьДанныеВместеСПодписьюПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Текст = НСтр("ru='Папка с файлами:';uk='Папка з файлами:'") + Символы.ПС;
	Текст = Текст + Контекст.ПутьКФайлам;
	Текст = Текст + Символы.ПС + Символы.ПС;
	
	Текст = Текст + НСтр("ru='Файлы:';uk='Файли:'") + Символы.ПС;
...
Процедура УстановитьРасширениеПослеПроверкиПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	ОбщегоНазначенияКлиент.ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(
		...
		НСтр("ru='Перед установкой расширения для работы с электронной подписью и шифрованием
                   ...
                   |;uk='Перед установкою розширення для роботи з електронним підписом і шифруванням
                   |необхідно встановити розширення для роботи з файлами.'"),
		Ложь);
...
Процедура ПредставлениеДанныхНажатие(Форма, Элемент, СтандартнаяОбработка, ТекущийСписокПредставлений) Экспорт
	...
	Если ТекущийСписокПредставлений.Количество() > 1 Тогда
		...
		Если НоваяФорма = Неопределено Тогда
			Возврат;
		КонецЕсли;
		НоваяФорма.УстановитьСписокПредставлений(ТекущийСписокПредставлений,);
	Иначе
		...
		Если Контекст.НаСторонеКлиента Тогда
			...
			Исключение
				...
				ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки",
					НСтр("ru='При записи подписи возникла ошибка:';uk='При запису підпису виникла помилка:'")
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиПодписи(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru='При записи подписи возникла ошибка:';uk='При запису підпису виникла помилка:'") + Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиЗашифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru='При записи зашифрованных данных возникла ошибка:';uk='При запису зашифрованих даних виникла помилка:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиРасшифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru='При записи расшифрованных данных возникла ошибка:';uk='При запису розшифрованих даних виникла помилка:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьДанныеИзОписанияДанныхПродолжение(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("Структура")
	 ...
	 Или ТипЗнч(Результат.Данные) <> Тип("ДвоичныеДанные")
	   И ТипЗнч(Результат.Данные) <> Тип("Строка") Тогда
		
		Если ТипЗнч(Результат) <> Тип("Структура") Или Не Результат.Свойство("ОписаниеОшибки") Тогда
			Ошибка = Новый Структура("ОписаниеОшибки", НСтр("ru='Некорректный тип данных.';uk='Некоректний тип даних.'"));
		Иначе
...
Процедура ПолучитьДанныеИзОписанияДанныхЗавершение(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ПомещенныеФайлы = Неопределено Или ПомещенныеФайлы.Количество() = 0 Тогда
		Результат = Новый Структура("ОписаниеОшибки",
			НСтр("ru='Передача данных отменена пользователем.';uk='Передача даних скасовано користувачем.'"));
	Иначе
