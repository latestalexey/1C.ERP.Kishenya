...
// Устарела. Необходимо использовать функцию
...
Функция СетевыеПараметрыИнтернетПоддержки() Экспорт
	...
	Результат.Вставить("ТаймаутСети",
		ИнтернетПоддержкаПользователейКлиентСервер.НастройкиСоединенияССерверами().ТаймаутПодключения);
	Возврат Результат;
...
// Устарела. Необходимо использовать функцию
// ИнтернетПоддержкаПользователейКлиентСервер.НастройкиСоединенияССерверами().
// Определяет сторону (клиент или сервер платформы), на которой должно
// выполняться взаимодействие с веб-сервисом ИПП.
//
// Параметры:
// СетевыеПараметрыИПП - Структура - в параметре возвращаются сетевые
...
Функция ПодключениеКСервисуИППССервера1СПредприятия(СетевыеПараметрыИПП = Неопределено) Экспорт
	...
	ОбращениеССервера = ИнтернетПоддержкаПользователейКлиентСервер.НастройкиСоединенияССерверами().УстанавливатьПодключениеНаСервере;
...
// Предназначена для выполнения на стороне сервера платформы
...
//		обращение к сервису ИПП осуществляется с сервера платформы;
...
Функция ПараметрыСозданияКонтекста(МестоЗапуска, ПриНачалеРаботыСистемы)
	...
	Результат.Вставить("РаботаВМоделиСервиса", ОбщегоНазначенияПовтИсп.РазделениеВключено());
	...
	Результат.Вставить("ОбращениеССервера", НастройкиСоединения.УстанавливатьПодключениеНаСервере);
	Результат.Вставить("ТаймаутСети"      , НастройкиСоединения.ТаймаутПодключения);
...
// Диспетчер выполнения команд сервиса ИПП на стороне сервера платформы
//
...
// Завершение сеанса Интернет-поддержки на стороне сервера платформы.
//
...
Процедура ЗаписатьОбщиеПараметрыИПП(Знач ОбщиеПараметры) Экспорт
	...
	Если Не ИнтернетПоддержкаПользователей.ПравоЗаписиПараметровИПП() Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав для записи параметров Интернет-поддержки.';uk='Недостатньо прав для запису параметрів Інтернет-підтримки.'");
	КонецЕсли;
...
Функция ОтправитьДанныеСообщенияВТехПоддержку(
	...
	Возврат ИнтернетПоддержкаПользователейКлиентСервер.ОтправитьДанныеСообщенияВТехПоддержку(
		Тема,
		Сообщение,
		Получатель,
		Вложения,
...
Функция ДопПараметрыОтправкиСообщения(Знач ДопПараметры) Экспорт
	...
	Если ДопПараметры.Свойство("Логин") Тогда
		
		Результат.Логин = ДопПараметры.Логин;
		Если ДопПараметры.Свойство("Пароль") Тогда
			Если Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
				Результат.Пароль = ДопПараметры.Пароль;
			КонецЕсли;
			Результат.ПарольЗаполнен = ЗначениеЗаполнено(ДопПараметры.Пароль);
		КонецЕсли;
	...
	Иначе
		...
		Если ДанныеАутентификации <> Неопределено Тогда
			...
			Если Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
				// Для полноправного пользователя возвращается также пароль для получения
				// тикета аутентификации.
				Результат.Пароль = ДанныеАутентификации.Пароль;
			КонецЕсли;
			Результат.ПарольЗаполнен = Истина;
		КонецЕсли;
		
	КонецЕсли;
	
	ДопПараметры.Вставить("Логин", Результат.Логин);
...
Функция ТекстВложенияТехническаяИнформация(ВидПриложения, ДопПараметры) Экспорт
	...
	Результат =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ИнтернетПоддержкаПользователей.ВерсияСтандартныеПодсистемы(),
			ВидПриложения,
	...
	Результат = Результат
		...
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Настройки соединения с серверами Интернет-поддержки:
                |	Таймаут: %1;
                |	Устанавливать соединение на сервере платформы: %2;
                |	Доменная зона: %3'
                |;uk='Настройки з''єднання з серверами Інтернет-підтримки:
                |Таймаут: %1;
                |Встановлювати з''єднання на сервері платформи: %2;
                |Доменна зона: %3'"),
	...
	Если ДоступнаРаботаСНастройкамиКлиентаЛицензирования() Тогда
		ИДКонфигурации = КлиентЛицензирования.ИДКонфигурации();
...
//
Функция СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейОшибка() Экспорт
