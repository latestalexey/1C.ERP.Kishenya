				...
				Если МодульРаботаВМоделиСервиса.ОбластьДанныхЗаблокирована(ОбластьДанных) Тогда
					...
					Иначе
						ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Не удалось выполнить обмен сообщениями по причине: область данных %1 заблокирована!';uk='Не вдалося виконати обмін повідомленнями через: область даних %1 заблокована!'"),
							ОбластьДанных);
					КонецЕсли;
				КонецЕсли;
    ...
    Если ОбщегоНазначенияПовтИсп.РазделениеВключено() И ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
        ...
        ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииЭтойПодсистемы(),
                ...
                НСтр("ru='Обработка очереди сообщений системы запущена из сеанса с установленными
                      ...
                      |;uk='Обробка черги повідомлень системи запущена з сеансу з встановленими
                      |значеннями роздільників. Обробка буде проводитися тільки для повідомлень,
                      |які збережені в розділеному довіднику, в елементах зі значеннями роздільників,
                      |які збігаються зі значеннями роздільників сеансу.'"));
        ...
        Попытка
            ...
            Попытка
                ...
                Если СчетчикПопыток > 3 Тогда
                    Если НЕ ЗначениеЗаполнено(ОбъектСообщения.ПодробноеПредставлениеОшибки) Тогда
                        ОбъектСообщения.ПодробноеПредставлениеОшибки = НСтр("ru='Исчерпано количество попыток обработки сообщения.';uk='Вичерпано кількість спроб обробки повідомлення.'");
                    КонецЕсли;
            ...
            Если ОбъектСообщения.Заблокировано Тогда
                ...
                ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииЭтойПодсистемы(),
                        ...
                        СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                            НСтр("ru='Исчерпано количество попыток обработки сообщения %1';uk='Вичерпано кількість спроб обробки повідомлення %1'"),
            ...
            Попытка
                ...
                Если Выборка.КонечнаяТочкаЗаблокирована Тогда
                    ОбъектСообщения.Заблокировано = Истина;
                    ВызватьИсключение НСтр("ru='Попытка обработки сообщения, полученного от заблокированной конечной точки.';uk='Спроба обробки повідомлення, отриманого від заблокованої кінцевої точки.'");
                КонецЕсли;

                Если НайденныеСтроки.Количество() = 0 Тогда
                    ОбъектСообщения.Заблокировано = Истина;
                    ВызватьИсключение НСтр("ru='Не назначен обработчик для сообщения.';uk='Не призначений обробник для повідомлення.'");
                КонецЕсли;
                ...
                Для Каждого СтрокаТаблицы Из НайденныеСтроки Цикл
                    ...
                    Если ТранзакцияАктивна() Тогда
                        ...
                        КонецЦикла;
                        ОбъектСообщения.Заблокировано = Истина;
                        ВызватьИсключение НСтр("ru='В обработчике сообщения не была зафиксирована транзакция.';uk='В обробнику повідомлення не була зафіксована транзакція.'");
                    КонецЕсли;
            ...
            Исключение
                ...
                ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииЭтойПодсистемы(),
                        УровеньЖурналаРегистрации.Ошибка,,,
                        СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                            НСтр("ru='Ошибка обработки сообщения %1: %2';uk='Помилка обробки повідомлення %1: %2'"),
            ...
            Если ОбрабатыватьСообщенияВНеразделенныхДанных И ОбщегоНазначенияПовтИсп.РазделениеВключено() И ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
                ...
                ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                    НСтр("ru='После обработки сообщения канала %1 не было выключено разделение сеанса!';uk='Після обробки повідомлення каналу %1 не був виключений розподіл сеансу!'"),
...
Процедура УстановитьВедущуюКонечнуюТочкуНаСторонеПолучателя(КодЭтойКонечнойТочки, КодВедущейКонечнойТочки) Экспорт
	...
	Если ПланыОбмена.ОбменСообщениями.НайтиПоКоду(КодЭтойКонечнойТочки) <> ЭтотУзел() Тогда
		СтрокаСообщенияОбОшибке = НСтр("ru='Заданы неверные параметры подключения к конечной точке. Параметры подключения указывают на другую конечную точку.';uk='Задані невірні параметри підключення до кінцевої точки. Параметри підключення вказують на іншу кінцеву точку.'");
		СтрокаСообщенияОбОшибкеДляЖурналаРегистрации = НСтр("ru='Заданы неверные параметры подключения к конечной точке.
            ...
            |;uk='Задані невірні параметри підключення до кінцевої точки.
            |Параметри підключення вказують на іншу кінцеву точку.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Попытка
		...
		Если УзелКонечнойТочки.Пустая() Тогда
			...
			ВызватьИсключение НСтр("ru='Конечная точка в базе-корреспонденте не обнаружена.';uk='Кінцева точка в базі-кореспонденті не виявлена.'");
	...
	Попытка
		...
		Пока ВозможностьЧтенияXML(ЧтениеXML) Цикл
			...
			Если ТипЗнч(Данные) = Тип("УдалениеОбъекта") Тогда
				...
				ВызватьИсключение НСтр("ru='Передача объекта УдалениеОбъекта через механизм быстрых сообщений не поддерживается!';uk='Передача об''єкта УдалениеОбъекта через механізм швидких повідомлень не підтримується!'");
...
Функция СобытиеЖурналаРегистрацииПодключениеКонечнойТочки() Экспорт
	...
	Возврат НСтр("ru='Обмен сообщениями.Подключение конечной точки';uk='Обмін повідомленнями.Підключення кінцевої точки'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУстановкаВедущейКонечнойТочки() Экспорт
	...
	Возврат НСтр("ru='Обмен сообщениями.Установка ведущей конечной точки';uk='Обмін повідомленнями.Установка провідною кінцевої точки'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииЭтойПодсистемы() Экспорт
	...
	Возврат НСтр("ru='Обмен сообщениями';uk='Обмін повідомленнями'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьПодключениеКонечнойТочкиНаСторонеОтправителя(Отказ,
	...
	Если Не УзелКонечнойТочки.Пустая() Тогда
		Отказ = Истина;
		СтрокаСообщенияОбОшибке = НСтр("ru='Конечная точка уже подключена к информационной базе; наименование точки: %1';uk='Кінцева точка вже підключена до інформаційної бази; найменування точки: %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
