...
Процедура ПроверитьВозможностьРаботыСВнешнимиРесурсами(РегламентноеЗадание, ЗависимостиЗаданий = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Приложение было перемещено.
            ...
            |;uk='Додаток було переміщено.
            |Регламентне завдання ""%1"", яке працює з зовнішніми ресурсами, відключено.'"), РегламентноеЗадание.Синоним);
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Изменилась строка соединения информационной базы.
            ...
            |;uk='Змінився рядок з''єднання інформаційної бази.
            |Можливо інформаційна база була переміщена.
            |Регламентне завдання ""%1"" відключено.'"), РегламентноеЗадание.Синоним);
...
Функция НастройкиПоУмолчанию()
	...
	НастройкиПодсистемы.Вставить("РасположениеКомандыСнятияБлокировки",
		НСтр("ru='Блокировку также можно снять позднее в разделе <b>Администрирование - Поддержка и обслуживание</b>.';uk='Блокування також можна зняти пізніше у розділі <b>Адміністрування - Підтримка та обслуговування</b>.'"));
...
Функция ВыполнитьРегламентноеЗаданиеВручную(Знач Задание) Экспорт
	...
	Если СвойстваПоследнегоФоновогоЗадания <> Неопределено
	   И СвойстваПоследнегоФоновогоЗадания.Состояние = СостояниеФоновогоЗадания.Активно Тогда
		...
		КонецЕсли;
	Иначе
		НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Запуск вручную: %1';uk='Запуск вручну: %1'"), ПредставлениеРегламентногоЗадания(Задание));
...
Функция ТекстНеОпределено() Экспорт
	...
	Возврат НСтр("ru='<не определено>';uk='<не визначено>'");
...
Процедура ОтменитьФоновоеЗадание(Идентификатор) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Фоновое задание не найдено на сервере.';uk='Фонове завдання не знайдене на сервері.'");
	КонецЕсли;
	
	Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Активно Тогда
		ВызватьИсключение НСтр("ru='Задание не выполняется, его нельзя отменить.';uk='Завдання не виконується, його не можна скасувати.'");
	КонецЕсли;
...
Функция ИдентификаторФайловойБазы(ЭтоФайловаяБаза)
	...
	Если ЭтоФайловаяБаза Тогда
        ...
        |;uk='%1
        |
        |Файл створений автоматично прикладним рішенням ""%2"".
        |Він містить ідентифікатор інформаційної бази і дозволяє визначити, що ця інформаційна база була скопійована.
        |
        |При копіюванні файлів інформаційної бази, в тому числі при створенні резервної копії, не слід скопіювати цей файл.
        |Одночасне використання двох копій інформаційної бази з однаковим ідентифікатором може призвести до конфліктів
        |при синхронізації даних, надсилання пошти та іншій роботі із зовнішніми ресурсами.
        |
        |Якщо файл відсутній у каталозі з інформаційною базою, то програма запитає адміністратора, чи повинна ця
        |інформаційна база працювати із зовнішніми ресурсами.'"), ИдентификаторБазы, Метаданные.Синоним);
...
Процедура УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Работа с внешними ресурсами заблокирована';uk='Робота з зовнішніми ресурсами заблокована'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция УстановленаБлокировкаРаботыСВнешнимиРесурсами(ЭтоВызовПередНачаломРаботыСистемы = Ложь, ПоказатьФорму = Ложь) Экспорт
	...
	Если ПараметрыБлокировки.Свойство("РазделениеВключено")
		...
		ТекстСообщения = НСтр("ru='Информационная база была перемещена из приложения в Интернете.';uk='Інформаційна база була переміщена з програми в Інтернеті.'");
	...
	Если ЭтоФайловаяБаза <> ПараметрыБлокировки.ЭтоФайловаяБаза Тогда
		ТекстСообщения = ?(ЭтоФайловаяБаза, НСтр("ru='Информационная база была перемещена из клиент-серверного режима работы в файловый.';uk='Інформаційна база була переміщена з клієнт-серверного режиму роботи в файловий.'"),
			НСтр("ru='Информационная база была перемещена из файловый режима работы в клиент-серверный.';uk='Інформаційна база була переміщена з файлового режиму роботи в клієнт-серверний.'"));
		УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
		Возврат Истина;
	КонецЕсли;
	...
	Если ЭтоФайловаяБаза Тогда
		...
		Если НайденныеФайлы.Количество() = 0 Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В каталоге информационной базы отсутствует файл проверки %1.';uk='В каталозі інформаційної бази відсутній файл перевірки %1.'"), ИмяФайлаПроверки);
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
			Возврат Истина;
		КонецЕсли;
		...
		Если ИдентификаторБазы <> ПараметрыБлокировки.ИдентификаторБазы Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Идентификатор информационной базы в файле проверки %1 не соответствует идентификатору в текущей базе.';uk='Ідентифікатор інформаційної бази у файлі перевірки %1 не відповідає ідентифікатору в поточній базі.'"), ИмяФайлаПроверки);
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
			Возврат Истина;
		КонецЕсли;
		...
		Если ИмяБазы <> СохраненноеИмяБазы
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Различаются имя информационной базы или имя компьютера.
                    ...
                    |;uk='Розрізняються ім''я інформаційної бази або ім''я комп''ютера.
                    |Збережене ім''я бази: %1, поточне ім''я бази: %2.
                    |Збережене ім''я комп''ютера: %3, поточне ім''я комп''ютера: %4'"),
