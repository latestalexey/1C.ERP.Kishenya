...
Функция Рассчитать(Объект, ВходныеПараметры) Экспорт
	...
	//++ НЕ УТКА
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ОтчетДавальцу") Тогда
		
		РезультатРасчета = РассчитатьПоОтчетуДавальцу(Объект, ВходныеПараметры);
	//-- НЕ УТКА
	КонецЕсли;
...
Процедура ПрименитьРезультатРасчета(Объект, ПримененныеСкидки, РеализацияСверхЗаказа = Ложь) Экспорт
	...
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ОтчетДавальцу") Тогда
		...
		ПрименитьРезультатРасчетаКОтчетуДавальцу(Объект, ПримененныеСкидки);
	//-- НЕ УТКА
	КонецЕсли;
...
// Процедура выполняет расчет скидок по отчету давальцу.
//
Функция РассчитатьПоОтчетуДавальцу(Объект, ВходныеПараметры) Экспорт
	...
	ТаблицаТовары = Объект.Продукция.Выгрузить();
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.Параметры.Вставить("Товары", ТаблицаТовары);
	Запрос.Параметры.Вставить("Склад", Справочники.Склады.ПустаяСсылка());
	Запрос.Параметры.Вставить("Серия", Справочники.СерииНоменклатуры.ПустаяСсылка());
	Запрос.Параметры.Вставить("Упаковка", Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка());
	...
	ПараметрыРасчета.Товары                     = Продукция;
	ПараметрыРасчета.ВалютаДокумента            = Объект.Валюта;
	ПараметрыРасчета.ВалютаУправленческогоУчета = Константы.ВалютаУправленческогоУчета.Получить();
	ПараметрыРасчета.Пользователь               = Объект.Менеджер;
	ПараметрыРасчета.Объект                     = Объект;
	...
	Если ВходныеПараметры.ПрименятьКОбъекту Тогда
		...
		ПрименитьРезультатРасчетаКОтчетуДавальцу(Объект, ПримененныеСкидкиНаценки);
...
Процедура ПрименитьРезультатРасчетаКОтчетуДавальцу(Объект, ПримененныеСкидкиНаценки) Экспорт
...
Функция ПолучитьГруппуУмножениеРодитель(СтрокаДерева)
	...
	Иначе
		...
		ГруппаУмноженияВерхнегоУровня = ПолучитьГруппуУмножениеРодитель(Родитель);
		...
		Если ГруппаУмноженияВерхнегоУровня = Неопределено
			...
			Возврат Родитель;
		Иначе
			Возврат ПолучитьГруппуУмножениеРодитель(Родитель);
		КонецЕсли;
...
//
Функция ПолучитьСуммуПримененныхСкидок(СтрокиДерева, КлючСвязи, Параметры)
	
	Сумма = 0;
	
	Для Каждого СтрокаТЧ Из СтрокиДерева Цикл
		
		Если СтрокаТЧ.РезультатРасчета.Количество() > 0 Тогда
			Сумма = Сумма + СтрокаТЧ.РезультатРасчета.Итог("Сумма");
		КонецЕсли;
...
Функция ОстатокСуммыПоСтроке(СтрокаДерева, Товар, ПараметрыРасчета, ЭтоУмножение = Истина) Экспорт
	...
	Если ЭтоУмножение Тогда
		...
		ГруппаУмножение = ПолучитьГруппуУмножениеРодитель(СтрокаДерева);
		
		Если ГруппаУмножение <> Неопределено Тогда
			СуммаПримененнойСкидки = ПолучитьСуммуПримененныхСкидок(ГруппаУмножение.Строки, Товар.КлючСвязи, ПараметрыРасчета);
		Иначе
...
Функция ПолучитьРасшифровкуСкидки(СтрокаДерева, Сумма, ПараметрыРасчета) Экспорт
...
Процедура РассчитатьСкидку(СтрокаДерева, ПараметрыРасчета)
	...
	ИначеЕсли СтрокаДерева.СпособПредоставления = Перечисления.СпособыПредоставленияСкидокНаценок.Сообщение
		...
		НоваяСтрока.Действует   = Истина;
		НоваяСтрока.Расшифровка = ПолучитьРасшифровкуСкидки(СтрокаДерева, 0, ПараметрыРасчета);
...
Функция ЗапросОпределенияПозицийНоменклатурыНаКоторыеПредоставляетсяСкидка(СкидкаНаценка, ПараметрыРасчета, ЗаполняемыеПараметры)
	...
	ТекстЗапроса = ТекстЗапроса + РазделительЗапросовПакета("#Результат#");
...
Функция ЗапросУсловияЗаДеньРождения(УсловиеПредоставления, ПараметрыРасчета)
	...
	|	И ДобавитьКДате(Партнеры.ДатаРождения, ГОД, Год(&ДатаНачала) - Год(Партнеры.ДатаРождения)) МЕЖДУ &ДатаНачала И &ДатаОкончания
	|");
	
	Запрос.УстановитьПараметр("ДатаОкончания", КонецДня(ПараметрыРасчета.ТекущаяДата) + УсловиеПредоставления.КоличествоДнейПослеДняРождения * 24*60*60);
	Запрос.УстановитьПараметр("ДатаНачала", НачалоДня(ПараметрыРасчета.ТекущаяДата) - УсловиеПредоставления.КоличествоДнейДоДняРождения * 24*60*60);
	Запрос.УстановитьПараметр("Партнер", ПараметрыРасчета.Партнер);
...
Процедура УдалитьВременныеТаблицы(Запрос)
	...
	Для Каждого ИмяТаблицы Из ТаблицыДляУничтожения Цикл
		Запрос.Текст = Запрос.Текст + СтрЗаменить(
			РазделительЗапросовПакета() + "УНИЧТОЖИТЬ ИмяТаблицы", "ИмяТаблицы", ИмяТаблицы) + Символы.ПС;
	КонецЦикла;
...
Процедура ПакетЗапросовВставитьЗапросВПакет(Запрос,
	...
	Иначе
		ПакетЗапросов.Запрос.Текст = ПакетЗапросов.Запрос.Текст + РазделительЗапросовПакета() + Запрос.Текст;
	КонецЕсли;
