...
// Параметры:
// ТаблицаОбеспечения - Таблица, полученная механизмом обеспечения
// ОтгружатьОднойДатой - Признак необходимости сворачивания строк в таблице с одинаковым вариантом обеспечения, в пределах одного идентификатора исходной строки документа.
//
Процедура СвернутьСтрокиДляОтгрузкиОднойДатой(ТаблицаОбеспечения, ОтгружатьОднойДатой) Экспорт
	...
	Для Счетчик = 1 По ВсегоСтрок Цикл
		...
		Если Счетчик < ВсегоСтрок Тогда
			...
			Если ПредыдущаяСтрока.Идентификатор = СтрокаТаблицы.Идентификатор И ПредыдущаяСтрока.ВариантОбеспечения = СтрокаТаблицы.ВариантОбеспечения И СтрокаТаблицы.Склад = ПредыдущаяСтрока.Склад Тогда
...
Функция СостояниеОбеспеченияЗаказов(МенеджерВременныхТаблиц, Объект, Фильтры, Режим) Экспорт
	...
	Запрос.Текст = Запрос.Текст
		...
		|
		|	ИЗ
		|		ВтТаблицаЗаказа КАК Т) КАК Набор
		|;
		|
		|////////////////////////////////////////////////
		|"
...
//++ НЕ УТКА
		|		ИЛИ ТИПЗНАЧЕНИЯ(Т.Заказ) = ТИП(Документ.ЗаказНаПроизводство) И &СгруппироватьПоЗаказам
//-- НЕ УТКА
		|			ТОГДА
		|				&НачалоТекущегоДня
		|			ИНАЧЕ
		...
		|	ВЫБОР КОГДА Т.ЖелаемаяДатаОтгрузки < &НачалоТекущегоДня
//++ НЕ УТКА
		|		ИЛИ ТИПЗНАЧЕНИЯ(&Ссылка) = ТИП(Документ.ЗаказНаПроизводство) И &СгруппироватьПоЗаказам
//-- НЕ УТКА
		|			ТОГДА
		|				&НачалоТекущегоДня
		|			ИНАЧЕ
