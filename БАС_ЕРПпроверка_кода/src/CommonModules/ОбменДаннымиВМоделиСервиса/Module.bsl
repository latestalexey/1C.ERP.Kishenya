...
Процедура ПриДобавленииОбработчиковСлужебныхСобытий(КлиентскиеОбработчики, СерверныеОбработчики) Экспорт
	...
	СерверныеОбработчики[
		"СтандартныеПодсистемы.БазоваяФункциональность\ПриДобавленииПараметровРаботыКлиентаПриЗавершении"].Добавить(
		"ОбменДаннымиВМоделиСервиса");
	...
	СерверныеОбработчики["ТехнологияСервиса.РаботаВМоделиСервиса.ОбменДаннымиВМоделиСервиса\ПриСозданииАвтономногоРабочегоМеста"].Добавить(
		"ОбменДаннымиВМоделиСервиса");
					...
					Если Не РезультатЗапроса2.Пустой() Тогда
						...
						Если Выборка2.Следующий() Тогда
							...
							Если КонечнаяТочкаКорреспондента = Неопределено Тогда
								...
								ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не определена конечная точка корреспондента для адреса ""%1"".';uk='Не визначена кінцева точка кореспондента для адреси ""%1"".'"),
									Выборка2.Адрес);
							КонецЕсли;
...
Процедура ПеренестиПаролиВБезопасноеХранилище() Экспорт
	...
	|	НастройкиТранспортаОбменаОбластейДанных.КонечнаяТочкаКорреспондента КАК Узел,
	|	НастройкиТранспортаОбменаОбластейДанных.УдалитьFTPСоединениеПароль,
	|	НастройкиТранспортаОбменаОбластейДанных.УдалитьПарольАрхиваСообщенияОбмена
	|ИЗ
	...
	Пока РезультатЗапроса.Следующий() Цикл
		Если НЕ ПустаяСтрока(РезультатЗапроса.Узел) 
			...
			Попытка
				...
				НастройкиТранспортаОбмена = РегистрыСведений.НастройкиТранспортаОбменаОбластейДанных.СоздатьМенеджерЗаписи();
				НастройкиТранспортаОбмена.КонечнаяТочкаКорреспондента = РезультатЗапроса.Узел;
			...
			Исключение
				...
				СобытиеЖурналаРегистрацииОбменДанными = НСтр("ru='Обмен данными';uk='Обмін даними'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ОтносительныйКаталогОбменаИнформацией(Знач Каталог)
	...
	Если ПустаяСтрока(Каталог) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неправильный формат каталога обмена информацией ""%1"".';uk='Неправильний формат каталогу обміну інформацією ""%1"".'"),
			Каталог);
	КонецЕсли;
...
Процедура ВыполнитьДействиеСценарияОбменаДаннымиВПервойИнформационнойБазе(ИндексСтрокиСценария, СценарийОбменаДанными) Экспорт
	...
	Если СтрокаСценария.НомерИнформационнойБазы = 1 Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неизвестное действие ""%1"" при обмене данными между областями данных.';uk='Невідома дія ""%1"" при обміні даними між областями даних.'"),
...
Функция СформироватьНаименованиеПредопределенногоУзла() Экспорт
	...
	Возврат ?(ПустаяСтрока(ИмяПриложения), НСтр("ru='Приложение в Интернете';uk='Прикладна програма в Інтернеті'"), ИмяПриложения);
КонецФункции
...
Функция КонечнаяТочкаКорреспондента(Знач Корреспондент) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		СтрокаСообщения = НСтр("ru='Не назначена конечная точка корреспондента для ""%1"".';uk='Не призначена кінцева точка кореспондента для ""%1"".'");
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru='Поставляемые правила обмена не подходят для текущей конфигурации и предназначены для плана обмена %1 конфигурации %2 версии %3';uk='Правила обміну, що поставляються, не підходять для поточної конфігурації і призначені для обміну плану %1 конфігурації %2 версії %3'");
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые правила обмена данными.Загрузка поставляемых правил отменена';uk='Поставляються правила обміну даними.Завантаження поставляються правил скасовано'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Информация,,, ТекстСообщения);
...
Процедура ОбработатьПоставляемыеПравилаОбмена(Дескриптор, ПутьКФайлу)
	...
	Если ОписаниеПоставляемыхПравил.Использовать Тогда
		РегистрыСведений.ПравилаДляОбменаДанными.ЗагрузитьПоставляемыеПравила(ОписаниеПоставляемыхПравил.ИмяПланаОбмена, ПутьКФайлу);
	Иначе
		РегистрыСведений.ПравилаДляОбменаДанными.УдалитьПоставляемыеПравила(ОписаниеПоставляемыхПравил.ИмяПланаОбмена);
	КонецЕсли;
...
Процедура СоздатьНастройкуОбмена(
	...
	Попытка
		...
		Если ПустаяСтрока(КодЭтогоУзла) Тогда
			...
			СтрокаСообщения = НСтр("ru='Для предопределенного узла плана обмена ""%1"" не задан код.';uk='Для передобумовленого вузла плану обміну ""%1"" не заданий код.'");
		...
		Если ПустаяСтрока(ПолучитьФункциональнуюОпцию("ПрефиксИнформационнойБазы")) Тогда
			...
			Если ПустаяСтрока(Префикс) Тогда
				ВызватьИсключение НСтр("ru='В Менеджере сервиса не установлен префикс для этого приложения.';uk='В Менеджері сервісу не встановлений префікс для цього додатка.'");
			КонецЕсли;
		...
		Если ПроверитьКод И КодКорреспондента <> ФактическийКодКорреспондента Тогда
			...
			СтрокаСообщения = НСтр("ru='Ошибка назначения кода узла корреспондента.
                ...
                |;uk='Помилка призначення коду вузла кореспондента.
                |Призначене значення ""%1""
                |Фактичне значення ""%2"".'");
		...
		Если НастройкиТранспорта.ВидТранспортаСообщенийОбменаПоУмолчанию = Перечисления.ВидыТранспортаСообщенийОбмена.FILE Тогда
			...
			Если ПустаяСтрока(FILEОбщийКаталогОбменаИнформацией) Тогда
				...
				СтрокаСообщения = НСтр("ru='Не задан каталог обмена информацией для конечной точки ""%1"".';uk='Не заданий каталог обміну інформацією для кінцевої точки ""%1"".'");
			...
			Если Не ОбщийКаталог.Существует() Тогда
				...
				СтрокаСообщения = НСтр("ru='Каталог обмена информацией ""%1"" не существует.';uk='Каталог обміну інформацією ""%1"" не існує.'");
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Вид транспорта сообщений обмена ""%1"" для конечной точки %2 не поддерживается.';uk='Вид транспорту повідомлень обміну ""%1"" для кінцевої точки %2 не підтримується.'"),
...
Процедура СохранитьДанныеСессии(Знач Сообщение, Знач Представление = "") Экспорт
	...
	Если Не ПустаяСтрока(Представление) Тогда
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=' {%1}';uk=' {%1}'"), Представление);
	КонецЕсли;
	
	СтрокаСообщения = НСтр("ru='Сессия обмена сообщениями системы ""%1"" успешно завершена.%2';uk='Сесія обміну повідомленнями системи ""%1"" успішно завершена.%2'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ЗафиксироватьНеуспешноеВыполнениеСессии(Знач Сообщение, Знач Представление = "") Экспорт
	...
	СтрокаСообщения = НСтр("ru='Ошибка выполнения сессии обмена сообщениями системы ""%1"".%2
        ...
        |;uk='Помилка виконання сесії обміну повідомленнями системи ""%1"".%2
        |Опис помилки з кореспондента: %3'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
...
Функция СобытиеЖурналаРегистрацииНастройкаСинхронизацииДанных() Экспорт
	...
	Возврат НСтр("ru='Обмен данными в модели сервиса.Настройка синхронизации данных';uk='Обмін даними в моделі сервісу.Настройка синхронізації даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииМониторСинхронизацииДанных() Экспорт
	...
	Возврат НСтр("ru='Обмен данными в модели сервиса.Монитор синхронизации данных';uk='Обмін даними в моделі сервісу.Монітор синхронізації даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииСинхронизацииДанных() Экспорт
	...
	Возврат НСтр("ru='Обмен данными в модели сервиса.Синхронизация данных';uk='Обмін даними в моделі сервісу.Синхронізація даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииСессииОбменаСообщениямиСистемы()
	
	Возврат НСтр("ru='Обмен данными в модели сервиса.Сессии обмена сообщениями системы';uk='Обмін даними в моделі сервісу.Сесії обміну повідомленнями системи '",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция НайтиУзелИнформационнойБазы(Знач ИмяПланаОбмена, Знач КодУзла)
	...
	Если Результат = Неопределено Тогда
		Сообщение = НСтр("ru='Не найден узел плана обмена. Имя плана обмена %1; код узла %2 или %3';uk='Не знайдений вузол плану обміну. Ім''я плану обміну %1; код вузла %2 або  %3'");
...
Процедура ПередЗаписьюОбщихДанных(Объект, Отказ)
	...
	Если ТолькоПросмотр Тогда
        ...
        |;uk='Зміна нерозподілених даних (%1), що завантажуються з програми, в Автономному робочому місці заборонена.
...
Процедура ПриСозданииАвтономногоРабочегоМеста() Экспорт
	...
	Если ПользователиСлужебныйВМоделиСервисаБТС.ПользовательЗарегистрированКакНеразделенный(
		...
		ВызватьИсключение НСтр("ru='Создать автономное рабочее место можно только от имени разделенного пользователя.
            ...
            |;uk='Створити автономне робоче місце можна тільки від імені розділеного користувача.
