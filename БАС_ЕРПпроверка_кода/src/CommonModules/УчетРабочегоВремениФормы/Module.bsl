...
Процедура ДокументыОтклоненийПриСозданииНаСервере(Форма, ОписаниеДокумента, ЗначенияДляПервоначальногоЗаполнения, ДобавлятьЭлементыФормы = Истина, ДобавлятьРеквизитыФормы = Истина, ОтложенноеИзменение = Ложь) Экспорт
	...
	ДанныеФормы = УчетРабочегоВремениКлиентСервер.ДокументыОтклоненийДанныеФормыПоОписанию(Форма, ОписаниеДокумента, Ложь);
...
Процедура ДокументыОтклоненийПризнакВнутрисменноеПриИзменении(Форма, ОписаниеДокумента) Экспорт
	ДанныеФормы = УчетРабочегоВремениКлиентСервер.ДокументыОтклоненийДанныеФормыПоОписанию(Форма, ОписаниеДокумента);
...
Функция ДокументыОтклоненийСписокВыбораЗамещаемогоВремени(ДанныеФормы) Экспорт
	...
	ОписаниеПлановыхВидовВремени = УчетРабочегоВремени.ОписаниеПлановыхВидовВремениСотрудников(
	...
	Если ОписаниеПлановыхВидовВремени.Количество() = 0 Тогда
		...
		ОписаниеВидаВремени = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Явка, "Наименование, БуквенныйКод");
		ОписаниеВидаВремени.Вставить("ВидВремени", Явка);
	...
	Для Каждого ОписаниеВидаВремени Из ОписаниеПлановыхВидовВремени Цикл
		...
		ОписаниеВидаВремени = Новый Структура("ВидВремени, Представление", ОписаниеВидаВремени.ВидВремени, ВидВремениЗамещаемыйПредставление);										
		ДанныеВыбора.Добавить(ОписаниеВидаВремени, ВидВремениЗамещаемыйПредставление);				
	КонецЦикла;	
...
Процедура ДокументыОтклоненийУстановитьЗамещаемыйВидВремени(Форма, ОписаниеДокумента) 
	ДанныеФормы = УчетРабочегоВремениКлиентСервер.ДокументыОтклоненийДанныеФормыПоОписанию(Форма, ОписаниеДокумента, Истина);
...
Процедура ДокументыОтклоненийУстановитьПредставлениеЗамещаемогоВидаВремени(Форма, ОписаниеДокумента) 
	...
	Если ЗначениеЗаполнено(ДанныеФормы.ЗамещаемыйВидВремени) Тогда	
		ОписаниеВидаВремени = УчетРабочегоВремени.ОписаниеВидовВремени(ДанныеФормы.ЗамещаемыйВидВремени).Получить(ДанныеФормы.ЗамещаемыйВидВремени);
...
Функция ДокументыОтклоненийПредставлениеЗамещаемогоВидаВремени(ВидВремениЗамещаемый, НаименованиеЗамещающегоВидаВремени, НаименованиеЗамещаемогоВидаВремени) Экспорт
	Явка = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.Явка");
	...
	Если ВидВремениЗамещаемый = Явка Тогда
		ШаблонПредставления = НСтр("ru='%1 в дневное время';uk='%1 у денний час'");
	ИначеЕсли ВидВремениЗамещаемый = НочныеЧасы Тогда
		ШаблонПредставления = НСтр("ru='%1 в ночное время';uk='%1 в нічний час'");	
	ИначеЕсли ВидВремениЗамещаемый = ВечерниеЧасы Тогда
		ШаблонПредставления = НСтр("ru='%1 в вечернее время';uk='%1 у вечірній час'");	
	Иначе
		ШаблонПредставления = НСтр("ru='%1 вместо %2';uk='%1 замість %2'");
	КонецЕсли;
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредставления, НаименованиеЗамещающегоВидаВремени, НаименованиеЗамещаемогоВидаВремени);
...
Процедура ТабельПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт 
	...
	БуквенныйКодИмяРеквизита = УчетРабочегоВремени.БуквенныйКодИмяРеквизита(Форма.Объект.ПериодРегистрации);
...
Процедура ТабельПриЧтенииДанных(Форма)
	...
	УчетРабочегоВремениКлиентСервер.ТабельУстановитьДоступностьЭлементовПериодаВводаДанных(Форма);
	...
	ИсправлениеДокументовЗарплатаКадры.УстановитьПоляИсправления(Форма, "ПериодическиеСведения");
...
Процедура ТабельУстановитьСвойстваТаблицыДанных(Форма)
	...
	Если Не Форма.Объект.ИсправленныйДокумент.Пустая() Тогда 
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "Заполнить", "Доступность", Ложь);
	КонецЕсли;
...
Процедура ТабельОбновитьДанныеПоСотрудникам(Форма) Экспорт
	...
	Для Каждого СтрокаДанныхОВремени Из Объект.ДанныеОВремени Цикл
		Если СтрокаДанныхОВремени.ЭтоПерваяСтрокаПоСотруднику Тогда 
			МассивСтрокПоСотруднику = УчетРабочегоВремениКлиентСервер.ТабельСтрокиПоСотруднику(Объект.ДанныеОВремени, СтрокаДанныхОВремени.НомерСтрокиСотрудник);
...
Процедура ТабельЗаполнитьДанныеПоСотруднику(Форма, ЗаполнятьТерриторииИУсловия = Ложь) Экспорт
	...
	Если ГоловнаяСтрокаСотрудника.НеЗаполнятьПриИзмененииСотрудника Тогда
		...
		МассивСтрокПоСотруднику = УчетРабочегоВремениКлиентСервер.ТабельСтрокиПоСотруднику(Объект.ДанныеОВремени, ГоловнаяСтрокаСотрудника.НомерСтрокиСотрудник);
...
Процедура ТабельЗаполнитьДанныеПоСотрудникам(Форма, СписокСотрудников = Неопределено, СтрокиПоСотрудникам = Неопределено, ВысотаСтроки = 2, ЗаполнятьТерриторииИУсловия = Ложь)
	КодыВидовВремени = УчетРабочегоВремени.КодыВидовВремени(Форма.Объект.ПериодРегистрации);
...
Процедура ТабельДанныеВРеквизит(Форма)
	...
	КодыВидовВремени = УчетРабочегоВремени.КодыВидовВремени(Объект.ПериодРегистрации);
	...
	Для Каждого СтрокаДанныхОВремени Из Объект.ДанныеОВремени Цикл
		...
		Для НомерДня = 1 По 31 Цикл
			...
			СтрокаДанныхОВремени["Время" + НомерДня + "Представление"] = УчетРабочегоВремениКлиентСервер.ТабельПредставлениеВремениПоВиду(ОбозначениеВремени, Часы);
		КонецЦикла;	
		
		ТекущийСотрудник = СтрокаДанныхОВремени.Сотрудник;
	КонецЦикла;		
	
	ОбозначенияВидовВремени = УчетРабочегоВремениКлиентСервер.ТабельОбозначенияВидовВремени(Форма.ОписаниеВидовВремени);
	Для Каждого ГоловнаяСтрока Из СоответствиеСотрудниковСтрокам Цикл
		МассивСтрок = УчетРабочегоВремениКлиентСервер.ТабельСтрокиПоСотруднику(Объект.ДанныеОВремени, ГоловнаяСтрока.Значение.НомерСтрокиСотрудник);
	...
	УчетРабочегоВремениКлиентСервер.ТабельУстановитьИнфонадписьВысотаСтрок(Форма);
КонецПроцедуры
...
Процедура ТабельЗаполнитьНормуВремени(Форма)
	...
	СтрокиПоСотрудникам = УчетРабочегоВремениКлиентСервер.ТабельГоловныеСтрокиСотрудников(Объект.ДанныеОВремени);
	...
	ВыборкаПоНормеВремени = УчетРабочегоВремени.СформироватьЗапросПоНормеВремени(ТаблицаСотрудников, НеучитываемыеДокументы).Выбрать();
	...
	Пока ВыборкаПоНормеВремени.СледующийПоЗначениюПоля("Сотрудник") Цикл
		...
		СтрокиПоСотруднику = УчетРабочегоВремениКлиентСервер.ТабельСтрокиПоСотруднику(Объект.ДанныеОВремени, ГоловнаяСтрокаСотрудника.НомерСтрокиСотрудник);
...
Процедура ТабельДополнитьСтрокуПоСотруднику(ДанныеОВремени, ГоловнаяСтрока, КоличествоДобавляемыхСтрок)
	Для Сч = 1 По КоличествоДобавляемыхСтрок Цикл
		УчетРабочегоВремениКлиентСервер.ТабельНоваяСтрокаПоСотруднику(ДанныеОВремени, ГоловнаяСтрока);			
	КонецЦикла;	
КонецПроцедуры	
...
Процедура ТабельУстановитьВысотуСтрокПоСотрудникам(Форма, НоваяВысотаСтроки) Экспорт
	...
	Если Форма.ВысотаСтроки <> НоваяВысотаСтроки Тогда
		ГоловныеСтроки = УчетРабочегоВремениКлиентСервер.ТабельГоловныеСтрокиСотрудников(Форма.Объект.ДанныеОВремени);
		
		Если НоваяВысотаСтроки > Форма.ВысотаСтроки Тогда
			Для Каждого ГоловнаяСтрокаСотрудника Из ГоловныеСтроки Цикл
				СтрокиПоСотруднику = УчетРабочегоВремениКлиентСервер.ТабельСтрокиПоСотруднику(Форма.Объект.ДанныеОВремени, ГоловнаяСтрокаСотрудника.НомерСтрокиСотрудник); 
				
				Если СтрокиПоСотруднику.Количество() < НоваяВысотаСтроки Тогда 
					Для НомерСтроки = Форма.ВысотаСтроки + 1 По НоваяВысотаСтроки Цикл
						УчетРабочегоВремениКлиентСервер.ТабельНоваяСтрокаПоСотруднику(Форма.Объект.ДанныеОВремени, ГоловнаяСтрокаСотрудника);
					КонецЦикла;	
				КонецЕсли;	
...
Процедура ТабельПоместитьОписаниеВидовВремениВДанныеФормы(Форма) Экспорт
	...
	Выборка = УчетРабочегоВремени.СформироватьЗапросПоВидамВремени().Выбрать();	
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(Выборка[БуквенныйКодИмяРеквизита])
			И Выборка.ВидВремени <> ВидВремениВыходные Тогда
			Продолжить;
		КонецЕсли;	
...
Процедура ТабельУдалитьСтрокиПоСотруднику(Форма, ИдентификаторУдаляемойСтроки) Экспорт
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		...
		УдаляемыеСтроки = УчетРабочегоВремениКлиентСервер.ТабельСтрокиПоСотруднику(Объект.ДанныеОВремени, ТекущаяСтрока.НомерСтрокиСотрудник);
...
Процедура ТабельЗаполнитьСтрокиПоСотруднику(Форма, ДанныеОВремениПоСотруднику, МассивСтрокПоСотруднику, КодыВидовВремени, ВысотаСтроки, ЗаполнятьТерриторииИУсловия = Ложь) Экспорт
	...
	Для Каждого СтрокаДанныхОВремени Из ДанныеОВремениПоСотруднику Цикл
		...
		Если НомерВидаВремениНаДату > МассивСтрокПоСотруднику.Количество() Тогда
			СтрокаТабеля = УчетРабочегоВремениКлиентСервер.ТабельНоваяСтрокаПоСотруднику(Объект.ДанныеОВремени, МассивСтрокПоСотруднику[0]);	
		...
		СтрокаТабеля["Время" + НомерДня + "Представление"] = УчетРабочегоВремениКлиентСервер.ТабельПредставлениеВремениПоВиду(БуквенноеОбозначение, СтрокаДанныхОВремени.Часы);  
...
Процедура ТабельЗаполнитьИтогиПоСотрудникамДокумента(Форма)
	...
	Для Каждого СтрокаДанныхОВремени Из Форма.Объект.ДанныеОВремени Цикл
		Если СтрокаДанныхОВремени.Сотрудник <> ТекущийСотрудник 
			...
			УчетРабочегоВремениКлиентСервер.ТабельЗаполнитьИтогиПоСотруднику(СтрокиПоСотруднику, ОбозначенияВидовВремени);	
