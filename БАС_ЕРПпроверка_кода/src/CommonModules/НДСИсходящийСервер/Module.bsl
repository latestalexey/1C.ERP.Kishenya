...
#Область Этап1 
	...
	Пока ВыборкаПоПревышениям.Следующий() Цикл
		...
		Если АналитикиИзменились(СтарыеАналитики, ВыборкаПоПревышениям) = Истина Тогда
			...
			СуммаПревышения = ВыборкаПоПревышениям.СуммаПревышения;
		...
		Для каждого Строка из НайденныеСтроки Цикл
			...
			Если СуммаПревышения < Строка.СуммаСНДС Тогда
...
#Область Этап4
	...
	Пока ВыборкаПоПревышениямНО.Следующий() Цикл
		...
		НайтиСтрокиНаСуммуПревышения(ВыборкаПоПревышениямНО.СуммаПревышения, ВыборкаПоПревышениямНО.ДокументПоставки, ТаблицаНоменклатурыДляЗаполнения, ТаблицаНоменклатурыНаСумму);
...
/////////////////////////////////////////Отдельной процедурой///////////////////////////////////////////////
			...
			ИначеЕсли ЗначениеЗаполнено(СтрокаСНалоговой.ОсновнаяНалоговая) Тогда
				...
				СтрокаНовойНалоговой.ВидОперацииОсновнойНалоговой = ?(СтрокаСНалоговой.ОсновнаяНалоговая.СпецРежимНалогообложения = 7,
																	  "07_" + СформироватьВидОперацииПоСтавке7(СтрокаСНалоговой.ОсновнаяНалоговая.ВидОперации),
			...
			Для каждого СтрокаТовара Из СтрокиПоВидуОперации Цикл
				...
				ИначеЕсли СтрокаТовара.СпособЗаполнения = Перечисления.СпособыЗаполненияНоменклатурыНалоговыхДокументов.УслугиССдержанием Тогда
					...
					//++ НЕ УТ
					Если ТипЗнч(СтрокаТовара.ДокументПоставки) = Тип("ДокументСсылка.ОтчетДавальцу") Тогда
						ТЧДляПоиска = СтрокаТовара.ДокументПоставки.Продукция;
					Иначе
					//-- НЕ УТ
			...
			Если СтрокаСНалоговой = Неопределено ИЛИ (ДатаЗапрета <> Неопределено) Тогда
				...
				НоваяСтрока.ВидОперацииОсновнойНалоговой = ?(НалоговыйОбъект.СпецРежимНалогообложения = 7,
															 "07_" + СформироватьВидОперацииПоСтавке7(НалоговыйОбъект.ВидОперации),
...
#Область Этап5 
	...
	Пока ВыборкаПоПревышениям.Следующий() Цикл
		...
		Если АналитикиИзменились(СтарыеАналитики, ВыборкаПоПревышениям) = Истина Тогда
			...
			Если СуммаПревышенияНО > 0 Тогда
				...
				Иначе
					...
					Если СтарыеАналитики.РазрезВидПоставки = Перечисления.ВидыПоставки.Розница Тогда
						...
						ВидОперацииДляЗаполнения = ПолучитьВидОперации(СтарыеАналитики.ОбъектРасчетов, СтарыеАналитики.РазрезВидПоставки);
					Иначе
						...
						ВидОперацииДляЗаполнения = ПолучитьВидОперации(НоменклатураДляАвансовыхНН.СтавкаНДС, СтарыеАналитики.РазрезВидПоставки);
					КонецЕсли;
			...
			Если (ТипЗнч(СтарыеАналитики.ОбъектРасчетов) = Тип("ДокументСсылка.ЗаказКлиента") ИЛИ ТипЗнч(СтарыеАналитики.ОбъектРасчетов) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента"))
				...
				Для каждого Строка из ТаблицаВидовОперацийННДляЗаказа Цикл
					Строка.ВидОперации = ПолучитьВидОперации(Строка.СтавкаНДС, СтарыеАналитики.РазрезВидПоставки)
				КонецЦикла;
				...
				Если СуммаАвансаПослеЗаказа > 0 Тогда
					...
					Если ТаблицаВидовОперацийННДляЗаказа.Найти(НоменклатураДляАвансовыхНН.СтавкаНДС, "СтавкаНДС") = Неопределено Тогда
						...
						СтрокаВидаОперации.ВидОперации = ПолучитьВидОперации(НоменклатураДляАвансовыхНН.СтавкаНДС, СтарыеАналитики.РазрезВидПоставки);
					КонецЕсли;
		...
		Иначе
			...
			ИначеЕсли ВыборкаПоПревышениям.ВидОперацииОсновнойНалоговой = "07_"+ СформироватьВидОперацииПоСтавке7(Перечисления.ВидыОперацийНалоговыхДокументов.РозницаКонрагентуОблагаемыеОперации) Тогда
				ПропускатьПоВидуОперации = Истина;
				ВидОперацииОсновнойНалоговой = "07_"+ СформироватьВидОперацииПоСтавке7(Перечисления.ВидыОперацийНалоговыхДокументов.ИтоговаяРозницаОблагаемыеОперации);
			ИначеЕсли ВыборкаПоПревышениям.ВидОперацииОсновнойНалоговой = "07_"+ СформироватьВидОперацииПоСтавке7(Перечисления.ВидыОперацийНалоговыхДокументов.РозницаКонрагентуОсвобожденныеОперации) Тогда
				ПропускатьПоВидуОперации = Истина;
				ВидОперацииОсновнойНалоговой = "07_"+ СформироватьВидОперацииПоСтавке7(Перечисления.ВидыОперацийНалоговыхДокументов.ИтоговаяРозницаОсвобожденныеОперации);
			ИначеЕсли НЕ ЗначениеЗаполнено(ВыборкаПоПревышениям.ВидОперацииОсновнойНалоговой) Тогда
				...
				ВидОперацииОсновнойНалоговой = ПолучитьВидОперации(НоменклатураДляАвансовыхНН.СтавкаНДС, ВыборкаПоПревышениям.РазрезВидПоставки)
			Иначе
				ПропускатьПоВидуОперации = Ложь;
...
/////////////////////////////////////////Отдельной процедурой///////////////////////////////////////////////
			...
			Иначе
				...
				Если СтарыеАналитики.РазрезВидПоставки = Перечисления.ВидыПоставки.Розница Тогда
					НоменклатураДляРозницы = СтарыеАналитики.АналитикаУчетаПоПартнерам.Организация.НоменклатураЗаполненияИтоговыхНалоговыхПоРознице;
					ВидОперацииОсновнойНалоговой = ПолучитьВидОперации(СтарыеАналитики.ОбъектРасчетов, СтарыеАналитики.РазрезВидПоставки);
				Иначе
					НоменклатураДляАвансовыхНН = ОпределитьНомеклатуруЗаполненияНалоговыхНаАванс(СтарыеАналитики.АналитикаУчетаПоПартнерам.Организация, СтарыеАналитики.АналитикаУчетаПоПартнерам.Договор, КешНоменклатурыДляАвансовыхНН);
					ВидОперацииОсновнойНалоговой = ПолучитьВидОперации(НоменклатураДляАвансовыхНН.СтавкаНДС, СтарыеАналитики.РазрезВидПоставки);
				КонецЕсли;
			...
			Если СуммаПревышенияНО > 0 Тогда
				...
				Если СтарыеАналитики.РазрезВидПоставки = Перечисления.ВидыПоставки.Розница Тогда
					НоменклатураДляРозницы = СтарыеАналитики.АналитикаУчетаПоПартнерам.Организация.НоменклатураЗаполненияИтоговыхНалоговыхПоРознице;
					ВидОперации = ПолучитьВидОперации(СтарыеАналитики.ОбъектРасчетов, СтарыеАналитики.РазрезВидПоставки);
				Иначе
					НоменклатураДляАвансовыхНН = ОпределитьНомеклатуруЗаполненияНалоговыхНаАванс(НалоговыйОбъект.Организация, НалоговыйОбъект.Договор, КешНоменклатурыДляАвансовыхНН);
					ВидОперации = ПолучитьВидОперации(НоменклатураДляАвансовыхНН.СтавкаНДС, СтарыеАналитики.РазрезВидПоставки);
				КонецЕсли;
...
#КонецОбласти // Этап5
	...
	Для каждого СтрокаНалоговой Из ТаблицаСформированыхНалоговых Цикл
		СтрокаНалоговой.НалоговыйДокументДляКорректировки.ЗаполнитьРеквизитыПослеАвтоформирования();
		СтрокаНалоговой.НалоговыйДокументДляКорректировки.Записать(РежимЗаписиДокумента.Проведение);
	КонецЦикла;
...
Процедура НайтиИСторнироватьСтроки(СуммаПревышенияНО, НалоговыйОбъект, ТаблицаНоменклатурыКОчистке, ОбъектРасчетов, ОсновнаяНалоговая, КоэффициентНаправлениеДвижения, ЭтоПереоценка, ВидОперацииОсновнойНалоговой, День);
	...
	Если ВидОперацииОсновнойНалоговой = Перечисления.ВидыОперацийНалоговыхДокументов.ИтоговаяРозницаОблагаемыеОперации
		 ИЛИ ВидОперацииОсновнойНалоговой = "07_"+ СформироватьВидОперацииПоСтавке7(Перечисления.ВидыОперацийНалоговыхДокументов.ИтоговаяРозницаОблагаемыеОперации) Тогда
	...
	ИначеЕсли ВидОперацииОсновнойНалоговой = Перечисления.ВидыОперацийНалоговыхДокументов.ИтоговаяРозницаОсвобожденныеОперации
			  ИЛИ ВидОперацииОсновнойНалоговой = "07_"+ СформироватьВидОперацииПоСтавке7(Перечисления.ВидыОперацийНалоговыхДокументов.ИтоговаяРозницаОсвобожденныеОперации) Тогда
...
Процедура НайтиСтрокиНаСуммуПревышения(СуммаПревышения, ДокументПоставки, ТаблицаНоменклатурыДляЗаполнения, ТаблицаНоменклатурыНаСумму)
	...
	Для каждого Строка Из СтрокиПоДокументуОтгрузки Цикл
		...
		Если СуммаПревышенияНО > Строка.СуммаВзаиморасчетовОстаток Тогда
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
			НоваяСтрока.ВидОперации = ПолучитьВидОперации(НоваяСтрока.СтавкаНДС, НоваяСтрока.ВидПоставки);
...
Функция ПолучитьВидОперации(СтавкаНДС, ВидПоставки)
	
	Если ВидПоставки = Перечисления.ВидыПоставки.Розница Тогда
		
		Если СтавкаНДС = Перечисления.СтавкиНДС.НДС20 ИЛИ СтавкаНДС = Перечисления.СтавкиНДС.НДС0 Тогда
...
Функция СформироватьВидОперацииПоСтавке7(ВидОперации)
	...
	Если ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.ИтоговаяРозницаОблагаемыеОперации Тогда
		НовыйВидОперации = "07_" + "ИтоговаяРозницаОблагаемыеОперации";
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.ИтоговаяРозницаОсвобожденныеОперации Тогда
		НовыйВидОперации = "07_" + "ИтоговаяРозницаОсвобожденныеОперации";
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.НеНДСОперации Тогда
		НовыйВидОперации = "07_" + "НеНДСОперации";
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.ОблагаемыеОперации Тогда
		НовыйВидОперации = "07_" + "ОблагаемыеОперации";
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.ОсвобожденныеОперации Тогда
		НовыйВидОперации = "07_" + "ОсвобожденныеОперации";
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.ПродажаНижеОбычнойЦены Тогда
		НовыйВидОперации = "07_" + "ПродажаНижеОбычнойЦены";
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.РаботыОтНерезидента Тогда
		НовыйВидОперации = "07_" + "РаботыОтНерезидента";
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.РозницаКонрагентуОблагаемыеОперации Тогда
		НовыйВидОперации = "07_" + "РозницаКонрагентуОблагаемыеОперации";
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.РозницаКонрагентуОсвобожденныеОперации Тогда
		НовыйВидОперации = "07_" + "РозницаКонрагентуОсвобожденныеОперации";
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.УсловнаяПродажа Тогда
		НовыйВидОперации = "07_" + "УсловнаяПродажа";
	КонецЕсли;
...
Процедура ВыполнитьПроведениеДокументовПоВозникшемуНДС(НачальныеГраницы) Экспорт
	...
	|	ВЫБОР
	|		КОГДА СистемыНалогообложенияОрганизаций.УчетнаяПолитика.СистемаНалогообложения ЕСТЬ NULL 
	|				ИЛИ СистемыНалогообложенияОрганизаций.УчетнаяПолитика.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Неплательщик)
	|				ИЛИ СистемыНалогообложенияОрганизаций.УчетнаяПолитика.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.ЕдиныйНалог)
	|				ИЛИ СистемыНалогообложенияОрганизаций.УчетнаяПолитика.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.НалогНаПрибыль)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоНеплательщик
	...
	//++ НЕ УТ
	МассивДопустимыхТиповРегистратора.Добавить(Тип("ДокументСсылка.ОтчетДавальцу"));
	//-- НЕ УТ
...
Функция ПолучитьТекстЗапросаАнализаУсловныхПродаж() Экспорт
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	УсловныеПродажи.ВидУсловнойПродажи,
	|	УсловныеПродажи.ПредполагаемыйВидУсловнойПродажи,
	|	ВЫБОР
	|		КОГДА НЕ УсловныеПродажи.ПредполагаемоеКоличество = УсловныеПродажи.Количество
	|			ТОГДА 4
	|		КОГДА НЕ УсловныеПродажи.ПредполагаемыйНДСРегл = УсловныеПродажи.НДСРегл
	|			ТОГДА 3
	|		КОГДА НЕ УсловныеПродажи.ПредполагаемаяСтоимостьРегл = УсловныеПродажи.СтоимостьРегл
	|				ИЛИ НЕ УсловныеПродажи.НДСРегл = УсловныеПродажи.НДСРеглРучныеКорректировки
	|			ТОГДА 2
	|		ИНАЧЕ 1
	...
	|	УсловныеПродажи.ПредполагаемоеКоличество,
	|	УсловныеПродажи.Количество,
	|	УсловныеПродажи.ПредполагаемаяСтоимостьРегл,
	|	УсловныеПродажи.СтоимостьРегл,
	|	УсловныеПродажи.ПредполагаемыйНДСРегл,
	|	УсловныеПродажи.НДСРегл,
	|	УсловныеПродажи.НДСРеглРучныеКорректировки
	|ПОМЕСТИТЬ АнализУсловныхПродаж
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВЫБОР
	|			КОГДА НДСУсловныеПродажи.Организация ЕСТЬ NULL 
	|				ТОГДА НДСПредполагаемыеУсловныеПродажи.Организация
	|			ИНАЧЕ НДСУсловныеПродажи.Организация
	|		КОНЕЦ КАК Организация,
	|		ВЫБОР
	|			КОГДА НДСУсловныеПродажи.Номенклатура ЕСТЬ NULL 
	|				ТОГДА НДСПредполагаемыеУсловныеПродажи.Номенклатура
	|			ИНАЧЕ НДСУсловныеПродажи.Номенклатура
	|		КОНЕЦ КАК Номенклатура,
	|		ВЫБОР
	|			КОГДА НДСУсловныеПродажи.Характеристика ЕСТЬ NULL 
	|				ТОГДА НДСПредполагаемыеУсловныеПродажи.Характеристика
	|			ИНАЧЕ НДСУсловныеПродажи.Характеристика
	|		КОНЕЦ КАК Характеристика,
	|		ВЫБОР
	|			КОГДА НДСУсловныеПродажи.СтавкаНДС ЕСТЬ NULL 
	|				ТОГДА НДСПредполагаемыеУсловныеПродажи.СтавкаНДС
	|			ИНАЧЕ НДСУсловныеПродажи.СтавкаНДС
	|		КОНЕЦ КАК СтавкаНДС,
	|		ВЫБОР
	|			КОГДА НДСУсловныеПродажи.НалоговоеНазначение ЕСТЬ NULL 
	|				ТОГДА НДСПредполагаемыеУсловныеПродажи.НалоговоеНазначение
	|			ИНАЧЕ НДСУсловныеПродажи.НалоговоеНазначение
	|		КОНЕЦ КАК НалоговоеНазначение,
	|		ВЫБОР
	|			КОГДА НДСУсловныеПродажи.НалоговоеНазначениеПоФакту ЕСТЬ NULL 
	|				ТОГДА НДСПредполагаемыеУсловныеПродажи.НалоговоеНазначениеПоФакту
	|			ИНАЧЕ НДСУсловныеПродажи.НалоговоеНазначениеПоФакту
	|		КОНЕЦ КАК НалоговоеНазначениеПоФакту,
	|		ВЫБОР
	|			КОГДА НДСУсловныеПродажи.ВидУсловнойПродажи ЕСТЬ NULL 
	|				ТОГДА ВЫБОР
	|						КОГДА НДСПредполагаемыеУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяУсловнаяПродажа)
	|							ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.УсловнаяПродажа)
	|						КОГДА НДСПредполагаемыеУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемоеВосстановлениеНДС)
	|							ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ВосстановлениеНДС)
	|						КОГДА НДСПредполагаемыеУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяСводнаяУсловнаяПродажа)
	|							ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.СводнаяУсловнаяПродажа)
	|						КОГДА НДСПредполагаемыеУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяУсловнаяПродажаНеоборотныхАктивов)
	|							ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.УсловнаяПродажаНеоборотныхАктивов)
	|						ИНАЧЕ НЕОПРЕДЕЛЕНО
	|					КОНЕЦ
	|			ИНАЧЕ НДСУсловныеПродажи.ВидУсловнойПродажи
	|		КОНЕЦ КАК ВидУсловнойПродажи,
	|		ВЫБОР
	|			КОГДА НДСПредполагаемыеУсловныеПродажи.ВидУсловнойПродажи ЕСТЬ NULL 
	|				ТОГДА ВЫБОР
	|						КОГДА НДСУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.УсловнаяПродажа)
	|							ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяУсловнаяПродажа)
	|						КОГДА НДСУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ВосстановлениеНДС)
	|							ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемоеВосстановлениеНДС)
	|						КОГДА НДСУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.СводнаяУсловнаяПродажа)
	|							ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяСводнаяУсловнаяПродажа)
	|						КОГДА НДСУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.УсловнаяПродажаНеоборотныхАктивов)
	|							ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяУсловнаяПродажаНеоборотныхАктивов)
	|						ИНАЧЕ НЕОПРЕДЕЛЕНО
	|					КОНЕЦ
	|			ИНАЧЕ НДСПредполагаемыеУсловныеПродажи.ВидУсловнойПродажи
	|		КОНЕЦ КАК ПредполагаемыйВидУсловнойПродажи,
	|		ЕСТЬNULL(НДСУсловныеПродажи.КоличествоОборот, 0) КАК Количество,
	|		ЕСТЬNULL(НДСПредполагаемыеУсловныеПродажи.КоличествоОборот, 0) КАК ПредполагаемоеКоличество,
	|		ЕСТЬNULL(НДСУсловныеПродажи.СтоимостьРеглОборот, 0) КАК СтоимостьРегл,
	|		ЕСТЬNULL(НДСПредполагаемыеУсловныеПродажи.СтоимостьРеглОборот, 0) КАК ПредполагаемаяСтоимостьРегл,
	|		ЕСТЬNULL(НДСУсловныеПродажи.НДСРеглОборот, 0) КАК НДСРегл,
	|		ЕСТЬNULL(НДСПредполагаемыеУсловныеПродажи.НДСРеглОборот, 0) КАК ПредполагаемыйНДСРегл,
	|		ЕСТЬNULL(НДСУсловныеПродажи.НДСРеглРучныеКорректировкиОборот, 0) КАК НДСРеглРучныеКорректировки
	|	ИЗ
	|		РегистрНакопления.НДСУсловныеПродажи.Обороты(
	|				&ПериодНачало,
	|				&ПериодОкончание,
	|				Месяц,
	|				(Организация В (&Организация) ИЛИ &ПоВсемОрганизациям)
	|				И (ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяУсловнаяПродажа)
	|					ИЛИ ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемоеВосстановлениеНДС)
	|					ИЛИ ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяСводнаяУсловнаяПродажа)
	|					ИЛИ ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяУсловнаяПродажаНеоборотныхАктивов))) КАК НДСПредполагаемыеУсловныеПродажи
	|			ПОЛНОЕ СОЕДИНЕНИЕ РегистрНакопления.НДСУсловныеПродажи.Обороты(
	|					&ПериодНачало,
	|					&ПериодОкончание,
	|					Месяц,
	|					(Организация В (&Организация) ИЛИ &ПоВсемОрганизациям)
	|					И (ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.УсловнаяПродажа)
	|						ИЛИ ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ВосстановлениеНДС)
	|						ИЛИ ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.СводнаяУсловнаяПродажа)
	|						ИЛИ ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.УсловнаяПродажаНеоборотныхАктивов))) КАК НДСУсловныеПродажи
	|			ПО НДСПредполагаемыеУсловныеПродажи.Организация = НДСУсловныеПродажи.Организация
	|				И НДСПредполагаемыеУсловныеПродажи.Номенклатура = НДСУсловныеПродажи.Номенклатура
	|				И НДСПредполагаемыеУсловныеПродажи.Характеристика = НДСУсловныеПродажи.Характеристика
	|				И НДСПредполагаемыеУсловныеПродажи.СтавкаНДС = НДСУсловныеПродажи.СтавкаНДС
	|				И НДСПредполагаемыеУсловныеПродажи.НалоговоеНазначение = НДСУсловныеПродажи.НалоговоеНазначение
	|				И НДСПредполагаемыеУсловныеПродажи.НалоговоеНазначениеПоФакту = НДСУсловныеПродажи.НалоговоеНазначениеПоФакту
	|				И (НДСПредполагаемыеУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяУсловнаяПродажа)
	|						И НДСУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.УсловнаяПродажа)
	|					ИЛИ НДСПредполагаемыеУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемоеВосстановлениеНДС)
	|						И НДСУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ВосстановлениеНДС)
	|					ИЛИ НДСПредполагаемыеУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяСводнаяУсловнаяПродажа)
	|						И НДСУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.СводнаяУсловнаяПродажа)
	|					ИЛИ НДСПредполагаемыеУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.ПредполагаемаяУсловнаяПродажаНеоборотныхАктивов)
	|						И НДСУсловныеПродажи.ВидУсловнойПродажи = ЗНАЧЕНИЕ(Перечисление.ВидУсловнойПродажи.УсловнаяПродажаНеоборотныхАктивов))) КАК УсловныеПродажи";
		...
		Пока ВыборкаПоСпецРежиму.Следующий() Цикл
			...
			Пока ВыборкаНалоговыхНакладных.Следующий() Цикл
				...
				Пока ВыборкаСтрокНН.Следующий() Цикл
					...
					СуммаБазыНДС = СуммаКорректировкиНДССтроки / НДСОбщегоНазначенияКлиентСервер.ПолучитьСтавкуНДСЧислом(ВыборкаСтрокНН.СтавкаНДС);
			...
			Если РаспределеннаяСумма <> СуммаКРаспределению Тогда
				...
				Если НалоговыйОбъект <> Неопределено Тогда
					...
					БазаНДСНераспределенная = СуммаНДСНераспределенная / НДСОбщегоНазначенияКлиентСервер.ПолучитьСтавкуНДСЧислом(ПоследняяСтрока.СтавкаНДС);
...
Функция ТекстЗапросаНалоговыхДокументов(НалоговаяНакладная = Истина, ОтборПоДокументам = Истина)
	...
	Если НалоговаяНакладная Тогда
		...
		|	И (НалоговыйДокумент.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20)
		|			ИЛИ НалоговыйДокумент.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7))
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтрокиНН
		|ИТОГИ
		...
		Тогда
			...
			Если ВыдаватьСообщенение = Истина Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Форма налоговой накладной, утвержденная 14.01.2014 приказом Миндоходов № 10, не предусматривает наличие ставки НДС  7%. 
                               ...
                               |uk = ''Форма податкової накладню, що затверждена 14.01.2014 наказом Міндоходів № 10, не передбачає наявності ставки ПДВ 7%.
                               ...
                               |;uk='Форма податкової накладної, затверджена 14.01.2014 наказом Міндоходів № 10, не передбачає наявність ставки ПДВ 7%. 
                               |В даній податковій накладній присутня номенклатура, яка оподатковується як за ставкою 7% так і за іншими ставками.
                               |Згідно рекомендації Міндоходів, викладеної в листі від 04.04.2014 № 7860/7/99-99-19-04-02-17, на номенклатуру, що оподатковується ПДВ за ставкою 7%, необхідно виписувати окрему податкову накладну!'';
                               |uk = ''Форма податкової накладної, що затверждена 14.01.2014 наказом Міндоходів № 10, що не передбачає наявності ставки ПДВ 7%.
                               |В податковій накладній наявна номенклатура, що оподатковується ПДВ як за ставкою 7% так і за іншими ставками.
                               |Згідно рекомендації Міндоходів, що викладена в листі від 04.04.2014 № 7860/7/99-99-19-04-02-17, на номенклатуру, що оподатковується ПДВ за ставкою 7%, необхідно складати окрему податкову накладну!'"));
