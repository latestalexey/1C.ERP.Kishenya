...
Процедура ОткрытьФормуДокументаИзменившегоПозицию(СсылкаНаДокумент, ПозицияШтатногоРасписания) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаНаДокумент) Тогда
		...
		ОткрытьФорму(ПолноеИмяМетаданных + ".ФормаОбъекта", ПараметрыОткрытия);
...
Процедура РедактированиеСостоянияШРПриИзмененииНачислений(Форма, НачисленияПутьКДанным, ИмяТаблицыПозиции, ИмяТаблицыНачисления, УдалениеНачисления = Ложь)
...
//                
//
Процедура РедактированиеСоставаНачисленийПриВводеНового(Форма, ОписаниеТаблицыНачислений, НовоеНачисление)
	...
	УправлениеШтатнымРасписаниемКлиентСервер.УстановитьКомментарииДействийСНачислением(
		НовоеНачисление, 
		ЗарплатаКадрыРасширенныйКлиентСервер.МаксимальноеКоличествоПоказателейПоОписаниюТаблицы(Форма, ОписаниеТаблицыНачислений), 
		Форма.ПолучитьФункциональнуюОпциюФормы("ИспользоватьВилкуСтавокВШтатномРасписании"));	
...
Процедура УдалитьВыделенныеСтрокиПозицийИзДокумента(Форма)
	...
	Для каждого ИдентификаторСтроки Из Форма.Элементы.Позиции.ВыделенныеСтроки Цикл
		Форма.Объект.Позиции.Удалить(Форма.Объект.Позиции.НайтиПоИдентификатору(ИдентификаторСтроки));
	КонецЦикла;
...
Процедура ЗакрытьПозицию(Форма, РазрешеноУдаление = Ложь, ОповещениеЗавершения = Неопределено) Экспорт
	...
	Если ЕстьНовыеПозиции Тогда
		...
		Иначе
			...
			ПоказатьПредупреждение(, НСтр("ru='Среди выделенных строк, есть еще не созданные позиции штатного расписания, 
                ...
                |;uk='Серед виділених рядків, є ще не створені позиції штатного розкладу, 
                |якщо ви не хочете створювати ці позиції, видалите відповідні рядки.'"));
	...
	Если КоличествоПозиций = 1 Тогда
		...
		Если Действие = ПредопределенноеЗначение("Перечисление.ДействияСПозициямиШтатногоРасписания.ПустаяСсылка") Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Хотите закрыть действующую позицию ""%1""?';uk='Хочете закрити діючу позицію ""%1""?'"), Элементы.Позиции.ТекущиеДанные.Позиция);
		Иначе
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Хотите отменить закрытие позиции ""%1""?';uk='Хочете скасувати закриття позиції ""%1""?'"), Элементы.Позиции.ТекущиеДанные.Позиция);
		КонецЕсли;
	ИначеЕсли КоличествоПозиций > 1 Тогда  
		Действие = Форма.Объект.Позиции.НайтиПоИдентификатору(Элементы.Позиции.ВыделенныеСтроки[0]).Действие;
		Если Действие = ПредопределенноеЗначение("Перечисление.ДействияСПозициямиШтатногоРасписания.ПустаяСсылка") Тогда
			Если КоличествоПозиций <= 4 Тогда
				СтрокаПодстановки = НСтр("ru='действующие позиции';uk='діючі позиції'");
			Иначе
				...
				Если ПоследниеДвеЦифры >= 5 И ПоследниеДвеЦифры <= 20 Тогда
					СтрокаПодстановки = НСтр("ru='действующих позиций';uk='діючих позицій'");
				Иначе
					...
					Если ПоследняяЦифра = 1 Тогда 
						СтрокаПодстановки = НСтр("ru='действующую позицию';uk='діючу позицію'");
				...
				КонецЕсли;
			КонецЕсли;	
			ТекстВопроса =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Хотите закрыть %1 %2 штатного расписания?';uk='Хочете закрити %1 %2 штатного розкладу?'"), Формат(КоличествоПозиций, "ЧГ=0"), СтрокаПодстановки);
		Иначе
			ТекстВопроса =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Хотите отменить закрытие %1 позиций штатного расписания?';uk='Хочете скасувати закриття %1 позицій штатного розкладу?'"), Формат(КоличествоПозиций, "ЧГ=0"));
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(, НСтр("ru='Не выбраны закрываемые позиции.';uk='Не вибрані закриваються позиції.'"));
		Возврат;
	КонецЕсли;
	...
	Если РазрешеноУдаление Тогда
		...
		Если Действие = ПредопределенноеЗначение("Перечисление.ДействияСПозициямиШтатногоРасписания.ПустаяСсылка") Тогда
			...
			Если КоличествоПозиций > 1 Тогда
				КнопкиДиалога.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Закрыть позиции';uk='Закрити позиції'"));
			Иначе
				КнопкиДиалога.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Закрыть позицию';uk='Закрити позицію'"));
			КонецЕсли;
			
		Иначе
			КнопкиДиалога.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Отменить закрытие';uk='Скасувати закриття'"));
		КонецЕсли;
		
		Если КоличествоПозиций > 1 Тогда
			КнопкиДиалога.Добавить("УдалитьСтроки", НСтр("ru='Удалить строки';uk='Видалити рядки'"));
		Иначе
			КнопкиДиалога.Добавить("УдалитьСтроки", НСтр("ru='Удалить строку';uk='Вилучити рядок'"));
		КонецЕсли;
		
		КнопкиДиалога.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Отмена';uk='Відмінити'"));
		
		Оповещение = Новый ОписаниеОповещения("ЗакрытьПозициюЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВопрос(Оповещение, ТекстВопроса, КнопкиДиалога, , "УдалитьСтроки", НСтр("ru='Закрытие позиций';uk='Закриття позицій'"));
...
Процедура ПозицииПриАктивизацииСтроки(Форма, РазрешеноУдаление = Ложь) Экспорт
...
Процедура ПозицииПриНачалеРедактирования(Форма, ИдентификаторСтроки, НоваяСтрока, Копирование) Экспорт
	
	Элементы = Форма.Элементы;
	ТекущиеДанные = Форма.Объект.Позиции.НайтиПоИдентификатору(ИдентификаторСтроки);
	...
	Если НоваяСтрока Тогда
		...
		ТекущиеДанные.Комментарий = УправлениеШтатнымРасписаниемКлиентСервер.ПолучитьКомментарийКДействиюСПозициейШР(
			ТекущиеДанные, Форма.ДоступноЧтениеНачисленийШтатногоРасписания);
...
Процедура НачисленияПриАктивизацииСтроки(Форма) Экспорт
	
	ОписаниеКоманднойПанелиНачислений = Форма.ОписаниеКоманднойПанелиНачислений();
...
Процедура НачисленияПриНачалеРедактирования(Форма, НоваяСтрока, Копирование) Экспорт
	...
	ЗарплатаКадрыРасширенныйКлиент.РедактированиеСоставаНачисленийПриНачалеРедактирования(Форма, "Начисления", 0);
...
Процедура НачисленияПриОкончанииРедактирования(Форма, Элемент, НоваяСтрока, ОтменаРедактирования) Экспорт
	
	Если НЕ ОтменаРедактирования Тогда
		Если НоваяСтрока Тогда
			ОписаниеТаблицыНачислений = Форма.ОписаниеТаблицыНачисленийНаКлиенте();
			РедактированиеСоставаНачисленийПриВводеНового(Форма, ОписаниеТаблицыНачислений, Элемент.ТекущиеДанные);
		КонецЕсли;
	КонецЕсли; 
...
Процедура НачисленияПередУдалением(Форма, Отказ) Экспорт
	
	Отказ = Истина;
	
	ОписаниеКоманднойПанелиНачислений = Форма.ОписаниеКоманднойПанелиНачислений();
	
	ЗарплатаКадрыРасширенныйКлиент.РедактированиеСоставаНачисленийОтменитьНачисление(Форма, "Начисления", 0, ОписаниеКоманднойПанелиНачислений);
