...
// Заголовки писем с установленым свойством ПисьмоЗагружено = Истина не будут получены с почтового сервера.
//
// Параметры:
	...
	Пока ЕстьДанные Цикл
	    ...
	    Пока ЕстьДанные
	    	...
	    	Если ЗначениеЗаполнено(Выборка.ПрисоединенныйФайл) Тогда
	    		Попытка
		    		ДанныеФайла = ПрисоединенныеФайлы.ПолучитьДанныеФайла(Выборка.ПрисоединенныйФайл, Неопределено, Ложь, Ложь);
		    		ДвоичныеДанныеФайла = ПрисоединенныеФайлы.ПолучитьДвоичныеДанныеФайла(Выборка.ПрисоединенныйФайл);
			    ...
			    Исключение
			    	...
			    	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			    		НСтр("ru='Ошибка при загрузке вложения ""%1"" в ""%2"": %3';uk='Помилка при завантаженні вкладення ""%1"" в ""%2"": %3'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			    		Выборка.ПрисоединенныйФайл,
			    		Выборка.Ссылка,
			    		ПодробноеПредставлениеОшибки(ИнфоОбОшибке));
			    	
			    	ЗаписьЖурналаРегистрации(
			    		"СверкаДанныхУчетаНДС.ЗагрузкаИзПочты",
