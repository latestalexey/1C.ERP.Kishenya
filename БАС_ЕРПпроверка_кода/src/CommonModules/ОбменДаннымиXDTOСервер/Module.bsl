...
Функция ПКОПоИмени(КомпонентыОбмена, Имя) Экспорт
	...
	Если ПравилоКонвертации = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найдено ПКО с именем %1';uk='Не знайдено ПКО з ім''ям %1'"), Имя);
...
Процедура ПроизвестиВыгрузкуДанных(КомпонентыОбмена) Экспорт
	...
	Исключение
		ТекстОшибки = Символы.ПС + СтрШаблон(НСтр("ru='Событие: %1.
                ...
                |%2.'
                |;uk='Подія: %1.
                |Обробник: ПередКонвертацией.
                |
                |Помилка виконання обробника.
                |%2.'"),
...
Функция ОбъектXDTOИзДанныхXDTO(
	...
	Для Каждого Свойство Из ТипXDTO.Свойства Цикл
		...
		Иначе
			ТекстОшибки = СтрШаблон(НСтр("ru='Неизвестный тип свойства <%1>. Тип объекта: %2';uk='Невідомий тип властивості <%1>. Тип об''єкта: %2'"), Свойство.Имя, Строка(ТипXDTO));
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(НСтр("ru='Ошибка формирования объекта XDTO: Тип свойства <%1>. Имя свойства: <%2>.';uk='Помилка формування об''єкта XDTO: Тип властивості <%1>. Ім''я властивості: <%2>.'"), ТипСвойства, Свойство.Имя)
	...
	Если ПравилоКонвертации.ЭтоКонстанта Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |Тип источника: %1
                |Тип приемника: %2'
                |;uk='Помилка XML-схеми. Для приймача повинна бути задана одна властивість.
                |Тип джерела: %1
                |Тип приймача: %2'"),
...
Функция СтруктураОбъектаXDTOВДанныеИБ(КомпонентыОбмена, ДанныеXDTO, ПравилоКонвертации, Действие = "КонвертироватьИЗаписать") Экспорт
	...
	Если ДанныеИБ = Неопределено Тогда
		Если ПравилоКонвертации.ЭтоРегистрСведений Тогда
	...
	Если ЗаписыватьОбъектВИБ Тогда
		...
		КонецЕсли;
		Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена И НЕ ПравилоКонвертации.ЭтоРегистрСведений Тогда
			...
			Иначе
				...
				Для Каждого ТекущееПКО Из ИспользованиеПКО Цикл
					...
					Если ТекущееПКО.Значение Тогда
						...
						СтрокаТаблицы.ПоляТаблицы = ПравилоКонвертации.РеквизитыШапкиПолученныхДанныхСтрокой;
...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Если ТранзакцияАктивна() Тогда
			ВызватьИсключение НСтр("ru='Блокировка на обмен данными не может быть установлена в активной транзакции.';uk='Блокування на обмін даними не може бути встановлене в активній транзакції.'");
		КонецЕсли;
...
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена) Экспорт
	...
	Пока НомерИтерации = 0 Цикл
		...
		Исключение
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Ошибка при загрузке данных: %1';uk='Помилка при завантаженні даних: %1'");
		...
		Если (ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента
			...
			Если СообщениеОтНеобновленнойНастройки(ЧтениеXML) Тогда
				СтрокаСообщенияОбОшибке = НСтр("ru='Получение данных от источника, в котором не выполнено
                    ...
                    |;uk='Отримання даних від джерела, в якому не виконано
                    |оновлення настройки синхронізації даних. Необхідно:'")
					+ Символы.ПС + НСтр("ru='1) Выполнить повторную синхронизацию данных через некоторое время.';uk='1) Виконати повторну синхронізацію даних через деякий час.'")
                    ...
                    |;uk='2) Виконати синхронізацію даних на стороні джерела, після цього 
                    |повторно виконати синхронізацію даних в цій інформаційній базі.'")
					+ Символы.ПС + НСтр("ru='(1 - для вида транспорта Через интернет, 2 - для вида транспорта Другое)';uk='(1 - для виду транспорту Через інтернет, 2 - для виду транспорту Інше)'");
				ЗаписатьВПротоколВыполнения(КомпонентыОбмена, СтрокаСообщенияОбОшибке);
			Иначе
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			Если МенеджерПланаОбмена.ФорматОбмена() <> ФорматОбмена.БазовыйФормат Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Формат сообщения обмена <%1> не соответствует формату плана обмена <%2>.';uk='Формат повідомлення обміну <%1> не відповідає формату плану обміну <%2>.'"),
			...
			Если ВерсииФорматаОбменаМассив(КомпонентыОбмена.УзелКорреспондента).Найти(ФорматОбмена.Версия) = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Версия формата сообщения обмена <%1> не поддерживается.';uk='Версія формату повідомлення обміну <%1> не підтримується.'"),
					ФорматОбмена.Версия);
			КонецЕсли;
			...
			Если МаксимальнаяОбщаяВерсия <> НомерВерсииКорреспондента Тогда
				...
				ЗаписатьВПротоколВыполнения(КомпонентыОбмена, 
				                            НСтр("ru='Изменен номер версии формата обмена.';uk='Змінено номер версії формату обміну.'"),,Ложь,,, Истина);
			КонецЕсли;
...
Процедура ЗаписатьОбъектВИБ(КомпонентыОбмена, Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Попытка изменение неразделенных данных (%1) в разделенном режиме.';uk='Спроба зміни нерозділених даних (%1) у розділеному режимі.'");
...
Функция МенеджерОбменаВерсииФормата(Знач УзелИнформационнойБазы, Знач ВерсияФормата) Экспорт
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не определен Менеджер конвертации для версии формата обмена <%1>.';uk='Не визначено Менеджер конвертації для версії формату обміну <%1>.'"),
			ВерсияФормата);
	КонецЕсли;
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2)
	...
	Если Версия1.Количество() < 2 ИЛИ Версия1.Количество() > 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неправильный формат параметра СтрокаВерсии1: %1';uk='Неправильний формат параметра СтрокаВерсии1: %1'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, ".");
	Если Версия2.Количество() < 2 ИЛИ Версия2.Количество() > 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неправильный формат параметра СтрокаВерсии2: %1';uk='Неправильний формат параметра СтрокаВерсии2: %1'"), СтрокаВерсии2);
	КонецЕсли;
...
Функция ТаблицаПравилКонвертации(XMLСхема, МенеджерОбмена , НаправлениеОбмена, ПравилаОбработкиДанных)
	...
	ПравилаКонвертации.Колонки.Добавить("ЭтоРегистрСведений");
	...
	Для НомерИтерации = 1 По КоличествоСтрок Цикл
		...
		Если НаправлениеОбмена = "Получение" Тогда
			...
			ПравилоКонвертации.ТипПолученныхДанныхСтрокой = ИмяТипаДанныхПоОбъектуМетаданных(ПравилоКонвертации.ОбъектДанных, Неопределено);
		...
		Если ПравилоКонвертации.ОбъектДанных <> Неопределено Тогда
			...
			ПравилоКонвертации.ТипДанных = Тип(ИмяТипаДанныхПоОбъектуМетаданных(ПравилоКонвертации.ОбъектДанных, ИмяБазовогоТипа));
		...
		ПравилоКонвертации.ЭтоРегистрСведений        = (ИмяБазовогоТипа = ОбщегоНазначения.ИмяТипаРегистрыСведений());
...
Функция ТаблицаПравилКонвертацииПредопределенныхДанных(XMLСхема, МенеджерОбмена , НаправлениеОбмена)
	...
	Для Каждого ПравилоКонвертации Из ПравилаКонвертации Цикл
		...
		ПравилоКонвертации.ТипДанных = Тип(ИмяТипаДанныхПоОбъектуМетаданных(ПравилоКонвертации.ТипДанных, Неопределено));
	...
	Если ТипXDTO = Неопределено Тогда
		...
		ИначеЕсли НайденныеПравила.Количество() > 1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Помилка правил конвертації напередвизначених даних.
                |Задано більше одного правила конвертації для типу джерела <%1>.'"),
				Строка(Тип));
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Помилка правил конвертації напередвизначених даних.
            |Правило конвертації не визначено для типу джерела <%1>.'"),
			Строка(Тип));
	...
	Иначе
		...
		ИначеЕсли НайденныеПравила.Количество() > 1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Помилка правил конвертації напередвизначених даних.
                |Задано більше одного правила конвертації для типу джерела <%1> і типу приймача <%2>.'"),
				Строка(Тип),
				Строка(ТипXDTO));
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Помилка правил конвертації напередвизначених даних.
            |Правило конвертації не визначено для типу джерела <%1> і типу приймача <%2>.'"),
			Строка(Тип),
			Строка(ТипXDTO));
...
Функция ЗначениеПеречисленияXDTO(Знач КонвертацииЗначений, Знач Значение)
	...
	Если ЗначениеXDTO = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Не знайдено правило конвертації для значення напередвизначених даних.
            |Тип значення джерела: <%1>
            |Значення джерела: <%2>'"),
			ТипЗнч(Значение),
			Строка(Значение));
	КонецЕсли;
...
Процедура КонвертацияСвойстваXDTOВЭлементСтруктуры(Источник, Свойство, Приемник, ИмяДляСвойстваСоставногоТипа = "")
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтрШаблон(НСтр("ru='Ошибка чтения объекта XDTO, имя свойства: <%1>.';uk='Помилка читання об''єкта XDTO, ім''я властивості: <%1>.'"), ИмяСвойства)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Процедура КонвертацияСвойствСтруктурыОбъектаXDTO(
	...
	Исключение
		ТекстОшибки = Символы.ПС + СтрШаблон(НСтр("ru='Событие: %1.
                ...
                |Об''єкт: %2.
                |
                |Помилка конвертації властивостей.
                |%3.'"),
			КомпонентыОбмена.НаправлениеОбмена,
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтрШаблон(НСтр("ru='Ошибка конвертации свойства объекта XDTO, имя свойства: <%2>.';uk='Помилка конвертації властивості об''єкта XDTO, ім''я властивості: <%2>.'"), ЗначениеСвойства)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция ИнициализироватьПолученныеДанные(ПравилоКонвертации)
	...
	ИначеЕсли ПравилоКонвертации.ЭтоСправочник
		...
		КонецЕсли;
	ИначеЕсли ПравилоКонвертации.ЭтоРегистрСведений Тогда
...
Процедура ОтложенноеЗаполнениеОбъектов(КомпонентыОбмена)
	...
	Исключение
		ТекстОшибки = Символы.ПС + СтрШаблон(НСтр("ru='Событие: %1.
                ...
                |%2.'
                |;uk='Подія: %1.
                |Обробник: ПередОтложеннымЗаполнением.
                |
                |Помилка виконання обробника.
                |%2.'"),
			КомпонентыОбмена.НаправлениеОбмена,
	...
	Для Каждого СтрокаТаблицы Из ЗагруженныеОбъекты Цикл
		...
		Исключение
			ТекстОшибки = Символы.ПС + СтрШаблон(НСтр("ru='Событие: %1.
                    ...
                    |%3.'
                    |;uk='Подія: %1.
                    |Обробник: ОтложенноеЗаполнениеОбъектов.
                    |Об''єкт: %2.
                    |
                    |Помилка виконання обробника.
                    |%3.'"),
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Если ЭлементыФормата.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неканоническое имя формата обмена <%1>';uk='Неканонічне ім''я формату обміну <%1>'"), ФорматОбмена);
	КонецЕсли;
...
Функция СсылкаПоУИД(ТипЗначенияОбъектаИБ, УИДОбъектаXDTO, УзелОбмена)
	...
	Если НайденнаяСсылка.Пустая() ИЛИ НЕ ОбщегоНазначения.СсылкаСуществует(НайденнаяСсылка) Тогда
...
Функция НайтиСсылкуПоПубличномуИдентификатору(УИДОбъектаXDTO, КомпонентыОбмена, ТипЗначенияОбъектаИБ)
	...
	Если Выборка.Количество() > 0 Тогда
		...
		Если  КоличествоСсылокПодходящегоТипа > 1 Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для уникального идентификатора <%1> и узла <%2> назначено несколько ссылок.';uk='Для унікального ідентифікатора <%1> і вузла <%2> призначено кілька посилань.'"),
...
Процедура ПрименитьУдалениеОбъектов(КомпонентыОбмена, МассивОбъектовКУдалению, МассивЗагруженныхОбъектов)
	...
	Для Каждого ЭлементКУдалению Из МассивОбъектовКУдалению Цикл
		...
		Иначе
			...
			ТипПолученныхДанныхСтрокой = ИмяТипаДанныхПоОбъектуМетаданных(Объект.Метаданные(), Неопределено);
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не найдено ПОД для типа ссылки XDTO.
                    |Тип ссылки XDTO: %1
                    |Описание ошибки: %2'
                    |;uk='Не знайдено ПОД для типу посилання XDTO.
                    |Тип посилання XDTO: %1
                    |Опис помилки: %2'"),
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не найдено ПОД для типа объекта XDTO.
                    |Тип объекта XDTO: %1
                    |Описание ошибки: %2'
                    |;uk='Не знайдено ПОД тип об''єкта XDTO.
                    |Тип об''єкта XDTO: %1
                    |Опис помилки: %2'"),
...
Функция ПОДПоОбъектуМетаданных(КомпонентыОбмена, ОбъектМетаданных)
	...
	Если ПравилоОбработки = Неопределено Тогда
		ТекстОшибки = Символы.ПС + СтрШаблон(НСтр("ru='Не найдено ПОД для объекта метаданных.
                |Объект метаданных: %3.'
                |;uk='Не знайдено ПОД для об''єкта метаданих.
                |Об''єкт метаданих: %3.'"),
...
Функция ПОДПоИмени(КомпонентыОбмена, Имя)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найдено ПОД с именем %1';uk='Не знайдено ПОД з ім''ям %1'"), Имя);
...
Процедура ПриКонвертацииДанныхXDTO(ДанныеXDTO, ПолученныеДанные, КомпонентыОбмена, Знач ИмяОбработчика)
	...
	Исключение
		ТекстОшибки = Символы.ПС + СтрШаблон(НСтр("ru='Событие: %1.
                ...
                |%3.'
                |;uk='Подія: %1.
                |Обробник: ПриКонвертацииДанныхXDTO.
                |Об''єкт: %2.
                |
                |Помилка виконання обробника.
                |%3.'"),
...
Процедура ПередЗаписьюПолученныхДанных(ПолученныеДанные, ДанныеИБ, КомпонентыОбмена, ИмяОбработчика, КонвертацияСвойств)
	...
	Исключение
		ТекстОшибки = Символы.ПС + СтрШаблон(НСтр("ru='Событие: %1.
                ...
                |%3.'
                |;uk='Подія: %1.
                |Обробник: ПередЗаписьюПолученныхДанных.
                |Об''єкт: %2.
                |
                |Помилка виконання обробника.
                |%3.'"),
...
Функция ПредставлениеОбъектаДляПротокола(Объект)
	...
	Иначе
		Если ТипОбъекта = Тип("ОбъектXDTO") Тогда
			...
			Если КоллекцияСвойств.Количество() > 0 И КоллекцияСвойств.Получить("КлючевыеСвойства") <> Неопределено Тогда
				...
				Если ЗначениеЗаполнено(Номер) Тогда
					ПредставлениеОбъекта = СтрШаблон(НСтр("ru='%1 №%2 от %3';uk='%1 №%2 від %3'"), Наименование, Номер, Дата);
				ИначеЕсли ЗначениеЗаполнено(Код) Тогда
					ПредставлениеОбъекта = СтрШаблон(НСтр("ru='%1, %2';uk='%1, %2'"), Наименование, Код);
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли ТипОбъекта = Тип("Структура") Тогда
			...
			КонецЕсли;
		Иначе // СправочникОбъект или ДокументОбъект.
...
Функция ВерсииФорматаОбмена(Знач УзелИнформационнойБазы)
	...
	Если ВерсииФорматаОбмена.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Не задані версії формату обміну.
            |Ім''я плану обміну: %1
            |Процедура: ПолучитьВерсииФорматаОбмена(<ВерсииФорматаОбмена>)'"),
...
Процедура ПроверитьВерсию(Знач Версия)
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неканоническое представление версии формата обмена: <%1>.';uk='Неканонічне подання версії формату обміну: <%1>.'"), Версия);
	КонецЕсли;
...
Функция ИмяТипаДанныхПоОбъектуМетаданных(Знач ОбъектМетаданных, Знач ИмяБазовогоТипа)
	ЭтоКонстанта = Ложь;
	...
	ЭтоРегистрСведений = (ИмяБазовогоТипа = ОбщегоНазначения.ИмяТипаРегистрыСведений());
	...
	ИначеЕсли ЭтоРегистрСведений Тогда
		Результат = "[ТипТаблицы]НаборЗаписей.[ИмяТаблицы]";
	Иначе
