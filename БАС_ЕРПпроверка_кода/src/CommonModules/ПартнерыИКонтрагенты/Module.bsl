...
Функция ЗаголовокРеквизитаПартнерВЗависимостиОтХозяйственнойОперации( ХозяйственнаяОперация) Экспорт
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствПоЗаймамВыданным Тогда
		ЗаголовокПартнер = НСтр("ru='Дебитор';uk='Дебітор'");
	//++ НЕ УТ
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья Тогда
		ЗаголовокПартнер = НСтр("ru='Давалец';uk='Давалець'");
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПроизводствоУПереработчика Тогда
		ЗаголовокПартнер = НСтр("ru='Переработчик';uk='Переробник'");
	//-- НЕ УТ
	Иначе
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
...
Процедура ЗаполнитьПанельИнформацииПоДаннымПартнера(Форма, Партнер) Экспорт
	...
	Если Партнер = Неопределено Тогда
		...
		Иначе
			Форма.НаименованиеТекущегоПартнера                = НСтр("ru='Партнер не выбран';uk='Партнер не вибрано'");
...
Процедура ПроверитьКорректностьЗаполненияКонтрагента(Объект, Контрагент, Отказ) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.ИННПлательщикаНДС) Тогда 	
		...
		Если НЕ Объект.ПлательщикНДС И (УпрощенныйВводДоступен()) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
