...
Функция ПровестиДокументы(Документы) Экспорт
	...
	Для Каждого ДокументСсылка Из Документы Цикл
		...
		Если ДокументОбъект.ПроверитьЗаполнение() Тогда
			...
			КонецПопытки;
		Иначе
			ПредставлениеОшибки = НСтр("ru='Поля документа не заполнены.';uk='Поля документа не заповнені.'");
		КонецЕсли;
...
Функция МестаИспользования(Знач НаборСсылок, Знач АдресРезультата = "") Экспорт
	...
	Для Каждого Строка Из МестаИспользования Цикл
		...
		ИначеЕсли МетаКонстанты.Содержит(Мета) Тогда
			Представление = Мета.Представление() + " (" + НСтр("ru='константа';uk='константа'") + ")";
			
		ИначеЕсли МетаПоследовательности.Содержит(Мета) Тогда
			Представление = Мета.Представление() + " (" + НСтр("ru='последовательность';uk='послідовність'") + ")";
...
Функция ПрочитатьXMLВТаблицу(Знач XML) Экспорт
	...
	// В случае макета "НачальноеЗаполнение" представления реквизитов получаются на текущем
	//  языке информационной базы Имена колонок таких реквизитов содержат суффикс "#"
	ИменаКолонокXML = СтрЗаменить(ИменаКолонок, "#", ?(Локализация.КодЯзыкаИнформационнойБазы() = "uk", "Ukr", ""));
	...
	Если Не СравниваютсяМассивы Тогда
		...
		Если ПустаяСтрока(ИменаКолонок) Тогда
			Если ТипыОсобыхКоллекций.Найти(ТипЗнч(КоллекцияСтрок1)) <> Неопределено
				...
				КонецЕсли;
			Иначе
				ТекстИсключения = НСтр("ru='Для коллекции типа %1 необходимо указать имена полей, по которым производится сравнение';uk=' Для колекції типу %1 необхідно зазначити імена полів, по яких проводиться порівняння'");
...
// Возвращает таблицу значений, полученную из временной таблицы запроса
// Используется для отладки 
//
// Параметры:
//  ЗапросИлиИМВТ 			- Запрос или МенеджерВременныхТаблиц    - 
//  ИмяВременнойТаблицы  	- Строка 								- имя временной таблицы
//
// Возвращаемое значение:
//   ТаблицаЗначений   - таблица значений, полученная из временной таблицы запроса
//
Функция ПоказатьВременнуюТаблицу(Знач ЗапросИлиИМВТ, Знач ИмяВременнойТаблицы) Экспорт 
	
    Запрос = Новый Запрос;
    Если ТипЗнч(ЗапросИлиИМВТ) = Тип("Запрос") Тогда
        Запрос.МенеджерВременныхТаблиц = ЗапросИлиИМВТ.МенеджерВременныхТаблиц;
    ИначеЕсли ТипЗнч(ЗапросИлиИМВТ) = Тип("МенеджерВременныхТаблиц") Тогда
        Запрос.МенеджерВременныхТаблиц = ЗапросИлиИМВТ;
    Иначе 
        Возврат Неопределено;
	КонецЕсли;
	
    Запрос.Текст = "ВЫБРАТЬ * ИЗ " + СокрЛП(ИмяВременнойТаблицы);
...
Функция ИдентификаторCOMСоединителя(Знач ИмяCOMСоединителя) Экспорт
	...
	ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='На задан CLSID для класса %1';uk='На заданий CLSID для класу %1'"), ИмяCOMСоединителя);
...
Функция WSПрокси(Знач АдресWSDL,
	...
	Если ДелатьКонтрольныйВызов И Таймаут <> Неопределено И Таймаут > 20 Тогда
		...
		Попытка
			WSПроксиPing.Ping();
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='WSПрокси';uk='WSПрокси'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ВыполнитьЗапросВнеТранзакции(Знач Запрос) Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение(НСтр("ru='Транзакция активна. Выполнение запроса вне транзакции невозможно.';uk='Транзакція активна. Виконання запиту поза транзакцією неможливо.'"));
	КонецЕсли;
	...
	Если СтандартныеПодсистемыСервер.НеобходимоОбновлениеПараметровРаботыПрограммы() Тогда
		ВызватьИсключение
			НСтр("ru='Вход в программу временно невозможен в связи с обновлением на новую версию.
                       ...
                       |;uk='Вхід в програму тимчасово неможливий у зв''язку з оновленням на нову версію.
                       |Рекомендується забороняти виконання регламентних завдань на час оновлення.'");
	КонецЕсли;
	   ...
	   И ЗначениеЗаполнено(Константы.ГлавныйУзел.Получить()) Тогда
		...
		ВызватьИсключение
			НСтр("ru='Вход в программу временно невозможен до восстановления связи с главным узлом.
                       ...
                       |;uk='Вхід в програму тимчасово неможливий до відновлення зв''язку з головним вузлом.
                       |Рекомендується забороняти виконання регламентних завдань на час відновлення.'");
	КонецЕсли;
		...
		Если Не Доступно Тогда
			ВызватьИсключение
				НСтр("ru='Регламентное задание недоступно по функциональным опциям
                    ...
                    |Выполнение прервано.'
                    |;uk='Регламентне завдання недоступне за функціональним опціями
                    |або не підтримує роботу в поточному режимі роботи програми.
                    |Виконання перервано.'");
		КонецЕсли;
...
Функция ИнтервалВремениСтрокой(ВремяНачала, ВремяОкончания = Неопределено) Экспорт
	...
	ИначеЕсли ВремяНачала > ВремяОкончания Тогда
		ВызватьИсключение НСтр("ru='Дата окончания интервала не может быть меньше даты начала.';uk='Дата закінчення інтервалу не може бути менша дати початку.'");
	КонецЕсли;
	...
	Если ВеличинаИнтервалаВДнях > 365 Тогда
		ОписаниеИнтервала = НСтр("ru='более года';uk='більше року'");
	ИначеЕсли ВеличинаИнтервалаВДнях > 31 Тогда
		ОписаниеИнтервала = НСтр("ru='более месяца';uk='більше місяця'");
	...
	Иначе
		ОписаниеИнтервала = НСтр("ru='менее одного дня';uk='менше одного дня'");
	КонецЕсли;
...
Функция Просклонять(Знач ФИО, Падеж, Результат, Пол = Неопределено) Экспорт
	...
	ВызватьИсключение НСтр("ru='Для вызова ОбщегоНазначения.Просклонять необходима подсистема «Склонение представлений объектов».';uk='Для виклику ОбщегоНазначения.Просклонять необхідна підсистема «Схиляння уявлень об''єктів».'");
...
Функция ДеревоМетаданныхКонфигурации(Отбор = Неопределено) Экспорт
	...
	НоваяСтрокаКоллекцииОбъектовМетаданных("Константы",               НСтр("ru='Константы';uk='Константи'"),                 БиблиотекаКартинок.Константа,              БиблиотекаКартинок.Константа,                    КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("Справочники",             НСтр("ru='Справочники';uk='Довідники'"),               БиблиотекаКартинок.Справочник,             БиблиотекаКартинок.Справочник,                   КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("Документы",               НСтр("ru='Документы';uk='Документи'"),                 БиблиотекаКартинок.Документ,               БиблиотекаКартинок.ДокументОбъект,               КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("ПланыВидовХарактеристик", НСтр("ru='Планы видов характеристик';uk='Плани видів характеристик'"), БиблиотекаКартинок.ПланВидовХарактеристик, БиблиотекаКартинок.ПланВидовХарактеристикОбъект, КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("ПланыСчетов",             НСтр("ru='Планы счетов';uk='Плани рахунків'"),              БиблиотекаКартинок.ПланСчетов,             БиблиотекаКартинок.ПланСчетовОбъект,             КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("ПланыВидовРасчета",       НСтр("ru='Планы видов расчета';uk='Плани видів розрахунку'"),       БиблиотекаКартинок.ПланВидовРасчета,       БиблиотекаКартинок.ПланВидовРасчетаОбъект,       КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыСведений",        НСтр("ru='Регистры сведений';uk='Регістри відомостей'"),         БиблиотекаКартинок.РегистрСведений,        БиблиотекаКартинок.РегистрСведений,              КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыНакопления",      НСтр("ru='Регистры накопления';uk='Регістри накопичення'"),       БиблиотекаКартинок.РегистрНакопления,      БиблиотекаКартинок.РегистрНакопления,            КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыБухгалтерии",     НСтр("ru='Регистры бухгалтерии';uk='Регістри бухгалтерії'"),      БиблиотекаКартинок.РегистрБухгалтерии,     БиблиотекаКартинок.РегистрБухгалтерии,           КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыРасчета",         НСтр("ru='Регистры расчета';uk='Регістри розрахунку'"),          БиблиотекаКартинок.РегистрРасчета,         БиблиотекаКартинок.РегистрРасчета,               КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("БизнесПроцессы",          НСтр("ru='Бизнес-процессы';uk='Бізнес-процеси'"),           БиблиотекаКартинок.БизнесПроцесс,          БиблиотекаКартинок.БизнесПроцессОбъект,          КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("Задачи",                  НСтр("ru='Задачи';uk='Задачі'"),                    БиблиотекаКартинок.Задача,                 БиблиотекаКартинок.ЗадачаОбъект,                 КоллекцииОбъектовМетаданных);
...
Функция ОбъектПоПолномуИмени(ПолноеИмя) Экспорт
	...
	Если МассивСтрок.Количество() >= 2 Тогда
		...
		Имя = МассивСтрок[1];
	Иначе
		ВызватьИсключение СтрЗаменить(НСтр("ru='Некорректное полное имя отчета или обработки ""%1"".';uk='Некоректне повне ім''я звіту чи обробки ""%1"".'"), "%1", ПолноеИмя);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтрЗаменить(НСтр("ru='""%1"" не является отчетом или обработкой.';uk='""%1"" не є звітом або обробкою.'"), "%1", ПолноеИмя);
	КонецЕсли;
КонецФункции
...
Функция ВидОбъектаПоТипу(Тип) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неверный тип значения параметра (%1)';uk='Невірний тип значення параметра (%1)'"), Строка(Тип));
...
Процедура ЗаблокироватьИБ(Знач ПроверитьОтсутствиеДругихСеансов = Истина) Экспорт
	...
	Иначе
		...
		Иначе
			ВызватьИсключение(НСтр("ru='Подсистема ""Работа в модели сервиса"" не доступна';uk='Підсистема ""Робота в моделі сервісу"" не доступна'"));
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьВерсииИнтерфейса(Знач Адрес, Знач Пользователь, Знач Пароль = Неопределено, Знач Интерфейс = Неопределено) Экспорт
	...
	Если Не ПараметрыПодключения.Свойство("URL") 
		...
		ВызватьИсключение(НСтр("ru='Не задан URL сервиса.';uk='Не заданий URL сервісу.'"));
	КонецЕсли;
...
Функция ПолучитьВерсииИнтерфейсаЧерезВнешнееСоединение(ВнешнееСоединение, Знач ИмяИнтерфейса) Экспорт
	...
	Исключение
		СтрокаСообщения = НСтр("ru='Корреспондент не поддерживает версионирование интерфейсов подсистем.
            |Описание ошибки: %1'
            |;uk='Кореспондент не підтримує версіонування інтерфейсів підсистем.
            |Опис помилки: %1'");
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Получение версий интерфейса';uk='Одержання версій інтерфейсу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗаписатьДанныеВБезопасноеХранилище(Владелец, Данные, Ключ = "Пароль") Экспорт
	...
	ОбщегоНазначенияКлиентСервер.Проверить(ЗначениеЗаполнено(Владелец), 
        ...
        |;uk='Неприпустиме значення параметра %1 %2. 
        |параметр повинен містити посилання; передано значення: %3 (тип %4).'"),
...
Функция ОбщийМодуль(Имя) Экспорт
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Общий модуль ""%1"" не найден.';uk='Загальний модуль ""%1"" не знайдений.'"), Имя);
	КонецЕсли;
...
Функция ПодготовитьДанныеКэшаВерсий(Знач ТипДанных, Знач ПараметрыПолучения) Экспорт
	...
	Иначе
		ШаблонТекста = НСтр("ru='Неизвестный тип данных кэша версий: %1';uk='Невідомий тип даних кеша версій: %1'");
...
Функция WSОпределения(Знач АдресWSDL, Знач ИмяПользователя, Знач Пароль, Знач Таймаут = 10) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Получение WSDL';uk='Одержання WSDL'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Функция ПолучитьWSDL(Знач Адрес, Знач ИмяПользователя, Знач Пароль, Знач Таймаут)
	...
	Если НЕ ОписаниеФайла.Статус Тогда
		ВызватьИсключение(НСтр("ru='Ошибка получения файла описания web-сервиса:';uk='Помилка одержання файлу опису web-сервісу:'") + Символы.ПС + ОписаниеФайла.СообщениеОбОшибке)
	КонецЕсли;
	...
	Если Определения.Сервисы.Количество() = 0 Тогда
		ШаблонСообщения = НСтр("ru='Ошибка получения файла описания web-сервиса:
            ...
            |%1'
            |;uk='Помилка одержання файлу опису web-сервісу:
            |В отриманому файлі не втримується жодного опису сервісу.
            |
            |Можливо, адреса файлу опису зазначень невірно:
            |    %1'");
...
Процедура ПроверкаФиксированностиДанных(Данные, ДанныеВЗначенииФиксированныхТипов = Ложь)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Ошибка в функции ФиксированныеДанные общего модуля ОбщегоНазначения.
                   ...
                   |;uk='Помилка у функції ФиксированныеДанные загального модуля ОбщегоНазначения.
...
Функция СерверныйМодульМенеджера(Имя)
	...
	Если Не ОбъектНайден Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Об''єкт метаданих ""%1"" не знайдений,
            |або для нього не підтримується одержання модуля менеджера.'"), Имя);
	КонецЕсли;
...
Функция РазметитьМестаИспользования(Знач ПараметрыВыполнения, Знач Ссылка, Знач ЦелеваяСсылка, Знач ТаблицаПоиска)
	...
	Для Каждого МестоИспользования Из Результат.МестаИспользования Цикл
		...
		Иначе
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Замена ссылок в ""%1"" не поддерживается.';uk='Заміна посилань у ""%1"" не підтримується.'"), Информация.ПолноеИмя);
...
Процедура ПроизвестиЗаменуВКонстанте(Результат, Знач МестоИспользования, Знач ПараметрыЗаписи, Знач ВнутренняяТранзакция = Истина)
	...
	Если ВнутренняяТранзакция Тогда
		...
		Исключение
			...
			Ошибка = НСтр("ru='Не удалось заблокировать константу %1';uk='Не вдалося заблокувати константу %1'");
	...
	Если СостояниеОперации = "" Тогда
		...
		Если ЗаменаПроизведена Тогда
			...
			Исключение
				...
				Ошибка = НСтр("ru='Не удалось записать %1 по причине: %2';uk='Не вдалося записати %1 через: %2'");
				Ошибка = СтрЗаменить(Ошибка, "%1", ПредставлениеДанных);
...
Процедура ПроизвестиЗаменуВНаборе(Результат, Знач МестоИспользования, Знач ПараметрыВыполнения, Знач ВнутренняяТранзакция = Истина)
	...
	Если ВнутренняяТранзакция Тогда
		...
		Исключение
			// Добавляем запись в результат о неудачной попытке блокировке.
			Ошибка = НСтр("ru='Не удалось заблокировать набор  %1';uk='Не вдалося заблокувати набір %1'");
			Ошибка = СтрЗаменить(Ошибка, "%1", ПредставлениеДанных);
	...
	Для Каждого Ссылка Из Удаляемые Цикл
		...
		Исключение
			...
			ЗарегистрироватьОшибкуЗамены(Результат, Ссылка, 
				...
				НСтр("ru='Ошибка удаления';uk='Помилка вилучення'") + Символы.ПС + СокрЛП( КраткоеПредставлениеОшибки(ИнформацияОбОшибке))));
		КонецПопытки;
...
Процедура ДобавитьРезультатыЗаменыИзмененныхОбъектов(Результат, ТаблицаПовторногоПоиска)
	...
	Для Каждого Строка Из ТаблицаПовторногоПоиска Цикл
		...
		ЗарегистрироватьОшибкуЗамены(Результат, Ссылка, 
			...
			НСтр("ru='Заменены не все места использования. Возможно места использования были добавлены или изменены другим пользователем.';uk='Замінені не всі місця використання. Можливо місця використання були додані або змінені іншим користувачем.'")));
	КонецЦикла;
...
Функция ОписаниеИзмеренийНабора(Знач Мета, Кэш)
	...
	Если Метаданные.РегистрыСведений.Содержит(Мета) Тогда
		...
		Если МетаПериод = Периодичность.ПозицияРегистратора Тогда
			...
			ДанныеИзмерения.Представление = НСтр("ru='Регистратор';uk='Реєстратор'");
		...
		ИначеЕсли МетаПериод = Периодичность.Год Тогда
			...
			ДанныеИзмерения.Представление = НСтр("ru='Период';uk='Період'");
...
Процедура ЗаписатьОбъект(Знач Объект, Знач ПараметрыЗаписи)
	...
	Если ТестРеквизитов.Иерархический Или ТестРеквизитов.ВидыСубконто <> Неопределено Тогда 
		...
		Если Объект.Родитель = Объект.Ссылка Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При записи ""%1"" возникает циклическая ссылка в иерархии.';uk='При записі ""%1"" виникає циклічне посилання в ієрархії.'"),
				Строка(Объект));
			КонецЕсли;
	...
	Для Каждого МетаВладелец Из ТестРеквизитов.Владельцы Цикл
		...
		Если Объект.Владелец = Объект.Ссылка Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При записи ""%1"" возникает циклическая ссылка в подчинении.';uk='При записі ""%1"" виникає циклічне посилання в підпорядкуванні.'"),
				Строка(Объект));
		КонецЕсли;
		Прервать;
...
Функция СобытиеЖурналаРегистрацииЗаменыСсылок()
	Возврат НСтр("ru='Поиск и удаление ссылок';uk='Пошук і вилучення посилань'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
...
Процедура ЗаменитьСсылкуДлиннойТранзакцией(Результат, Знач Дубль, Знач ПараметрыВыполнения, Знач ТаблицаПоиска)
	...
	Исключение
		...
		Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось заблокировать все места использования %1:';uk='Не вдалося заблокувати всі місця використання %1:'") + Символы.ПС + ПредставлениеОшибки, Дубль);
		ЗарегистрироватьОшибкуЗамены(Результат, Дубль, 
...
Процедура ЗарегистрироватьОшибкуВТаблицу(Результат, Дубль, Оригинал, Данные, Информация, ТипОшибки, ИнформацияОбОшибке)
	...
	Если ТипОшибки = "БлокировкаДляРегистра" Тогда
		НовыйШаблон = НСтр("ru='Не удалось начать редактирование %1: %2';uk='Не вдалося почати редагування %1: %2'");
		ОшибкаВСтаромФормате.ТипОшибки = "ОшибкаБлокировки";
	ИначеЕсли ТипОшибки = "БлокировкаДляУдаленияДубля" Тогда
		НовыйШаблон = НСтр("ru='Не удалось начать удаление: %2';uk='Не вдалося почати вилучення: %2'");
		ОшибкаВСтаромФормате.ТипОшибки = "ОшибкаБлокировки";
	ИначеЕсли ТипОшибки = "УдалитьНаборДубля" Тогда
		НовыйШаблон = НСтр("ru='Не удалось очистить сведения о дубле в %1: %2';uk='Не вдалося очистити відомості про дублі %1: %2'");
		ОшибкаВСтаромФормате.ТипОшибки = "ОшибкаЗаписи";
	ИначеЕсли ТипОшибки = "ЗаписатьНаборОригинала" Тогда
		НовыйШаблон = НСтр("ru='Не удалось обновить сведения в %1: %2';uk='Не вдалося оновити відомості %1: %2'");
		ОшибкаВСтаромФормате.ТипОшибки = "ОшибкаЗаписи";
	Иначе
	...
	НовыйШаблон = НовыйШаблон + Символы.ПС + Символы.ПС + НСтр("ru='Подробности в журнале регистрации.';uk='Подробиці в журналі реєстрації.'");
...
Функция ВычислитьЗначениеДанныхПоПути(Знач Данные, Знач ПутьДанных)
	...
	Для Каждого ИмяРеквизита Из Имена Цикл
		...
		Если Тест[ИмяРеквизита] = Неопределено Тогда
			...
			ЗаполнитьЗначенияСвойств(Тест, Результат);
			Если Тест[ИмяРеквизита] = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка вычисления %1 для объекта %2. Реквизит %3 не найден';uk='Помилка обчислення %1 для об''єкта %2. Реквізит %3 не знайдений'"),
