...
Функция СоздатьАдминистратора(ПользовательИБ = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("ru='Справочник Пользователи недоступен в неразделенном режиме.';uk='Довідник Користувачі недоступний в нерозподіленому режимі.'");
	КонецЕсли;
	...
	Если ПользовательИБ = Неопределено Тогда
		...
		Если ПользователиИБ.Количество() = 0 Тогда
			Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
				ВызватьИсключение
					НСтр("ru='Невозможно автоматически создать первого администратора области данных.';uk='Неможливо автоматично створити першого адміністратора області даних.'");
			КонецЕсли;
			...
			Если Метаданные.Роли.ПолныеПрава = РольАдминистратораСистемы(Истина) Тогда
				ТекстОшибки =
					НСтр("ru='Список пользователей информационной базы не пустой, однако
                               ...
                               |;uk='Список користувачів інформаційної бази не порожній, однак
                               |не вдалося знайти ні одного користувача з роллю Повні права.
                               |
                               |Ймовірно, користувачі створювалися в конфігураторі.
                               |Потрібно призначити роль Повні права хоча б одному користувачеві.'");
			Иначе
				ТекстОшибки =
					НСтр("ru='Список пользователей информационной базы не пустой, однако не удалось
                               ...
                               |;uk='Список користувачів інформаційної бази не порожній, проте не вдалося
                               |знайти ні одного користувача з ролями Повні права і Адміністратор системи.
                               |
                               |Ймовірно, користувачі створювалися в конфігураторі.
                               |Потрібно призначити ролі Повні права і Адміністратор системи хоча б одному користувачеві.'");
			КонецЕсли;
			...
			Если Метаданные.Роли.ПолныеПрава = РольАдминистратораСистемы(Истина) Тогда
				ТекстОшибки =
					НСтр("ru='Невозможно создать пользователя в справочнике для пользователя
                               ...
                               |;uk='Неможливо створити користувача в довіднику для користувача
                               |інформаційної бази ""%1"",
                               |так як у нього немає ролі Повні права.
                               |
                               |Ймовірно, користувач був створений в конфігураторі. Для автоматичного
                               |створення користувача в довіднику потрібно призначити йому роль Повні права.'");
			Иначе
				ТекстОшибки =
					НСтр("ru='Невозможно создать пользователя в справочнике для пользователя
                               |информационной базы ""%1"",
                               ...
                               |;uk='Неможливо створити користувача в довіднику для користувача
                               |інформаційної бази ""%1"",
                               |тому що у нього немає ролей Повні права і Адміністратор системи.
                               |
                               |Ймовірно, користувач був створений в конфігураторі.
                               |Для автоматичного створення користувача в довіднику необхідно
                               |призначити йому ролі Повні права і Адміністратор системи.'");
			КонецЕсли;
	...
	Пользователь.ДополнительныеСвойства.Вставить(
		...
		?(ПользовательИБ = Неопределено,
		  НСтр("ru='Выполнено создание первого администратора.';uk='Виконано створення першого адміністратора.'"),
		  ?(ПользовательСоздан,
		    НСтр("ru='Администратор сопоставлен с новым пользователем справочника.';uk='Адміністратор зіставлений з новим користувачем довідника.'"),
		    НСтр("ru='Администратор сопоставлен с существующим пользователем справочника.';uk='Адміністратор зіставлений з існуючим користувачем довідника.'")) ) );
...
Функция ПрочитатьПользователяИБ(Знач Идентификатор,
	...
	Если ПользовательИБ = Неопределено Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пользователь информационной базы ""%1"" не найден.';uk='Користувач інформаційної бази ""%1"" не знайдений.'"),
			Идентификатор);
		Возврат Ложь;
	КонецЕсли;
	...
	ИначеЕсли СоздатьНового = Истина Тогда
		ОписаниеОшибки = ОписаниеОшибкиПриЗаписиПользователяИБ(
			НСтр("ru='Невозможно создать пользователя информационной базы
                       ...
                       |;uk='Неможливо створити користувача інформаційної бази
                       | %1,
                       |тому що він уже існує.'"),
			СтарыеСвойства.Имя,
			...
			Если НЕ СтарыйПарольСовпадает Тогда
				ОписаниеОшибки = ОписаниеОшибкиПриЗаписиПользователяИБ(
					НСтр("ru='При записи пользователя информационной базы
                               ...
                               |;uk='При записі користувача інформаційної бази
                               |%1,
                               |старий пароль зазначень не вірно.'"),
					СтарыеСвойства.Имя,
					СтарыеСвойства.УникальныйИдентификатор);
				Возврат Ложь;
			КонецЕсли;
		...
		Исключение
			ОписаниеОшибки = ОписаниеОшибкиПриЗаписиПользователяИБ(
				НСтр("ru='При удалении пользователя информационной базы
                           ...
                           |
                           |""%2"".'
                           |;uk='При вилученні користувача інформаційної бази
                           |%1 відбулася помилка:
                           |
                           |""%2"".'"),
				ПользовательИБ.Имя,
						...
						Если СтрДлина(ЗначениеСвойства) > 64 Тогда
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Ошибка записи пользователя информационной базы
                                           ...
                                           |;uk='Помилка запису користувача інформаційної бази
                                           |Ім''я (для входу): ""%1""
                                           |перевищує довжину 64 символу.'"),
								ЗначениеСвойства);
		...
		Если Строка.НеоднозначныйИдентификатор <> ТекущийНеоднозначныйИдентификатор Тогда
			...
			Иначе
				...
				Если ТекущийПользовательИБ = Неопределено Тогда
					ИмяДляВхода = НСтр("ru='<не найден>';uk='<не знайдено>'");
				Иначе
				...
				Если ИдентификаторПользователяСервиса Тогда
					ОписаниеОшибки = ОписаниеОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Пользователю сервиса с идентификатором ""%1""
                                   ...
                                   |;uk='Користувачеві сервісу з ідентифікатором ""%1""
                                   |відповідає більше одного елемента в довіднику Користувачі:'"),
						ТекущийНеоднозначныйИдентификатор);
				Иначе
					ОписаниеОшибки = ОписаниеОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Пользователю ИБ ""%1"" с идентификатором ""%2""
                                   |соответствует более одного элемента в справочнике Пользователи:'
                                   |;uk='Користувачеві ІБ ""%1"" з ідентифікатором ""%2""
                                   |відповідає більше одного елемента в довіднику Користувачі:'"),
						ИмяДляВхода,
						ТекущийНеоднозначныйИдентификатор);
				КонецЕсли;
		...
		Иначе
			...
			ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + "	"
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='""%1"" с идентификатором ссылки %2';uk='""%1"" з ідентифікатором посилання %2'"),
					Строка.Пользователь,
...
Функция ОписаниеОшибкиПриЗаписиПользователяИБ(ШаблонОшибки,
	...
	Если ЗаписатьВЖурнал Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Пользователи.Ошибка записи пользователя ИБ';uk='Користувачі.Помилка запису користувача ІБ'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
				"""" + ИмяДляВхода + """ (" + ?(ЗначениеЗаполнено(ИдентификаторПользователяИБ),
					НСтр("ru='Новый';uk='Новий'"), Строка(ИдентификаторПользователяИБ)) + ")",
