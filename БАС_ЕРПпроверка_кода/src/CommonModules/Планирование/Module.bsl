...
Процедура ОбработкаПроверкиЗаполненияВФорме(Таблица, СтруктураДействий, Отказ, ПроверяемыеРеквизиты, Знач КроссТаблица = Истина) Экспорт 
	...
	//++ НЕ УТ
	ТекстОшибкиСпецификация 	= НСтр("ru='Не заполнена колонка ""Спецификация""';uk='Не заповнена колонка ""Специфікація""'");
	//-- НЕ УТ
	...
	КонецЕсли; 
	//++ НЕ УТ
	Если СтруктураДействий.Свойство("ЗаполнятьСпецификацию") Тогда 
		ЗаполнятьСпецификацию = СтруктураДействий.ЗаполнятьСпецификацию;
	КонецЕсли; 
	//-- НЕ УТ
	...
	Для Каждого СтрокаТЧ Из Таблица Цикл
		...
		//++ НЕ УТ
		Если ЗаполнятьСпецификацию и Не ЗначениеЗаполнено(СтрокаТЧ.Спецификация) Тогда
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ТекстОшибкиСпецификация + АдресОшибки,
				,
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(ПутьКТаблице, СтрокаТЧ.НомерСтроки, "Спецификация"),
				,
				Отказ);
...
Процедура ОбработкаПроверкиЗаполненияПоСценариюВидуПлана(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт 
	...
	//++ НЕ УТ
	РеквизитыСценария = РеквизитыСценария + ", ОтражаетсяВБюджетировании";
	//-- НЕ УТ
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.ПланПроизводства") Тогда
		...
		Если НЕ ПараметрыСценария.ИспользоватьДляПланированияМатериалов Тогда
	...
	Если ЗначениеЗаполнено(Объект.ВидПлана) Тогда
		...
		//++ НЕ УТ
		РеквизитыВидаПлана = РеквизитыВидаПлана + ",ОтражаетсяВБюджетировании, ОтражаетсяВБюджетированииОплаты";
		//-- НЕ УТ
		...
		Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПланЗакупок") Тогда
			...
			КонецЕсли;
			//++ НЕ УТ
			ВидПланаОтражаетсяВБюджетировании = НастройкиЗаполнения.ОтражаетсяВБюджетировании 
				ИЛИ НастройкиЗаполнения.ОтражаетсяВБюджетированииОплаты;
			
			Если Не ПараметрыСценария.ОтражаетсяВБюджетировании И ВидПланаОтражаетсяВБюджетировании Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Настройки отражения бюджетирования в сценарии не соответствую настройкам вида плана.';uk='Настройки відображення бюджетування в сценарії не відповідають настройкам виду плану.'"),
					Объект, 
					"Сценарий",
					,
					Отказ);
...
#КонецОбласти 
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство") Тогда
		Документы.ПланПроизводства.ДополнительныеПараметрыДляРасчетаПоФормуле(Параметры, ДопПараметры);
	КонецЕсли;
	//-- НЕ УТ
...
Функция МакетыОперандовПлана() Экспорт 
	...
	//++ НЕ УТ
	Документы.ПланПроизводства.МакетыОперандовПлана(Макеты);
	//-- НЕ УТ
...
Процедура ЗапускВыполненияФоновогоПроведения(ДокументСсылка) Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Проведение документа: %Ссылка%';uk='Проведення документу: %Ссылка%'");
	НаименованиеЗадания = СтрЗаменить(НаименованиеЗадания, "%Ссылка%", ДокументСсылка);
	
	ИмяЭкспортнойПроцедуры = "РегистрыНакопления.ПланыПроизводства.ВыполнитьПроведение";
...
// Заполняет движения документа "План производства"
//
Процедура ОтразитьПланПроизводства(ДополнительныеСвойства, Движения, Отказ) Экспорт

	Таблица = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПланыПроизводства;
	...
	Движения.ПланыПроизводства.Записывать = Истина;
	Движения.ПланыПроизводства.Загрузить(Таблица);
	
	Если ДополнительныеСвойства.Свойство("СценарийПланирования") Тогда
		Движения.ПланыПроизводства.ДополнительныеСвойства.Вставить("СценарийПланирования",ДополнительныеСвойства.СценарийПланирования);
	КонецЕсли;
	
	Движения.ПланыПотребленияМатериалов.Записывать = Ложь;
...
Процедура СбалансированностьПлановПередЗагрузкойВариантаНаСервере(ЭтаФорма, НовыеНастройкиКД) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство")
		//++ НЕ УТ
		ИЛИ Не ПолучитьФункциональнуюОпцию("ИспользоватьПланированиеПроизводства")
		//-- НЕ УТ
		Тогда
...
Функция ПолучитьПользовательскиеНастройкиСОтборомПоДокументам(ОтборПоДокументам)
	...
	|		ДокументПлана.Сценарий
	|	ИЗ
	|		Документ.ПланПроизводства КАК ДокументПлана
	|	ГДЕ
	|		ДокументПлана.Ссылка В(&МассивСсылок)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	|		ДокументПланаТовары.Номенклатура
	|	ИЗ
	|		Документ.ПланПроизводства.Продукция КАК ДокументПланаТовары
	|	ГДЕ
	|		ДокументПланаТовары.Ссылка В(&МассивСсылок)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
