...
Процедура ДополнитьПолучателей(Объект, Узлы) Экспорт
	...
	Для Каждого Элемент Из Узлы Цикл
		...
		Исключение
			...
			СтрокаСообщения = НСтр("ru='Для состава плана обмена [ИмяПланаОбмена] не указана регистрация объекта [ПолноеИмя]';uk='Для складу плану обміну [ИмяПланаОбмена] не зазначена реєстрація об''єкта [ПолноеИмя]'");
	...
	Иначе
		...
		Если РежимВыгрузкиОбъекта = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПриНеобходимости Тогда
			...
			Если ЭтоНовыйОбъект Тогда
				ВызватьИсключение НСтр("ru='Регистрация незаписанных объектов, выгружаемых по ссылке, не поддерживается.';uk='Реєстрація незаписаних об''єктів, що вивантажують по посиланню, не підтримується.'");
			КонецЕсли;
...
Процедура ПроверитьВозможностьИзмененияНастроекОбменаДанными(Источник, Отказ) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		ВызватьИсключение НСтр("ru='Изменение наименования и кода синхронизации данных недопустимо.';uk='Зміна найменування й коду синхронізації даних неприпустимо.'");
...
Процедура ЗарегистрироватьИзменениеОбъекта(ИмяПланаОбмена, Объект, Отказ, ДополнительныеПараметры = Неопределено)
		...
		Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			...
			Иначе
				...
				Если РазделенныеДанные(ОбъектМетаданных) Тогда
					ВызватьИсключение НСтр("ru='Регистрация изменений разделенных данных в неразделенном режиме.';uk='Реєстрація змін розділених даних у нерозділеному режимі.'");
				КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Обмен данными.Правила регистрации объектов';uk='Обмін даними. Правила реєстрації об''єктів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ОбъектМодифицированДляПланаОбмена(Источник, ОбъектМетаданных, ИмяПланаОбмена, РежимЗаписи, ЗарегистрироватьОбъектКВыгрузке)
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка определения модифицированности объекта: %1';uk='Помилка визначення модифицированности об''єкта: %1'"),
...
Процедура ВыполнитьПравилаРегистрацииОбъектовДляПланаОбмена(МассивУзловРезультат, Объект, ИмяПланаОбмена, ДополнительныеПараметры)
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |Описание ошибки:
            |%2'
            |;uk='Помилка виконання правил реєстрації об''єктів для плану обміну %1.
            |Опис помилки:
            |%2'"),
			ИмяПланаОбмена,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПолучитьЗначенияСвойствДляСсылки(Ссылка, СвойстваОбъекта, Знач СвойстваОбъектаСтрокой, Знач ОбъектМетаданныхИмя) Экспорт
	...
	Исключение
		СтрокаСообщения = НСтр("ru='Ошибка при получении свойств ссылки. Ошибка выполнения запроса: [ОписаниеОшибки]';uk='Помилка при одержанні властивостей посилання. Помилка виконання запиту: [ОписаниеОшибки]'");
...
Функция МассивУзловПоЗначениямСвойств(ЗначенияСвойств, Знач ТекстЗапроса, Знач ИмяПланаОбмена, Знач ИмяРеквизитаФлага, Знач Выгрузка = Ложь) Экспорт
	...
	Исключение
		СтрокаСообщения = НСтр("ru='Ошибка при получении списка узлов получателей. Ошибка выполнения запроса: [ОписаниеОшибки]';uk='Помилка при одержанні списку вузлів одержувачів. Помилка виконання запиту: [ОписаниеОшибки]'");
		СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОписаниеОшибки]", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение СтрокаСообщения;
	КонецПопытки;
...
Функция НаборЗаписейПоТипу(ОбъектМетаданных)
	...
	Иначе
		...
		СтрокаСообщения = НСтр("ru='Для объекта метаданных %1 не предусмотрено набора записей.';uk='Для об''єкта метаданих %1 не передбачене набору записів.'");
			...
			Если СтрокаДерева.ВидЭлементаОтбора = ОбменДаннымиСервер.ЭлементОтбораСвойствоАлгоритмЗначения() Тогда
				...
				Исключение
					...
					СтрокаСообщения = НСтр("ru='Ошибка алгоритма вычисления значения константы:
                                                ...
                                                |// {Окончание алгоритма}
                                                |'
                                                |;uk='Помилка алгоритму обчислення значення константи:
                                                |План обміну: [ИмяПланаОбмена]
                                                |Об''єкт метаданих: [ОбъектМетаданныхИмя]
                                                |Опис помилки: [Описание]
                                                |Алгоритм:
                                                |// {Початок алгоритму}
                                                |[ЗначениеКонстанты]
	...
	Если ПРО.ЕстьОбработчикПередОбработкой Тогда
		...
		Исключение
			СтрокаСообщения = НСтр("ru='Ошибка при выполнении обработчика: ""[ИмяОбработчика]"";
                ...
                |Объект метаданных: [ОбъектМетаданныхИмя]
                |Описание ошибки: [Описание]'
                |;uk='Помилка при виконанні обробника: ""[ИмяОбработчика]"";
                |План обміну: [ИмяПланаОбмена];
                |Об''єкт метаданних: [ОбъектМетаданныхИмя]
                |Опис помилки: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("ru='Перед обработкой';uk='Перед обробкою'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
	...
	Если ПРО.ЕстьОбработчикПриОбработке Тогда
		...
		Исключение
			СтрокаСообщения = НСтр("ru='Ошибка при выполнении обработчика: ""[ИмяОбработчика]""; План обмена: [ИмяПланаОбмена]; Объект метаданных: [ОбъектМетаданныхИмя]
                |Описание ошибки: [Описание]'
                |;uk='Помилка при виконанні обробника: ""[ИмяОбработчика]""; План обміну: [ИмяПланаОбмена]; Об''єкт метаданих: [ОбъектМетаданныхИмя]
                |Опис помилки: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("ru='При обработке';uk='При обробці'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Описание]",            ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение СтрокаСообщения;
	...
	Если ПРО.ЕстьОбработчикПриОбработкеДополнительный Тогда
		...
		Исключение
			СтрокаСообщения = НСтр("ru='Ошибка при выполнении обработчика: ""[ИмяОбработчика]""; План обмена: [ИмяПланаОбмена]; Объект метаданных: [ОбъектМетаданныхИмя]
                ...
                |;uk='Помилка при виконанні обробника: ""[ИмяОбработчика]""; План обміну: [ИмяПланаОбмена]; Об''єкт метаданих: [ОбъектМетаданныхИмя]
                |Опис помилки: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("ru='При обработке (дополнительный)';uk='При обробці (додатковий)'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Описание]",            ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение СтрокаСообщения;
	...
	Если ПРО.ЕстьОбработчикПослеОбработки Тогда
		...
		Исключение
			СтрокаСообщения = НСтр("ru='Ошибка при выполнении обработчика: ""[ИмяОбработчика]""; План обмена: [ИмяПланаОбмена]; Объект метаданных: [ОбъектМетаданныхИмя]
                ...
                |;uk='Помилка при виконанні обробника: ""[ИмяОбработчика]""; План обміну: [ИмяПланаОбмена]; Об''єкт метаданих: [ОбъектМетаданныхИмя]
                |Опис помилки: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("ru='После обработки';uk='Після обробки'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Описание]",            ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение СтрокаСообщения;
...
Функция ДанныеРазличаются(Данные1, Данные2, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='Задано недопустимое значение параметра [1] метода ОбщегоНазначения.ЗначенияСвойствИзменены.';uk='Задане неприпустиме значення параметра [1] методу ОбщегоНазначения.ЗначенияСвойствИзменены.'");
		...
		Если ОбменДаннымиПовтИсп.ИспользуетсяВерсионирование(Отправитель) Тогда
			Если СсылкаСуществует Тогда
				...
				Если ЗаписатьОбъект Тогда
					Комментарий = НСтр("ru='Предыдущая версия (автоматическое разрешение конфликта).';uk='Попередня версія (автоматичний дозвіл конфлікту).'");
				Иначе
					Комментарий = НСтр("ru='Текущая версия (автоматическое разрешение конфликта).';uk='Поточна версія (автоматичний дозвіл конфлікту).'");
				КонецЕсли;
			...
			Если ЗаписатьОбъект Тогда
				...
				СведенияОВерсииОбъекта.Вставить("Комментарий", НСтр("ru='Текущая версия (автоматическое разрешение конфликта).';uk='Поточна версія (автоматичний дозвіл конфлікту).'"));
			Иначе
				СведенияОВерсииОбъекта = Новый Структура;
				СведенияОВерсииОбъекта.Вставить("АвторВерсии", Отправитель);
				СведенияОВерсииОбъекта.Вставить("ТипВерсииОбъекта", "НепринятыеДанныеПоКоллизии");
				СведенияОВерсииОбъекта.Вставить("Комментарий", НСтр("ru='Отклоненная версия (автоматическое разрешение конфликта).';uk='Відхилена версія (автоматичний дозвіл конфлікту).'"));
			КонецЕсли;
...
Процедура ЗарегистрироватьЗапретЗагрузкиДанныхПоДате(ЭлементДанных, Отправитель, СообщениеОбОшибке)
	...
	Если ОбменДаннымиПовтИсп.ИспользуетсяВерсионирование(Отправитель.Ссылка) И ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(ЭлементДанных.Метаданные()) Тогда
		...
		Если СсылкаСуществует Тогда
			...
			Комментарий = НСтр("ru='Версия создана при синхронизации данных.';uk='Версія створена при синхронізації даних.'");
		...
		Иначе
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='%1 запрещено загружать в запрещенный период.%2%2%3';uk=' %1 заборонено завантажувати в заборонений період. %2%2%3'");
...
Процедура ЗарегистрироватьПредупреждениеОКоллизииВЖурналеРегистрации(Объект, МетаданныеОбъекта, ЗаписатьОбъект, ЭтоСсылочныйТип)
	...
	Если ЗаписатьОбъект Тогда
        ...
        |;uk='Виник конфлікт змін об''єктів.
        |Об''єкт цієї інформаційної бази був замінений версією об''єкта із другої інформаційної бази.'");
	...
	Иначе
        ...
        |;uk='Виник конфлікт змін об''єктів.
        |Об''єкт із другої інформаційної бази не прийнятий. Об''єкт цієї інформаційної бази не змінений.'");
...
Функция ПолучитьДанныеОбъектаСтрокойДоИзменения(Объект, МетаданныеОбъекта, ЭтоСсылочныйТип, СсылкаСуществует, ОбъектВБазе)
	...
	Если ЭтоСсылочныйТип Тогда
		...
		Иначе
			...
			СтрокаОбъекта = НСтр("ru='Объект удален';uk='Об''єкт вилучений'");
