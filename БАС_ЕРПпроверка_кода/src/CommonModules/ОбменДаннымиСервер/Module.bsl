...
Функция СтруктураЗаголовкаГиперссылкиМонитораПроблем(Узлы = Неопределено) Экспорт
	...
	Если Количество > 0 Тогда
		...
		Заголовок = НСтр("ru='Предупреждения (%1)';uk='Попередження (%1)'");
	...
	Иначе
		...
		Заголовок = НСтр("ru='Предупреждений нет';uk='Попереджень немає'");
		...
		Если Реквизит <> Неопределено Тогда
			...
			Если ТипыРеквизита.Количество() <> 1 Тогда
				...
				СтрокаСообщения = НСтр("ru='Составной тип данных для значений по умолчанию не поддерживается.
                    |Реквизит ""%1"".'
                    |;uk='Складений тип даних для значень за замовчуванням не підтримується.
                    |Реквізит ""%1"".'");
			...
			Если Не ОбщегоНазначения.ЭтоСправочник(ОбъектМетаданных) Тогда
				...
				СтрокаСообщения = НСтр("ru='Выбор значений по умолчанию поддерживается только для справочников.
                    |Реквизит ""%1"".'
                    |;uk='Вибір значень за замовчуванням підтримується тільки для довідників.
                    |Реквізит ""%1"".'");
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, Реквизит.ПолноеИмя());
				ВызватьИсключение СтрокаСообщения;
			КонецЕсли;
...
Функция ПредставлениеДатыСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаСинхронизации) Тогда
		Возврат НСтр("ru='Синхронизация не выполнялась.';uk='Синхронізація не виконувалася.'");
	КонецЕсли;
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Последняя синхронизация: %1';uk='Остання синхронізація: %1'"), ОтносительнаяДатаСинхронизации(ДатаСинхронизации));
КонецФункции
...
Функция ОтносительнаяДатаСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаСинхронизации) Тогда
		...
		Возврат НСтр("ru='Никогда';uk='Ніколи'");
	...
	ИначеЕсли Интервал < 60 * 5 Тогда // 5 мин
		...
		Результат = НСтр("ru='Сейчас';uk='Зараз'");
		
	ИначеЕсли Интервал < 60 * 15 Тогда // 15 мин
		
		Результат = НСтр("ru='5 минут назад';uk='5 хвилин тому'");
		
	ИначеЕсли Интервал < 60 * 30 Тогда // 30 мин
		
		Результат = НСтр("ru='15 минут назад';uk='15 хвилин тому'");
		
	ИначеЕсли Интервал < 60 * 60 * 1 Тогда // 1 час
		
		Результат = НСтр("ru='30 минут назад';uk='30 хвилин назад'");
		
	ИначеЕсли Интервал < 60 * 60 * 2 Тогда // 2 часа
		
		Результат = НСтр("ru='1 час назад';uk='1 годину тому'");
		
	ИначеЕсли Интервал < 60 * 60 * 3 Тогда // 3 часа
		
		Результат = НСтр("ru='2 часа назад';uk='2 години тому'");
	...
	Иначе
		...
		Если КоличествоДнейРазницы = 0 Тогда // сегодня
			...
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сегодня, %1';uk='Сьогодні, %1'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
			
		ИначеЕсли КоличествоДнейРазницы = 1 Тогда // вчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вчера, %1';uk='Вчора, %1'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
			
		ИначеЕсли КоличествоДнейРазницы = 2 Тогда // позавчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Позавчера, %1';uk='Позавчора, %1'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
...
Процедура ЗагрузитьПриоритетныеДанныеВПодчиненныйУзелРИБ(Отказ = Ложь) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Обмен данными.Загрузка приоритетных данных';uk='Обмін даними.Завантаження пріоритетних даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		ВызватьИсключение
			НСтр("ru='Ошибка загрузки приоритетных данных из сообщения обмена.
                       ...
                       |;uk='Помилка завантаження пріоритетних даних з повідомлення обміну.
                       |См. подробиці в журналі реєстрації.'");
	КонецПопытки;
	УстановитьПривилегированныйРежим(Истина);
	УстановитьРежимЗагрузкиСообщенияОбменаДаннымиПередЗапуском("ЗагрузкаРазрешена", Ложь);
	УстановитьПривилегированныйРежим(Ложь);
		...
		Если КонфигурацияИзменена() Тогда
			ВызватьИсключение
				НСтр("ru='Загружены изменения программы, полученные из главного узла.
                           ...
                           |;uk='Завантажені зміни програми, отримані з головного вузла.
                           |Завершіть роботу програми. Відкрийте програму в конфігураторі
                           |та виконайте команду ""Оновити конфігурацію бази даних (F7)"".
                           |
                           |Після цього запустіть програму.'");
		КонецЕсли;
...
Процедура ЗаписатьДанныеДляОбновленияВФайл(Параметры, Данные, ВидДанных, ПолноеИмяОбъекта = "") Экспорт
	...
	Если Параметры.ЗаписьИзмененийДляПодчиненногоУзлаРИБСФильтрами = Неопределено Тогда
		ТекстИсключения = НСтр("ru='В обработчике неправильно организована работа с параметрами регистрации данных к обработке.';uk='В обробнику неправильно організована робота з параметрами реєстрації даних до обробки.'");
...
Процедура ОбновитьРегламентныеЗаданияСценариевОбменовДанными() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Отказ Тогда
			ВызватьИсключение НСтр("ru='Ошибка при обновлении регламентного задания для сценария обмена данными.';uk='Помилка при оновленні регламентного завдання для сценарію обміну даними.'");
		КонецЕсли;
...
Процедура РассчитатьПроцентВыгрузки(КоличествоВыгруженных, КоличествоОбъектовКВыгрузке) Экспорт
	...
	Если КоличествоОбъектовКВыгрузке = 0 Тогда
		...
		Шаблон = НСтр("ru='Обработано: %1 объектов.';uk='Опрацьовано: %1 об''єктів.'");
	...
	Иначе
		...
		Шаблон = НСтр("ru='Обработано: %1 из %2 объектов.';uk='Опрацьовано: %1 з %2 об''єктів.'");
...
Процедура РассчитатьПроцентЗагрузки(КоличествоЗагруженных, КоличествоОбъектовКЗагрузке, РазмерФайлаСообщенияОбмена) Экспорт
	...
	Если РазмерФайлаСообщенияОбмена <> 0 Тогда
		Шаблон = НСтр("ru='Размер сообщения %1 МБ';uk='Розмір повідомлення %1 МБ'");
...
Функция FTPИмяСервераИПуть(Знач СтрокаПодключения) Экспорт
	...
	Если (ВРег(Лев(СтрокаПодключения, 6)) <> "FTP://"
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Строка подключения к FTP-ресурсу не соответствует формату: ""%1""';uk='Рядок підключення до FTP-ресурсу не відповідає формату: ""%1""'"), СтрокаПодключения);
	КонецЕсли;
	...
	Если ПараметрыПодключения.Количество() < 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В строке подключения к FTP-ресурсу не указано имя сервера: ""%1""';uk='У рядку підключення до FTP-ресурсу не зазначене ім''я сервера: ""%1""'"), СтрокаПодключения);
	КонецЕсли;
...
Функция ВыбратьИзменения(Знач Узел, Знач НомерСообщения, Знач ФильтрВыборки = Неопределено) Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ru='Выборка изменений данных запрещена в активной транзакции.';uk='Вибірка змін даних заборонена в активній транзакції.'");
	КонецЕсли;
...
Функция НастройкиПланаОбменаПоУмолчанию(ИмяПланаОбмена)
	...
	ЗаголовокФормыПомощника = НСтр("ru='Синхронизация данных с %Программа% (настройка)';uk='Синхронізація даних з %Программа% (настройка)'");
...
Процедура ПроверитьВозможностьВыполненияОбменов() Экспорт
	...
	Если Не Пользователи.РолиДоступны("ВыполнениеСинхронизацииДанных") Тогда
		...
		ВызватьИсключение НСтр("ru='Недостаточно прав для синхронизации данных.';uk='Недостатньо прав для синхронізації даних.'");
	...
	ИначеЕсли ОбновлениеИнформационнойБазы.НеобходимоОбновлениеИнформационнойБазы()
		...
		ВызватьИсключение НСтр("ru='Информационная база находится в состоянии обновления.';uk='Інформаційна база перебуває в стані оновлення.'");
...
Процедура ПроверитьИспользованиеОбменаДанными() Экспорт
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
		...
		ТекстСообщения = НСтр("ru='Синхронизация данных запрещена администратором.';uk='Синхронізація даних заборонена адміністратором.'");
...
Процедура УдалитьДвиженияУДокумента(ДокументОбъект) Экспорт
	...
	Для Каждого СтрокаДвижения Из ТаблицаДвижений Цикл
		...
		Если Не ПравоДоступа("Изменение", Набор.Метаданные()) Тогда
			...
			ТекстИсключения = НСтр("ru='Нарушение прав доступа: %1';uk='Порушення прав доступу: %1'");
...
Процедура ПриЗаполненииПоставляемыхПрофилейГруппДоступа(ОписанияПрофилей, ПараметрыОбновления) Экспорт
	...
	ОписаниеПрофиля.Наименование =
		НСтр("ru='Синхронизация данных с другими программами';uk='Синхронізація даних з іншими програмами'",Метаданные.ОсновнойЯзык.КодЯзыка);
	ОписаниеПрофиля.Описание =
		НСтр("ru='Дополнительно назначается тем пользователям, которым должны быть доступны средства
                   ...
                   |;uk='Додатково призначається тим користувачам, яким повинні бути доступні засоби
                   |для моніторингу й синхронізації даних з іншими програмами.'",Метаданные.ОсновнойЯзык.КодЯзыка);
...
Процедура ВыполнитьОбменДаннымиПоСценариюОбменаДанными(Отказ, НастройкаВыполненияОбмена, НомерСтроки = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СтрокаСообщения = НСтр("ru='Начало процесса обмена данными по настройке %1';uk='Початок процесу обміну даними по настройці %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьОбменДаннымиПоРегламентномуЗаданию(КодСценарияОбмена) Экспорт
	...
	Если Не ЗначениеЗаполнено(КодСценарияОбмена) Тогда
		ВызватьИсключение НСтр("ru='Не задан сценарий обмена данными.';uk='Не заданий сценарій обміну даними.'");
	КонецЕсли;
	...
	Иначе
		СтрокаСообщения = НСтр("ru='Сценарий обмена данными с кодом %1 не найден.';uk='Сценарій обміну даними з кодом %1 не знайдений.'");
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталог(Отказ, УзелИнформационнойБазы, ВидТранспортаСообщенийОбмена, ВыводитьСообщения = Истина) Экспорт
	...
	Если СтруктураНастроекОбмена.Отказ Тогда
		...
		Если ВыводитьСообщения Тогда
			НСтрока = НСтр("ru='При инициализации обработки транспорта сообщений обмена возникли ошибки.';uk='При ініціалізації обробки транспорту повідомлень обміну виникли помилки.'");
	...
	Если СтруктураНастроекОбмена.РезультатВыполненияОбмена <> Неопределено Тогда
		...
		Если ВыводитьСообщения Тогда
			НСтрока = НСтр("ru='При получении сообщений обмена возникли ошибки.';uk='При одержанні повідомлень обміну виникли помилки.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,,,, Отказ);
		КонецЕсли;
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондента(Отказ, УзелИнформационнойБазы, ВыводитьСообщения = Истина) Экспорт
	...
	Исключение
		Если ВыводитьСообщения Тогда
			Сообщение = НСтр("ru='Не удалось произвести обмен: %1';uk='Не вдалося зробити обмін: %1'");
	...
	Если Отказ Тогда
		...
		Если ВыводитьСообщения Тогда
			...
			Сообщение = НСтр("ru='Не удалось выгрузить данные: %1';uk='Не вдалося вивантажити дані: %1'");
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондентаЧерезВебСервис(
	...
	Если Прокси = Неопределено Тогда
		...
		Отказ = Истина;
		Сообщение = НСтр("ru='Ошибка при установке подключения ко второй информационной базе: %1';uk='Помилка при установці підключення до другої інформаційної бази: %1'");
	...
	Исключение
		...
		Отказ = Истина;
		Сообщение = НСтр("ru='При выгрузке данных возникли ошибки во второй информационной базе: %1';uk='При вивантаженні даних виникли помилки в другій інформаційній базі: %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(Сообщение, СтруктураНастроекОбмена, Истина);
	...
	Если ДлительнаяОперация Тогда
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(НСтр("ru='Ожидание получения данных от базы-корреспондента...';uk='Очікування одержання даних від бази-кореспондента...'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), СтруктураНастроекОбмена);
		Возврат Результат;
	КонецЕсли;
	...
	Исключение
		...
		Отказ = Истина;
		Сообщение = НСтр("ru='Возникли ошибки при получении сообщения обмена из сервиса передачи файлов: %1';uk='Виникли помилки при отриманні повідомлення обміну із сервісу передачі файлів: %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(Сообщение, СтруктураНастроекОбмена, Истина);
	...
	Исключение
		Отказ = Истина;
		Сообщение = НСтр("ru='При получении сообщения обмена возникли ошибки: %1';uk='При одержанні повідомлення обміну виникли помилки: %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(Сообщение, СтруктураНастроекОбмена, Истина);
...
Процедура ВыполнитьОбменДаннымиДляУзлаИнформационнойБазыЧерезФайлИлиСтроку(ПараметрыОбмена) Экспорт
	...
	Если ПараметрыОбмена.УзелИнформационнойБазы = Неопределено Тогда
		...
		Если ПараметрыОбмена.УзелИнформационнойБазы.Пустая()
			...
			ИначеЕсли ОшибкаПерехода Тогда
				СтрокаСообщенияОбОшибке = НСтр("ru='Не удалось выполнить переход на синхронизацию данных через универсальный формат.';uk='Не вдалося виконати перехід на синхронізацію даних через універсальний формат.'");
				ВызватьИсключение СтрокаСообщенияОбОшибке;
			КонецЕсли;
		КонецЕсли;
		
		Если ПараметрыОбмена.УзелИнформационнойБазы.Пустая() Тогда
			СтрокаСообщенияОбОшибке = НСтр("ru='Узел плана обмена %1 с кодом %2 не найден.';uk='Вузол плану обміну %1 з кодом %2 не знайдений.'");
	...
	Если СтруктураНастроекОбмена.Отказ Тогда
		СтрокаСообщенияОбОшибке = НСтр("ru='Ошибка при инициализации процесса обмена данными.';uk='Помилка при ініціалізації процесу обміну даними.'");
		ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбмена);
		ВызватьИсключение СтрокаСообщенияОбОшибке;
	КонецЕсли;
		...
		Если СтруктураНастроекОбмена.ИспользоватьПередачуБольшогоОбъемаДанных Тогда
			...
			Если РезультатВыполненияОбменаВыполнено(СтруктураНастроекОбмена.РезультатВыполненияОбмена) И НЕ Отказ Тогда
				...
				Попытка
					...
					Если ПараметрыОбмена.ДлительнаяОперация Тогда
						ЗаписьЖурналаРегистрацииОбменаДанными(НСтр("ru='Ожидание загрузки данных в базе-корреспонденте...';uk='Очікування завантаження даних у базі-кореспонденті...'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), СтруктураНастроекОбмена);
						Возврат;
					КонецЕсли;
...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазыПоВнешнемуСоединению(Отказ, УзелИнформационнойБазы,
	...
	Если ЭтоПланОбменаXDTO Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПланОбменаСсылка.Code) Тогда
			...
			ТекстСообщения = НСтр("ru='Необходим переход на синхронизацию данных через универсальный формат в базе-корреспонденте.';uk='Необхіден перехід на синхронізацію даних через універсальний формат в базі-кореспонденті.'");
			...
			Если СтруктураПараметров.Ошибка Тогда
				ЕстьОшибки = Истина;
				НСтрока = НСтр("ru='Ошибка при переходе на синхронизацию данных через универсальный формат: %1. Обмен отменен.';uk='Помилка при переході на синхронізацію даних через універсальний формат: %1. Обмін скасовано.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			...
			ИначеЕсли РезультатПерехода = Неопределено Тогда
				ЕстьОшибки = Истина;
				СтрокаСообщенияОбОшибке = НСтр("ru='Переход на синхронизацию данных через универсальный формат не выполнен';uk='Перехід на синхронізацію даних через універсальний формат не виконано'");
			КонецЕсли;
			Если ЕстьОшибки Тогда
				...
				Возврат;
			Иначе
				Сообщение = НСтр("ru='Переход на синхронизацию данных через универсальный формат завершен успешно.';uk='Перехід на синхронізацію даних через універсальний формат завершено успішно.'");
	...
	Если СтруктураНастроекОбмена.ПроизводитьЗагрузкуДанных Тогда
		Если НЕ ЭтоПланОбменаXDTO Тогда
			...
			Если ПравилаКонвертацииОбъектов = Неопределено Тогда
				...
				НСтрока = НСтр("ru='Не заданы правила конвертации во второй информационной базе для плана обмена %1. Обмен отменен.';uk='Не задані правила конвертації в другій інформаційній базі для плану обміну %1. Обмін скасовано.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Иначе
			...
			Исключение
				ЗаписьЖурналаРегистрацииОбменаДанными(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Возникла ошибка во второй информационной базе: %1';uk='Виникла помилка в другій інформаційній базі: %1'"),
...
Процедура ПередЧтениемСообщенияОбмена(Знач Получатель, СообщениеОбмена, СтандартнаяОбработка)
	...
	Если ЭтоПодчиненныйУзелРИБ()
		...
		Если ТипЗнч(СохраненноеСообщениеОбмена) = Тип("ДвоичныеДанные") Тогда
			...
			ЗаписатьСобытиеПолученияДанных(Получатель, НСтр("ru='Сообщение обмена получено из кэша.';uk='Повідомлення обміну отримане з кешу.'"));
...
Процедура ВыполнитьСтандартнуюЗагрузкуИзмененийДляУзла(
	...
	Если ЧтениеСообщения.Отправитель <> УзелИнформационнойБазы Тогда // Сообщение предназначено не для этого узла.
		...
		ЗаписьЖурналаРегистрации(КлючСообщенияЖурналаРегистрации, УровеньЖурналаРегистрации.Ошибка,
			УзелИнформационнойБазы.Метаданные(), УзелИнформационнойБазы, НСтр("ru='Сообщение обмена содержит данные для другого узла информационной базы.';uk='Повідомлення обміну містить дані для іншого вузла інформаційної бази.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		//
		Возврат;
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииУстановкаПодключенияКWebСервису() Экспорт
	...
	Возврат НСтр("ru='Обмен данными.Установка подключения к web-сервису';uk='Обмін даними. Установка підключення до web-сервісу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииЗагрузкаПравилДляОбменаДанными() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Загрузка правил';uk='Обмін даними. Завантаження правил'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииСозданиеОбменаДанными() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Создание обмена данными';uk='Обмін даними.Створення обміну даними'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииУдалениеВременногоФайла() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Удаление временного файла';uk='Обмін даними.Вилучення тимчасового файлу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииОбменДанными() Экспорт
	
	Возврат НСтр("ru='Обмен данными';uk='Обмін даними'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ПолучитьФайлИзХранилищаВСервисе(Знач ИдентификаторФайла, Знач УзелИнформационнойБазы, Знач РазмерЧасти = 1024, Знач ПараметрыАутентификации = Неопределено) Экспорт
	...
	Иначе
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Начало получения сообщения обмена из Интернета (количество частей файла %1).';uk='Початок одержання повідомлення обміну з Інтернету (кількість частин файлу %1).'"),
		...
		Если Разархиватор.Элементы.Количество() = 0 Тогда
			...
			Исключение
				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииУдалениеВременногоФайла(),
					УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
			ВызватьИсключение(НСтр("ru='Файл архива не содержит данных.';uk='Файл архіву не містить даних.'"));
		КонецЕсли;
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Окончание получения сообщения обмена из Интернета (размер сжатого сообщения обмена %1 Мб).';uk='Закінчення одержання повідомлення обміну з Інтернету (розмір стислого повідомлення обміну %1 Мб).'"),
...
Процедура ПриПолученииФайлаИзХранилища(Знач ИдентификаторФайла, ИмяФайла)
	...
	Если РезультатЗапроса.Пустой() Тогда
		Описание = НСтр("ru='Файл с идентификатором %1 не обнаружен.';uk='Файл із ідентифікатором %1 не виявлений.'");
...
Процедура ВыполнитьКонтрольЗаписиНеразделенныхДанных(Знач Данные) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		ПредставлениеИсключения = НСтр("ru='Нарушение прав доступа!';uk='Порушення прав доступу!'");
		ТекстИсключения = НСтр("ru='Нарушение прав доступа!';uk='Порушення прав доступу!'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьИнициализациюСтруктурыНастроекОбмена(СтруктураНастроекОбмена, НастройкаВыполненияОбмена, НомерСтроки)
	...
	СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = НСтр("ru='Обмен данными';uk='Обмін даними'");
...
Процедура ВыполнитьПроверкуОсновныхПолейСтруктурыНастроекОбмена(СтруктураНастроекОбмена)
	...
	Если НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.УзелИнформационнойБазы) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Не задан узел информационной базы с которым нужно производить обмен информацией. Обмен отменен.';uk='Не заданий вузол інформаційної бази з яким потрібно проводити обмін інформацією. Обмін скасовано.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
		
	ИначеЕсли НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.ВидТранспортаОбмена) Тогда
		
		СтрокаСообщенияОбОшибке = НСтр("ru='Не задан вид транспорта обмена. Обмен отменен.';uk='Не заданий вид транспорту обміну. Обмін скасовано.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.ДействиеПриОбмене) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Не указано выполняемое действие (выгрузка / загрузка). Обмен отменен.';uk='Не зазначена виконувана дія (вивантаження / завантаження). Обмін відмінний.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
Процедура ВыполнитьПроверкуСтруктурыОбменаНаВалидность(СтруктураНастроекОбмена, ИспользоватьНастройкиТранспорта = Истина)
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы.ПометкаУдаления Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Узел информационной базы помечен на удаление. Обмен отменен.';uk='Вузол інформаційної бази позначений на вилучення. Обмін скасовано.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы = СтруктураНастроекОбмена.ТекущийУзелПланаОбмена Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Нельзя организовать обмен данными с текущим узлом информационной базы. Обмен отменен.';uk='Не можна організувати обмін даними з поточним вузлом інформаційної бази. Обмін скасовано.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли ПустаяСтрока(СтруктураНастроекОбмена.УзелИнформационнойБазыКод)
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Один из узлов обмена имеет пустой код. Обмен отменен.';uk='Один з вузлів обміну має порожній код. Обмін скасовано.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.ОтладкаОбработчиковВыгрузки Тогда
		...
		Если Не ФайлОбработкиВыгрузки.Существует() Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Файл внешней обработки для отладки выгрузки не существует. Обмен отменен.';uk='Файл зовнішньої обробки для налагодження вивантаження не існує. Обмін відмінний.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.ОтладкаОбработчиковЗагрузки Тогда
		...
		Если Не ФайлОбработкиЗагрузки.Существует() Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Файл внешней обработки для отладки загрузки не существует. Обмен отменен.';uk='Файл зовнішньої обробки для налагодження завантаження не існує. Обмін відмінний.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
Процедура УстановитьПравилаОбменаВыгрузкиДанных(ОбработкаОбменаДаннымиXML, СтруктураНастроекОбмена)
	...
	Если ПравилаКонвертацииОбъектов = Неопределено Тогда
		...
		// Правила обмена должны быть указаны.
		НСтрока = НСтр("ru='Не заданы правила конвертации для плана обмена %1. Выгрузка данных отменена.';uk='Не задані правила конвертації для плану обміну %1. Вивантаження даних скасоване.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура УстановитьПравилаОбменаЗагрузкиДанных(ОбработкаОбменаДаннымиXML, СтруктураНастроекОбмена)
	...
	Если ПравилаКонвертацииОбъектов = Неопределено Тогда
		...
		// Правила обмена должны быть указаны.
		НСтрока = НСтр("ru='Не заданы правила конвертации для плана обмена %1. Загрузка данных отменена.';uk='Не задані правила конвертації для плану обміну %1. Завантаження даних скасоване.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтрока, СтруктураНастроекОбмена.ИмяПланаОбмена);
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
...
Процедура ЗарегистрироватьОшибкуПроведенияДокумента(Объект, УзелОбмена, ТекстИсключения) Экспорт
	...
	Если Не ПустаяСтрока(СокрЛП(ТекстСообщения)) Тогда
		...
		ПричинаОшибки = " " + НСтр("ru='По причине %1.';uk='Через %1.'");
	...
	СтрокаСообщения = НСтр("ru='Не удалось провести документ %1, полученный из другой информационной базы.%2
        ...
        |;uk='Не вдалося провести документ %1, отриманий з іншої інформаційної бази. %2
        |Можливо не заповнені всі реквізити, обов''язкові до заповнення.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ЗарегистрироватьОшибкуЗаписиОбъекта(Объект, УзелОбмена, ТекстИсключения) Экспорт
	...
	СтрокаСообщения = НСтр("ru='Не удалось записать объект %1, полученный из другой информационной базы.%2
        |Возможно не заполнены все реквизиты, обязательные к заполнению.'
        |;uk='Не вдалося записати об''єкт %1, отриманий з іншої інформаційної бази. %2
        |Можливо не заповнені всі реквізити, обов''язкові до заповнення.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, Строка(Объект), ПричинаОшибки);
...
Процедура ПроверитьВозможностьАдминистрированияОбменов() Экспорт
	...
	Если Не ЕстьПраваНаАдминистрированиеОбменов() Тогда
		...
		ВызватьИсключение НСтр("ru='Недостаточно прав для администрирования синхронизации данных.';uk='Недостатньо прав для адміністрування синхронізації даних.'");
...
Процедура ПроверитьВозможностьВнешнегоСоединения()
	...
	Если ОбщегоНазначения.ЭтоLinuxСервер() Тогда
		...
		ВызватьИсключение НСтр("ru='Синхронизация данных через прямое подключение на сервере под управлением ОС Linux недоступно.
            ...
            |;uk='Синхронізація даних через пряме підключення на сервері під управлінням ОС Linux недоступно.
            |Для синхронізації даних через пряме підключення потрібно використовувати ОС Windows.'");
...
Процедура ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбмена) Экспорт
	...
	Если СтруктураНастроекОбмена.ЭтоОбменВРИБ Тогда
		СтрокаСообщения = НСтр("ru='%1, %2';uk='%1, %2'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Иначе
		СтрокаСообщения = НСтр("ru='%1, %2; Объектов обработано: %3';uk='%1, %2; Об''єктів оброблено: %3'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
							СтруктураНастроекОбмена.РезультатВыполненияОбмена,
							СтруктураНастроекОбмена.ДействиеПриОбмене,
...
Функция РаспаковатьZipФайл(Знач ПолноеИмяФайлаАрхива, Знач ПутьРаспаковкиФайлов, Знач ПарольАрхива = "") Экспорт
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ru='Ошибка при распаковке файлов архива: %1 в каталог: %2';uk='Помилка при розпакуванні файлів архіву: %1 у каталог: %2'");
...
Функция ЗапаковатьВZipФайл(Знач ПолноеИмяФайлаАрхива, Знач МаскаУпаковкиФайлов, Знач ПарольАрхива = "") Экспорт
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ru='Ошибка при запаковке файлов архива: %1 из каталог: %2';uk='Помилка при запакуванні файлів архіву: %1 з каталог: %2'");
...
Функция ПолучитьКлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, ДействиеПриОбмене) Экспорт
	...
	КлючСообщения = НСтр("ru='Обмен данными.[ИмяПланаОбмена].[ДействиеПриОбмене]';uk='Обмін даними.[ИмяПланаОбмена].[ДействиеПриОбмене]'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПроверитьОбязательныеРеквизитыФормы(Форма, Знач Реквизиты)
	...
	Если ОтсутствующиеРеквизиты.Количество() > 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Отсутствуют обязательные реквизиты формы настройки узла: %1';uk='Відсутні обов''язкові реквізити форми настройки вузла: %1'"),
...
Процедура ВнешнееСоединениеОбновитьНастройкиОбменаДанными(Знач ИмяПланаОбмена, Знач КодУзла, Знач ЗначенияПоУмолчаниюНаУзле) Экспорт
	...
	Если Не ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		Сообщение = НСтр("ru='Не найден узел плана обмена; имя плана обмена %1; код узла %2';uk='Не знайдений вузол плану обміну; ім''я плану обміну %1; код вузла %2'");
...
Процедура ВыполнитьПроверкуНаличияПравилОбменаЗагруженныхИзФайла(ПравилаОбменаЗагруженныеИзФайла, ПравилаРегистрацииЗагруженныеИзФайла)
	...
	Если Не Результат.Пустой() Тогда
		...
		СтрокаСообщения = НСтр("ru='Для планов обмена %1 используются правила обмена, загруженные из файла.
                ...
                |;uk='Для планів обміну %1 використовуються правила обміну, завантажений з файлу.
                |Ці правила можуть бути несумісні з новою версією програми.
                |Для попередження можливого виникнення помилок при роботі з програмою рекомендується актуалізувати правила обміну з файлу.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПроверитьПодключениеОбработкиТранспортаСообщенийОбмена(Отказ, СтруктураНастроек, ВидТранспорта, СообщениеОбОшибке = "") Экспорт
	...
	Если Не ОбработкаОбъект.ПодключениеУстановлено() Тогда
		...
		ДополнительноеСообщение = НСтр("ru='Техническую информацию об ошибке см. в журнале регистрации.';uk='Технічну інформацію про помилку див. у журналі реєстрації.'");
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Транспорт сообщений обмена';uk='Транспорт повідомлень обміну'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция УстановитьВнешнееСоединениеСБазой(СтруктураНастроек) Экспорт
	...
	Если НетПолныхПрав Тогда
		Результат.ПодробноеОписаниеОшибки = НСтр("ru='Пользователю, указанному для подключения к другой программе, должны быть назначены роли ""Администратор системы"" и ""Полные права""';uk='Користувачеві, вказаному для підключення до іншого програмі, повинні бути призначені ролі ""Адміністратор системи"" і ""Повні права""'");
	...
	Иначе
		...
		Если СостояниеНеДопустимо Тогда
			Результат.ПодробноеОписаниеОшибки = НСтр("ru='Другая программа находится в состоянии обновления.';uk='Інша програма перебуває в стані оновлення.'");
			Результат.КраткоеОписаниеОшибки   = Результат.ПодробноеОписаниеОшибки;
			Результат.Соединение = Неопределено;
		КонецЕсли;
...
Функция ЕстьПодключениеККорреспонденту(Знач Корреспондент, Знач СтруктураНастроек, СообщениеПользователю = "") Экспорт
	...
	СобытиеЖурналаРегистрации = НСтр("ru='Обмен данными.Проверка подключения';uk='Обмін даними.Перевірка підключення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьОбновлениеВерсииТиповыхПравилДляОбменаДанными(ПравилаОбменаЗагруженныеИзФайла, ПравилаРегистрацииЗагруженныеИзФайла)
	...
	Для Каждого ЗаписьПланаОбмена Из Результат Цикл
		...
		Если ПравилаОбменаЗагруженныеИзФайла.Найти(ИмяПланаОбмена) = Неопределено
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выполняется обновление правил конвертации данных для плана обмена %1';uk='Виконується оновлення правил конвертації даних для плану обміну %1'"), ИмяПланаОбмена);
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииОбменДанными(),
		...
		Если ПравилаРегистрацииЗагруженныеИзФайла.Найти(ИмяПланаОбмена) = Неопределено
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выполняется обновление правил регистрации данных для плана обмена %1';uk='Виконується оновлення правил реєстрації даних для плану обміну %1'"), ИмяПланаОбмена);
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииОбменДанными(),
				УровеньЖурналаРегистрации.Информация,,, ТекстСообщения);
	...
	Если Отказ Тогда
		ВызватьИсключение НСтр("ru='При обновлении правил обмена данными возникли ошибки (см. Журнал регистрации).';uk='При оновлення правил обміну даними виникли помилки (див. Журнал реєстрації).'");
	КонецЕсли;
	...
	Если Файл.Существует() И Файл.ЭтоФайл() Тогда
		...
		Если МаксимальныйДопустимыйРазмерСообщения <> 0 Тогда
			...
			Если РазмерПакета > МаксимальныйДопустимыйРазмерСообщения Тогда
				...
				СтрокаСообщения = НСтр("ru='Размер исходящего пакета составил %1 Кбайт, что превышает допустимое ограничение %2 Кбайт.';uk='Розмір вихідного пакета склав %1 Кбайт, що перевищує припустиме обмеження %2 Кбайт.'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
		...
		Если УзелИнформационнойБазы <> Неопределено Тогда
			...
			Попытка
				...
				Если Отказ Тогда
					ВызватьИсключение НСтр("ru='Получение данных из главного узла завершилось с ошибками.';uk='Одержання даних з головного вузла завершилося з помилками.'");
				КонецЕсли;
			Исключение
				УстановитьПривилегированныйРежим(Истина);
				УстановитьРежимЗагрузкиСообщенияОбменаДаннымиПередЗапуском("ЗагрузкаРазрешена", Ложь);
			...
			Иначе
				...
				ЗаписатьСобытиеПолученияДанных(ГлавныйУзел(),
					НСтр("ru='Обнаружен возврат к конфигурации базы данных.
                               ...
                               |;uk='Виявлено повернення до конфігурації бази даних.
                               |Синхронізація скасована.'"));
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Функция РазличаютсяВерсииКорреспондента(ИмяПланаОбмена, КлючСообщенияЖурналаРегистрации, ВерсияВЭтойПрограмме,
	...
	Если ЗначениеЗаполнено(ВерсияВЭтойПрограмме) И ЗначениеЗаполнено(ВерсияВДругойПрограмме)
		...
		Если ВерсияВЭтойПрограммеБезНомераСборки <> ВерсияВДругойПрограммеБезНомераСборки Тогда
			...
			ШаблонСообщения = НСтр("ru='Синхронизация данных может быть выполнена некорректно, т.к. версия программы ""%1"" (%2) в правилах конвертации этой программы отличается от версии %3 в правилах конвертации в другой программе. Убедитесь, что загружены актуальные правила, подходящие для используемых версий обеих программ.';uk='Синхронізація даних може бути виконана некоректно, оскільки версія програми ""%1"" (%2) у правилах конвертації цієї програми відрізняється від версії %3 в правилах конвертації в іншій програмі. Переконайтеся, що завантажені актуальні правила, відповідні для використовуваних версій обох програм.'");
...
Процедура ПриЗаполненииСпискаТекущихДелПредупрежденияСинхронизации(ТекущиеДела)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ru='Предупреждения при синхронизации';uk='Попередження при синхронізації'");
...
Процедура ПриЗаполненииСпискаТекущихДелНеобходимоОбновление(ТекущиеДела)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Важное         = Истина;
		Дело.Представление  = НСтр("ru='Обновить версию программы';uk='Оновити версію програми'");
		...
		Иначе
			...
			Дело.ПараметрыФормы = Новый Структура("Заголовок,ИмяМакета",
				НСтр("ru='Установка обновления';uk='Установка оновлення'"), "ИнструкцияКакВыполнитьУстановкуОбновленияВручную");
		КонецЕсли;
		Дело.Владелец       = Раздел;
...
Процедура ПриЗаполненииСпискаТекущихДелПроверитьСовместимостьСТекущейВерсией(ТекущиеДела)
	...
	Дело.Представление = НСтр("ru='Правила обмена';uk='Правила обміну'");
	...
	Если ГруппаДела = Неопределено Тогда
		...
		ГруппаДела.Представление = НСтр("ru='Проверить совместимость';uk='Перевірити сумісність'");
...
Процедура УстановитьСообщениеОбменаДаннымиИзГлавногоУзла(СообщениеОбмена, ГлавныйУзел) Экспорт
	...
	ЗаписатьСобытиеПолученияДанных(ГлавныйУзел, НСтр("ru='Сообщение обмена записано в кэш.';uk='Повідомлення обміну записане в кеш.'"));
...
Процедура ОчиститьСообщениеОбменаДаннымиИзГлавногоУзла() Экспорт
	...
	ЗаписатьСобытиеПолученияДанных(ГлавныйУзел(), НСтр("ru='Сообщение обмена удалено из кэша.';uk='Повідомлення обміну вилучено з кешу.'"));
...
Функция СловарьКонтейнераВнешнегоМодуля() Экспорт
	...
	Результат.Вставить("Именительный", НСтр("ru='Настройка синхронизация данных';uk='Настройка синхронізація даних'"));
	Результат.Вставить("Родительный", НСтр("ru='Настройки синхронизации данных';uk='Настройки синхронізації даних'"));
...
Функция ИнтерактивноеИзменениеВыгрузки(Знач УзелИнформационнойБазы, Знач АдресХранилищаФормы, Знач ЕстьСценарийУзла=Неопределено) Экспорт
	...
	ПараметрыСценарияДополнения.ВариантБезДополнения.Вставить("Пояснение", 
		НСтр("ru='Будут отправлены только данные согласно общим настройкам.';uk='Будуть відправлені тільки дані відповідно до загальних настройок'")); 
	...
	ПараметрыСценарияДополнения.ВариантВсеДокументы.Вставить("Пояснение",
		НСтр("ru='Дополнительно будут отправлены все документы за период, удовлетворяющие условиям отбора.';uk='Додатково будуть відправлені всі документи за період, що задовольняють умовам відбору.'")); 
	...
	ПараметрыСценарияДополнения.ВариантПроизвольныйОтбор.Вставить("Пояснение",
		НСтр("ru='Дополнительно будут отправлены данные согласно отбору.';uk='Додатково будуть відправлені дані відповідно до відбору.'")); 
	...
	ПараметрыСценарияДополнения.ВариантДополнительно.Вставить("Пояснение",
		НСтр("ru='Будут отправлены дополнительные данные по настройкам.';uk='Будуть відправлені додаткові дані по настройкам.'")); 
...
Функция ПредставлениеОтбораДополненияВыгрузки(Знач Период, Знач Отбор, Знач ОписаниеПустогоОтбора=Неопределено) Экспорт
	...
	Если ПустаяСтрока(ОтборСтрокой) Тогда
		Если ОписаниеПустогоОтбора=Неопределено Тогда
			ОтборСтрокой = НСтр("ru='Все объекты';uk='Всі об''єкти'");
		Иначе
...
Функция ПредставлениеДетальногоДополненияВыгрузки(Знач ДополнительнаяРегистрация, Знач ОписаниеПустогоОтбора=Неопределено) Экспорт
	...
	ИначеЕсли ОписаниеПустогоОтбора=Неопределено Тогда
		Возврат НСтр("ru='Дополнительные данные не выбраны';uk='Додаткові дані не обрані'");
...
Функция ДополнениеВыгрузкиИмяАвтоСохраненияНастроек() Экспорт
	Возврат НСтр("ru='Последняя отправка (сохраняется автоматически)';uk='Останнє відправлення (зберігається автоматично)'");
КонецФункции
