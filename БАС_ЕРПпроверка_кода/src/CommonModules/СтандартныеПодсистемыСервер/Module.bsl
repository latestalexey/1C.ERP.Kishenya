...
Процедура ОбновитьВсеПараметрыРаботыПрограммы() Экспорт
	...
	ОбновитьПараметрыРаботыПрограммы(Ложь);
...
Процедура СохранитьПараметрыАутентификацииНаСайте(СохраняемыеПараметры) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.Проверить(Не ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей"), 
		НСтр("ru='Недопустимый вызов процедуры.';uk='Неправильний виклик процедури.'"), "СтандартныеПодсистемыСервер.СохранитьПараметрыАутентификацииНаСайте");
	...
	Если ВключитьВСостав.Количество() <> 0
		...
		Если ВключитьВСостав.Количество() <> 0 Тогда
			...
			ОписаниеИсключения1 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В состав плана обмена %1 должны входить следующие объекты метаданных: %2';uk=' До складу плану обміну %1 повинні входити наступні об''єкти метаданих: %2'"),
				ИмяПланаОбмена,
		...
		Если ИсключитьИзСостава.Количество() <> 0 Тогда
			...
			ОписаниеИсключения2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В состав плана обмена %1 НЕ должны входить следующие объекты метаданных: %2';uk=' До складу плану обміну %1 НЕ повинні входити наступні об''єкти метаданих: %2'"),
				ИмяПланаОбмена,
		...
		Если ОтключитьАвторегистрацию.Количество() <> 0 Тогда
			...
			ОписаниеИсключения3 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В составе плана обмена %1 не должно быть объектов с установленным признаком авторегистрации.
                    ...
                    |;uk='У складі плану обміну %1 не повинне бути об''єктів із установленою ознакою авторегістрації.
                    |Потрібно заборонити авторегістрацію для наступних об''єктів метаданих: %2'"),
				ИмяПланаОбмена,
...
Функция ПараметрыКлиентаНаСервере() Экспорт
	...
	Если ПараметрыКлиента.Количество() = 0
	   И ТекущийРежимЗапуска() <> Неопределено Тогда
		...
		ВызватьИсключение НСтр("ru='Не заполнены параметры клиента на сервере.';uk='Не заповнені параметри клієнта на сервері.'");
	КонецЕсли;
...
Процедура ПроверитьОбновлениеПараметровРаботыПрограммы(ИмяКонстанты, ИменаПараметров = "", Отказ = Неопределено) Экспорт
	...
	Если ИменаПараметров <> "" Тогда
		...
		Если ТребуетсяОбновление Тогда
			...
			Если ТекущийРежимЗапуска() = Неопределено Тогда
				ВызватьИсключение
					НСтр("ru='Вход в программу временно невозможен в связи с обновлением на новую версию.';uk='Вхід у програму тимчасово неможливий у зв''язку з оновленням на нову версію.'");
			Иначе
				ВызватьИсключение
					НСтр("ru='Недопустимое обращение к необновленным параметрам работы программы
                               ...
                               |;uk='Неприпустиме звертання до неоновлених параметрів роботи програми
                               |(наприклад, до деяких параметрів сеансу):
                               |- якщо цей обіг виконується з форми на початковій сторінці
                               |  (робочому столі), те необхідно переконатися, що в ній є виклик
                               |  процедури ОбщегоНазначения.ПриСозданииНаСервере;
                               |- в інших випадках необхідно перенести виклик прикладного коду
                               |  після оновлення параметрів роботи програми.'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
			...
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				...
				ИмяСобытия = НСтр("ru='Идентификаторы объектов метаданных.Требуется загрузить критичные изменения';uk='Ідентифікатори об''єктів метаданих.Потрібно завантажити критичні зміни'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
				...
				ТекстОшибки =
					НСтр("ru='Информационная база не может быть обновлена из-за проблемы в главном узле:
                               ...
                               |%1'
                               |;uk='Інформаційна база не може бути оновлена через проблеми в головному вузлі:
                               |- головний вузол був некоректно оновлено (можливо не був збільшений номер версії конфігурації,
                               |через це не заповнився довідник Ідентифікатори об''єктів метаданих);
                               |- або були скасовані до вивантаження пріоритетні дані (елементи
                               |довідника Ідентифікатори об''єктів метаданих).
                               |
                               |Необхідно виконати оновлення головного вузла, зареєструвати до вивантаження
                               |пріоритетні дані та повторити синхронізацію даних:
                               |- в головному вузлі запустіть програму з параметром /C ЗапуститьОбновлениеИнформационнойБазы;
                               |%1'");
				...
				Если ВыполнитьЗагрузку Тогда
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
						НСтр("ru='- затем повторите синхронизацию данных с этой информационной базой
                                   ...
                                   |;uk='- потім повторіть синхронізацію даних з цією інформаційною базою
                                   |(спочатку в головному вузлі, потім в цій інформаційній базі після перезапуску).'"));
				Иначе
					// Настройка подчиненного узла РИБ при первом запуске.
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
						НСтр("ru='- затем повторите создание подчиненного узла.';uk='- потім повторіть створення підпорядкованого вузла.'"));
				КонецЕсли;
		...
		ОбновитьПараметрыРаботыПрограммы(Ложь, ОшибкаУстановкиМонопольногоРежима, ВФоне);
...
// а также из формы установки расширений конфигурации в пользовательском режиме.
//
// Параметры:
...
Процедура ЗарегистрироватьОбъектНаВсехУзлах(Знач Объект, Знач ИмяПланаОбмена) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
			ВызватьИсключение НСтр("ru='Регистрация изменений неразделенных данных в разделенном режиме.';uk='Реєстрація змін нерозділених даних у розділеному режимі.'");
		КонецЕсли;
		...
		Если Не ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных("ПланОбмена." + ИмяПланаОбмена,
			ОбщегоНазначенияПовтИсп.РазделительОсновныхДанных()) Тогда
				ВызватьИсключение НСтр("ru='Регистрация изменений для неразделенных планов обмена не поддерживается.';uk='Реєстрація змін для нерозділених планів обміну не підтримується.'");
		КонецЕсли;
		
		Если ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(Объект.Метаданные().ПолноеИмя(),
			ОбщегоНазначенияПовтИсп.РазделительОсновныхДанных()) Тогда
				ВызватьИсключение НСтр("ru='Регистрация изменений для разделенных объектов не поддерживается.';uk='Реєстрація змін для розділених об''єктів не підтримується.'");
		КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешения.Добавить(РаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаВременныхФайлов(Истина, Истина,
		НСтр("ru='Для возможности работы программы.';uk='Для можливості роботи програми.'")));
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Важное        = Истина;
	Дело.Представление = НСтр("ru='Скорость работы снижена';uk='Швидкість роботи знижена'");
	Дело.Форма         = "Обработка.РекомендацияПоПовышениюСкоростиРаботы.Форма.Форма";
	Дело.Владелец      = НСтр("ru='Скорость работы программы';uk='Швидкість роботи програми'");
...
Процедура ОчиститьКаталогВременныхФайлов(ПутьККаталогу) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Стандартные подсистемы';uk='Стандартні підсистеми'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка очистки каталога временных файлов ""%1"":%2';uk='Помилка очищення каталогу тимчасових файлів ""%1"":%2'"),
				ПутьККаталогу,
...
Функция УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика() Экспорт
	...
	Возврат Символы.ПС + Символы.ПС 
		+ НСтр("ru='Для разработчика: возможно требуется обновить вспомогательные данные,
                    ...
                    |- либо запустить программу с параметром командной строки платформы
                    ...
                    |;uk='Для розробника: можливо потрібно оновити допоміжні дані,
                    |які впливають на роботу програми. Для виконання оновлення:
                    |- скористатися зовнішньою обробкою
                    |""Інструменти розробника: Оновлення допоміжних даних"",
                    |- або запустити програму з параметром командного рядка платформи
                    |""/С ЗапуститьОбновлениеИнформационнойбазы"",
                    |- або збільшити номер версії конфігурації, щоб при черговому запуску
                    |виконано процедури оновлення даних інформаційної бази.'");
...
Функция ДобавитьЭлементУсловногоОформления(Форма, Инструкция) Экспорт
	...
	Для Каждого Поле Из Поля Цикл
		...
		Если РежимОтладки И Форма.Элементы.Найти(ИмяЭлемента) = Неопределено Тогда
			Шаблон = НСтр("ru='Ошибка добавления условного оформления: Не найден элемент ""%1"".';uk='Помилка додавання умовного оформлення: Не знайдений елемент ""%1"".'");
...
Функция ДобавитьПараметрыРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Отказ = Ложь;
	Если ЭтоВызовПередНачаломРаботыСистемы Тогда
		ПользователиСлужебный.ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры, Отказ);
	КонецЕсли;
	Если Отказ Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ОбработчикиСобытий() Экспорт
	...
	Если ОбязательныеСобытияБезОбработчиков.Количество() > 0 Тогда
		ИмяСобытия  = НСтр("ru='Обработчики событий';uk='Обробники подій'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		
		Комментарий = НСтр("ru='Для следующих обязательных событий не определены обработчики:';uk='Для наступних обов''язкових подій не визначені обробники:'")
		...
		ВызватьИсключение НСтр("ru='Для обязательных событий не определены обработчики.
                                     ...
                                     |;uk='Для обов''язкових подій не визначені обробники.
...
Процедура ЗапретитьПометкуУдаленияПредопределенныхЭлементовПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Источник.ЭтоНовый() Тогда
		ВызватьИсключение
			НСтр("ru='Недопустимо создавать предопределенный элемент помеченный на удаление.';uk='Неприпустимо створювати напередвизначений елемент відмічений для вилучення.'");
	Иначе
		   ...
		   И СтарыеСвойства.ПометкаУдаления <> Истина Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Недопустимо помечать на удаление предопределенный элемент:
                           |""%1"".'
                           |;uk='Неприпустимо відмічати для вилучення напередвизначений елемент:
                           |""%1"".'"),
		...
		ИначеЕсли СтарыеСвойства.ИмяПредопределенныхДанных = ""
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Недопустимо связывать с именем предопределенного элемент, помеченный на удаление:
                           |""%1"".'
                           |;uk='Неприпустимо пов''язувати з ім''ям напередвизначеного елемент, який відмічений для вилучення:
                           |""%1"".'"),
				Строка(Источник.Ссылка));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗапретитьУдалениеПредопределенныхЭлементовПередУдалением(Источник, Отказ) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Недопустимо удалять предопределенный элемент
                   |""%1"".'
                   |;uk='Неприпустимо видаляти напередвизначений елемент
                   |""%1"".'"),
		Строка(Источник.Ссылка));
...
Процедура ПередЗапускомПрограммы()
	...
	Если Метаданные.ВариантВстроенногоЯзыка <> Метаданные.СвойстваОбъектов.ВариантВстроенногоЯзыка.Русский Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Вариант встроенного языка конфигурации ""%1"" не поддерживается.
                       ...
                       |;uk='Варіант убудованої мови конфігурації ""%1"" не підтримується.
                       |Необхідно використовувати варіант мови ""%2"".'"),
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, МинимальнаяВерсияПлатформы) < 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для запуска необходима версия платформы %1 или выше.';uk='Для запуску необхідна версія платформи %1 або вище.'"), МинимальнаяВерсияПлатформы);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(НедоступныйРежим) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Режим совместимости конфигурации с платформой версии %1 не поддерживается.
                       |Для запуска установите в конфигурации режим совместимости с платформой
                       |версии не ниже 8.3.6 или ""Не использовать"".'
                       |;uk='Режим сумісності конфігурації з платформою версії %1 не підтримується.
                       |Для запуску встановіть в конфігурації режим сумісності з платформою
                       |версії не нижче 8.3.6 або ""Не використовувати"".'"),
			НедоступныйРежим);
	КонецЕсли;
	
	// Проверка заполнения версии конфигурации.
	Если ПустаяСтрока(Метаданные.Версия) Тогда
		ВызватьИсключение НСтр("ru='Не заполнено свойство конфигурации Версия.';uk='Не заповнена властивість конфігурації Версія.'");
	Иначе
		Попытка
		...
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не правильно заполнено свойство конфигурации Версия: ""%1"".
                           ...
                           |;uk='Не правильно заповнена властивість конфігурації Версія: ""%1"".
                           |Правильний формат, наприклад: ""1.2.3.45"".'"),
				Метаданные.Версия);
		КонецПопытки;
	 ...
	 Или Не Метаданные.ОсновныеРоли.Содержит(Метаданные.Роли.ПолныеПрава) Тогда
		ВызватьИсключение
			НСтр("ru='В конфигурации в свойстве ОсновныеРоли не указаны стандартные роли
                       ...
                       |;uk='У конфігурації у властивості ОсновныеРоли не зазначені стандартні ролі
                       |АдминистраторСистемы й ПолныеПрава або зазначені зайві ролі.'");
	КонецЕсли;
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при подготовке списка событий.
                           ...
                           |Событие ""%1""
                           |уже добавлено.'
                           |;uk='Помилка при підготовці списку подій.
                           |
                           |Подія ""%1""
                           |уже додано.'"),
				Событие);
				 ...
				 ИЛИ НЕ ЗначениеЗаполнено(Обработчик.Модуль) Тогда
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Ошибка при подготовке обработчиков события
                                   ...
                                   |;uk='Помилка при підготовці обробників події
                                   |""%1"".
                                   |
                                   |Помилка в імені модуля ""%2"".'"),
						Событие,
						Обработчик.Модуль);
				КонецЕсли;
				...
				Иначе
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Ошибка при подготовке обработчиков события
                                   ...
                                   |""%1"".
                                   |
                                   |Обробник ""%2"" уже доданий для події
                                   |""%3"".'"),
						Событие,
						ИмяОбработчика,
...
Процедура ОбновитьПараметрыРаботыПрограммы(НайтиИзменения = Ложь,
	...
	Попытка
		ПроверитьОбновитьПараметрыРаботыПрограммы(ЕстьИзменения, ТолькоПроверка, БезИзменений, ВФоне);
	Исключение
		Если СнятьМонопольныйРежим Тогда
			УстановитьМонопольныйРежим(Ложь);
		КонецЕсли;
	...
	Если Не НайтиИзменения И ТолькоПроверка Тогда
		Если ЕстьИзменения Тогда
			...
			КонецПопытки;
			Попытка
				ОбновитьПараметрыРаботыПрограммы(ЕстьИзменения);
			Исключение
				УстановитьМонопольныйРежим(Ложь);
				ВызватьИсключение;
			КонецПопытки;
		...
		Иначе
			...
			ПроверитьОбновитьПараметрыРаботыПрограммы(ЕстьИзменения, Ложь, БезИзменений, ВФоне);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьОбновитьПараметрыРаботыПрограммы(ЕстьИзменения, ТолькоПроверка, БезИзменений, ВФоне)
	...
	Если НЕ БезИзменений.Свойство("БазоваяФункциональностьИдентификаторыОбъектовМетаданных") Тогда
		...
		Иначе
			...
			Если ТолькоПроверка Тогда
				...
				Справочники.ИдентификаторыОбъектовМетаданных.ВыполнитьОбновлениеДанных(Ложь, ЕстьТекущиеИзменения, ТолькоПроверка, ЕстьТекущиеИзменения);
			Иначе
				Справочники.ИдентификаторыОбъектовМетаданных.ВыполнитьОбновлениеДанных(ЕстьТекущиеИзменения, Ложь, Ложь);
			КонецЕсли;
			Если ЕстьТекущиеИзменения Тогда
				ЕстьИзменения = Истина;
				Если ТолькоПроверка Тогда
