...
Процедура ЗаблокироватьТекущуюОбластьДанных(Знач ПроверитьОтсутствиеДругихСеансов = Ложь, Знач РазделяемаяБлокировка = Ложь) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение(НСтр("ru='Блокировка области может быть установлена только при включенном использовании разделителей';uk='Блокування області може бути встановлена тільки при включеному використанні роздільників'"));
	КонецЕсли;
		...
		Исключение
			...
			Если ТекущаяПопытка = КоличествоПопыток Тогда
				ШаблонКомментария = НСтр("ru='Не удалось установить блокировку области данных по причине:
                    |%1'
                    |;uk='Не вдалося встановити блокування області даних через:
                    |%1'");
		...
		Если КонфликтующиеСеансы.Количество() > 0 Тогда
			...
			Для Каждого КонфликтующийСеанс Из КонфликтующиеСеансы Цикл
				...
				ТекстСеансы = ТекстСеансы + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='%1 (сеанс - %2)';uk='%1 (сеанс - %2)'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			...
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Операция не может быть выполнена, т.к. в приложении работают другие пользователи: %1';uk='Операція не може бути виконана, т. я. в додатку працюють інші користувачі: %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				ТекстСеансы);
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СменитьСтатусОбластиИУведомитьМенеджер(Менеджер, ?(Менеджер.Статус = Перечисления.СтатусыОбластейДанных.Новая,
				"ФатальнаяОшибка", "ОшибкаУдаления"), НСтр("ru='Исчерпано количество попыток обработки области';uk='Вичерпана кількість спроб обробки області'"));
...
Процедура УстановитьРазделениеСеанса(Знач Использование = Неопределено, Знач ОбластьДанных = Неопределено) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
		ВызватьИсключение(НСтр("ru='Изменить разделение сеанса возможно только из сеанса запущенного без указания разделителей';uk='Змінити поділ сеансу можливе лише з сеансу запущеного без вказівки роздільників'"));
	КонецЕсли;
...
Функция ЗначениеРазделителяСеанса() Экспорт
	...
	Иначе
		Если НЕ ОбщегоНазначения.ИспользованиеРазделителяСеанса() Тогда
			ВызватьИсключение(НСтр("ru='Не установлено значение разделителя';uk='Не встановлене значення роздільника'"));
		КонецЕсли;
...
Процедура ОбработатьИнформациюОбОшибкеWebСервиса(Знач ИнформацияОбОшибке, Знач ИмяПодсистемы = "", Знач ИмяВебСервиса = "", Знач ИмяОперации = "") Экспорт
	...
	ИмяСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1.Ошибка вызова операции web-сервиса';uk='%1.Помилка виклику операції web-сервісу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		ИмяПодсистемы);
	
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Ошибка при вызове операции %1 веб-сервиса %2: %3';uk='Помилка при виклику операції %1 веб-сервісу %2: %3'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		ИмяОперации,
		ИмяВебСервиса,
...
Функция ПараметрыВыборки(Знач ПолноеИмяОбъектаМетаданных) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Функция ПараметрыВыборки() не должна использоваться для объекта %1.';uk='Функція ПараметрыВыборки() не повинна використовуватися для об''єкту %1.'"),
...
Функция СобытиеЖурналаРегистрацииКопированиеОбластиДанных() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Копирование области данных';uk='Робота в моделі сервісу.Копіювання області даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииБлокировкаОбластиДанных() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Блокировка области данных';uk='Робота в моделі сервісу.Блокування області даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПодготовкаОбластиДанных() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Подготовка области данных';uk='Робота в моделі сервісу.Підготовка області даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПолучениеФайлаИзХранилища() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Получение файла из хранилища';uk='Робота в моделі сервісу.Отримання файлу з сховища'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииДобавлениеФайлаОбменЧерезФС() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Добавление файла.Обмен через ФС';uk='Робота в моделі сервісу.Додавання файлу.Обмін через ФС'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииДобавлениеФайлаОбменНеЧерезФС() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Добавление файла.Обмен не через ФС';uk='Робота в моделі сервісу.Додавання файлу.Обмін не через ФС'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУдалениеВременногоФайла() Экспорт
	...
	Возврат НСтр("ru='Работа в модели сервиса.Удаление временного файла';uk='Робота в моделі сервісу.Видалення тимчасового файлу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПриЗаполненииТаблицыПараметровИБ(Знач ТаблицаПараметров) Экспорт
	...
	СтрокаПараметра.Описание = НСтр("ru='Версия конфигурации';uk='Версія конфігурації'");
...
Функция ПолучитьФайлИзХранилища(Знач ИдентификаторФайла, Знач ПараметрыПодключения, 
		...
		Попытка
			...
			Если СвойстваФайла.Существует() Тогда
				...
				ЗаписатьВЖурналСобытиеФайловогоХранилища(
					НСтр("ru='Извлечение';uk='Витягування'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					ИдентификаторФайла,
	...
	Исключение
		...
		ФайлОбъединен = Ложь;
		ЗаписьЖурналаРегистрации(НСтр("ru='Выполнение операции объединения файлов';uk='Виконання операції об''єднання файлів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ФайлОбъединен Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Выполнение операции чтения zip файла';uk='Виконання операції читання zip файлу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ФайлВременный.Существует() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Выполнение операции удаления временного файла';uk='Виконання операції вилучення тимчасового файлу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ПрочитанZipФайл Тогда
		...
		ВызватьИсключение(НСтр("ru='При чтении zip файла произошла ошибка.';uk='При читанні zip файлу виникла помилка.'"));
		
	КонецЕсли;
	
	Если ВПолученномАрхивеБолееОдногоФайла Тогда
		
		ВызватьИсключение(НСтр("ru='В полученном архиве содержится более одного файла.';uk='В отриманому архіві міститься більше одного файлу.'"));
...
Процедура ПроверитьВозможностьИспользованияКонфигурацииВМоделиСервиса() Экспорт
	...
	Если ОписаниеБТС = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В конфигурацию не внедрена библиотека ""Библиотека технологии сервиса"".
                  ...
                  |""Библиотека технологии сервиса"" версии не младше %1!'
                  |;uk='У конфігурацію не впроваджена бібліотека ""Бібліотека технології сервісу"".
                  ...
                  |""Бібліотека технології сервісу"" версії не молодше %1!'",Метаданные.ОсновнойЯзык.КодЯзыка),
			ТребуемаяВерсияБТС());
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБТС, ТребуемаяВерсияБТС()) < 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для использования конфигурации в модели сервиса с текущей версией БСП требуется
                      |обновить используемую версию библиотеки ""Библиотека технологии сервиса""!
                      |
                      |Используемая версия: %1, требуется версия не младше %2!'
                      |;uk='Для використання конфігурації в моделі сервісу з поточною версією БСП вимагається
                      |оновити версію бібліотеки ""Бібліотека технології сервісу""!
                      |
...
Процедура ВызватьИсключениеОтсутствуетПодсистемаБТС(Знач ИмяПодсистемы) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Невозможно выполнить операцию по причине - в конфигурации не внедрена подсистема ""%1"".
              ...
              |;uk='Неможливо виконати операцію по причині - в конфігурації не впроваджена підсистема ""%1"".
              |Дана підсистема поставляється до складу бібліотеки технології сервісу, яка повинна окремо впроваджуватися до складу конфігурації.
              |Перевірте наявність і коректність впровадження підсистеми ""%1"".'"),
		ИмяПодсистемы);
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		ОписаниеОшибки =
			НСтр("ru='Запуск приложения временно недоступен.
                       ...
                       |;uk='Запуск програми тимчасово недоступний.
                       |Виконуються регламентні операції по обслуговуванню програми.
                       |
                       |Спробуйте запустити програму через кілька хвилин.'");
...
Функция ПолучитьМенеджерЗаписиОбластиДанных(Знач ОбластьДанных, Знач Статус)
	...
	Попытка
		...
		Если НЕ МенеджерЗаписи.Выбран() Тогда
			ШаблонСообщения = НСтр("ru='Область данных %1 не найдена';uk='Область даних %1 не знайдена'");
		...
		ИначеЕсли МенеджерЗаписи.Статус <> Статус Тогда
			ШаблонСообщения = НСтр("ru='Статус области данных %1 не равен ""%2""';uk='Статус області даних %1 не дорівнює ""%2""'");
...
Процедура СменитьСтатусОбластиИУведомитьМенеджер(Знач МенеджерЗаписи, Знач РезультатПодготовки, Знач СообщениеОбОшибке)
	...
	ИначеЕсли РезультатПодготовки = "ОшибкаУдаления" Тогда
		...
		ВключатьСообщениеОбОшибке = Истина;
	Иначе
		ВызватьИсключение НСтр("ru='Неожиданный код возврата';uk='Несподіваний код повернення'");
	КонецЕсли;
...
Функция ПодготовитьОбластьДанныхКИспользованиюИзВыгрузки(Знач ОбластьДанных, Знач ИДФайлаВыгрузки, СообщениеОбОшибке)
	...
	Если ИмяФайлаВыгрузки = Неопределено Тогда
		...
		СообщениеОбОшибке = НСтр("ru='Нет файла начальных данных для области';uk='Немає файлу початкових даних для області'");
...
Функция ПоместитьФайлВХранилище(Знач АдресДанныеФайл, Знач ПараметрыПодключения, Знач ИмяФайла = "")
	...
	Если ОбменЧерезФС Тогда
		...
		Если ЦелевойФайл.Существует() Тогда
			...
			Если СвойстваФайла.ПолноеИмя = ЦелевойФайл.ПолноеИмя Тогда
				...
				ЗаписатьВЖурналСобытиеФайловогоХранилища(
					НСтр("ru='Помещение';uk='Приміщення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					Результат,
	...
	Если Не ОбменЧерезФС Тогда
		...
		Если Описание.Данные = Неопределено Тогда
			...
			Иначе
				ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Добавление файла в хранилище. Не найден файл %1.';uk='Додавання файлу в сховище. Не знайдений файл %1.'"),
...
Функция ПолучитьИмяФайлаСДанными(Знач АдресДанныеФайл, Знач ИмяФайла = "")
	...
	Если ТипЗнч(АдресДанныеФайл) = Тип("Строка") Тогда // Адрес данных файла во временном хранилище.
		Если ПустаяСтрока(АдресДанныеФайл) Тогда
			ВызватьИсключение(НСтр("ru='Неверный адрес хранилища.';uk='Невірна адреса сховища.'"));
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(АдресДанныеФайл) = Тип("Файл") Тогда // Объект типа Файл.
		Если Не АдресДанныеФайл.Существует() Тогда
			ВызватьИсключение(НСтр("ru='Файл не найден.';uk='Файл не знайдений.'"));
		КонецЕсли;
	...
	Иначе
		ВызватьИсключение(НСтр("ru='Неверный тип данных';uk='Невірний тип даних'"));
	КонецЕсли;
...
Процедура ЗаписатьВЖурналСобытиеФайловогоХранилища(Знач Событие,
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Хранилище файлов';uk='Сховище файлів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + "." + Событие,
...
Функция СтруктурныйОбъектВОбъектXDTO(Знач ЗначениеСтруктурногоТипа)
	...
	Если Не СериализуемыйСтруктурныйТип(СтруктурныйТип) Тогда
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Тип %1 не является структурным или его сериализация в настоящее время не поддерживается.';uk='Тип %1 не є структурним або його серіалізація в даний час не підтримується.'"),
			СтруктурныйТип);
...
Функция ЗагрузитьОбластьИзПоставляемыхДанных(Знач ОбластьДанных, Знач ИДФайлаВыгрузки, Знач Вариант, СообщениеОФатальнойОшибке)
		...
		Иначе
			СообщениеОФатальнойОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Указанный файл начальных данных не подходит для данной конфигурации.
                    |Дескриптор файла: %1'
                    |;uk='Зазначений файл початкових даних не підходить для даної конфігурації.
                    |Дескриптор файлу: %1'"),
	...
	Если Не ФайлДанныхНайден Тогда
		...
		Если Дескрипторы.Descriptor.Количество() = 0 Тогда
			...
			НСтр("ru='В менеджере сервиса нет файла начальных данных для текущей версии конфигурации.';uk='В менеджері сервісу немає файлу початкових даних для поточної версії конфігурації.'");
			Возврат "ФатальнаяОшибка";
		КонецЕсли;
		
		Если Дескрипторы.Descriptor[0].FileGUID <> ИДФайлаВыгрузки Тогда
			СообщениеОФатальнойОшибке = 
			НСтр("ru='Файл начальных данных, имеющийся в менеджере сервиса, отличается от указанного в сообщении на подготовку области. Область не может быть подготовлена.';uk='Файл початкових даних, наявний в менеджері сервісу, що відрізняється від зазначеного в повідомленні на підготовку області. Область не може бути підготовлена.'");
			Возврат "ФатальнаяОшибка";
		КонецЕсли;
		...
		Если ИмяФайлаВыгрузки = Неопределено Тогда
			СообщениеОФатальнойОшибке = 
			НСтр("ru='В менеджере сервиса больше нет требуемого файла начальных данных, вероятно он был заменен. Область не может быть подготовлена.';uk='В менеджері сервісу більше немає необхідного файлу початкових даних, імовірно він був замінений. Область не може бути підготовлена.'");
			Возврат Ложь;
		КонецЕсли;
...
Процедура КонтрольНеразделенныхДанныхПриЗаписи(Знач Источник)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() И ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ПредставлениеИсключения = НСтр("ru='Нарушение прав доступа!';uk='Порушення прав доступу!'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура КонтрольНеразделенныхДанныхПриОбновлении() Экспорт
	...
	Если НарушенияКонтроляВключенияНеразделенныхДанныхВКонтролирующиеПодписки.Количество() > 0 Тогда
		...
		ВызываемыеИсключения.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Все объекты метаданных, не входящие в состав разделителей БСП (%1),
                  ...
                  |;uk='Всі об''єкти метаданих, що не входять до складу роздільників БСП (%1),
                  |повинні бути включені до складу підписок на події (%2), контролюючих
                  |неможливість запису неподілених даних в розділених сеансах.
                  |Такі об''єкти метаданих не задовольняють цьому критерію: %3.'"),
	...
	Если НарушенияКонтроляРазделенияОбъектовНесколькимиРазделителями.Количество() > 0 Тогда
		...
		ВызываемыеИсключения.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Все объекты метаданных конфигурации должны быть разделены не более чем одним разделителем БСП (%1).
                  ...
                  |;uk='Всі об''єкти метаданих конфігурації повинні бути розділені не більш ніж одним роздільником БСП (%1).
                  |Такі об''єкти не задовольняють цьому критерію: %2'"),
	...
	Если Не ПустаяСтрока(РезультирующееИсключение) Тогда
		...
		РезультирующееИсключение = НСтр("ru='Обнаружены ошибки в структуре метаданных конфигурации:';uk='Виявлені помилки в структурі метаданих конфігурації:'") + Символы.ПС + Символы.ВК + РезультирующееИсключение;
	...
	Если ПорядокПрикладныхДанных <= ПорядокВнутреннихДанных Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Обнаружено нарушение структуры метаданных конфигурации: общий реквизит %1 должен
                  ...
                  |%2 по порядку!'
                  |;uk='Виявлено порушення структури метаданих конфігурації: загальний реквізит %1 повинен
                  |бути розташований в дереві метаданих конфігурації до загального реквізиту
                  |%2 по порядку!'"),
...
// Возвращает минимальную версию библиотеки "Библиотека технологии сервиса", совместно с которой
