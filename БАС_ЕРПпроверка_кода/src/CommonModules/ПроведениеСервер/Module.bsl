...
Процедура ВыполнитьКонтрольРезультатовПроведения(Объект, Отказ) Экспорт
	...
	Если ЕстьИзмененияВТаблице(ДанныеТаблиц,"ДвиженияЗаказыМатериаловВПроизводствоИзменение") Тогда
		...
		|			(Распоряжение, Номенклатура, Характеристика, Серия, КодСтроки, Склад, Назначение, Подразделение) В
		|				(ВЫБРАТЬ
		|					Таблица.Распоряжение,
		|					Таблица.Номенклатура,
		|					Таблица.Характеристика,
	...
	Если ЕстьИзмененияВТаблице(ДанныеТаблиц,"ДвиженияРаспоряженияНаСписаниеПоНормативамИзменение") Тогда
		...
		|			,
		|			(Организация, Подразделение, Номенклатура, Характеристика, Распоряжение, КодСтроки, НалоговоеНазначение, Назначение) В
		|				(ВЫБРАТЬ
		|					Таблица.Организация,
		|					Таблица.Подразделение,
		|					Таблица.Номенклатура,
		...
		|			И Таблица.НалоговоеНазначение = РаспоряженияНаСписаниеПоНормативам_ОперативныйКонтроль.НалоговоеНазначение 
	...
	Для Каждого Результат Из МассивРезультатов Цикл
		...
		ИначеЕсли ИмяКонтроля = Врег("ВТ_РаспоряженияНаВыпускПродукцииОперативныйКонтроль")
			ИЛИ ИмяКонтроля = Врег("ВТ_РаспоряженияНаВыпускПродукцииКонтрольНаДату") Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(Объект) = Тип("ДокументОбъект.МаршрутныйЛистПроизводства") Тогда
			...
			Если Выборка.КонтрольНаДату Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='На позицию %1 уже оформлен выпуск и распоряжение должно быть оформлено не позже выпуска';uk='На позицію %1 вже оформлений випуск і розпорядження має бути оформлено не пізніше випуску'"),
					НоменклатураКлиентСервер.ПредставлениеНоменклатуры(Выборка.Номенклатура, Выборка.Характеристика));
			Иначе
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='На позицию %1 уже оформлен выпуск (%2 %3)';uk='На позицію %1 вже оформлений випуск (%2 %3)'"),
					НоменклатураКлиентСервер.ПредставлениеНоменклатуры(Выборка.Номенклатура, Выборка.Характеристика),
		...
		Иначе
			...
			Если Выборка.КонтрольНаДату Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Выпуск позиции %1 должен быть оформлен не раньше даты завершения выполнения маршрутного листа %2';uk='Випуск позиції %1 повинен бути оформлений не раніше дати завершення виконання маршрутного листа %2'"),
					НоменклатураКлиентСервер.ПредставлениеНоменклатуры(Выборка.Номенклатура, Выборка.Характеристика),
					Выборка.ДатаВыпуска);
			Иначе
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Количество позиций %1 превышает количество по распоряжению (%2 %3)';uk='Кількість позицій %1 перевищує кількість за розпорядженням (%2 %3)'"),
					НоменклатураКлиентСервер.ПредставлениеНоменклатуры(Выборка.Номенклатура, Выборка.Характеристика),
					Строка(-Выборка.Количество),
					Строка(Выборка.ЕдиницаИзмерения));
			КонецЕсли;
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗаказыМатериаловВПроизводство(Объект, Отказ, РезультатЗапроса)
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПередачаМатериаловВПроизводство") Тогда
	...
	Иначе
		...
		ШаблонСообщенияСоСкладомСПодразделением = НСтр("ru='Номенклатура %НоменклатураХарактеристика%, склад %Склад%, подразделение %Подразделение% 
            ...
            |;uk='Номенклатура %НоменклатураХарактеристика%, склад %Склад%, підрозділ %Подразделение% 
            |По рядку %КодСтроки% вже оформлена передача матеріалів у кількості більшій, ніж зазначено в документі, %Количество% %Единица%'");
			
		ШаблонСообщенияСоСкладомБезПодразделения = НСтр("ru='Номенклатура %НоменклатураХарактеристика%, склад %Склад% 
            |По строке %КодСтроки% уже оформлена передача материалов в количестве большем, чем указано в документе, на %Количество% %Единица%'
            |;uk='Номенклатура %НоменклатураХарактеристика%, склад %Склад% 
            |По рядку %КодСтроки% вже оформлена передача матеріалів у кількості більшій, ніж зазначено в документі, на %Количество% %Единица%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструРаспоряженияНаСписаниеПоНормативам(Объект, Отказ, РезультатЗапроса)
	...
	ШаблонВыпускПродукцииБезРаспоряжений = НСтр("ru='На изделие %1 уже списаны материалы (%2 %3)';uk='На виріб %1 вже списані матеріали (%2 %3)'");
	ШаблонСписаниеМатериалов = НСтр("ru='На изделие %1 еще не оформлен выпуск продукции (%2 %3)';uk='На виріб %1 ще не оформлений випуск продукції (%2 %3)'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструТрудозатратыКОформлению(Объект, Отказ, РезультатЗапроса)
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ВыработкаСотрудников") Тогда
		ШаблонСообщения = НСтр("ru='Количество по виду работ ""%ВидРабот%""
            ...
            |;uk='Кількість за видом робіт ""%ВидРабот%""
            | перевищує або не відповідає даним розпорядження (на %Количество% %Единица%)'");
	Иначе
		ШаблонСообщения = НСтр("ru='Выработка сотрудников по виду работ ""%ВидРабот%""
            | уже оформлена на %Количество% %Единица%'
            |;uk='Виробіток працівників за видом робіт ""%ВидРабот%""
            |вже оформлена на %Количество% %Единица%'");
	КонецЕсли;
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструУслугиДавальцуКОформлению(Объект, Отказ, РезультатЗапроса)
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ОтчетДавальцу") Тогда
		...
		ШаблонСообщения = НСтр("ru='Номенклатура %НоменклатураХарактеристика%
            ...
            |;uk='Номенклатура %НоменклатураХарактеристика%
            |Не оформлений випуск продукції за призначенням %Заказ% на %Количество% %Единица%'");
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.ВыпускПродукции") Тогда
		...
		ШаблонСообщения = НСтр("ru='Номенклатура %НоменклатураХарактеристика%
            ...
            |;uk='Номенклатура %НоменклатураХарактеристика%
            |Звіт давальцю вже оформлений за розпорядженням %Заказ% на %Количество% %Единица%'");
	...
	Иначе
		...
		ШаблонСообщения = НСтр("ru='Номенклатура %НоменклатураХарактеристика%
            ...
            |;uk='Номенклатура %НоменклатураХарактеристика%
            |Оформлено більше ніж передано за розпорядженням %Заказ% на %Количество% %Единица%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗаказыМатериаловСУчетомКорректировок(Объект, Отказ, РезультатЗапроса)
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ЗаказНаПроизводство") Тогда
		ШаблонСообщения = НСтр("ru='По материалу ""%НоменклатураХарактеристика%"" введены корректировки, которые превышают количество по заказу на %Количество% %Единица%';uk='За матеріалом ""%НоменклатураХарактеристика%"" введені коригування, які перевищують кількість замовлення на %Количество% %Единица%'");
	Иначе
	//-- НЕ УТКА
		
		ШаблонСообщения = НСтр("ru='Корректировка заказа материала ""%НоменклатураХарактеристика%"" превышает количество по заказу на %Количество% %Единица%';uk='Коригування замовлення матеріалу ""%НоменклатураХарактеристика%"" перевищує кількість замовлення на %Количество% %Единица%'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЭтапыПроизводства(Объект, Отказ, РезультатЗапроса)
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ЗаказНаПроизводство") Тогда
		...
		ШаблонСообщения = НСтр("ru='Номенклатура %НоменклатураХарактеристика%
            ...
            |;uk='Номенклатура %НоменклатураХарактеристика%
            |По рядку %НомерСтроки% оформлено виконання етапів графіка виробництва в більшій кількості одиниць виробу або партії, ніж заплановано замовленням'");
	...
	Иначе
		...
		ШаблонСообщения = НСтр("ru='Превышено выполнение этапа графика производства на %Количество% %ЕдиницаИзмерения%';uk='Перевищено виконання етапу графіка виробництва на %Количество% %ЕдиницаИзмерения%'");
		...
		Пока Выборка.Следующий() Цикл
			...
			ЕдиницаИзмерения = ОбщегоНазначенияУТКлиентСервер.ФормаМножественногоЧисла(
				НСтр("ru='единицу или партию изделия';uk='одиницю або партію виробу'"), НСтр("ru='единицы или партии изделия';uk='одиниці або партії виробу'"), НСтр("ru='единиц или партий изделий';uk='одиниць або партій виробів'"), Количество);
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструБракВПроизводстве(Объект, Отказ, РезультатЗапроса)
	...
	ШаблонСообщения = НСтр("ru='Ошибка соответствия этапов восстановления и брака.';uk='Помилка відповідності етапів відновлення і браку.'");
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструТоварыПереданныеПереработчику(Объект, Отказ, РезультатЗапроса)
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПередачаСырьяПереработчику") Тогда
		ШаблонСообщения = НСтр("ru='Номенклатура %НоменклатураХарактеристика% 
            ...
            |;uk='Номенклатура %НоменклатураХарактеристика% 
            |За матеріалом оформлений звіт переробника або повернення сировини і матеріалів на %Количество% %Единица%'");
	Иначе
		ШаблонСообщения = НСтр("ru='Номенклатура %НоменклатураХарактеристика% 
            |Превышен остаток сырья и материалов переданных переработчику на %Количество% %Единица%'
            |;uk='Номенклатура %НоменклатураХарактеристика% 
            |Перевищений залишок сировини і матеріалів переданих переробнику на %Количество% %Единица%'");
	КонецЕсли; 
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструДенежныеДокументы(Объект, Отказ, РезультатЗапроса)
	...
	ШаблонКоличество = НСтр("ru='Денежный документ %1
        ...
        |;uk='Грошовий документ %1
        |Перевищений залишок по організації %2 у підрозділі %3 МВО %4 %5 шт.'");
	ШаблонСумма = НСтр("ru='Денежный документ %1
        |Превышен остаток в наличии по организации %2 в подразделении %3 у МОЛ %4 на %6 %7'
        |;uk='Грошовий документ %1
        |Перевищений залишок по організації %2 у підрозділі %3 МВО %4 %6 %7'");
