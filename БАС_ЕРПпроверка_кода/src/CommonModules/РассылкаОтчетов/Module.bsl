...
Функция ПодготовитьПараметрыИВыполнитьРассылку(Рассылка, ПараметрыЖурнала = Неопределено, ПредварительныеНастройки = Неопределено) Экспорт
	...
	Если Не ПараметрыЖурнала.Свойство("ИмяСобытия") Тогда
		ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru='Рассылка отчетов. Запуск по требованию';uk='Розсилка звітів. Запуск на вимогу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	КонецЕсли;
	...
	Если Не Рассылка.Подготовлена
		...
		Если Не Рассылка.Подготовлена Тогда
			Причина = Причина + Символы.ПС + НСтр("ru='Рассылка не подготовлена';uk='Розсилка не підготовлена'");
		КонецЕсли;
		Если Рассылка.ПометкаУдаления Тогда
			Причина = Причина + Символы.ПС + НСтр("ru='Рассылка помечена на удаление';uk='Розсилка відмічена для вилучення'");
		КонецЕсли;
		
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("ru='Завершение';uk='Завершення'"), СокрЛП(Причина));
		Возврат Ложь;
	...
	КонецЕсли;
	Если ФорматыПоУмолчанию.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru='Не установлены форматы по умолчанию.';uk='Не встановлені формати за замовчуванням.'");
	КонецЕсли;
	...
	Если Не ПараметрыДоставки.ИспользоватьПапку
		...
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("ru='Не выбран способ доставки.';uk='Не обраний спосіб доставки.'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция ВыполнитьРассылку(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	ЗаписьЖурнала(ПараметрыЖурнала, ,
		НСтр("ru='Инициализирована рассылка %1, автор: %2';uk='Ініційована розсилка %1, автор: %2'"), , 
		"'"+ Строка(Рассылка) +"'", 
	...
	Для Каждого СтрокаОтчет Из ТаблицаОтчетов Цикл
		ТекстЖурнала = НСтр("ru='Отчет %1 формируется';uk='Звіт %1 формується'");
		Если СтрокаОтчет.Настройки = Неопределено Тогда
			ТекстЖурнала = ТекстЖурнала + Символы.ПС + НСтр("ru='(пользовательские настройки не заданы)';uk='(користувацькі настройки не задані)'");
		КонецЕсли;
		...
		Попытка
			...
			КонецЕсли;
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Примечание,
				НСтр("ru='Отчет %1 успешно сформирован';uk='Звіт %1 успішно сформований'"), ,
				"'"+ Строка(СтрокаОтчет.Отчет) +"'");
			НомерОтчета = НомерОтчета + 1;
		Исключение
			ЗаписьЖурнала(ПараметрыЖурнала, ,
				НСтр("ru='Отчет %1 не сформирован:';uk='Звіт %1 не сформований:'"), ИнформацияОбОшибке(),
				"'"+ Строка(СтрокаОтчет.Отчет) +"'");
		КонецПопытки;
	КонецЦикла;
	...
	Если ДеревоОтчетов.Строки.Найти(3, "Уровень", Истина) = Неопределено Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("ru='Не сохранено ни одного отчета';uk='Не збережене жодного звіту'"), ,);
	...
	Для Каждого СтрокаПолучатель Из ДеревоОтчетов.Строки Цикл
		...
		Исключение
			ЗаписьЖурнала(ПараметрыЖурнала, ,
				НСтр("ru='Не удалось отправить отчеты получателю %1:';uk='Не вдалося відправити звіти одержувачу %1:'"), ИнформацияОбОшибке(), 
	...
	Если РассылкаВыполнена Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, , НСтр("ru='Рассылка выполнена';uk='Розсилка виконана'"));
	Иначе
		ЗаписьЖурнала(ПараметрыЖурнала, , НСтр("ru='Рассылка не выполнена';uk='Розсилка не виконана'"));
	КонецЕсли;
...
Процедура ВыполнитьРассылкиВФоновомЗадании(ПараметрыВыполнения, АдресРезультата) Экспорт
	...
	Если Всего = 0 Тогда
		ТекстСообщения = НСтр("ru='Выбранные группы не содержат рассылок отчетов.';uk='Обрані групи не містять розсилань звітів.'");
	ИначеЕсли Всего <= 5 Тогда
		ТекстСообщения = "";
		Для Каждого СтрокаТаблицы Из ТаблицаРассылок Цикл
			Если Не СтрокаТаблицы.Подготовлена Тогда
				ШаблонСообщения = НСтр("ru='Рассылка ""%1"" не подготовлена.';uk='Розсилка ""%1"" не підготовлена.'");
			ИначеЕсли Не СтрокаТаблицы.Выполнена Тогда
				ШаблонСообщения = НСтр("ru='Рассылка ""%1"" не выполнена.';uk='Розсилка ""%1"" не виконана.'");
			ИначеЕсли СтрокаТаблицы.СОшибками Тогда
				ШаблонСообщения = НСтр("ru='Рассылка ""%1"" выполнена с ошибками.';uk='Розсилка ""%1"" виконана з помилками.'");
			Иначе
				ШаблонСообщения = НСтр("ru='Рассылка ""%1"" выполнена.';uk='Розсилка ""%1"" виконана.'");
			КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |С ошибками: %4
            |Не выполнено: %5'
            |;uk='Підготовлене розсилань: %1 з %2
            |Виконане: %3
            |З помилками: %4
            |Не виконане: %5'"),
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ru='Проблемы с рассылками отчетов';uk='Проблеми з розсилками звітів'");
...
Процедура ФормаОтчетаДобавитьКоманды(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	КомандаСоздать.Заголовок = НСтр("ru='Создать рассылку отчетов...';uk='Створити розсилку звітів...'");
	КомандаСоздать.Подсказка = НСтр("ru='Создать новую рассылку отчетов и добавить в нее отчет с текущими настройками.';uk='Створити нову розсилку звітів і додати в нього звіт з поточними настройками.'");
	...
	КомандаПрисоединить.Заголовок = НСтр("ru='Включить в существующую рассылку отчетов...';uk='Включити в існуючу розсилку звітів...'");
	КомандаПрисоединить.Подсказка = НСтр("ru='Присоединить отчет с текущими настройками к существующей рассылке отчетов.';uk='Приєднати звіт з поточними настройками до існуючої розсилки звітів.'");
	...
	Подменю.Заголовок = НСтр("ru='Отправить рассылку';uk='Відправити розсилку'");
...
Процедура ВыполнитьРассылкуПоРасписанию(Рассылка) Экспорт
	...
	Если Не Рассылка.ВыполнятьПоРасписанию Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='У рассылки отчетов ""%1"" отключен флажок ""Выполнять по расписанию""';uk='У розсилки звітів ""%1"" відключений прапорець ""Виконувати за розкладом""'"),
			Строка(Рассылка));
	КонецЕсли;
	
	// Параметры записи в журнал регистрации.
	ПараметрыЖурнала = Новый Структура("ИмяСобытия, Метаданные, Данные");
	ПараметрыЖурнала.ИмяСобытия = НСтр("ru='Рассылка отчетов. Запуск по расписанию';uk='Розсилка звітів. Запуск за розкладом'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СформироватьСписокПолучателейРассылки(ПараметрыЖурнала, Рассылка) Экспорт
	...
	ШаблонСообщенияОбОшибке = НСтр("ru='При формировании списка получателей ""%1"" возникла ошибка:';uk='При формуванні списку одержувачів ""%1"" виникла помилка:'");
	...
	Если СписокПолучателей.Количество() = 0 Тогда
        ...
        |;uk='Не вдалося сформувати список одержувачів %1 розсилки %2 по одній з можливих причин:
        | - В одержувачів не заповнена поштова адреса %3
        | - Не заповнений список одержувачів
        | - Обрані порожні групи одержувачів
        | - Одержувачі відмічені для вилучення
        | - Недостатньо правий доступу до довідника'");
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	Если ТипЗнч(ПараметрыОтчета.Отчет) <> Тип("СправочникСсылка.ВариантыОтчетов") Тогда
		ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Рассылка дополнительных отчетов поддерживается через подсистему ""Варианты отчетов"".
            |Убедитесь, что отчет ""%1"" подключен к подсистеме ""Варианты отчетов"".'
            |;uk='Розсилка додаткових звітів підтримується через підсистему ""Варіанти звітів"".
            |Переконайтеся, що звіт ""%1"" підключений до підсистеми ""Варіанти звітів"".'"),
	...
	Исключение
		ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При подключении и загрузке настроек отчета ""%1"" возникла ошибка.';uk='При підключенні і завантаження налаштувань звіту ""%1"" виникла помилка.'"),
			Строка(ПараметрыОтчета.Отчет));
		ЗаписьЖурнала(
			ПараметрыЖурнала,
			УровеньЖурналаРегистрации.Ошибка,
	...
	Если РассылкаОтчетовПовтИсп.ИсключаемыеОтчеты().Найти(Подключение.СсылкаОтчета) <> Неопределено Тогда
		ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Звіт ""%1"" не підтримує розсилку.
            |Докладніше див. у процедурі ОпределитьИсключаемыеОтчеты модуля РассылкаОтчетовПереопределяемый.'"),
...
Функция СформироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, Получатель = Неопределено)
	...
	Если Не ПараметрыОтчета.Свойство("Инициализирован") Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, ,
			НСтр("ru='Отчет ''%1'' не инициализирован';uk='Звіт ''%1'' не ініціалізований'"), ,
			Строка(ПараметрыОтчета.Отчет));
		Возврат Результат;
	КонецЕсли;
	
	// Настройки подключения отчета.
	ПараметрыФормирования = Новый Структура("Объект, ПолноеИмя, СсылкаВарианта, КлючВарианта, Метаданные, АдресСхемы");
	...
	Если Не Формирование.Успех Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ru='Отчет ""%1"":';uk='Звіт ""%1"":'"), Формирование.ТекстОшибки,
			Строка(ПараметрыОтчета.Отчет));
...
Функция ВыполнитьДоставку(ПараметрыЖурнала, ПараметрыДоставки, Вложения) Экспорт
	Результат = Ложь;
	ШаблонСообщенияОбОшибке = НСтр("ru='Ошибка доставки отчетов';uk='Помилка доставки звітів'");
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ПараметрыДоставки.ТолькоУведомить Тогда
			ШаблонСообщенияОбОшибке = НСтр("ru='Невозможно отправить уведомление о рассылке по электронной почте:';uk='Неможливо надіслати повідомлення про розсилку по електронній пошті:'");
			ВложенияПисьма = Новый Соответствие;
		Иначе
			ШаблонСообщенияОбОшибке = НСтр("ru='Невозможно отправить отчет по электронной почте:';uk='Неможливо відправити звіт електронною поштою:'");
	...
	Если Рассылка.ИспользоватьСетевойКаталог Тогда
		Если ЗначениеЗаполнено(Рассылка.СетевойКаталогWindows) Тогда
			Элемент = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаФайловойСистемы(
				...
				Истина,
				Истина,
				НСтр("ru='Сетевой каталог для публикации отчетов с сервера Windows.';uk='Мережевий каталог для публікації звітів із сервера Windows.'"));
		...
		Если ЗначениеЗаполнено(Рассылка.СетевойКаталогLinux) Тогда
			Элемент = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаФайловойСистемы(
				...
				Истина,
				Истина,
				НСтр("ru='Сетевой каталог для публикации отчетов с сервера Linux.';uk='Мережевий каталог для публікації звітів із сервера Linux.'"));
			Разрешения.Добавить(Элемент);
		КонецЕсли;
	КонецЕсли;
	Если Рассылка.ИспользоватьFTPРесурс Тогда
		Если ЗначениеЗаполнено(Рассылка.FTPСервер) Тогда
			Элемент = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
				...
				Рассылка.FTPПорт,
				НСтр("ru='FTP ресурс для публикации отчетов.';uk='FTP ресурс для публікації звітів.'"));
			Разрешения.Добавить(Элемент);
		КонецЕсли;
	КонецЕсли;
	Возврат Разрешения;
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	Для Каждого Формат Из ПараметрыОтчета.Форматы Цикл
		...
		Если СтандартнаяОбработка = Истина Тогда
			ЗаголовокОшибки = НСтр("ru='Ошибка записи отчета %1 в формат %2:';uk='Помилка запису звіту %1 у формат %2:'");
			
			Если ПараметрыФормата.ТипФайла = Неопределено Тогда
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					ЗаголовокОшибки, НСтр("ru='Формат не поддерживается';uk='Формат не підтримується'"),
					"'"+ СтрокаОтчет.Ключ +"'", 
					"'"+ ПараметрыФормата.Имя +"'");
				Продолжить;
			КонецЕсли;
		...
		Если Не ВременныйФайл.Существует() Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				ЗаголовокОшибки + Символы.ПС + НСтр("ru='Файл %3 не найден.';uk='Файл %3 не знайдений.'"), ,
				"'"+ СтрокаОтчет.Ключ +"'", 
				"'"+ ПараметрыФормата.Имя +"'",
...
Функция ЗаполнитьПараметрыРассылкиПараметрамиПоУмолчанию(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала)
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		Если Не ЗначениеЗаполнено(ПараметрыДоставки.Папка) Тогда
			...
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
				НСтр("ru='Папка не заполнена, доставка в папку отключена';uk='Папка не заповнена, доставка в папку відключена'"));
		Иначе
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
				МодульРаботаСФайламиСлужебный = ОбщегоНазначения.ОбщийМодуль("РаботаСФайламиСлужебный");
			...
			Если Не ПравоДоступа Тогда
				...
				УстановитьПривилегированныйРежим(Ложь);
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					НСтр("ru='Недостаточно прав на изменение файлов папки %1, доставка в папку отключена';uk='Недостатньо прав для зміны файлів папки %1, доставку в папку відключено'"), ,
					"'"+ ПредставлениеПапки +"'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыДоставки.ИспользоватьСетевойКаталог Тогда
		Если Не ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогWindows) 
			...
			Если ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогWindows) Тогда
				ЗначениеПодстановки = НСтр("ru='Linux';uk='Linux'");
			ИначеЕсли ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогLinux) Тогда
				ЗначениеПодстановки = НСтр("ru='Windows';uk='Windows'");
			Иначе
				ЗначениеПодстановки = НСтр("ru='Windows и Linux';uk='Windows і Linux'");
			КонецЕсли;
			
			ПараметрыДоставки.ИспользоватьСетевойКаталог = Ложь;
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("ru='Сетевой каталог %1 не выбран, доставка в сетевой каталог отключена';uk='Мережний каталог %1 не обраний, доставка в мережний каталог відключена'"), ,
				ЗначениеПодстановки);
	...
	Если ПараметрыДоставки.ИспользоватьFTPРесурс И Не ЗначениеЗаполнено(ПараметрыДоставки.Сервер) Тогда
		...
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ru='FTP сервер не заполнен, доставка в папку на FTP ресурс отключена';uk='FTP сервер не заповнено, доставку в папку на FTP ресурс відключено'"));
	КонецЕсли;
	
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту И Не ЗначениеЗаполнено(ПараметрыДоставки.УчетнаяЗапись) Тогда
		ПараметрыДоставки.ИспользоватьЭлектроннуюПочту = Ложь;
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ru='Учетная запись не выбрана, доставка по электронной почте отключена';uk='Обліковий запис не обраний, доставка по електронній пошті відключена'"));
	КонецЕсли;
	...
	Если ПараметрыДоставки.Персонализирована Тогда
		Если Не ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("ru='Персонализированная рассылка может быть отправлена только по электронной почте';uk='Персоналізована розсилка може бути відправлена тільки електронноб поштою'"));
			Возврат Ложь;
		КонецЕсли;
		ПараметрыДоставки.ИспользоватьПапку          = Ложь;
		ПараметрыДоставки.ИспользоватьСетевойКаталог = Ложь;
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ПараметрыДоставки.ТолькоУведомить
			...
			И Не ПараметрыДоставки.ИспользоватьFTPРесурс Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
				НСтр("ru='Использование уведомлений по электронной почте возможно только совместно с другими способами доставки';uk='Використання повідомлень по електронній пошті можливо тільки разом з іншими способами доставки'"));
			Возврат Ложь;
		КонецЕсли;
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		Если ПараметрыДоставки.ЗаполнитьСформированныеОтчетыВШаблонеСообщения Тогда
			ПараметрыДоставки.ПредставлениеОтчетовПолучателя = 
				...
				+ Символы.ПС 
				+ Символы.ПС
				+ НСтр("ru='Файлы отчетов запакованы в архив';uk='Файли звітів запаковані в архів'")
				+ " ";
		КонецЕсли;
...
Функция ПравоВывода(ПараметрыЖурнала)
	...
	Если Не ПравоВывода Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ru='У пользователя %1 недостаточно прав на вывод информации';uk='У користувача %1 недостатньо прав на вивід інформації'"), ,
...
Функция ПредставлениеСпособовДоставки(ПараметрыДоставки)
	Префикс = НСтр("ru='Результат';uk='Результат'");
	ТекстПредставления = "";
	Суффикс = "";
	...
	Если Не ПараметрыДоставки.ТолькоУведомить Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + НСтр("ru='и';uk='і'")) 
		+ " "
		+ НСтр("ru='отправлен по почте (см. вложения)';uk='відправлений поштою (див. вкладення)'");
	...
	Если ПараметрыДоставки.ВыполненаВПапку Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + НСтр("ru='и';uk='і'")) 
		+ " "
		+ НСтр("ru='доставлен в папку';uk='доставлений у папку'")
		+ " ";
	...
	Если ПараметрыДоставки.ВыполненаВСетевойКаталог Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + НСтр("ru='и';uk='і'")) 
		+ " "
		+ НСтр("ru='доставлен в сетевой каталог';uk='доставлений у мережний каталог'")
		+ " ";
	...
	Если ПараметрыДоставки.ВыполненаНаFTP Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + НСтр("ru='и';uk='і'")) 
		+ " "
		+ НСтр("ru='доставлен на FTP ресурс';uk='доставлений на FTP ресурс'")
		+ " ";
