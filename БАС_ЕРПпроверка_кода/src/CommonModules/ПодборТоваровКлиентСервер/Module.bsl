...
Процедура УстановитьОтборПоИерархииНоменклатурыПоставщика(Форма) Экспорт
	...
	ОтборПоИерархииНоменклатурыПоставщика = Форма.ТекущаяИерархияНоменклатурыПоставщика;
...
//	Форма - УправляемаяФорма - форма списка номенклатуры или форма подбора.
//
Процедура УдалитьОтборПоИерархииНоменклатурыПоставщика(Форма) Экспорт
...
Функция ПараметрыТовара(Номенклатура = Неопределено) Экспорт
	...
	ПараметрыТовара.Вставить("Марка");
	ПараметрыТовара.Вставить("РольАссортимента");
	ПараметрыТовара.Вставить("РейтингПродаж", ПредопределенноеЗначение("Справочник.РейтингиПродажНоменклатуры.ПустаяСсылка"));
	ПараметрыТовара.Вставить("СтадияАссортимента");
	ПараметрыТовара.Вставить("ТоварнаяКатегория");
	...
	ПараметрыТовара.Вставить("ВариантКомплектацииНоменклатуры");
...
Процедура УстановитьТекстИнформационнойНадписи(Форма) Экспорт
	...
	Количество = НРег(ЧислоПрописью(Корзина.Количество(), "Л = ru_RU; ДП = Ложь; НП = Истина; НД = Ложь;", НСтр("ru='позиция,позиции,позиций,ж,,,,,0';uk='позиція,позиції,позицій,ж,,,,,0'")));
	
	ИнформационнаяНадпись = НСтр("ru='Всего подобрано [Количество] товаров, на сумму [Сумма] [Валюта] [СкрытьПоказать]';uk='Всього підібрано [Количество] товарів, на суму [Сумма] [Валюта] [СкрытьПоказать]'");
	
	СкрытьПоказать = "";
	
	Если Форма.РежимПодбораБезКоличественныхПараметров Или Форма.РежимПодбораБезСуммовыхПараметров Тогда
		ИнформационнаяНадпись = НСтр("ru='Всего подобрано [Количество] товаров [СкрытьПоказать]';uk='Всього підібрано [Количество] товарів [СкрытьПоказать]'");
	КонецЕсли;
	
	СкрытьПоказать = ?(Форма.ПоказыватьПодобранныеТовары, НСтр("ru='(скрыть)';uk='(приховати)'"), НСтр("ru='(показать)';uk='(показати)'"));
...
Процедура СформироватьИнформационнуюНадписьПодобранныхТоваров(ИнформационнаяНадпись, КоличествоТоваров, СуммаТоваров, Валюта) Экспорт
	...
	ИнформационнаяНадпись = НСтр("ru='Всего подобрано товаров [КоличествоТоваров] на сумму [СуммаТоваров] [Валюта]';uk='Всього підібрано товарів [КоличествоТоваров] на суму [СуммаТоваров] [Валюта]'");
...
Процедура УстановитьЗаголовокФормыПодбора(Форма) Экспорт
	...
	Иначе
		ТекстДокумент = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 (новый)';uk='%1 (новий)'"), Документ.Метаданные().Синоним);
...
Функция СформироватьЗаголовокФормыПодбора(Заголовок, Документ) Экспорт
	...
	Иначе
		...
		ТекстДокумент = НСтр("ru='%1 (новый)';uk='%1 (новий)'");
