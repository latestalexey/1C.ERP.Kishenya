...
Функция ПропуститьЗапись(ТекстСсылкиТекущегоЭлемента)
	...
	//++ НЕ УТКА
	Если Метаданные.ФункциональныеОпции.Найти("ИспользоватьПланированиеПроизводства") <> Неопределено Тогда
...
Функция ПредставлениеСостояния(Состояние) Экспорт
	...
	ИначеЕсли Состояние = "ЦельДостигнута" Тогда
		Представление = НСтр("ru='Цель достигнута';uk='Мета досягнута'");
...
Процедура УстановитьНастройкиИзмерительнойДиаграммы(Диаграмма, ВариантАнализа, ИсточникДанных)
	...
	Если ЦелевойТренд = Перечисления.ВидыЦелевыхТрендовПоказателей.Максимизация Тогда
		Если ИсточникДанных.СвойстваЗоныАнализаИТренда.НомерЗоныАнализа > 0 Тогда
			...
			Подсказка = НСтр("ru='Диапазон достижения цели: Цель = %1 < Показатель < Макс. = %2';uk='Діапазон досягнення мети: Ціль = %1 < Показник < Макс. = %2'");
	...
	ИначеЕсли ЦелевойТренд = Перечисления.ВидыЦелевыхТрендовПоказателей.Минимизация Тогда
		Если ИсточникДанных.СвойстваЗоныАнализаИТренда.НомерЗоныАнализа > 0 Тогда
			...
			Подсказка = НСтр("ru='Диапазон достижения цели: Мин. = %1 < Показатель < Цель = %2';uk='Діапазон досягнення мети: Мін. = %1 < Показник < Ціль = %2'"); 
			ПолосаБольшеМинимумаМеньшеЦели.Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Подсказка, МинимальноеЗначениеЦели, ЦелевоеЗначение);
	...
	ИначеЕсли ЦелевойТренд = Перечисления.ВидыЦелевыхТрендовПоказателей.Диапазон Тогда
		Если ИсточникДанных.СвойстваЗоныАнализаИТренда.НомерЗоныАнализа > 0 Тогда
			...
			ПолосаБольшеЦелиМеньшеМаксимума.ЦветФона = ЦветаПоказателя["ЦелевоеЗначение"];
			Подсказка = НСтр("ru='Диапазон достижения цели: Цель (мин.) = %1 < Показатель < Цель (макс.) = %2';uk='Діапазон досягнення мети: Ціль (мін) = %1 < Показник < Ціль (макс.) = %2'");
...
Процедура ПодготовитьИсточник(ИсточникДанных, ВариантАнализа)
	...
	ИначеЕсли ТипАнализа = ТипыАнализа.СравнениеСПрошлымПериодом Тогда 
		...
		КолонкиДанных.Добавить("ПриведенноеГраничноеПозитивноеОтклонение", ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15,3), НСтр("ru='Приведенное граничное позитивное отклонение';uk='Приведене граничне позитивне відхилення'"));
		КолонкиДанных.Добавить("ПриведенноеГраничноеНегативноеОтклонение", ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15,3), НСтр("ru='Приведенное граничное негативное отклонение';uk='Приведене граничне негативне відхилення'"));
