	   ...
	   И Объект.СуммаДокумента = 0 Тогда
		...
		ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				Основание);
		//++ НЕ УТ
		ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ДоговорЗаймаСотруднику") И ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Займ по договору ""%1"" еще не выдан или полностью погашен.';uk='Позику по договору ""%1"" ще не видано або повністю погашено.'"),
				Основание);
		ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ДоговорЗаймаСотруднику") И ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств") Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Займ по договору ""%1"" еще не выдан или полностью погашен.';uk='Позику по договору ""%1"" ще не видано або повністю погашено.'"),
				Основание);
		ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ДоговорЗаймаСотруднику") И ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Остаток к выдаче по договору ""%1"" равен 0. Выберите невыданный займ.';uk='Залишок до видачі по договору ""%1"" дорівнює 0. Виберіть невидану позику.'"),
				Основание);
		ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ДоговорЗаймаСотруднику") И ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.СписаниеБезналичныхДенежныхСредств") Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Остаток к перечислению по договору ""%1"" равен 0. Выберите невыданный займ.';uk='Залишок до перерахування по договору ""%1"" дорівнює 0. Виберіть невидану позику.'"),
				Основание);
		//-- НЕ УТ
		Иначе
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
