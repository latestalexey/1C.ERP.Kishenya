...
Процедура ВыполнитьПодключаемуюКомандуСоздатьНаОснованииПодтверждениеЗаписи(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ВыполнитьПодключаемуюКомандуСоздатьНаОснованииПодготовкаОбъектовСоздатьНаОсновании(ДополнительныеПараметры);
	
КонецПроцедуры

Процедура ВыполнитьПодключаемуюКомандуСоздатьНаОсновании(ОбъектыОснований, ДополнительныеПараметры) Экспорт
	...
	ВыполнитьПодключаемуюКомандуСоздатьНаОснованииЗавершение(Истина, ДополнительныеПараметры);
	
КонецПроцедуры

Процедура ВыполнитьПодключаемуюКомандуСоздатьНаОснованииЗавершение(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	ВводНаОснованииКлиент.ВыполнитьКомандуСоздатьНаОсновании(ОписаниеКоманды.Идентификатор,
			ОбъектыОснований, Форма, ОписаниеКоманды);
...
Процедура ВыполнитьПодключаемуюКомандуСоздатьНаОснованииПодготовкаОбъектовСоздатьНаОсновании(ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.ОписаниеКоманды.ТипыОбъектовСозданияНаОсновании.Количество() <> 0 Тогда // требуется проверка типа
		...
		ОбъектыНеСоответствующиеПоТипу = Новый Массив;
		Для Каждого ОбъектСоздатьНаОсновании Из ОбъектыОснований Цикл
			Если ДополнительныеПараметры.ОписаниеКоманды.ТипыОбъектовСозданияНаОсновании.Найти(ТипЗнч(ОбъектСоздатьНаОсновании)) <> Неопределено Тогда
				ЕстьСоздаваемыеНаОснованииОбъекты = Истина;
				Прервать;
			Иначе
				ОбъектыНеСоответствующиеПоТипу.Добавить(ОбъектСоздатьНаОсновании);
			КонецЕсли;
		КонецЦикла;
		
		Если Не ЕстьСоздаваемыеНаОснованииОбъекты Тогда
			ТекстСообщения = ВводНаОснованииВызовСервера.СообщениеОПредназначенииКомандыСоздатьНаОсновании(ДополнительныеПараметры.ОписаниеКоманды.ТипыОбъектовСозданияНаОсновании);
			ПоказатьПредупреждение(, ТекстСообщения);
			Возврат;
		КонецЕсли;
		
		Если ОбъектыНеСоответствующиеПоТипу.Количество() > 0 Тогда
			Для каждого Объект Из ОбъектыНеСоответствующиеПоТипу Цикл
				СообщитьКомандаФормаНедоступна(Объект);
				ИндексОбъекта = ОбъектыОснований.Найти(Объект);
				Если ИндексОбъекта <> Неопределено Тогда
					ОбъектыОснований.Удалить(ИндексОбъекта);
				КонецЕсли;
			КонецЦикла; 
		КонецЕсли;
	...
	Если ДополнительныеПараметры.ОписаниеКоманды.ПроверкаПроведенияПередСозданиемНаОсновании Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьПодключаемуюКомандуСоздатьНаОсновании", ЭтотОбъект, ДополнительныеПараметры);
		ВводНаОснованииКлиент.ПроверитьПроведенностьДокументов(ОписаниеОповещения, ОбъектыОснований, ДополнительныеПараметры.Форма);
		Возврат;
	КонецЕсли;
	
	ВыполнитьПодключаемуюКомандуСоздатьНаОсновании(ОбъектыОснований, ДополнительныеПараметры);
	
КонецПроцедуры

Процедура ПроверитьПроведенностьДокументовДиалогПроведения(Параметры) Экспорт
	
	Если ВводНаОснованииВызовСервера.ДоступноПравоПроведения(Параметры.НепроведенныеДокументы) Тогда
...
Процедура ПроверитьПроведенностьДокументовПроведениеДокументов(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ШаблонСообщения = НСтр("ru='Документ %1 не проведен: %2';uk='Документ %1 не проведений: %2'");
...
Процедура СообщитьКомандаФормаНедоступна(Объект)
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Создание на основании %1 не выполнено: выбранная команда недоступна.';uk='Створення на підставі %1 не виконано: обрана команда недоступна.'"), Объект);
