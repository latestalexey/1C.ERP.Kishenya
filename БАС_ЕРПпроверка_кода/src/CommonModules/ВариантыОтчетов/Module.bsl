...
Функция ИспользуемыеРазделы() Экспорт
	...
	Для Каждого ЭлементСписка Из СписокРазделов Цикл
		...
		Иначе
			ШаблонЗаголовка = НСтр("ru='Отчеты раздела ""%1""';uk='Звіти розділу ""%1""'");
		КонецЕсли;
		...
		Если РазделМетаданные = ИдентификаторНачальнойСтраницы Тогда
			...
			Строка.Представление = НСтр("ru='Начальная страница';uk='Початкова сторінка'");
		Иначе
...
Функция ОписаниеОтчета(Настройки, ОтчетЗначениеИлиМетаданные) Экспорт
	...
	Если СтрокаОтчет = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Помилка отримання опису звіту ""%1"": він не підключений до підсистеми ""%2"".
            |Перевірте властивість ""Сховище варіантів"" у властивостях звіту.'"),
...
Функция ОписаниеВарианта(Настройки, ОтчетСтрокаДереваИлиЗначениеИлиМетаданные, КлючВарианта) Экспорт
	...
	Если СтрокаВариант = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Вариант ""%1"" не найден для отчета ""%2"".';uk='Варіант ""%1"" не знайдений для звіту ""%2"".'"),
			КлючВарианта,
...
Процедура ДобавитьОбработчикиПолногоОбновления(Обработчики, Версия) Экспорт
	...
	Если РазрешеноИндексированиеНеразделенныхДанных() Тогда
		...
		Обработчик.Комментарий   = НСтр("ru='Полное обновление индекса поиска отчетов, предусмотренных в программе.
            ...
            |;uk='Повне оновлення індексу пошуку звітів, передбачених у програмі.
            |Пошук звітів тимчасово недоступний.'");
	КонецЕсли;
	...
	Обработчик.Комментарий = НСтр("ru='Полное обновление индекса поиска отчетов, сохраненных пользователями.
        |Поиск отчетов временно недоступен.'
        |;uk='Повне оновлення індексу пошуку звітів, збережених користувачами.
        |Пошук звітів тимчасово недоступний.'");
...
Процедура СброситьПользовательскиеНастройки(Ключ, ТипыНастроек = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Некорректное значение параметра ""Отчет""';uk='Некоректне значення параметра ""Отчет""'");
	КонецЕсли;
	...
	Для Каждого ПолноеИмяВарианта Из КлючиВариантов Цикл
		...
		Пока Истина Цикл
			...
			Исключение
				...
				ОшибкаПоВарианту(
					Неопределено,
					НСтр("ru='В процессе выборки пользовательских настроек отчетов из системного хранилища возникла ошибка:';uk='У процесі вибірки налаштувань користувача звітів з системного сховища виникла помилка:'")
					+ Символы.ПС
...
Процедура ПеренестиПользовательскиеИзСтандартногоХранилища(ИменаОтчетов = "") Экспорт
	ПредставлениеПроцедуры = НСтр("ru='Прямая конвертация вариантов отчетов';uk='Пряма конвертація варіантів звітів'");
	...
	Пока Истина Цикл
		...
		Исключение
			...
			ОшибкаПоВарианту(
				Неопределено,
				НСтр("ru='В процессе выборки вариантов отчетов из стандартного хранилища возникла ошибка:';uk='У процесі вибірки варіантів звітів зі стандартного сховища виникла помилка:'")
				+ Символы.ПС
				+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура ЗагрузитьПользовательские(ТаблицаВариантов = Неопределено) Экспорт
	...
	ПредставлениеПроцедуры = НСтр("ru='Завершить конвертацию вариантов отчетов';uk='Завершити конвертацію варіантів звітів'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Для Каждого СтрокаТаблицы Из ОпределенныеВарианты Цикл
		...
		Иначе
			ОшибкаПоВарианту(
				...
				НСтр("ru='Вариант ""%1"" отчета ""%2"": не найден автор ""%3""';uk='Варіант ""%1"" звіту ""%2"": не знайдений автор ""%3""'"),
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если РазрешеноИндексированиеНеразделенныхДанных() Тогда
		...
		Обработчик.Комментарий   = НСтр("ru='Обновление индекса поиска отчетов, предусмотренных в программе.
            |Поиск отчетов временно недоступен.'
            |;uk='Оновлення індексу пошуку звітів, передбачених у програмі.
            |Пошук звітів тимчасово недоступний.'");
	КонецЕсли;
	...
	Обработчик.Комментарий = НСтр("ru='Уменьшает количество быстрых настроек
        ...
        |;uk='Зменшує кількість швидких налаштувань
        |у користувацьких звітах до 2 шт.'");
...
Функция ПодключитьОтчетИЗагрузитьНастройки(Параметры) Экспорт
		...
		Если ТипЗнч(Результат.СсылкаВарианта) <> Тип("СправочникСсылка.ВариантыОтчетов")
			...
			Если Результат.СсылкаВарианта = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В методе ""%1"" не указаны параметры ""%2"".';uk='У методі ""%1"" не вказані параметри ""%2"".'"),
	...
	Если ТипЗнч(Результат.НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		...
		Если ВариантНастроекКД = Неопределено Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Вариант ""%1"" не найден в схеме отчета ""%2"".';uk='Варіант ""%1"" не знайдено у схемі звіту ""%2"".'"),
...
Функция ПодключитьОтчетОбъект(СсылкаОтчета, ПолучатьМетаданные)
	...
	Если СсылкаОтчета = Неопределено Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В методе ""%1"" не указан параметр ""%2"".';uk='У методі ""%1"" не вказано параметр ""%2"".'"),
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("Строка") Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Отчет ""%1"" записан как внешний и не может быть подключен из программы';uk='Звіт ""%1"" записаний як зовнішній і не може бути підключений з програми'"),
			Результат.Ссылка);
		Возврат Результат;
	КонецЕсли;
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
		...
		Если ПолучатьМетаданные Тогда
			...
			Если Результат.Метаданные = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отчет ""%1"" не найден в программе';uk='Звіт ""%1"" не знайдено у програмі'"),
					Результат.Имя);
				Возврат Результат;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриЗаписиДополнительногоОтчета(ТекущийОбъект, Отказ, ВнешнийОбъект) Экспорт
	...
	Если Не ВариантыОтчетовПовтИсп.ПравоДобавления() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Недостаточно прав доступа для записи вариантов дополнительного отчета ""%1"".';uk='Недостатньо прав доступу для запису варіантів додаткового звіту ""%1"".'"),
...
Функция ПараметрыОткрытия(ВариантСсылка) Экспорт
	...
	Иначе
		...
		Если Не Выборка.Следующий() Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Недостаточно прав для открытия варианта ""%1"".';uk='Недостатньо прав для відкриття варіанту ""%1"".'"), Строка(ВариантСсылка));
		КонецЕсли;
...
Процедура ПриПодключенииОтчета(ПараметрыОткрытия) Экспорт
		...
		Если Метаданные.Отчеты.Найти(ПараметрыОткрытия.ИмяОтчета) = Неопределено Тогда
			СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(ПараметрыОткрытия,
				НСтр("ru='Отчет ""%1"" не найден в программе.
                    ...
                    |;uk='Звіт ""%1"" не знайдено у програмі.
                    |Можливо, розширення зі звітом було відключено.'"));
		Иначе
	...
	ИначеЕсли ПараметрыОткрытия.ТипОтчета = "Дополнительный" Тогда
		...
		Иначе
			СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(ПараметрыОткрытия,
				НСтр("ru='Невозможно открыть вариант дополнительного отчета';uk='Неможливо відкрити варіант додаткового звіту'"),
				НСтр("ru='Не внедрена обслуживающая подсистема.';uk='Не впроваджена обслуговуюча підсистема.'"));
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьОтчет(Знач Параметры, Знач ПроверятьЗаполнение, Знач ПолучатьФлажокПустой) Экспорт
	...
	Если Не Подключение.Успех Тогда
		Результат.ТекстОшибки = НСтр("ru='Не удалось сформировать отчет:';uk='Не вдалося сформувати звіт:'") + Символы.ПС + Подключение.ТекстОшибки;
		Возврат Результат;
	КонецЕсли;
	...
	Если ПроверятьЗаполнение Тогда
		...
		Если Не ПроверкаПройдена Тогда
			Результат.ТекстОшибки = НСтр("ru='Отчет не прошел проверку заполнения:';uk='Звіт не пройшов перевірку заповнення:'");
...
Функция ДеревоПредопределенных(ФильтрПоТипуОтчетов = "Внутренний") Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		Если СтрокаОтчет.ИспользуетСКД Тогда
			...
			Исключение
				ТекстОшибки = НСтр("ru='Не удалось прочитать схему отчета ""%1"":';uk='Не вдалося прочитати схему звіту ""%1"":'");
			...
			Если СхемаКД <> Неопределено Тогда
				...
				Исключение
					ТекстОшибки = НСтр("ru='Не удалось прочитать список вариантов отчета ""%1"" из схемы:';uk='Не вдалося прочитати список варіантів звіту ""%1"" зі схеми:'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ОтчетМетаданные.Имя);
					ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					ПредупреждениеПоВарианту(Неопределено, ТекстОшибки);
				КонецПопытки;
			...
			Если ВариантыНастроек = Неопределено Тогда
				...
				Исключение
					ТекстОшибки = НСтр("ru='Не удалось прочитать список вариантов отчета ""%1"" из модуля менеджера:';uk='Не вдалося прочитати список варіантів звіту ""%1"" з модуля менеджера:'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ОтчетМетаданные.Имя);
					ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			...
			Если ВариантыНастроек <> Неопределено Тогда
				Для Каждого ВариантНастроекКД Из ВариантыНастроек Цикл
					...
					Если РазрешеноИндексирование И ТипЗнч(ВариантНастроекКД) = Тип("ВариантНастроекКомпоновкиДанных") Тогда
						...
						Исключение
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Не удалось прочитать настройки варианта ""%1"" отчета ""%2"":';uk='Не вдалося прочитати настройки варіанту ""%1"" звіту ""%2"":'"),
...
Процедура УстановитьРежимВыводаОтчетаВПанеляхОтчетов(ДеревоВариантов, Отчет, ВыводитьОтчетыВместоВариантов)
	...
	Иначе
		...
		Если СтрокаОтчет = Неопределено Тогда
			ПредупреждениеПоВарианту(Неопределено, НСтр("ru='Отчет ""%1"" не подключен к подсистеме.';uk='Звіт ""%1"" не підключений до підсистеми.'"), Отчет.Имя);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	СтрокаОтчет.ГруппироватьПоОтчету = ВыводитьОтчетыВместоВариантов;
	...
	Для Каждого Изменение Из Изменения Цикл
		...
		Если Найденные.Количество() > 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Помилка реєстрації змін імені варіанта звіту ""%1"":
                |Актуальне ім''я варіанта ""%2"" (старе ім''я ""%3"")
                |так саме значитися як старе ім''я ""%4"" (актуальне ім''я ""%5"").'"),
		...
		Если Найденные.Количество() > 2 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Помилка реєстрації змін імені варіанта звіту ""%1"":
                |Старе ім''я варіанта ""%2"" (актуальне ім''я ""%3"")
                |так саме значитися як старе ім''я 
                |варіанта звіту ""%4"" (актуальне ім''я ""%5"").'"),
				Строка(Изменение.Отчет),
				Изменение.СтароеИмяВарианта,
				Изменение.АктуальноеИмяВарианта,
...
Функция СформироватьИнформациюОбОтчетеПоПолномуИмени(ОтчетПолноеИмя) Экспорт
	...
	Если ВРег(Префикс) = "ОТЧЕТ" Тогда
		...
		Если Результат.ОтчетМетаданные = Неопределено Тогда
			...
			ПредупреждениеПоВарианту(
				Неопределено,
				НСтр("ru='Отчет ""%1"" не найден в программе, он будет значиться как внешний.';uk='Звіт ""%1"" не знайдений у програмі, він буде значитися як зовнішній.'"),
				ОтчетПолноеИмя);
		ИначеЕсли Не ПравоДоступа("Просмотр", Результат.ОтчетМетаданные) Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Недостаточно прав доступа к отчету ""%1"".';uk='Недостатньо прав доступу до звіту ""%1"".'"),
				ОтчетПолноеИмя);
		КонецЕсли;
	...
	Иначе
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для отчета ""%1"" невозможно определить тип (не установлен префикс).';uk='Для звіту ""%1"" неможливо визначити тип (не встановлений префікс).'"),
			ОтчетПолноеИмя);
		Возврат Результат;
	КонецЕсли;
...
Функция ГлобальныеНастройки() Экспорт
	...
	Результат.Поиск.Вставить("ПодсказкаВвода", НСтр("ru='Наименование, поле или автор отчета';uk='Найменування, поле або автор звіту'"));
...
Процедура ЗапускПроцедуры(ПредставлениеПроцедуры)
	ИнформацияПоВарианту(Неопределено, НСтр("ru='Запуск процедуры ""%1"".';uk='Запуск процедури ""%1"".'"), ПредставлениеПроцедуры);
КонецПроцедуры
...
Процедура ЗавершениеПроцедуры(ПредставлениеПроцедуры, Изменено = Неопределено)
	Текст = НСтр("ru='Завершение процедуры ""%1"".';uk='Завершення процедури ""%1"".'");
	Если Изменено <> Неопределено Тогда
		Текст = Текст + " " + НСтр("ru='Изменено %2 объектов.';uk='Змінено %2 об''єктів.'");
	КонецЕсли;
...
Процедура ПерейтиКРедакции21() Экспорт
	ПредставлениеПроцедуры = НСтр("ru='Перейти к редакции 2.1';uk='Перейти до редакції 2.1'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗаполнитьСсылкиПредопределенных() Экспорт
	ПредставлениеПроцедуры = НСтр("ru='Заполнить ссылки предопределенных вариантов отчетов';uk='Заповнити посилання напередвизначених варіантів звітів'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура СократитьКоличествоБыстрыхНастроек(ВходящиеПараметры = Неопределено) Экспорт
	ПредставлениеПроцедуры = НСтр("ru='Сокращение количества быстрых настроек в отчетах';uk='Скорочення кількості швидких настройок у звітах'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОтчетов Цикл
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда // Возникла проблема.
			ТекстОшибки = НСтр("ru='Вариант ""%1"" отчета ""%2"":';uk='Варіант ""%1"" звіту ""%2"":'")
			+ Символы.ПС + НСтр("ru='При уменьшении количества быстрых настроек пользовательского возникла ошибка:';uk='При зменшенні кількості швидких настройок користувача виникла помилка:'")
	...
	Если Ошибок > 0 И НомерПопытки <= 30 Тогда
		...
		ТекстОшибки = ПредставлениеПроцедуры + ":"
			+ Символы.ПС + НСтр("ru='Не удалось уменьшить количество быстрых настроек %1 отчетов.';uk='Не вдалося зменшити кількість швидких настройок %1 звітів.'");
...
Функция ОперативноеОбновлениеОбщихДанных(Режим, РазделенныеОбработчики)
	...
	Кэш.Вставить("Уточнение", ?(Кэш.Режим = "ОбщиеДанныеКонфигурации", НСтр("ru='метаданные конфигурации';uk='метадані конфігурації'"), НСтр("ru='метаданные расширений';uk='метадані розширень'")));
...
Функция ОтложенноеОбновлениеДанных(Режим, Полное)
	...
	Кэш.Вставить("Уточнение", НРег(ОтчетыКлиентСервер.ИмяВПредставление(Режим)));
	Кэш.Уточнение = Кэш.Уточнение + ", " + ?(Полное, НСтр("ru='полное';uk='повне'"), НСтр("ru='по изменениям';uk='по змінам'"));
...
Процедура ОбновитьИндексПоиска(Кэш)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обновление индекса поиска (%1)';uk='Оновлення індексу пошуку (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Пока Выборка.Следующий() Цикл
		...
		ПримечаниеПоВарианту(Выборка.Ссылка, НСтр("ru='Индексация схемы';uk='Індексація схеми'"));
		...
		Если Кэш.ОбщиеДанные Тогда
			...
			Если Найденные.Количество() = 0 Тогда
				ТекстОшибки = НСтр("ru='Вариант ""%1"" не найден для отчета ""%2""';uk='Варіант ""%1"" не знайдено для звіту ""%2""'");
				ОшибкаПоВарианту(ВариантОбъект.Ссылка, ТекстОшибки, ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
...
Процедура АктуализироватьКлючиПредопределенных(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обновление ключей вариантов отчетов (%1)';uk='Оновлення ключів варіантів звітів (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ОбновитьНастройкиПредопределенных(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обновление настроек предопределенных (%1)';uk='Оновлення настройок напередвизначених (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура АктуализироватьСоставОбласти(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обновление вариантов отчетов (%1)';uk='Оновлення варіантів звітів (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ПометитьНаУдалениеВариантыУдаленныхОтчетов(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Удаление вариантов удаленных отчетов (%1)';uk='Вилучення варіантів віддалених звітів (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗарегистрироватьЗамерыВариантаКОбновлению(Кэш, Знач СтарыйКлюч, Знач АктуальныйКлюч, Знач АктуальноеНаименование)
	...
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Отчет ""%1"" (открытие)';uk='Звіт ""%1"" (відкриття)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Кэш.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Формирование";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Формирование";
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Отчет ""%1"" (формирование)';uk='Звіт ""%1"" (формування)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Кэш.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Настройки";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Настройки";
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Отчет ""%1"" (настройки)';uk='Звіт ""%1"" (настройки)'"), АктуальноеНаименование);
КонецПроцедуры
...
Функция СписокГлобальныхКлючевыхОпераций()
	...
	СписокОпераций.Добавить("ПанельОтчетов.Открытие",       НСтр("ru='Панель отчетов (открытие)';uk='Панель звітів (відкриття)'"));
	СписокОпераций.Добавить("ПанельОтчетов.Поиск",          НСтр("ru='Панель отчетов (поиск в режиме просмотра)';uk='Панель звітів (пошук в режимі перегляду)'"));
	СписокОпераций.Добавить("ПанельОтчетов.РежимНастройки", НСтр("ru='Панель отчетов (режим настройки)';uk='Панель звітів (режим)'"));
	СписокОпераций.Добавить("СписокОтчетов.Открытие", НСтр("ru='Список отчетов (открытие)';uk='Список звітів (відкриття)'"));
	СписокОпераций.Добавить("СписокОтчетов.Поиск",    НСтр("ru='Список отчетов (поиск)';uk='Список звітів (пошук)'"));
...
Процедура ЗаписатьПараметрыВариантовОтчетов(Кэш)
	...
	КонецЕсли;
	ПредставлениеПроцедуры = НСтр("ru='Запись неразделенного кэша в константу';uk='Запис неподіленого кеша в константу'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗафиксироватьТекущуюВерсиюРасширений(Кэш)
	...
	ПредставлениеПроцедуры = НСтр("ru='Запись регистра версий расширений';uk='Запис регістра версій розширень'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Функция ПроиндексироватьСодержимоеСхемы(ВариантОбъект) Экспорт
	...
	Если СхемаКД = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Для варіанта ""%1"" звіту ""%2"" не заповнені параметри пошуку:
            |Найменування полів або Найменування параметрів і відборів.'"),
			ВариантОбъект.КлючВарианта,
			ВариантОбъект.Отчет);
		Если ЭтоПредопределенный Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС
				+ НСтр("ru='Подробнее - см. процедуру ""НастроитьВариантыОтчетов"" модуля ""ВариантыОтчетовПереопределяемый"".';uk='Детальніше - див. процедуру ""НастроитьВариантыОтчетов"" модуля ""ВариантыОтчетовПереопределяемый"".'");
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		Если ТипЗнч(ВариантОбъект) = Тип("СправочникОбъект.ПредопределенныеВариантыОтчетов")
			...
			Текст = НСтр("ru='Не удалось прочитать настройки предопределенного варианта отчета ""%1"".';uk='Не вдалося прочитати настройки напередвизначеного варіанту звіту ""%1"".'");
...
Функция УменьшитьКоличествоБыстрыхНастроек(ВариантОбъект, ОтчетОбъект)
	...
	НастройкиКД = ВариантОбъект.Настройки.Получить();
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		ТекстОшибки = НСтр("ru='Обнаружены пустые настройки пользовательского варианта ""%1"" отчета ""%2"".';uk='Виявлені порожні настройки користувацького варіанту ""%1"" звіту ""%2"".'");
		ОшибкаПоВарианту(ВариантОбъект.Ссылка, ТекстОшибки, ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
	...
	Если ОтчетыСНастройками.Найти(ОтчетСсылка) <> Неопределено Тогда
		Если ОтчетОбъект = Неопределено Тогда
			...
			Если Подключение.Успех Тогда
				ОтчетОбъект = Подключение.Объект;
			Иначе
				Текст = НСтр("ru='Не удалось получить настройки отчета ""%1"":';uk='Не вдалося отримати  настройки звіту ""%1"":'") + Символы.ПС + Подключение.ТекстОшибки;
	...
	Если ЕстьОтборПоПодсистемам Или ЕстьСтрокаПоиска Тогда
		...
		Если Не ЕстьСтрокаПоиска И Не ПолучатьИтоговуюТаблицу Тогда
			...
			ТекстЗапроса = СтрЗаменить(
				...
				"НЕОПРЕДЕЛЕНО");
		Иначе
			Запрос.УстановитьПараметр("НаименованиеРабочегоСтола", НСтр("ru='Начальная страница';uk='Початкова сторінка'"));
		КонецЕсли;
