...
Процедура ВедомостьСпособВыплатыПриИзмененииНаСервере(Форма) Экспорт
	...
	Если Форма.ХарактерВыплаты <> ИнформацияОСпособеВыплаты.ПараметрыЗаполнения.ХарактерВыплаты Тогда
		...
		ВзаиморасчетыССотрудникамиКлиентСерверРасширенный.УстановитьОтображениеВХО(Форма, Ложь);
	КонецЕсли;	
...
Процедура ВедомостьПриПолученииДанныхСтрокиСоставаНДФЛ(Форма, СтрокаСостава)
	...
	Если ЕстьПрошлыеПериоды Тогда
		РасшифровкаПериодов = НСтр("ru='в т.ч. в счет ранее исчисленного';uk='в т. ч. в рахунок раніше обчисленого'");
	Иначе
	КонецЕсли;	

	Если Форма.ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Аванс И ТипыОснований.Количество() > 0 Тогда
		
		РасшифровкаОснований = НСтр("ru='за';uk='за'") + " ";
...
Процедура ВедомостьОбновитьВзносыНаСервере(Форма) Экспорт
	...
	ВедомостьЗаполнитьВзносы(Форма, СтрокиСостава);
	
КонецПроцедуры

Процедура ВедомостьЗаполнитьВзносы(Форма, СтрокиСостава)
	...
	Если ТекущийОбъект.МожноЗаполнитьЗарплату() Тогда
		ВзаиморасчетыССотрудникамиРасширенный.ВедомостьОбновитьВзносы(ТекущийОбъект, ФизЛица);
	КонецЕсли;	
...
Функция ВедомостьАдресВХранилищеВзносы(Форма) Экспорт
	Возврат ПоместитьВоВременноеХранилище(Форма.Объект.ВзносыФОТ.Выгрузить(), Форма.УникальныйИдентификатор);
КонецФункции

Процедура ВедомостьРедактироватьВзносыТаблицыЗавершениеНаСервере(Форма, РезультатыРедактирования) Экспорт
	
	Взносы	= ПолучитьИзВременногоХранилища(РезультатыРедактирования.АдресВХранилищеВзносы);
	
	Форма.Объект.ВзносыФОТ.Загрузить(Взносы);
