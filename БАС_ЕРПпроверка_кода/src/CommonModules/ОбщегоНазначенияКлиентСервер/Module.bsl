	...
	Для Каждого Элемент Из Источник Цикл
		Если Заменять <> Истина И Приемник.Свойство(Элемент.Ключ) Тогда
			Если Заменять = Ложь Тогда
				Продолжить;
			Иначе
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Пересечение ключей источника и приемника: ""%1"".';uk='Перетин ключів джерела і приймача: ""%1"".'"), Элемент.Ключ);
			КонецЕсли
		КонецЕсли;
...
Процедура УстановитьСостояниеПоляТабличногоДокумента(ПолеТабличногоДокумента, Состояние = "НеИспользовать") Экспорт
	...
	Если ТипЗнч(ПолеТабличногоДокумента) = Тип("ПолеФормы") 
		...
		ИначеЕсли ВРег(Состояние) = "НЕАКТУАЛЬНОСТЬ" Тогда
			...
			ОтображениеСостояния.Картинка                       = Новый Картинка;
			ОтображениеСостояния.Текст                          = НСтр("ru='Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.';uk='Звіт не сформований. Натисніть ""Сформувати"" для одержання звіту.'");;
		ИначеЕсли ВРег(Состояние) = "ФОРМИРОВАНИЕОТЧЕТА" Тогда  
			ОтображениеСостояния.Видимость                      = Истина;
			ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
			ОтображениеСостояния.Картинка                       = БиблиотекаКартинок.ДлительнаяОперация48;
			ОтображениеСостояния.Текст                          = НСтр("ru='Отчет формируется...';uk='Звіт формується...'");
		Иначе
...
// Возвращает имя COM-класса для работы с платформой через COM-соединение.
//
...
Функция УстановитьВнешнееСоединениеСБазой(Параметры) Экспорт
	...
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		...
		КраткоеОписаниеОшибки = НСтр("ru='Прямое подключение к информационной базе недоступно на сервере под управлением ОС Linux.';uk='Пряме підключення до інформаційної бази недоступно на сервері під управлінням ОС Linux.'");
	#Иначе
		ЭтоLinux = ЭтоLinuxКлиент();
		КраткоеОписаниеОшибки = НСтр("ru='Прямое подключение к информационной базе недоступно на клиенте под управлением ОС Linux.';uk='Пряме підключення до інформаційної бази недоступне на клієнті під управлінням ОС Linux.'");
	#КонецЕсли
	...
	Исключение
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Не удалось подключится к другой программе: %1';uk='Не вдалося підключиться до іншої програми: %1'");
	...
	Если ФайловыйВариантРаботы Тогда
		...
		Если ПустаяСтрока(Параметры.КаталогИнформационнойБазы) Тогда
			СтрокаСообщенияОбОшибке = НСтр("ru='Не задано месторасположение каталога информационной базы.';uk='Не задане місце розташування каталогу інформаційної бази.'");
	...
	Иначе
		...
		Если ПустаяСтрока(Параметры.ИмяСервера1СПредприятия) Или ПустаяСтрока(Параметры.ИмяИнформационнойБазыНаСервере1СПредприятия) Тогда
			СтрокаСообщенияОбОшибке = НСтр("ru='Не заданы обязательные параметры подключения: ""Имя сервера""; ""Имя информационной базы на сервере"".';uk='Не задані обов''язкові параметри підключення: ""Ім''я сервера""; ""Ім''я інформаційної бази на сервері"".'");
			ОшибкаПроверкиЗаполнения = Истина;
		КонецЕсли;
	...
	Исключение
		Информация = ИнформацияОбОшибке();
		СтрокаСообщенияОбОшибке = НСтр("ru='Не удалось подключиться к другой программе: %1';uk='Не вдалося підключитися до іншої програми: %1'");
	...
	Для Каждого СтрокаАдреса Из МассивАдресов Цикл
		...
		Если АдресОпределен Тогда 
			Если ЕстьНедопустимыеСимволыВПсевдониме Тогда
				ОписаниеОшибки = НСтр("ru='Представление содержит недопустимые символы';uk='Представлення містить неприпустимі символи'");
			КонецЕсли;
		Иначе
			Если ЕстьПризнакНаличияАдресаВСтроке Тогда 
				ОписаниеОшибки = НСтр("ru='Адрес электронной почты содержит ошибки';uk='Адреса електронної пошти містить помилки'");
			Иначе
				ОписаниеОшибки = НСтр("ru='Строка не содержит адреса электронной почты';uk='Рядок не містить адреси електронної пошти'");
			КонецЕсли;
		КонецЕсли;	
...
Функция РазобратьСтрокуСПочтовымиАдресами(Знач СтрокаПереданныхАдресов, ВызыватьИсключение = Истина) Экспорт
	...
	СообщениеНедопустимыеСимволы = НСтр("ru='Недопустимый символ ""%1"" в адресе электронной почты ""%2""';uk='Неприпустимий символ ""%1"" в адресі електронної пошти ""%2""'");
	СообщениеНеверныйФорматПочтовогоАдреса = НСтр("ru='Некорректный адрес электронной почты ""%1""';uk='Некоректна адреса електронної пошти ""%1""'");
...
Процедура Проверить(Знач Условие, Знач Сообщение = "", Знач КонтекстПроверки = "") Экспорт
	...
	Если Условие <> Истина Тогда
		Если ПустаяСтрока(Сообщение) Тогда
			ТекстИсключения = НСтр("ru='Недопустимая операция';uk='Неприпустима операція'"); // Assertion failed
		Иначе
			ТекстИсключения = Сообщение;
		КонецЕсли;
		Если Не ПустаяСтрока(КонтекстПроверки) Тогда
			ТекстИсключения = ТекстИсключения + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='в %1';uk='в %1'"), КонтекстПроверки);
		КонецЕсли;
...
Процедура ПроверитьПараметр(Знач ИмяПроцедурыИлиФункции, Знач ИмяПараметра, Знач ЗначениеПараметра, 
	...
	Проверить(ТипЗнч(ИмяПроцедурыИлиФункции) = Тип("Строка"), 
		НСтр("ru='Недопустимо значение параметра ИмяПроцедурыИлиФункции';uk='Неприпустиме значення параметра ИмяПроцедурыИлиФункции'"), Контекст);
	Проверить(ТипЗнч(ИмяПараметра) = Тип("Строка"), 
		НСтр("ru='Недопустимо значение параметра ИмяПараметра';uk='Неприпустиме значення параметра ИмяПараметра'"), Контекст);
		
	ЭтоОписаниеТипов = ТипЗнч(ОжидаемыеТипы) = Тип("ОписаниеТипов");
	Проверить(ЭтоОписаниеТипов Или ТипЗнч(ОжидаемыеТипы) = Тип("Тип"), 
		НСтр("ru='Недопустимо значение параметра ОжидаемыеТипы';uk='Неприпустиме значення параметра ОжидаемыеТипы'"), Контекст);
	...
	Проверить(ЭтоКорректныйТип, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НедопустимыйПараметр,
		...
		ОжидаемыеТипы, 
		?(ЗначениеПараметра <> Неопределено, ЗначениеПараметра, НСтр("ru='Неопределено';uk='Невизначено'")),
	...
	Если ТипЗнч(ЗначениеПараметра) = Тип("Структура") И ОжидаемыеТипыСвойств <> Неопределено Тогда
		...
		НетСвойства = НСтр("ru='Недопустимое значение параметра %1 (Структура) в %2. 
            ...
            |;uk='Неприпустиме значення параметра %1 (Структура) в %2. 
            |У структурі очікувалася властивість %3 (тип %4).'");
		НедопустимоеСвойство = НСтр("ru='Недопустимое значение свойства %1 в параметре %2 (Структура) в %3. 
            ...
            |;uk='Неприпустиме значення властивості %1 у параметрі %2 (Структура) в %3. 
            |Очікувалося: %4; передане значення: %5 (тип %6).'");
		Для каждого Свойство Из ОжидаемыеТипыСвойств Цикл
			...
			Проверить(ЭтоКорректныйТип, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НедопустимоеСвойство, 
				...
				?(ЗначениеСвойства <> Неопределено, ЗначениеСвойства, НСтр("ru='Неопределено';uk='Невизначено'")),
...
Функция ПроверитьИПодготовитьПочтовыйАдрес(Знач ПолноеИмяАдресата, Знач ПочтовыйАдрес)
	...
	НедопустимыйСимволВИмениАдресата = НСтр("ru='Недопустимый символ в имени адресата.';uk='Неприпустимий символ в імені адресата.'");
	НедопустимыйСимволВПочтовомАдресе = НСтр("ru='Недопустимый символ в почтовом адресе.';uk='Неприпустимий символ у поштовій адресі.'");
	СимволыОбрамления = "<>[]";
