...
Процедура УстановитьПараметрыВыбораДоговораСЗаказчиком(Объект, Элемент) Экспорт
	
	ПараметрыВыбораДоговора = Новый Массив;
	
	ПараметрыВыбораДоговора.Добавить(Новый ПараметрВыбора("Отбор.ТипДоговора", Перечисления.ТипыДоговоров.СПокупателем));
...
Процедура ЗаполнитьДокументПоЗаявкеНаРасходованиеДенежныхСредств(
	...
	Если Не Выборка.ЭтоВыплатаЗарплаты Тогда
		...
		//++ НЕ УТ
		|	МИНИМУМ(тчРасшифровкаПлатежа.СчетУчета) КАК СчетУчета,
		|	МИНИМУМ(тчРасшифровкаПлатежа.Субконто1) КАК Субконто1,
		|	МИНИМУМ(тчРасшифровкаПлатежа.Субконто2) КАК Субконто2,
		|	МИНИМУМ(тчРасшифровкаПлатежа.Субконто3) КАК Субконто3,
		//-- НЕ УТ
	...
	//++ НЕ УТ
	Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыПоЗарплатномуПроекту
		И Константы.ИспользоватьНачислениеЗарплаты.Получить() Тогда
		
		СтруктураПараметров = Новый Структура();
		СтруктураПараметров.Вставить("Заявка", Заявка);
		СтруктураПараметров.Вставить("Списание", Документы.СписаниеБезналичныхДенежныхСредств.ПустаяСсылка());
		СтруктураПараметров.Вставить("ХозяйственнаяОперация", Выборка.ХозяйственнаяОперация);
		СтруктураПараметров.Вставить("Организация", Выборка.Организация);
		
		ЗаполнитьПоВедомостямИЗаявкамКОплате(ТаблицаВедомостей, СтруктураПараметров);
		
		Если ТаблицаВедомостей.Итог("Сумма") > 0 Тогда
			ДанныеЗаполнения.Вставить("СуммаДокумента", ТаблицаВедомостей.Итог("Сумма"));
		Иначе 
			ДанныеЗаполнения.Вставить("СуммаДокумента", Выборка.СуммаДокумента);
		КонецЕсли;
		
	ИначеЕсли Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыЧерезКассу
		И Константы.ИспользоватьНачислениеЗарплаты.Получить() Тогда
		
		ТабВедомости = Новый ТаблицаЗначений;
		ТабВедомости.Колонки.Добавить("Ведомость");
		ТабВедомости.Колонки.Добавить("Сумма");
		...
		ЗаполнитьПоВедомостямИЗаявкамКОплате(ТабВедомости, СтруктураПараметров);
		
		Если ТабВедомости.Количество() > 0 Тогда
			
			ДанныеЗаполнения.Вставить("Ведомость", ТабВедомости[0].Ведомость);
			
			Параметры = Новый Структура;
			Параметры.Вставить("МассивВедомостей", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТабВедомости[0].Ведомость));
			Параметры.Вставить("Организация", Выборка.Организация);
			Параметры.Вставить("ИсключатьОплаченные", Истина);
			Параметры.Вставить("ОтображатьДепонированные", Истина);
			
			ТаблицаВедомостей.Загрузить(ДанныеРаботниковПоВедомостям(Параметры));
...
//	ТаблицаРаботников - ТаблицаЗначений, ТабличнаяЧасть - Таблица, в которую помещаются работники певой ведомости
//
...
Функция ДанныеРаботниковПоВедомостям(СтруктураПараметров, МенеджерВременныхТаблиц = Неопределено) Экспорт
	...
	|	ДанныеВедомостейНаОплату.ГруппаУчетаНачислений КАК ГруппаУчетаНачислений,
	...
	|	ДанныеВедомостейНаОплату.ГруппаУчетаНачислений
	|";
	
	ТаблицаРаботников = Запрос.Выполнить().Выгрузить();
	
	Если СтруктураПараметров.Свойство("Работник") И НЕ СтруктураПараметров.Свойство("ВсеПоРаботнику") Тогда
		
		Если ТаблицаРаботников.Количество() > 0 Тогда
            Если СтруктураПараметров.Свойство("СписокГрупп") Тогда
				Для Каждого СтрокаТаблицы из ТаблицаРаботников Цикл
					Если СтруктураПараметров.СписокГрупп.Найти(СтрокаТаблицы.ГруппаУчетаНачислений) = Неопределено Тогда
						Возврат Новый Структура("ГруппаУчетаНачислений, Сумма", СтрокаТаблицы.ГруппаУчетаНачислений, СтрокаТаблицы.Сумма);;
					КонецЕсли;	
				КонецЦикла;
				Возврат Новый Структура("ГруппаУчетаНачислений, Сумма", ТаблицаРаботников[0].ГруппаУчетаНачислений, ТаблицаРаботников[0].Сумма);
			Иначе	
				Возврат Новый Структура("ГруппаУчетаНачислений, Сумма", ТаблицаРаботников[0].ГруппаУчетаНачислений, ТаблицаРаботников[0].Сумма);
			КонецЕсли;	
		Иначе
...
Процедура ЗаполнитьПоВедомостямИЗаявкамКОплате(ТаблицаВедомостей, СтруктураПараметров) Экспорт
	...
	Если СтруктураПараметров.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыПоЗарплатномуПроекту Тогда
		...
		|ИЗ
		|	Документ.ЗаявкаНаРасходованиеДенежныхСредств.ВедомостиНаВыплатуЗарплаты КАК ЗаявкаВедомости
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СписаниеБезналичныхДенежныхСредств.ВедомостиНаВыплатуЗарплаты КАК СписаниеВедомости
	...
	ИначеЕсли СтруктураПараметров.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета Тогда
		...
		|			И ОстаткиКВыплате.ПодотчетноеЛицо = Ведомости.Сотрудник.ФизическоеЛицо
		|ГДЕ
		...
		|			И ДенежныеСредстваКВыплате.ПодотчетноеЛицо = Ведомости.Сотрудник.ФизическоеЛицо
		|ГДЕ
	...
	ИначеЕсли СтруктураПараметров.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыЧерезКассу
		...
		|ИЗ
		|	Документ.ЗаявкаНаРасходованиеДенежныхСредств.ВедомостиНаВыплатуЗарплаты КАК ЗаявкаВедомости
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ДенежныеСредстваКВыплате.Остатки КАК КВыплатеОстатки
...
Процедура ДанныеОбОплатеВедомостей(МенеджерВременныхТаблиц, Параметры) Экспорт
	...
	|	Работники.ГруппаУчетаНачислений КАК ГруппаУчетаНачислений,
	...
	|	Оплата.ГруппаУчетаНачислений КАК ГруппаУчетаНачислений,
	|	ИСТИНА КАК Оплачена,
	|	ВЫБОР
	...
	|	ВТРаботники.ГруппаУчетаНачислений,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаРасходованиеДенежныхСредств.ВедомостиНаВыплатуЗарплаты КАК ВедомостиЗаявки
	...
	|			И ВТРаботники.ГруппаУчетаНачислений = ВТСостоянияВедомостей.ГруппаУчетаНачислений
	|ГДЕ
...
Функция ДанныеРасшифровкиПлатежаДокументаДДСПоДоговоруЗаймаСотруднику(ДанныеЗаполнения, ЭтоПоступление = Ложь, 
	...
	Если ДанныеЗаполнения.Свойство("ДоговорЗаймаСотруднику") Тогда
		ДействующиеДоговора = Новый Массив;
		ДействующиеДоговора.Добавить(ДанныеЗаполнения.ДоговорЗаймаСотруднику);
	Иначе
	...
	Если ЭтоПоступление Тогда
		...
		Для Каждого Договор Из ДействующиеДоговора Цикл
			...
			Задолженность = ЗаймыСотрудникам.ОстатокЗадолженности(
			Договор,
			ДатаАктуальности,
			ДокументИсключение);
...
Функция СтатьиДДСЗаймовСотрудникуПоУмолчанию(ДатаАктуальности, ДокументИслючение = Неопределено,
		ТипДокумента = "ПриходныйКассовыйОрдер") Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", ДокументИслючение);
...
Функция ПолучитьРасшифровкуПлатежаНДС(СуммаДокумента, ВалютаДокумента, МассивДокументов, ОплатаОтКлиента = Ложь) Экспорт
	...
	|	ПО
	|		Документ.Ссылка = ТаблицаДокумента.Ссылка
	|
	|ГДЕ
...
Функция ТекстЗапросаЗаказыКПоступлению() Экспорт
	...
	|	Распоряжение.Валюта,
	|	Распоряжение.ХозяйственнаяОперация,
...
// Формирует текст запроса для дополнения динамического списка заказов к оплате ведомостями на выплату зарплаты в безналичной форме
//
// Возвращаемое значение:
//    Строка - Текст запроса
//
Функция ТекстЗапросаВедомостиКОплатеВБезналичнойФорме() Экспорт
	...
	|	
	|	ИСТИНА                                                                            КАК ТребуетсяОплата,
	|	СУММА(ТаблицаЗарплата.КВыплате)                                                   КАК СуммаКОплате,
	|	ДанныеДокумента.СуммаПоДокументу                                                  КАК СуммаДокумента,
	...
	|ИЗ
	|	Документ.ВедомостьНаВыплатуЗарплатыВБанк КАК ДанныеДокумента
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВедомостьНаВыплатуЗарплатыВБанк.Зарплата КАК ТаблицаЗарплата
	|		ПО ДанныеДокумента.Ссылка = ТаблицаЗарплата.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОплатаВедомостейНаВыплатуЗарплаты КАК ОплатаВедомостей
	|		ПО (ТаблицаЗарплата.Ссылка = ОплатаВедомостей.Ведомость)
	|			И (ТаблицаЗарплата.Сотрудник.ФизическоеЛицо = ОплатаВедомостей.ФизическоеЛицо)
	|ГДЕ
	|	ОплатаВедомостей.Регистратор ЕСТЬ NULL 
	|	И ДанныеДокумента.Проведен
	|	И &ВыводитьВедомостиНаВыплатуЗарплаты
	|{ГДЕ
	|	ДанныеДокумента.Организация В (&РазрешенныеЗначенияПоляОрганизация)
	|}
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеДокумента.Ссылка,
	...
	|	НЕОПРЕДЕЛЕНО,
	|	ЗНАЧЕНИЕ(Справочник.Приоритеты.ПустаяСсылка),
	|	НЕОПРЕДЕЛЕНО,
	|	
	|	ИСТИНА,
	|	СУММА(ТаблицаЗарплата.КВыплате),
	|	ДанныеДокумента.СуммаПоДокументу,
	|	&ВалютаРегламентированногоУчета,
	...
	|ИЗ
	|	Документ.ВедомостьНаВыплатуЗарплатыПеречислением КАК ДанныеДокумента
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВедомостьНаВыплатуЗарплатыПеречислением.Зарплата КАК ТаблицаЗарплата
	|		ПО ДанныеДокумента.Ссылка = ТаблицаЗарплата.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОплатаВедомостейНаВыплатуЗарплаты КАК ОплатаВедомостей
	|		ПО (ТаблицаЗарплата.Ссылка = ОплатаВедомостей.Ведомость)
	|			И (ТаблицаЗарплата.Сотрудник.ФизическоеЛицо = ОплатаВедомостей.ФизическоеЛицо)
...
// Формирует текст запроса для дополнения динамического списка заказов к оплате ведомостями на выплату зарплаты наличными
//
// Возвращаемое значение:
//    Строка - Текст запроса
//
Функция ТекстЗапросаВедомостиКОплатеНаличными() Экспорт
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	Зарплата.Ссылка                                                            КАК Ссылка,
	|	Зарплата.Ссылка.Дата                                                       КАК Дата,
	|	Зарплата.Ссылка.Номер                                                      КАК Номер,
	|	Зарплата.Ссылка.Проведен                                                   КАК Проведен,
	|	
	|	ТИПЗНАЧЕНИЯ(Зарплата.Ссылка)                                               КАК Тип,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыЧерезКассу)     КАК ХозяйственнаяОперация,
	|	&ВалютаРегламентированногоУчета                                            КАК Валюта,
	|	
	|	НЕОПРЕДЕЛЕНО                                                               КАК Партнер,
	|	НЕОПРЕДЕЛЕНО                                                               КАК Контрагент,
	...
	|	
	|	ИСТИНА                                                                     КАК ТребуетсяОплата,
	|	СУММА(Зарплата.КВыплате)                                                   КАК СуммаКОплате,
	|	Зарплата.Ссылка.СуммаПоДокументу                                           КАК СуммаДокумента,
	|	&ВалютаРегламентированногоУчета                                            КАК ВалютаДокумента,
	...
	|	И &ВыводитьВедомостиНаВыплатуЗарплаты
	|{ГДЕ
	|	Зарплата.Ссылка.Организация В (&РазрешенныеЗначенияПоляОрганизация)
	|}
	|
	|СГРУППИРОВАТЬ ПО
	|	Зарплата.Ссылка,
	...
	|	Зарплата.Ссылка.Номер,
	|	Зарплата.Ссылка.Проведен,
	|	ТИПЗНАЧЕНИЯ(Зарплата.Ссылка),
	...
	|	Зарплата.Ссылка.СуммаПоДокументу,
	|	&ВалютаРегламентированногоУчета,
...
Процедура ПроверитьЗаполнениеРасшифровкиПлатежа(
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Организация в %1 в строке %2 списка ""Расшифровка платежа"" не соответствует организации документа %3';uk='Організація %1 в рядку %2 списку ""Розшифровка платежу"" не відповідає організації документа %3'"),
			ТекстДокумент,
			Выборка.НомерСтроки,
			ДокументОбъект.Организация);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
Процедура ПроверитьСуммуПоЕдиномуНалогу(ДокументОбъект, Отказ, СуммаДляКонтроля = 0) Экспорт
	...
	Если ДокументОбъект.СуммаПоЕдиномуНалогу > СуммаДляКонтроля Тогда
	    ...
	    Текст = НСтр("ru='Сумма дохода по единому налогу больше, чему сумма документа';uk='Сума доходу по єдиному податку більше, чому сума документа'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Текст,
			ДокументОбъект,
...
Функция СформироватьДанныеЗаполненияСписанияДСПоНесколькимДокументам(МассивСсылок) Экспорт
	...
	Если СообщитьОбОшибкахФормированияДанныхЗаполненияСписанияДС(Выборка) Тогда
		...
		ТекстОшибки = НСтр("ru='Ввод одного документа Списание безналичных ДС на основании выделенных распоряжений невозможен';uk='Введення одного документа Списання безготівкових коштів на підставі виділених розпоряджень неможливе'");
...
Процедура ЗарегистрироватьОплатуВедомостей(Объект, ДополнительныеСвойства, Отказ) Экспорт
	...
	ВзаиморасчетыССотрудниками.ЗарегистрироватьОплатуВедомостейДокументом(
		Объект,
		Отказ,
		ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаОплатаВедомостей);
...
Процедура ЗарегистрироватьВозвратПоВедомостям(Объект, ДополнительныеСвойства, Отказ) Экспорт
	...
	Для Каждого Ведомость Из ТаблицаОплатаВедомостей Цикл
		ВзаиморасчетыССотрудниками.ЗарегистрироватьНевыплатуПоВедомости(Объект.Движения, Отказ, Ведомость.Ведомость, МассивФизЛиц);
	КонецЦикла;
...
Процедура РассчитатьИЗарегистрироватьУдержанныеНалоги(Объект, ДополнительныеСвойства, Отказ) Экспорт
...
//
Функция ДанныеВедомостейНаВыплатуЗарплаты(МассивВедомостей) Экспорт
	...
	|	ИСТИНА КАК ТребуетсяЗаполнениеСчета,
	...
	|	МАКСИМУМ(ВТДанныеЗаполнения.ФормаОплатыБезналичная) КАК ФормаОплатыБезналичная,
	|	МАКСИМУМ(ВТДанныеЗаполнения.ФормаОплатыНаличная) КАК ФормаОплатыНаличная,
	|	МАКСИМУМ(ВТДанныеЗаполнения.ФормаОплатыЗаявки) КАК ФормаОплатыЗаявки,
	|	МАКСИМУМ(ВТДанныеЗаполнения.ТребуетсяЗаполнениеСчета) КАК ТребуетсяЗаполнениеСчета,
	|	МАКСИМУМ(ВТДанныеЗаполнения.ХозяйственнаяОперацияПоЗарплате) КАК ХозяйственнаяОперацияПоЗарплате,
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТДанныеЗаполнения.ХозяйственнаяОперацияПоЗарплате) КАК КоличествоХозОпераций,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТДанныеЗаполнения.Организация) КАК КоличествоОрганизаций,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТДанныеЗаполнения.ЗарплатныйПроект) КАК КоличествоЗарплатныхПроектов,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТДанныеЗаполнения.Банк) КАК КоличествоБанков,
	|	МАКСИМУМ(ВТДанныеЗаполнения.Касса) КАК Касса
	|ИЗ
	...
	Если ЗначениеЗаполнено(РеквизитыШапки.ВедомостьНеПроведена) Тогда
		Текст = НСтр("ru='Документ %Документ% не проведен. Ввод на основании непроведенного документа запрещен.';uk='Документ %Документ% не проведено. Введення на підставі непроведенного документа заборонене.'");
	...
	Если РеквизитыШапки.КоличествоХозОпераций > 1 Тогда
		Текст = НСтр("ru='У выделенных ведомостей отличается форма оплаты. Ввод одного документа на основании выделенных ведомостей невозможен.';uk='У виділених відомостей відрізняється форма оплати. Введення одного документа на підставі виділених відомостей неможливе.'");
		СтруктураОшибки = Новый Структура("Идентификатор, Текст", "ОшибкаФормаОплаты", Текст);
		Результат.МассивОшибок.Добавить(СтруктураОшибки);
	КонецЕсли;
	
	Если РеквизитыШапки.КоличествоОрганизаций > 1 Тогда
		Текст = НСтр("ru='У выделенных ведомостей отличается поле ""Организация"". Ввод одного документа на основании выделенных ведомостей невозможен.';uk='У виділених відомостей відрізняється полі ""Організація"". Введення одного документа на підставі виділених відомостей неможливе.'");
	...
	Если РеквизитыШапки.КоличествоЗарплатныхПроектов > 1 Тогда
		Текст = НСтр("ru='У выделенных ведомостей отличается поле ""Зарплатный проект"". Ввод одного документа на основании выделенных ведомостей невозможен.';uk='У виділених відомостей відрізняється полі ""Зарплатний проект"". Введення одного документа на підставі виділених відомостей неможливе.'");
	...
	Если РеквизитыШапки.КоличествоБанков > 1 Тогда
		Текст = НСтр("ru='У выделенных ведомостей отличается поле ""Банк"". Ввод одного документа на основании выделенных ведомостей невозможен.';uk='У виділених відомостей відрізняється полі ""Банк"". Введення одного документа на підставі виділених відомостей неможливе.'");
