...
Функция ТипПлатформыСервераСтрокой() Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неизвестный тип платформы ""%1""';uk='Невідомий тип платформи ""%1""'"), Строка(СистемнаяИнфо.ТипПлатформы));
...
Функция ЗаписатьОшибкуВЖурналРегистрацииПриЗапускеИлиЗавершении(ПрекратитьРаботу, Знач Событие, Знач ТекстОшибки) Экспорт
	...
	Если Событие = "Запуск" Тогда
		ИмяСобытия = НСтр("ru='Запуск программы';uk='Запуск програми'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Если ПрекратитьРаботу Тогда
			НачалоОписанияОшибки = НСтр("ru='Возникла исключительная ситуация при запуске программы. Запуск программы аварийно завершен.';uk='Виникла виняткова ситуація при запуску програми. Запуск програми аварійно завершений.'");
		Иначе
			НачалоОписанияОшибки = НСтр("ru='Возникла исключительная ситуация при запуске программы.';uk='Виникла виняткова ситуація при запуску програми.'");
		КонецЕсли;
	Иначе
		ИмяСобытия = НСтр("ru='Завершение программы';uk='Завершення програми'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		НачалоОписанияОшибки = НСтр("ru='Возникла исключительная ситуация при завершении программы.';uk='Виникла виняткова ситуація при завершенні програми.'");
	КонецЕсли;
