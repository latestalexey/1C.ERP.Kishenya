...
Процедура ПередВыгрузкойТипа(Контейнер, Сериализатор, ОбъектМетаданных, Отказ) Экспорт
	...
	Если Не ОбщегоНазначенияБТС.ЭтоСсылочныеДанные(ОбъектМетаданных) Тогда 
		ВызватьИсключение НСтр("ru='Замена ссылок доступна только в ссылочных данных';uk='Заміна посилань доступна тільки в посилальних даних'");
	КонецЕсли;
...
Процедура ПослеВыгрузкиОбъекта(Контейнер, МенеджерВыгрузкиОбъекта, Сериализатор, Объект, Артефакты) Экспорт
	...
	Если Контейнер.ДополнительныеСвойства.ОбщиеДанныеТребующиеСопоставленияСсылок.Найти(ОбъектМетаданных) <> Неопределено Тогда
		...
		Если Не ОбщегоНазначенияБТС.ЭтоСсылочныеДанные(ОбъектМетаданных) Тогда 
			ВызватьИсключение НСтр("ru='Подмена ссылок доступна только в ссылочных данных';uk='Підміна посилань доступна тільки в посилальних даних'");
		КонецЕсли;
		
		МенеджерОбъекта = ТехнологияСервисаИнтеграцияСБСП.МенеджерОбъектаПоПолномуИмени(ПолноеИмяОбъектаМетаданных);
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура        = "ВыгрузкаЗагрузкаНеразделенныхДанных.КонтрольИспользованияСсылокНаНеразделенныеДанныеВРазделенных";
	
	Обработчик                  = Обработчики.Добавить();
	Обработчик.Версия           = "*";
	Обработчик.МонопольныйРежим = Ложь;
	Обработчик.ОбщиеДанные      = Истина;
	Обработчик.Процедура        = "ВыгрузкаЗагрузкаНеразделенныхДанных.КонтрольЗаполненияПолейЕстественногоКлючаДляНеразделенныхОбъектов";
...
Процедура КонтрольИспользованияСсылокНаНеразделенныеДанныеВРазделенных() Экспорт
	...
	Попытка
		...
		Кэш = ВыгрузкаЗагрузкаНеразделенныхДанныхПовтИсп.КонтрольСсылокНаНеразделенныеДанныеВРазделенныхПриВыгрузке();
		
	Исключение
		
		ТекстОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение СтрШаблон(НСтр("ru='Обнаружены ошибки в структуре метаданных конфигурации: %1';uk='Виявлені помилки в структурі метаданих конфігурації: %1'",Метаданные.ОсновнойЯзык.КодЯзыка),
			ТекстОшибки
		);
...
Процедура КонтрольЗаполненияПолейЕстественногоКлючаДляНеразделенныхОбъектов() Экспорт
	...
	Попытка
		...
		Кэш = ВыгрузкаЗагрузкаНеразделенныхДанныхПовтИсп.ЗависимостиНеразделенныхОбъектовМетаданных();
...
Процедура ПроверитьПоляЕстественногоКлюча(Знач ОбъектМетаданных, Знач ПоляЕстественногоКлюча)
	...
	Если ПоляЕстественногоКлюча = Неопределено Или ПоляЕстественногоКлюча.Количество() = 0 Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Для типа данных %1 не указаны естественные ключи для замены ссылок.
                  ...
                  |;uk='Для типу даних %1 не вказані природні ключі для заміни посилань.
                  |Перевірте обробник ПриОпределенииТиповТребующихЗагрузкиВЛокальнуюВерсию.'"),
...
Процедура КонтрольСсылокНаНеразделенныеДанныеПриВыгрузке(Контейнер, Объект, ПоляДляКонтроляСсылокНаНеразделенныеДанные)
	...
	Для Каждого ПолеДляКонтроляСсылокНаНеразделенныеДанные Из ПоляДляКонтроляСсылокНаНеразделенныеДанные Цикл
		...
		Если СтруктураИмениОбъекта[0] <> СтруктураИмениПоля[0] Или СтруктураИмениОбъекта[1] <> СтруктураИмениПоля[1] Тогда
			...
			ВызватьИсключение НСтр("ru='Некорректный кэш контроля неразделенных данных при выгрузке!';uk='Некоректний кеш контролю нерозподілених даних при вивантаженні!'");
		...
		Иначе
			ВызватьИсключение СтрШаблон(НСтр("ru='Объект метаданных %1 не поддерживается!';uk='Об''єкт метаданих %1 не підтримується!'",Метаданные.ОсновнойЯзык.КодЯзыка),
				ПолноеИмяОбъектаМетаданных
			);
		КонецЕсли;
...
Процедура КонтрольСсылкиНаНеразделенныеДанныеПриВыгрузке(Контейнер, Знач ПроверяемаяСсылка, Знач ИсходныйОбъект, Знач ИсходныйОбъектМетаданных, Знач ИмяПоля)
	...
	Если Не ОбщегоНазначения.ЭтоСсылка(ТипЗначения) Тогда
		// Контроль требудется только для значений ссылочного типа
		Возврат;
	КонецЕсли;
	...
	ШаблонОшибки =
		НСтр("ru='Объект метаданных %1 включен в перечень объектов, для которых не требуется сопоставление ссылок при выгрузке / загрузке
              ...
              |%5
              |---------------------------------------------------------------------------------------------------------------------------'
              |;uk='Об''єкт метаданих %1 включений в перелік об''єктів, для яких не потрібно зіставлення посилань при вивантаженні / завантаженні
              |даних (переобумовленій процедурі 
              |ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповОбщихДанныхНеТребующихСопоставлениеСсылокПриЗагрузке(),
              |але при цьому для нього не забезпечуються вимоги відсутності посилань, які не зіставляються, при вивантаженні.
              |
              |Посилання, які не зіставляються, виявлене при вивантаженні об''єкту %2, у якого в якості значення реквізиту %3
              |встановлене посилання на об''єкт %1, яке не зможе бути 
              |коректно зіставлене при завантаженні даних.
              |Потрібно переглянути логіку використання об''єкта %1 і 
              |забезпечити для нього відсутність посилань, які не зіставляються, 
              |у даних, що вивантажуються.
              |
              |Діагностична інформація:
              |1. Серіалізація об''єкта, що вивантажується:
              |---------------------------------------------------------------------------------------------------------------------------
              |%4
              |---------------------------------------------------------------------------------------------------------------------------
              |2. Серіалізація об''єкта посилання, яке не зіставляється
              |---------------------------------------------------------------------------------------------------------------------------
              |%5
              |---------------------------------------------------------------------------------------------------------------------------'");
