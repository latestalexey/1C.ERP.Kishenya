...
Функция ПолучитьМассивДатПоКалендарю(Знач ГрафикРаботы, Знач ДатаОт, Знач МассивДней, Знач РассчитыватьСледующуюДатуОтПредыдущей = Ложь, ВызыватьИсключение = Истина) Экспорт
	...
	Если Не ЗначениеЗаполнено(ГрафикРаботы) Тогда
		Если ВызыватьИсключение Тогда
			ВызватьИсключение НСтр("ru='Не указан график работы или производственный календарь.';uk='Не зазначений графік роботи або виробничий календар.'");
		КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ДатаПоКалендарю = Неопределено Тогда
			СообщениеОбОшибке = НСтр("ru='Производственный календарь «%1» не заполнен с даты %2 на указанное количество рабочих дней.';uk='Виробничий календар «%1» не заповнений з дати %2 на вказану кількість робочих днів.'");
			Если ВызыватьИсключение Тогда
...
Функция ПолучитьРазностьДатПоКалендарю(Знач ГрафикРаботы, Знач ДатаНачала, Знач ДатаОкончания, ВызыватьИсключение = Истина) Экспорт
	...
	Если Результат.Пустой() Тогда
		Если ВызыватьИсключение Тогда
			СообщениеОбОшибке = НСтр("ru='График работы «%1» не заполнен на период %2.';uk='Графік роботи «%1» не заповнений на період %2.'");
...
Функция ПолучитьДатыРабочихДней(График, НачальныеДаты, ПолучатьПредшествующие = Ложь, ВызыватьИсключение = Истина, ИгнорироватьНезаполненностьГрафика = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе 
			...
			КонецЕсли;
			Если ВызыватьИсключение Тогда
				ТекстСообщения = НСтр("ru='Невозможно определить ближайшую рабочую дату для даты %1, возможно, график работы не заполнен.';uk='Неможливо визначити найближчу робочу дату для дати %1, можливо, графік роботи не заповнений.'");
...
Функция РасписанияРаботыНаПериод(Графики, ДатаНачала, ДатаОкончания) Экспорт
	...
	ВызватьИсключение НСтр("ru='Подсистема «Графики работы» не обнаружена.';uk='Підсистема «Графіки роботи» не виявлена.'");
...
Функция ПроизводственныйКалендарьУкраины() Экспорт
...
Функция ПолучитьКалендарныеДниБезПраздниковЗаПериод(ДатаНачала, ДатаОкончания) Экспорт
	
	ГрафикРаботы = ПроизводственныйКалендарьУкраины();
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	КОЛИЧЕСТВО(КалендарныеГрафики.Дата) КАК КалендарныхДней
	|ИЗ
	|	РегистрСведений.ДанныеПроизводственногоКалендаря КАК КалендарныеГрафики
	|ГДЕ
	...
	Если Выборка.Следующий() Тогда
		Возврат Выборка.КалендарныхДней	
	Иначе 
		Возврат 0	
	КонецЕсли
...
Функция СоздатьВТТаблицаПраздничныхДней(МенеджерВременныхТаблиц, ДатаНачала, ДатаОкончания) Экспорт
	...
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.Текст = "ВЫБРАТЬ
	|	КОЛИЧЕСТВО(КалендарныеГрафики.Дата) КАК ПраздничныхДней,
	|	НАЧАЛОПЕРИОДА(КалендарныеГрафики.Дата, МЕСЯЦ) КАК Месяц
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	НАЧАЛОПЕРИОДА(КалендарныеГрафики.Дата, МЕСЯЦ)";
