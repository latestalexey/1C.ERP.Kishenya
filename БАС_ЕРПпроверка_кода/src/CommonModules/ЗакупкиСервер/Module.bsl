...
Процедура ПроверитьКорректностьЗаполненияДокументаЗакупки(ДокументЗакупки, Отказ) Экспорт
	...
	КонтролироватьРасчетыСПереработчиком	= ПолучитьФункциональнуюОпцию("НеЗакрыватьЗаказыПереработчикамБезПолнойОплаты");
	ОтработкаЗаказаПереработчику			= ПолучитьФункциональнуюОпцию("НеЗакрыватьЗаказыПереработчикамБезПолнойОтработки");
	//-- НЕ УТ
	
	//++ НЕ УТКА
	ОтработкаЗаказаДавальца					= ПолучитьФункциональнуюОпцию("НеЗакрыватьЗаказыДавальцевБезПолнойОтработки");
	//-- НЕ УТКА
	...
	ИначеЕсли ИмяТаблицы = "Документ.ЗаказПереработчику" Тогда
		...
		Если ДокументЗакупки.Статус = Перечисления.СтатусыЗаказовПереработчикам.Закрыт и Не ВсеСтрокиОтменены Тогда
			
			ПоДоговорам = ДокументЗакупки.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов;
			Если Не ПоДоговорам И КонтролироватьРасчетыСПереработчиком Тогда
				МассивПроверок.Добавить("КонтрольЗавершенияРасчетов");
			КонецЕсли;
			
			Если ОтработкаЗаказаПереработчику Тогда
	...
	ИначеЕсли ИмяТаблицы = "Документ.ЗаказДавальца" Тогда
		...
		Если ДокументЗакупки.Статус = Перечисления.СтатусыЗаказовДавальцев.Закрыт и Не ВсеСтрокиОтменены Тогда
			
			Если ОтработкаЗаказаДавальца Тогда
				МассивПроверок.Добавить("КонтрольПоступления");
			КонецЕсли;
	...
	Для ТекИндекс = 0 По МассивРезультатов.Количество()-1 Цикл
		...
		ИначеЕсли МассивПроверок[ТекИндекс] = "КонтрольОформленияОтчетов" Тогда
			...
			СообщитьОбОшибкахКонтрольОформленияОтчетов(Выборка, ДокументЗакупки, Отказ);
	...
	Если ТаблицаПроверяемыеТовары.Количество() > 0 Тогда
		...
		Иначе
			ЗапросТекст = "ВЫБРАТЬ
				...
				|	НЕ ТаблицаТовары.ДокументПоступления = ЗНАЧЕНИЕ(Документ.ПоступлениеТоваровУслуг.ПустаяСсылка)
				//++ НЕ УТКА
				|	И НЕ ТаблицаТовары.ДокументПоступления = ЗНАЧЕНИЕ(Документ.ПоступлениеСырьяОтДавальца.ПустаяСсылка)
				//-- НЕ УТКА
				|	И НЕ ТаблицаТовары.ДокументПоступления = НЕОПРЕДЕЛЕНО
				|ИНДЕКСИРОВАТЬ ПО
				|	Номенклатура,
		...
		|
		|ВЫБРАТЬ
		|	ПоступлениеСырьяОтДавальца.Номенклатура,
		|	ПоступлениеСырьяОтДавальца.Характеристика,
		|	ПоступлениеСырьяОтДавальца.Серия,
		|	ПоступлениеСырьяОтДавальца.Назначение,
		|	СУММА(ПоступлениеСырьяОтДавальца.КоличествоУпаковок),
		|	СУММА(ПоступлениеСырьяОтДавальца.Количество),
		|	СРЕДНЕЕ(ПоступлениеСырьяОтДавальца.Цена),
		|	ПоступлениеСырьяОтДавальца.Ссылка
		|ИЗ
		|	Документ.ПоступлениеСырьяОтДавальца.Товары КАК ПоступлениеСырьяОтДавальца
		|
		|ГДЕ
		|	ПоступлениеСырьяОтДавальца.Ссылка.Проведен
		|	И (Номенклатура,Характеристика,Назначение,Серия) В 
		|	(ВЫБРАТЬ 
		|		ПроверяемыеТовары.Номенклатура КАК Номенклатура,
		|		ПроверяемыеТовары.Характеристика КАК Характеристика,
		...
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		Документ.ВозвратСырьяДавальцу.Товары КАК ТаблицаПроверяемыеТовары
		|	ПО
		|		(ТаблицаПроверяемыеТовары.ДокументПоступления = ПроверяемыеТовары.ДокументПоступления)
		|		И ПроверяемыеТовары.Номенклатура = ТаблицаПроверяемыеТовары.Номенклатура
		|		И ПроверяемыеТовары.Характеристика = ТаблицаПроверяемыеТовары.Характеристика
...
Функция ЗаполнитьТоварыПоФактическойПриемке(Товары, ПараметрыЗаполнения, ЕстьПринятыеТовары, ДополнительныеПараметрыПоиска = Неопределено) Экспорт
	...
	Для Каждого СтрокаКДобавлению Из НераспределенныеСтроки Цикл
		...
		Если СтрокиТоваров.Количество() = 0 Тогда
			...
			//++ НЕ УТ
			Если ТипПоступления = Тип("ДокументСсылка.ПоступлениеОтПереработчика") Тогда
				НоваяСтрока.Распоряжение = Неопределено;
			КонецЕсли;
			//-- НЕ УТ
...
Функция ТекстЗапросаПроверкаПринимающихсяТоваровПоРаспоряжениям(ТипПоступления)
	...
	//++ НЕ УТ
	ИначеЕсли ТипПоступления = Тип("ДокументСсылка.ПоступлениеОтПереработчика") Тогда
		...
		|ГДЕ
		|	ТоварыПоступления.Распоряжение В (&МассивРаспоряжений)
		|	И ТоварыПоступления.Ссылка <> &ДокументПоступления
		|	И ТоварыПоступления.Ссылка.Проведен
		|
		|СГРУППИРОВАТЬ ПО
		...
		//++ НЕ УТКА
	ИначеЕсли ТипПоступления = Тип("ДокументСсылка.ПоступлениеСырьяОтДавальца") Тогда
		...
		|ПОМЕСТИТЬ втТоварыПоступления
		|ИЗ
		|	Документ.ПоступлениеСырьяОтДавальца.Товары КАК ТоварыПоступления
		|ГДЕ
		|	ТоварыПоступления.ЗаказДавальца В (&МассивРаспоряжений)
		|	И ТоварыПоступления.Ссылка <> &ДокументПоступления
		|	И ТоварыПоступления.Ссылка.Проведен
		|
		|СГРУППИРОВАТЬ ПО
		...
		|";
		//-- НЕ УТКА
	ИначеЕсли ТипПоступления = Тип("ДокументСсылка.ВозвратСырьяОтПереработчика") Тогда
		...
		|		Документ.ВозвратСырьяОтПереработчика.Товары КАК ТоварыПоступления
		|	ПО
		|		Документ.Ссылка = ТоварыПоступления.Ссылка
		|
		|ГДЕ
		|	ТоварыПоступления.ЗаказПереработчику В (&МассивРаспоряжений)
		|	И Документ.Ссылка <> &ДокументПоступления
		|	И Документ.Проведен
		|
		|СГРУППИРОВАТЬ ПО
		|	ТоварыПоступления.Номенклатура,
...
Функция ТекстЗапросаПоРаспоряжениям(ТипПоступления)
	...
	ИначеЕсли ТипПоступления = Тип("ДокументСсылка.ПоступлениеОтПереработчика") Тогда
		...
		ТекстЗапросаПоРаспоряжениям = 
		"ВЫБРАТЬ
		|	ТоварыПоступления.Ссылка 			КАК Распоряжение,
		|	ТоварыПоступления.Номенклатура		КАК Номенклатура,
		|	ТоварыПоступления.Характеристика	КАК Характеристика,
		|	ТоварыПоступления.Серия				КАК Серия,
		|	ТоварыПоступления.Склад				КАК Склад,
	...
	ИначеЕсли ТипПоступления = Тип("ДокументСсылка.ВозвратСырьяОтПереработчика") Тогда
		...
		ТекстЗапросаПоРаспоряжениям = 
		"ВЫБРАТЬ
		|	Документ.Ссылка 					КАК Распоряжение,
		|	ТоварыПоступления.Номенклатура		КАК Номенклатура,
		|	ТоварыПоступления.Характеристика	КАК Характеристика,
		|	ТоварыПоступления.Серия				КАК Серия,
		|	Документ.Склад						КАК Склад,
...
Процедура СформироватьЗапросВременнаяТаблицаТоварыДокументаЗакупки(ТекстЗапроса,
	...
	ИначеЕсли ИмяТаблицы = "Документ.ВыпускПродукции" Тогда
		...
		ТекстЗапроса = ТекстЗапроса + "
			...
			|	Товары.Склад                    КАК Склад
			//++ НЕ УТКА
			|	,ВЫРАЗИТЬ(Товары.Распоряжение КАК Документ.МаршрутныйЛистПроизводства) КАК Распоряжение
			//-- НЕ УТКА
		...
		ПредставлениеТабличнойЧасти = НСтр("ru='Товары и работы';uk='Товари і роботи'");
	...
	ИначеЕсли ИмяТаблицы = "Документ.ЗаказПереработчику" Тогда
		...
		ПредставлениеТабличнойЧасти = НСтр("ru='Выпускаемая продукция';uk='Продукція, що випускається'");
...
// Формирует запрос для контроля завершения оформления отчетов по заказу переработчику
//
// Параметры:
// ТекстЗапроса - Строка - текстовая строка, к которой необходимо добавить текст запроса
//
Процедура СформироватьЗапросКонтрольОформленияОтчетов(ТекстЗапроса)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|	ВЫБОР КОГДА ЕСТЬNULL(УслугиПереработчика.СуммаКонечныйОстаток, 0) <= 0 ТОГДА
		|		ЛОЖЬ
		|	ИНАЧЕ
		|		ИСТИНА
		...
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
		|		ВременнаяТаблицаДокументЗакупки КАК Документ
		|	ПО
		|		ИСТИНА
		|;
		|";
...
Процедура СообщитьОбОшибкахКорректностьЗаполненияНоменклатурыПоставщика(Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru='В строке не может быть выбрана ""%НоменклатураПоставщика%"", т.к. в предыдущих строках она соответствует другой номенклатуре';uk='У рядку не може бути обрана ""%НоменклатураПоставщика%"", т. к. в попередніх рядках вона відповідає інший номенклатурі'");
...
Процедура СообщитьОбОшибкахКонтрольПоступления (Знач Выборка,
	...
	Если Выборка.Следующий() Тогда 
		Если Выборка.ЕстьНезавершенноеПоступление Тогда
			...
			Если ТипЗнч(ДокументЗакупки) = Тип("ДокументОбъект.ЗаказПереработчику") Тогда
				ТекстОшибки = НСтр("ru='Продукция по заказу поступила не полностью.
                |Закрытие заказа возможно только с полностью поступившими/отмененными строками'
                |;uk='Продукція на замовлення надійшла не повністю.
                |Закриття замовлення можливе тільки з повністю надійшовшими/скасованими рядками'");
				
				//++ НЕ УТКА
			ИначеЕсли ТипЗнч(ДокументЗакупки) = Тип("ДокументОбъект.ЗаказДавальца") Тогда
				ТекстОшибки = НСтр("ru='Сырье и материалы по заказу поступили не полностью.
                |Закрытие заказа возможно только с полностью поступившими/отмененными строками'
                |;uk='Сировина і матеріали на замовлення надійшли не повністю.
                |Закриття замовлення можливе тільки з повністю надійшовшими/скасованими рядками'");
				//-- НЕ УТКА
			КонецЕсли;
	...
	Если ДокументЗакупки.ЭтоНовый() Тогда
			...
			Если ТипЗнч(ДокументЗакупки) = Тип("ДокументОбъект.ЗаказПереработчику") Тогда
				ТекстОшибки = НСтр("ru='Продукция по заказу не поступила.
                |Закрытие заказа возможно только с полностью поступившими/отмененными строками'
                |;uk='Продукція по замовленню не надійшла.
                |Закриття замовлення можливе тільки з повністю надійшовшими/скасованими рядками'");
				//++ НЕ УТКА
			ИначеЕсли ТипЗнч(ДокументЗакупки) = Тип("ДокументОбъект.ЗаказДавальца") Тогда
				ТекстОшибки = НСтр("ru='Сырье и материалы по заказу не поступили.
                |Закрытие заказа возможно только с полностью поступившими/отмененными строками'
                |;uk='Сировина і матеріали на замовлення не надійшли.
                |Закриття замовлення можливе тільки з повністю надійшовшими/скасованими рядками'");
				//-- НЕ УТКА
			КонецЕсли;
...
// Выводит сообщения об ошибках оформления отчетов переработчика
//
// Параметры:
// Выборка          - Выборка из результата запроса
// ДокументЗакупки  - ДокументОбъект, для которого необходимо вывести сообщения об ошибках
...
Процедура СообщитьОбОшибкахКонтрольОформленияОтчетов(Знач Выборка,
	...
	Если ДокументЗакупки.ЭтоНовый() Тогда
		...
		ТекстОшибки = НСтр("ru='Отчеты переработчика не сформированы. 
        |Закрытие заказа возможно только после их оформления'
        |;uk='Звіти переробника не сформовані. 
