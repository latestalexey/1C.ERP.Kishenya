...
Процедура СброситьФиксациюИзмененийРеквизита(Форма, ИмяРеквизита, ТекущаяСтрока = 0) Экспорт
	...
	Если НЕ ОписаниеФиксацииРеквизита  = Неопределено Тогда
		...
		ИначеЕсли ОписаниеФиксацииРеквизита.Значение.РеквизитСтроки И ПолучитьПризнакФиксацииРеквизитаТЧ(Форма, ОписаниеФиксацииРеквизита, ТекущаяСтрока) Тогда
			...
			ОбновитьОтображениеПредупреждения(Форма, ОписаниеФиксацииРеквизита, ТекущаяСтрока = 0);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
// 	Параметры:
//    	Форма - управляемая форма к которой подключается механизм.
//    	Путь - Строка, название таблицы к которой относится реквизит.
//    	ПараметрыФиксацииВторичныхДанных - структура см.
//    	                                   ФиксацияВторичныхДанныхВДокументахКлиентСервер.ПараметрыФиксацииВторичныхДанных.
//
Процедура ОбновитьОтображениеПредупрежденийТЧ(Форма, Путь, ТекущаяСтрока = 0) Экспорт
	ИменаРеквизитовИЭлементов = ИменаСлужебныхРеквизитовИЭлементовМеханизмаФиксацииДанных();	
	ПараметрыФиксацииВторичныхДанных = Форма[ИменаРеквизитовИЭлементов.Получить("ПараметрыФиксацииВторичныхДанных")];
	Для каждого ОписаниеФиксацииРеквизита Из ПараметрыФиксацииВторичныхДанных.ОписаниеФиксацииРеквизитов Цикл
		Если НЕ ОписаниеФиксацииРеквизита.Значение.РеквизитСтроки Или ОписаниеФиксацииРеквизита.Значение.Путь <> Путь Тогда
			Продолжить;
		КонецЕсли;
...
//    	ОписаниеФиксацииРеквизита - Структура с полями - см. описание СтруктураПараметровОписанияФиксацииРеквизитов().
//
Функция ПолучитьПризнакФиксацииРеквизитаТЧ(Форма, ОписаниеФиксацииРеквизита, ТекущаяСтрока) Экспорт
	...
	Возврат НЕ ТекущаяСтрока = Неопределено 
			И НЕ Объект.НайтиПоИдентификатору(ТекущаяСтрока) = Неопределено
			И Объект.НайтиПоИдентификатору(ТекущаяСтрока)[ПоследнееПоле];
...
Процедура Подключаемый_ЗафиксироватьИзменениеРеквизитаВФорме(Форма, Элемент, ОписаниеЭлементов, ТекущаяСтрока = 0) Экспорт
	...
	ФиксацияВторичныхДанныхВДокументахКлиентСервер.ЗафиксироватьИзменениеРеквизита(Форма, ИмяРеквизита, ТекущаяСтрока)
...
//    	ОписаниеФиксацииРеквизита - Структура с полями - см. описание СтруктураПараметровОписанияФиксацииРеквизитов().
//
Процедура ОбновитьОтображениеПредупреждения(Форма, ОписаниеФиксацииРеквизита, ТекущаяСтрока = 0)
	
	Если ОписаниеФиксацииРеквизита.Значение.ОтображатьПредупреждение Тогда
		
		Если ОписаниеФиксацииРеквизита.Значение.РеквизитСтроки Тогда
		    Зафиксирован = ПолучитьПризнакФиксацииРеквизитаТЧ(Форма, ОписаниеФиксацииРеквизита, ТекущаяСтрока);
		Иначе	
			Зафиксирован = ПолучитьПризнакФиксацииРеквизита(Форма, ОписаниеФиксацииРеквизита);
		КонецЕсли;
		
		Если Зафиксирован Или НЕ ОписаниеФиксацииРеквизита.Значение.Используется Тогда
		...
		Для каждого ИмяЭлемента Из МассивЭлементов Цикл
			...
			Если ТипЗнч(Форма.Элементы[ИмяЭлемента]) = Тип("ПолеФормы") 
				...
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, ИмяЭлемента, "ШрифтЗаголовка", Новый Шрифт(, , Зафиксирован));
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
//
Процедура УстановитьФиксациюРеквизита(Форма, ОписаниеФиксацииРеквизита, ТекущаяСтрока = 0)
	...
	Если НЕ ОписаниеФиксацииРеквизита.Значение.РеквизитСтроки И НЕ ПолучитьПризнакФиксацииРеквизита(Форма, ОписаниеФиксацииРеквизита) Тогда
		...
		ОбновитьОтображениеПредупреждения(Форма, ОписаниеФиксацииРеквизита);
	ИначеЕсли ОписаниеФиксацииРеквизита.Значение.РеквизитСтроки И НЕ ПолучитьПризнакФиксацииРеквизитаТЧ(Форма, ОписаниеФиксацииРеквизита, ТекущаяСтрока) Тогда
		УстановитьПризнакФиксацииРеквизитаТЧ(Форма, ОписаниеФиксацииРеквизита, Истина, ТекущаяСтрока);
		ОбновитьОтображениеПредупреждения(Форма, ОписаниеФиксацииРеквизита, ТекущаяСтрока);
	КонецЕсли;
