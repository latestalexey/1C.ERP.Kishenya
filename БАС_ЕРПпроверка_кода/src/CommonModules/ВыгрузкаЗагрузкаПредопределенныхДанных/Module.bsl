	...
	Если ОбщегоНазначенияБТС.ЭтоСсылочныеДанныеПоддерживающиеПредопределенныеЭлементы(Объект.Метаданные()) Тогда
		...
		Иначе
			...
			НСтр("ru='Объект метаданных %1 не может быть обработан обработчиком
                  ...
                  |ВыгрузкаЗагрузкаПредопределенныхданных.ПередВыгрузкойОбъекта(),
                  |оскільки не потрібно забезпечувати зіставлення посилань на його напередвизначені елементи!'",Метаданные.ОсновнойЯзык.КодЯзыка),
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru='Объект метаданных %1 не может быть обработан обработчиком
                  ...
                  |;uk='Об''єкт метаданих %1 не може бути оброблений обробником
                  |ВыгрузкаЗагрузкаПредопределенныхДанных.ПередВыгрузкойОбъекта(),
                  |т. я. не може містити напередвизначених елементів!'",Метаданные.ОсновнойЯзык.КодЯзыка),
			Объект.Метаданные().ПолноеИмя()
		);
		...
		Если ЗначениеЗаполнено(СтрокаТаблицыИсходныхСсылок.ИмяПредопределенныхДанных) Тогда
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				Иначе
					...
					ВызватьИсключение СтрШаблон(
						НСтр("ru='Обнаружено дублирование предопределенных элементов %1 в таблице %2!';uk='Виявлене дублювання напередвизначених елементів %1 у таблиці %2!'",Метаданные.ОсновнойЯзык.КодЯзыка),
