...
Процедура ПриДобавленииПодсистемы(Описание) Экспорт
	...
	Описание.ОбновлятьПараллельноСПодсистемами.Добавить("BASКомплекснаяАвтоматизация");
	//-- НЕ УТ
...
#Область Версия_3_2_1
	...
	Обработчик.Комментарий = НСтр("ru='Справочник ""Условия цен поставщиков"" трансформируется в справочник ""Виды цен поставщиков"". Данные из соглашения-владельца переносятся в вид цены поставщика.';uk='Довідник ""Умови цін постачальників"" трансформується у довідник ""Види цін постачальників"". Дані із угоди-власника переносяться у вид ціни постачальника.'");
	
	Обработчик = Обработчики.Добавить();
    Обработчик.Версия = "3.2.1.7";
    Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Процедура = "Справочники.СоглашенияСПоставщиками.ЗаполнитьВидЦеныПоставщика";
	Обработчик.Комментарий = НСтр("ru='Для каждого соглашения с поставщиком создается вид цены поставщика и записывается в реквизит ""Вид цены поставщика"".';uk='Для кожної угоди з постачальником створюється вид ціни постачальника і записується в реквізит ""Вид ціни постачальника"".'");
...
#Область Версия_3_2_2
	...
	Обработчик.Комментарий = НСтр("ru='Установка значения константы ИнтерфейсВерсии82  в ""Истина"".';uk='Установка значення константи ИнтерфейсВерсии82 в ""Істина"".'");
	...
	Обработчик.Комментарий = НСтр("ru='Заполняет новые реквизиты справочника ""Варианты анализа целевых показателей"", такие как - ""Периодичность расчета показателя"", ""Режим шкалы значений"", ""Включать Ноль"" и ""Вариант отображения по умолчанию"".';uk='Заповнює нові реквізити довідника ""Варіанти аналізу цільових показників"", такі як - ""Періодичність розрахунку показника"", ""Режим шкали значень"", ""Включати Нуль"" і ""Варіант відображення за замовчуванням"".'");
    ...
    |До завершення роботи оброблювача оновлення список ""Статті руху грошових коштів"" знаходиться в некоректному стані.'");
	...
	Обработчик.Комментарий = НСтр("ru='Заполняет константу ""Максимальное количество элементов в дереве отборов номенклатуры"" значением по умолчанию 50.';uk='Заповнює константу ""Максимальна кількість елементів у дереві відборів номенклатури"" значенням за замовчуванням 50.'");
...
#Область Версия_3_2_2
    ...
    |До завершення роботи оброблювача оновлення документи знаходяться в некоректному стані.'");
    ...
    |;uk='Переносить руху документа ""Заявка на витрачання ГК"" з регістра ""Рухи Грошові кошти - Інші активи/пасиви"" у регістр ""Рухи Грошові кошти - Доходи/Витрати"". 
    |До завершення роботи оброблювача поновлення руху документів знаходяться в некоректному стані.'");
    ...
    |- заповнюється спосіб доставки за замовчуванням ""Силами постачальника до нашого складу"".'");
	...
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.БлокируемыеОбъекты = "Документ.ПоступлениеТоваровУслуг";
	Обработчик.Комментарий = НСтр("ru='Заполняет способ доставки по умолчанию ""Силами поставщика до нашего склада"".';uk='Заповнює спосіб доставки за замовчуванням ""Силами постачальника до нашого складу"".'");
    ...
    |;uk='Замінює вимір ""Аналітика обліку за партнерам"" з урахуванням договору.
    ...
    |До завершения работы обработчика обновления движения документов находятся в некорректном состоянии.'
    |;uk='Переносить руху документа ""Надходження безготівкових ГК"" з регістра Рухи Грошові кошти - Інші активи/пасиви"" регістр ""Рухи кошти - Доходи/Витрати"". 
    |До завершення роботи оброблювача поновлення рухи документів знаходяться в некоректному стані.'");
	...
	Обработчик.Комментарий = НСтр("ru='Заполняет дату отгрузки в документах ""Заказ клиента"" в статусе ""На согласовании"" согласно гарантированному сроку отгрузки по основному способу обеспечения.';uk='Заповнює дату відвантаження в документах ""Замовлення клієнта"" в статус ""На узгодженні"" згідно гарантованого терміну відвантаження за основним способом забезпечення.'");
    ...
    |;uk='Розбиває рядок табличної частини ""Товари"" документів ""Замовлення на збирання/розбирання"", якщо пов''язаної накладної після перенесення серій змінилися коди рядків у табличній частині ""Товари"", З накладної переносяться нові коди рядків. До завершення роботи оброблювача оновлення 
    ...
    |;uk='Розбиває документи ""Замовлення на збирання (розбирання)"", в яких не вказана серія комплекту і за якими оформлені ""Збирання (розбирання) товарів"":
    |- якщо серія комплекту нефінансова (тип політики відрізняється від ""Облік собівартості по серіям"") і не на всі кількість замовлення оформлені накладні (немає серій для перезаполнения).'");
    ...
    |До завершення роботи оброблювача оновлення робочих місцях ""Внутрішні товарорухи"", ""Складання (розбирання) товарів"", ""Переміщення товарів"", ""Внутрішні споживання товарів"" не будуть відображатися документи.'");
    ...
    |До завершення роботи оброблювача оновлення відображення документа може бути некоректним.'");
    ...
    |До завершення роботи оброблювача оновлення інформація щодо взаємозаліків може бути некоректною.'");
...
#Область Версия_3_2_3
	...
	Обработчик.Комментарий = НСтр("ru='Заполняет в регистре ""Сезонные коэффициенты"" сезонную группу бизнес региона по умолчанию.  Пока работа обработчика не завершена, информация в регистре некорректна.';uk='Заповнює в регістрі ""Сезонні коефіцієнти"" сезонну групу бізнес регіону за замовчуванням. Поки робота обробника не завершена, інформація в регістрі некоректна.'");
	...
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.БлокируемыеОбъекты = "";
    Обработчик.Комментарий = НСтр("ru='Исправляет движения документов в регистре накопления ""Товары к оформлению излишков и недостач"".';uk='Виправляє рухи документів в регістрі накопичення ""Товари до оформлення надлишків і недостач"".'");
    ...
    |До завершення роботи оброблювача оновлення інформація про залишки грошових коштів у дорозі може бути некоректною.'");
	...
	Обработчик.Комментарий = НСтр("ru='Для документов поступления товаров выполняется формирование недостающих движений по регистру ""Партии товаров оргнизаций"".
                                ...
                                |;uk='Для документів надходження товарів виконується формування відсутніх рухів по регістру ""Партії товарів організацій"".
