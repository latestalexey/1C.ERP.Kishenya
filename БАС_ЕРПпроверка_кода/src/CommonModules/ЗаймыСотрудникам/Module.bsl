...
Процедура ЗарегистрироватьЧастичноПогашенныйЗаем(Движения, Договор, СуммаЗайма, Проценты, ДатаРегистрации, Организация, ФизическоеЛицо, Отказ) Экспорт 
	...
	Если Проценты > 0 Тогда 
		...
		НоваяЗапись = Движения.ВзаиморасчетыПоЗаймамСотрудникам.ДобавитьПриход();
		НоваяЗапись.Период = ДатаРегистрации;
		НоваяЗапись.ФизическоеЛицо = ФизическоеЛицо;
		НоваяЗапись.Организация = Организация;
		НоваяЗапись.ДоговорЗайма = Договор;
...
Процедура ЗарегистрироватьПогашениеЗайма(Движения, Договор, Сумма, ДатаПогашения, Организация, ФизическоеЛицо, Регистратор, Отказ) Экспорт
	...
	// в первую очередь сумма погашения пойдет на погашение процентов, 
	// остаток - на погашение займа.
	...
	Если РасчетЗаймов.Количество() = 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Невозможно зарегистрировать погашение займа на дату %1. 
                               ...
                               |;uk='Неможливо зареєструвати погашення позики на дату %1. 
                               |Існують більш пізні операції за договором позики або позику повністю погашений.'"),
	...
	ВсегоПогашеноПроцентов	= Мин(ВсегоНачисленоПроцентов, Сумма);
	ВсегоПогашеноЗайма		= Макс(Сумма - ВсегоНачисленоПроцентов, 0);
	...
	МассивПогашеноЗайма		= ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(ВсегоПогашеноЗайма, МассивДней);
	Если ВсегоНачисленоПроцентов > 0 Тогда
		...
		МассивПогашеноПроцентов	 = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(ВсегоПогашеноПроцентов, МассивНачисленоПроцентов);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСтатьиФинансированияЗарплатаРасширенный") Тогда
		...
		ЗаймыПоРабочимМестам = УчетНачисленнойЗарплаты.УдержанияПоРабочимМестам(УдержанияЗаймов, НачалоМесяца(ДатаПогашения), Организация, , Истина);
...
Функция ОстатокЗадолженности(Договор, ДатаАктуальности, Регистратор = Неопределено) Экспорт
	...
	ОстатокЗадолженности = Новый Структура("ОбщаяСумма, СуммаЗайма, Проценты, Валюта", 0, 0, 0, "грн.");
	...
	СуммаЗайма = РасчетЗаймов[РасчетЗаймов.Количество() - 1].ОстатокЗаймаНаДатуОперации;
	Проценты = РасчетЗаймов.Итог("НачисленоПроцентов");
...
Функция ИнформацияОЗадолженности(Договор, ДатаДокумента, ДокументСсылка) Экспорт
	...
	СтруктураЗадолженности = ЗаймыСотрудникам.ОстатокЗадолженности(Договор, ДатаЗадолженности, ДокументСсылка);
	
	Если СтруктураЗадолженности.ОбщаяСумма = 0 Тогда
		ИнфоТекст = НСтр("ru='Задолженность на %1 отсутствует.';uk='Заборгованість на %1 відсутня.'");
	Иначе
		Если СтруктураЗадолженности.ОбщаяСумма < 0 Тогда
			ИнфоТекст = НСтр("ru='На %1 обнаружена переплата в размере %3 %2.';uk='На %1 виявлена переплата у розмірі %3 %2.'");
		Иначе
			Если СтруктураЗадолженности.Проценты = 0 Тогда
				ИнфоТекст = НСтр("ru='На %1 сумма задолженности составляет %3 %2. Задолженность по процентам отсутствует.';uk='На %1 сума заборгованості становить %3 %2. Заборгованість за відсотками відсутня.'");
			Иначе
				ИнфоТекст = НСтр("ru='На %1 сумма задолженности составляет %3 %2. В том числе: основной долг %4 %2, проценты %5 %2.';uk='На %1 сума заборгованості становить %3 %2. У тому числі: основний борг %4 %2, проценти %5 %2.'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ДействующиеДоговорыЗаймаПоФизическомуЛицу(Организация, ФизическоеЛицо, ДатаАктуальности = Неопределено, ВключатьНевыданныеДоговоры = Ложь) Экспорт
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ВзаиморасчетыПоЗаймамСотрудникам.Остатки(
	|				&ДатаОкончания,
	|				Организация = &Организация
	|					И ФизическоеЛицо = &ФизическоеЛицо) КАК ВзаиморасчетыПоЗаймамСотрудникамОстатки
	|		ПО ДоговорЗаймаСотруднику.Ссылка = ВзаиморасчетыПоЗаймамСотрудникамОстатки.ДоговорЗайма
	|ГДЕ
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ДоговорЗаймаСотруднику.Ссылка КАК ДоговорЗайма,
	|	СУММА(ВзаиморасчетыПоЗаймамСотрудникамОбороты.СуммаЗаймаПриход + ВзаиморасчетыПоЗаймамСотрудникамОбороты.ПроцентыПриход) КАК ОборотПриход,
	|	&ВключатьНевыданныеДоговоры
	|ПОМЕСТИТЬ ВТДоговорыБезДвижений
	|ИЗ
	|	Документ.ДоговорЗаймаСотруднику КАК ДоговорЗаймаСотруднику
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ВзаиморасчетыПоЗаймамСотрудникам.Обороты(
	|				,
	|				&ДатаОкончания,
	|				Период,
	|				Организация = &Организация
	|					И ФизическоеЛицо = &ФизическоеЛицо) КАК ВзаиморасчетыПоЗаймамСотрудникамОбороты
...
Функция РасчетЗаймовСотрудникам(Организация, ДатаОперации, Регистратор, ДоговорыЗайма = Неопределено, Сотрудники = Неопределено, Подразделение = Неопределено, ИсключатьУволенныхСотрудников = Ложь) Экспорт
	...
	// Если не указаны ни сотрудники, ни договоры, выявляем только договоры с работающими на дату операции сотрудниками.
	Если Сотрудники = Неопределено И ДоговорыЗайма = Неопределено Тогда
		ПоВсемСотрудникам = Ложь;
		// Создаем ВТФизическиеЛица по работающим в организации на конец месяца.
		КадровыйУчет.СоздатьВТФизическиеЛицаРаботавшиеВОрганизации(
			МенеджерВременныхТаблиц, Истина, Организация, КонецМесяца(ДатаОперации), КонецМесяца(ДатаОперации), Подразделение, , , "ВТФизическиеЛица");
	Иначе
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	И (ВзаиморасчетыПоЗаймам.ФизическоеЛицо В
	|				(ВЫБРАТЬ
	|					ВТФизическиеЛица.ФизическоеЛицо
	|				ИЗ
	|					ВТФизическиеЛица)
	|			ИЛИ &ПоВсемСотрудникам)
	|	И (ВзаиморасчетыПоЗаймам.ДоговорЗайма В (&ДоговорыЗайма)
	|			ИЛИ &ПоВсемДоговорамЗайма)
	...
	Запрос.УстановитьПараметр("ПоВсемДоговорамЗайма", ДоговорыЗайма = Неопределено);
	Запрос.УстановитьПараметр("ПоВсемСотрудникам", ПоВсемСотрудникам);
	...
	// По физическим лицам из договоров займа нужно получить историю изменения подразделений 
	// за периоды указанные для каждого в отдельности.
	// При этом кто-то из физических лиц может к моменту операции уже не работать, 
	// тогда подразделение не заполняем.
	// При этом нужно еще и отобрать по подразделению (если оно заполнено в параметрах).
	...
	// Получаем основных сотрудников физических лиц.
	ОсновныеСотрудники = КадровыйУчет.ОсновныеСотрудникиФизическихЛиц(СписокФизическихЛиц, Истина, Организация, ДатаОперации);
	МассивСотрудников = ОбщегоНазначения.ВыгрузитьКолонку(ОсновныеСотрудники, "Сотрудник", Истина);
	...
	ПараметрыПолученияРабочихМест.НачалоПериода		= МинимальнаяДата;
	ПараметрыПолученияРабочихМест.ОкончаниеПериода  = ДатаОперации;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВидПлатежей = Перечисления.ВидыПлатежейПогашенияЗаймаСотруднику.АннуитетныеПлатежи Тогда
			...
			НоваяСтрока.РазмерПлатежа = Выборка.РазмерПлатежа / Выборка.ДнейВМесяце * Выборка.Дней;
			НоваяСтрока.ПогашениеПроцентов = НоваяСтрока.НачисленоПроцентов + Выборка.ОстатокПроцентовНаНачало;
...
Процедура ПроверитьДатуВыдачиЗайма(ДатаВыдачи, ДатаПредоставления, Поле, Отказ) Экспорт 
	...
	Если ДатаВыдачи < ДатаПредоставления Тогда 
		...
		ШаблонСообщения = НСтр("ru='Месяц предоставления займа - %1. Дата выдачи не может быть меньше даты предоставления.';uk='Місяць надання позики - %1. Дата видачі не може бути менша дати надання.'");
...
Процедура ПроверитьДатыДоговораПриВводеНачальныхОстатков(Организация, ДатаПогашения, ДатаВыдачи, ДатаПредоставления, Отказ) Экспорт 
	...
	Если ДатаПогашения < ДатаПредоставления Тогда 
		ШаблонСообщения = НСтр("ru='Месяц предоставления займа - %1. Дата ввода остатков не может быть меньше даты предоставления.';uk='Місяць надання позики - %1. Дата введення залишків не може бути менша дати надання.'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Формат(ДатаПредоставления, "ДФ=""ММММ гггг"""));
...
Процедура СоздатьВТНалогНаМатериальнуюВыгоду(МенеджерВременныхТаблиц)
	...
	|	ПогашениеЗаймов.ФизическоеЛицо,
	|	ПогашениеЗаймов.Подразделение,
	|	ПогашениеЗаймов.ДатаОперации КАК МесяцНалоговогоПериода,
	|	ПогашениеЗаймов.ИдентификаторСтроки,
...
Функция ОстатокНевыданныхСумм(Договор, ДатаАктуальности, Регистратор = Неопределено) Экспорт 
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |ИЗ
	               |	РегистрСведений.УсловияДоговораЗаймаСотруднику.СрезПоследних(&Период, ДоговорЗайма = &Договор) КАК УсловияДоговораЗайма
