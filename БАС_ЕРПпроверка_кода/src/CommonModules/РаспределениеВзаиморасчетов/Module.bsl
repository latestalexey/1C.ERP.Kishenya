...
Процедура РаспределитьВсеРасчетыСКлиентами(ОкончаниеПериодаРасчета, АналитикиКРасчету = Неопределено) Экспорт
	...
	Пока НачалоПериода < ОкончаниеПериодаРасчета Цикл
		...
		УстановитьГраницыРасчетов(НачалоПериода, ОкончаниеПериода, ИмяРегистраЗаданий, ИмяОперативногоРегистра, ИмяКонстантыЗаданий, АналитикиРасчета);
		...
		НачалоПериода = НачалоРасчета(ИмяРегистраЗаданий, ОкончаниеПериодаРасчета, АналитикиРасчета);
	КонецЦикла;
...
Процедура РассчитатьРасчетыСКлиентами(НачалоПериода, ОкончаниеПериода, АналитикиРасчета, РасчетныеРасчетыСКлиентами = Неопределено)
	...
	//++ НЕ УТ
	ВернутьДокументыКОтражениюВРеглУчете(РасчетныеРасчетыСКлиентами);
	//-- НЕ УТ
...
Процедура РассчитатьРасчетыСПоставщиками(НачалоПериода, ОкончаниеПериода, АналитикиРасчета, РасчетныеРасчетыСПоставщиками = Неопределено)
	...
	//++ НЕ УТ
	ВернутьДокументыКОтражениюВРеглУчете(РасчетныеРасчетыСПоставщиками);
	//-- НЕ УТ
...
Процедура УстановитьГраницыРасчетов(РассчитаныйПериод, ОкончаниеПериодаРасчета, ИмяРегистраЗаданий, ИмяОперативногоРегистра, ИмяКонстантыЗаданий, АналитикиРасчета)
	УстановитьПривилегированныйРежим(Истина);
	
	Если НачалоРасчета(ИмяРегистраЗаданий, ОкончаниеПериодаРасчета, АналитикиРасчета) < РассчитаныйПериод Тогда
		Возврат;
	КонецЕсли;
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ИмяКонстантыЗаданий КАК НомерКРасчету
	|	ПО ИСТИНА
	|ГДЕ
	...
	|	ГДЕ
	|		Движения.АналитикаУчетаПоПартнерам В (&АналитикиРасчета)
	|			ИЛИ &ПоВсемАналитикам
...
Функция РасчетныеРасчетыСКлиентами(ДанныеДляРасчета, Регистраторы, ЦепочкиДвижений = Неопределено, Тест = Ложь) // ДанныеДляРасчета будут изменены
	...
	КонтекстДвижений = ОписаниеДвижений(
		...
		"Сумма", "Сумма", "РасчетныйДокумент", "Период", "Период, Приоритет, Регистратор");
...
Функция ТекстРасчетыСКлиентамиИнициализация()
	Возврат "
		...
		|	ЕСТЬNULL(Выручка.ВалютаДокумента, НЕОПРЕДЕЛЕНО) КАК ВалютаДокумента
		...
		|	ПО Расчеты.Регистратор = Выручка.Регистратор
		|		И Расчеты.АналитикаУчетаПоПартнерам = Выручка.АналитикаУчетаПоПартнерам
		|		И Расчеты.ЗаказКлиента = Выручка.ЗаказКлиента
		|		И Выручка.ВалютаДокумента <> ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
...
Функция ТекстОстаткиРасчетовСКлиентами()
	Возврат "
		...
		|	ДД.РасчетныйДокумент КАК РасчетныйДокумент,
		|	ДД.Валюта КАК Валюта,
		|	ВЫБОР КОГДА (ДД.ДолгОстаток + ДД.ПредоплатаОстаток) < 0
		|		ТОГДА 0 - (ДД.ДолгОстаток + ДД.ПредоплатаОстаток)
		|		ИНАЧЕ (ДД.ДолгОстаток + ДД.ПредоплатаОстаток)
		|	КОНЕЦ КАК Сумма,
		|	ВЫБОР КОГДА ДД.КВозвратуОстаток < 0
		|		ТОГДА 0 - ДД.КВозвратуОстаток
		|		ИНАЧЕ ДД.КВозвратуОстаток
		|	КОНЕЦ КАК КВозврату,
		|	ВЫБОР КОГДА ДД.ДолгОстаток < 0
		|		ТОГДА 0 - ДД.ДолгОстаток
		|		ИНАЧЕ ДД.ДолгОстаток
		|	КОНЕЦ КАК Долг,
		|	ВЫБОР КОГДА ДД.ДолгУпрОстаток < 0
		|		ТОГДА 0 - ДД.ДолгУпрОстаток
		|		ИНАЧЕ ДД.ДолгУпрОстаток
		|	КОНЕЦ КАК ДолгУпр,
		|	ВЫБОР КОГДА ДД.ДолгРеглОстаток < 0
		|		ТОГДА 0 - ДД.ДолгРеглОстаток
		|		ИНАЧЕ ДД.ДолгРеглОстаток
		|	КОНЕЦ КАК ДолгРегл,
		|	ВЫБОР КОГДА ДД.ПредоплатаОстаток < 0
		|		ТОГДА 0 - ДД.ПредоплатаОстаток
		|		ИНАЧЕ ДД.ПредоплатаОстаток
		|	КОНЕЦ КАК Предоплата,
		|	ВЫБОР КОГДА ДД.ПредоплатаУпрОстаток < 0
		|		ТОГДА 0 - ДД.ПредоплатаУпрОстаток
		|		ИНАЧЕ ДД.ПредоплатаУпрОстаток
		|	КОНЕЦ КАК ПредоплатаУпр,
		|	ВЫБОР КОГДА ДД.ПредоплатаРеглОстаток < 0
		|		ТОГДА 0 - ДД.ПредоплатаРеглОстаток
		|		ИНАЧЕ ДД.ПредоплатаРеглОстаток
		|	КОНЕЦ КАК ПредоплатаРегл,
		|	ВЫБОР КОГДА ДД.ЗалогЗаТаруОстаток < 0
		|		ТОГДА 0 - ДД.ЗалогЗаТаруОстаток
		|		ИНАЧЕ ДД.ЗалогЗаТаруОстаток
		|	КОНЕЦ КАК ЗалогЗаТару,
		|	ВЫБОР КОГДА ДД.ЗалогЗаТаруРеглОстаток < 0
		|		ТОГДА 0 - ДД.ЗалогЗаТаруРеглОстаток
		|		ИНАЧЕ ДД.ЗалогЗаТаруРеглОстаток
...
Функция ТекстОтгрузкиПрошлыхПериодов()
	Возврат "
		...
		|	СУММА(ДД.Долг) КАК Долг,
		|	СУММА(ДД.ДолгУпр) КАК ДолгУпр,
		|	СУММА(ДД.ДолгРегл) КАК ДолгРегл,
		|	СУММА(ДД.Предоплата) КАК Предоплата,
		|	СУММА(ДД.ПредоплатаУпр) КАК ПредоплатаУпр,
		|	СУММА(ДД.ПредоплатаРегл) КАК ПредоплатаРегл,
		|	СУММА(ДД.ЗалогЗаТару) КАК ЗалогЗаТару,
		|	СУММА(ДД.ЗалогЗаТаруРегл) КАК ЗалогЗаТаруРегл,
		|	ДД.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
		|	ДД.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств,
		|	ДД.ДатаПлатежа КАК ДатаПлатежа,
		|	ДД.Регистратор КАК ДокументОтгрузки,
		|	НЕОПРЕДЕЛЕНО КАК РасчетПоКурсуДокумента
		|ИЗ
		|	РегистрНакопления.РасчетыСКлиентамиПоДокументам КАК ДД
		|
...
Функция РасчетныеРасчетыСПоставщиками(ДанныеДляРасчета, Регистраторы, ЦепочкиДвижений = Неопределено, Тест = Ложь) // ДанныеДляРасчета будут изменены
	...
	КонтекстДвижений = ОписаниеДвижений(
		...
		ПоляПотреблений,
		"КВозврату, Долг, ДолгУпр, ДолгРегл, Предоплата, ПредоплатаУпр, ПредоплатаРегл, ЗалогЗаТару, ЗалогЗаТаруРегл, Сумма",
		"Сумма", "Сумма", "РасчетныйДокумент", "Период", "Период, Приоритет");
...
Функция ТекстРасчетыСПоставщикамиИнициализация()
	Возврат "
		...
		|	Расчеты.Валюта                                  КАК Валюта,
		|	ЕСТЬNULL(Закупки.ВалютаДокумента, НЕОПРЕДЕЛЕНО) КАК ВалютаДокумента
		|ПОМЕСТИТЬ РегистраторыКРасчету
		|ИЗ
		...
		|	ПО Расчеты.Регистратор = Закупки.Регистратор
		|		И Аналитики.Организация = Закупки.Организация
		|		И Аналитики.Партнер = Закупки.Партнер
		|		И Аналитики.Контрагент = Закупки.Контрагент
		|		И Аналитики.Договор = Закупки.Договор
		|		И Расчеты.ЗаказПоставщику = Закупки.Заказ
		|ГДЕ
		|	Расчеты.Период МЕЖДУ &НачалоПериода И &ОкончаниеПериода
		|	И Расчеты.Сумма <> 0
		|;
...
Процедура ЗаполнитьРасчетыСКлиентами(РасчетнаяПартия, Расход, Приход, Валюты) // РасчетыСКлиентамиПоДокументам
	...
	Если Расход.РасчетПоКурсуДокумента И Расход.Долг <> 0 Тогда
	...
	Если (ЭтоКорректировка И РасчетнаяПартия.Долг <> 0 И Расход.ВидДвижения = ВидДвиженияНакопления.Приход)
		ИЛИ (ЭтоКорректировка И РасчетнаяПартия.Предоплата <> 0 И Расход.ВидДвижения = ВидДвиженияНакопления.Расход) Тогда
...
Процедура СформироватьДвиженияПоУправленческомуУчету(Контекст, РегистраторыКОтражению)
	//++ НЕ УТКА
	МеждународныйУчетПроведениеСервер.ЗарегистрироватьДокументыРасчетовСПартнерамиКОтражениюВМеждународномУчете(РегистраторыКОтражению);
	//-- НЕ УТКА
	Если Контекст = "РасчетыСКлиентами" Тогда
...
Процедура СортироватьИсточникиПоПолямСортировки(Источники, ПоляСортировки, Приходы, НаправлениеСортировки = "Убыв")
	...
	Для Каждого ПолеСортировки Из ПоляСортировки Цикл
		ТаблицаСортировки.Колонки.Добавить(ПолеСортировки.Ключ);
		Если ЗначениеЗаполнено(УсловиеСортировки) Тогда
			УсловиеСортировки = УсловиеСортировки + ", ";
		КонецЕсли;
		УсловиеСортировки = УсловиеСортировки + ПолеСортировки.Ключ + " " + НаправлениеСортировки;
	КонецЦикла;
	Для Каждого Источник Из Источники Цикл
		...
		Для Каждого ПолеСортировки Из ПоляСортировки Цикл
			СтрокаСортировки[ПолеСортировки.Ключ] = СтрокаПрихода[ПолеСортировки.Ключ];
		КонецЦикла;
	КонецЦикла;
...
Процедура РассчитатьПартииПоЦепочкамИзВыборки(ОписаниеДвижений, РезультатЗапроса, РасчетныеПартии, ЦепочкиДвижений, Регистраторы,  Тест = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если РасчетныеПартии.Количество() > 1000 Тогда
			...
			КонецЕсли;
			//++ НЕ УТ
			ВернутьДокументыКОтражениюВРеглУчете(РасчетныеПартии);
			//-- НЕ УТ
...
Процедура ВернутьДокументыКОтражениюВРеглУчете(Расчеты)
...
Процедура ПроверитьБлокировкуВходящихДанныхПриОбновленииИБ(ПредставлениеОперации)
	...
	//++ НЕ УТ
	ВходящиеДанные.Вставить(Метаданные.Документы.ПоступлениеУслугПоЛизингу);
	//-- НЕ УТ
