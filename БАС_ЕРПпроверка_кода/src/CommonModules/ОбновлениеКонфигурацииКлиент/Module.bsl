...
#Если НЕ ВебКлиент Тогда
	...
	Если НастройкиОбновления.КонфигурацияИзменена И НастройкиОбновления.ЕстьДоступДляПроверкиОбновления Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Обновление конфигурации';uk='Оновлення конфігурації'"),
			"e1cib/app/Обработка.ПоискИУстановкаОбновлений",
			НСтр("ru='Конфигурация отличается от основной конфигурации информационной базы.';uk='Конфігурація відрізняється від основної конфігурації інформаційної бази.'"), 
	...
	Если ЕстьДоступноеОбновлениеВИнтернете И НастройкиОбновления.ЕстьДоступДляПроверкиОбновления Тогда
		...
		Параметры.НуженФайлОбновления = НастройкиОбновленияКонфигурации.НуженФайлОбновления;
		Параметры.ФлагАвтоПереходаНаСтраницуСОбновлением = Истина;
		ПоказатьОповещениеПользователя(НСтр("ru='Доступно обновление конфигурации';uk='Доступне оновлення конфігурації'"),
			"e1cib/app/Обработка.ПоискИУстановкаОбновлений",
			НСтр("ru='Версия:';uk='Версія:'") + " " + Параметры.ПараметрыФайлаПроверкиОбновления.Version, 
			БиблиотекаКартинок.Информация32);
		Возврат;	
	КонецЕсли;
...
Функция АдресСтраницыИнформацииОПолученииДоступаКПользовательскомуСайту()
	...
	АдресСтраницы = "https://login.1c.eu/registration"; // Значение по умолчанию
...
Процедура ПроверитьНаличиеОбновленияЧерезИнтернет(ВыдаватьСообщения = Ложь, ДоступноОбновлениеДляНовойРедакции = Ложь) Экспорт
	...
	Состояние(НСтр("ru='Проверка наличия обновления в Интернете';uk='Перевірка наявності оновлень в Інтернеті'"));
	Параметры = ПолучитьДоступноеОбновлениеКонфигурации(); 
	...
	Если ТипЗнч(Параметры.ПараметрыФайлаПроверкиОбновления) = Тип("Строка") Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Предупреждение",
			НСтр("ru='Невозможно подключиться к сети Интернет для проверки обновлений.';uk='Неможливо підключитися до мережі Інтернет для перевірки оновлень.'"));
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияКонфигурации, Параметры.ПоследняяВерсияКонфигурации) >= 0 Тогда
		...
		Если ОбновленияНеОбнаружено Тогда
			...
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Информация",
				НСтр("ru='Обновление не требуется: последняя версия конфигурации уже установлена.';uk='Оновлення не потрібно: остання версія конфігурації вже встановлена.'"));
	...
	ТекстСообщения = НСтр("ru='Обнаружена более новая версия конфигурации в Интернете: %1.';uk='Виявлена більш нова версія конфігурації в Інтернеті: %1.'");
...
Процедура ПроверитьОбновлениеПоРасписанию() Экспорт
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),, 
		НСтр("ru='Проверка наличия обновления в сети Интернет по расписанию.';uk='Перевірка наявності оновлення в мережі Інтернет за розкладом.'"));
	...
	Если Параметры.ИсточникОбновления <> -1 И Параметры.ИмяСтраницы = НаименованиеСтраницыДоступногоОбновления Тогда
		...
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обнаружена новая версия конфигурации: %1';uk='Виявлена нова версія конфігурації: %1'"), Параметры.ПараметрыФайлаПроверкиОбновления.Version));
	...
	Иначе
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),, 
			НСтр("ru='Доступных обновлений не обнаружено.';uk='Доступних оновлень не виявлено.'"));
	КонецЕсли;
	ОбновлениеКонфигурацииВызовСервера.ЗаписатьСтруктуруНастроекПомощника(НастройкиОбновленияКонфигурации, ПараметрыПриложения["СтандартныеПодсистемы.СообщенияДляЖурналаРегистрации"]);
...
Процедура ПриПолученииСпискаПредупрежденийЗавершенияРаботы(Предупреждения) Экспорт
	...
	Если ПараметрыПриложения["СтандартныеПодсистемы.ПредлагатьОбновлениеИнформационнойБазыПриЗавершенииСеанса"] = Истина Тогда
		...
		ПараметрыПредупреждения.ТекстФлажка  = НСтр("ru='Установить обновление конфигурации';uk='Установити оновлення конфігурації'");
...
Функция ПроверитьЛегальностьСкачиванияОбновления(ПараметрыЗапроса) Экспорт
	...
	Исключение
		ТекстОшибки = НСтр("ru='Ошибка создания описания веб-сервиса проверки легальности получения обновления.';uk='Помилка створення опису веб-сервіса перевірки легальності одержання оновлення.'");
	...
	Попытка
		...
		Если НРег(СокрЛП(ОтветСервера)) <> "ready" Тогда
			...
			ТекстОшибки = НСтр("ru='Сервис проверки легальности получения обновлений временно недоступен.
                ...
                |;uk='Сервіс перевірки легальності одержання оновлень тимчасово недоступний.
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru='Не удалось подключиться к сервису проверки легальности получения обновлений.
            ...
            |;uk='Не вдалося підключитися до сервісу перевірки легальності одержання оновлень.
            |Перевірте параметри підключення до Інтернету'");
		Возврат СтруктураОтветаВебСервера(ТекстОшибки, Истина,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПроверитьЛегальностьПолученияОбновлений(ДополнительныеПараметры, ОписаниеСервисаПроверкиЛегальности)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru='Ошибка выполнения проверки легальности получения обновления.
            ...
            |;uk='Помилка виконання перевірки легальності одержання оновлення.
	...
	Иначе
		...
		Результат = СтруктураОтветаВебСервера(НСтр("ru='Неожиданный ответ сервиса проверки легальности получения обновлений';uk='Несподівана відповідь сервісу перевірки легальності одержання оновлень'"), Истина);
...
Функция СтруктураОтветаВебСервера(ТекстОшибки, ЗаписьВЖурналРегистрации,
	...
	Если ЗаписьВЖурналРегистрации Тогда
		...
		Если ПустаяСтрока(ТекстСообщения) Тогда
			ТекстСообщения = НСтр("ru='%ТекстОшибки. Код ошибки: %КодОшибки.';uk=' %ТекстОшибки. Код помилки: %КодОшибки.'");
...
Функция ОписаниеСервисаПроверкиЛегальности()
	...
	Исключение
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса.
            ...
            |;uk='Помилка при створенні опису веб-сервіса.
            |Не вдалося одержати WSDL-опис із сервера перевірки легальності завантаження оновлення (%1): %2.'"),
	...
	Исключение
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
            ...
            |;uk='Помилка при створенні опису веб-сервіса (%1).
            |Помилка читання WSDL-опису веб-сервіса перевірки легальності завантаження оновлення: %2.'"),
			АдресWSDL, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение ТекстИсключения;
	КонецПопытки;
	...
	Если УзлыСхемы.Количество() = 0 Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
            ...
            |;uk='Помилка при створенні опису веб-сервіса (%1).
            |Помилка читання WSDL-опису веб-сервіса перевірки легальності завантаження оновлення:
            |Відсутній елемент опису типів даних (<wsdl:types ...>).'"), АдресWSDL);
		ВызватьИсключение ТекстИсключения;
	...
	Если УзелОписанияСхемы = Неопределено Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
            |Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:
            |Отсутствует элемент описания типов данных (<xs:schema ...>)'
            |;uk='Помилка при створенні опису веб-сервіса (%1).
            |Помилка читання WSDL-опису веб-сервіса перевірки легальності завантаження оновлення:
            |Відсутній елемент опису типів даних (<xs:schema ...>)'"), АдресWSDL);
		ВызватьИсключение ТекстИсключения;
	...
	Исключение
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
            ...
            |%2'
            |;uk='Помилка при створенні опису веб-сервіса (%1).
            |Помилка при створенні схеми даних з WSDL-опису веб-сервіса перевірки легальності завантаження оновлення:
	...
	Исключение
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
            ...
            |;uk='Помилка при створенні опису веб-сервіса (%1).
            |Помилка при створенні фабрики XDTO з WSDL-опису веб-сервіса перевірки легальності завантаження оновлення: %2'"),
			АдресWSDL, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение ТекстИсключения;
	...
	Если НЕ ЗначениеЗаполнено(URIСервиса) Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
            |Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:
            |Отсутствует URI пространства имен в WSDL-описании.'
            |;uk='Помилка при створенні опису веб-сервіса (%1).
            |Помилка читання WSDL-опису веб-сервіса перевірки легальності завантаження оновлення:
            |Відсутній URI простору імен в WSDL-описі.'"), АдресWSDL);
		ВызватьИсключение ТекстИсключения;
	...
	Если УзлыСервисов.Количество() = 0 Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
            ...
            |;uk='Помилка при створенні опису веб-сервіса (%1).
            |Помилка читання WSDL-опису веб-сервіса перевірки легальності завантаження оновлення:
            |Відсутнє опис веб-сервісів в WSDL-описі (<wsdl:service ...>).'"), АдресWSDL);
		ВызватьИсключение ТекстИсключения;
	...
	Если УзлыПортов.Количество() = 0 Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
            ...
            |;uk='Помилка при створенні опису веб-сервіса (%1).
            |Помилка читання WSDL-опису веб-сервіса перевірки легальності завантаження оновлення:
            |Відсутнє опис портів в WSDL-описі (<wsdl:port ...>).'"), АдресWSDL);
		ВызватьИсключение ТекстИсключения;
	...
	Если НЕ ЗначениеЗаполнено(ИмяПорта) Тогда
		...
		ТекстИсключения = СтрЗаменить(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
                                            ...
                                            |;uk='Помилка при створенні опису веб-сервісу (%1).
                                            |Помилка читання WSDL-опису веб-сервісу інтернет-підтримки користувачів:
                                            |Не вдалося визначити ім''я порту сервісу (%2).'"), АдресWSDL, ИмяСервиса);
	...
	Если НЕ ЗначениеЗаполнено(АдресПорта) Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
                |Ошибка чтения WSDL-описания веб-сервиса интернет-поддержки пользователей:
                |Не удалось определить URL заданного порта сервиса (%2).'
                |;uk='Помилка при створенні опису веб-сервіса (%1).
                |Помилка читання WSDL-опису веб-сервіса інтернет-підтримки користувачів:
                |Не вдалося визначити URL заданого порту сервісу (%2).'"), АдресWSDL, ИмяПорта);
...
Функция СервисДоступен(ОписаниеСервисаПроверкиЛегальности)
	...
	Исключение
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при вызове операции isReady сервиса (%1): %2';uk='Помилка при виклику операції isReady сервісу (%1): %2'"),
	...
	Если ТипЗнч(Значение) = Тип("Структура") Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при вызове операции isReady сервиса (%1)
            |Ошибка SOAP: %2'
            |;uk='Помилка при виклику операції isReady сервісу (%1)
            |Помилка SOAP: %2'"), ОписаниеСервисаПроверкиЛегальности.АдресWSDL, ОписаниеИсключенияSOAPВСтроку(Значение));
		ВызватьИсключение ТекстИсключения;
...
Функция ОбновлениеПолученоЛегально(ПараметрыЗапроса, ОписаниеСервисаПроверкиЛегальности)
	...
	Исключение
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при вызове операции process сервиса (%1): %2';uk='Помилка при виклику операції process сервісу (%1): %2'"),
			ОписаниеСервисаПроверкиЛегальности.АдресWSDL, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение ТекстИсключения;
...
Функция ОтправитьЗапросSOAP(ТекстКонверта, ОписаниеСервисаПроверкиЛегальности)
	...
	Исключение
		ТекстИсключения = НСтр("ru='Ошибка сетевого соединения при отправке SOAP-запроса.';uk='Помилка мережевого з''єднання при відправленні SOAP-запиту.'")
			+ Символы.ПС
			+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение ТекстИсключения;
	КонецПопытки;
...
Функция ПрочитатьОтветВКонвертеSOAP(ТелоОтвета, ОписаниеСервисаПроверкиЛегальности, ТипЗначения)
	...
	Исключение
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка чтения ответа SOAP:
            ...
            |Тело ответа:
            |%2'
            |;uk='Помилка читання відповіді SOAP:
            | %1
            |Тіло відповіді:
	...
	Если ЧтениеОтвета.ТипУзла = ТипУзлаXML.НачалоЭлемента
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка SOAP-Сервера при обработке запроса: %1';uk='Помилка SOAP-сервера при обробці запиту: %1'"), ОписаниеИсключенияSOAPВСтроку(ДеталиИсключения));
		ВызватьИсключение ТекстИсключения;
	...
	Исключение
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка чтения объекта (%1) в конверте SOAP:
            ...
            |Тело ответа:
            |%3'
            |;uk='Помилка читання об''єкта (%1) у конверті SOAP:
            | %2
            |Тіло відповіді:
...
Функция ОписаниеИсключенияSOAPВСтроку(ИсключениеSOAP)
	...
	Если НЕ ПустаяСтрока(ИсключениеSOAP.FaultCode) Тогда
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Код ошибки: %1';uk='Код помилки: %1'"), ИсключениеSOAP.FaultCode);
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(ИсключениеSOAP.FaultString) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Строка ошибки: %1';uk='Рядок помилки: %1'"), ИсключениеSOAP.FaultString);
		Результат = Результат + Символы.ПС + ТекстОшибки;
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(ИсключениеSOAP.FaultActor) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Источник ошибки: %1';uk='Джерело помилки: %1'"), ИсключениеSOAP.FaultActor);
		Результат = Результат + Символы.ПС + ТекстОшибки;
	КонецЕсли;
