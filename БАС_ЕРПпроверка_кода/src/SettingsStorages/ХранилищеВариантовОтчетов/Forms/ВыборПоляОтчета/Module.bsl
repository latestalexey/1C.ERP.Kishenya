...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("Режим", Режим) Тогда
		ВызватьИсключение НСтр("ru='Не передан служебный параметр ""Режим"".';uk='Не переданий службовий параметр ""Режим"".'");
	КонецЕсли;
	...
	ИначеЕсли Режим = "Отборы" Или Режим = "ВыбранныеПоля" Или Режим = "Сортировка" Или Режим = "ПоляГруппировки" Тогда
		ИмяТаблицы = Режим;
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Некорректное значение параметра ""Режим"": ""%1"".';uk='Некоректне значення параметру ""Режим: ""%1"".'"), Строка(Режим));
	КонецЕсли;
	...
	Если ИмяТаблицы = "ПоляГруппировки" Тогда
		...
		Если Режим = "СтруктураВарианта" Тогда
			...
			СтрокаДерева.Представление  = НСтр("ru='<Детальные записи>';uk='<Детальні записи>'");
...
Процедура ВыбратьИЗакрыть()
	...
	Если ТипЗнч(ДоступноеПолеКД) = Тип("ДоступноеПолеКомпоновкиДанных")
		...
		Если ДоступноеПолеКД.Папка Тогда
			ПоказатьПредупреждение(, НСтр("ru='Выберите элемент';uk='Оберіть елемент'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
