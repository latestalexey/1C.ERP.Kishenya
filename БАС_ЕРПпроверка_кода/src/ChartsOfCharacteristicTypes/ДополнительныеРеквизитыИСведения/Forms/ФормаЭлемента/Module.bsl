...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ПараметрыЗаписи.Свойство("КогдаНаименованиеУжеИспользуется") Тогда
		...
		Если ЗначениеЗаполнено(ТекстВопроса) Тогда
			...
			Кнопки.Добавить("ПродолжитьЗапись",            НСтр("ru='Продолжить запись';uk='Продовжити запис'"));
			Кнопки.Добавить("ВернутьсяКВводуНаименования", НСтр("ru='Вернуться к вводу наименования';uk='Повернутися до введення найменування'"));
	   ...
	   И НЕ Объект.ДополнительныеЗначенияСВесом Тогда
		...
		ТекстВопроса =
			НСтр("ru='Очистить введенные весовые коэффициенты?
                       ...
                       |;uk='Очистити уведені вагові коефіцієнти?
                       |
                       |Дані будуть записані.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("ОчиститьИЗаписать", НСтр("ru='Очистить и записать';uk='Очистити й записати'"));
...
Процедура КомандаДалее(Команда)
	...
	Если ИндексСтраницы = 0
		...
		ТекстПредупреждения = НСтр("ru='Элемент не выбран!';uk='Елемент не вибрано!'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	...
	Если ВариантТекстаВопроса = "ПереходКСпискуЗначений" Тогда
		ТекстВопроса =
			НСтр("ru='Переход к работе со списком значений
                       ...
                       |
                       |Данные будут записаны.'
                       |;uk='Перехід до роботи зі списком значень
                       |можливий тільки після запису даних.
                       |
                       |Дані будуть записані.'");
	Иначе
		ТекстВопроса =
...
Процедура РедактироватьФорматЗначенияЗавершение(Текст, Неопределен) Экспорт
	...
	Если Текст <> Неопределено Тогда
		...
		Для каждого Подстрока Из Массив Цикл
			Если СтрНайти(Подстрока, "ДП=") > 0 ИЛИ СтрНайти(Подстрока, "DE=") > 0 Тогда
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
					+ НСтр("ru='Представление пустой даты не поддерживается в полях ввода.';uk='Представлення порожньої дати не підтримується в полях введення.'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ЧН=") > 0 ИЛИ СтрНайти(Подстрока, "NZ=") > 0 Тогда
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
					+ НСтр("ru='Представление пустого числа не поддерживается в полях ввода.';uk='Представлення порожнього числа не підтримується в полях введення.'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ДФ=") > 0 ИЛИ СтрНайти(Подстрока, "DF=") > 0 Тогда
				Если СтрНайти(Подстрока, "ддд") > 0 ИЛИ СтрНайти(Подстрока, "ddd") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ НСтр("ru='Кратное название дня недели не поддерживается в полях ввода.';uk='Кратна назва дня тижня не підтримується в полях введення.'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "дддд") > 0 ИЛИ СтрНайти(Подстрока, "dddd") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ НСтр("ru='Полное название дня недели не поддерживается в полях ввода.';uk='Повна назва дня тижня не підтримується в полях введення.'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "МММ") > 0 ИЛИ СтрНайти(Подстрока, "MMM") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ НСтр("ru='Кратное название месяца не поддерживается в полях ввода.';uk='Кратна назва місяця не підтримується в полях введення.'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "ММММ") > 0 ИЛИ СтрНайти(Подстрока, "MMMM") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ НСтр("ru='Полное название месяца не поддерживается в полях ввода.';uk='Повна назва місяця не підтримується в полях введення.'");
				КонецЕсли;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ДЛФ=") > 0 ИЛИ СтрНайти(Подстрока, "DLF=") > 0 Тогда
				Если СтрНайти(Подстрока, "ДД") > 0 ИЛИ СтрНайти(Подстрока, "DD") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ НСтр("ru='Длинная дата (месяц прописью) не поддерживается в полях ввода.';uk='Довга дата (місяць прописом) не підтримується в полях введення.'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		...
		Если ЗначениеЗаполнено(ТекстПредупреждения) Тогда
			ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
				+ НСтр("ru='В местах использования полей надписей ограничений нет.';uk='У місцях використання полів написів обмежень немає.'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьНастройкиПомощника(ТекущаяСтраница = Неопределено)
	...
	ШаблонЗаголовкаСписка        = НСтр("ru='Выберите %1 для включения в набор ""%2""';uk='Оберіть %1 для включення в набір ""%2""'");
	ШаблонЗаголовкаПереключателя = НСтр("ru='Выберите вариант добавления дополнительного %1 ""%2"" в набор ""%3""';uk='Оберіть варіант додавання додаткового %1 ""%2"" в набір ""%3""'");
	
	Если ТекущаяСтраница = Элементы.ВыборРеквизита Тогда
		Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
			Заголовок = НСтр("ru='Добавление дополнительного сведения';uk='Додавання додаткової відомості'");
		Иначе
			Заголовок = НСтр("ru='Добавление дополнительного реквизита';uk='Додавання додаткового реквізиту'");
		КонецЕсли;
		
		Элементы.КоманднаяПанельЛево.Доступность = Ложь;
		Элементы.КомандаДалее.Заголовок = НСтр("ru='Далее >';uk='Далі >'");
		
		Элементы.ДекорацияЗаголовок.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонЗаголовкаСписка,
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр("ru='дополнительное сведение';uk='додаткова відомість'"), НСтр("ru='дополнительный реквизит';uk='додатковий реквізит'")),
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВыборДействия Тогда
		...
		Элементы.РежимДобавленияРеквизита.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр("ru='сведения';uk='відомості'"), НСтр("ru='реквизита';uk='реквізиту'")),
	...
	Иначе
		Элементы.КомандаДалее.Заголовок = НСтр("ru='Готово';uk='Готово'");
		Элементы.КоманднаяПанельЛево.Доступность = Истина;
	КонецЕсли;
...
Процедура ОбновитьСоставЭлементовФормы(ТекстПредупреждения = "")
				   ...
				   И НЕ Объект.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) Тогда
					...
					ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Недопустимо удалять тип ""%1"",
                                   ...
                                   |;uk='Неприпустимо вилучити тип ""%1"",
                                   |тому що додаткові значення вже уведені.
                                   |Спочатку потрібно видалити введені додаткові значення.
                                   |
                                   |Вилучений тип відновлений.'"),
			...
			Если НЕ СтарыйТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) Тогда
				...
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Недопустимо одновременно использовать типы значения
                               ...
                               |
                               |Второй тип удален.'
                               |;uk='Неприпустимо одночасно використовувати типи значення
                               |""%1"" й
                               |""%2"".
                               |
                               |Другий тип вилучений.'"),
					Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектов")),
					Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) );
	...
	Иначе
		...
		Если СвойстваВладельца.ЭтоДополнительноеСведение <> Истина Тогда
			ШаблонУточнения = НСтр("ru='Список значений общий с реквизитом ""%1"" набора ""%2""';uk='Список значень загальний з реквізитом ""%1"" набору ""%2""'");
		Иначе
			ШаблонУточнения = НСтр("ru='Список значений общий со сведением ""%1"" набора ""%2""';uk='Список значень загальний з відомістю ""%1"" набору ""%2""'");
		КонецЕсли;
	...
	Иначе
		...
		Если СписокНаборов.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(Объект.НаборСвойств)
			   И СписокНаборов.Количество() < 2 Тогда
				...
				Если Объект.ЭтоДополнительноеСведение Тогда
					ШаблонУточнения = НСтр("ru='Сведение входит в набор: %1';uk='Відомість входить у набір: %1'");
				Иначе
					ШаблонУточнения = НСтр("ru='Реквизит входит в набор: %1';uk='Реквізит входить у набір: %1'");
				КонецЕсли;
			...
			Иначе
				Если СписокНаборов.Количество() > 1 Тогда
					Если Объект.ЭтоДополнительноеСведение Тогда
						ШаблонУточнения = НСтр("ru='Общее сведение входит в %1 %2';uk='Загальна відомість входить в %1 %2'");
					Иначе
						ШаблонУточнения = НСтр("ru='Общий реквизит входит в %1 %2';uk='Загальний реквізит входить в %1 %2'");
					КонецЕсли;
					
					СтрокаНаборы = ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(СписокНаборов.Количество(),
						"Л = ru_RU", НСтр("ru='набор,набора,наборов,,,,,,0';uk='набір,набору,наборів,,,,,,0'"));
				...
				Иначе
					Если Объект.ЭтоДополнительноеСведение Тогда
						ШаблонУточнения = НСтр("ru='Общее сведение входит в набор: %1';uk='Загальна відомість входить до набору: %1'");
					Иначе
						ШаблонУточнения = НСтр("ru='Общий реквизит входит в набор: %1';uk='Загальний реквізит входить у набір: %1'");
					КонецЕсли;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
				Если Объект.ЭтоДополнительноеСведение Тогда
					ТекстКомментария = НСтр("ru='Сведение не входит в набор';uk='Відомість не входить у набір'");
				Иначе
					ТекстКомментария = НСтр("ru='Реквизит не входит в набор';uk='Реквізит не входить у набір'");
				КонецЕсли;
			Иначе
				Если Объект.ЭтоДополнительноеСведение Тогда
					ТекстКомментария = НСтр("ru='Общее сведение не входит в наборы';uk='Загальна відомість не входить у набори'");
				Иначе
					ТекстКомментария = НСтр("ru='Общий реквизит не входит в наборы';uk='Загальний реквізит не входить у набори'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если Элементы.УточнениеНаборовКомментарий.Гиперссылка Тогда
			Элементы.УточнениеНаборовКомментарий.Подсказка = НСтр("ru='Переход к набору';uk='Перехід до набору'");
		Иначе
...
Процедура ЗаполнитьСписокДействийПриДобавленииРеквизита()
	...
	Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
		ДобавитьОбщий = НСтр("ru='Добавить общее сведение в набор (рекомендуется)
            ...
            |;uk='Додати загальну відомість в набір (рекомендується)
            |
            |Вибрана відомість вже входить в кілька наборів, тому рекомендується також включити його в набір ""як є"".
            |У цьому випадку буде можливо відбирати з нього дані різних типів в списках та звітах.'");
		СделатьОбщим = НСтр("ru='Сделать дополнительное сведение общим и добавить в набор
            ...
            |В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.'
            |;uk='Зробити додаткову відомість загальною і додати в набір
            |
            |Цей варіант підходить для тих випадків, коли відомість повинна бути однаковою для обох наборів.
            |У цьому випадку буде можливо відбирати з нього дані різних типів в списках та звітах.'");
		СделатьПоОбразцу = НСтр("ru='Сделать копию сведения по образцу (с общим списком значений)
            ...
            |;uk='Зробити копію відомості за зразком (із загальним списком значень)
            |
            |Список значень цього відомості буде однаковий для обох наборів.
            |За допомогою цього варіанту зручно виконувати централізована настройка списку значень відразу для декількох однотипних дані.
            |При цьому можна відредагувати найменування і ряд інших властивостей відомості.'");
		СоздатьКопированием = НСтр("ru='Сделать копию сведения
            |
            |Будет создана копия сведения%1'
            |;uk='Зробити копію відомості
            |
            |Буде створена копія відомості%1'");
	Иначе
		ДобавитьОбщий = НСтр("ru='Добавить общий реквизит в набор (рекомендуется)
            ...
            |В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.'
            |;uk='Додати загальний реквізит у набір (рекомендується)
            |
            |Обраний реквізит вже входить в кілька наборів, тому рекомендується також включити його в набір ""як є"".
            |У цьому випадку буде можливо відбирати з нього дані різних типів в списках та звітах.'");
		СделатьОбщим = НСтр("ru='Сделать реквизит общим и добавить в набор
            ...
            |В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.'
            |;uk='Зробити реквізит загальним і додати в набір
            |
            |Цей варіант підходить для тих випадків, коли реквізит повинен бути однаковий для обох наборів.
            |У цьому випадку буде можливо відбирати з нього дані різних типів в списках та звітах.'");
		СделатьПоОбразцу = НСтр("ru='Сделать копию реквизита по образцу (с общим списком значений)
            ...
            |;uk='Зробити копію реквізиту за зразком (із загальним списком значень)
            |
            |Список значень цього реквізиту буде однаковий для обох наборів.
            |За допомогою цього варіанту зручно виконувати централізовану настройку списку значень відразу для декількох однотипних реквізитів.
            |При цьому можна відредагувати найменування і ряд інших властивостей реквізиту.'");
		СоздатьКопированием = НСтр("ru='Сделать копию реквизита
            |
            |Будет создана копия реквизита%1'
            |;uk='Зробити копію реквізиту
            |
            |Буде створена копія реквізиту%1'");
	КонецЕсли;
	...
	Если РеквизитСоСпискомДополнительныхЗначений Тогда
		ШаблонВставки = " " + НСтр("ru='и всех его значений.';uk='і всіх його значень.'");
	Иначе
		ШаблонВставки = ".";
	КонецЕсли;
...
Процедура УстановитьЗаголовкиГиперссылок()
	...
	ШаблонЗависимостьЗадана = НСтр("ru='при заданном условии';uk='за заданої умови'");
	ШаблонЗависимостьНеЗадана = НСтр("ru='всегда';uk='завжди'");
...
Процедура УстановитьЗаголовокФормы()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ru='(Дополнительное сведение)';uk='(Додаткова відомість)'");
			Иначе
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ru='(Дополнительный реквизит)';uk='(Додатковий реквізит)'");
			КонецЕсли;
		Иначе
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ru='(Общее дополнительное сведение)';uk='(Загальна додаткова відомість)'");
			Иначе
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ru='(Общий дополнительный реквизит)';uk='(Загальний додатковий реквізит)'");
			КонецЕсли;
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = НСтр("ru='Дополнительное сведение (создание)';uk='Додаткова відомість (створення)'");
			Иначе
				Заголовок = НСтр("ru='Дополнительный реквизит (создание)';uk='Додатковий реквізит (створення)'");
			КонецЕсли;
		Иначе
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = НСтр("ru='Общее дополнительное сведение (создание)';uk='Загальна додаткова відомість (створення)'");
			Иначе
				Заголовок = НСтр("ru='Общий дополнительный реквизит (создание)';uk='Загальний додатковий реквізит (створення)'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
		   ...
		   И УправлениеСвойствамиСлужебный.ТипЗначенияСодержитЗначенияСвойств(Выборка.ТипЗначения) Тогда
			...
			Если ПервыеЗначения.Количество() = 0 Тогда
				Если РезультатыЗапроса[1].Пустой() Тогда
					ПредставлениеЗначений = НСтр("ru='Значения еще не введены';uk='Значення ще не введені'");
				Иначе
					ПредставлениеЗначений = НСтр("ru='Значения помечены на удаление';uk='Значення відмічені для вилучення'");
				КонецЕсли;
			Иначе
...
Функция НаименованиеУжеИспользуется(Знач Заголовок, Знач ТекущееСвойство, Знач НаборСвойств, НовоеНаименование)
	...
	Если ЗначениеЗаполнено(Выборка.НаборСвойств) Тогда
		Если Выборка.ЭтоДополнительноеСведение Тогда
			ТекстВопроса = НСтр("ru='Существует дополнительное сведение с наименованием
                                      |""%1"".'
                                      |;uk='Існує додаткова відомість із найменуванням
                                      |""%1"".'");
		Иначе
			ТекстВопроса = НСтр("ru='Существует дополнительный реквизит с наименованием
                                      |""%1"".'
                                      |;uk='Існує додатковий реквізит з найменуванням
                                      |""%1"".'");
		КонецЕсли;
	Иначе
		Если Выборка.ЭтоДополнительноеСведение Тогда
			ТекстВопроса = НСтр("ru='Существует общее дополнительное сведение с наименованием
                                      |""%1"".'
                                      |;uk='Існує загальна додаткова відомість із найменуванням
                                      |""%1"".'");
		Иначе
			ТекстВопроса = НСтр("ru='Существует общий дополнительный реквизит с наименованием
                                      |""%1"".'
                                      |;uk='Існує загальний додатковий реквізит з найменуванням
                                      |""%1"".'");
		КонецЕсли;
	КонецЕсли;
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстВопроса + Символы.ПС + Символы.ПС
                                 ...
                                 |;uk='Рекомендується використовувати інше найменування,
                                 |інакше програма може працювати некоректно.'"),
		НовоеНаименование);
...
Процедура УстановитьЗаголовокКнопкиФормата(Форма)
	...
	Если ПустаяСтрока(Форма.Объект.ФорматСвойства) Тогда
		ТекстЗаголовка = НСтр("ru='Формат по умолчанию';uk='Формат за замовчуванням'");
	Иначе
		ТекстЗаголовка = НСтр("ru='Формат установлен';uk='Формат Встановлений'");
	КонецЕсли;
