...
Процедура ЗаписатьРасписание(Знач МодельРасписания, Знач МодельПланирования, ЗаписьВыполнена) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Запись пооперационного расписания';uk='Запис поопераційного розкладу'"),
...
Процедура ИзменитьСтатусОпераций(Знач Операции, Знач НовыйСтатус) Экспорт
	...
	Попытка
		...
		Иначе
			...
			ТекстИсключения = НСтр("ru='Действие не может быть выполнено по причине: %1 в процессе пересчета';uk='Дія не може бути виконана по причині: %1 в процесі перерахунку'");
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстИсключения, ?(Операции.Количество()=1, НСтр("ru='операция находится';uk='операція знаходиться'"), НСтр("ru='операции находятся';uk='операції знаходяться'")));
...
Процедура НазначитьШтрихкоды(Операции) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Данные.Изменение';uk='Дані.Зміна'"), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура ПриПереходеСРегистрацииОтклоненийНаРегистрациюФакта(Знач ПараметрыЗадания, АдресХранилища) Экспорт
	...
	Попытка
		...
		Иначе
			...
			ТекстИсключения = НСтр("ru='При попытке изменения способа управления операциями возникала ошибка:
                ...
                |;uk='При спробі зміни способу управління операціями виникала помилка:
                |операції розкладу підрозділу знаходяться в процесі перерахунку'");
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Изменение способа управления операциями';uk='Зміна способу управління операціями'"),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
