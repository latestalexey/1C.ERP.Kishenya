...
Процедура УстановитьВсем(Команда)
	...
	ПоказатьВводДаты(ОбработчикВводаДаты,ДатыЗапрета[0].ДатаЗапрета,НСтр("ru='Введите дату';uk='Введіть дату'"),ЧастиДаты.Дата);
...
Процедура ОчиститьВсем(Команда)
	...
	ТекстВопроса = НСтр("ru='Убрать все даты?';uk='Прибрати всі дати?'");
	Если Элементы.ДатыЗапрета.ВыделенныеСтроки.Количество() > 1 Тогда
		ТекстВопроса = НСтр("ru='Убрать выбранные даты?';uk='Прибрати вибрані дати?'");
	КонецЕсли;
