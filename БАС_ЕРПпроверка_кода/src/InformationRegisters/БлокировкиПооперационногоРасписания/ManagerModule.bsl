...
// Устанавливает транзакционные блокировки на записи регистра сведений БлокировкиПооперационногоРасписания,
...
Процедура ЗаблокироватьПодразделение(Подразделение, Режим) Экспорт
	...
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.БлокировкиПооперационногоРасписания");
...
//
// Параметры:
//  Коллекция -  РезультатЗапроса,Табличная часть,ТаблицаЗначений - коллекция,
...
Процедура ЗаблокироватьИзмененСпособУправления(Подразделение) Экспорт
	...
	Набор = РегистрыСведений.БлокировкиПооперационногоРасписания.СоздатьНаборЗаписей();
...
// Записывает данные о блокируемых операциях расписания в регистр сведений БлокировкиПооперационногоРасписания.
...
//  Операции - Массив - содержит идентификаторы блокируемых операций.
//
Процедура ЗаблокироватьРасчетРасписания(НомерСеанса, НачалоСеанса, Подразделение, Пользователь, Операции) Экспорт
	...
	Для каждого Операция Из Операции Цикл
		...
		Запись.Подразделение = Подразделение;
		Запись.ИдентификаторОперации = Операция;
		Запись.ПричинаБлокировки = ПричинаБлокировки;
...
Функция УстановленныеБлокировки(Подразделение=Неопределено, Операции=Неопределено, ПричинаБлокировки=Неопределено)
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	БлокировкиПооперационногоРасписания.НомерСеанса,
	|	БлокировкиПооперационногоРасписания.НачалоСеанса,
	|	БлокировкиПооперационногоРасписания.ПричинаБлокировки,
	|	БлокировкиПооперационногоРасписания.Пользователь
	|ИЗ
	|	РегистрСведений.БлокировкиПооперационногоРасписания КАК БлокировкиПооперационногоРасписания
	|ГДЕ
...
Функция БлокировкаСеансаАктуальна(НомерСеанса, НачалоСеанса)
	...
	Для каждого Сеанс Из Сеансы Цикл
		Если Сеанс.НомерСеанса = НомерСеанса И Сеанс.НачалоСеанса = НачалоСеанса Тогда
			Актуальна = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
