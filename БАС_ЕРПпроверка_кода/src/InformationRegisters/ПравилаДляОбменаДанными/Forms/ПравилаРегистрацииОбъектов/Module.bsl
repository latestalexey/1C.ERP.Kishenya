...
Процедура ЗагрузитьПравила(Команда)
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ru='Укажите, из какого файла загрузить правила';uk='Вкажіть, з якого файлу завантажити правила'"));
	ПараметрыДиалога.Вставить("Фильтр",
		  НСтр("ru='Файлы правил регистрации (*.xml)';uk='Файли правил реєстрації (*.xml)'") + "|*.xml|"
		+ НСтр("ru='Архивы ZIP (*.zip)';uk='Архіви ZIP (*.zip)'")   + "|*.zip");
...
Процедура ВыгрузитьПравила(Команда)
	...
	ФильтрИмени = НСтр("ru='Файлы правил (*.xml)';uk='Файли правил (*.xml)'") + "|*.xml";
	...
	Если ПустаяСтрока(ЧастиИмени.ИмяБезРасширения) Тогда
		ПолноеИмяФайла = НСтр("ru='Правила регистрации';uk='Правила реєстрації'");
	Иначе
...
Процедура ВыполнитьЗагрузкуПравил(Знач АдресПомещенногоФайла, Знач ИмяФайла, Знач ЭтоАрхив)
	...
	Состояние(НСтр("ru='Выполняется загрузка правил в информационную базу...';uk='Виконується завантаження правил в інформаційну базу...'"));
...
Процедура ЗагрузитьПравилаЗавершение(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстОшибки) И ПустаяСтрока(АдресПомещенногоФайла) Тогда
		ТекстОшибки = НСтр("ru='Ошибка передачи файла на сервер';uk='Помилка передачі файлу на сервер'");
	КонецЕсли;
