	...
	Иначе
		ВызватьИсключение
			НСтр("ru='Ошибка в процедуре ОбновитьГруппировкиПользователей
                       ...
                       |;uk='Помилка в процедурі ОбновитьГруппировкиПользователей
                       |модуля менеджера регістра відомостей Групи значень доступу.
                       |
                       |Вказані невірні параметри.'");
	КонецЕсли;
...
Процедура ОбновитьГруппыЗначенияДоступа(ЗначениеДоступа, ЕстьИзменения)
	...
	ЗаголовокОшибки =
		НСтр("ru='Ошибка при обновлении групп значений доступа.
                   |
                   |'
                   |;uk='Помилка при оновленні груп значень доступу.
                   |
                   |'");
	...
	Если СвойстваВидаДоступа = Неопределено Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для типа ""%1""
                       ...
                       |;uk='Для типу ""%1""
                       |не настроєно використання груп значень доступу.'"),
			 ...
			 ИЛИ ТабличнаяЧастьМетаданные.Реквизиты.Найти("ГруппаДоступа") = Неопределено Тогда
				...
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У типа значений доступа ""%1""
                               ...
                               |;uk='У типу значень доступу ""%1""
                               |не створена спеціальна таблична частина ГруппыДоступа
                               |зі спеціальним реквізитом ГруппаДоступа.'"),
					Строка(ТипЗначенияДоступа));
			Иначе
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У значения доступа ""%1""
                               ...
                               |%3'
                               |;uk='Значення доступу ""%1""
                               |типу ""%2""
                               |не вдалося прочитати табличну частину ГруппаДоступа
                               |з реквізитом ГруппаДоступа за помилку:
                               |%3'"),
			...
			Если МетаданныеТипа.Реквизиты.Найти("ГруппаДоступа") = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У типа значений доступа ""%1""
                               ...
                               |;uk='У типу значень доступу ""%1""
                               |не створено спеціальний реквізит ГруппаДоступа.'"),
					Строка(ТипЗначенияДоступа));
			Иначе
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У значения доступа ""%1""
                               ...
                               |;uk='Значення доступу ""%1""
                               |типу ""%2""
                               |не вдалося прочитати реквізит ГруппаДоступа за помилку:
                               |%3'"),
					Строка(ЗначениеДоступа),
					Строка(ТипЗначенияДоступа),
					КраткоеПредставлениеОшибки(Ошибка));
	...
	Иначе
		ВызватьИсключение
			НСтр("ru='Ошибка в процедуре ОбновитьГруппыИсполнителей
                       ...
                       |
                       |Указаны неверные параметры.'
                       |;uk='Помилка в процедурі ОбновитьГруппыИсполнителей
                       |модуля менеджера регістра відомостей ГруппыЗначенийДоступа.
                       |
                       |Вказані невірні параметри.'");
	КонецЕсли;
