		...
		Если СостояниеИзменено Тогда
			...
			Исключение
				...
				ИначеЕсли ТипЗнч(СтрокаТаблицы.Заказ) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
					ТекстСообщения = НСтр("ru='Не удалось отразить состояние заказа давальца: %Ссылка% по причине: %Причина%';uk='Не вдалося відобразити стан замовлення давальца: %Ссылка% по причині: %Причина%'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Ссылка%", СтрокаТаблицы.Заказ);
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ТаблицаСостоянийЗаказов(МассивСсылок)
	...
	ТекстЗапросаВременныхТаблиц = ТекстЗапросаДляРасчетаДатСобытийСостоянийЗаказов()
		...
		//++ НЕ УТКА
		+ ТекстЗапросаДляРасчетаСуммыОтгрузкиПоЗаказамДавальцев()
...
Функция ТекстЗапросаДляРасчетаСостоянийОбеспеченияЗаказа()
	...
	ТекстЗапроса = "
		...
		|				И ЕСТЬNULL(ОбеспечениеЗаказовОстатки.НаличиеПодЗаказОстаток,0) < ЗаказДавальцаТовары.Количество
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ ЛОЖЬ
		|		КОНЕЦ)
		|ИЗ
