...
Процедура ОбновитьДанныеДляПодбораСотрудников(Сотрудники, ОтключитьБизнесЛогику = Ложь, ДополнительныеСвойства = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	Сотрудники.Сотрудник = Сотрудники.ГоловнойСотрудник
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	Запрос.Текст =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	Подработки.Сотрудник
		|ИЗ
		|	ВТПодработки КАК Подработки";
		
	РезультатЗапросаПоПодработкам = Запрос.Выполнить();
	
	Если РезультатЗапросаПоСотрудникам.Пустой()
		И РезультатЗапросаПоПодработкам.Пустой() Тогда
		
		Возврат;
		
	КонецЕсли;
	
	Если Не РезультатЗапросаПоСотрудникам.Пустой()
		И Не РезультатЗапросаПоПодработкам.Пустой() Тогда
	...
	Иначе
		ИмяВТКадроваяИсторияСотрудников = "ВТРегистрСведений";
		ИмяВТСостоянияПодработок = "ВТРегистрСведений";
	КонецЕсли;
	...
	Если Не РезультатЗапросаПоПодработкам.Пустой() Тогда
	...
	Если Не РезультатЗапросаПоСотрудникам.Пустой()
		...
		Запрос.Текст =
			...
			|	РегистрСведений.ВидДоговора
			|ИЗ
			|	ВТСостоянияПодработок КАК РегистрСведений";
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТКадроваяИсторияСотрудников", ИмяВТКадроваяИсторияСотрудников);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТСостоянияПодработок", ИмяВТСостоянияПодработок);
	...
	Запрос.Текст =
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРегистрСведений КАК РегистрСведенийПоследующие
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	РегистрСведенийСПериодами.Сотрудник,
	...
	Запрос.Текст =
		...
		|	РегистрСведенийПериодПоследнейЗаписи.ВидСобытия,
		|	РегистрСведенийПериодПоследнейЗаписи.КоличествоСтавок,
		|	РегистрСведенийПериодПоследнейЗаписи.ВидЗанятости,
		|	РегистрСведенийПериодПоследнейЗаписи.ВидДоговора,
		|	КадровыеДанныеСотрудников.ТарифнаяСтавка,
		|	КадровыеДанныеСотрудников.ФОТ,
		|	КадровыеДанныеСотрудников.Надбавка,
		|	КадровыеДанныеСотрудников.СпособРасчетаАванса,
		|	КадровыеДанныеСотрудников.Аванс
		|ПОМЕСТИТЬ ВТНаборыЗаписей
		|ИЗ
		|	ВТРегистрСведенийСПериодами КАК РегистрСведенийСПериодами
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРегистрСведений КАК РегистрСведенийПериодПоследнейЗаписи
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРегистрСведений КАК РегистрСведенийНачалоСобытия
		...
		|	НаборыЗаписей.Должность,
		|	НаборыЗаписей.ДолжностьПоШтатномуРасписанию,
		|	ВЫБОР
		|		КОГДА ИзмененныеСотрудники.Сотрудник = ИзмененныеСотрудники.Сотрудник.ГоловнойСотрудник
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ЭтоГоловнойСотрудник,
		|	НаборыЗаписей.ВидСобытия,
		|	НаборыЗаписей.КоличествоСтавок,
		|	НаборыЗаписей.ВидЗанятости,
		|	НаборыЗаписей.ВидДоговора,
		|	НаборыЗаписей.Период КАК Начало,
		|	НаборыЗаписей.Окончание,
		|	НаборыЗаписей.ТарифнаяСтавка,
		|	НаборыЗаписей.ФОТ,
		|	НаборыЗаписей.Надбавка,
		|	НаборыЗаписей.СпособРасчетаАванса,
		|	НаборыЗаписей.Аванс
		|ИЗ
