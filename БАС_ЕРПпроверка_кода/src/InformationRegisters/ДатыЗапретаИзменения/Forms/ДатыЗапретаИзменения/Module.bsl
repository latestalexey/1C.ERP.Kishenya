...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		...
		Если НЕ СвойстваРазделов.ДатыЗапретаЗагрузкиВнедрены Тогда
			ВызватьИсключение НСтр("ru='Даты запрета загрузки данных не применяются.';uk='Дати заборони завантаження даних не застосовуються.'");
		КонецЕсли;
		
		Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
			ВызватьИсключение НСтр("ru='Недостаточно прав для работы с датами запрета загрузки.';uk='Недостатньо прав для роботи з датами заборони завантаження.'");
		КонецЕсли;
		
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ru='Использование дат запрета загрузки выключено в настройках программы.';uk='Використання дат заборони завантаження вимкнено в настройках програми.'");
		
		Заголовок = НСтр("ru='Даты запрета загрузки данных';uk='Дати заборони завантаження даних'");
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("НетЗапрета").Представление =
			НСтр("ru='Нет запрета загрузки данных';uk='Немає заборони завантаження даних'");
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("ДляВсехПользователей").Представление =
			НСтр("ru='Для всех информационных баз';uk='Для всіх інформаційних баз'");
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("ПоПользователям").Представление =
			НСтр("ru='По информационным базам';uk='За інформаційними базами'");
		
		Элементы.ПользователиПолноеПредставление.Заголовок =
			НСтр("ru='Программа: информационная база';uk='Програма: інформаційна база'");
	...
	Иначе
		Если Не Пользователи.РолиДоступны("ДобавлениеИзменениеДатЗапретаИзменения") Тогда
			ВызватьИсключение НСтр("ru='Недостаточно прав для работы с датами запрета изменения.';uk='Недостатньо прав для роботи з датами заборони зміни.'");
		КонецЕсли;
		
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ru='Использование дат запрета изменения выключено в настройках программы.';uk='Використання дат заборони зміни вимкнено в настройках програми.'");
		
		ЗначениеДляВсехПользователей = Перечисления.ВидыНазначенияДатЗапрета.ДляВсехПользователей;
		
		СписокТиповПользователей.Добавить(
			Тип("СправочникСсылка.Пользователи"),        НСтр("ru='Пользователь';uk='Користувач'"));
		
		СписокТиповПользователей.Добавить(
			Тип("СправочникСсылка.ВнешниеПользователи"), НСтр("ru='Внешний пользователь';uk='Зовнішній користувач'"));
	КонецЕсли;
		...
		Если ТекущийСпособУказания <> СпособУказанияЗначениеВСписке.Значение Тогда
			...
			ПоказатьВопрос(
				...
				ТекстСообщенияСпособУказанияНеИспользован(
					...
					ТекущийПользователь,
					ЭтотОбъект) + Символы.ПС + Символы.ПС + НСтр("ru='Продолжить?';uk='Продовжити?'"),
				РежимДиалогаВопрос.ДаНет);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьКоманднуюПанельДатЗапрета(Контекст)
	...
	Иначе
		Если Контекст.СпособУказанияДатыЗапрета = "ПоРазделам" Тогда
			...
			УстановитьСвойство(Элементы.ДатыЗапретаРазделы.Заголовок,   НСтр("ru='Разделы';uk='Розділи'"));
			УстановитьСвойство(Элементы.ДатыЗапретаПодобрать.Видимость, Ложь);
			УстановитьСвойство(Элементы.ДатыЗапретаДобавить.Видимость,  Ложь);
			УстановитьСвойство(Элементы.ДатыЗапретаКонтекстноеМенюДобавить.Видимость, Ложь);
			УстановитьСвойство(Элементы.ДатыЗапретаИзменить.Отображение, ОтображениеКнопки.Авто);
		...
		ИначеЕсли Контекст.СпособУказанияДатыЗапрета = "ПоОбъектам" Тогда
			...
			УстановитьСвойство(Элементы.ДатыЗапретаРазделы.Видимость,   Истина);
			УстановитьСвойство(Элементы.ДатыЗапретаРазделы.Заголовок,   НСтр("ru='Общая дата';uk='Загальна дата'"));
			УстановитьСвойство(Элементы.ДатыЗапретаПодобрать.Видимость, Истина);
			УстановитьСвойство(Элементы.ДатыЗапретаДобавить.Видимость,  Истина);
			УстановитьСвойство(Элементы.ДатыЗапретаКонтекстноеМенюДобавить.Видимость, Истина);
			УстановитьСвойство(Элементы.ДатыЗапретаИзменить.Отображение, ОтображениеКнопки.Авто);
	...
	Иначе
		...
		Если Контекст.РазрешитьИзменениеДанныхДоДатыЗапрета Тогда
			...
			Если Контекст.НачалоДня > СрокРазрешения Тогда
				ШаблонНадписи = НСтр("ru='Срок возможности изменения данных с %2 по %3 истек %1';uk='Термін можливості зміни даних з %2 по %3 минув %1'");
			Иначе
				Если РассчитатьДатуЗапрета Тогда
				...
				ШаблонНадписи =
					НСтр("ru='По %1 возможно изменение данных с %2 по %3
                               ...
                               |;uk='За %1 можлива зміна даних з %2 по %3
                               |Після %1 буде заборонено зміну даних з %3'")
					+ Символы.ПС;
			КонецЕсли;
		...
		Контекст.Элементы.ПояснениеНепроизвольнойДаты.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Запрещено изменение данных по %1';uk='Заборонена зміна даних до %1'"),
...
Функция СписокОписанийДатыЗапрета()
	...
	Список.Добавить("",                "(" + НСтр("ru='По умолчанию';uk='По умовчанню'") + ")");
	Список.Добавить("ПроизвольнаяДата",      НСтр("ru='Произвольная дата';uk='Довільна дата'"));
	Список.Добавить("КонецПрошлогоГода",     НСтр("ru='Конец прошлого года';uk='Кінець минулого року'"));
	Список.Добавить("КонецПрошлогоКвартала", НСтр("ru='Конец прошлого квартала';uk='Кінець минулого кварталу'"));
	Список.Добавить("КонецПрошлогоМесяца",   НСтр("ru='Конец прошлого месяца';uk='Кінець минулого місяця'"));
	Список.Добавить("КонецПрошлойНедели",    НСтр("ru='Конец прошлой недели';uk='Кінець минулого тижня'"));
	Список.Добавить("ПредыдущийДень",        НСтр("ru='Предыдущий день';uk='Попередній день'"));
	...
	Если Контекст.Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Для каждого ЗначениеСписка Из Контекст.СписокТиповПользователей Цикл
			Если ТипЗнч(ЗначениеСписка.Значение) = ТипЗнч(Пользователь) Тогда
				...
				Иначе
					Возврат ЗначениеСписка.Представление + ": " + НСтр("ru='<Все информационные базы>';uk='<Усі інформаційні бази>'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ТекстПредставленияОбщейДаты()
	...
	Возврат "<" + НСтр("ru='Общая дата';uk='Загальна дата'") + ">";
...
Функция ТекстВопросаЗакрытьФорму()
	...
	Возврат НСтр("ru='Закрыть форму?';uk='Закрити форму?'");
...
Функция ТекстВопросаОбновитьДанные()
	...
	Возврат НСтр("ru='Обновить данные?';uk='Оновити дані?'");
...
Функция ТекстВопросаУдалитьВсеДатыЗапретаКромеДатДляВсехПользователей()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Удалить все даты запрета, кроме установленных %1?';uk='Видалити усі дати заборони, крім встановлених %1?'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстВопросаУдалитьВсеДатыЗапрета()
	...
	Возврат НСтр("ru='Удалить все даты запрета?';uk='Видалити усі дати заборони?'");
...
Функция ТекстСообщенияЗначениеДляВсехПользователейНеИзменяется()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Значение %1 не изменяется.';uk='Значення %1 не змінюється.'"),
		ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
...
Функция ТекстСообщенияКомментарийДляВсехПользователейНеИзменяется()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Комментарий %1 не изменяется.';uk='Коментар %1 не змінюється.'"),
		ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
...
Функция ТекстСообщенияСначалаВыберитеПользователя()
	...
	Возврат НСтр("ru='Сначала выберите пользователя.';uk='Спочатку виберіть користувача.'");
...
Функция ТекстСообщенияУстановкаДатыЗапретаНеИспользован(УстановкаДатыЗапретаВФорме, УстановкаДатыЗапретаВБазеДанных)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Установка даты запрета ""%1"" не настроена,
                   ...
                   |;uk='Установка дати заборони ""%1"" не настроєна,
                   |тому буде збережена установка дати заборони ""%2"".'"),
	...
	Если НазначениеДляВсех(ТекущийПользователь) Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Способ указания даты запрета ""%1""
                       ...
                       |;uk='Спосіб зазначення дати заборони ""%1""
                       |не настроєн ""%2"", тому буде збережено
                       |спосіб зазначення дати заборони ""%3"".'"),
	...
	Иначе
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Способ указания даты запрета ""%1""
                       ...
                       |способ указания даты запрета ""%3"".'
                       |;uk='Спосіб зазначення дати заборони ""%1""
                       |не настроєн для ""%2"", тому буде збережено
                       |спосіб зазначення дати заборони ""%3"".'"),
			СпособУказанияВФорме,
			ТекущийПользователь,
			СпособУказанияВБазеДанных);
...
Функция ТекстВопросаОчиститьНезаполненныеСтроки()
	...
	Возврат НСтр("ru='Очистить незаполненные строки?';uk='Очистити незаповнені рядки?'");
...
Функция ТекстСообщенияДляУдаленияВыделитеОднуСтроку()
	...
	Возврат НСтр("ru='Для удаления выделите одну строку.';uk='Для вилучення виділіть один рядок.'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляВсехПользователей()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Удалить даты запрета %1?';uk='Видалити дати заборони %1?'"),
...
Функция ТекстВопросаУдалитьДатыЗапретаДляПользователя()
	...
	Возврат НСтр("ru='Удалить даты запрета для пользователя?';uk='Видалити дати заборони для користувача?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляГруппыПользователей()
	...
	Возврат НСтр("ru='Удалить даты запрета для группы пользователей?';uk='Видалити дати заборони для групи користувачів?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляВнешнегоПользователя()
	...
	Возврат НСтр("ru='Удалить даты запрета для внешнего пользователя?';uk='Видалити дати заборони для зовнішнього користувача?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляГруппыВнешнихПользователей()
	...
	Возврат НСтр("ru='Удалить даты запрета для группы внешних пользователей?';uk='Видалити дати заборони для групи зовнішніх користувачів?'");
...
Функция ТекстВопросаУдалитьДатыЗапрета()
	...
	Возврат НСтр("ru='Удалить даты запрета?';uk='Видалити дати заборони?'");
...
Функция ТекстСообщенияДляВсехПользователейДатыЗапретаНеУстановлены()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 даты запрета не установлены.';uk='%1 дати заборони не встановлені.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияЗначениеУжеДобавленоВСписок()
	...
	Возврат НСтр("ru='Значение уже добавлено в список.';uk='Значення вже додане в список.'");
...
Функция ТекстСообщенияОбновитьДанныеФормыF5()
	...
	Возврат НСтр("ru='Обновите данные формы (F5).';uk='Оновіть дані форми (F5).'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРазделовИОбъектов()
	...
	Возврат НСтр("ru='Удалить даты запрета для разделов и объектов?';uk='Видалити дати заборони для розділів і об''єктів?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляОбъектов()
	...
	Возврат НСтр("ru='Удалить даты запрета для объектов?';uk='Видалити дати заборони для об''єктів?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРазделов()
	...
	Возврат НСтр("ru='Удалить даты запрета для разделов?';uk='Видалити дати заборони для розділів?'");
...
Функция ТекстСообщенияВВыбранномРазделеДатыЗапретаДляОбъектовНеУстанавливаются()
	...
	Возврат НСтр("ru='В выбранном разделе даты запрета для объектов не устанавливаются.';uk='В обраному розділі дати заборони для об''єктів не встановлюються.'");
...
Функция ТекстСообщенияОбщаяДатаМожетБытьУстановлена()
	...
	Возврат НСтр("ru='<Общая дата> может быть установлена.';uk='<Загальна дата> може бути встановлена.'");
...
Функция ТекстСообщенияРазделыУжеЗаполненыМожноУстановитьДатыЗапретаДляРазделов()
	...
	Возврат НСтр("ru='Разделы уже заполнены,
                       ...
                       |для разделов.'
                       |;uk='Розділи вже заповнені,
                       |можна встановити дати заборони
                       |для розділів.'");
...
Функция ТекстСообщенияОбъектУжеВыбранМожноУстановитьДатуЗапрета()
	...
	Возврат НСтр("ru='Объект уже выбран,
                       ...
                       |;uk='Об''єкт вже вибраний,
                       |можна встановити дату заборони.'");
...
Функция ТекстСообщенияСначалаВыберитеОбъект()
	...
	Возврат НСтр("ru='Сначала выберите объект.';uk='Спочатку виберіть об''єкт.'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРазделаИОбъектов()
	...
	Возврат НСтр("ru='Удалить даты запрета для раздела и объектов?';uk='Видалити дати заборони для розділу і об''єктів?'");
...
Функция ТекстВопросаУдалитьДатыЗапретаДляРаздела()
	...
	Возврат НСтр("ru='Удалить дату запрета для раздела?';uk='Видалити дату заборони для розділу?'");
...
Функция ТекстВопросаУдалитьОбщуюДатуЗапрета()
	...
	Возврат НСтр("ru='Удалить общую дату запрета?';uk='Видалити загальну дату заборони?'");
...
Функция ТекстВопросаУдалитьДатуЗапретаОбъекта()
	...
	Возврат НСтр("ru='Удалить дату запрета для объекта?';uk='Видалити дату заборони для об''єкта?'");
...
Функция ТекстСообщенияДляВсехПользователейРазделыВсегдаПоказываются()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 разделы всегда показываются.';uk='%1 розділи завжди показуються.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстСообщенияПриУдаленииДатаЗапретаОчищается()
	...
	Возврат НСтр("ru='При удалении дата запрета очищается.';uk='При вилученні дата заборони очищається.'");
...
Функция ТекстСообщенияДляВсехПользователейОбщаяДатаВсегдаПоказывается()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 <Общая дата> всегда показывается.';uk='%1 <Загальна дата> завжди показується.'"),
		ЗначениеДляВсехПользователей);
...
Функция ТекстЗаголовкаРаздела()
	...
	Возврат НСтр("ru='Раздел';uk='Розділ'");
...
Функция ТекстЗаголовкаРазделаСОбъектами()
	...
	Возврат НСтр("ru='Раздел, объект';uk='Розділ об''єкт'");
...
Функция ТекстСообщенияНедостаточноПравДляВыбораВнешнихПользователей()
	...
	Возврат НСтр("ru='Недостаточно прав для выбора внешних пользователей.';uk='Недостатньо прав для вибору зовнішніх користувачів.'");
...
Функция ТекстЗаголовкаВыборТипаДанных()
	...
	Возврат НСтр("ru='Выбор типа данных';uk='Вибір типу даних'");
...
Функция ТекстСообщенияНастройкиСНевыбраннымиПользователямиНеСохранены(Контекст)
	...
	Если Контекст.Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Возврат НСтр("ru='Настройки с невыбранными информационными базами не сохранены.';uk='Настройки з невибраними інформаційними базами не збережені.'");
	КонецЕсли;
	
	Возврат НСтр("ru='Настройки с невыбранными пользователями не сохранены.';uk='Настройки з невибранними користувачами не збережені.'");
...
Функция ТекстСообщенияНастройкиСНевыбраннымиОбъектамиНеСохранены()
	...
	Возврат НСтр("ru='Настройки с невыбранными объектами не сохранены.';uk='Настройки з невибраними об''єктами не збережені.'");
...
Функция ТекстСообщенияНастройкиСНезаполненнымиДатамиЗапретаДляОбъектовНеСохранены()
	...
	Возврат НСтр("ru='Настройки с незаполненными датами запрета для объектов не сохранены.';uk='Настройки з незаповненими датами заборони для об''єктів не збережені.'");
...
Функция ТекстСообщенияПользователиБезНастроекДатЗапретаНеСохранены()
	...
	Возврат НСтр("ru='Пользователи без настроек дат запрета не сохранены.';uk='Користувачі без настройок дат заборони не збережені.'");
...
Функция ТекстСообщенияНастройкиСНезаполненнымиДатамиЗапретаДляРазделовНеСохранены()
	...
	Возврат НСтр("ru='Настройки с незаполненными датами запрета для разделов не сохранены.';uk='Настройки з незаповненими датами заборони для розділів не збережені.'");
...
Функция ТекстСообщенияОбщаяДатаУжеПоказана()
	...
	Возврат НСтр("ru='<Общая дата> уже показана.';uk='<Загальна дата> вже показана.'");
...
Функция ТекстСообщенияВсеРазделыУжеПоказаны()
	...
	Возврат НСтр("ru='Все разделы уже показаны.';uk='Всі розділи вже показані.'");
...
Функция ЗаголовокВыборТребуемыхРазделов()
	...
	Возврат НСтр("ru='Выбор требуемых разделов';uk='Вибір необхідних розділів'");
...
Функция ТекстСообщенияСнятоВыделениеСНезаполненныхСтрок()
	...
	Возврат НСтр("ru='Снято выделение с незаполненных строк.';uk='Знято виділення з незаповнених рядків.'");
...
Функция ТекстСообщенияВыделенныеСтрокиНеЗаполнены()
	...
	Возврат НСтр("ru='Выделенные строки не заполнены.';uk='Виділені рядки не заповнені.'");
