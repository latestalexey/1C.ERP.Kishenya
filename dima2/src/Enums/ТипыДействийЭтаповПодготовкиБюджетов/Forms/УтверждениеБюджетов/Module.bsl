...
Процедура ЗаполнитьШагиДляУтвержденияРекурсивно(ТаблицаЗадач, СтрокаШагаПроцесса, ШагиДляУтверждения)
	...
	Для каждого ПредыдущаяЗадача Из СтрокаШагаПроцесса.ПредыдущиеЗадачи Цикл
		...
		Если СтрокаПредыдущейЗадачи.Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.ВводПлана
			Или СтрокаПредыдущейЗадачи.Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.ВводЭкземпляраБюджета Тогда
...
&НаКлиенте
Процедура ИзменениеНастроекКонтрольныхОтчетов(Результат, ДополнительныеПараметры) Экспорт
	
	Если ЗначениеЗаполнено(Результат) Тогда
		ИзменениеНастроекКонтрольныхОтчетовСервер(Результат);
	КонецЕсли;
...
&НаСервере
Процедура ИзменениеНастроекКонтрольныхОтчетовСервер(АдресВоВременномХранилище)
	
	ТаблицаНастройкиКонтрольныхОтчетов = ПолучитьИзВременногоХранилища(АдресВоВременномХранилище);
