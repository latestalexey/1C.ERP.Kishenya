...
Процедура ЗагрузитьПоставляемыеПравила(ИмяПланаОбмена, ИмяФайлаПравил) Экспорт
	...
	Если ОбменДаннымиСервер.РаспаковатьZipФайл(ИмяФайлаПравил, ИмяВременнойПапки) Тогда
		...
		Если СписокРаспакованныхФайлов.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ru='При распаковке архива не найден файл с правилами.';uk='При розпакуванні архіву не знайдений файл із правилами.'");
		КонецЕсли;
		...
		Если СписокРаспакованныхФайлов.Количество() <> 3 Тогда
            ...
            |;uk='Не вірний формат комплекту правил. Очікувана кількість файлів в архіві - три. Очікуються файли:
            |ExchangeRules.xml - правила конвертації для поточної програми;
            |CorrespondentExchangeRules.xml - правила конвертації для програми-кореспондента;
            |RegistrationRules.xml - правила реєстрації для поточної програми.'");
		КонецЕсли;
		...
		Для Каждого ПолученныйФайл Из СписокРаспакованныхФайлов Цикл
			...
			Иначе
                ...
                |CorrespondentExchangeRules.xml - правила конвертации для программы-корреспондента;
                |RegistrationRules.xml - правила регистрации для текущей программы.'
                |;uk='Імена файлів в архіві не відповідають очікуваним. Очікуються файли:
                |ExchangeRules.xml - правила конвертації для поточної програми;
                |CorrespondentExchangeRules.xml - правила конвертації для програми-кореспондента;
                |RegistrationRules.xml - правила реєстрації для поточної програми.'");
			КонецЕсли;
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='Не удалось распаковать архив с правилами.';uk='Не вдалося розпакувати архів із правилами.'");
	КонецЕсли;
	...
	Если ЗагрузкаПравилРегистрации.ФлагОшибки Тогда
		ВызватьИсключение НСтр("ru='Ошибка при загрузке правил регистрации.';uk='Помилка при завантаженні правил реєстрації.'");
	КонецЕсли;
...
Процедура УдалитьПоставляемыеПравила(ИмяПланаОбмена) Экспорт
	...
	Для Каждого ВидПравил Из Перечисления.ВидыПравилДляОбменаДанными Цикл
		...
		Если ЕстьОшибки Тогда
			ВызватьИсключение НСтр("ru='Ошибка при загрузке правил из конфигурации.';uk='Помилка при завантаженні правил конфігурації.'");
		Иначе
...
Процедура ЗагрузитьПравила(Отказ, Запись, АдресВременногоХранилища = "", ИмяФайлаПравил = "", ЭтоАрхив = Ложь) Экспорт
	...
	Если ЭтоАрхив Тогда
		...
		Если ОбменДаннымиСервер.РаспаковатьZipФайл(ИмяВременногоАрхива, ИмяВременнойПапки) Тогда
			...
			// В архиве не было ни одного файла - отказываемся от загрузки.
			Если СписокРаспакованныхФайлов.Количество() = 0 Тогда
				НСтрока = НСтр("ru='При распаковке архива не найден файл с правилами.';uk='При розпакуванні архіву не знайдений файл із правилами.'");
			...
			Если ЭтоПравилаКонвертации Тогда
				...
				ИначеЕсли СписокРаспакованныхФайлов.Количество() = 1 Тогда
					НСтрока = НСтр("ru='В архиве найден один файл правил конвертации. Ожидаемое количество файлов в архиве - два. Ожидаются файлы:
                        |ExchangeRules.xml - правила конвертации для текущей программы;
                        |CorrespondentExchangeRules.xml - правила конвертации для программы-корреспондента.'
                        |;uk='В архіві знайдений один файл правил конвертації. Очікувана кількість файлів в архіві - два. Очікуються файли:
                        |ExchangeRules.xml - правила конвертації для поточної програми;
                        |CorrespondentExchangeRules.xml - правила конвертації для програми-кореспондента.'");
					ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
				// В архиве оказалось несколько файлов, хотя должен быть один - отказываемся от загрузки.
				ИначеЕсли СписокРаспакованныхФайлов.Количество() > 1 Тогда
					НСтрока = НСтр("ru='При распаковке архива найдено несколько файлов. Должен быть только один файл с правилами.';uk='При розпакуванні архіву знайдено кілька файлів. Повинен бути тільки один файл із правилами.'");
					ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
				КонецЕсли;
		...
		Иначе // Если не удалось распаковать файл - отказываемся от загрузки.
			НСтрока = НСтр("ru='Не удалось распаковать архив с правилами.';uk='Не вдалося розпакувати архів із правилами.'");
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
		КонецЕсли;
...
Функция ПолучитьДвоичныеДанныеИзМакетаКонфигурации(Отказ, ИмяПланаОбмена, ИмяМакета)
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ru='Ошибка получения макета конфигурации %1 для плана обмена %2';uk='Помилка одержання макета конфігурації %1 для плану обміну %2'");
...
Процедура ВыполнитьПроверкуЗаполненияПолей(Отказ, Запись)
	...
	Если ПустаяСтрока(Запись.ИмяПланаОбмена) Тогда
		...
		НСтрока = НСтр("ru='Укажите план обмена.';uk='Укажіть план обміну.'");
	...
	ИначеЕсли Запись.ИсточникПравил = Перечисления.ИсточникиПравилДляОбменаДанными.МакетКонфигурации
		...
		НСтрока = НСтр("ru='Укажите типовые правила.';uk='Укажіть типові правила.'");
...
Функция ИнформацияОПравилахИзФайла(ИмяФайлаПравил)
	...
	Если Не ((ПравилаОбмена.ЛокальноеИмя = "ПравилаОбмена") И (ПравилаОбмена.ТипУзла = ТипУзлаXML.НачалоЭлемента)) Тогда
		ВызватьИсключение НСтр("ru='Ошибка формата правил обмена';uk='Помилка формату правил обміну'");
	КонецЕсли;
...
Функция ПравилаКонвертацииСовместимыСТекущейВерсией(ИмяПланаОбмена, ОписаниеОшибки, ДанныеОПравилах)
	...
	Если ИмяКонфигурацииИзПравил <> ИмяКонфигурацииИнформационнойБазы Тогда
		...
		ОписаниеОшибки.Вставить("ТекстОшибки", НСтр("ru='Правила не могут быть загружены, т.к. они предназначены для программы ""%1"". Следует использовать правила из конфигурации или загрузить корректный комплект правил из файла.';uk='Правила не можуть бути завантажені, оскільки вони призначені для програми ""%1"". Слід використовувати правила з конфігурації або завантажити коректний комплект правил з файлу.'"));
	...
	Если РезультатСравнения <> 0 Тогда
		...
		Если РезультатСравнения < 0 Тогда
			...
			ТекстОшибки = НСтр("ru='Синхронизация данных может работать некорректно, так как загружаемые правила предназначены для предыдущей версии программы ""%1"" (%2). Рекомендуется использовать правила из конфигурации или загрузить комплект правил, предназначенный для текущей версии программы (%3).';uk='Синхронізація даних може працювати некоректно, так як завантажувані правила призначені для попередньої версії програми ""%1"" (%2). Рекомендується використовувати правила з конфігурації або завантажити комплект правил, призначений для поточної версії програми (%3).'");
			ВидОшибки = "УстаревшаяВерсияКонфигурации";
			
		Иначе
			
			ТекстОшибки = НСтр("ru='Синхронизация данных может работать некорректно, так как загружаемые правила предназначены для более новой версии программы ""%1"" (%2). Рекомендуется обновить версию программы или использовать комплект правил, предназначенный для текущей версии программы (%3).';uk='Синхронізація даних може працювати некоректно, так як завантажувані правила призначені для більш нової версії програми ""%1"" (%2). Рекомендується оновити версію програми або використовувати комплект правил, призначений для поточної версії програми (%3).'");
...
////////////////////////////////////////////////////////////
//Обработчик перехода с 3.1 на BAS УТ
Процедура ОчиститьПравилаОбмена() Экспорт 

	ПустойНабор = РегистрыСведений.ПравилаДляОбменаДанными.СоздатьНаборЗаписей();
