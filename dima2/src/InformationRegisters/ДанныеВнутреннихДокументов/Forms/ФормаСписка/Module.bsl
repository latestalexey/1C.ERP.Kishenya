...
Процедура СоздатьПередачуМатериаловВПроизводство(Команда)
	...
	//++ НЕ УТ
	СтруктураОснование = Новый Структура("Склад, ХозяйственнаяОперация",
		ОтборСклад, ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СборкаТоваров"));
	СтруктураПараметры = Новый Структура("Основание", СтруктураОснование);
	ОткрытьФорму("Документ.ПередачаМатериаловВПроизводство.ФормаОбъекта", СтруктураПараметры, Элементы.СписокОформлено);
	//-- НЕ УТ
...
Процедура СоздатьВозвратМатериаловИзПроизводства(Команда)
	...
	СтруктураПараметры = Новый Структура("Основание", СтруктураОснование);
	ОткрытьФорму("Документ.ВозвратМатериаловИзПроизводства.ФормаОбъекта", СтруктураПараметры, Элементы.СписокОформлено);
	//-- НЕ УТ
	Возврат; // В УТ11 не используется
...
Процедура СоздатьВыпускПродукции(Команда)
	...
	ЗначенияЗаполнения.Вставить("ВыпускПоРаспоряжениям", Ложь);
	...
	ОткрытьФорму("Документ.ВыпускПродукции.ФормаОбъекта", ПараметрыФормы, Элементы.СписокОформлено);
	//-- НЕ УТ
	Возврат; // В УТ11 не используется
...
Процедура ОповеститьПользователяОПроведенииДокументов(ДокументыДляОбработки, ДанныеОНеОбработанныхДокументах, Проведение)
	...
	ШаблонСообщения = ?(Проведение, 
		НСтр("ru='Документ %1 не проведен: %2';uk='Документ %1 не проведений: %2'"),
...
Процедура ПеречитатьКОформлению(ДокументДляПересчета = Неопределено)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство") Тогда
		КОформлениюЗаказыМатериаловВПроизводство = 
			РегистрыНакопления.ЗаказыМатериаловВПроизводство.РаспоряженияРМВнутреннееТовародвижение(ОтборСклад, РаспоряженияДляПересчета);
	КонецЕсли;
	//-- НЕ УТ
	...
	//++ НЕ УТ
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(КОформлениюЗаказыМатериаловВПроизводство, ТаблицаКОформлению);
	КонецЕсли;
	//-- НЕ УТ
...
Функция ЗаполнитьПоляШапкиДокументов(Таблица)
	...
	|	Документ.Дата
	|ИЗ
	|	Документ.ЗаказМатериаловВПроизводство КАК Документ
	|ГДЕ
	|	Документ.Ссылка В
	|			(ВЫБРАТЬ
	|				ВтТовары.Ссылка
	...
	|	Документ.Дата
	|ИЗ
	|	Документ.ЗаказНаПроизводство КАК Документ
	|ГДЕ
	|	Документ.Ссылка В
	|			(ВЫБРАТЬ
	|				ВтТовары.Ссылка
