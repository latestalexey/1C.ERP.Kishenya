...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	|	УточняемыеДанные.УчетНовыхРабочихМест КАК УчетНовыхРабочихМест
	...
	|	ВЫБОР
	|		КОГДА УточняемыеДанные.Организация = УточняемыеДанные.Организация.ГоловнаяОрганизация
	|			ТОГДА УточняемыеДанные.УчетНовыхРабочихМест
	|		ИНАЧЕ ЕСТЬNULL(НастройкиУчетаСтраховыхВзносов.ПрименяютсяРезультатыСпециальнойОценкиУсловийТруда, ЛОЖЬ)
	|	КОНЕЦ КАК УчетНовыхРабочихМест
	|ИЗ
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
                ...
                |;uk='Не вдалося змінити посаду ""%1"" 
                |Можливо, позиція редагується іншим користувачем'"),
