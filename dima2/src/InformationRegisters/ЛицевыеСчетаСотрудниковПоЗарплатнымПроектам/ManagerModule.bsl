...
Процедура ЗаполнитьПериодРегистрации() Экспорт
	...
	|	Сотрудники.Ссылка КАК Документ
	|ИЗ
	...
	|ГДЕ
	|	ЛицевыеСчета.Документ = ЗНАЧЕНИЕ(Документ.ПодтверждениеОткрытияЛицевыхСчетовСотрудников.ПустаяСсылка)
	...
	Пока Выборка.Следующий() Цикл
		МенеджерЗаписи = РегистрыСведений.ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.СоздатьМенеджерЗаписи();
