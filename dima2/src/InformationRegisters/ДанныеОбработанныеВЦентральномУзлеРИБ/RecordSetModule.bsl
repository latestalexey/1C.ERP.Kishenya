...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Иначе
		Если Количество() > 0
			...
			Отказ = Истина;
			ТекстИсключения = НСтр("ru='Запись в РегистрСведений.ДанныеОбработанныеВЦентральномУзлеРИБ возможна только при отметке выполнения отложенного обработчика обновления информационной базы, который выполняется только в центральном узле.';uk='Запис у РегистрСведений.ДанныеОбработанныеВЦентральномУзлеРИБ можлива тільки при позначці виконання відкладеного обробника оновлення інформаційної бази, яка виконується тільки в центральному вузлі.'");
