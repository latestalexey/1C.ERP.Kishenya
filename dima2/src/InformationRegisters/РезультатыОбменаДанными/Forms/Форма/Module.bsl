...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() И ИспользуетсяВерсионирование Тогда
		...
		Элементы.КоллизииАвторДругойВерсии.Заголовок = НСтр("ru='Версия получена из приложения';uk='Версія отримана з додатка'");
	...
	Если Элемент.ТекущиеДанные <> Неопределено Тогда
		...
		Если Элемент.ТекущиеДанные.ДругаяВерсияПринята Тогда
			...
			ПричинаКонфликта = НСтр("ru='Конфликт был разрешен автоматически в пользу программы ""%1"".
                ...
                |;uk='Конфлікт був вирішений автоматично на користь програми ""%1"".
                |Версія в цій програмі була замінена на версію з іншої програми.'");
		...
		Иначе
			...
			ПричинаКонфликта =НСтр("ru='Конфликт был разрешен автоматически в пользу этой программы.
                ...
                |;uk='Конфлікт був вирішений автоматично на користь цієї програми.
                |Версія в цій програмі була збережена, версія з іншої програми була відхилена.'");
...
Процедура ПринятьВерсиюНепринятые(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Принять версию, несмотря на запрет загрузки?';uk='Прийняти версію, незважаючи на заборону завантаження?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура ИзменитьРезультатКонфликта(Команда)
	...
	Если Элементы.Коллизии.ТекущиеДанные <> Неопределено Тогда
		...
		Если Элементы.Коллизии.ТекущиеДанные.ДругаяВерсияПринята Тогда
			...
			ТекстВопроса = НСтр("ru='Заменить версию, полученную из другой программы, на версию из этой программы?';uk='Замінити версію, отриману з іншої програми, на версію із цієї програми?'");
			
		Иначе
			
			ТекстВопроса = НСтр("ru='Заменить версию этой программы на версию, полученную из другой программы?';uk='Замінити версію цієї програми на версію, отриману з іншої програми?'");
...
Процедура ЗаполнитьСписокУзлов()
	...
	Если КонтекстноеОткрытие И ОтсутствуетОбменПоПравилам Тогда
		Заголовок = НСтр("ru='Конфликты при синхронизации данных';uk='Конфлікти при синхронізації даних'");
...
Процедура ПоказатьОтличия(Элемент)
	...
	Если СравниваемыеВерсии.Количество() <> 2 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Нет версии для сравнения.';uk='Немає версії для порівняння.'"));
		Возврат;
...
Процедура ОбновитьЗаголовкиСтраниц()
	...
	УстановитьЗаголовокСтраницы(Элементы.СтраницаНепроведенныеДокументы, НСтр("ru='Непроведенные документы';uk='Непроведені документи'"), КоличествоНепроведенныхДокументов());
	УстановитьЗаголовокСтраницы(Элементы.СтраницаНезаполненныеРеквизиты, НСтр("ru='Незаполненные реквизиты';uk='Незаповнені реквізити'"), КоличествоНезаполненныхРеквизитов());
	
	Если ИспользуетсяВерсионирование Тогда
		УстановитьЗаголовокСтраницы(Элементы.СтраницаКоллизии, НСтр("ru='Конфликты';uk='Конфлікти'"), КоличествоКоллизий());
		УстановитьЗаголовокСтраницы(Элементы.СтраницаНепринятыеПоДатеЗапрета, НСтр("ru='Непринятые по дате запрета';uk='Неприйняті по даті заборони'"), КоличествоНепринятых());
	КонецЕсли;
