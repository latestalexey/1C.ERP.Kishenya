...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для каждого Период Из Периоды Цикл
		...
		Иначе	
			...
			НоваяСтрокаИстории.КомандаРедактированияРаспределения = НСтр("ru='Не заполнено';uk='Не заповнено'");
		КонецЕсли;
	...
	ОтражениеЗарплатыВФинансовомУчетеУП.УстановитьЗаголовокСпособаОтражения(ЭтаФорма, "ТаблицаИсторииСпособОтраженияЗарплатыВБухучете");
	//ERP конец
...
Функция ПредставлениеРаспределения(ТаблицаРаспределения)
	...
	Если ТаблицаРаспределения.Количество() = 0 Тогда
		Возврат НСтр("ru='Не заполнено';uk='Не заповнено'");
	КонецЕсли;
	...
	Если ЕстьОшибки Тогда
		Возврат НСтр("ru='Заполнено с ошибками';uk='Заповнено з помилками'");
	Иначе
...
Процедура КомандаОК(Команда)
	...
	Если ОшибкиРаспределенияВТаблицеИстрии() Тогда
		Возврат;
	КонецЕсли;
...
&НаСервере
Функция ОшибкиРаспределенияВТаблицеИстрии()
	...
	Для каждого СтрокаТЗ Из ТаблицаИстории Цикл
		...
		Если СтрокаТЗ.Распределяется Тогда
			...
			Если ТаблицаРаспределения.Количество() = 0 Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1: не указано распределение по счетам, субконто на %2.';uk='%1: не вказано розподіл по рахунках, субконто на %2.'"), Синоним, СтрокаТЗ.ПериодСтрокой);
...
Процедура ТаблицаИсторииПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		Если Элемент.ТекущиеДанные <> Неопределено Тогда
			...
			Элемент.ТекущиеДанные.КомандаРедактированияРаспределения = НСтр("ru='Не заполнено';uk='Не заповнено'");
	...
	Если НЕ ОтменаРедактирования Тогда
		Если Элемент.ТекущиеДанные <> Неопределено Тогда
			Если НЕ ЗначениеЗаполнено(Элемент.ТекущиеДанные.Период) Тогда
				СообщениеОбОшибке = НСтр("ru='Необходимо указать месяц, с которого будет действовать настройка бухучета зарплаты';uk='Необхідно вказати місяць, з якого буде діяти налаштування бухобліку зарплати'");
