...
Функция ПодставитьСоединение(ТекстЗапроса, ИмяПараметра, ПоляСоединения = Неопределено) Экспорт
	...
	Пока Истина Цикл
		...
		ПодстрокаЗамены =
			...
			|		 И НастройкаГруппировки.Номенклатура   = Т.Номенклатура
			|		 И НастройкаГруппировки.Характеристика = Т.Характеристика
			|		 И НастройкаГруппировки.Склад          = Т.Склад
			|		 И НастройкаГруппировки.Группировка <> ЗНАЧЕНИЕ(Справочник.ГруппировкиТоварныхОграничений.ПустаяСсылка)
			|		
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТоварныеОграничения КАК ТаблицаНастроек
			|		ПО ВЫБОР
			|				КОГДА НЕ НастройкаГруппировки.Склад ЕСТЬ NULL ТОГДА
			|					
			|					ТаблицаНастроек.Номенклатура     = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
			|					И ТаблицаНастроек.Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
			|					И ТаблицаНастроек.Склад          = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
			|					И ТаблицаНастроек.Группировка    = НастройкаГруппировки.Группировка
			|					
			|				КОГДА НЕ НастройкаДетальная.Склад ЕСТЬ NULL ТОГДА
			|					
			|					ТаблицаНастроек.Номенклатура     = Т.Номенклатура
			|					И ТаблицаНастроек.Характеристика = Т.Характеристика
			|					И ТаблицаНастроек.Склад          = Т.Склад
			|					И ТаблицаНастроек.Группировка    = ЗНАЧЕНИЕ(Справочник.ГруппировкиТоварныхОграничений.ПустаяСсылка)
			|					
			|				ИНАЧЕ
			|					
			|					ТаблицаНастроек.Номенклатура     = Т.Номенклатура
