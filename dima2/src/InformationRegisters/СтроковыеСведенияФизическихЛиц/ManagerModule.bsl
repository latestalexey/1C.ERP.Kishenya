...
Функция ПредставлениеНаграды(Награда)
	ПредставлениеНаграды = НСтр("ru='%1, %2 №%3 от %4';uk='%1, %2 №%3 від %4'");
...
Функция ДанныеОбОбучении(ФизическоеЛицо)
	...
	|			ИЛИ ОбразованиеФизическихЛиц.ВидДополнительногоОбучения = ЗНАЧЕНИЕ(Перечисление.ВидыПрофессиональнойПодготовки.ПовышениеКвалификации)
	|			ИЛИ ОбразованиеФизическихЛиц.ВидДополнительногоОбучения = ЗНАЧЕНИЕ(Перечисление.ВидыПрофессиональнойПодготовки.Переподготовка))";
	Запрос.УстановитьПараметр("ФизическоеЛицо", ФизическоеЛицо);
	Результат = Запрос.Выполнить();
	Если Не Результат.Пустой() Тогда
...
Функция ПредставлениеПериодаОбучения(Результат, УчебноеЗаведение, ДатаНачала, ДатаОкончания)
	...
	ИначеЕсли ЗначениеЗаполнено(ДатаНачала) Тогда
		ПредставлениеПериодаОбучения = ПредставлениеПериодаОбучения + НСтр("ru='(с %3)';uk='(з %3)'");
	ИначеЕсли ЗначениеЗаполнено(ДатаОкончания) Тогда
		ПредставлениеПериодаОбучения = ПредставлениеПериодаОбучения + НСтр("ru='(по %4)';uk='(по %4)'");
	КонецЕсли;
...
Процедура ЗаписатьОшибкуОбновленияСтроковыхСведений(ФизическоеЛицо)
	...
	ТекстСообщения = НСтр("ru='Не удалось обработать %1 по причине: %2';uk='Не вдалося обробити %1 з причини: %2'");
