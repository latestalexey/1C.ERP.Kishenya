...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Для каждого СтрокаСоСтажем Из ЭтотОбъект Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаСоСтажем.ДатаОтсчета) Тогда
			СтрокаСоСтажем.ДатаОтсчета = ТекущаяДатаСеанса();
		КонецЕсли; 
	...
	Если ОтборТолькоПоФизическомуЛицу Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаВидовСтажа Цикл
			...
			Если СтрокиСтажа.Количество() > 1 Тогда
				...
				НСтр("ru='Вид стажа с категорией ""%1"" уже существует.';uk='Вид стажу з категорією ""%1 вже існує.'"),
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка.КатегорияСтажа = Перечисления.КатегорииСтажа.Прочее Тогда
				Продолжить;
			КонецЕсли;
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вид стажа с категорией ""%1"" уже существует.';uk='Вид стажу з категорією ""%1 вже існує.'"),	Выборка.КатегорияСтажа);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,,,Отказ);
		КонецЦикла;
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока ВыборкаСтажей.Следующий() Цикл
			Если ЗначениеЗаполнено(ВыборкаСтажей.ДнейРасширенногоСтажа) И ВыборкаСтажей.ДнейСтажа > ВыборкаСтажей.ДнейРасширенногоСтажа Тогда
				ТекстОшибки = НСтр("ru='%1: Расширенный страховой стаж не может быть меньше обычного страхового';uk='%1: Розширений страховий стаж не може бути менше звичайного страхового'");
