...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураДанныхОтчетОДебиторскойИКредиторскойЗадолженности	= Новый Структура;
	...
	ИмяРаздела = "ОтчетОДебиторскойИКредиторскойЗадолженности";
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.ИмяСтраницы              = "ОтчетОДебиторскойИКредиторскойЗадолженности";
	СтрокаУровня1.Представление            = НСтр("ru='Регламентированный отчет о взаиморасчетах с нерезидентами';uk='Регламентований звіт про взаєморозрахунки з нерезидентами'");
...
Процедура СохранитьДанныеРаздела(НаимРаздела, ОчищатьРазделЕслиЕстьМногострочность)
	...
	//
	ИначеЕсли НаимРаздела = "ОтчетОДебиторскойИКредиторскойЗадолженности" Тогда
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	// Многострочные и обычные разделы.
	//
	ИначеЕсли ИмяТекРаздела = "ОтчетОДебиторскойИКредиторскойЗадолженности" Тогда
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("ОргНазв", ОргНазв, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");

	
	Если Месяц(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
		ПериодОтчета = "станом на Рік " +Формат(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ=yyyy");
	Иначе	
		НомерКварталаРимскими = "";
		Для сч = 1 по Месяц(КонецКвартала(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета))/3 Цикл
			НомерКварталаРимскими = НомерКварталаРимскими + "I";
		КонецЦикла;
		// Период в заголовке отчета 
		ПериодОтчета = "станом на " + НомерКварталаРимскими + " квартал " + Формат(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ=yyyy") + " року";
	КонецЕсли;
	ЗаполнитьПоказательНаСервере("ПериодОтчета", ПериодОтчета, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
    ...
    ЗаполнитьПоказательНаСервере("ОргЮрАдрес",  ОргЮрАдрес, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
    ЗаполнитьПоказательНаСервере("ОргАдрес", ОргАдрес, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
	ЗаполнитьПоказательНаСервере("Телефон", Телефон, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
	ЗаполнитьПоказательНаСервере("FAX", Факс, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
	ЗаполнитьПоказательНаСервере("EMAIL", EMAIL, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
	
	КолСимволов = Мин(СтрДлина(Строка(ЕДРПОУ)),8);
	
	Для НомЯчейки = 1 По КолСимволов Цикл
		ЗаполнитьПоказательНаСервере("ЕДРПОУ_" + НомЯчейки, Сред(Строка(ЕДРПОУ), НомЯчейки, 1), Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
		ЗаполнитьПоказательНаСервере("ЕДРПОУ1_" + НомЯчейки, Сред(Строка(ЕДРПОУ), НомЯчейки, 1), Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
	КонецЦикла;
	
	ЗаполнитьПоказательНаСервере("Директор", ФИОДиректора, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");									
	...
	ЗаполнитьПоказательНаСервере("ИСПОЛНИТЕЛЬ",  ФИОИсполнителя, 	Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
...
Процедура ВосстановитьСохраненныеДанные()
	...
	КонецЦикла;
	ИмяТекРаздела = "ОтчетОДебиторскойИКредиторскойЗадолженности";
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если    ИмяРаздела = "ОтчетОДебиторскойИКредиторскойЗадолженности" Тогда
...
Процедура ОчиститьНаСервере() Экспорт
	...
	СтруктураДанныхОтчетОДебиторскойИКредиторскойЗадолженности.Очистить();
...
Процедура ВводЧислаВСкобкахЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьПоказательНаСервере(ИмяЯчейки, РезультатВыбора, ЭтаФорма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
...
Процедура УстановитьЗначениеПоказателя(Имя, Значение)
	...
	СтруктураДанныхОтчетОДебиторскойИКредиторскойЗадолженности[Имя] = Значение;
...
Процедура _ДобавитьСтрокуРасшифровки(Форма, ИмяЯчейки, ИмяПоказателя, ЗнакОперации, ИмяСлагаемого, Сумма, Счет = Неопределено, КорСчет = Неопределено, Сдвиг = Неопределено, ПраваяГраница = Неопределено, ИмяРаздела = Неопределено, НомерЛиста = Неопределено, МнЧ = Неопределено, НомерСтроки = Неопределено, Субконто1 = Неопределено, Субконто2 = Неопределено, Субконто3 = Неопределено)
	...
	ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяЯчейки, ИмяПоказателя, ЗнакОперации, ИмяСлагаемого + " " + Счет.Код, Сумма, "ОтчетОДебиторскойИКредиторскойЗадолженности", ДополнительныеПараметры);
