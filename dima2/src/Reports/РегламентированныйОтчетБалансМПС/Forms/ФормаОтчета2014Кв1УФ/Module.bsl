...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
   ...
   КонецПопытки;	
	...
	// строка 031 основные средства:начальная стоимость + биологические активі + незавершенное строительство
	...
	//строка 210 Поточна дебиторская задолженность
	П11553 = СНД(Счет34);
	...
	П11553 = П11553 + СНД(Счет46) + СНД(Счет45); 
	_ДобавитьСтрокуРасшифровки( ЭтаФорма, "П11553", "",	"+", "!СНД",СНД(Счет46), Счет46,,НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета), СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	...
	П11554 = П11554 + СКД(Счет46) + СКД(Счет45); 
	_ДобавитьСтрокуРасшифровки( ЭтаФорма, "П11554", "",	"+", "!СКД",СКД(Счет46), Счет46);
	...
	// это проводки на 79 в обход счето затрат, в конфигурациях 8 - нет таких проводок. а в 77 бывают.
