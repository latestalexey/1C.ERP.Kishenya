...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2007УФ";
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/10/2009";
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = НСтр("ru='Отчет о трудоустройстве инвалидов';uk= 'Звіт з працевлаштування інвалідів'");
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	// Многострочные и обычные разделы.
	//
	ИначеЕсли ИмяТекРаздела = "Уведомление" Тогда
...
Процедура ЗаполнитьДатуПодписи(Форма)
	...
	Если ЧастиДатыПодписи.Свойство("ДвеПоследниеЦифрыГода") Тогда
		ЗаполнитьПоказательНаСервере("ГодДаты",ЧастиДатыПодписи.ДвеПоследниеЦифрыГода,Форма,"Уведомление");
	Иначе
		ЗаполнитьПоказательНаСервере("ГодДаты",Формат(Форма.ДатаПодписи, "ДФ=yy"),Форма,"Уведомление");
	Конецесли;
	
	ЗаполнитьПоказательНаСервере("PERIOD",Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета,Форма,"Уведомление");
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ФИОБухгалтера     = СведенияДатаПодписи.ФИОБух;
	...
	ЗаполнитьПоказательНаСервере("КВЕД",  КодКВЕД, Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("КФВ",   КодКФВ, Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("КОПФГ", КодОПФГ, Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("КОДУ",  КодСПОДУ, Форма, "Уведомление");
	
	ЗаполнитьПоказательНаСервере("ADRESS", ЮридическийАдрес, Форма, "Уведомление");
	
	ЗаполнитьПоказательНаСервере("РУКОВОДИТЕЛЬ",   ФИОДиректора, 	Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("РУКОВОДИТЕЛЬ1",  ФИОДиректора, 	Форма, "Уведомление");
	
	ЗаполнитьПоказательНаСервере("ГЛАВБУХ",   ФИОБухгалтера, 	Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("ГЛАВБУХ1",  ФИОБухгалтера, 	Форма, "Уведомление");
	...
	КонецЕсли;
	ЗаполнитьПоказательНаСервере("РСЧЕТ", 	ДопСведенияОбОрганизации.НомерСчета, Форма, "Уведомление");	
	...
	ЗаполнитьПоказательНаСервере("Телефоны", Телефон, Форма, "Уведомление");	
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	R_01 = ПолучитьПоказательНаСервере("R_01", Форма, "Уведомление");
	
	Если R_01 >= 8 И R_01 <= 25 Тогда
		R_03 = 1 ;
	ИначеЕсли R_01 > 25 Тогда 
		R_03 = окр(R_01 *0.04,0,1);
	Иначе
	    R_03  = 0;
	КонецЕсли;	
	
	ЗаполнитьПоказательНаСервере("R_03",  R_03, Форма, "Уведомление");
	
	R_04 = ПолучитьПоказательНаСервере("R_04", Форма, "Уведомление");
	
	R_05 =?(R_01 = 0, 0, R_04 /R_01);
	ЗаполнитьПоказательНаСервере("R_05",  R_05, Форма, "Уведомление");
	
	R_02 = ПолучитьПоказательНаСервере("R_02", Форма, "Уведомление");
	R_06  = 0;
	Если  R_03 - R_02 > 0 Тогда  
			Если R_01 > 8 И R_01 <= 25 Тогда
				R_06  = 1 * R_05 / 2 ;
		ИначеЕсли R_01 >  25 Тогда 
			R_06 = (R_03 - R_02) * R_05;
		КонецЕсли;
	КонецЕсли;
	ЗаполнитьПоказательНаСервере("R_06",  R_06, Форма, "Уведомление");
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	ПараметрыСредняяЧисленность.Вставить("КонецПериода", КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
	...
	R_01 = ТаблЧисленностиССЧ[0].Средняя; 
	
	СтруктураДанныхУведомление.Вставить("R_01",  R_01);
	...
	R_02 = ТаблЧисленностиССЧ[0].Средняя; 
	
	СтруктураДанныхУведомление.Вставить("R_02",  R_02);
	...
	|ВЫБРАТЬ
	|  СУММА(СуммаВзаиморасчетовПриход) КАК РезультатШтат
	|ИЗ
	...
	|	ПО ТекКадровыеДанные.Сотрудник = ВзаиморасчетыСРаботниками.Сотрудник
	|	 И ТекКадровыеДанные.ТекущийВидЗанятости = Значение(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы)
	|ГДЕ ПериодВзаиморасчетов МЕЖДУ &НачДата И &КонДата
	|";
	
	ЗапросОбшийФОТ.УстановитьПараметр("НачДата", НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета));
	...
	ЗапросОбшийФОТ.УстановитьПараметр("ДатаНачалаНачислений", ДатаНачалаНачисленийТекПериода(НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета), СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, СтруктураРеквизитовФормы.Организация, "ВзаиморасчетыССотрудниками"));
	...
	СтруктураДанныхУведомление.Вставить("R_04",  R_04);
...
Функция ПолучитьДанныеФормыНаСервере();
	...
	СтруктураРеквизитовФормы.ДопПоказателиОтчетаЗвит1С.Вставить("мДатаНачалаПериодаОтчета", СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
