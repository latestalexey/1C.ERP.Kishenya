...
Процедура СформироватьОтчет(ТабличныйДокумент, Список) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ОблЗаголовокРаздела.Параметры.Заголовок = НСтр("ru='Основные сведения';uk='Основні відомості'");
		...
		ОблЗаголовокРаздела.Параметры.Заголовок = НСтр("ru='Амортизация';uk='Амортизація'");
		ТабличныйДокумент.Вывести(ОблЗаголовокРаздела);
...
Функция ТекстЗапроса()
	...
	|	ЕСТЬNULL(СостоянияТекущие.Состояние, ЗНАЧЕНИЕ(Перечисление.СостоянияОС.НеПринятоКУчету)) КАК Состояние,
	|	ЕСТЬNULL(СостоянияСписания.Период, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаДоПринятияКУчету,
	|	СостоянияТекущие.ИнвентарныйНомер
	|ПОМЕСТИТЬ СостоянияТекущие
	|ИЗ
	|	Справочник.ОбъектыЭксплуатации КАК Объекты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОсновныеСредстваМеждународныйУчет.СрезПоследних(, ОсновноеСредство В (&Объекты)) КАК СостоянияТекущие
	...
	|	ДвиженияПринятияКУчету.ОсновноеСредство КАК ОсновноеСредство
	...
	|		ПО СостоянияТекущие.Объект = ДвиженияПринятияКУчету.ОсновноеСредство
	...
	|	Объекты.ГруппаОСМеждународныйУчет КАК Группа,
	...
	|		ПО Объекты.Ссылка = ПериодыСостояний.ОсновноеСредство
	|ГДЕ
	|	Объекты.Ссылка В(&Объекты)
	|
	|УПОРЯДОЧИТЬ ПО
