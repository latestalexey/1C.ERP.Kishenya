...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	ПозицияКорсчета = СтрНайти(ИсходныйТекстЗапроса, ", " + ЗавершающаяЧастьЗапроса);
		...
		Для Каждого Ячейка Из СтрокаМакета.Ячейки Цикл
			Если Ячейка.Элементы.Количество() = 1 Тогда
				Если ТипЗнч(Ячейка.Элементы[0].Значение) = Тип("Строка") 
					И ВРег(Строка(Ячейка.Элементы[0].Значение)) = "КОР. СЧЕТ" Тогда // Не локализуется.
...
Процедура ДобавитьУсловиеПоВыбраннымРесурсам(ПараметрыОтчета, Схема)
	...
	Если ТекстУсловияДт <> "" Тогда
		...
		+ " ИЛИ " + Лев(ТекстУсловияКт, СтрДлина(ТекстУсловияКт) - 1) + " <> 0" + " ТОГДА ИСТИНА ИНАЧЕ ИСТИНА КОНЕЦ"; // Не локализуется.
