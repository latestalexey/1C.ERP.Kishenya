...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.ОпцияКадровыйПереводДвумяСтроками = ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить("РегламентированнаяОтчетность", "РегламентированнаяОтчетность_ЕСВ_ОпцияКадровыйПереводДвумяСтроками", Ложь);	
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить("РегламентированнаяОтчетность", "РегламентированнаяОтчетность_ЕСВ_ОпцияКадровыйПереводДвумяСтроками", 	  ЭтаФорма.ОпцияКадровыйПереводДвумяСтроками);	
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов)
	...
	ИначеЕсли ИмяСтраницы = "Таблица5" Тогда	
		...
		Макет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("Таблица5_2018");
		...
		Для каждого СтрокаГруппы Из T5T1R Цикл
			...
			Если ЗначениеЗаполнено(СтрокаГруппы.T5T1RXXXXG18D) Тогда
				Строка.Параметры.ДатаСозданияРМ = СтрокаГруппы.T5T1RXXXXG18D;
			Иначе			
				Строка.Параметры.ДатаСозданияРМ = "";
			КонецЕсли;

			Строка.Параметры.Профессия 			= СтрокаГруппы.T5T1RXXXXG12S;
			Строка.Параметры.КодЗКППТР 			= СтрокаГруппы.T5T1RXXXXG13;
			Строка.Параметры.КодКП 	   			= СтрокаГруппы.T5T1RXXXXG14S;
			Строка.Параметры.Должность 			= СтрокаГруппы.T5T1RXXXXG15S;
			Строка.Параметры.ДокументОснование 	= СтрокаГруппы.T5T1RXXXXG16S;
			Строка.Параметры.Основание		 	= СтрокаГруппы.T5T1RXXXXG17S;
			Строка.Параметры.ВоенноеЗвание	 	= СтрокаГруппы.T5T1RXXXXG19S;
		...
		Парам1.Вставить("АвтоМасштаб", Ложь);
		Парам1.Вставить("МасштабПечати", 90);
	...
	ИначеЕсли ИмяСтраницы = "Таблица6" Тогда
		...
		Макет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("Таблица6_2018");
	...
	ИначеЕсли ИмяСтраницы = "Таблица7" Тогда
		...
		Макет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("Таблица7_2018");
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" Тогда
		...
		Если Метаданные.Справочники.ОснованияУвольнения.Реквизиты.Найти("СтатьяЗакона") = Неопределено Тогда
			...
			"ДатаПриема, ДатаУвольнения, ФизическоеЛицо, ВидЗанятости, ПринятНаНовоеРабочееМесто, ДатаСозданияРабочегоМеста, Должность, ПриказОбУвольненииНомер, ПриказОбУвольненииДата, ПриказОбУвольненииСтатьяКЗоТ, ДоляНеполногоРабочегоВремени",
	...
	Иначе
		...
		Если Метаданные.Справочники.ОснованияУвольнения.Реквизиты.Найти("СтатьяЗакона") = Неопределено Тогда
			ТаблицаКадровыеДанныеСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(
				Истина, 
				ТаблицаСотрудниковОрганизации.ВыгрузитьКолонку("Сотрудник"),                                                     
				"ДатаПриема, ДатаУвольнения, ФизическоеЛицо, ВидЗанятости, ПринятНаНовоеРабочееМесто, ДатаСозданияРабочегоМеста, Должность, ПриказОбУвольненииНомер, ПриказОбУвольненииДата, ПриказОбУвольненииСтатьяКЗоТ",
				КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
		Иначе	
	...
	Запрос.УстановитьПараметр("ДатаНачСлед", ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета,1));
	...
	Если Метаданные.Справочники.ОснованияУвольнения.Реквизиты.Найти("СтатьяЗакона") = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "СотрудникиОрганизации.ПриказОбУвольненииСтатьяЗакона КАК СтатьяЗакона,", "СотрудникиОрганизации.ПриказОбУвольненииСтатьяКЗоТ КАК СтатьяЗакона,");	
	КонецЕсли;	
	...
	ТаблицаКадровыеДанныеСотрудниковДляУЧ = КадровыйУчет.КадровыеДанныеСотрудников(
			...
			КонецДня(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета));
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" Тогда
		...
		|	ВсеДоговораГПХ.СтатьяЗакона КАК СтатьяЗакона,
		|	ВсеДоговораГПХ.ОснованиеДата КАК ОснованиеДата,
		|	ВсеДоговораГПХ.ОснованиеНомер КАК ОснованиеНомер
		...
		Если Метаданные.Документы.ДоговорРаботыУслуги.Реквизиты.Найти("ОснованиеПрекращения") = Неопределено Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПериодыДействияДоговоровГражданскоПравовогоХарактера.ДокументОснование.ОснованиеПрекращения", """""");	
		КонецЕсли;
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Иначе
				ПериодМинимум = НачалоМесяца(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
			КонецЕсли;
			Если ЗначениеЗаполнено(Выборка.ПериодМаксимум) Тогда
				ПериодМаксимум = КонецМесяца(Выборка.ПериодМаксимум);
			Иначе
				ПериодМаксимум = КонецМесяца(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
			КонецЕсли;	
		Иначе
			ПериодМинимум = НачалоМесяца(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
			ПериодМаксимум = КонецМесяца(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
...
//		|	КОНЕЦ 
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" Тогда
		...
		|	NULL КАК СтатьяЗакона,
		|	КадроваяИсторияСотрудников.Должность КАК Должность,
		|	КадроваяИсторияСотрудников.ПредыдущаяДолжность КАК ПредыдущаяДолжность,
		...
		|	КадроваяИсторияСотрудников.Регистратор.Дата КАК ОснованиеДата,
		|	КадроваяИсторияСотрудников.Регистратор.Номер КАК ОснованиеНомер,
		|	ВЫБОР
		|		КОГДА КадроваяИсторияСотрудников.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы)
		|			ТОГДА 1
		|		ИНАЧЕ 2
		|	КОНЕЦ КАК Категория
		|ПОМЕСТИТЬ ВТКадровыеДвижения
		|ИЗ
		|	РегистрСведений.КадроваяИсторияСотрудников КАК КадроваяИсторияСотрудников
		|   ВНУТРЕННЕЕ СОЕДИНЕНИЕ
		|   ВТСотрудникиОрганизации КАК СотрудникиОрганизации
		|	ПО КадроваяИсторияСотрудников.Сотрудник = СотрудникиОрганизации.Сотрудник
		|ГДЕ КадроваяИсторияСотрудников.Период МЕЖДУ &ДатаНач И КОНЕЦПЕРИОДА(&ДатаНачСлед, ДЕНЬ)
		|	И ((КадроваяИсторияСотрудников.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Прием) И КадроваяИсторияСотрудников.Период <= &ДатаКон)
		...
		|	И КадроваяИсторияСотрудников.ВидЗанятости <> ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ВнутреннееСовместительство)		
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	СотрудникиОрганизации.СтатьяЗакона КАК СтатьяЗакона,
		|	СотрудникиОрганизации.Должность КАК Должность,
		|	NULL КАК ПредыдущаяДолжность,
		|	ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Увольнение) КАК ВидСобытия,
		|	NULL КАК ДатаСозданияРабочегоМеста,
		|	СотрудникиОрганизации.ПриказОбУвольненииДата КАК ОснованиеДата,
		|	СотрудникиОрганизации.ПриказОбУвольненииНомер КАК ОснованиеНомер,
		|	ВЫБОР
		|		КОГДА СотрудникиОрганизации.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы)
		|			ТОГДА 1
		|		ИНАЧЕ 2
		...
		|	ПО СотрудникиОрганизации.Сотрудник = ДоговораГПХ.Сотрудник
		|ГДЕ СотрудникиОрганизации.ДатаУвольнения МЕЖДУ &ДатаНач И &ДатаКон
		|	И СотрудникиОрганизации.ВидЗанятости <> ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ВнутреннееСовместительство)		
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ДоговораГПХ.Сотрудник.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ДоговораГПХ.Сотрудник КАК Сотрудник,
		|	ДоговораГПХ.ДатаНачала КАК ДатаПриема,
		|	ДоговораГПХ.ДатаОкончания КАК ДатаУвольнения,
		|	ДоговораГПХ.СтатьяЗакона КАК СтатьяЗакона,
		|	NULL КАК Должность,
		|	NULL КАК ПредыдущаяДолжность,
		|	ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.ПустаяСсылка) КАК ВидСобытия,
		|	NULL КАК ДатаСозданияРабочегоМеста,
		|	ДоговораГПХ.ОснованиеДата КАК ОснованиеДата,
		|	ДоговораГПХ.ОснованиеНомер КАК ОснованиеНомер,
		|	3 КАК Категория
		|ИЗ
		|   ВТДоговораГПХ КАК ДоговораГПХ
		|
		|";
		Если Метаданные.РегистрыСведений.КадроваяИсторияСотрудников.Реквизиты.Найти("ИзменениеДолжности") = Неопределено Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "КадроваяИсторияСотрудников.ИзменениеДолжности", "ИСТИНА");	
		КонецЕсли;
		Запрос.Выполнить();
	...
	Иначе
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
			...
			|		КОГДА КадроваяИсторияСотрудников.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы)
			|			ТОГДА 1
			|		КОГДА НЕ (ДоговораГПХ.Сотрудник ЕСТЬ NULL) 
			|          	ТОГДА 3
			|		ИНАЧЕ 2
			|	КОНЕЦ КАК Категория
			|ПОМЕСТИТЬ ВТКадровыеДвижения
			...
			|ГДЕ КадроваяИсторияСотрудников.Период МЕЖДУ &ДатаНач И &ДатаНачСлед
			|	И ((КадроваяИсторияСотрудников.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Прием) И КадроваяИсторияСотрудников.Период <= &ДатаКон)
			|      ИЛИ (КадроваяИсторияСотрудников.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Перемещение) И КадроваяИсторияСотрудников.ИзменениеДолжности))
			|	И КадроваяИсторияСотрудников.ВидЗанятости <> ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ВнутреннееСовместительство)		
			|
			...
			Если Метаданные.РегистрыСведений.КадроваяИсторияСотрудников.Реквизиты.Найти("ИзменениеДолжности") = Неопределено Тогда
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "КадроваяИсторияСотрудников.ИзменениеДолжности", "ИСТИНА");	
			КонецЕсли;
			Если Метаданные.РегистрыСведений.КадроваяИсторияСотрудников.Реквизиты.Найти("ПредыдущаяДолжность") = Неопределено Тогда
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "КадроваяИсторияСотрудников.ПредыдущаяДолжность", "NULL");	
			КонецЕсли;
		...
		Иначе
			...
			|   ВТДоговораГПХ КАК ДоговораГПХ
			|	ПО СотрудникиОрганизации.Сотрудник = ДоговораГПХ.Сотрудник
			|ГДЕ СотрудникиОрганизации.ДатаПриема МЕЖДУ &ДатаНач И &ДатаКон
			|	И СотрудникиОрганизации.ВидЗанятости <> ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ВнутреннееСовместительство)		
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПринятыеУволенныеВТекущемПериоде.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ВТФизическиеЛица.ФизическоеЛицо.КодПоДРФО КАК КодПоДРФО,
	|	ПринятыеУволенныеВТекущемПериоде.ДатаПриема КАК ДатаНачала,
	|	ПринятыеУволенныеВТекущемПериоде.ДатаУвольнения КАК ДатаОкончания,
	|	ПринятыеУволенныеВТекущемПериоде.ВидСобытия КАК ВидСобытия,
	...
	|	Должности.КодЗКППТР КАК ДолжностьКодЗКППТР,
	...
	|	ДолжностиПредыдущий.КодЗКППТР КАК ДолжностьКодЗКППТРПредыдущий,
	|	ПринятыеУволенныеВТекущемПериоде.СтатьяЗакона КАК СтатьяЗакона,
	|	ПринятыеУволенныеВТекущемПериоде.ОснованиеДата КАК ОснованиеДата,
	|	ПринятыеУволенныеВТекущемПериоде.ОснованиеНомер КАК ОснованиеНомер,
	...
	|	ЕСТЬNULL(ГПХПоОсновномуМР.Категория, 0) КАК ГПХПоОснМестуРаботы,
	|	ПринятыеУволенныеВТекущемПериоде.Категория КАК Категория,
	|   ПринятыеУволенныеВТекущемПериоде.ДатаСозданияРабочегоМеста КАК ДатаСозданияРабочегоМеста
	|ИЗ
	|	ВТКадровыеДвижения КАК ПринятыеУволенныеВТекущемПериоде
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТФизическиеЛица КАК ВТФизическиеЛица
	|		ПО ПринятыеУволенныеВТекущемПериоде.ФизическоеЛицо = ВТФизическиеЛица.ФизическоеЛицо
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТГПХПоОсновномуМР КАК ГПХПоОсновномуМР
	|		ПО ПринятыеУволенныеВТекущемПериоде.ФизическоеЛицо = ГПХПоОсновномуМР.ФизическоеЛицо
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Должности КАК Должности
	|		ПО ПринятыеУволенныеВТекущемПериоде.Должность = Должности.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Должности КАК ДолжностиПредыдущий
	|		ПО ПринятыеУволенныеВТекущемПериоде.ПредыдущаяДолжность = ДолжностиПредыдущий.Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	"""" КАК Должность,
	|	"""" КАК ДолжностьКодКП,
	|	"""" КАК ДолжностьНаименованиеПоКП,
	|	"""" КАК ДолжностьКодЗКППТР,
	|	"""" КАК ДолжностьПредыдущий,
	|	"""" КАК ДолжностьКодКППредыдущий,
	|	"""" КАК ДолжностьНаименованиеПоКППредыдущий,
	|	"""" КАК ДолжностьКодЗКППТРПредыдущий,
	|	"""" КАК СтатьяЗакона,
	|	СоциальныеОтпуска.ОснованиеДата КАК ОснованиеДата,
	|	СоциальныеОтпуска.ОснованиеНомер КАК ОснованиеНомер,
	|	ВТФизическиеЛица.Гражданин,
	|	ВТФизическиеЛица.Фамилия КАК Фамилия,
	|	ВТФизическиеЛица.Имя КАК Имя,
	|	ВТФизическиеЛица.Отчество КАК Отчество,
	|	0 КАК ГПХПоОснМестуРаботы,
	|	СоциальныеОтпуска.Категория КАК Категория,
	|   NULL КАК ДатаСозданияРабочегоМеста
	|ИЗ
	...
	Если Метаданные.Справочники.Должности.Реквизиты.Найти("НаименованиеПоКП") = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Должности.НаименованиеПоКП", """""");	
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Должности.КодЗКППТР", """""");	
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЭтаФорма.ОпцияКадровыйПереводДвумяСтроками И Выборка.ВидСобытия = Перечисления.ВидыКадровыхСобытий.Перемещение И Выборка.Категория <> 3 И (НачалоДня(Выборка.ДатаНачала)-1)>= СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета Тогда
			...
			Строка["T5T1RXXXXG7"]  		= Выборка.ГПХПоОснМестуРаботы;
			Строка["T5T1RXXXXG8"]  		= Выборка.Категория;
			...
			//Строка["T5T1RXXXXG111"]
			Строка["T5T1RXXXXG112"]  	= ДЕНЬ(НачалоДня(Выборка.ДатаНачала)-1);
			Строка["T5T1RXXXXG12S"]  	= СокрЛП(Выборка.ДолжностьНаименованиеПоКППредыдущий);
			Строка["T5T1RXXXXG13"]  	= СокрЛП(Выборка.ДолжностьКодЗКППТРПредыдущий);
			Строка["T5T1RXXXXG14S"]  	= СокрЛП(Выборка.ДолжностьКодКППредыдущий);
			Строка["T5T1RXXXXG15S"]  	= СокрЛП(Выборка.ДолжностьПредыдущий);
			Строка["T5T1RXXXXG16S"]  	= "№ " + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Выборка.ОснованиеНомер, Истина, Истина) + " від " + Формат(Выборка.ОснованиеДата, "Л=uk; ДФ='dd.MM.yyyy р.'"); 
			//Строка["T5T1RXXXXG17S"]
			//Строка["T5T1RXXXXG18D"]
			
			Номер = Номер + 1;
		КонецЕсли;	

		Если Выборка.ВидСобытия = Перечисления.ВидыКадровыхСобытий.Перемещение И Месяц(Выборка.ДатаНачала) <> Месяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) Тогда
		...
		Если ЗначениеЗаполнено(Выборка.ДатаНачала) Тогда
			Строка["T5T1RXXXXG111"]  	= ДЕНЬ(Выборка.ДатаНачала);
		КонецЕсли;	
		Если ЗначениеЗаполнено(Выборка.ДатаОкончания) Тогда
			Строка["T5T1RXXXXG112"]  	= ДЕНЬ(Выборка.ДатаОкончания);
		КонецЕсли;
		Если Выборка.Категория <> 3 Тогда
			Строка["T5T1RXXXXG12S"]  	= СокрЛП(Выборка.ДолжностьНаименованиеПоКП);
			Строка["T5T1RXXXXG13"]  	= СокрЛП(Выборка.ДолжностьКодЗКППТР);
			Строка["T5T1RXXXXG14S"]  	= СокрЛП(Выборка.ДолжностьКодКП);
			Строка["T5T1RXXXXG15S"]  	= СокрЛП(Выборка.Должность);
		КонецЕсли;	
		Если Выборка.Категория = 4 ИЛИ Выборка.Категория = 5 ИЛИ Выборка.Категория = 6 Тогда
			Если ЗначениеЗаполнено(Выборка.ДатаНачала) И ЗначениеЗаполнено(Выборка.ОснованиеНомер) И ЗначениеЗаполнено(Выборка.ОснованиеДата) Тогда
				Строка["T5T1RXXXXG16S"]  	= "№ " + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Выборка.ОснованиеНомер, Истина, Истина) + " від " + Формат(Выборка.ОснованиеДата, "Л=uk; ДФ='dd.MM.yyyy р.'"); 
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(Выборка.ОснованиеНомер) И ЗначениеЗаполнено(Выборка.ОснованиеДата) Тогда
			Строка["T5T1RXXXXG16S"]  	= "№ " + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Выборка.ОснованиеНомер, Истина, Истина) + " від " + Формат(Выборка.ОснованиеДата, "Л=uk; ДФ='dd.MM.yyyy р.'"); 
		КонецЕсли;	
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ДатаНачала) Тогда
			Строка["T7T1RXXXXG10"]  	= День(Выборка.ДатаНачала);
		КонецЕсли;
		Если ЗначениеЗаполнено(Выборка.ДатаОкончания) Тогда
			Строка["T7T1RXXXXG11"]  	= День(Выборка.ДатаОкончания);
		КонецЕсли;	
		...
		Строка["T7T1RXXXXG15S"]  	= ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Выборка.НомерПриказа, Истина, Истина);
	...
	|						ТОГДА &ДнейВМесяце
	|				КОГДА ПериодыТО.ДатаНачала <> ДАТАВРЕМЯ(1, 1, 1)
	|					ТОГДА ДЕНЬ(ПериодыТО.ДатаОкончания) - 1
	|				ИНАЧЕ ДЕНЬ(ПериодыТО.ДатаОкончания)
	|				КОНЕЦ - ВЫБОР
	|					КОГДА ПериодыТО.ДатаНачала = ДАТАВРЕМЯ(1, 1, 1) 
	|						ТОГДА 1
	|					КОГДА ПериодыТО.ДатаОкончания <> ДАТАВРЕМЯ(1, 1, 1)
	|						ТОГДА ДЕНЬ(ПериодыТО.ДатаНачала) - 1
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПериодыТО КАК ПериодыТО
	|		ПО ЕСВФОТ.ФизическоеЛицо = ПериодыТО.ФизическоеЛицо
	...
	Пока Выборка.Следующий() Цикл
		...
		Строка["T6T1RXXXXG111"]  	= Месяц(Выборка.Период);
		Строка["T6T1RXXXXG112"]  	= Год(Выборка.Период);
		...
		Если Выборка.ВидЕСВ = Перечисления.ВидыЕСВ.Больничные Тогда
			...
			Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" Тогда
				...
				Если НайденныеСтроки.Количество() > 0 Тогда
					Строка["T6T1RXXXXG13"] = НайденныеСтроки[0].КалендарныеДниБольничные;
				КонецЕсли;	
			Иначе
		...
		Если Выборка.ВидЕСВ = Перечисления.ВидыЕСВ.ОсновнаяЗарплата  Тогда
			...
			Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" Тогда
				...
				НайденныеСтроки = ДанныеУчетаВремениПоСостояниям.НайтиСтроки(Новый Структура("ФизическоеЛицо, Период", Выборка.ФизическоеЛицо, НачалоМесяца(Выборка.Период)));
				Если НайденныеСтроки.Количество() > 0 Тогда
					Строка["T6T1RXXXXG14"] = НайденныеСтроки[0].КалендарныеДниОтпускаБезСохраненияЗП;
				КонецЕсли;	
			Иначе
		...
		Если Выборка.ВидЕСВ = Перечисления.ВидыЕСВ.Декретные Тогда
			...
			Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" Тогда
				...
				НайденныеСтроки = ДанныеУчетаВремениПоСостояниям.НайтиСтроки(Новый Структура("ФизическоеЛицо, Период", Выборка.ФизическоеЛицо, НачалоМесяца(Выборка.Период)));
				Если НайденныеСтроки.Количество() > 0 Тогда
					Строка["T6T1RXXXXG16"] = НайденныеСтроки[0].КалендарныеДниОтпускПоБеременностиИРодам;
				КонецЕсли;	
			Иначе
		...
		Для каждого Строка Из Таблица Цикл
			...
			Для каждого Колонка Из КолонкиТаблицы Цикл
				...
				Если  ИдМногострочногоРаздела.Значение = СтруктураРеквизитовФормы.мИдГруппы5
					И Колонка.Имя = "T5T1RXXXXG13" 
					И ЗначениеПоказателя = 0 Тогда
					
					ЗначениеПоказателя = Неопределено;
					
				ИначеЕсли  ИдМногострочногоРаздела.Значение = СтруктураРеквизитовФормы.мИдГруппы5
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("Списки2018");
