...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("H10G1D", ТекДата, Форма, "НалоговыйДокумент");
	
	ЗаполнитьПоказательНаСервере("HORIG", "",  Форма, "НалоговыйДокумент");
	ЗаполнитьПоказательНаСервере("HCOPY", "X", Форма, "НалоговыйДокумент");
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	ДанныеШапки.Свойство("H10G1D",   СтруктураДанныхНалоговыйДокумент.H10G1D);
	...
	Для каждого СтрокаТЧ Из ДанныеТаблицы Цикл
		...
		Если  СтруктураРеквизитовФормы.мВерсияФормы = "01/12/2014" Тогда
			
			// ошибка при выходе конфигураций. При печати отчета и создании структуры данных для XML
			// были перепутаны имена колонок для 0% и 7%
			// данная версия отчета расчитана на "неправильные" конфигурации
			// данные этих колонок меняются местами
			ИмяЯчейки1 = СтруктураРеквизитовФормы.мИдГруппы1 + "G10";
			ИмяЯчейки2 = СтруктураРеквизитовФормы.мИдГруппы1 + "G111";
			
			ВремЗнач = СтрокаМногострочки[ИмяЯчейки1];
			СтрокаМногострочки[ИмяЯчейки1] = СтрокаМногострочки[ИмяЯчейки2];
			СтрокаМногострочки[ИмяЯчейки2] = ВремЗнач;
