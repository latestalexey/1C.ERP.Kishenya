...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2015";
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "БП" Тогда
		НастройкаПечати = НалоговыйУчетПовтИсп.НастройкаПечатиНалоговыхДокументов(Параметры.Организация, НачалоМесяца(Параметры.мДатаКонцаПериодаОтчета));
	...
	Иначе
	    ДополнятьНаименованиеАдресДаннымиГоловнойОрганизацииВНН = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ДополнятьНаименованиеАдресДаннымиГоловнойОрганизацииВНН = Форма.ДополнятьНаименованиеАдресДаннымиГоловнойОрганизацииВНН;
	...
	Если ДополнятьНаименованиеАдресДаннымиГоловнойОрганизацииВНН Тогда
	...
	ЗаполнитьПоказательНаСервере("H10G2S", 		ФИОДиректора, 		Форма, "НалоговыйДокумент");
...
Процедура УдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, НомерПервойСтроки = 1) Экспорт
	...
	Если СтруктураГруппы.Количество() > ИсходноеКоличествоСтрокГруппы Тогда
		...
		ИмяПервойЯчейкиУдаляемойСтроки = Сред(ИмяПервойКолонки,1,1) + Формат(НомУдаляемойСтроки,"ЧЦ=4;ЧВН=0;ЧГ=;") + Сред(ИмяПервойКолонки,2);
			...
			Для НСтр = НомУдаляемойСтроки + 1 По СтруктураГруппы.Количество() Цикл
				...
				Для Каждого КолонкаИмя ИЗ КолонкиГруппы Цикл
					...
					Если СтруктураГруппы[0].Свойство(КолонкаИмя) Тогда
						...
						ИмяСдвинутойЯчейки = Сред(КолонкаИмя,1,1) + Формат(НСтр,"ЧЦ=4;ЧВН=0;ЧГ=;") + Сред(КолонкаИмя,2);
						НовоеИмяСдвинутойЯчейки = Сред(КолонкаИмя,1,1) + Формат(НСтр - 1,"ЧЦ=4;ЧВН=0;ЧГ=;") + Сред(КолонкаИмя,2);
	...
	ИмяТекущейОбласти = Сред(ИмяПервойКолонки,1,1) + Формат(НомТекущейСтроки,"ЧЦ=4;ЧВН=0;ЧГ=;") + Сред(ИмяПервойКолонки,2);
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	ДанныеШапки.Свойство("H_2", СтруктураДанныхНалоговыйДокумент.HERPN);
	ДанныеШапки.Свойство("H_1", СтруктураДанныхНалоговыйДокумент.H01);
	...
	ДанныеШапки.Свойство("H01G1D",    СтруктураДанныхНалоговыйДокумент.H01G1D);
	ДанныеШапки.Свойство("H01G2S",    СтруктураДанныхНалоговыйДокумент.H01G2S);
	...
	ДанныеШапки.Свойство("H02G2D",   СтруктураДанныхНалоговыйДокумент.H02G2D);
	ДанныеШапки.Свойство("H02G3S",   СтруктураДанныхНалоговыйДокумент.H02G3S);
	
	ДанныеШапки.Свойство("H10G2S",   СтруктураДанныхНалоговыйДокумент.H10G2S);
	...
	Для каждого СтрокаТЧ Из ДанныеТаблицы Цикл
		...
		Для каждого Колонка Из ДанныеТаблицы.Колонки Цикл
			...
			Если  ИмяXSD = "G1D" И ТипЗнч(СтрокаТЧ[ИмяКолонки]) = Тип("Строка") Тогда
...
Функция ПолучитьДанныеФормыНаСервере();
	...
	Для каждого ИдМногострочногоРаздела Из мСтруктураМногострочныхРазделов Цикл
		...
		Для каждого Строка Из Таблица Цикл
			...
			Если ЗначениеЗаполнено(Строка.RG9) Тогда
			    СтруктураДанных.Вставить("RGRATE_" + НомерСтроки, "20%");
			ИначеЕсли ЗначениеЗаполнено(Строка.RG111) Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(Строка.RG11) Тогда
