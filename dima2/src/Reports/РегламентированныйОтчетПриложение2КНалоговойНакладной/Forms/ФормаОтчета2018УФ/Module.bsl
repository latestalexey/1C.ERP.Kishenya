...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = НСтр("ru='Приложение 2 к Налоговой накладной';uk='Додаток 2 до Податкової накладної'");
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("C_DOC_CNT",	1, Форма, "СлужебныеДанные");  //номер документа в периоде. 
...
Функция ПолучитьСледующийНомерДокументаВПериоде(Организация, НачПериода, КонПериода)
	...
	Запрос.УстановитьПараметр("ИсточникОтчета", "РегламентированныйОтчетПриложение2КНалоговойНакладной");
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	ДанныеШапки.Свойство("HERPN0", СтруктураДанныхНалоговыйДокумент.HERPN0);
	...
	ДанныеШапки.Свойство("HPODFILL",  СтруктураДанныхНалоговыйДокумент.HPODFILL);
	ДанныеШапки.Свойство("HPODNUM",   СтруктураДанныхНалоговыйДокумент.HPODNUM);
	ДанныеШапки.Свойство("HPODNUM1",  СтруктураДанныхНалоговыйДокумент.HPODNUM1);
	ДанныеШапки.Свойство("HPODNUM2",  СтруктураДанныхНалоговыйДокумент.HPODNUM2);
	...
	ДанныеШапки.Свойство("R02G9",   СтруктураДанныхНалоговыйДокумент.R02G9);
	ДанныеШапки.Свойство("R01G111", СтруктураДанныхНалоговыйДокумент.R01G111);
	ДанныеШапки.Свойство("R02G111", СтруктураДанныхНалоговыйДокумент.R02G111);
	ДанныеШапки.Свойство("R007G03", СтруктураДанныхНалоговыйДокумент.R007G03);
	ДанныеШапки.Свойство("R006G03", СтруктураДанныхНалоговыйДокумент.R006G03);
	ДанныеШапки.Свойство("R01G11",  СтруктураДанныхНалоговыйДокумент.R01G11);
	ДанныеШапки.Свойство("R001G03", СтруктураДанныхНалоговыйДокумент.R001G03);
	...
	ДанныеШапки.Свойство("R0301G1D", СтруктураДанныхНалоговыйДокумент.R0301G1D);
	ДанныеШапки.Свойство("R0301G2",  СтруктураДанныхНалоговыйДокумент.R0301G2);
	ДанныеШапки.Свойство("R0301G3",  СтруктураДанныхНалоговыйДокумент.R0301G3);
	ДанныеШапки.Свойство("R0301G4",  СтруктураДанныхНалоговыйДокумент.R0301G4);
	ДанныеШапки.Свойство("R0301G5",  СтруктураДанныхНалоговыйДокумент.R0301G5);
	ДанныеШапки.Свойство("R0302G1D", СтруктураДанныхНалоговыйДокумент.R0302G1D);
	ДанныеШапки.Свойство("R0302G2",  СтруктураДанныхНалоговыйДокумент.R0302G2);
	ДанныеШапки.Свойство("R0302G3",  СтруктураДанныхНалоговыйДокумент.R0302G3);
	ДанныеШапки.Свойство("R0302G4",  СтруктураДанныхНалоговыйДокумент.R0302G4);
	ДанныеШапки.Свойство("R0302G5",  СтруктураДанныхНалоговыйДокумент.R0302G5);
	...
	Для каждого СтрокаТЧ Из ДанныеТаблицы Цикл
		...
		КонецЦикла;
		Если НЕ РасчетЦеныТочный Тогда
			СтрокаМногострочки.RG6 = Окр(СтрокаМногострочки.RG6,2)	
		КонецЕсли;
		
		НомерСтроки = НомерСтроки + 1;
	КонецЦикла;
	
	ДанныеМногострочки.Свернуть("НомерСтроки,RG001,RG21,RG22,RG3S,RG4,RG4S,RG32,RG33,RG1052S,RG6,RG7,RG008,RG009,RG011,RG11U10","RG5,RG8,RG010");
	
	Для каждого СтрокаМногострочки Из ДанныеМногострочки Цикл
		Если РасчетЦеныТочный Тогда
			...
			ИначеЕсли  ЗначениеЗаполнено(СтрокаМногострочки.RG7)
				И ЗначениеЗаполнено(СтрокаМногострочки.RG8)
				И ЗначениеЗаполнено(СтрокаМногострочки.RG010) Тогда
				
				СтрокаМногострочки.RG7 = СтрокаМногострочки.RG010 / СтрокаМногострочки.RG8;
...
Функция ПолучитьДанныеФормыНаСервере();
	...
	ПолныйНомер = СокрЛП(ДанныеТП.HNUM1)+?(ЗначениеЗаполнено(СокрЛП(ДанныеТП.HNUM2)), "/" + СокрЛП(ДанныеТП.HNUM2), "");
	
	ПолныйНомерНН = СокрЛП(ДанныеТП.HPODNUM1)+?(ЗначениеЗаполнено(СокрЛП(ДанныеТП.HPODNUM2)), "/" + СокрЛП(ДанныеТП.HPODNUM2), "");
	ПолныйНомерНН = СокрЛП(ДанныеТП.HPODNUM)+?(ПолныйНомерНН = "", "", "/"+ПолныйНомерНН);
	
	СтруктураРеквизитовФормы.ДопПоказателиОтчетаЗвит1С.Вставить("ПолныйНомер", ПолныйНомер);
	СтруктураРеквизитовФормы.ДопПоказателиОтчетаЗвит1С.Вставить("ПолныйНомерНН", ПолныйНомерНН);
