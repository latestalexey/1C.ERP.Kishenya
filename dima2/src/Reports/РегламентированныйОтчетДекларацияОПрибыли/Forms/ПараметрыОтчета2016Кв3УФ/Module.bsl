...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НалогНаНедвижимость			   = Параметры.НалогНаНедвижимость;	
...
Процедура ДействияПриЗакрытии()
	...
	ВладелецФормы.СтруктураРеквизитовФормы.НалогНаНедвижимость 			 = НалогНаНедвижимость;
...
Процедура ЗаполнитьПрибыльПрошлогоГода(Команда)
	...
	Если ПолучитьДанныеЗаПрошлыйГодИз1С Тогда
		...
		ОблагаемаяПрибыльПрошлогоГода = ЗаполнитьПрибыльПрошлогоГодаНаСервере();
		...
		Попытка
			...
			Иначе
				...
				Иначе
					...
					ОблагаемаяПрибыльПрошлогоГода = ПолучитьЗначениеПоказателяЗвит1С(Данные, "A07");	// рядок 7 Декларації
...
&НаСервере
Функция ЗаполнитьПрибыльПрошлогоГодаНаСервере()
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если    Таблица = "СписаниеНеБезнадежнойЗадолженности_Статьи"
		ИЛИ Таблица = "ПередачаНеприбыльным_СтатьиЗатрат" Тогда
