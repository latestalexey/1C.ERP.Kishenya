...
Функция СформироватьОтчет()
	...
	ОбластьЗаголовок.Параметры.ЗаголовокОтчета =  НСтр("ru='Сравнительный анализ динамики ассортимента и продаж';uk='Порівняльний аналіз динаміки асортименту та продажів'") 
	+ ?(ЭтоРасшифровка, НСтр("ru=' (детально по номенклатуре)';uk=' (детально по номенклатурі)'"), "");
	...
	ТекстПараметров = 
	  НСтр("ru='Период анализа : с';uk='Період аналізу : з'")
	  + " " + Формат(ДатаНачалаПериодаАнализа, "ДЛФ=D") + " "
	  + НСтр("ru='по';uk='по'") + " "
	  + Формат(ДатаОкончанияПериодаАнализа, "ДЛФ=D")
	  + Символы.ПС
	  + НСтр("ru='Период сравнения : с';uk='Період порівняння : з'") + " "
...
Функция РезультатВыборкиДанных(ПредставлениеОтбора, МассивПолей)
	Запрос=Новый Запрос;
	Запрос.УстановитьПараметр("ПрочиеМарки", НСтр("ru='Прочие марки';uk='Інші марки'"));
	...
	Иначе
		СтрокаСообщения = НСтр("ru='Не указана дата начала периода анализа';uk='Не зазначена дата початку періоду аналізу'");
	...
	Иначе
		СтрокаСообщения = НСтр("ru='Не указана дата окончания периода анализа';uk='Не зазначена дата закінчення періоду аналізу'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения);
		Возврат Неопределено;
	КонецЕсли;
	...
	Иначе
		СтрокаСообщения = НСтр("ru='Не указана дата начала периода сравнения';uk='Не зазначена дата початку періоду порівняння'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения);
		Возврат Неопределено;
	КонецЕсли;
	...
	Иначе
		СтрокаСообщения = НСтр("ru='Не указана дата окончания периода сравнения';uk='Не зазначена дата закінчення періоду порівняння'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения);
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ЭлементыОтбора.Количество() > 0 Тогда
		...
		Для Каждого ЭлементОтбора ИЗ ЭлементыОтбора Цикл
			...
			Если ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено
				...
				СтрокаСообщения = НСтр("ru='Вид сравнения ""%1"" не может быть использован в данном варианте отчета.';uk='Вид порівняння ""%1"" не може бути використаний у даному варіанті звіту.'");
	...
	|	" + ?(ЭтоРасшифровка, "Т.ПредставленностьНоменклатура", "0") + " КАК ПредставленностьНоменклатура,
	|	" + ?(ЭтоРасшифровка, "Т.НаполненностьНоменклатура", "0") + " КАК НаполненностьНоменклатура,
	|	Т.Квота КАК Квота,
	...
	Иначе
		...
		|";
	КонецЕсли;
	ТекстЗапросаИтоговый = "ВЫБРАТЬ " + ТекстВыбранныхПолей + "
...
Функция ТекстЗапросаДнейПериода()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДОБАВИТЬКДАТЕ(&ДатаНачалаПериодаАнализа, ДЕНЬ, ТЦ1.Ц + 10 * ТЦ2.Ц + 100 * ТЦ3.Ц + 1000 * ТЦ4.Ц) КАК День
	|ПОМЕСТИТЬ втДней
	|ИЗ
	|	ТЦ КАК ТЦ1,
	|	ТЦ КАК ТЦ2,
	...
	|	ТЦ КАК ТЦ4
	|ГДЕ
	|	ДОБАВИТЬКДАТЕ(&ДатаНачалаПериодаАнализа, ДЕНЬ, ТЦ1.Ц + 10 * ТЦ2.Ц + 100 * ТЦ3.Ц + 1000 * ТЦ4.Ц) <= &ДатаОкончанияПериодаАнализа
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Функция ТекстЗапросаАссортимент()
	...
	|			%УсловиеНоменклатураКАссортименту%
...
Функция ТекстЗапросаВыручка()
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЕСТЬNULL(ИсторияФорматов.ФорматМагазина, ЗНАЧЕНИЕ(Справочник.ФорматыМагазинов.ПустаяСсылка)) КАК ФорматМагазина,
	|	ВыручкаОбороты.АналитикаУчетаНоменклатуры.Склад КАК Магазин,
	...
	|	СУММА(ВыручкаОбороты.СебестоимостьРеглОборот) КАК СебестоимостьРеглОборот,
	|	СУММА(ВыручкаОбороты.СуммаВыручкиРеглОборот) КАК СуммаВыручкиРеглОборот
	|ПОМЕСТИТЬ ВыручкаОбороты
	|ИЗ
	|	РегистрНакопления.ВыручкаИСебестоимостьПродаж.Обороты(&ДатаНачалаПериодаАнализа, &ДатаОкончанияПериодаАнализа, День, АналитикаУчетаНоменклатуры.Склад.ТипСклада = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.РозничныйМагазин)
	|	%УсловиеВыручка%
	|	) КАК ВыручкаОбороты
	|		ЛЕВОЕ СОЕДИНЕНИЕ втТекущиеКвоты КАК ТекущиеКвоты
	...
	|			И (ТекущиеКвоты.Магазин = ВыручкаОбороты.АналитикаУчетаНоменклатуры.Склад)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИсторияФорматов КАК ИсторияФорматов
	|		ПО ВыручкаОбороты.АналитикаУчетаНоменклатуры.Склад = ИсторияФорматов.Склад
	|
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(ИсторияФорматов.ФорматМагазина, ЗНАЧЕНИЕ(Справочник.ФорматыМагазинов.ПустаяСсылка)),
	|	ВыручкаОбороты.АналитикаУчетаНоменклатуры.Склад,
	...
	Если ЭтоРасшифровка Тогда
		...
		|ИЗ
		|	РегистрНакопления.ВыручкаИСебестоимостьПродаж.Обороты(&ДатаНачалаПериодаАнализа, &ДатаОкончанияПериодаАнализа, , АналитикаУчетаНоменклатуры.Склад.ТипСклада = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.РозничныйМагазин)
		|	%УсловиеВыручка%
		|	) КАК ВыручкаОбороты
		|		ЛЕВОЕ СОЕДИНЕНИЕ ИсторияФорматов КАК ИсторияФорматов
		|		ПО ВыручкаОбороты.АналитикаУчетаНоменклатуры.Склад = ИсторияФорматов.Склад
...
Процедура ДобавитьУсловияИПараметры(Запрос, ТекстЗапроса, ЭлементыОтбора)
	...
	СчетчикУсловий = 0;
	СоответствиеПолейУсловия = Новый Соответствие;
	СоответствиеПолейУсловия.Вставить("Магазин", "АналитикаУчетаНоменклатуры.Склад");
...
Процедура ВывестиШапку(ТабличныйДокументРезультат, Макет)
	...
	Если ЭтоРасшифровка Тогда
		ОбластьШапкаГруппировки.Параметры.ЗаголовокГруппировок = НСтр("ru='Товарная категория / Марка / Номенклатура';uk='Товарна категорія / Марка / Номенклатура'");
	Иначе
		ОбластьШапкаГруппировки.Параметры.ЗаголовокГруппировок = НСтр("ru='Вид номенклатуры / Товарная категория';uk='Вид номенклатури / Товарна категорія'");
	КонецЕсли;
	...
	Если ЭтоРасшифровка Тогда
		...
		Если НомерКолонкиРасшифровки = 2 Тогда
			МассивКолонок.Добавить(НСтр("ru='Количество периода анализа';uk='Кількість періоду аналізу'"));
			МассивКолонок.Добавить(НСтр("ru='Количество периода сравнения';uk='Кількість періоду порівняння'"));
			МассивКолонок.Добавить(НСтр("ru='Изменение количества абсолютное';uk='Зміна кількості абсолютна'"));
			МассивКолонок.Добавить(НСтр("ru='Изменение количества %';uk='Зміна кількості %'"));
		ИначеЕсли НомерКолонкиРасшифровки = 3 Тогда
			МассивКолонок.Добавить(НСтр("ru='Выручка периода анализа';uk='Виторг періоду аналізу'"));
			МассивКолонок.Добавить(НСтр("ru='Выручка периода сравнения';uk='Виторг періоду порівняння'"));
			МассивКолонок.Добавить(НСтр("ru='Изменение выручки абсолютное';uk='Зміна виторгу абсолютна'"));
			МассивКолонок.Добавить(НСтр("ru='Изменение выручки %';uk='Зміна виторгу %'"));
		ИначеЕсли НомерКолонкиРасшифровки = 4 Тогда
			МассивКолонок.Добавить(НСтр("ru='Валовая прибыль периода анализа';uk='Валовий прибуток періоду аналізу'"));
			МассивКолонок.Добавить(НСтр("ru='Валовая прибыль периода сравнения';uk='Валовий прибуток періоду порівняння'"));
			МассивКолонок.Добавить(НСтр("ru='Изменение валовой прибыли абсолютное';uk='Зміна валового прибутку абсолютна'"));
			МассивКолонок.Добавить(НСтр("ru='Изменение валовой прибыли %';uk='Зміна валового прибутку %'"));
		ИначеЕсли НомерКолонкиРасшифровки = 5 Тогда
			МассивКолонок.Добавить(НСтр("ru='Квота периода анализа';uk='Квота періоду аналізу'"));
			МассивКолонок.Добавить(НСтр("ru='Квота периода сравнения';uk='Квота періоду порівняння'"));
			МассивКолонок.Добавить(НСтр("ru='Изменение квоты абсолютное';uk='Зміна квоти абсолютна'"));
			МассивКолонок.Добавить(НСтр("ru='Изменение квоты %';uk='Зміна квоти %'"));
		ИначеЕсли НомерКолонкиРасшифровки = 6 Тогда
			МассивКолонок.Добавить(НСтр("ru='Общая наполнен- ность периода анализа';uk='Загальна наповнен - ність періоду аналізу'"));
			МассивКолонок.Добавить(НСтр("ru='Средняя наполнен- ность периода анализа';uk='Середня наповненість періоду аналізу'"));
			МассивКолонок.Добавить(НСтр("ru='Общая наполнен- ность периода сравнения';uk='Загальна наповнен - ність періоду порівняння'"));
			МассивКолонок.Добавить(НСтр("ru='Средняя наполнен- ность периода сравнения';uk='Середня наповненість періоду порівняння'"));
			МассивКолонок.Добавить(НСтр("ru='Изменение средней наполнен- ности абсолютное';uk='Зміна середньої наповненості абсолютна'"));
			МассивКолонок.Добавить(НСтр("ru='Изменение средней наполнен- ности %';uk='Зміна середньої наповненості %'"));
		ИначеЕсли НомерКолонкиРасшифровки = 7 Тогда
			МассивКолонок.Добавить(НСтр("ru='Общая представлен- ность периода анализа';uk='Загальна представлен - ність періоду аналізу'"));
			МассивКолонок.Добавить(НСтр("ru='Средняя представлен- ность периода анализа';uk='Середня представлений - ність періоду аналізу'"));
			МассивКолонок.Добавить(НСтр("ru='Общая представлен- ность периода сравнения';uk='Загальна представлен - ність періоду порівняння'"));
			МассивКолонок.Добавить(НСтр("ru='Средняя представлен- ность периода сравнения';uk='Середня представлений - ність періоду порівняння'"));
			МассивКолонок.Добавить(НСтр("ru='Изменение средней представлен- ности абсолютное';uk='Зміна середньої представленості абсолютна'"));
			МассивКолонок.Добавить(НСтр("ru='Изменение средней представлен- ности %';uk='Зміна середньої представленості %'"));
		КонецЕсли;
	...
	Иначе
		...
		ОбластьШапкаПоказательПродаж.Параметры.ЗаголовокПоказателя = НСтр("ru='Изменение количества %';uk='Зміна кількості %'");
		ТабличныйДокументРезультат.Присоединить(ОбластьШапкаПоказательПродаж, 1, , Истина);
		
		ОбластьШапкаПоказательПродаж.Параметры.ЗаголовокПоказателя = НСтр("ru='Изменение выручки %';uk='Зміна виторгу %'");
		ТабличныйДокументРезультат.Присоединить(ОбластьШапкаПоказательПродаж, 1, , Истина);
		
		ОбластьШапкаПоказательПродаж.Параметры.ЗаголовокПоказателя = НСтр("ru='Изменение валовой прибыли %';uk='Зміна валового прибутку %'");
		ТабличныйДокументРезультат.Присоединить(ОбластьШапкаПоказательПродаж, 1, , Истина);
		
		ОбластьШапкаПоказательАссортимента.Параметры.ЗаголовокПоказателя = НСтр("ru='Изменение квоты %';uk='Зміна квоти %'");
		ТабличныйДокументРезультат.Присоединить(ОбластьШапкаПоказательАссортимента, 1, , Истина);
		
		ОбластьШапкаПоказательАссортимента.Параметры.ЗаголовокПоказателя = НСтр("ru='Изменение наполнен- ности %';uk='Зміна наповненості %'");
		ТабличныйДокументРезультат.Присоединить(ОбластьШапкаПоказательАссортимента, 1, , Истина);
		
		ОбластьШапкаПоказательАссортимента.Параметры.ЗаголовокПоказателя = НСтр("ru='Изменение представлен- ности %';uk='Зміна представленості %'");
		ТабличныйДокументРезультат.Присоединить(ОбластьШапкаПоказательАссортимента, 1, , Истина);
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыИВывестиОбласть(Выборка,
	...
	МассивОбластей,
	УровеньГруппировкиВнешний = -1,
	СтруктураРасшифровки)
	
	ОбластьГруппировки = МассивОбластей[0];
	ОбластьПоказательПродаж = МассивОбластей[1];
	ОбластьПоказательАссортимента = МассивОбластей[2];
	...
	Если ЭтоРасшифровка Тогда
		...
		Для Каждого КолонкаМассива Из МассивКолонок Цикл
			...
			ТабличныйДокументРезультат.Присоединить(ОбластьПоказатель, 1, , Истина);
		КонецЦикла;
	Иначе
