...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("Штампик1", "Додаток", 									Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("Штампик2", "до постанови Кабінету Міністрів України", 	Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("Штампик3", "від 17 червня 2015 р. N 413",					Форма, "Уведомление");
	...
	ЗаполнитьПоказательНаСервере("HBUH",  ФИОГлавБуха, 		Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("HKBOS", ИННФИОДиректора, 	Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("HKBUH", ИННГлавБуха, 		Форма, "Уведомление");
...
Функция ПолучитьСледующийНомерДокументаВПериоде(Организация, НачПериода, КонПериода)
	...
	Запрос.УстановитьПараметр("ИсточникОтчета", "РегламентированныйОтчетУведомлениеОПриемеРаботника");
...
Процедура ДатаПодписиПриИзменении(Элемент)
	...
	Если ДатаНач > ДатаКон Тогда
		ДатаКон	= ДатаНач; 
	КонецЕсли;
	СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета  = ДатаКон;
	СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета = НачалоМесяца(ДатаПодписи);
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов)
	...
	ЗначениеВКлетку(СтруктураДанныхУведомление.HTIN + "          ", Ложь, 10, "ЕДРПОУ", Шапка, Истина);
	
	Шапка.Параметры.НазваниеОрганизации = СтруктураДанныхУведомление.HNAME;
	Шапка.Параметры.Начальная 		= СтруктураДанныхУведомление.H01;
	Шапка.Параметры.Отменяющая 		= СтруктураДанныхУведомление.H02;
	...
	ЗначениеВКлетку(СтруктураДанныхУведомление.HKBOS + "          ", Ложь, 10, "РуководительИНН", Подвал, Истина);
	ЗначениеВКлетку(СтруктураДанныхУведомление.HKBUH + "          ", Ложь, 10, "БухгалтерИНН", Подвал, Истина);
	Подвал.Параметры.РуководительФИО = СтруктураДанныхУведомление.HBOS;
	Подвал.Параметры.БухгалтерФИО 	 = СтруктураДанныхУведомление.HBUH;
	
	НоваяСтраница = Истина;
	КоличествоСтрок = 1;
	Для каждого СтрокаГруппы Из T1R Цикл
		...
		Строка.Параметры.Категория  = СтрокаГруппы.T1RXXXXG4;
		
		ЗначениеВКлетку(СтрокаГруппы.T1RXXXXG5S + "          ", Ложь,    10, "ДРФО",    Строка, Истина);
		Строка.Параметры.Фамилия  = СтрокаГруппы.T1RXXXXG61S;
		Строка.Параметры.Имя 	  = СтрокаГруппы.T1RXXXXG62S;
		Строка.Параметры.Отчество = СтрокаГруппы.T1RXXXXG63S;
		
		Строка.Параметры.НомерПриказа = СтрокаГруппы.T1RXXXXG7S;
				
		Если ЗначениеЗаполнено(СтрокаГруппы.T1RXXXXG8D) Тогда
		    ЗначениеВКлетку(Формат(СтрокаГруппы.T1RXXXXG8D, "ДФ=ddMMyyyy"), Ложь, 8, "ДатаПриема", Строка, Истина);
		Иначе	
			ЗначениеВКлетку("", Ложь,    8, "ДатаПриема", Строка, Истина);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтрокаГруппы.T1RXXXXG9D) Тогда
		    ЗначениеВКлетку(Формат(СтрокаГруппы.T1RXXXXG9D, "ДФ=ddMMyyyy"), Ложь, 8, "ДатаНачала", Строка, Истина);
		Иначе	
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" Тогда
		...
		ТаблицаКадровыеДанныеСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(
			...
			"ДатаПриема, ФизическоеЛицо, КодПоДРФО, ВидЗанятости, ПриказОПриеме, ПриказОПриемеНомер, ПриказОПриемеДата",
	...
	|	СотрудникиОрганизации.КодПоДРФО КАК КодПоДРФО,
	|	СотрудникиОрганизации.ВидЗанятости КАК ВидЗанятости,
	|	СотрудникиОрганизации.ДатаПриема КАК ДатаНачала,
	|	СотрудникиОрганизации.ПриказОПриеме КАК ПриказОПриеме,
	|	СотрудникиОрганизации.ПриказОПриемеНомер КАК ПриказОПриемеНомер,
	|	СотрудникиОрганизации.ПриказОПриемеДата КАК ПриказОПриемеДата
	...
	|ИЗ
	|	&ТаблицаКадровыеДанныеФизическихЛиц КАК ФизическиеЛица
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ФизическиеЛица.Имя КАК Имя,
	|	ФизическиеЛица.Отчество КАК Отчество,
	|	СотрудникиОрганизации.КодПоДРФО КАК ИНН,
	|	ВЫБОР
	...
	|	СотрудникиОрганизации.ПриказОПриемеНомер КАК НомерПриказа,
	|	СотрудникиОрганизации.ПриказОПриемеДата КАК ДатаПриказа
	|ИЗ
	|	ВТСотрудникиОрганизации КАК СотрудникиОрганизации
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТФизическиеЛица КАК ФизическиеЛица
	|		ПО СотрудникиОрганизации.ФизическоеЛицо = ФизическиеЛица.ФизическоеЛицо
	|ГДЕ
	|	СотрудникиОрганизации.ДатаНачала МЕЖДУ &ДатаНач И &ДатаКон
	|
	|УПОРЯДОЧИТЬ ПО
	|	Фамилия,
	|	Имя,
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Результат.Следующий() Цикл
			...
			Если ПерваяСтрока Тогда
				ПерваяСтрока = НЕ ПерваяСтрока;	
				Строка = СтруктураГруппы[0];
			Иначе
				Строка = СтруктураГруппы.Добавить();
			КонецЕсли;
			
			Строка["T1RXXXXG4"] = Результат.Категория;      		   
			Строка["T1RXXXXG5S"] = Результат.ИНН;      		   
			Строка["T1RXXXXG61S"] = Результат.Фамилия;      		   
			Строка["T1RXXXXG62S"] = Результат.Имя;
			Строка["T1RXXXXG63S"] = Результат.отчество;
			Строка["T1RXXXXG7S"] = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Результат.НомерПриказа, Истина, Истина);
			Строка["T1RXXXXG8D"] = Результат.ДатаПриказа;
			Строка["T1RXXXXG9D"] = Результат.ДатаНачала;
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Для Сч = 1 По КоличествоСтраниц Цикл
		...
		НовСтрока.ПредставлениеСхемы 	 = НСтр("ru='Уведомление о приеме работника на работу';uk='Повідомлення про прийом працівника на роботу'");
...
Процедура ОткрытьФормуПараметров(Команда)
	...
	мПараметрыФормы.Вставить("ДатаНач", ДатаНач);
	мПараметрыФормы.Вставить("ДатаКон", ДатаКон);
