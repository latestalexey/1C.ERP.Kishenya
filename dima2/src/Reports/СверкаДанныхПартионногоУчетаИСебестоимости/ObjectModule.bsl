...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ДобавитьПояснениеОДоступныхОрганизациях(ДокументРезультат);
...
Функция СформироватьТаблицуСверкиРегистров(КомпоновщикНастроек)
	...
	Запрос.УстановитьПараметр("УчитыватьСебестоимостьТоваровПоВидамЗапасов",
		Константы.УчитыватьСебестоимостьТоваровПоВидамЗапасов.Получить());
	...
	ТекстЗапроса = ""
		...
		+ ТекстЗапросаПартииПереданныеНаКомиссию()
			+ ОбъединениеТекстовЗапроса()
...
Функция ТекстЗапросаПартииПереданныеНаКомиссию()
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	""%1"" 										КАК ИсточникДанных,
	|	""ПартииПереданныеНаКомиссию"" 				КАК РазделИсточникаДанных,
	|	1											КАК Знак,
	|
	|	Партии.Организация 							КАК Организация,
	|	Аналитика.Склад 							КАК Склад,
...
//++ НЕ УТ
	|		КОГДА ТИПЗНАЧЕНИЯ(Партии.ДокументВыпуска) = ТИП(Документ.ВыпускПродукции) 
	|			  ИЛИ ТИПЗНАЧЕНИЯ(Партии.ДокументВыпуска) = ТИП(Документ.ПоступлениеОтПереработчика) 
	|			  ИЛИ ТИПЗНАЧЕНИЯ(Партии.ДокументВыпуска) = ТИП(Документ.ОтчетПереработчика) 
	|			ТОГДА ВЫБОР
	|				КОГДА (Партии.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Облагаемая)
	|		      			И Партии.ДокументВыпуска.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая))
	...
	|		        		И Партии.ДокументВыпуска.НалоговоеНазначение = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность))
	|					ТОГДА Партии.НДСРеглНачальныйОстаток
	|				ИНАЧЕ 0 
	|			КОНЕЦ
//-- НЕ УТ
...
Процедура ПолучитьДоступныеОрганизации(Организация)
	...
	Запрос.УстановитьПараметр("Период", 	 КонецМесяца(НачалоПериода));
	
	МассивОрганизаций = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Организация");
	
КонецПроцедуры

Процедура ДобавитьПояснениеОДоступныхОрганизациях(ДокументРезультат)
	
	ТекстПредупреждения = 
		НСтр("ru='Отчет сформирован только по тем организациям, у которых на начало периода отчета использовался метод оценки стоимости ""ФИФО (скользящая оценка)"".';uk='Звіт сформований лише по тих організаціях, у яких на початок періоду звіту використовувався метод оцінки вартості ""ФІФО (змінна оцінка)"".'");
