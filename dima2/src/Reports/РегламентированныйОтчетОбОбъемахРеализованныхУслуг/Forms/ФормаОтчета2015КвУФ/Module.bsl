...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2015КвУФ";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	ПроцентСтавкиНДСВРознице = ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить("РегламентированнаяОтчетность", "РегламентированныйОтчетОбОбъемахРеализованныхУслуг_ПроцентСтавкиНДСВРознице", 20);	
	СтруктураРеквизитовФормы.Вставить("ПроцентСтавкиНДСВРознице", ПроцентСтавкиНДСВРознице);
...
Функция Сохранить(Автосохранение = Ложь, НаимТекРаздела) Экспорт	
	...
	СписокСохранения.Вставить("ПроцентСтавкиНДСВРознице", СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице);
...
Процедура ВосстановитьСохраненныеДанные()
	...
	СписокСохранения.Свойство("ПроцентСтавкиНДСВРознице", СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице);
...
Процедура УстановитьЗначениеПоказателя(Имя, Значение)
	...
	СтруктураДанныхУведомление[Имя] = Значение;
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Счет30   = ПС.Касса;						// Розничная выручка АТТ: 301, 302 и только для УПП счет 309
	Счет64   = ПС.РасчетыПоНалогамИПлатежам;
	Счет70_3 = ПС.ДоходОтРеализацииРаботИУслуг;
	Попытка
		Счет70_9 = ПС.ДоходОтРозничныхПродаж;		// Розничная выручка НТТ
	Исключение
		Счет70_9  = ПС.ПустаяСсылка();
	КонецПопытки;	
	...
	// 01. Обсяг реалізованих послуг (у т. ч. ПДВ)
	A01 = КО(мБухИтоги, Счет70_3);
	_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A01", "Показник ""Обсяг реалізованих послуг (у т. ч. ПДВ)""", "+", "!КО", A01, Счет70_3,,СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	
	// Дополнительй случай, продажа в розницу через 709 счет:
	// * УПП (АТТ, НТТ),
	// * УТП (АТТ, НТТ без суммового учета),
	// * БП  (НТТ по покупным ценам).			
	//Если РегламентированнаяОтчетность.ИДКонфигурации() = "УПП" Тогда
	//	// ООРП Дт 30  Кт 703 Сумма с НДС
	//	//      Дт 703 Кт 709 НДС
	//	//
	//	// ПКО  Дт 709 Кт 64  НДС
	//	ОБ_703_709 = ОБ(мБухИтоги, Счет70_3, Счет70_9); // Расчитаем часто используемый показатель на будующее
	//	
	//	A01_доп = 0; // Для данного показателя дополнительные расчеты не требуются.  
	//Иначе
		// ООРП Дт 709 Кт 703 Сумма без НДС
		//
		// ПКО  Дт 30  Кт 709 Сумма с НДС
		//      Дт 709 Кт 64  НДС		
	ОБ_709_703 = ОБ(мБухИтоги, Счет70_9, Счет70_3);
	
	// Для розничных продаж через 709 счет кредитовый оборот по 703 счету без НДС, дополним недостающий НДС 		
    A01_доп = Окр(ОБ_709_703 * СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице/100, 2); // Сумма без НДС -> НДС
	Если A01_доп <> 0 Тогда
		_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A01", "", "+", "!ОБ", A01_доп, Счет70_9, Счет70_3, СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A01", "", "+", "#З урахуванням ставки ПДВ "+Строка(СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице), , , , СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	КонецЕсли;
	//КонецЕсли;	
    A01 = A01 + A01_доп;
	УстановитьЗначениеПоказателя("A01", A01);
	
	
	// 02. Обсяг реалізованих послуг (у т. ч. ПДВ), з рядка 02 реалізовано послуг населенню
	ОБ_30_703 = ОБ(мБухИтоги, Счет30, Счет70_3); // Расчитаем часто используемый показатель
	A02 = ОБ_30_703;
	_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A02", "Показник ""Обсяг реалізованих послуг (у т. ч. ПДВ), населенню""", "+", "!ОБ",A02, Счет30, Счет70_3, СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);	
	
	// Дополнительй случай, продажа в розницу через 7091 счет
	//Если РегламентированнаяОтчетность.ИДКонфигурации() = "УПП" Тогда
	//	A02_доп = 0; // Для данного показателя дополнительные расчеты не требуются 
	//Иначе
    A02_доп = Окр(ОБ_709_703 * (100+СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице)/100, 2); // Сумма без НДС -> Сумма с НДС
	Если A02_доп <> 0 Тогда
		_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A03", "", "+", "!ОБ", A02_доп, Счет70_9, Счет70_3, СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	КонецЕсли;	
	//КонецЕсли;	
	A02 = A02 + A02_доп;	
	УстановитьЗначениеПоказателя("A02", A02);
	
	// 03. Обсяг реалізованих послуг підприємствам (установам)																						
	УстановитьЗначениеПоказателя("A03", A01 - A02);
	_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A03", "Показник ""Обсяг реалізованих послуг (у т. ч. ПДВ), Підприємствам, установам""", "+", "Значення рядка 01",A01, , , СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A03", "", "-", "Значення рядка 02",A02 , , , СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	
	
	// 05. Податок на додану вартість [з рядка 01]
	A05 = ОБ(мБухИтоги, Счет70_3, Счет64);
	_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A05", "Показник ""Податок на додану вартість [з рядка 02]""", "+", "!ОБ", A05, Счет70_3, Счет64, СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	
	// Дополнительй случай, продажа в розницу через 7091 счет.	
	//Если РегламентированнаяОтчетность.ИДКонфигурации() = "УПП" Тогда
	//	A05_доп = ОБ_703_709; // НДС
	//	Если A05_доп <> 0 Тогда
	//		_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A05", "", "+", "!ОБ", A05_доп, Счет70_3, Счет70_9, СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	//	КонецЕсли;
	//Иначе
	A05_доп = Окр(ОБ_709_703 * СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице/100, 2); // Сумма без НДС -> НДС
	Если A05_доп <> 0 Тогда
		_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A05", "", "+", "!ОБ", A05_доп, Счет70_9, Счет70_3, СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A05", "", "+", "#З урахуванням ставки ПДВ "+Строка(СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице), , , , СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	КонецЕсли;
	//КонецЕсли;
	A05 = A05 + A05_доп;
	УстановитьЗначениеПоказателя("A05", A05);
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "БП" Тогда
		// 21. Середня кількість працівників, осіб (в цілих числах)
		...
		УстановитьЗначениеПоказателя("A21", ТаблЧисленностиССЧ[0].Средняя);
		
		// 22. Середньооблікова кількість штатних працівників, осіб
		ПараметрыСредняяЧисленность.Вставить("ОтборШтат", Истина);
		ТаблЧисленностиССЧ = КадровыйУчет.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.Организация, ПараметрыСредняяЧисленность); 
		УстановитьЗначениеПоказателя("A22", ТаблЧисленностиССЧ[0].Средняя);
		
		// 23 ФОТ штатных работников
		//Если    РегламентированнаяОтчетность.ИДКонфигурации() = "УПП" Тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ
						|   СУММА(Начисления.Результат) КАК СуммаШтат
		                |ИЗ
		                |	РегистрРасчета.Начисления КАК Начисления
						|//#ЕстьКадровыйУчет ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КадроваяИсторияСотрудников.СрезПоследних(&КонецПериода, ) КАК РаботникиОрганизаций
						|//#НетКадровыйУчет  ЛЕВОЕ СОЕДНИЕНИЕ РегистрСведений.ТекущиеКадровыеДанныеСотрудников КАК РаботникиОрганизаций
		                ...
		                |		ИНАЧЕ ИСТИНА
		                |	КОНЕЦ
						|//#ЕстьКадровыйУчет И (РаботникиОрганизаций.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы) ИЛИ РаботникиОрганизаций.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ВнутреннееСовместительство))
						|//#ЕстьКадровыйУчет И РаботникиОрганизаций.Организация = &парамОрганизация
						|//#НетКадровыйУчет И (РаботникиОрганизаций.ТекущийВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы) ИЛИ РаботникиОрганизаций.ТекущийВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ВнутреннееСовместительство))
						|//#НетКадровыйУчет И РаботникиОрганизаций.ГоловнаяОрганизация = &парамОрганизация
						|";
						
		
		Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") = Истина Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "//#ЕстьКадровыйУчет", "");					
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "//#НетКадровыйУчет", "");					
		КонецЕсли;
		...
		НачисленияЗаКвартал = Запрос.Выполнить().Выбрать();
		Если НачисленияЗаКвартал.Следующий() Тогда
			УстановитьЗначениеПоказателя("A23", НачисленияЗаКвартал.СуммаШтат);	
		КонецЕсли;
	КонецЕсли;	
...
Процедура ОткрытьФормуПараметров(Команда)
	...
	мПараметрыФормы.Вставить("ПроцентСтавкиНДСВРознице", 	СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице);
