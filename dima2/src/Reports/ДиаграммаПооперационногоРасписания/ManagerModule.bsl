...
//		* Параметры - Структура - параметры формирования и вывода расписания, состав см. функцию ПараметрыВывода.
//		* ДиаграммаГанта - ДиаграммаГанта - объект, в который будет выведено расписание.
//  АдресХранилища - УникальныйИдентификатор, Строка - адрес во временном хранилище, по которому надо поместить сформированную диаграмму.
//
...
//
// Параметры:
//  Форма - УправляемаяФорма - форма, в которую будет выведена легенда.
//  МоделированиеВозможно - Булево - флаг, определяющий может ли быть выполнен расчет (и соответственно моделирование)
...
Функция ПодсказкаИнтервала(Знач ЗначенияЗаполнения)
	...
	Если ЗначенияЗаполнения.Состояние = ТекстРабота()
		...
		Если НЕ ЗначенияЗаполнения.ПараллельнаяЗагрузка Тогда
			...
			Результат = Результат
				+ Символы.ПС
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Операция: %1';uk='Операція: %1'"), ЗначенияЗаполнения.ОперацияПредставление)
				+ Символы.ПС
			...
			Если ЗначениеЗаполнено(ЗначенияЗаполнения.Загрузка) Тогда
				Результат = Результат
					...
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Загрузка: %1 %2';uk='Завантаження: %1 %2'"),
		...
		Иначе
			...
			Результат = Результат
				+ Символы.ПС
				+ НСтр("ru='Параллельная загрузка';uk='Паралельне завантаження'");
			
		КонецЕсли;
		
	ИначеЕсли ЗначенияЗаполнения.Состояние = ТекстРемонт() Тогда
		
		Результат = Результат + Символы.ПС + НСтр("ru='Ремонт';uk='Ремонт'");
...
Функция ТекстПереналадка()
	...
	Возврат НСтр("ru='Переналадка';uk='Переналадка'");
	
КонецФункции

Функция ТекстРабота()
	
	Возврат НСтр("ru='Работа';uk='Робота'");
	
КонецФункции

Функция ТекстРемонт()
	
	Возврат НСтр("ru='Ремонт';uk='Ремонт'");
...
Функция ДобавитьВФормуГруппуОбозначения(Форма, ИмяГруппы)
	...
	Результат.Заголовок = НСтр("ru='Обозначения';uk='Позначення'");
	Результат.Поведение = ПоведениеОбычнойГруппы.Свертываемая;
	Результат.ЗаголовокСвернутогоОтображения = НСтр("ru='Показать обозначения';uk='Показати позначення'");
	...
	Результат.Подсказка = НСтр("ru='Обозначения элементов диаграммы';uk='Позначення елементів діаграми'");
...
Процедура ДобавитьВЛегендуОписаниеРабочегоЦентра(Форма, Родитель)
	...
	Пояснение = НСтр("ru='- рабочий центр';uk='- робочий центр'");
...
Процедура ДобавитьВЛегендуОписаниеОблсуживающегоРабочегоЦентра(Форма, Родитель)
	...
	Пояснение = НСтр("ru='- вспомогательный рабочий центр';uk='- допоміжний робочий центр'");
...
Процедура ДобавитьВЛегендуОписаниеДополнительногоРабочегоЦентра(Форма, Родитель)
	...
	Пояснение = НСтр("ru='- дополнительный рабочий центр';uk='- додатковий робочий центр'");
...
Процедура ДобавитьВЛегендуОписаниеМоделиПланирования(Форма, Родитель)
	...
	Пояснение = НСтр("ru='- модель планирования';uk='- модель планування'");
...
Процедура ДобавитьЛегендуОперацийОбработкиИСборки(Форма, Родитель)
	...
	ЗаголовокГруппы = НСтр("ru='Операции обработки и сборки';uk='Операції обробки і збирання'");
...
Процедура ДобавитьЛегендуПрочихОпераций(Форма, Родитель)
	...
	ЗаголовокГруппы = НСтр("ru='Прочие операции';uk='Інші операції'");
	ГруппаЛегендаЦветов = ДобавитьВФормуГруппуЛегендаЦветов(Форма, Родитель, ИмяГруппы, ЗаголовокГруппы);
...
Процедура ДобавитьВЛегендуОписаниеЦветаНеНачато(Форма, Родитель)
	...
	Пояснение = НСтр("ru='Выполнение не начато';uk='Виконання не розпочато'");
...
Процедура ДобавитьВЛегендуОписаниеЦветаПараллельнаяЗагрузка(Форма, Родитель)
	...
	Пояснение = НСтр("ru='Параллельная загрузка';uk='Паралельне завантаження'");
...
Процедура ДобавитьВЛегендуОписаниеЦветаНачато(Форма, Родитель)
	...
	Пояснение = НСтр("ru='Выполнение начато';uk='Виконання розпочато'");
...
Процедура ДобавитьВЛегендуОписаниеЦветаЗавершено(Форма, Родитель)
	...
	Пояснение = НСтр("ru='Выполнение завершено';uk='Виконання завершено'");
...
Процедура ДобавитьВОписаниеПредставлениеЦвета(Форма, Родитель, ИмяЭлемента, Цвет)
	...
	НовыйЭлемент.Заголовок = НСтр("ru='Цвет';uk='Колір'");
	НовыйЭлемент.Вид = ВидДекорацииФормы.Надпись;
...
Функция ПредставлениеМаршрутногоЛиста(Номер, Дата)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Маршрутный лист №%1 от %2';uk='Маршрутний лист №%1 від %2'"),
