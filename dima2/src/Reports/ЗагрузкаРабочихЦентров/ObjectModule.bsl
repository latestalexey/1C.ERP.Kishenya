...
Процедура УстановитьРасширенноеОформлениеСерии(Серия) Экспорт
	...
	Если СтрНайти(Серия.Текст, НСтр("ru='Высокая загрузка, выше доступного времени';uk='Високе завантаження, вище доступного часу'")) <> 0
		ИЛИ СтрНайти(Серия.Текст, НСтр("ru='Высокая загрузка, ниже доступного времени';uk='Високе завантаження, нижче доступного часу'")) <> 0 Тогда
		
		Серия.Цвет = WebЦвета.ЛососьСветлый;
		
	ИначеЕсли СтрНайти(Серия.Текст, НСтр("ru='Доступное время';uk='Доступний час'")) Тогда
		
		Серия.Цвет = WebЦвета.СветлоСерый;
		
	ИначеЕсли СтрНайти(Серия.Текст, НСтр("ru='Нормальная загрузка';uk='Нормальне завантаження'")) Тогда
		
		Серия.Цвет = WebЦвета.СинийСоСтальнымОттенком;
		
	ИначеЕсли СтрНайти(Серия.Текст, НСтр("ru='Низкая загрузка';uk='Низьке завантаження'")) > 0 Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначенияОтбораДанных.Перегрузка > 100 Тогда
		ТекстСообщения = НСтр("ru='Параметр перегрузки не может превышать 100%.';uk='Параметр перевантаження не може перевищувати 100%.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ); 
	КонецЕсли; 
	
	Если ЗначенияОтбораДанных.Перегрузка < ЗначенияОтбораДанных.Недозагрузка
		И ЗначенияОтбораДанных.Перегрузка <> 0 Тогда
		ТекстСообщения = НСтр("ru='Параметр перегрузки должен быть выше параметра недозагрузки.';uk='Параметр перевантаження повинен бути вище параметра недозавантаження.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ); 
	КонецЕсли; 
...
Функция ДанныеОЗагрузке(НастройкиОсновнойСхемы)
	...
	|			ТОГДА ВЫБОР
	|					КОГДА МаршрутныйЛистПроизводстваОперации.РабочийЦентр.Календарь <> ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
	...
	|					КОГДА МаршрутныйЛистПроизводстваОперации.Ссылка.Подразделение.ГрафикРаботы <> ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
	|						ТОГДА МаршрутныйЛистПроизводстваОперации.Ссылка.Подразделение.ГрафикРаботы
	...
	|		КОГДА МаршрутныйЛистПроизводства.Подразделение.ГрафикРаботы <> ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
	|			ТОГДА МаршрутныйЛистПроизводства.Подразделение.ГрафикРаботы
	|		ИНАЧЕ ОсновнойКалендарьПредприятия.Значение
	|	КОНЕЦ,
