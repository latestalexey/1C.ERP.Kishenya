...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = НСтр("ru='Отчет';uk='Звіт'");
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Уведомление", "ЕДРПОУ_",  8, КодЕДРПОУ, Ложь);		
	
	ЗаполнитьПоказательНаСервере("ОргДиректор",   	ФИОДиректора, 		Форма, "Уведомление");

	ЗаполнитьПоказательНаСервере("ОргЮрАдрес", ОргЮрАдрес, Форма, "Уведомление");
	
	ЗаполнитьПоказательНаСервере("ОргАдрес", ОргАдрес, Форма, "Уведомление");
	
	ЗаполнитьПоказательНаСервере("ЭлПочта", ЭлПочта, Форма, "Уведомление");

	ЗаполнитьПоказательНаСервере("Телефон", Телефон, Форма, "Уведомление");
	
	ЗаполнитьПоказательНаСервере("Факс", Факс, Форма, "Уведомление");
	
	ЗаполнитьПоказательНаСервере("ПериодОтчета", Формат(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = ' гггг';Л = uk_UA") + " рік", Форма, "Уведомление");
	...
	ЗаполнитьПоказательНаСервере("ИСПОЛНИТЕЛЬ",  ФИОИсполнителя, 	Форма, "Уведомление");
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" Тогда
		...
		ПараметрыПолученияСотрудников.КадровыеДанные	= "ДатаПриема, ДатаУвольнения, ДатаРождения, ВидЗанятости, Пол, ДоляНеполногоРабочегоВремени, Пенсионер, ВидСобытия";
		...
		Запрос.УстановитьПараметр("ДатаАктуальности_Год",  Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
		Запрос.УстановитьПараметр("ДатаАктуальности_Месяц",Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
		Запрос.УстановитьПараметр("ДатаАктуальности_День", День(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
		...
		|	СотрудникиОрганизации.Пенсионер КАК Пенсионер,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СотрудникиОрганизации.ФизическоеЛицо КАК КолОбщее,
		|	ВЫБОР
		|		КОГДА СотрудникиОрганизации.ДоляНеполногоРабочегоВремени < 1 И СотрудникиОрганизации.Пол = &Пол
		|			ТОГДА ""НеполноеРВ""
		|	КОНЕЦ КАК ОбщееКолЖенНеполноеРабочееВремя,
		|	ВЫБОР
		|		КОГДА СотрудникиОрганизации.ДоляНеполногоРабочегоВремени < 1 
		|			ТОГДА ""НеполноеРВ""
		|	КОНЕЦ КАК ОбщееКолНеполноеРабочееВремя,
		|	ВЫБОР
		|		КОГДА СотрудникиОрганизации.Пол = &Пол
		|			ТОГДА ""Женский""
		|	КОНЕЦ КАК ОбщееКолЖен,
		|	ВЫБОР
		|		КОГДА ВЫБОР
		|				КОГДА ГОД(СотрудникиОрганизации.ДатаРождения) = 1
		|					ТОГДА 0
		|				ИНАЧЕ &ДатаАктуальности_Год - ГОД(СотрудникиОрганизации.ДатаРождения)
		|			КОНЕЦ МЕЖДУ 15 И 35
		|			ТОГДА ВЫБОР
		|					КОГДА МЕСЯЦ(СотрудникиОрганизации.ДатаРождения) * 100 + ДЕНЬ(СотрудникиОрганизации.ДатаРождения) < &ДатаАктуальности_Месяц * 100 + &ДатаАктуальности_День
		|						ТОГДА ВЫБОР
		|								КОГДА СотрудникиОрганизации.Пол = &Пол
		|									ТОГДА ""Женский""
		|							КОНЕЦ
		|				КОНЕЦ
		|	КОНЕЦ КАК КолЖен15_35,
		|	ВЫБОР
		|		КОГДА ВЫБОР
		|				КОГДА ГОД(СотрудникиОрганизации.ДатаРождения) = 1
		|					ТОГДА 0
		...
		|	КОЛИЧЕСТВО(ОбщееКолЖенНеполноеРабочееВремя),
		|	КОЛИЧЕСТВО(ОбщееКолНеполноеРабочееВремя),
		|	КОЛИЧЕСТВО(ОбщееКолЖен),
		|	КОЛИЧЕСТВО(КолЖен15_35),
		|	КОЛИЧЕСТВО(Кол15_35)
		|ПО
		|	ОБЩИЕ";
		...
		КолЖенНеполноеРабочееВремяОбщее = ВыборкаЗапроса.Итог("ОбщееКолЖенНеполноеРабочееВремя"); 
		КолНеполноеРабочееВремяОбщее = ВыборкаЗапроса.Итог("ОбщееКолНеполноеРабочееВремя"); 
		
		СтруктураДанныхУведомление.П12020_1 = КолНеполноеРабочееВремяОбщее;
		СтруктураДанныхУведомление.П12020_2 = КолЖенНеполноеРабочееВремяОбщее;
		...
		ПараметрыЗаполнения.ДатаАктуальности  = КонецМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		ПараметрыЗаполнения.ДатаНачала = СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета;
		ПараметрыЗаполнения.ДатаОкончания = СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета;
		ПараметрыЗаполнения.МесяцДатаНачала = НачалоМесяца(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
		ПараметрыЗаполнения.МесяцДатаОкончания = КонецМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		...
		МассивВидыВремениОтпускПоУходуЗаРебенком = Новый Массив();
		МассивВидыВремениОтпускПоУходуЗаРебенком.Добавить(Справочники.ВидыИспользованияРабочегоВремени.ОтпускПоУходуЗаРебенком);
		Запрос.УстановитьПараметр("ВидыВремениОтпускПоУходуЗаРебенком", МассивВидыВремениОтпускПоУходуЗаРебенком);
		...
		|ГДЕ
		|	ДанныеУчетаВремениПоСостояниям.ВидУчетаВремени В(&ВидыВремениДекретные)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	ПО ДанныеУчетаВремениПоСостояниям.Сотрудник = СотрудникиОрганизации.Сотрудник И СотрудникиОрганизации.Пол = &Пол
		|ГДЕ
		|	ДанныеУчетаВремениПоСостояниям.ВидУчетаВремени В(&ВидыВремениОтпускПоУходуЗаРебенком)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		КоличествоОтпускПоБеременностиИРодам = РезультатыЗапроса[0].Выгрузить().Количество();
		КоличествоОтпускПоУходуЗаРебенкомЖен = РезультатыЗапроса[1].Выгрузить().Количество();
		КоличествоОтпускПоУходуЗаРебенком = РезультатыЗапроса[2].Выгрузить().Количество();
		
		СтруктураДанныхУведомление.П12140_2 = КоличествоОтпускПоБеременностиИРодам;
		СтруктураДанныхУведомление.П12150_1 = КоличествоОтпускПоУходуЗаРебенком;
		СтруктураДанныхУведомление.П12150_2 = КоличествоОтпускПоУходуЗаРебенкомЖен;
	Иначе	
		...
		|	РаботникиОрганизацийСрезПоследних.ФизическоеЛицо КАК КолОбщее,
		|	ВЫБОР
		|		КОГДА РаботникиОрганизацийСрезПоследних.ФизическоеЛицо.Пол = &Пол
		|			ТОГДА ""Женский""
		|	КОНЕЦ КАК ОбщееКолЖен,
		|	ВЫБОР
		|		КОГДА ВЫБОР
		|				КОГДА ГОД(РаботникиОрганизацийСрезПоследних.ФизическоеЛицо.ДатаРождения) = 1
		|					ТОГДА 0
		|				ИНАЧЕ &ДатаАктуальности_Год - ГОД(РаботникиОрганизацийСрезПоследних.ФизическоеЛицо.ДатаРождения) 
		|			  КОНЕЦ МЕЖДУ 15 И 35
		|			ТОГДА ВЫБОР
		|                     КОГДА МЕСЯЦ(РаботникиОрганизацийСрезПоследних.ФизическоеЛицо.ДатаРождения) * 100 + ДЕНЬ(РаботникиОрганизацийСрезПоследних.ФизическоеЛицо.ДатаРождения) < &ДатаАктуальности_Месяц * 100 + &ДатаАктуальности_День
		|                       ТОГДА ВЫБОР
		|					          КОГДА РаботникиОрганизацийСрезПоследних.ФизическоеЛицо.Пол = &Пол  
		|							    ТОГДА ""Женский""
		|                              КОНЕЦ
		...
		|	КОНЕЦ КАК Кол15_35
		|ИЗ
		|" + ?(ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет"),"
		//ИспользоватьКадровыйУчет//
		|	РегистрСведений.КадроваяИсторияСотрудников.СрезПоследних(&НаДату, ГоловнаяОрганизация = &Организация)
		//ИспользоватьКадровыйУчет//
		|","
		...
		|		ПО РаботникиОрганизацийСрезПоследних.ФизическоеЛицо = СведенияОПенсионерахСрезПоследних.ФизЛицо
		|" + ?(ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет"),"
		//ИспользоватьКадровыйУчет//
		|ГДЕ НЕ   РаботникиОрганизацийСрезПоследних.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Увольнение)
		|		И РаботникиОрганизацийСрезПоследних.ВидЗанятости = &ОсновноеМестоРаботы
		//ИспользоватьКадровыйУчет//
		|","
		//НЕ ИспользоватьКадровыйУчет//
		|ГДЕ    РаботникиОрганизацийСрезПоследних.ГоловнаяОрганизация = &Организация
		|	    И НЕ РаботникиОрганизацийСрезПоследних.ДатаУвольнения = ДАТАВРЕМЯ(1,1,1)
		|		И РаботникиОрганизацийСрезПоследних.ТекущийВидЗанятости = &ОсновноеМестоРаботы
		//НЕ ИспользоватьКадровыйУчет//
		|") + "
		|ИТОГИ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ КолОбщее),
		...
		Запрос.УстановитьПараметр("НаДату",СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	...
	СтруктураДанныхУведомление.П12010_1 = КолОбщее;
	СтруктураДанныхУведомление.П12010_2 = КолЖенщин;
	
	Кол15_35    = ВыборкаЗапроса.Итог("Кол15_35");
	КолЖен15_35 = ВыборкаЗапроса.Итог("КолЖен15_35");
	
	
	СтруктураДанныхУведомление.П12030_1 = Кол15_35;
	СтруктураДанныхУведомление.П12030_2 = КолЖен15_35;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	ЗаполнитьПоказательНаСервере("П13010_1", 
								    ПолучитьПоказательНаСервере("П13020_1", Форма, "Уведомление")
								  + ПолучитьПоказательНаСервере("П13050_1", Форма, "Уведомление"),
								  Форма, "Уведомление");	
								  
	ЗаполнитьПоказательНаСервере("П13010_2", 
								    ПолучитьПоказательНаСервере("П13020_2", Форма, "Уведомление")
								  + ПолучитьПоказательНаСервере("П13050_2", Форма, "Уведомление"),
								  Форма, "Уведомление");	
								  
	ЗаполнитьПоказательНаСервере("П13010_3", 
								    ПолучитьПоказательНаСервере("П13020_3", Форма, "Уведомление")
								  + ПолучитьПоказательНаСервере("П13050_3", Форма, "Уведомление"),
								  Форма, "Уведомление");	
								  
	ЗаполнитьПоказательНаСервере("П13010_4", 
								    ПолучитьПоказательНаСервере("П13020_4", Форма, "Уведомление")
								  + ПолучитьПоказательНаСервере("П13050_4", Форма, "Уведомление"),
								  Форма, "Уведомление");	
								  
	ЗаполнитьПоказательНаСервере("П13010_5", 
								    ПолучитьПоказательНаСервере("П13020_5", Форма, "Уведомление")
								  + ПолучитьПоказательНаСервере("П13050_5", Форма, "Уведомление"),
								  Форма, "Уведомление");	
														 
								  
	ЗаполнитьПоказательНаСервере("П14010_1", 
								    ПолучитьПоказательНаСервере("П14020_1", Форма, "Уведомление")
								  + ПолучитьПоказательНаСервере("П14030_1", Форма, "Уведомление"),
								  Форма, "Уведомление");	
								  
	ЗаполнитьПоказательНаСервере("П14010_2", 
								    ПолучитьПоказательНаСервере("П14020_2", Форма, "Уведомление")
								  + ПолучитьПоказательНаСервере("П14030_2", Форма, "Уведомление"),
								  Форма, "Уведомление");	
								  
	ЗаполнитьПоказательНаСервере("П14010_3", 
								    ПолучитьПоказательНаСервере("П14020_3", Форма, "Уведомление")
								  + ПолучитьПоказательНаСервере("П14030_3", Форма, "Уведомление"),
								  Форма, "Уведомление");	
								  
	ЗаполнитьПоказательНаСервере("П14010_4", 
								    ПолучитьПоказательНаСервере("П14020_4", Форма, "Уведомление")
								  + ПолучитьПоказательНаСервере("П14030_4", Форма, "Уведомление"),
								  Форма, "Уведомление");	
								  
	ЗаполнитьПоказательНаСервере("П14010_5", 
								    ПолучитьПоказательНаСервере("П14020_5", Форма, "Уведомление")
								  + ПолучитьПоказательНаСервере("П14030_5", Форма, "Уведомление"),
								  Форма, "Уведомление");	
								  
	ЗаполнитьПоказательНаСервере("П14010_6", 
								    ПолучитьПоказательНаСервере("П14020_6", Форма, "Уведомление")
								  + ПолучитьПоказательНаСервере("П14030_6", Форма, "Уведомление"),
								  Форма, "Уведомление");	
								  
								  
	ЗаполнитьПоказательНаСервере("П14010_7", 
								    ПолучитьПоказательНаСервере("П14020_7", Форма, "Уведомление")
								  + ПолучитьПоказательНаСервере("П14030_7", Форма, "Уведомление"),
								  Форма, "Уведомление");	
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если  ВРЕГ(Лев(ИмяЯчейки, 6)) = "ЕДРПОУ" Тогда
		...
		Значение = ТабличныйДокумент.Области.ЕДРПОУ.Значение;
		...
		ПоказатьВводСтроки(ОписаниеОповещения,Значение, НСтр("ru='Введите значение';uk='Введіть значення'"), 9,Ложь);
