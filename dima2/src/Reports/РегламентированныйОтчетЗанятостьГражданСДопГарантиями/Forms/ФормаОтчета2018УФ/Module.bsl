...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	ПараметрыПолученияСотрудников.НачалоПериода = СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета;
	...
	|	И СотрудникиОрганизации.ДатаПриема <= &ДатаКон
	|	И (СотрудникиОрганизации.ДатаУвольнения > &ДатаКон ИЛИ СотрудникиОрганизации.ДатаУвольнения = ДАТАВРЕМЯ(1,1,1)) 
	|";
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации()  =  "ЕРП" Тогда
		Если УчетнаяЧисленность > 20 Тогда
			
			ТаблЧисленностиССЧ = КадровыйУчет.СреднесписочнаяЧисленностьРаботающих(СтруктураРеквизитовФормы.Организация, НачалоГода(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, -12)),КонецГода(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, -12))); 
			СтруктураДанныхУведомление.П011 = ТаблЧисленностиССЧ.ЧисленностьРаботников;
			СтруктураДанныхУведомление.П021 = ТаблЧисленностиССЧ.ЧисленностьЛицСГарантиямиНеИнвалиды;
			
		Иначе
			
			ТаблЧисленностиССЧ = КадровыйУчет.СреднесписочнаяЧисленностьРаботающих(СтруктураРеквизитовФормы.Организация, НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета),КонецГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета)); 
			СтруктураДанныхУведомление.П041 = ТаблЧисленностиССЧ.ЧисленностьРаботников;
			СтруктураДанныхУведомление.П051 = ТаблЧисленностиССЧ.ЧисленностьЛицСГарантиямиМеньше10ЛетДоПенсии;
	...
	Иначе	
		Если УчетнаяЧисленность > 20 Тогда
			...
			ПараметрыСредняяЧисленность.Вставить("НачалоПериода", НачалоГода(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, -12)));
			ПараметрыСредняяЧисленность.Вставить("КонецПериода", КонецГода(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, -12)));
			...
			СтруктураДанныхУведомление.П011 =  ТаблЧисленностиССЧ[0].Средняя;
			
			ПараметрыСредняяЧисленность.Вставить("ОтборДопГарантииНеИнвалиды", Истина);
			ТаблЧисленностиССЧ = КадровыйУчет.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.Организация, ПараметрыСредняяЧисленность); 
			СтруктураДанныхУведомление.П021 =  ТаблЧисленностиССЧ[0].Средняя;
		...
		Иначе
			...
			ПараметрыСредняяЧисленность.Вставить("КонецПериода", КонецГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета));
			ПараметрыСредняяЧисленность.Вставить("ВидСредней", "ССЧ") ;
			ПараметрыСредняяЧисленность.Вставить("ОтборШтат", Истина);
			ТаблЧисленностиССЧ = КадровыйУчет.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.Организация, ПараметрыСредняяЧисленность); 
			СтруктураДанныхУведомление.П041 =  ТаблЧисленностиССЧ[0].Средняя;
			
			ПараметрыСредняяЧисленность.Вставить("ОтборДопГарантии10Лет", Истина);
			ТаблЧисленностиССЧ = КадровыйУчет.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.Организация, ПараметрыСредняяЧисленность); 
			СтруктураДанныхУведомление.П051 =  ТаблЧисленностиССЧ[0].Средняя;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	ЗаполнитьПоказательНаСервере("П031", ПолучитьПоказательНаСервере("П011", Форма, "Уведомление") * 0.05, Форма, "Уведомление");
