...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "22/08/2011";
...
Процедура ЗаполнитьДатуПодписи(Форма)
	...
	ДАТАОТЧЕТА = "на  "+ Формат(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета,"ДЛФ=ДД;Л = uk_UA");
	ДАТАОТЧЕТА = Лев(ДАТАОТЧЕТА, СтрДлина(ДАТАОТЧЕТА) -2) + " року";    // р. -> року
	ЗаполнитьПоказательНаСервере("ДАТАОТЧЕТА",ДАТАОТЧЕТА,Форма,"Уведомление");
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("ЭЛПОЧТА", HEMAIL, Форма, "Уведомление");
	
	
	ЗаполнитьПоказательНаСервере("ЕДРПОУ",   	  КодЕДРПОУ,		Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("ФИОДиректора",  ФИОДиректора, 	Форма, "Уведомление");
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если Лев(ИмяЯчейки, 7) = "RXXXXG9" Тогда
		
		ТекЗначение = ТабличныйДокумент.Области[ИмяЯчейки].Значение;
		
		ТабличныйДокумент.Области[ИмяЯчейки].Значение  = ?(ЗначениеЗаполнено(ТекЗначение), "", "V");;
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |	ВЫБОР КОГДА ЕстьNULL(РаботникиОрганизаций.ПринятНаНовоеРабочееМесто, Ложь) = Истина ТОГДА ""V"" Иначе """" Конец КАК НовоеРМ
	               |ИЗ
				   |	РегистрСведений.КадроваяИсторияСотрудников КАК РаботникиОрганизаций
				   
				   |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизическихЛиц.СрезПоследних(&ДатаОкончания, ) КАК ФИОФизЛиц
				   |		ПО РаботникиОрганизаций.Сотрудник.Физическоелицо = ФИОФизЛиц.ФизическоеЛицо
				   
				   |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФизическиеЛица.КонтактнаяИнформация КАК КонтактнаяИнформацияАдрес
	               |		ПО РаботникиОрганизаций.Сотрудник.Физическоелицо = КонтактнаяИнформацияАдрес.Ссылка
	               ...
	               |		ПО РаботникиОрганизаций.Сотрудник.Физическоелицо = КонтактнаяИнформацияТелефон.Ссылка
	               ...
	               |ГДЕ
				   |	РаботникиОрганизаций.ГоловнаяОрганизация = &Организация
				   |	И РаботникиОрганизаций.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
	               |	И РаботникиОрганизаций.ВидЗанятости <> &ВидЗанятостиВнутрСовместитель
				   |	И РаботникиОрганизаций.ВидСобытия <> &ВидЗанятостиВнутрСовместитель";
	...
	Запрос.УстановитьПараметр("ВидКонтактнойИнформацииАдрес", Справочники.ВидыКонтактнойИнформации.АдресПоПропискеФизическиеЛица);
	...
	Если  РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "БП" Тогда
		Запрос.УстановитьПараметр("ВидКонтактнойИнформацииТелефон", Справочники.ВидыКонтактнойИнформации.ТелефонРабочийКонтактныеЛица);
	Иначе
		Запрос.УстановитьПараметр("ВидКонтактнойИнформацииТелефон", Справочники.ВидыКонтактнойИнформации.ТелефонРабочийФизическиеЛица);
	КонецЕсли;

	Запрос.УстановитьПараметр("ВидЗанятостиВнутрСовместитель", Перечисления.ВидыЗанятости.ВнутреннееСовместительство);
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Результат.Следующий() Цикл
			Если КвоСтрок = 0 Тогда
				ТекСтрока = СтруктураГруппы[0];	
			Иначе
				ТекСтрока = СтруктураГруппы.Добавить(); 		
			КонецЕсли;
			КвоСтрок = КвоСтрок + 1;
			
			ТекСтрока.RXXXXG1 = Результат.ФИО;      		   
			ТекСтрока.RXXXXG2 = Результат.ИНН;
			ТекСтрока.RXXXXG3 = Результат.Адрес;
			ТекСтрока.RXXXXG4 = Результат.Должность;
			ТекСтрока.RXXXXG5 = Результат.КодДолжности;
			ТекСтрока.RXXXXG6 = "№ " + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Результат.Приказ,Истина,Истина) + " від";
			ТекСтрока.RXXXXG6D = Результат.ДатаПриказа;
			ТекСтрока.RXXXXG7 = Результат.ДатаНачала;
			ТекСтрока.RXXXXG9 = Результат.НовоеРМ;
