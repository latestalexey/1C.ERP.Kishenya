...
&НаСервере
//Процедура выполняет расчет показателя первичной стоимости ОС на конесервации
...
&НаСервере
//Процедура выполняет расчет показателя первичной стоимости ОС на конесервации
Процедура  РасчетПервичнойСтоимостиОсНаКонсервации();
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр ("мДатаКонцаПериодаОтчета",Новый Граница(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, ВидГраницы.Включая));
	Запрос.УстановитьПараметр ("Организация",СтруктураРеквизитовФормы.Организация);
	Запрос.УстановитьПараметр ("НеНачислять",ЛОЖЬ);
    ...
    |СтоимостьОСБухгалтерскийУчетОстатки.ОсновноеСредство КАК ОсновноеСредство1,
	|СтоимостьОСБухгалтерскийУчетОстатки.СтоимостьОстаток - СтоимостьОСБухгалтерскийУчетОстатки.АмортизацияОстаток КАК КонОстаток
	|ИЗ
	|РегистрСведений.НачислениеАмортизацииОСБухгалтерскийУчет.СрезПоследних(&мДатаКонцаПериодаОтчета, Организация = &Организация) КАК НачислениеАмортизацииОСБухгалтерскийУчетСрезПоследних
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОСБухгалтерскийУчет.Остатки(&мДатаКонцаПериодаОтчета, Организация = &Организация) КАК СтоимостьОСБухгалтерскийУчетОстатки
	|ПО НачислениеАмортизацииОСБухгалтерскийУчетСрезПоследних.ОсновноеСредство = СтоимостьОСБухгалтерскийУчетОстатки.ОсновноеСредство
	|ГДЕ
	|НачислениеАмортизацииОСБухгалтерскийУчетСрезПоследних.НачислятьАмортизацию = &НеНачислять
	|ИТОГИ СУММА (КонОстаток) ПО
	|ОБЩИЕ";
	
	Запрос.Текст = ТекстЗапроса;
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТаблицаОСКонсервация = РезультатЗапроса.Выгрузить();
		УстановитьЗначениеПоказателя("П263", ТаблицаОСКонсервация[0].КонОстаток);
	Иначе
		УстановитьЗначениеПоказателя("П263", 0);
	КонецЕсли;
КонецПроцедуры	
