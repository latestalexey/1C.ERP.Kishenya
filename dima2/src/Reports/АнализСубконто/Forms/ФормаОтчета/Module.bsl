...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Анализ субконто %1 %2';uk='Аналіз субконто %1 %2'"),
		ТекстСубконто,
...
Процедура ЗаполнитьПризнакиУчета()
	...
	Запрос.Текст = 
		...
		|	ВС1.Ссылка КАК Счет,
		|	ВС1.Ссылка.УчетПоПодразделениям КАК УчетПоПодразделениям
		...
		|	ВС2.Ссылка,
		|	ВС2.Ссылка.УчетПоПодразделениям
		|ИЗ
		|	ПланСчетов.Хозрасчетный.ВидыСубконто КАК ВС2
		|ГДЕ
		...
		|	ВС3.Ссылка,
		|	ВС3.Ссылка.УчетПоПодразделениям
		|ИЗ
		|	ПланСчетов.Хозрасчетный.ВидыСубконто КАК ВС3
		|ГДЕ
		...
		|	МАКСИМУМ(ВидыСубконто.УчетПоПодразделениям) КАК УчетПоПодразделениям,
	...
	Иначе
		...
		Пока ВыборкаСчета.Следующий() Цикл
			...
			ЕстьУчетПоПодразделениям = ?(ВыборкаСчета.УчетПоПодразделениям = Истина, БухгалтерскийУчетПереопределяемый.ИспользоватьПодразделения(), ЕстьУчетПоПодразделениям);
