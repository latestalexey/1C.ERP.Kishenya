...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы9 = "D9T1R";
...
Функция СформироватьСтруктуруДанныхДопСтрокСтраницы(Форма, ИмяСтраницы) Экспорт
	...
	Если ИмяСтраницы = "ДекларацияАкциз" Тогда
		...
		РегламентированнаяОтчетностьКлиентСервер.ОбновитьСтруктуруМногострочногоРаздела(Форма, Форма.СтруктураРеквизитовФормы.мИдГруппы9);
		...
		ПромТаблицаДопСтрокРаздела = Неопределено;
		Форма.мСтруктураМногострочныхРазделов.Свойство(Форма.СтруктураРеквизитовФормы.мИдГруппы9, ПромТаблицаДопСтрокРаздела);
		ПромТаблицаДопСтрокРаздела = Форма[ПромТаблицаДопСтрокРаздела];
		ТаблицаДопСтрокГруппы = Форма["Хранилище" + Форма.СтруктураРеквизитовФормы.мИдГруппы9].Добавить().Таблица;
		КопироватьДанныеФормы(ПромТаблицаДопСтрокРаздела, ТаблицаДопСтрокГруппы);
		СтруктураДанныхДопСтрокРаздела.Вставить(Форма.СтруктураРеквизитовФормы.мИдГруппы9, ТаблицаДопСтрокГруппы);
...
Процедура ЗаполнитьДатуПодписи(Форма)
	...
	ЗаполнитьПоказательНаСервере("D1DATE1", Форма.ДатаПодписи, Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("D1DATE2", Форма.ДатаПодписи, Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("D9DATE1", Форма.ДатаПодписи, Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("D9DATE2", Форма.ДатаПодписи, Форма, "ДекларацияАкциз");
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	HTINJ = "";
	HTINF = "";
	Если Форма.СтруктураРеквизитовФормы.ОрганизацияЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
		ЗаполнитьПоказательНаСервере("HTINJ",  КодЕДРПОУ, Форма, "ДекларацияАкциз");	
		HTINJ = КодЕДРПОУ;
	Иначе	
		ЗаполнитьПоказательНаСервере("HTINF",  КодЕДРПОУ, Форма, "ДекларацияАкциз");	
		HTINF = КодЕДРПОУ;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(НалоговаяИнспекция) Тогда
		ЗаполнитьПоказательНаСервере("HSTI", НалоговаяИнспекция.Наименование, 	Форма, "ДекларацияАкциз");
	...
	ЗаполнитьПоказательНаСервере("DHBOS", ФИОДиректора, Форма, "ДекларацияАкциз");
	...
	ЗаполнитьПоказательНаСервере("DHBUH", ФИОГлавБуха,  Форма, "ДекларацияАкциз");
	...
	ЗаполнитьПоказательНаСервере("RE00G1",  "X",   Форма, "ДекларацияАкциз");
	
	ЗаполнитьСведенияОбОрганизацииПриложения6_8(Форма, 1, 1);
	
	// приложение 9
	ЗаполнитьПоказательНаСервере("H01G1S",  "Ґ",   Форма, "ДекларацияАкциз");
	
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияАкциз", "D9HZY_",  4, ГодОтчета, Ложь);	
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияАкциз", "D9HZM_",  2, МесяцОтчета, Ложь);	
	
	ЗаполнитьПоказательНаСервере("D9HBOS",  ФИОДиректора, Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("D9HBUH",  ФИОГлавБуха,  Форма, "ДекларацияАкциз");
	
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияАкциз", "D9HKBOS_",  10, ИННДиректора, Ложь);	
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияАкциз", "D9HKBUH_",  10, ИННГлавБуха, Ложь);	
...
Процедура ЗаполнитьСведенияОбОрганизацииПриложения6_8(Форма, НомПрил6 = Неопределено, НомПрил8 = Неопределено)
	...
	HTINJ = ПолучитьПоказательНаСервере("HTINJ", Форма, "ДекларацияАкциз");
	HTINF = ПолучитьПоказательНаСервере("HTINF", Форма, "ДекларацияАкциз");
	...
	Для НомПрил = НачНом По КонКом Цикл
		...
		ЗаполнитьПоказательНаСервере("D6H01G1S_" + ТекНомПрил,  "Ґ",   Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("D6H02G2S_" + ТекНомПрил,  "Ґ1",  Форма, "ДекларацияАкциз");
	...
	Для НомПрил = НачНом По КонКом Цикл
		...
		ЗаполнитьПоказательНаСервере("D8H01G1S_" + ТекНомПрил,  "Ґ",   Форма, "ДекларацияАкциз");
		...
		Для  Сч = 1 По 10 Цикл
			...
			Если Сч <= 8 Тогда
			    ТекЦифра = Сред(HTINJ, Сч, 1);
				ЗаполнитьПоказательНаСервере("D8HTINJ" + Сч + "_" + ТекНомПрил,  ТекЦифра,  Форма, "ДекларацияАкциз");	
			КонецЕсли;
			
		    ТекЦифра = Сред(HTINF, Сч, 1);
			ЗаполнитьПоказательНаСервере("D8HTINF" + Сч + "_" + ТекНомПрил,  ТекЦифра,  Форма, "ДекларацияАкциз");	
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяСтраницыПанели Тогда
		...
		ИдГрупп.Добавить(Форма.СтруктураРеквизитовФормы.мИдГруппы9);
	...
	Для каждого Приложение6 Из Форма.D6 Цикл
		...
		Итог = 0;
		Для НомКол = 1 По 11 Цикл
			
			Приложение6["D6R0" + НомКол + "0G5"] = Приложение6["D6R0" + НомКол + "0G3"] - Приложение6["D6R0" + НомКол + "0G4"];
			Приложение6["D6R0" + НомКол + "0G7"] = Приложение6["D6R0" + НомКол + "0G5"] * Приложение6["D6R0" + НомКол + "0G6"] / 100;
			
			Итог = Итог + ОкруглитьЧислоПоФормату(Форма, Приложение6["D6R0" + НомКол + "0G7"]);
			
            ЗаполнитьПоказательНаСервере("D6R0" + НомКол + "0G5_" + НомСтр, Приложение6["D6R0" + НомКол + "0G5"], Форма, "ДекларацияАкциз");
			ЗаполнитьПоказательНаСервере("D6R0" + НомКол + "0G7_" + НомСтр, Приложение6["D6R0" + НомКол + "0G7"], Форма, "ДекларацияАкциз");
			
		КонецЦикла;
		
		Приложение6["D6R01G7"] = Итог;
		ЗаполнитьПоказательНаСервере("D6R01G7_" + НомСтр, Итог, Форма, "ДекларацияАкциз");

	КонецЦикла;
	ЗаполнитьПоказательНаСервере("R06G1", "Х", Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("R06G2", Форма.D6.Количество(), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("R06G3", Форма.D6.Количество(), Форма, "ДекларацияАкциз");
	
	//////////////////////////
	// расчет тела декларации
	//////////////////////////
	
	ЗаполнитьПоказательНаСервере("RE0011G4",  Форма.D6.Итог("D6R010G7"), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("RE0012G4",  Форма.D6.Итог("D6R020G7"), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("RE0013G4",  Форма.D6.Итог("D6R030G7"), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("RE0014G4",  Форма.D6.Итог("D6R040G7"), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("RE0015G4",  Форма.D6.Итог("D6R050G7"), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("RE0016G4",  Форма.D6.Итог("D6R060G7"), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("RE0017G4",  Форма.D6.Итог("D6R070G7"), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("RE0018G4",  Форма.D6.Итог("D6R080G7"), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("RE0019G4",  Форма.D6.Итог("D6R090G7"), Форма, "ДекларацияАкциз");
	
	ЗаполнитьПоказательНаСервере("RE00110G4", Форма.D6.Итог("D6R0100G7"), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("RE00111G4", Форма.D6.Итог("D6R0110G7"), Форма, "ДекларацияАкциз");
	
	ЗаполнитьПоказательНаСервере("RE0010G4", Форма.D6.Итог("D6R01G7"), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("RE0020G4", Форма.D6.Итог("D6R01G7"), Форма, "ДекларацияАкциз");
	...
	Для каждого Приложение6 Из Форма.D6 Цикл
		...
		Форма.DT1R[НомСтрЧисло - 1].DT1RXXXXG2S = Приложение6.D6HKOATUU;
		Форма.DT1R[НомСтрЧисло - 1].DT1RXXXXG4  = Приложение6.D6R01G7;
		
		ЗаполнитьПоказательНаСервере("DT1RXXXXG2S_"+ НомСтр , Приложение6.D6HKOATUU, Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("DT1RXXXXG4_" + НомСтр , Приложение6.D6R01G7, Форма, "ДекларацияАкциз");
	...
	КонецЦикла;
	Если ЭтоУточненка ИЛИ  ЗначениеЗаполнено(ПолучитьПоказательНаСервере("HZMP", Форма, "ДекларацияАкциз")) Тогда
		ЗаполнитьПоказательНаСервере("R08G1", "Х", Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R08G2", Форма.D8.Количество(), Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R08G3", Форма.D8.Количество(), Форма, "ДекларацияАкциз");
	Иначе
		ЗаполнитьПоказательНаСервере("R08G1", "", Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R08G2",  0, Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R08G3",  0, Форма, "ДекларацияАкциз");
	КонецЕсли;
	
	//////////////////////////
	// расчет приложения 9
	//////////////////////////
	Если ЗначениеЗаполнено(ПолучитьПоказательНаСервере("D9T1RXXXXG2S_1", Форма, "ДекларацияАкциз")) Тогда
		ЗаполнитьПоказательНаСервере("R09G1","Х", Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R09G2", 1,  Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R09G3", 1,  Форма, "ДекларацияАкциз");
	Иначе
		ЗаполнитьПоказательНаСервере("R09G1", "", Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R09G2", 0,  Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R09G3", 0,  Форма, "ДекларацияАкциз");
	КонецЕсли;
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли  ИмяЯчейки = "HTINJ" Тогда
		
		Если НЕ СтруктураРеквизитовФормы.ОрганизацияЮридическоеФизическоеЛицо = ПредопределенноеЗначение("Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо") Тогда
			Возврат;
		КонецЕсли;
		
		Значение = ТабличныйДокумент.Области.HTINJ.Значение;
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
																				 "HTINJ",  "ДекларацияАкциз",
																				 "", "",
																				 8, Ложь, Неопределено));
		
		ПоказатьВводЧисла(ОписаниеОповещения,Значение, НСтр("ru='Введите значение';uk='Введіть значення'"), 8,0);
		
	ИначеЕсли  ИмяЯчейки = "HTINF" Тогда
		
		Если НЕ СтруктураРеквизитовФормы.ОрганизацияЮридическоеФизическоеЛицо = ПредопределенноеЗначение("Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо") Тогда
			Возврат;
		КонецЕсли;
		
		Значение = ТабличныйДокумент.Области.HTINF.Значение;
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
																				 "HTINF",  "ДекларацияАкциз",
																				 "", "",
																				 8, Ложь, Неопределено));
...
Процедура ВводПоказателяВПолеТабличногоДокументЗавершение(Значение, ДополнительныеПараметры) Экспорт
	...
	Если    ИмяЯчейки = "HTINJ"
		ИЛИ ИмяЯчейки = "HTINF" Тогда
	
		ЗаполнитьПоказательНаСервере("HTIN", Значение, ЭтаФорма, ДополнительныеПараметры.ИмяСтраницы);	
		ЗаполнитьПоказательНаСервере("TIN", Значение, ЭтаФорма, ДополнительныеПараметры.ИмяСтраницы);	
	...
	Если    Найти(ИмяЯчейки, "HKBOS") > 0 
		...
		РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(ЭтаФорма, "ДекларацияАкциз", "D9" + ИмяЯчейки + "_", 10, Значение, ДополнительныеПараметры.ДополнитьЗначение, ДополнительныеПараметры.НольКакНоль);
	...
	Если    Найти(ИмяЯчейки, "HBOS") > 0 
		...
		ЗаполнитьПоказательНаСервере("D9" + ИмяЯчейки, Значение, ЭтаФорма, ДополнительныеПараметры.ИмяСтраницы);
...
Функция ПолучитьДанныеДляЗаполнения(ТекКОАТУУ = Неопределено) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	РозничныеПродажиПодакцизныхТоваровОбороты.ДляКоммерческогоИспользования КАК ДляКоммерческогоИспользования,
	...
	ДеревоОтчетов.Колонки.Добавить("Месторасположение");
	
	// получим данные по месторасположению
	Если  РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП"  Тогда 
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               |	РозничныеПродажиПодакцизныхТоваровОбороты.Склад.ТерриторияПоКОАТУУ.НалоговаяИнспекция КАК НалоговаяИнспекция,
		               |	РозничныеПродажиПодакцизныхТоваровОбороты.Склад.Ссылка,
		               |	ЕСТЬNULL(СкладыКонтактнаяИнформация.Представление, """""""") КАК Местонахождение
		               |ИЗ
		               |	РегистрНакопления.РозничныеПродажиПодакцизныхТоваров.Обороты(&ДатаНач, &ДатаКон, , Организация = &Организация) КАК РозничныеПродажиПодакцизныхТоваровОбороты
		               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Склады.КонтактнаяИнформация КАК СкладыКонтактнаяИнформация
		               |		ПО РозничныеПродажиПодакцизныхТоваровОбороты.Склад.Ссылка = СкладыКонтактнаяИнформация.Ссылка
	...
	Иначе	
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               |	РозничныеПродажиПодакцизныхТоваровОбороты.Склад.ТерриторияПоКОАТУУ.НалоговаяИнспекция КАК НалоговаяИнспекция,
		               |	ВЫРАЗИТЬ(РозничныеПродажиПодакцизныхТоваровОбороты.Склад.Местонахождение КАК СТРОКА(254)) КАК Местонахождение
		               |ИЗ
		               |	РегистрНакопления.РозничныеПродажиПодакцизныхТоваров.Обороты(&ДатаНач, &ДатаКон, , Организация = &Организация) КАК РозничныеПродажиПодакцизныхТоваровОбороты
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	ВЫРАЗИТЬ(РозничныеПродажиПодакцизныхТоваровОбороты.Склад.Местонахождение КАК СТРОКА(254))
		               |ИТОГИ ПО
		               |	НалоговаяИнспекция";
	КонецЕсли;
					
	ВыборкаНалоговая = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);				
	Пока ВыборкаНалоговая.Следующий() Цикл
	
		НайденнаяСтрокаДерева = ДеревоОтчетов.Строки.Найти(ВыборкаНалоговая.НалоговаяИнспекция, "НалоговаяИнспекция", Ложь);	
		Если Не НайденнаяСтрокаДерева = Неопределено Тогда
			
			ТекМесторасположение = "";
			ВыборкаРасположение = ВыборкаНалоговая.Выбрать(ОбходРезультатаЗапроса.Прямой);
			Пока ВыборкаРасположение.Следующий() Цикл
				ТекМесторасположение = ТекМесторасположение + "; " + СокрЛП(ВыборкаРасположение.Местонахождение);
			КонецЦикла;
			
			НайденнаяСтрокаДерева.Месторасположение = Сред(ТекМесторасположение, 3);
...
Процедура ВнестиДанныеВДекларацию(ДеревоОтчетов)
	...
	Для каждого ДанныеВРазрезеНИ Из ДеревоОтчетов.Строки Цикл
		...
		СтруктураДанных.HSTI 		= НалоговаяИнспекция.Наименование;
		...
		СтруктураДанных.H07G3S 		= ДанныеВРазрезеНИ.Месторасположение;
		...
		Для каждого ДанныеВРазрезеКОАТУУ Из ДанныеВРазрезеНИ.Строки Цикл
			...
			Для каждого СтрокаПродаж Из ДанныеПоПродажам Цикл
				...
				Если СтрокаПродаж.ДляКоммерческогоИспользования = Истина Тогда
					ВнестиДанныеВПриложение6(СтрокаПрил6, СтрокаПродаж.СтатьяДекларации, Истина, СтрокаПродаж.База);
				КонецЕсли;
				
			КонецЦикла;
			// потом внесем данные по обычным продажам, при этом учтем продажи для комменрческого использования (добавим к обычным продажам)
			Для каждого СтрокаПродаж Из ДанныеПоПродажам Цикл
				
				Если НЕ СтрокаПродаж.ДляКоммерческогоИспользования = Истина Тогда
					ВнестиДанныеВПриложение6(СтрокаПрил6, СтрокаПродаж.СтатьяДекларации, Ложь, СтрокаПродаж.База);
				КонецЕсли;
		...
		Если НЕ СтруктураРеквизитовФормы.ПолучениеДанныхДляЗвит1С = Истина Тогда
			...
			Сообщить(НСтр("ru='Сформирована декларация в налоговую инспекцию: ';uk= 'Сформована декларація в податкову інспекцію: '") + НалоговаяИнспекция.Наименование + ", код: " + НалоговаяИнспекция.Код + ", район: " + НалоговаяИнспекция.КодАдмРайона, СтатусСообщения.Внимание);	
...
Процедура ВнестиДанныеВПриложение6(СтрокаПрил6, Статья, Коммерч, Сумма)
	...
	// ставка строго 5%
	СтрокаПрил6[ИмяСтроки + "6"] = 5.00;	
		
	Если Коммерч = Истина Тогда
		СтрокаПрил6[ИмяСтроки + "4"]= Сумма;
		СтрокаПрил6[ИмяСтроки + "3"]= Сумма;
	Иначе
...
Функция ПолучитьДанныеФормыНаСервере();
	...
	Для каждого Страница Из ТаблицаСтраницыРаздела1 Цикл
		...
		// приложение 9
		Таблица = Страница.ДанныеДопСтрок[0].Значение.D9T1R.Выгрузить();
		КолонкиТаблицы = Таблица.Колонки;
		СтруктураДанных = Новый Структура;
		...
		КонецЦикла;
		ПоказателиОтчета.Вставить("D9_" + НомСтрСтрока + "T1R", СтруктураДанных);		
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Для каждого Страница Из ТаблицаСтраницыРаздела1 Цикл
		...
		НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0209502", "F0209502");  
		...
		Для  Инд = 1 ПО Страница.ДанныеДопСтрок[0].Значение.D6.Количество() Цикл
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0295602", "F0295602");  
		...
		Для  Инд = 1 ПО Страница.ДанныеДопСтрок[0].Значение.D8.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0295803", "F0295803");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0295901", "F0295901");  
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение 9';uk='Додаток 9'");
		НовСтрока.НазваниеСтраницыПанели ="ДекларацияАкциз_" + НомСтрСтрока;
		НовСтрока.Префикс = "D9_" + НомСтрСтрока;
...
Функция СформироватьДанныеДляЗвит1С(ОтчетЗвит1С = Неопределено) Экспорт 
	...
	Если Сред(глМенеджерЗвит1С.ИмяСхемыЗвит1С,2,5) = "02095" Тогда
		
		глМенеджерЗвит1С.ПредупреждениеЗвит1С("Створіть додаток 6 до декларації, вкажіть в ньому код КОАТТУ торгівельної точки та заповніть додаток даними з 1С.
		|Потім оновіть поля декларації (F6, при включеному авторозрахунку) після заповнення всіх додатків 6!", "Завантаження даних з ""FREDO Звіт""");
		
	Иначе	
		
		ТекКОАТУУ = ОтчетЗвит1С.GetValue("MAIN", "SP_TO");
		
		
		Результат = ЗаполнитьДекларациюПоТекущемуКОАТУУ(ТекКОАТУУ);
		Если    Результат = Неопределено Тогда
			глМенеджерЗвит1С.ПредупреждениеЗвит1С("В 1С немає даних про продажу в торгових точках розташованих в КОАТУУ " + ТекКОАТУУ, "Завантаження даних з ""FREDO Звіт""");
		КонецЕсли;
