...
Процедура ЗаполнитьДеревоСобытий()
	...
	//++ НЕ УТ
	УзелДерева = ДобавленноеВДеревоВидСобытия(ВидыСобытий,"ВыполнениеГрафикаПроизводства", НСтр("ru='Выполнение графика';uk='Виконання графіка'"));
	Если ЕстьПравоЗаказМатериалов И ЕстьПравоРегистрЗаказыМатериалов Тогда
		ДобавленноеВДеревоВидСобытия(УзелДерева, "ЗаказМатериаловВПроизводство", НСтр("ru='Заказы материалов в производство';uk='Замовлення матеріалів у виробництво'"));
	КонецЕсли;
	Если ЕстьПравоПередачаМатериалов Тогда
		ДобавленноеВДеревоВидСобытия(УзелДерева, "ПередачаМатериаловВПроизводство", НСтр("ru='Передачи материалов в производство';uk='Передачі матеріалів у виробництво'"));
	КонецЕсли;
	УдалитьРодительскийУзелДереваЕслиПустой(УзелДерева);
	//-- НЕ УТ
