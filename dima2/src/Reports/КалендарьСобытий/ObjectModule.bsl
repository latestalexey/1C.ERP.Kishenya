...
Функция ТекстЗапросаПоВидуСобытия(Запрос,ВидСобытия,СтруктураФормированияОтчета)
	...
	//++ НЕ УТ
	ИначеЕсли ВидСобытия = "ЗаказМатериаловВПроизводство" Тогда
		Возврат ТекстЗапросаЗаказМатериаловВПроизводство(Запрос,СтруктураФормированияОтчета);
	ИначеЕсли ВидСобытия = "ПередачаМатериаловВПроизводство" Тогда
		Возврат ТекстЗапросаПередачаМатериаловВПроизводство(Запрос,СтруктураФормированияОтчета);
	//-- НЕ УТ
...
Функция ТекстЗапросаПрочиеЗадачиИсполнителя(Запрос,СтруктураФормированияОтчета)
	...
	//++ НЕ УТ
	|		И НЕ ЗадачаИсполнителя.Предмет ССЫЛКА Документ.ПередачаМатериаловВПроизводство
	|		И НЕ ЗадачаИсполнителя.Предмет ССЫЛКА Документ.ЗаказМатериаловВПроизводство
	//-- НЕ УТ
...
Функция ТекстЗапросаЗаказМатериаловВПроизводство(Запрос,СтруктураФормированияОтчета)

	ТекстЗапроса = "
	|ВЫБРАТЬ
	|	ДокументЗаказМатериаловВПроизводство.Ссылка КАК СсылкаНаОбъектСобытия,
	|	ДокументЗаказМатериаловВПроизводство.Ссылка КАК Предмет,
	|	&ПодготовитьКОтгрузке КАК ТипСобытия,
	|	ДокументЗаказМатериаловВПроизводство.Дата КАК ДатаОбъектаСобытия,
	|	МИНИМУМ(КОНЕЦПЕРИОДА(ЗаказМатериаловВПроизводствоТовары.ДатаОтгрузки,ДЕНЬ)) КАК ДатаСобытия,
	|	ДокументЗаказМатериаловВПроизводство.Представление КАК Описание,
	|	NULL КАК ДопРеквизитОписание1,
	|	NULL КАК ДопРеквизитОписание2,
	|	ВЫБОР
	|		КОГДА НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ) < ЗаказМатериаловВПроизводствоТовары.ДатаОтгрузки
	|			ТОГДА &Предстоящее
	|		КОГДА НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ) = ЗаказМатериаловВПроизводствоТовары.ДатаОтгрузки
	|			ТОГДА &Текущее
	|		ИНАЧЕ &Просроченное
	|	КОНЕЦ КАК Статус,
	|	NULL КАК Партнер,
	|	ДокументЗаказМатериаловВПроизводство.Ответственный КАК Ответственный
	|%ТекстПоместить%
	|ИЗ
	|	Документ.ЗаказМатериаловВПроизводство.Товары КАК ЗаказМатериаловВПроизводствоТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказМатериаловВПроизводство КАК ДокументЗаказМатериаловВПроизводство
	|		ПО ЗаказМатериаловВПроизводствоТовары.Ссылка = ДокументЗаказМатериаловВПроизводство.Ссылка
	|ГДЕ
	|	ДокументЗаказМатериаловВПроизводство.Проведен
	|	И ДокументЗаказМатериаловВПроизводство.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовМатериаловВПроизводство.КОбеспечению)
	|	И ЗаказМатериаловВПроизводствоТовары.ДатаОтгрузки < &ДатаОкончания  %ТекстУсловияОтборДокумент%
	|
	|СГРУППИРОВАТЬ ПО
	|	ДокументЗаказМатериаловВПроизводство.Ссылка,
	|	ДокументЗаказМатериаловВПроизводство.Дата,
	|	ДокументЗаказМатериаловВПроизводство.Представление,
	|	ДокументЗаказМатериаловВПроизводство.Ответственный,
	|	ВЫБОР
	|		КОГДА НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ) < ЗаказМатериаловВПроизводствоТовары.ДатаОтгрузки
	|			ТОГДА &Предстоящее
	|		КОГДА НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ) = ЗаказМатериаловВПроизводствоТовары.ДатаОтгрузки
	...
	|	ДокументЗаказМатериаловВПроизводство.Ссылка КАК СсылкаНаОбъектСобытия,
	|	ДокументЗаказМатериаловВПроизводство.Ссылка КАК Предмет,
	|	&ВыполнитьЗаказМатериаловВПроизводство КАК ТипСобытия,
	|	ДокументЗаказМатериаловВПроизводство.Дата КАК ДатаОбъектаСобытия,
	|	КОНЕЦПЕРИОДА(ЗаказМатериаловВПроизводствоТовары.ДатаОтгрузки,ДЕНЬ) КАК ДатаСобытия,
	|	ЗаказыМатериаловВПроизводствоОстатки.Распоряжение.Представление КАК Описание,
	|	NULL КАК ДопРеквизитОписание1,
	|	NULL КАК ДопРеквизитОписание2,
	|	ВЫБОР
	|		КОГДА НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ) < ЗаказМатериаловВПроизводствоТовары.ДатаОтгрузки
	...
	|			,
	|			Распоряжение.Товары.ДатаОтгрузки < &ДатаОкончания
	|				И Распоряжение.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовМатериаловВПроизводство.КВыполнению) %ТекстУсловияОтбор%) КАК ЗаказыМатериаловВПроизводствоОстатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказМатериаловВПроизводство.Товары КАК ЗаказМатериаловВПроизводствоТовары
	|		ПО ЗаказыМатериаловВПроизводствоОстатки.Распоряжение = ЗаказМатериаловВПроизводствоТовары.Ссылка
	|			И ЗаказыМатериаловВПроизводствоОстатки.КодСтроки = ЗаказМатериаловВПроизводствоТовары.КодСтроки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказМатериаловВПроизводство КАК ДокументЗаказМатериаловВПроизводство
	|		ПО ЗаказыМатериаловВПроизводствоОстатки.Распоряжение = ДокументЗаказМатериаловВПроизводство.Ссылка
	|ГДЕ
	|	ЗаказыМатериаловВПроизводствоОстатки.КОформлениюОстаток > 0 
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ДокументЗаказМатериаловВПроизводство.Ссылка КАК СсылкаНаОбъектСобытия,
	|	ДокументЗаказМатериаловВПроизводство.Ссылка КАК Предмет,
	|	&ЗакрытьЗаказМатериаловВПроизводство КАК ТипСобытия,
	|	ДокументЗаказМатериаловВПроизводство.Дата КАК ДатаОбъектаСобытия,
	|	КОНЕЦПЕРИОДА(&ТекущаяДата, ДЕНЬ) КАК ДатаСобытия,
	|	ДокументЗаказМатериаловВПроизводство.Представление КАК Описание,
	|	NULL КАК ДопРеквизитОписание1,
	...
	|	ДокументЗаказМатериаловВПроизводство.Ответственный КАК Ответственный
	|ИЗ
	|	Документ.ЗаказМатериаловВПроизводство КАК ДокументЗаказМатериаловВПроизводство
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыМатериаловВПроизводство.Остатки(, Распоряжение.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовМатериаловВПроизводство.КВыполнению) %ТекстУсловияОтбор%) КАК ЗаказыМатериаловВПроизводствоОстатки
	|		ПО ЗаказыМатериаловВПроизводствоОстатки.Распоряжение = ДокументЗаказМатериаловВПроизводство.Ссылка
	|ГДЕ
	|	ЕСТЬNULL(ЗаказыМатериаловВПроизводствоОстатки.КОформлениюОстаток, 0) = 0 
	|	И ДокументЗаказМатериаловВПроизводство.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовМатериаловВПроизводство.КВыполнению) 
	...
	Запрос.УстановитьПараметр("ВыполнитьЗаказМатериаловВПроизводство",НСтр("ru='Выполнить заказ';uk='Виконати замовлення'"));
	Запрос.УстановитьПараметр("ЗакрытьЗаказМатериаловВПроизводство",НСтр("ru='Закрыть заказ';uk='Закрити замовлення'"));
	
	ДобавитьЗапросПоЗадачам(ТекстЗапроса,"Документ.ЗаказМатериаловВПроизводство",СтруктураФормированияОтчета);
	
	Если НЕ ПустаяСтрока(СтруктураФормированияОтчета.ТекстУсловияПоПользователям) Тогда
		
		ТекстУсловияОтбор ="
		|И Распоряжение.Ответственный" + СтруктураФормированияОтчета.ТекстУсловияПоПользователям;
		
		ТекстУсловияОтборДокумент ="
		|И ДокументЗаказМатериаловВПроизводство.Ответственный" + СтруктураФормированияОтчета.ТекстУсловияПоПользователям;
	...
	ДобавитьВМассивПроверяемыхНаПредметДоступаТипов(СтруктураФормированияОтчета,"Документ.ЗаказМатериаловВПроизводство");
	
	Возврат ТекстЗапроса;

КонецФункции

Функция ТекстЗапросаПередачаМатериаловВПроизводство(Запрос,СтруктураФормированияОтчета)

	ТекстЗапроса = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПередачаМатериаловВПроизводство.Ссылка КАК СсылкаНаОбъектСобытия,
	|	ПередачаМатериаловВПроизводство.Ссылка КАК Предмет,
	|	&ПроконтролироватьОтгрузкуПередачаМатериалов КАК ТипСобытия,
	|	ПередачаМатериаловВПроизводство.Дата КАК ДатаОбъектаСобытия,
	|	КОНЕЦПЕРИОДА(&ТекущаяДата, ДЕНЬ) КАК ДатаСобытия,
	|	ПередачаМатериаловВПроизводство.Представление КАК Описание,
	|	NULL КАК ДопРеквизитОписание1,
	|	NULL КАК ДопРеквизитОписание2,
	|	&Текущее КАК Статус,
	|	NULL КАК Партнер,
	|	ПередачаМатериаловВПроизводство.Ответственный КАК Ответственный
	|%ТекстПоместить%
	|ИЗ
	|	Документ.ПередачаМатериаловВПроизводство КАК ПередачаМатериаловВПроизводство
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Склады КАК Склады
	|		ПО ПередачаМатериаловВПроизводство.Склад = Склады.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыКОтгрузке.Остатки(, ДокументОтгрузки ССЫЛКА Документ.ПередачаМатериаловВПроизводство %ТекстУсловияОтборДокументОтгрузки%) КАК ТоварыКОтгрузкеОстатки
	|		ПО ТоварыКОтгрузкеОстатки.ДокументОтгрузки = ПередачаМатериаловВПроизводство.Ссылка
	|ГДЕ
	|	ПередачаМатериаловВПроизводство.Проведен
	|	И ВЫБОР
	|			КОГДА Склады.ИспользоватьОрдернуюСхемуПриОтгрузке
	|						И ПередачаМатериаловВПроизводство.Дата >= Склады.ДатаНачалаОрдернойСхемыПриОтгрузке
	|				ТОГДА ТоварыКОтгрузкеОстатки.КОтгрузкеОстаток > 0
	|						ИЛИ ТоварыКОтгрузкеОстатки.СобираетсяОстаток > 0
	|						ИЛИ ТоварыКОтгрузкеОстатки.СобраноОстаток > 0
	|			ИНАЧЕ ИСТИНА
	...
	Запрос.УстановитьПараметр("ПроконтролироватьОтгрузкуПередачаМатериалов",НСтр("ru='Проконтролировать отгрузку';uk='Проконтролювати відвантаження'"));
	
	ДобавитьЗапросПоЗадачам(ТекстЗапроса,"Документ.ПередачаМатериаловВПроизводство",СтруктураФормированияОтчета);
	
	Если НЕ ПустаяСтрока(СтруктураФормированияОтчета.ТекстУсловияПоПользователям) Тогда
		
		ТекстУсловияОтбор ="
		|И ПередачаМатериаловВПроизводство.Ответственный" + СтруктураФормированияОтчета.ТекстУсловияПоПользователям;
		ТекстУсловияОтборДокументОтгрузки ="
		|И ДокументОтгрузки.Ответственный" + СтруктураФормированияОтчета.ТекстУсловияПоПользователям;
	...
	ДобавитьВМассивПроверяемыхНаПредметДоступаТипов(СтруктураФормированияОтчета,"Документ.ПередачаМатериаловВПроизводство");
