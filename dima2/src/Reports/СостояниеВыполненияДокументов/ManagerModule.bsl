...
Процедура СформироватьОтчетСостояниеВыполненияДокументов(ВходящиеДокументы, ТаблицаОтчета) Экспорт
	...
	Если СтруктураПараметров.ВыводитьТаблицуУслугДавальцуКОформлению <> 0
		...
		ВсегоТаблиц = ВсегоТаблиц + 1;
	КонецЕсли;
	Если СтруктураПараметров.ВыводитьТаблицуПродукцияКОформлениюВОтчетеПереработчику <> 0 Тогда
		ТекстыЗапроса.Добавить(ТекстЗапросаТаблицаПродукцияКОформлениюВОтчетеПереработчику(), "ТаблицаПродукцияКОформлениюВОтчетеПереработчику");
		ВсегоТаблиц = ВсегоТаблиц + 1;
	КонецЕсли;
	//-- НЕ УТКА
	...
	//++ НЕ УТ
	Если СтруктураПараметров.ВыводитьТаблицуСырьеУПереработчика <> 0 Тогда
		
		ТекстЗапросаТаблицаСырьеУПереработчика(ТекстыЗапроса);
...
Функция ТекстЗапросаВтОбъектыРасчетов(ИмяТаблицы, СтруктураПараметров)
	...
	Если ИмяТаблицы = "Документ.ОтчетДавальцу" Тогда
		ТекстЗапроса = ТекстЗапроса + "
			...
			|	Истина
			|ИЗ
			|	Документ.ОтчетДавальцу.Продукция КАК Документ
			|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДокументы КАК ВтДокументы
			|		ПО Документ.Ссылка = ВтДокументы.ДокументСсылка
			|ГДЕ
			|	Документ.Ссылка.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
...
Функция ТекстЗапросаТаблицаГрафикиОплаты(ИмяТаблицы, СтруктураПараметров)
	...
	Если СтруктураПараметров.ЭтоЗаказ Тогда
		...
		//++ НЕ УТ
		Если ИмяТаблицы = "Документ.ЗаказПереработчику" Тогда
			СуммаПлатежа = "ДокументЭтапыГрафикаОплаты.СуммаПлатежа";
		КонецЕсли; 
		//-- НЕ УТ
		
		//++ НЕ УТКА
		Если ИмяТаблицы = "Документ.ЗаказДавальца" Тогда
			СуммаПлатежа = "ДокументЭтапыГрафикаОплаты.СуммаПлатежа";
		КонецЕсли; 
		//-- НЕ УТКА
...
Функция ТекстЗапросаТаблицаУслугиДавальцуКОформлению()
	...
	|	УслугиДавальцуКОформлениюОстатки.КОформлениюОстаток * УслугиДавальцуКОформлениюОстатки.СуммаОстаток / ЕСТЬNULL(УслугиДавальцуКОформлениюОстатки.ЗаказаноОстаток, 1) КАК Сумма,
	|	ВложенныйЗапрос.НомерСтроки КАК НомерСтроки
	|ИЗ
	|	РегистрНакопления.УслугиДавальцуКОформлению.Остатки(, ) КАК УслугиДавальцуКОформлениюОстатки
	...
	|			ЗаказДавальцаПродукция.Ссылка) КАК ВложенныйЗапрос
...
Функция ТекстЗапросаТаблицаСырьеУПереработчика(ТекстыЗапроса)
	...
	|	ВЫРАЗИТЬ(АналитикаНоменклатуры.Склад КАК Справочник.Партнеры) КАК Переработчик
	|ПОМЕСТИТЬ АналитикаНоменклатуры
	|ИЗ
	|	РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаНоменклатуры
	|ГДЕ
	|	(АналитикаНоменклатуры.Номенклатура, АналитикаНоменклатуры.Характеристика, АналитикаНоменклатуры.Склад) В
	|			(ВЫБРАТЬ
	|				ДокументТовары.Номенклатура,
	|				ДокументТовары.Характеристика,
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ДокументТовары.Переработчик КАК Переработчик,
	|		ДокументТовары.Номенклатура КАК Номенклатура,
	|		ДокументТовары.Характеристика КАК Характеристика,
	|		ДокументТовары.Количество КАК Заказано,
	|		0 КАК Оформлено
	|	ИЗ
	...
	|			ПО (ОтчетПереработчика.ЗаказПереработчику = ВтДокументы.ДокументСсылка)
...
Функция ВывестиТаблицуПродукцияКОформлениюВОтчетеПереработчику(ТаблицаОтчета, Макет, Документ, ТаблицыРезультатов, СтруктураПараметров)
	...
	ИмяОбласти = "ЗаголовокТаблицы";
	Область = Макет.ПолучитьОбласть(ИмяОбласти);
	Заголовок = НСтр("ru='Продукция к оформлению в отчете переработчику (%Кол-во%)';uk='Продукція до оформлення в звіті переробнику (%Кол-во%)'");
	Область.Параметры.Заголовок = СтрЗаменить(Заголовок, "%Кол-во%", ПродукцияКОформлению.Строки.Количество());
	ТаблицаОтчета.Вывести(Область);
	
	ТаблицаОтчета.НачатьГруппуСтрок();
	
	ИмяОбласти = "ШапкаТаблицыКОформлениюВОтчетеПереработчику";
	Область = Макет.ПолучитьОбласть(ИмяОбласти);
	Область.Параметры.ТекстТоварУслуга = СтруктураПараметров.ТекстТоварУслугаПоступление;
	ТаблицаОтчета.Вывести(Область);
	
	ИмяОбласти = "СтрокаТаблицыКОформлениюВОтчетеПереработчику";
	Область = Макет.ПолучитьОбласть(ИмяОбласти);
	
	ИмяОбластьСтроки = "ТаблицаКОформлениюВОтчетеПереработчикуСтрока";
	ОбластьСтроки = Область.Области[ИмяОбластьСтроки];
...
Функция ВывестиТаблицуУслугиДавальцуКОформлению(ТаблицаОтчета, Макет, Документ, ТаблицыРезультатов, СтруктураПараметров)
	...
	ИмяОбласти = "ЗаголовокТаблицы";
	Область = Макет.ПолучитьОбласть(ИмяОбласти);
	Заголовок = НСтр("ru='Услуги давальцу к оформлению (%Кол-во%)';uk='Послуги давальцю до оформлення (%Кол-во%)'");
	Область.Параметры.Заголовок = СтрЗаменить(Заголовок, "%Кол-во%", УслугиДавальцуКОформлению.Строки.Количество());
	ТаблицаОтчета.Вывести(Область);
...
Функция ВывестиТаблицуСырьеУПереработчика(ТаблицаОтчета, Макет, Документ, ТаблицыРезультатов, СтруктураПараметров)
	...
	ИмяОбласти = "ЗаголовокТаблицы";
	Область = Макет.ПолучитьОбласть(ИмяОбласти);
	Заголовок = НСтр("ru='Сырье и материалы у переработчика';uk='Сировина і матеріали у переробника'");
	Область.Параметры.Заголовок = Заголовок;
	ТаблицаОтчета.Вывести(Область);
	
	ТаблицаОтчета.НачатьГруппуСтрок();
	
	ИмяОбласти = "ШапкаТаблицыСырьеУПереработчика";
	Область = Макет.ПолучитьОбласть(ИмяОбласти);
	Область.Параметры.ТекстТоварУслуга = СтруктураПараметров.ТекстТоварУслугаОтгрузка;
	ТаблицаОтчета.Вывести(Область);
	
	ИмяОбласти = "СтрокаТаблицыСырьеУПереработчика";
	Область = Макет.ПолучитьОбласть(ИмяОбласти);
	
	ИмяОбластьСтроки = "ТаблицаСырьеУПереработчикаСтрока";
	ОбластьСтроки = Область.Области[ИмяОбластьСтроки];
...
Функция ЕстьПравоДоступаКНаборуДанных(ИмяНабораДанных, СтруктураПараметров)
	...
	ИначеЕсли ИмяНабораДанных = "УслугиДавальцуКОформлению" Тогда
		...
		ЕстьПравоДоступа = ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.УслугиДавальцуКОформлению);
...
Функция ПолучитьМассивСтатусовБезОтгрузки(ИмяТаблицы)
	...
	//++ НЕ УТ
	МассивСтатусов.Добавить(Перечисления.СтатусыЗаказовМатериаловВПроизводство.КОбеспечению);
