...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = НСтр("ru='Декларация плательщика единого налога (юр. лица)';uk='Декларація платника єдиного податку (юр. особи)'");
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("HKBOS", ИННДиректора, Форма, "ДекларацияЕН");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияЕН", "HKBOS_",  10, ИННДиректора, Ложь);	
	
	ЗаполнитьПоказательНаСервере("HBUH",  ФИОГлавБуха,  Форма, "ДекларацияЕН");
	ЗаполнитьПоказательНаСервере("HKBUH", ИННГлавБуха,  Форма, "ДекларацияЕН");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияЕН", "HKBUH_",  10, ИННГлавБуха, Ложь);	
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияЕН", "HTIN_",  8, КодЕДРПОУ, Ложь);	
	
	ЗаполнитьПоказательНаСервере("HKVED", КодКВЕД, Форма, "ДекларацияЕН");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияЕН", "HKVED_", 7, КодКВЕД, Ложь);	
	
	ЗаполнитьПоказательНаСервере("HKOPFG", КодОПФГ, Форма, "ДекларацияЕН");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияЕН", "HKOPFG_",4, КодОПФГ, Ложь);	
	
	ЗаполнитьПоказательНаСервере("HLOC", ЮридическийАдрес, Форма, "ДекларацияЕН");
	
	//HEMAIL  = БухгалтерскийУчетПереопределяемый.ПолучитьEmailИзКонтактнойИнформации(Организация);	
	ЗаполнитьПоказательНаСервере("HEMAIL", HEMAIL, Форма, "ДекларацияЕН");
		...
		Телефон	  = Телефон.Номер;	ОставитьТолькоЦифры(Телефон);
		Факс	  = Факс.Номер;		ОставитьТолькоЦифры(Факс);
		
	ЗаполнитьПоказательНаСервере("HINTURB", КодГорода, Форма, "ДекларацияЕН");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияЕН", "HINTURB_",  5, КодГорода, Ложь);	
	
	ЗаполнитьПоказательНаСервере("HTEL", Телефон, Форма, "ДекларацияЕН");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияЕН", "HTEL_",  7, Телефон, Ложь);	
	
	ЗаполнитьПоказательНаСервере("HFAX", Факс, Форма, "ДекларацияЕН");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияЕН", "HFAX_",  7, Факс, Ложь);	
	
	ЗаполнитьПоказательНаСервере("HZIP", Индекс, Форма, "ДекларацияЕН");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияЕН", "HZIP_",  5, Индекс, Ложь);	
		
	
	// налоговая инспекция из справочника налоговых инспекций
	Если ЗначениеЗаполнено(НалоговаяИнспекция) Тогда
		ЗаполнитьПоказательНаСервере("HSTI", НалоговаяИнспекция.Наименование, 	Форма, "ДекларацияЕН");
	...
	ТекНомерВПериоде = ПолучитьСледующийНомерДокументаВПериоде(Форма); 
	ЗаполнитьПоказательНаСервере("HNY", ТекНомерВПериоде, Форма, "ДекларацияЕН");	
...
&НаСервереБезКонтекста
Функция ПолучитьСледующийНомерДокументаВПериоде(Форма)

	Организация = Форма.СтруктураРеквизитовФормы.Организация;
	
	НачГода = НачалоГода(Форма.СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
	КонГода = КонецГода(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	
	ИмяОтчета = Метаданные.Отчеты[Сред(Лев(СтрЗаменить(Форма.ИмяФормы, "Внешний", ""), Найти(СтрЗаменить(Форма.ИмяФормы, "Внешний", ""), ".Форма.") - 1), 7)].Имя;
	...
	Запрос.УстановитьПараметр("КонГода", КонГода);
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	R005G3 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R001G3", Форма, "ДекларацияЕН"))
	...
	ЗаполнитьПоказательНаСервере("R005G3", R005G3, Форма, "ДекларацияЕН");
	
	R005G4 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R001G4", Форма, "ДекларацияЕН"))
												  + ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R002G4", Форма, "ДекларацияЕН"))
												  + ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R003G4", Форма, "ДекларацияЕН"))
												  + ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R004G4", Форма, "ДекларацияЕН"));
	ЗаполнитьПоказательНаСервере("R005G4", R005G4, Форма, "ДекларацияЕН");
	
	R006G3 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R001G3", Форма, "ДекларацияЕН")) * 0.02;
	ЗаполнитьПоказательНаСервере("R006G3", R006G3, Форма, "ДекларацияЕН");
	
	R006G4 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R001G4", Форма, "ДекларацияЕН")) * 0.04;
	ЗаполнитьПоказательНаСервере("R006G4", R006G4, Форма, "ДекларацияЕН");
	
	R007G3 =  (ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R002G3", Форма, "ДекларацияЕН"))
			  + ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R003G3", Форма, "ДекларацияЕН"))
			  + ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R004G3", Форма, "ДекларацияЕН"))) * 0.02 * 2;
	ЗаполнитьПоказательНаСервере("R007G3", R007G3, Форма, "ДекларацияЕН");
												  
	R007G4 =  (ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R002G4", Форма, "ДекларацияЕН"))
			+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R003G4", Форма, "ДекларацияЕН"))
			+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R004G4", Форма, "ДекларацияЕН"))) * 0.04 * 2;
	ЗаполнитьПоказательНаСервере("R007G4", R007G4, Форма, "ДекларацияЕН");

	R008G3 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R006G3", Форма, "ДекларацияЕН"))
	...
	R008G4 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R006G4", Форма, "ДекларацияЕН"))
			+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R007G4", Форма, "ДекларацияЕН"));
	ЗаполнитьПоказательНаСервере("R008G4", R008G4, Форма, "ДекларацияЕН");
	
	
	R010G3 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R008G3", Форма, "ДекларацияЕН"))
			- ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R009G3", Форма, "ДекларацияЕН"));
	ЗаполнитьПоказательНаСервере("R010G3", R010G3, Форма, "ДекларацияЕН");
												  
	R010G4 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R008G4", Форма, "ДекларацияЕН"))
			- ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R009G4", Форма, "ДекларацияЕН"));
	ЗаполнитьПоказательНаСервере("R010G4", R010G4, Форма, "ДекларацияЕН");
	
	R011G3 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R011G3", Форма, "ДекларацияЕН"));
	R012G3 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R012G3", Форма, "ДекларацияЕН"));
	Если R012G3 > R011G3 Тогда
		R013G3 = R012G3 - R011G3;	
		R014G3 = 0;
	Иначе	
		R013G3 = 0;	
		R014G3 = - (R012G3 - R011G3);	
	КонецЕсли;
	ЗаполнитьПоказательНаСервере("R013G3", R013G3, Форма, "ДекларацияЕН");
	...
	КонецЕсли;		
	ЗаполнитьПоказательНаСервере("R015G3", R015G3, Форма, "ДекларацияЕН");
	ЗаполнитьПоказательНаСервере("R016G3", R016G3, Форма, "ДекларацияЕН");
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 4)) = "HKVE" Тогда
	...
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 4)) = "HKOP" Тогда
		
		Значение = ТабличныйДокумент.Области.HKOPFG.Значение;
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
																				 "HKOPFG",  "ДекларацияЕН",
																				 "HKOPFG_", "ДекларацияЕН",
																				 4, Ложь, Неопределено));
	...
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 4)) = "HINT" Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
																				 "HINTURB",  "ДекларацияЕН",
																				 "HINTURB_", "ДекларацияЕН",
																				 5, Ложь, Неопределено));
		
		ПоказатьВводСтроки(ОписаниеОповещения,Значение, НСтр("ru='Введите значение';uk='Введіть значення'"), 5);
		
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 4)) = "HTEL" Тогда
	...
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 4)) = "HFAX" Тогда
	...
	ИначеЕсли ВРЕГ(Лев(ИмяЯчейки, 5)) = "HKBOS" Тогда
	...
	ИначеЕсли ВРЕГ(Лев(ИмяЯчейки, 5)) = "HKBUH" Тогда
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Если ОграничениеСуммыДоходаЕдинщика.Количество() > 0 Тогда
		ОграничениеСуммыДоходаЕдинщика = ОграничениеСуммыДоходаЕдинщика[0].ОбъемДоходаЕНТретьяГруппа;
	Иначе	
	...
	ДоходПревышениеМаксимумаНДС = 0;
	ДоходДругойСпособРасчетовНДС  = 0;
	ДоходВДНеИзСвидетельстваНДС  = 0;
	
	ДоходПревышениеМаксимумаНеНДС  = 0;
	ДоходДругойСпособРасчетовНеНДС = 0;
	ДоходВДНеИзСвидетельстваНеНДС = 0;
	...
	Запрос.УстановитьПараметр("Группа", Перечисления.ГруппыПлательщиковЕдиногоНалога.ТретьяГруппа);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если    Выборка.Статья = СтрокиДекларации.ЕННК_ДоходыРеализация
			...
			Иначе	
				...
				Если Выборка.ПлательщикНДС = Истина Тогда
					...
					ДоходПревышениеМаксимумаНДС = ДоходПревышениеМаксимумаНДС + ПревышениеДохода;
				Иначе
					ДоходНеНДС = ДоходНеНДС + Выборка.Сумма - ПревышениеДохода;	
					ДоходПревышениеМаксимумаНеНДС = ДоходПревышениеМаксимумаНеНДС + ПревышениеДохода;
				КонецЕсли;
		...
		ИначеЕсли Выборка.Статья = СтрокиДекларации.ЕННК_ДоходыДоходыДругойСпособРасчетов Тогда
			...
			Если Выборка.ПлательщикНДС = Истина Тогда
				ДоходДругойСпособРасчетовНДС = ДоходДругойСпособРасчетовНДС + Выборка.Сумма;	
			Иначе
				ДоходДругойСпособРасчетовНеНДС = ДоходДругойСпособРасчетовНеНДС + Выборка.Сумма;					
			КонецЕсли;
			
		ИначеЕсли Выборка.Статья = СтрокиДекларации.ЕННК_ДоходыДоходыПоВидуДеятельностиНеЕдиныйНалог
			  ИЛИ Выборка.Статья = СтрокиДекларации.ЕННК_ДоходыДоходыПоВидуДеятельностиНеУказанномуВСвидетельстве Тогда	
			  
			Если Выборка.ПлательщикНДС = Истина Тогда
				ДоходВДНеИзСвидетельстваНДС = ДоходВДНеИзСвидетельстваНДС + Выборка.Сумма;	
			Иначе
				ДоходВДНеИзСвидетельстваНеНДС = ДоходВДНеИзСвидетельстваНеНДС + Выборка.Сумма;					
			КонецЕсли;
	...
	СтруктураДанныхДекларацияЕН.R001G3 = ДоходНДС;
	СтруктураДанныхДекларацияЕН.R001G4 = ДоходНеНДС;
	
	СтруктураДанныхДекларацияЕН.R002G3 = ДоходПревышениеМаксимумаНДС;
	СтруктураДанныхДекларацияЕН.R003G3 = ДоходДругойСпособРасчетовНДС;
	СтруктураДанныхДекларацияЕН.R004G3 = ДоходВДНеИзСвидетельстваНДС;
	
	СтруктураДанныхДекларацияЕН.R002G4 = ДоходПревышениеМаксимумаНеНДС;
	СтруктураДанныхДекларацияЕН.R003G4 = ДоходДругойСпособРасчетовНеНДС;
	СтруктураДанныхДекларацияЕН.R004G4 = ДоходВДНеИзСвидетельстваНеНДС;
	
	
	/////////////////////////////////////////////////////////////////////////////////////////////////
	// значение строки 7 и 22 предыдущей декларации
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) > 3 Тогда
		...
		Если ПолучитьданныеЗаПрошлыйПериодИз1С Тогда	
			...
			ПоказателиПрошлойДекларации = ПоказателиПредыдущейДекларации(ДатПрошлогоПериода1,
																		ДатПрошлогоПериода2,
																		"РегламентированныйОтчетРасчетЕдиногоНалога",
																		НСтр("ru='""Декларация по Единому налогу""';uk='""Декларація по Єдиному податку""'"),
																		"ДекларацияЕН",
																		 "РасчетЕдиногоНалога",
																		НСтр("ru='строки 9';uk='рядка 9'"));
			
			Если НЕ ПоказателиПрошлойДекларации = Неопределено Тогда
				Попытка
					Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '20151001' Тогда
						Ст9_3  = ПоказателиПрошлойДекларации.R008G3;
						Ст9_4  = ПоказателиПрошлойДекларации.R008G4;
					Иначе
						Если ЗначениеЗаполнено(ПоказателиПрошлойДекларации.R0071G3) Тогда // налог с плательщика НДС
							Ст9_3  = ПоказателиПрошлойДекларации.R007G3;
							Ст9_4  = 0;
						Иначе	
							Ст9_3  = 0;
							Ст9_4  = ПоказателиПрошлойДекларации.R007G3;
						КонецЕсли;
					КонецЕсли;
					
					СтруктураДанныхДекларацияЕН.R009G3 = Ст9_3;
					СтруктураДанныхДекларацияЕН.R009G4 = Ст9_4;
			...
			Если Данные = Неопределено Тогда
				...
				Если глМенеджерЗвит1С.ИмяСхемыЗвит1С = ПараметрыЗаполненияЗвит1С.ИмяСхемы Тогда
					ТекстСообщения = "Збереженого звіту ""Декларація з єдиного податку "" ("+ПараметрыЗаполненияЗвит1С.ИмяСхемы+") за "+ПредставлениеПериода(ПараметрыЗаполненияЗвит1С.ДатПрошлогоПериода1, ПараметрыЗаполненияЗвит1С.ДатПрошлогоПериода2, "Л=uk_UA")+" не виявлено!
						                 |Автоматичне заповнення показника рядка 9!";
					глМенеджерЗвит1С.ПредупреждениеЗвит1С(ТекстСообщения, "Завантаження даних з ""FREDO Звіт""");
				КонецЕсли;
			Иначе
				
				Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '20151001' Тогда
					Ст9_3  = Данные.GetValue("MAIN", "A8");
					Ст9_4  = Данные.GetValue("MAIN", "B8");
				Иначе
					Если ЗначениеЗаполнено(Данные.GetValue("MAIN", "A71")) Тогда // налог с плательщика НДС
						Ст9_3  = Данные.GetValue("MAIN", "A7");
						Ст9_4  = 0;
					Иначе	
						Ст9_3  = 0;
						Ст9_4  = Данные.GetValue("MAIN", "A7");
					КонецЕсли;
				КонецЕсли;
				
			КонецЕсли;
			
			ЗаполнитьПоказательНаСервере("R009G3", Ст9_3, ЭтаФорма, "РасчетЕдиногоНалога");
			ЗаполнитьПоказательНаСервере("R009G4", Ст9_4, ЭтаФорма, "РасчетЕдиногоНалога");
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	НовСтрока.ИмяСхемы = "J0103506"; 
	НовСтрока.ПредставлениеСхемы 	 = НСтр("ru='Декларация плательщика единого налога (юр. лица)';uk='Декларація патника єдиного податку (юр. особи)'");
