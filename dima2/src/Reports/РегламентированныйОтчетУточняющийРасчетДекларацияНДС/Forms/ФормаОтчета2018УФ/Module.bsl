...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Иначе
		...
		Если    ОкруглитьЧислоПоФормату(Форма, R60GA6) <> ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R013G4", Форма, "Приложение1") + ПолучитьПоказательНаСервере("R013G7", Форма, "Приложение1")) 
			ИЛИ ОкруглитьЧислоПоФормату(Форма, R60GB6) <> ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R013G5", Форма, "Приложение1")) Тогда	
			ТекстСообщенияОбОшибкахРасчета = ТекстСообщенияОбОшибкахРасчета + Символы.ПС + Символы.Таб 
	...
	Иначе
		...
		Если    ОкруглитьЧислоПоФормату(Форма, R130GA6) <> ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R023G4", Форма, "Приложение1")) 
			ИЛИ ОкруглитьЧислоПоФормату(Форма, R130GB6) <> ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R023G5", Форма, "Приложение1")) Тогда	
			ТекстСообщенияОбОшибкахРасчета = ТекстСообщенияОбОшибкахРасчета + Символы.ПС + Символы.Таб 
	...
	УточняемыйГод = ПолучитьПоказательНаСервере("HZYP",  Форма, "СлужебныеДанные");
	УточняемыйКв  = ПолучитьПоказательНаСервере("HZKVP", Форма, "СлужебныеДанные");
	УточняемыйМес = ПолучитьПоказательНаСервере("HZMP",  Форма, "СлужебныеДанные");
	...
	Для СтрНомер = 1 По 12 Цикл
		...
		Если  НовосозданноеПредприятие
			И ПолучитьПоказательНаСервере(ИмяТекущейЯчейкиМес, Форма, "Приложение9") >= УточнМес1
			И ПолучитьПоказательНаСервере(ИмяТекущейЯчейкиМес, Форма, "Приложение9") <= УточнМес2
			И ПолучитьПоказательНаСервере(ИмяТекущейЯчейкиГод, Форма, "Приложение9") = УточняемыйГод Тогда
			//
...
Процедура ЗаполнитьКолонку4ДаннымиИзОтчета()
	...
	Для каждого Показатель Из Показатели Цикл
		...
		Если     ВРЕГ(Прав(ИмяПоказателя, 2)) = "GA" 
			 ...
			 // теперь преобразуем к данным формы ФормаОтчета2018
...
Процедура ЗаполнитьИнформациюОНезарегистрированныхДокументах()
	...
	Запрос.УстановитьПараметр("ДатаНачала", 	СтруктураРеквизитовФормы.ДатаНач);
	Запрос.УстановитьПараметр("ДатаОкончания", 	СтруктураРеквизитовФормы.ДатаКон);
	...
	Если ЗначениеЗаполнено(УточняемыйМес) Тогда
		...
		УточняемыйПериодНачало = НачалоМесяца(УточняемыйПериод);
	Иначе
		ТекПериодичность = "КВАРТАЛ";
		УточняемыйПериод = КонецМесяца(Дата(УточняемыйГод,УточняемыйКв*3,1));
		УточняемыйПериодНачало = НачалоКвартала(УточняемыйПериод);
	КонецЕсли;
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" Тогда
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   |	И ЕСТЬNULL(НО.Регистратор.ВключаетсяВУточняющийРасчет, Ложь) = Истина
					   |    И &УточняемыйПериод = КОНЕЦПЕРИОДА(ЕстьNULL(НО.Регистратор.ДатаОтгрузкиОплаты, ДАТАВРЕМЯ(1,1,1)), "+ТекПериодичность+")				   
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |    И ЕСТЬNULL(НО.Регистратор.ВключаетсяВУточняющийРасчет, ЛОЖЬ) = Истина
				   |    И &УточняемыйПериод = КОНЕЦПЕРИОДА(ЕстьNULL(НО.Регистратор.УточняемыйПериод, ДАТАВРЕМЯ(1,1,1)), "+ТекПериодичность+")				   
	...
	Пока Выборка.Следующий() Цикл
		...
		Если  Выборка.Дата < УточняемыйПериодНачало Тогда
			Если УточняемыйПериод >= '2018-07-01' Тогда
				Продолжить;
			ИначеЕсли Выборка.ЭтоКорректировка Тогда
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	СтруктураДанныхДекларация.R60GB  = СтруктураДанныхПриложение1.R013G5;
	...
	СтруктураДанныхДекларация.R130GB = СтруктураДанныхПриложение1.R023G5;
