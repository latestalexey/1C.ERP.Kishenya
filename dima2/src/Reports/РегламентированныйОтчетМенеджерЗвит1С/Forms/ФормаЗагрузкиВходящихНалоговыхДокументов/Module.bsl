	...
	Пока Выборка.Следующий() Цикл
		...
		Для каждого Строка Из СуществующиеДокБезСвязиСоЗвит Цикл
			...
			Если ЗначениеЗаполнено(Строка.Ссылка) И (НЕ Выборка.ПометкаУдаления) Тогда
				Строка.Комментарий = Строка.Комментарий + ?(ПустаяСтрока(Строка.Комментарий),"", Символы.ПС) + НСтр("ru='Есть дубль документа в 1С: ';uk='Є дубль документу в 1С: '") + "(" + Выборка.Ссылка+")";
				Продолжить;
			КонецЕсли;
			
			Если Выборка.ПометкаУдаления Тогда
				Строка.Комментарий = Строка.Комментарий + ?(ПустаяСтрока(Строка.Комментарий),"", Символы.ПС) + НСтр("ru='Есть помеченный на удаление документ в 1С: ';uk='Є помічений на вилучення документ в 1С: '") + "(" + Выборка.Ссылка+")";
...
Процедура ПрописатьXMLНаСервере(Ссылка, DOC_ID, Комментарий, Флаг, XML)
	...
	Исключение
		...
		Комментарий = НСтр("ru='Для документа 1С не установлена связь документом из """"FREDO Звіт"""". Необходимо загрузить XML в документ 1С.';uk='Для документа 1С не встановлено зв''язок з документом """"FREDO Звіт"""". Необхідно завантажити XML у документ 1С.'");
		Флаг        = Истина;
...
Процедура ЗаполнитьКонтрагентовНаСервере()
	...
	Пока Выборка.Следующий() Цикл
		...
		Для каждого Строка Из ДокументыЗвит1С.НайтиСтроки(Новый Структура("Контрагент, КонтрагентИНН", Справочники.Контрагенты.ПустаяСсылка(), СокрЛП(Выборка.КонтрагентИНН))) Цикл
	...
	Для каждого Строка Из ДокументыЗвит1С Цикл
		...
		Если Не ЗначениеЗаполнено(Строка.Ссылка) Тогда
			...
			Если Строка.Контрагент.Пустая() Тогда
				Строка.Флаг = Ложь;
				Строка.Комментарий = НСтр("ru='В 1С не обнаружен контрагент с ИНН: ';uk='В 1С не виявлено контрагент з ІПН: '") + Строка.КонтрагентИНН;
			КонецЕсли; 
