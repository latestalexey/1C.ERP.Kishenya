...
Функция ДополнитьКешТаблицу(ТекущийОтчет, КешТаблица)
	...
	Если ТекОтчетМетаданные = Неопределено Тогда
		Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
			Сообщить("FREDO Звіт. Заполнение соответствия отчетов <-> поддерживаемых схем Звит. Ошибка получения метаданных отчета """+ТекущийОтчет+"""!", СтатусСообщения.Внимание);
		КонецЕсли;
	...
	Если МакетПоказателиЗвит1С.Области.Найти("Структура") = Неопределено Тогда
		Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
			Сообщить("FREDO Звіт. Заполнение соответствия отчетов <-> поддерживаемых схем Звит. В отчете """+ТекущийОтчет+""" в макете ПоказателиЗвит1С отсутствует служебная область ""Структура""!", СтатусСообщения.Внимание);
		КонецЕсли;
		Возврат ДобавленныеСтроки;
	КонецЕсли;
	...
	Для сч = 1 По ОбластьСтруктура.ВысотаТаблицы Цикл
		...
		Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
			...
			Если МакетПоказателиЗвит1С.Области.Найти(ИмяОбласти) = Неопределено Тогда
				Сообщить("FREDO Звіт. Заполнение соответствия отчетов <-> поддерживаемых схем Звит. В макете ""ПоказателиЗвит1С"" отчета """+ТекущийОтчет+""" отсутствует область """+ИмяОбласти+""", которая описывает заполнение схемы " + ИмяСхемыЗвит1С+ "!", СтатусСообщения.Внимание);
			КонецЕсли;
			
			Если ТекОтчетМетаданные.Формы.Найти(ИмяФормыОтчета) = Неопределено Тогда
				Сообщить("FREDO Звіт. Заполнение соответствия отчетов <-> поддерживаемых схем Звит. В отчете """+ТекущийОтчет+""" ссылка на несуществующую форму отчета  """+ИмяФормыОтчета+""", которая заполненяет схему " + ИмяСхемыЗвит1С + "!", СтатусСообщения.Внимание);
			КонецЕсли;
...
Процедура ЗаписатьВОтчетЗвит1С(Отчет, ИмяТаблицыЗвит1С, ИмяЗвит1С, Значение) Экспорт
	...
	Исключение
		...
		ИначеЕсли ФлагОтладкиПакетноеТестирование Тогда
			...
			Сообщить("FREDO Звіт. Ошибка при установки показателя (имя схемы """+Отчет.CharCode+?(ПустаяСтрока(ИмяТаблицыЗвит1С),"",""", имя таблицы """+ИмяТаблицыЗвит1С)+""", имя в Звите """+ИмяЗвит1С+""", тип значения """+ТипЗнч(Значение)+""", значение """+ Значение+""")", СтатусСообщения.Внимание);
...
Функция ПолучитьЗначениеПоказателя1С(Данные, ИмяТаблицы,ИмяПоказателя)
	...
	Исключение
		Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
			Сообщить("FREDO Звіт. Ошибка получения показателя (табличное поле """ + ИмяТаблицы + """, показатель """ + ИмяПоказателя + """). Имя табличного поля или показателя указано не верно!", СтатусСообщения.Внимание);
		КонецЕсли;
	КонецПопытки;
...
Функция ВозможенЗапускЗвит1С(ВыводитьСообщенияОбОшибках)
	...
	#Если ВебКлиент Тогда
		Если ВыводитьСообщенияОбОшибках Тогда
			ПоказатьПредупреждение(,"Работа с FREDO Звіт в режиме Веб-клиента не поддерживатся. Воспользуйтесь тонким клиентом 1С");	
		КонецЕсли;
		Возврат Ложь;
	#КонецЕсли
...
Функция ПроверитьОрганизацию(ВыводитьСообщенияОбОшибках)
	...
	Если РезультатЗапроса.Пустой() Тогда
		Если ВыводитьСообщенияОбОшибках Тогда
			...
			////////					|Необходимо указать дату начала использования ""FREDO Звіт"" в карточке организации.
			////////					|
			////////					|Открыть справочник организаций?';uk= ' нема організації що використовують ""FREDO Звіт""
			////////					|Необхідно вказати дату початку використання ""FREDO Звіт"" в картці організації.
			////////					|
			...
			ТекстВопроса = "На "+Формат(ТекущаяДата(), "ДФ=dd.MM.yyyy")+
			               НСтр("ru=' нет организаций использующих ""FREDO Звіт""."
"Необходимо указать дату начала использования ""FREDO Звіт"" в карточке организации.';uk=' немає організацій використовують ""FREDO Звіт""."
"Необхідно зазначити дату початку використання ""FREDO Звіт"" у картці організації.'");
...
Функция Инициализация(ВыводитьСообщенияОбОшибках) Экспорт
	...
	Попытка
		...
		Если Не ПодключениеУспешно Тогда
			...
			Если ВыводитьСообщенияОбОшибках Тогда
				Попытка
					Сообщить("Выполняется установка внешней компоненты для работы с ""FREDO Звіт""...");
				...
				Исключение
					ВызватьИсключение "Не удалось установить внешнюю компоненту ""ZvitAPI.dll";				
				КонецПопытки;
...
Процедура УстановкаКомпонентыЗвит1СЗавершение(ДопПараметры) Экспорт
	...
	Иначе		
		Сообщить(НСтр("ru='Установка внешней компоненты для работы с FREDO Звіт не выполнена!"
	"Возможно, используется 64-битный вариант платформы. Работа с FREDO Звіт в 64-битном варианте платформы не поддерживается.';uk='Встановлення зовнішньої компоненти для роботи з FREDO Звіт не виконано!"
	"Можливо, використовуєтеться 64-бітний варіант платформи. Робота з FREDO Звіт у 64-бітному варіанті платформи не підтримується.'"));
	КонецЕсли; 
...
Функция ЗапуститьЗвит1С(НаПереднийПлан = Неопределено, Организация = Неопределено) Экспорт
	...
	Если    РезультатЗапуска = Неопределено 
		...
		ПоказатьПредупреждение(,НСтр("ru='Запуск ""FREDO Звіт"" отменен. Возможные причины:"
""
" 1. Нажата кнопка ""Відмінити"" в форме выбора рабочего каталога ""FREDO Звіт"""
" 2. В каталоге с ""FREDO Звіт"" остался файл ""zvit1c.ini"" (для запуска ""FREDO Звіт"" достаточно удалить данный файл)';uk='Запуск ""FREDO Звіт"" скасовано. Можливі причини:"
""
"1. Натиснута кнопка ""Відмінити"" у формі вибору робочого каталогу ""FREDO Звіт"""
"2. В каталозі з ""FREDO Звіт"" залишився файл ""zvit1c.ini"" (для запуску ""FREDO Звіт"" досить видалити даний файл)'"));	
		Возврат Ложь;
...
Процедура ОбработкаВнешнегоСобытияЗвит1С(Источник, Событие, Данные) Экспорт
	...
	ИначеЕсли Команда = "REQUESTDBINFO" Тогда
		...
		//////ВерсияПриложения = "8.2.19.385";
		//////глКомпонентаЗвит1С.Set1CVersion(ВерсияПриложения);
	...
	ИначеЕсли Команда = "REPORT" Тогда
		...
		Если ОрганизацииЗвит1С.Количество() = 0 Тогда
			ТекстОшибки = "В 1С не знайдено фірму з ЄДРПОУ """+ОтчетЗвит1С.FIRM_EDRPOU+""""+ " на "+Формат(ДатаНачалаПериодаОтчета, "Л=uk_UA; ДФ=dd.MM.yyyy")+"
		...
		ИначеЕсли ОрганизацииЗвит1С.Количество() > 1 Тогда
			ТекстОшибки = "В 1С знайдено декілька фірм з однаковим ЄДРПОУ """+ОтчетЗвит1С.FIRM_EDRPOU+""""+" на "+Формат(ДатаНачалаПериодаОтчета, "Л=uk_UA; ДФ=dd.MM.yyyy")+Символы.ПС;
...
Функция ВыгрузитьДокумент(ДокументыМассив, ФормаДокумента = Неопределено) Экспорт
	...
	Для каждого ДокументСсылка Из ДокументыМассив Цикл
		...
		Иначе
			Сообщить("Документ " + ДокументСсылка + " пропущен: недопустимый вид документа");
			Продолжить;	
		КонецЕсли;
		...
		ИначеЕсли НЕ ТекЭтоИсходящиеНалоговыеДокументы = ЭтоИсходящиеНалоговыеДокументы Тогда
			Сообщить("Документ " + ДокументСсылка + " пропущен: вид документа не соответствует виду первого обрабатываемого документа");
			Продолжить;	
		КонецЕсли;
		
		Организация = ПолучитьОрганизациюДокумента(ДокументСсылка);
		Если Не ЗначениеЗаполнено(Организация) Тогда
			Сообщить("Документ " + ДокументСсылка + " пропущен: пустая организация в документе");
			Продолжить;	
		КонецЕсли;
		...
		ИначеЕсли НЕ ТекОрганизация = Организация Тогда
			Сообщить("Документ " + ДокументСсылка + " пропущен: организация не соответствует организации первого обрабатываемого документа");
			Продолжить;	
		КонецЕсли;
...
Функция ОпределитьТипДокументаПоСсылкеНаСервере(Документ) Экспорт
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" 
		...
		ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
...
//////		
//////		ОбработкаВнешнегоСобытияЗвит1С(Источник, Событие, Данные);	
//////		
//////	КонецЕсли;
//////	
//////КонецПроцедуры
