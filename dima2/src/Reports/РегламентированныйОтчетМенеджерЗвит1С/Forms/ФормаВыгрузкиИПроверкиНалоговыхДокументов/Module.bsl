...
Процедура ЗаполнитьНачальныеНастройки()
	...
	ПредставлениеСостоянийДляИсходящихНалоговыхДокументов.Вставить(-2, НСтр("ru='В ""FREDO Звіт"" обнаружено более одного документа соответствующего документу ""1С""';uk='В ""FREDO Звіт"" виявлено більше одного документа відповідного документу ""1С""'"));
	...
	ПредставлениеСостоянийДляВходящихНалоговыхДокументов.Вставить(-2, НСтр("ru='В ""FREDO Звіт"" обнаружено более одного вытяга соответствующего документу ""1С""';uk='В ""FREDO Звіт"" виявлено більше одного витяга відповідного документу ""1С""'")); // Несуществующее состояние для входящих 
	...
	Элементы.РасчетЦеныТочный.Видимость = мЭтоИсходящиеНалоговыеДокументы;
	...
	ЭлементСпискаЗначений = Элементы.СпецРежимНалогообложения.СписокВыбора.НайтиПоЗначению(СпецРежимНалогообложения);
...
Процедура ТаблицаДокументовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если мЭтоИсходящиеНалоговыеДокументы Тогда
		...
		МенюВыбора.Добавить("1С",     "Открыть документ в 1С");
		МенюВыбора.Добавить("1СЗвит", "Открыть документ в FREDO Звіт");
...
Процедура ОткрытьВ1С(Команда)
	...
	Если ТекущаяСтрока = Неопределено Тогда
		ПоказатьПредупреждение(,"Укажите документ, который необходимо открыть!");
		Возврат;
	КонецЕсли;
...
Процедура Заполнить(Команда)
	...
	Если Не ЗначениеЗаполнено(ДатаНачалаИспользованияЗвит1С) Тогда
		ТекстСообщения = НСтр("ru='Выгрузка в систему ""FREDO Звіт"" невозможна, ""FREDO Звіт"" не используется для организации';uk='Вивантаження в систему ""FREDO Звіт"" неможливе, ""FREDO Звіт"" не використовується для організації'")
		                     +" """+Строка(Организация)+"""."+Символы.ПС+НСтр("ru='Не указана дата начала использования ""FREDO Звіт"" в карточке организации';uk='Не зазначена дата початку використання ""FREDO Звіт"" у картці організації'");
		ПоказатьПредупреждение(,ТекстСообщения);	
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьКорректностьДокумента(ДанныеДокумента1С, ДокументКорректен_ОписаниеОшибки = "")
	...
	Если НЕ (ДанныеДокумента1С.Организация_ЕДРПОУ = Организация_ЕДРПОУ) Тогда
		ДокументКорректен_ОписаниеОшибки = ДокументКорректен_ОписаниеОшибки + 
											  ...
											  "Код ЕДРПОУ организации документа не совпадает с кодом для Организации из обработки";
	...
	Если Не ЗначениеЗаполнено(ДанныеДокумента1С.Контрагент_ИНН) 
		...
		ДокументКорректен_ОписаниеОшибки = ДокументКорректен_ОписаниеОшибки + 
											 ?(ЗначениеЗаполнено(ДокументКорректен_ОписаниеОшибки), Символы.ПС, "") + 
											 "Для контрагента не удалось определить ИНН";
		ДокументКорректен = Ложь;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ДанныеДокумента1С.ДатаВходящегоДокумента) Тогда
		ДокументКорректен_ОписаниеОшибки = ДокументКорректен_ОписаниеОшибки +
											 ?(ЗначениеЗаполнено(ДокументКорректен_ОписаниеОшибки), Символы.ПС, "") + 
											 "Не указана дата входящего документа";
		ДокументКорректен = Ложь;
	КонецЕсли;
...
Функция ОпределитьПорядковыйНомерИсходящейНН(СтрокаТаблицы)
	...
	НомерПорядковый = СокрЛП(ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(НомерДокумента, Истина, Истина));
	...
	Если ОшибкаРазбораНомера = Истина Тогда
		ОписаниеОшибкиРазбораНомера = "Указан некорректный номер входящего документа";
	КонецЕсли;
...
Процедура ТаблицаДокументовЗаполнитьРеквизитыДокументов1СЗвит(МассивДокументов = Неопределено)
	...
	Для каждого СтрокаЗапроса Из ТаблицаЗапроса Цикл
		...
		Если СтрокиТаблицы.Количество() = 0 Тогда
			Продолжить;
		Иначе	
			// во всех строкат таблицы данные должны быть одинаковыми - возьмем из первой строки по данной ссылке
		...
		Для каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
			...
			Если мЭтоИсходящиеНалоговыеДокументы И (СтрокаТаблицы.Звит1С_Состояние > -1) Тогда
				...
				Иначе
					...
					СостояниеСтр = СостояниеСтр + ". Не совпадают ключевые поля: " + ТекстОшибки;
				КонецЕсли;
...
Функция ПроверитьСостояниеРегистрацииВ1СЗвит(МассивДокументов)
	...
	Если Не ЗначениеЗаполнено(Организация_ЕДРПОУ)  Тогда
		РезультатФомирования.ОписаниеОшибки = "Для организации " + Организация + " не указан код ЕДРПОУ!";
		Возврат РезультатФомирования;
	КонецЕсли;
...
Процедура ТаблицаДокументовВыгрузитьДокументыИлиЗапросВ1СЗвит()
	...
	Если МассивСтрокДокументов.Количество() = 0 Тогда
		...
		ТекстПредупреждения = "Не выбраны корректные документы для выгрузки/формирования запроса!";
	...
	Если мЭтоИсходящиеНалоговыеДокументы Тогда
		Если БылиФатальныеОшибки Тогда
			ТекстСообщения = "Выгрузка прервана с ошибками";
		ИначеЕсли БылиОшибки Тогда
			ТекстСообщения = "Выгрузка завершена с ошибками";
		Иначе
			ТекстСообщения = "Выгрузка успешно завершена";
		КонецЕсли;	
	Иначе	
		Если БылиФатальныеОшибки Тогда
			ТекстСообщения = "Формирование и отправка запроса состояния прервано с ошибками";
		ИначеЕсли БылиОшибки Тогда
			ТекстСообщения = "Формирование и отправка запроса состояния завершено с ошибками";
		Иначе
			ТекстСообщения = "Формирование и отправка запроса состояния успешно завершено";
		КонецЕсли;	
	КонецЕсли;
...
Функция СформироватьИОтправитьЗапросСостоянияРегистрацииДокументовВЕРНН()
	...
	// Для организации без ЕДРПОУ бесполезно подключаться к FREDO Звіту
	Если Не ЗначениеЗаполнено(Организация_ЕДРПОУ)  Тогда
		РезультатФомирования.ТекстОшибки = "Для организации " + Организация + " не указан код ЕДРПОУ!";
		Возврат РезультатФомирования;
	КонецЕсли;
	...
	Состояние("Формирование и отправка запроса состояния регистрации в ЕРНН в ""FREDO Звіт""");
	...
	Для каждого СтрокаТаблицыДокументов Из МассивСтрокДокументов Цикл
		...
		Если Не СтрокаТаблицыДокументов.ДокументКорректен 
		 ИЛИ Не СтрокаТаблицыДокументов.Звит1С_ДокументКорректен Тогда
			ТекстСообщения = "Документ пропущен, состояние регистрации в ЕРНН не определено";
		...
		Иначе	
			...
			Если СтрокаТаблицыДокументов.Звит1С_ДокументЗарегистрирован И Не ЗначениеЗаполнено(СтрокаТаблицыДокументов.Звит1С_ДатаРегистрации) Тогда
				ТекстСообщения = "Документ пропущен, дата регистрации в ЕРНН не определена";
				Сообщить(Строка(ДокументСсылка)+" - "+ТекстСообщения, СтатусСообщения.Информация);
			...
			Попытка
				Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.Приложение2КНалоговойНакладной") Тогда				
					Если ЗначениеРеквизита = Истина ИЛИ ЗначениеРеквизита = Перечисления.СтатусыРегистрацииВЕРНННалоговыхДокументов.ЗарегистрированВЕРНН Тогда
						ДокументОбъект.ДатаВключенияВЕдиныйРеестрНалоговыхНакладных = ДатаРегистрации;	
					Иначе	
...
Функция ВыгрузитьДокументВЗвит1С(СтрокаТаблицыДокументов) 
	...
	Если мВыбраннаяФорма = Неопределено Тогда
		ТекстОшибки = "Выгрузка налоговых документов за период " + Формат(СтрокаТаблицыДокументов.Дата, "ДФ=dd.MM.yyyy") + " не поддерживается";
	...
	Исключение
		ТекстОшибки = "Ошибка открытия формы " + мВыбраннаяФорма + " отчета " + ИмяОтчета;
		Возврат СформироватьРезультатВыгрузкиДокумента(ТекстОшибки, Истина, Истина);
	КонецПопытки;
	
	ТекФорма.СтруктураРеквизитовФормы.Организация = Организация;
	Попытка
		ТекФорма.РасчетЦеныТочный = РасчетЦеныТочный;
	Исключение
	КонецПопытки;
	...
	Если СоответствиеОтчета = Неопределено 
	 ...
	 ИЛИ СоответствиеОтчета.ИмяФормыОтчета <> мВыбраннаяФорма Тогда
		ТекстОшибки = "Нет возможности выгрузить документ по схеме " + ИмяСхемы + " с помощью отчета " + ИмяОтчета + ", форма: " + мВыбраннаяФорма + ". Возможно необходимо обновить справочник регламентированных отчетов";
		Возврат СформироватьРезультатВыгрузкиДокумента(ТекстОшибки, Истина);
	КонецЕсли;
	...
	Если глМенеджерЗвит1С.ФлагОтладки Тогда
		Сообщить("FREDO Звіт. Создание отчета из 1С (ЕДРПОУ """+КодПоЕДРПОУ+""", схема """+ИмяСхемыЗвит1С+""", тип периода """+ТипПериода+
	...
	ИначеЕсли РезультатаСоздания = 2 Тогда
		...
		ТекстОшибки = НСтр("ru='Отчет уже отправлен в контролирующий орган (квитанция не получена)';uk='Звіт уже відправлений до контролюючого органу (квитанція не отримана)'");
		Возврат СформироватьРезультатВыгрузкиДокумента(ТекстОшибки, Истина);
...
"Изменены ключевые параметры, выгрузка отменена.';uk='Виявлено звіт ""FREDO Звіт"" відповідний документу, що вивантажується з ""1С"" ."
