...
Процедура ЗаполнитьНачальныеНастройки()
	...
	ПредставлениеСостоянийДокументов.Добавить(-1, НСтр("ru='Не выгружен в ""FREDO ДокМен""';uk= 'Не вивантажен в ""FREDO ДокМен""'"));
	ПредставлениеСостоянийДокументов.Добавить( 0, НСтр("ru='Выгружен в ""FREDO ДокМен""';uk= 'Вивантажен в ""FREDO ДокМен""'"));
...
Процедура ТаблицаДокументовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если мЭтоИсходящиеНалоговыеДокументы Тогда
		...
		МенюВыбора.Добавить("1С",     НСтр("ru='Открыть документ в 1С';uk= 'Відкрити документ в 1С'"));
...
Процедура Заполнить(Команда)
	...
	Если Не ЗначениеЗаполнено(ДатаНачалаИспользованияЗвит1С) Тогда
		ТекстСообщения = НСтр("ru='Выгрузка в систему ""FREDO ДокМен"" невозможна, ""FREDO ДокМен"" не используется для организации';uk='Вивантаження в систему ""FREDO ДокМен"" неможливе, ""FREDO ДокМен"" не використовується для організації'")
		                     +" """+Строка(Организация)+"""."+Символы.ПС+НСтр("ru='Не указана дата начала использования ""FREDO ДокМен"" в карточке организации';uk='Не зазначена дата початку використання ""FREDO ДокМен"" у картці організації'");
		ПоказатьПредупреждение(,ТекстСообщения);	
		Возврат;
	КонецЕсли;
...
Процедура ТаблицаДокументовЗаполнитьРеквизитыДокументов1С(МассивДокументов = Неопределено)
	...
	Для каждого Документ Из МассивДокументов Цикл
		...
		Для каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
			...
			СтрокаТаблицы.Номер 			 = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Документ.Номер, Истина, Истина);
...
Функция ПодключитьсяКЭДО(МассивДокументов)
	...
	Если Не ЗначениеЗаполнено(Организация_ЕДРПОУ)  Тогда
		Результат.ОписаниеОшибки = "Для организации " + Организация + " не указан код ЕДРПОУ!";
		Возврат Результат;
	КонецЕсли;
...
Процедура ТаблицаДокументовВыгрузитьДокументыИлиЗапросВ1СЗвит()
	...
	ИначеЕсли БылиОшибки Тогда
		ТекстСообщения = НСтр("ru='Выгрузка завершена с ошибками';uk= 'Вивантаження заверешене  з помилками'");
	Иначе
...
Функция ВыгрузитьДокументВЗвит1С(СтрокаТаблицыДокументов) 
	...
	Если глМенеджерЗвит1С.ФлагОтладки Тогда
		Сообщить("FREDO ДокМен. Создание отчета из 1С (ЕДРПОУ """+КодПоЕДРПОУ+""", схема """+ИмяСхемыЗвит1С+""", тип периода """+ТипПериода+
	...
	Если РезультатаСоздания = 0 Тогда
		...
		ОтчетЗвит1С = Результат;
	 	Если глМенеджерЗвит1С.ФлагОтладки Тогда
			Сообщить("FREDO ДокМен. Создан новый отчет");
		КонецЕсли;
	...
	ИначеЕсли РезультатаСоздания = 1 Тогда
		...
		ОтчетЗвит1С = Результат;
		Если глМенеджерЗвит1С.ФлагОтладки Тогда
			Сообщить("FREDO ДокМен. Открыт существующий отчет");
		КонецЕсли;
	...
	ИначеЕсли РезультатаСоздания = 4 Тогда
		...
		ОтчетЗвит1С = Результат;
		Если глМенеджерЗвит1С.ФлагОтладки Тогда
			Сообщить("FREDO ДокМен. Открыт существующий документ");
		КонецЕсли;
