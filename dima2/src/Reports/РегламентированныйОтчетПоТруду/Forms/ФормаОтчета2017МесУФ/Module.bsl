...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2017Мес";
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" Тогда
		...
		|				И (НачислениеУдержание ССЫЛКА ПланВидовРасчета.Начисления ИЛИ НачислениеУдержание = ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.ДоговорРаботыУслуги)
		|				ИЛИ НачислениеУдержание = ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.ДоговорАвторскогоЗаказа))) КАК Начисления
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" ТОГДА
		ЗапросСуммаЗадолжКонец.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СУММА(ВзаиморасчетыСРаботниками.СуммаКВыплатеОстаток) КАК Результат,
		|	СУММА(ВЫБОР
		|			КОГДА ВзаиморасчетыСРаботниками.ПериодВзаиморасчетов < &ПериодЗадолженностиПрошлыхЛет
		|				ТОГДА ВзаиморасчетыСРаботниками.СуммаКВыплатеОстаток
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК РезультатПрошлыхЛет,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВзаиморасчетыСРаботниками.ФизическоеЛицо) КАК РезультатКоличество
		|ИЗ
		|	РегистрНакопления.ЗарплатаКВыплате.Остатки(
		|			&НачДата,
		|			Организация = &Организация
		|				И ПериодВзаиморасчетов < &ПериодЗадолженности) КАК ВзаиморасчетыСРаботниками
		|ИМЕЮЩИЕ
		|	СУММА(ВзаиморасчетыСРаботниками.СуммаКВыплатеОстаток) > 0";
    Иначе
		...
		|ИМЕЮЩИЕ
		|	СУММА(ВзаиморасчетыСРаботниками.СуммаВзаиморасчетовОстаток) > 0
		|";
	КонецЕсли;	
