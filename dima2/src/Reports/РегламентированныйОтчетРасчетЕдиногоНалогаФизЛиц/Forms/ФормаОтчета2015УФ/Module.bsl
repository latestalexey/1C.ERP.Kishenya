...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	R010G3 =  (ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R005G3", Форма, "ДекларацияЕН"))) * 0.02;
	ЗаполнитьПоказательНаСервере("R010G3", R010G3, Форма, "ДекларацияЕН");
												  
	R011G3 =  (ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R006G3", Форма, "ДекларацияЕН"))) * 0.04;
...
Процедура ЗаполнитьРазделПоГруппеЕРП(Группа)
	...
	ИначеЕсли Группа = Перечисления.ГруппыПлательщиковЕдиногоНалога.ТретьяГруппа Тогда
		...
		// учтем всумме ограничения сумму дохода, полученную
		// когда физ.лицо находилось в предыдущих группах в текущем году
		Если ОграничениеСуммыДоходаЕдинщика > 0 Тогда
			ОграничениеСуммыДоходаЕдинщика =  ОграничениеСуммыДоходаЕдинщика 
			                                - СтруктураДанныхДекларацияЕН.R001G3
...
&НаСервере
Процедура ЗаполнитьАвансовыеПлатежиУТ()
	...
	|ИЗ
	|	Перечисления КАК Перечисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ УчетнаяПолитика КАК УчетнаяПолитикаЗапрос
	|		ПО Перечисления.Период = УчетнаяПолитикаЗапрос.ПериодПеречисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаОрганизаций КАК УчетнаяПолитикаОрганизаций
	|		ПО (УчетнаяПолитикаОрганизаций.Организация = &Организация)
	|			И (УчетнаяПолитикаОрганизаций.Период = УчетнаяПолитикаЗапрос.ПериодУчетнойПолитики)
	|ГДЕ
	|	(УчетнаяПолитикаОрганизаций.ГруппаПлательщикаЕдиногоНалога = ЗНАЧЕНИЕ(Перечисление.ГруппыПлательщиковЕдиногоНалога.ПерваяГруппа)
	|			ИЛИ УчетнаяПолитикаОрганизаций.ГруппаПлательщикаЕдиногоНалога = ЗНАЧЕНИЕ(Перечисление.ГруппыПлательщиковЕдиногоНалога.ВтораяГруппа))
	|";
...
&НаСервере
Процедура ЗаполнитьРазделПоГруппеУТ(Группа)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	НАЧАЛОПЕРИОДА(КнигаДоходовРасходовПоЕдиномуНалогуОбороты.Период, ДЕНЬ) КАК Период,
	               |	КнигаДоходовРасходовПоЕдиномуНалогуОбороты.Валюта,
	               |	КнигаДоходовРасходовПоЕдиномуНалогуОбороты.Статья,
	               |	СУММА(КнигаДоходовРасходовПоЕдиномуНалогуОбороты.СуммаОборот) КАК Сумма
	               |ПОМЕСТИТЬ ПродажиПоДням
	               |ИЗ
	               |	РегистрНакопления.КнигаДоходовРасходовПоЕдиномуНалогу.Обороты(&ДатаНач, &ДатаКон, День, Организация = &Организация) КАК КнигаДоходовРасходовПоЕдиномуНалогуОбороты
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	КнигаДоходовРасходовПоЕдиномуНалогуОбороты.Статья,
	               |	КнигаДоходовРасходовПоЕдиномуНалогуОбороты.Валюта,
	               |	НАЧАЛОПЕРИОДА(КнигаДоходовРасходовПоЕдиномуНалогуОбороты.Период, ДЕНЬ)
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               ...
	               |	МАКСИМУМ(КурсыВалют.Период) КАК ПериодКурсовВалюта
	               |ПОМЕСТИТЬ КурсыВалютДаты
	               |ИЗ
	               |	ПродажиПоДням КАК Продажи
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалют
	               |		ПО (КурсыВалют.Период <= Продажи.Период)
	               |			И (КурсыВалют.Валюта = Продажи.Валюта)
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	Продажи.Период,
	               |	КурсыВалют.Валюта
	               ...
	               |	Продажи.Статья,
	               |	СУММА(Продажи.Сумма * ЕСТЬNULL(КурсыВалют.Курс, 0) / ВЫБОР
	               ...
	               |ИЗ
	               |	ПродажиПоДням КАК Продажи
	               |		ЛЕВОЕ СОЕДИНЕНИЕ КурсыВалютДаты КАК КурсыВалютДаты
	               |		ПО Продажи.Период = КурсыВалютДаты.ПериодПродаж
	               |			И Продажи.Валюта = КурсыВалютДаты.Валюта
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалют
	               |		ПО    (КурсыВалют.Валюта = Продажи.Валюта)
	               |			И (КурсыВалют.Период = КурсыВалютДаты.ПериодКурсовВалюта)
	               |
	               |СГРУППИРОВАТЬ ПО
                   ...
                   |      Продажи.Период КАК ПериодПродаж,
                   |      МАКСИМУМ(УчетнаяПолитикаОрганизаций.Период) КАК ПериодУчетнойПолитики
                   |ПОМЕСТИТЬ УчетнаяПолитикаПродаж
                   |ИЗ
                   |      Продажи КАК Продажи
                   |                  ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаОрганизаций КАК УчетнаяПолитикаОрганизаций
                   |                  ПО (УчетнаяПолитикаОрганизаций.Период <= Продажи.Период)
	               ...
	               |ВЫБРАТЬ
	               |	ВЫБОР
	               |		КОГДА УчетнаяПолитикаОрганизаций.УчетнаяПолитика.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.ЕдиныйНалогИНДС)
	               |			ТОГДА ИСТИНА
	               |		ИНАЧЕ ЛОЖЬ
	               |	КОНЕЦ КАК ПлательщикНДС,
	               |	УчетнаяПолитикаОрганизаций.ГруппаПлательщикаЕдиногоНалога КАК Группа,
	               ...
	               |ИЗ
	               |	Продажи КАК Продажи
	               |		ЛЕВОЕ СОЕДИНЕНИЕ УчетнаяПолитикаПродаж КАК УчетнаяПолитикаПродаж
	               |		ПО Продажи.Период = УчетнаяПолитикаПродаж.ПериодПродаж
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаОрганизаций  КАК УчетнаяПолитикаОрганизаций
	               |		ПО (УчетнаяПолитикаОрганизаций.Организация = &Организация)
	               |			И (УчетнаяПолитикаОрганизаций.Период = УчетнаяПолитикаПродаж.ПериодУчетнойПолитики)
	               |ГДЕ
	               |	УчетнаяПолитикаОрганизаций.ГруппаПлательщикаЕдиногоНалога = &Группа
	               |
	               |УПОРЯДОЧИТЬ ПО
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) > 3 Тогда
		...
		Если ПолучитьданныеЗаПрошлыйПериодИз1С Тогда	
			...
			Если НЕ ПоказателиПрошлойДекларации = Неопределено Тогда
				Попытка
					Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 2015 И Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 9 Тогда
						// старая декларация
						Попытка
						R013G3 = ПоказателиПрошлойДекларации.R027G3;
	...
	Если ТипЗнч(ПараметрыЗаполненияЗвит1С) = Тип("Структура") Тогда
		...
		Попытка
			...
			Иначе
				...
				Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 2015 И Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 9 Тогда
					R013G3 = Данные.GetValue("MAIN", "A27");
				Иначе
			...
			ЗаполнитьПоказательНаСервере("R013G3", R013G3, ЭтаФорма, "РасчетЕдиногоНалога");
