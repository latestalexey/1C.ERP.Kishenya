...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	Если ПустаяСтрока(ТекстКорСубконто) Тогда
		ОбщийТекстСубконто = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 и ...';uk='%1 та ...'"),
			ОбщийТекстСубконто);
	Иначе
		ОбщийТекстСубконто = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 и %2';uk='%1 і %2'"),
			ОбщийТекстСубконто,
			ТекстКорСубконто);
	КонецЕсли;
	
	ЗаголовокОтчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Обороты между субконто %1 %2';uk='Обороти між субконто %1 %2'"),
		ОбщийТекстСубконто,
