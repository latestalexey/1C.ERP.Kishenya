...
Функция ТекстЗапросаПередачаМатериаловВПроизводство()
	// Строковые литералы локализуются в настройках СКД отчета
	// Наборы данных - Поля - Доступные значения
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	""Передача материалов в производство"" КАК ВидОперации,
	|	""Оформление складских документов"" КАК ТипОтклонения,
	|	ВЫБОР
	|		КОГДА ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
	|			ТОГДА ""Завершите оформление расходных ордеров на товары""
	...
	|	РегистрНакопления.ТоварыКОтгрузке.Остатки(
	|		&Период, 
	|		ТИПЗНАЧЕНИЯ(ДокументОтгрузки) В (
//++ НЕ УТКА
	|			ТИП(Документ.ЗаказНаПроизводство), 
//-- НЕ УТКА
	|			ТИП(Документ.ЗаказМатериаловВПроизводство), 
	|			ТИП(Документ.ПередачаМатериаловВПроизводство)) 
	|		{(ДокументОтгрузки).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}
	|	) КАК ТоварыКОтгрузке
	|ГДЕ
	|	ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
	...
	|	""Оформление финансовых документов"",
	|	""Оформите передачу материалов в производство в статусе """"Принято"""""",
	|	-ПередачаМатериаловВПроизводство.Количество,
	|	0,
	|	ВЫБОР
	|		КОГДА НЕ ПередачаМатериаловВПроизводство.Ссылка.Распоряжение.Ссылка ЕСТЬ NULL 
	|			ТОГДА ПередачаМатериаловВПроизводство.Ссылка.Распоряжение.Дата
	|		ИНАЧЕ ПередачаМатериаловВПроизводство.Ссылка.Дата
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА НЕ ПередачаМатериаловВПроизводство.Ссылка.Распоряжение.Ссылка ЕСТЬ NULL 
	|			ТОГДА ПередачаМатериаловВПроизводство.Ссылка.Распоряжение
	|		ИНАЧЕ ПередачаМатериаловВПроизводство.Ссылка
	|	КОНЕЦ,
	|	ПередачаМатериаловВПроизводство.Ссылка.Подразделение,
	|	ПередачаМатериаловВПроизводство.Номенклатура,
	|	ПередачаМатериаловВПроизводство.Характеристика,
	|	ПередачаМатериаловВПроизводство.Серия,
	|	ПередачаМатериаловВПроизводство.Назначение
	|{ВЫБРАТЬ
	|	Склад.*,
	|	Номенклатура.*,
	|	Характеристика.*,
	...
	|	Назначение.*}
	|ИЗ
	|	Документ.ПередачаМатериаловВПроизводство.Товары КАК ПередачаМатериаловВПроизводство
	|ГДЕ
	|	ПередачаМатериаловВПроизводство.Ссылка.Проведен
	|	И ПередачаМатериаловВПроизводство.Ссылка.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыПередачМатериаловВПроизводство.Принято)
	|	И ПередачаМатериаловВПроизводство.Ссылка.Дата <= КОНЕЦПЕРИОДА(&Период, ДЕНЬ)
	|{ГДЕ
	|	(""Передача материалов в производство"") КАК ВидОперации,
	|	(""Оформление финансовых документов"") КАК ТипОтклонения,
	|	(""Оформите передачу материалов в производство в статусе """"Принято"""""") КАК Рекомендация,
	|	(-ПередачаМатериаловВПроизводство.Количество) КАК НезавершеннаяСкладскаяОперация}";
	
	Возврат ТекстЗапроса;
КонецФункции

Функция ТекстЗапросаВозвратыМатериаловИзПроизводства()
	// Строковые литералы локализуются в настройках СКД отчета
	// Наборы данных - Поля - Доступные значения
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	""Возвраты материалов из производства"" КАК ВидОперации,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюОстаток > 0
	|			ТОГДА ""Оформление финансовых документов""
	|		ИНАЧЕ ""Оформление складских документов""
	...
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюОстаток > 0
	|			ТОГДА ""Оформите возврат материалов из производства""
	|		ИНАЧЕ ""Оформите приходные ордера на товары""
	|	КОНЕЦ КАК Рекомендация,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюОстаток > 0
	...
	|	РегистрНакопления.ТоварыКОформлениюПоступления.Остатки(
	|		&Период, 
	|		ТИПЗНАЧЕНИЯ(ДокументПоступления) В (ТИП(Документ.ВозвратМатериаловИзПроизводства)) 
	|		{(ДокументПоступления).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}
	|	) КАК ТоварыКОформлениюПоступления
	|{ГДЕ
	|	(""Возвраты материалов из производства"") КАК ВидОперации,
...
Функция ТекстЗапросаПриемкаГотовойПродукции()
	// Строковые литералы локализуются в настройках СКД отчета
	// Наборы данных - Поля - Доступные значения
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Функция ТекстЗапросаПереработкаСырьяУПереработчика()
	// Строковые литералы локализуются в настройках СКД отчета
	// Наборы данных - Поля - Доступные значения
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	""Переработка сырья у переработчика"" КАК ВидОперации,
	|	ВЫБОР
	|		КОГДА ТоварыКОтгрузке.КОформлениюОстаток < 0
	|			ТОГДА ""Оформление складских документов""
	...
	|			ТОГДА ""Завершите оформление расходных ордеров на товары""
	|		КОГДА ТоварыКОтгрузке.КОтгрузкеОстаток < 0
	|			ТОГДА ""Оформите передачу сырья переработчику""
	|	КОНЕЦ КАК Рекомендация,
	|	ВЫБОР
	|		КОГДА ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
	...
	|	Назначение.*}
	|ИЗ
	|	РегистрНакопления.ТоварыКОтгрузке.Остатки(&Период, 
	|	ТИПЗНАЧЕНИЯ(ДокументОтгрузки) В (ТИП(Документ.ЗаказПереработчику)) 
	|	{(ДокументОтгрузки).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}) КАК ТоварыКОтгрузке
	|ГДЕ
	|	ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
	|{ГДЕ
	|	(""Переработка сырья у переработчика"") КАК ВидОперации,
...
Функция ТекстЗапросаВозвратыСырьяОтПереработчиков()
	// Строковые литералы локализуются в настройках СКД отчета
	// Наборы данных - Поля - Доступные значения
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	""Возвраты сырья от переработчиков"" КАК ВидОперации,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюОстаток > 0
	|			ТОГДА ""Оформление финансовых документов""
	|		ИНАЧЕ ""Оформление складских документов""
	...
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюОстаток > 0
	|			ТОГДА ""Оформите возврат сырья от переработчика""
	|		ИНАЧЕ ""Оформите приходные ордера на товары""
	|	КОНЕЦ КАК Рекомендация,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюОстаток > 0
	...
	|	РегистрНакопления.ТоварыКОформлениюПоступления.Остатки(
	|		&Период, 
	|		ТИПЗНАЧЕНИЯ(ДокументПоступления) В (ТИП(Документ.ВозвратСырьяОтПереработчика)) 
	|		{(ДокументПоступления).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}
	|	) КАК ТоварыКОформлениюПоступления
	|{ГДЕ
	|	(""Возвраты сырья от переработчиков"") КАК ВидОперации,
...
Функция ТекстЗапросаПоступленияПродукцииОтПереработчиков()
	// Строковые литералы локализуются в настройках СКД отчета
	// Наборы данных - Поля - Доступные значения
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	""Поступления продукции от переработчиков"" КАК ВидОперации,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюОстаток > 0
	|			ТОГДА ""Оформление финансовых документов""
	|		ИНАЧЕ ""Оформление складских документов""
	...
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюОстаток > 0
	|			ТОГДА ""Оформите поступление от переработчика""
	|		ИНАЧЕ ""Оформите приходные ордера на товары""
	|	КОНЕЦ КАК Рекомендация,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюОстаток > 0
	...
	|	РегистрНакопления.ТоварыКОформлениюПоступления.Остатки(
	|		&Период, 
	|		ТИПЗНАЧЕНИЯ(ДокументПоступления) В (ТИП(Документ.ЗаказПереработчику)) 
	|		{(ДокументПоступления).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}
	|	) КАК ТоварыКОформлениюПоступления
	|{ГДЕ
	|	(""Поступления продукции от переработчиков"") КАК ВидОперации,
...
Функция ТекстЗапросаПереработкаСырьяДавальца()
	// Строковые литералы локализуются в настройках СКД отчета
	// Наборы данных - Поля - Доступные значения
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	""Переработка сырья давальца"" КАК ВидОперации,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюОстаток > 0
	|			ТОГДА ""Оформление финансовых документов""
	|		ИНАЧЕ ""Оформление складских документов""
	...
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюОстаток > 0
	|			ТОГДА ""Оформите поступление сырья от давальца""
	|		ИНАЧЕ ""Оформите приходные ордера на товары""
	|	КОНЕЦ КАК Рекомендация,
	|	ВЫБОР
	|		КОГДА ТоварыКОформлениюПоступления.КОформлениюОстаток > 0
	...
	|	РегистрНакопления.ТоварыКОформлениюПоступления.Остатки(
	|		&Период, 
	|		ТИПЗНАЧЕНИЯ(ДокументПоступления) В (ТИП(Документ.ЗаказДавальца)) 
	|		{(ДокументПоступления).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}
	|	) КАК ТоварыКОформлениюПоступления
	|{ГДЕ
	|	(""Переработка сырья давальца"") КАК ВидОперации,
...
Функция ТекстЗапросаВозвратыСырьяДавальцам()
	// Строковые литералы локализуются в настройках СКД отчета
	// Наборы данных - Поля - Доступные значения
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	""Возвраты сырья давальцам"" КАК ВидОперации,
	|	""Оформление складских документов"" КАК ТипОтклонения,
	|	ВЫБОР
	|		КОГДА ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
	|			ТОГДА ""Завершите оформление расходных ордеров на товары""
	...
	|	РегистрНакопления.ТоварыКОтгрузке.Остатки(
	|		&Период, 
	|		ТИПЗНАЧЕНИЯ(ДокументОтгрузки) В (ТИП(Документ.ВозвратСырьяДавальцу)) 
	|		{(ДокументОтгрузки).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}
	|	) КАК ТоварыКОтгрузке
	|ГДЕ
	|	ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
...
Функция ТекстЗапросаПередачиПродукцииДавальцам()
	// Строковые литералы локализуются в настройках СКД отчета
	// Наборы данных - Поля - Доступные значения
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	""Передачи продукции давальцам"" КАК ВидОперации,
	|	""Оформление складских документов"" КАК ТипОтклонения,
	|	ВЫБОР
	|		КОГДА ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
	|			ТОГДА ""Завершите оформление расходных ордеров на товары""
	...
	|	РегистрНакопления.ТоварыКОтгрузке.Остатки(
	|		&Период, 
	|		ТИПЗНАЧЕНИЯ(ДокументОтгрузки) В (ТИП(Документ.ЗаказДавальца)) 
	|		{(ДокументОтгрузки).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}
	|	) КАК ТоварыКОтгрузке
	|ГДЕ
	|	ТоварыКОтгрузке.КОтгрузкеОстаток - ТоварыКОтгрузке.КОформлениюОстаток <> 0
...
Функция ТекстЗапросаКонтрольОформленияДокументовТовародвижений()
	...
	//++ НЕ УТ
	//++ НЕ УТКА
	ПравоЧтениеЗаказНаПроизводство				= ПравоДоступа("Чтение", Метаданные.Документы.ЗаказНаПроизводство);
	//-- НЕ УТКА
	
	ПравоЧтениеЗаказМатериаловВПроизводство		= ПравоДоступа("Чтение", Метаданные.Документы.ЗаказМатериаловВПроизводство);
	ПравоЧтениеПередачаМатериаловВПроизводство	= ПравоДоступа("Чтение", Метаданные.Документы.ПередачаМатериаловВПроизводство);
	
	ПравоЧтениеВозвратМатериаловИзПроизводства	= ПравоДоступа("Чтение", Метаданные.Документы.ВозвратМатериаловИзПроизводства);
	
	ПравоЧтениеВыпускПродукции					= ПравоДоступа("Чтение", Метаданные.Документы.ВыпускПродукции);
	
	ПравоЧтениеЗаказПереработчику				= ПравоДоступа("Чтение", Метаданные.Документы.ЗаказПереработчику);
	ПравоЧтениеВозвратСырьяОтПереработчика		= ПравоДоступа("Чтение", Метаданные.Документы.ВозвратСырьяОтПереработчика);
	//++ НЕ УТКА
	ПравоЧтениеЗаказДавальца					= ПравоДоступа("Чтение", Метаданные.Документы.ЗаказДавальца);
	ПравоЧтениеВозвратСырьяДавальцу				= ПравоДоступа("Чтение", Метаданные.Документы.ВозвратСырьяДавальцу);
	//-- НЕ УТКА
	//-- НЕ УТ 
	...
	Если ПравоЧтениеТоварыКОтгрузке
		//++ НЕ УТКА
		И ПравоЧтениеЗаказНаПроизводство
		//-- НЕ УТКА
		И ПравоЧтениеЗаказМатериаловВПроизводство
		И ПравоЧтениеПередачаМатериаловВПроизводство
		И ТекстыЗапроса.Свойство("ТекстЗапросаПередачаМатериаловВПроизводство") Тогда
		ПодключитьЗапрос(ТекстЗапроса, ТекстЗапросаПередачаМатериаловВПроизводство(), КоличествоПодключенныхЗапросов);
	КонецЕсли;
	Если ПравоЧтениеТоварыКОформлениюПоступления
		И ПравоЧтениеВозвратМатериаловИзПроизводства
		И ТекстыЗапроса.Свойство("ТекстЗапросаВозвратыМатериаловИзПроизводства") Тогда
		ПодключитьЗапрос(ТекстЗапроса, ТекстЗапросаВозвратыМатериаловИзПроизводства(), КоличествоПодключенныхЗапросов);
	КонецЕсли;
	Если ПравоЧтениеТоварыКОформлениюПоступления
		//++ НЕ УТКА
		И ПравоЧтениеЗаказНаПроизводство
		//-- НЕ УТКА
		И ПравоЧтениеВыпускПродукции
		И ТекстыЗапроса.Свойство("ТекстЗапросаПриемкаГотовойПродукции") Тогда
		ПодключитьЗапрос(ТекстЗапроса, ТекстЗапросаПриемкаГотовойПродукции(), КоличествоПодключенныхЗапросов);
	КонецЕсли;
	
	// Переработка сырья у переработчика
	Если ПравоЧтениеТоварыКОтгрузке
		И ПравоЧтениеЗаказПереработчику
		И ТекстыЗапроса.Свойство("ТекстЗапросаПереработкаСырьяУПереработчика") Тогда
		ПодключитьЗапрос(ТекстЗапроса, ТекстЗапросаПереработкаСырьяУПереработчика(), КоличествоПодключенныхЗапросов);
	КонецЕсли;
	Если ПравоЧтениеТоварыКОформлениюПоступления
		И ПравоЧтениеВозвратСырьяОтПереработчика
		И ТекстыЗапроса.Свойство("ТекстЗапросаВозвратыСырьяОтПереработчиков") Тогда
		ПодключитьЗапрос(ТекстЗапроса, ТекстЗапросаВозвратыСырьяОтПереработчиков(), КоличествоПодключенныхЗапросов);
	КонецЕсли;
	Если ПравоЧтениеТоварыКОформлениюПоступления
		И ПравоЧтениеЗаказПереработчику
		И ТекстыЗапроса.Свойство("ТекстЗапросаПоступленияПродукцииОтПереработчиков") Тогда
		ПодключитьЗапрос(ТекстЗапроса, ТекстЗапросаПоступленияПродукцииОтПереработчиков(), КоличествоПодключенныхЗапросов);
	КонецЕсли;
	//-- НЕ УТ 
	
	//++ НЕ УТКА
	// Переработка сырья давальца
	Если ПравоЧтениеТоварыКОформлениюПоступления
		И ПравоЧтениеЗаказДавальца
		И ТекстыЗапроса.Свойство("ТекстЗапросаПереработкаСырьяДавальца") Тогда
		ПодключитьЗапрос(ТекстЗапроса, ТекстЗапросаПереработкаСырьяДавальца(), КоличествоПодключенныхЗапросов);
	КонецЕсли;
	Если ПравоЧтениеТоварыКОтгрузке
		И ПравоЧтениеВозвратСырьяДавальцу
		И ТекстыЗапроса.Свойство("ТекстЗапросаВозвратыСырьяДавальцам") Тогда
		ПодключитьЗапрос(ТекстЗапроса, ТекстЗапросаВозвратыСырьяДавальцам(), КоличествоПодключенныхЗапросов);
	КонецЕсли;
	Если ПравоЧтениеТоварыКОтгрузке
		И ПравоЧтениеЗаказДавальца
		И ТекстыЗапроса.Свойство("ТекстЗапросаПередачиПродукцииДавальцам") Тогда
		ПодключитьЗапрос(ТекстЗапроса, ТекстЗапросаПередачиПродукцииДавальцам(), КоличествоПодключенныхЗапросов);
	КонецЕсли;
	//-- НЕ УТКА
...
Функция ДоступныеВидыОперацийИЗапросы()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство") Тогда
		ДобавитьВидОперацииИЗапрос(Доступные, "Передача материалов в производство", "ТекстЗапросаПередачаМатериаловВПроизводство");
		ДобавитьВидОперацииИЗапрос(Доступные, "Возвраты материалов из производства", "ТекстЗапросаВозвратыМатериаловИзПроизводства");
		ДобавитьВидОперацииИЗапрос(Доступные, "Приемка готовой продукции", "ТекстЗапросаПриемкаГотовойПродукции");
	КонецЕсли;
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне") Тогда
		ДобавитьВидОперацииИЗапрос(Доступные, "Переработка сырья у переработчика", "ТекстЗапросаПереработкаСырьяУПереработчика");
		ДобавитьВидОперацииИЗапрос(Доступные, "Возвраты сырья от переработчиков", "ТекстЗапросаВозвратыСырьяОтПереработчиков");
		ДобавитьВидОперацииИЗапрос(Доступные, "Поступления продукции от переработчиков", "ТекстЗапросаПоступленияПродукцииОтПереработчиков");
	КонецЕсли;
	//-- НЕ УТ
	//++ НЕ УТКА
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья") Тогда
		ДобавитьВидОперацииИЗапрос(Доступные, "Переработка сырья давальца", "ТекстЗапросаПереработкаСырьяДавальца");
		ДобавитьВидОперацииИЗапрос(Доступные, "Возвраты сырья давальцам", "ТекстЗапросаВозвратыСырьяДавальцам");
		ДобавитьВидОперацииИЗапрос(Доступные, "Передачи продукции давальцам", "ТекстЗапросаПередачиПродукцииДавальцам");
	КонецЕсли;
	//-- НЕ УТКА
