...
Процедура ЗаменитьАналитикуУчетаПоПартнерамОчиститьЗаказыДанныеДляОбновления(Параметры) Экспорт
	...
	|	ИЛИ ТИПЗНАЧЕНИЯ(Расчеты.РасчетныйДокумент) = ТИП(Документ.ЗаказПоставщику)
	|	ИЛИ Расчеты.ДатаПлатежа <> Расчеты.УдалитьДатаПлатежа
	|");
	
	ДополнительныеПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыОтметкиОбработки();
	ДополнительныеПараметры.Вставить("ЭтоДвижения", Истина);
	ДополнительныеПараметры.Вставить("ПолноеИмяРегистра", "РегистрНакопления.РасчетыСПоставщикамиПоДокументам");
...
Процедура ЗаменитьАналитикуУчетаПоПартнерамОчиститьЗаказы(Параметры) Экспорт
	...
	|		ТОГДА Расчеты.ЗаказПоставщику
	|	ИНАЧЕ
	|		ЕСТЬNULL(Расчеты.ЗаказПоставщику.Договор, ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка))
	|	КОНЕЦ                                                КАК Договор
	|ИЗ
	...
	|				ВЫБОР КОГДА Расчеты.ЗаказПоставщику ССЫЛКА Справочник.ДоговорыКонтрагентов
	|					ТОГДА Расчеты.ЗаказПоставщику
	|				ИНАЧЕ ЕСТЬNULL(Расчеты.ЗаказПоставщику.Договор, ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка))
	|			КОНЕЦ)
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЗапросДанных.УстановитьПараметр("Регистратор", Выборка.Ссылка);
