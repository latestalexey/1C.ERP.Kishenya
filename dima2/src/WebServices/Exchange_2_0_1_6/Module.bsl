...
Функция СоздатьОбменДанными(ИмяПланаОбмена, СтрокаПараметров, НастройкаОтборовXDTO, ЗначенияПоУмолчаниюXDTO)
	...
	Если Отказ Тогда
		Сообщение = НСтр("ru='При создании настройки обмена во второй информационной базе возникли ошибки: %1';uk='При створенні настройки обміну в другій інформаційній базі виникли помилки: %1'");
...
Функция GetFilePart(TransferId, PartNumber, PartData)
	...
	Если ИменаФайлов.Количество() = 0 Тогда
		...
		ШаблонСообщения = НСтр("ru='Не найден фрагмент %1 сессии передачи с идентификатором %2';uk='Не знайдений фрагмент %1 сесії передачі з ідентифікатором %2'");
	...
	ИначеЕсли ИменаФайлов.Количество() > 1 Тогда
		...
		ШаблонСообщения = НСтр("ru='Найдено несколько фрагментов %1 сессии передачи с идентификатором %2';uk='Знайдено кілька фрагментів %1 сесії передачі з ідентифікатором %2'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Строка(PartNumber), Строка(TransferId));
		ВызватьИсключение(ТекстСообщения);
	...
	Для НомерЧасти = 1 По PartQuantity Цикл
		...
		Если НайтиФайлы(ИмяФайла).Количество() = 0 Тогда
			ШаблонСообщения = НСтр("ru='Не найден фрагмент %1 сессии передачи с идентификатором %2.
                    ...
                    |;uk='Не знайдений фрагмент %1 сесії передачі з ідентифікатором %2.
                    |Необхідно переконатися, що в настройках програми задані параметри
                    |""Каталог тимчасових файлів для Linux"" й ""Каталог тимчасових файлів для Windows"".'");
...
Функция TestConnection(ИмяПланаОбмена, КодУзла, Результат)
	...
	Если ПланыОбмена[ИмяПланаОбмена].НайтиПоКоду(КодУзла).Пустая() Тогда
		Результат = НСтр("ru='Возможность синхронизации данных отключена администратором приложения в Интернете.';uk='Можливість синхронізації даних відключена адміністратором додатка в Інтернеті.'");
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПроверитьБлокировкуИнформационнойБазыДляОбновления()
	...
	Если ЗначениеЗаполнено(ОбновлениеИнформационнойБазыСлужебный.ИнформационнаяБазаЗаблокированаДляОбновления()) Тогда
		...
		ВызватьИсключение НСтр("ru='Синхронизация данных временно недоступна в связи с обновлением приложения в Интернете.';uk='Синхронізація даних тимчасово недоступна у зв''язку з оновленням додатка в Інтернеті.'");
...
Процедура ВыполнитьВыгрузкуДанныхВКлиентСерверномРежиме(ИмяПланаОбмена,
	...
	Если ФоновыеЗадания.ПолучитьФоновыеЗадания (Отбор).Количество() = 1 Тогда
		ВызватьИсключение НСтр("ru='Синхронизация данных уже выполняется.';uk='Синхронізація даних уже виконується.'");
	КонецЕсли;
	...
	ФоновоеЗадание = ФоновыеЗадания.Выполнить("ОбменДаннымиСервер.ВыполнитьВыгрузкуДляУзлаИнформационнойБазыВСервисПередачиФайлов",
										Параметры,
										КлючФоновогоЗадания,
										НСтр("ru='Выполнение обмена данными через веб-сервис.';uk='Виконання обміну даними через веб-сервіс.'"));
	...
	Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Завершено Тогда
		...
		ВызватьИсключение НСтр("ru='Ошибка при выгрузке данных через веб-сервис.';uk='Помилка при вивантаженні даних через веб-сервіс.'");
	КонецЕсли;
...
Процедура ВыполнитьЗагрузкуДанныхВКлиентСерверномРежиме(ИмяПланаОбмена,
	...
	Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Завершено Тогда
		...
		ВызватьИсключение НСтр("ru='Ошибка при загрузке данных через веб-сервис.';uk='Помилка при завантаженні даних через веб-сервіс.'");
	КонецЕсли;
...
Функция ЗарегистрироватьДанныеДляНачальнойВыгрузки(Знач ИмяПланаОбмена, Знач КодУзла, ДлительнаяОперация, ИдентификаторОперации, ТолькоСправочники)
	...
	Иначе
		...
		ФоновоеЗадание = ФоновыеЗадания.Выполнить(ИмяМетода, Параметры,, НСтр("ru='Создание обмена данными.';uk='Створення обміну даними.'"));
