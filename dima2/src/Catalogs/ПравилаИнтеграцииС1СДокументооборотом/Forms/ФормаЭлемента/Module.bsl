...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(СокращенноеНаименованиеКонфигурации) Тогда
		...
		Элементы.ПравилаЗаполненияРеквизитовПриЗагрузкеПредставлениеРеквизитаОбъектаПотребителя.Заголовок = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Реквизит %1';uk='Реквізит %1'"), СокращенноеНаименованиеКонфигурации);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(ПредставлениеОбъектаПотребителя) Тогда 
		...
		Сообщение.Текст = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("ru='Объект конфигурации';uk='Об''єкт конфігурації'"));
	...
	Если Не ЗначениеЗаполнено(ПредставлениеОбъектаДокументооборота) Тогда 
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("ru='Объект Документооборота';uk='Об''єкт Документообігу'"));
		...
		Для каждого Строка из ПравилаЗаполненияРеквизитовПриВыгрузке Цикл
			Если Строка.ОбязательноеЗаполнение 
				...
				Сообщение.Текст = СтрШаблон(
					НСтр("ru='Не установлено обязательное правило заполнения для реквизита ""%1""';uk='Не встановлено обов''язкове правило заповнення для реквізиту ""%1""'"),
...
Процедура ТипОбъектаДокументооборотаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СписокВыбора = СписокОбъектовДокументооборота(ДоступенФункционалКорреспонденты);
...
Процедура ТипОбъектаДокументооборотаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение <> Неопределено Тогда
		...
		СтандартнаяОбработка = Ложь;
		ВыбранныйЭлемент = СписокОбъектовДокументооборота(ДоступенФункционалКорреспонденты).НайтиПоЗначению(ВыбранноеЗначение);
...
Процедура ПравилаЗаполненияРеквизитовПриВыгрузкеПередУдалением(Элемент, Отказ)
	...
	Если Объект.ШаблонЗапрещаетИзменение
		...
		ПоказатьПредупреждение(, НСтр("ru='Выбранный шаблон запрещает изменение реквизитов.';uk='Вибраний шаблон забороняє зміну реквізитів.'"));
		Возврат;
	КонецЕсли;
	
	Если ТекущиеДанные.ИмяРеквизитаОбъектаДокументооборота = "documentType"
		И ТекущиеДанные.ЗаполненВШаблоне Тогда
		ПоказатьПредупреждение(, НСтр("ru='Вид документа указан в шаблоне и не может быть изменен.';uk='Вид документа зазначений в шаблоні і не може бути змінений.'"));
		Возврат;
	КонецЕсли;
	...
	ТекстВопроса = НСтр("ru='Очистить правило заполнения реквизита?';uk='Очистити правило заповнення реквізиту?'");
...
Процедура ЗаполнитьПоУмолчаниюПриВыгрузке(Команда)
	...
	Если НужноЗадатьВопрос Тогда
		...
		ТекстВопроса = НСтр("ru='Правила заполнения для реквизитов будут заменены 
                                ...
                                |;uk='Правила заповнення реквізитів будуть замінені 
                                |на правила заповнення за замовчуванням. Продовжити?'");
...
Процедура ОбработатьФормуСогласноВерсииСервиса()
	...
	Иначе // версия известна
		...
		Иначе
			...
			ДоступенФункционалКорреспонденты = ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.3.2.3", Истина);
...
Функция СписокОбъектовДокументооборота(Корреспонденты)

	Список = Новый СписокЗначений;
	
	Список.Добавить("DMIncomingDocument", НСтр("ru='Входящий документ';uk='Вхідний документ'"));
	Список.Добавить("DMOutgoingDocument", НСтр("ru='Исходящий документ';uk='Вихідний документ'"));
	Список.Добавить("DMInternalDocument", НСтр("ru='Внутренний документ';uk='Внутрішній документ'"));
	Если Корреспонденты Тогда
		Список.Добавить("DMCorrespondent",    НСтр("ru='Корреспондент';uk='Кореспондент'"));
	КонецЕсли;
...
Процедура ЗаполнитьПоУмолчаниюПриВыгрузкеНаСервере(НачальноеЗаполнение = Ложь)
	...
	Если СтрокиВидаДокумента.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(СтрокаПравил.ТипЗначенияРеквизита) И ДоступенФункционалКорреспонденты Тогда
...
Процедура ЗаполнитьПоУмолчаниюПриЗагрузкеНаСервере(НачальноеЗаполнение = Ложь)
	...
	СтруктураСостояний.Вставить("statusRegistration", НСтр("ru='Состояние регистрации';uk='Стан реєстрації'"));
	СтруктураСостояний.Вставить("statusConsideration", НСтр("ru='Состояние рассмотрения';uk='Стан розгляду'"));
	СтруктураСостояний.Вставить("statusApproval", НСтр("ru='Состояние согласования';uk='Стан погодження'"));
	СтруктураСостояний.Вставить("statusConfirmation", НСтр("ru='Состояние утверждения';uk='Стан затвердження'"));
	СтруктураСостояний.Вставить("statusPerformance", НСтр("ru='Состояние исполнения';uk='Стан виконання'"));
