...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			...
			НСтр("ru='Подготовленные ответственным за список';uk='Підготовлені відповідальним за список'"), Ложь,
...
Процедура ВидПользователейНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект, НСтр("ru='Выбор вида пользователей';uk='Вибір виду користувачів'"), Ложь, Истина, ОписаниеОповещения);
...
Процедура ГруппыВнешнихПользователейПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ВыбиратьИерархически Тогда
		ПоказатьПредупреждение(,
			НСтр("ru='Для перетаскивания пользователя в группы необходимо отключить
                       ...
                       |;uk='Для перетягування користувача в групи необхідно відключити
                       |прапорець ""Показувати користувачів нижчих груп"".'"));
		Возврат;
	КонецЕсли;
	...
	Если Строка = ХранимыеПараметры.ГруппаВсеПользователи
		...
		СообщениеПользователю = Новый Структура("Сообщение, ЕстьОшибки, Пользователи",
			НСтр("ru='Из групп с типом участников ""Все пользователи заданного типа"" исключение пользователей невозможно.';uk='Із груп з типом учасників ""Всі користувачі заданого типу"" виключення користувачів неможливо.'"),
			Истина,
			Неопределено);
	Иначе
		...
		ДействиеСПользователем = 
			...
			НСтр("ru='Включить';uk='Включити'"),
			?(Перемещение, НСтр("ru='Переместить';uk='Перемістити'"), НСтр("ru='Скопировать';uk='Скопіювати'")));
		
		Если ГруппаПомеченаНаУдаление Тогда
			ШаблонДействия = ?(Перемещение, НСтр("ru='Группа ""%1"" помечена на удаление. %2';uk='Групу ""%1"" позначено на вилучення. %2'"), 
				НСтр("ru='Группа ""%1"" помечена на удаление. %2';uk='Групу ""%1"" позначено на вилучення. %2'"));
		...
		Если КоличествоПользователей = 1 Тогда
			Если ДействиеИсключитьПользователя Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Исключить пользователя ""%1"" из группы ""%2""?';uk='Виключити користувача ""%1"" з групи ""%2""?'"),
			...
			ИначеЕсли Не ГруппаПомеченаНаУдаление Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='%1 пользователя ""%2"" в группу ""%3""?';uk='%1 користувача ""%2"" у групу ""%3""?'"),
					ДействиеСПользователем,
					Строка(ПараметрыПеретаскивания.Значение[0]),
					Строка(Строка));
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='%1 пользователя ""%2"" в эту группу?';uk='%1 користувача ""%2"" у цю групу?'"),
					ДействиеСПользователем,
					Строка(ПараметрыПеретаскивания.Значение[0]));
			КонецЕсли;
		Иначе
			Если ДействиеИсключитьПользователя Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Исключить пользователей (%1) из группы ""%2""?';uk='Виключити користувачів (%1) з групи ""%2""?'"),
					КоличествоПользователей,
					Строка(Элементы.ГруппыВнешнихПользователей.ТекущаяСтрока));
				
			ИначеЕсли Не ГруппаПомеченаНаУдаление Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='%1 пользователей (%2) в группу ""%3""?';uk='%1 користувачів (%2) у групу ""%3""?'"),
					ДействиеСПользователем,
					КоличествоПользователей,
					Строка(Строка));
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='%1 пользователей (%2) в эту группу?';uk='%1 користувачів (%2) у дану групу?'"),
					ДействиеСПользователем,
					КоличествоПользователей);
			КонецЕсли;
		КонецЕсли;
...
Процедура ВнешниеПользователиСписокПриАктивизацииСтроки(Элемент)
	...
	Если Элементы.ВнешниеПользователиСписок.ТекущиеДанные = Неопределено Тогда
	...
	Если ТекущиеДанные.ВсеОбъектыАвторизации Тогда
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Невозможно добавить подгруппу к группе ""%1"", 
                       ...
                       |тому що в число її учасників входять всі користувачі обраних видів.'"),
...
Процедура ИзменитьПараметрыРасширеннойФормыПодбора()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьГруппыПользователей") Тогда
		...
		Элементы.ВнешниеПользователиСписок.Заголовок             = НСтр("ru='Пользователи в группе';uk='Користувачі в групі'");
...
Процедура ОбновитьЗаголовокСпискаВыбранныхПользователейИГрупп()
	...
	Если ХранимыеПараметры.ИспользоватьГруппы Тогда
		ЗаголовокВыбранныеПользователиИГруппы = НСтр("ru='Выбранные пользователи и группы (%1)';uk='Обрані користувачі й групи (%1)'");
	Иначе
		ЗаголовокВыбранныеПользователиИГруппы = НСтр("ru='Выбранные пользователи (%1)';uk='Обрані користувачі (%1)'");
	КонецЕсли;
	...
	Иначе
		...
		Если ХранимыеПараметры.ИспользоватьГруппы Тогда
			Элементы.СписокВыбранныхПользователейИГрупп.Заголовок = НСтр("ru='Выбранные пользователи и группы';uk='Обрані користувачі й групи'");
		Иначе
			Элементы.СписокВыбранныхПользователейИГрупп.Заголовок = НСтр("ru='Выбранные пользователи';uk='Обрані користувачі'");
		КонецЕсли;
	...
	Если Параметры.РежимВыбора Тогда
		...
		Если Параметры.ЗакрыватьПриВыборе = Ложь Тогда
			...
			Если ВыборГруппВнешнихПользователей Тогда
				...
				Иначе
					Заголовок = НСтр("ru='Подбор внешних пользователей и групп';uk='Підбір зовнішніх користувачів і груп'");
				КонецЕсли;
				
				Элементы.ВыбратьВнешнегоПользователя.Заголовок =
					НСтр("ru='Выбрать внешних пользователей';uk='Вибрати зовнішніх користувачів'");
				
				Элементы.ВыбратьГруппуВнешнихПользователей.Заголовок =
					НСтр("ru='Выбрать группы';uk='Вибрати групи'");
				
			Иначе
				Если ХранимыеПараметры.РасширенныйПодбор Тогда
					Заголовок = ХранимыеПараметры.ЗаголовокФормыПодбора;
				Иначе
					Заголовок = НСтр("ru='Подбор внешних пользователей';uk='Підбір зовнішніх користувачів'");
				КонецЕсли;
			КонецЕсли;
		Иначе
			// Режим выбора.
			Если ВыборГруппВнешнихПользователей Тогда
				Заголовок = НСтр("ru='Выбор внешнего пользователя или группы';uk='Вибір зовнішнього користувача або групи'");
				
				Элементы.ВыбратьВнешнегоПользователя.Заголовок = НСтр("ru='Выбрать внешнего пользователя';uk='Вибрати зовнішнього користувача'");
			Иначе
				Заголовок = НСтр("ru='Выбор внешнего пользователя';uk='Вибір зовнішнього користувача'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ГруппыВнешнихПользователейПеретаскиваниеЗавершение(СообщениеПользователю)
	...
	Если СообщениеПользователю.ЕстьОшибки = Ложь Тогда
		ПоказатьОповещениеПользователя(
			НСтр("ru='Перемещение пользователей';uk='Переміщення користувачів'"), , СообщениеПользователю.Сообщение, БиблиотекаКартинок.Информация32);
	Иначе
		...
		Если СообщениеПользователю.Пользователи <> Неопределено Тогда
			Подробно = НСтр("ru='Следующие пользователи не были включены в выбранную группу:';uk='Наступні користувачі не були включені в обрану групу:'");
