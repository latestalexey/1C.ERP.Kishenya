...
// Процедура заполнения колонки "ИнвентарныйУчет" в таблице формы
//
// Параметры:
// 		ТаблицаФормы - ДанныеФормыКоллекция
//
Процедура ЗаполнитьПризнакиПартииТМЦВЭксплуатации(ТаблицаФормы) Экспорт
	...
	|ВЫБРАТЬ
	|	Таблица.НомерСтроки - 1 КАК ИндексСтроки,
	|	ЕСТЬNULL(Таблица.ПартияТМЦВЭксплуатации.ИнвентарныйУчет, ЛОЖЬ) КАК ИнвентарныйУчет
	|ИЗ
	|	Таблица КАК Таблица";
	
	Запрос.УстановитьПараметр("Таблица", ТаблицаФормы.Выгрузить(,"НомерСтроки, ПартияТМЦВЭксплуатации"));
...
#Область ЗаполнитьСпособПогашенияСтоимостиНУ_2_1_2
