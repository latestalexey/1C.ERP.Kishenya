...
Процедура ПроверкаИспользования(ОбъектыРасширений = Ложь) Экспорт
	...
	Если СтандартныеПодсистемыПовтИсп.ОтключитьИдентификаторыОбъектовМетаданных() Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Справочник ""%1"" не используется.';uk='Довідник ""%1"" не використовується.'"), НазваниеСправочника(ОбъектыРасширений));
	КонецЕсли;
	   ...
	   И ЗначениеЗаполнено(ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени("Константа.ГлавныйУзел").Получить()) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Справочник ""%1"" не может использоваться
                       ...
                       |
                       |Для восстановления связи с главным узлом запустите платформу и
                       |нажмите кнопку Восстановить или программно установите главный узел,
                       |сохраненный в константе Главный узел.
                       |
                       |Для подтверждения отмены связи с главным узлом запустите платформу и
                       |нажмите кнопку Отключить или программно очистите константу Главный узел.'
                       |;uk='Довідник ""%1"" не може використовуватися
                       |в інформаційній базі з непідтвердженим скасуванням головного вузла.
                       |
                       |Для відновлення зв''язку з головним вузлом запустити платформу і
                       |натисніть кнопку Відновити або програмно встановіть головний вузол,
                       |збережений у константі Головний вузол.
                       |
                       |Для підтвердження скасування зв''язку з головним вузлом запустити платформу і
                       |натисніть кнопку Вимкнути або програмно очистіть константу Головний вузол.'"),
				...
				Если Не ЗначениеЗаполнено(СвойстваЗагружаемого.Ссылка) Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Ошибка загрузки идентификаторов объектов метаданных.
                                   ...
                                   |""%1"".'
                                   |;uk='Помилка завантаження ідентифікаторів об''єктів метаданих.
                                   |Неможливо завантажити новий елемент у якого не зазначене посилання нового:
                                   |""%1"".'"),
						Объект.ПолноеИмя);
				КонецЕсли;
			КонецЕсли;
			...
			Если ПолныеИменаЗагружаемых.Получить(НРег(Объект.ПолноеИмя)) <> Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка загрузки идентификаторов объектов метаданных.
                               ...
                               |""%1"".'
                               |;uk='Помилка завантаження ідентифікаторів об''єктів метаданих.
                               |Неможливо завантажити два елементи в яких збігається повне ім''я:
                               |""%1"".'"),
					Объект.ПолноеИмя);
			КонецЕсли;
				...
				Если КлючиЗагружаемых.Получить(КлючОбъектаМетаданных) <> Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Ошибка загрузки идентификаторов объектов метаданных.
                                   ...
                                   |""%1"".'
                                   |;uk='Помилка завантаження ідентифікаторів об''єктів метаданих.
                                   |Неможливо завантажити два елементи в яких збігається ключ об''єкта метаданих:
                                   |""%1"".'"),
				...
				Если СвойстваЗагружаемого.ОбъектМетаданныхПоКлючу <> СвойстваЗагружаемого.ОбъектМетаданныхПоПолномуИмени Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Ошибка загрузки идентификаторов объектов метаданных.
                                   ...
                                   |;uk='Помилка завантаження ідентифікаторів об''єктів метаданих.
                                   |Неможливо завантажити елемент у якого ключ об''єкта метаданих
                                   |""%1"" не відповідає повному імені ""%2"".'"),
	   ...
	   И УстановленныеРасширения <> ПараметрыСеанса.УстановленныеРасширения Тогда
		...
		ТекстОшибки =
			НСтр("ru='Невозможно определить объекты метаданных расширений в фоновом задании
                       ...
                       |;uk='Неможливо визначити об''єкти метаданих розширень у фоновому завданні
                       |без перезапуску сеансу, оскільки склад розширень був змінений.'");
	   ...
	   И ЗначениеЗаполнено(ПараметрыСеанса.УстановленныеРасширения) Тогда
		...
		ТекстОшибки =
			НСтр("ru='Невозможно определить объекты метаданных расширений в сеансе
                       |неразделенного пользователя, т.к. расширения не подключены.'
                       |;uk='Неможливо визначити об''єкти метаданих розширень в сеансі
                       |неподіленого користувача, т. к. розширення не підключені.'");
		...
		Если СвойстваРасширений = Неопределено Тогда
			ТекстОшибки =
				НСтр("ru='Невозможно определить объекты метаданных расширений
                           |без перезапуска сеанса, т.к. состав расширений был изменен.'
                           |;uk='Неможливо визначити об''єкти метаданих розширень
                           |без перезапуску сеансу, оскільки склад розширень був змінений.'");
			ВызватьИсключениеПоОшибке(Ложь, ТекстОшибки);
		КонецЕсли;
	Иначе
		...
		Если ОбъектМетаданных = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных().
                           ...
                           |""%1"".'
                           |;uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных().
                           |
                           |Об''єкт метаданих не знайдений по типу:
                           |""%1"".'"),
...
Функция ИдентификаторОбъектаМетаданныхПоПолномуИмени(ПолноеИмяОбъектаМетаданных) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных().';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных().'");
	...
	Если Выгрузка.Количество() = 0 Тогда
		...
		Если ОбъектМетаданных = Неопределено Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Объект метаданных не найден по полному имени:
                           |""%1"".'
                           |;uk='Об''єкт метаданих не знайдений за повним ім''ям:
                           |""%1"".'"),
		   ...
		   И Не ЭтоПодсистема(ОбъектМетаданных) Тогда
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Объект метаданных не поддерживается:
                           ...
                           |;uk='Об''єкт метаданих не підтримується:
                           |""%1"".
                           |
                           |Допустимі тільки типи об''єктів метаданих перелічені у коментарі до функції.'"),
				ПолноеИмяОбъектаМетаданных);
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + ОписаниеОшибки;
		КонецЕсли;
		...
		ШаблонОшибки = ?(ДоступныИдентификаторыОбъектовРасширений,
			НСтр("ru='Для объекта метаданных ""%1""
                       ...
                       |;uk='Для об''єкта метаданих ""%1""
                       |не знайдено ідентифікатор в довіднику ""Ідентифікатори об''єктів метаданих"" і
                       |регістрі відомостей ""Ідентифікатори об''єктів розширень"".'"),
			НСтр("ru='Для объекта метаданных ""%1""
                       |не найден идентификатор в справочнике ""Идентификаторы объектов метаданных"".'
                       |;uk='Для об''єкта метаданих ""%1""
                       |не знайдено ідентифікатор в довіднику ""Ідентифікатори об''єктів метаданих"".'"));
	...
	ИначеЕсли Выгрузка.Количество() > 1 Тогда
		ШаблонОшибки = ?(ДоступныИдентификаторыОбъектовРасширений,
			НСтр("ru='Для объекта метаданных ""%1""
                       ...
                       |регистре сведений ""Идентификаторы объектов версий расширений"".'
                       |;uk='Для об''єкта метаданих ""%1""
                       |знайдено декілька ідентифікаторів, у довіднику ""Ідентифікатори об''єктів метаданих"" і
                       |регістрі відомостей ""Ідентифікатори об''єктів розширень"".'"),
			НСтр("ru='Для объекта метаданных ""%1""
                       |найдено несколько идентификаторов в справочнике ""Идентификаторы объектов метаданных"".'
                       |;uk='Для об''єкта метаданих ""%1""
                       |знайдено декілька ідентифікаторів, у довіднику ""Ідентифікатори об''єктів метаданих"".'"));
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ПолноеИмяОбъектаМетаданных)
			+ СтандартныеПодсистемыСервер.УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика();
	КонецЕсли;
		...
		Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для объекта метаданных ""%1""
                           ...
                           |;uk='Для об''єкта метаданих ""%1""
                           |знайдено ідентифікатор в довіднику ""%2"",
                           |якому відповідає віддалений об''єкт метаданих.'"),
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для объекта метаданных ""%1""
                           ...
                           |;uk='Для об''єкта метаданих ""%1""
                           |знайдено ідентифікатор в довіднику ""%2"",
                           |який відповідає іншому об''єкту метаданих ""%3"".'"),
...
Функция ОбъектМетаданныхПоИдентификатору(Идентификатор) Экспорт
	...
	Выгрузка = Запрос.Выполнить().Выгрузить();
	ЗаголовокОшибки = НСтр("ru='Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().';uk='Помилка при виконанні функції ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().'");
			...
			ИначеЕсли РезультатПроверки.КлючОбъектаМетаданных = Неопределено Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Идентификатору ""%1""
                               ...
                               |""%3"".'
                               |;uk='Ідентифікатором ""%1""
                               |знайденому в довіднику ""%2"",
                               |відповідає неіснуючий об''єкт метаданих
                               |""%3"".'"),
			...
			Иначе
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Идентификатору ""%1""
                               ...
                               |;uk='Ідентифікатору ""%1""
                               |знайденому в довіднику ""%2"",
                               |відповідає віддалений об''єкт метаданих.'"),
					Строка(Идентификатор), НазваниеСправочника);
			КонецЕсли;
		Иначе
			ОписаниеОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Идентификатору ""%1""
                           ...
                           |;uk='Ідентифікатору ""%1""
                           |знайденому в довіднику ""%2"",
                           |відповідає об''єкт метаданих ""%3"",
                           |повне ім''я якого відрізняється від заданого в ідентифікаторі.'"),
	...
	Если Выгрузка[0].ПометкаУдаления Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Идентификатор ""%1""
                       ...
                       |;uk='Ідентифікатор користувача ""%1""
                       |знайдений у довіднику ""%2"",
                       |але значення реквізиту ""Позначка вилучення"" встановлено Істина.'"),
			Строка(Идентификатор), НазваниеСправочника);
...
Процедура ДобавитьПереименование(Итог, ВерсияИБ, СтароеПолноеИмя, НовоеПолноеИмя, ИдентификаторБиблиотеки = "") Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ru='Ошибка в процедуре ПриДобавленииПереименованийОбъектовМетаданных общего модуля ОбщегоНазначенияПереопределяемый.';uk='Помилка в процедурі ПриДобавленииПереименованийОбъектовМетаданных загального модуля ОбщегоНазначенияПереопределяемый.'");
	...
	Если СтароеИмяКоллекции <> НовоеИмяКоллекции Тогда
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не совпадают имена типов переименованного объекта метаданных.
                       |Прежний тип: ""%1"",
                       |новый тип: ""%2"".'
                       |;uk='Не збігаються імена типів перейменованого об''єкта метаданих.
                       |Колишній тип: ""%1"",
                       |новий тип: ""%2"".'"),
			СтароеПолноеИмя,
			НовоеПолноеИмя);
	...
	Если Итог.КоллекцииБезКлюча[СтароеИмяКоллекции] = Неопределено Тогда
		...
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для типа объекта метаданных ""%1"" не требуется описывать переименование,
                       ...
                       |%2.'
                       |;uk='Для типу об''єкта метаданих ""%1"" не потрібно описувати перейменування,
                       |тому що відомості про об''єкти метаданих цього типу оновляються автоматично.
                       |
                       |Описувати перейменування потрібно тільки для наступних типів:
                       | %2.'"),
			СтароеПолноеИмя,
			СписокДопустимыхТипов);
	КонецЕсли;
...
Функция СвойстваКоллекцийОбъектовМетаданных(ОбъектыРасширений = Ложь) Экспорт
	...
	Строка.Синоним         = НСтр("ru='Константы';uk='Константи'");
	Строка.ИмяВЕдЧисле     = "Константа";
	Строка.СинонимВЕдЧисле = НСтр("ru='Константа';uk='Константа'");
	...
	Строка.Синоним         = НСтр("ru='Планы видов расчета';uk='Плани видів розрахунку'");
	Строка.ИмяВЕдЧисле     = "ПланВидовРасчета";
	Строка.СинонимВЕдЧисле = НСтр("ru='План видов расчета';uk='План видів розрахунку'");
	...
	Строка.Синоним         = НСтр("ru='Регистры накопления';uk='Регістри накопичення'");
	Строка.ИмяВЕдЧисле     = "РегистрНакопления";
	Строка.СинонимВЕдЧисле = НСтр("ru='Регистр накопления';uk='Регістр накопичення'");
	...
	Строка.Синоним         = НСтр("ru='Регистры расчета';uk='Регістри розрахунку'");
	Строка.ИмяВЕдЧисле     = "РегистрРасчета";
	Строка.СинонимВЕдЧисле = НСтр("ru='Регистр расчета';uk='Регістр розрахунку'");
	...
	Строка.Имя             = "Задачи";
	Строка.Синоним         = НСтр("ru='Задачи';uk='Задачі'");
	Строка.ИмяВЕдЧисле     = "Задача";
	Строка.СинонимВЕдЧисле = НСтр("ru='Задача';uk='Задача'");
	...
	Если Не Объект.ПометкаУдаления Тогда
		ВызватьИсключениеПоОшибке(ОбъектыРасширений,
			НСтр("ru='Удаление идентификаторов объектов, у которых значение
                       ...
                       |;uk='Вилучення ідентифікаторів об''єктів, у яких значення
                       |реквізиту ""Позначка вилучення"" встановлено Хибність неприпустимо.'"));
	КонецЕсли;
...
Процедура ОбновитьДанные(ЕстьИзменения, ЕстьУдаленные, ТолькоПроверка,
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СписокПереименованийОбъектовМетаданных) Тогда
			СписокКритичныхИзменений = НСтр("ru='Переименование идентификаторов объектов метаданных СтароеПолноеИмя -> НовоеПолноеИмя:';uk='Перейменування ідентифікаторів об''єктів метаданих СтароеПолноеИмя -> НовоеПолноеИмя:'")
		...
		Если ЗначениеЗаполнено(СписокНовыхОбъектовМетаданных) Тогда
			СписокКритичныхИзменений = СписокКритичныхИзменений
				+ НСтр("ru='Добавление новых идентификаторов объектов метаданных:';uk='Додавання нових ідентифікаторів об''єктів метаданих:'")
		...
		Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
			ЗаписьЖурналаРегистрации(?(ОбъектыРасширений,
					НСтр("ru='Идентификаторы объектов расширений.Выполнены критичные изменения';uk='Ідентифікатори об''єктів розширень.Виконані критичні зміни'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					НСтр("ru='Идентификаторы объектов метаданных.Выполнены критичные изменения';uk='Ідентифікатори об''єктів метаданих.Виконано критичні зміни'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
		...
		Если СтрЧислоВхождений(ВРег(ПолноеИмя), ВРег("Подсистема.")) > 1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при поиске дочерней подсистемы по полному имени (при поиске ""%1"" была найдена ""%2"").
                           ...
                           |;uk='Помилка при пошуку дочірньої підсистеми по повному імені (при пошуку ""%1"" була знайдена ""%2"").
                           |Не слід називати підсистеми однаково, або використовувати більш нову версію платформи.'"),
				ПолноеИмя,
				ОбъектМетаданных.ПолноеИмя());
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при поиске объекта метаданных по полному имени (при поиске ""%1"" был найден ""%2"").';uk='Помилка при пошуку об''єкта метаданих по повному імені (при пошуку ""%1"" був знайдений ""%2"").'"),
				ПолноеИмя,
				ОбъектМетаданных.ПолноеИмя());
		КонецЕсли;
	КонецЕсли;
...
Функция НазваниеСправочника(ОбъектыРасширений)
	...
	Если ОбъектыРасширений Тогда
		НазваниеСправочника = НСтр("ru='Идентификаторы объектов расширений';uk='Ідентифікатори об''єктів розширень'");
	Иначе
		НазваниеСправочника = НСтр("ru='Идентификаторы объектов метаданных';uk='Ідентифікатори об''єктів метаданих'");
	КонецЕсли;
...
Процедура ВызватьИсключениеПоОшибке(ОбъектыРасширений, ТекстОшибки)
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Ошибка при работе со справочником ""%1"".';uk='Помилка при роботі з довідником ""%1"".'"),
		НазваниеСправочника(ОбъектыРасширений));
		...
		Если Объект.ЭтоНовый() Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений,
				НСтр("ru='Создание нового идентификатора объекта
                           ...
                           |;uk='Створення нового ідентифікатора об''єкта
                           |можливо тільки автоматично при відновленні даних довідника.'"));
		...
		ИначеЕсли ЗапрещеноИзменятьПолноеИмя(Объект) Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При изменении идентификатора объекта указано
                           ...
                           |;uk='При зміні ідентифікатора об''єкта вказано
                           |повне ім''я ""%1"", яке може бути
                           |встановлено тільки автоматично при відновленні даних довідника.'"),
				Объект.ПолноеИмя));
		Иначе
			...
			Если ЗначениеЗаполнено(НазваниеСправочника) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При изменении идентификатора объекта указано
                               ...
                               |;uk='При зміні ідентифікатора об''єкта вказано
                               |повне ім''я ""%1"",
                               |яке вже використовується в довіднику ""%2"".'"),
		   ...
		   И Не ЭтоКоллекция(Объект.ПолучитьСсылкуНового(), ЭтоОбъектРасширений(Объект)) Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений,
				НСтр("ru='Добавление новых элементов может быть выполнено
                           ...
                           |;uk='Додавання нових елементів може бути виконане
			...
			Если ВРег(Объект.ПолноеИмя) <> ВРег(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ПолноеИмя")) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений,
					НСтр("ru='Изменение реквизита ""Полное имя"" может быть выполнено
                               |только в главном узле распределенной информационной базы.'
                               |;uk='Зміна реквізиту ""Повне ім''я"" може бути виконане
                               |тільки в головному вузлі розподіленої інформаційної бази.'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ОписаниеОшибкиИдентификаторыОбъектовРасширенийНедоступныВНеразделенномРежиме()
	...
	Возврат
		НСтр("ru='Справочник ""Идентификаторы объектов расширений""
                   ...
                   |;uk='Довідник ""Ідентифікатори об''єктів розширень""
                   |не може використовуватися в нерозподіленому режимі.'");
	...
	Для Каждого Сеанс Из Сеансы Цикл
		Если Сеанс.НомерСеанса = НомерСеансаИнформационнойБазы() Тогда
			Если Сеанс.ИмяПриложения = "BackgroundJob" Тогда
				ТекстОшибки = НСтр("ru='Невозможно запустить второе фоновое задание в файловой ИБ.';uk='Неможливо запустити друге фонове завдання у файловій ІБ.'");
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
			Если Сеанс.ИмяПриложения = "COMConnection" Тогда
				ТекстОшибки = НСтр("ru='Невозможно запустить фоновое задание во внешнем соединении с файловой ИБ.';uk='Неможливо запустити фонове завдання у зовнішньому з''єднанні з файловою ІБ.'");
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Попытка
		Для Каждого СтрокаТаблицы Из ТаблицаСсылок Цикл
				...
				Если ОбрабатываемаяСсылка <> Неопределено И Параметры.Объект <> Неопределено Тогда
					...
					Исключение
						...
						Если ВыполнятьВТранзакции Тогда
							ВызватьИсключение;
						КонецЕсли;
						СообщитьОбОшибке(ИнформацияОбОшибке(), ОбъектыРасширений);
					КонецПопытки;
			...
			ИначеЕсли Метаданные.РегистрыСведений.Содержит(СтрокаТаблицы.Метаданные) Тогда
				...
				КонецПопытки;
			Иначе
				СообщитьОбОшибке(НСтр("ru='Значения не заменяются в данных типа';uk='Значення не заміняються в даних типу'")
					+ ": " + СтрокаТаблицы.Метаданные, ОбъектыРасширений);
			КонецЕсли;
		КонецЦикла;
...
Процедура СообщитьОбОшибке(Знач Описание, ОбъектыРасширений)
	...
	ЗаписьЖурналаРегистрации(
		?(ОбъектыРасширений,
			НСтр("ru='Идентификаторы объектов расширений.Замена идентификатора';uk='Ідентифікатори об''єктів розширень.Заміна ідентифікатора'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			НСтр("ru='Идентификаторы объектов метаданных.Замена идентификатора';uk='Ідентифікатори об''єктів метаданих.Заміна ідентифікатора'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
