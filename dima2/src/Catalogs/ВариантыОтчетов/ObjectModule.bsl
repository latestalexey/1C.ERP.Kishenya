...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Наименование <> "" И ВариантыОтчетов.НаименованиеЗанято(Отчет, Ссылка, Наименование) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='""%1"" занято, необходимо указать другое наименование.';uk='""%1"" зайняте, необхідно зазначити інше найменування.'"), Наименование),
			,
			"Наименование");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ Пользовательский И ПользователемИзмененаПометкаУдаления Тогда
		Если ПометкаУдаления Тогда
			ТекстОшибки = НСтр("ru='Пометка на удаление предопределенного варианта отчета запрещена.';uk='Позначка на вилучення визначеного варіанта звіту заборонена.'");
		Иначе
			ТекстОшибки = НСтр("ru='Снятие пометки удаления предопределенного варианта отчета запрещена.';uk='Зняття позначки вилучення визначеного варіанта звіту заборонене.'");
		КонецЕсли;
	...
	Если НЕ ПометкаУдаления И ПользователемИзмененаПометкаУдаления Тогда
		...
		Если НаименованиеЗанято ИЛИ КлючВариантаЗанят Тогда
			ТекстОшибки = НСтр("ru='Ошибка снятия пометки удаления варианта отчета:';uk='Помилка зняття позначки вилучення варіанта звіту:'");
			Если НаименованиеЗанято Тогда
				ТекстОшибки = ТекстОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Наименование ""%1"" уже занято другим вариантом этого отчета.';uk='Найменування ""%1"" уже зайнято іншим варіантом цього звіту.'"),
					Наименование);
			Иначе
				ТекстОшибки = ТекстОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ключ варианта ""%1"" уже занят другим вариантом этого отчета.';uk='Ключ варіанта ""%1"" уже зайнятий іншим варіантом цього звіту.'"),
					КлючВарианта);
			КонецЕсли;
            ...
            |;uk='Перед зняттям позначки вилучення варіанта звіту
            |необхідно встановити позначку вилучення конфліктуючого варіанта звіту.'");
			ВариантыОтчетов.ОшибкаПоВарианту(Ссылка, ТекстОшибки);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроиндексироватьНастройки()
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось проиндексировать схему варианта ""%1"" отчета ""%2"":';uk='Не вдалося проіндексувати схему варіанту ""%1"" звіту ""%2"":'"),
			КлючВарианта,
			Строка(Отчет));
...
Процедура ПроверитьЗаполнениеПредопределенного(Отказ)
	...
	ИначеЕсли ТипОтчета <> ВариантыОтчетовКлиентСервер.ТипОтчета(Отчет) Тогда
		ТекстОшибки = НСтр("ru='Противоречивые значения полей ""%1"" и ""%2""';uk='Суперечливі значення полів ""%1"" та ""%2""'");
...
Функция НеЗаполненоПоле(ИмяПоля)
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не заполнено поле ""%1""';uk='Не заповнене поле ""%1""'"), ИмяПоля);
КонецФункции
