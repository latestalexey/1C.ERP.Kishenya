...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НастройкаУчетнойЗаписиЗаголовок.Заголовок = ?(КонтекстныйРежим,
		НСтр("ru='Для отправки писем необходимо настроить учетную запись электронной почты';uk='Для відправки листів необхідно настроїти обліковий запис електронної пошти'"),
		НСтр("ru='Введите параметры учетной записи';uk='Введіть параметри облікового запису'"));
		
	Если Не КонтекстныйРежим Тогда
		Заголовок = НСтр("ru='Создание учетной записи электронной почты';uk='Створення облікового запису електронної пошти'");
	Иначе
		Заголовок = НСтр("ru='Настройка учетной записи электронной почты';uk='Настройка облікового запису електронної пошти'");
	КонецЕсли;
...
Процедура ПоказатьВопросПередЗакрытиемФормы()
	ТекстВопроса = НСтр("ru='Введенные данные не записаны. Закрыть форму?';uk='Введені дані не записані. Закрити форму?'");
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗакрытиеФормыПодтверждено", ЭтотОбъект);
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("Закрыть", НСтр("ru='Закрыть';uk='Закрити'"));
	Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru='Не закрывать';uk='Не закривати'"));
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Введенные данные не записаны. Закрыть форму?';uk='Введені дані не записані. Закрити форму?'"), Кнопки,,
		КодВозвратаДиалога.Отмена, НСтр("ru='Настройка учетной записи';uk='Настройка облікового запису'"));
КонецПроцедуры
	...
	Если ИспользоватьДляОтправки Тогда
		Результат.Добавить(
			РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
				...
				НСтр("ru='Электронная почта.';uk='Електронна пошта.'")));
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеНаСтраницеНастройкаУчетнойЗаписи(Отказ)
	...
	Если ПустаяСтрока(АдресЭлектроннойПочты) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Введите адрес электронной почты';uk='Введіть адресу електронної пошти'"), , "АдресЭлектроннойПочты", , Отказ);
	ИначеЕсли Не ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(АдресЭлектроннойПочты, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Адрес электронной почты введен неверно';uk='Адреса електронної пошти введена невірно'"), , "АдресЭлектроннойПочты", , Отказ);
	КонецЕсли;
	
	Если ПустаяСтрока(ПарольДляПолученияПисем) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо ввести пароль';uk='Необхідно ввести пароль'"), , "ПарольДляПолученияПисем", , Отказ);
	КонецЕсли;
...
Процедура НастроитьЭлементыТекущейСтраницы()
	...
	Если ТекущаяСтраница = Элементы.УчетнаяЗаписьУспешноНастроена Тогда
		Если КонтекстныйРежим Тогда
			ЗаголовокКнопкиДалее = НСтр("ru='Продолжить';uk='Продовжити'");
		Иначе
			ЗаголовокКнопкиДалее = НСтр("ru='Закрыть';uk='Закрити'");
		КонецЕсли;
	Иначе
		Если ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи
			И ПроверкаЗавершиласьСОшибками Тогда
				ЗаголовокКнопкиДалее = НСтр("ru='Повторить';uk='Повторити'");
		ИначеЕсли ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи
			И СпособНастройки = "Автоматически" Тогда
			Если КонтекстныйРежим Тогда
				ЗаголовокКнопкиДалее = НСтр("ru='Настроить';uk='Настроїти'");
			Иначе
				ЗаголовокКнопкиДалее = НСтр("ru='Создать';uk='Створити'");
			КонецЕсли;
		Иначе
			ЗаголовокКнопкиДалее = НСтр("ru='Далее >';uk='Далі >'");
		КонецЕсли;
	КонецЕсли;
	...
	Если Не КонтекстныйРежим И ТекущаяСтраница = Элементы.УчетнаяЗаписьУспешноНастроена Тогда
		Элементы.КнопкаПерейтиКНастройкам.Заголовок = НСтр("ru='Перейти к учетной записи';uk='Перейти до облікового запису'");
	Иначе
		Элементы.КнопкаПерейтиКНастройкам.Заголовок = НСтр("ru='Настроить вручную';uk='Налаштувати вручну'");
	КонецЕсли;
	...
	Если ТекущаяСтраница = Элементы.УчетнаяЗаписьУспешноНастроена Тогда
		Элементы.НадписьУчетнаяЗаписьУспешноНастроена.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Настройка параметров учетной записи
                |%1 завершена.'
                |;uk='Настройка параметрів облікового запису
...
Функция ПроверитьПодключениеКСерверуИсходящейПочты()
	...
	Тема = НСтр("ru='Тестовое сообщение BAS ERP';uk='Тестове повідомлення BAS ERP'");
	Тело = НСтр("ru='Это сообщение отправлено подсистемой электронной почты BAS ERP';uk='Це повідомлення надіслано підсистемою електронної пошти BAS ERP'");
...
Процедура ПроверитьНастройкиУчетнойЗаписи()
	...
	Если Не ПустаяСтрока(СообщениеСервераИсходящейПочты) Тогда
		ТекстОшибки = НСтр("ru='Не удалось подключиться к серверу исходящей почты:';uk='Не вдалося підключитися до сервера вихідної пошти:'") + Символы.ПС
			+ СообщениеСервераИсходящейПочты + Символы.ПС;
	КонецЕсли;
	
	Если Не ПустаяСтрока(СообщениеСервераВходящейПочты) Тогда
		ТекстОшибки = ТекстОшибки
			+ НСтр("ru='Не удалось подключиться к серверу входящей почты:';uk='Не вдалося підключитися до сервера вхідної пошти:'") + Символы.ПС
			+ СообщениеСервераВходящейПочты;
	КонецЕсли;
...
Процедура СоздатьУчетнуюЗапись()
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка при создании учетной записи электронной почты';uk='Помилка при створенні облікового запису електронної пошти'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура НастроитьПараметрыПодключенияАвтоматически()
    ...
    |;uk='Не вдалося визначити настройки з''єднання. 
    |Настройте параметри підключення вручну.'");
	...
	Если СтрНайти(НРег(АдресЭлектроннойПочты), "@gmail.com") > 0 Тогда
		СообщенияОбОшибках = СообщенияОбОшибках + Символы.ПС
			+ НСтр("ru='См. также рекомендации по настройке почты Gmail:
                    ...
                    |;uk='См. також рекомендації по налаштуванню пошти Gmail:
                    |http://buh.ru/articles/documents/42429/#briefly_43166'");
	КонецЕсли;
...
Процедура ПодобратьНастройкиУчетнойЗаписи()
	...
	НаименованиеЗадания = НСтр("ru='Поиск настроек почтового сервера';uk='Пошук настройок поштового сервера'");
