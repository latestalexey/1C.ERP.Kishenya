...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если ПараметрКоманды.Пустая() Тогда
		ТекстСообщения = НСтр("ru='Не указана версия приложения для выгрузки';uk='Не вказана версія програми для вивантаження'");
...
Процедура ВыгрузитьПриложение(ВерсияМобильногоПриложения)
	...
	#Если Не ВебКлиент Тогда
		...
		Если СтруктураПриложения.ЭтоГруппа Тогда
			ТекстСообщения = НСтр("ru='Не выбрана версия приложения для выгрузки';uk='Не обрана версія програми для вивантаження'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
		...
		ДиалогВыбораФайла.ПолноеИмяФайла = НСтр("ru='Мобильное приложение ';uk='Мобільний додаток '") + СтруктураПриложения.ИмяПриложения + НСтр("ru='_Версия ';uk='_Версия '") + СтруктураПриложения.НомерВерсии;
		...
		Если ДиалогВыбораФайла.Выбрать() Тогда
			...
			ТекстСообщения = НСтр("ru='Приложение выгружено';uk='Додаток вивантажено'");
