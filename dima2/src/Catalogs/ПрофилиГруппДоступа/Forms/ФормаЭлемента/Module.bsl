	...
	Если Не Элементы.Роли.ТолькоПросмотр Тогда
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль ""%1"" недоступна для назначения профиля.';uk='Роль ""%1"" недоступна для призначення профілю.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль ""%1"" в строке %%1 недоступна для назначения профиля.';uk='Роль ""%1"" в рядку %%1 недоступна для призначення профілю.'"), Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ВосстановитьПоНачальномуЗаполнению(Команда)
	...
	ПоказатьВопрос(
		...
		НСтр("ru='Восстановить профиль по содержимому начального заполнения?';uk='Відновити профіль за вмістом початкового заповнення?'"),
...
Процедура ВыбратьНазначение(Команда)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект, НСтр("ru='Выбор назначения профиля групп доступа';uk='Вибір призначення профілю груп доступу'"),,, ОписаниеОповещения);
КонецПроцедуры
	...
	Если ОбновитьГруппыДоступа Тогда
		Текст =
			НСтр("ru='Профиль ""%1"" восстановлен по содержимому начального заполнения,
                       ...
                       |;uk='Профіль ""%1"" відновлений по вмісту початкового заповнення,
                       |групи доступу профілю оновлені.'");
	Иначе
		Текст =
			НСтр("ru='Профиль ""%1"" восстановлен по содержимому начального заполнения,
                       |группы доступа профиля не обновлены.'
                       |;uk='Профіль ""%1"" відновлений по вмісту початкового заповнення,
                       |групи доступу профілю не оновлені.'");
	КонецЕсли;
...
Функция ТекстВопросаОбновитьГруппыДоступаПрофиля()
	...
	Возврат
		НСтр("ru='Обновить группы доступа, использующие этот профиль?
                   ...
                   |;uk='Оновити групи доступу, що використовують цей профіль?
                   |
                   |Будуть вилучені зайві види доступу з визначеними для них
                   |значеннями доступу та додані відсутні види доступу.'");
