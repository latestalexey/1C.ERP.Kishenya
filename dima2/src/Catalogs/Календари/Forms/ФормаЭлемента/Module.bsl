...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ШаблонЗаполнения.НайтиСтроки(Новый Структура("ДеньВключенВГрафик", Истина)).Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не отмечены дни, включаемые в график работы';uk='Не відзначені дні, що включаються в графік роботи'"), , "Объект.ШаблонЗаполнения", , Отказ);
	КонецЕсли;
...
Процедура НомерТекущегоГодаПриИзменении(Элемент)
	...
	Если МодифицированностьРезультата Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Записать измененные данные за %1 год?';uk='Записати змінені дані за %1 рік?'"), Формат(НомерПредыдущегоГода, "ЧГ=0"));
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Заполнить расписание';uk='Заповнити розклад'"));
...
Процедура СформироватьШаблонЗаполнения(СпособЗаполнения, ШаблонЗаполнения, Знач ДлинаЦикла, Знач ДатаОтсчета = Неопределено)
	...
	Если СпособЗаполнения = ПредопределенноеЗначение("Перечисление.СпособыЗаполненияГрафикаРаботы.ПоНеделям") Тогда
		ШаблонЗаполнения[0].ПредставлениеДня = НСтр("ru='Понедельник';uk='Понеділок'");
		ШаблонЗаполнения[1].ПредставлениеДня = НСтр("ru='Вторник';uk='Вівторок'");
		ШаблонЗаполнения[2].ПредставлениеДня = НСтр("ru='Среда';uk='Середа'");
		ШаблонЗаполнения[3].ПредставлениеДня = НСтр("ru='Четверг';uk='Четвер'");
		ШаблонЗаполнения[4].ПредставлениеДня = НСтр("ru='Пятница';uk='П''ятниця'");
		ШаблонЗаполнения[5].ПредставлениеДня = НСтр("ru='Суббота';uk='Субота'");
		ШаблонЗаполнения[6].ПредставлениеДня = НСтр("ru='Воскресенье';uk='Неділя'");
	Иначе
		ДатаДня = ДатаОтсчета;
...
Функция ПредставлениеРасписанияДня(Форма, НомерДня)
	...
	Если Секунд = 0 Тогда
		Возврат НСтр("ru='Заполнить расписание';uk='Заповнити розклад'");
	КонецЕсли;
...
Процедура УточнитьЗаполненностьГрафика(Форма)
	...
	Если Не ЗначениеЗаполнено(Форма.ДатаЗаполненности) Тогда
		Форма.ЗаполненностьИнформационныйТекст = НСтр("ru='График работы не заполнен';uk='Графік роботи не заповнений'");
		Форма.ТребуетЗаполнения = Истина;
	Иначе	
		Если Не ЗначениеЗаполнено(Форма.Объект.ГоризонтПланирования) Тогда
			ИнформационныйТекст = НСтр("ru='График работы заполнен до %1';uk='Графік роботи заповнений до %1'");
		...
		Иначе											
			...
			ИнформационныйТекст = НСтр("ru='График работы заполнен до %1, с учетом горизонта планирования график должен быть заполнен до %2';uk='Графік роботи заповнений до %1, з урахуванням горизонту планування графік повинен бути заповнений до %2'");
...
Процедура ЗаполнитьИнформационныйТекстРезультатаЗаполнения(Форма)
	...
	Если Форма.РучноеРедактирование Тогда
		ИнформационныйТекст = НСтр("ru='График работы на текущий год изменен вручную. Нажмите «Заполнить по шаблону» чтобы вернуться к автоматическому заполнению.';uk='Графік роботи на поточний рік змінений вручну. Натисніть «Заповнити за шаблоном» щоб повернутися до автоматичного заповнення.'");
	...
	Иначе
		Если Форма.РезультатЗаполненПоШаблону Тогда
			Если ЗначениеЗаполнено(Форма.Объект.ПроизводственныйКалендарь) Тогда
				ИнформационныйТекст = НСтр("ru='График работы автоматически обновляется при изменении производственного календаря за текущий год.';uk='Графік роботи автоматично оновлюється при зміні виробничого календаря за поточний рік.'");
				ИнформационнаяКартинка = БиблиотекаКартинок.Информация;
			КонецЕсли;
		Иначе
			ИнформационныйТекст = НСтр("ru='Отображаемый результат не соответствует настройке шаблона. Нажмите «Заполнить по шаблону», чтобы увидеть как выглядит график работы с учетом изменений шаблона.';uk='Результат, що відображається, не відповідає настройці шаблону. Натисніть «Заповнити за шаблоном», щоб побачити як виглядає графік роботи з урахуванням змін шаблону.'");
			ИнформационнаяКартинка = БиблиотекаКартинок.Предупреждение;
			ДоступноЗаполнениеПоШаблону = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьИнформационныйТекстРучногоРедактирования(Форма)
	...
	Если Форма.РучноеРедактирование Тогда
		ИнформационнаяКартинка = БиблиотекаКартинок.Предупреждение;
		ИнформационныйТекст = НСтр("ru='График работы на текущий год изменен вручную. Изменения выделены в результатах заполнения.';uk='Графік роботи на поточний рік змінений вручну. Зміни виділені в результатах заповнення.'");
	КонецЕсли;
