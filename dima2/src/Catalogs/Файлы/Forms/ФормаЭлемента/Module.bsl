	...
	Если Не Параметры.ФайлОснование.Пустая() И ФайлОснованиеПодписан Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл ""%1"" подписан.
                       ...
                       |;uk='Файл ""%1"" підписаний.
                       |Копіювання відомостей про ЕП в новий файл зробить його недоступним для зміни.
                       |
                       |Скопіювати відомості про ЕП в новий файл?'"),
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ФайлРедактировался И Не ПолученОтветНаВопросФайлЗанят Тогда
		...
		Если ПараметрыРаботыКлиента.АвторизованныйПользователь = Объект.Редактирует Тогда
            ...
            |
            |Закрыть карточку?'
            |;uk='Файл зайнятий для редагування.
            |
            |Закрити картку?'");
	...
	Если РежимСоздания = "ИзШаблона" И НЕ Объект.ПодписанЭП Тогда
		Если НовыйФайл И НовыйФайлЗаписан И (Не ФайлРедактировался) Тогда
			Если Не ПолученОтветНаВопросОткрытьДляРедактирования Тогда
				Отказ = Истина;
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Открыть файл ""%1"" для редактирования?';uk='Відкрити файл ""%1"" для редагування?'"),
	...
	Если Объект.Ссылка.Пустая() Тогда 
		ТекстВопроса =
			НСтр("ru='Данные еще не записаны. Выполнение команды
                       ...
                       |;uk='Дані ще не записані. Виконання команди
                       |""Зашифрувати"" можливо тільки після запису даних.
                       |
...
Процедура Удалить(Команда)
	...
	ПоказатьВопрос(Обработчик, НСтр("ru='Удалить выделенные подписи?';uk='Вилучити виділені підписи?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ЗаполнитьСписокПодписей()
	...
	Иначе
		ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Электронные подписи (%1)';uk='Електронні підписи (%1)'"),
