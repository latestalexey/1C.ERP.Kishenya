...
Процедура СформироватьИнформациюОНачислениях()
	...
	Запрос.Текст =
		...
		|	ПлановыйФОТ.Основание,
		|	МАКСИМУМ(ПлановыеНачисления.Период) КАК ПериодПлановыхНачислений
		|ПОМЕСТИТЬ ВТПлановыйФОТСПериодамиПлановыхНачислений
		|ИЗ
		|	ВТПлановыйФОТ КАК ПлановыйФОТ
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПлановыеНачисления КАК ПлановыеНачисления
		|		ПО ПлановыйФОТ.Сотрудник = ПлановыеНачисления.Сотрудник
		|			И ПлановыйФОТ.Начисление = ПлановыеНачисления.Начисление
		|			И ПлановыйФОТ.ПериодЗаписи >= ПлановыеНачисления.ПериодЗаписи
		|
		|СГРУППИРОВАТЬ ПО
		|	ПлановыйФОТ.ПериодЗаписи,
		|	ПлановыйФОТ.Сотрудник,
		...
		|	ПлановыйФОТ.ПериодЗаписи,
		|	ПлановыйФОТ.Сотрудник,
		|	ПлановыйФОТ.Основание КАК Регистратор
		|ПОМЕСТИТЬ ВТВсеПериодыПредварительно
		|ИЗ
		|	ВТПлановыйФОТСПериодамиПлановыхНачислений КАК ПлановыйФОТ
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПлановыеНачисления КАК ПлановыеНачисления
		|		ПО ПлановыйФОТ.Сотрудник = ПлановыеНачисления.Сотрудник
		|			И ПлановыйФОТ.Начисление = ПлановыеНачисления.Начисление
		|			И ПлановыйФОТ.ПериодПлановыхНачислений = ПлановыеНачисления.Период
		|ГДЕ
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Пока ВыборкаПоПериодам.Следующий() Цикл
				...
				СтрокаДокумента.ФОТ = ВыборкаПоПериодам.ВкладВФОТ;
				...
				Пока ВыборкаПоНачислениям.Следующий() Цикл
					...
					Иначе
						...
						СтрокаНачисления.Начисление = ВыборкаПоНачислениям.Начисление;
						СтрокаНачисления.ФОТ = ВыборкаПоНачислениям.ВкладВФОТ;
						СтрокаНачисления.Представление = НСтр("ru='Доп. тариф/коэфф.';uk='Дод. тариф/коеф.'");
