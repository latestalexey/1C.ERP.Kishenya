...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Схема = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Схема компоновки данных не корректна.';uk='Схема компоновки даних не коректна.'"),
			,
		...
		Для Каждого ПараметрСхемы Из Схема.Параметры Цикл
			...
			Если ПараметрСхемы.ЗапрещатьНезаполненныеЗначения Тогда
				...
				Если ВывестиСообщение Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						СтрШаблон(НСтр("ru='Не заполнено значение параметра ""%1""';uk='Не заповнено значення параметра ""%1""'"), ПараметрСхемы.Имя),
						,
						"Объект.СхемаКомпоновкиДанных",,
						Отказ);
				КонецЕсли;
...
Процедура РедактироватьСхемуКомпоновкиДанных(Команда)
	...
	ЗаголовокФормыНастройкиСхемыКомпоновкиДанных = НСтр("ru='Настройка схемы компоновки данных для автоматического начисления ""%1""';uk='Настройка схеми компонування даних для автоматичного нарахування ""%1""'");
