...
Процедура РедактироватьПодписку(ТекущиеДанные)
	...
	Если ТекущиеДанные.Принудительная Тогда
		ТекстПредупреждения = НСтр("ru='Для принудительных групп рассылок и оповещений подписки не настраиваются.';uk='Для примусових груп розсилок і оповіщень підписки не налаштовуються.'");
...
Процедура Подписаться(Команда)
	...
	Если МассивКПодписке.Количество() > 0 Тогда
		...
		ТекстОповещения = НСтр("ru='Подписка';uk='Підписка'");
		ТекстПояснения  = НСтр("ru='Подписка выполнена для %1 из %2 групп рассылок и оповещений.';uk='Підписка виконана для %1 з %2 груп розсилок і оповіщень.'");
	...
	Иначе
		...
		ТекстОповещения = НСтр("ru='Подписки не выполнены';uk='Підписки не виконані'");
		ТекстПояснения  = НСтр("ru='Нет групп рассылок и оповещений для которых необходимо выполнить подписку';uk='Немає груп розсилок та повідомлень для яких необхідно виконати підписку'");
...
Процедура ОтменитьПодписку(Команда)
	...
	Если МассивКОтменеПодписки.Количество() > 0 Тогда
		...
		ТекстОповещения = НСтр("ru='Отмена подпискок';uk='Скасування підписок'");
		ТекстПояснения  = НСтр("ru='Подписка отменена для %1 из %2 групп рассылок и оповещений.';uk='Підписка скасована для %1 з %2 груп розсилок і оповіщень.'");
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПояснения,
	...
	Иначе
		...
		ТекстОповещения = НСтр("ru='Подписки не отменены';uk='Підписки не скасовані'");
		ТекстПояснения  = НСтр("ru='Нет групп рассылок и оповещений для которых необходимо отменять подписку';uk='Немає груп розсилок та повідомлень для яких необхідно скасувати підписку'");
...
Процедура ЗаполнитьТаблицуПодписок()
	...
	Запрос.УстановитьПараметр("Действует",      НСтр("ru='Действует';uk='Діє'"));
	Запрос.УстановитьПараметр("Принудительная", НСтр("ru='Принудительно';uk='Примусово'"));
	Запрос.УстановитьПараметр("Приостановлена", НСтр("ru='Приостановлена';uk='Призупинена'"));
	Запрос.УстановитьПараметр("ДляПисемИSMS",   НСтр("ru='Email и SMS';uk='Email і SMS'"));
	Запрос.УстановитьПараметр("ДляПисем",       НСтр("ru='Email';uk='Email'"));
	Запрос.УстановитьПараметр("ДляSMS",         НСтр("ru='SMS';uk='SMS'"));
...
Процедура УправлениеВидимостью()
	...
	Если АвторизованВнешнийПользователь Тогда
		...
		Элементы.ТаблицаПодписокГруппаРассылокИОповещений.Заголовок = НСтр("ru='Подписка';uk='Підписка'");
	Иначе
