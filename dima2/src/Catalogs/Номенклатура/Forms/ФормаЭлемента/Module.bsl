...
Функция ПараметрыПереходаПоГиперссылке(ИмяЭлемента)
	...
	ИначеЕсли ИмяЭлемента = "ГиперссылкаПерейтиНастройкаПередачиМатериаловВПроизводство" Тогда
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Номенклатура", Объект.Ссылка);
		
		ПараметрыПереходаПоГиперссылке = Новый Структура;
		ПараметрыПереходаПоГиперссылке.Вставить("ИмяФормы", "Обработка.НастройкаПередачиМатериаловВПроизводство.Форма.НастройкаПоНоменклатуре");
		ПараметрыПереходаПоГиперссылке.Вставить("ПараметрыФормы",ПараметрыФормы);
...
Процедура ЗаполнитьСоставОбязательныхРеквизитов()
	...
	ТаблицаРеквизитов = Справочники.Номенклатура.ТаблицаОбязательныхРеквизитов(Объект.ВидНоменклатуры);	
	...
	Для Каждого СтрокаТаблицыДопРеквизитов Из ТаблицаОбязательныхРеквизитов Цикл
		...
		Если Не СтрокаТаблицыДопРеквизитов.ОбязателенДляЗаполнения Тогда
			Продолжить;
		КонецЕсли;
		...
		СтрокаТаблицыДопРеквизитов.ПутьКДанным = ИмяЭлементаФормы;
...
Процедура ЗаполнитьСоставДополнительныхУникальныхРеквизитов()
	...
	ТаблицРеквизитов = Справочники.Номенклатура.ТаблицаУникальныхРеквизитов(Объект.ВидНоменклатуры);
	
	ТаблицаУникальныхРеквизитов.Загрузить(ТаблицРеквизитов);
...
Процедура НастроитьФорму()
	...
	ИначеЕсли РеквизитыВидаНоменклатуры.НастройкаИспользованияСерий= Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваровПоНомеру Тогда
		НастройкаИспользованияСерий = НСтр("ru='Партии товаров с одинаковым номером';uk='Партії товарів з однаковим номером'");
	ИначеЕсли РеквизитыВидаНоменклатуры.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваровПоНомеруИСрокуГодности Тогда
		НастройкаИспользованияСерий = НСтр("ru='Партии товаров с одинаковым номером и сроком годности';uk='Партії товарів з однаковим номером і  строком придатності'");
	ИначеЕсли РеквизитыВидаНоменклатуры.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваровПоСрокуГодности Тогда
		НастройкаИспользованияСерий = НСтр("ru='Партии товаров с одинаковым сроком годности';uk='Партії товарів з однаковим  строком придатності'");
	ИначеЕсли РеквизитыВидаНоменклатуры.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ЭкземплярТовара Тогда
		НастройкаИспользованияСерий = НСтр("ru='Экземпляры товаров с уникальным серийным номером';uk='Екземпляри товарів з унікальним серійним номером'");
	КонецЕсли;
	...
	НастроитьВидимостьЭлементовМенюПерейти();
	НастроитьВидимостьСворачиваемыхГрупп();
	
КонецПроцедуры

&НаСервере
Процедура НастроитьВидимостьЭлементовМенюПерейти()
...
Процедура СкрытьРаскрытьВсеГруппы(Свернуть = Неопределено)
	ИзменитьСвернутостьГруппы("ОсновныеПараметрыУчета",Свернуть);
	ИзменитьСвернутостьГруппы("ЕдиницыИзмерения",Свернуть);
	ИзменитьСвернутостьГруппы("РегламентированныйУчет",Свернуть);
	ИзменитьСвернутостьГруппы("СведенияОПроизовдителе",Свернуть);
	
	ИзменитьСвернутостьГруппы("КартинкаОписание",Свернуть);
	ИзменитьСвернутостьГруппы("ДополнительныеРеквизиты",Свернуть);
	ИзменитьСвернутостьГруппы("СведенияОПроизовдителе",Свернуть);
	ИзменитьСвернутостьГруппы("ПланированиеИМаркетинг",Свернуть);
	ИзменитьСвернутостьГруппы("Обеспечение",Свернуть);
	ИзменитьСвернутостьГруппы("Цены",Свернуть);
	ИзменитьСвернутостьГруппы("Шаблоны",Свернуть);
КонецПроцедуры
...
Процедура ПроверитьЗаполнениеОбязательныхРеквизитов(Отказ)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОбязательныхРеквизитов Цикл
	...
	Если Не ЗначениеЗаполнено(Объект.ШаблонЭтикетки) И (ИспользоватьИндивидуальныйШаблонЭтикетки = 1) Тогда
		ТекстСообщения = СтрЗаменить(ТекстОшибки, "%ИмяРеквизита%", НСтр("ru='Шаблон этикетки';uk='Шаблон етикетки'"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"Объект.ШаблонЭтикетки",,Отказ);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.ШаблонЦенника) И (ИспользоватьИндивидуальныйШаблонЦенника = 1) Тогда
		ТекстСообщения = СтрЗаменить(ТекстОшибки, "%ИмяРеквизита%", НСтр("ru='Шаблон ценника';uk='Шаблон цінника'"));
...
Процедура ПроверитьВозможностьСозданияИзмененияЭлементаПоГруппеДоступа(Отказ)
	...
	Если ЗначениеЗаполнено(Объект.ГруппаДоступа) Тогда
		...
		Если Не Справочники.ГруппыДоступаНоменклатуры.ЕстьПравоИзменения(Новый Структура("ГруппаДоступа, Ссылка", РеквизитыВидаНоменклатуры.ГруппаДоступа, Объект.Ссылка)) Тогда
...
Процедура НоменклатураГТДНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ТекстВопроса = НСтр("ru='Для выбора кода для НН и номера ГТД по умолчанию необходимо записать объект. Записать?';uk='Для вибору коду для ПН та номери ВМД за замовчуванням необхідно записати об''єкт. Записати?'");
