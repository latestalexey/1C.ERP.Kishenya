...
//	Возвращаемое значение:
//		Структура - поля ВестиУчетПоГТД, НоменклатураГТД, ЕдиницаИзмерения, СтавкаНДС, ГруппаФинансовогоУчета, ЭтоУслуга, ПодакцизныйТовар, СтатьяДекларацииПоАкцизномуНалогу
//
Функция ЗначенияРеквизитовНоменклатуры(Номенклатура) Экспорт
	...
	СтруктураРеквизитов = Новый Структура("ВестиУчетПоГТД, СтавкаНДС, ЭтоУслуга, ЕдиницаИзмерения, ГруппаФинансовогоУчета, НоменклатураГТД, ПодакцизныйТовар, СтатьяДекларацииПоАкцизномуНалогу",
		ВестиУчетПоГТД,
		СтавкаНДС,
		ЭтоУслуга,
		ЕдиницаИзмерения,
...
Функция ПравоДоступаНаОбъектМетаданныхПоРеквизиту(Право, РеквизитОбъектМетаданных)
	...
	ПравоДоступаНаОбъектМетаданных = Истина;
	Для Каждого Тип Из МассивтиповДляПроверки Цикл
		ОбъектМетаданных = Метаданные.НайтиПоТипу(Тип);
		Если ОбъектМетаданных <> Неопределено Тогда
			ПравоДоступаНаОбъектМетаданных = ПравоДоступаНаОбъектМетаданных И ПравоДоступа(Право, ОбъектМетаданных);
		КонецЕсли;
	КонецЦикла;
	
	Возврат ПравоДоступаНаОбъектМетаданных;
КонецФункции

Функция ВидимостьРеквизита(ИмяРеквизита,
							РеквизитыОтключенныеПоФО,
							РежимВидимостиПоказатьТолькоВажные,
							РеквизитыВидимыеВРежимеПоказатьОсновные,
							ТаблицаОбязательныхРеквизитов,
							ТаблицаУникальныхРеквизитов)
	...
	Возврат ПравоДоступа("Просмотр", Метаданные.Справочники.Номенклатура.Реквизиты[ИмяРеквизита])
			...
			Или РеквизитыВидимыеВРежимеПоказатьОсновные.Свойство(ИмяРеквизита)
			Или ТаблицаОбязательныхРеквизитов.Найти(ИмяРеквизита,"Наименование") <> Неопределено
			Или ТаблицаУникальныхРеквизитов.Найти(ИмяРеквизита,"Наименование") <> Неопределено)
...
Функция СтруктураВидимостиЭлементовФормы(Объект, РежимВидимостиПоказатьТолькоВажные = Ложь) Экспорт
	...
	СтруктураВидимостиПоТипу = Новый Структура;
	СтруктураВидимостиПоРеквизитам = Новый Структура;
	...
	РеквизитыВидаНоменклатуры = ?(Не ЗначениеЗаполнено(Объект.ВидНоменклатуры),
		Новый Структура("ИспользоватьСерии, НастройкаИспользованияСерий, ИспользоватьСрокГодностиСерии, ВариантОказанияУслуг, ШаблонЭтикетки, ШаблонЦенника", Ложь, Ложь, Ложь, Неопределено, Неопределено, Неопределено),
		ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ВидНоменклатуры, "ИспользоватьСерии, НастройкаИспользованияСерий, ИспользоватьСрокГодностиСерии, ВариантОказанияУслуг, ШаблонЭтикетки, ШаблонЦенника"));
	
	РеквизитыОтключенныеПоФО  = РеквизитыОтключенныеПоФО();
	РеквизитыВидимыеВРежимеПоказатьОсновные  = РеквизитыВидимыеВРежимеПоказатьОсновные();
	ТаблицаОбязательныхРеквизитов = ТаблицаОбязательныхРеквизитов(Объект.ВидНоменклатуры);
	ТаблицаУникальныхРеквизитов = ТаблицаУникальныхРеквизитов(Объект.ВидНоменклатуры);
	...
	Для Каждого СтрТабл из НастройкиРеквизитов Цикл
		...
		СтруктураВидимостиПоТипу.Вставить(ИмяРеквизита, СтрТабл.ДействиеСРеквизитом = "Добавить"
	...
	СтруктураВидимостиПоРеквизитам.Вставить("ФайлКартинки");
	СтруктураВидимостиПоРеквизитам.Вставить("Описание");
	СтруктураВидимостиПоРеквизитам.Вставить("ФайлОписанияДляСайта");
	...
	Для Каждого СтрокаТаблицыДопРеквизитов Из ТаблицаОбязательныхРеквизитов Цикл
		...
		Если СтрокаТаблицыДопРеквизитов.ЭтоДопРеквизит Тогда
			...
			НаборСвойств = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицыДопРеквизитов.СвойствоДопРеквизита, "НаборСвойств");
			
			ИмяЭлементаФормы = "ДополнительныйРеквизитЗначение_" 
							+ СтрЗаменить(ВРег(Строка(НаборСвойств.УникальныйИдентификатор())), "-", "x")
							+ "_"
							+ СтрЗаменить(ВРег(Строка(СтрокаТаблицыДопРеквизитов.СвойствоДопРеквизита.УникальныйИдентификатор())), "-", "x");
							
			СтруктураВидимости.Вставить(ИмяЭлементаФормы, Истина);
			
		КонецЕсли;
		
	КонецЦикла;

	//Установка видимости для уникальных реквизитов
	
	Для Каждого СтрокаТаблицыДопРеквизитов Из ТаблицаУникальныхРеквизитов Цикл
	...
	СтруктураВидимостиПоРеквизитам.Вставить("Производитель");
	СтруктураВидимостиПоРеквизитам.Вставить("Марка");
	
	#КонецОбласти
	
	#Область ПланированиеИМаркетинг
	
	СтруктураВидимостиПоРеквизитам.Вставить("ТоварнаяКатегория");
	СтруктураВидимостиПоРеквизитам.Вставить("КоллекцияНоменклатуры");
	СтруктураВидимостиПоРеквизитам.Вставить("СезоннаяГруппа");
	СтруктураВидимостиПоРеквизитам.Вставить("РейтингПродаж");
	...
	ГиперссылкаПерейтиНастройкаПередачиМатериаловВПроизводствоВидимость =
		...
		И (Объект.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Товар
			Или Объект.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.МногооборотнаяТара)
		И ПравоДоступа("Чтение", Метаданные.РегистрыСведений.НастройкаПередачиМатериаловВПроизводство)
		И Не РежимВидимостиПоказатьТолькоВажные;
	//-- НЕ УТКА
	
	ГиперссылкаПерейтиПроизводствоВидимость = Ложь;
	//++ НЕ УТКА
	ГиперссылкаПерейтиПроизводствоВидимость = ИспользоватьПроизводство
		И ПравоДоступа("Просмотр", Метаданные.ОбщиеФормы.ПрименениеНоменклатурыВПроизводстве)
		И Не РежимВидимостиПоказатьТолькоВажные;
	//-- НЕ УТКА
	...
	СтруктураВидимостиПоРеквизитам.Вставить("ЦеноваяГруппа");
	...
	ВидимостьИспользоватьИндивидуальныйШаблонЭтикетки = ВидимостьРеквизита("ИспользоватьИндивидуальныйШаблонЭтикетки",
								РеквизитыОтключенныеПоФО,
								РежимВидимостиПоказатьТолькоВажные,
								РеквизитыВидимыеВРежимеПоказатьОсновные,
								ТаблицаОбязательныхРеквизитов,
	...
	ВидимостьШаблонЭтикетки = ВидимостьРеквизита("ШаблонЭтикетки",
								...
								ТаблицаОбязательныхРеквизитов,
								ТаблицаУникальныхРеквизитов);
	ВидимостьИспользоватьИндивидуальныйШаблонЦенника = ВидимостьРеквизита("ИспользоватьИндивидуальныйШаблонЦенника",
								РеквизитыОтключенныеПоФО,
								РежимВидимостиПоказатьТолькоВажные,
								РеквизитыВидимыеВРежимеПоказатьОсновные,
								ТаблицаОбязательныхРеквизитов,
	...
	СтруктураВидимости.Вставить("ИспользоватьИндивидуальныйШаблонЭтикетки",	ВидимостьШаблонЭтикетки И ВидимостьИспользоватьИндивидуальныйШаблонЭтикетки);
	СтруктураВидимости.Вставить("ШаблонЭтикетки", 							ВидимостьШаблонЭтикетки И ВидимостьИспользоватьИндивидуальныйШаблонЭтикетки);
	СтруктураВидимости.Вставить("ВидНоменклатурыШаблонЭтикетки", 			ЗначениеЗаполнено(РеквизитыВидаНоменклатуры.ШаблонЭтикетки) И ВидимостьШаблонЭтикетки
																			И ВидимостьИспользоватьИндивидуальныйШаблонЭтикетки И Не Объект.ИспользоватьИндивидуальныйШаблонЭтикетки);
	СтруктураВидимости.Вставить("ВидНоменклатурыШаблонЭтикеткиНеУказан",	Не ЗначениеЗаполнено(РеквизитыВидаНоменклатуры.ШаблонЭтикетки) И ВидимостьШаблонЭтикетки
																			И ВидимостьИспользоватьИндивидуальныйШаблонЭтикетки И Не Объект.ИспользоватьИндивидуальныйШаблонЭтикетки);
	СтруктураВидимости.Вставить("ШаблонЭтикеткиОтступ", 					ВидимостьШаблонЭтикетки И ВидимостьИспользоватьИндивидуальныйШаблонЭтикетки
																			И Объект.ИспользоватьИндивидуальныйШаблонЭтикетки);
		
	СтруктураВидимости.Вставить("ШаблоныЭтикотокЦенниковОтступ", 			ВидимостьШаблонЦенника И ВидимостьИспользоватьИндивидуальныйШаблонЦенника);
	СтруктураВидимости.Вставить("ИспользоватьИндивидуальныйШаблонЦенника", 	ВидимостьШаблонЦенника И ВидимостьИспользоватьИндивидуальныйШаблонЦенника);
	СтруктураВидимости.Вставить("ШаблонЦенника", 							ВидимостьШаблонЦенника И ВидимостьИспользоватьИндивидуальныйШаблонЦенника);
	СтруктураВидимости.Вставить("ВидНоменклатурыШаблонЦенника", 			ЗначениеЗаполнено(РеквизитыВидаНоменклатуры.ШаблонЦенника) И ВидимостьШаблонЭтикетки
																			И ВидимостьИспользоватьИндивидуальныйШаблонЭтикетки И Не Объект.ИспользоватьИндивидуальныйШаблонЦенника);
	СтруктураВидимости.Вставить("ВидНоменклатурыШаблонЦенникаНеУказан", 	ВидимостьШаблонЭтикетки И ВидимостьИспользоватьИндивидуальныйШаблонЭтикетки
																			И Не Объект.ИспользоватьИндивидуальныйШаблонЦенника);
	СтруктураВидимости.Вставить("ШаблонЦенникаОтступ", 						ВидимостьШаблонЦенника И ВидимостьИспользоватьИндивидуальныйШаблонЦенника
	...
	ГиперссылкаПерейтиТоварыДругогоКачестваВидимость = СтруктураВидимостиПоТипу.Качество
	...
	СтруктураВидимостиПоРеквизитам.Вставить("ГруппаДоступа");
	...
	СтруктураВидимостиПоРеквизитам.Вставить("ГруппаАналитическогоУчета");
	...
	СтруктураВидимости.Вставить("НоменклатураГТДДекорация", СтруктураВидимостиПоТипу.НоменклатураГТД);
	...
	Для Каждого ЭлементСтруктуры Из СтруктураВидимостиПоТипу Цикл
		Если СтруктураВидимости.Свойство(ЭлементСтруктуры.Ключ) Тогда
			ТекстОшибки = НСтр("ru='Для реквизита ""%ИмяРеквизита%"" уже настроена видимость.';uk='Для реквізиту ""%ИмяРеквизита%"" вже налаштована видимість.'");
			ТекстСообщения = СтрЗаменить(ТекстОшибки, "%ИмяРеквизита%", ЭлементСтруктуры.Ключ);
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Иначе
			СтруктураВидимости.Вставить(ЭлементСтруктуры.Ключ, ЭлементСтруктуры.Значение);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого ЭлементСтруктуры Из СтруктураВидимостиПоРеквизитам Цикл
		Если СтруктураВидимости.Свойство(ЭлементСтруктуры.Ключ) Тогда
			ТекстОшибки = НСтр("ru='Для реквизита ""%ИмяРеквизита%"" уже настроена видимость.';uk='Для реквізиту ""%ИмяРеквизита%"" вже налаштована видимість.'");
			ТекстСообщения = СтрЗаменить(ТекстОшибки, "%ИмяРеквизита%", ЭлементСтруктуры.Ключ);
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Иначе
			СтруктураВидимости.Вставить(ЭлементСтруктуры.Ключ, ВидимостьРеквизита(ЭлементСтруктуры.Ключ,
								РеквизитыОтключенныеПоФО,
								РежимВидимостиПоказатьТолькоВажные,
								РеквизитыВидимыеВРежимеПоказатьОсновные,
								ТаблицаОбязательныхРеквизитов,
	...
	СтруктураВидимости.Вставить("ТоварыНаСкладахПоСрокамГодности", (ЭтоТовар
			Или ЭтоМногооборотнаяТара)
		И РеквизитыВидаНоменклатуры.ИспользоватьСерии
		И (РеквизитыВидаНоменклатуры.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваровПоСрокуГодности
			Или РеквизитыВидаНоменклатуры.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваровПоНомеруИСрокуГодности)
...
Функция СтруктураЗаголовковЭлементовФормы(Объект, СтруктураВидимостиЭлементов) Экспорт
	...
	СтруктураЗаголовков.Вставить("ЕдиницаИзмерения", ?(ЭтоТовар Или ЭтоМногооборотнаяТара,
															...
															НСтр("ru='Единица измерения';uk='Одиниця виміру'")));
	...
	Если РазделВиден("ГиперссылкаПерейтиСоставНабора", СтруктураВидимостиЭлементов) Тогда 
		...
		Иначе
			...
			СтруктураВидимостиЭлементов.Вставить("ГиперссылкаПерейтиСоставНабораВнимание", Истина);
		КонецЕсли;
	...
	Если РазделВиден("ГиперссылкаПерейтиСегменты", СтруктураВидимостиЭлементов) Тогда
		...
		Выборка = Выборки[9].Выбрать();
		Пока Выборка.Следующий() Цикл
			
			Если СегментыСервер.ВходитВСегмент(Объект.Ссылка, Выборка.Сегмент) Тогда
				Сегменты.Добавить(Выборка.Сегмент);
			КонецЕсли;
		...
		Если Количество = 1 Тогда
			ЗаголовокГиперссылки = Новый ФорматированнаяСтрока(НСтр("ru='Сегмент';uk='Сегмент'") + " " + Сегменты[0],,,,"Сегменты");
		Иначе
			ЗаголовокГиперссылки = Новый ФорматированнаяСтрока(НСтр("ru='Сегменты';uk='Сегменти'") + " (" + Количество + ")",,,,"Сегменты");
		КонецЕсли;
...
//
Функция ТаблицаОбязательныхРеквизитов(ВидНоменклатуры) Экспорт
	...
	|	РеквизитыДляКонтроля.ОбязателенДляЗаполнения
	|ИЗ
...
// Возвращает таблицу с реквизитами, участвующими в контроле уникальности.
// Параметры:
//	ВидНоменклатуры - СправочникОбъект.ВидыНоменклатуры - элемент справочника виды номенклатуры
//
// Возвращаемое значение:
//	ТаблицаЗначений - таблица с реквизитами, участвующими в контроле уникальности
//
Функция ТаблицаУникальныхРеквизитов(ВидНоменклатуры) Экспорт
...
Функция ПолучитьТабличныйДокументКарточкиНоменклатуры(Номенклатура, ПараметрыВывода = Неопределено) Экспорт	
	...
	Если ГлаваВидна("ОсновныеПараметрыУчета", СтруктураВидимости) Тогда 
		...
		Если РазделВиден("НастройкаИспользованияСерий", СтруктураВидимости) Тогда
			...
			ИначеЕсли НастройкаИспользованияСерий= Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваровПоНомеру Тогда
				НастройкаИспользованияСерийСтрока = НСтр("ru='Партии товаров с одинаковым номером';uk='Партії товарів з однаковим номером'");
			ИначеЕсли НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваровПоНомеруИСрокуГодности Тогда
				НастройкаИспользованияСерийСтрока = НСтр("ru='Партии товаров с одинаковым номером и сроком годности';uk='Партії товарів з однаковим номером і  строком придатності'");
			ИначеЕсли НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваровПоСрокуГодности Тогда
				НастройкаИспользованияСерийСтрока = НСтр("ru='Партии товаров с одинаковым сроком годности';uk='Партії товарів з однаковим  строком придатності'");
			ИначеЕсли НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ЭкземплярТовара Тогда
				НастройкаИспользованияСерийСтрока = НСтр("ru='Экземпляры товаров с уникальным серийным номером';uk='Екземпляри товарів з унікальним серійним номером'");
			КонецЕсли;
...
Функция ТекстЗначенияНеЗаполнен(ИмяРаздела)
	...
	Если ИмяРаздела = "Наименование" Тогда
		Возврат НСтр("ru='<не указано>';uk='<не вказано>'");	
...
// Возвращает имена реквизитов, которые видмы в режиме "ПоказатьОсновные".
//
// Возвращаемое значение:
//	Структура, элементы - Строка - имена реквизитов
//
Функция РеквизитыВидимыеВРежимеПоказатьОсновные() Экспорт
...
//
// Параметры:
//	ТипНоменклатуры - Перечисления.ТипыНоменклатуры
//	АлкогольнаяПродукция - Булево
//	ИспользоватьСрокГодностиСерии - Булево
//	ВариантОказанияУслуг - Перечисления.ВариантыОказанияУслуг
//
// Возвращаемое значение:
...
//
Функция ЗависимостьРеквизитовОтТипаНоменклатуры(ТипНоменклатуры, АлкогольнаяПродукция, 
			ИспользоватьСрокГодностиСерии = Ложь, ВариантОказанияУслуг = Неопределено, ИспользованиеХарактеристик = Неопределено, 
			ВестиУчетПоГТД = Ложь, ИзмеряетсяТолькоВСуммовомВыражении = Ложь, ПодакцизныйТовар = Ложь) Экспорт
			
	
	Если ИспользоватьСрокГодностиСерии = Неопределено Тогда
		ИспользоватьСрокГодностиСерии = Ложь;
	КонецЕсли;
	Если ВариантОказанияУслуг = Неопределено Тогда
		ВариантОказанияУслуг = Ложь;
	КонецЕсли;
	...
	НоваяСтрока.ДействиеСРеквизитом = ?((ЭтоУслуга Или ЭтоРабота)
										...
										И ИспользованиеХарактеристик = Перечисления.ВариантыИспользованияХарактеристикНоменклатуры.НеИспользовать,
										"Добавить",
										"Удалить");
...
//	Массив, элементы - Строка - имена реквизитов
//
Функция ОбязательноОтображаемыеПриСозданииРеквизиты() Экспорт
...
// Возвращает имена реквизитов, которые не участвуют в отборах.
//
// Возвращаемое значение:
//	Массив, элементы - Строка - имена реквизитов
//
