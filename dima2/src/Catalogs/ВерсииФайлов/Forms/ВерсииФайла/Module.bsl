...
Процедура СделатьАктивнойВыполнить()
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ru='Смена активной версии разрешена только для незанятых файлов.';uk='Зміна активної версії дозволена тільки для незайнятих файлів.'"));
	КонецЕсли;
	...
	Если ЧислоВыделенныхСтрок = 2 ИЛИ ЧислоВыделенныхСтрок = 1 Тогда
		...
		Если Не РасширениеПоддерживается Тогда
			ТекстПредупреждения =
				НСтр("ru='Сравнение версий поддерживается только для файлов следующих типов: 
                           ...
                           |;uk='Порівняння версій підтримується тільки для файлів наступних типів: 
                           |   Текстовий документ (.txt)
                           |   Документ формату RTF (.rtf) 
                           |   Документ Microsoft Word (.doc, .docx) 
                           |   Документ HTML (.html .htm) 
                           |   Текстовий документ OpenDocument (.odt)'");
		...
		Если СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ЭтоБазоваяВерсияКонфигурации Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Данная операция не поддерживается в базовой версии.';uk='Дана операція не підтримується в базовій версії.'"));
			Возврат;
		КонецЕсли;
...
Процедура СравнениеВерсийАвтомат(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ТекущийШаг = 2 Тогда
		...
		ТекстСостояния = НСтр("ru='Выполняется сравнение версий файла ""%1""...';uk='Виконується порівняння версій файлу ""%1""...'");
