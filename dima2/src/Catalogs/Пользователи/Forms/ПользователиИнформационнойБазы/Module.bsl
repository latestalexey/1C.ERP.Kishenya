...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь(, Не РазделениеВключено) Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав для открытия списка пользователей информационной базы.';uk='Недостатньо прав для відкриття списку користувачів інформаційної бази.'");
	КонецЕсли;
...
Процедура ОтменитьСопоставление(Команда)
	...
	Кнопки.Добавить("ОтменитьСопоставление", НСтр("ru='Отменить сопоставление';uk='Скасувати зіставлення'"));
	Кнопки.Добавить("ОставитьСопоставление", НСтр("ru='Оставить сопоставление';uk='Залишити зіставлення'"));
	...
	ПоказатьВопрос(
		...
		НСтр("ru='Отмена сопоставления пользователя информационной базы с пользователем в справочнике.
                   ...
                   |;uk='Скасування зіставлення користувача інформаційної бази з користувачем у довіднику.
                   |
                   |Скасування зіставлення потрібно вкрай рідко - тільки якщо зіставлення було виконано некоректно, наприклад,
                   |при оновленні інформаційної бази, тому не рекомендується скасовувати зіставлення по будь-якій іншій причині.'"),
		Кнопки,
		,
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<Нет данных>';uk='<Немає даних>'"));
...
Процедура УдалитьТекущегоПользователяИБ(УдалитьСтроку = Ложь)
	...
	ПоказатьВопрос(
		...
		НСтр("ru='Удалить пользователя информационной базы?';uk='Вилучити користувача інформаційної бази?'"),
