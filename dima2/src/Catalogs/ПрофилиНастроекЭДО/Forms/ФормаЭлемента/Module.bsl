...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ОбменСКонтрагентамиСлужебныйКлиент.ПроверитьИспользованиеИнтернетПоддержкаПользователей() Тогда
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Объект.ПометкаУдаления И Не ПропускатьПроверкуНастроекЭДО И ОтсутствуютПодчиненныеНастройкиЭДО() Тогда
		ТекстВопроса = НСтр("ru='Подключить контрагента к обмену электронными документами?';uk='Підключити контрагента до обміну електронними документами?'");
		...
		Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
			...
			Если ТипВыбЗначения = Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования") Тогда
				...
				Если МассивСтрок.Количество() > 0 Тогда
					ТекстПредупреждения = НСтр("ru='Выбранный сертификат уже зарегистрирован в профиле настроек ЭДО.';uk='Вибраний сертифікат вже зареєстрований в профілі настройок ЕДО.'");
...
Процедура НадписьЛичныйКабинетУчастникаОбменаЭДНажатие(Элемент)
	...
	Если Объект.СертификатыПодписейОрганизации.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Для входа в личный кабинет должен быть зарегистрирован хотя бы один сертификат';uk='Для входу в особистий кабінет повинен бути зареєстрований хоча б один сертифікат'");
		ПоказатьПредупреждение(, ТекстПредупреждения, 30);
		Возврат;
	КонецЕсли;
	...
	Если Модифицированность Тогда
		ТекстВопроса = НСтр("ru='Выполнить действие можно только в записанном профиле настроек ЭДО.
            |Записать?'
            |;uk='Виконати дію можна тільки в записаному профілі настройок ЕДО.
            |Записати?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
...
Процедура ТекстПриглашенийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("ru='Шаблон текста для приглашений контрагентов';uk='Шаблон тексту запрошень для контрагентів'");
...
Процедура ИдентификаторОрганизацииОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Если Не КонтролироватьСодержимоеНастроекЭД Тогда
		ТекстВопроса = НСтр("ru='Изменения ""Профиля настроек ЭДО"" будут применены для связанных ""Настроек ЭДО"" с отключенным расширенным режимом.
                                |Продолжить?'
                                |;uk='Зміни ""Профілю настройок ЕДО"" будуть застосовані для пов''язаних ""Настройок ЕДО"" з відключеним розширеним режимом.
                                |Продовжити?'");
...
Процедура ТестПрофиляНастроек(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='В текущий профиль настроек ЭДО внесены изменения. Продолжить выполнение теста?';uk='У поточний профіль настройок ЕДО внесені зміни. Продовжити виконання тесту?'");
...
Процедура ДобавитьСертификат(Команда)
	...
	ТекстВопроса = НСтр("ru='Операция добавления сертификата в профиль настроек ЭДО займет несколько минут.
                            |Продолжить?'
                            |;uk='Операція додавання сертифіката в профіль настройок ЕДО займе кілька хвилин.
                            |Продовжити?'");
...
Процедура ТестПрофиляНастроекПродолжить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(Объект.СпособОбменаЭД) Тогда
		Если Объект.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту") Тогда
			...
			РаботаСПочтовымиСообщениямиКлиент.ПроверитьУчетнуюЗапись(Объект.РесурсВходящихДокументов);
		КонецЕсли;
		...
		Если Объект.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезКаталог") Тогда
            ...
            |%1'
            |;uk='Тест. Перевірка доступу до загальному каталогу для обміну ЕД.
            |%1'");
			Попытка
	...
	Иначе
		...
		Состояние(НСтр("ru='Тест настроек.';uk='Тест настройок.'"),
			,
			НСтр("ru='Выполняется тестирование связи с сервисом ЭДО. Подождите...';uk='Виконується тестування зв''язку з сервісом ЕДО. Почекайте...'"));
...
Процедура ПослеПолученияОтпечатковПроверитьСертификаты(Отпечатки, Параметры = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Сертификат) Тогда
		ОбменСКонтрагентамиСлужебныйКлиент.ТестНастроекСПроверкойСертификата(Сертификат, , , ЭтаФорма);
	Иначе
		ТекстСообщения = НСтр("ru='Нет доступных сертификатов. Тест не выполнен.';uk='Немає доступних сертифікатів. Тест не виконаний.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ДействияПослеТестаСвязиЭДО(РезультатВыполнения, Контекст = Неопределено) Экспорт
	...
	Если ТипЗнч(РезультатВыполнения) = Тип("Структура") Тогда
		Если РезультатВыполнения.Успех = Истина Тогда
			ТекстСообщения = НСтр("ru='Тест профиля настроек выполнен успешно.';uk='Тест профілю налаштувань виконаний успішно.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПерейтиВЛичныйКабинет(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	ОбменСКонтрагентамиСлужебныйКлиент.ПолучитьСоответствиеСоглашенийИПараметровСертификатов(ОбработкаОповещения, Массив);
...
Процедура ПерейтиВЛичныйКабинетОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстСообщения = НСтр("ru='Нет доступных сертификатов, среди зарегистрированных по данному профилю настроек ЭДО.';uk='Немає доступних сертифікатів, серед зареєстрованих за даним профілем настройок ЕДО.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,
			,
	...
	Если ЗначениеЗаполнено(СертификатПодписи) Тогда
		// Здесь необходимо вставить код старта механизма работы с оператором ЭДО
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементов()
	...
	Элементы.ИсходящиеДокументыИспользоватьЭП.Видимость = ИспользоватьЭП И ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(Объект.СпособОбменаЭД);
...
Процедура ЗарегистрироватьСертификат1СЭДО()
	...
	Если Не ЗначениеЗаполнено(Объект.КодРегиона) Тогда
		...
		ВыбратьАдрес(Ложь);
	Иначе
		ЗарегистрироватьСертификат1СЭДОПродолжить(Неопределено);
	КонецЕсли;
...
Процедура ЗарегистрироватьСертификат1СЭДОПродолжить(РезультатВыполнения, Контекст = Неопределено) Экспорт
	...
	ОбменСКонтрагентамиСлужебныйКлиент.ТестНастроекСПроверкойСертификата(ДобавляемыйСертификат,
...
Процедура ПодготовитьДанныеДляРегистрационногоПакета1СЭДОПродолжение2(РезультатВыполнения, Контекст) Экспорт
	...
	Если Не ЗначениеЗаполнено(ОператорЭДОСертификат) Тогда
		...
		ТекстСообщения = НСтр("ru='Ошибка получения сертификата оператора ЭДО.
                                    ...
                                    |Необхідно повторити додавання сертифіката.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ПодготовитьДанныеДляРегистрационногоПакета1СЭДОЗавершение(РезультатВыполнения, Контекст) Экспорт
	...
	ЕстьОшибки = Ложь;
	СформироватьИОтправитьРегистрационныйПакет1СЭДО(Контекст.РеквизитыПакета, ЗашифрованныеДвоичныеДанныеОператораЭДО,
...
Процедура СформироватьИОтправитьРегистрационныйПакет1СЭДО(РеквизитыПакета, ЗашифрованныеДвоичныеДанныеОператораЭДО, ПодписанныеДвоичныеДанныеДляОператораЭДО,
	...
	ТекстЗаголовкаСообщения = НСтр("ru='Отправка заявления на изменение данных на сервисе 1С-ЭДО';uk='Відправлення заяви на зміну даних на сервісі 1С-ЕДО'");
...
Процедура ОткрытьФормуВыбораСертификатаЭП()
	...
	Если Объект.ПометкаУдаления Тогда
		ТекстСообщения = НСтр("ru='Для выполнения действия необходимо снять пометку удаления.';uk='Для виконання дії необхідно зняти позначку на вилучення.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если ОбменСКонтрагентамиСлужебныйКлиент.ПроверитьИспользованиеИнтернетПоддержкаПользователей() Тогда
		...
		Если ПустаяСтрока(Объект.ИдентификаторОрганизации)
			...
			ТекстПредупреждения = НСтр("ru='Перед добавлением новых сертификатов в профиль настроек ЭДО
                                            ...
                                            |;uk='Перед додаванням нових сертифікатів в профіль настройок ЕДО
                                            |необхідно отримати ідентифікатор учасника обміну ЕД'");
			ПоказатьПредупреждение(, ТекстПредупреждения, 30);
			Возврат;
