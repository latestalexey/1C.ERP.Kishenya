...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипСтрокиДерева = "Раздел" Тогда
		...
		Элементы.Формулировка.Заголовок             = НСтр("ru='Имя раздела';uk='Ім''я розділу'");
		Заголовок                                   = НСтр("ru='Раздел шаблона анкеты';uk='Розділ шаблону анкети'");
...
Процедура ЗаметкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(ОповещениеОЗакрытии, Элемент.ТекстРедактирования, НСтр("ru='Заметки';uk='Замітки'"));
...
Процедура ПеренестиВШаблон(Команда)
	...
	Если Не ЗначениеЗаполнено(Формулировка) Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнена формулировка';uk='Не заповнене формулювання'"),,"Формулировка");
	КонецЕсли;
	
	Если ТипСтрокиДерева = "Вопрос" И (Не ЗначениеЗаполнено(ЭлементарныйВопрос)) Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан элементарный вопрос';uk='Не вказано елементарне питання'"),,"ЭлементарныйВопрос");
	КонецЕсли; 
