...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого КлючИЗначение Из ПараметрыРеквизитов Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Поле %1 не заполнено.';uk='Поле %1 не заповнено.'"),
...
Процедура ПроверитьСертификат(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Сертификат еще не записан.';uk='Сертифікат ще не записаний.'"));
		Возврат;
	КонецЕсли;
...
Процедура СертификатОтозван(Команда)
	...
	Если Объект.Отозван Тогда
		ПоказатьПредупреждение(, НСтр("ru='После записи отменить отзыв будет невозможно.';uk='Після запису відгук скасувати буде неможливо.'"));
	КонецЕсли;
