...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоАутентификация Тогда
		Элементы.ФормаРасшифровать.Заголовок = НСтр("ru='ОК';uk='ОК'");
		Элементы.ПояснениеУсиленногоПароля.Заголовок = НСтр("ru='Нажмите ОК, чтобы перейти к вводу пароля.';uk='Натисніть ОК, щоб перейти до введення пароля.'");
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(СертификатПрограмма) Тогда
		Контекст.ОшибкаНаКлиенте.Вставить("ОписаниеОшибки",
			НСтр("ru='У выбранного сертификата не указана программа для закрытого ключа.
                       ...
                       |;uk='У обраного сертифіката не вказана програма для закритого ключа.
                       |Оберіть сертифікат повторно з повного списку або
                       |відкрийте сертифікат і вкажіть програму вручну.'"));
...
Процедура ЗаписатьСертификатыШифрованияНаСервере(ОписаниеОбъектов, ИдентификаторФормы, Ошибка)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		Ошибка.Вставить("ОписаниеОшибки", НСтр("ru='При очистке сертификатов шифрования возникла ошибка:';uk='При очищенні сертифікатів шифрування виникла помилка:'")
...
Процедура ПоказатьОшибку(ОшибкаНаКлиенте, ОшибкаНаСервере)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ru='Не удалось расшифровать данные';uk='Не вдалося розшифрувати дані'"), "",
