...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПроверкаПриВыборе Тогда
		Элементы.ФормаПроверить.Заголовок = НСтр("ru='Проверить и продолжить';uk='Перевірити і продовжити'");
		Элементы.ФормаЗакрыть.Заголовок   = НСтр("ru='Отмена';uk='Відмінити'");
	КонецЕсли;
		...
		Если ДополнительныеПроверки.Количество() = 0 Тогда
			ВызватьИсключение
				НСтр("ru='Для проверки сертификата отключены стандартные проверки,
                           ...
                           |;uk='Для перевірки сертифіката відключені стандартні перевірки,
                           |при цьому додаткових перевірок не зазначено.'");
		КонецЕсли;
...
Процедура ПоказатьПредупреждениеОНевозможностиПродолжения()
	...
	ПоказатьПредупреждение(,
		НСтр("ru='Не удалось продолжить, т.к. пройдены не все требуемые проверки.';uk='Не вдалося продовжити, оскільки не пройдені всі необхідні перевірки.'"));
...
Процедура ПроверитьНаСторонеКлиентаПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	Если Контекст.СертификатКриптографии.Субъект.Свойство("SN") Тогда
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = НСтр("ru='В описании субъекта сертификата не найдено поле ""SN"".';uk='В описі суб''єкта сертифіката не знайдено полі ""SN"".'");
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		ОписаниеОшибки = Результат.ОписаниеОшибки + Символы.ПС + Символы.ПС
			+ НСтр("ru='Проверка подписания, созданной подписи и расшифровки не могут быть выполнены.';uk='Перевірка підписання, створеної підпису і розшифрування не можуть бути виконані.'");
	Иначе
		ОписаниеОшибки = "";
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПроверкиСертификата(Результат, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru='Программа для использования закрытого ключа не указана в сертификате.';uk='Програма для використання закритого ключа не зазначена у сертифікаті.'");
	...
	Иначе
		ОписаниеОшибки = Результат + Символы.ПС + Символы.ПС
			+ НСтр("ru='Проверка подписания, созданной подписи, шифрования и
                         ...
                         |;uk='Перевірка підписання, створеного підпису, шифрування і
                         |розшифровки не можуть бути виконані.'");
	КонецЕсли;
...
Процедура УстановитьЭлемент(Форма, НачалоЭлемента, НаСервере, ОписаниеОшибки = Неопределено, ЭтоОшибка = Ложь)
	...
	Если ОписаниеОшибки = Неопределено Тогда
		...
		ЭлементКартинка.Подсказка   = НСтр("ru='Проверка не выполнялась.';uk='Перевірка не виконувалася.'");
	...
	Иначе
		...
		ЭлементКартинка.Подсказка   = НСтр("ru='Проверка выполнена успешно.';uk='Перевірка виконана успішно.'");;
