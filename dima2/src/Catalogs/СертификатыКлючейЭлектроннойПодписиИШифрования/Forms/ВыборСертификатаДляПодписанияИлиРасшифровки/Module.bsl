...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДобавлениеВСписок Тогда
		...
		Элементы.Выбрать.Заголовок = НСтр("ru='Добавить';uk='Додати'");
		
		Элементы.ПояснениеУсиленногоПароля.Заголовок =
			НСтр("ru='Нажмите Добавить, чтобы перейти к вводу пароля.';uk='Натисніть Додати, щоб перейти до введення пароля.'");
		
		ЛичныйСписокПриДобавлении = Параметры.ЛичныйСписокПриДобавлении;
		Элементы.ПоказыватьВсе.Подсказка =
			НСтр("ru='Показать все сертификаты без отбора (например, включая добавленные и просроченные)';uk='Показати всі сертифікати без відбору (наприклад, включаючи додані і прострочені)'");
	КонецЕсли;
	...
	Если ДляШифрованияИРасшифровки = Истина Тогда
		Если Параметры.ДобавлениеВСписок Тогда
			Заголовок = НСтр("ru='Добавление сертификата для шифрования и расшифровки данных';uk='Додавання сертифіката для шифрування і розшифровки даних'");
		Иначе
			Заголовок = НСтр("ru='Выбор сертификата для шифрования и расшифровки данных';uk='Обрати сертифікат для шифрування і розшифровки даних'");
		КонецЕсли;
	ИначеЕсли ДляШифрованияИРасшифровки = Ложь Тогда
		Если Параметры.ДобавлениеВСписок Тогда
			Заголовок = НСтр("ru='Добавление сертификата для подписания данных';uk='Додавання сертифіката для підписання даних'");
		КонецЕсли;
	ИначеЕсли ЭлектроннаяПодпись.ИспользоватьШифрование() Тогда
		Заголовок = НСтр("ru='Добавление сертификата для подписания и шифрования данных';uk='Додавання сертифіката для підписання та шифрування даних'");
	Иначе
		Заголовок = НСтр("ru='Добавление сертификата для подписания данных';uk='Додавання сертифіката для підписання даних'");
	КонецЕсли;
	...
	Если СоздаватьЭлектронныеПодписиНаСервере Тогда
		Элементы.ГруппаСертификаты.Заголовок =
			НСтр("ru='Личные сертификаты на компьютере и сервере';uk='Особисті сертифікати на комп''ютері і сервері'");
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Организации") Тогда
		...
		Элементы.СертификатОрганизация.Видимость =
			...
			Или МодульОрганизацииСлужебный.ИспользуетсяНесколькоОрганизаций();
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Элементы.СертификатОрганизация.Видимость
	   ...
	   И Не ЗначениеЗаполнено(СертификатОрганизация) Тогда
		...
		ТекстСообщения = НСтр("ru='Поле Организация не заполнено.';uk='Поле Організація не заповнено.'");
...
Процедура СертификатыНедоступныНаКлиентеНадписьНажатие(Элемент)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ru='Сертификаты на компьютере';uk='Сертифікати на комп''ютері'"), "", ОшибкаПолученияСертификатовНаКлиенте, Новый Структура);
...
Процедура СертификатыНедоступныНаСервереНадписьНажатие(Элемент)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ru='Сертификаты на сервере';uk='Сертифікати на сервері'"), "", ОшибкаПолученияСертификатовНаСервере, Новый Структура);
...
Процедура Выбрать(Команда)
	...
	Иначе
		ПроверитьСертификат(Новый ОписаниеОповещения("ВыбратьПослеПроверкиСертификата", ЭтотОбъект, Контекст));
	КонецЕсли;
...
Процедура ВыбратьПослеПроверкиСертификата(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			ЗаголовокФормы = НСтр("ru='Проверка шифрования и расшифровки';uk='Перевірка шифрування і розшифрування'");
		Иначе
			ЗаголовокФормы = НСтр("ru='Проверка установки электронной подписи';uk='Перевірка установки електронного підпису'");
		КонецЕсли;
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
...
Процедура ОбновитьСписокСертификатовНаСервере(Знач СвойстваСертификатовНаКлиенте)
	...
	ЭлектроннаяПодписьСлужебный.ОбновитьСписокСертификатов(Сертификаты, СвойстваСертификатовНаКлиенте,
		ДобавлениеВСписок, Истина, ОшибкаПолученияСертификатовНаСервере, ПоказыватьВсе);
...
Процедура ПерейтиКВыборуТекущегоСертификата(Оповещение)
	...
	Если Элементы.Сертификаты.ТекущиеДанные = Неопределено Тогда
		Результат.ОписаниеОшибки = НСтр("ru='Выделите сертификат, который будет использоваться.';uk='Оберіть сертифікат, який буде використовуватися.'");
	...
	Если ТекущиеДанные.ЭтоЗаявление Тогда
		...
		Результат.ОписаниеОшибки =
			НСтр("ru='Для этого сертификата заявление на выпуск еще не исполнено.
                       ...
                       |;uk='Для цього сертифікату заяву на випуск ще не виконане.
                       |Відкрийте заяву на випуск сертифіката і виконайте необхідні кроки.'");
		ВыполнитьОбработкуОповещения(Оповещение, Результат);
		Возврат;
	КонецЕсли;
	...
	Если СертификатНаСервере Тогда
		...
		Иначе
			Результат.ОписаниеОшибки = НСтр("ru='Сертификат не найден на сервере (возможно удален).';uk='Сертифікат не знайдено на сервері (можливо вилучений).'");
			Результат.ОбновитьСписокСертификатов = Истина;
			ВыполнитьОбработкуОповещения(Оповещение, Результат);
		КонецЕсли;
		Возврат;
...
Процедура ПерейтиКВыборуТекущегоСертификатаПослеПоискаСертификата(РезультатПоиска, Контекст) Экспорт
	...
	Если ТипЗнч(РезультатПоиска) <> Тип("СертификатКриптографии") Тогда
		Если РезультатПоиска.Свойство("СертификатНеНайден") Тогда
			Контекст.Результат.ОписаниеОшибки = НСтр("ru='Сертификат не найден на компьютере (возможно удален).';uk='Сертифікат не знайдений на комп''ютері (можливо вилучений).'");
		Иначе
...
Процедура ПерейтиКВыборуТекущегоСертификатаПослеЗаполненияСвойствСертификата(Контекст)
	...
	Если ДобавлениеВСписок Тогда
		Строка = ?(ЗначениеЗаполнено(Сертификат), НСтр("ru='Обновить';uk='Оновити'"), НСтр("ru='Добавить';uk='Додати'"));
...
Процедура ПроверитьСертификатПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии")
	   И Результат.Общая Тогда
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			Результат.Вставить("ЗаголовокОшибки", НСтр("ru='Не удалось пройти проверку шифрования по причине:';uk='Не вдалося пройти перевірку шифрування з причини:'"));
		Иначе
			Результат.Вставить("ЗаголовокОшибки", НСтр("ru='Не удалось пройти проверку подписания по причине:';uk='Не вдалося пройти перевірку підписання з причини:'"));
		КонецЕсли;
...
Функция ПроверитьСертификатИЗаписатьВСправочник(Знач ЗначениеПароля, ОшибкаНаСервере)
	...
	Если ЭлектроннаяПодписьСлужебный.МенеджерКриптографии("", Ложь, ОшибкаНаСервере) = Неопределено
	   И ОшибкаНаСервере.Общая Тогда
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			ОшибкаНаСервере.Вставить("ЗаголовокОшибки", НСтр("ru='Не удалось пройти проверку шифрования по причине:';uk='Не вдалося пройти перевірку шифрування з причини:'"));
		Иначе
			ОшибкаНаСервере.Вставить("ЗаголовокОшибки", НСтр("ru='Не удалось пройти проверку подписания по причине:';uk='Не вдалося пройти перевірку підписання з причини:'"));
		КонецЕсли;
		Возврат Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьОшибкуШифрования(Ошибка, ОписаниеОшибки, ОписаниеПрограммы, ПредставлениеОшибки)
	...
	ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось пройти проверку шифрования с помощью программы %1 по причине:
                   |%2'
                   |;uk='Не вдалося пройти перевірку шифрування за допомогою програми %1 з причини:
                   |%2'"),
...
Процедура ЗаполнитьОшибкуРасшифровки(Ошибка, ОписаниеОшибки, ОписаниеПрограммы, ПредставлениеОшибки)
	...
	ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось пройти проверку расшифровки с помощью программы %1 по причине:
                   |%2'
                   |;uk='Не вдалося пройти перевірку розшифровки за допомогою програми %1 з причини:
                   |%2'"),
		ОписаниеПрограммы.Наименование,
		ПредставлениеОшибки);
...
Процедура ЗаполнитьОшибкуПодписания(Ошибка, ОписаниеОшибки, ОписаниеПрограммы, ПредставлениеОшибки, ПустыеДанные)
	...
	ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось пройти проверку подписания с помощью программы %1 по причине:
                   |%2'
                   |;uk='Не вдалося пройти перевірку підписання за допомогою програми %1 з причини:
                   |%2'"),
		ОписаниеПрограммы.Наименование,
		ПредставлениеОшибки);
