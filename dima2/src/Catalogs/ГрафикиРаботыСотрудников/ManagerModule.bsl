...
Функция ТекстовоеОписаниеГрафиковРаботы(МассивСсылок) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.СпособЗаполнения = Перечисления.СпособыЗаполненияГрафикаРаботы.ПоНеделям Тогда
			ШаблонОписания = НСтр("ru='%1 ч., %2 - дневная рабочая неделя';uk='%1 ч., %2 - денний робочий тиждень'");
		...
		Иначе
			ШаблонОписания = НСтр("ru='%1 ч., сменный';uk='%1 ч., змінний'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='График работы';uk='Графік роботи'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ФормаГрафикаРаботы") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
				...
				"ФормаГрафикаРаботы", НСтр("ru='График работы';uk='Графік роботи'"),
...
Функция ПечатнаяФормаГрафиковРаботы(МассивОбъектов, Год)
	...
	ТабличныйДокумент.ИмяПараметровПечати = "ГрафикиРаботыСотрудников_ФормаГрафикаРаботы";
	...
	Пока ВыборкаДанныхОВремени.СледующийПозначениюПоля("ГрафикРаботы") Цикл
		...
		Если ОписаниеГрафика.КоличествоВидовВремени > 1 Тогда 													  
			ОбластьЗаголовокТаблицы.Параметры.ОбластьДнейЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Часы по видам времени %1, выведены в формате %2';uk='Години за видами часу %1, виведені у форматі %2'"), 
				ОписаниеГрафика.ВидыВремениСтрока, 
				ОписаниеГрафика.ОбозначениеВидовВремени);	
		Иначе														  
			ОбластьЗаголовокТаблицы.Параметры.ОбластьДнейЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Часов за день (%1)';uk='Годин за день (%1)'"),
				ОписаниеГрафика.ВидыВремениСтрока);
		КонецЕсли;	
...
Функция ТекстОписанияГрафикаРаботы(ОписаниеГрафикаРаботы) Экспорт
	...
	Если Не ОписаниеГрафикаРаботы.НеполноеРабочееВремя Тогда 
		РабочееВремяТекст = НСтр("ru='Полная занятость';uk='Повна зайнятість'");
	ИначеЕсли ОписаниеГрафикаРаботы.НеполныйРабочийДень Тогда 
		РабочееВремяТекст = НСтр("ru='Неполный рабочий день';uk='Неповний робочий день'");
	Иначе
		РабочееВремяТекст = НСтр("ru='Неполная рабочая неделя';uk='Неповний робочий тиждень'");
	КонецЕсли;
	...
	Если ОписаниеГрафикаРаботы.СпособЗаполнения = Перечисления.СпособыЗаполненияГрафикаРаботы.ПоНеделям Тогда
		...
		Если КоличествоРабочихДней = 5 Тогда
			ВидГрафикаСтрока = НСтр("ru='Пятидневка';uk='П''ятиденка'");
		...
		Если НомераВыходныхДней.Количество() = 0 Тогда
			ВыходныеДниТекст = НСтр("ru='без выходных';uk='без вихідних'");
		Иначе	
        	ПеречислениеДнейТекст = "";
	...
	Иначе
		ВидГрафикаСтрока = НСтр("ru='Скользящий график';uk='Гнучкий графік'");	
	КонецЕсли;	
	
	СтрокаНастройкиЗаполнения = НСтр("ru='%1, раб. дни: %2, выходн. дни: %3
                                      |%4. Длительность рабочей недели: %5 чс.'
                                      |;uk='%1, раб. дні: %2, выходн. дні: %3
                                      |%4. Тривалість робочого тижня: %5 год.'");
