...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.КроссТаблица Тогда
		...
		Если ЗаполнятьПартнераВТЧ Тогда
			СтруктураДействий.Вставить("ЗаполнятьПартнераВТЧ",НСтр("ru='Клиент';uk='Клієнт'"));
		КонецЕсли; 
...
Процедура СоглашениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПродажиКлиент.НачалоВыбораСоглашенияСКлиентом(Элемент, СтандартнаяОбработка, Объект.Партнер, Объект.Соглашение,,,,,Объект);
...
Процедура ТоварыСоглашениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПродажиКлиент.НачалоВыбораСоглашенияСКлиентом(Элемент, СтандартнаяОбработка, ТекущиеДанные.Партнер, ТекущиеДанные.Соглашение);
...
Процедура ПодобратьТовары(Команда)
	...
	Если Объект.ПланироватьПоСумме И ЗаполнятьСоглашение Тогда
		...
		ПараметрыФормы.Вставить("ЗаголовокКнопкиЗапрашиватьКоличествоИЦену", НСтр("ru='Запрашивать количество и цену';uk='Запитувати кількість і ціну'"));
...
Процедура СценарийПриИзмененииСервер()
	...
	Реквизиты = "Периодичность, ОтображатьНомерПериода, Валюта, ПланПродажПланироватьПоСумме";
	//++ НЕ УТ
...
Процедура УстановитьВидимость()
	...
	Элементы.ГруппаДоговор.Видимость = ИспользуютсяДоговорыКонтрагентов И ЗаполнятьСоглашение И Объект.ЗаполнятьПланОплат;
	...
	Элементы.СтатьяБюджетовОплатыДоОтгрузки.Видимость = Объект.ОтражаетсяВБюджетированииОплаты;
	Элементы.СтатьяБюджетовОплатыПослеОтгрузки.Видимость = Объект.ОтражаетсяВБюджетированииОплатыКредит;
	Элементы.СтатьяБюджетовОплатыДоОтгрузки.Заголовок = 
...
Функция ПолучитьПараметрыВидаПлана(Знач ВидПлана, Знач ОбновлятьПравило, АдресПравилаЗаполнения, АдресПользовательскихНастроек)
	...
	Реквизиты = "ЗаполнятьПодразделение, ЗаполнятьПартнера, 
		...
		|ЗаполнятьСкладВТЧ, ЗапретитьРедактированиеПравила, ЗаполнятьПоФормуле, ЗаполнятьПланОплат, ЗаполнятьМенеджера, ЗаполнятьФорматМагазина";
	//++ НЕ УТ
	Реквизиты = Реквизиты + ", ОтражаетсяВБюджетировании, СтатьяБюджетов, 
		|ОтражаетсяВБюджетированииОплаты, СтатьяБюджетовОплат, 
		|ОтражаетсяВБюджетированииОплатыКредит, СтатьяБюджетовОплатКредит";
	//-- НЕ УТ
...
Процедура ЗаполнитьЭтапыОплатыПоШаблонуСервер(ГрафикСоглашенияЗаполнен, ГрафикЗаполнен)
	...
	Иначе
		...
		Для каждого СуммаНаДату Из СуммыПоДатам Цикл
			...
			ПараметрыЗаполнения.Вставить("ГрафикОплаты", Объект.ГрафикОплаты);
			ПараметрыЗаполнения.Вставить("ЭтапыГрафикаОплаты", Объект.ПланОплаты);
			ПараметрыЗаполнения.Вставить("Дата", СуммаНаДату.ДатаОтгрузки);
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	КолонкиПоиска.Добавить(НСтр("ru='Уникальный идентификатор (Клиент)';uk='Унікальний ідентифікатор (Клієнт)'"),"ИдентификаторПартнер");
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров) Экспорт
	...
	//++ НЕ УТ
	Реквизиты = Реквизиты + ", ИспользоватьВПланированииПроизводства, ИспользоватьДляПланированияМатериалов";
	//-- НЕ УТ
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='План продаж +15%';uk='План продажів +15%'"), "[ПланыПродаж] * 1.15");
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='План продаж с учетом корректировки по факту продаж';uk='План продажів з урахуванням коригування за фактом продажу'"), 
																		"Макс([ФактыПродаж], [ПланыПродаж]) - [СвободныеОстатки]");
																		
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Факт продаж с учетом наличия товаров на остатках';uk='Факт продажів з урахуванням наявності товарів на залишках'"), 
																		"[ФактыПродаж] / [ДоляДнейНаличияТовараНаОстатках]");
...
Функция ПараметрыВидаПлана(Форма)
	...
	ПараметрыВидаПлана = Новый Структура("ЗаполнятьПодразделение, ЗаполнятьПартнера, ЗаполнятьСоглашение, ЗаполнятьСклад,
		|ЗаполнятьМенеджера, ЗаполнятьФорматМагазина");
...
Функция ПараметрыОтбора(Форма)
	...
	ПараметрыОтбора = Новый Структура("Партнер, Соглашение, Склад, Подразделение, Менеджер, ФорматМагазина");
