...
Процедура СтрокаФизическиеЛицаНажатие(Элемент, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ведомость) И ИспользоватьНачислениеЗарплаты Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Поле ""Ведомость"" не заполнено';uk='Поле ""Відомість"" не заповнено'"),
			,
			"Ведомость",
			"Объект",
			Отказ);
	...
	Если НЕ Отказ Тогда
		...
		ПараметрыФормы.Вставить("Работник", Объект.ПодотчетноеЛицо);
		ПараметрыФормы.Вставить("ОтборПоРаботнику", Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыРаботнику"));
