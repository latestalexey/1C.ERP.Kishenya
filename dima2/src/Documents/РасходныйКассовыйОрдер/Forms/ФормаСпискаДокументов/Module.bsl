...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИспользоватьНачислениеЗарплаты Тогда
		...
		ВыплатаЗарплатыПоЗаявкам.ТекстЗапроса = ТекстЗапросаВыплатаЗарплатыПоЗаявкам();
	КонецЕсли;
	//-- НЕ УТ
...
&НаСервере
Функция ТекстЗапросаВыплатаЗарплатыПоЗаявкам()
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	Заявки.Ссылка КАК Заявка,
	|	Заявки.Ссылка.Касса КАК Касса,
	|	КВыплатеОстатки.ЗаявкаНаРасходованиеДенежныхСредств КАК Ведомость,
	|	КВыплатеОстатки.ЗаявкаНаРасходованиеДенежныхСредств.Организация КАК Организация,
	|	КВыплатеОстатки.ЗаявкаНаРасходованиеДенежныхСредств.Подразделение КАК Подразделение,
	|	ВЫРАЗИТЬ(КВыплатеОстатки.ЗаявкаНаРасходованиеДенежныхСредств.Комментарий КАК СТРОКА(150)) КАК Комментарий,
	|	СУММА(-КВыплатеОстатки.СуммаОстаток) КАК Сумма
	|ИЗ
	|	РегистрНакопления.ДенежныеСредстваКВыплате.Остатки КАК КВыплатеОстатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаРасходованиеДенежныхСредств.ВедомостиНаВыплатуЗарплаты КАК Заявки
	|		ПО (Заявки.Ведомость = КВыплатеОстатки.ЗаявкаНаРасходованиеДенежныхСредств)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаРасходованиеДенежныхСредств КАК ДанныеДокумента
	|		ПО (ДанныеДокумента.Ссылка = Заявки.Ссылка)
	|			И (ДанныеДокумента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаРасходованиеДенежныхСредств.КОплате))
	|			И (ДанныеДокумента.Проведен)
	|ГДЕ
	|	(КВыплатеОстатки.ЗаявкаНаРасходованиеДенежныхСредств ССЫЛКА Документ.ВедомостьНаВыплатуЗарплатыВКассу
	|			ИЛИ КВыплатеОстатки.ЗаявкаНаРасходованиеДенежныхСредств ССЫЛКА Документ.ВедомостьНаВыплатуЗарплатыРаздатчиком)
	|	И (КВыплатеОстатки.ПодотчетноеЛицо = &Работник
	|			ИЛИ &ВыплатаПоВедомости)
	|
	|СГРУППИРОВАТЬ ПО
	|	Заявки.Ссылка,
	|	Заявки.Ссылка.Касса,
	|	КВыплатеОстатки.ЗаявкаНаРасходованиеДенежныхСредств,
	|	КВыплатеОстатки.ЗаявкаНаРасходованиеДенежныхСредств.Подразделение,
	|	КВыплатеОстатки.ЗаявкаНаРасходованиеДенежныхСредств.Организация,
	|	ВЫРАЗИТЬ(КВыплатеОстатки.ЗаявкаНаРасходованиеДенежныхСредств.Комментарий КАК СТРОКА(150))";
