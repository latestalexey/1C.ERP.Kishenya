...
//++ НЕ УТКА
	...
	|	ПередачаДавальцуТовары.ЗаказДавальца,
	|	ПередачаДавальцуТовары.ЗаказДавальца.СпособДоставки,
	|	НЕОПРЕДЕЛЕНО,
	|	""НеверныйСпособДоставки""
	|ИЗ
	|	Документ.ПередачаДавальцу.Товары КАК ПередачаДавальцуТовары
	|ГДЕ
	|	ПередачаДавальцуТовары.Ссылка В(&МассивОбъектов)
	|	И ПередачаДавальцуТовары.ЗаказДавальца.СпособДоставки В (&СпособыДоставкиСНашимУчастием)
	|
//-- НЕ УТКА
//++ НЕ УТ
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ПередачаСырьяПереработчикуТовары.ЗаказПереработчику,
	|	ПередачаСырьяПереработчикуТовары.ЗаказПереработчику.СпособДоставки,
	|	НЕОПРЕДЕЛЕНО,
	|	""НеверныйСпособДоставки""
	|ИЗ
	|	Документ.ПередачаСырьяПереработчику.Товары КАК ПередачаСырьяПереработчикуТовары
	|ГДЕ
	|	ПередачаСырьяПереработчикуТовары.Ссылка В(&МассивОбъектов)
	|	И ПередачаСырьяПереработчикуТовары.ЗаказПереработчику.СпособДоставки В (&СпособыДоставкиСНашимУчастием)
	|
//-- НЕ УТ
	|";
...
Функция ПолучитьТолькоДокументыОснованияСТоварами(МассивВсехРаспоряжений, Отказ = Неопределено) Экспорт
	...
	//++ НЕ УТ
	Документы.ПередачаСырьяПереработчику.ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, Ложь);
	//-- НЕ УТ
	
	//++ НЕ УТКА
	ПараметрыЗаполнения = ПродажиСервер.ПараметрыЗаполненияВременнойТаблицыТоваров();
	ПараметрыЗаполнения.ПересчитыватьВВалютуРегл = Ложь;
	Документы.ПередачаДавальцу.ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения);
	Документы.ВозвратСырьяДавальцу.ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, Ложь);
	//-- НЕ УТКА
...
//++ НЕ УТКА
	...
	|		Документ.ВозвратСырьяДавальцу КАК ВозвратСырьяДавальцу
	|	ГДЕ
	|		ВозвратСырьяДавальцу.Ссылка = &ДокументСсылка
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ДокументТовары.ЗаказДавальца.АдресДоставки,
	|		ВЫБОР
	|			КОГДА ДокументТовары.ЗаказДавальца.Грузополучатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|				ТОГДА ДокументТовары.ЗаказДавальца.Контрагент
	|			ИНАЧЕ ДокументТовары.ЗаказДавальца.Грузополучатель
	|		КОНЕЦ,
	|		ДокументТовары.ЗаказДавальца.Организация,
	|		ДокументТовары.ЗаказДавальца.Контрагент,
	|		ВЫБОР
	|			КОГДА ДокументТовары.ЗаказДавальца.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчика)
	|				ТОГДА ДокументТовары.ЗаказДавальца.ПеревозчикПартнер
	|			ИНАЧЕ НЕОПРЕДЕЛЕНО
...
//++ НЕ УТ
	...
	|		ДокументТовары.ЗаказПереработчику.АдресДоставки,
	|		ВЫБОР
	|			КОГДА ДокументТовары.ЗаказПереработчику.Грузополучатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|				ТОГДА ДокументТовары.ЗаказПереработчику.Контрагент
	|			ИНАЧЕ ДокументТовары.ЗаказПереработчику.Грузополучатель
	|		КОНЕЦ,
	|		ДокументТовары.ЗаказПереработчику.Организация,
	|		ДокументТовары.ЗаказПереработчику.Контрагент,
	|		ВЫБОР
	|			КОГДА ДокументТовары.ЗаказПереработчику.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.СиламиПеревозчика)
	|				ТОГДА ДокументТовары.ЗаказПереработчику.ПеревозчикПартнер
	|			ИНАЧЕ НЕОПРЕДЕЛЕНО
...
Функция ПроверитьЗаданияНаПеревозкуДляОформленияТранспортныхНакладных(МассивЗаданийНаПеревозку, ВыделенныеСтрокиАдресов = Неопределено) Экспорт
	...
	ШаблонОшибкиТНСозданы		   = НСтр("ru='Для документа ""%Документ%"" не требуется оформлять транспортные накладные, т.к. они уже оформлены или в задании на перевозку нет ни одного маршрута.';uk='Для документа ""%Документ%"" не потрібно оформляти транспортні накладні, т. к. вони вже оформлені або у завданні на перевезення немає жодного маршруту.'");
	ШаблонОшибкиТНСозданыПоАдресам = НСтр("ru='По выделенным адресам не требуется оформлять транспортные накладные, т.к. они уже оформлены.';uk='За виділеними адресами не потрібно оформляти транспортні накладні, т. к. вони вже оформлені.'");
...
//++ НЕ УТ
	...
	|		ПО ДокументРаспоряжение.Ссылка = ДокументРаспоряжениеТовары.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеЗаданияНаПеревозку КАК ДанныеЗаданияНаПеревозкуЗаказы
	|		ПО (ДокументРаспоряжениеТовары.ЗаказПереработчику = ДанныеЗаданияНаПеревозкуЗаказы.Распоряжение)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДокументыФизическихЛиц.СрезПоследних КАК ДокументыФизическихЛиц
	|		ПО (ДанныеЗаданияНаПеревозкуЗаказы.Водитель = ДокументыФизическихЛиц.Физлицо)
	|			И (ДокументыФизическихЛиц.ВидДокумента = ЗНАЧЕНИЕ(Справочник.ВидыДокументовФизическихЛиц.ВодительскоеУдостоверение))
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады.КонтактнаяИнформация КАК СкладыКонтактнаяОтправитель
...
//++ НЕ УТ
	...
	|						И ВЫБОР
	|							КОГДА ТаблицаТоваров.ЗаказПереработчику.Грузополучатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|								ТОГДА ТаблицаТоваров.ЗаказПереработчику.Контрагент
	|							ИНАЧЕ ТаблицаТоваров.ЗаказПереработчику.Грузополучатель
	|						КОНЕЦ = &Грузополучатель
...
Процедура ЗаполнитьПлательщиков(Параметры) Экспорт 
	...
	//++ НЕ УТКА
 	...
 	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ТранспортнаяНакладная.ДокументыОснования КАК ДокументыОснования
 	|			ПО (ДокументыОснования.Ссылка = СсылкиДляОбработки.Ссылка)
 	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТЗаблокированыПередачаДавальцу КАК Документ
 	|			ПО (ДокументыОснования.ДокументОснование = Документ.Ссылка)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
 	|	
 	...
 	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ТранспортнаяНакладная.ДокументыОснования КАК ДокументыОснования
 	|			ПО (ДокументыОснования.Ссылка = СсылкиДляОбработки.Ссылка)
 	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТЗаблокированыВозвратСырьяДавальцу КАК Документ
 	|			ПО (ДокументыОснования.ДокументОснование = Документ.Ссылка)
 	|
	//-- НЕ УТКА
	//++ НЕ УТ
 	...
 	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ТранспортнаяНакладная.ДокументыОснования КАК ДокументыОснования
 	|			ПО (ДокументыОснования.Ссылка = СсылкиДляОбработки.Ссылка)
 	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТЗаблокированыПередачаСырьяПереработчику КАК Документ
 	|			ПО (ДокументыОснования.ДокументОснование = Документ.Ссылка)
 	|	
	//-- НЕ УТ
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	//++ НЕ УТКА
 	|				ИЛИ ТранспортнаяНакладнаяДокументыОснования.ДокументОснование ССЫЛКА Документ.ПередачаДавальцу
 	|				ИЛИ ТранспортнаяНакладнаяДокументыОснования.ДокументОснование ССЫЛКА Документ.ВозвратСырьяДавальцу
	//-- НЕ УТКА
	//++ НЕ УТ
 	|				ИЛИ ТранспортнаяНакладнаяДокументыОснования.ДокументОснование ССЫЛКА Документ.ПередачаСырьяПереработчику
	//-- НЕ УТ
	...
	//++ НЕ УТКА
	ТаблицыДляЧтения.Добавить("Документ.ПередачаДавальцу", 				"ВТЗаблокированыПередачаДавальцу");
	ТаблицыДляЧтения.Добавить("Документ.ВозвратСырьяДавальцу", 			"ВТЗаблокированыВозвратСырьяДавальцу");
	//-- НЕ УТКА
	//++ НЕ УТ
	ТаблицыДляЧтения.Добавить("Документ.ПередачаСырьяПереработчику", 	"ВТЗаблокированыПередачаСырьяПереработчику");
	//-- НЕ УТ
