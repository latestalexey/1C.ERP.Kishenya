...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ЗначениеЗаполнено(ИсправленныйДокумент) Тогда
		СотрудникиДляРегистрацииСторноЗаписей = Новый Массив;
		
		Для Каждого КлючЗначение Из СотрудникиСИсправленнымиДанными Цикл
			СотрудникиДляРегистрацииСторноЗаписей.Добавить(КлючЗначение.Ключ);
		КонецЦикла;	
			
		УчетРабочегоВремени.ЗарегистрироватьСторноЗаписиПоДокументу(Движения, ПериодРегистрации, ИсправленныйДокумент, СотрудникиДляРегистрацииСторноЗаписей);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого ОшибкиПоСотруднику Из Ошибки Цикл
		Если ОшибкиПоСотруднику.Значение.Количество() > ДопустимоеКоличествоНесоответствийПланФакт Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='По сотруднику %1 больше 5-ти несоответствий плановых видов времени фактическим.
                                                                                        ...
                                                                                        |;uk='По співробітнику %1 більше 5-ти невідповідностей планових видів часу фактичним.
                                                                                        |Рекомендується ввести індивідуальний графік.'"),
	...
	УчетРабочегоВремени.ПроверитьЗаполненияЧасов(МенеджерВТ, ОписанияОшибокВводаВремени, Отказ);
