...
#Область ТекстВозвратКомиссионногоТовара // (Дт  :: Кт 0241)
...
#Область ТекстВозвратСебестоимости // (Дт 63 :: Кт 28) 
	...
	|	КОНЕЦ КАК Сумма,
	|
	|	ВЫБОР КОГДА Операция.ВозвратПринятойМногооборотнойТары И Аналитика.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара) ТОГДА
...
#Область ТекстПереносОплатыНаАванс // (Дт 3771 :: Кт 631)
	...
	|ВЫБРАТЬ //// Перенос претензии на авансы (Дт 3771 :: Кт 631)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	ЕСТЬNULL(Расчеты.ГруппаФинансовогоУчета, 
	...
	|	ЕСТЬNULL(Расчеты.ЗаказПоставщику.НаправлениеДеятельности,
	|		ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) КАК НаправлениеДеятельностиДт,
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеДт,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДт,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Перенос оплаты на аванс"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстОтклонениеВСтоимостиРасходы // (Дт 9 :: Кт 28)
	...
	|	И НЕ (Операция.ВозвратПринятойМногооборотнойТары И Строки.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
...
#Область ТекстНДССоСтоимостиПриСторноУсловнойПродажиРасходы // (Дт 9 :: Кт 28)
	...
	|		ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаНДСРегл, ВидыЗапасовДокумента.СуммаНДС)
	|	ИНАЧЕ
	|		0
	|	КОНЕЦ КАК Сумма,
	|
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: из стоимости при сторно условной продажи (расходы)"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|		И СуммыДокументовВВалютеРегл.СуммаНДСРегл <> 0
	|
    |ГДЕ
...
#Область ТекстНДСНеподтвержденный // (Дт 63 :: Кт 6442)
	ТекстНДСНеподтвержденный = "
	...
	|	0 КАК СуммаВРКт,
    |
	|	""НДС: налоговый кредит неподтвержденный (возврат)"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	...
	|	И НЕ (Операция.ВозвратПринятойМногооборотнойТары И Аналитика.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	|
	|СГРУППИРОВАТЬ ПО
	|	Операция.Ссылка,
	|	ВЫБОР
	|		КОГДА Операция.ВозвратПринятойМногооборотнойТары
...
#КонецОбласти 
	...
	Возврат ТекстВозвратКомиссионногоТовара
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВозвратСебестоимости
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстПереносОплатыНаАванс
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстОтклонениеВСтоимостиРасходы
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстОтклонениеВСтоимостиДоходы
    + " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДСНеподтвержденный
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстСторноНДСОжидаемыйКомиссионный
    + " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДССоСтоимостиПриСторноУсловнойПродажиРасходы
	;
...
Функция ТекстЗапросаВТОтраженияВРеглУчетеУКР() Экспорт
	...
	|	Расчеты.Валюта,
	|	Расчеты.ЗаказПоставщику.ГруппаФинансовогоУчета
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
