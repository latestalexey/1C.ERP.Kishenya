...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипОснования = Тип("Структура") И ДанныеЗаполнения.Свойство("ПлатежиПоНалогам") Тогда
...
Процедура ЗаполнитьПоВедомостямНаВыплатуЗарплаты(Знач ДанныеКоманды, ДанныеЗаполнения, МассивВедомостей)
	...
	Если ВедомостиНаВыплатуЗарплаты.Количество() = 0 Тогда
		Текст = НСтр("ru='На основании выбранных ведомостей заявки уже созданы или оплата не требуется.';uk='На підставі обраних відомостей заявки вже створені або оплата не потрібна.'");
		ВызватьИсключение Текст;
	КонецЕсли;
	
	ДанныеЗаполнения.СуммаДокумента = ВедомостиНаВыплатуЗарплаты.Итог("Сумма");
...
Процедура ЗаполнитьРеквизитыЗначениямиПоУмолчанию()
	...
	Приоритет = Справочники.Приоритеты.ПолучитьПриоритетПоУмолчанию(Неопределено);
...
Функция УстановитьСтатус(НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеНовогоСтатуса = Перечисления.СтатусыЗаявокНаРасходованиеДенежныхСредств.Согласована Тогда
...
Процедура ПроверитьВедомостиНаВыплатуЗарплаты(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		ТекстОшибки = НСтр("ru='По документу %Ведомость% уже оформлена %Заявка%';uk='За документом %Ведомость% вже оформлена %Заявка%'");
