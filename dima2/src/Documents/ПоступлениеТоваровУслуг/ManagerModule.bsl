...
#Область ЗакупкаЗапасов // (Дт 20-28 :: Кт 63)
	ЗакупкаЗапасов = "
	...
	|
	// ДТ - Валюта, Подразделение, Счет, НалоговоеНазначение
	|	НЕОПРЕДЕЛЕНО			КАК ВалютаДт,
	|	Склады.Подразделение	КАК ПодразделениеДт,
...
#Область ЗакупкаУслугиНаВнеоборотныеАктивы // (Дт 15 :: Кт 63)
	...
	|ВЫБРАТЬ //// Получение услуг на прочие активы (Дт 15 :: Кт Кт 63)
	|
	|	Операция.Ссылка КАК Ссылка,
	// Период, Организация, ИдентификаторСтроки 
	|	Операция.Дата КАК Период,
...
#Область ЗакупкаНДСОжидаемыйКомиссионный // (Дт 6442 :: Кт 6441)
	ЗакупкаНДСОжидаемыйКомиссионный = "
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговый кредит: поступление"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#КонецОбласти
	...
	+                      ПриемНаКомиссию
	// Закупка
	+ " ОБЪЕДИНИТЬ ВСЕ " + ЗакупкаЗапасов
	+ " ОБЪЕДИНИТЬ ВСЕ " + ЗакупкаУслуги
	+ " ОБЪЕДИНИТЬ ВСЕ " + ЗакупкаУслугиНаВнеоборотныеАктивы
	// НДС по закупке
	+ " ОБЪЕДИНИТЬ ВСЕ " + ЗакупкаНДСОжидаемый
	+ " ОБЪЕДИНИТЬ ВСЕ " + ЗакупкаНДСОжидаемыйКомиссионный 
	+ "";
...
Функция ТекстЗапросаТаблицаПартииПрочихРасходов(Запрос, ТекстыЗапроса, Регистры)
    ...
    //++ НЕ УТ
	|		ИЛИ
	|		(ТаблицаТовары.СтатьяРасходов.ВариантРаспределенияРасходов = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
...
// в "Акты о расхождениях после поступления" при обновлении на версию BAS УТ 3.2.3 
Процедура ПеренестиРасхожденияВАкты(Параметры) Экспорт
	...
	ВыборкаПоступленияСРасхождениями = Результат[2].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	ВыборкаРеквизитыПечати           = Результат[3].Выбрать();
	
	//Здесь все документы, пришедшие из очереди
	ТаблицаСсылокКОбработке = Результат[4].Выгрузить();
	СоответствиеОбработанныхДокументов = Новый Соответствие;
	
	ВыборкаИспользованиеАктов = Результат[5].Выбрать();
...
Функция ТекстЗапросаПереносРасхожденийВАкты() 
	...
	|			ТОГДА ЕСТЬNULL(ПоступлениеТоваровУслугРасхождения.ЗаказПоставщику, ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка))
	...
	|	ПоступлениеТоваровУслугРасхождения.ВариантРасхождений,
	|	ПоступлениеТоваровУслугРасхождения.ВариантОтраженияРасхождений,
	|	ПоступлениеТоваровУслугРасхождения.СуммаВзаиморасчетов,
	|	ПоступлениеТоваровУслугРасхождения.СуммаНДСВзаиморасчетов,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг.УдалитьРасхождения КАК ПоступлениеТоваровУслугРасхождения
	|		ПО ТоварыПоДокументуИФакт.Ссылка = ПоступлениеТоваровУслугРасхождения.Ссылка
	|			И ТоварыПоДокументуИФакт.Номенклатура = ПоступлениеТоваровУслугРасхождения.Номенклатура
	|			И ТоварыПоДокументуИФакт.НоменклатураПоставщика = ПоступлениеТоваровУслугРасхождения.НоменклатураПоставщика
	|			И ТоварыПоДокументуИФакт.Характеристика = ПоступлениеТоваровУслугРасхождения.Характеристика
	|			И ТоварыПоДокументуИФакт.Упаковка = ПоступлениеТоваровУслугРасхождения.Упаковка
	|			И ТоварыПоДокументуИФакт.ВидЦеныПоставщика = ПоступлениеТоваровУслугРасхождения.ВидЦеныПоставщика
	|			И (ВЫБОР
	|				КОГДА ПоступлениеТоваровУслугРасхождения.ВариантРасхождений = ЗНАЧЕНИЕ(Перечисление.ВариантыРасхождений.Недостачи)
	|					ТОГДА ТоварыПоДокументуИФакт.КоличествоПоДокументу - ТоварыПоДокументуИФакт.Количество = ПоступлениеТоваровУслугРасхождения.Количество
	|				КОГДА ПоступлениеТоваровУслугРасхождения.ВариантРасхождений = ЗНАЧЕНИЕ(Перечисление.ВариантыРасхождений.Излишки)
	|					ТОГДА ТоварыПоДокументуИФакт.Количество - ТоварыПоДокументуИФакт.КоличествоПоДокументу = ПоступлениеТоваровУслугРасхождения.Количество
	|			КОНЕЦ)
	|
	|УПОРЯДОЧИТЬ ПО
