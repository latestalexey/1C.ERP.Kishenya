...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ЗаменаВФормеЗаказе И Модифицированность Тогда
		Отказ = Истина;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ШаблонСообщения = НСтр("ru='Не заполнена колонка ""Заменить на спецификацию"" в строке %1';uk='Не заповнена колонка ""Замінити на специфікацію"" в рядку %1'");
	НомерСтроки = 1;
...
Процедура ЗаполнитьПоОтбору()
	...
	Пока Выборка.Следующий() Цикл
		...
		НомерДокумента = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Выборка.Номер, Ложь, Истина);
...
Процедура ОбработатьПараметрыФормы()
	...
	Если Параметры.Свойство("Заказ") Тогда
		...
		Заголовок = НСтр("ru='Замена недействующих спецификаций в заказе';uk='Заміна недіючих специфікацій в замовленні'");
		
		Элементы.ФормаВыполнитьЗамену.Заголовок = НСтр("ru='Перенести в заказ';uk='Перенести в замовлення'");
		
		Элементы.СписокЗаказовПредставлениеСтроки.Заголовок = НСтр("ru='Номер строки';uk='Номер рядка'");
	...
	Иначе
		...
		Заголовок = НСтр("ru='Замена недействующих спецификаций в заказах';uk='Заміна недіючих специфікацій в замовленнях'");
		
		Элементы.СписокЗаказовПредставлениеСтроки.Заголовок = НСтр("ru='Заказ и номер строки';uk='Замовлення та номер рядка'");
	...
	Иначе
		...
		ИначеЕсли ВыполненоЗамен = ВсегоЗамен Тогда
			...
			ТекстЗавершеннойОперации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru='Выполнена замена в %1 из %2 заказах';uk='Виконана заміна у %1 з %2 замовленнях'"), 
		...
		Иначе
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                                            ...
                                            |Завершить замену?'
                                            |;uk='Заміна виконана не у всіх замовленнях (в %1 з %2).
                                            |Завершити заміну?'"), 
											Формат(ВыполненоЗамен, "ЧГ=0"), 
											Формат(ВсегоЗамен, "ЧГ=0"));
...
Процедура ВыполнитьЗаменуВЗаказахНаСервере()
	...
	Для каждого СтрокаЗаказ Из ТаблицаЗаказы Цикл
		...
		СтруктураПоиска = Новый Структура("Заказ,Заменить,БольшеНеИспользуется,ПроизводствоЗапущено", СтрокаЗаказ.Заказ, Истина, Ложь, Ложь);
  		СписокСтрок = СписокЗаказов.НайтиСтроки(СтруктураПоиска);
		ДанныеЗамены = Новый Массив;
