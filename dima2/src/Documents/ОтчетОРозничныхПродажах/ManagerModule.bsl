...
#Область ТекстВыручкаОтРеализацииСобственногоТовара // (Дт 36Р :: Кт 70)
	...
	|
	|	СУММА(ЕСТЬNULL(Суммы.СуммаБезНДСРегл + Суммы.СуммаНДСРегл, Строки.СуммаСНДС)) КАК Сумма,
	|
...
#Область ТекстВыручкаОтРеализацииУслуг // (Дт 36Р :: Кт 70)
	...
	|   НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	СУММА(ЕСТЬNULL(Суммы.СуммаБезНДСРегл + Суммы.СуммаНДСРегл, 
	|		Строки.Сумма +
...
#Область ТекстВыручкаОтРеализацииСобственногоТовараАкциз // (Дт 36Р :: Кт 70)
	ТекстВыручкаОтРеализацииСобственногоТовараАкциз = "
	|ВЫБРАТЬ //// Выручка от реализации собственного товара акциз (Дт 36Р :: Кт 70)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ВыручкаОтПродаж) КАК ВидСчетаКт,
	|	Аналитика.Номенклатура.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчетаКт,
	|	Операция.Склад КАК МестоУчетаКт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	|	Операция.Подразделение КАК ПодразделениеКт,
	...
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетКт,
	|	Аналитика.Номенклатура.ГруппаФинансовогоУчета КАК СубконтоКт1,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|
	|	0 КАК ВалютнаяСуммаКт,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Выручка от реализации собственного товара (акциз)"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстНДССРеализацииТоваров // (Дт 70 :: Кт 643)
	ТекстНДССРеализацииТоваров = "
	|ВЫБРАТЬ  //// НДС с релизации товаров (Дт 70 :: Кт 643)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: обязательства неподтвержденные, товары"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстНДССРеализацииУслуг // (Дт 70 :: Кт 643)
	ТекстНДССРеализацииУслуг = "
	|ВЫБРАТЬ  //// НДС с релизации услуг (Дт 70 :: Кт 643)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: обязательства неподтвержденные, услуги"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстАкцизСРеализацииТоваров // (Дт 70 :: Кт 6416)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Акцизный налог розничной выручки"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстСебестоимостьРеализованногоТовара // (Дт 90 :: Кт 28)
...
#Область ТекстСебестоимостьРеализованныхРабот // (Дт 90 :: Кт 23)
...
#Область ТекстВыручкаОтРеализацииКомиссионногоТовара // (Дт 36Р :: Кт 70)
...
#Область ТекстПродажаКомиссионногоТовара // (Дт  :: Кт 024)
...
#Область ТекстОплатаПлатежнойКартой // (Дт 333 :: Кт 36Р)
	...
	|   НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	СУММА(ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаБезНДСРегл, Строки.Сумма)) КАК Сумма,
	|
	|	НЕОПРЕДЕЛЕНО КАК ВидСчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК ГруппаФинансовогоУчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	...
	|	Операция.Организация,
	|	Операция.Валюта,
	|	Строки.ЭквайринговыйТерминал.БанковскийСчет.Подразделение,
	|	Строки.ЭквайринговыйТерминал.Эквайер,
	|	Строки.Сумма,
	|	Операция.Валюта,
	|	Операция.Склад.Подразделение,
	|	Операция.Склад
...
#Область ТекстПоступлениеОплатыВКассу // (Дт 30 :: Кт 36P)
	...
	|
	|ГДЕ
	|	НЕ (ЕСТЬNULL(ОплатаКартой.Сумма, 0) = 0 И ЕСТЬNULL(ОплатаСертификатом.Сумма, 0) = 0)
	|
	|СГРУППИРОВАТЬ ПО
	|	Операция.Ссылка,
	|	Операция.Организация,
...
#Область ТекстОплатаПодарочнымСертификатом // (Дт 36, 37 :: Кт 36Р)
	...
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	СУММА(ЕСТЬNULL(СуммыРегл.СуммаБезНДСРегл, Строки.Сумма)) КАК Сумма,
	|
	...
	|	Операция.КассаККМ.Подразделение,
	|	Операция.Валюта,
	|	Операция.Склад.Подразделение,
	|	Операция.Склад
	|";
...
#Область ТекстВычетИзДоходовОтВозвратаСобственногоТовара // (Дт 70 :: Кт 36Р)
	ТекстВычетИзДоходовОтВозвратаСобственногоТовара = "
	|ВЫБРАТЬ //// Вычет из доходов от продаж в рознице (возврат) (Дт 70 :: Кт 36Р)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	0 КАК СуммаВРКт,
	|
	|	""Вычет из доходов от продаж в рознице (возврат)"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстНДСВозвратТоваров // (Дт 70 :: Кт 643)
	ТекстНДСВозвратТоваров = "
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: обязательства неподтвержденные, товары (возврат)"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстВычетИзДоходовОтВозвратаСобственногоТовараАкциз // (Дт 70 :: Кт 6416)
	ТекстВычетИзДоходовОтВозвратаСобственногоТовараАкциз = "
	|ВЫБРАТЬ //// Вычет из доходов от продаж в рознице (возврат акциз) (Дт 70 :: Кт 6416)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ВычетыИзДоходов) КАК ВидСчетаДт,
	|	Аналитика.Номенклатура.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчетаДт,
	|	Операция.Склад КАК МестоУчетаДт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	|	Операция.Подразделение КАК ПодразделениеДт,
	...
	|	0 КАК СуммаВРКт,
	|
	|	""Акцизный налог розничной выручки (возврат)"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстВычетыИзДоходовОтВозвратаКомиссионногоТовара // (Дт 70 :: Кт 36)
	ТекстВычетыИзДоходовОтВозвратаКомиссионногоТовара = "
	...
	|	0 КАК СуммаВРКт,
	|
	|	""Вычет из доходов от реализации комиссионного товара в рознице (возврат)"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКорректировкиНДСПартийТоваров // (Дт 90 :: Кт 6435)
	ТекстКорректировкиНДСПартийТоваров = "
	|ВЫБРАТЬ   //// Себестоимость реализованного товара (Дт 90 :: Кт 6435)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.СебестоимостьПродаж) КАК ВидСчетаДт,
	|	КорректировкиНДСПартий.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчетаДт,
	|	Операция.Склад КАК МестоУчетаДт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	|	Операция.Подразделение КАК ПодразделениеДт,
	...
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДт,
	|	КорректировкиНДСПартий.ГруппаФинансовогоУчета КАК СубконтоДт1,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	0 КАК ВалютнаяСуммаДт,
	...
	|	И КорректировкиНДСПартий.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|";
#КонецОбласти 

#Область ТекстКорректировкиНДСПартийРабот // (Дт 90 :: Кт 6435)
	ТекстКорректировкиНДСПартийРабот = "
	|ВЫБРАТЬ //// Себестоимость реализованных работ (Дт 90 :: Кт 6435)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|		(Операция.Ссылка = КорректировкиНДСПартий.Ссылка)
	|ГДЕ
	|	КорректировкиНДСПартий.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|";
#КонецОбласти

#Область ТекстКорректировкиНДСПартийКомиссионногоТовара // (Дт 6441 :: Кт 6435)
	ТекстКорректировкиНДСПартийКомиссионногоТовара = "
	|ВЫБРАТЬ //// Условная продажа  (Дт 6441 :: Кт 6435)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	...
	|
	|	КорректировкиНДСПартий.Контрагент КАК СубконтоДт1,
	|	КорректировкиНДСПартий.Договор КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
...
#КонецОбласти 
	...
	Возврат ТекстВыручкаОтРеализацииСобственногоТовара
		...
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВыручкаОтРеализацииСобственногоТовараАкциз
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДССРеализацииТоваров
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДССРеализацииУслуг
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстАкцизСРеализацииТоваров
		...
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВычетИзДоходовОтВозвратаСобственногоТовара
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДСВозвратТоваров
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВычетИзДоходовОтВозвратаСобственногоТовараАкциз
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВычетыИзДоходовОтВозвратаКомиссионногоТовара
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКорректировкиНДСПартийТоваров
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКорректировкиНДСПартийРабот
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКорректировкиНДСПартийКомиссионногоТовара
		;
...
Функция ТекстЗапросаВТОтраженияВРеглУчетеУКР() Экспорт
	...
	|			ПО ДокументыКОтражению.Ссылка = ОплатаПлатежнымиКартами.Ссылка
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|	ИЗ
	|		ДокументыКОтражению КАК ДокументыКОтражению
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах.ПодарочныеСертификаты КАК ПодарочныеСертификаты
	|			ПО ДокументыКОтражению.Ссылка = ПодарочныеСертификаты.Ссылка
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ПодарочныеСертификаты.Ссылка
	|	
	|	ИМЕЮЩИЕ
	|		СУММА(ПодарочныеСертификаты.Сумма) <> 0) КАК ОплатаНеВКассу
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах.ОплатаПлатежнымиКартами КАК ОплатаКартой
	|		ПО ДокументыКОтражению.Ссылка = ОплатаКартой.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютеРегл КАК СуммыДок
	|		ПО (ОплатаКартой.Ссылка = СуммыДок.Регистратор)
	...
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах.ПодарочныеСертификаты КАК ОплатаСертификатом
	|		ПО ДокументыКОтражению.Ссылка = ОплатаСертификатом.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютеРегл КАК СуммыДок
	|		ПО (ОплатаСертификатом.Ссылка = СуммыДок.Регистратор)
	...
	|				ТОГДА ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаБезНДСРегл + СуммыДокументовВВалютеРегл.СуммаНДСРегл, ТаблицаТовары.Сумма + ТаблицаТовары.СуммаНДС)
	|			ИНАЧЕ ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаБезНДСРегл, ТаблицаТовары.Сумма)
	|		КОНЕЦ) КАК Сумма
	|ПОМЕСТИТЬ ТаблицаТовары
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах.Товары КАК ТаблицаТовары
	|		ПО ДокументыКОтражению.Ссылка = ТаблицаТовары.Ссылка
	...
	|ГДЕ
	|	НЕ ТаблицаТовары.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	|
	|СГРУППИРОВАТЬ ПО
	...
	|		ПО ДокументыКОтражению.Ссылка = ТаблицаВидыЗапасов.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютеРегл КАК СуммыДокументовВВалютеРегл
	...
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах.ВидыЗапасовПоВозвратамЗакрытойСмены КАК ТаблицаВидыЗапасов
	|		ПО ДокументыКОтражению.Ссылка = ТаблицаВидыЗапасов.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютеРегл КАК СуммыДокументовВВалютеРегл
	|		ПО (ТаблицаВидыЗапасов.Ссылка = СуммыДокументовВВалютеРегл.Регистратор)
	|			И (ТаблицаВидыЗапасов.ИдентификаторСтроки = СуммыДокументовВВалютеРегл.ИдентификаторСтроки)
