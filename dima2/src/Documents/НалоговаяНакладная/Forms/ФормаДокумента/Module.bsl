...
//++ НЕ УТ 			
		 ИЛИ Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийНалоговыхДокументов.УсловнаяПродажаНеоборотныхАктивов")
//-- НЕ УТ 
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.СтатусАвтокорректировки = ПредопределенноеЗначение("Перечисление.СтатусыАвтокорректировкиНалоговыхДокументов.ИсправлятьТекущийДокумент")
	   И (Объект.СтатусРегистрацииВЕРНН = ПредопределенноеЗначение("Перечисление.СтатусыРегистрацииВЕРНННалоговыхДокументов.НаправленНаРегистрациюВЕРНН") ИЛИ
		...
		ТекстВопроса = НСтр("ru='При регистрации документа в ЕРНН рекомендуется использовать режим
                                |автокорректировки: Формировать ""Приложение 2 к налоговой накладной"". 
                                |
                                |Все равно продолжить?'
                                |;uk='При реєстрації документа в ЄРПН рекомендується використовувати режим
                                |автокоригування: Формувати ""Додаток 2 до податкової накладної"". 
                                |
                                |Все одно продовжити?'");
...
&НаКлиенте
Процедура ТоварыПоДаннымПользователяПриИзменении(Элемент)
	
	// Дублируем проверку вида операции для уменьшения нагрузки на сервер
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийНалоговыхДокументов.ПродажаНижеОбычнойЦены") Тогда
		ТоварыПоДаннымПользователяПриИзмененииСервер();
	КонецЕсли;
...
//++ НЕ УТ 
	 ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.УсловнаяПродажаНеоборотныхАктивов
//-- НЕ УТ 
...
//++ НЕ УТ
		  Объект.ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.УсловнаяПродажаНеоборотныхАктивов Или
//-- НЕ УТ		 
	...
	Для каждого ТекущаяСтрока из Объект.ТоварыПоДаннымПользователя Цикл
		...
		Если ТекущаяСтрока.СтавкаНДС <> СтараяСтавкаНДС Тогда
			...
			НДСИсходящийСервер.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения, Истина);
...
&НаСервере
Процедура ТоварыПоДаннымПользователяПриИзмененииСервер()
	
	// Дублируем проверку вида операции для уменьшения нагрузки на сервер
	Если Объект.ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.ПродажаНижеОбычнойЦены Тогда
		Документы.НалоговаяНакладная.УстановитьТипПричиныНевыдачиПокупателюПоУмолчанию(Объект);
		НастроитьЭлементыФормы();
	КонецЕсли;
...
Функция ПересчитатьСуммуПревышения(ТекущаяСтрока)
	...
	ТекущаяСтрока.СуммаБезНДСРегл = ПересчитатьВВалютуРегл(ТекущаяСтрока.СуммаПревышения);
...
Функция ПересчитатьСуммуНДСПревышения(ТекущаяСтрока)
	...
	ТекущаяСтрока.СуммаНДСРегл = ПересчитатьВВалютуРегл(ТекущаяСтрока.СуммаНДСПревышения);
...
Процедура ЗаполнитьТЧПоДаннымПользователяИзТоварыСервер()
	...
	Для каждого СтрокаТовары из Объект.Товары Цикл
		...
		Если ТекущаяСтрока.СуммаБезНДСРегл = 0 ИЛИ ТекущаяСтрока.СуммаНДСРегл = 0 Тогда
...
//++ НЕ УТ
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.УсловнаяПродажаНеоборотныхАктивов Тогда
		ОписаниеТипа = Новый ОписаниеТипов("ДокументСсылка.КорректировкаНалоговогоНазначенияОС");
		ЗаголовокВходящийНалоговый = НСтр("ru='Объект для заполнения содержания';uk='Об''єкт для заповнення змісту'");
		ОтметкаНезаполненогоОбъектЗаполненияСодержанияУсловнаяПродажа = Истина;
//-- НЕ УТ
	Иначе
...
// ГруппаСтраницыИтого
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Сводная", "Доступность", НЕ (ЭтоУсловнаяПродажа ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийНалоговыхДокументов.ПродажаНижеОбычнойЦены));
...
Процедура ЗаполнитьСтатьюДекларацииНДСНалоговыеОбязательстваСервер(СтатьяДекларацииНДСНалоговыеОбязательства, СтавкаНДС)
	...
	Документы.НалоговаяНакладная.ЗаполнитьСтатьюДекларацииНДСНалоговыеОбязательства(Объект.ВидОперации, Объект.Контрагент, Объект.Договор,  СтатьяДекларацииНДСНалоговыеОбязательства, СтавкаНДС)
...
Функция ПересчитатьВВалютуРегл(Цена)
	...
	СтруктураКурсовВалюты = РаботаСКурсамиВалют.ПолучитьКурсВалюты(Объект.Валюта, ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, ТекущаяДата()));
...
Процедура ЗаполнитьПоПревышениямОбычнойЦеныЗаПериодСервер(СтандартныйПериод)
	...
	|	И НалоговаяНакладнаяТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20)
    ...
    |	НалоговаяНакладная.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|	И НалоговаяНакладная.Проведен
	|	И НалоговаяНакладнаяТоварыПоДаннымПользователя.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20)
	|	И НалоговаяНакладная.СтатусАвтокорректировки = ЗНАЧЕНИЕ(Перечисление.СтатусыАвтокорректировкиНалоговыхДокументов.ИсправлятьТехническуюТЧ)
...
Процедура ПересчитатьСуммыСервер(ИмяТабЧасти, ПересчитатьВалютныеСуммы)
	...
	Для Каждого ТекущаяСтрока ИЗ Объект[ИмяТабЧасти] Цикл
		...
		Иначе
			...
			Иначе	
				ТекущаяСтрока.ЦенаРегл        = ПересчитатьВВалютуРегл(ТекущаяСтрока.Цена);		
				ТекущаяСтрока.СуммаБезНДСРегл = ТекущаяСтрока.ЦенаРегл * ТекущаяСтрока.КоличествоУпаковок;
				ТекущаяСтрока.СуммаНДСРегл	  = ТекущаяСтрока.СуммаБезНДСРегл * ПроцентНДС;
			КонецЕсли;
...
//++ НЕ УТ 
	СписокВидовОпераций.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийНалоговыхДокументов.УсловнаяПродажаНеоборотныхАктивов"));
//-- НЕ УТ 
