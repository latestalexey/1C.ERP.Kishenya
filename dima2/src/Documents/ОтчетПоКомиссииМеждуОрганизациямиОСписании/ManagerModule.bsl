...
#Область ТекстКомитентСписаниеКомиссионногоТовара // (Дт  :: Кт 0242)
	ТекстКомитентСписаниеКомиссионногоТовара = "
	|ВЫБРАТЬ //// Списание комиссионного товара (Дт  :: Кт 0242)
	|
...
#Область ТекстКомитентВзаиморасчетыПоКомиссионномуТовару // (Дт 36 :: Кт 70)
	...
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	|	Операция.Подразделение КАК ПодразделениеКт,
	|	ВидыЗапасовКомитента.Назначение.НаправлениеДеятельности КАК НаправлениеДеятельностиКт,
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеКт,
	|
	...
	|	КОНЕЦ,
	|	ВидыЗапасовКомитента.Назначение.НаправлениеДеятельности,
	|	Операция.Подразделение,
	|	ВЫБОР
	|		КОГДА &ФормироватьВидыЗапасовПоГруппамФинансовогоУчета
	|			ТОГДА Строки.ВидЗапасовКомитента.ГруппаФинансовогоУчета
...
#Область ТекстКомитентВыручкаОтПродажСобственногоТовара // (Дт 36 :: Кт 70)
	ТекстКомитентВыручкаОтПродажСобственногоТовара = "
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Выручка от продаж собственного товара"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКомитентСписаниеСебестоимостиТовара // (Дт 90 :: Кт 283)
	ТекстКомитентСписаниеСебестоимостиТовара = "
...
#Область ТекстКомитентОтраженияНДССписанногоТовара // (Дт 70 :: Кт 643)
	ТекстКомитентОтраженияНДССписанногоТовара = "
	...
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	|	Операция.Подразделение КАК ПодразделениеДт,
	|	Строки.ВидЗапасовКомитента.Назначение.НаправлениеДеятельности КАК НаправлениеДеятельностиДт,
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеДт,	
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДт,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт1,
	...
	|		Аналитика.Номенклатура.ГруппаФинансовогоУчета
	|	КОНЕЦ,
	|	Строки.ВидЗапасовКомитента.Назначение.НаправлениеДеятельности,
	|	Операция.Дата,
	|	Операция.Организация,
...
#Область ТекстКомиссионерЗадолженностьПередКомитентом // (Дт 68ОК :: Кт 63)
	...
	|	НЕОПРЕДЕЛЕНО КАК ВалютаДт,
	|	Операция.Подразделение КАК ПодразделениеДт,
	|	ВидыЗапасовКомитента.Назначение.НаправлениеДеятельности КАК НаправлениеДеятельностиДт,
	|
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеДт,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТоварыКОформлениюОтчетовКомитентуОСписании) КАК СчетДт,
...
#Область ТекстКомиссионерОтраженияНДССписанногоТовара // (Дт 644 :: Кт 68ОК, 6435)
	ТекстКомиссионерОтраженияНДССписанногоТовара = "
	...
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	|	Операция.Подразделение КАК ПодразделениеКт,
	|	Строки.ВидЗапасовКомитента.Назначение.НаправлениеДеятельности КАК НаправлениеДеятельностиКт,
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеКт,	
	|
	|	ВЫБОР
	...
	|		КОГДА НЕ Строки.ВидЗапасов.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая)
	|				И НЕ Строки.НалоговоеНазначениеСписания.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая)
	|			ТОГДА ""НДС: налоговый кредит: отгрузка""
	|		КОГДА НЕ Строки.ВидЗапасов.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая)
	|				И Строки.НалоговоеНазначениеСписания.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая)
	|			ТОГДА ""Условная продажа""
...
#КонецОбласти
	...
	Возврат ТекстКомитентСписаниеКомиссионногоТовара 	
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКомитентВзаиморасчетыПоКомиссионномуТовару 
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКомитентВыручкаОтПродажСобственногоТовара
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКомитентОтраженияНДССписанногоТовара
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКомитентСписаниеСебестоимостиТовара
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКомиссионерЗадолженностьПередКомитентом
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКомиссионерОтраженияНДССписанногоТовара
	;
