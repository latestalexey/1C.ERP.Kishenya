...
Функция ТекстОтраженияВРеглУчетеУКР() Экспорт
	...
	#Область ТекстПродажаКомиссионногоТовара // (Дт  :: Кт 0242)
...
#Область ТекстОтражениеВзаиморасчетовПоКомиссионномуТовару // (Дт 36 :: Кт 70)
...
#Область ТекстСебестоимостьРеализованногоТовара // (Дт 90 :: Кт 283)
...
#Область ТекстЗатратыНаКомиссионноеВознаграждение // (Дт 9X :: Кт 377, 685)
...
#Область ТекстНДСНаКомиссионноеВознаграждение // (Дт 644 :: Кт 377, 685)
...
#Область ТекстУдержатьВознаграждениеАванс // (Дт 377, 685 Кт 371)
...
#Область ТекстУдержатьВознаграждениеОплатаДолга // (Дт 377, 685 Кт 36)
...
#Область ТекстКорректировкаНДСПроданногоТовара // Дт 6431 :: Кт 6432)
	ТекстКорректировкаНДСПроданногоТовара = "
...
#КонецОбласти 
	...
	Возврат ТекстПродажаКомиссионногоТовара 
		...
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКорректировкаНДСПроданногоТовара;
		;
КонецФункции
...
Функция ТекстЗапросаТаблицаРасчетыСКлиентами(Запрос, ТекстыЗапроса, Регистры)
	...
	|		0
	|	КОНЕЦ КАК КОплате,
	|	ВЫРАЗИТЬ((ВЫБОР КОГДА ЭтапыГрафикаОплаты.СуммаПлатежа < 0 ТОГДА
	|						-ЭтапыГрафикаОплаты.СуммаПлатежа
	|				   ИНАЧЕ
	|						ЭтапыГрафикаОплаты.СуммаПлатежа
	|			КОНЕЦ) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15, 2)) КАК СуммаРегл,
	|	ВЫРАЗИТЬ((ВЫБОР КОГДА ЭтапыГрафикаОплаты.СуммаПлатежа < 0 ТОГДА
	|						-ЭтапыГрафикаОплаты.СуммаПлатежа
	|				   ИНАЧЕ
	|						ЭтапыГрафикаОплаты.СуммаПлатежа
	|			КОНЕЦ) * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(15, 2)) КАК СуммаУпр
	|ИЗ
	|	Документ.ОтчетКомиссионера.ЭтапыГрафикаОплаты КАК ЭтапыГрафикаОплаты
	|ГДЕ
	|	ЭтапыГрафикаОплаты.Ссылка = &Ссылка
