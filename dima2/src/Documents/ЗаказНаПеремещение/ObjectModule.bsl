...
Процедура ЗаполнитьПоЗаказуКлиента(ЗаказКлиента)
	...
	|	ЗаказКлиента.Организация     КАК Организация,
	|	ЗаказКлиента.Сделка          КАК Сделка,
	|	ЕСТЬNULL(ЗаказКлиента.Склад.ЭтоГруппа, ЛОЖЬ) КАК СкладГруппа,
	|	ЗаказКлиента.Подразделение   КАК Подразделение,
	|	ВЫБОР
	...
	Если ТаблицаТовары.Количество() > 0 Тогда
		...
		Если Реквизиты.СкладГруппа Тогда

			ВГраница = ТаблицаТовары.Количество() - 1;

			ПервыйСклад = ТаблицаТовары[0].Склад;
			Для Индекс = 0 По ВГраница Цикл
				Если ТаблицаТовары[ВГраница - Индекс].Склад <> ПервыйСклад Тогда
					ТаблицаТовары.Удалить(ВГраница - Индекс);
				КонецЕсли;
			КонецЦикла;
...
Процедура ЗаполнитьПоЗаказуМатериаловВПроизводство(ЗаказМатериалов)
	...
	|	ТаблицаДокумента.Склад           КАК СкладДокумента,
	|	ТаблицаДокумента.Подразделение   КАК Подразделение,
	|	ЛОЖЬ КАК ЕстьОшибкиСтатус
	|ИЗ
	|	Документ.ЗаказМатериаловВПроизводство КАК ТаблицаДокумента
	|ГДЕ
	|	ТаблицаДокумента.Ссылка = &ЗаказМатериалов");
	...
	ПолучатьУслуги = Ложь;
	ПолучатьРаботы = Ложь;
	ТаблицаТовары = ЗапасыСервер.ТаблицаОстатковКЗаказу(ЗаказМатериалов, , ПолучатьУслуги, ПолучатьРаботы);
