...
Процедура ПрочитатьДанные()
	...
	Если ЗначениеЗаполнено(СведенияОСемье.СоставСемьиПолный) Тогда 
		ЭтаФорма.СоставСемьиСправочно = НСтр("ru='Состав семьи по карточке Т-2';uk='Склад сім''ї по картці Т-2'") + ": " + Символы.ПС + СведенияОСемье.СоставСемьиПолный;
	КонецЕсли;
...
Процедура УстановитьИнфонадписьИсправления(Форма)
	...
	Иначе						   
		ТекстПодсказки = НСтр("ru='Данные о гражданине заполняются по личной карточке автоматически.';uk='Дані про громадянина заповнюються по особистій картці автоматично.'");
		ЗарплатаКадрыКлиентСервер.УстановитьРасширеннуюПодсказкуЭлементуФормы(Форма, "ОтменитьВсеИсправления", ТекстПодсказки);
