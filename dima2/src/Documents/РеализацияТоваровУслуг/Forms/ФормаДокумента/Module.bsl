...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.РеализацияПоЗаказам И НЕ РеализацияСверхЗаказа 
		...
		Если ИспользоватьРеализациюПоНесколькимЗаказам Тогда
			ТекстСообщения = НСтр("ru='Уточните склад реализации и заполните товары с помощью команды ""Заполнить по заказам""';uk='Уточніть склад реалізації і заповніть товари з допомогою команди ""Заповнити по замовленням""'");
		Иначе
			ТекстСообщения = НСтр("ru='Уточните склад реализации и заполните товары с помощью команды ""Заполнить по заказу""';uk='Уточніть склад реалізації і заповніть товари з допомогою команди ""Заповнити по замовленню""'");
		КонецЕсли;
	...
	Если Объект.РеализацияПоЗаказам Тогда
		...
		Если ИспользоватьРеализациюПоНесколькимЗаказам Тогда
			...
			Если Объект.Товары.Количество() = 0 Тогда
				ТекстВопроса = НСтр("ru='Список Товары будет перезаполнен остатками неоформленных товаров по заказам. Продолжить?';uk='Список Товари буде перезаполнен залишками неоформлених товарів по замовленнях. Продовжити?'");
			Иначе
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		СписокСвойств = "НоменклатураНабора, ХарактеристикаНабора, Номенклатура, Характеристика, Упаковка, ВидЦены, Цена, КоличествоУпаковок, СрокПоставки, ПроцентРучнойСкидки";
