...
#Область ТекстПередачаНаСубкомиссию // (Дт 0242 :: Кт 0241)
	...
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеДт,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТоварыПринятыеНаКомиссиюНаСубкомиссии) КАК СчетДт,	
	|
	|	Операция.Контрагент КАК СубконтоДт1,
	|	Строки.Номенклатура КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
...
#Область ТекстВыручкаОтРеализацииКомисионногоТовара // (Дт 36 :: Кт 70)
	...
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	СУММА(Строки.СуммаВзаиморасчетов) КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	0 КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
...
#Область ТекстВыручкаОтРеализацииСобственногоТовара // (Дт 36 :: Кт 70)
...
#Область ТекстВыручкаОтРеализацииРаботУслуг // (Дт 36 :: Кт 70)
...
#Область ТекстНДССРеализации // (Дт 70 :: Кт 643)
	ТекстНДССРеализации = "
...
#Область ТекстНДССРеализацииРаботыУслуги // (Дт 70 :: Кт 643)
	ТекстНДССРеализацииРаботыУслуги = "
	|ВЫБРАТЬ //// Выручка от реализации работ и услуг (Дт 70 :: Кт 643)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
...
#Область ТекстЗалоговаяСтоимостьПереданнойТары // (Дт 68 :: Кт 284)
	ТекстЗалоговаяСтоимостьПереданнойТары = "
...
#Область ТекстОтклонениеВСтоимостиРасходы // (Дт 9X :: Кт 284)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Отклонение в стоимости возвращенных товаров на расходы"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстОтклонениеВСтоимостиДоходы // (Дт 284 :: Кт 73)
	ТекстОтклонениеВСтоимостиДоходы = "
	|ВЫБРАТЬ //// Отклонение в стоимости возвращенных товаров (Дт 284 :: Кт 73)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Отклонение в стоимости возвращенных товаров в доходы"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
#Область ТекстНДСПередачаНаКомиссию // (Дт 6431 :: Кт 6432)
	ТекстНДСПередачаНаКомиссию = "
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговые обязательства: отгрузка"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстКорректировкаНДСПартийРеализацияТоваров // (Дт 90 :: Кт 6435)
	ТекстКорректировкаНДСПартийРеализацияТоваров = "
	...
	|	КорректировкиНДСПартий.Склад КАК МестоУчетаДт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	|	Операция.Подразделение КАК ПодразделениеДт,
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) КАК НаправлениеДеятельностиДт,
...
#Область ТекстКорректировкаНДСПартийРеализацияРабот // (Дт 90 :: Кт 23)
	ТекстКорректировкаНДСПартийРеализацияРабот = "
	|ВЫБРАТЬ ////  Условная продажа при использовании облагаемого товара в необлагаемых операциях (Дт 90 :: Кт 23)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
...
#Область ТекстКорректировкаНДСПартийПередачаНаКомиссию // (Дт 283 :: Кт 6435)
	ТекстКорректировкаНДСПартийПередачаНаКомиссию = "
	|ВЫБРАТЬ //// Условная продажа при передаче на комиссию (Дт 283 :: Кт 6435)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
...
#Область ТекстКорректировкаНДСПартийРеализацияИПередачаНаСубКомиссиюКомиссионногоТовара // (Дт 6441 :: Кт 6435)
	ТекстКорректировкаНДСПартийРеализацияИПередачаНаСубКомиссиюКомиссионногоТовара = "
...
#КонецОбласти 
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДССРеализацииРаботыУслуги
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстЗалоговаяСтоимостьПереданнойТары
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстОтклонениеВСтоимостиРасходы
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстОтклонениеВСтоимостиДоходы
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДСПередачаНаКомиссию
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКорректировкаНДСПартийРеализацияТоваров
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКорректировкаНДСПартийРеализацияРабот
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКорректировкаНДСПартийПередачаНаКомиссию
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстКорректировкаНДСПартийРеализацияИПередачаНаСубКомиссиюКомиссионногоТовара
	;
...
Функция ПечатьДокументаДляЭДО(ДокументСсылка) Экспорт
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		Если    ВыборкаТовары.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга
		    ...
		    СтрокаДок.ТипНоменклатуры = "По";
		Иначе	
			СтрокаДок.ТипНоменклатуры = "То";
			ТолькоУслуги = Ложь;
		КонецЕсли;
