...
Функция СформироватьПечатнуюФормуКоммерческогоПредложения(МассивОбъектов, ОбъектыПечати, КомплектыПечати, ПараметрыПечати = Неопределено, ПараметрыВывода)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		ЗаполнитьТабличныйДокументПоДаннымОбъекта(ТабличныйДокумент, ПолучитьДанныеОбъектаПоВыборке(ДанныеПечати, Товары, ПараметрыВывода), ПараметрыВывода);
...
Функция ПолучитьДанныеОбъектаПоВыборке(Выборка, Товары, ПараметрыВывода = Неопределено)
	...
	Для Каждого ВыборкаТовары из ТаблицаТовары Цикл
		...
		НаименованиеТовара = ПрефиксИПостфикс.Префикс + НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(
			...
			,
			,
			ВыборкаТовары.ЭтоВозвратнаяТара) + ПрефиксИПостфикс.Постфикс;
