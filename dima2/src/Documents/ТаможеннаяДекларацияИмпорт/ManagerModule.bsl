...
#Область ТекстТаможенныйСборНаРасходы // (Дт 9X :: Кт 63)
	ТекстТаможенныйСборНаРасходы = "
	|ВЫБРАТЬ // Таможенный сбор на расходы (Дт 9X :: Кт 63)
	|
...
#Область ТекстТаможенныйСборНаСебестоимость // (Дт 28 :: Кт 63)
	ТекстТаможенныйСборНаСебестоимость = "
	...
	|	ЕСТЬNULL(ВидыЗапасов.Назначение.НаправлениеДеятельности,
	|		ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) КАК НаправлениеДеятельностиДт,
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеДт,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДт,
...
#Область ТекстРасчетыПоОбязательнымПлатежамНаСебестоимость // (Дт 28 :: Кт 642)
	ТекстРасчетыПоОбязательнымПлатежамНаСебестоимость = "
	|ВЫБРАТЬ // Таможенная пошлина на себестоимость (Дт 28 :: Кт 642)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
#Область ТекстРасчетыПоОбязательнымПлатежамНаРасходы // (Дт 9X :: Кт 642)
	ТекстРасчетыПоОбязательнымПлатежамНаРасходы = "
	...
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	СУММА(Строки.СуммаПошлины) КАК Сумма,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.Расходы) КАК ВидСчетаДт,
	|	Операция.СтатьяРасходовСбор КАК АналитикаУчетаДт,
	|	Операция.Подразделение КАК МестоУчетаДт,
	...
	|	Операция.СтатьяРасходовСбор,
	|	Операция.Подразделение,
	|	Операция.АналитикаРасходовСбор,
	|	Операция.Подразделение,
	|	Операция.СтатьяРасходовСбор
	|";
#КонецОбласти 
...
#Область ТекстРасчетыПоОбязательнымПлатежамНаВзаиморасчеты // (Дт 642 :: Кт 63)
	ТекстРасчетыПоОбязательнымПлатежамНаВзаиморасчеты = "
...
#Область ТекстРасчетыПоАкцизуНаСебестоимость // (Дт 28 :: Кт 6416)
	ТекстРасчетыПоАкцизуНаСебестоимость = "
...
#Область ТекстРасчетыПоАкцизуНаВзаиморасчеты // (Дт 6416 :: Кт 63)
	ТекстРасчетыПоАкцизуНаВзаиморасчеты = "
	|ВЫБРАТЬ // Таможенная пошлина (Дт 6416 :: Кт 63)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
#Область ТекстНДСНаСебестоимостьЗакупкаПодНеоблагаемуюДеятельность // (Дт 28 :: Кт 63)
	ТекстНДСНаСебестоимостьЗакупкаПодНеоблагаемуюДеятельность = "
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: на себестоимость запасов"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстНДСНаРасходыЗакупкаПодНеоблагаемуюДеятельность // (Дт 9X :: Кт 63)
	ТекстНДСНаРасходыЗакупкаПодНеоблагаемуюДеятельность = "
...
#КонецОбласти 
	...
	Возврат
		ТекстТаможенныйСборНаРасходы
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстТаможенныйСборНаСебестоимость
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстРасчетыПоОбязательнымПлатежамНаСебестоимость
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстРасчетыПоОбязательнымПлатежамНаРасходы
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстРасчетыПоОбязательнымПлатежамНаВзаиморасчеты
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстРасчетыПоАкцизуНаСебестоимость
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстРасчетыПоАкцизуНаРасходы
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстРасчетыПоАкцизуНаВзаиморасчеты
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДСНаСебестоимостьЗакупкаПодНеоблагаемуюДеятельность
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДСНаРасходыЗакупкаПодНеоблагаемуюДеятельность
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНалоговыйКредит
...
Функция ТекстЗапросаТаблицаПрочиеРасходы(Запрос, ТекстыЗапроса, Регистры)
    ...
    //++ НЕ УТ
	|	ВЫБОР КОГДА ТоварыДокумента.ВариантРаспределенияРасходов В (
	...
	|	СУММА(ВЫБОР КОГДА ТоварыДокумента.ВариантРаспределенияРасходов =
	|		ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
	|	ТОГДА 
	|		ТоварыДокумента.СуммаБезНДСУпр
	|	ИНАЧЕ
	|		0
	|	КОНЕЦ) КАК СуммаБезНДС,
    |
	|	СУММА(ВЫБОР КОГДА ТоварыДокумента.ВариантРаспределенияРасходов В (
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПрочиеАктивы),
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)) ТОГДА
	|		0
	|	КОГДА ТоварыДокумента.ВидДеятельностиНДС <> ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая) ТОГДА
	...
	|		ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
	|	ИНАЧЕ
	|		ТоварыДокумента.НалоговоеНазначение КОНЕЦ,
    //-- НЕ УТ
...
Функция ТекстЗапросаТаблицаНДСУсловныеПродажи(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И &ОрганизацияПлательщикНДС
	|	И Разделы.СтавкаНДС В (ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20), ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7))
	|	
	|СГРУППИРОВАТЬ ПО
	|	Товары.Ссылка,
	|	Разделы.СтавкаНДС,
