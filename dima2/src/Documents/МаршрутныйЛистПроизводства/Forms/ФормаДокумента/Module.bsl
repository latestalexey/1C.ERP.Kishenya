...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Документы.МаршрутныйЛистПроизводства.ПроверитьСвязанныеОбъектыОбработаныОбработчикамиОбновления(Объект, ЭтотОбъект);
	...
	Если Объект.УправлениеМаршрутнымиЛистами = Перечисления.УправлениеМаршрутнымиЛистами.МетодикаББВ Тогда
		...
		Если Объект.ПланироватьРаботуВидовРабочихЦентров Тогда
			...
			Если ФактическоеНачалоББВ <> '000101010000'
				...
				ТекстСообщения = НСтр("ru='Дата начала выполнения маршрутного листа должна быть раньше даты готовности к работе ключевого рабочего центра.';uk='Дата початку виконання маршрутного листа повинна бути раніше дати готовності до роботи ключового робочого центру.'");
		...
		Иначе	
			...
			Если ФактическоеНачалоУББВ <> '000101010000'
				...
				ТекстСообщения = НСтр("ru='Дата начала выполнения маршрутного листа должна быть раньше даты окончания.';uk='Дата початку виконання маршрутного листа повинна бути раніше дати закінчення.'");
...
Процедура ЗапланированоПриИзменении(Элемент)
	...
	Если ЗапланированоДоРедактирования <> 0 И Объект.Запланировано <> 0 Тогда
		...
		ТекстВопроса = НСтр("ru='Для измененения поля ""Запланировано"" требуется пересчитать нормативы.
                                |Пересчитать нормативы?'
                                |;uk='Для зміни поля ""Заплановано"" потрібно перерахувати нормативи.
                                |Перерахувати нормативи?'");
								
		СписокКнопок = Новый СписокЗначений;
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Пересчитать нормативы';uk='Перерахувати нормативи'"));
		СписокКнопок.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru='Отменить редактирование';uk='Скасувати редагування'"));
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗапланированоПриИзмененииЗавершение", ЭтотОбъект);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок,,, НСтр("ru='Измененение поля ""Запланировано""';uk='Оновлення поля ""Заплановано""'"));
...
Процедура ОписаниеБракаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияКомментария(
		...
		НСтр("ru='Описание брака';uk='Опис браку'"));
...
Процедура ПриоритетПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Приоритет) И НЕ Объект.УправлениеМаршрутнымиЛистами
		...
		ТекстВопроса = НСтр("ru='Назначить выбранный приоритет операциям?';uk='Призначити вибраний пріоритет операцій?'");
...
Процедура ОперацииПередУдалением(Элемент, Отказ)
	...
	Если УдаляемыеОперации.Количество() = 1 Тогда
		ТекстВопроса = НСтр("ru='Удалить операцию?';uk='Вилучити операцію?'");
	Иначе
		ТекстВопроса = НСтр("ru='Удалить выбранные операции?';uk='Вилучити вибрані операції?'");
	КонецЕсли; 
...
Процедура ВыходныеИзделияПередУдалением(Элемент, Отказ)
	...
	Для каждого ИдентификаторСтроки Из Элементы.ВыходныеИзделия.ВыделенныеСтроки Цикл
		...
		Если ТекущиеДанные.ПроизводитсяВПроцессе Тогда
			ПоказатьПредупреждение(,НСтр("ru='Из списка продукции нельзя удалить полуфабрикат.';uk='Зі списку продукції не можна вилучити напівфабрикат.'"));
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	КонецЦикла; 
...
Процедура ВозвратныеОтходыПередУдалением(Элемент, Отказ)
	...
	Для Каждого ИдентификаторСтроки Из Элементы.ВозвратныеОтходы.ВыделенныеСтроки Цикл
		...
		Если ТекущиеДанные.ПроизводитсяВПроцессе Тогда
			ПоказатьПредупреждение(,НСтр("ru='Из списка нельзя удалить полуфабрикат.';uk='Зі списку неможна вилучити напівфабрикат.'"));
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	КонецЦикла;
...
Процедура МатериалыИУслугиПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ ОтменаРедактирования ИЛИ НЕ НоваяСтрока Тогда
		...
		Если ДанныеСтроки.Количество + ДанныеСтроки.КоличествоОтклонение < 0 Тогда
			ПоказатьПредупреждение(,НСтр("ru='Отклонение от норматива не должно приводить к отрицательному потреблению.';uk='Відхилення від нормативу не повинно призводити до від''ємного споживання.'"));
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура МатериалыИУслугиПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если НЕ ПустаяСтрока(ТекущиеДанные.СведенияАвтовыбора) Тогда
		...
		Если НЕ ТекущиеДанные.Номенклатура.Пустая() И (НЕ ТекущиеДанные.Характеристика.Пустая()	ИЛИ НЕ ТекущиеДанные.ХарактеристикиИспользуются) Тогда
			...
			УправлениеДаннымиОбИзделияхКлиентСервер.ЗаполнитьИнформациюПоАвтовыборуМатериала(ТекущиеДанные);
...
Процедура МатериалыИУслугиМатериалИзрасходованПриИзменении(Элемент)
	...
	ТекущиеДанные = Элементы.МатериалыИУслуги.ТекущиеДанные;
	Если ТекущиеДанные.МатериалИзрасходован Тогда
		ТекущиеДанные.ДатаРасхода = ТекущаяДата();
	Иначе
...
Процедура ТрудозатратыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ ОтменаРедактирования ИЛИ НЕ НоваяСтрока Тогда
		...
		Если ДанныеСтроки.Количество + ДанныеСтроки.КоличествоОтклонение < 0 Тогда
			ПоказатьПредупреждение(,НСтр("ru='Отклонение от норматива не должно приводить к отрицательным трудозатратам.';uk='Відхилення від нормативу не повинно призводити до від''ємних трудовитрат.'"));
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура КомандаПорядокВыполненияОперацийЗадаетсяПорядкомСтрок(Команда)
	...
	Если Объект.ПорядокВыполненияОперацийЗадаетсяПроизвольно Тогда
		ТекстВопроса = НСтр("ru='Установить порядок выполнения операций по порядку строк?';uk='Установити порядок виконання операцій по порядку рядків?'");
...
Процедура КомандаПроверитьПорядокОпераций(Команда)
	...
	Если Объект.Операции.Количество() <> 0 
		...
		ТекстПредупреждения = НСтр("ru='Порядок операций указан верно.';uk='Порядок операцій вказано вірно.'");
...
Процедура КомандаЗаполнитьПорядокОперацийПоследовательно(Команда)
	...
	Если НЕ Объект.ПорядокВыполненияОперацийЗадаетсяПроизвольно Тогда
		ПоказатьПредупреждение(,НСтр("ru='Порядок выполнения операций задается в порядке строк.
                                    ...
                                    |;uk='Порядок виконання операцій визначається в порядку рядків.
                                    |Виконувати команду не потрібно.'")); 
		Возврат;
	КонецЕсли;
	...
	ТекстВопроса = НСтр("ru='Заполнить порядок выполнения в соответствии с порядком строк?';uk='Заповнити порядок виконання відповідно до порядку рядків?'");
...
Процедура КомандаИзменитьКачествоНоменклатуры(Команда)
	...
	Если ТекущиеДанные.ПроизводитсяВПроцессе Тогда
		ПоказатьПредупреждение(,НСтр("ru='Нельзя изменить качество полуфабриката';uk='Не можна змінити якість напівфабрикату'"));
		Возврат;
	КонецЕсли;
...
Процедура КомандаРазбитьСтрокуИзделий(Команда)
	...
	Если Элементы.ВыходныеИзделия.ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыРазбиенияСтроки.Заголовок = НСтр("ru='Введите фактическое количество в новой строке';uk='Введіть фактичну кількість у новому рядку'");
...
Процедура КомандаИзменитьРасписание(Команда)
	...
	ТекстВопроса = НСтр("ru='Для продолжения необходимо записать документ.';uk='Для продовження необхідно записати документ.'");
...
Процедура КомандаОтметитьРасходМатериалов(Команда)
	...
	Если МассивСтрок.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Необходимо материалы для которых заполнено фактическое количество.';uk='Необхідно матеріали для яких заповнена фактична кількість.'");
		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	...
	РаботаСДиалогамиКлиент.ВвестиДатуСКонтролемПустогоЗначения(
			ТекущаяДата(), 
			НСтр("ru='Введите дату расхода материалов (работ)';uk='Введіть дату видатку матеріалів (робіт)'"), 
...
Процедура ПерейтиКМатериалам(Команда)
	...
	Если НЕ ТекущиеДанные = Неопределено Тогда
		...
		Иначе
			...
			ТекстПредупреждения = НСтр("ru='Материалы по выбранной операции отсутствуют';uk='Матеріали за вибраною операцією відсутні'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
...
Процедура ПерейтиКТрудозатратам(Команда)
	...
	Если НЕ ТекущиеДанные = Неопределено Тогда
		...
		Иначе
			...
			ТекстПредупреждения = НСтр("ru='Трудозатраты по выбранной операции отсутствуют';uk='Трудовитрати за вибраною операцією відсутні'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
...
Процедура ЗаполнитьСлужебныеРеквизиты()
	...
	НоменклатураСервер.ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВКоллекции(
		...
		Новый Структура("ЗаполнитьПризнакХарактеристикиИспользуются, ЗаполнитьПризнакАртикул, ЗаполнитьПризнакТипНоменклатуры",
			Новый Структура("Номенклатура", "ХарактеристикиИспользуются"),
	...
	Для каждого СтрокаМатериал Из Объект.МатериалыИУслуги Цикл
		УправлениеДаннымиОбИзделияхКлиентСервер.ЗаполнитьИнформациюПоАвтовыборуМатериала(СтрокаМатериал);
...
Процедура ЗаполнитьСтроковыеРеквизитыФормы()
	...
	Если ВыборкаРаспоряжение.Следующий() Тогда
		РаспоряжениеСтрока = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											...
											ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаРаспоряжение.РаспоряжениеНомер, Ложь, Истина),
...
Процедура ЗаполнитьРеквизитОперация(Форма, ИмяТабличнойЧасти, ТекущиеДанные = Неопределено)
	...
	Для каждого ЭлементКоллекции Из Объект[ИмяТабличнойЧасти] Цикл
		...
		Если ТекущиеДанные = Неопределено Тогда
			...
			КонецЕсли;
		ИначеЕсли ЭлементКоллекции.КлючСвязиОперации <> ТекущиеДанные.КлючСвязи Тогда
			Продолжить;
		КонецЕсли;
		
		Если СтрокаОперация <> Неопределено Тогда
			ЭлементКоллекции.Операция          = СтрокаОперация.Операция;
			ЭлементКоллекции.НомерОперации     = СтрокаОперация.НомерОперации;
		Иначе
...
Процедура ИзменитьСтатусОперацииПланированиеНаСервере(ОперацияКлючСвязи, НовыйСтатус, ТекстПредупреждения)
	...
	Если ТекущийСтатус = Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru='Для выбранной операции не рассчитано расписание производства,
            ...
            |;uk='Для вибраної операції не розрахований розклад виробництва,
            |статус не може бути встановлений.'");
			
	ИначеЕсли ТекущийСтатус = НовыйСтатус Тогда
		
		ТекстПредупреждения = НСтр("ru='Выбранный статус выполнения уже установлен.';uk='Вибраний статус виконання вже встановлений.'");
...
Процедура ИзменитьСтатусОперацииРегистрация(НовыйСтатус)
	...
	Если НЕ ТекущиеДанные = Неопределено Тогда
		...
		Иначе
			...
			ПоказатьПредупреждение(,НСтр("ru='Выбранный статус выполнения уже установлен.';uk='Вибраний статус виконання вже встановлений.'"));
...
Функция ПроверитьИВернутьДанныеВыбраннойОперации()
	...
	ТекущиеДанные = Элементы.Операции.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ru='Выберите строку табличной части';uk='Виберіть рядок табличної частини'"));
	КонецЕсли;
...
Процедура ПереместитьОперацию(Знач Направление)
	...
	Если Объект.ПорядокВыполненияОперацийЗадаетсяПроизвольно И НЕ ПорядокСтрокСовпадаетСПорядкомВыполненияОпераций Тогда
		ПоказатьПредупреждение(,НСтр("ru='Невозможно переместить операцию,
                                    ...
                                    |;uk='Неможливо перемістити операцію,
                                    |т. к. задано довільний порядок виконання.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПодтвердитьЗаполнениеДокумента(ОбработчикПослеПодтверждения)
	...
	Если Объект.Операции.Количество() <> 0
		...
		ТекстВопроса = НСтр("ru='Перезаполнить маршрутный лист?';uk='Перезаповнити маршрутний лист?'");
		...
		Если НачалоДня(НачалоРаботыКлючевогоРабочегоЦентра) = НачалоДня(ОкончаниеРаботыКлючевогоРабочегоЦентра) Тогда
			Если ПериодСТочностьюДоЧасов Тогда
				ИнформацияОРасписанииКлючевогоРЦ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
														НСтр("ru='Выполнение маршрутного листа ключевым рабочим центром запланировано на %1 с %2 по %3.';uk='Виконання маршрутного листа ключовим робочим центром заплановано на %1 з %2 по %3.'"),
			...
			Иначе
				ИнформацияОРасписанииКлючевогоРЦ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
														НСтр("ru='Выполнение маршрутного листа ключевым рабочим центром запланировано на %1.';uk='Виконання маршрутного листа ключовим робочим центром заплановано на %1.'"),
														Формат(НачалоРаботыКлючевогоРабочегоЦентра, ФорматДаты));
			КонецЕсли; 
		Иначе
			ИнформацияОРасписанииКлючевогоРЦ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
													НСтр("ru='Выполнение маршрутного листа ключевым рабочим центром запланировано с %1 по %2.';uk='Виконання маршрутного листа ключовим робочим центром заплановано з %1 по %2.'"),
													Формат(НачалоРаботыКлючевогоРабочегоЦентра, ФорматДаты),
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<определяется при формировании расписания>';uk='<визначається при формуванні розкладу>'"));
	...
	// Установка/снятие свойства ОтметкаНезаполненного в ТЧ Операции у нормативов времени
	#Область ОтметкаНезаполненногоОперации
	...
	Если Объект.УправлениеМаршрутнымиЛистами = Перечисления.УправлениеМаршрутнымиЛистами.ПооперационноеПланирование Тогда
		...
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ОперацииВремяШтучное.Имя);
		
		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Операции.ВремяВыполнения");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
		
		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Операции.ВремяШтучное");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
		
		Элемент.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Истина);
		
		// Время штучное ед. изм.
		Элемент = УсловноеОформление.Элементы.Добавить();
		
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ОперацииВремяШтучноеЕдИзм.Имя);
		
		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Операции.ВремяВыполненияЕдИзм");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
		
		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Операции.ВремяШтучноеЕдИзм");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
		...
		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Операции.ВремяПЗ");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
		...
		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Операции.ВремяПЗЕдИзм");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	...
	Если Объект.УправлениеМаршрутнымиЛистами = Перечисления.УправлениеМаршрутнымиЛистами.РегистрацияОпераций Тогда
		
		Элемент = УсловноеОформление.Элементы.Добавить();
		
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ОперацииВремяВыполнения.Имя);
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ОперацииВремяВыполненияЕдИзм.Имя);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ПриИзмененииМаршрута(Форма)

	ОперативныйУчетПроизводстваКлиентСервер.ПриИзмененииМаршрута(
			Форма.Объект, 
			Форма.НачатоВыполнениеМаршрутногоЛиста);

	Форма.ФактическоеНачалоББВ = Форма.Объект.ФактическоеНачало;
	Форма.ФактическоеНачалоУББВ = Форма.Объект.ФактическоеНачало;
			
	ДоступностьЭлементовМаршрута(Форма);
...
&НаКлиенте
Процедура ЗапланированоПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ЗапланированоПриИзмененииНаСервере();
		УстановитьРасписаниеНеАктуально();
	Иначе
		Объект.Запланировано = ЗапланированоДоРедактирования;
	КонецЕсли;
...
Процедура ПоказатьСхемуОпераций()
	...
	ЗаголовокСхемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Схема операций маршрутного листа № %1';uk='Схема операцій маршрутного листа № %1'"), 
								ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Объект.Номер, Ложь, Истина));
...
Функция СформироватьСхемуОпераций()
	...
	Для каждого СтрокаОперация Из Объект.Операции Цикл
		...
		НоваяСтрока.ЭтапыПредшественники.Колонки.Добавить("ЭтапПроцесса");
	...
	Если Объект.Статус = Перечисления.СтатусыМаршрутныхЛистовПроизводства.Выполнен
		...
		Если Выборка.Следующий() Тогда
			...
			ЗаголовокКоманды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Сформировать новый маршрутный лист чтобы допроизвести %1 ед/партий изделий';uk='Сформувати новий маршрутний лист щоб довиробити %1 од/партій виробів'"),
...
Процедура ЗаписатьОбъектИПродолжитьВыполнение(ВыполняемоеОповещение, ТекстВопроса, НедопускаетсяМодифицированность = Ложь)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) ИЛИ Модифицированность Тогда
		...
		СписокКнопок = Новый СписокЗначений;
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Записать и продолжить';uk='Записати й продовжити'"));
...
Процедура ОперацииВремяШтучноеПриИзменении(Элемент)
	...
	ОперативныйУчетПроизводстваКлиентСервер.РассчитатьОбщееВремяВыполненияОперации(
		Элементы.Операции.ТекущиеДанные, Объект.Запланировано);
