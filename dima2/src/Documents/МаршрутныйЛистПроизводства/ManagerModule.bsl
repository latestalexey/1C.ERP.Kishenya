...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСоздатьНаОсновании) Экспорт
	...
	Документы.ВозвратМатериаловИзПроизводства.ДобавитьКомандуСоздатьНаОсновании(КомандыСоздатьНаОсновании);
	
	Документы.ПриходныйОрдерНаТовары.ДобавитьКомандуСоздатьНаОсновании(КомандыСоздатьНаОсновании);
	
	Обработки.ВводКорректировкиЗаказаМатериалов.ДобавитьКомандуСоздатьНаОснованииОтгрузкаПоМаршрутнымЛистам(КомандыСоздатьНаОсновании);
	
	Документы.ВыпускПродукции.ДобавитьКомандуСоздатьНаОснованииМаршрутныхЛистов(КомандыСоздатьНаОсновании);
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСоздатьНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.МаршрутныйЛистПроизводства) Тогда
		...
		КомандаСоздатьНаОсновании.Идентификатор = Метаданные.Документы.МаршрутныйЛистПроизводства.ПолноеИмя();
		КомандаСоздатьНаОсновании.Представление = ВводНаОсновании.ПредставлениеОбъекта(Метаданные.Документы.МаршрутныйЛистПроизводства);
...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов) Экспорт
	...
	//++ НЕ УТКА
	ВариантыОтчетовУТПереопределяемый.ДобавитьКомандуПередачаМатериаловПоЗаказуМЛ(КомандыОтчетов);
	//-- НЕ УТКА
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ПараметрыУказанияСерийМатериалов.Спецификация = ?(Объект.СпецификацияПолуфабриката.Пустая(), 
															Объект.Спецификация, 
	...
	ИспользуетсяСправочноеУказаниеСерий = НоменклатураСервер.ИспользуетсяСправочноеУказаниеСерийВМаршуртныхЛистах(Объект.Спецификация, Перечисления.ВидыСтрокДереваСпецификаций.Материал);
	...
	ПараметрыУказанияСерийМатериалов.СкладскиеОперации.Добавить(Перечисления.СкладскиеОперации.РасходМатериаловПриВыполненииМаршрутныхЛистов);
	...
	ПараметрыУказанияСерийИзделий.Спецификация = ?(Объект.СпецификацияПолуфабриката.Пустая(), 
														Объект.Спецификация, 
														Объект.СпецификацияПолуфабриката);
	...
	ИспользуетсяСправочноеУказаниеСерий = НоменклатураСервер.ИспользуетсяСправочноеУказаниеСерийВМаршуртныхЛистах(Объект.Спецификация, Перечисления.ВидыСтрокДереваСпецификаций.ВыходноеИзделие);
	...
	ПараметрыУказанияСерийИзделий.СкладскиеОперации.Добавить(Перечисления.СкладскиеОперации.ОтражениеВыходныхИзделийВМаршрутныхЛистах);
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерий(ПараметрыУказанияСерий) Экспорт
	...
	Если ПараметрыУказанияСерий.СкладскиеОперации.Найти(Перечисления.СкладскиеОперации.ОтражениеВыходныхИзделийВМаршрутныхЛистах) <> Неопределено Тогда
...
Функция ПроверкаПередСменойСтатуса(ВыборкаПроверки, НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если ВыборкаПроверки.ТекущийСтатус = Перечисления.СтатусыМаршрутныхЛистовПроизводства.Отменен Тогда
		...
		ТекстОшибки = НСтр("ru='Не допускается изменять статус отмененного маршрутного листа.';uk='Не допускається змінювати статус скасованого маршрутного листа.'");
...
// Возвращает новый номер в очереди маршрутных листов.
//
Функция НовыйНомерОчереди(Подразделение) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	МаршрутныйЛистПроизводства.Очередь + 1 КАК НовыйНомерОчереди
	|ИЗ
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	МаршрутныйЛистПроизводства.Очередь УБЫВ");
...
Функция ПроверитьПорядокОпераций(Операции, КлючДанных = Неопределено, ПутьКДанным = "", Отказ = Ложь) Экспорт
	...
	СтруктураСообщений.Вставить("НомерСледующейБольше",    НСтр("ru='Номер операции ""%1"" должен быть больше номера следующей.';uk='Номер операції ""%1"" повинен бути більше номера наступної.'"));
...
//				* 0 - материалов хватит всем созданным и переданным к выполнению МЛ (индикатор не выводится)
//				* 1 - материалов хватит текущему МЛ, но остальным МЛ может не хватить (выводится желтый индикатор)
//				* 2 - материалов не хватает текущему МЛ (выводится красный индикатор)
//
	...
	Для каждого Запись Из Расписание Цикл
		...
		Исключение
			...
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось изменить маршрутный лист %1. Возможно, документ в настоящий момент редактируется.';uk='Не вдалося змінити маршрутний лист %1. Можливо, документ зараз редагується.'"),
...
//	ДокументОбъект - ДокументОбъект.МаршрутныйЛистПроизвоства - владелец изменяемых операций.
//
...
//
Функция ИменаТаблицДляКонтроляБлокировкиРаспоряжений() Экспорт
	
	Результат = Новый Массив;
	
	Результат.Добавить("Документ.ЗаказНаПроизводство");
...
//
// Параметры:
//  Объект - ДокументОбъект.МаршрутныйЛистПроизводства, ДанныеФормыСтруктура - объект, который необходимо проверить.
...
Процедура ПроверитьСвязанныеОбъектыОбработаныОбработчикамиОбновления(Объект, Форма = Неопределено) Экспорт
	...
	ЭтоНовыйОбъект = ТипЗнч(Объект) = Тип("ДокументОбъект.МаршрутныйЛистПроизводства") И Объект.ЭтоНовый()
	...
	ТаблицыДляКонтроля = ИменаТаблицДляКонтроляБлокировкиРаспоряжений();
	СписокСсылок = Новый Массив;
	СписокСсылок.Добавить(Объект.Распоряжение);
	...
	ОбработкаЗавершена = (ЗаблокированныеРаспоряжения.Количество() = 0);
...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	Если ИмяРегистра = "МатериалыИРаботыВПроизводстве" Тогда
		...
		ПереопределениеРасчетаПараметров.Вставить("НалоговоеНазначение", "ТабличнаяЧасть.Ссылка.Распоряжение.НалоговоеНазначение");
	...
	ИначеЕсли ИмяРегистра = "ОбеспечениеЗаказовРаботами" Тогда
		...
		ТекстЗапроса = ТекстЗапросаТаблицаОбеспечениеЗаказовРаботами(Неопределено, ТекстыЗапроса, Неопределено);
		СинонимТаблицыДокумента = "ТабличнаяЧасть";
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.Текст =
		...
		|	Реквизиты.Распоряжение.НалоговоеНазначение          КАК НалоговоеНазначение,
...
Функция ТекстЗапросаТаблицаДоступностьВидовРабочихЦентров(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеСервер.ЕстьТаблицаЗапроса("ВТЗанятоЗаказом", ТекстыЗапроса) Тогда
...
Функция ТекстЗапросаТаблицаТрудозатратыКОформлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|	Трудозатраты.Количество + Трудозатраты.КоличествоОтклонение КАК Количество,
	...
	|	И Трудозатраты.Бригада <> ЗНАЧЕНИЕ(Справочник.Бригады.ПустаяСсылка)";
...
Функция ТекстЗапросаТаблицаМатериалыИРаботыВПроизводстве(Запрос, ТекстыЗапроса, Регистры)
	...
	|		И (Аналитика.Склад = &ПодразделениеЭтапа)
...
Функция ТекстЗапросаТаблицаРаспоряженияНаВыпускПродукции(Запрос, ТекстыЗапроса, Регистры)
	...
	|			И (ТабличнаяЧасть.Получатель = КлючиАналитикиУчета.Склад)
	|			И (ВЫБОР
...
Функция ТекстЗапросаТаблицаДвижениеТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	//Движения по нормативу, для не выполненных маршрутных листов (график поступления продукции).
	|ВЫБРАТЬ
	|	&СрокПоставки                     КАК Период,
	|	&Распоряжение                     КАК Распоряжение,
	|
	...
	|	И ТабличнаяЧасть.КоличествоФакт = 0
	// только когда поступление выполняется на склад (незаполненные значения запрещены)
	|	И ТабличнаяЧасть.Получатель ССЫЛКА Справочник.Склады
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	//Движения по маршрутному листу (график поступления продукции по факту).
	|ВЫБРАТЬ
	|	&СрокПоставки                     КАК Период,
	|	&Распоряжение                     КАК Распоряжение,
	|
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	//Движения по нормативу, для не выполненных маршрутных листов (график поступления возвратных отходов).
	|ВЫБРАТЬ
	|	&СрокПоставки                     КАК Период,
	|	&Распоряжение                     КАК Распоряжение,
	|
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	//Движения по маршрутному листу (график поступления возвратных отходов по факту).
	|ВЫБРАТЬ
	|	&СрокПоставки                     КАК Период,
	|	&Распоряжение                     КАК Распоряжение,
	|
...
Функция ТекстЗапросаТаблицаТоварыКПоступлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТабличнаяЧасть.Получатель КАК Склад,
	|	ТабличнаяЧасть.КоличествоФакт КАК КПоступлению
	|ИЗ
	|	Документ.МаршрутныйЛистПроизводства.ВыходныеИзделия КАК ТабличнаяЧасть
	|ГДЕ
	|	ТабличнаяЧасть.Ссылка = &Ссылка
	...
	|	И &Период >= ВЫРАЗИТЬ(ТабличнаяЧасть.Получатель КАК Справочник.Склады).ДатаНачалаОрдернойСхемыПриПоступлении
	|	И ТабличнаяЧасть.Получатель ССЫЛКА Справочник.Склады
	|
	|	И &Статус В (
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыМаршрутныхЛистовПроизводства.Создан),
...
Функция ТекстЗапросаТаблицаОбеспечениеЗаказов(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = "
	// Сторно движения заказа на производство
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)  КАК ВидДвижения,
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	//Движения по нормативу, для невыполненных маршрутных листов (график поступления продукции по нормативу).
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)  КАК ВидДвижения,
	|	&Окончание                              КАК Период,
	|	ТабличнаяЧасть.Номенклатура             КАК Номенклатура,
	...
	|	ТабличнаяЧасть.Назначение               КАК Назначение,
	|	0                                       КАК Потребность,
	|	-ТабличнаяЧасть.КоличествоФакт          КАК КЗаказу
	|ИЗ
	|	Документ.МаршрутныйЛистПроизводства.ВыходныеИзделия КАК ТабличнаяЧасть
	|
	|ГДЕ
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	//Движения по нормативу, для невыполненных маршрутных листов (график поступления возвратных отходов по нормативу).
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)  КАК ВидДвижения,
	|	&Окончание                              КАК Период,
	|	ТабличнаяЧасть.Номенклатура             КАК Номенклатура,
...
Функция ТекстЗапросаТаблицаОбеспечениеЗаказовРаботами(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТабличнаяЧасть.Назначение                    КАК Назначение,
	|	ТабличнаяЧасть.КоличествоОтклонение          КАК КОбеспечению
	|ИЗ
	|	Документ.МаршрутныйЛистПроизводства.РаспоряжениеСпецификация КАК ТабличнаяЧасть
	|
	|ГДЕ
...
Функция ТекстЗапросаТаблицаДвиженияСерийТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЗНАЧЕНИЕ(Перечисление.СкладскиеОперации.РасходМатериаловПриВыполненииМаршрутныхЛистов) КАК СкладскаяОперация,
	|	&Ссылка                             КАК Документ,
	|	&Период                             КАК Период,
	|	ИСТИНА                              КАК ЭтоСкладскоеДвижение
	|ИЗ
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "МаршрутныйЛист") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
						...
						ПечатнаяФормаМаршрутногоЛиста(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода),
						,
						"Документ.МаршрутныйЛистПроизводства.ПФ_MXL_МаршрутныйЛист",
						,
...
Функция ПечатнаяФормаМаршрутногоЛиста(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода)
	
	УстановитьПривилегированныйРежим(Истина);
	
	КодЯзыкаПечать = ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм;
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.МаршрутныйЛистПроизводства.ПФ_MXL_МаршрутныйЛист", КодЯзыкаПечать);
...
Процедура ВывестиТабличнуюЧасть(ПараметрыВывода, СтруктураПоиска, ВыборкаПоТабличнойЧасти, Макет, МассивВыводимыхОбластей, НомерСтраницы, ТабличныйДокумент)
	...
	Для каждого СтрокаТовары Из ВыборкаПоТабличнойЧасти Цикл
		...
		Если ПараметрыВывода.Свойство("НаПродукцию") Тогда
			НомерДокумента = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(СтрокаТовары.Номер);
			МаршрутныйЛистПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
														НСтр("ru='№%1 от %2';uk='№%1 від %2'",КодЯзыкаПечать),
														НомерДокумента, 
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерийМатериалы()
	...
	|				И ПолитикиУчетаСерий.ПолитикаУчетаСерий.УказыватьДляМатериаловПриВыполненииМаршрутныхЛистов
	|			ТОГДА ВЫБОР
	|					КОГДА Товары.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка) И НЕ &ФактОтбора И НЕ Товары.МатериалИзрасходован
	|						ТОГДА 21
	|					КОГДА Товары.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерийИзделия()
	...
	|		КОГДА ПолитикиУчетаСерий.ПолитикаУчетаСерий ЕСТЬ NULL 
	|			ТОГДА 0
	|		КОГДА ПолитикиУчетаСерий.ПолитикаУчетаСерий.УказыватьДляИзделийПриВыполненииМаршрутныхЛистов
	|			ТОГДА ВЫБОР
 	...
 	Пока Выборка.Следующий() Цикл
 		...
 		Попытка
			...
			Если ДокументОбъект = Неопределено 
				...
				ИЛИ ДокументОбъект.Статус = Перечисления.СтатусыМаршрутныхЛистовПроизводства.Отменен Тогда
			...
			Для каждого ДанныеСтроки Из ДокументОбъект.ВыходныеИзделия Цикл
				Если ДанныеСтроки.КоличествоОтклонение = 0 
					И ДанныеСтроки.КоличествоФакт <> ДанныеСтроки.Количество Тогда
					ДанныеСтроки.КоличествоФакт = ДанныеСтроки.Количество;
					ДанныеСтроки.КоличествоУпаковокФакт = ДанныеСтроки.КоличествоУпаковок;
				КонецЕсли;
			КонецЦикла;
			Для каждого ДанныеСтроки Из ДокументОбъект.ВозвратныеОтходы Цикл
				Если ДанныеСтроки.КоличествоОтклонение = 0
					И ДанныеСтроки.КоличествоФакт <> ДанныеСтроки.Количество Тогда
					ДанныеСтроки.КоличествоФакт = ДанныеСтроки.Количество;
					ДанныеСтроки.КоличествоУпаковокФакт = ДанныеСтроки.КоличествоУпаковок;
 		...
 		Исключение
			...
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
									...
									Метаданные.Документы.МаршрутныйЛистПроизводства,
									Выборка.Ссылка,
									ТекстСообщения);
...
Процедура ОбновитьОбъект_ОтметитьКОбработке(Параметры) Экспорт
	...
	СписокСсылок = СписокДокументовВКоторыхНарушеноРавенство();
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, СписокСсылок);
	
КонецПроцедуры

Функция СписокДокументовВКоторыхНарушеноРавенство() Экспорт
	...
	|	ТЧ.Ссылка
	|ИЗ
	|	Документ.МаршрутныйЛистПроизводства.ВыходныеИзделия КАК ТЧ
	|ГДЕ
	|	ТЧ.КоличествоОтклонение = 0
	|	И ТЧ.КоличествоФакт <> ТЧ.Количество
	|	И ТЧ.Ссылка.Проведен
	|	И ТЧ.Ссылка.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыМаршрутныхЛистовПроизводства.Отменен)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТЧ.Ссылка
	|ИЗ
	|	Документ.МаршрутныйЛистПроизводства.ВозвратныеОтходы КАК ТЧ
	|ГДЕ
	|	ТЧ.КоличествоОтклонение = 0
	|	И ТЧ.КоличествоФакт <> ТЧ.Количество
	|	И ТЧ.Ссылка.Проведен
...
Функция ПользовательскоеПредставлениеДокумента(ДокументСсылка)
	...
	НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(НомерДата.Номер);
