...
Процедура ЗаполнитьДанныеРаспоряжения() Экспорт
	...
	// Определим коэффициент по которому будем рассчитывать количество требуемых матереалов
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Документы.МаршрутныйЛистПроизводства.ПроверитьСвязанныеОбъектыОбработаныОбработчикамиОбновления(ЭтотОбъект);
	...
	ПараметрыОкругления.СуффиксДопРеквизита = "Факт";
	...
	ИначеЕсли Очередь = 0
		...
		И (Статус = Перечисления.СтатусыМаршрутныхЛистовПроизводства.Создан 
		...
		Очередь = Документы.МаршрутныйЛистПроизводства.НовыйНомерОчереди(Подразделение);
		СостояниеРасписания = Перечисления.СостоянияРасписанияРабочихЦентров.НеСформировано;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Документы.МаршрутныйЛистПроизводства.ИнициализироватьДанныеДокумента(Ссылка, ДополнительныеСвойства);
	
	ПроведениеСервер.ПодготовитьНаборыЗаписейКРегистрацииДвижений(ЭтотОбъект);
	
	
	// Закрытие заказа на производство
	...
	// Для оформления выпуска, нарядов
	...
	ОперативныйУчетПроизводства.ОтразитьПотребностьВМатериалахВПроизводстве(ДополнительныеСвойства, Движения, Отказ);
	
	СкладыСервер.ОтразитьДвиженияСерийТоваров(ДополнительныеСвойства, Движения, Отказ);
	
	// Пооперационное планирование
	ОперативныйУчетПроизводства.ОтразитьОперацииМаршрутовПроизводства(ДополнительныеСвойства, Движения, Отказ);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПроверитьОперацииВремяШтучноеИПЗ(Отказ);
	...
	Если (Произведено + Брак) > Запланировано И Запланировано <> 0 Тогда
		Если Произведено <> 0 И Брак = 0 Тогда
			ТекстСообщения = НСтр("ru='Количество произведенных единиц/партий изделий не может превышать запланированное количество.';uk='Кількість вироблених одиниць/партій виробів не може перевищувати заплановану кількість.'");
		ИначеЕсли Произведено = 0 И Брак <> 0 Тогда
			ТекстСообщения = НСтр("ru='Количество брака не может превышать запланированное количество.';uk='Кількість браку не може перевищувати заплановану кількість.'");
		Иначе
			ТекстСообщения = НСтр("ru='Количество произведенных единиц/партий изделий и брака не может превышать запланированное количество.';uk='Кількість вироблених одиниць/партій виробів і браку не може перевищувати заплановану кількість.'");
		КонецЕсли;
...
Процедура ЗаполнитьМатериалыНаОснованииТабличнойЧастиЗаказа(МатериалыИУслугиЗаказа, ВыполнениеЭтапов, ИспользованныеМатериалы)
	...
	ЗапланированоПроизводством = ВыполнениеЭтапов[0].ЗапланированоПроизводством + Запланировано;
	...
	Для каждого МатериалЗаказа Из МатериалыИУслугиЗаказа Цикл
		...
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, МатериалЗаказа);
  		СписокСтрок = ИспользованныеМатериалы.НайтиСтроки(СтруктураПоиска);
		Если МатериалЗаказа.КоличествоОкруглено Тогда
			...
			КоличествоУпаковоНорматив = ЗапланированоПроизводством / МатериалЗаказа.КоличествоЭтап * МатериалЗаказа.КоличествоУпаковок 
										- ?(СписокСтрок.Количество() <> 0, СписокСтрок[0].Количество * МатериалЗаказа.УпаковкаКоэффициент, 0);
			КоличествоНорматив =  КоличествоУпаковоНорматив * МатериалЗаказа.УпаковкаКоэффициент;
		Иначе
			КоличествоНорматив = ЗапланированоПроизводством / МатериалЗаказа.КоличествоЭтап * МатериалЗаказа.Количество 
										- ?(СписокСтрок.Количество() <> 0, СписокСтрок[0].Количество, 0);
			КоличествоУпаковоНорматив = КоличествоНорматив / МатериалЗаказа.УпаковкаКоэффициент;
		КонецЕсли; 
		...
		НоваяСтрока.КоличествоУпаковок = КоличествоУпаковоНорматив;
		НоваяСтрока.КоличествоФакт = НоваяСтрока.Количество;
		НоваяСтрока.КоличествоУпаковокФакт = НоваяСтрока.КоличествоУпаковок;
	КонецЦикла; 
...
Функция ТекстЗапросаМатериалыЗаказа()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыМатериаловСУчетомКорректировок КАК ЗаказыМатериалов
...
Процедура ПроверитьЗаполнениеСклада(Отказ)
	...
	ШаблонСообщенияПодразделение = НСтр("ru='Не указано подразделение-получатель в строке %1.';uk='Не зазначено підрозділ-одержувач в рядку %1.'");
	ШаблонСообщенияСклад = НСтр("ru='Не указан склад-получатель в строке %1.';uk='Не зазначено склад-одержувач в рядку %1.'");
...
Процедура ПроверитьЗаполнениеВыполненногоМЛ(Отказ)
	...
	// ВозвратныеОтходы
	ШаблонСообщения = НСтр("ru='Не заполнена колонка ""Статья калькуляции"" в строке %1 списка ""Побочный и промежуточный выпуск"".';uk='Не заповнена колонка ""Стаття калькуляції"" в рядку %1 списку ""Побічний і проміжний випуск"".'");
	...
	// МатериалыИУслуги
	ШаблонСообщения = НСтр("ru='Не заполнена колонка ""Статья калькуляции"" в строке %1 списка ""Материалы и работы"".';uk='Не заповнена колонка ""Стаття калькуляції"" в рядку %1 списку ""Матеріали і роботи"".'");
	Для каждого СтрокаМатериал Из МатериалыИУслуги Цикл
	...
	// Трудозатраты
	ШаблонСообщенияСтатья = НСтр("ru='Не заполнена колонка ""Статья калькуляции"" в строке %1 списка ""Трудозатраты"".';uk='Не заповнена колонка ""Стаття калькуляції"" в рядку %1 списку ""Трудовитрати"".'");
	ШаблонСообщенияБригада = НСтр("ru='Не заполнена колонка ""Бригада"" в строке %1 списка ""Трудозатраты"".';uk='Не заповнена колонка ""Бригада"" в рядку %1 списку ""Трудовитрати"".'");
...
Процедура ПроверитьЗаполнениеОтклонений(ИмяТаблицы, Отказ)
	...
	Если НРег(ИмяТаблицы) = НРег("ВыходныеИзделия") Тогда
		...
		Если Статус = Перечисления.СтатусыМаршрутныхЛистовПроизводства.Создан Тогда
			ШаблонСообщенияКоличество = НСтр("ru='Не заполнена колонка ""Норматив"" в строке %1 списка ""Продукция"".';uk='Не заповнена колонка ""Норматив"" в рядку %1 списку ""Продукція"".'");
		Иначе
			ШаблонСообщенияКоличество = НСтр("ru='Необходимо заполнить колонку ""Норматив"" или ""Факт"" в строке %1 списка ""Продукция"".';uk='Необхідно заповнити колонку ""Норматив"" або ""Факт"" в рядку %1 списку ""Продукція"".'");
		КонецЕсли; 
	...
	ИначеЕсли НРег(ИмяТаблицы) = НРег("ВозвратныеОтходы") Тогда
		...
		ШаблонСообщения = НСтр("ru='Отклонение от норматива не должно приводить к отрицательному выпуску.';uk='Відхилення від нормативу не повинно призводити до від''ємного випуску.'");
		Если Статус = Перечисления.СтатусыМаршрутныхЛистовПроизводства.Создан Тогда
			ШаблонСообщенияКоличество = НСтр("ru='Не заполнена колонка ""Норматив"" в строке %1 списка ""Побочный и промежуточный выпуск"".';uk='Не заповнена колонка ""Норматив"" в рядку %1 списку ""Побічний і проміжний випуск"".'");
		Иначе
			ШаблонСообщенияКоличество = НСтр("ru='Необходимо заполнить колонку ""Норматив"" или ""Факт"" в строке %1 списка ""Побочный и промежуточный выпуск"".';uk='Необхідно заповнити колонку ""Норматив"" або ""Факт"" в рядку %1 списку ""Побічний і проміжний випуск"".'");
		КонецЕсли; 
	...
	ИначеЕсли НРег(ИмяТаблицы) = НРег("МатериалыИУслуги") Тогда
		...
		Если Статус = Перечисления.СтатусыМаршрутныхЛистовПроизводства.Создан Тогда
			ШаблонСообщенияКоличество = НСтр("ru='Не заполнена колонка ""Норматив"" в строке %1 списка ""Материалы и работы"".';uk='Не заповнена колонка ""Норматив"" в рядку %1 списку ""Матеріали і роботи"".'");
		Иначе
			ШаблонСообщенияКоличество = НСтр("ru='Необходимо заполнить колонку ""Норматив"" или ""Факт"" в строке %1 списка ""Материалы и работы"".';uk='Необхідно заповнити колонку ""Норматив"" або ""Факт"" в рядку %1 списку ""Матеріали і роботи"".'");
		КонецЕсли; 
	...
	ИначеЕсли НРег(ИмяТаблицы) = НРег("Трудозатраты") Тогда
		...
		Если Статус = Перечисления.СтатусыМаршрутныхЛистовПроизводства.Создан Тогда
			ШаблонСообщенияКоличество = НСтр("ru='Не заполнена колонка ""Норматив"" в строке %1 списка ""Трудозатраты"".';uk='Не заповнена колонка ""Норматив"" в рядку %1 списку ""Трудовитрати"".'");
		Иначе
			ШаблонСообщенияКоличество = НСтр("ru='Необходимо заполнить колонку ""Норматив"" или ""Факт"" в строке %1 списка ""Трудозатраты"".';uk='Необхідно заповнити колонку ""Норматив"" або ""Факт"" в рядку %1 списку ""Трудовитрати"".'");
		КонецЕсли; 
...
Процедура ПроверитьЗаполнениеДолиСтоимости(Отказ)
	...
	ШаблонСообщения = НСтр("ru='Не заполнена колонка ""Доля стоимости"" в строке %1 списка ""Побочный и промежуточный выпуск""';uk='Не заповнена колонка ""Частка вартості"" в рядку %1 списку ""Побічний і проміжний випуск""'");
	Для каждого СтрокаТаблицы Из ВыходныеИзделия Цикл
...
Процедура ОтключитьПроверкуПоУсловию(МассивНепроверяемыхРеквизитов)
	...
	Если УправлениеМаршрутнымиЛистами = Перечисления.УправлениеМаршрутнымиЛистами.РегистрацияОпераций Тогда
		МассивНепроверяемыхРеквизитов.Добавить("Операции.ВремяВыполнения");
		МассивНепроверяемыхРеквизитов.Добавить("Операции.ВремяВыполненияЕдИзм");
	КонецЕсли;
...
Процедура ПроверитьПериодВыполненияВведенныйВручную(Отказ)
	...
	ШаблонСообщенияНачало    = НСтр("ru='Не заполнена колонка ""Начало"" в строке %1 списка ""Операции"".';uk='Не заповнена колонка ""Початок"" в рядку %1 списку ""Операції"".'");
	ШаблонСообщенияОкончание = НСтр("ru='Не заполнена колонка ""Окончание"" в строке %1 списка ""Операции"".';uk='Не заповнена колонка ""Закінчення"" в рядку %1 списку ""Операції"".'");
...
Процедура ПроверитьДатыМаршрута(Отказ)
	...
	Если Начало > Окончание И Начало <> '000101010000' И Окончание <> '000101010000' Тогда
		ТекстСообщения = НСтр("ru='Начало выполнения маршрутного листа должно быть раньше даты окончания выполнения маршрутного листа.';uk='Початок виконання маршрутного листа повинний бути раніше дати закінчення виконання маршрутного листа.'");
	...
	Если ДатаВыполненияРаботВПредварительномБуфере <> '000101010000'
		...
		ТекстСообщения = НСтр("ru='Дата готовности к работе ключевого рабочего центра должна быть раньше даты завершения работы ключевого рабочего центра.';uk='Дата готовності до роботи ключового робочого центру повинна бути раніше дати завершення роботи ключового робочого центру.'");
	...
	ИначеЕсли ФактическоеОкончаниеРаботыКлючевогоРабочегоЦентра <> '000101010000'
		...
		ТекстСообщения = НСтр("ru='Дата завершения работы ключевого рабочего центра должна быть раньше даты завершения выполнения маршрутного листа.';uk='Дата завершення роботи ключового робочого центру повинна бути раніше дати завершення виконання маршрутного листа.'");
...
Процедура ПроверитьОперацииВремяШтучноеИПЗ(Отказ)
	
	Если УправлениеМаршрутнымиЛистами = Перечисления.УправлениеМаршрутнымиЛистами.ПооперационноеПланирование Тогда
		
		ШаблонТекста = НСтр("ru='Не заполнена колонка %1 в строке %2 списка ""Операции""';uk='Не заповнена колонка %1 в рядку %2 списку ""Операції""'");
		
		Для ТекИндекс = 0 По Операции.Количество()-1 Цикл
			
			СтрокаОперация = Операции[ТекИндекс];
			
			Если НЕ ЗначениеЗаполнено(СтрокаОперация.ВремяВыполнения)
				И НЕ ЗначениеЗаполнено(СтрокаОперация.ВремяШтучное) Тогда
				
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонТекста, НСтр("ru='""Время штучное""';uk='""Час штучний""'"), СтрокаОперация.НомерСтроки);
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ТекстОшибки,
					ЭтотОбъект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Операции", СтрокаОперация.НомерСтроки, "ВремяШтучное"),
					,
					Отказ);
				
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(СтрокаОперация.ВремяВыполненияЕдИзм)
				И НЕ ЗначениеЗаполнено(СтрокаОперация.ВремяШтучноеЕдИзм) Тогда
				
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонТекста, НСтр("ru='""Время штучное ед. изм.""';uk='""Час штучний од. вим.""'"), СтрокаОперация.НомерСтроки);
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ТекстОшибки,
					ЭтотОбъект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Операции", СтрокаОперация.НомерСтроки, "ВремяШтучноеЕдИзм"),
					,
					Отказ);
			...
			Если НЕ ЗначениеЗаполнено(СтрокаОперация.ВремяВыполнения)
				И НЕ ЗначениеЗаполнено(СтрокаОперация.ВремяПЗ) Тогда
				
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонТекста, НСтр("ru='""Время ПЗ""';uk='""Час ПЗ""'"), СтрокаОперация.НомерСтроки);
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ТекстОшибки,
					ЭтотОбъект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Операции", СтрокаОперация.НомерСтроки, "ВремяПЗ"),
					,
					Отказ);
			...
			Если НЕ ЗначениеЗаполнено(СтрокаОперация.ВремяВыполненияЕдИзм)
				И НЕ ЗначениеЗаполнено(СтрокаОперация.ВремяПЗЕдИзм) Тогда
				
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонТекста, НСтр("ru='""Время ПЗ ед. изм.""';uk='""Час ПЗ од. вим.""'"), СтрокаОперация.НомерСтроки);
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ТекстОшибки,
					ЭтотОбъект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Операции", СтрокаОперация.НомерСтроки, "ВремяПЗЕдИзм"),
					,
					Отказ);
...
Процедура ПересчитатьНормативы(Коэффициент, ПересчитатьОтклонениеОтНормативов = Истина) Экспорт
	...
	ОперативныйУчетПроизводстваКлиентСервер.ПересчитатьКоличествоТабличнойЧасти(ВыходныеИзделия,  Коэффициент, Истина);
	ОперативныйУчетПроизводстваКлиентСервер.ПересчитатьКоличествоТабличнойЧасти(ВозвратныеОтходы, Коэффициент, Истина);
	ОперативныйУчетПроизводстваКлиентСервер.ПересчитатьКоличествоТабличнойЧасти(МатериалыИУслуги, Коэффициент, Истина);
	ОперативныйУчетПроизводстваКлиентСервер.ПересчитатьКоличествоТабличнойЧасти(Трудозатраты,     Коэффициент, Ложь);
	
	Если Статус = Перечисления.СтатусыМаршрутныхЛистовПроизводства.Создан Тогда
		
		ОперативныйУчетПроизводстваКлиентСервер.ПересчитатьКоличествоТабличнойЧасти(МатериалыИУслуги, 1, Истина, "Факт", "");
		ОперативныйУчетПроизводстваКлиентСервер.ПересчитатьКоличествоТабличнойЧасти(Трудозатраты,     1, Ложь,   "Факт", "");
...
Процедура ПересчитатьФакт(ИсходныйНорматив, ВариантПересчетаФакта) Экспорт
	...
	// Факт расчитывается так: Норматив * Коэффициент
	ОперативныйУчетПроизводстваКлиентСервер.ПересчитатьКоличествоТабличнойЧасти(ВыходныеИзделия,  КоэффициентВыход, Истина, "Факт", "");
	ОперативныйУчетПроизводстваКлиентСервер.ПересчитатьКоличествоТабличнойЧасти(ВозвратныеОтходы, КоэффициентВыход, Истина, "Факт", "");
	
	ОперативныйУчетПроизводстваКлиентСервер.ПересчитатьКоличествоТабличнойЧасти(МатериалыИУслуги, Коэффициент, Истина, "Факт", "");
	ОперативныйУчетПроизводстваКлиентСервер.ПересчитатьКоличествоТабличнойЧасти(Трудозатраты,     Коэффициент, Ложь,   "Факт", "");
	
КонецПроцедуры

Процедура ЗаполнитьНазначениеПроизводимыхВПроцессеПолуфабрикатов() Экспорт
...
Процедура ЗаполнитьСтатусПредварительныйБуфер()
	...
	Если ГотовоКРаботеКлючевогоРабочегоЦентра 
		...
		ЗатраченоВремени = РассчитатьЗатраченноеВремя(КлючевойРабочийЦентрИЕгоРасписание.НачалоРаботыКлючевогоРабочегоЦентра, 
									...
									ЕдиницаИзмеренияПредварительногоБуфера, 
									ДатаВыполненияРаботВПредварительномБуфере,
									ПараметрыПодразделения.ГрафикРаботы);
	Иначе
		ЗатраченоВремени = 0;
	КонецЕсли;
	...
	Если ЗавершеноВыполнениеМаршрутногоЛиста 
		...
		Иначе
			...
			Если НормативноеВремя > 86400 Тогда
				НормативноеВремя = КалендарныеГрафики.ПолучитьРазностьДатПоКалендарю(
												ПараметрыПодразделения.ГрафикРаботы,
												Начало,
												Окончание);
...
Функция РассчитатьЗатраченноеВремя(КонтрольнаяДата, НормативноеВремя, ЕдиницаИзмеренияВремени, ДатаВыполненияРабот, ГрафикРаботы)
	...
	Иначе
		ОстатокВремени = КалендарныеГрафики.ПолучитьРазностьДатПоКалендарю(ГрафикРаботы, ДатаВыполненияРабот, КонтрольнаяДата);
...
Процедура ПроверитьОперации(Отказ)
	...
	Если УправлениеМаршрутнымиЛистами = Перечисления.УправлениеМаршрутнымиЛистами.ПооперационноеПланирование
		...
		ШаблонСообщения = НСтр("ru='Не заполнена колонка %1 в строке %2 списка ""Операции"".';uk='Не заповнена колонка %1 в рядку %2 списку ""Операції"".'");
		НомерСтроки = 0;
		...
		Для каждого Строка Из Операции Цикл
			...
			Если ПараллельнаяЗагрузка И НЕ ЗначениеЗаполнено(Строка.Загрузка) Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонСообщения,
					НСтр("ru='""Загрузка (для единицы)""';uk='""Завантаження (для одиниці)""'"),
					НомерСтроки);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Поле,, Отказ);
			КонецЕсли;
