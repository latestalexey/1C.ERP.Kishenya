...
Процедура ДеревоКатегорийПроцентОтклоненияПриИзменении(Элемент)
	...
	Если ТекущаяСтрока.ПроцентОтклонения > 100 Тогда
		ТекстПредупреждения = НСтр("ru='Отклонение не может превышать 100%';uk='Відхилення не може перевищувати 100%'");
...
Процедура ДеревоКатегорийПередУдалением(Элемент, Отказ)
	...
	ПоказатьВопрос(
		...
		НСтр("ru='Удалить строку?';uk='Видалити рядок?'"), 
...
Процедура ЗаполнитьПоДаннымДругогоФормата(Команда)
	Если Объект.ТоварныеКатегории.Количество()>0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоДаннымДругогоФорматаЗавершение", ЭтотОбъект), НСтр("ru='Табличная часть содержит строки и будет перезаполнена';uk='Таблична частина містить рядки і буде перезаполнена'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
	КонецЕсли;
...
Процедура УстановитьПроцентОтклоненияВсемСтрокам(Команда)
	Процент = 0;
	ТекстЗаголовкаВводаЧисла = НСтр("ru='Введите % отклонения';uk='Введіть % відхилення'");
...
Процедура ДобавитьМарку(Команда)
	...
	Если ТекСтрока = Неопределено Тогда ПоказатьПредупреждение(Неопределено, НСтр("ru='Марку можно добавить только в подчинение какой-либо категории';uk='Марку можна додати тільки в підпорядкування якої-небудь категорії'"));
	Иначе
		ТипСтроки = 0;
		...
		Иначе
			ПоказатьПредупреждение(Неопределено, НСтр("ru='Марку можно добавить только в подчинение какой-либо категории';uk='Марку можна додати тільки в підпорядкування якої-небудь категорії'"));
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
    ...
    Если ВыбраннаяМарка<>Неопределено Тогда
        ...
        Если ТекСтрокаКоллекция<>Неопределено Тогда
            ...
            Если МаркаУжеДобавлена(СтрокаРодителя.ПолучитьИдентификатор(), ВыбраннаяМарка) Тогда
                ПоказатьПредупреждение(Неопределено, НСтр("ru='Марка уже добавлена в данной категории';uk='Марка вже додана в даній категорії'"));
            Иначе
...
Процедура ЗагрузитьКвотыИзФайла(Команда)
	...
	ПараметрыЗагрузки.Заголовок = НСтр("ru='Загрузка квот ассортимента из файла';uk='Завантаження квот асортименту з файлу'");
...
Функция НаличиеПлана()
	...
	Если ЗначениеЗаполнено(Объект.ОбъектПланирования) И ЗначениеЗаполнено(Объект.ДатаНачалаДействия) Тогда
		...
		Если ЕстьПлан Тогда
			ТекстПредупреждения = НСтр("ru='На %1 для выбранного формата уже существует документ установки квот.';uk='На %1 для обраного формату вже існує документ установки квот.'")
	...
	Для Каждого СтрокаИсточника ИЗ ИсточникРодитель.Строки Цикл
		...
		Если СтрокаИсточника.Марка <> Null Тогда
			...
			Иначе
				...
				СтрокаПриемника.Представление = НСтр("ru='Прочие марки';uk='Інші марки'");
			КонецЕсли;
		...
		Иначе
			...
			ТекстВида = НСтр("ru='%ОсновнойВид%';uk='%ОсновнойВид%'");
			...
			Если Не СтрокаИсточника.ЭтоГруппаВидов Тогда 
				...
				Если НайденныеДополнительныеВиды <> Неопределено Тогда
					ТекстВида = НСтр("ru='%ОсновнойВид% (а так же %ДополнительныеВиды%)';uk='%ОсновнойВид% (а також %ДополнительныеВиды%)'");
