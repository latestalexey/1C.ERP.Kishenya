...
Процедура ЗаполнитьТоварыПоОстаткамКОформлению(ДатаЗаполнения) Экспорт
	...
	Структура = Новый Структура("Организация, Партнер, Соглашение, Валюта, ЦенаВключаетНДС, Товары",
		Комиссионер,
		Организация,
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если Не Проведен
	 ...
	 ИЛИ ИзмениласьТЧТоварыОтносительноТЧВидыЗапасов(МенеджерВременныхТаблиц) Тогда
		...
		ДополнительныеСвойства.Вставить("ЭтоОтчетОПродажах", Истина);
		ВидыЗапасов.Очистить();
...
Процедура ДозаполнитьВидыЗапасов(ВременныеТаблицы)
	...
	ВидыЗапасов.Сортировать("АналитикаУчетаНоменклатуры, НомерГТД, Количество УБЫВ");
	...
	Для Каждого СтрокаОстатка Из ТаблицаОстатков Цикл
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			Если СтрокаОстатка.Количество >= Количество Тогда
				...
				СтрокаОстатка.Количество = СтрокаОстатка.Количество * СтрокаОстатка.Знак - СтрокаЗапасов.Количество;
