...
Процедура ЗаполнитьДокументНаОснованииЗаказаМатериаловВПроизводство(Знач ЗаказМатериалов)
	...
	//Заполнение табличной части.
	ТаблицаТовары = ЗапасыСервер.ТаблицаОстатковКЗаказу(ЗаказМатериалов);
	Товары.Загрузить(ТаблицаТовары);
	...
	Если Не ЭтоНовый() И ЗаказНеОплачивается Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Заказ поставщику оплачен. Нельзя устанавливать операцию %1';uk='Замовлення постачальнику оплачений. Не можна встановлювати операцію %1'"),
				ХозяйственнаяОперация);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				Текст,
				ЭтотОбъект,
...
Процедура ЗаполнитьДокументНаОснованииЗаказаПереработчику(Знач ЗаказПереработчику)
	...
	//Заполнение табличной части.
	ТаблицаТовары = ЗапасыСервер.ТаблицаОстатковКЗаказу(ЗаказПереработчику);
