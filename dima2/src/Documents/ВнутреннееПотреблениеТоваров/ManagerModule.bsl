...
//++ НЕ УТКА
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказНаРемонт) Тогда
		...
		Запрос.Текст = Запрос.Текст + 
		"ВЫБРАТЬ
		|	ЗаказНаРемонтМатериалыИРаботы.НомерСтроки,
		|	ЗаказНаРемонтМатериалыИРаботы.Упаковка,
		|	ЗаказНаРемонтРемонты.СтатьяРасходов,
		|	НЕОПРЕДЕЛЕНО КАК АналитикаРасходов,
		|	ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка) КАК ФизическоеЛицо,
		|	МАКСИМУМ(ЗаказНаРемонтМатериалыИРаботы.СтатусУказанияСерий) КАК СтатусУказанияСерий,
		|	ЗаказНаРемонтМатериалыИРаботы.Серия КАК Серия,
		...
		|	СУММА(НовыеСтроки.Количество) КАК Количество,
		|	НовыеСтроки.Номенклатура КАК Номенклатура,
		|	НовыеСтроки.КодСтроки КАК КодСтроки,
		|	НовыеСтроки.Характеристика КАК Характеристика,
		|	ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка) КАК Назначение,
		|	ЗаказНаРемонтМатериалыИРаботы.Ссылка КАК ЗаказНаВнутреннееПотребление
		|ИЗ
		|	Документ.ЗаказНаРемонт.МатериалыИРаботы КАК ЗаказНаРемонтМатериалыИРаботы
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втНовыеСтроки КАК НовыеСтроки
		|		ПО ЗаказНаРемонтМатериалыИРаботы.Номенклатура = НовыеСтроки.Номенклатура
		|			И ЗаказНаРемонтМатериалыИРаботы.Характеристика = НовыеСтроки.Характеристика
		|			И ЗаказНаРемонтМатериалыИРаботы.Серия = НовыеСтроки.Серия
		|			И ЗаказНаРемонтМатериалыИРаботы.Ссылка = НовыеСтроки.ЗаказНаВнутреннееПотребление
		|			И ЗаказНаРемонтМатериалыИРаботы.КодСтроки = НовыеСтроки.КодСтроки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаРемонт.Ремонты КАК ЗаказНаРемонтРемонты
		|		ПО ЗаказНаРемонтМатериалыИРаботы.Ссылка = ЗаказНаРемонтРемонты.Ссылка
		|			И ЗаказНаРемонтМатериалыИРаботы.КодРемонта = ЗаказНаРемонтРемонты.КодРемонта
		|
		|СГРУППИРОВАТЬ ПО
		|	ЗаказНаРемонтМатериалыИРаботы.НомерСтроки,
		|	НовыеСтроки.Характеристика,
		...
		|	НовыеСтроки.КодСтроки,
		|	ЗаказНаРемонтМатериалыИРаботы.Ссылка,
		|	ЗаказНаРемонтРемонты.СтатьяРасходов";
...
#Область ТекстСписаниеКомиссионныхТоваров // (Дт  :: Кт 0241)
	...
	|ВЫБРАТЬ //// Списание комиссионных (Дт  :: Кт 0241)
...
#Область ТекстСписаниеТоваровНаРасходы // (Дт 23, 9 :: Кт 28, 68ОК)
	ТекстСписаниеТоваровНаРасходы = "
	|ВЫБРАТЬ //// Списание на расходы товаров (Дт 23, 9 :: Кт 28, 68ОК)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
...
#Область ТекстСписаниеТоваровДавальца // (Дт  :: Кт 0221/023)
	ТекстСписаниеТоваровДавальца = "
	|ВЫБРАТЬ //// Списание материалов давальца (Дт  :: Кт 0221/023)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
...
#Область ТекстСписаниеТоваровКорректировкиНДСПартий // (Дт 23, 9 :: Кт 6435)
	ТекстСписаниеТоваровКорректировкиНДСПартий = "
	|ВЫБРАТЬ //// Условная продажа списание товаров (Дт 23, 9 :: Кт 6435)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	...
	|	
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.Расходы) КАК ВидСчетаДт,
	|	КорректировкиНДСПартий.СтатьяРасходов КАК АналитикаУчетаДт,
	|	Операция.Подразделение КАК МестоУчетаДт,
	|	
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	|	Операция.Подразделение КАК ПодразделениеДт,
	...
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДт,
	|	ВЫБОР КОГДА СтатьиСтроительства.Ссылка ЕСТЬ НЕ NULL
	|		ТОГДА КорректировкиНДСПартий.АналитикаРасходов
	|		ИНАЧЕ КорректировкиНДСПартий.Номенклатура
	|	КОНЕЦ КАК СубконтоДт1,
	|	КорректировкиНДСПартий.СтатьяРасходов КАК СубконтоДт2,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.Прочее) КАК СубконтоДт3, 
	|	
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	...
	|		ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиСтроительства
	|	ПО
	|		КорректировкиНДСПартий.СтатьяРасходов = СтатьиСтроительства.Ссылка
	|		И СтатьиСтроительства.ВариантРаспределенияРасходов = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиНаПрочиеАктивы
	|	ПО
	|		КорректировкиНДСПартий.СтатьяРасходов = СтатьиНаПрочиеАктивы.Ссылка
	|		И СтатьиНаПрочиеАктивы.ВариантРаспределенияРасходов = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПрочиеАктивы) 
	|	
	|ГДЕ
	|	Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию)
	|	И КорректировкиНДСПартий.СтатьяРасходов <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)
	|	И СтатьиНаПрочиеАктивы.Ссылка ЕСТЬ NULL
	|	И КорректировкиНДСПартий.ТипЗапасов <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|
	|";	
#КонецОбласти 

#Область ТекстСписаниеТоваровНаСтатьиАктивовПассивовКорректировкиНДСПартий // (Дт ХХ.ХХ :: Кт 6435)
	ТекстСписаниеТоваровНаСтатьиАктивовПассивовКорректировкиНДСПартий = "
	|ВЫБРАТЬ //// Условная продажа при списании на прочие активы (Дт ХХ.ХХ :: Кт 6435)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	ТаблицаТовары.НомерСтроки КАК ИдентификаторСтроки,
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		ВтСтрокиДляКорректировкиНДСПартий КАК Строки
	|	ПО
	|		Операция.Ссылка = Строки.Ссылка
	|		И Строки.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|
	...
	|		ПО (Операция.Ссылка = КорректировкиНДСПартий.Ссылка)
	|			И (Строки.АналитикаУчетаНоменклатуры = КорректировкиНДСПартий.АналитикаУчетаНоменклатуры)
	|			И (Строки.СтатьяРасходов = КорректировкиНДСПартий.СтатьяРасходов)
	|			И (Строки.АналитикаРасходов = КорректировкиНДСПартий.АналитикаРасходов)
	|			И (Строки.НалоговоеНазначение = КорректировкиНДСПартий.НалоговоеНазначение)
	|			И (Строки.КорНалоговоеНазначение = КорректировкиНДСПартий.КорНалоговоеНазначение)
	|
	|ГДЕ
	|	Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах)
	|	И Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию)
...
#Область ТекстПередачаВЭксплуатациюПогашениеСтоимостиМБПКорректировкиНДСПартий // (Дт 9, 23 :: Кт 6435)
	ТекстПередачаВЭксплуатациюПогашениеСтоимостиМБПКорректировкиНДСПартий = "
	|////////////////////////////////////////////////////////////////////////////////////////////////////
	|// ТекстПогашеннаяСтоимость                         // (Дт 9, 23 :: Кт 6435)
	|ВЫБРАТЬ
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДт,
	|	КорректировкиНДСПартий.СтатьяРасходов КАК СубконтоДт1,
	|	КорректировкиНДСПартий.Номенклатура КАК СубконтоДт2,
...
#КонецОбласти 
	...
	Возврат
		...
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстПогашениеСтоимости
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстПогашениеСтоимостиМБП
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстСписаниеТоваровКорректировкиНДСПартий
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстСписаниеТоваровНаСтатьиАктивовПассивовКорректировкиНДСПартий
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстПередачаВЭксплуатациюПогашениеСтоимостиМБПКорректировкиНДСПартий
		+ "";
...
Функция ПеренестиСерии10ВТЧТовары_ПроверитьДанныеОбновлены(Данные, МетаданныеИОтбор) Экспорт
	
	Если МетаданныеИОтбор.Метаданные = Метаданные.НайтиПоПолномуИмени(ПолноеИмяОбъекта()) Тогда
		Возврат ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы(Данные, МетаданныеИОтбор);
	КонецЕсли;
...
//++ НЕ УТ
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "МШ2") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "МШ2", НСтр("ru='Ведомость МШ-2';uk='Відомість МШ-2'"),
			ПечатьМШ2(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),
			,
			,
			,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "МШ7") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "МШ7", НСтр("ru='Ведомость МШ-7';uk='Відомість МШ-7'"),
			ПечатьМШ7(МассивОбъектов, ОбъектыПечати, ПараметрыВывода,ИмяМакета),
			,
			,
			,
			Ложь // ЭтоМногоязычнаяПечатнаяФорма
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ОЗ1") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ОЗ1", НСтр("ru='Форма ОЗ-1';uk='Форма ОЗ-1'"),
			ПечатьОЗ1(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),
			,
			,
			,
			Ложь // ЭтоМногоязычнаяПечатнаяФорма
...
//++ НЕ УТ
Функция ПечатьМШ2(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)	
	
	УстановитьПривилегированныйРежим(Истина);

	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.ОриентацияСтраницы = ОриентацияСтраницы.Ландшафт;
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ВнутреннееПотреблениеТоваров_МШ2";

	// Вывод заголовка
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ВнутреннееПотреблениеТоваров.ПФ_MXL_UK_МШ2");
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл	
		...
		|	ВнутреннееПотреблениеТоваров.Организация.НаименованиеПолное КАК ПредставлениеОрганизации,
		|	ВнутреннееПотреблениеТоваров.Склад                          КАК Склад,
		|	ВнутреннееПотреблениеТоваров.Организация.КодПоЕДРПОУ    	КАК КодПоЕДРПОУ,
		|	ВнутреннееПотреблениеТоваров.Подразделение			 	  	КАК ПредставлениеПодразделения
		|ИЗ
		|	Документ.ВнутреннееПотреблениеТоваров КАК ВнутреннееПотреблениеТоваров
		|
		|ГДЕ
		...
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Ссылка", Ссылка);
		Запрос.УстановитьПараметр("ВидТМЦ", Перечисления.ВидыТМЦВЭксплуатации.МалоценныйБыстроизнашивающийсяПредмет);
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВнутреннееПотреблениеТоваровТовары.Номенклатура,
		|	ВнутреннееПотреблениеТоваровТовары.Характеристика,
		|	ВнутреннееПотреблениеТоваровТовары.Номенклатура.Код КАК НоменклатурныйНомерКод,
		|	ВнутреннееПотреблениеТоваровТовары.Номенклатура.Артикул КАК НоменклатурныйНомерАртикул,
		|	ВнутреннееПотреблениеТоваровТовары.ФизическоеЛицо КАК Сотрудник,
		|	ВнутреннееПотреблениеТоваровТовары.КатегорияЭксплуатации.СрокЭксплуатации КАК СрокСлужбы,
		...
		|	Документ.ВнутреннееПотреблениеТоваров.Товары КАК ВнутреннееПотреблениеТоваровТовары
		|ГДЕ
		|	ВнутреннееПотреблениеТоваровТовары.КатегорияЭксплуатации.ВидТМЦ = &ВидТМЦ
		|	И ВнутреннееПотреблениеТоваровТовары.Ссылка = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ВнутреннееПотреблениеТоваровТовары.Номенклатура,
		|	ВнутреннееПотреблениеТоваровТовары.Характеристика,
		|	ВнутреннееПотреблениеТоваровТовары.ФизическоеЛицо,
		|	ВнутреннееПотреблениеТоваровТовары.КатегорияЭксплуатации.СрокЭксплуатации,
		|	ВнутреннееПотреблениеТоваровТовары.Номенклатура.Код,
		|	ВнутреннееПотреблениеТоваровТовары.Номенклатура.Артикул
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки
		|ИТОГИ ПО
	...
	Пока ВыборкаФизическоеЛицо.Следующий() Цикл
		// Обработка выборки ВыборкаФизическоеЛицо
		...
		ОбластьШапка.Параметры.Заполнить(ВыборкаФизическоеЛицо);
		...
		ОбластьШапка.Параметры.ТабельныйНомер	= "";
		
		ОсновныеСотрудникиФизическихЛиц = КадровыйУчет.ОсновныеСотрудникиФизическихЛиц(ВыборкаФизическоеЛицо.Сотрудник, Истина, Шапка.Организация, Шапка.Дата);
		Если ЗначениеЗаполнено(ОсновныеСотрудникиФизическихЛиц) Тогда
			Для каждого Строка Из ОсновныеСотрудникиФизическихЛиц Цикл
			    ДанныеФизЛица = КадровыйУчет.КадровыеДанныеСотрудников(Истина, Строка.Сотрудник, "Должность,ТабельныйНомер", Ссылка.Дата);			
			КонецЦикла;
			Для каждого СтрокаДанные Из ДанныеФизЛица Цикл			
				ОбластьШапка.Параметры.Должность		= СтрокаДанные.Должность;
				ОбластьШапка.Параметры.Сотрудник		= СтрокаДанные.ФизическоеЛицо;
				ОбластьШапка.Параметры.ТабельныйНомер	= СтрокаДанные.ТабельныйНомер;
			КонецЦикла;
		КонецЕсли;
		
		ШтрихкодированиеПечатныхФорм.ВывестиШтрихкодВТабличныйДокумент(ТабДокумент, Макет, ОбластьШапка, Ссылка);
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			// Обробка выборки ВыборкаДетальныеЗаписи
			ОбластьСтрока.Параметры.Заполнить(ВыборкаДетальныеЗаписи);
			
			Если ТоварКод = "Артикул" Тогда
				ОбластьСтрока.Параметры.НоменклатурныйНомер = ВыборкаДетальныеЗаписи.НоменклатурныйНомерАртикул;
			Иначе
				ОбластьСтрока.Параметры.НоменклатурныйНомер = ВыборкаДетальныеЗаписи.НоменклатурныйНомерКод;
			КонецЕсли;
			
			ОбластьСтрока.Параметры.Номенклатура = НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(
													ВыборкаДетальныеЗаписи.Номенклатура,
													ВыборкаДетальныеЗаписи.Характеристика);
			ОбластьСтрока.Параметры.ДатаДокумента = Шапка.Дата;
...
Функция ПечатьМШ7(МассивОбъектов, ОбъектыПечати, ПараметрыВывода, ИмяМакета)
	...
	|	Документ.Организация КАК Организация,
	|	Документ.Организация.Префикс КАК Префикс,
	|	""Передача в експлуатацію"" КАК КодОперации,
	|	Документ.Товары.(
...
Функция ПечатьОЗ1(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)	
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл	
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВнутреннееПотреблениеТоваров.Номер 	                      	КАК НомерАкта,
		|	ВнутреннееПотреблениеТоваров.Дата	                      	КАК ДатаАкта,
		|	ВнутреннееПотреблениеТоваров.Организация                    КАК Организация,
		|	ВнутреннееПотреблениеТоваров.Организация.НаименованиеПолное КАК ПредставлениеОрганизации,
		|	ВнутреннееПотреблениеТоваров.Склад                          КАК СдалоПодразделение,
		|	ВнутреннееПотреблениеТоваров.Организация.КодПоЕДРПОУ    	КАК КодПоЕДРПОУ,
		|	ВнутреннееПотреблениеТоваров.Подразделение			 	  	КАК ПринялоПодразделение,
		|	""Передача в експлуатацію"" 								КАК ВидОперации
		|ИЗ
		|	Документ.ВнутреннееПотреблениеТоваров КАК ВнутреннееПотреблениеТоваров
		|
		|ГДЕ
		...
		|	ВнутреннееПотреблениеТоваровТовары.Характеристика,
		|	ВнутреннееПотреблениеТоваровТовары.ФизическоеЛицо КАК Сотрудник,
		|	ВнутреннееПотреблениеТоваровТовары.КатегорияЭксплуатации.СчетУчета КАК СчетДт,
		|	СУММА(ВнутреннееПотреблениеТоваровТовары.Количество) КАК Количество,
		|	МИНИМУМ(ВнутреннееПотреблениеТоваровТовары.НомерСтроки) КАК НомерСтроки
		|ИЗ
		|	Документ.ВнутреннееПотреблениеТоваров.Товары КАК ВнутреннееПотреблениеТоваровТовары
		|ГДЕ
		|	ВнутреннееПотреблениеТоваровТовары.КатегорияЭксплуатации.ВидТМЦ В(&ВидТМЦ)
		|	И ВнутреннееПотреблениеТоваровТовары.Ссылка = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ВнутреннееПотреблениеТоваровТовары.Номенклатура,
		|	ВнутреннееПотреблениеТоваровТовары.Характеристика,
		|	ВнутреннееПотреблениеТоваровТовары.ФизическоеЛицо,
		|	ВнутреннееПотреблениеТоваровТовары.КатегорияЭксплуатации.СчетУчета
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки";
		...
		Пока Выборка.Следующий() Цикл
			...
			ОсновныеСотрудникиФизическихЛицПринял = КадровыйУчет.ОсновныеСотрудникиФизическихЛиц(Выборка.Сотрудник, Истина, Ссылка.Организация, Ссылка.Дата);
