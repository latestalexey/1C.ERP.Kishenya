...
Процедура ОбновитьСписок()
	...
	Пока ВыборкаТоварнаяКатегория.Следующий() Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Если СтруктураРеквизитов.Свойство(НоваяСтрокаРеквизит.Реквизит) Тогда
...
Функция СоздатьДокументыБлокировкиНормативовНаСервере(ДатаНачалаДействия)
	...
	Для каждого ВыбраннаяСтрока Из Элементы.Нормативы.ВыделенныеСтроки Цикл
		...
		Если ЗначениеЗаполнено(Норматив.Период) И Норматив.Период = ДатаНачалаДействия Тогда
			...
			Объект.Комментарий = НСтр("ru='Блокировка установлена следующей датой т.к. совпадает с датой начала действия норматива.';uk='Блокування встановлене наступною датою т. к. збігається з датою початку дії нормативу.'");
		Иначе
