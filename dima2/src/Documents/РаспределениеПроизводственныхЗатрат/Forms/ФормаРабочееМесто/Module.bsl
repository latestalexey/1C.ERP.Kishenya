...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_РаспределениеПроизводственныхЗатрат" Тогда
		...
		ТекущаяСтрока.Документ             = Параметр.Ссылка;
...
Процедура КомандаРасшифроватьПоле(Команда)
	...
	Если НЕ РасшифроватьПоле(ТекущийЭлементЗатраты.Имя) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Расшифровка выбранного поля не предусмотрена.';uk='Розшифровка вибраного поля не передбачена.'"));
	КонецЕсли;
...
&НаКлиенте
Процедура СписатьЗатраты(Команда)
	ОткрытьФорму("Документ.СписаниеЗатратНаВыпуск.Форма.ФормаСпискаДокументов", ,ЭтаФорма);
КонецПроцедуры
...
Процедура ЗаполнитьЗатраты()
	...
	Элементы.ЗатратыОрганизация.Видимость = НЕ ЗначениеЗаполнено(Организация);
...
Процедура ЗаполнитьФактическийОстатокПоУчетномуСервер()
	...
	Для Каждого Строка ИЗ ВыделенныеСтроки Цикл
		...
		СтруктураДанных.Вставить("СтатусУказанияСерий", ДанныеСтроки.СтатусУказанияСерий);
...
Функция СоздатьНовыйДокумент(СтруктураРеквизитов)
	...
	ЗаполнитьЗначенияСвойств(ДокументОбъект, 
									СтруктураРеквизитов,
									"Организация, Номенклатура, Характеристика, Серия, СтатусУказанияСерий, Подразделение, Назначение");
...
Процедура ЗатратыСохранитьФактОстаток(ТекущаяСтрока)
	...
	СтруктураДанных.Вставить("СтатусУказанияСерий",	ТекущаяСтрока.СтатусУказанияСерий);
...
Функция РасшифроватьПоле(ИмяПоля, СтандартнаяОбработка = Ложь)
	...
	ИначеЕсли ИмяПоля = "ЗатратыКРаспределению" Тогда
...
&НаКлиенте
Процедура РасшифровкаЗатратыПоступило()
	...
	ПараметрыФормыРасшифровки.Вставить("Период", Новый СтандартныйПериод(НачалоМесяца(Период), КонецМесяца(Период)));
...
&НаКлиенте
Процедура РасшифровкаРасходовНаПроизводство()
