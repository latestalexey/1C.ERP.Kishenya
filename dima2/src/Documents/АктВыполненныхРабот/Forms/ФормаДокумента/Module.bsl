	...
	Если Объект.АктПоЗаказам Тогда
		...
		Если ИспользоватьАктыВыполненныхРаботПоНесколькимЗаказам Тогда
			...
			Если Объект.Услуги.Количество() = 0 Тогда
				ТекстВопроса = НСтр("ru='Список Услуги будет перезаполнен остатками неоказанных услуг по заказам. Продолжить?';uk='Список Послуги буде перезаполнен залишками ненаданих послуг по замовленнях. Продовжити?'");
			Иначе
...
Процедура АктВыполненныхРаботПоЗаказамПриИзмененииПослеВыбора(Результат, ДополнительныеПараметры) Экспорт
    ...
    Если Объект.Услуги.Количество() = 0 Тогда
        ТекстВопроса = СтрЗаменить(НСтр("ru='Список Услуги будет перезаполнен остатками неоформленных услуг по документу %ЗаказКлиента%. Продолжить?';uk='Список Послуги буде перезаполнен залишками неоформлених послуг за документом %ЗаказКлиента%. Продовжити?'"), "%ЗаказКлиента%", ЗаказКлиента);
    Иначе
