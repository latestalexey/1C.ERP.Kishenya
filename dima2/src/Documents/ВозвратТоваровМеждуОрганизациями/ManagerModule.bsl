...
#Область ТекстСторноНачисленияНДСПолучателя                // (Дт 70 :: Кт 643)
	...
	|ГДЕ
	|	Строки.ВидЗапасовПолучателя.ТипЗапасов В (ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар), ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар))
...
#Область ТекстСторноСебестоимостиРеализованногоТовара      // (сторно Дт 90 :: Кт 28)
...
#Область ТекстВозвратПроданногоКомиссионногоТовара         // (Дт :: Кт 0241 сторно)
	ТекстВозвратПроданногоКомиссионногоТовара = "
	|ВЫБРАТЬ //// Возврат переданного комиссионного товара (Дт :: Кт 0241 сторно)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.ОрганизацияПолучатель КАК Организация,
...
#Область ТекстОтправительСебестоимость                     // (Дт 63 :: Кт 28)
	...
	|ВЫБРАТЬ //// Себестоимости реализованного товара (сторно) (Дт 63 :: Кт 28)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	...
	|		ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаБезНДСРегл, Строки.СуммаСНДС - Строки.СуммаНДС)
	|	ИНАЧЕ
	|		ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаБезНДСРегл + СуммыДокументовВВалютеРегл.СуммаНДСРегл, Строки.СуммаСНДС)
	|	КОНЕЦ КАК Сумма,
	|
	...
	|
	|	ВЫБОР КОГДА (НЕ Строки.ЦелевоеНалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая)) ТОГДА
	|		Строки.СуммаСНДС - Строки.СуммаНДС
	|	ИНАЧЕ
	|		Строки.СуммаСНДС
...
#Область ТекстОтправительНДС                               // (Дт 6442 :: Кт 63)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговый кредит неподтвержденный: сторно"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстСторноНДСПередачаНаКомисссию                 // (Дт 6431 :: Кт 6432)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: обязательства, товары (сторно)"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстСторноНДСПриемНаКомисссию                    // (Дт 6442 :: Кт 6441)
	ТекстСторноНДСПриемНаКомисссию = "
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговый кредит: поступление (сторно)"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#КонецОбласти 
	...
	Возврат ТекстВозвратОтКомиссионера
			...
			+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстСторноНДСПриемНаКомисссию
			;
...
Функция СформироватьПечатнуюФормуВозвратПоставщику(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Пока ВыборкаПоТоварам.Следующий() Цикл
			...
			) + ?(ВыборкаПоТоварам.ЭтоВозвратнаяТара, НСтр("ru=' (возвратная тара)';uk=' (зворотна тара)'"), "");
			ТабличныйДокумент.Присоединить(ОбластьТовар);
...
#Область ОбновитьСтатусыОтраженияВРеглУчетеДокументовСНеколькимиОрганизациями
