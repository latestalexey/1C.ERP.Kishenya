...
Процедура СписокПолучателейПисьмаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.СтатусПисьма <> ПредопределенноеЗначение("Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Отправлено") Тогда
		...
		Если Элемент.Имя = "СписокПолучателейПисьма" Тогда
			ГруппаПодбора = НСтр("ru='Кому';uk='Кому'");
		ИначеЕсли Элемент.Имя = "СписокПолучателейКопий" Тогда
			ГруппаПодбора = НСтр("ru='Копии';uk='Копії'");
		ИначеЕсли Элемент.Имя = "СписокПолучателейСкрытыхКопий" Тогда
			ГруппаПодбора = НСтр("ru='Скрытые';uk='Сховані'");
		КонецЕсли;
...
Процедура ОтправитьПереслатьВыполнить(Команда)
	...
	Иначе
		...
		Если Объект.ПолучателиПисьма.Количество() = 0 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Необходимо указать хотя бы одного получателя письма.';uk='Необхідно вказати хоча б одного одержувача листа.'"),, "Объект.СписокПолучателейПисьма");
			Возврат;
...
Процедура УточнитьКонтакты(Команда)
	...
	Если Объект.СтатусПисьма <> ПредопределенноеЗначение("Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Отправлено") Тогда
		...
		РедактироватьСписокПолучателей(Истина, НСтр("ru='Копии';uk='Копії'"));
...
Процедура ПрикрепитьВходящееПисьмоОснованиеКакВложениеЕслиНеобходимо(ТекущийОбъект)
	...
	Если ТекущийОбъект.СтатусПисьма = Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Исходящее 
		...
		ИмяФайла = ВременныйКаталог + НСтр("ru='Пересылаемое сообщение';uk='Повідомлення, що пересилається'") + ".htm";
		...
		ПараметрыФайла.Вставить("ИмяБезРасширения", НСтр("ru='Пересылаемое сообщение';uk='Повідомлення, що пересилається'"));
...
Процедура СвойстваВложения(Команда)
	...
	Если ТекущиеДанные.Ссылка = Неопределено Тогда
		...
		ТекстВопроса = НСтр("ru='Свойства файла доступны только после его записи. Записать?';uk='Властивості файлу доступні тільки після його запису. Записати?'");
...
Функция СформироватьИсходящееОбычныйТекст(ВыборкаДанныеВходящегоПисьма, ТекущийОбъект)
	...
	СтрокаШапка = НСтр("ru='-----Пересылаемое сообщение-----';uk='-----Повідомлення, що пересилається-----'");
	
	СтрокаШапка = СтрокаШапка + Символы.ПС+ НСтр("ru='От';uk='Від'") + ": "+ ВыборкаДанныеВходящегоПисьма.ОтправительПредставление
		          + ?(ВыборкаДанныеВходящегоПисьма.ИмяОбъектаМетаданных = "ЭлектронноеПисьмоВходящее",
		          "[" + ВыборкаДанныеВходящегоПисьма.ОтправительАдрес +"]",
		          "");
		
	СтрокаШапка = СтрокаШапка + Символы.ПС+ НСтр("ru='Отправлено';uk='Відправлене'") + ": " 
	              + Формат(ВыборкаДанныеВходящегоПисьма.Дата,"ДЛФ=DT");
	
	СтрокаШапка = СтрокаШапка + Символы.ПС+ НСтр("ru='Кому';uk='Кому'") + ": " 
	...
	Если ТаблицаПолучателейКопий.Количество() > 0 Тогда
		СтрокаШапка = СтрокаШапка + Символы.ПС+ НСтр("ru='Копии';uk='Копії'") + ": "
		+ Взаимодействия.ПолучитьПредставленияПолучателейВходящегоПисьма(ТаблицаПолучателейКопий);
	КонецЕсли;
	
	СтрокаШапка = СтрокаШапка + Символы.ПС+ НСтр("ru='Тема';uk='Тема'") + ": " + ВыборкаДанныеВходящегоПисьма.Тема;
...
Процедура ОтправитьВыполнить()
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Выбранная учетная запись не доступна для отправки писем';uk='Вибраний обліковий запис не доступний для відправки листів'"),, "ОтправительПредставление", "Объект");
		Возврат;
	КонецЕсли;
	...
	Если НайденныеСтроки[0].УдалятьПослеОтправки Тогда
			...
			СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Отправить';uk='Відправити'"));
			СписокКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Отправить и сохранить';uk='Надіслати та зберегти'"));
			...
			ТекстВопроса = НСтр("ru='Для данной учетной записи не предусмотрено сохранение отправленных писем в системе.
                                |Продолжить?'
                                |;uk='Для даного облікового запису не передбачено збереження надісланих листів в системі.
                                |Продовжити?'");
			
			ОбработчикОповещенияЗакрытия = Новый ОписаниеОповещения("ВопросОНеСохраненииОтправленногоПисьма", ЭтотОбъект);
			ПоказатьВопрос(ОбработчикОповещенияЗакрытия,ТекстВопроса, СписокКнопок,, КодВозвратаДиалога.Да, НСтр("ru='Отправка письма';uk='Відправлення листа'"));
...
Процедура УстановитьЗаголовокКнопкиПоУмолчанию()
	...
	Если Объект.СтатусПисьма = Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Отправлено Тогда
		Элементы.Отправить.Заголовок = НСтр("ru='Переслать';uk='Переслати'");
	ИначеЕсли Объект.СтатусПисьма = Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Исходящее Тогда
		Элементы.Отправить.Заголовок = НСтр("ru='Отправить сейчас';uk='Надіслати зараз'");
	ИначеЕсли Объект.СтатусПисьма = Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Черновик Тогда
		...
		Элементы.Отправить.Заголовок = НСтр("ru='Отправить';uk='Відправити'");
...
Процедура РедактироватьСписокПолучателей(ДляПодбора, ГруппаПодбора = "")
	...
	ПараметрыОткрытия.Вставить("ГруппаПоУмолчанию",?(ПустаяСтрока(ГруппаПодбора),НСтр("ru='Кому';uk='Кому'"), ГруппаПодбора));
