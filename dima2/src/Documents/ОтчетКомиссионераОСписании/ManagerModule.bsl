...
#Область ТекстСписаниеКомиссионныхТоваров // (Дт  :: Кт 0242)
	ТекстСписаниеКомиссионныхТоваров = "
	|ВЫБРАТЬ //// Списание комиссионных товаров (Дт  :: Кт 0242)
	|
...
#Область ТекстОтражениеВзаиморасчетовПоКомиссионномуТовару // (Дт 36 :: Кт 70) 
	...
	|ВЫБРАТЬ //// Взаиморасчеты по комиссионному товару (Дт 36 :: Кт 70)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
#Область ТекстСписаниеСебестоимостиТовара // (Дт 90 :: Кт 283)
...
#Область ТекстОтраженияНДССписанногоТовара // (Дт 70 :: Кт 643)
	ТекстОтраженияНДССписанногоТовара = "
	|ВЫБРАТЬ //// НДС списанного товара (Дт 70 :: Кт 643)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	Операция.СтатьяДоходов КАК ГруппаФинансовогоУчетаДт,
	|	Операция.Подразделение КАК МестоУчетаДт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	|	Операция.Подразделение КАК ПодразделениеДт,
	...
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДт,
	|
	|	Операция.СтатьяДоходов КАК СубконтоДт1,
	|	ВЫБОР КОГДА &ФормироватьВидыЗапасовПоГруппамФинансовогоУчета ТОГДА
	|		Строки.ВидЗапасов.ГруппаФинансовогоУчета
	|	ИНАЧЕ
	|		Аналитика.Номенклатура.ГруппаФинансовогоУчета
...
#КонецОбласти  
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстОтраженияНДССписанногоТовара
