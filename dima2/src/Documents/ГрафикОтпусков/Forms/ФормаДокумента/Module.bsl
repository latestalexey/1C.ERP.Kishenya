...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Объект.ДатаВычисленияОстатков = Объект.Дата;
...
Процедура СотрудникиДатаНачалаПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ДатаНачала) 
		...
		ТекущиеДанные.ДатаОкончания = ОстаткиОтпусковКлиентСервер.ДатаОкончанияОтпуска(ТекущиеДанные.Сотрудник, ТекущиеДанные.ДатаНачала, ТекущиеДанные.КоличествоДней, Истина);
...
Процедура ПроверитьПравильностьОформленныхОтпусков(Команда)
	...
	Иначе
		...
		ПараметрыОткрытия.Вставить("ДатаОстатков", Объект.ДатаВычисленияОстатков);
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	ПараметрыЗагрузки.Заголовок = НСтр("ru='Загрузка списка сотрудников из файла';uk='Завантаження списку співробітників з файлу'");
...
Процедура ЗагрузитьСотрудниковИзФайлаНаСервере(АдресЗагруженныхДанных)
	...
	ОстаткиОтпуска = ОстаткиОтпусков.ОстаткиОтпусков(ТаблицаДанных);
	
	Для Каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		
		СтрокаТаблицы.ФизическоеЛицо = СоответствиеФизЛиц[СтрокаТаблицы.Сотрудник];
		
		СтрокиОстаткиОтпуска = ОстаткиОтпуска.НайтиСтроки(Новый Структура("Сотрудник, ДатаНачалаПериода, ДатаОкончанияПериода", СтрокаТаблицы.Сотрудник, СтрокаТаблицы.ДатаНачала, СтрокаТаблицы.ДатаОкончания));
				
		Если СтрокиОстаткиОтпуска.Количество() > 0 Тогда
			
			СтрокаОстатка = СтрокиОстаткиОтпуска[0];
		
			СтрокаТаблицы.КоличествоДней = СтрокаОстатка.КоличествоДнейВПериоде;
	...
	Для каждого СтрокаТаблицыОтпусков Из ТаблицаОтпусков Цикл
		...
		Если СтрокиОтпусков.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТаблицыОтпусков.Перенос) Тогда
				ЗаполняемаяСтрока.ОписаниеПеренесенногоОтпуска = НСтр("ru='на период с';uk='на період з'") + " " + Формат(СтрокаТаблицыОтпусков.ДатаНачалаПеренесенногоОтпуска, "ДЛФ=D") + " " + НСтр("ru='по';uk='по'") + " " + Формат(СтрокаТаблицыОтпусков.ДатаОкончанияПеренесенногоОтпуска, "ДЛФ=D")
			КонецЕсли;
		КонецЕсли; 
...
Процедура ОбработатьТекущийОтпуск(ИдентификаторСтроки)
	...
	Если Модифицированность Или Не ЗначениеЗаполнено(ТекущиеДанные.Отпуск) Тогда
		Оповещение = Новый ОписаниеОповещения("ОбработатьТекущийОтпускЗавершение", ЭтотОбъект);
		ДобавитьОтпускКДокументу(Оповещение);
	Иначе 
		ОбработатьТекущийОтпускЗавершение(ТекущиеДанные, Неопределено);
	КонецЕсли;
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если Модифицированность Тогда
			...
			ТекстВопроса = НСтр("ru='Оформить перенос можно только после записи этого документа.
                                    |Записать этот документ?'
                                    |;uk='Оформити перенесення можна тільки після запису цього документа.
                                    |Записати цей документ?'");
	...
	Если ИндексСтрокиТекущегоОтпуска <> Неопределено Тогда
		...
		Для ИндексСтроки = ИндексСтрокиТекущегоОтпуска + 1 По СтрокиПоСотруднику.Количество() - 1 Цикл
			...
			Если ДанныеСтроки.ДатаНачала <= ДатаОкончания Тогда
				...
				ДанныеСтроки.ДатаОкончания = ОстаткиОтпусковКлиентСервер.ДатаОкончанияОтпуска(ДанныеСтроки.Сотрудник, ДанныеСтроки.ДатаНачала, ДанныеСтроки.КоличествоДней, Истина);
...
Функция ДанныеОРасхожденияхСУчетом()
	...
	Для каждого СтрокаТаблицыСотрудников Из ТаблицаСотрудников Цикл
		...
		Если СтрокиПоСотруднику.Количество() > 0 Тогда
			...
			ОстаткиОтпуска = ОстаткиОтпусков.ОстатокОтпускаСотрудникаНаДату(СтрокаТаблицыСотрудников.Сотрудник, Объект.ДатаВычисленияОстатков);
