...
Процедура СуммаДокументаПриИзмененииСервер()
	...
	Если (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПогашениеЗаймаСотрудником Или
		Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствПоЗаймамВыданным)
...
Процедура ТипСуммыКредитаДепозитаПриИзмененииНаСервере(Идентификатор)
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПогашениеЗаймаСотрудником Тогда
		...
		СтатьиДДС = ДенежныеСредстваСервер.СтатьиДДСЗаймовСотрудникуПоУмолчанию(ДатаАктуальности, , "ПоступлениеБезналичныхДенежныхСредств");
...
Процедура РаспределитьСуммуНаСервере()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПогашениеЗаймаСотрудником Тогда
		//++ НЕ УТ
		РасшифровкаПлатежа = ДенежныеСредстваСервер.ДанныеРасшифровкиПлатежаДокументаДДСПоДоговоруЗаймаСотруднику(Объект, Истина, Объект.Ссылка, "ПоступлениеБезналичныхДенежныхСредств");
		//-- НЕ УТ
	Иначе
		...
		Для Каждого Договор Из ДействующиеДоговоры Цикл
			...
			Для Каждого Строка Из ДанныеРасшифровки Цикл
				ЗаполнитьЗначенияСвойств(РасшифровкаПлатежа.Добавить(), Строка);
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
Процедура ПодтверждениеЗачисленияЗарплатыПриИзмененииНаСервере()
	...
	//++ НЕ УТ
	Объект.СуммаДокумента = ДенежныеСредстваСервер.СуммаНезачисленныхПлатежейПоПодтверждениюБанка(Объект.ПодтверждениеЗачисленияЗарплаты);
	//-- НЕ УТ
