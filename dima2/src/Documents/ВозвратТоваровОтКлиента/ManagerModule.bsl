...
#Область ТекстСторноПередачаНаСубкомиссию // (Дт 0242 :: Кт 0241)
...
#Область ТекстВозвратНДС // (Дт 704 :: Кт 643)
	ТекстСторноНДС = "
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: обязательства неподтвержденные, товары (сторно)"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстВычетИзДоходаОтРеализацииСобственногоИКомиссионногоТовара // (Дт 704 :: Кт 36)
	...
	|	0 КАК СуммаВРКт,
	|
	|	""Вычеты из доходов при возврате товара "" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
...
#Область ТекстВозвратТоваровДругогоКачества // (Дт 28 :: Кт 28)
...
#Область ТекстСторноПродажаКомиссионногоТовара // (Дт :: Кт 0241)
...
#Область ТекстВозвратПереданнойТары // (Дт 28 :: Кт 36, 37)
	...
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	-ЕСТЬNULL(Стоимости.Знак * Стоимости.Сумма, Строки.Знак * Строки.Сумма) КАК Сумма,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НаСкладе) КАК ВидСчетаДт,
	|	Строки.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчетаДт,
	|	Операция.Склад КАК МестоУчетаДт,
	...
	|ВЫБРАТЬ //// Отклонение в стоимости возвращамой тары на расходы (Дт 9 :: Кт 36, 37)
	|	
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	(Стоимости.Знак * Стоимости.Сумма) + ЕСТЬNULL(СуммыВВалютеРегл.СуммаБезНДСРегл + СуммыВВалютеРегл.СуммаНДСРегл, ТаблицаВидыЗапасов.СуммаСНДС) КАК Сумма,
	|
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Отклонение в стоимости возвращамой тары на расходы"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|	ПО
	|		Стоимости.Ссылка = ТаблицаВидыЗапасов.Ссылка
	|		И Стоимости.ИдентификаторСтроки = ТаблицаВидыЗапасов.ИдентификаторСтроки
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.СуммыДокументовВВалютеРегл КАК СуммыВВалютеРегл
	|	ПО
	|		ТаблицаВидыЗапасов.Ссылка = СуммыВВалютеРегл.Регистратор
	|		И ТаблицаВидыЗапасов.ИдентификаторСтроки = СуммыВВалютеРегл.ИдентификаторСтроки
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Расчеты КАК Расчеты
	|	ПО 
	...
	|			ЛОЖЬ
	|		КОНЕЦ)
	|	И ((Стоимости.Знак * Стоимости.Сумма) + ЕСТЬNULL(СуммыВВалютеРегл.СуммаБезНДСРегл + СуммыВВалютеРегл.СуммаНДСРегл, ТаблицаВидыЗапасов.СуммаСНДС)) > 0
	|";
...
#Область ТекстВосстановлениеАвансаОтКлиента // (Дт 681 :: Кт 36)
...
#Область ТекстСторноНДСПередачаНаКомисссию // (Дт 6431 :: Кт 6432)
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС: налоговые обязательства: сторно"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#КонецОбласти 
	...
	Возврат ТекстВозвратОтКомиссионера
		...
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВычетИзДоходаОтРеализацииСобственногоИКомиссионногоТовара
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстСторноНДС		
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстСторноСебестоимостиРеализованногоТовара
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВозвратТоваровДругогоКачества
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстСторноПродажаКомиссионногоТовара
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВозвратПереданнойТары
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстОтклонениеВСтоимостиТарыРасходы
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстОтклонениеВСтоимостиТарыДоходы
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВосстановлениеАвансаОтКлиента
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстСторноНДСПередачаНаКомисссию
		+ "";
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Списание комиссионного товара с раздела учета "Товары принятые на комиссию"
	|ВЫБРАТЬ
	|	3 КАК Порядок,
	|	ТаблицаВидыЗапасов.НомерСтроки КАК НомерСтрокиДокумента,
	|	&Период КАК Период,
	...
	|			И НЕ ТаблицаВидыЗапасов.ВидЗапасовОтгрузки.РеализацияЗапасовДругойОрганизации
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Заявление на возврат товаров';uk='Заяву на повернення товарів'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
...
Функция ДоступныеДляШаблоновПечатныеФормы() Экспорт
	...
	МассивДоступныхПечатныхФорм.Добавить(ШаблоныСообщенийСервер.СтруктураПараметровДоступнойПечатнойФормы(
	                                     "ЗаявлениеНаВозвратТоваровОтКлиента", НСтр("ru='Заявление на возврат товаров';uk='Заяву на повернення товарів'"), 
