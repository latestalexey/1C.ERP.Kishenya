...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Договор займа';uk='Договір позики'");
	...
	КомандаПечати.Представление = НСтр("ru='Отчет о погашении займа';uk='Звіт про погашення позики'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ДоговорЗайма") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			"ДоговорЗайма", 
			НСтр("ru='Договор займа';uk='Договір позики'"), 
			ДоговорыЗайма(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),
			,
			,
			,
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ГрафикПогашенияЗайма") Тогда
		...
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ДоговорЗаймаСотруднику") Тогда
			ДанныеПечатиОбъектов = Документы.ДоговорЗаймаСотруднику.ДанныеПечатиДоговоровЗайма(МассивОбъектов, КодЯзыкаПечать);
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ИзменениеУсловийДоговораЗаймаСотруднику") Тогда
			ДанныеПечатиОбъектов = Документы.ИзменениеУсловийДоговораЗаймаСотруднику.ДанныеПечатиДополнительногоСоглашения(МассивОбъектов, КодЯзыкаПечать);
		КонецЕсли;
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм, 
			"ГрафикПогашенияЗайма", 
			НСтр("ru='График погашения займа';uk='Графік погашення позики'"), 
			ГрафикПогашенияЗайма(МассивОбъектов, ОбъектыПечати, ДанныеПечатиОбъектов, ПараметрыВывода),
			,
			,
			,
			Истина); // ЭтоМногоязычнаяПечатнаяФорма
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ОтчетОПогашенииЗайма") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм, "ОтчетОПогашенииЗайма", НСтр("ru='Отчет о погашении займа';uk='Звіт про погашення позики'"), ОтчетОПогашенииЗайма(МассивОбъектов, ОбъектыПечати));
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КарточкаУчетаДоговораЗайма") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм, 
			"КарточкаУчетаДоговораЗайма", 
			НСтр("ru='Карточка учета договора займа';uk='Картка обліку договору позики'"), 
			КарточкаДоговораЗайма(МассивОбъектов, ОбъектыПечати, ДанныеПечатиОбъектов, ПараметрыВывода),
			,
			,
			,
			Истина); // ЭтоМногоязычнаяПечатнаяФорма
...
Функция ДоговорыЗайма(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	ДанныеПечатиОбъектов = ДанныеПечатиДоговоровЗайма(МассивОбъектов, КодЯзыкаПечать);
...
Функция ГрафикПогашенияЗайма(МассивОбъектов, ОбъектыПечати, ДанныеПечатиОбъектов, ПараметрыВывода)
	
	КодЯзыкаПечать = ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм;
	
	ДокументРезультат = Новый ТабличныйДокумент;
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ДоговорЗаймаСотруднику.ПФ_MXL_ГрафикПогашенияЗайма", КодЯзыкаПечать);
	ПервыйДокумент = Истина;
	Для Каждого ДокументСсылка Из МассивОбъектов Цикл
...
Функция ДанныеПечатиДоговоровЗайма(МассивОбъектов, КодЯзыкаПечать) Экспорт	
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеПечати.Вставить("ПредставлениеДокумента", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='к договору займа №%1 от %2';uk='до договору позики №%1 від %2'",КодЯзыкаПечать), 
		...
		ДанныеПечати.Вставить("ЮридическийАдресОрганизации", 
			УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(
				Выборка.Организация, Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации));
...
Функция ПараметрыПечатнойФормыДоговораЗайма() Экспорт
	...
	ПараметрыПечатнойФормы.Вставить("НаименованиеВалюты", "грн.");
...
Функция КарточкаДоговораЗайма(МассивОбъектов, ОбъектыПечати, ДанныеПечатиОбъектов, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ДоговорЗаймаСотруднику.ПФ_MXL_КарточкаУчетаДоговораЗайма", КодЯзыкаПечать);
	...
	Для Каждого ДокументСсылка Из МассивОбъектов Цикл
		...
		Если ДанныеПечати.РазмерПогашения > 0 Тогда
			Область.Параметры.НазваниеПлатежа = НСтр("ru='Размер погашения';uk='Розмір погашення'",КодЯзыкаПечать);
			Область.Параметры.РазмерПлатежа = ДанныеПечати.РазмерПогашения;
		Иначе
			Область.Параметры.НазваниеПлатежа = НСтр("ru='Размер платежа';uk='Розмір платежу'",КодЯзыкаПечать);
