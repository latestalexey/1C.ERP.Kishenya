...
#Область ТекстОприходованиеТоваровПоСебестоимости // (Дт 28 :: Кт 28, 68ОК)
...
#Область ТекстОприходованиеТоваровПоУказаннойСтоимости // (Дт 28 :: 28, 68ОК)
	ТекстОприходованиеТоваровПоУказаннойСтоимости = "
	|ВЫБРАТЬ ///// Оприходование товаров по указанной стоимости (Дт 28 :: 28, 68ОК)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
#Область ТекстОприходованиеТоваровОтклонениеВСтоимости // (Дт 9X :: Кт 28, 68ОК)
...
#Область ТекстОприходованиеТоваровОтклонениеВСтоимостиДоходы // (Дт 28, 68ОК :: Кт 7X)
	...
	|ВЫБРАТЬ //// Отклонение в стоимости собственных товаров (Дт 28, 68ОК :: Кт 7X)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеКт,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ВозмещениеРанееСписанныхАктивов) КАК СчетКт,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиДоходов.ПустаяСсылка) КАК СубконтоКт1,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Отклонение в стоимости собственных товаров в доходы"" КАК Содержание
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстЗапросаТаблицаОбеспечениеЗаказов(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаТовары.Характеристика КАК ХарактеристикаОприходование,
	|	&Склад          			 КАК Склад,
	|	ТаблицаТовары.Назначение     КАК Назначение,
	|	-ТаблицаТовары.Количество 	 КАК КЗаказу,
	|	ТаблицаТовары.Количество	 КАК НаличиеПодЗаказ
