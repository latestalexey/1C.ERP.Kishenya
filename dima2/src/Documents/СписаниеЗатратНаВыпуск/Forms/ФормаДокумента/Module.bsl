...
Процедура СпецификацияПриИзменении(Элемент)
	...
	Если СохраненноеНазначение <> Объект.Спецификация
		И (Объект.ВыходныеИзделия.Количество() > 0
		Или Объект.ВозвратныеОтходы.Количество() > 0) Тогда
		
		ТекстВопроса = НСтр("ru='Списки выходных изделий будут очищены. Продолжить?';uk='Списки вихідних виробів будуть очищені. Продовжити?'");
...
Процедура НазначениеПриИзменении(Элемент)
	...
	Если СохраненноеНазначение <> Объект.Назначение
		...
		ТекстВопроса = НСтр("ru='Строки продукции и материалов под назначение будут удалены. Продолжить?';uk='Рядки продукції і матеріалів під призначення будуть видалені. Продовжити?'");
...
Процедура ВыходныеИзделияПередУдалением(Элемент, Отказ)
	...
	Если Не РазвернутьВыходныеИзделия Тогда
		СтруктураОтбора = Новый Структура("Номенклатура, Характеристика, Подразделение, Серия, Назначение");
	Иначе
...
Процедура ЗаполнитьМатериалыИУслугиПоНормативам(Команда)
	...
	Если Объект.МатериалыИУслуги.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьМатериалыИУслугиПоСпецификацииЗавершение", ЭтотОбъект), НСтр("ru='Список ""Материалы и работы"" будет перезаполнен. Продолжить?';uk='Список ""Матеріали і роботи"" буде перезаполнен. Продовжити?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
...
Процедура ЗаполнитьМатериалыИУслугиПоСпецификацииФрагмент()
	...
	Если Объект.МатериалыИУслуги.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Отсутствуют данные для заполнения. Возможно, в ресурсной спецификации не указаны материалы и работы.';uk='Відсутні дані для заповнення. Можливо, у ресурсній специфікації не зазначені матеріали і роботи.'"));
	КонецЕсли;
...
Процедура ПодобратьИзОбособленныхОстатков(Команда)
	...
	СтруктураРеквизитов.Вставить("Назначение");
...
Процедура ЗаполнитьТрудозатратыПоНормативам(Команда)
	...
	Если Объект.Трудозатраты.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьТрудозатратыПоСпецификацииЗавершение", ЭтотОбъект), НСтр("ru='Список ""Трудозатраты"" будет перезаполнен. Продолжить?';uk='Список ""Робота"" буде перезаповнений. Продовжити?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
...
Процедура ЗаполнитьТрудозатратыПоНормативамФрагмент()
	...
	Если Объект.Трудозатраты.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Отсутствуют данные для заполнения. Возможно, в ресурсной спецификации не указаны виды работ.';uk='Відсутні дані для заповнення. Можливо, у ресурсній специфікації не зазначені види робіт.'"));
	КонецЕсли;
...
Процедура ПродукцияРазвернутьПоРаспоряжениям(Команда)
	...
	ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени(
		"Документ.СписаниеЗатратНаВыпуск.ФормаДокумента.Команда.ПродукцияРазвернутьПоРаспоряжениям");
...
Процедура ЗаполнитьВсеПоНормативам(Команда)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени(
		"Документ.СписаниеЗатратНаВыпуск.ФормаДокумента.Команда.ЗаполнитьВсеПоНормативам");
	...
	Если Объект.ВыходныеИзделия.Количество() > 0 Или
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьВсеПоНормативамЗавершение", ЭтотОбъект), НСтр("ru='Списки будут перезаполнены. Продолжить?';uk='Списки будуть переповнені сьогодні. Продовжити?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
...
Процедура ЗаполнитьВсеПоНормативамФрагмент()
	...
	Если Объект.ВыходныеИзделия.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Отсутствуют данные для заполнения. Возможно, выпуск продукции еще не оформлен или затраты на все выпуски уже списаны.';uk='Відсутні дані для заповнення. Можливо, випуск продукції ще не оформлений або витрати на всі випуски вже списані.'"));
	КонецЕсли;
...
Процедура ЗаполнитьВсеОбособленнымиОстатками(Команда)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени(
		"Документ.СписаниеЗатратНаВыпуск.ФормаДокумента.Команда.ЗаполнитьВсеОбособленнымиОстатками");
	...
	Если Объект.ВыходныеИзделия.Количество() > 0 Или
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьВсеОбособленнымиОстаткамиЗавершение", ЭтотОбъект), НСтр("ru='Списки будут перезаполнены. Продолжить?';uk='Списки будуть переповнені сьогодні. Продовжити?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
...
Процедура ЗаполнитьПродукцию(УчитыватьЛимиты)
	...
	Если Объект.ВыходныеИзделия.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПродукциюПоСпецификацииЗавершение", ЭтотОбъект, ДополнительныеПараметры),
			НСтр("ru='Список ""Продукция"" будет перезаполнен. Продолжить?';uk='Список ""Продукція"" буде перезаполнен. Продовжити?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
...
Процедура ЗаполнитьВозвратныеОтходы(УчитыватьЛимиты)
	...
	Если Объект.ВозвратныеОтходы.Количество() > 0 Тогда
		...
		ДополнительныеПараметры = Новый Структура("УчитыватьЛимиты", УчитыватьЛимиты);
			...
			НСтр("ru='Список ""Возвратные отходы"" будет перезаполнен. Продолжить?';uk='Список ""Зворотні відходи"" буде перезаполнен. Продовжити?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
...
Процедура ОбновитьТаблицуОбъекта(ИмяТЧ, Идентификатор)
	...
	Иначе
		Если ИмяТЧ = "ВыходныеИзделия" Тогда
			СтруктураОтбора = Новый Структура("Номенклатура, Характеристика, Подразделение, Серия, Назначение");
		Иначе
			СтруктураОтбора = Новый Структура("Номенклатура, Характеристика, Подразделение, Серия, Назначение, СтатьяКалькуляции");
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеТабличнойЧасти(ИмяТЧ, Отказ)
	...
	ПроверяемыеРеквизиты.Добавить("Номенклатура", НСтр("ru='Номенклатура';uk='Номенклатура'"));
	ПроверяемыеРеквизиты.Добавить("Характеристика", НСтр("ru='Характеристика';uk='Характеристика'"));
	ПроверяемыеРеквизиты.Добавить("КоличествоУпаковок", НСтр("ru='Количество';uk='Кількість'"));
	ПроверяемыеРеквизиты.Добавить("Подразделение", НСтр("ru='Подразделение';uk='Підрозділ'"));
	ПроверяемыеРеквизиты.Добавить("Распоряжение", НСтр("ru='Распоряжение';uk='Розпорядження'"));
	ПроверяемыеРеквизиты.Добавить("КодСтроки", НСтр("ru='Код строки';uk='Код рядка'"));
	Если ИмяТЧ = "ВыходныеИзделия" Тогда
	...
	Если ИмяТЧ = "ВыходныеИзделия" Тогда
		...
		ШаблонСообщенияБезНомера = НСтр("ru='Не заполнено поле %1 в списке Продукция';uk='Не заповнено поле %1 у списку Продукція'");
		ШаблонСообщенияДоляСтоимости = НСтр("ru='Ошибка заполнения доли стоимости в строке %1 списка Продукция.
            ...
            |;uk='Помилка заповнення частки вартості в рядку %1 списку Продукція.
            |Можливо, частка вартості занадто мала для розподілу по всіх випусках'");
	Иначе
		ШаблонСообщения = НСтр("ru='Не заполнено поле %1 в строке %2 списка Возвратные отходы';uk='Не заповнено поле %1 в рядку %2 списку Зворотні відходи'");
		ШаблонСообщенияБезНомера = НСтр("ru='Не заполнено поле %1 в списке Возвратные отходы';uk='Не заповнено поле %1 у списку Зворотні відходи'");
		ШаблонСообщенияДоляСтоимости = НСтр("ru='Ошибка заполнения доли стоимости в списке Продукция.
            |Возможно, доля стоимости слишком мала для распределения по всем выпускам'
            |;uk='Помилка заповнення частки вартості в списку Продукція.
            |Можливо, частка вартості занадто мала для розподілу по всіх випусках'");
	КонецЕсли;
	...
	Если ИмяТЧ = "ВыходныеИзделия" Тогда
		...
		Если Объект.ВыходныеИзделия.Количество() = 0 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не введено ни одной строки в список Выходные изделия.';uk='Не введено жодного рядка в список Вихідні вироби.'"),
				,
				"ВыходныеИзделия",
				,
				Отказ);
...
Процедура УстановитьУсловноеОформление()
	...
	// Указание доли стоимости не требуется, если спецификация не задана.
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Справочники.РесурсныеСпецификации.ПустаяСсылка();
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<для обособленных>';uk='<для відокремлених>'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<все материалы распределены>';uk='<всі матеріали розподілені>'"));
...
Процедура СвернутьРазвернутьТЧ(ИмяТЧ, Развернуть = Ложь)
	...
	Если Не Развернуть Тогда
		...
		ШаблонПредставления = НСтр("ru='Всего распоряжений: %1';uk='Всього розпоряджень: %1'");
...
Функция РеквизитыЗаполнены(МассивПроверок)
	...
	Если МассивПроверок.Найти("Номенклатура") <> Неопределено Тогда
		Если Не ЗначениеЗаполнено(Объект.Номенклатура) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не указано основная продукция, на которое списываются затраты.';uk='Не зазначена основна продукція, на який списуються витрати.'"),
			Объект.Ссылка,
			"Объект.Номенклатура");
	...
	Если МассивПроверок.Найти("Количество") <> Неопределено Тогда
		Если Не ЗначениеЗаполнено(Объект.Количество) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не указано количество продукции, на которое списываются затраты.';uk='Не зазначено кількість продукції, на яку списуються витрати.'"),
				Объект.Ссылка,
				"Объект.Количество");
	...
	Если МассивПроверок.Найти("Назначение") <> Неопределено Тогда
		Если Не ЗначениеЗаполнено(Объект.Назначение) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Поле ""Назначение"" не заполнено.';uk='Поле ""Призначення"" не заповнено.'"),
				Объект.Ссылка,
				"Объект.Назначение");
...
Процедура УстановитьПредставлениеДокумента()
	...
	Шаблоны.Добавить(НСтр("ru='Списание затрат на производство изделия <b>%1</b> в количестве <b>%2 %3</b> по спецификации <b>%4</b>. В том числе под назначение <b>%5</b> - <b>%6 %7</b>.';uk='Списання витрат на виробництво виробу <b>%1</b> у кількості <b>%2 %3</b> по специфікації <b>%4</b>. У тому числі під призначення <b>%5</b> - <b>%6 %7</b>.'"));
	Шаблоны.Добавить(НСтр("ru='Списание затрат на производство изделия <b>%1</b> в количестве <b>%2 %3</b> по спецификации <b>%4</b>, под назначение <b>%5</b>.';uk='Списання витрат на виробництво виробу <b>%1</b> у кількості <b>%2 %3</b> по специфікації <b>%4</b>, під призначення <b>%5</b>.'"));
	Шаблоны.Добавить(НСтр("ru='Списание затрат на производство изделия <b>%1</b> в количестве <b>%2 %3</b> по спецификации <b>%4</b>.';uk='Списання витрат на виробництво виробу <b>%1</b> у кількості <b>%2 %3</b> по специфікації <b>%4</b>.'"));
	Шаблоны.Добавить(НСтр("ru='Списание затрат на производство изделия <b>%1</b> в количестве <b>%2 %3</b>.';uk='Списання витрат на виробництво виробу <b>%1</b> у кількості <b>%2 %3</b>.'"));
	Шаблоны.Добавить(НСтр("ru='Списание затрат на производство изделия <b>%1</b> в количестве <b>%2 %3</b> под назначение <b>%4</b>.';uk='Списання витрат на виробництво виробу <b>%1</b> у кількості <b>%2 %3</b> під призначення <b>%4</b>.'"));
