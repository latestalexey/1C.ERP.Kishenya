...
Процедура ПоказателиПоказательОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.ИндексируемыеПоказатели.НайтиСтроки(Новый Структура("Показатель", ВыбранноеЗначение)).Количество() > 0 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Такой показатель уже выбран';uk='Такий показник вже вибрано'"));
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ ПоказателиПозиций
...
Процедура ДополнитьФормуПоказателиПозиций()
	...
	Для Каждого СтрокаПоказателя Из ТаблицаПоказателей Цикл
		...
		Если ФОИспользоватьВилкуСтавокВШтатномРасписании Тогда
			...
			НовыйПоказательТекущееЗначениеМин = Новый РеквизитФормы(СтрокаПоказателя.ПутьКДанным + "ТекущееЗначение" + "Мин", ТипЗначениеПоказателя, ИмяТЧПоказателиПозиций, СтрокаПоказателя.Заголовок + "(" + НСтр("ru='тек. значение';uk='тек. значення'") + ")");
			ДобавляемыеРеквизиты.Добавить(НовыйПоказательТекущееЗначениеМин);
			НовыйПоказательТекущееЗначениеМакс = Новый РеквизитФормы(СтрокаПоказателя.ПутьКДанным + "ТекущееЗначение" + "Макс", ТипЗначениеПоказателя, ИмяТЧПоказателиПозиций, СтрокаПоказателя.Заголовок + "(" + НСтр("ru='тек. значение';uk='тек. значення'") + ")");
		...
		Иначе
			...
			НовыйПоказательТекущееЗначение = Новый РеквизитФормы(СтрокаПоказателя.ПутьКДанным + "ТекущееЗначение", ТипЗначениеПоказателя, ИмяТЧПоказателиПозиций, СтрокаПоказателя.Заголовок + "(" + НСтр("ru='тек. значение';uk='тек. значення'") + ")");
...
Процедура ДобавитьЭлементыПоказателя(СтрокаПоказателя, ГруппаПоказателиПозиций)
	...
	Если ФОИспользоватьВилкуСтавокВШтатномРасписании Тогда
		ГруппаЗначенияПоказателя.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1(мин) / %1(макс)';uk='%1(мін) / %1(макс)'"), СтрокаПоказателя.Заголовок);
...
Процедура ОбновитьЗаголовокГруппыФОТ(Форма, ГруппаФОТ)
	...
	Если ФОИспользоватьВилкуСтавокВШтатномРасписании Тогда		
		ГруппаФОТ.Заголовок = НСтр("ru='ФОТ(мин) / ФОТ(макс)';uk='ФОП(мін) / ФОП(макс)'");
	Иначе
		ГруппаФОТ.Заголовок = НСтр("ru='ФОТ';uk='ФОП'");
	КонецЕсли;
				...
				Если НЕ ТекущиеЗначенияПоказателей = Неопределено Тогда
					...
					Если ПоказательИспользуется Тогда
						...
						Если ФОИспользоватьВилкуСтавокВШтатномРасписании Тогда
							...
							ПредставлениеТекущегоМин = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='было: %1';uk='було: %1'"), СуммаПодстановкиМин);
							...
							ПредставлениеТекущегоМакс = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='было: %1';uk='було: %1'"), СуммаПодстановкиМакс);
...
Функция КлючевыеРеквизитыЗаполненияФормыОписаниеКлючевыхРеквизитов() Экспорт
	...
	Массив.Добавить(Новый Структура("ЭлементФормы, Представление", "МесяцСтрокой", 		НСтр("ru='месяца начисления';uk='місяці нарахування'")));
	Возврат Массив
КонецФункции
...
Функция АдресВХранилищеНачисленийИУдержаний(ИдентификаторСтроки)
	...
	ДанныеПозиции = УправлениеШтатнымРасписанием.ДанныеПозицииШтатногоРасписания(ПозицияШтатногоРасписания, Объект.МесяцИндексации, Ложь);
	...
	ПараметрыОткрытия.Подразделение = ДанныеПозиции["Подразделение"];;
	ПараметрыОткрытия.ГрафикРаботы = ДанныеПозиции["ГрафикРаботыСотрудников"];;
