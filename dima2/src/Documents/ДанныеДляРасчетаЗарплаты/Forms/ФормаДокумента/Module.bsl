...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НастройкиДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты 
		Или НастройкиДокумента.ВидыРаботЗаполняютсяВДокументе Тогда
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для	Каждого КлючИЗначение Из ПроверяемыеПоля Цикл
		...
		Если ЧастиПутиКРеквизиту.Количество() > 1 И ЧастиПутиКРеквизиту[0] = "Объект" 
			...
			Если Не ЗначениеЗаполнено(ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(ЭтаФорма, ПутьКРеквизиту)) Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Поле ""%1"" не заполнено';uk='Поле ""%1"" не заповнене'"), Элементы[ИмяПоляФормы].Заголовок);
		...
		Иначе
			...
			Для Каждого СтрокаКоллекции Из ЭтаФорма[ИмяТаблицы] Цикл
				Если Не ЗначениеЗаполнено(СтрокаКоллекции[ИмяРеквизита]) Тогда
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Поле ""%1"" не заполнено';uk='Поле ""%1"" не заповнене'"), Элементы[ИмяПоляФормы].Заголовок);
					Поле = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						"%1[%2].%3", ИмяТаблицы, ЭтаФорма[ИмяТаблицы].Индекс(СтрокаКоллекции), ИмяРеквизита);
	...
	Если НастройкиДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты И
		...
		Для Каждого СтрокаКоллекции Из ДанныеСводно Цикл
			...
			Если НачалоМесяца(СтрокаКоллекции.Период) <> НачалоМесяца(Период) Тогда
				...
				НСтр("ru='В таблице обнаружены даты, относящиеся к разным месяцам';uk='В таблиці виявлені дати, що відносяться до різних місяців'"), , "ДанныеСводно", , Отказ);
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Если НастройкиДокумента.ИспользоватьПериодОкончания Тогда
		ТекстСообщения = НСтр("ru='Дата окончания меньше даты начала';uk='Дата закінчення менше дати початку'");
		Если НастройкиДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты Тогда
			Для Каждого СтрокаКоллекции Из ДанныеСводно Цикл
				Если ЗначениеЗаполнено(СтрокаКоллекции.ПериодОкончания) И СтрокаКоллекции.ПериодОкончания < СтрокаКоллекции.Период Тогда
					Поле = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						"ДанныеСводно[%1].%2",
						ДанныеСводно.Индекс(СтрокаКоллекции),
						"ПериодОкончания");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , Поле, , Отказ);
				КонецЕсли;
			КонецЦикла;
		Иначе
			Если ЗначениеЗаполнено(Объект.ПериодОкончания) И Объект.ПериодОкончания < Объект.Период Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Объект.ПериодОкончания", , Отказ);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииДатыДокументаНаСервере()
	...
	ЗаполнитьРасценкуРабот();
	РассчитатьСтоимостьРаботПоВсемСтрокам();
...
Процедура ПериодРегистрацииСтрокойПриИзменении(Элемент)
	...
	ЗарплатаКадрыКлиент.ВводМесяцаПриИзменении(ЭтаФорма, "Объект.Период", "ПериодРегистрацииСтрокой", Модифицированность);
...
Процедура ПериодРегистрацииСтрокойНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗарплатаКадрыКлиент.ВводМесяцаНачалоВыбора(ЭтаФорма, ЭтаФорма, "Объект.Период", "ПериодРегистрацииСтрокой", , Оповещение);
...
Процедура ПериодРегистрацииСтрокойРегулирование(Элемент, Направление, СтандартнаяОбработка)
	...
	ЗарплатаКадрыКлиент.ВводМесяцаРегулирование(ЭтаФорма, "Объект.Период", "ПериодРегистрацииСтрокой", Направление, Модифицированность);
...
Процедура ДанныеДетальноЗначениеНаДеньПриИзменении(Элемент)
	...
	ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеДетально, ИмяРеквизита + "_ЗначениеНаДень_" + ЧастиИмениПоля[3]);
	ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеДетально, ИмяРеквизита + "_КоличествоЗначений");
	ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеДетально, ИмяРеквизита + "_СуммаЗначений");
...
Процедура Подключаемый_ДанныеСводноКолонкаПриИзменении(Элемент)
	...
	ЗаполнитьИтогоПоКолонке(ЭтаФорма, ЭтаФорма[ИмяТаблицы], ИмяРеквизита);
...
Процедура ДанныеСводноПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Для Каждого СведенияОПоказателе Из ПоказателиРасчетаЗарплаты Цикл
		Если Не СведенияОПоказателе.ВводитсяДетально И СведенияОПоказателе.ВводитсяВТаблице Тогда
			ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеСводно, СведенияОПоказателе.ИмяРеквизита);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СведенияВидаРабот Из ВидыРабот Цикл
		Если Не СведенияВидаРабот.ВводитсяДетально И СведенияВидаРабот.ВводитсяВТаблице Тогда
			ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеСводно, СведенияВидаРабот.ИмяРеквизита);
		КонецЕсли;
	КонецЦикла;
...
Процедура ВыполненныеРаботыРасценкаПриИзменении(Элемент)
	РассчитатьСтоимостьРаботПоСтроке(ЭтаФорма, Элементы.ВыполненныеРаботы.ТекущаяСтрока);
КонецПроцедуры

&НаКлиенте
Процедура ВыполненныеРаботыСуммаПриИзменении(Элемент)
	РассчитатьОбъемВыполненныхРабот(ЭтаФорма);
	РаспределитьСтоимостьРаботПоСотрудникам();
КонецПроцедуры
...
Процедура ИсполнителиРаботТарифнаяСтавкаПриИзменении(Элемент)
	Элементы.ИсполнителиРабот.ТекущиеДанные.ФиксТарифнаяСтавка = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ИсполнителиРаботОтработаноДнейПриИзменении(Элемент)
	Элементы.ИсполнителиРабот.ТекущиеДанные.ФиксОтработанноеВремя = Истина;
КонецПроцедуры
...
Процедура Заполнить(Команда)
	...
	ЗадаватьВопрос = ДанныеДетально.Количество() > 0 Или ДанныеСводно.Количество() > 0;
	
	Если ЗадаватьВопрос Тогда
		
		ТекстВопроса = НСтр("ru='Таблица документа будет очищена. 
                             |Продолжить?'
                             |;uk='Таблиця документа буде очищена. 
                             |Продовжити?'");
...
Процедура ОбновитьИтогиДанныеДетально()
	...
	Для Каждого СведенияОПоказателе Из ПоказателиРасчетаЗарплаты Цикл
		Если Не СведенияОПоказателе.ВводитсяДетально Тогда
			ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеДетально, СведенияОПоказателе.ИмяРеквизита);
			Продолжить;
		КонецЕсли;
		// - по количеству
		ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеДетально, СведенияОПоказателе.ИмяРеквизита + "_КоличествоЗначений");
		// - по сумме значений
		ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеДетально, СведенияОПоказателе.ИмяРеквизита + "_СуммаЗначений");
		// - Для колонки на каждый день.
		Для НомерДня = 1 По 31 Цикл
			ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеДетально, СведенияОПоказателе.ИмяРеквизита + "_ЗначениеНаДень_" + НомерДня);
		КонецЦикла;
	КонецЦикла;
	...
	Для Каждого СведенияОВидеВремени Из ВидыВремени Цикл
		...
		КонецЕсли;
		// - по количеству
		ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеДетально, СведенияОВидеВремени.ИмяРеквизита + "_КоличествоЗначений");
		// - по сумме значений
		ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеДетально, СведенияОВидеВремени.ИмяРеквизита + "_СуммаЗначений");
		// - Для колонки на каждый день.
		Для НомерДня = 1 По 31 Цикл
			ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеДетально, СведенияОВидеВремени.ИмяРеквизита + "_ЗначениеНаДень_" + НомерДня);
		КонецЦикла;
	КонецЦикла;
	...
	Для Каждого СведенияВидаРабот Из ВидыРабот Цикл
		...
		КонецЕсли;
		// - по количеству
		ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеДетально, СведенияВидаРабот.ИмяРеквизита + "_КоличествоЗначений");
		// - по сумме значений
		ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеДетально, СведенияВидаРабот.ИмяРеквизита + "_СуммаЗначений");
		// - Для колонки на каждый день.
		Для НомерДня = 1 По 31 Цикл
			ЗаполнитьИтогоПоКолонке(ЭтаФорма, ДанныеДетально, СведенияВидаРабот.ИмяРеквизита + "_ЗначениеНаДень_" + НомерДня);
		КонецЦикла;
	КонецЦикла;
...
Процедура ПриПолученииДанныхНаСервере(ТекущийОбъект = Неопределено)
	...
	ЗарплатаКадрыКлиентСервер.ЗаполнитьМесяцПоДате(ЭтаФорма, "Объект.Период", "ПериодРегистрацииСтрокой");	
...
Процедура НастроитьЭлементыВвода()
	...
	// В процедуре выполняется подготовка формы для ввода данных на основании вида документ.
	
	Если НастройкиДокумента <> Неопределено Тогда
		// Если настройки определены - повторно не выполняем.
		Возврат;
	КонецЕсли;
	
	// Настройки вида документа помещаем в структуру 
	// для дальнейшего использования при работе формы.
	СтруктураНастроек = Справочники.ВидыДокументовВводДанныхДляРасчетаЗарплаты.НастройкиВидаДокумента(Объект.ВидДокумента);
	...
	Элементы.РабочаяОбластьГруппа.Подсказка = СтруктураНастроек.Подсказка;
	Элементы.РабочаяОбластьГруппа.ОтображениеПодсказки = ?(ЗначениеЗаполнено(СтруктураНастроек.Подсказка), ОтображениеПодсказки.ОтображатьСверху, ОтображениеПодсказки.Нет);
	
	ЗаполнитьСведенияОПоказателях();
	ЗаполнитьСведенияОВидахВремени();
	ЗаполнитьСведенияОВидахРабот();
	
	ЭтоВводПостоянныхПоказателей = СтруктураНастроек.ЭтоВводПостоянныхПоказателей;
	...
	Элементы.Организация.Видимость = Не СтруктураНастроек.НесколькоОрганизаций;
	Элементы.Подразделение.Видимость = СтруктураНастроек.ПоказыватьПодразделение;
	Элементы.Сотрудник.Видимость = Не СтруктураНастроек.НесколькоСотрудников 
	...
	Если ЗначениеЗаполнено(СтруктураНастроек.Организация)
		И ЗначениеЗаполнено(Объект.Организация) Тогда
		Элементы.Организация.Вид = ВидПоляФормы.ПолеНадписи;
		Элементы.Организация.Шрифт = ЖирныйШрифт;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СтруктураНастроек.Подразделение) 
	...
	Если Элементы.Организация.Видимость 
		И Не ЗначениеЗаполнено(СтруктураНастроек.Организация) Тогда
		МассивПроверяемыхПолей.Вставить("Объект.Организация", "Организация");
	КонецЕсли;
	
	Если Элементы.Подразделение.Видимость 
		И СтруктураНастроек.ПроверятьЗаполнениеПодразделения 
		И Не ЗначениеЗаполнено(СтруктураНастроек.Подразделение) Тогда
	...
	Если СтруктураНастроек.ВидыРаботЗаполняютсяВДокументе Тогда
		Страница = Элементы.ВидыРаботЗаполняютсяВДокументеСтраница;
	Иначе
		ИспользуетсяТаблицаДетально = Не ЭтоВводПостоянныхПоказателей 
									И СтруктураНастроек.НесколькоСотрудников
									И (ЕстьРазовыеПоказателиСотрудников 
										И (ЕстьОперативныеПоказателиСотрудников Или ВидыРабот.Количество() > 0
										Или ВидыВремени.Количество() > 0 И Не СтруктураНастроек.ВремяВводитсяСводно 
										И СтруктураНастроек.ЗначенияПоказателейВводятсяНаРазныеДаты) 
									Или ВидыВремени.Количество() > 0 И СтруктураНастроек.ВремяВводитсяЗаМесяц И Не СтруктураНастроек.ВремяВводитсяСводно);
	...
	СтруктураНастроек.Вставить("ИспользуетсяТаблицаДетально", ИспользуетсяТаблицаДетально);
	...
	Иначе
		Если (ЕстьОперативныеПоказатели Или ВидыРабот.Количество() > 0 
			Или (Не СтруктураНастроек.ВремяВводитсяСводно И Не ИспользуетсяТаблицаДетально)) И Не ЕстьРазовыеПоказатели Тогда
			...
			Если СтруктураНастроек.ВыполненныеРаботыВводятсяСводно Тогда
				Элементы.ПериодСтраницы.ТекущаяСтраница = Элементы.ПериодСтраница;
				Элементы.ДатаНачала.Заголовок = НСтр("ru='За период с';uk='За період з'");
				Элементы.ПериодСтраницы.Видимость = Истина;
				МассивПроверяемыхПолей.Вставить("Объект.Период", "ДатаНачала");
	...
	ВидимостьЗаполнить = Не СтруктураНастроек.ЗначенияПоказателейВводятсяНаРазныеДаты;
	...
	Элементы.ПериодГруппа.Видимость = Не СтруктураНастроек.ВидыРаботЗаполняютсяВДокументе 
		И (Не СтруктураНастроек.ЗначенияПоказателейВводятсяНаРазныеДаты Или ИспользуетсяТаблицаДетально)
		Или СтруктураНастроек.ВыполненныеРаботыВводятсяСводно;
	Элементы.ПериодГруппаКолонок.Видимость = СтруктураНастроек.ЗначенияПоказателейВводятсяНаРазныеДаты И ЭтоВводПостоянныхПоказателей;
	Элементы.ДанныеСводноДата.Видимость = СтруктураНастроек.ЗначенияПоказателейВводятсяНаРазныеДаты И Не ЭтоВводПостоянныхПоказателей;
	
	Элементы.ДатаОкончания.Видимость = СтруктураНастроек.ИспользоватьПериодОкончания Или СтруктураНастроек.ВыполненныеРаботыВводятсяСводно;
	Элементы.ДанныеСводноПериодОкончания.Видимость = СтруктураНастроек.ИспользоватьПериодОкончания;
	
	Элементы.ВыполненныеРаботыПоСотрудникамПериод.Видимость = Не СтруктураНастроек.ВыполненныеРаботыВводятсяСводно;
	
	Если Не Элементы.ПериодГруппа.Видимость Тогда
		МассивПроверяемыхПолей.Удалить("Объект.Период");
		МассивПроверяемыхПолей.Удалить("ПериодРегистрацииСтрокой");
	КонецЕсли;
	
	// Настройки документа держим в форме для дальнейшего использования.
	...
	Если СтруктураНастроек.ВидыРаботЗаполняютсяВДокументе Тогда
		...
		Если СтруктураНастроек.ВыполненныеРаботыРаспределяютсяПоСотрудникам Тогда
			НастроитьЭлементыРаспределенияВыполненныхРабот();
		КонецЕсли;
		Возврат;
	КонецЕсли;
	
	Если СтруктураНастроек.ЗначенияПоказателейВводятсяНаРазныеДаты Тогда
		Если ЭтоВводПостоянныхПоказателей Тогда
	...
	Элементы.ДанныеСводно.ИзменятьПорядокСтрок = Не СтруктураНастроек.ЗаполнятьСписокОбъектов;
	Элементы.ДанныеСводно.ИзменятьСоставСтрок = Не СтруктураНастроек.ЗаполнятьСписокОбъектов;
	Элементы.ДанныеСводно.ПоложениеКоманднойПанели = ?(СтруктураНастроек.ЗаполнятьСписокОбъектов, 
														ПоложениеКоманднойПанелиЭлементаФормы.Нет,
														ПоложениеКоманднойПанелиЭлементаФормы.Авто);
	Элементы.ДанныеСводноОбъект.ТолькоПросмотр = СтруктураНастроек.ЗаполнятьСписокОбъектов;
	Если СтруктураНастроек.ЗаполнятьСписокОбъектов Тогда
		Элементы.ДанныеСводноГруппа.Заголовок = Неопределено;
	КонецЕсли;
	
	// ...И детальных данных по дням.
	Элементы.ДанныеДетально.ИзменятьСоставСтрок = Не СтруктураНастроек.ЗаполнятьСписокОбъектов;
	Элементы.ДанныеДетально.ПоложениеКоманднойПанели = ?(СтруктураНастроек.ЗаполнятьСписокОбъектов, 
														ПоложениеКоманднойПанелиЭлементаФормы.Нет,
														ПоложениеКоманднойПанелиЭлементаФормы.Авто);
	Элементы.ДанныеДетальноОбъект.ТолькоПросмотр = СтруктураНастроек.ЗаполнятьСписокОбъектов;
	...
	Элементы.ДанныеТаблицей.Видимость = ИспользуетсяТаблицаДетально
										Или СтруктураНастроек.НесколькоОрганизаций
										Или СтруктураНастроек.НесколькоПодразделений
										Или СтруктураНастроек.НесколькоСотрудников
										Или (СтруктураНастроек.ЗначенияПоказателейВводятсяНаРазныеДаты И ЭтоВводПостоянныхПоказателей);
	...
	ОбъектыВСтроках = СтруктураНастроек.НесколькоОрганизаций
					Или СтруктураНастроек.НесколькоПодразделений	
					Или СтруктураНастроек.НесколькоСотрудников;
	...
	Если СтруктураНастроек.НесколькоСотрудников Тогда
	...
	Иначе
		Если СтруктураНастроек.НесколькоПодразделений Тогда
		...
		Иначе
			Если СтруктураНастроек.НесколькоОрганизаций Тогда
				// В списке вводятся подразделения.
...
&НаСервере
Процедура НастроитьЭлементыРаспределенияВыполненныхРабот()
	...
	Элементы.ИсполнителиРаботТарифнаяСтавка.Видимость = НастройкиДокумента.ВыполненныеРаботыРаспределяютсяСУчетомТарифныхСтавок;
	Если НастройкиДокумента.ВыполненныеРаботыРаспределяютсяСУчетомТарифныхСтавок Тогда
	...
	Элементы.ИсполнителиРаботОтработанноеВремяГруппа.Видимость = НастройкиДокумента.ВыполненныеРаботыРаспределяютсяСУчетомОтработанногоВремени;
	Если НастройкиДокумента.ВыполненныеРаботыРаспределяютсяСУчетомОтработанногоВремени Тогда
		ЭлементОформления = УсловноеОформление.Элементы.Добавить();
		// Поля
		ОформляемоеПоле = ЭлементОформления.Поля.Элементы.Добавить();
...
Процедура УстановитьСвойстваПроверяемыхПолей()
	...
	Для Каждого КлючИЗначение Из ПроверяемыеПоля Цикл
		...
		Если ЧастиПутиКРеквизиту.Количество() > 1 И ЧастиПутиКРеквизиту[0] = "Объект" 
			Или ЧастиПутиКРеквизиту.Количество() = 1 Тогда
			Элементы[ИмяПоляФормы].ОтметкаНезаполненного = ЗначениеЗаполнено(ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(ЭтаФорма, ПутьКРеквизиту));
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура ЗаполнитьСписокОбъектов(Заполнять = Ложь)
	...
	Если Не АвтоЗаголовок Тогда
		...
		Если Параметры.Ключ.Пустая() Тогда
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='%1 (создание)';uk='%1 (створення)'"), ПредставлениеДокумента);
		Иначе
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='%1 %2 от %3';uk='%1 %2 від %3'"), ПредставлениеДокумента, Объект.Номер, Объект.Дата);
		КонецЕсли;
	КонецЕсли;
...
Процедура УточнитьСписокСотрудников()
	...
	Для Каждого СтрокаСотрудников Из ТаблицаСотрудников Цикл
		Если ЭтаФорма[ИмяТаблицы].НайтиСтроки(Новый Структура("Объект", СтрокаСотрудников.Сотрудник)).Количество() > 0 Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
		ЭтаФорма[ИмяТаблицы].Удалить(УдаляемаяСтрока);
	КонецЦикла;
...
Процедура ОформитьПоляТаблицыДнейМесяца()
	...
	УчетРабочегоВремениФормы.ОформитьПоляТаблицыДнейМесяца(Элементы, Объект.Период, "ДанныеДетальноДень%1Группа");
...
Процедура ПроверитьДублиСтрок(ИмяТаблицы, СоставКлюча, Отказ) 
	...
	Для Каждого СтрокаКоллекции Из ЭтаФорма[ИмяТаблицы] Цикл
		...
		НайденныеСтроки = ЭтаФорма[ИмяТаблицы].НайтиСтроки(ОтборСтрок);
		...
		КонецЕсли;
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Строка по %1 встречается в таблице более одного раза';uk='Рядок %1 зустрічається в таблиці більше одного разу'"), ОтборСтрок.Объект);
		ПолеФормы = ИмяТаблицы + "[" + ЭтаФорма[ИмяТаблицы].Индекс(НайденныеСтроки[0]) + "].Объект";
...
&НаСервере
Процедура ЗаполнитьСведенияОПоказателях()
	...
	|	ВЫБОР
	|		КОГДА ВидДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ВЫБОР
	...
	|							И ВидДокумента.НесколькоОрганизаций
	...
	|							И ВидДокумента.НесколькоПодразделений
	...
	|							И ВидДокумента.НесколькоСотрудников
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыДокументовВводДанныхДляРасчетаЗарплаты КАК ВидДокумента
	|			ПО (ВидДокумента.Ссылка = ПоказателиВидаДокумента.Ссылка)
...
Процедура ПрочитатьЗначенияПоказателей(ТекущийОбъект = Неопределено)
	...
	Для Каждого СтрокаОбъекты Из ТекущийОбъект.ЗначенияПоказателей Цикл
		...
		Если Не СведенияОПоказателе.ВводитсяВТаблице Тогда
			ЭтаФорма[СведенияОПоказателе.ИмяРеквизита] = СтрокаОбъекты.Значение;
			Продолжить;
		КонецЕсли;
		...
		КонецЕсли;
		НайденныеСтроки = ЭтаФорма[СведенияОПоказателе.ИмяТаблицы].НайтиСтроки(ПараметрыОтбора);
		Если НайденныеСтроки.Количество() = 0 Тогда
			// Если строка пока не добавлена - добавим.
			СтрокаКоллекции = ЭтаФорма[СведенияОПоказателе.ИмяТаблицы].Добавить();
	...
	Для Каждого СведенияОПоказателе Из НайденныеСтроки Цикл
		// Итоги по колонкам:
		ЗаполнитьИтогоПоКолонке(ЭтаФорма, ?(НастройкиДокумента.ИспользуетсяТаблицаДетально, ДанныеДетально, ДанныеСводно), СведенияОПоказателе.ИмяРеквизита);
	КонецЦикла;
...
Процедура ЗаполнитьТабличнуюЧастьПоказателей(ТекущийОбъект, Отказ)
	...
	Для Каждого СведенияОПоказателе Из ПоказателиРасчетаЗарплаты Цикл
		...
		Если Не СведенияОПоказателе.ВводитсяВТаблице Тогда
			...
			НоваяСтрока.Значение = ЭтаФорма[СведенияОПоказателе.ИмяРеквизита];
			Продолжить;
		КонецЕсли;
		...
		Иначе
			...
			Для Каждого СтрокаКоллекции Из ЭтаФорма[СведенияОПоказателе.ИмяТаблицы] Цикл
...
Процедура НастроитьЭлементыВводаЗначенийПоказателей(ИмяТаблицыДанных)
	...
	ЗарплатаКадры.ЗаполнитьМассивИменРеквизитовФормы(ЭтаФорма, ИменаСуществующихРеквизитов, ИмяТаблицыДанных);
	// Добавляем реквизиты формы
...
Процедура ПрочитатьДанныеОВремени(ТекущийОбъект = Неопределено)
	...
	Для Каждого СтрокаОбъекты Из ТекущийОбъект.ДанныеОВремениСводно Цикл
		...
		Если ПустаяСтрока(СведенияОВидеВремени.ИмяТаблицы) Тогда
			ЭтаФорма[СведенияОВидеВремени.ИмяРеквизита + "_СводноеВремяДни"] = СтрокаОбъекты.Дней;
			ЭтаФорма[СведенияОВидеВремени.ИмяРеквизита + "_СводноеВремяЧасы"] = СтрокаОбъекты.Часов;
			Продолжить;
		КонецЕсли;
		// Находим строку в таблице формы, она могла быть добавлена 
		// в связи с другим видом времени сотрудника.
		НайденныеСтроки = ЭтаФорма[СведенияОВидеВремени.ИмяТаблицы].НайтиСтроки(Новый Структура("Объект", СтрокаОбъекты.Сотрудник));
		Если НайденныеСтроки.Количество() = 0 Тогда
			// Если строка пока не добавлена - добавим.
			СтрокаКоллекции = ЭтаФорма[СведенияОВидеВремени.ИмяТаблицы].Добавить();
		...
		Иначе
			...
			Иначе
				...
				Если ПустаяСтрока(СведенияОВидеВремени.ИмяТаблицы) Тогда
					...
					ЭтаФорма[СведенияОВидеВремени.ИмяРеквизитаСводноЧасов] = СтрокаОбъекты["Часов" + День(ТекущийОбъект.Период)];
				Иначе
	...
	Для Каждого СведенияОВидеВремени Из ВидыВремени Цикл
		...
		Если ПустаяСтрока(СведенияОВидеВремени.ИмяТаблицы) Тогда
			Если НастройкиДокумента.ВремяВводитсяСводно Тогда
				...
				НоваяСтрока.Дней = ЭтаФорма[СведенияОВидеВремени.ИмяРеквизита + "_СводноеВремяДни"];
				НоваяСтрока.Часов = ЭтаФорма[СведенияОВидеВремени.ИмяРеквизита + "_СводноеВремяЧасы"];
			Иначе
				НоваяСтрока = ТекущийОбъект.ДанныеОВремениДетально.Добавить();
				НоваяСтрока.Сотрудник = ТекущийОбъект.Сотрудник;
				НоваяСтрока.ВидВремени = СведенияОВидеВремени.ВидВремени;
				НоваяСтрока["Часов" + День(Объект.Период)] = ЭтаФорма[СведенияОВидеВремени.ИмяРеквизита + "_СводноеВремяЧасы"];
			КонецЕсли;
			Продолжить;
		КонецЕсли;
		Если НастройкиДокумента.ИспользуетсяТаблицаДетально Тогда
...
Процедура НастроитьЭлементыСводногоВводаДанныхОВремени(ИмяТаблицыДанных = "")
			...
			ДобавляемыеРеквизиты.Добавить(
				Новый РеквизитФормы(ВидВремени.ИмяРеквизитаСводноДней, 
					Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2, 0, ДопустимыйЗнак.Неотрицательный)), 
						ИмяТаблицыДанных, НСтр("ru='Дней';uk='Днів'"), Истина));
			ДобавляемыеРеквизиты.Добавить(
				Новый РеквизитФормы(ВидВремени.ИмяРеквизитаСводноЧасов, 
					Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(5, 2, ДопустимыйЗнак.Неотрицательный)), 
						ИмяТаблицыДанных, НСтр("ru='Часов';uk='Годин'"), Истина));
		КонецЕсли;
	КонецЦикла;
	...
	Если Не НастройкиДокумента.ИспользуетсяТаблицаДетально И ВводятсяТаблицей И ВидыВремени.Количество() > 0 Тогда
		...
		ДобавляемыеРеквизиты.Добавить(
			Новый РеквизитФормы("ДанныеСводно_СводноеВремяДни_Итог", 
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2, 0, ДопустимыйЗнак.Неотрицательный)), 
					ИмяТаблицыДанных, НСтр("ru='Всего дней';uk='Всього днів'"), Истина));
		ДобавляемыеРеквизиты.Добавить(
			Новый РеквизитФормы("ДанныеСводно_СводноеВремяЧасы_Итог", 
				Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(5, 2, ДопустимыйЗнак.Неотрицательный)), 
					ИмяТаблицыДанных, НСтр("ru='Всего часов';uk='Всього годин'"), Истина));
	КонецЕсли;
	...
	Для Каждого СтрокаВидаВремени Из ВидыВремени Цикл
		...
		ЗаголовокВидаВремени = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru='%1 (%2)';uk='%1 (%2)'"), СтрокаВидаВремени.ВидВремени, СтрокаВидаВремени.БуквенныйКод);
		...
		Если ПоказыватьПолеДней Тогда
			...
			ПолеВводаДней.Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru='Количество дней за %1 по виду времени %2';uk='Кількість днів за %1 по виду часу %2'"), 
			...
			Если ВводятсяТаблицей Тогда
				...
				ПолеВводаДней.Заголовок = НСтр("ru='Дней';uk='Днів'");
		...
		ПолеВводаЧасов.Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Количество часов за %1 по виду времени %2';uk='Кількість годин за %1 по виду часу %2'"), 
									Формат(Объект.Период, "ДФ='ММММ гггг'"), 
									СтрокаВидаВремени.ВидВремени);
		Если ВводятсяТаблицей Тогда
			...
			ПолеВводаЧасов.Заголовок = НСтр("ru='Часов';uk='Годин'");
			Если Не СтрокаВидаВремени.ВводитсяДетально Тогда
	...
	Если Не НастройкиДокумента.ИспользуетсяТаблицаДетально И ВводятсяТаблицей И ВидыВремени.Количество() > 0 Тогда
		...
		ГруппаВидВремени = Элементы.Добавить(ИмяГруппыВидВремени, Тип("ГруппаФормы"), ГруппаСводноВремя);
		ГруппаВидВремени.Заголовок = ?(НастройкиДокумента.ВремяВводитсяСводно, "Всего", НСтр("ru='Всего часов';uk='Всього годин'"));
		ГруппаВидВремени.Вид = ВидГруппыФормы.ГруппаКолонок;
		ГруппаВидВремени.ОтображатьВШапке = Истина;
		ГруппаВидВремени.ГоризонтальноеПоложениеВШапке = ГоризонтальноеПоложениеЭлемента.Центр;
		ГруппаВидВремени.Группировка = ГруппировкаКолонок.Горизонтальная;
		...
		Если НастройкиДокумента.ВремяВводитсяСводно Тогда
			...
			ПолеВводаИтогоДней.Подсказка = НСтр("ru='Итоговое количество дней';uk='Підсумкова кількість днів'");
			...
			ПолеВводаИтогоДней.Заголовок = НСтр("ru='Дней';uk='Днів'");
		КонецЕсли;
		...
		ПолеВводаИтогоЧасов.Подсказка = НСтр("ru='Итоговое количество часов';uk='Підсумкова кількість годин'");
		...
		ПолеВводаИтогоЧасов.Заголовок = НСтр("ru='Часов';uk='Годин'");
	КонецЕсли;
...
Процедура ПрочитатьДанныеОВыполненныхРаботах(ТекущийОбъект = Неопределено)
	...
	Для Каждого СтрокаОбъекты Из ТекущийОбъект.ВыполненныеРаботы Цикл
		...
		Если Не СведенияВидаРабот.ВводитсяВТаблице Тогда
			ЭтаФорма[СведенияВидаРабот.ИмяРеквизита] = СтрокаОбъекты.ОбъемВыполненныхРабот;
			Продолжить;
		КонецЕсли;
		// Находим строку в таблице формы, она могла быть добавлена 
		// в связи с другим показателем объекта.
		...
		Если Не НастройкиДокумента.ИспользуетсяТаблицаДетально 
			...
			ПараметрыОтбора.Вставить("Период", СтрокаОбъекты.Период);
		КонецЕсли;
		НайденныеСтроки = ЭтаФорма[СведенияВидаРабот.ИмяТаблицы].НайтиСтроки(ПараметрыОтбора);
		Если НайденныеСтроки.Количество() = 0 Тогда
			// Если строка пока не добавлена - добавим.
			СтрокаКоллекции = ЭтаФорма[СведенияВидаРабот.ИмяТаблицы].Добавить();
...
Процедура ЗаполнитьТабличнуюЧастьВыполненныеРаботы(ТекущийОбъект, Отказ)
	...
	Для Каждого СведенияВидаРабот Из ВидыРабот Цикл
		...
		Если Не СведенияВидаРабот.ВводитсяВТаблице Тогда
			...
			НоваяСтрока.ОбъемВыполненныхРабот = ЭтаФорма[СведенияВидаРабот.ИмяРеквизита];
			Продолжить;
		КонецЕсли;
		// Если вводится в таблице, обходим ее.
		Если СведенияВидаРабот.ВводитсяДетально Тогда
		...
		Иначе
			// Отдельная колонка в таблицах.
			Для Каждого СтрокаКоллекции Из ЭтаФорма[СведенияВидаРабот.ИмяТаблицы] Цикл
...
Процедура РаспределитьСтоимостьРаботПоСотрудникам(ОтменятьИсправления = Ложь)
	...
	Для Каждого СтрокаТаблицы Из Объект.ЗначенияПоказателей Цикл
		...
		КонецЕсли;
		Если СтрокаТаблицы.ФиксЗначение Тогда
			ЗафиксированныеСуммы = ЗафиксированныеСуммы + СтрокаТаблицы.Значение;
			Продолжить;
		КонецЕсли;
		...
		Если УчитыватьОтработанноеВремя Тогда
			ОтработанноеВремя = СтрокаТаблицы.ОтработаноДней;
	...
	РаспределенныеСуммы = ОбщегоНазначенияКлиентСервер.РаспределитьСуммуПропорциональноКоэффициентам(
		Макс(Объект.ВыполненныеРаботы.Итог("Сумма") - ЗафиксированныеСуммы, 0), КоэффициентыРаспределения);
		
	Если РаспределенныеСуммы = Неопределено Тогда
		Если Объект.ВыполненныеРаботы.Итог("Сумма") <= ЗафиксированныеСуммы Тогда
	...
	Пока Индекс < СтрокиРаспределения.Количество() Цикл
		СтрокиРаспределения[Индекс].Значение = РаспределенныеСуммы[Индекс];
		Индекс = Индекс + 1;
	КонецЦикла;
...
&НаСервере
Процедура ЗаполнитьРасценкуРабот()
	
	ТекущаяСтрока = Элементы.ВыполненныеРаботы.ТекущаяСтрока;
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ТекущиеДанные = Объект.ВыполненныеРаботы.НайтиПоИдентификатору(ТекущаяСтрока);
	ТекущиеДанные.Расценка = Справочники.ВидыРаботСотрудников.ДействующаяРасценкаВидаРабот(ТекущиеДанные.ВидРабот, Объект.Дата);
...
Процедура ЗаполнитьТарифныеСтавки(ВыделенныеСтроки = Неопределено, ОтменятьИсправления = Ложь)
	...
	Если Не НастройкиДокумента.ВыполненныеРаботыРаспределяютсяСУчетомТарифныхСтавок Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьОтработанноеВремя(ВыделенныеСтроки = Неопределено, ОтменятьИсправления = Ложь)
	...
	Если Не НастройкиДокумента.ВыполненныеРаботыРаспределяютсяСУчетомОтработанногоВремени Тогда
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура РассчитатьСтоимостьРаботПоВсемСтрокам()
	
	Для Каждого СтрокаТаблицы Из Объект.ВыполненныеРаботы Цикл
		РассчитатьСтоимостьРаботПоСтроке(ЭтаФорма, СтрокаТаблицы.ПолучитьИдентификатор());
	КонецЦикла;
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура РассчитатьСтоимостьРаботПоСтроке(Форма, ИдентификаторСтроки)
	
	СтрокаТаблицы = Форма.Объект.ВыполненныеРаботы.НайтиПоИдентификатору(ИдентификаторСтроки);
