...
//++ НЕ УТКА
	Если ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
		...
		Таблица = Товары.ВыгрузитьКолонки("НомерСтроки, НовоеНазначение");
		ШаблонСообщенияПустое = НСтр("ru='Не заполнена колонка ""Новое назначение"" в строке %НомерСтроки% списка ""Товары"".';uk='Не заповнена колонка ""Нове призначення"" в рядку %НомерСтроки% списку ""Товари"".'");
		ШаблонСообщенияРавные = НСтр("ru='Значение в колонке ""Новое назначение"" в строке %НомерСтроки% списка ""Товары"" не должно совпадать со значением в колонке ""Исходное назначение"".';uk='Значення у колонці ""Нове призначення"" в рядку %НомерСтроки% списку ""Товари"" не має збігатися зі значенням у колонці ""Початкове призначення"".'");
		Для Каждого СтрокаТовары Из Товары Цикл
			
			Если Не ЗначениеЗаполнено(СтрокаТовары.НовоеНазначение) Тогда
				
				ТекстСообщения = СтрЗаменить(ШаблонСообщенияПустое, "%НомерСтроки%", СтрокаТовары.НомерСтроки);
			...
			ИначеЕсли СтрокаТовары.НовоеНазначение = СтрокаТовары.ИсходноеНазначение Тогда
		...
		Если Таблица.Количество() > 0 Тогда
			...
			Запрос.Текст =
				...
				|ВЫБРАТЬ
				|	Таблица.НомерСтроки   КАК НомерСтроки,
				|	ИсходныйЗаказ.Партнер КАК Партнер
				|ИЗ
				|	ВтТаблица КАК Таблица
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца КАК ИсходныйЗаказ
				|		ПО ИсходныйЗаказ.Ссылка = &Заказ
				|
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Назначения КАК СпрНазначения
				|		ПО Таблица.НовоеНазначение = СпрНазначения.Ссылка
				|
				|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца КАК НовыйЗаказ
				|		ПО НовыйЗаказ.Ссылка = СпрНазначения.Заказ
				|
				|ГДЕ
				|	НовыйЗаказ.Ссылка ЕСТЬ NULL ИЛИ НовыйЗаказ.Партнер <> ИсходныйЗаказ.Партнер";
			
			Запрос.УстановитьПараметр("Заказ",   Заказ);
			Запрос.УстановитьПараметр("Таблица", Таблица);
			Выборка = Запрос.Выполнить().Выбрать();
			ШаблонСообщения = НСтр("ru='В колонке ""Новое назначение"" в строке %НомерСтроки% должно быть указано назначение заказа давальца клиента ""%Клиент%"".';uk='У колонці ""Нове призначення"" в рядку %НомерСтроки% повинно бути вказано призначення замовлення давальця клієнта ""%Клиент%"".'");
			Пока Выборка.Следующий() Цикл
				
				ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%НомерСтроки%", Выборка.НомерСтроки);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Клиент%", Выборка.Партнер);
