...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	//++ НЕ УТ
	|		КОГДА Документ.Распоряжение ССЫЛКА Документ.ВозвратМатериаловИзПроизводства
	|			ТОГДА ВЫРАЗИТЬ(Документ.Распоряжение КАК Документ.ВозвратМатериаловИзПроизводства).Подразделение
	|		КОГДА Документ.Распоряжение ССЫЛКА Документ.ЗаказПереработчику
	|			ТОГДА ВЫРАЗИТЬ(Документ.Распоряжение КАК Документ.ЗаказПереработчику).Партнер
	|		КОГДА Документ.Распоряжение ССЫЛКА Документ.ВозвратСырьяОтПереработчика
	|			ТОГДА ВЫРАЗИТЬ(Документ.Распоряжение КАК Документ.ВозвратСырьяОтПереработчика).Партнер
	|		КОГДА Документ.Распоряжение ССЫЛКА Документ.ВыпускПродукции
	|			ТОГДА ВЫРАЗИТЬ(Документ.Распоряжение КАК Документ.ВыпускПродукции).Подразделение
	|		КОГДА Документ.Распоряжение ССЫЛКА Документ.ПоступлениеОтПереработчика
	|			ТОГДА ВЫРАЗИТЬ(Документ.Распоряжение КАК Документ.ПоступлениеОтПереработчика).Партнер
	//-- НЕ УТ
	//++ НЕ УТКА
	|		КОГДА Документ.Распоряжение ССЫЛКА Документ.ПоступлениеСырьяОтДавальца
	|			ТОГДА ВЫРАЗИТЬ(Документ.Распоряжение КАК Документ.ПоступлениеСырьяОтДавальца).Партнер
	|		КОГДА Документ.Распоряжение ССЫЛКА Документ.ЗаказДавальца
	|			ТОГДА ВЫРАЗИТЬ(Документ.Распоряжение КАК Документ.ЗаказДавальца).Партнер
	//-- НЕ УТКА
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	//++ НЕ УТ
	            Или ТипЗнч(Реквизиты.Распоряжение) = Тип("ДокументСсылка.ЗаказПереработчику")
	//++ НЕ УТКА			
				Или ТипЗнч(Реквизиты.Распоряжение) = Тип("ДокументСсылка.ЗаказДавальца")
				Или ТипЗнч(Реквизиты.Распоряжение) = Тип("ДокументСсылка.ЗаказНаПроизводство")
	//-- НЕ УТКА
	//-- НЕ УТ
...
Функция ВозможноЗаполнениеСерийПоРаспоряжению(Объект, ВыдаватьОшибки) Экспорт
	...
	ИначеЕсли Объект.СкладскаяОперация = Перечисления.СкладскиеОперации.ПриемкаПродукцииИзПроизводства Тогда
		...
		Если Не ПолучитьФункциональнуюОпцию("ИспользоватьСерииНоменклатурыПроизводство") 
 			...
 			//++ НЕ УТКА
			И Не НоменклатураСервер.ИспользуетсяСправочноеУказаниеСерийВМаршуртныхЛистах(, Перечисления.ВидыСтрокДереваСпецификаций.ВыходноеИзделие) 
			//-- НЕ УТКА
			Тогда
			Если ВыдаватьОшибки Тогда
				ТекстСообщения = НСтр("ru='На настроено указание серий в производственных документах.';uk='На настроєне зазначення серій у виробничих документах.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
			Возврат Ложь;	
		КонецЕсли;
		...
		Если СкладыСервер.ИспользоватьАдресноеХранение(Объект.Склад,Объект.Помещение,Объект.Дата) Тогда
			Если ВыдаватьОшибки Тогда
				ТекстСообщения = НСтр("ru='На складе ""%Склад%"" ведется учет остатков в разрезе складских ячеек, поэтому серии нужно указывать в разрезе упаковок. В производстве серии заполняются без учета упаковок. Заполнить серии по данным выпуска продукции нельзя.';uk='На складі ""%Склад%"" ведеться облік залишків у розрізі складських комірок, тому серії потрібно вказувати в розрізі упаковок. У виробництві серії заповнюються без обліку упаковок. Заповнити серії за даними випуску продукції не можна.'");
...
Функция ТаблицаСерийВыпущеннойПродукции(Объект)
	...
	Если ТипЗнч(Объект.Распоряжение) = Тип("ДокументСсылка.ВыпускПродукции") Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		...
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(ВыходныеИзделия.Назначение.ДвиженияПоСкладскимРегистрам, ЛОЖЬ)
		|				ТОГДА ВыходныеИзделия.Назначение
		|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|		КОНЕЦ КАК Назначение
		|	ИЗ
		|		Документ.ВыпускПродукции.Товары КАК ВыходныеИзделия
		|	ГДЕ
		|		ВыходныеИзделия.Ссылка = &Распоряжение
		|		И ВыходныеИзделия.Склад = &Склад
		|		И ВыходныеИзделия.Ссылка.Проведен
		|		И ВыходныеИзделия.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|	ГДЕ
		|		ТаблицаСерии.Ссылка = &Распоряжение
		|		И ТаблицаСерии.Склад = &Склад
		...
		|		Документ.ПриходныйОрдерНаТовары.Товары КАК ТаблицаСерии
		|	ГДЕ
		|		ТаблицаСерии.Ссылка.Распоряжение = &Распоряжение
		|		И ТаблицаСерии.Ссылка <> &ТекущийПриходныйОрдер
		|		И ТаблицаСерии.Ссылка.Склад = &Склад
		|		И ТаблицаСерии.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|		И ТаблицаСерии.Ссылка.Проведен) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.Серия,
		|	ВложенныйЗапрос.Номенклатура,
		...
		|	СУММА(ВложенныйЗапрос.Количество) > 0";
	//++ НЕ УТКА
	ИначеЕсли ТипЗнч(Объект.Распоряжение) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ВложенныйЗапрос.Серия КАК Серия,
		|	СУММА(ВложенныйЗапрос.Количество) КАК Количество,
		...
		|	(ВЫБРАТЬ
		|		ВыходныеИзделия.Серия КАК Серия,
		|		ВыходныеИзделия.КоличествоФакт КАК Количество,
		|		ВыходныеИзделия.Номенклатура КАК Номенклатура,
		|		ВыходныеИзделия.Характеристика КАК Характеристика,
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(ВыходныеИзделия.Назначение.ДвиженияПоСкладскимРегистрам, ЛОЖЬ)
		...
		|		И ВыходныеИзделия.Получатель = &Склад
		|		И ВыходныеИзделия.Ссылка.Проведен
		|		И ВыходныеИзделия.КоличествоФакт <> 0
		|		И ВыходныеИзделия.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		...
		|		КОНЕЦ
		|	ИЗ
		|		Документ.МаршрутныйЛистПроизводства.ВыходныеИзделияСерии КАК ТаблицаСерии
		|	ГДЕ
		|		ТаблицаСерии.КоличествоФакт <> 0
		|		И ТаблицаСерии.Ссылка В
		|				(ВЫБРАТЬ
		...
		|				ИМЕЮЩИЕ
		|					МИНИМУМ(ВЫБОР
		|							КОГДА ВыходныеИзделия.Получатель = &Склад
		|								ТОГДА ИСТИНА
		|							ИНАЧЕ ЛОЖЬ
		|						КОНЕЦ) = ИСТИНА)
		|	
	...
	//-- НЕ УТКА
	Иначе
		ТекстИсключения = НСтр("ru='Тип распоряжения не поддерживается.';uk='Тип розпорядження не підтримується.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
