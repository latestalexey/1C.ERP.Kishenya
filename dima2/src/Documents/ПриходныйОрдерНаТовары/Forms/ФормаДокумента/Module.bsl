...
Процедура СкладПомещениеПриИзмененииСервер(ИмяЭлемента)
	...
	ИспользоватьСтатусыПриходныхОрдеров = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Склад,
...
Функция  СформироватьПараметрыДляПечати(Знач ВыделенныеСтроки)
	...
	СтруктураДействий = Новый Структура;
	Если Не Объект.СкладскаяОперация = Перечисления.СкладскиеОперации.ВозвратНепринятыхТоваров
		И Не Объект.СкладскаяОперация = Перечисления.СкладскиеОперации.ВозвратНепринятыхТоваровУстарел Тогда
...
Процедура СписокУпаковокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	Если НаборУпаковок = Ложь Тогда
		...
		Текст = НСтр("ru='Добавить упаковку невозможно, т.к. для номенклатуры не включено использование упаковок.
                ...
                |;uk='Додати упаковку неможливо, т. к. для номенклатури не включено використання упаковок.
