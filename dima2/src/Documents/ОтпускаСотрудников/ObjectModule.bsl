	...
	Для каждого СтрокаСотрудника Из Сотрудники Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаСотрудника.КоличествоДнейКомпенсации)
			...
			Если Не ЗначениеЗаполнено(СтрокаСотрудника.ДатаНачала)
				...
				ТекстСообщения = НСтр("ru='не задан период отпуска';uk='не заданий період відпустки'");
				ИмяРеквизита = "ДатаНачала";
				
			Иначе
				
				ТекстСообщения = НСтр("ru='неверно задан период отпуска';uk='невірно заданий період відпустки'");
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='По сотруднику %1';uk='По співробітнику %1'"),СтрокаСотрудника.Сотрудник)
				+ " " + ТекстСообщения;
