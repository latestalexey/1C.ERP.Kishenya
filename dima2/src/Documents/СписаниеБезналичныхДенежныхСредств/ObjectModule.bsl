...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("ЗаказПоставщику") Тогда
		...
		ИначеЕсли ТипЗнч(ДанныеЗаполнения.ЗаказПоставщику) = Тип("ДокументСсылка.ПоступлениеДенежныхДокументов") Тогда
			Документы.ПоступлениеДенежныхДокументов.ЗаполнитьПоОснованию(ЭтотОбъект, ДанныеЗаполнения.ЗаказПоставщику);
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		Для каждого СтрокаМассива Из ДанныеЗаполнения.ДокументОснование Цикл
			...
			//++ НЕ УТ
			ИначеЕсли ТипЗнч(СтрокаМассива) = Тип("ДокументСсылка.ПоступлениеДенежныхДокументов") Тогда
				ЗаполнитьПоПоступлениюДенежныхДокументов(СтрокаМассива, 0, ДанныеЗаполнения);
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыВБанк")
		ИЛИ ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыПеречислением")
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("ПлатежиПоНалогам") Тогда
		...
		Для Каждого СтрокаПлатежа Из ПлатежиПоНалогам.ТаблицаРасшифровкаПлатежа Цикл
			...
			НоваяСтрока.СтатьяРасходов = ПланыВидовХарактеристик.СтатьиАктивовПассивов.НалогиИВзносыСОплатыТруда;
		КонецЦикла;			
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ДенежныеСредстваСервер.ОперацияПоЗарплате(ХозяйственнаяОперация) Тогда
		...
		//++ НЕ УТ
		МассивВедомостей = ВедомостиНаВыплатуЗарплаты.ВыгрузитьКолонку("Ведомость");
		...
		Если ДанныеВедомостей.МассивОшибок.Количество() > 0 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст, ЭтотОбъект, "ВедомостиНаВыплатуЗарплаты", , Отказ);
		КонецЕсли;
		//-- НЕ УТ
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплатыУТ") И ПроведеноБанком Тогда
...
Процедура ЗаполнитьПоВедомостямНаВыплатуЗарплаты(Знач ДанныеКоманды, ДанныеЗаполнения)
	...
	Если ТипЗнч(ДанныеКоманды) = Тип("Структура") Тогда
		МассивОбъектов = ДанныеКоманды.МассивВедомостей;
	Иначе
		МассивОбъектов = Новый Массив;
		МассивОбъектов.Добавить(ДанныеКоманды);
	КонецЕсли;
	
	ДанныеВедомостей = ДенежныеСредстваСервер.ДанныеВедомостейНаВыплатуЗарплаты(МассивОбъектов);
	...
	ДанныеЗаполнения = Новый Структура("Организация, ХозяйственнаяОперация, СуммаДокумента");
	ЗаполнитьЗначенияСвойств(ДанныеЗаполнения, ДанныеВедомостей.РеквизитыШапки);
	ДанныеЗаполнения.ХозяйственнаяОперация = ДанныеВедомостей.РеквизитыШапки.ХозяйственнаяОперацияПоЗарплате;
	
	СтруктураПараметров = Новый Структура();
	СтруктураПараметров.Вставить("Организация",           ДанныеЗаполнения.Организация);
	СтруктураПараметров.Вставить("ХозяйственнаяОперация", ДанныеЗаполнения.ХозяйственнаяОперация);
	...
	СтруктураПараметров.Вставить("МассивВедомостей",      МассивОбъектов);
	...
	ДенежныеСредстваСервер.ЗаполнитьПоВедомостямКОплате(ВедомостиНаВыплатуЗарплаты, СтруктураПараметров);
	...
	СтруктураПараметровБанковскогоСчета.Организация    		= Организация;
	ДанныеЗаполнения.Вставить("БанковскийСчет", ЗначениеНастроекПовтИсп.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(СтруктураПараметровБанковскогоСчета));
