...
Процедура ОбработкаВыбора(РезультатВыбора, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "ОбщаяФорма.ВедомостиНаВыплатуЗарплаты" Тогда
		...
		ТекущиеДанные = Элементы.ВедомостиНаВыплатуЗарплаты.ТекущиеДанные;
		ТекущиеДанные.Ведомость = РезультатВыбора.Ведомость;
		
		Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыПоЗарплатномуПроекту") Тогда
		...
		Элементы.ВедомостиНаВыплатуЗарплаты.ЗакончитьРедактированиеСтроки(Ложь);
...
Процедура ВопросВалютаНеСовпадаетЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если КодОтвета = КодВозвратаДиалога.Да Тогда
		...
		//++ НЕ УТ
		ЗаполнитьПредставлениеСчетаРеглУчетаСервер();
		//-- НЕ УТ
...
Процедура ВедомостиНаВыплатуЗарплатыВедомостьНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("ОбщаяФорма.ВедомостиНаВыплатуЗарплаты", Новый Структура("ОтборВедомостей", СтруктураПараметров), ЭтаФорма);
	...
	Если Параметры.Ключ.Пустая() Тогда
		ТекстВопроса =
			НСтр("ru='Данные еще не записаны.
                    ...
                    |Виконання дії ""Вивантажити файл обміну з банком"" можливе тільки після запису даних.
...
//++ НЕ УТ
&НаСервере
Процедура ЗаполнитьПредставлениеСчетаРеглУчетаСервер()
...
Процедура ЗаполнитьВедомостиПоОстаткамНаСервере()
	...
	СтруктураПараметров = Новый Структура();
	СтруктураПараметров.Вставить("Заявка",                Объект.ЗаявкаНаРасходованиеДенежныхСредств);
	СтруктураПараметров.Вставить("Списание",              Объект.Ссылка);
	СтруктураПараметров.Вставить("Валюта",                Объект.Валюта);
	СтруктураПараметров.Вставить("ХозяйственнаяОперация", Объект.ХозяйственнаяОперация);
	СтруктураПараметров.Вставить("Организация",           Объект.Организация);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЗаявкиНаРасходованиеДенежныхСредств") Тогда
		ДенежныеСредстваСервер.ЗаполнитьПоВедомостямИЗаявкамКОплате(Объект.ВедомостиНаВыплатуЗарплаты, СтруктураПараметров);
	Иначе
		ДенежныеСредстваСервер.ЗаполнитьПоВедомостямКОплате(Объект.ВедомостиНаВыплатуЗарплаты, СтруктураПараметров);
	КонецЕсли;
...
Процедура УстановитьПараметрыПоВыплатеЗарплаты()
	...
	ПараметрыУчетнойПолитики = РегистрыСведений.УчетнаяПолитикаОрганизаций.ПараметрыУчетнойПолитики(Объект.Организация, ПериодУчетнойПолитики);
...
Процедура РасшифровкаПлатежаДоговорЛизингаПриИзменении(Элемент)
	...
	//++ НЕ УТ
	РасшифровкаПлатежаДоговорЛизингаПриИзмененииНаСервере(ТекущаяСтрока.ДоговорЛизинга,
														ТекущаяСтрока.ТипПлатежаПоЛизингу,
														ТекущаяСтрока.СтатьяДвиженияДенежныхСредств,
														ТекущаяСтрока.ВалютаВзаиморасчетов,
														ТекущаяСтрока.СуммаВзаиморасчетов);
	//-- НЕ УТ
...
//++ НЕ УТ
&НаСервереБезКонтекста
Процедура РасшифровкаПлатежаДоговорЛизингаПриИзмененииНаСервере(Договор, ТипПлатежа, СтатьяДДС, ВалютаВзаиморасчетов, СуммаВзаиморасчетов)
	
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Договор, "СтатьяДвиженияДенежныхСредств,ВалютаВзаиморасчетов");
	СтатьяДДС = Реквизиты.СтатьяДвиженияДенежныхСредств;
	ВалютаВзаиморасчетов = Реквизиты.ВалютаВзаиморасчетов;
	СуммаВзаиморасчетов = 0;
	ТипыПлатежа = Справочники.ДоговорыЛизинга.ТипыПлатежейПоДоговору(Договор);
	ТипПлатежа = ТипыПлатежа[0];
