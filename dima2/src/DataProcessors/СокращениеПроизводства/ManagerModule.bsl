...
Процедура СостояниеЗаказовНаПроизводство(Параметры, АдресХранилища) Экспорт
	...
	|		ЕСТЬNULL(ЭтапВыпускаПоГрафику.Подразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)) КАК ПодразделениеИзготовитель,
...
Функция РезультатРедактированияСтрокиСпецификацииЗаказа(Параметры) Экспорт
	...
	Если СтруктураДанных.Свойство("БезопасноеСокращение", БезопасноеСокращение) = Ложь Тогда
	...
	КорректировкиЗаказаМатериалов = Обработки.ВводКорректировкиЗаказаМатериалов.СкорректироватьЗаказМатериаловВПроизводство(
														Параметры.Заказ, Параметры.КлючСвязиПродукция, СокращенныеЭтапы);
	...
	// Продукция
	Продукция = СтруктураДанных.ВыходныеИзделия.НайтиСтроки(ОтборПродукция)[0];
	
	НовоеКоличествоПродукции = Макс(
		БезопасноеСокращение,
		Продукция.КоличествоЗапланировано,
		Продукция.ВыходныхИзделийНаОдинЭтап);

	Если НовоеКоличествоПродукции <> СпецификацияСтрокиЗаказа.СтруктураПродукции.КоличествоУпаковок Тогда
		
		СпецификацияСтрокиЗаказа.СтруктураПродукции.КоличествоУпаковок = НовоеКоличествоПродукции; 
		
		Если ЗначениеЗаполнено(Продукция.Упаковка) Тогда
			СпецификацияСтрокиЗаказа.СтруктураПродукции.Количество = СпецификацияСтрокиЗаказа.СтруктураПродукции.КоличествоУпаковок * Продукция.УпаковкаКоэффициент;
		Иначе
			СпецификацияСтрокиЗаказа.СтруктураПродукции.Количество = СпецификацияСтрокиЗаказа.СтруктураПродукции.КоличествоУпаковок;
		КонецЕсли;
...
Процедура ЗаполнитьСтрокуДереваСостояниеЗаказов(ТипСтроки, ТекущаяСтрока, ДанныеЗаполнения)
	...
	Если ТипСтроки = 1 Тогда
		...
		Если НЕ ДанныеЗаполнения.ГрафикРассчитан Тогда
			ТекущаяСтрока.ПредставлениеНоменклатураЭтап = ТекущаяСтрока.ПредставлениеНоменклатураЭтап + ". " + НСтр("ru='График не рассчитан.';uk='Графік не розрахований.'");		
		КонецЕсли;
