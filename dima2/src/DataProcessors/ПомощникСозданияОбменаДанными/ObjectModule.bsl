...
Процедура ВыполнитьДействияПоНастройкеНовогоОбменаДаннымиЧерезВебСервисВДвухБазах(Отказ,
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение НСтр("ru='При создании настройки обмена во второй информационной базе возникли ошибки.';uk='При створенні настройки обміну в другій інформаційній базі виникли помилки.'");
		КонецЕсли;
	...
	Если ЭтоНастройкаРаспределеннойИнформационнойБазы
		...
		Если ГлавныйУзел = Неопределено Тогда
			...
			ВызватьИсключение НСтр("ru='Главный узел для текущей информационной базы не определен.
                            ...
                            |;uk='Головний вузол для поточної інформаційної бази не визначений.
                            |Можливо, інформаційна база не є підпорядкованим вузлом у РІБ.'");
		КонецЕсли;
...
Процедура ВыполнитьЗагрузкуПараметровМастера(Отказ, СтрокаXML) Экспорт
	...
	Если СтруктураНастроек.Свойство("ИмяПланаОбмена")
		...
		ПолеСтрокаСообщенияОбОшибке = НСтр("ru='Файл содержит настройки обмена для другой информационной базы.';uk='Файл містить настройки обміну для іншої інформаційної бази.'");
	...
	Если Не Отказ Тогда
		...
		Если ВерсияФорматаФайлаНастроекОбменаДанными = "1.0" Тогда
			...
			ЭтотОбъект.НаименованиеЭтойБазы = НСтр("ru='Эта информационная база';uk='Дана інформаційна база'");
	...
	Если Не ПустаяСтрока(ПрефиксИнформационнойБазы)
		...
		ПолеСтрокаСообщенияОбОшибке = НСтр("ru='На первом этапе настройки обмена был неправильно указан префикс второй информационной базы.
                ...
                |;uk='На першому етапі настройки обміну був неправильно зазначений префікс другої інформаційної бази.
                |Настройку обміну потрібно почати заново.'");
		//
		ОбменДаннымиСервер.СообщитьОбОшибке(СтрокаСообщенияОбОшибке(), Отказ);
...
Функция СчитатьДанныеВСтруктуру(ЧтениеXML)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Тогда
		...
		ВызватьИсключение НСтр("ru='Ошибка чтения XML';uk='Помилка читання XML'");
