...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		ИмяДействия = ?(ТолькоПолучитьУзелСсылки, НСтр("ru='Конвертация ссылки на объект';uk='Конвертація посилання на об''єкт'"), НСтр("ru='Конвертация объекта';uk='Конвертація об''єкта'"));
		
		ТекстСообщения = НСтр("ru='[ИмяДействия]: [Объект]([ТипОбъекта]), ПКО: [ПКО](НаименованиеПКО)';uk='[ИмяДействия]: [Объект]([ТипОбъекта]), ПКО: [ПКО](НаименованиеПКО)'");
	...
	Если ЕстьГлобальныйОбработчикПередКонвертациейОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(64, ОписаниеОшибки(), ПКО, Источник, НСтр("ru='ПередКонвертациейОбъекта (глобальный)';uk='ПередКонвертациейОбъекта (глобальний)'"));
		КонецПопытки;
		...
		Если УзелСсылки = Неопределено Тогда
			...
			Если СтруктураСвойств.ИмяТипа = "Перечисление"
				...
				ЗП.Текст               = НСтр("ru='В правиле конвертации значений (ПКЗ) необходимо сопоставить значение Источника значению Приемника.
                                                    ...
                                                    |;uk='У правилі конвертації значень (ПКЗ) необхідно зіставити значення Джерела значенню Приймача.
                                                    |Якщо підходящого значення приймача нема, то вказати порожнє значення.'");
				//
...
Процедура ВыгрузитьДанныеПоПравилу(Правило) Экспорт
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = НСтр("ru='ПРАВИЛО ВЫГРУЗКИ ДАННЫХ: %1 (%2)';uk='ПРАВИЛО ВИВАНТАЖЕННЯ ДАНИХ: %1 (%2)'");
...
Процедура ЗаписатьОбъектВИБ(Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='Попытка изменения неразделенных данных (%1) в разделенном сеансе.';uk='Спроба зміни нерозділених даних (%1) у розділеному сеансі.'");
...
Процедура ВыполнитьВыгрузкуДанных(ОбработкаДляЗагрузкиДанных = Неопределено) Экспорт
	...
	Если ЭтоОбменЧерезВнешнееСоединение() Тогда
		...
		Если ОбработкаДляЗагрузкиДанных().ФлагОшибки() Тогда
			...
			СтрокаСообщения = НСтр("ru='ВНЕШНЕЕ СОЕДИНЕНИЕ: %1';uk='ЗОВНІШНЄ З''ЄДНАННЯ: %1'");
...
Процедура ВыполнитьЗагрузкуДанныхВИнформационнуюБазу(ТаблицыДляЗагрузки) Экспорт
	...
	СтрокаСообщения = НСтр("ru='Начало процесса обмена данными для узла: %1';uk='Початок процесу обміну даними для вузла: %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	// Запись в журнале регистрации.
	СтрокаСообщения = НСтр("ru='%1, %2; Обработано %3 объектов';uk='%1, %2; Оброблено %3 об''єктів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВнешнееСоединениеОбработчикКонвертацииПередЗагрузкойДанных(Отказ) Экспорт
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(22, ОписаниеОшибки(), НСтр("ru='ПередЗагрузкойДанных (конвертация)';uk='ПередЗагрузкойДанных (конвертація)'"));
			Отказ = Истина;
		КонецПопытки;
...
Процедура ВнешнееСоединениеПослеЗагрузкиДанных() Экспорт
	...
	Если Не ФлагОшибки() Тогда
		...
		Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
			...
			Исключение
				ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ОписаниеОшибки(), НСтр("ru='ПослеЗагрузкиДанных (конвертация)';uk='ПослеЗагрузкиДанных (конвертація)'"));
			КонецПопытки;
...
Функция ПолучитьИнформациюОПравилах(ЭтоПравилаКорреспондента = Ложь) Экспорт
	...
	Если ЭтоПравилаКорреспондента Тогда
		СтрокаИнформации = НСтр("ru='Правила конвертации корреспондента (%1) от %2';uk='Правила конвертації кореспондента (%1) від %2'");
	Иначе
		СтрокаИнформации = НСтр("ru='Правила конвертации этой информационной базы (%1) от %2';uk='Правила конвертації цієї інформаційної бази (%1) від %2'");
	КонецЕсли;
...
Функция ОткрытьФайлВыгрузки()
	...
	Исключение
		ПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при открытии файла для записи сообщения обмена.
                ...
                |Описание ошибки:
                |%2'
                |;uk='Помилка при відкритті файлу для запису повідомлення обміну.
                |Ім''я файлу ""%1"".
                |Опис помилки:
                |%2'"),
...
Процедура ИнициализироватьВедениеПротоколаОбмена()
	...
	Исключение
		ФайлПротоколаДанных = Неопределено;
		СтрокаСообщения = НСтр("ru='Ошибка при попытке записи в файл протокола данных: %1. Описание ошибки: %2';uk='Помилка при спробі запису у файл протоколу даних: %1. Опис помилки: %2'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВосстановитьПравилаИзВнутреннегоФормата() Экспорт
	...
	Если ВерсияФорматаХраненияПравил <> ВерсияФорматаХраненияПравилОбмена() Тогда
		ВызватьИсключение НСтр("ru='Версия формата хранения правил обмена не соответствует ожидаемой.
            ...
            |;uk='Версія формату зберігання правил обміну не відповідає очікуваній.
            |Потрібно виконати завантаження правил обміну повторно.'");
	КонецЕсли;
...
Процедура НачатьЧтениеСообщения(ЧтениеСообщения, АнализДанных = Ложь)
	...
	Если ФайлОбмена.ТипУзла <> ТипУзлаXML.НачалоЭлемента Тогда
		ВызватьИсключение НСтр("ru='Ошибка формата сообщения обмена.';uk='Помилка формату повідомлення обміну.'");
	КонецЕсли;
	...
	Если ТаблицаПравилКонвертации.Количество() = 0 Тогда
		...
		Если ФлагОшибки() Тогда
			ВызватьИсключение НСтр("ru='При загрузке правил обмена данными возникли ошибки.';uk='При завантаженні правил обміну даними виникли помилки.'");
		КонецЕсли;
	Иначе
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(22, ОписаниеОшибки(), НСтр("ru='ПередЗагрузкойДанных (конвертация)';uk='ПередЗагрузкойДанных (конвертація)'"));
		КонецПопытки;
		
		Если Отказ Тогда
			ВызватьИсключение НСтр("ru='Отказ от загрузки сообщения обмена в обработчике ПередЗагрузкойДанных (конвертация).';uk='Відмова від завантаження повідомлення обміну в обробники ПередЗагрузкойДанных (конвертація).'");
		КонецЕсли;
	...
	Если ФайлОбмена.ЛокальноеИмя = "ИнформацияОТипахДанных" Тогда
		...
		Иначе
			...
			Если ФлагОшибки() Тогда
				ВызватьИсключение НСтр("ru='При загрузке информации о типах данных возникли ошибки.';uk='При завантаженні інформації про типи даних виникли помилки.'");
			КонецЕсли;
		КонецЕсли;
		ФайлОбмена.Прочитать();
	КонецЕсли;
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ru='Блокировка получения данных не может быть установлена в активной транзакции.';uk='Блокування одержання даних не може бути встановлена в активній транзакції.'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |Подробности:
            |%1'
            |;uk='Помилка установки блокування на обмін даними.
            |Можливо, обмін даними виконується іншим сеансом.
            |
            |Подробиці:
            |%1'"),
...
Процедура ВыполнитьОбработчикПослеЗагрузкиДанных()
	...
	Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ОписаниеОшибки(), НСтр("ru='ПослеЗагрузкиДанных (конвертация)';uk='ПослеЗагрузкиДанных (конвертація)'"));
		КонецПопытки;
	...
	Попытка
		...
		Если Отказ = Истина Тогда
			...
			Если Не ПустаяСтрока(ПричинаОтказа) Тогда
				...
				СтрокаСообщения = НСтр("ru='Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация) по причине: %1';uk='Відмова від завантаження повідомлення обміну в обробники ПослеЗагрузкиПараметров (конвертація) через: %1'");
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, ПричинаОтказа);
				ВызватьИсключение СтрокаСообщения;
			Иначе
				ВызватьИсключение НСтр("ru='Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация).';uk='Відмова від завантаження повідомлення обміну в обробники ПослеЗагрузкиПараметров (конвертація).'");
			КонецЕсли;
	...
	Пока НомерВариантаПоиска <= 10
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(73, ОписаниеОшибки(), "", "", 
				ТипОбъекта, Неопределено, НСтр("ru='Последовательность полей поиска';uk='Послідовність полів пошуку'"));
...
Процедура ЗагрузитьТабличнуюЧасть(Объект, ИмяТабличнойЧасти, ОбщаяИнформацияОТипеДокумента, ПараметрыОбъекта, ПКО)
	...
	Исключение
		...
		Текст = НСтр("ru='Имя табличной части: %1';uk='Ім''я табличної частини: %1'");
...
Процедура КомментарииКЗагрузкеОбъекта(НПП, ИмяПравила, Источник, ТипОбъекта, ГНПП = 0)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = НСтр("ru='Загрузка объекта № %1';uk='Завантаження об''єкта № %1'");
...
Функция ПрочитатьНаборЗаписейРегистра()
	...
	Если ЕстьГлобальныйОбработчикПередЗагрузкойОбъекта Тогда
		...
		Исключение
			...
			ТипОбъекта, Неопределено, НСтр("ru='ПередЗагрузкойОбъекта (глобальный)';uk='ПередЗагрузкойОбъекта (глобальний)'"));
			...
			Если ЕстьГлобальныйОбработчикПослеЗагрузкиОбъекта Тогда
				...
				Исключение
					...
					ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(54, ОписаниеОшибки(), ИмяПравила, Источник,
						ТипОбъекта, Объект, НСтр("ru='ПослеЗагрузкиОбъекта (глобальный)';uk='ПослеЗагрузкиОбъекта (глобальний)'"));
			...
			Если ИмяТипаОбъекта = "Документ" Тогда
				...
				Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
					...
					Исключение
						...
						СтрокаСообщения = НСтр("ru='Ошибка при записи документа: %1. Описание ошибки: %2';uk='Помилка при записі документа: %1. Опис помилки: %2'");
...
Процедура ВыполнитьПереходНаНовыйОбмен()
	...
	СтрокаСообщения = НСтр("ru='Выполнен автоматический переход на синхронизацию данных через формат EnterpriseData.';uk='Виконаний автоматичний перехід на синхронізацію даних через формат EnterpriseData.'");
	ЗаписьЖурналаРегистрацииОбменДанными(СтрокаСообщения, УровеньЖурналаРегистрации.Информация);
	ПолеРезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Отменено;
	ВызватьИсключение НСтр("ru='Синхронизация данных по старой настройке отменена.';uk='Синхронізація даних за старою настройкою скасована.'");
КонецПроцедуры
	...
	Если ВыгружатьГруппуЧерезФайл Тогда
		...
		Исключение
			...
			СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании временного файла для выгрузки данных.
                    ...
                    |Описание ошибки:
                    |%2'
                    |;uk='Помилка при створенні тимчасового файлу для вивантаження даних.
                    |Ім''я файлу ""%1"".
                    |Опис помилки:
                    |%2'"),
...
Процедура ВызватьСобытияПередВыгрузкойОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		ИмяСобытия = НСтр("ru='ВыгрузкаОбъекта: %1';uk='ВыгрузкаОбъекта: %1'");
	...
	Если ЕстьГлобальныйОбработчикПередВыгрузкойОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(65, ОписаниеОшибки(), Правило.Имя, НСтр("ru='ПередВыгрузкойОбъектаВыборки (глобальный)';uk='ПередВыгрузкойОбъектаВыборки (глобальний)'"), Объект);
		КонецПопытки;
...
Процедура ВызватьСобытияПослеВыгрузкиОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
	...
	Если ЕстьГлобальныйОбработчикПослеВыгрузкиОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(69, ОписаниеОшибки(), Правило.Имя, НСтр("ru='ПослеВыгрузкиОбъектаВыборки (глобальный)';uk='ПослеВыгрузкиОбъектаВыборки (глобальний)'"), Объект);
		КонецПопытки;
	КонецЕсли;
...
Процедура ОтработатьУдалениеОбъекта(ДанныеОбУдаленииОбъекта, СтрокаСообщенияОбОшибке = "")
	...
	Если Конвертация.Свойство("ПередОтправкойИнформацииОбУдалении", ТекстСобытия) Тогда
		...
		Если Не ПустаяСтрока(ТекстСобытия) Тогда
			...
			Исключение
				СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(76, ОписаниеОшибки(), НСтр("ru='ПередОтправкойИнформацииОбУдалении (конвертация)';uk='ПередОтправкойИнформацииОбУдалении (конвертація)'"));
...
Функция ПолучитьСтруктуруРеквизитовРегистрации(ТаблицаПКС)
	...
	Для Каждого ПКС Из МассивСтрокПКС Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='Обмен данными.Загрузка правил конвертации';uk='Обмін даними.Завантаження правил конвертації'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ВыполнитьПроверкуРеквизитовРегистрацииОбъектов(ТаблицаРеквизитовРегистрации)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРеквизитовРегистрации Цикл
		...
		Исключение
			...
			СтрокаСообщения = НСтр("ru='Тип объекта не определен: %1';uk='Тип об''єкта не визначений: %1'");
			...
			Для Каждого Реквизит Из СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Если ОбщегоНазначения.ЭтоЗадача(ОбъектМД) Тогда
					...
					Если НЕ (ОбъектМД.Реквизиты.Найти(Реквизит.Ключ) <> Неопределено
						...
						СтрокаСообщения = НСтр("ru='Неправильно указаны реквизиты шапки объекта ""%1"". Реквизит ""%2"" не существует.';uk='Неправильно зазначені реквізити шапки об''єкта ""%1"". Реквізит ""%2"" не існує.'");
		...
		Иначе
			...
			Если МетаТаблица = Неопределено Тогда
				ЗаписатьВПротоколВыполнения(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Табличная часть (стандартная табличная часть, движения) ""%1"" объекта ""%2"" не существует.';uk='Таблична частина (стандартна таблична частина, рухи) ""%1"" об''єкта ""%2"" не існує.'"),
			...
			Для Каждого Реквизит Из СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Если Не РеквизитНайденВТабличнойЧастиРеквизитовРегистрацииОбъектов(МетаТаблица, Реквизит.Ключ) Тогда
					ЗаписатьВПротоколВыполнения(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Реквизит ""%3"" не существует в табличной части (стандартной табличной части, движениях) ""%1"" объекта ""%2"".';uk='Реквізит ""%3"" не існує в табличній частині (стандартній табличній частині, рухах) ""%1"" єкта ""%2"".'"),
...
Процедура ОтключитьОбработкуДляОтладки()
	...
	Если ОбработкаВыгрузки <> Неопределено Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='Обмен данными';uk='Обмін даними'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура ИнициализацияСообщений()
	...
	СообщенияОбОшибках.Вставить(2,  НСтр("ru='Ошибка распаковки файла обмена. Файл заблокирован.';uk='Помилка розпакування файлу обміну. Файл заблокований.'"));
	СообщенияОбОшибках.Вставить(3,  НСтр("ru='Указанный файл правил обмена не существует.';uk='Зазначений файл правил обміну не існує.'"));
	СообщенияОбОшибках.Вставить(4,  НСтр("ru='Ошибка при создании COM-объекта Msxml2.DOMDocument';uk='Помилка при створенні COM-об''єкта Msxml2.DOMDocument'"));
	СообщенияОбОшибках.Вставить(5,  НСтр("ru='Ошибка открытия файла обмена';uk='Помилка відкриття файлу обміну'"));
	СообщенияОбОшибках.Вставить(6,  НСтр("ru='Ошибка при загрузке правил обмена';uk='Помилка при завантаженні правил обміну'"));
	СообщенияОбОшибках.Вставить(7,  НСтр("ru='Ошибка формата правил обмена';uk='Помилка формату правил обміну'"));
	СообщенияОбОшибках.Вставить(8,  НСтр("ru='Не корректно указано имя файла для выгрузки данных';uk='Не коректно зазначене ім''я файлу для вивантаження даних'")); // не используется
	СообщенияОбОшибках.Вставить(9,  НСтр("ru='Ошибка формата файла обмена';uk='Помилка формату файлу обміну'"));
	СообщенияОбОшибках.Вставить(10, НСтр("ru='Не указано имя файла для выгрузки данных (Имя файла данных)';uk='Не зазначене ім''я файлу для вивантаження даних (Ім''я файлу даних)'"));
	СообщенияОбОшибках.Вставить(11, НСтр("ru='Ссылка на несуществующий объект метаданных в правилах обмена';uk='Посилання на неіснуючий об''єкт метаданних у правилах обміну'"));
	СообщенияОбОшибках.Вставить(12, НСтр("ru='Не указано имя файла с правилами обмена (Имя файла правил)';uk='Не вказано ім''я файлу з правилами обміну (Ім''я файлу правил)'"));
			
	СообщенияОбОшибках.Вставить(13, НСтр("ru='Ошибка получения значения свойства объекта (по имени свойства источника)';uk='Помилка одержання значення властивості об''єкта (по імені властивості джерела)'"));
	СообщенияОбОшибках.Вставить(14, НСтр("ru='Ошибка получения значения свойства объекта (по имени свойства приемника)';uk='Помилка одержання значення властивості об''єкта (по імені властивості приймача)'"));
	
	СообщенияОбОшибках.Вставить(15, НСтр("ru='Не указано имя файла для загрузки данных (Имя файла для загрузки)';uk='Не зазначене ім''я файлу для завантаження даних (Ім''я файлу для завантаження)'"));
			
	СообщенияОбОшибках.Вставить(16, НСтр("ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';uk='Помилка одержання значення властивості підпорядкованого об''єкта (по імені властивості джерела)'"));
	СообщенияОбОшибках.Вставить(17, НСтр("ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)';uk='Помилка одержання значення властивості підпорядкованого об''єкта (по імені властивості приймача)'"));
	СообщенияОбОшибках.Вставить(18, НСтр("ru='Ошибка при создании обработки с кодом обработчиков';uk='Помилка при створенні обробки з кодом обробників'"));
	СообщенияОбОшибках.Вставить(19, НСтр("ru='Ошибка в обработчике события ПередЗагрузкойОбъекта';uk='Помилка в обробнику події ПередЗагрузкойОбъекта'"));
	СообщенияОбОшибках.Вставить(20, НСтр("ru='Ошибка в обработчике события ПриЗагрузкеОбъекта';uk='Помилка в обробнику події ПриЗагрузкеОбъекта'"));
	СообщенияОбОшибках.Вставить(21, НСтр("ru='Ошибка в обработчике события ПослеЗагрузкиОбъекта';uk='Помилка в обробнику події ПослеЗагрузкиОбъекта'"));
	СообщенияОбОшибках.Вставить(22, НСтр("ru='Ошибка в обработчике события ПередЗагрузкойДанных (конвертация)';uk='Помилка в обробнику події ПередЗагрузкойДанных (конвертація)'"));
	СообщенияОбОшибках.Вставить(23, НСтр("ru='Ошибка в обработчике события ПослеЗагрузкиДанных (конвертация)';uk='Помилка в обробнику події ПослеЗагрузкиДанных (конвертація)'"));
	СообщенияОбОшибках.Вставить(24, НСтр("ru='Ошибка при удалении объекта';uk='Помилка при вилученні об''єкта'"));
	СообщенияОбОшибках.Вставить(25, НСтр("ru='Ошибка при записи документа';uk='Помилка при записі документа'"));
	СообщенияОбОшибках.Вставить(26, НСтр("ru='Ошибка записи объекта';uk='Помилка запису об''єкта'"));
	СообщенияОбОшибках.Вставить(27, НСтр("ru='Ошибка в обработчике события ПередОбработкойПравилаОчистки';uk='Помилка в обробнику події ПередОбработкойПравилаОчистки'"));
	СообщенияОбОшибках.Вставить(28, НСтр("ru='Ошибка в обработчике события ПослеОбработкиПравилаОчистки';uk='Помилка в обробнику події ПослеОбработкиПравилаОчистки'"));
	СообщенияОбОшибках.Вставить(29, НСтр("ru='Ошибка в обработчике события ПередУдалениемОбъекта';uk='Помилка в обробнику події ПередУдалениемОбъекта'"));
	
	СообщенияОбОшибках.Вставить(31, НСтр("ru='Ошибка в обработчике события ПередОбработкойПравилаВыгрузки';uk='Помилка в обробнику події ПередОбработкойПравилаВыгрузки'"));
	СообщенияОбОшибках.Вставить(32, НСтр("ru='Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки';uk='Помилка в обробнику події ПослеОбработкиПравилаВыгрузки'"));
	СообщенияОбОшибках.Вставить(33, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';uk='Помилка в обробнику події ПередВыгрузкойОбъекта'"));
	СообщенияОбОшибках.Вставить(34, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';uk='Помилка в обробнику події ПослеВыгрузкиОбъекта'"));
			
	СообщенияОбОшибках.Вставить(41, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';uk='Помилка в обробнику події ПередВыгрузкойОбъекта'"));
	СообщенияОбОшибках.Вставить(42, НСтр("ru='Ошибка в обработчике события ПриВыгрузкеОбъекта';uk='Помилка в обробнику події ПриВыгрузкеОбъекта'"));
	СообщенияОбОшибках.Вставить(43, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';uk='Помилка в обробнику події ПослеВыгрузкиОбъекта'"));
			
	СообщенияОбОшибках.Вставить(45, НСтр("ru='Не найдено правило конвертации объектов';uk='Не знайдене правило конвертації об''єктів'"));
		
	СообщенияОбОшибках.Вставить(48, НСтр("ru='Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств';uk='Помилка в обробнику події ПередОбработкойВыгрузки групи властивостей'"));
	СообщенияОбОшибках.Вставить(49, НСтр("ru='Ошибка в обработчике события ПослеОбработкиВыгрузки группы свойств';uk='Помилка в обробнику події ПослеОбработкиВыгрузки групи властивостей'"));
	СообщенияОбОшибках.Вставить(50, НСтр("ru='Ошибка в обработчике события ПередВыгрузкой (объекта коллекции)';uk='Помилка в обробнику події ПередВыгрузкой (об''єкта колекції)'"));
	СообщенияОбОшибках.Вставить(51, НСтр("ru='Ошибка в обработчике события ПриВыгрузке (объекта коллекции)';uk='Помилка в обробнику події ПриВыгрузке (об''єкта колекції)'"));
	СообщенияОбОшибках.Вставить(52, НСтр("ru='Ошибка в обработчике события ПослеВыгрузки (объекта коллекции)';uk='Помилка в обробнику події ПослеВыгрузки (об''єкта колекції)'"));
	СообщенияОбОшибках.Вставить(53, НСтр("ru='Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)';uk='Помилка в глобальному обробнику події ПередЗагрузкойОбъекта (конвертація)'"));
	СообщенияОбОшибках.Вставить(54, НСтр("ru='Ошибка в глобальном обработчике события ПослеЗагрузкиОбъекта (конвертация)';uk='Помилка в глобальному обробнику події ПослеЗагрузкиОбъекта (конвертація)'"));
	СообщенияОбОшибках.Вставить(55, НСтр("ru='Ошибка в обработчике события ПередВыгрузкой (свойства)';uk='Помилка в обробнику події ПередВыгрузкой (властивості)'"));
	СообщенияОбОшибках.Вставить(56, НСтр("ru='Ошибка в обработчике события ПриВыгрузке (свойства)';uk='Помилка в обробнику події ПриВыгрузке (властивості)'"));
	СообщенияОбОшибках.Вставить(57, НСтр("ru='Ошибка в обработчике события ПослеВыгрузки (свойства)';uk='Помилка в обробнику події ПослеВыгрузки (властивості)'"));
	
	СообщенияОбОшибках.Вставить(62, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)';uk='Помилка в обробнику події ПередВыгрузкойДанных (конвертація)'"));
	СообщенияОбОшибках.Вставить(63, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиДанных (конвертация)';uk='Помилка в обробнику події ПослеВыгрузкиДанных (конвертація)'"));
	СообщенияОбОшибках.Вставить(64, НСтр("ru='Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)';uk='Помилка в глобальному обробнику події ПередКонвертациейОбъекта (конвертація)'"));
	СообщенияОбОшибках.Вставить(65, НСтр("ru='Ошибка в глобальном обработчике события ПередВыгрузкойОбъекта (конвертация)';uk='Помилка в глобальному обробнику події ПередВыгрузкойОбъекта (конвертація)'"));
	СообщенияОбОшибках.Вставить(66, НСтр("ru='Ошибка получения коллекции подчиненных объектов из входящих данных';uk='Помилка одержання колекції підпорядкованих об''єктів із вхідних даних'"));
	СообщенияОбОшибках.Вставить(67, НСтр("ru='Ошибка получения свойства подчиненного объекта из входящих данных';uk='Помилка одержання властивості підпорядкованого об''єкта із вхідних даних'"));
	СообщенияОбОшибках.Вставить(68, НСтр("ru='Ошибка получения свойства объекта из входящих данных';uk='Помилка одержання властивості об''єкта із вхідних даних'"));
	
	СообщенияОбОшибках.Вставить(69, НСтр("ru='Ошибка в глобальном обработчике события ПослеВыгрузкиОбъекта (конвертация)';uk='Помилка в глобальному обробнику події ПослеВыгрузкиОбъекта (конвертація)'"));
	
	СообщенияОбОшибках.Вставить(71, НСтр("ru='Не найдено соответствие для значения Источника';uk='Не знайдена відповідність для значення Джерела'"));
	
	СообщенияОбОшибках.Вставить(72, НСтр("ru='Ошибка при выгрузке данных для узла плана обмена';uk='Помилка при вивантаженні даних для вузла плану обміну'"));
	
	СообщенияОбОшибках.Вставить(73, НСтр("ru='Ошибка в обработчике события ПоследовательностьПолейПоиска';uk='Помилка в обробнику події ПоследовательностьПолейПоиска'"));
	СообщенияОбОшибках.Вставить(74, НСтр("ru='Необходимо перезагрузить правила обмена для выгрузки данных.';uk='Необхідно перезавантажити правила обміну для вивантаження даних.'"));
	
	СообщенияОбОшибках.Вставить(75, НСтр("ru='Ошибка в обработчике события ПослеЗагрузкиПравилОбмена (конвертация)';uk='Помилка в обробнику події ПослеЗагрузкиПравилОбмена (конвертація)'"));
	СообщенияОбОшибках.Вставить(76, НСтр("ru='Ошибка в обработчике события ПередОтправкойИнформацииОбУдалении (конвертация)';uk='Помилка в обробнику події ПередОтправкойИнформацииОбУдалении (конвертація)'"));
	СообщенияОбОшибках.Вставить(77, НСтр("ru='Ошибка в обработчике события ПриПолученииИнформацииОбУдалении (конвертация)';uk='Помилка в обробнику події ПриПолученииИнформацииОбУдалении (конвертація)'"));
	
	СообщенияОбОшибках.Вставить(78, НСтр("ru='Ошибка при выполнении алгоритма после загрузки значений параметров';uk='Помилка при виконанні алгоритму після завантаження значень параметрів'"));
	
	СообщенияОбОшибках.Вставить(79, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл';uk='Помилка в обробнику події ПослеВыгрузкиОбъектаВФайл'"));
	...
	СообщенияОбОшибках.Вставить(80, НСтр("ru='Ошибка установки свойства предопределенного элемента.
        ...
        |;uk='Помилка установки властивості напередвизначеного елемента.
        |Не можна позначати на вилучення напередвизначений елемент. Позначка на вилучення для об''єкта не встановлена.'"));
	//
	СообщенияОбОшибках.Вставить(83, НСтр("ru='Ошибка обращения к табличной части объекта. Табличная часть объекта не может быть изменена.';uk='Помилка звертання до табличної частини об''єкта. Таблична частина об''єкта не може бути змінена.'"));
	СообщенияОбОшибках.Вставить(84, НСтр("ru='Коллизия дат запрета изменения.';uk='Колізія дат заборони зміни.'"));
	
	СообщенияОбОшибках.Вставить(173, НСтр("ru='Ошибка блокировки узла обмена. Возможно синхронизация данных уже выполняется';uk='Помилка блокування вузла обміну. Можливо синхронізація даних уже виконується'"));
	СообщенияОбОшибках.Вставить(174, НСтр("ru='Сообщение обмена было принято ранее';uk='Повідомлення обміну було прийнято раніше'"));
	СообщенияОбОшибках.Вставить(175, НСтр("ru='Ошибка в обработчике события ПередПолучениемИзмененныхОбъектов (конвертация)';uk='Помилка в обробнику події ПередПолучениемИзмененныхОбъектов (конвертація)'"));
	СообщенияОбОшибках.Вставить(176, НСтр("ru='Ошибка в обработчике события ПослеПолученияИнформацииОбУзлахОбмена (конвертация)';uk='Помилка в обробнику події ПослеПолученияИнформацииОбУзлахОбмена (конвертація)'"));
		
	СообщенияОбОшибках.Вставить(1000, НСтр("ru='Ошибка при создании временного файла выгрузки данных';uk='Помилка при створенні тимчасового файлу вивантаження даних'"));
...
Процедура ЗагрузитьПравилаОбмена(Источник="",
	...
	Если РежимОбмена <> "Загрузка" И Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(75, ОписаниеОшибки(), НСтр("ru='ПослеЗагрузкиПравилОбмена (конвертация)';uk='ПослеЗагрузкиПравилОбмена (конвертація)'"));
			Отказ = Истина;
...
Процедура УдалитьОбъектПоСсылке(Ссылка, СтрокаСообщенияОбОшибке)
	...
	Если Не ПустаяСтрока(Конвертация.ПриПолученииИнформацииОбУдалении) Тогда
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(77, ОписаниеОшибки(), НСтр("ru='ПриПолученииИнформацииОбУдалении (конвертация)';uk='ПриПолученииИнформацииОбУдалении (конвертація)'"));
			Отказ = Истина;
					...
					Если СтрокаТаблицы = Неопределено Тогда
						...
						Для Каждого Колонка Из ТаблицаДанныхСообщенияОбмена.Колонки Цикл
							...
							Если Колонка.ТипЗначения.СодержитТип(ТипСтрока) Тогда
								...
								СтрокаТаблицы[Колонка.Имя] = НСтр("ru='Удаление объекта';uk='Вилучення об''єкта'");
				...
				Попытка
					...
					Если Отказ = Истина Тогда
						...
						Если Не ПустаяСтрока(ПричинаОтказа) Тогда
							...
							СтрокаСообщения = НСтр("ru='Загрузка данных отменена по причине: %1';uk='Завантаження даних скасовано через: %1'");
							СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, ПричинаОтказа);
							ВызватьИсключение СтрокаСообщения;
						Иначе
							ВызватьИсключение НСтр("ru='Загрузка данных отменена';uk='Завантаження даних скасоване'");
						КонецЕсли;
...
Процедура ПрочитатьДанныеПоОбмену(ЧтениеСообщения, АнализДанных)
	...
	Если Не ЗначениеЗаполнено(УзелОбменаПолучатель)
		...
		СтрокаСообщения = НСтр("ru='Не найден узел обмена для загрузки данных. План обмена: %1, Код: %2';uk='Не знайдений вузол обміну для завантаження даних. План обміну: %1, Код: %2'");
	...
	Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
		...
		Если НомерПринятого >= ЧтениеСообщения.НомерСообщения Тогда // Номер сообщения меньше либо равен ранее принятому.
			...
			ВызватьИсключение НСтр("ru='Сообщение обмена было принято ранее';uk='Повідомлення обміну було прийнято раніше'");
		КонецЕсли;
		...
		Если УдалитьРегистрациюИзменений Тогда // Удаляем регистрацию изменений.
			...
			Если ТранзакцияАктивна() Тогда
				ВызватьИсключение НСтр("ru='Удаление регистрации изменений данных не может быть выполнено в активной транзакции.';uk='Вилучення реєстрації змін даних не може бути виконане в активній транзакції.'");
			КонецЕсли;
	...
	Если Не ПустаяСтрока(Конвертация.ПослеПолученияИнформацииОбУзлахОбмена) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(176, ОписаниеОшибки(), НСтр("ru='ПослеПолученияИнформацииОбУзлахОбмена (конвертация)';uk='ПослеПолученияИнформацииОбУзлахОбмена (конвертація)'"));
		КонецПопытки;
	...
	Для Каждого ОбъектСоответствия Из ОбъектыДляОтложеннойЗаписи() Цикл
		...
		Попытка
			...
			Иначе
				...
				ОписаниеОшибки = НСтр("ru='Ошибка проверки заполнения реквизитов';uk='Помилка перевірки заповнення реквізитів'");
...
Процедура ВыполнитьВыгрузку(СтрокаСообщенияОбОшибке = "")
	...
	Если Не ПустаяСтрока(Конвертация.ПередВыгрузкойДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(62, ОписаниеОшибки(), НСтр("ru='ПередВыгрузкойДанных (конвертация)';uk='ПередВыгрузкойДанных (конвертація)'"));
			Отказ = Истина;
		КонецПопытки; 
	...
	Если Не Отказ И Не ПустаяСтрока(Конвертация.ПослеВыгрузкиДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(63, ОписаниеОшибки(), НСтр("ru='ПослеВыгрузкиДанных (конвертация)';uk='ПослеВыгрузкиДанных (конвертація)'"));
		КонецПопытки;
...
Процедура ВыполнитьВыгрузкуЗарегистрированныхДанных(ЗаписьСообщения, СтрокаСообщенияОбОшибке, ТаблицаПравилВыгрузкиИспользуемые)
	...
	Если Не ПустаяСтрока(Конвертация.ПередПолучениемИзмененныхОбъектов) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(175, ОписаниеОшибки(), НСтр("ru='ПередПолучениемИзмененныхОбъектов (конвертация)';uk='ПередПолучениемИзмененныхОбъектов (конвертація)'"));
			Возврат;
		КонецПопытки;
