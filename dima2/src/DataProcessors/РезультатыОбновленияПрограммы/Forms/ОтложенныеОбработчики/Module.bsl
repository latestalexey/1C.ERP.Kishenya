...
Процедура ГиперссылкаОтложенноеОбновлениеНажатие(Элемент)
	...
	Иначе
		ТекстПредупреждения = НСтр("ru='Обработка данных еще не выполнялась.';uk='Обробка даних ще не виконувалася.'");
...
Процедура Приостановить(Команда)
	...
	ТекстВопроса = НСтр("ru='Остановка дополнительных процедур обработки данных
        ...
        |;uk='Зупинка додаткових процедур обробки даних
        |може призвести до нестабільної роботи або непрацездатності програми.
        |Виконувати відключення рекомендується у разі виявлення помилки
        |в процедурі обробки даних і тільки після консультації зі службою підтримки,
        |тому що процедури обробки даних можуть залежати один від одного.'");
...
Процедура СформироватьТаблицуОтложенныхОбработчиков(ВыполненыВсеОбработчики = Истина, НачальноеЗаполнение = Ложь)
	...
	Если ОбработчикиНеВыполнялись Тогда
		Элементы.ТекстПояснения.Заголовок = НСтр("ru='Рекомендуется запустить невыполненные процедуры обработки данных.';uk='Рекомендується запустити невиконані процедури обробки даних.'");
	Иначе
		Элементы.ТекстПояснения.Заголовок = НСтр("ru='Невыполненные процедуры рекомендуется запустить повторно.';uk='Невиконані процедури рекомендується запустити повторно.'");
	КонецЕсли;
...
Процедура ДобавитьОтложенныйОбработчик(СтрокаОбработчика, ОбработчикиНеВыполнялись, ВыполненыВсеОбработчики, НачальноеЗаполнение, ВыполняетсяИзменениеПриоритета)
	...
	ШаблонИнформацияОПроцедуреОбновления = НСтр("ru='%1%2Процедура ""%3"" обработки данных %4.';uk='%1%2Процедура ""%3"" обробки даних %4.'");
	...
	Если СтрокаОбработчика.Статус = "Выполнено" Тогда
		...
		ПредставлениеСтатусаВыполнения = НСтр("ru='завершилась успешно';uk='завершилася успішно'");
		СтрокаСписка.СтатусПредставление = НСтр("ru='Выполнено';uk='Виконано'");
	...
	ИначеЕсли СтрокаОбработчика.Статус = "Выполняется" Тогда
		...
		ПредставлениеСтатусаВыполнения = НСтр("ru='в данный момент выполняется';uk='у цей момент виконується'");
		Если Прогресс <> Неопределено Тогда
			ШаблонСтатус = НСтр("ru='Выполняется (%1%)';uk='Виконується (%1%)'");
			СтрокаСписка.СтатусПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСтатус, Прогресс)
		Иначе
			СтрокаСписка.СтатусПредставление = НСтр("ru='Выполняется';uk='Виконується'");
		КонецЕсли;
	ИначеЕсли СтрокаОбработчика.Статус = "Ошибка" Тогда
		
		ОбработчикиНеВыполнялись = Ложь;
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("ru='Процедура ""%1"" обработки данных завершилась с ошибкой:';uk='Процедура ""%1"" обробки даних завершилася з помилкою:'") + Символы.ПС + Символы.ПС;
		...
		СтрокаСписка.СтатусПредставление = НСтр("ru='Ошибка';uk='Помилка'");
	...
	ИначеЕсли СтрокаОбработчика.Статус = "Приостановлен" Тогда
		...
		ОбработчикиНеВыполнялись = Ложь;
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("ru='остановлена администратором';uk='зупинена адміністратором'");
		СтрокаСписка.СтатусПредставление = НСтр("ru='Остановлено';uk='Зупинено'");
		СтрокаСписка.СтатусКартинка    = БиблиотекаКартинок.ЗнакСтоп;
	Иначе
		
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("ru='еще не выполнялась';uk='ще не виконувалась'");
		СтрокаСписка.СтатусПредставление = НСтр("ru='Не выполнялась';uk='Не виконувалася'");
	КонецЕсли;
...
Функция ДлительностьВыполненияПроцедурыОбновления(ДлительностьВыполнения)
	...
	ШаблонСекунды = НСтр("ru='%1 сек.';uk='%1 сек.'");
	ШаблонМинуты = НСтр("ru='%1 мин. %2 сек.';uk='%1 хв. %2 сек.'");
	ШаблонЧасы = НСтр("ru='%1 ч. %2 мин.';uk='%1 ч. %2 хв.'");
	...
	Если ДлительностьВСекундах < 1 Тогда
		Возврат НСтр("ru='менее секунды';uk='менше секунди'")
