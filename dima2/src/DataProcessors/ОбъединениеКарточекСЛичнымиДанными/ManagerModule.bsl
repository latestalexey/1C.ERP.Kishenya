...
Функция ОбъединитьИОчиститьЗадвоенныеДанныеФизическихЛиц(ПараметрыЗадания, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	Исключение
		...
		ДобавитьЗаписьВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось изменить данные %1';uk='Не вдалося змінити дані %1'"),
	...
	Для каждого ОписаниеРегистраСведений Из РезультатОбъединения.Строки Цикл
		...
		Исключение
			...
			ДобавитьЗаписьВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось сохранить данные регистра сведений %1';uk='Не вдалося зберегти дані регістра відомостей %1'"),
	...
	Если ВладелецОставляемыхФИО <> Неопределено И ВладелецОставляемыхФИО <> ОставляемоеФизическоеЛицо Тогда
		...
		Исключение
			...
			ДобавитьЗаписьВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось сохранить историю изменения ФИО %1';uk='Не вдалося зберегти історію зміни ПІБ %1'"),
					ОставляемоеФизическоеЛицо),
				ОставляемоеФизическоеЛицо,
				ИнформацияОбОшибке(),
				ВыполненоБезОшибок);
	...
	Если ВыполненоБезОшибок Тогда
		...
		Для каждого УдаляемоеФизическоеЛицо Из МассивФизическихЛиц Цикл
			...
			Исключение
				...
				ДобавитьЗаписьВЖурналРегистрации(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не удалось установить пометку удаления %1';uk='Не вдалося встановити позначку видалити %1'"),
	...
	Попытка
		...
		РегистрыСведений.ТекущиеКадровыеДанныеСотрудников.УстановитьОсновноеРабочееМестоВОрганизации(НаборЗаписейТекущихКадровыхДанных);
	...
	Исключение
		...
		ДобавитьЗаписьВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось обновить информацию об основном месте работы %1';uk='Не вдалося оновити інформацію про основному місці роботи, %1'"),
				ОставляемоеФизическоеЛицо),
			ОставляемоеФизическоеЛицо,
			ИнформацияОбОшибке(),
			ВыполненоБезОшибок);
	...
	Если ДанныеОставляемогоФизическогоЛица.Количество() > 0 Тогда
		...
		Если СтрокаДанных.Наименование <> НовоеНаименование
			...
			Исключение
				...
				ДобавитьЗаписьВЖурналРегистрации(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не удалось обновить наименование %1';uk='Не вдалося оновити найменування %1'"),
						ОставляемоеФизическоеЛицо),
					ОставляемоеФизическоеЛицо,
					ИнформацияОбОшибке(),
					ВыполненоБезОшибок);
	...
	Для каждого ДанныеСотрудника Из ДанныеСотрудников Цикл
		...
		Если ДанныеСотрудника.Наименование <> НовоеНаименование Тогда
			...
			Исключение
				...
				ДобавитьЗаписьВЖурналРегистрации(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не удалось обновить наименование сотрудника %1';uk='Не вдалося оновити найменування співробітника %1'"),
...
Функция ЗаменитьСсылкиВСтрокеСОбъектом(СтрокаСОбъектом, СсылкаЗамены)
	...
	Исключение
		...
		ДобавитьЗаписьВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось произвести замену ссылок в %1';uk='Не вдалося провести заміну посилань у %1'"),
...
Процедура ЗаполнитьРезультатПоКоллекции(Родитель, Коллекция, Уровень, МассивУдаляемыхСтрок, НайденныеДубли)
	...
	Для каждого ЭлементКоллекции Из Коллекция Цикл
		...
		Если Уровень < 2 ИЛИ СтрокаОбрабатывается Тогда
			...
			Если ЭлементКоллекции.Поле = "Ссылка" Тогда
				НоваяСтрокаРезультата.Представление = НСтр("ru='Личные данные';uk='Особисті дані'");
			Иначе
