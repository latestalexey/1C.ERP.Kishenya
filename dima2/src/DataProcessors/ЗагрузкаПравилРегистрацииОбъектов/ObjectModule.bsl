...
Функция ПолучитьИнформациюОПравилах() Экспорт
	...
	СтрокаИнформации = НСтр("ru='Правила регистрации объектов этой информационной базы (%1) от %2';uk='Правила реєстрації об''єктів цієї інформаційної бази (%1) від %2'");
...
Процедура ВыполнитьПроверкуНаличияПланаОбмена()
	...
	Если Регистрация.ИмяПланаОбмена <> ИмяПланаОбменаДляЗагрузки Тогда
		...
		ОписаниеОшибки = НСтр("ru='В правилах регистрации указан план обмена %1, а загрузка выполняется для плана обмена %2';uk='У правилах реєстрації зазначений план обміну %1, а завантаження виконується для плану обміну %2'");
...
Функция ИнициализацияСообщений()
	...
	Сообщения.Вставить(0, НСтр("ru='Внутренняя ошибка';uk='Внутрішня помилка'",КодОсновногоЯзыка));
	Сообщения.Вставить(1, НСтр("ru='Ошибка открытия файла правил';uk='Помилка відкриття файлу правил'",КодОсновногоЯзыка));
	Сообщения.Вставить(2, НСтр("ru='Ошибка при загрузке правил';uk='Помилка при завантаженні правил'",КодОсновногоЯзыка));
	Сообщения.Вставить(3, НСтр("ru='Ошибка формата правил';uk='Помилка формату правил'",КодОсновногоЯзыка));
	Сообщения.Вставить(4, НСтр("ru='Ошибка при получении файла правил для чтения';uk='Помилка при одержанні файлу правил для читання'",КодОсновногоЯзыка));
	Сообщения.Вставить(5, НСтр("ru='Загружаемые правила регистрации не предназначены для текущего плана обмена.';uk='правила, що завантажуються, реєстрації не призначені для поточного плану обміну.'",КодОсновногоЯзыка));
