...
Процедура Сформировать()
	...
	|	Склады.Ссылка,
	|	Склады.Наименование,
	|	Склады.КонтролироватьОбеспечение,
	|	Склады.ЭтоГруппа
	|ПОМЕСТИТЬ Склады
	|ИЗ
	...
	Запрос.Текст =
		...
		|		ПО ТоварныеОграничения.Номенклатура   = ВсеТоварыИСклады.Номенклатура
		|		 И ТоварныеОграничения.Характеристика = ВсеТоварыИСклады.Характеристика
		|		 И ТоварныеОграничения.Склад          = ВсеТоварыИСклады.Склад
		...
		|	ВЫРАЗИТЬ(ЕСТЬNULL(ТаблицаОстатков.Количество, 0) / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки, 1) КАК ЧИСЛО(15, 3)) КАК ДоступныйОстатокСейчас,
		...
		|		ПО ТаблицаОстатков.Номенклатура   = ДанныеПоДоступностиИМУЗ.Номенклатура
		|		 И ТаблицаОстатков.Характеристика = ДанныеПоДоступностиИМУЗ.Характеристика
		|		
		|УПОРЯДОЧИТЬ ПО
	...
	ЗапросИерархияСкладов.УстановитьПараметр("Склад", Соглашение.Склад);
					...
					Пока ВыборкаКоличество.Следующий() Цикл
						...
						Пока ВыборкаСклад.Следующий() Цикл
							...
							Пока ВыборкаДетали.Следующий() Цикл
								...
								Если ВыборкаДетали.Доступность = НСтр("ru='Согласно графика';uk='Згідно графіка'") Тогда

									Если ВыборкаДетали.Доступно > 0 Тогда

										Если ВыборкаДетали.Период = НачалоДня(ТекущаяДата()) Тогда

											Если ВыборкаДетали.Количество/ВыборкаДетали.Доступно < 0.5 Тогда
												ЗначениеДоступность = НСтр("ru='В наличии';uk='В наявності'");
											...
											Иначе
												ЗначениеДоступность = НСтр("ru='Требует уточнения';uk='Потребує уточнення'");
												ДобавитьВСоотвествиеСкладовЗапись(СоответствиеДанныеСклада, ВыборкаДетали.Склад, ЗначениеДоступность, ВыборкаДетали.ДоступныйОстатокСейчас);
											КонецЕсли;

										Иначе
											ЗначениеДоступность = НСтр("ru='Ожидается';uk='Очікується'") + " " + Формат(ВыборкаДетали.Период,"ДЛФ=D");
									...
									Иначе
										...
										Если ВыборкаДетали.ДатаПоставки < НачалоДня(ТекущаяДата()) Тогда
											ЗначениеДоступность = НСтр("ru='Нет на складе';uk='Немає на складі'");
											ДобавитьВСоотвествиеСкладовЗапись(СоответствиеДанныеСклада, ВыборкаДетали.Склад, ЗначениеДоступность, ВыборкаДетали.ДоступныйОстатокСейчас);
										Иначе
											ЗначениеДоступность = НСтр("ru='Ожидается';uk='Очікується'") + " " + Формат(ВыборкаДетали.ДатаПоставки,"ДЛФ=D");
								...
								Если ЗначениеДоступность = НСтр("ru='Нет на складе';uk='Немає на складі'") Тогда
	...
	Для Каждого СтрокаТаблицыГруппыСкладов Из ТаблицаГруппСкладов Цикл
		...
		Для Каждого ЭлементМассива ИЗ СтрокаТаблицыГруппыСкладов.МассивПодчиненныхСкладов Цикл
			...
			Если ДанныеСклада <> Неопределено Тогда
				Если ДанныеСклада.Доступность = НСтр("ru='В наличии';uk='В наявності'") ИЛИ 
					ДанныеСклада.Доступность = НСтр("ru='Услуга доступна';uk='Послуга доступна'") Тогда
				...
				Иначе
					...
					Если ДанныеСклада.Доступность = НСтр("ru='Ожидается';uk='Очікується'") + " " Тогда
		...
		Если ТекстДоступность = НСтр("ru='Ожидается';uk='Очікується'") + " " Тогда
			 ТекстДоступность = ТекстДоступность + Формат(ДатаДоступности, "ДЛФ=D");
		КонецЕсли;
		
		Если ПустаяСтрока(ТекстДоступность) Тогда
			ТекстДоступность = НСтр("ru='Нет в наличии';uk='Немає в наявності'");
		КонецЕсли;
		
		Если СтрокаТаблицыГруппыСкладов.Область <> Неопределено Тогда
			Если ДоступенПросмотрОстатков Тогда
				МассивИменОбластей = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаТаблицыГруппыСкладов.Область.Имя, ":");
			...
			Если ТекстДоступность = НСтр("ru='Нет в наличии';uk='Немає в наявності'") Тогда
...
Процедура ДобавитьВСоотвествиеСкладовЗапись(Соответствие, Склад, Доступность, ДоступныйОстатокСейчас, ПериодДоступности = Неопределено)
	...
	Соответствие.Вставить(Склад, Новый Структура("Доступность, ПериодДоступности, ДоступныйОстатокСейчас", Доступность, ПериодДоступности, ДоступныйОстатокСейчас));
		...
		Иначе
			Для Каждого ЭлементМассива Из МассивСкладовТекущий Цикл
				...
				Если СтрокаТаблицы <> Неопределено Тогда
					СтрокаТаблицы.МассивПодчиненныхСкладов.Добавить(СтрокаДерева.Склад);
				КонецЕсли;
			КонецЦикла
		КонецЕсли;
