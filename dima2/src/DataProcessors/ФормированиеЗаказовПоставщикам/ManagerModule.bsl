...
Процедура ЗаписатьПараметрыПоддержанияЗапаса(Таблица) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТоварныеОграничения КАК Т
	|		ПО &ПодстановкаТоварногоОграничения
	|ГДЕ
	|	Т.Номенклатура ЕСТЬ NULL
	|	ИЛИ Т.МетодОбеспеченияПотребностей <> ЗНАЧЕНИЕ(Перечисление.МетодыОбеспеченияПотребностей.ПоддержаниеЗапасаМинМакс)
	|	ИЛИ НовыеПараметры.МинимальныйОстаток <> Т.МинимальноеКоличествоЗапаса
	|	ИЛИ НовыеПараметры.МаксимальныйОстаток <> Т.МаксимальноеКоличествоЗапаса
	|	ИЛИ НовыеПараметры.МаксимальныйОстаток = 0 И НЕ Т.Номенклатура ЕСТЬ NULL";
...
Функция СоздатьЗаказы(Объект) Экспорт
	...
	Запрос.Текст =
		...
		|	Т.Характеристика                          КАК Характеристика,
		|	Т.Назначение                              КАК Назначение,
		|	Т.Назначение.НаправлениеДеятельности      КАК НаправлениеДеятельности,
		|	Т.ПодразделениеПолучатель                 КАК ПодразделениеПолучатель,
		|	Т.ВидЦеныПоставщика                       КАК ВидЦеныПоставщика,
		|
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	Т.Соглашение, Т.Партнер, Т.Склад, Т.Назначение.НаправлениеДеятельности,
...
Функция ТаблицаТоварныеОграничения()
	...
	Текст =
		...
		|
		|	ЕСТЬNULL(Т.МинимальноеКоличествоЗапаса, 0)  КАК МинимальныйОстаток,
		|	ЕСТЬNULL(Т.МаксимальноеКоличествоЗапаса, 0) КАК МаксимальныйОстаток,
		|
...
Функция ВременнаяТаблицаЗаказы()
	...
	Текст =
		...
		|	Т.Период                     КАК ДатаОтгрузки,
		|
		|	СУММА(Т.КоличествоНеОбеспечено) КАК Требуется
		|
		|ИЗ
		|	РегистрНакопления.ГрафикОтгрузкиТоваров КАК Т
		|
		...
		|СГРУППИРОВАТЬ ПО
		|	Т.Номенклатура, Т.Характеристика, Т.Склад,
		|	Т.Назначение, Т.Распоряжение, Т.Период
		|ИМЕЮЩИЕ
		...
		|	Т.Период                     КАК ДатаОтгрузки,
		|
		|	СУММА(Т.КоличествоПодЗаказ) КАК Требуется
		|
		|ИЗ
		|	РегистрНакопления.ГрафикОтгрузкиТоваров КАК Т
		|
