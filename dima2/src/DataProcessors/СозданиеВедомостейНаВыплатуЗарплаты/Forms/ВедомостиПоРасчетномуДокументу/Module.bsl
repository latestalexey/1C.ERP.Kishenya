...
Процедура РасчетныйДокументПриИзмененииНаСервере()
	...
	ИнформацияОСпособеВыплаты = ВзаиморасчетыССотрудникамиВызовСервераРасширенный.ИнформацияОСпособеВыплаты(СпособВыплаты);
	
	Округление = ИнформацияОСпособеВыплаты.ПараметрыРасчета.Округление;
	ПроцентВыплаты = ИнформацияОСпособеВыплаты.ПараметрыРасчета.ПроцентВыплаты;
	ПорядокЗаполненияНалогов = Перечисления.ПорядокЗаполненияНалогов.НДФЛИВзносы;
...
Процедура СоздатьВедомостиНаСервере(СообщитьПользователю)
	...
	ПравилаПроверки.Добавить("РасчетныйДокумент",	НСтр("ru='Не выбран документ';uk='Не вибраний документ'"));
	ПравилаПроверки.Добавить("СпособВыплаты",		НСтр("ru='Не указан способ выплаты';uk='Не вказаний спосіб виплати'"));
	ПравилаПроверки.Добавить("ДатаВыплаты",			НСтр("ru='Не задана дата выплаты';uk='Не задана дата виплати'"));
	ПравилаПроверки.Добавить("Округление", НСтр("ru='Не указан способ округления';uk='Не вказаний спосіб округлення'"));
	ПравилаПроверки.Добавить("ПорядокЗаполненияНалогов", НСтр("ru='Не указан порядок заполнения налогов';uk='Не вказаний порядок заповнення податків'"));
	...
	Если МожноСоздатьВедомости Тогда
		...
		ПараметрыСозданияВедомостей.Округление				= Округление;
		ПараметрыСозданияВедомостей.ПроцентВыплаты			= ПроцентВыплаты;
		ПараметрыСозданияВедомостей.ПорядокЗаполненияНалогов = ПорядокЗаполненияНалогов;
