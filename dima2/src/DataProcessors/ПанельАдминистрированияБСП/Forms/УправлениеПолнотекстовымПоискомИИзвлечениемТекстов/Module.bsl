...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РежимРаботы.ЭтоАдминистраторПрограммы И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ФайловыеФункции") Тогда
		Если РежимРаботы.БазоваяВерсия ИЛИ РежимРаботы.Файловый Тогда
			СписокВыбора = Элементы.ИзвлекатьТекстыФайловНаСервере.СписокВыбора;
			СписокВыбора[0].Представление = НСтр("ru='Все рабочие станции работают под управлением ОС Windows';uk='Всі робочі станції працюють під управлінням ОС Windows'");
			СписокВыбора[1].Представление = НСтр("ru='Одна или несколько рабочих станций работают под управлением ОС Linux';uk='Одна або декілька робочих станцій, що працюють під управлінням ОС Linux'");
		КонецЕсли;
	Иначе
	...
	Иначе
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ru='Управление полнотекстовым поиском';uk='Управління повнотекстовим пошуком'");
		Элементы.ОписаниеРаздела.Заголовок =
			НСтр("ru='Включение и отключение полнотекстового поиска, обновление индекса полнотекстового поиска.';uk='Включення і відключення повнотекстового пошуку, оновлення індексу повнотекстового пошуку.'");
	КонецЕсли;
...
Процедура ОбновитьИндекс(Команда)
	Состояние(
        ...
        |;uk='Іде оновлення повнотекстового індексу...
        |Будь ласка, почекайте.'"));
	
	ОбновитьИндексСервер();
	
	Состояние(НСтр("ru='Обновление полнотекстового индекса завершено.';uk='Оновлення повнотекстового індексу завершене.'"));
КонецПроцедуры
...
Процедура ОчиститьИндекс(Команда)
	Состояние(
        ...
        |Пожалуйста, подождите.'
        |;uk='Іде очищення повнотекстового індексу...
        |Будь ласка, почекайте.'"));
	
	ОчиститьИндексСервер();
	
	Состояние(НСтр("ru='Очистка полнотекстового индекса завершена.';uk='Очищення повнотекстового індексу завершені.'"));
КонецПроцедуры
...
Процедура ПроверитьИндекс(Команда)
	Состояние(
        ...
        |Пожалуйста, подождите.'
        |;uk='Йде перевірка повнотекстового індексу...
	...
	Исключение
		ТекстСообщенияОбОшибке = 
			НСтр("ru='В настоящее время проверка индекса невозможна, так как выполняется его очистка или обновление.';uk='В даний час перевірка індексу неможлива, так як виконується його очищення та оновлення.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщенияОбОшибке);
	КонецПопытки;
	
	Состояние(НСтр("ru='Проверка полнотекстового индекса завершена.';uk='Перевірка повнотекстового індексу завершена.'"));
КонецПроцедуры
...
Процедура Подключаемый_ПриИзмененииРеквизита(Элемент, ОбновлятьИнтерфейс = Истина)
	...
	Если Результат.Свойство("НеУдалосьУстановитьРежимПолнотекстовогоПоиска") Тогда
		...
		ТекстВопроса = НСтр("ru='Для изменения режима полнотекстового поиска требуется завершение сеансов всех пользователей, кроме текущего.';uk='Для зміни режиму повнотекстового пошуку потрібно завершення сеансів всіх користувачів, крім поточного.'");
		...
		ИначеЕсли РеквизитПутьКДанным = "ИспользоватьПолнотекстовыйПоиск" Тогда
			...
			Исключение
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Полнотекстовый поиск';uk='Повнотекстовий пошук'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "", ИндексПроверен = Ложь)
	...
	Если РеквизитПутьКДанным = "" Или РеквизитПутьКДанным = "ИспользоватьПолнотекстовыйПоиск" Тогда
		...
		Элементы.ИспользоватьПолнотекстовыйПоискПриПодбореТоваров.Доступность = (ИспользоватьПолнотекстовыйПоиск = 1);
	...
	Если РеквизитПутьКДанным = ""
		...
		Если ИспользоватьПолнотекстовыйПоиск = 1 Тогда
			...
			Если ИндексПроверен И Не ИндексСодержитКорректныеДанные Тогда
				СтатусИндекса = НСтр("ru='Требуется очистка и обновление';uk='Потрібно очищення і оновлення'");
			ИначеЕсли ИндексАктуален Тогда
				СтатусИндекса = НСтр("ru='Обновление не требуется';uk='Оновлення не потрібно'");
			Иначе
				СтатусИндекса = НСтр("ru='Требуется обновление';uk='Потрібне оновлення'");
			КонецЕсли;
		Иначе
			ДатаАктуальностиИндекса = '00010101';
			ИндексАктуален = Ложь;
			ФлагДоступность = Ложь;
			СтатусИндекса = НСтр("ru='Полнотекстовый поиск отключен';uk='Повнотекстовий пошук відключений'");
		КонецЕсли;
	...
	Если Элементы.ГруппаАвтоматическоеИзвлечениеТекстов.Видимость
		...
		Иначе
			РасписаниеПредставление = НСтр("ru='Автоматическое извлечение текстов не выполняется.';uk='Автоматичний витяг текстів не виконується.'");
		КонецЕсли;
