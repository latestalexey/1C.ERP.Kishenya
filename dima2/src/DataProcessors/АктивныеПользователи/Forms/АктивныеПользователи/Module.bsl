...
Процедура ЗавершитьСеанс(Команда)
	...
	Если КоличествоВыделенныхСтрок = 0 Тогда
		...
		ПоказатьПредупреждение(,НСтр("ru='Не выбраны пользователи для завершения сеансов.';uk='Не обрані користувачі для завершення сеансів.'"));
		Возврат;
	...
	Если ПараметрыРаботыКлиента.РазделениеВключено И ПараметрыРаботыКлиента.ДоступноИспользованиеРазделенныхДанных Тогда
		...
		Если ЗавершаемыйСеанс = НомерСеансаИнформационнойБазы Тогда
			ПоказатьПредупреждение(,НСтр("ru='Невозможно завершить текущий сеанс. Для выхода из программы можно закрыть главное окно программы.';uk='Неможливо завершити поточний сеанс. Для виходу із програми можна закрити головне вікно програми.'"));
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Если ЗапрашиватьПараметрыАдминистрированияИБ Тогда
			...
			ЗаголовокФормы = НСтр("ru='Завершение сеанса';uk='Завершення сеансу'");
			ПоясняющаяНадпись = НСтр("ru='Для завершения сеанса необходимо ввести параметры
                |администрирования кластера серверов'
                |;uk='Для завершення сеансу необхідно ввести параметри
                |адміністрування кластера серверів'");
...
Процедура ОткрытьЖурналРегистрации()
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Выберите пользователей для просмотра журнала регистрации.';uk='Оберіть користувачів для перегляду журналу реєстрації.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПослеЗавершенияСеанса(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Если ДополнительныеПараметры.Свойство("НомераСеансов")
			...
			ТекстОповещения = НСтр("ru='Сеансы %1 завершены.';uk='Сеанси %1 завершені.'");
			...
			ПоказатьОповещениеПользователя(НСтр("ru='Завершение сеансов';uk='Завершення сеансів'"),, ТекстОповещения);
		...
		Иначе
			...
			ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сеанс %1 завершен.';uk='Сеанс %1 завершений.'"), НомерСеанса);
			ПоказатьОповещениеПользователя(НСтр("ru='Завершение сеанса';uk='Завершення сеансу'"),, ТекстОповещения);
