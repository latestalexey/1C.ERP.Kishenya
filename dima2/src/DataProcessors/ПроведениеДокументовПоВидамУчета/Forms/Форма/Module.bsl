...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ВидыУчетов") Тогда
		...
		Если ВидыУчетов = "ДанныеДляРасчетаСреднего" Тогда
			...
			Заголовок = НСтр("ru='Обновить данные для расчета среднего заработка';uk='Оновити дані для розрахунку середнього заробітку'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПровестиДокументы(Команда)
	...
	Если Не ЗначениеЗаполнено(ВидыУчетов) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнены виды учетов';uk='Не заповнені види обліків'"));
		Возврат;
	КонецЕсли;
...
Функция ПровестиДокументыНаСервере()
	...
	НаименованиеЗадания = НСтр("ru='Обновление данных для расчета среднего заработка';uk='Оновлення даних для розрахунку середнього заробітку'");
