...
Процедура ОчиститьАгрегатыПоРегистрам(Команда)
	...
	ТекстВопроса = НСтр("ru='Очистка агрегатов может привести к существенному замедлению отчетов.';uk='Очищення агрегатів може призвести до істотного уповільнення звітів.'");
	...
	Если Результат.МожноПолучить Тогда
		...
		#Иначе
			...
			Если Расширение = "zip" Тогда
				Фильтр = НСтр("ru='Архив ZIP (*.%1)|*.%1';uk='Архів ZIP (*.%1)|*.%1'");
			ИначеЕсли Расширение = "xml" Тогда
				Фильтр = НСтр("ru='Документ XML (*.%1)|*.%1';uk='Документ XML (*.%1)|*.%1'");
			Иначе
				Фильтр = "";
			КонецЕсли;
...
Функция ПолучитьОптимальныеАгрегатыСервер()
	...
	Для НомерСтроки = 1 По Всего Цикл
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда
			ЕстьОшибки = Истина;
			ЗаголовокОшибки = НСтр("ru='Не удалось получить оптимальные агрегаты регистра ""%1"":';uk='Не вдалося отримати оптимальні агрегати регістра ""%1"":'");
	...
	Если Успешно > 0 Тогда
		...
		Иначе
			КраткоеИмяФайла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Оптимальные агрегаты регистров накопления %1.zip';uk='Оптимальні агрегати регістрів накопичення %1.zip'"),
		...
		Если ЕстьОшибки Тогда
			СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось получить оптимальные агрегаты регистра накопления ""%1"".';uk='Не вдалося отримати оптимальні агрегати регістру накопичення ""%1"".'"),
					ИмяРегистра),
				ПодробныйТекстОшибок);
		Иначе
			СтандартныеПодсистемыКлиентСервер.ВывестиОповещение(
				Результат,
				НСтр("ru='Оптимальные агрегаты успешно получены.';uk='Оптимальні агрегати успішно отримані.'"),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='%1 (регистр накопления)';uk='%1 (регістр накопичення)'"),
					ИмяРегистра),
	...
	ИначеЕсли Успешно = 0 Тогда
		...
		СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(
			Результат,
			НСтр("ru='Не удалось получить оптимальные агрегаты регистров накопления.';uk='Не вдалося отримати оптимальні агрегати регістрів накопичення.'"),
			ПодробныйТекстОшибок);
	ИначеЕсли ЕстьОшибки Тогда
		...
		СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |Не получены: %3.'
                |;uk='Агрегати успішно отримані для %1 з %2 регістрів.
                |Не отримано: %3.'"),
				Успешно,
				Всего,
				Всего - Успешно),
	...
	Иначе
		...
		СтандартныеПодсистемыКлиентСервер.ВывестиОповещение(
			Результат,
			НСтр("ru='Агрегаты успешно получены.';uk='Агрегати успішно отримані.'"),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Регистры накопления (%1)';uk='Регістри накопичення (%1)'"),
				Успешно),
			БиблиотекаКартинок.Успешно32);
	КонецЕсли;
