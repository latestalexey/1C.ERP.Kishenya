		...
		Если ДокументПроведенУспешно Тогда
			...
			Иначе
				ЗаписьЖурналаРегистрации(
					...
					ДокументОбъект.ДополнительныеСвойства.ДляПроведения.МетаданныеДокумента,
					ДокументОбъект.Ссылка,
					НСтр("ru='Выполнено перезаполнение видов запасов документа. Документ перепроведен';uk='Виконано перезаполнение видів запасів документа. Документ перепроведен'"), 
					РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
			КонецЕсли;
...
Процедура СформироватьЗаданияДляМенеджеровСервер()
	...
	Для Каждого СтрокаТаблицы Из ТаблицаИзмененныхДокументов Цикл
		...
		НовоеЗадание.Содержание = НСтр("ru='В документе изменены коды УКТВЭД и номера ГТД. Требуется перепечатать реестр кодов УКТВЭД и номеров ГТД .';uk='У документі змінені коди УКТЗЕД та номери ВМД. Потрібно передрукувати реєстру кодів УКТЗЕД та номерів ВМД .'");
