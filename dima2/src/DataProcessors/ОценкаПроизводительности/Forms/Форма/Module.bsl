...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщаяПроизводительностьСистемы.Пустая() Тогда
		Объект.ОбщаяПроизводительностьСистемы = НСтр("ru='Общая производительность системы';uk='Загальна продуктивність системи'");
	Иначе
	...
	Исключение
		ТекстСообщения = НСтр("ru='Не удалось загрузить настройки.';uk='Не вдалося завантажити настройки.'");
...
Процедура УказатьAPDEX(Команда)
	...
	Если СтрокаТЧ[Элемент.Имя] = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Отсутствуют замеры производительности.
            ...
            |;uk='Відсутні заміри продуктивності.
            |Розрахувати цільовий час неможливо.'"));
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("УказатьAPDEXЗавершение", ЭтотОбъект);
	Подсказка = НСтр("ru='Введите желаемое значение APDEX';uk='Введіть бажане значення APDEX'"); 
	ЗначениеAPDEX = 0;
...
Процедура УказатьAPDEXЗавершение(Знач ЗначениеAPDEX, Знач ДополнительныеПараметры) Экспорт
	...
	Если 0 > ЗначениеAPDEX Или ЗначениеAPDEX > 1 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Введен неправильный показатель APDEX.
            ...
            |;uk='Введено неправильний показник APDEX.
            |Припустимі значення від 0 до 1.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьПоказатели(ЗначенияОтбора = Неопределено)
	...
	Если ТаблицаКлючевыхОпераций = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Период установлен не верно.';uk='Період установлений не вірно.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьДиаграмму(ТаблицаКлючевыхОпераций)
	...
	ТекстЗаголовка = НСтр("ru='Диаграмма производительности с %1 по %2 - шаг: %3';uk='Діаграма продуктивності з %1 по %2 - крок: %3'");
...
Процедура ОбменятьПриоритеты(КлючеваяОперация1, Приоритет1, КлючеваяОперация2, Приоритет2)
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Выполнение операции';uk='Опрацювання операції'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
 		ВызватьИсключение;
	КонецПопытки;
КонецПроцедуры
...
Процедура УдалитьФайлыНаСервере(Каталог)
	...
	Исключение
		Событие = НСтр("ru='Удалить файлы на сервере';uk='Вилучити файли на сервері'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ВычислитьДатыВременногоОтрезка(ДатаНачала, ДатаОкончания, ИндексПериода)
	...
	Если КоличествоШагов <= ИндексПериода Тогда
		ВызватьИсключение НСтр("ru='Количество шагов не может быть меньше индекса.';uk='Кількість кроків не може бути менше індексу.'");
	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из Объект.Производительность Цикл
		...
		Если СтрокаТЧ.ЦелевоеВремя = 0 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Целевое время обязательно должно быть заполнено.';uk='Цільовий час обов''язково повинне бути заповнене.'"),
				,
				"Производительность[" + Объект.Производительность.Индекс(СтрокаТЧ) + "].ЦелевоеВремя",
				"Объект");
...
Процедура ВыбратьФайлЭкспортаПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Состояние(НСтр("ru='Экспорт данных...';uk='Експорт даних...'"));
