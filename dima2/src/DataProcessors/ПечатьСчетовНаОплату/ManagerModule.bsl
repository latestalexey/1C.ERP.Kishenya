...
Процедура ЗаполнитьРеквизитыШапкиСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, ТаблицаТовары, ПараметрыВывода)	
	...
	Иначе
		...
		СтруктураДанныхШапки.Вставить("МФОБанкаОрганизации", СведенияОПоставщике.МФО);
	...
	Если ТаблицаЭтапыОплаты.Количество() > 1 Тогда
		...
		Для Каждого ТекЭтап Из ТаблицаЭтапыОплаты Цикл
			...
			Иначе
				...
				ПараметрыСтроки.ТекстНДС = НСтр("ru='Залог за тару. Без налога (НДС).';uk='Застава за тару. Без податку (ПДВ).'");
			КонецЕсли;
		...
		Если ДанныеПечати.ЧастичнаяОплата ИЛИ ТипЗнч(ДанныеПечати.ДокументОснование) = Тип("ДокументСсылка.ОтчетКомитенту") Тогда
			...
			Если ДанныеПечати.УчитыватьНДС И НЕ ТолькоЗалогЗаТару Тогда
				...
				Для Каждого ТекСтавкаНДС Из СоответствиеСтавокНДС Цикл
					...
					СтруктураДанныхИтогоНДС.Вставить("НДС", ФормированиеПечатныхФорм.ТекстНДСПоСтавке(ТекСтавкаНДС.Ключ, ДанныеПечати.ЦенаВключаетНДС, КодЯзыкаПечать));
				...
				СтруктураДанныхПодвалСНДС = Новый Структура("ВсегоСНДС", ФормированиеПечатныхФорм.ФорматСумм(ДанныеПечати.СуммаДокумента + ?(ДанныеПечати.ЦенаВключаетНДС, 0, ИтогоСуммаНДС)));
