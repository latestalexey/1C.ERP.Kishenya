...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Параметры.Свойство("КодУзлаИнформационнойБазы", КодУзлаИнформационнойБазы) Тогда
		...
		Если Объект.УзелИнформационнойБазы.Пустая() Тогда
			...
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ru='Настройка обмена данными не найдена.';uk='Настройка обміну даними не знайдена.'"), Отказ);
			Возврат;
	...
	Иначе
		...
		ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ru='Непосредственное открытие помощника не предусмотрено.';uk='Безпосереднє відкриття помічника не передбачено.'"), Отказ);
		Возврат;
	...
	Если Не ОбменДаннымиПовтИсп.ЭтоУзелУниверсальногоОбменаДанными(Объект.УзелИнформационнойБазы) Тогда
		...
		ОбменДаннымиСервер.СообщитьОбОшибке(
			НСтр("ru='Для выбранного узла выполнение обмена данными с настройкой не предусмотрено.';uk='Для обраного вузла виконання обміну даними з настройкою не передбачено.'"), Отказ);
		Возврат;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ИзменитьЗаголовок Тогда
		Элементы.ЗаголовокСтраницы.Заголовок = НСтр("ru='Параметры подключения';uk='Параметри підключення'");
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ТекущаяСтраница = Элементы.СтраницаНачала Тогда
		ТекстПодтверждения = НСтр("ru='Отменить синхронизацию данных?';uk='Скасувати синхронізацію даних?'");
		
	Иначе
		ТекстПодтверждения = НСтр("ru='Прервать синхронизацию данных?';uk='Перервати синхронізацію даних?'");
...
Процедура ВыполнитьЗагрузкуДанныхДляСтроки(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		НСтрока = НСтр("ru='Выберите имя таблицы в поле статистической информации.';uk='Виберіть ім''я таблиці в полі статистичної інформації.'");
	...
	Если ЕстьНесопоставленныеОбъекты Тогда
		НСтрока = НСтр("ru='Имеются несопоставленные объекты.
                             ...
                             |;uk='Є незіставлені об''єкти.
                             |При завантаженні даних будуть створені дублі незіставлених об''єктів. Продовжити?'");
...
Процедура ОткрытьФормуСопоставления(Команда)
	...
	Если Не ТекущиеДанные.ИспользоватьПредварительныйПросмотр Тогда
		ПоказатьПредупреждение(, НСтр("ru='Для типа данных нельзя выполнить сопоставление объектов.';uk='Для типу даних не можна виконати зіставлення об''єктів.'"));
		Возврат;
	КонецЕсли;
...
Процедура ДополнениеВыгрузкиОчисткаОбщегоОтбора(Команда)
	...
	ТекстЗаголовка = НСтр("ru='Подтверждение';uk='Підтвердження'");
	ТекстВопроса   = НСтр("ru='Очистить общий отбор?';uk='Очистити загальний відбір?'");
...
Процедура ДополнениеВыгрузкиОчисткаДетальногоОтбора(Команда)
	ТекстЗаголовка = НСтр("ru='Подтверждение';uk='Підтвердження'");
	ТекстВопроса   = НСтр("ru='Очистить детальный отбор?';uk='Очистити детальний відбір?'");
...
Процедура ДополнениеВыгрузкиИсторияОтборов(Команда)
	...
	Текст = НСтр("ru='Сохранить текущую настройку...';uk='Зберегти поточну настройку...'");
...
Процедура ДополнениеВыгрузкиИсторияОтборовВыборИзМеню(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ПредставлениеНастройки)=Тип("Строка") Тогда
		...
		НСтр("ru='Восстановить настройки ""%1""?';uk='Відновити настройки ""%1""?'"), ПредставлениеНастройки);
...
Процедура ВыполнитьЗагрузкуДанныхДляСтрокиПродолжение(Знач ВыделенныеСтроки) 
	...
	Если Отказ Тогда
		НСтрока = НСтр("ru='При загрузке данных возникли ошибки.
                             ...
                             |;uk='При завантаженні даних виникли помилки.
	...
	Состояние(НСтр("ru='Загрузка данных завершена.';uk='Завантаження даних завершено.'"));
КонецПроцедуры
...
Процедура ОткрытьКаталогОбменаИнформациейУзла()
	...
	Если ПустаяСтрока(ИмяКаталога) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не задан каталог обмена информацией.';uk='Не заданий каталог обміну інформацією.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПослеПроверкиРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстПредупреждения = НСтр("ru='Расширение для работы с файлами не установлено, открытие каталога не возможно.';uk='Розширення для роботи з файлами не встановлено, відкриття каталогу не можливо.'");
...
Процедура ПослеОпределенияСуществованияФайла(Существует, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Каталог ""%1"" не существует или к нему нет доступа.';uk='Каталог ""%1"" не існує або до нього немає доступу.'"),
...
Процедура ОпределитьЭтоКаталог(ЭтоКаталог, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='""%1"" является файлом, а не каталогом.';uk='""%1"" є файлом, а не каталогом.'"),
			ДополнительныеПараметры.ИмяКаталога);
		ПоказатьПредупреждение(, ТекстПредупреждения);
	КонецЕсли;
...
Процедура ВидТранспортаСообщенийОбменаПриИзмененииНаСервере()
	...
	Если ОбменЧерезПодключениеККорреспонденту Тогда
		...
		Иначе
			...
			ВызватьИсключение НСтр("ru='Заданный сценарий синхронизации данных не поддерживается.';uk='Заданий сценарій синхронізації даних не підтримується.'");
...
Процедура ОбновитьСтраницыНастроекТранспорта()
	...
	Иначе
		...
		СписокВыбораТранспорта.Добавить(Неопределено, НСтр("ru='подключение не настроено';uk='підключення не настроєне'") );
...
Процедура ОбновитьОтображениеЭлементовСтатусаОбменаДанными()
	...
	Если Элементы.СтраницыСтатусаЗагрузкиДанных.ТекущаяСтраница=Элементы.СтатусЗагрузкиНеопределено Тогда
		Элементы.ПерейтиВЖурналРегистрацииСобытийЗагрузкиДанных.Заголовок = НСтр("ru='Загрузка данных не произведена';uk='Завантаження даних не зроблене'");
	Иначе
	...
	Если Элементы.СтраницыСтатусаВыгрузкиДанных.ТекущаяСтраница=Элементы.СтатусВыгрузкиНеопределено Тогда
		Элементы.ПерейтиВЖурналРегистрацииСобытийВыгрузкиДанных.Заголовок = НСтр("ru='Выгрузка данных не произведена';uk='Вивантаження даних не зроблено'");
	Иначе
...
Функция Подключаемый_СтраницаНачала_ПриПереходеДалее(Отказ)
	...
	ИначеЕсли Объект.ВидТранспортаСообщенийОбмена.Пустая() Тогда
		...
		НСтрока = НСтр("ru='Укажите вариант подключения';uk='Укажіть варіант підключення'");
	...
	ИначеЕсли ОбменЧерезВебСервис И ПустаяСтрока(WSПароль) Тогда
		...
		НСтрока = НСтр("ru='Не указан пароль.';uk='Не зазначений пароль.'");
...
Процедура ТранспортСообщенияОбмена()
	...
	Исключение
		...
		ЗаписатьОшибкуВЖурналРегистрации(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
			НСтр("ru='Помощник интерактивного обмена данными.Транспорт сообщения обмена';uk='Помічник інтерактивного обміну даними.Транспорт повідомлення обміну'"));
		Возврат;
	КонецПопытки;
...
Процедура АнализДанных()
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Анализ данных сообщения обмена';uk='Аналіз дані повідомлення обміну'"));
...
Процедура АнализДанныхДлительнаяОперацияОкончание()
	...
	Исключение
		ПропуститьПолучениеДанных = Истина;
		ЗаписатьОшибкуВЖурналРегистрации(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
			НСтр("ru='Помощник интерактивного обмена данными.Анализ данных';uk='Помічник інтерактивного обміну даними.Аналіз даних'"));
		Возврат;
	КонецПопытки;
	...
	ИначеЕсли РезультатАнализа.Свойство("Отказ") Тогда
		...
		Если ЗафиксироватьОшибку Тогда
			ОбменДаннымиВызовСервера.ЗафиксироватьЗавершениеОбменаСОшибкой(
				...
				"ЗагрузкаДанных",
				ТекущаяДатаСеанса(),
				НСтр("ru='Не удалось загрузить данные из ""%1"" (этап анализа данных).';uk='Не вдалося завантажити дані з ""%1"" (етап аналізу даних).'"));
		КонецЕсли;
	Иначе
...
Процедура ЗагрузкаДанных()
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Загрузка данных из сообщения обмена';uk='Завантаження даних з повідомлення обміну'"));
...
Процедура ВыгрузкаДанных()
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Выгрузка данных из сообщения обмена';uk='Вивантаження даних з повідомлення обміну'"));
...
Процедура УстановитьОписаниеДополненияОбщегоОтбора()
	...
	Если НетОтбора Тогда
		Текст = НСтр("ru='Все документы';uk='Всі документи'");
	КонецЕсли;
...
Процедура УстановитьОписаниеДополненияДетально()
	...
	НетОтбора = ПустаяСтрока(Текст);
	Если НетОтбора Тогда
		Текст = НСтр("ru='Дополнительные данные не выбраны';uk='Додаткові дані не обрані'");
	КонецЕсли;
