	...
	Если ТребуетсяПрименениеРазрешенийВКластереСерверов() Тогда
		...
		Исключение
			ВызватьИсключение НСтр("ru='Ошибка конкурентного доступа к настройке разрешений на использование внешних ресурсов.
                                ...
                                |;uk='Помилка конкурентного доступу до настройки дозволів на використання зовнішніх ресурсів.
                                |Повторіть пізніше.'");
		КонецПопытки;
...
Функция НовоеОписаниеПрофиляБезопасности(Знач ТипПрограммногоМодуля, Знач ИдентификаторПрограммногоМодуля) Экспорт
	...
	Шаблон = НСтр("ru='[ИБ %1] %2 ""%3""';uk='[ІБ %1] %2 ""%3""'");
	...
	Если ПустаяСтрока(ИмяИБ) Тогда
		ВызватьИсключение НСтр("ru='Строка соединения информационной базы не содержит имени информационной базы!';uk='Рядок з''єднання інформаційної бази не містить імені інформаційної бази!'");
	КонецЕсли;
	
	Если ТипПрограммногоМодуля = Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка() Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ИмяИБ,
			НСтр("ru='Профиль безопасности для информационной базы';uk='Профіль безпеки для інформаційної бази'"), СтрокаСоединенияИнформационнойБазы());
	Иначе
