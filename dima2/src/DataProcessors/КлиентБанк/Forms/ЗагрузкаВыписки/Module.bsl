	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Для импрта выписки из банковского счета используется XML файл.
                ...
                |Імпорт з XML не поддержуется для Web клієнта. Виберіть інший клієнт.'"),
			,
	...
	Для каждого Счет из Объект.БанковскиеСчета Цикл
		...
		Если ЗначениеЗаполнено(Счет.НастройкаОбмена) Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
				...
				НСтр("ru='Для импрта выписки из банковского счета %1 используется XML файл. Импорт из XML не поддержуется для Web клиента. Выберите другой клиент.';uk='Для імпорту виписки з банківського рахунку %1 використовується XML файл. Імпорт з XML не поддержуется для Web клієнта. Виберіть інший клієнт.'"), Счет.Ссылка);
			КонецЕсли; 
			#КонецЕсли
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
