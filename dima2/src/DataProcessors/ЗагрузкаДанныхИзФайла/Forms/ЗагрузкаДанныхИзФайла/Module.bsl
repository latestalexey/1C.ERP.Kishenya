...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение(НСтр("ru='Недостаточно прав для открытия загрузки данных из файла';uk='Недостатньо прав для відкриття завантаження даних з файлу'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ПодтверждениеЗакрытияФормы <> Неопределено Тогда
		Если ПодтверждениеЗакрытияФормы <> Истина Тогда
			...
			Отказ = Истина;
			ТекстВопроса = НСтр("ru='Введенные данные не будут записаны. Закрыть форму?';uk='Введені дані не будуть записані. Закрити форму?'");
...
Процедура ОтменитьСопоставление(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ru='Отменить сопоставление?';uk='Скасувати зіставлення?'"), РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура Назад(Команда)
	...
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ЗаполнениеТаблицыДанными Тогда
		...
		ЭтотОбъект.Заголовок = НСтр("ru='Загрузка данных в справочник';uk='Завантаження даних в довідник'");
		ОчиститьТаблицу();
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных ИЛИ Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.НеНайдено Тогда
		...
		Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда 
			Элементы.Далее.Заголовок = НСтр("ru='Вставить в список';uk='Вставити в список'");
		Иначе
...
Процедура ПереходКСледующемуШагуЗагрузкиДанных()
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.РезультатыСопоставления Тогда
		...
		Элементы.Далее.Заголовок = НСтр("ru='Вставить в список';uk='Вставити в список'");
		Элементы.Далее.КнопкаПоУмолчанию = Истина;
		Элементы.Назад.Заголовок = НСтр("ru='< В начало';uk='< На початок'");
		...
		Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			Если УстановитьОформлениеДляПолейСНеоднозначностью(Отбор) Тогда
				ПоказатьВопрос(Новый ОписаниеОповещения("ПослеВопросаОДобавлениеВТабличнуюЧасть", ЭтотОбъект),
					НСтр("ru='Строки в которых не заполнены обязательные колонки будут пропущены.';uk='Рядки, у яких не заповнені обов''язкові колонки будуть пропущені.'")
					+ Символы.ПС + НСтр("ru='Продолжить?';uk='Продовжити?'"), РежимДиалогаВопрос.ДаНет);
				Возврат;
			КонецЕсли;
			...
			Закрыть(АдресЗагруженныхДанных);
		ИначеЕсли ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
			Элементы.Назад.Заголовок = НСтр("ru='< В начало_';uk='< На початок'");
			ЗакрытьФормуИВернутьМассивСсылок();
		Иначе
...
Процедура ВыполнитьСопоставление()
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Выполнено сопоставление';uk='Виконано зіставлення'"),, НСтр("ru='Сопоставлено элементов:';uk='Зіставлено елементів:'") + " " + Строка(КоличествоСопоставленныхПоКолонкам));
	ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
КонецПроцедуры
...
Процедура ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла()
	...
	ТекстВсе = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Все (%1)';uk='Всі (%1)'"), Статистика.Всего);
	
	Элементы.СоздаватьЕслиНеСопоставлено.Заголовок = НСтр("ru='Несопоставленные (';uk='Незіставлені ('") + Статистика.НеСопоставленных + ")";
	Элементы.ОбновлятьСуществующиеЗаголовок.Заголовок = НСтр("ru='Сопоставленные элементы (';uk='Зіставлені елементи ('") + Строка(Статистика.Сопоставленных) + ")";
	
	СписокВыбора = Элементы.ФильтрТаблицаСопоставления.СписокВыбора;
	СписокВыбора.Очистить();
	СписокВыбора.Добавить("Все", ТекстВсе, Истина);
	СписокВыбора.Добавить("Несопоставленные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Несопоставленные (%1 из %2)';uk='Незіставлені (%1 з %2)'"),
		Статистика.НеСопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Сопоставленные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сопоставленные (%1 из %2)';uk='Зіставлені (%1 з %2)'"),
		Статистика.Сопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Неоднозначные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неоднозначные (%1 из %2)';uk='Неоднозначні (%1 з %2)'"),
	...
	Если Статистика.Неоднозначных > 0 Тогда 
		...
		Элементы.ОписаниеНеоднозначность.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='(неоднозначностей: %1)';uk='(невизначеностей: %1)'"),
...
Процедура ВыгрузитьШаблонВФайлЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогВыбораФайла.Фильтр                      = НСтр("ru='Книга Excel 97 (*.xls)|*.xls|Книга Excel 2007 (*.xlsx)|*.xlsx|Электронная таблица OpenDocument (*.ods)|*.ods|Текстовый документ c разделителями (*.csv)|*.csv|Табличный документ (*.mxl)|*.mxl';uk='Книга Excel 97 (*.xls)|*.xls|Книга Excel 2007 (*.xlsx)|*.xlsx|Електронна таблиця OpenDocument (*.ods)|*.ods|Текстовий документ c роздільниками (*.csv)|*.csv|Табличний документ (*.mxl)|*.mxl'");
...
Процедура ИнициализацияВставкиИзБуфераОбмена()
	...
	Если Параметры.Свойство("ПредставлениеПоля") Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вставка из буфера обмена (%1)';uk='Вставка з буфера обміну (%1)'"), Параметры.ПредставлениеПоля);
	Иначе
		Заголовок = НСтр("ru='Вставка из буфера обмена';uk='Вставка з буфера обміну'");
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовФормы()
	ЭтотОбъект.Заголовок = ?(ПустаяСтрока(Параметры.Заголовок), НСтр("ru='Загрузка данных в справочник';uk='Завантаження даних в довідник'"), Параметры.Заголовок);
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
        ...
        |;uk='Вибір довідника для завантаження даних з електронних таблиць, розташованих у зовнішніх файлах 
        |(наприклад: Microsoft Office Excel, OpenOffice Calc та ін).'");
		ВидЗагрузкиДанных = 1;
	КонецЕсли;
...
Процедура СформироватьМакетПоТипуЗагрузки()
	...
	Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
		ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка данных в справочник ""%1""';uk='Завантаження даних у довідник ""%1""'"), ПредставлениеСправочника(ИмяОбъектаСопоставления));
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхНаСервере(ФоновоеЗадание = Ложь)
	...
	Иначе
		...
		НСтр("ru='ЗагрузкаДанныхИзФайла: Выполнение серверного метода обработки ЗаполнитьТаблицуСопоставленияДаннымиИзШаблона';uk='ЗагрузкаДанныхИзФайла: Виконання серверного методу обробки ЗаполнитьТаблицуСопоставленияДаннымиИзШаблона'"));
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанных()
	...
	Если ТаблицаСДаннымиПустая() Тогда
		ПоказатьПредупреждение(, (НСтр("ru='Для перехода к этапу сопоставления и загрузки данных, необходимо заполнить таблицу.';uk='Для переходу до етапу зіставлення і завантаження даних, необхідно заповнити таблицю.'")));	
		Возврат;
	КонецЕсли;
	...
	Если СписокНезаполненныхКолонок.Количество() > 0 Тогда
		Если СписокНезаполненныхКолонок.Количество() =1  Тогда
			ТекстПроКолонки = НСтр("ru='Обязательная колонка""';uk='Обов''язкова колонка""'") + " " + СписокНезаполненныхКолонок[0]
				+ НСтр("ru='"" содержит незаполненные строки, эти строки будут пропущены при загрузке';uk='""містить незаповнені рядки, ці рядки будуть пропущені при завантаженні'");
		Иначе
			ТекстПроКолонки = НСтр("ru='Обязательные колонки""';uk='Обов''язкові колонки""'") + " " + СтрСоединить(СписокНезаполненныхКолонок,", ")
				+ НСтр("ru='"" содержат незаполненные строки, эти строки будут пропущены при загрузке';uk='""містять незаповнені рядки, ці рядки будуть пропущені при завантаженні'");
		КонецЕсли;
		ТекстПроКолонки = ТекстПроКолонки + Символы.ПС + НСтр("ru='Продолжить?';uk='Продовжити?'");
...
Функция ФоновоеЗаданиеЗаписьПолучитьРезультат()
	...
	Исключение
		...
		ТекстСообщенияОбОшибке = НСтр("ru='Не удалось произвести запись данных.
                                    ...
                                    |;uk='Не вдалося виконати запис даних.
                                    |Можлива причина: Завантажуються некоректні дані.
                                    |Технічна інформація: %1'");
...
Процедура ПоказатьОтчет(Отчет)
	...
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("ВсеЭлементы", НСтр("ru='Все (';uk='Усі ('") + Отчет.Всего + ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Новые", НСтр("ru='Новые (';uk='Нові ('") + Отчет.Создано+ ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Обновленные", НСтр("ru='Обновленные (';uk='Оновлені ('") + Отчет.Обновлено+ ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Пропущенные", НСтр("ru='Пропущенные (';uk='Пропущені ('") + Отчет.Пропущено+ ")");
...
Функция ФоновоеЗаданиеЗагрузкаФайлаПолучитьРезультат()
	...
	Исключение
		...
		ТекстСообщенияОбОшибке = НСтр("ru='Не удалось произвести загрузку данных.
                                    |Возможная причина: Загружаемые данные некорректные.
                                    |Техническая информация: %1'
                                    |;uk='Не вдалося провести завантаження даних.
                                    |Можлива причина: Завантажуються некоректні дані.
                                    |Технічна інформація: %1'");
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщенияОбОшибке, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииЗагрузкаДанныхИзФайла(),
...
Функция ФоновоеЗаданиеСопоставлениеПолучитьРезультат()
	...
	Исключение
		...
		ТекстСообщенияОбОшибке = НСтр("ru='Не удалось произвести сопоставление данных.
                                    |Возможная причина: Загружаемые данные некорректные.
                                    |Техническая информация: %1'
                                    |;uk='Не вдалося провести зіставлення даних.
                                    |Можлива причина: Завантажуються некоректні дані.
                                    |Технічна інформація: %1'");
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщенияОбОшибке, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииЗагрузкаДанныхИзФайла(),
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент()
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Если Статистика.Сопоставленных > 0 Тогда
			ТекстНайдено = НСтр("ru='Из %1 введенных строк в список будут вставлены: %2.';uk='З %1 введених рядків у списку будуть вставлені: %2.'");
			...
			Если Статистика.Неоднозначных > 0 И Статистика.НеНайдено > 0 Тогда 
				ТекстНеНайдено = НСтр("ru='%3 строк будут пропущены:';uk='%3 рядків будуть пропущені:'") + Символы.ПС + "  - " + НСтр("ru='Нет данных в программе: %1';uk='Немає даних в програмі: %1'") 
					+ Символы.ПС + "  - " +НСтр("ru='Несколько вариантов для вставки: %2';uk='Кілька варіантів для вставки: %2'");
				ТекстНеНайдено = НСтр("ru='Будет пропущено строк: %3
                |  - Нет данных в программе: %1
                |  - Несколько вариантов для вставки: %2'
                |;uk='Буде пропущено рядків: %3
                |- Немає даних в програмі: %1
                |- Кілька варіантів для вставки: %2'");
			...
			ИначеЕсли Статистика.Неоднозначных > 0 Тогда
				ТекстНеНайдено = НСтр("ru='Строки, для которых в программе имеется несколько вариантов, будут пропущены: %1';uk='Рядки, для яких в програмі є кілька варіантів, будуть пропущені: %1'");
			...
			ИначеЕсли Статистика.НеНайдено > 0 Тогда
				ТекстНеНайдено = НСтр("ru='Строки, для которых в программе нет соответствующих данных, будут пропущены: %1';uk='Рядки, для яких в програмі немає відповідних даних, будуть пропущені: %1'");
				ТекстНеНайдено = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНеНайдено, Статистика.НеНайдено);
			КонецЕсли;
			ТекстНеНайдено = ТекстНеНайдено + Символы.ПС + НСтр("ru='Для просмотра пропущенных строк и подбора данных для вставки нажмите ""Далее"".';uk='Для перегляду пропущених рядків і підбору даних для вставки натисніть ""Далі"".'");
			...
			Элементы.Далее.Видимость = Истина;
			Элементы.Назад.Заголовок = НСтр("ru='< Назад';uk='< Назад'");
			Элементы.Далее.Заголовок = НСтр("ru='Далее >';uk='Далі >'");
			...
			ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПоискаСсылок, Ложь, НСтр("ru='Далее >';uk='Далі >'"));
		Иначе
	...
	Иначе 
		...
		Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Истина, Элементы.ПояснениеДляУниверсальнойЗагрузки, Истина, НСтр("ru='Загрузить данные >';uk='Завантажити дані >'"));
		...
		ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляТабличнойЧасти, Истина, НСтр("ru='Загрузить данные';uk='Завантажити дані'"));
			УстановитьОформлениеДляПолейСНеоднозначностью(Новый Структура("РезультатСопоставленияСтроки", "Неоднозначность"));
			
		ИначеЕсли ТипЗагрузки = "ВнешняяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПрикладнойЗагрузки, Ложь, НСтр("ru='Загрузить данные >';uk='Завантажити дані >'"));
		Иначе
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПрикладнойЗагрузки, Ложь, НСтр("ru='Загрузить данные >';uk='Завантажити дані >'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаписатьЗагружаемыеДанныеОтчет(ФоновоеЗадание = Ложь)
	...
	Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
		...
		РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор, 
				...
				ПараметрыВызоваСервера, 
				НСтр("ru='Подсистема ЗагрузкаДанныхИзФайла: Запись загружаемых  данных';uk='Підсистема ЗагрузкаДанныхИзФайла: Запис даних, що завантажуються'"));
...
Процедура УстановитьОформлениеДанных(СписокКолонок = Неопределено)
	...
	Если ЗначениеЗаполнено(СписокКолонок) Тогда
		Для каждого ИмяКолонки Из СписокКолонок Цикл
			...
			ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<неоднозначность>';uk='<невизначеність>'"));
		КонецЦикла;
	Иначе
		ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
		ПолеОформления = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
...
Функция МенеджерОбъекта(ИмяОбъектаСопоставления)
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Объект ""%1"" не найден';uk='Об''єкт ""%1"" не знайдено'"), ИмяОбъектаСопоставления);
		КонецЕсли;
...
Процедура СформироватьОтчетОЗагрузке(ТипОтчета = "ВсеЭлементы", ФоновоеЗадание = Ложь, РассчитыватьПроцентПрогресса = Ложь)
	...
	РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор, 
			...
			ПараметрыВызоваСервера, 
			НСтр("ru='Подсистема ЗагрузкаДанныхИзФайла: Выполнение серверного метода обработки сформировать отчет о загрузке';uk='Підсистема ЗагрузкаДанныхИзФайла: Виконання серверного методу обробки сформувати звіт про завантаження'"));
...
Процедура ПоказатьИнформационнуюСтрокуПроОбязательныеКолонки()
	...
	Если Элементы.СтраницыЗаполненияДанных.ТекущаяСтраница = Элементы.СтраницаВариантЗагрузкаИзФайла Тогда
        ...
        |;uk='Для завантаження даних необхідно зберегти бланк у файл для заповнення іншою програмою. 
        |Потім завантажити заповнену таблицю в одному з форматів:
        |• Книги Microsoft Excel 97 (.xls) і Excel 2007 (.xlsx)
        |• Електронні таблиці LibreOffice Calc (.ods)
        |• Текст із роздільниками (.csv)
        |• Табличний документ (.mxl)'") + Символы.ПС;
	Иначе
		ТекстПодсказки = НСтр("ru='Для заполнения таблицы необходимо скопировать данные в таблицу из внешнего файла через буфер обмена.';uk='Для заповнення таблиці необхідно скопіювати дані в таблицю із зовнішнього файлу через буфер обміну.'") + Символы.ПС;
	КонецЕсли;
	...
	Если ОбязательныеКолонки.Количество() > 0 Тогда 
		...
		Если ОбязательныеКолонки.Количество() = 1 Тогда
			ТекстПодсказки = ТекстПодсказки + НСтр("ru='Колонка, обязательная для заполнения:';uk='Колонка, обов''язкова для заповнення:'") + " " + СписокКолонок;
		Иначе
			ТекстПодсказки = ТекстПодсказки + НСтр("ru='Колонки, обязательные для заполнения:';uk='Колонки, обов''язкові для заповнення:'") + " " + СписокКолонок;
		КонецЕсли;
...
Процедура ДобавитьСтандартныеКолонкиВТаблицуСопоставления(ВременнаяТЗ, СтруктураОбъектаСопоставления, ДобавитьИдентификатор,
	...
	Если ДобавитьИдентификатор Тогда 
		ВременнаяТЗ.Колонки.Добавить("Идентификатор", Новый ОписаниеТипов("Число"), НСтр("ru='п/п';uk='п/п'"));
	КонецЕсли;
	...
	Если ДобавитьРезультатСопоставленияСтроки Тогда 
		ВременнаяТЗ.Колонки.Добавить("РезультатСопоставленияСтроки", Новый ОписаниеТипов("Строка"), НСтр("ru='Результат';uk='Результат'"));
	КонецЕсли;
	Если ДобавитьОписаниеОшибки Тогда
		ВременнаяТЗ.Колонки.Добавить("ОписаниеОшибки", Новый ОписаниеТипов("Строка"), НСтр("ru='Причина';uk='Причина'"));
	КонецЕсли;
		...
		Если СтрНачинаетсяС(Колонка.Имя, "ТЧ_") Тогда
			...
			Если КолонкиГруппировки.Количество() = 1 Тогда
				...
				Если КолонкаУровень2 <> Неопределено Тогда 
					...
					Если ТипКолонки <> Неопределено И СтрНайти(ТипКолонки.ПолноеИмя(), "Справочник") > 0 Тогда
						НовыйЭлемент.Заголовок = НСтр("ru='Данные из файла';uk='Дані з файлу'");
					Иначе
			...
			ИначеЕсли КолонкиГруппировки.Количество() > 1 Тогда
				...
				Префикс = НСтр("ru='Данные из файла:';uk='Дані з файлу:'");
...
Функция СобытиеЖурналаРегистрацииЗагрузкаДанныхИзФайла() 
	...
	Возврат НСтр("ru='Загрузка данных из файла';uk='Завантаження даних з файлу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПослеВыбораФайлаДляСохранения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ВыбранныйФайл.Имя) Тогда
			...
			Иначе
				ПоказатьПредупреждение(, НСтр("ru='Шаблон файла не был сохранен.';uk='Шаблон файлу не був збережений.'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, Расширение, ФоновоеЗадание = Ложь)
	...
	РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор, 
		...
		ПараметрыВызоваСервера, 
		НСтр("ru='Подсистема ЗагрузкаДанныхИзФайла: Выполнение серверного метода загрузка данных из файла';uk='Підсистема ЗагрузкаДанныхИзФайла: Виконання серверного методу завантаження даних з файлу'"));
