...
Функция ЗаголовокГруппыОтбораВсехДокументов() Экспорт
	Возврат НСтр("ru='Все документы';uk='Всі документи'");
КонецФункции
...
Функция ЗаголовокГруппыОтбораВсехСправочников() Экспорт
	Возврат НСтр("ru='Все справочники';uk='Всі довідники'");
КонецФункции
...
Функция ИнициализироватьКомпоновщик(СписокИменМетаданных = Неопределено, ОграничиватьИспользованиеОтбором = Ложь, АдресСохраненияСхемы = Неопределено)
	...
	ПараметрАвтоматически.Значение = НСтр("ru='По общим правилам';uk='За загальними правилами'");
	
	ПараметрДополнительно = Параметры.Найти("ПричинаРегистрацииДополнительно");
	ПараметрДополнительно.Значение = НСтр("ru='Дополнительно';uk='Додатково'");
	
	ПараметрПоСсылке = Параметры.Найти("ПричинаРегистрацииПоСсылке");
	ПараметрПоСсылке.Значение = НСтр("ru='По ссылке';uk='За посиланням'");
...
Процедура ДобавитьДополнительныеНаборыКомпоновкиТабличныхЧастей(ЭлементыИсточника, ПараметрыДобавления)
	...
	Если МетаданныеОбъекта = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Некорректное имя метаданных ""%1"" для регистрации на узле ""%2""';uk='Некоректне ім''я метаданих ""%1"" для реєстрації на вузлі ""%2""'"),
...
Функция ТекстДополнительныхПараметров()
	...
	Если ВариантВыгрузки = 0 Тогда
		...
		Возврат НСтр("ru='Без дополнительных данных.';uk=' Без додаткових даних.'");
