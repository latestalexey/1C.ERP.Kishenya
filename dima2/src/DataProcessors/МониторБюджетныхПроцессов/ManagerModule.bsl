...
Процедура ЗаполнитьДиаграмму(МодельБюджетирования, Результат, НачалоПериода, КонецПериода, 
	...
	Если Группировки = "Вариант1" Тогда
		ТаблицаЗадач = Справочники.МоделиБюджетирования.ПолучитьБюджетныеЗадачи(МодельБюджетирования, НачалоПериода, КонецПериода, ТипОтчета > 0, Ложь);
	Иначе
		ТаблицаЗадач = Справочники.МоделиБюджетирования.ПолучитьБюджетныеЗадачи(МодельБюджетирования, НачалоПериода, КонецПериода, ТипОтчета > 0, Истина);
	КонецЕсли;
	...
	Компоновка = Обработки.МониторБюджетныхПроцессов.ПолучитьМакет("СхемаКомпоновкиДанных");
	...
	ДеревоРезультат = ФинансоваяОтчетностьСервер.ВыгрузитьРезультатСКД(Компоновка, КомпоновщикНастроек.Настройки, 
																		ВнешниеНаборыДанных, Группировки = "Вариант1");
	...
	Если Группировки = "Вариант1" Тогда
		СтруктураПоиска = Новый Структура("ЭтапПодготовкиБюджетовДействие, ПовторяемыйЭтапПодготовкиБюджетов",
		...
		Для Каждого ПовторяемаяГруппа из МассивПовторяемых Цикл
			Действие = НастройкиДействий[ПовторяемаяГруппа].НастройкаДействия.Получить();
	...
	Для Каждого СтрокаПериода из ДеревоПериодов.Строки Цикл
		...
		Если Не ПоИсполнителям ИЛИ ЗначениеЗаполнено(СтрокаПериода.Родитель) Тогда
			...
			Для Каждого НайденнаяСтрока из НайденныеСтроки Цикл
				...
				Если ЗначениеЗаполнено(НайденнаяСтрока.ПовторяемыйЭтапПодготовкиБюджетов) Тогда
					СтрокаЗадачи.ЭтапПодготовкиБюджетовНаименование = 
						НайденнаяСтрока.ПовторяемыйЭтапПодготовкиБюджетовНаименование + " ("
				...
				Если ЗначениеЗаполнено(НайденнаяСтрока.ПроцессПодготовкиБюджетов) Тогда
					Процесс = НСтр("ru=', Процесс №';uk=', Процес №'") + НайденнаяСтрока.ПроцессПодготовкиБюджетовНомер;
				КонецЕсли;
				СтрокаЗадачи.ЭтапПодготовкиБюджетовНаименование = 
...
Процедура ОбработатьПовторяемыеЭтапыПроцесса(ДеревоПриемник, ДеревоЭтапов, СоответствиеРодителей)
	...
	Для Каждого СтрокаДерева из ДеревоЭтапов.Строки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДерева.ПовторяемыйЭтапПодготовкиБюджетов) Тогда
			РодителиГруппыСсылки = СоответствиеРодителей[СтрокаДерева.ЭтапПодготовкиБюджетов];
			Если РодителиГруппыСсылки.Найти(СтрокаДерева.ПовторяемыйЭтапПодготовкиБюджетов) <> Неопределено Тогда
				НужноДобавлять = Ложь;
			Иначе
				СтрокаДерева.ЭтапПодготовкиБюджетовНаименование = СтрокаДерева.ПовторяемыйЭтапПодготовкиБюджетовНаименование;
				СтрокаДерева.ЭтапПодготовкиБюджетовОтветственный = СтрокаДерева.ПовторяемыйЭтапПодготовкиБюджетовОтветственный;
				СтрокаДерева.ЭтапПодготовкиБюджетовЭтоГруппа = СтрокаДерева.ПовторяемыйЭтапПодготовкиБюджетовЭтоГруппа;
				СтрокаДерева.ЭтапПодготовкиБюджетовВыполнятьАвтоматически = СтрокаДерева.ПовторяемыйЭтапПодготовкиБюджетовВыполнятьАвтоматически;
			КонецЕсли;
		КонецЕсли;
...
Функция СкопироватьДеревоПроцессаВДеревоРезультат(Процесс, Источник, Приемник, 
	...
	Для Каждого СтрокаИсточника из Источник Цикл
		...
		Если ЗначениеЗаполнено(НоваяСтрока.ПовторяемыйЭтапПодготовкиБюджетов) Тогда
			Реквизиты = КэшРеквизитовЭтапов.Найти(НоваяСтрока.ПовторяемыйЭтапПодготовкиБюджетов);
			НоваяСтрока.ЭтапПодготовкиБюджетов 							= НоваяСтрока.ПовторяемыйЭтапПодготовкиБюджетов;
		...
		Иначе
			...
			СтруктураПоиска = Новый Структура(
				"ЭтапПодготовкиБюджетов, ПовторяемыйЭтапПодготовкиБюджетов, ПроцессПодготовкиБюджетов",
				СтрокаИсточника.ЭтапПодготовкиБюджетов, СтрокаИсточника.ПовторяемыйЭтапПодготовкиБюджетов, Процесс);
...
Процедура ЗаполнитьЗадачиОсновногоПланаПоПервомуПроцессу(Процесс, МассивСтрокКЗаполнениюЗадач, 
	...
	Для Каждого СтрокаКЗаполнению из МассивСтрокКЗаполнениюЗадач Цикл
		...
		СтруктураПоиска = Новый Структура("ПроцессПодготовкиБюджетов, ЭтапПодготовкиБюджетов,
										|ПовторяемыйЭтапПодготовкиБюджетов, Период",
										Процесс, СтрокаКЗаполнению.ЭтапПодготовкиБюджетов,
										СтрокаКЗаполнению.ПовторяемыйЭтапПодготовкиБюджетов, СтрокаКЗаполнению.Период);
...
Процедура ДополнитьДеревоВыводаПроцессами(МодельБюджетирования, ДеревоЗадач, ТаблицаЗадач, ДатаНачала, ДатаОкончания, ДопПараметры)
	...
	|ГДЕ Таблица.Период МЕЖДУ &ДатаНачала И &ДатаОкончания //%ФильтрНаИсполнителя
	|;
	...
	Если ДопПараметры <> Неопределено и ДопПараметры.Свойство("ФлагИсполнитель") Тогда
		Если ДопПараметры.ФлагИсполнитель Тогда
			...
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "//%ФильтрНаИсполнителя", " И Таблица.Исполнитель = &Исполнитель");
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьЗначениеДерева(СтрокаДерева)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Неизвестная группировка диаграммы ганта бюджетного процесса!';uk='Невідоме групування діаграми ганта бюджетного процесу!'")
	КонецЕсли;
...
Процедура ДобавитьИнтервалДиаграммы(Интервал, СтрокаДерева, Тип, УровеньЗадачи = Ложь)
	...
	Для Каждого СтрокаЗадачи из СтрокаДерева.Строки Цикл
		...
		Если Тип = "Факт" И ЗначениеЗаполнено(СтрокаЗадачи.Задача) Тогда
			ПредставлениеЗначения = НСтр("ru='%1 №%2 от %3 (%4)';uk='%1 №%2 від %3 (%4)'");
		...
		Если УровеньЗадачи Тогда
			ДополнениеПредставления = НСтр("ru=' с %1 по %2';uk=' з %1 по %2'");
	...
	Для Каждого СтрокаДерева из СтрокиДерева.Строки Цикл
		...
		Если СтрокаДерева.Строки.Количество() Тогда
			...
			Если Точка.Значение = Справочники.ЭтапыПодготовкиБюджетов.ПустаяСсылка() Тогда
				Точка.Текст = НСтр("ru='<Вне бюджетного процесса>';uk='<Поза бюджетного процесу>'");
			Иначе
				Если Группировки = "Вариант1" Тогда
					...
					Если ЗначениеЗаполнено(СтрокаДерева.ЭтапПодготовкиБюджетов) Тогда
						...
						Иначе
							...
							Если СтрокаДерева.ЭтапПодготовкиБюджетовВыполнятьАвтоматически = Истина Тогда
								Точка.Текст = Точка.Текст + НСтр("ru=', <Автоматически>';uk=', <Автоматично>'");
				...
				Иначе
					...
					Иначе
						...
						РасшифровкаПериода = НСтр("ru='Период планирования: %1';uk='Період планування: %1'");
		...
		Иначе
			...
			Иначе
				...
				Если СтрокаДерева.Исполнитель <> СтрокаДерева.Родитель.ЭтапПодготовкиБюджетовОтветственный Тогда
					ТочкаЗадачи.Текст = ТочкаЗадачи.Текст + ", " + Строка(СтрокаДерева.Исполнитель);
				КонецЕсли;
				Если ТочкаРодитель.Текст = НСтр("ru='<Вне бюджетного процесса>';uk='<Поза бюджетного процесу>'") Тогда
...
Процедура ОформитьДиаграммуПослеВывода(Результат, ТипОтчета, Группировки, Знач НачалоПериода, Знач КонецПериода)
	...
	Если НачалоКвартала(НачалоПериода) <> НачалоКвартала(КонецПериода) Тогда
		...
		НоваяШкала.Формат = "ДФ='к """ + НСтр("ru='кв.';uk='кв.'") + """ yyyy'";
		Добавлено = Добавлено + 1;
	КонецЕсли;
	...
	Если Группировки = "Вариант1" Тогда
		Заголовок = НСтр("ru='Этап процесса /%Период планирования';uk='Етап процесу /%Период планування'");
	ИначеЕсли Группировки = "Вариант2" Тогда
		Заголовок = НСтр("ru='Период планирования /%Задача';uk='Період планування /%Задача'");
	ИначеЕсли Группировки = "Вариант3" Тогда
		Заголовок = НСтр("ru='Исполнитель /%Период планирования /%Задача';uk='Виконавець /%Период планування /%Задача'");
	КонецЕсли;
