...
Процедура ПользовательСсылкаПриИзменении(Элемент)
	...
	Если ПользовательСсылка <> Неопределено
		...
		ПоказатьПредупреждение(,НСтр("ru='У выбранного пользователя нет настроек, которые можно было бы
                ...
                |;uk='У обраного користувача налаштувань, які можна було б
                |скопіювати, виберіть користувача.'"));
	...
	Если ПользовательСсылка <> Неопределено
		...
		Если ПользователиПолучателиНастроек.МассивПользователей.Найти(ПользовательСсылка) <> Неопределено Тогда
			ПоказатьПредупреждение(,НСтр("ru='Нельзя копировать настройки пользователя самому себе,
                    ...
                    |;uk='Не можна копіювати настройки користувача самому собі,
                    |виберіть іншого користувача.'"));
				ПользовательСсылка = ПользовательСсылкаСтарый;
				Возврат;
		КонецЕсли;
...
Процедура Скопировать(Команда)
	...
	Если ПользовательСсылка = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Выберите пользователя, чьи настройки необходимо скопировать.';uk='Виберіть користувача, чиї настройки необхідно скопіювати.'"), , "ПользовательСсылка");
		Возврат;
	КонецЕсли;
	
	Если КоличествоПользователей = 0 И ПереключательКомуКопироватьНастройки <> "ВсемПользователям" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Выберите одного или несколько пользователей, которым необходимо скопировать настройки.';uk='Виберіть одного або кілька користувачів, яким необхідно скопіювати настройки.'"), , "Приемник");
		Возврат;
	КонецЕсли;
	
	Если ПереключательКопируемыеНастройки = "СкопироватьОтдельные" И КоличествоНастроек = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Выберите настройки, которые необходимо скопировать.';uk='Виберіть настройки, які необхідно скопіювати.'"), , "ПереключательКопируемыеНастройки");
		Возврат;
	КонецЕсли;
...
Процедура СкопироватьНастройки(ИмяКоманды)
	...
	Иначе
		ПояснениеКомуСкопированыНастройки = НСтр("ru='всем пользователям';uk='всім користувачам'");
	КонецЕсли;
	...
	Если ПереключательКопируемыеНастройки = "СкопироватьОтдельные" Тогда
		...
		Если Отчет = Неопределено Тогда
			...
			ПоказатьОповещениеПользователя(НСтр("ru='Копирование настроек';uk='Копіювання настройок'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
	...
	Иначе
		...
		Если Не НастройкиСкопированы Тогда
			...
			ТекстПредупреждения = НСтр("ru='Настройки не были скопированы, так как у пользователя ""%1"" не было сохранено ни одной настройки.';uk='Настройки не були скопійовані, тому що в користувача ""%1"" не було збережено жодної настройки.'");
		...
		ТекстПояснения = НСтр("ru='Скопированы все настройки %1';uk='Скопійовано всі настройки %1'");
