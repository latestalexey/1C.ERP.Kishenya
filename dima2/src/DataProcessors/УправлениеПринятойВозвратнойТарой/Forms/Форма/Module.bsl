...
Функция СообщитьОбОшибкахФормированияДанныхЗаполненияВозвратаВыкупаТары(ВыборкаРеквизиты)
	...
	//++ НЕ УТКА
	Если ВыборкаРеквизиты.ЕстьОтличияЗаказДавальца Тогда
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрЗаменить(ТекстСообщения, "%ПредставлениеПоля%", НСтр("ru='Заказ давальца';uk='Замовлення давальця'")),
			,
			,
			,
			Отказ);
...
Процедура ОформитьДокумент(ТипОперации)
	...
	//++ НЕ УТКА
	ЕстьПоступления = Ложь;
	ЕстьПоступленияСырья = Ложь;
	//-- НЕ УТКА
	...
	Для Каждого ТекСтрока Из Элементы.ПринятаяТара.ВыделенныеСтроки Цикл
		...
		//++ НЕ УТКА
		Если ТипЗнч(ДанныеСтроки.ДокументПоступления) = Тип("ДокументСсылка.ПоступлениеСырьяОтДавальца") Тогда
			ЕстьПоступленияСырья = Истина;
		Иначе
			ЕстьПоступления = Истина;
		КонецЕсли;
		//-- НЕ УТКА
	...
	//++ НЕ УТКА
	Если ЕстьПоступления И ЕстьПоступленияСырья Тогда
		
		ТекстСообщения = НСтр("ru='Команда не может быть выполнена одновременно для тары, полученной от давальцев, и для тары, полученной от поставщиков.';uk='Команда не може бути виконана одночасно для тари, отриманої від давальців, та для тари, отриманої від постачальників.'");
	...
	//++ НЕ УТКА
	ИначеЕсли ТипОперации = "Возврат" И ЕстьПоступленияСырья Тогда
		ИмяФормыДокумента = "Документ.ВозвратСырьяДавальцу.Форма.ФормаДокумента";
	//-- НЕ УТКА
	Иначе // "Возврат"
...
Функция ПоместитьТаруВоВременноеХранилищеСервер(МассивСтрок)
	...
	//++ НЕ УТКА
	ТипПоступления = Новый ОписаниеТипов("ДокументСсылка.ПоступлениеТоваровУслуг, ДокументСсылка.ПоступлениеСырьяОтДавальца");
	//-- НЕ УТКА
	...
	//++ НЕ УТКА
	|	МИНИМУМ(втТаблицаТары.ДокументПоступления.ЗаказДавальца)         КАК ЗаказДавальца,
	//-- НЕ УТКА
	...
	|	ВЫБОР
	|		КОГДА
	|			КОЛИЧЕСТВО(РАЗЛИЧНЫЕ втТаблицаТары.ДокументПоступления.ЗаказДавальца) > 1
	|		ТОГДА
	|			ИСТИНА
	|		ИНАЧЕ
	|			ЛОЖЬ
	|	КОНЕЦ КАК ЕстьОтличияЗаказДавальца,
	//-- НЕ УТКА
	|	ВЫБОР
	|		КОГДА
