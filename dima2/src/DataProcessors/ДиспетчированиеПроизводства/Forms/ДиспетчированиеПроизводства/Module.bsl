...
Процедура ОтборПодразделениеПриИзменении(Элемент)
	...
	Иначе
		ТекстПредупреждения = НСтр("ru='В выбранном подразделении используется пооперационное планирование.
                                        ...
                                        |;uk='У вибраному підрозділі використовується поопераційне планування.
                                        |Диспетчеризація виробництва виконується в робочому місці ""Поопераційне планування"".'");
...
Процедура КомандаРазбитьМаршрутныйЛист(Команда)
	...
	ИначеЕсли ТекущиеДанные.Запланировано < 2  Тогда
		ПоказатьПредупреждение(,НСтр("ru='По маршрутному листу должно быть запланировано произвести не менее двух единиц/партий изделий.';uk='По маршрутному листу повинно бути заплановано виробити не менше двох одиниць/партій виробів.'"));
		Возврат;
	ИначеЕсли ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыМаршрутныхЛистовПроизводства.Выполнен") Тогда
		ПоказатьПредупреждение(,НСтр("ru='Не допускается разбиение выполненного маршрутного листа.';uk='Не допускається розбиття виконаного маршрутного листа.'"));
		Возврат;
	КонецЕсли;
...
Процедура КомандаПерейтиОтМаршрутногоЛистаКДиспетчированиюРЦ(Команда)
	...
	Иначе
		ТекстПредупреждения = НСтр("ru='Не указано какому РЦ назначено выполнение маршрутного листа.';uk='Не зазначено якому РЦ призначено виконання маршрутного листа.'");
  		ПоказатьПредупреждение(,ТекстПредупреждения);
	КонецЕсли; 
...
Процедура Подключаемый_ВыполнитьКомандуПечати(Команда)
	...
	УправлениеПечатьюКлиент.ВыполнитьПодключаемуюКомандуПечати(Команда, ЭтаФорма, Элементы.СписокМаршрутныхЛистов);
...
Функция ЗапуститьЗаданиеОбновитьДанные(СтруктураДействий)
	...
	НаименованиеЗадания = НСтр("ru='Получение данных для диспетчирования производства';uk='Отримання даних для диспетчеризації виробництва'");
...
Процедура ОбновитьНеобходимостьСформироватьМЛ(Результат)
	...
	Элементы.СформироватьМаршрутныеЛисты1.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																	НСтр("ru='Сформировать маршрутные листы (%1)';uk='Сформувати маршрутні листи (%1)'"), 
																	КоличествоМЛСтрокой);
	...
	Если КоличествоМЛ = 1 Тогда
		ПодсказкаКоманды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Требуется сформировать один маршрутный лист';uk='Потрібно сформувати один маршрутний лист'"),
									КоличествоМЛСтрокой);
	ИначеЕсли КоличествоМЛ > 1 Тогда
		ПодсказкаКоманды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Требуется сформировать %1 маршрутных листов';uk='Потрібно сформувати %1 маршрутних листів'"),
									КоличествоМЛСтрокой);
	Иначе
		ПодсказкаКоманды = НСтр("ru='Формировать маршрутные листы не требуется';uk='Формувати маршрутні листи не потрібно'");
	КонецЕсли; 
...
Процедура ОбновитьИнформационнуюСтрокуСпискаМЛ(Результат)
	...
	Если Результат.Свойство("АвтоОбновление") И НЕ Результат.АвтоОбновление Тогда
		...
		МассивСтрок.Добавить(НСтр("ru='Завершено формирование маршрутных листов, для обновления нажмите';uk='Завершено формування маршрутних листів, для оновлення натисніть'"));
		МассивСтрок.Добавить(НСтр("ru=' (';uk=' ('"));
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='обновить';uk='оновити'"),,,, "#ОбновитьСписокМЛ"));
		МассивСтрок.Добавить(НСтр("ru=') ';uk=') '"));
...
Процедура ОбновитьИндикаторыДиспетчированияМЛ()
	...
	Элементы.СформироватьРасписание1.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																	НСтр("ru='Сформировать расписание (%1)';uk='Сформувати розклад (%1)'"), 
	...
	Если ТребуетсяСформироватьРасписание = 1 Тогда
		ПодсказкаКоманды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Требуется сформировать расписание для одного маршрутного листа';uk='Потрібно сформувати розклад для одного маршрутного листа'"),
									ТребуетсяСформироватьРасписаниеСтрокой);
	ИначеЕсли ТребуетсяСформироватьРасписание > 1 Тогда
		ПодсказкаКоманды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Требуется сформировать расписание для %1 маршрутных листов';uk='Потрібно сформувати розклад для %1 маршрутних листів'"),
									ТребуетсяСформироватьРасписаниеСтрокой);
	Иначе
		ПодсказкаКоманды = НСтр("ru='Нет маршрутных листов, по которым требуется формировать расписание';uk='Немає маршрутних листів, за якими потрібно формувати розклад'");
	КонецЕсли;
...
Процедура ОбновитьДиспетчированиеРЦ(Результат)
	...
	ОформитьЭлементыОтбора("ОтборРабочиеЦентрыОтставание", НСтр("ru='Есть отставание от расписания';uk='Є відставання від розкладу'"), ДанныеДиспетчированиеРЦ.ОтставаниеОтРасписания);
	ОформитьЭлементыОтбора("ОтборРабочиеЦентрыОпережение", НСтр("ru='Есть опережение расписания';uk='Є випередження розкладу'"), ДанныеДиспетчированиеРЦ.ОпережениеОтРасписания);
	ОформитьЭлементыОтбора("ОтборРабочиеЦентрыРасписаниеНеСформировано", НСтр("ru='Не сформировано расписание';uk='Не сформовано розклад'"), ДанныеДиспетчированиеРЦ.НеСформированоРасписание);
	...
	Если ВидыРЦТребующиеВводаГрафикаРаботы = 0 Тогда
		Элементы.СтраницыГрафикРаботыРаботыРЦ.ТекущаяСтраница = Элементы.СтраницаПерейтиКГрафикамРаботыРаботыРЦ;
		Элементы.СтраницыДопПанельСвернутаяГрафикРаботыРаботыРЦ.ТекущаяСтраница = Элементы.СтраницаДопПанельСвернутаяПерейтиКГрафикамРаботыРаботыРЦ;
	Иначе
		Элементы.СтраницыГрафикРаботыРаботыРЦ.ТекущаяСтраница = Элементы.СтраницаЗаполнитьГрафикРаботыРаботыРЦ;
		Элементы.СтраницыДопПанельСвернутаяГрафикРаботыРаботыРЦ.ТекущаяСтраница = Элементы.СтраницаДопПанельСвернутаяЗаполнитьГрафикРаботыРаботыРЦ;
		
		Элементы.ГрафикиРаботыРЦ2.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
														НСтр("ru='Заполнить графики работы РЦ (%1)';uk='Заповнити графіки роботи РЦ (%1)'"),
...
Процедура ЗаполнитьПериодыГруппы(СписокСтрок, ТочкаДиаграммы, СерияДиаграммы, Диаграмма, НачалоПолногоПериода, ОкончаниеПолногоПериода)
	...
	Для каждого СтрокаПериод Из СписокСтрок Цикл
		...
		Интервал.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru='Маршрутный лист №%1 от %2 (%3)';uk='Маршрутний лист №%1 від %2 (%3)'"),
										ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(СтрокаПериод.Номер, Ложь, Истина),
...
Процедура ЗаполнитьВыборПериода()
	...
	Если НЕ ОтборПодразделение.Пустая() Тогда
		...
		ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Текущий (%1)';uk='Поточний (%1)'"),
									ПериодСтрока);										
		...
		Если ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.День
			...
			ДатаСледующегоПериода = КалендарныеГрафики.ПолучитьДатуПоКалендарю(ГрафикРаботыПодразделения, ДатаКонтроля, 1);
		КонецЕсли; 
		Если ДатаСледующегоПериода = '000101010000' Тогда
		...
		ПериодСтрока = ОперативныйУчетПроизводства.ИнтервалПланированияСтрокой(
										...
										ИнтервалПланирования);
		ПредставлениеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Следующий (%1)';uk='Наступний (%1)'"),
									ПериодСтрока);										
	...
	СписокВыбораПериода.Добавить("", НСтр("ru='<без отбора>';uk='<без відбору>'"));
...
Процедура УдалитьВыбранныеМаршрутныеЛисты()
	...
	Если КоличествоВсего = 1 Тогда
		ТекстВопроса = НСтр("ru='Пометить на удаление?';uk='Відмітити на вилучення?'");
	Иначе
		ТекстВопроса = НСтр("ru='Пометить на удаление выбранные документы?';uk='Відмітити на вилучення вибрані документи?'");
	КонецЕсли; 
	
	ОписаниеОповещения = Новый ОписаниеОповещения("УдалитьВыбранныеМаршрутныеЛистыЗавершение", ЭтотОбъект, КоличествоВсего);
	СписокКнопок = Новый СписокЗначений;
	СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Пометить на удаление';uk='Відмітити для вилучення'"));
...
Процедура УдалитьВыбранныеМаршрутныеЛистыЗавершение(РезультатВопроса, КоличествоВсего) Экспорт
	...
	Если КоличествоОбработанных > 0 Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Помечено на удаление %1 из %2 документов';uk='Відмічено для вилучення %1 з %2 документів'"),
...
Процедура УправлениеДопПанелью()
	...
	Элементы.ГруппаДопПанельДиспетчированияМЛ.Видимость = НЕ СвернутьДопПанель;
	Элементы.ГруппаДопПанельСвернутаяДиспетчированияМЛ.Видимость = СвернутьДопПанель;

	Элементы.ГруппаДопПанельДиспетчированияРЦ.Видимость = НЕ СвернутьДопПанель;
	Элементы.ГруппаДопПанельСвернутаяДиспетчированияРЦ.Видимость = СвернутьДопПанель;
...
Процедура ПередатьКВыполнению(ВыбраннаяСтрока = Неопределено)
	...
	Если МассивСсылок.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Не требуется передавать к выполнению выбранные маршрутные листы.';uk='Не потрібно передавати до виконання вибрані маршрутні листи.'");
  		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	
	УстановитьСтатусМЛ("КВыполнению", НСтр("ru='К выполнению';uk='До виконання'"), МассивСсылок);
...
Процедура ОтметитьНачалоВыполнения(ВыбраннаяСтрока = Неопределено)
	...
	Если МассивСсылок.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Не требуется отмечать начало выполнения.';uk='Не потрібно відзначати початок виконання.'");
  		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Функция ОтметитьНачалоВыполненияНаСервере(ПараметрыДокумента, МассивСсылок)
	...
	Для каждого Ссылка Из МассивСсылок Цикл
		...
		ДокументОбъект.ПриИзмененииСтатуса(ДатаСобытия);
...
Процедура ОтметитьГотовностьКРаботеКлючевогоРЦ(ВыбраннаяСтрока = Неопределено)
	...
	Если МассивСсылок.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Не требуется отмечать готовность к работе ключевого РЦ.';uk='Не потрібно відзначати готовність до роботи ключового РЦ.'");
  		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ОтметитьЗавершениеРаботыКлючевогоРЦ(ВыбраннаяСтрока = Неопределено)
	...
	Если МассивСсылок.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Не требуется отмечать завершение работы ключевого РЦ.';uk='Не потрібно відзначати завершення роботи ключового РЦ.'");
  		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ОтметитьВыполненным(ВыбраннаяСтрока = Неопределено)
	...
	Если ДанныеДокументов.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Не требуется отмечать выполненными маршрутные листы.';uk='Не потрібно відзначати виконаними маршрутні листи.'");
  		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	...
	Иначе
		ПараметрыДокумента = Новый Структура("ФактическоеОкончание,ПричинаЗадержки", ТекущаяДата(), Неопределено);
...
Процедура УстановитьСтатусМЛ(ЗначениеСтатуса, ПредставлениеСтатуса, МассивСсылок = Неопределено)
	...
	ТекстВопроса = НСтр("ru='У выбранных документов будет установлен статус ""%1"". Продолжить?';uk='У вибраних документів буде встановлено статус ""%1"". Продовжити?'");
...
Процедура ОформитьВыработкуСотрудников(ВыбраннаяСтрока = Неопределено)
	...
	ОперативныйУчетПроизводстваКлиент.ОформитьВыработкуСотрудниковПоРаспоряжениям(МассивСсылок, ВидНаряда);
...
Процедура УправлениеВидомДиспетчированияРЦ(Форма)
	...
	Иначе
		СтраницаКоманды = Форма.Элементы.СтраницаКомандыДиспетичрованиеРЦДиаграмма;
...
Процедура УстановитьПредставлениеОтбора(Форма, Действие = "")
	...
	Если ВсеДействия ИЛИ Действие = "УстановитьОтборПоРаспоряжению" Тогда
		...
		Если ЕстьОтборПоРаспоряжению ИЛИ ЕстьОтборПоЭтапам Тогда
			...
			ДопИнформацияДляДиспетчированияМЛ = НСтр("ru='установлен отбор по';uk='встановлено відбір за'");
			
			Если ЕстьОтборПоРаспоряжению Тогда
				ДопИнформацияДляДиспетчированияМЛ = ДопИнформацияДляДиспетчированияМЛ + " " + НСтр("ru='заказу';uk='замовлення'");
			КонецЕсли;
			
			Если ЕстьОтборПоЭтапам И ЕстьОтборПоРаспоряжению Тогда
				ДопИнформацияДляДиспетчированияМЛ = ДопИнформацияДляДиспетчированияМЛ + " " + НСтр("ru='и';uk='і'");
			КонецЕсли;
			
			Если ЕстьОтборПоЭтапам Тогда
				ДопИнформацияДляДиспетчированияМЛ = ДопИнформацияДляДиспетчированияМЛ + " " + НСтр("ru='этапам';uk='етапам'");
			КонецЕсли;
...
Функция ЗапуститьЗаданиеСформироватьМаршрутныеЛисты(ПараметрыЗаданияСформироватьМаршрутныеЛисты)
	...
	НаименованиеЗадания = НСтр("ru='Формирование маршрутных листов';uk='Формування маршрутних листів'");
	РезультатЗапускаЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
									УникальныйИдентификатор,
...
Функция ВыполняетсяОбновлениеЧитаемыхДанных()
	...
	|ИЗ
	|	РегистрНакопления.ТрудозатратыКОформлению.Изменения КАК ТаблицаИзменений
	|ГДЕ
	...
	|ИЗ
	|	Документ.МаршрутныйЛистПроизводства.Изменения КАК ТаблицаИзменений
	|ГДЕ
	|	ТаблицаИзменений.Узел ССЫЛКА ПланОбмена.ОбновлениеИнформационнойБазы
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ТаблицаИзменений.Регистратор
	|ИЗ
	|	РегистрНакопления.ДоступностьВидовРабочихЦентров.Изменения КАК ТаблицаИзменений
	|ГДЕ
	|	ТаблицаИзменений.Узел ССЫЛКА ПланОбмена.ОбновлениеИнформационнойБазы
	|
	|ОБЪЕДИНИТЬ ВСЕ
