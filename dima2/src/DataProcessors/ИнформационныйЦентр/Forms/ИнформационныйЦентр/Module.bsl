...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ТехнологияСервисаИнтеграцияСБСП.ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка) Тогда
	...
	СсылкаПоискаИнформации = "http://its.1c.ua/db/morphmergedua#search:all:";
	...
	Если ИспользоватьРазделениеПоОбластямДанных Тогда // для модели сервиса
		...
		УстановленаИнтеграцияСоСлужбойПоддержки = ИнформационныйЦентрСервер.УстановленаИнтеграцияСоСлужбойПоддержки();
...
Процедура ОбращенияВПоддержкуНажатие(Элемент)
	...
	Если УстановленаИнтеграцияСоСлужбойПоддержки Тогда 
		ИнформационныйЦентрКлиент.ОткрытьОбращенияВСлужбуПоддержки();
	Иначе
		ОткрытьФорму("Обработка.ИнформационныйЦентр.Форма.УдалитьОбращенияПользователя");
	КонецЕсли;
...
Процедура СформироватьСписокНовостей()
	...
	ЕщеСообщения.Заголовок                = НСтр("ru='Еще сообщения';uk='Ще повідомлення'");
