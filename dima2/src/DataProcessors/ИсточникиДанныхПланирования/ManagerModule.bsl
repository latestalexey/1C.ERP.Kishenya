...
Функция СобираемыеТовары(ТаблицаТовары)
	...
	|	И СпрСпособОбеспечения.ТипОбеспечения = ЗНАЧЕНИЕ(Перечисление.ТипыОбеспечения.СборкаРазборка)";
...
Процедура ДобавитьОбязательныеКолонкиКомплектации(ТаблицаРезультата, КомпоновщикНастроекКомпоновкиДанных, ДополнительныеПараметры)
	...
	Если ТаблицаРезультата.Колонки.Найти("Период") = Неопределено Тогда
		...
		ТаблицаРезультата.ЗаполнитьЗначения(ТекущаяДата(), "Период");
...
Процедура ДобавитьКомплектующиеТоваров(ТаблицаРезультата, СобираемыеТовары, Разузловать = Ложь, ЗначенияРеквизитовПоУмолчанию = Неопределено, ОтборПоТипуОбеспечения = Неопределено)
	...
	Пока СобираемыеТовары.Количество() <> 0 Цикл
		...
		ТаблицаКомплектующие.Индексы.Добавить("ТоварНоменклатура,ТоварХарактеристика,Период");
		...
		Для каждого СтрокаТовар Из ТаблицаТовары Цикл
			...
			СтруктураПоиска = Новый Структура("ТоварНоменклатура,ТоварХарактеристика,Период", 
										СтрокаТовар.Номенклатура, СтрокаТовар.Характеристика, СтрокаТовар.Период);
...
Функция ТоварыИКомплектующие(ТаблицаТовары)
	...
	Запрос = Новый Запрос;
	Запрос.Текст = 
	РегистрыСведений.СхемыОбеспечения.ВременнаяТаблицаФорматыСкладов()
	+"ВЫБРАТЬ
	|	ТаблицаТовары.Склад                 КАК Склад,
	|	ТаблицаТовары.Период                КАК Период,
	|	ВЫРАЗИТЬ(ТаблицаТовары.Номенклатура КАК Справочник.Номенклатура) КАК Номенклатура,
	...
	Запрос.Текст = РегистрыСведений.СхемыОбеспечения.ПодставитьСоединениеДляПолученияСпособаОбеспечения(
		...
		"ТаблицаКомплектующие.Номенклатура,ТаблицаКомплектующие.Характеристика,Товары.Склад");

	Если ТаблицаТовары.Колонки.Найти("ВариантКомплектации") = Неопределено Тогда
		ТаблицаТовары.Колонки.Добавить("ВариантКомплектации", Новый ОписаниеТипов("СправочникСсылка.ВариантыКомплектацииНоменклатуры"));
	КонецЕсли;
	...
	Возврат Новый Структура("ТаблицаТовары, ТаблицаКомплектующие", 
							Результат[2].Выгрузить(), Результат[3].Выгрузить());
