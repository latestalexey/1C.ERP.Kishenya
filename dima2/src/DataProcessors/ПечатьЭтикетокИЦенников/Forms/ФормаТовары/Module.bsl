...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоАдресВременногоХранилища(Параметры.АдресВХранилище) Тогда
		...
		Иначе
			...
			Объект.ИсходныеДанныеТовары.Загрузить(Товары);
...
Процедура ЗаполнитьТаблицуТоваров(Команда)
	...
	Иначе
		...
		ТекстВопроса = НСтр("ru='Сохранить данные о количестве, шаблонах в табличной части при перезаполнении?';uk='Зберегти дані про кількість, шаблони в табличній частині при перезаполнении?'");
