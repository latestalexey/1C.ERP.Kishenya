...
#Область ВыполнениеЗапроса
	...
	ИначеЕсли Режим = "Доставка" Тогда
		...
		Если ИсходныеДанные <> Неопределено Тогда
			...
			ТекстВременнойТаблицы =
			"	(ВЫБРАТЬ
			|		ЗНАЧЕНИЕ(Документ.ПередачаДавальцу.ПустаяСсылка) КАК Ссылка,
			|		0 КАК Порядок,
			|		0 КАК КоличествоПозиций,
			|		0 КАК Мест,
			|		0 КАК ОбъемНакладной,
			...
			ТекстВременнойТаблицы =
			"	(ВЫБРАТЬ
			|		ЗНАЧЕНИЕ(Документ.ПередачаСырьяПереработчику.ПустаяСсылка) КАК Ссылка,
			|		0 КАК Порядок,
			|		0 КАК КоличествоПозиций,
			|		0 КАК Мест,
			|		0 КАК ОбъемНакладной,
			...
			ТекстВременнойТаблицы =
			"	(ВЫБРАТЬ
			|		ЗНАЧЕНИЕ(Документ.ЗаказПереработчику.ПустаяСсылка) КАК Ссылка,
			|		0 КАК Порядок,
			|		0 КАК КоличествоПозиций,
			|		0 КАК Мест,
			|		0 КАК ОбъемНакладной,
...
Функция СформироватьПечатнуюФормуЭтикеткиУпаковочныеЛисты(ДанныеПечати, ОбъектыПечати)
	...
	Если Не ДанныеПечати.Свойство("ШаблонЭтикетки") Тогда
		...
		Если Не ЗначениеЗаполнено(ШаблонЭтикетки) Тогда
			ТекстИсключения = НСтр("ru='Печать этикетки не возможна, т.к. в настройках программы не выбран шаблон этикетки по умолчанию. Обратитесь к администратору.';uk='Друк етикетки не можливий, оскільки в настройках програми не вибрано шаблон етикетки за замовчуванням. Зверніться до адміністратора.'");
...
Функция ДанныеДляПечатиЭтикетокДоставки(ПараметрКоманды) Экспорт
	...
	Если ТипЗнч(ПараметрКоманды) = Тип("ДокументСсылка.ЗаданиеНаПеревозку") Тогда
		...
		|ВЫБРАТЬ
		|	РаспоряженияПоЗаданию.Распоряжение,
		|	ДокументРаспоряжение.ЗаказДавальца.ПеревозчикПартнер,
		|	РаспоряженияПоЗаданию.Вес,
		|	РаспоряженияПоЗаданию.Объем,
		|	ВЫБОР
		|		КОГДА ДокументРаспоряжение.ЗаказДавальца.ПеревозчикПартнер.ШаблонЭтикетки = ЗНАЧЕНИЕ(Справочник.ШаблоныЭтикетокИЦенников.ПустаяСсылка)
		|				ИЛИ ДокументРаспоряжение.ЗаказДавальца.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз)
		|				ИЛИ ДокументРаспоряжение.ЗаказДавальца.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ДоКлиента)
		|			ТОГДА ШаблонЭтикеткиДляДоставки.Значение
		|		ИНАЧЕ ДокументРаспоряжение.ЗаказДавальца.ПеревозчикПартнер.ШаблонЭтикетки
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА РаспоряженияПоЗаданию.ОрдерныйСклад
		|			ТОГДА РаспоряженияПоЗаданию.ВсегоМест
		...
		|ИЗ
		|	РаспоряженияПоЗаданию КАК РаспоряженияПоЗаданию
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПередачаДавальцу КАК ДокументРаспоряжение
		|		ПО РаспоряженияПоЗаданию.Распоряжение = ДокументРаспоряжение.Ссылка
		...
		|ИЗ
		|	РаспоряженияПоЗаданию КАК РаспоряженияПоЗаданию
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца КАК ДокументРаспоряжение
		|		ПО РаспоряженияПоЗаданию.Распоряжение = ДокументРаспоряжение.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца.Продукция КАК ДокументРаспоряжениеТовары
		|		ПО РаспоряженияПоЗаданию.Распоряжение = ДокументРаспоряжениеТовары.Ссылка
		|			И РаспоряженияПоЗаданию.Склад = ДокументРаспоряжениеТовары.Склад,
		|	Константа.ШаблонЭтикеткиДляДоставки КАК ШаблонЭтикеткиДляДоставки
		|
		...
		|ВЫБРАТЬ
		|	РаспоряженияПоЗаданию.Распоряжение,
		|	ДокументРаспоряжение.ЗаказПереработчику.ПеревозчикПартнер,
		|	РаспоряженияПоЗаданию.Вес,
		|	РаспоряженияПоЗаданию.Объем,
		|	ВЫБОР
		|		КОГДА ДокументРаспоряжение.ЗаказПереработчику.ПеревозчикПартнер.ШаблонЭтикетки = ЗНАЧЕНИЕ(Справочник.ШаблоныЭтикетокИЦенников.ПустаяСсылка)
		|				ИЛИ ДокументРаспоряжение.ЗаказПереработчику.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз)
		|				ИЛИ ДокументРаспоряжение.ЗаказПереработчику.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ДоКлиента)
		|			ТОГДА ШаблонЭтикеткиДляДоставки.Значение
		|		ИНАЧЕ ДокументРаспоряжение.ЗаказПереработчику.ПеревозчикПартнер.ШаблонЭтикетки
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА РаспоряженияПоЗаданию.ОрдерныйСклад
		|			ТОГДА РаспоряженияПоЗаданию.ВсегоМест
		...
		|ИЗ
		|	РаспоряженияПоЗаданию КАК РаспоряженияПоЗаданию
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПередачаСырьяПереработчику КАК ДокументРаспоряжение
		|		ПО РаспоряженияПоЗаданию.Распоряжение = ДокументРаспоряжение.Ссылка
		...
		|ИЗ
		|	РаспоряженияПоЗаданию КАК РаспоряженияПоЗаданию
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику КАК ДокументРаспоряжение
		|		ПО РаспоряженияПоЗаданию.Распоряжение = ДокументРаспоряжение.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику.Материалы КАК ДокументРаспоряжениеТовары
		|		ПО РаспоряженияПоЗаданию.Распоряжение = ДокументРаспоряжениеТовары.Ссылка
		|			И РаспоряженияПоЗаданию.Склад = ДокументРаспоряжениеТовары.Склад,
		|	Константа.ШаблонЭтикеткиДляДоставки КАК ШаблонЭтикеткиДляДоставки
		|
	...
	Иначе
		...
		|
		|ВЫБРАТЬ
		|	ПередачаСырьяПереработчикуТовары.Ссылка,
		|	СУММА(ВЫБОР
		|			КОГДА ПередачаСырьяПереработчикуТовары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
		|					ИЛИ &ТекстЗапросаВесУпаковки3 = 0
		|				ТОГДА &ТекстЗапросаВесНоменклатуры3 * ПередачаСырьяПереработчикуТовары.Количество
		|			ИНАЧЕ &ТекстЗапросаВесУпаковки3 * ПередачаСырьяПереработчикуТовары.КоличествоУпаковок
		|		КОНЕЦ),
		|	СУММА(ВЫБОР
		|			КОГДА ПередачаСырьяПереработчикуТовары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
		|					ИЛИ &ТекстЗапросаОбъемУпаковки3 = 0
		|				ТОГДА &ТекстЗапросаОбъемНоменклатуры3 * ПередачаСырьяПереработчикуТовары.Количество
		|			ИНАЧЕ &ТекстЗапросаОбъемУпаковки3 * ПередачаСырьяПереработчикуТовары.КоличествоУпаковок
		|		КОНЕЦ),
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПередачаСырьяПереработчикуТовары.НомерСтроки),
		|	ВЫБОР
		|		КОГДА ДокументЗаказПереработчику.ПеревозчикПартнер.ШаблонЭтикетки = ЗНАЧЕНИЕ(Справочник.ШаблоныЭтикетокИЦенников.ПустаяСсылка)
		|				ИЛИ ДокументЗаказПереработчику.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз)
		|				ИЛИ ДокументЗаказПереработчику.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ДоКлиента)
		|			ТОГДА ШаблонЭтикеткиДляДоставки.Значение
		|		ИНАЧЕ ДокументЗаказПереработчику.ПеревозчикПартнер.ШаблонЭтикетки
		|	КОНЕЦ
		|ИЗ
		|	Документ.ПередачаСырьяПереработчику.Товары КАК ПередачаСырьяПереработчикуТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику КАК ДокументЗаказПереработчику
		|		ПО ПередачаСырьяПереработчикуТовары.Ссылка.ЗаказПереработчику = ДокументЗаказПереработчику.Ссылка,
		|	Константа.ШаблонЭтикеткиДляДоставки КАК ШаблонЭтикеткиДляДоставки
		|ГДЕ
		|	ПередачаСырьяПереработчикуТовары.Ссылка В(&ПараметрКоманды)
		|
		|СГРУППИРОВАТЬ ПО
		|	ПередачаСырьяПереработчикуТовары.Ссылка,
		|	ВЫБОР
		...
		|
		|ВЫБРАТЬ
		|	ПередачаДавальцуТовары.Ссылка,
		|	СУММА(ВЫБОР
		|			КОГДА ПередачаДавальцуТовары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
		|					ИЛИ &ТекстЗапросаВесУпаковки4 = 0
		|				ТОГДА &ТекстЗапросаВесНоменклатуры4 * ПередачаДавальцуТовары.Количество
		|			ИНАЧЕ &ТекстЗапросаВесУпаковки4 * ПередачаДавальцуТовары.КоличествоУпаковок
		|		КОНЕЦ),
		|	СУММА(ВЫБОР
		|			КОГДА ПередачаДавальцуТовары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
		|					ИЛИ &ТекстЗапросаОбъемУпаковки4 = 0
		|				ТОГДА &ТекстЗапросаОбъемНоменклатуры4 * ПередачаДавальцуТовары.Количество
		|			ИНАЧЕ &ТекстЗапросаОбъемУпаковки4 * ПередачаДавальцуТовары.КоличествоУпаковок
		|		КОНЕЦ),
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПередачаДавальцуТовары.НомерСтроки),
		|	ВЫБОР
		|		КОГДА ДокументЗаказДавальца.ПеревозчикПартнер.ШаблонЭтикетки = ЗНАЧЕНИЕ(Справочник.ШаблоныЭтикетокИЦенников.ПустаяСсылка)
		|				ИЛИ ДокументЗаказДавальца.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз)
		|				ИЛИ ДокументЗаказДавальца.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ДоКлиента)
		|			ТОГДА ШаблонЭтикеткиДляДоставки.Значение
		|		ИНАЧЕ ДокументЗаказДавальца.ПеревозчикПартнер.ШаблонЭтикетки
		|	КОНЕЦ
		|ИЗ
		|	Документ.ПередачаДавальцу.Товары КАК ПередачаДавальцуТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца КАК ДокументЗаказДавальца
		|		ПО ПередачаДавальцуТовары.Ссылка.ЗаказДавальца = ДокументЗаказДавальца.Ссылка,
		|	Константа.ШаблонЭтикеткиДляДоставки КАК ШаблонЭтикеткиДляДоставки
		|ГДЕ
		|	ПередачаДавальцуТовары.Ссылка В(&ПараметрКоманды)
		|
		|СГРУППИРОВАТЬ ПО
		|	ПередачаДавальцуТовары.Ссылка,
		|	ВЫБОР
		...
		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса, 
			"&ТекстЗапросаВесНоменклатуры3", 
			Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаВесУпаковки("ПередачаСырьяПереработчикуТовары.Номенклатура.ЕдиницаИзмерения", "ПередачаСырьяПереработчикуТовары.Номенклатура"));

		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса, 
			"&ТекстЗапросаОбъемНоменклатуры3", 
			Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаОбъемУпаковки("ПередачаСырьяПереработчикуТовары.Номенклатура.ЕдиницаИзмерения", "ПередачаСырьяПереработчикуТовары.Номенклатура"));

		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса, 
			"&ТекстЗапросаВесУпаковки3", 
			Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаВесУпаковки("ПередачаСырьяПереработчикуТовары.Упаковка", "ПередачаСырьяПереработчикуТовары.Номенклатура"));

		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса, 
			"&ТекстЗапросаОбъемУпаковки3", 
			Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаОбъемУпаковки("ПередачаСырьяПереработчикуТовары.Упаковка", "ПередачаСырьяПереработчикуТовары.Номенклатура"));
		//-- НЕ УТ
		
		//++ НЕ УТКА
		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса, 
			"&ТекстЗапросаВесНоменклатуры4", 
			Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаВесУпаковки("ПередачаДавальцуТовары.Номенклатура.ЕдиницаИзмерения", "ПередачаДавальцуТовары.Номенклатура"));

		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса, 
			"&ТекстЗапросаОбъемНоменклатуры4", 
			Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаОбъемУпаковки("ПередачаДавальцуТовары.Номенклатура.ЕдиницаИзмерения", "ПередачаДавальцуТовары.Номенклатура"));

		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса, 
			"&ТекстЗапросаВесУпаковки4", 
			Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаВесУпаковки("ПередачаДавальцуТовары.Упаковка", "ПередачаДавальцуТовары.Номенклатура"));

		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса, 
			"&ТекстЗапросаОбъемУпаковки4", 
			Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаОбъемУпаковки("ПередачаДавальцуТовары.Упаковка", "ПередачаДавальцуТовары.Номенклатура"));
		//-- НЕ УТКА
