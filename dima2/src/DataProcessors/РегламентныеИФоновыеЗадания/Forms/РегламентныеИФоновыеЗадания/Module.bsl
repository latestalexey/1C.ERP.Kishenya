...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав доступа.
                                     ...
                                     |
                                     |Робота з регламентними й фоновими завданнями
                                     |виконується тільки адміністраторами.'");
	КонецЕсли;
...
Процедура ТаблицаРегламентныеЗаданияПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ВыделенныеСтроки.Количество() > 1 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выберите одно регламентное задание.';uk='Виберіть одне регламентне завдання.'"));
		
	ИначеЕсли Элемент.ТекущиеДанные.Предопределенное Тогда
		ПоказатьПредупреждение(, НСтр("ru='Невозможно удалить предопределенное регламентное задание.';uk='Неможливо вилучити визначене регламентне завдання.'") );
	Иначе
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ТаблицаРегламентныеЗаданияПередУдалениемЗавершение", ЭтотОбъект),
			НСтр("ru='Удалить регламентное задание?';uk='Вилучити регламентне завдання?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура ВыполнитьРегламентноеЗаданиеВручную(Команда)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выберите регламентное задание.';uk='Виберіть регламентне завдання.'"));
		Возврат;
	КонецЕсли;
	...
	Для каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		Если ТекущиеДанные.Параметризуемое И КоличествоВыделенныхЗаданий = 1 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Выбранное регламентное задание нельзя выполнить вручную.';uk='Обране регламентне завдання не можна виконати вручну.'"));
			Возврат;
		...
		Если ПараметрыВыполнения.ЗапускВыполнен Тогда
			...
			ПоказатьОповещениеПользователя(
				НСтр("ru='Запущена процедура регламентного задания';uk='Запущено процедуру регламентного завдання'"), ,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1.
                    |Процедура запущена в фоновом задании %2'
                    |;uk='%1.
                    |Процедура запущена у фоновому завданні %2'"),
		...
		ИначеЕсли ПараметрыВыполнения.ПроцедураУжеВыполняется Тогда
			МассивСообщенийОбОшибках.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                    ...
                    |;uk='Процедура регламентного задачі ""%1""
                    |  уже виконується у фоновому задачі ""%2"", початому %3.'"),
					ТекущиеДанные.Наименование,
	...
	Если КоличествоОшибок > 0 Тогда
		ЗаголовокТекстаПроОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Задания выполнены с ошибками (%1 из %2)';uk='Завдання виконані з помилками (%1 из %2)'"),
...
Процедура ОтменитьФоновоеЗадание(Команда)
	...
	Если Элементы.ТаблицаФоновыеЗадания.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выберите фоновое задание.';uk='Виберіть фонове завдання.'"));
	Иначе
		...
		ПоказатьПредупреждение(,
			НСтр("ru='Задание отменено, но состояние отмены будет
                       ...
                       |;uk='Завдання скасоване, але стан скасування буде
                       |установлений сервером тільки через секунди,
                       |можливо буде потрібно оновити дані вручну.'"));
	КонецЕсли;
...
Процедура ВыполнитьНеВФоне(Команда)
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Выполнена процедура регламентного задания';uk='Виконано процедуру регламентного завдання'"), ,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1.
            |Процедура выполнена вручную без фонового задания'
            |;uk='%1.
            |Процедура виконана вручну без фонового завдання'"),
			ТекущиеДанные.Наименование),
		БиблиотекаКартинок.ВыполнитьРегламентноеЗаданиеВручную);
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("ru='<не определено>';uk='<не визначено>'");
		...
		ПоказатьОповещениеПользователя(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1.
                           ...
                           |;uk='%1.
                           |Процедура завершена у фоновому завданні %2'"),
...
Функция СформироватьТаблицуФоновыхЗаданийВФоне()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Регламентные задания. Обновление списка фоновых заданий';uk='Регламентні завдання. Оновлення списку фонових завдань'");
	...
	Для каждого Задание Из ТекущаяТаблица Цикл
		...
		Если ЗначениеЗаполнено(Обновляемое.ИдентификаторРегламентногоЗадания) Тогда
			...
			Обновляемое.НаименованиеРегламентногоЗадания
				= ?(Строки.Количество() = 0, НСтр("ru='<не найдено>';uk='<не знайдено>'"), Строки[0].Наименование);
		Иначе
