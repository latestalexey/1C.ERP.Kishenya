...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбменДаннымиВМоделиСервисаПовтИсп.СинхронизацияДанныхПоддерживается() Тогда
		...
		ВызватьИсключение НСтр("ru='Синхронизация данных для конфигурации не поддерживается!';uk='Синхронізація даних для конфігурації не підтримується!'");
	...
	Если Параметры.Свойство("ДополнительнаяНастройка") Тогда
		ВариантНастройкиОбмена = Параметры.ДополнительнаяНастройка;
	КонецЕсли;
	...
	Заголовок = СтрШаблон(НСтр("ru='Настройка синхронизации данных с ""%1""';uk='Настройка синхронізації даних з ""%1""'"), НаименованиеКорреспондента);
	Элементы.ОписаниеСинхронизацииДанных.Заголовок = СтрШаблон(НСтр("ru='Описание синхронизации данных с ""%1""';uk='Опис синхронізації даних з ""%1""'"), НаименованиеКорреспондента);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ОбщегоНазначенияКлиент.ПредлагатьСоздаватьРезервныеКопии() Тогда
		...
		Текст = СтрШаблон(НСтр("ru='Перед настройкой синхронизации рекомендуется сделать <a href = %1 >резервную копию данных</a>.';uk='Перед настройкою синхронізації рекомендується зробити <a href = %1 >резервну копію даних</a>.'"),
			"СоздатьРезервнуюКопию");
		
		Элементы.НадписьРезервноеКопирование.Заголовок = СтроковыеФункцииКлиентСервер.ФорматированнаяСтрока(Текст);
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ru='Отменить настройку синхронизации данных?';uk='Скасувати настройку синхронізації даних?'");
...
Процедура СообщитьОбОшибке(Отказ)
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось выполнить операцию.';uk='Не вдалося виконати операцію.'"),,,, Отказ);
...
Функция Подключаемый_Начало_ПриПереходеДалее(Отказ)
	...
	Если ПредусмотреныНастройкиОтборовНаУзле Тогда
		...
		Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо задать обязательные настройки.';uk='Необхідно задати обов''язкові настройки.'"),,, "ОписаниеНастроекВыгрузкиДанных", Отказ);
...
Процедура ОжиданиеАнализаДанных_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Настройка синхронизации данных между приложениями в сети Интернет (шаг 1)';uk='Настройка синхронізації даних між додатками в мережі Інтернет (крок 1)'"));
...
Процедура ВыполнитьАвтоматическоеСопоставлениеДанных(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Настройка синхронизации данных между приложениями в сети Интернет (автоматическое сопоставление данных)';uk='Настройка синхронізації даних між додатками в мережі Інтернет (автоматичне зіставлення даних)'"));
...
Процедура ПолучитьСтатистикуСопоставления(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Настройка синхронизации данных между приложениями в сети Интернет (получение статистики сопоставления)';uk='Настройка синхронізації даних між додатками в мережі Інтернет (одержання статистики зіставлення)'"));
...
Процедура ОжиданиеСинхронизацииДанных_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Настройка синхронизации данных между приложениями в сети Интернет (синхронизация справочников)';uk='Настройка синхронізації даних між додатками в мережі Інтернет (синхронізація довідників)'"));
...
Процедура ЗагрузитьПараметрыУчетаКорреспондента(Отказ, СообщениеОбОшибке = "", СообщениеОбОшибкеКорреспондента = "")
	...
	Попытка
		...
		Если ПустаяСтрока(СообщениеОбОшибкеКорреспондента) Тогда
			СообщениеОбОшибкеКорреспондента = НСтр("ru='Не заданы параметры учета в приложении ""%1"".';uk='Не задані параметри обліку в програмі ""%1"".'");
			СообщениеОбОшибкеКорреспондента = СтрШаблон(СообщениеОбОшибкеКорреспондента, НаименованиеКорреспондента);
		КонецЕсли;
		...
		Если ПустаяСтрока(СообщениеОбОшибке) Тогда
			СообщениеОбОшибке = НСтр("ru='Не заданы параметры учета в этом приложении.';uk='Не задані параметри обліку в цьому додатку.'");
		КонецЕсли;
...
Функция Подключаемый_ПравилаПолученияДанных_ПриПереходеДалее(Отказ)
	...
	Если ПредусмотреныЗначенияПоУмолчаниюНаУзле Тогда
		...
		Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо задать обязательные настройки.';uk='Необхідно задати обов''язкові настройки.'"),,, "ОписаниеЗначенийПоУмолчанию", Отказ);
	...
	Если ПредусмотреныЗначенияПоУмолчаниюНаУзлеБазыКорреспондента Тогда
		...
		Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо задать обязательные настройки для второго приложения.';uk='Необхідно задати обов''язкові настройки для другого додатка.'")
...
Процедура ОжиданиеСохраненияНастроек_ОбработкаДлительнойОперации(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Настройка синхронизации данных между приложениями в сети Интернет (шаг 2)';uk='Настройка синхронізації даних між додатками в мережі Інтернет (крок 2)'"));
