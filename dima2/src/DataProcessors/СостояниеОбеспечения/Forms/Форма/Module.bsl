...
//++ НЕ УТКА
		Или ДоступныеДокументы.Найти(Документы.ЗаказДавальца) <> Неопределено
//-- НЕ УТКА
//++ НЕ УТ
		Или  ДоступныеДокументы.Найти(Документы.ЗаказПереработчику) <> Неопределено
//-- НЕ УТ
		Тогда
...
//++ НЕ УТКА
		Или ДоступныеДокументы.Найти(Документы.ЗаказНаРемонт) <> Неопределено
//-- НЕ УТКА
		Тогда
...
//++ НЕ УТ
	Если ДоступныеДокументы.Найти(Документы.ЗаказМатериаловВПроизводство) <> Неопределено Тогда

		ТекстЗапроса = ТекстЗапроса + РегистрыНакопления.ЗаказыМатериаловВПроизводство.ТекстЗапросаЗаказовКОбеспечению(ЕстьФильтр);

	КонецЕсли;
//-- НЕ УТ
	
//++ НЕ УТКА
	Если ДоступныеДокументы.Найти(Документы.ЗаказНаПроизводство) <> Неопределено Тогда

		ТекстЗапроса = ТекстЗапроса + РегистрыНакопления.ЗаказыМатериаловСУчетомКорректировок.ТекстЗапросаЗаказовКОбеспечению(ЕстьФильтр);
...
//++ НЕ УТ
	Если ДоступныеДокументы.Найти(Документы.ЗаказМатериаловВПроизводство) <> Неопределено Тогда
		...
		ДобавитьСекциюОбъединения(ТекстЗапроса, ПервыйЗапрос);
		ТекстЗапроса = ТекстЗапроса + РегистрыНакопления.ЗаказыМатериаловВПроизводство.ТекстЗапросаЗаказовНоменклатуры();
		ПервыйЗапрос = Ложь;
...
//++ НЕ УТКА
	Если ДоступныеДокументы.Найти(Документы.ЗаказНаПроизводство) <> Неопределено Тогда
		...
		ДобавитьСекциюОбъединения(ТекстЗапроса, ПервыйЗапрос);
		ТекстЗапроса = ТекстЗапроса + РегистрыНакопления.ЗаказыМатериаловСУчетомКорректировок.ТекстЗапросаЗаказовНоменклатуры();
		ПервыйЗапрос = Ложь;
...
//++ НЕ УТКА
		ЕстьЗаказНаПроизводство = ЕстьЗаказНаПроизводство
			Или ТипЗнч(ПотребностиНовые[ИндексНовой].Заказ) = Тип("ДокументСсылка.ЗаказНаПроизводство");
//-- НЕ УТКА
...
//++ НЕ УТКА
	ИначеЕсли ТипЗнч(ДанныеЗаполнения.Заказ) = Тип("ДокументСсылка.ЗаказНаПроизводство") И РежимГруппировкиМатериалов <> "СгруппироватьПоЗаказам" Тогда

		Шаблон = НСтр("ru='Заказ на производство %1';uk='Замовлення на виробництво %1'");
		ЗаказПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ДанныеЗаполнения.НомерДокумента);
...
//++ НЕ УТКА
	Если ТипЗнч(ДанныеЗаполнения.Заказ) = Тип("ДокументСсылка.ЗаказНаПроизводство") И РежимГруппировкиМатериалов <> "СгруппироватьПоЗаказам" Тогда

		Если РежимГруппировкиМатериалов = "СгруппироватьПоИзделиям" Тогда
			Шаблон = НСтр("ru='%1 (Строка %2: %3)';uk='%1 (Рядок %2: %3)'");
		Иначе
			Шаблон = НСтр("ru='%1 (Строка %2: %3, %4)';uk='%1 (Рядок %2: %3, %4)'");
		КонецЕсли;
		ЗаказПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			Шаблон, ЗаказПредставление, ДанныеЗаполнения.НомерСтрокиПродукция, ДанныеЗаполнения.Продукция,
			ДанныеЗаполнения.Этап);
...
Функция ЗаполнитьДатыОбеспеченияПотребностей(ОстаткиИОбороты, Итоги)
	...
	Для Каждого Заказ Из Товары.ПолучитьЭлементы() Цикл
		...
		Для Каждого Потребность Из Заказ.ПолучитьЭлементы() Цикл
			...
			Иначе
				...
				ЗаполнитьДатыОбеспеченияПотребности(Потребность, ТаблицаОстатков, СписокКолонокНовый, ПолностьюВНаличии);
	...
	ЗаполнитьДатыОбеспеченияОбособленныхПотребностей(
		ТаблицаОбособленныхПотребностей, СписокКолонокНовый, ПолностьюВНаличии, ОстаткиИОбороты, Итоги);
	...
	Если ВсегоСтрок > 0 Тогда
		...
		Для Индекс = 0 По ВсегоСтрок Цикл
			...
			Если ОбеспечениеКлиентСервер.ИзменилсяКлюч(Ключ, ТекущаяСтрока) Или Индекс = ВсегоСтрок Тогда
				...
				Если ПотребностиОднойАналитики.Количество() = 1 Тогда

					Потребность = Товары.НайтиПоИдентификатору(ПотребностиОднойАналитики[0].Идентификатор);
					ПараметрыОтбора = ?(Потребность.ТипНоменклатуры = Работа, ПараметрыОтбораРабот, ПараметрыОтбораОбособленные);
					ЗаполнитьЗначенияСвойств(ПараметрыОтбора, Потребность);
					ТаблицаОстатков = ?(Потребность.ТипНоменклатуры = Работа,
						ОстаткиИобороты.Работы.НайтиСтроки(ПараметрыОтбора),
						ОстаткиИобороты.ТоварыОбособленные.НайтиСтроки(ПараметрыОтбора));
					ЗаполнитьДатыОбеспеченияПотребности(Потребность, ТаблицаОстатков, СписокКолонокНовый, ПолностьюВНаличии);

				ИначеЕсли ПотребностиОднойАналитики.Количество() > 1 Тогда
					...
					Счетчик = ПотребностиОднойАналитики.Количество() - 1;
					Пока Счетчик >= 0 Цикл

						Потребность = Товары.НайтиПоИдентификатору(ПотребностиОднойАналитики[Счетчик].Идентификатор);
						Счетчик = Счетчик - 1;
						ЗаполнитьДатыОбеспеченияПотребности(Потребность, ТаблицаОстатков, СписокКолонокНовый, ПолностьюВНаличии, Истина);
...
&НаСервере
Процедура ЗаполнитьДатыОбеспеченияПотребности(Потребность, ТаблицаОстатков, СписокКолонокНовый,
	ЗаказПолностьюВНаличии, КорректироватьОстатки = Ложь)

	ДатыОбеспечения = ДатыОбеспеченияПотребности(Потребность, ТаблицаОстатков, КорректироватьОстатки);
...
Процедура ЗаполнитьВНаличииЗаказано(Потребность, ДатыОбеспечения);
	...
	Если Потребность.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно
		Или Потребность.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ОтгрузитьОбособленно
			И Потребность.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа Тогда
	...
	ИначеЕсли Потребность.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.СоСклада
		Или Потребность.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить
		Или Потребность.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Товар
			И Потребность.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ОтгрузитьОбособленно Тогда
...
&НаСервере
Функция ДатыОбеспеченияПотребности(Потребность, ТаблицаОстатков, КорректироватьТаблицуОстатков = Ложь)

	ВставитьДвижение = Ложь;
	...
	КорректироватьИтог =
		Потребность.ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.Отгрузить")
		...
		Или (Потребность.ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно")
			И СтатусКОбеспечению(Потребность));
	...
	Корректировать =
		Потребность.ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.ИзЗаказов")
		Или (Потребность.ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно")
				И Потребность.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа")
				И СтатусКОбеспечению(Потребность));
	...
	Пока Индекс < ТаблицаОстатков.Количество() Цикл
		...
		Если Корректировать И Потребность.ДатаОтгрузки >= ДвижениеГрафика.Период Тогда
			...
			Если КорректироватьТаблицуОстатков Тогда
				Если Потребность.ДатаОтгрузки = ДвижениеГрафика.Период Тогда
					ДвижениеГрафика.Оборот = ДвижениеГрафика.Оборот + Потребность.Количество;
				Иначе
					ВставитьДвижение = Истина;
					ИндексДляВставки = Индекс;
				КонецЕсли;
			КонецЕсли;
	...
	Если КорректироватьТаблицуОстатков Тогда
		НовоеЗначениеОстатка = ТаблицаОстатков[0].Остаток + Потребность.Количество;
		Если ВставитьДвижение Тогда
			ДвижениеГрафика = ТаблицаОстатков.Вставить(ИндексДляВставки);
			ДвижениеГрафика.Оборот = Потребность.Количество;
			ДвижениеГрафика.Период = Потребность.ДатаОтгрузки;
		КонецЕсли;
		ТаблицаОстатков[0].Остаток = НовоеЗначениеОстатка;
	КонецЕсли;
...
&НаСервере
Функция СтатусКОбеспечению(СтрокаТовары)
	
//++ НЕ УТ
	Если ТипЗнч(СтрокаТовары.Заказ) = Тип("ДокументСсылка.ЗаказПереработчику") Тогда
		Если СтрокаТовары.Статус = Перечисления.СтатусыЗаказовПереработчикам.НеСогласован Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
//-- НЕ УТ
...
//++ НЕ УТКА
	Если ТипЗнч(СтрокаТовары.Заказ) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
		Если СтрокаТовары.Статус = Перечисления.СтатусыЗаказовДавальцев.НеСогласован
			Или СтрокаТовары.Статус = Перечисления.СтатусыЗаказовДавальцев.Согласован Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
//-- НЕ УТКА
	...
	Если ТипЗнч(СтрокаТовары.Заказ) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		Если СтрокаТовары.Статус = Перечисления.СтатусыЗаказовКлиентов.НеСогласован Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;

	Если ТипЗнч(СтрокаТовары.Заказ) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		Если СтрокаТовары.Статус = Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КВозврату Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
//++ НЕ УТКА
			Если ТипЗнч(Заказ.Заказ) = Тип("ДокументСсылка.ЗаказНаПроизводство") И СтрокиТовары.Количество() > 0 Тогда
				НомерСтрокиПредставление = НомерСтрокиПредставление + 1;
				СтрокаТовары.НомерСтрокиПредставление = НомерСтрокиПредставление;
			КонецЕсли;
//-- НЕ УТКА
...
//-- НЕ УТКА
	...
	//++ НЕ УТКА
		Если Параметры.ВызовДляСтрокиПродукции Тогда
			РежимГруппировкиМатериалов = "СгруппироватьПоЭтапам";
		ИначеЕсли Параметры.ВызовИзФормыЗаказа И ТипЗнч(Параметры.РеквизитыЗаказа.Ссылка) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
			РежимГруппировкиМатериалов = "СгруппироватьПоИзделиям";
		КонецЕсли;
	//-- НЕ УТКА
...
//++ НЕ УТКА
#Область ОбработкаЗаказаНаПроизводство

&НаСервере
Функция СтрокиСпискаДляОбработки(ПараметрыВыполненияКоманды, ПараметрыЗаполненияОбеспечения)
	
	ТекущаяСтрока = Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	
	Если ТекущаяСтрока.ТипСтроки = 0 Тогда
		ГруппировкаСтрок = ТекущаяСтрока.ПолучитьРодителя();
	Иначе
		ГруппировкаСтрок = ТекущаяСтрока;
	КонецЕсли;
	
	Если ПараметрыВыполненияКоманды.ТипКоманды = "Строка" Тогда // гиперссылка в колонке "Рекомендация".
		
		Если ПараметрыЗаполненияОбеспечения.НеОтгружатьЧастями Тогда
			
			ТаблицаОтбораСтрок = СтрокиГруппировкиСпиcкаСвязанные(ГруппировкаСтрок);
			
		Иначе
			
			ТаблицаОтбораСтрок = Новый ТаблицаЗначений();
			ТаблицаОтбораСтрок.Колонки.Добавить("Заказ", ОбеспечениеКлиентСервер.ОписаниеТиповЗаказыКОбеспечению());
			ТаблицаОтбораСтрок.Колонки.Добавить("НомерСтроки", ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15, 0));
			ЗаполнитьЗначенияСвойств(ТаблицаОтбораСтрок.Добавить(), ТекущаяСтрока);
			
		КонецЕсли;
		
	Иначе // выбор в форме "Действия с заказом".
		
		ТаблицаОтбораСтрок = СтрокиГруппировкиСпискаПоРекомендациям(ГруппировкаСтрок, ПараметрыЗаполненияОбеспечения.ДействияСЗаказом);
		
	КонецЕсли;
	
	Возврат ТаблицаОтбораСтрок;
	
КонецФункции

&НаСервере
Функция ЭтоСвязанныйСдвиг(СтрокаТовары)
	
	ИмяКолонки     = ИмяКолонкиКоличества(СтрокаТовары, "Картинка", ФорматИмениКолонки);
	ИндексКартинки = СтрокаТовары[ИмяКолонки];
	
	ЭтоCвязанныйСдвиг = ИндексКартинки = ИндексКартинкиДействий("СвязанныйСдвиг")
	                    Или ИмяКолонки = "КартинкаНеОбеспечено" И ИндексКартинки = 0;
	
	Возврат ЭтоCвязанныйСдвиг;
	
КонецФункции

&НаСервере
Функция СтрокиГруппировкиСпиcкаСвязанные(ГруппировкаСтрок)
	
	ТаблицаОтбораСтрок = Новый ТаблицаЗначений();
	ТаблицаОтбораСтрок.Колонки.Добавить("Заказ", ОбеспечениеКлиентСервер.ОписаниеТиповЗаказыКОбеспечению());
	ТаблицаОтбораСтрок.Колонки.Добавить("НомерСтроки", ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15, 0));
	Для Каждого СтрокаТовары Из ГруппировкаСтрок.ПолучитьЭлементы() Цикл
		
		Если ЭтоСвязанныйСдвиг(СтрокаТовары) Тогда
			
			ЗаполнитьЗначенияСвойств(ТаблицаОтбораСтрок.Добавить(), СтрокаТовары);
...
&НаСервере
Функция СтрокиГруппировкиСпискаПоРекомендациям(ГруппировкаСтрок, ДействияСЗаказом)
	
	ТаблицаОтбораСтрок = Новый ТаблицаЗначений();
	ТаблицаОтбораСтрок.Колонки.Добавить("Заказ", ОбеспечениеКлиентСервер.ОписаниеТиповЗаказыКОбеспечению());
	ТаблицаОтбораСтрок.Колонки.Добавить("НомерСтроки", ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15, 0));
	Если ДействияСЗаказом.ОтгружатьЧастями <> Неопределено И ДействияСЗаказом.ОтгружатьЧастями И ГруппировкаСтрок.НеОтгружатьЧастями Тогда
	...
	Иначе
		...
		Для Каждого СтрокаТовары Из ГруппировкаСтрок.ПолучитьЭлементы() Цикл
			...
			Если РекомендацияДоступна(СтрокаТовары, ДействияСЗаказом) Тогда
		...
		//Сформировать список строк, подлежащих обработке
		Если ВыполнитьДействия Тогда
			
			ИскатьДублиАналитики = Ложь;
			Для Каждого СтрокаТовары Из ГруппировкаСтрок.ПолучитьЭлементы() Цикл
			
				ДобавитьСтроку = РекомендацияДоступна(СтрокаТовары, ДействияСЗаказом) Или ЭтоСвязанныйСдвиг(СтрокаТовары);
				...
				Если ДобавитьСтроку Тогда
					...
					Если СтрокаТовары.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно Тогда
						...
						ТаблицаАналитики = ТаблицаАналитикиПотребности();
						ТаблицаАналитики.Колонки.Добавить("СтрокаДобавлена");
						ТаблицаАналитики.Колонки.Добавить("Строка");
					КонецЕсли;
				...
				Если ИскатьДублиАналитики И СтрокаТовары.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно Тогда
					...
					НоваяСтрока.СтрокаДобавлена = ДобавитьСтроку;
					НоваяСтрока.Строка          = СтрокаТовары;
			...
			Если ИскатьДублиАналитики Тогда
				...
				ДобавитьСтрокиЗаданнойАналитики(ТаблицаАналитики, ТаблицаОтбораСтрок);
			КонецЕсли;
...
&НаСервере
Процедура ДобавитьСтрокиЗаданнойАналитики(Таблица, ТаблицаСтрок)
	
	Таблица.Сортировать("Номенклатура, Характеристика, Склад, Подразделение, Назначение, СтрокаДобавлена УБЫВ");
	Ключ = ОбеспечениеКлиентСервер.КлючНоменклатураХарактеристикаПодразделениеНазначениеСклад();
	...
	Для Счетчик = 0 По ВГраница Цикл
		...
		Если ОбеспечениеКлиентСервер.ИзменилсяКлюч(Ключ, ТекущаяСтрока) Тогда
			ЗаполнитьЗначенияСвойств(Ключ, ТекущаяСтрока);
			ВключатьДубли = ТекущаяСтрока.СтрокаДобавлена;
		ИначеЕсли ВключатьДубли И Не ТекущаяСтрока.СтрокаДобавлена Тогда
			ЗаполнитьЗначенияСвойств(ТаблицаСтрок.Добавить(), ТекущаяСтрока.Строка);
		КонецЕсли;
...
&НаСервере
Функция ТаблицаПотребностейЗаказаНаПроизводство(ТаблицаОтбораСтрок)
	...
	Запрос.Текст =
		...
		|	РегистрЗаказы.ДатаПотребности       КАК ДатаОтгрузки,
		|	РегистрЗаказы.ДатаПотребности       КАК ДатаОтгрузкиРабот,
		|
		|	ВЫБОР КОГДА ВтФильтр.КодСтроки ЕСТЬ NULL ТОГДА
		|				ЛОЖЬ
		|			ИНАЧЕ
		|				ИСТИНА
		...
		|			ИЗ
		|				ВтФильтр КАК Таблица)) КАК РегистрЗаказы
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтФильтр КАК ВтФильтр
		|		ПО РегистрЗаказы.Распоряжение = ВтФильтр.Заказ
		|		 И РегистрЗаказы.КодСтроки = ВтФильтр.КодСтроки
		|
	...
	Запрос.УстановитьПараметр("Таблица", ТаблицаОтбораСтрок);
...
Процедура ЗаполнитьОбеспечениеДляЗаказаНаПроизводство(МенеджерДокумента, Ссылка, ТаблицаОбеспечения, ТаблицаТовары)
	...
	СтатусЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "Статус");
	
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЗаказНаПроизводство")
		И (СтатусЗаказа = Перечисления.СтатусыЗаказовНаПроизводство.КПроизводству
		...
		ОповеститьЗаказОбИзменении = Ложь;
		
	Иначе
		
		ВГраница = ТаблицаОбеспечения.Количество() - 1;
		Для Индекс = 0 По ВГраница Цикл
			ТаблицаОбеспечения[Индекс].Идентификатор = ТаблицаТовары[ТаблицаОбеспечения[Индекс].Идентификатор].ИндексСтрокиТабЧастиЗаказа;
		КонецЦикла;
		
		ТаблицаОбеспечения.Сортировать("Идентификатор, Отгружено УБЫВ, Приоритет УБЫВ, ВариантОбеспечения, ДатаОтгрузки, Склад", Новый СравнениеЗначений());
		...
		ОповеститьЗаказОбИзменении = Истина;
...
//++ НЕ УТ
	//Используется при вызове из обработки Редактирование спецификации строки заказа,
	//если выполнялись рекомендации для других строк продукции.
	ТаблицаЗаказа.Колонки.Добавить("Скрывать");
//-- НЕ УТ
...
//++ НЕ УТКА
	Если ОповеститьЗаказОбИзменении И ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
		
		Оповестить("ЗаполнитьОбеспечениеВДокументе_ЗаказНаПроизводство");
...
//++ НЕ УТКА
	Если ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
		...
		ПараметрыВыполненияКоманды = Новый Структура("ТипКоманды", Источник);
		ПараметрыЗаполненияОбеспечения = Новый Структура("НеОтгружатьЧастями, ДействияСЗаказом", НеОтгружатьЧастями, Параметр);
		ТаблицаОтбораСтрок = СтрокиСпискаДляОбработки(ПараметрыВыполненияКоманды, ПараметрыЗаполненияОбеспечения);
		
		Если Параметры.ВызовИзФормыЗаказа Тогда
			
		Иначе
			
			ТаблицаТовары = ТаблицаПотребностейЗаказаНаПроизводство(ТаблицаОтбораСтрок);
...
//++ НЕ УТ
	Если ПравоДоступа(Право, Метаданные.Документы.ЗаказПереработчику)
		И (ПравоДоступа(Право, Метаданные.РегистрыНакопления.ЗаказыКлиентов) Или Право = "Изменение") Тогда
		Фильтр.Добавить(Документы.ЗаказПереработчику);
	КонецЕсли;
	
	Если ПравоДоступа(Право, Метаданные.Документы.ЗаказМатериаловВПроизводство)
		И (ПравоДоступа(Право, Метаданные.РегистрыНакопления.ЗаказыМатериаловВПроизводство) Или Право = "Изменение") Тогда
		Фильтр.Добавить(Документы.ЗаказМатериаловВПроизводство);
	КонецЕсли;
...
//++ НЕ УТКА
	...
	Если ПравоДоступа(Право, Метаданные.Документы.ЗаказДавальца)
		И (ПравоДоступа(Право, Метаданные.РегистрыНакопления.ЗаказыКлиентов) Или Право = "Изменение") Тогда
		Фильтр.Добавить(Документы.ЗаказДавальца);
	КонецЕсли;
	
	Если ПравоДоступа(Право, Метаданные.Документы.ЗаказНаПроизводство)
		И (ПравоДоступа(Право, Метаданные.РегистрыНакопления.ЗаказыНаВнутреннееПотребление) Или Право = "Изменение") Тогда
			Фильтр.Добавить(Документы.ЗаказНаПроизводство);
	КонецЕсли;
	
	Если ПравоДоступа(Право, Метаданные.Документы.ЗаказНаРемонт)
		И (ПравоДоступа(Право, Метаданные.РегистрыНакопления.ЗаказыНаВнутреннееПотребление) Или Право = "Изменение") Тогда
		Фильтр.Добавить(Документы.ЗаказНаРемонт);
	КонецЕсли;
//-- НЕ УТКА
