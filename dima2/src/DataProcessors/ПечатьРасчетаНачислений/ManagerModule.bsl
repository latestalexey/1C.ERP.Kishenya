...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПодробныйРасчетНачислений") Тогда
		...
		ТабличныйДокумент = ТабличныйДокументПодробногоРасчетаНачислений(МассивОбъектов, ОбъектыПечати, ПараметрыВывода, "ПодробныйРасчетНачислений");
		Если НЕ ТабличныйДокумент = Неопределено Тогда
			УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ПодробныйРасчетНачислений", НСтр("ru='Подробный расчет начислений';uk='Докладний розрахунок нарахувань'"), ТабличныйДокумент,,,,Истина);
		КонецЕсли;
	КонецЕсли;
...
Функция ТабличныйДокументПодробногоРасчетаНачислений(МассивСсылок, ОбъектыПечати, ПараметрыВывода, ИмяМакета)	
	...
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПодробныйРасчетНачислений";
	...
	Для каждого ОписаниеМенеджера Из МассивОписанийМенеджеров Цикл
		...
		Если ОписаниеМенеджера.ОписаниеДокумента.ЕстьРасчетСреднегоЗаработка Тогда
			Если ОписаниеМенеджера.ОписаниеДокумента.СреднийЗаработокОбщий Тогда
				...
				ОбластиСреднегоЗаработкаДляВстраивания = Обработки.ПечатьРасчетаСреднегоЗаработка.ОбластиДляВстраивания(ДанныеДокументов, "РасчетСреднегоЗаработкаКомпактный",,ПараметрыВывода);
			Иначе 
				...
				ОбластиСреднегоЗаработкаДляВстраивания = Обработки.ПечатьРасчетаСреднегоЗаработкаФСС.ОбластиДляВстраивания(ДанныеДокументов, Ложь, Истина,КодЯзыкаПечать);
			КонецЕсли;
		КонецЕсли;
		...
		Для каждого Ссылка Из ОписаниеМенеджера.МассивСсылок Цикл
			...
			Для каждого ДанныеДокумента Из СтрокиДанныхДокументов Цикл
				...
				ЗаполнитьТабличныйДокументПодробногоРасчетаНачислений(ДанныеДляПечати, ОбъектыПечати, ИмяМакета, ТабличныйДокумент, КодЯзыкаПечать);			
...
Процедура ЗаполнитьТабличныйДокументПодробногоРасчетаНачислений(ДанныеДляПечати, ОбъектыПечати, ИмяМакета, ТабличныйДокумент, КодЯзыкаПечать)
	...
	Для Каждого Сотрудник Из ДанныеДокумента.МассивСотрудников Цикл
		...
		ОбластиМакета = ОбластиМакетаПодробногоРасчетаНачислений(КодЯзыкаПечать);
		...
		ЗначенияПараметров.Вставить("СинонимДокумента", 				Формат(ДанныеДляПечати.СинонимДокумента, "Л="+КодЯзыкаПечать));
		ЗначенияПараметров.Вставить("НомерДокумента", 					ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(СокрЛП(ДанныеДокумента.НомерДокумента), Истина, Истина));
		ЗначенияПараметров.Вставить("ДатаДокумента", 					Формат(ДанныеДокумента.ДатаДокумента, "Л="+КодЯзыкаПечать+"; ДЛФ=DD"));
		...
		Если НачисленияПоСотруднику.Количество() > 0 Тогда
			...
			Если ДанныеДляПечати.ЕстьРасчетЗарплаты Тогда
				...
				Если НачисленияСекцииПоСотруднику.Количество() > 0 Тогда
					НазваниеСекции = НСтр("ru='Расчет оплаты труда';uk='Розрахунок оплати праці'");
					ВывестиСекциюНачислений(ТабличныйДокумент, ОбластиМакета, НомерРаздела, НачисленияСекцииПоСотруднику, ДанныеДляПечати.Показатели, НазваниеСекции);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ОбластиМакетаПодробногоРасчетаНачислений(КодЯзыкаПечать)
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Обработка.ПечатьРасчетаНачислений.ПФ_MXL_ПодробныйРасчетНачислений", КодЯзыкаПечать);
...
Процедура ВывестиСекциюСреднийЗаработокШапка(ТабличныйДокумент, СекцияСреднийЗаработокШапка, НомерРаздела, ДанныеДокумента)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба")
		...
		ТекстОбласти = Строка(НомерРаздела) + ". " + НСтр("ru='Сохраняемое денежное содержание';uk='Зберігається грошове утримання'")
			+ Символы.ПС
			+ Формат(ДанныеДокумента.СохраняемоеДенежноеСодержание, "ЧДЦ=2");
			
	Иначе
		ТекстОбласти = Строка(НомерРаздела) + ". " + НСтр("ru='Расчет среднего заработка';uk='Розрахунок середнього заробітку'");
	КонецЕсли;
