...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Иначе
			ВызватьИсключение СтрЗаменить(
				НСтр("ru='Некорректные параметры объектов назначения открытия команды ""%1""';uk='Некоректні параметри об''єктів призначення відкриття команди ""%1""'"),
	...
	Если Не ПроконтролироватьНастройки() И ОткрытиеСУзломПараметром Тогда
		...
		ТекстСообщения = СтрЗаменить(
			НСтр("ru='Для ""%1"" редактирование регистрации объектов недоступно.';uk='Для ""%1"" редагування реєстрації об''єктів недоступно.'"),
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) <> Тип("Структура") 
		...
		Ошибка = НСтр("ru='Неожиданный результат выбора из консоли запросов';uk='Несподіваний результат вибору з консолі запитів'");
	Иначе
	...
	Если ВыбранноеЗначение.ДействиеВыбора Тогда
		Текст = НСтр("ru='Зарегистрировать результат запроса
                         |на узле ""%1""?'
                         |;uk='Зареєструвати результат запиту
                         |на вузлі ""%1""?'"); 
	Иначе
		Текст = НСтр("ru='Отменить регистрацию результата запроса
                         |на узле ""%1""?'
                         |;uk='Скасувати реєстрацію результату запиту
                         |на вузлі ""%1""?'");
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "%1", Строка(УзелОбменаСсылка));
					 
	ЗаголовокВопроса = НСтр("ru='Подтверждение';uk='Підтвердження'");
...
Процедура ПоказатьРезультатВыгрузки(Команда)
	...
	Если Сериализация.Количество() > 0 Тогда
		...
		ЗаголовокТекста = НСтр("ru='Результат стандартной выгрузки (РИБ)';uk='Результат стандартного вивантаження (РІБ)'");
...
Процедура ЗарегистрироватьИОМИПредопределенные(Команда)
	...
	ТекстВопроса     = СтрЗаменить( 
		НСтр("ru='Зарегистрировать данные для восстановления подчиненного узла РИБ
             |на узле ""%1""?'
             |;uk='Зареєструвати дані для відновлення підпорядкованого вузла РІБ
             |на вузлі ""%1""?'"),
		"%1", УзелОбменаСсылка);
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Не выгружалось';uk='Не вивантажувалося'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Нет изменений';uk='Немає змін'"));
...
Процедура РедактироватьНомерСообщенияКонстанты()
	...
	Подсказка = НСтр("ru='Номер отправленного';uk='Номер відправленого'"); 
...
Процедура УстановитьЗаголовокНомеровСообщений()
	...
	Текст = НСтр("ru='№ отправленного %1, № принятого %2';uk='№ відправленого %1, № прийнятого %2'");
...
Процедура СообщитьОРезультатахРегистрации(Команда, Результаты)
	...
	Если ТипЗнч(Команда) = Тип("Булево") Тогда
		Если Команда Тогда
			ЗаголовокПредупреждения = НСтр("ru='Регистрация изменений:';uk='Реєстрація змін:'");
			ТекстПредупреждения = НСтр("ru='Зарегистрировано %1 изменений из %2
                                       |на узле ""%0""'
                                       |;uk='Зареєстровано %1 змін з %2
                                       |на вузлі ""%0""'");
		Иначе
			ЗаголовокПредупреждения = НСтр("ru='Отмена регистрации:';uk='Скасування реєстрації:'");
			ТекстПредупреждения = НСтр("ru='Отменена регистрация %1 изменений 
                                       |на узле ""%0"".'
                                       |;uk='Скасовано реєстрацію %1 змін 
                                       |на вузлі ""%0"".'");
		КонецЕсли;
	Иначе
		ЗаголовокПредупреждения = НСтр("ru='Изменение номера сообщения:';uk='Зміна номера повідомлення:'");
		ТекстПредупреждения = НСтр("ru='Номер сообщения изменен на %3
                                   |у %1 объекта(ов)'
                                   |;uk='Номер повідомлення змінений на %3
                                   |в %1 об''єкта(ів)'");
	КонецЕсли;
...
Процедура УдалитьРегистрациюКонстантыВСписке()
	...
	ИначеЕсли Количество = 1 Тогда
		Текст = НСтр("ru='Отменить регистрацию ""%2""
                         |на узле ""%1""?'
                         |;uk='Скасувати реєстрацію ""%2""
                         |на вузлі ""%1""?'"); 
	Иначе
		Текст = НСтр("ru='Отменить регистрацию выбранных констант
                         |на узле ""%1""?'
                         |;uk='Скасувати реєстрацію обраних констант
                         |на вузлі ""%1""?'"); 
	КонецЕсли;
...
Процедура УдалитьРегистрациюИзСпискаСсылок()
	...
	Иначе
		Текст = НСтр("ru='Отменить регистрацию выбранных объектов
                         |на узле ""%1""?'
                         |;uk='Скасувати реєстрацію обраних об''єктів
                         |на вузлі ""%1""?'"); 
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "%1", УзелОбменаСсылка);
...
Процедура ДобавитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации = Истина)
	...
	Текст = НСтр("ru='Зарегистрировать %1 для выгрузки на узле ""%2""?
                     ...
                     |;uk='Зареєструвати %1 для вивантаження на вузлі ""%2""?
                     |
                     |Зміна реєстрації великої кількості об''єктів може забрати тривалий час!'");
...
Процедура УдалитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации = Истина)
	...
	Текст = НСтр("ru='Отменить регистрацию %1 для выгрузки на узле ""%2""?
                     |
                     |Изменение регистрации большого количества объектов может занять продолжительное время!'
                     |;uk='Скасувати реєстрацію %1 для вивантаження на вузлі ""%2""?
                     |
                     |Зміна реєстрації великої кількості об''єктів може забрати тривалий час!'");
...
Процедура ОперацияСРезультатамиЗапроса(КомандаОперации)
	...
	Если ТекИмяФормы <> Неопределено Тогда
		...
		Если КомандаОперации Тогда
			Текст = НСтр("ru='Регистрация изменений результата запроса';uk='Реєстрація змін результату запиту'");
		Иначе
			Текст = НСтр("ru='Отмена регистрации изменений результата запроса';uk='Скасування реєстрації змін результату запиту'");
		КонецЕсли;
	...
	Текст = НСтр("ru='В настройках не указана обработка для выполнения запросов.
                            |Настроить сейчас?'
                            |;uk='У настройках не зазначена обробка для виконання запитів.
                            |Настроїти зараз?'");
	
	ЗаголовокВопроса = НСтр("ru='Настройки';uk='Настройки'");
...
Функция КонтрольСсылокДляВыбораЗапросом(Адрес)
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда 
		...
		Если Результат.Колонки.Найти("Ссылка") = Неопределено Тогда
			Возврат НСтр("ru='В последнем результате запроса отсутствует колонка ""Ссылка""';uk='В останньому результаті запиту відсутня колонка ""Посилання""'");
		КонецЕсли;
	Иначе		
		Возврат НСтр("ru='Ошибка получения данных результата запроса';uk='Помилка одержання даних результату запиту'");
	КонецЕсли;
...
Процедура НастроитьПустуюСтраницу(Наименование, ИмяТаблицы = Неопределено)
	...
	Иначе
		...
		Если Строка <> Неопределено Тогда
			ТекстКоличеств = НСтр("ru='Зарегистрировано объектов: %1
                                      ...
                                      |'
                                      |;uk='Зареєстровано об''єктів: %1
                                      |Вивантажено об''єктів: %2
                                      |Не вивантажено об''єктів: %3
                                      |'");
	...
	Текст = НСтр("ru='%1.
                     ...
                     |
                     |%2
                     |Для реєстрації або скасування реєстрації обміну даними на вузлі
                     |""%3""
                     |виберіть тип об''єкта ліворуч у дереві метаданих і скористайтеся
                     |командами ""Зареєструвати"" або ""Скасувати реєстрацію""'");
...
Функция ПолучитьОписаниеВыбранныхМетаданных(БезУчетаАвторегистрации, МетаИмяГруппа = Неопределено, МетаИмяУзел = Неопределено)
	...
	Если МетаИмяГруппа = Неопределено И МетаИмяУзел = Неопределено Тогда
		// Не указано ничего
		Текст = НСтр("ru='все объекты %1 по выбранной иерархии вида';uk='всі об''єкти %1 по обраній ієрархії виду'");
	...
	Иначе
		...
		Текст = НСтр("ru='все объекты типа ""%3"" %1';uk='всі об''єкти типу ""%3"" %1'");
	...
	Если БезУчетаАвторегистрации Тогда
		ТекстФлага = "";
	Иначе
		ТекстФлага = НСтр("ru='с признаком авторегистрации';uk='з ознакою автореєстрації'");
	КонецЕсли;
...
Процедура ОбработатьЗапретИзмененияУзла()
	...
	Если ОперацииРазрешены Тогда
		...
		Заголовок = НСтр("ru='Регистрация изменений для обмена данными';uk='Реєстрація змін для обміну даними'");
	Иначе
		Элементы.УзелОбменаСсылка.Видимость = Ложь;
		Заголовок = СтрЗаменить(НСтр("ru='Регистрация изменений для обмена с  ""%1""';uk='Реєстрація змін для обміну з  ""%1""'"), "%1", Строка(УзелОбменаСсылка));
	КонецЕсли;
...
Функция ПроверитьВерсиюИРежимСовместимостиПлатформы()
	...
	Если Не (Лев(Информация.ВерсияПриложения, 3) = "8.3"
		...
		ВызватьИсключение НСтр("ru='Обработка предназначена для запуска на версии платформы
            |8.3.5 с отключенным режимом совместимости или выше'
            |;uk='Обробка призначена для запуску на версії платформи
            |8.3.5 з відключеним режимом сумісності або вище'");
