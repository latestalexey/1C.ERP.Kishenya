...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаписьЖурналаРегистрации(ОбновлениеКонфигурации.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,,,
		НСтр("ru='Открытие помощника обновления конфигурации...';uk='Відкриття помічника оновлення конфігурації...'"));
	...
	Иначе
		...
		Если Не КонфигурацияГотоваКОбновлению Тогда
			...
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, "Информация",
				НСтр("ru='Конфигурация не может быть обновлена. Завершение проверки обновления.';uk='Конфігурація не може бути оновлена. Завершення перевірки оновлення.'"));
			Отказ = Истина;
			Возврат;
		КонецЕсли; 
...
Процедура НадписьКакОформитьПодпискуИТСНажатие(Элемент)
	ОбновлениеКонфигурацииКлиент.ОткрытьВебСтраницу("http://its.1c.ua/db/aboutitsu#content:20:hdoc");
КонецПроцедуры
...
Процедура НадписьПодробнееОбИТСОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ОбновлениеКонфигурацииКлиент.ОткрытьВебСтраницу("http://its.1c.ua/about");
КонецПроцедуры
...
Процедура ПолеФайлОбновленияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Диалог.Фильтр = НСтр("ru='Все файлы поставки (*.cf*;*.cfu)|*.cf*;*.cfu|Файлы поставки конфигурации (*.cf)|*.cf|Файлы поставки обновления конфигурации(*.cfu)|*.cfu';uk='Всі файли поставки (*.cf*;*.cfu)|*.cf*;*.cfu|Файли поставки конфігурації (*.cf)|*.cf|Файли поставки оновлення конфігурації(*.cfu)|*.cfu'");
...
Процедура НовоеВВерсии(Команда)
	...
	Если НЕ ПустаяСтрока(ИмяФайлаИнформацияОбОбновлении) Тогда
		ОбновлениеКонфигурацииКлиент.ОткрытьВебСтраницу(ИмяФайлаИнформацияОбОбновлении, НСтр("ru='Новое в версии';uk='Нове в версії'"));
	Иначе
		ПоказатьПредупреждение(, НСтр("ru='Информация об обновлении отсутствует.';uk='Інформація об оновленні відсутня.'"));
	КонецЕсли;
...
Процедура ПолучитьДоступныеОбновленияВИнтервале(Знач ВерсияОт, Знач ВерсияДо, АдресФайлаВоВремХранилище, 
	...
	Пока ТекущаяВерсияОт <> ВерсияДо Цикл
		...
		Если ТекущаяВерсияОт = НоваяТекущаяВерсияОт И НоваяТекущаяВерсияОт <> ВерсияДо Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Недоступно обновление на версию %1 с текущей версии %2';uk='Недоступне оновлення на версію %1 з поточної версії %2'"), ВерсияДо, ВерсияОт);
		КонецЕсли;
...
Процедура ВыполнитьЗагрузкуСпискаОбновлений(Знач АдресФайлаЗагрузки, ТаблицаОбновлений = Неопределено)
	...
	СообщениеОбОшибке = НСтр("ru='Ошибка при чтении файла списка обновлений';uk='Помилка при читанні файлу списку оновлень'") + " " + ПолныйПутьФайлаЗагрузки;
	...
	Если ТаблицаОбновлений = Неопределено Тогда // ошибки при чтении файла
		ВызватьИсключение НСтр("ru='Ошибка при чтении файла';uk='Помилка при читанні файлу'") + " " + ПолныйПутьФайлаЗагрузки;
	КонецЕсли;
...
Процедура ПередОткрытиемСтраницы(НоваяТекущаяСтраница = Неопределено)
	...
	ИначеЕсли НоваяТекущаяСтраница = Страницы.ПодключениеКСайту Тогда
		...
		Если Не ПустаяСтрока(Объект.ТехническаяИнформацияОбОшибке) Тогда
			Объект.ТехническаяИнформацияОбОшибке = НСтр("ru='Техническая информация об ошибке:';uk='Технічна інформація про помилку:'") + Символы.ПС + Объект.ТехническаяИнформацияОбОшибке;
		КонецЕсли;
	...
	ИначеЕсли НоваяТекущаяСтраница = Страницы.ДоступноеОбновление Тогда
		...
		Если СтруктураДоступногоОбновления <> Неопределено Тогда
			...
			Если ДоступноОбновлениеДляСледующейРедакции Или ТребуетсяОбновлениеПлатформы Тогда
				...
				ШаблонЗаголовка = НСтр("ru='Для обновления на эту версию требуется платформа
                ...
                |;uk='Для оновлення на цю версію потрібно платформа
                |не нижче версії <b>%1</b>. Необхідно <a href = ""КакОбновитьПлатформу"">перейти на нову версію платформи</a>
                |після чого можна буде встановити це оновлення.'");
	...
	ИначеЕсли НоваяТекущаяСтраница = Страницы.ОбновленияНеОбнаружено Тогда
		...
		Элементы.НадписьВерсияДляОбновления.Заголовок			= ?(ТипЗнч(СтруктураДоступногоОбновления) = Тип("Структура"), НСтр("ru='Доступная версия для обновления -';uk='Доступна версія для оновлення -'") + " " + СтруктураДоступногоОбновления.Версия, "");
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ВерсияКонфигурации,
			...
			Элементы.ГруппаОбновленияНеОбнаружено.Заголовок = НСтр("ru='Обновление не требуется';uk='Оновлення не потрібно'");
		...
		Иначе 
			...
			Элементы.ГруппаОбновленияНеОбнаружено.Заголовок = НСтр("ru='Обновления конфигурации не обнаружено';uk='Оновлення конфігурації не знайдено'");
		КонецЕсли;
	...
	КнопкаДалее.Заголовок = ?(ФункцияКнопкиДалее, НСтр("ru='Далее >';uk='Далі >'"), НСтр("ru='Готово';uk='Готово'"));
	КнопкаЗакрыть.Заголовок = ?(ФункцияКнопкиЗакрыть, НСтр("ru='Отмена';uk='Відмінити'"), НСтр("ru='Закрыть';uk='Закрити'"));
...
Функция ВосстановитьРезультатыПредыдущегоЗапуска(Отказ = Ложь)
	...
	Если ИмяСтраницы = Страницы.ДоступноеОбновление.Имя Тогда
		...
		Если ПроверитьФайлыОбновленияПолучены() Тогда
			...
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, "Информация",
				НСтр("ru='Обнаружено, что файлы обновления конфигурации уже были получены и сохранены локально.';uk='Виявлено, що файли оновлення конфігурації вже були отримані та збережені локально.'"));
...
Функция ОбработатьСтраницуПодключениеКСайту(ВыдаватьСообщения = Истина)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КодПользователяСервераОбновлений) Тогда
		Если ВыдаватьСообщения Тогда
			ПоказатьПредупреждение(, НСтр("ru='Укажите код пользователя для обновления.';uk='Введіть код користувача для оновлення.'"));
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.ПарольСервераОбновлений) Тогда
		Если ВыдаватьСообщения Тогда
			ПоказатьПредупреждение(, НСтр("ru='Укажите пароль пользователя для обновления.';uk='Вкажіть пароль користувача для оновлення.'"));
		КонецЕсли;
...
Функция ОбработатьСтраницуВыборРежимаОбновления(ВыдаватьСообщения = Истина, ФлагЗавершенияРаботы = Ложь)
	...
	Если ИнформационнаяБазаФайловая И НЕ РежимИмитацииКлиентСервернойИБ() И Объект.СоздаватьРезервнуюКопию = 2 Тогда
		...
		Если Не Файл.Существует() Или Не Файл.ЭтоКаталог() Тогда
			ПоказатьПредупреждение(, НСтр("ru='Укажите существующий каталог для сохранения резервной копии ИБ.';uk='Укажіть існуючий каталог для збереження резервної копії ІБ.'"));
	...
	Если Объект.РежимОбновления = 0 Тогда   // Обновить сейчас
		Если ИнформационнаяБазаФайловая И НЕ РежимИмитацииКлиентСервернойИБ() Тогда
			...
			Если НаличиеСоединений Тогда
				ПоказатьПредупреждение(, НСтр("ru='Невозможно продолжить обновление конфигурации, так как не завершены все соединения с информационной базой.';uk='Неможливо продовжити оновлення конфігурації, тому що не завершені всі з''єднання з інформаційною базою.'"));
				Возврат ТекущаяСтраница.Имя;
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли Объект.РежимОбновления = 2 Тогда  // Запланировать обновление
		...
		Если НЕ ЗапланироватьОбновлениеКонфигурации() Тогда
			ПоказатьПредупреждение(, НСтр("ru='Невозможно запланировать обновление конфигурации. Сведения об ошибке сохранены в журнал регистрации.';uk='Неможливо запланувати оновлення конфігурації. Відомості про помилку збережені в журнал реєстрації.'"));
			Возврат ТекущаяСтраница.Имя;
		КонецЕсли;
	...
	Сообщение = НСтр("ru='Выбран режим обновления:';uk='Обрано режим оновлення:'") + " ";
	Если Объект.РежимОбновления = 0 Тогда   // Обновить сейчас
		Сообщение = Сообщение + НСтр("ru='сейчас';uk='зараз'");
	ИначеЕсли Объект.РежимОбновления = 1 Тогда  // при завершении работы
		Сообщение = Сообщение + НСтр("ru='при завершении работы';uk='при завершенні роботи'");
	ИначеЕсли Объект.РежимОбновления = 2 Тогда  // Запланировать обновление
...
Процедура ОбработатьНажатиеКнопкиДалее(ФлагЗавершенияРаботы = Ложь)
	...
	Если Не СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().НастройкиОбновления.ЕстьДоступДляОбновления
		...
		ПоказатьПредупреждение(, НСтр("ru='Недостаточно прав для выполнения обновления конфигурации.';uk='Недостатньо прав для виконання оновлення конфігурації.'"));
	КонецЕсли;
...
Функция ОбработатьСтраницуДоступноеОбновление(ВыдаватьСообщения = Истина)
	...
	Если СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ИнформационнаяБазаФайловая И УстановленыРасширения Тогда
		ПоказатьПредупреждение(, НСтр("ru='Перед обновлением необходимо удалить все подключенные расширения конфигурации.
            ...
            |;uk='Перед оновленням необхідно видалити всі підключені розширення конфігурації.
            |Після завершення оновлення їх потрібно підключити заново.'"));
		Возврат Страницы.ДоступноеОбновление.Имя;
	КонецЕсли;
...
Функция ОбработатьСтраницуФайлОбновления(ВыдаватьСообщения = Истина)
	...
	Если Объект.НуженФайлОбновления = 1 Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.ИмяФайлаОбновления) Тогда
			Если ВыдаватьСообщения Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите файл поставки обновления конфигурации.';uk='Укажіть файл поставки оновлення конфігурації.'"),,"Объект.ИмяФайлаОбновления");
			КонецЕсли;
		...
		Если НЕ Файл.Существует() ИЛИ НЕ Файл.ЭтоФайл() Тогда
			Если ВыдаватьСообщения Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Файл поставки обновления конфигурации не найден.';uk='Файл поставки оновлення конфігурації не знайдений.'"),,"Объект.ИмяФайлаОбновления");
			КонецЕсли;
			ТекущийЭлемент = Элементы.ПолеФайлОбновления;
			Возврат Страницы.ФайлОбновления.Имя;
		КонецЕсли;
...
Процедура ПерейтиКВыборуРежимаОбновления(ЭтоПереходДалее = Ложь)
	...
	Если ПараметрыАдминистрирования = Неопределено Тогда
		...
		ЗаголовокФормы = НСтр("ru='Установка обновления';uk='Установка оновлення'");
		Если ЭтоФайловаяБаза Тогда
			ПоясняющаяНадпись = НСтр("ru='Для установки обновления необходимо ввести
                |параметры администрирования информационной базы'
                |;uk='Для установки оновлення необхідно ввести
                |параметри адміністрування інформаційної бази'");
		...
		Иначе
			ПоясняющаяНадпись = НСтр("ru='Для установки обновления необходимо ввести параметры
                ...
                |;uk='Для установки оновлення необхідно ввести параметри
                |адміністрування кластера серверів й інформаційної бази'");
...
Процедура ПослеПолученияПараметровАдминистрирования(Результат, ЭтоПереходДалее) Экспорт
	...
	Иначе
		...
		ТекстПредупреждения = НСтр("ru='Для установки обновления необходимо ввести параметры администрирования.';uk='Для встановлення оновлення необхідно ввести параметри адміністрування.'");
		...
		ТекстСообщения = НСтр("ru='Не удалось установить обновление программы, т.к. не были введены
            ...
            |;uk='Не вдалося встановити оновлення програми, т. я. не були введені
            |коректні параметри адміністрування інформаційної бази.'");
...
Функция ПроверитьНастройкиЭлектроннойПочты(ИмяТекущейСтраницы, ВыдаватьСообщения = Истина)
	Если Не ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(Объект.АдресЭлектроннойПочты) Тогда
		Если ВыдаватьСообщения Тогда
			ПоказатьПредупреждение(, НСтр("ru='Укажите допустимый адрес электронной почты.';uk='Укажіть припустиму адресу електронної пошти.'"));
		КонецЕсли;
...
Функция ПроверитьДопустимуюДатуОбновленияНаСервере(ДатаВремя)
	...
	Если ДатаВремя < Сейчас Тогда
		Возврат НСтр("ru='Обновление конфигурации может быть запланировано только на будущую дату и время.';uk='Оновлення конфігурації може бути заплановано тільки на майбутню дату й час.'");
	КонецЕсли;
	Если ДатаВремя > ДобавитьМесяц(Сейчас, 1) Тогда
		Возврат НСтр("ru='Обновление конфигурации может быть запланировано не позднее, чем через месяц относительно текущей даты.';uk='Оновлення конфігурації може бути заплановано не пізніше, ніж через місяць щодо поточної дати.'");
	КонецЕсли;
...
Функция ПолучитьФайлОписанияОбновления()
	...
	Исключение
		ТекстСообщения = НСтр("ru='Ошибка при удалении временного файла %1
            |%2'
            |;uk='Помилка при вилученні тимчасового файлу %1
            |%2'");
		ИмяСобытияЖурнала =	ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации();
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала,
...
Функция РаспаковатьДистрибутивОбновления()
	...
	ИмяСобытияЖурнала = ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации();
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, 
		"Информация", НСтр("ru='Выполняется распаковка дистрибутива обновления...';uk='Виконується розпакування дистрибутива оновлення...'"));
		...
		Попытка 
			...
			Если НЕ ФайлСуществуетНаКлиенте(ИмяФайлаОбновления) Тогда
				ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, "Ошибка",
					НСтр("ru='Дистрибутив обновления не содержит 1cv8.cfu:';uk='Дистрибутив оновлення не містить 1cv8.cfu:'")
			...
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, "Информация",
				НСтр("ru='Файлы дистрибутива обновления успешно распакованы:';uk='Файли дистрибутива оновлення успішно розпаковані:'")
					+ " " + ИмяФайлаОбновления);
		Исключение
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, "Ошибка",
				НСтр("ru='Ошибка при распаковке дистрибутива обновления:';uk='Помилка при розпакуванні дистрибутива оновлення:'")
					+ " " + ОписаниеОшибки());
			Возврат Ложь;
		КонецПопытки;
		...
		Попытка
			...
			Если НЕ ФайлСуществуетНаКлиенте(ИмяФайлаОбновления) Тогда
				ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
					"Ошибка", НСтр("ru='Дистрибутив обновления не содержит 1cv8.cfu:';uk='Дистрибутив оновлення не містить 1cv8.cfu:'")
						+ " " + Обновление.ПутьКЛокальномуФайлу);
				Возврат Ложь;
			КонецЕсли;
			Обновление.ПутьКЛокальномуФайлуОбновления = ИмяФайлаОбновления;
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
				"Информация", НСтр("ru='Файлы дистрибутива обновления успешно распакованы:';uk='Файли дистрибутива оновлення успішно розпаковані:'")
					+ " " + ИмяФайлаОбновления);
			ЧтениеZip.Закрыть();
		Исключение
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
				"Ошибка",  НСтр("ru='Ошибка при распаковке дистрибутива обновления:';uk='Помилка при розпакуванні дистрибутива оновлення:'")
					+ " " + ОписаниеОшибки());
			Возврат Ложь;
		КонецПопытки;
	КонецЦикла;
...
Функция ПолучитьФайлСпискаШаблонов(Знач ВыдаватьСообщения = Истина, ДоступноОбновлениеДляСледующейРедакции = Ложь)
	...
	Если Результат.Статус <> Истина Тогда
		...
		КонецПопытки;
		ТекстОшибки = НСтр("ru='Ошибка при получении файла списка шаблонов:';uk='Помилка при одержанні файлу списку шаблонів:'") + " " + Результат.СообщениеОбОшибке;
		Если ВыдаватьСообщения Тогда
	...
	Если НЕ ФайлСуществуетНаКлиенте(ПутьКФайлуСпискаШаблонов) Тогда
		Возврат НСтр("ru='Файл не существует:';uk='Файл не існує:'") + " " + ПутьКФайлуСпискаШаблонов;
	КонецЕсли;
	...
	Исключение
		ТекстОшибки	= НСтр("ru='Ошибка при распаковке файла со списком доступных обновлений:';uk='Помилка при розпакуванні файлу зі списком доступних оновлень:'") + " ";
...
Функция ПолучитьОбновление(ВыдаватьСообщения = Истина)
	...
	Сообщение = "";
	Если Объект.ИсточникОбновления = 0 Тогда
		Сообщение = Сообщение + НСтр("ru='Получение файлов из Интернета...';uk='Одержання файлів з Інтернету...'");
	Иначе
		Сообщение = Сообщение + НСтр("ru='Получение файла обновления из указанного источника...';uk='Одержання файлу оновлення із зазначеного джерела...'");
	КонецЕсли;
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
		"Информация", Сообщение);
	...
	Для Каждого Файл Из СписокФайловДляПолучения Цикл
		...
		Если Файл.Значение <> Неопределено И Файл.Значение.Получен <> Истина Тогда	// Может быть также Неопределено.
			Если Объект.ИсточникОбновления = 0 Тогда
				...
				Если Не РезультатПроверкиЛегальности.ЗначениеРезультата Тогда
					...
					ТекстОшибки = НСтр("ru='Не удалось подтвердить легальность получения обновления через Интернет по причине:
                        |%1'
                        |;uk='Не вдалося підтвердити легальність отримання оновлення через Інтернет з причини:
                        |%1'");
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Получаем файл %1 в %2';uk='Отримуємо файл %1 у %2'"),
			...
			Иначе
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Получаем файл %1 в %2';uk='Отримуємо файл %1 у %2'"), Файл.Значение.Адрес, Файл.Значение.ЛокальныйПуть);
				Элементы.НадписьПрогресс.Заголовок = Сообщение;
				ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), , Сообщение);
	...
	Если ПроверитьФайлыОбновленияПолучены() Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
			"Информация", НСтр("ru='Файлы обновления успешно получены.';uk='Файли оновлення успішно отримані.'"));
		ПерейтиКВыборуРежимаОбновления(Истина);
		Возврат Неопределено;
	Иначе
		Сообщение = НСтр("ru='Ошибка при получении файлов обновления.';uk='Помилка при отриманні файлів оновлення.'");
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
			"Ошибка", Сообщение);
...
Функция СкопироватьФайл(ИмяФайлаИсточник, ИмяФайлаНазначение, ВыдаватьСообщения = Ложь)
	...
	Исключение
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при копировании: %1
                ...
                |;uk='Помилка при копіюванні: %1
                |(Джерело: %2; Приймач: %3)'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
Функция ПолучитьФайлыОбновленияЧерезИнтернет(ВыдаватьСообщения, ДоступноОбновлениеДляСледующейРедакции = Ложь)
	...
	Если ДоступноеОбновление.ИмяСтраницы = Страницы.ДоступноеОбновление.Имя Тогда
		...
		Если Объект.ДоступныеОбновления.Количество() = 0 Тогда
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
				"Информация", НСтр("ru='Невозможно продолжить обновление: нет доступных обновлений.';uk='Неможливо продовжити оновлення: немає доступних оновлень.'"));
			Возврат Страницы.ОбновленияНеОбнаружено.Имя;
		КонецЕсли;
...
Функция ПроверитьОбновлениеИнтернет(ВыдаватьСообщения = Истина) 
	Страницы		= Элементы.СтраницыПомощника.ПодчиненныеЭлементы;
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), 
		"Информация", НСтр("ru='Проверка обновления в Интернете...';uk='Перевірка оновлення в Інтернеті...'"));
	...
	Если Объект.НуженФайлОбновления = 1 Тогда
		...
		Если НЕ Файл.Существует() ИЛИ НЕ Файл.ЭтоФайл() Тогда
			...
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), "Информация",
				НСтр("ru='Невозможно продолжить обновление: файл поставки обновления конфигурации не найден.';uk='Неможливо продовжити оновлення: файл поставки оновлення конфігурації не знайдений.'"));
...
Функция ТекстНадписиРезервнойКопииИБ()
	...
	Результат = НСтр("ru='Не создавать резервную копию ИБ';uk='Не створювати резервну копію ІБ'");
	
	Если Объект.СоздаватьРезервнуюКопию = 1 Тогда
		Результат = НСтр("ru='Создавать временную резервную копию ИБ';uk='Створювати тимчасову резервну копію ІБ'");
	ИначеЕсли Объект.СоздаватьРезервнуюКопию = 2 Тогда
		Результат = НСтр("ru='Создавать резервную копию ИБ';uk='Створювати резервну копію ІБ'");
	КонецЕсли; 
	
	Если Объект.ВосстанавливатьИнформационнуюБазу Тогда
		Результат = Результат + " " + НСтр("ru='и выполнять откат при нештатной ситуации';uk='і виконувати відкат при позаштатній ситуації'");
	Иначе
		Результат = Результат + " " + НСтр("ru='и не выполнять откат при нештатной ситуации';uk='і не виконувати відкат при позаштатній ситуації'");
	КонецЕсли;
...
Процедура ВыполнитьОбновлениеКонфигурации()
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ОбновлениеКонфигурацииКлиент.СобытиеЖурналаРегистрации(), "Информация",
		НСтр("ru='Выполняется процедура обновления конфигурации:';uk='Виконується процедура оновлення конфігурації:'") + " " + ИмяГлавногоФайлаСкрипта);
...
Функция ВозможенЗапускОбновления()
	...
	#Если ВебКлиент Тогда
		...
		ТекстСообщения = НСтр("ru='Обновление программы недоступно в веб-клиенте.';uk='Оновлення програми недоступно у веб-клиєнті.'");
	#КонецЕсли
	
	Если ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент() Тогда
		ВозможенЗапускОбновления = Ложь;
		ТекстСообщения = НСтр("ru='Обновление программы недоступно в клиенте под управлением ОС Linux.';uk='Оновлення програми недоступно в клієнті під управлінням ОС Linux.'");
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиент.КлиентПодключенЧерезВебСервер() Тогда
		ВозможенЗапускОбновления = Ложь;
		ТекстСообщения = НСтр("ru='Обновление программы недоступно при подключении через веб-сервер.';uk='Оновлення програми недоступно при підключенні через веб-сервер.'");
	КонецЕсли;
...
Процедура ПоказатьПорядокОбновления()
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ru='Описание порядка обновления отсутствует.';uk='Опис порядку оновлення відсутній.'"));
	КонецЕсли;
...
Процедура ПрочитатьДанныеВЖурналРегистрации()
	...
	Для НомерСтроки = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
		...
		Если Сред(ТекущаяСтрока, 3, 1) = "." И Сред(ТекущаяСтрока, 6, 1) = "." Тогда // Строка с датой
			...
			Если Комментарий = НСтр("ru='Обновление выполнено';uk='Оновлення виконано'") Тогда
				Объект.РезультатОбновления = Истина;
				Продолжить;
			ИначеЕсли Комментарий = НСтр("ru='Обновление не выполнено';uk='Оновлення не виконано'") Тогда
...
Функция СоздатьЗадачуПланировщикаScheduledJob(Знач ИмяФайлаПрограммы, Знач ДатаВремя, Знач ИмяСобытияЖурнала)
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании задачи планировщика: %1';uk='Помилка при створенні задачі планувальника: %1'"), ОписаниеОшибки());
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, "Ошибка", ТекстСообщения);
		Возврат 0;
	...
	Если КодОшибки <> 0 Тогда // Коды ошибок: http://msdn2.microsoft.com/en-us/library/aa389389(VS.85).aspx.
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании задачи планировщика: %1';uk='Помилка при створенні задачі планувальника: %1'"), КодОшибки);
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, "Ошибка", ТекстСообщения);
		Возврат 0;
	КонецЕсли;
	
	ТекстСообщения = НСтр("ru='Задача планировщика успешно запланирована (команда: %1; дата: %2; код задачи: %3).';uk='Задача планувальника успішно запланований (команда: %1; дата: %2; код задачі: %3).'");
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала,
...
Функция СоздатьЗадачуПланировщикаScheduleService(Знач ИмяФайлаПрограммы, Знач ДатаВремя, Знач ИмяСобытияЖурнала)
	...
	Попытка
		...
		ОписаниеЗадачи.RegistrationInfo.Description = НСтр("ru='Обновление конфигурации ';uk='Оновлення конфігурації '");
...
Функция УдалитьЗадачуПланировщикаScheduledJob(КодЗадачи, ИмяСобытияЖурнала)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Ошибка при удалении задачи планировщика: %1';uk='Помилка при вилученні задачі планувальника: %1'");
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, "Ошибка",
	...
	Если Не Результат Тогда // Коды ошибок: http://msdn2.microsoft.com/en-us/library/aa389957(VS.85).aspx.
		...
		Возврат Результат;
	КонецЕсли;
	ТекстСообщения = НСтр("ru='Задача планировщика успешно удалена (код задачи: %1).';uk='Задача планувальника успішно вилучена (код задачі: %1).'");
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала, "Информация",
...
Функция ИмяЗадачиScheduleService(Знач КодЗадачи)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обновление конфигурации (%1)';uk='Оновлення конфігурації (%1)'"), Формат(КодЗадачи, "ЧГ=0"));
