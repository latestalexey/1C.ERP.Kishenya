...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		ТекстОшибки = НСтр("ru='Недостаточно прав для выполнения операции.';uk='Недостатньо прав для виконання операції.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
	
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		И Не ОбщегоНазначения.ИспользованиеРазделителяСеанса() Тогда
		ТекстОшибки = НСтр("ru='Для удаления помеченных необходимо войти в область данных.';uk='Для вилучення позначених необхідно увійти в область даних.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	Если Элементы.СтраницыФормы.ТекущаяСтраница = Элементы.СтраницаДлительнаяОперация Тогда
        ...
        |Прервать?'
        |;uk='Вилучення відмічених ще виконується.
        |Перервати?'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Прервать);
		Кнопки.Добавить(КодВозвратаДиалога.Пропустить, НСтр("ru='Не прерывать';uk='Не переривати'"));
...
Процедура НеУдаленныеЗаменитьНа(Команда)
	...
	Если МассивСсылок.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выберите объекты';uk='Виберіть об''єкти'"));
		Возврат;
	КонецЕсли;
...
Процедура ФоновоеЗаданиеЗапуститьНаКлиенте(Действие)
	...
	Текст = НСтр("ru='Удаляются объекты, помеченные на удаление...';uk='Видаляються об''єкти, які відмічені для вилучення...'");
	Если ПараметрыЗадания.ИскатьПомеченные И ПараметрыЗадания.УдалятьПомеченные Тогда
		Состояние(НСтр("ru='Поиск и удаление помеченных объектов...';uk='Пошук та вилучення відмічених об''єктів...'"));
	ИначеЕсли ПараметрыЗадания.ИскатьПомеченные Тогда
		Текст = НСтр("ru='Поиск помеченных на удаление объектов...';uk='Пошук позначених на вилучення об''єктів,...'");
		Состояние(Текст);
	Иначе
		Состояние(НСтр("ru='Удаление выбранных объектов...';uk='Видалення виділених об''єктів...'"));
	КонецЕсли;
	...
	ИначеЕсли Результат.ОшибкаПриУстановкеМонопольногоРежима Тогда
		...
		Иначе
			СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(
				Результат,
				НСтр("ru='Не удалось запустить удаление помеченных объектов';uk='Не вдалося запустити видалення відмічених об''єктів'"),
...
Процедура ПоказатьСвязиНеУдаленныхНаКлиенте()
	...
	Если СтрокаДерева = Неопределено Или СтрокаДерева.НомерКартинки < 1 Тогда
        ...
        |;uk='Виберіть об''єкт, щоб дізнатися причину,
        |за якою його не вдалося видалити.'");
	Иначе
	...
	Иначе
		...
		Шаблон = НСтр("ru='Места использования объекта ""%1"" (%2):';uk='Місця використання об''єкта ""%1"" (%2):'");
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдаление(ТаблицаЭлемент)
	...
	Если КоличествоМожноУдалить = 0 Тогда
		ТекстОшибки = НСтр("ru='Выберите объект.';uk='Виберіть об''єкт.'");
		Если КоличествоВыделено = 1 Тогда
			Если ЕстьЗаписиРегистров Тогда
				ТекстОшибки = НСтр("ru='Удаление записи регистра выполняется из ее карточки.';uk='Видалення запису регістра виконується з його картки.'");
			ИначеЕсли ЕстьКонстанты Тогда
				ТекстОшибки = НСтр("ru='Очистка значения константы выполняется из ее карточки.';uk='Очищення значення константи виконується з її картки.'");
			КонецЕсли;
		Иначе
			Если ЕстьЗаписиРегистров Или ЕстьКонстанты Тогда
				ТекстОшибки = НСтр("ru='Удаление записей регистров или очистка значений констант выполняется из их карточек.';uk='Вилучення записів регістрів або очищення значень констант виконується з їхніх карток.'");
			КонецЕсли;
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстОшибки);
		Возврат;
	...
	Иначе
		Если УстанавливатьПометку Тогда
			ТекстВопроса = НСтр("ru='Пометить выделенные объекты (%1) на удаление?';uk='Позначити вибрані об''єкти (%1) на вилучення?'");
		Иначе
			ТекстВопроса = НСтр("ru='Снять с выделенных объектов (%1) пометку на удаление?';uk='Зняти з виділених об''єктів (%1) позначку на вилучення?'");
		КонецЕсли;
...
Процедура ВидимостьДоступность(Форма)
	...
	Элементы.КнопкаДалее.Заголовок = НСтр("ru='Удалить';uk='Вилучити'");
	Элементы.КнопкаЗакрыть.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
	Элементы.КнопкаНазад.Заголовок = НСтр("ru='< В начало';uk='< На початок'");
	...
	Если ТекущаяСтраница = Элементы.СтраницаВыборРежимаУдаления Тогда
		...
		Элементы.КнопкаЗакрыть.Заголовок = НСтр("ru='Отмена';uk='Відмінити'");
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПомеченныеНаУдаление Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаНазад.Заголовок = НСтр("ru='< Назад';uk='< Назад'");
		Элементы.КнопкаДалее.Видимость = Истина;
		Элементы.КнопкаДалее.Заголовок = НСтр("ru='Удалить';uk='Вилучити'");
		Элементы.КнопкаДалее.КнопкаПоУмолчанию = Истина;
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаДлительнаяОперация Тогда
		...
		Элементы.КнопкаЗакрыть.Заголовок = НСтр("ru='Прервать и закрыть';uk='Перервати і закрити'");
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПричиныНевозможностиУдаления Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаДалее.Видимость = Истина;
		Элементы.КнопкаДалее.Заголовок = НСтр("ru='Повторить удаление';uk='Повторити вилучення'");
		Элементы.КнопкаДалее.КнопкаПоУмолчанию = Истина;
...
Функция ФоновоеЗаданиеЗапустить(Знач ПараметрыЗадания)
	...
	РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		ПараметрыЗадания,
		НСтр("ru='Удаление помеченных объектов (интерактивное)';uk='Вилучення відмічених об''єктів (інтерактивне)'"));
...
Функция ИзменитьПометкуУдаленияОбъектов(МассивСсылок, ПометкаУдаления)
	...
	Если КоличествоОбъектов = 0 Тогда
		ОповещениеЗаголовок = НСтр("ru='Объект не найден';uk='Об''єкт не знайдений'");
	Иначе
		Если ПометкаУдаления Тогда
			ОповещениеЗаголовок = НСтр("ru='Пометка удаления установлена';uk='Позначка вилучення встановлена'");
		Иначе
			ОповещениеЗаголовок = НСтр("ru='Пометка удаления снята';uk='Позначка вилучення знята'");
		КонецЕсли;
...
Процедура ФоновоеЗаданиеПоказатьСтраницуДлительнойОперации(ПараметрыЗадания)
	...
	Если ПоказыватьБублик Тогда
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ru='Пожалуйста, подождите...';uk='Будь ласка, зачекайте...'");
	...
	ИначеЕсли ПараметрыЗадания.УдалятьПомеченные Тогда
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ru='Удаляются объекты, помеченные на удаление...';uk='Видаляються об''єкти, які відмічені для вилучення...'");
		Элементы.ФоновоеЗаданиеСостояние.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Центр;
	Иначе
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ru='Поиск объектов, помеченных на удаление...';uk='Пошук об''єктів, позначених на вилучення...'");
		Элементы.ФоновоеЗаданиеСостояние.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Центр;
	КонецЕсли;
КонецПроцедуры
	...
	Если РезультатВыполненияВФоне.УдалятьПомеченные Тогда
		...
		ИначеЕсли НеУдаленныеКоличество = 0 Тогда
			...
			ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Удаление помеченных объектов успешно завершено.
                    |Удалено объектов: %1.'
                    |;uk='Вилучення відмічених об`єктів успішно завершено.
                    |Вилучено об''єктів: %1.'"),
		...
		Иначе
			...
			ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Удаление помеченных объектов завершено.
                    |Удалено объектов: %1,
                    |Не удалено: %2.'
                    |;uk='Вилучення відмічених об''єктів завершено.
                    |Вилучено об''єктів: %1,
                    |Не вилучено: %2.'"),
				Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
			...
			Если УдаленныеКоличество = 0 Тогда
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не получилось удалить объекты, помеченные на удаление (%1):';uk='Не вийшло видалити об''єкти, які відмічені для вилучення (%1):'"),
					Формат(НеУдаленныеКоличество, "ЧН=0; ЧГ="));
			Иначе
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Успешно удалено: %1 из %2, остальные объекты не удалены (%3):';uk='Успішно вилучено: %1 з %2, інші об''єкти не вилучені (%3):'"),
					Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
		...
		Если ОповещениеТекст <> Неопределено Тогда
			СтандартныеПодсистемыКлиентСервер.ВывестиОповещение(
				Результат,
				НСтр("ru='Удаление помеченных';uk='Вилучення відмічених'"),
				ОповещениеТекст,
				ОповещениеКартинка,
				НавигационнаяСсылка);
		КонецЕсли;
	...
	Для Каждого Причина Из ПрепятствующиеУдалению Цикл
		...
		Иначе
			...
			Если Причина.ОбнаруженныйСсылка = Неопределено Тогда // Константа
				...
				СтрокаПрепятствующегоУдалению.Представление = ОбнаруженныйИнформация.ПредставлениеЭлемента + " (" + НСтр("ru='Константа';uk='Константа'") + ")";
			Иначе
	...
	Если ОбнаруженыРегистры И Не ОбнаруженыСсылочныеНеОтмеченные Тогда
		Если Не ПодписьДобавлена Тогда
			Элементы.НадписьВПодвале.Заголовок = ЗаголовокПодвала + " "
				+ НСтр("ru='Для устранения сложных зависимостей рекомендуется повторить удаление с включенным флажком ""Заблокировать всю работу в программе и ускорить удаление"" (нажмите кнопку ""< В начало"").';uk='Для усунення складних залежностей рекомендується повторити видалення з включеним параметром ""Заблокувати всю роботу у програмі і прискорити видалення"" (натисніть кнопку ""< На початок"").'");
		КонецЕсли;
	Иначе
