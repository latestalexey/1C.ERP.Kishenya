...
Процедура ПопробоватьУдалитьСсылку(ПараметрыВыполнения, Ссылка, Информация, Результат)
	...
	Если Объект.ПометкаУдаления <> Истина Тогда
		Результат.Успех = Ложь;
		Результат.ИнформацияОбОшибке = НСтр("ru='Объект не помечен на удаление.';uk='Об''єкт не відмічений для вилучення.'");
		Возврат;
	КонецЕсли;
	...
	Иначе
		Результат.Успех = Ложь;
		Результат.ИнформацияОбОшибке = НСтр("ru='Объект используется в других объектах программы.';uk='Об''єкт використовується в інших об''єктах програми.'");
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаписатьПредупреждение(Ссылка, ИнформацияОбОшибке)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Удаление помеченных';uk='Вилучення відмічених'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если СтрокаТаблицы.ОбнаруженныйСсылка = Неопределено Тогда
		...
		Иначе
			Причина.ОбнаруженныйСсылка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Обнаружены неразрешимые ссылки (%1)';uk='Виявлені нерозв''язні посилання (%1)'"),
...
Процедура КлиентОтметитьПрогрессОбходаКоллекции(ПараметрыВыполнения, ИмяКоллекции)
	...
	Если ИмяКоллекции = "ПередПоискомПомеченныхНаУдаление" Тогда
		...
		Текст = НСтр("ru='Подготовка к поиску объектов, помеченных на удаление.';uk='Підготовка до пошуку об''єктів, позначених на видалення.'");
		
	ИначеЕсли ИмяКоллекции = "НайтиПомеченныеНаУдаление" Тогда
		
		Текст = НСтр("ru='Поиск объектов, помеченных на удаление.';uk='Пошук об''єктів, які відмічені для вилучення.'");
		
	ИначеЕсли ИмяКоллекции = "ВсеПомеченныеНаУдаление" Тогда
		
		Текст = НСтр("ru='Анализ помеченных на удаление.';uk='Аналіз позначених на вилучення.'");
		
	ИначеЕсли ИмяКоллекции = "ТехнологическиеОбъекты" Тогда
		
		Текст = НСтр("ru='Подготовка к удалению.';uk='Підготовка до видалення.'");
		
	ИначеЕсли ИмяКоллекции = "МонопольноеУдаление" Тогда
		
		Текст = НСтр("ru='Выполняется удаление объектов.';uk='Виконується вилучення об''єктів.'");
	...
	ИначеЕсли ИмяКоллекции = "ПользовательскиеОбъекты" Тогда
		...
		Если НеУдалено = 0 Тогда // Переход на СтрШаблон невозможен.
			Текст = НСтр("ru='Удалено: %1 из %2 объектов.';uk='Вилучено: %1 з %2 об''єктів.'");
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, ПредставлениеНомер, ПредставлениеВсего);
		Иначе
			Текст = НСтр("ru='Обработано: %1 из %2 объектов, из них не удалено: %3.';uk='Оброблено: %1 з %2 об''єктів, з них не вилучено: %3.'");
	...
	ИначеЕсли ИмяКоллекции = "ПовторноУдаляемые" Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Повторная проверка не удаленных объектов: %1 из %2.';uk='Повторна перевірка не вилучених об''єктів: %1 з %2.'"),
	...
	ИначеЕсли ИмяКоллекции = "ПрепятствующиеУдалению" Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Анализ объектов, препятствующих удалению: %1 из %2.';uk='Аналіз об''єктів, що перешкоджають вилученню: %1 з %2.'"),
			Формат(ПараметрыВыполнения.Номер, "ЧН=0; ЧГ="),
			Формат(ПараметрыВыполнения.Всего, "ЧН=0; ЧГ="));
