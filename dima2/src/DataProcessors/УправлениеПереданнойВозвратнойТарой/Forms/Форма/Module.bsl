...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_РеализацияТоваровУслуг"
		...
		Или ИмяСобытия = "Запись_ВозвратСырьяОтПереработчика"
		//-- НЕ УТ
...
Функция СообщитьОбОшибкахФормированияДанныхЗаполненияВозвратаВыкупаТары(ВыборкаРеквизиты)
	...
	//++ НЕ УТ
	Если ВыборкаРеквизиты.ЕстьОтличияЗаказПереработчику Тогда
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрЗаменить(ТекстСообщения, "%ПредставлениеПоля%", НСтр("ru='Заказ переработчику';uk='Замовлення переробнику'")),
			,
			,
			,
			Отказ);
...
Процедура ПриВыбореСрокаВозвратаВыборДатыЗавершение(ДатаВозврата, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ДатаВозврата) Тогда
		...
		ОтборСрокВозврата = НСтр("ru='Истекает на %Дата%';uk='Минає на %Дата%'");
...
Процедура ОформитьДокумент(ТипОперации)
	...
	Для Каждого ТекСтрока Из Элементы.ПереданнаяТара.ВыделенныеСтроки Цикл
		...
		//++ НЕ УТ
		Если ТипЗнч(ДанныеСтроки.ДокументПередачи) = Тип("ДокументСсылка.ПередачаСырьяПереработчику") Тогда
			ЕстьПередачи = Истина;
		Иначе
	...
	//++ НЕ УТ
	Если ЕстьПередачи И ЕстьРеализации Тогда
		
		ТекстСообщения = НСтр("ru='Команда не может быть выполнена одновременно для тары, переданной переработчикам, и для тары, переданной клиентам.';uk='Команда не може бути виконана одночасно для тари, переданої переробникам, і для тари, переданої клієнтам.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
		
	ИначеЕсли ЕстьПередачи И ТипОперации = "Заявка" Тогда
		
		ТекстСообщения = НСтр("ru='Заявка на возврат тары, переданной переработчикам, не предусмотрена.';uk='Заявка на повернення тари, переданої переробникам, не передбачена.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	...
	//++ НЕ УТ
	ИначеЕсли ТипОперации = "Возврат" И ЕстьПередачи Тогда
		ИмяФормыДокумента = "Документ.ВозвратСырьяОтПереработчика.Форма.ФормаДокумента";
	//-- НЕ УТ
...
Функция ПоместитьТаруВоВременноеХранилищеСервер(МассивСтрок)
	...
	//++ НЕ УТ
	ТипРеализации = Новый ОписаниеТипов("ДокументСсылка.РеализацияТоваровУслуг, ДокументСсылка.ПередачаСырьяПереработчику");
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	МИНИМУМ(втТаблицаТары.ДокументРеализации.ЗаказПереработчику)         КАК ЗаказПереработчику,
	//-- НЕ УТ
	...
	|	ВЫБОР
	|		КОГДА
	|			КОЛИЧЕСТВО(РАЗЛИЧНЫЕ втТаблицаТары.ДокументРеализации.ЗаказПереработчику) > 1
	|		ТОГДА
	|			ИСТИНА
	|		ИНАЧЕ
	|			ЛОЖЬ
	|	КОНЕЦ КАК ЕстьОтличияЗаказПереработчику,
	//-- НЕ УТ
	|	ВЫБОР
	|		КОГДА
