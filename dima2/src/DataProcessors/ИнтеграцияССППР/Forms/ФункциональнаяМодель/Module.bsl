...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.НомерОкна = 1 Тогда
		Заголовок = НСтр("ru='Функциональная модель (функции)';uk='Функціональна модель (функції)'");
	ИначеЕсли Параметры.НомерОкна = 2 Тогда
		Заголовок = НСтр("ru='Функциональная модель (объекты и профили)';uk='Функціональна модель (об''єкти та профілі)'");
	Иначе
		Заголовок = НСтр("ru='Функциональная модель';uk='Функціональна модель'");
	КонецЕсли; 
	...
	ИначеЕсли ПараметрыОткрытия.Свойство("РазделИнтерфейса") Тогда
		...
		Если НачалоНавигации = "" Тогда
			...
			ТекстПараграфа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru='Для раздела ""%1"" не определены функции.';uk='Для розділу ""%1"" не визначені функції.'"),
...
Процедура ОбновитьДанные()
	...
	ДанныеФунциональнойМодели = ИнтеграцияССППРПовтИсп.ПодготовитьДанныеФунциональнойМодели();
...
Процедура ОткрытьФормуРабочегоМеста()
	...
	Иначе
		...
		ИначеЕсли СписокВыбораРабочихМест.Количество() > 1 Тогда
			...
			СписокВыбораРабочихМест.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ru='Выберите какую форму открыть';uk='Виберіть яку форму відкрити'"));
		КонецЕсли; 
	...
	Для каждого ДанныеФормыРабочегоМеста Из ФормыРабочегоМеста Цикл
		...
		Если Лев(ПолноеИмяФормы, 6) = "Отчет." Тогда
			ПредставлениеФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Отчет ""%1""';uk='Звіт ""%1""'"),
									ПредставлениеФормы);
		Иначе
			ПредставлениеФормы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Форма ""%1""';uk='Форма ""%1""'"),
									ПредставлениеФормы);
		КонецЕсли; 
...
Процедура ОткрытьФормуРабочегоМестаЗавершение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйЭлемент <> Неопределено Тогда
		...
		Исключение
			...
			ПоказатьПредупреждение(, ТекстПредупреждения,, НСтр("ru='Не удалось открыть рабочее место';uk='Не вдалося відкрити робоче місце'"));
		КонецПопытки; 
	КонецЕсли;
...
Процедура ПерейтиКОписаниюФормы(ИмяФормыРабочегоМеста, ЗаголовокРабочегоМеста, ПовторноеОткрытие, ОбъектСодержащийФорму = Неопределено)
	...
	Иначе
		...
		ТекстПараграфа = НСтр("ru='Форма не используется в функциях и в объектах данных.';uk='Форма не використовується у функціях і в об''єктах даних.'");
		СформироватьОписаниеНеНайдено(ТекстПараграфа, ПовторноеОткрытие);
...
Процедура СформироватьОписаниеНеНайдено(ЗаголовокОписания, ПовторноеОткрытие)
	...
	Если КоллекцияФункций.Количество() <> 0 Тогда
		...
		ДобавленныйТекст = ДобавитьЭлементПараграфа(НСтр("ru='Перейти к просмотру всех функций';uk='Перейти до перегляду всіх функцій'"), НовыйПараграф);
		...
		КонецЕсли; 
	Иначе
		ДобавитьЭлементПараграфа(НСтр("ru='В СППР отсутствует функциональная модель.';uk='У СППР відсутня функціональна модель.'"), НовыйПараграф);
		Если НЕ ПовторноеОткрытие Тогда
			УстановитьНавигациюДомой(0, Неопределено);
		КонецЕсли; 
	КонецЕсли; 
