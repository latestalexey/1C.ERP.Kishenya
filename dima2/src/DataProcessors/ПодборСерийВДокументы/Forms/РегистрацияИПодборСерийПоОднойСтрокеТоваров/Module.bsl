...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДобавленияСерий
		//++ НЕ УТ
		Или Параметры.ПараметрыУказанияСерий.СкладскиеОперации.Найти(Перечисления.СкладскиеОперации.РасходМатериаловПриВыполненииМаршрутныхЛистов) <> Неопределено
		//-- НЕ УТ
			Тогда
...
Процедура ЗагрузитьИзТСДЗавершение(Результат, Параметры) Экспорт
	...
	Если Результат.Результат Тогда
		...
		ЗакрытьФорму = ОбработатьШтрихкодыСервер(Результат, КэшированныеЗначения);
...
Процедура ПоискПоШтрихкодуЗавершение(ДанныхШтрихкода, ДополнительныеПараметры) Экспорт
	...
	ЗакрытьФорму = ОбработатьШтрихкодыСервер(ДанныхШтрихкода,КэшированныеЗначения);
	Если ЗакрытьФорму Тогда
		Закрыть(АдресВоВременномХранилище);
	КонецЕсли;
...
Функция ОбработатьШтрихкодыСервер(ДанныеШтрихкодов,КэшированныеЗначения)
	...
	Иначе
		...
		ПоляИзШтрихкода = НоменклатураКлиентСервер.ИнформацияОСерииИзШтрихкода(ДанныеШтрихкодов.Штрихкод, 
												НастройкиИспользованияСерий.ИспользоватьНомерСерии,
												НастройкиИспользованияСерий.ИспользоватьСрокГодностиСерии);
	...
	Если Не ЕстьОшибки Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				КоличествоСерий = КоличествоСерий + НоваяСтрока.КоличествоУпаковок;
...
Функция СохранитьВводСерийСервер()
	...
	ПараметрыПроверки = Новый Структура("ТолькоРедактированиеКоличества,
		|ИспользоватьСрокГодностиСерии,ИспользоватьНомерСерии, ИспользоватьКоличествоСерии, Номенклатура, Упаковка");
	ЗаполнитьЗначенияСвойств(ПараметрыПроверки, НастройкиИспользованияСерий);
	ЗаполнитьЗначенияСвойств(ПараметрыПроверки, ЭтаФорма, "Номенклатура, Упаковка");
...
Процедура ПодобратьСерию(Элемент,ВводимыйРеквизит,ПроверяемыйРеквизит)
	...
	Если НастройкиИспользованияСерий.ИспользоватьСрокГодностиСерии
		 И НастройкиИспользованияСерий.ИспользоватьНомерСерии Тогда
		Наименование = НСтр("ru='<Новая> %Номер% до %ГоденДо%';uk='<Нова> %Номер% до %ГоденДо%'");
	ИначеЕсли НастройкиИспользованияСерий.ИспользоватьНомерСерии Тогда
		Наименование = НСтр("ru='<Новая> %Номер%';uk='<Нова> %Номер%'");
	ИначеЕсли НастройкиИспользованияСерий.ИспользоватьСрокГодностиСерии Тогда
		Наименование = НСтр("ru='<Новая> до %ГоденДо%';uk='<Нова> до %ГоденДо%'");
	КонецЕсли;	
	
	Наименование = СтрЗаменить(Наименование, "%Номер%", ТекущиеДанные.Номер);
	Наименование = СтрЗаменить(Наименование, "%ГоденДо%", Формат(ТекущиеДанные.ГоденДо,НастройкиИспользованияСерий.ФорматнаяСтрокаСрокаГодности));
