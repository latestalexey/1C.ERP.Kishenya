...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Действия.Вставить("НеПринимать",        НСтр("ru='Не принимать';uk='Не приймати'"));
	Действия.Вставить("ПринятьБезПроверки", НСтр("ru='Принять без проверки';uk='Прийняти без перевірки'"));
	Действия.Вставить("ДобавитьВДокумент",     НСтр("ru='Добавить в документ';uk='Додати в документ'"));
	Действия.Вставить("УдалитьИзДокумента",    НСтр("ru='Удалить из документа';uk='Видалити з документу'"));
	
	СостоянияПроверки = Новый Структура;
	СостоянияПроверки.Вставить("НеПроверен",        НСтр("ru='<не проверен>';uk='<не перевірений>'"));
	СостоянияПроверки.Вставить("ПринятБезПроверки", НСтр("ru='<принят без проверки>';uk='<прийнято без перевірки>'"));
	СостоянияПроверки.Вставить("Проверен",          НСтр("ru='<проверен>';uk='<перевірений>'"));
	...
	Если Не ИспользоватьУпаковочныеЛисты Тогда
		...
		Элементы.ТоварыГруппаКоличествоСостояние.Заголовок            = НСтр("ru='Количество';uk='Кількість'");
		Элементы.ИзлишкиГруппаНоменклатураУпаковочныйЛист.Заголовок   = НСтр("ru='Товар';uk='Товар'");
		Элементы.НедостачиГруппаНоменклатураУпаковочныйЛист.Заголовок = НСтр("ru='Товар';uk='Товар'");
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗакрытьБезВопроса
		...
		КнопкиВопроса.Добавить(КодВозвратаДиалога.Да,НСтр("ru='Завершить';uk='Завершити'"));
		КнопкиВопроса.Добавить(КодВозвратаДиалога.Нет,НСтр("ru='Продолжить проверку';uk='Продовжити перевірку'"));
		ПоказатьВопрос(ОписаниеОповещения,
			НСтр("ru='Завершить проверку без переноса расхождений в документ?';uk='Завершити перевірку без перенесення розбіжностей у документ?'"), КнопкиВопроса);
		Отказ = Истина;
...
Процедура ПриПереходеНаДругойУровеньСервер(НавигационнаяСсылка)
	...
	Если НавигационнаяСсылка = Объект.Ссылка
		...
		Элементы.ЗавершитьПроверку.Заголовок = НСтр("ru='Завершить проверку';uk='Завершити перевірку'");
		Элементы.ЗавершитьПроверку.Картинка = Новый Картинка;
	Иначе
		Элементы.ЗавершитьПроверку.Заголовок = НСтр("ru='Завершить проверку упаковочного листа';uk='Завершити перевірку пакувального листа'");
...
Процедура ЗаполнитьПоОрдеру(Команда)
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ТекстВопроса = НСтр("ru='Список принятых товаров будет перезаполнен по документу.
            ...
            |;uk='Список прийнятих товарів буде перезаповнен по документу.
            |Заповнені дані будуть очищені, продовжити?'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(Истина, НСтр("ru='Продолжить';uk='Продовжити'"));
		Кнопки.Добавить(Ложь, НСтр("ru='Отмена';uk='Відмінити'"));
	...
	Иначе
		...
		Для Каждого ПровереннаяСтрока Из Товары Цикл
			...
			Если Остаток > 0 Тогда
				...
				НоваяСтрока.Комментарий = НСтр("ru='Строка добавлена по результатам проверки';uk='Рядок додан за результатами перевірки'");
