...
Функция ПроверитьФормулу(Формула, Операнды, Знач Поле = "", СтроковаяФормула = Ложь)
	...
	Если ЗначениеЗаполнено(Формула) Тогда
		...
		Исключение
			Результат = Ложь;
			СообщитьПользователю(
				НСтр("ru='В формуле обнаружены ошибки. Проверьте формулу. Формулы должны составляться по правилам написания выражений на встроенном языке платформы.';uk='У формулі виявлені помилки. Перевірте формулу. Формули повинні складатися за правилами написання виразів на вбудованій мові платформи.'"),
				Поле,
				ПутьКДанным);
		КонецПопытки;
