...
Процедура ВыполнитьВыбор()
	...
	Если НЕ ТекущиеДанные.СоставлятьРасписание ИЛИ НЕ ТекущиеДанные.Ключевой Тогда
		ПоказатьПредупреждение(,НСтр("ru='Формировать расписание не требуется';uk='Формувати розклад не потрібно'"));
		Возврат;
	КонецЕсли; 
...
Процедура УправлениеВводомПериода()
	...
	Список = ОбщегоНазначенияУТКлиентСервер.ПолучитьСписокПериодов(ВыбранныйПериодНачало, ВидПериода);
