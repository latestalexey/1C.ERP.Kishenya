...
Функция одТипЗначенияСтрокой(ЗначениеИлиТип) Экспорт
	...
	Иначе
		...
		Если Менеджер = Неопределено Тогда
			...
			Текст= НСтр("ru='Неизвестный тип:';uk='Невідомий тип:'") + Строка(ТипЗнч(ТипЗначения));
...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаПользователю = ?(ТолькоПолучитьУзелСсылки, НСтр("ru='Конвертация ссылки на объект: %1';uk='Конвертація посилання на об''єкт: %1'"), НСтр("ru='Конвертация объекта: %1';uk='Конвертація об''єкта: %1'"));
...
Процедура ПередатьИнформациюОЗаписиВПриемник(ИнформацияДляЗаписиВФайл, СтрокаОшибкиВБазеПриемнике = "") Экспорт
	...
	Если Не ПустаяСтрока(СтрокаОшибкиВБазеПриемнике) Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка в приемнике: %1';uk='Завантаження в приймачі: %1'"), СтрокаОшибкиВБазеПриемнике);
...
Процедура УдалитьВременныеФайлы(ИмяВременногоФайла) Экспорт
	...
	Если Не ПустаяСтрока(ИмяВременногоФайла) Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='Универсальный обмен данными в формате XML';uk='Універсальний обмін даними у форматі XML'",КодОсновногоЯзыка()),
...
Процедура ВыгрузитьОбработчикиСобытий(Отказ) Экспорт
	...
	Если ЭтоИнтерактивныйРежим Тогда
		...
		Если ФлагОшибки Тогда
			...
			СообщитьПользователю(НСтр("ru='При выгрузке обработчиков были обнаружены ошибки.';uk='При вивантаженні обробників були виявлені помилки.'"));
			
		Иначе
			
			СообщитьПользователю(НСтр("ru='Обработчики успешно выгружены.';uk='Обробники успішно вивантажені.'"));
...
Функция ЗаменитьВызовыАлгоритмовКодомЭтихАлгоритмовВОбработчике(Обработчик, АлгоритмВладелец, МассивЗапрошенныхЭлементов, Знач СтрокаПрефикса = "")
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса, НСтр("ru='{НАЧАЛО АЛГОРИТМА}';uk='{ПОЧАТОК АЛГОРИТМУ}'"));
		...
		Если АлгоритмыОбработчика.Количество() <> 0 Тогда // В этой строке есть вызовы алгоритмов.
			...
			Для Каждого Алгоритм Из АлгоритмыОбработчика Цикл
				...
				Если МассивЗапрошенныхЭлементов.Найти(ВРег(Алгоритм)) <> Неопределено Тогда // Рекурсивный вызов алгоритма.
					...
					ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, Алгоритм, СтрокаПрефиксаДляВложенногоКода, НСтр("ru='{РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА}';uk='{РЕКУРСИВНИЙ ВИКЛИК АЛГОРИТМУ}'"));
					
					СтрокаОператора = НСтр("ru='ВызватьИсключение ""РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА: %1"";';uk='ВызватьИсключение ""РЕКУРСИВНИЙ ВИКЛИК АЛГОРИТМУ: %1"";'");
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса, НСтр("ru='{КОНЕЦ АЛГОРИТМА}';uk='{КІНЕЦЬ АЛГОРИТМУ}'"));
...
Процедура КомментарииКЗагрузкеОбъекта(НПП, ИмяПравила, Источник, ТипОбъекта, ГНПП = 0)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		Если НПП <> 0 Тогда
			СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка объекта № %1';uk='Завантаження об''єкта № %1'"), НПП);
		Иначе
			СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка объекта № %1';uk='Завантаження об''єкта № %1'"), ГНПП);
		КонецЕсли;
...
Процедура ВыгрузитьГруппуСвойств(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКГС, УзелКоллекцииСвойств, 
	...
	Если ВыгружатьГруппуЧерезФайл Тогда
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(1000, ОписаниеОшибки(), НСтр("ru='Ошибка при создании временного файла для выгрузки данных';uk='Помилка при створенні тимчасового файлу для вивантаження даних'"));
...
Процедура ВыгрузкаОбъектаВыборки(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, ВыборкаДляВыгрузкиДанных = Неопределено)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Выгрузка объекта: %1';uk='Вивантаження об''єкта: %1'"), ПрОбъекта);
...
Процедура ВыгрузитьДанныеПоПравилу(Правило)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Правило выгрузки данных: %1 (%2)';uk='Правило вивантаження даних: %1 (%2)'"), СокрЛП(Правило.Имя), СокрЛП(Правило.Наименование));
			...
			Если ПоследнийОбъектМетаданных <> ТекущийОбъектМетаданных Тогда
				...
				Если ТекущаяСтрокаПравилаВыгрузки <> Неопределено Тогда
					...
					Если ФлагКомментироватьОбработкуОбъектов Тогда
						...
						СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Правило выгрузки данных: %1 (%2)';uk='Правило вивантаження даних: %1 (%2)'"),
...
Процедура ЗагрузитьЗначенияПараметровОбменаДанными()
	...
	Если СобытияПослеЗагрузкиПараметров.Свойство(Имя, АлгоритмПослеЗагрузкиПараметра)
		...
		Если ФлагРежимОтладкиОбработчиков Тогда
			...
			ВызватьИсключение НСтр("ru='Отладка обработчика ""После загрузки параметра"" не поддерживается.';uk='Налагодження обробника ""Після завантаження параметра"" не підтримуються.'");
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				ВызватьИсключение НСтр("ru='Отладка обработчика ""После загрузки правил обмена"" не поддерживается.';uk='Налагодження обробника ""Після завантаження правил обміну"" не підтримуються.'");
		...
		Исключение
			...
			Текст = НСтр("ru='Обработчик: ""ПослеЗагрузкиПравилОбмена"": %1';uk='Обробник: ""ПослеЗагрузкиПравилОбмена"": %1'");
...
Процедура ПроизвестиЧтениеДанных(РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
				...
				Если Не ПустаяСтрока(ТекстАлгоритма) Тогда
					...
					Попытка
						...
						Если ФлагРежимОтладкиОбработчиков Тогда
							...
							ВызватьИсключение НСтр("ru='Отладка обработчика ""После загрузки параметров"" не поддерживается.';uk='Налагодження обробника ""Після завантаження параметрів"" не підтримуються.'");
						...
						Если Отказ = Истина Тогда
							...
							Если Не ПустаяСтрока(ПричинаОтказа) Тогда
								СтрокаИсключения = ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка данных отменена по причине: %1';uk='Завантаження даних скасовано через: %1'"), ПричинаОтказа);
				...
				Если Не ПустаяСтрока(ТекстАлгоритма) Тогда
					...
					Попытка
						...
						Если ФлагРежимОтладкиОбработчиков Тогда
							...
							ВызватьИсключение НСтр("ru='Отладка глобального алгоритма не поддерживается.';uk='Налагодження глобального алгоритму не підтримується.'");
	...
	Исключение
		...
		СтрокаОшибки = ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при загрузке данных: %1';uk='Помилка при завантаженні даних: %1'"), ОписаниеОшибки());
...
Функция ВыполнитьДействияПередЧтениемДанных(СтрокаДанных = "") Экспорт
	...
	СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Начало загрузки: %1';uk='Початок завантаження: %1'"), ТекущаяДатаСеанса());
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(22, ОписаниеОшибки(), НСтр("ru='ПередЗагрузкойДанных (конвертация)';uk='ПередЗагрузкойДанных (конвертація)'"));
			Отказ = Истина;
		КонецПопытки;
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ОписаниеОшибки(), НСтр("ru='ПослеЗагрузкиДанных (конвертация)';uk='ПослеЗагрузкиДанных (конвертація)'"));
		КонецПопытки;
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru='Окончание загрузки: %1';uk='Закінчення завантаження: %1'"), ТекущаяДатаСеанса()), , Ложь, , , Истина);
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru='Загружено объектов: %1';uk='Завантажене об''єктів: %1'"), мСчетчикЗагруженныхОбъектов), , Ложь, , , Истина);
	
	ЗавершитьВедениеПротоколаОбмена();
	
	Если ЭтоИнтерактивныйРежим Тогда
		СообщитьПользователю(НСтр("ru='Загрузка данных завершена.';uk='Завантаження даних завершено.'"));
	КонецЕсли;
...
Процедура СжатьРезультирующийФайлОбмена()
	...
	Попытка
		...
		Архиватор = Новый ЗаписьZipФайла(ИмяФайлаОбмена, ПарольДляСжатияФайлаОбмена, НСтр("ru='Файл обмена данными';uk='Файл обміну даними'"));
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru='Начало выгрузки: %1';uk='Початок вивантаження: %1'"), ТекущаяДатаСеанса()), , Ложь, , , Истина);
	...
	Исключение
		...
		ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при выгрузке данных: %1';uk='Помилка при вивантаженні даних: %1'"), СтрокаОшибки), Неопределено, Истина, , , Истина);
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru='Окончание выгрузки: %1';uk='Закінчення вивантаження: %1'"), ТекущаяДатаСеанса()), , Ложь, , ,Истина);
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru='Выгружено объектов: %1';uk='Вивантажене об''єктів: %1'"), мСчетчикВыгруженныхОбъектов), , Ложь, , , Истина);
	...
	Если ЭтоИнтерактивныйРежим Тогда
		СообщитьПользователю(НСтр("ru='Выгрузка данных завершена.';uk='Вивантаження даних завершено.'"));
	КонецЕсли;
...
Процедура ИнициализацияСообщений()
	...
	одСообщения.Вставить(2,  НСтр("ru='Ошибка распаковки файла обмена. Файл заблокирован';uk='Помилка розпакування файлу обміну. Файл заблокований'"));
	одСообщения.Вставить(3,  НСтр("ru='Указанный файл правил обмена не существует';uk='Зазначений файл правил обміну не існує'"));
	одСообщения.Вставить(4,  НСтр("ru='Ошибка при создании COM-объекта Msxml2.DOMDocument';uk='Помилка при створенні COM-об''єкта Msxml2.DOMDocument'"));
	одСообщения.Вставить(5,  НСтр("ru='Ошибка открытия файла обмена';uk='Помилка відкриття файлу обміну'"));
	одСообщения.Вставить(6,  НСтр("ru='Ошибка при загрузке правил обмена';uk='Помилка при завантаженні правил обміну'"));
	одСообщения.Вставить(7,  НСтр("ru='Ошибка формата правил обмена';uk='Помилка формату правил обміну'"));
	одСообщения.Вставить(8,  НСтр("ru='Не корректно указано имя файла для выгрузки данных';uk='Не коректно зазначене ім''я файлу для вивантаження даних'"));
	одСообщения.Вставить(9,  НСтр("ru='Ошибка формата файла обмена';uk='Помилка формату файлу обміну'"));
	одСообщения.Вставить(10, НСтр("ru='Не указано имя файла для выгрузки данных (Имя файла данных)';uk='Не зазначене ім''я файлу для вивантаження даних (Ім''я файлу даних)'"));
	одСообщения.Вставить(11, НСтр("ru='Ссылка на несуществующий объект метаданных в правилах обмена';uk='Посилання на неіснуючий об''єкт метаданних у правилах обміну'"));
	одСообщения.Вставить(12, НСтр("ru='Не указано имя файла с правилами обмена (Имя файла правил)';uk='Не вказано ім''я файлу з правилами обміну (Ім''я файлу правил)'"));
	
	одСообщения.Вставить(13, НСтр("ru='Ошибка получения значения свойства объекта (по имени свойства источника)';uk='Помилка одержання значення властивості об''єкта (по імені властивості джерела)'"));
	одСообщения.Вставить(14, НСтр("ru='Ошибка получения значения свойства объекта (по имени свойства приемника)';uk='Помилка одержання значення властивості об''єкта (по імені властивості приймача)'"));
	
	одСообщения.Вставить(15, НСтр("ru='Не указано имя файла для загрузки данных (Имя файла для загрузки)';uk='Не зазначене ім''я файлу для завантаження даних (Ім''я файлу для завантаження)'"));
	
	одСообщения.Вставить(16, НСтр("ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';uk='Помилка одержання значення властивості підпорядкованого об''єкта (по імені властивості джерела)'"));
	одСообщения.Вставить(17, НСтр("ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)';uk='Помилка одержання значення властивості підпорядкованого об''єкта (по імені властивості приймача)'"));
	
	одСообщения.Вставить(19, НСтр("ru='Ошибка в обработчике события ПередЗагрузкойОбъекта';uk='Помилка в обробнику події ПередЗагрузкойОбъекта'"));
	одСообщения.Вставить(20, НСтр("ru='Ошибка в обработчике события ПриЗагрузкеОбъекта';uk='Помилка в обробнику події ПриЗагрузкеОбъекта'"));
	одСообщения.Вставить(21, НСтр("ru='Ошибка в обработчике события ПослеЗагрузкиОбъекта';uk='Помилка в обробнику події ПослеЗагрузкиОбъекта'"));
	одСообщения.Вставить(22, НСтр("ru='Ошибка в обработчике события ПередЗагрузкойДанных (конвертация)';uk='Помилка в обробнику події ПередЗагрузкойДанных (конвертація)'"));
	одСообщения.Вставить(23, НСтр("ru='Ошибка в обработчике события ПослеЗагрузкиДанных (конвертация)';uk='Помилка в обробнику події ПослеЗагрузкиДанных (конвертація)'"));
	одСообщения.Вставить(24, НСтр("ru='Ошибка при удалении объекта';uk='Помилка при вилученні об''єкта'"));
	одСообщения.Вставить(25, НСтр("ru='Ошибка при записи документа';uk='Помилка при записі документа'"));
	одСообщения.Вставить(26, НСтр("ru='Ошибка записи объекта';uk='Помилка запису об''єкта'"));
	одСообщения.Вставить(27, НСтр("ru='Ошибка в обработчике события ПередОбработкойПравилаОчистки';uk='Помилка в обробнику події ПередОбработкойПравилаОчистки'"));
	одСообщения.Вставить(28, НСтр("ru='Ошибка в обработчике события ПослеОбработкиПравилаОчистки';uk='Помилка в обробнику події ПослеОбработкиПравилаОчистки'"));
	одСообщения.Вставить(29, НСтр("ru='Ошибка в обработчике события ПередУдалениемОбъекта';uk='Помилка в обробнику події ПередУдалениемОбъекта'"));
	
	одСообщения.Вставить(31, НСтр("ru='Ошибка в обработчике события ПередОбработкойПравилаВыгрузки';uk='Помилка в обробнику події ПередОбработкойПравилаВыгрузки'"));
	одСообщения.Вставить(32, НСтр("ru='Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки';uk='Помилка в обробнику події ПослеОбработкиПравилаВыгрузки'"));
	одСообщения.Вставить(33, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';uk='Помилка в обробнику події ПередВыгрузкойОбъекта'"));
	одСообщения.Вставить(34, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';uk='Помилка в обробнику події ПослеВыгрузкиОбъекта'"));
	
	одСообщения.Вставить(39, НСтр("ru='Ошибка при выполнении алгоритма, содержащегося в файле обмена';uk='Помилка при виконанні алгоритму, що міститься у файлі обміну'"));
	
	одСообщения.Вставить(41, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';uk='Помилка в обробнику події ПередВыгрузкойОбъекта'"));
	одСообщения.Вставить(42, НСтр("ru='Ошибка в обработчике события ПриВыгрузкеОбъекта';uk='Помилка в обробнику події ПриВыгрузкеОбъекта'"));
	одСообщения.Вставить(43, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';uk='Помилка в обробнику події ПослеВыгрузкиОбъекта'"));
	
	одСообщения.Вставить(45, НСтр("ru='Не найдено правило конвертации объектов';uk='Не знайдене правило конвертації об''єктів'"));
	
	одСообщения.Вставить(48, НСтр("ru='Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств';uk='Помилка в обробнику події ПередОбработкойВыгрузки групи властивостей'"));
	одСообщения.Вставить(49, НСтр("ru='Ошибка в обработчике события ПослеОбработкиВыгрузки группы свойств';uk='Помилка в обробнику події ПослеОбработкиВыгрузки групи властивостей'"));
	одСообщения.Вставить(50, НСтр("ru='Ошибка в обработчике события ПередВыгрузкой (объекта коллекции)';uk='Помилка в обробнику події ПередВыгрузкой (об''єкта колекції)'"));
	одСообщения.Вставить(51, НСтр("ru='Ошибка в обработчике события ПриВыгрузке (объекта коллекции)';uk='Помилка в обробнику події ПриВыгрузке (об''єкта колекції)'"));
	одСообщения.Вставить(52, НСтр("ru='Ошибка в обработчике события ПослеВыгрузки (объекта коллекции)';uk='Помилка в обробнику події ПослеВыгрузки (об''єкта колекції)'"));
	одСообщения.Вставить(53, НСтр("ru='Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)';uk='Помилка в глобальному обробнику події ПередЗагрузкойОбъекта (конвертація)'"));
	одСообщения.Вставить(54, НСтр("ru='Ошибка в глобальном обработчике события ПослеЗагрузкиОбъекта (конвертация)';uk='Помилка в глобальному обробнику події ПослеЗагрузкиОбъекта (конвертація)'"));
	одСообщения.Вставить(55, НСтр("ru='Ошибка в обработчике события ПередВыгрузкой (свойства)';uk='Помилка в обробнику події ПередВыгрузкой (властивості)'"));
	одСообщения.Вставить(56, НСтр("ru='Ошибка в обработчике события ПриВыгрузке (свойства)';uk='Помилка в обробнику події ПриВыгрузке (властивості)'"));
	одСообщения.Вставить(57, НСтр("ru='Ошибка в обработчике события ПослеВыгрузки (свойства)';uk='Помилка в обробнику події ПослеВыгрузки (властивості)'"));
	
	одСообщения.Вставить(62, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)';uk='Помилка в обробнику події ПередВыгрузкойДанных (конвертація)'"));
	одСообщения.Вставить(63, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиДанных (конвертация)';uk='Помилка в обробнику події ПослеВыгрузкиДанных (конвертація)'"));
	одСообщения.Вставить(64, НСтр("ru='Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)';uk='Помилка в глобальному обробнику події ПередКонвертациейОбъекта (конвертація)'"));
	одСообщения.Вставить(65, НСтр("ru='Ошибка в глобальном обработчике события ПередВыгрузкойОбъекта (конвертация)';uk='Помилка в глобальному обробнику події ПередВыгрузкойОбъекта (конвертація)'"));
	одСообщения.Вставить(66, НСтр("ru='Ошибка получения коллекции подчиненных объектов из входящих данных';uk='Помилка одержання колекції підпорядкованих об''єктів із вхідних даних'"));
	одСообщения.Вставить(67, НСтр("ru='Ошибка получения свойства подчиненного объекта из входящих данных';uk='Помилка одержання властивості підпорядкованого об''єкта із вхідних даних'"));
	одСообщения.Вставить(68, НСтр("ru='Ошибка получения свойства объекта из входящих данных';uk='Помилка одержання властивості об''єкта із вхідних даних'"));
	
	одСообщения.Вставить(69, НСтр("ru='Ошибка в глобальном обработчике события ПослеВыгрузкиОбъекта (конвертация)';uk='Помилка в глобальному обробнику події ПослеВыгрузкиОбъекта (конвертація)'"));
	
	одСообщения.Вставить(71, НСтр("ru='Не найдено соответствие для значения Источника';uk='Не знайдена відповідність для значення Джерела'"));
	
	одСообщения.Вставить(72, НСтр("ru='Ошибка при выгрузке данных для узла плана обмена';uk='Помилка при вивантаженні даних для вузла плану обміну'"));
	
	одСообщения.Вставить(73, НСтр("ru='Ошибка в обработчике события ПоследовательностьПолейПоиска';uk='Помилка в обробнику події ПоследовательностьПолейПоиска'"));
	
	одСообщения.Вставить(74, НСтр("ru='Необходимо перезагрузить правила обмена для выгрузки данных';uk='Необхідно перезавантажити правила обміну для вивантаження даних'"));
	
	одСообщения.Вставить(75, НСтр("ru='Ошибка при выполнении алгоритма после загрузки значений параметров';uk='Помилка при виконанні алгоритму після завантаження значень параметрів'"));
	
	одСообщения.Вставить(76, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл';uk='Помилка в обробнику події ПослеВыгрузкиОбъектаВФайл'"));
	
	одСообщения.Вставить(77, НСтр("ru='Не указан файл внешней обработки с подключаемыми процедурами обработчиков событий';uk='Не зазначений файл зовнішньої обробки з процедурами, які підключаються, обробників подій'"));
	
	одСообщения.Вставить(78, НСтр("ru='Ошибка создания внешней обработки из файла с процедурами обработчиков событий';uk='Помилка створення зовнішньої обробки з файлу з процедурами обробників подій'"));
	...
	одСообщения.Вставить(79, НСтр("ru='Код алгоритмов не может быть интегрирован в обработчик из-за обнаруженного рекурсивного вызова алгоритмов. 
                             ...
                             |;uk='Код алгоритмів не може бути інтегрований в обробник через виявлений рекурсивний виклик алгоритмів. 
                             |Якщо в процесі налагодження немає необхідності налагоджувати код алгоритмів, то вкажіть режим ""не налагоджувати алгоритми""
                             |Якщо необхідно виконувати налагодження алгоритмів з рекурсивним викликом, то вкажіть режим  ""алгоритми налагоджувати як процедури"" 
                             |і повторіть вивантаження.'"));
	
	одСообщения.Вставить(80, НСтр("ru='Обмен данными можно проводить только под полными правами';uk='Обмін даними можна проводити тільки під повними правами'"));
	
	одСообщения.Вставить(1000, НСтр("ru='Ошибка при создании временного файла выгрузки данных';uk='Помилка при створенні тимчасового файлу вивантаження даних'"));
...
Функция ОпределитьДостаточностьПараметровДляПодключенияКИнформационнойБазе(СтруктураПодключения, СтрокаПодключения = "", СтрокаСообщенияОбОшибке = "")
	...
	Если СтруктураПодключения.ФайловыйРежим  Тогда
		...
		Если ПустаяСтрока(СтруктураПодключения.КаталогИБ) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Не задан каталог информационной базы-приемника';uk='Не заданий каталог інформаційної бази-приймача'");
	...
	Иначе
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяСервера) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Не задано имя сервера платформы информационной базы-приемника';uk='Не вказано ім''я сервера платформи інформаційної бази-приймача'");
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяИБНаСервере) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Не задано имя информационной базы-приемника на сервере платформы';uk='Не вказано ім''я інформаційної бази-приймача на сервері платформи'");
...
Функция ПодключитсяКИнформационнойБазе(СтруктураПодключения, СтрокаСообщенияОбОшибке = "")
	...
	Исключение
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='При попытке соединения с COM-сервером произошла следующая ошибка:
            |%1'
            |;uk='При спробі з''єднання з COM-сервером виникла наступна помилка:
            |%1'");
