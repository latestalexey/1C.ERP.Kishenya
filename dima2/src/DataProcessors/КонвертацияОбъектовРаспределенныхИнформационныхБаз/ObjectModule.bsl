		...
		Попытка
			...
			Если НайденыНеУникальныеЗаписи("Справочник.ИдентификаторыОбъектовМетаданных") Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					НСтр("ru='Перед загрузкой идентификаторов объектов метаданных
                               ...
                               |;uk='Перед завантаженням ідентифікаторів об''єктів метаданих
                               |у довіднику не знайдені унікальні записи.'"));
			КонецЕсли;
			...
			Если Отказ Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Загрузка приоритетных данных не выполнена.
                               ...
                               |%1'
                               |;uk='Завантаження пріоритетних даних не виконане.
                               |При завантаженні напередвизначених елементів не знайдені унікальні записи.
                               |З наступних причин продовження неможливе.
                               |%1'"),
					ОписаниеОтказа);
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(ДублиПредопределенных) Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Предопределенные элементы.Найдены не уникальные записи.';uk='Напередвизначені елементи.Знайдені не унікальні записи.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='При загрузке предопределенных элементов найдены не уникальные записи.
                                   |%1'
                                   |;uk='При завантаженні напередвизначених елементів знайдені не унікальні записи.
                                   |%1'"),
						ДублиПредопределенных));
			КонецЕсли;
...
Процедура ДобавитьОписаниеДублейПредопределенного(ЗаписанныйОбъект, ДублиПредопределенных, Отказ, ОписаниеОтказа)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОшибкиНеУникальностиЗаписей(),
				НСтр("ru='При загрузке предопределенных элементов найдены не уникальные записи.';uk='При завантаженні напередвизначених елементів знайдені не унікальні записи.'"));
		КонецЕсли;
	...
	Если ЗаписатьВЖурнал Тогда
		Если КоличествоДублей = 1 Тогда
			Шаблон = НСтр("ru='(загружаемая ссылка: %1, ссылка дубля: %2)';uk='(завантажуване посилання: %1, посилання дубля: %2)'");
		Иначе
			Шаблон = НСтр("ru='(загружаемая ссылка: %1, ссылки дублей: %2)';uk='(завантажуване посилання: %1, посилання дублів: %2)'");
		КонецЕсли;
...
Функция ШаблонОшибкиНеУникальностиЗаписей()
	Возврат
		НСтр("ru='Загрузка приоритетных данных не выполнена.
                   ...
                   |   - нажмите Выполнить.
                   |3. После этого запустите платформу и выполните повторную синхронизацию данных.'
                   |;uk='Завантаження пріоритетних даних не виконане.
                   |%1
                   |Потрібно виправлення інформаційної бази.
                   |1. Відкрийте конфігуратор, перейдіть в меню Адміністрування,
                   |виберіть пункт ""Тестування і виправлення ..."".
                   |2. У формі
                   |- увімкніть тільки пункт ""Перевірка логічної цілісності інформаційної бази"";
                   |- виберіть варіант ""Тестування і виправлення, а не Тільки тестування"";
                   |- натисніть кнопку Виконати.
                   |3. Після цього запустіть платформу потім знову виконайте синхронізацію даних.'");
КонецФункции
...
Функция ОшибкаОткрытияФайлаСообщенияОбмена()
	...
	Возврат НСтр("ru='Ошибка открытия файла сообщения обмена';uk='Помилка відкриття файлу повідомлення обміну'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаНачалаЧтенияФайлаСообщенияОбмена()
	
	Возврат НСтр("ru='Ошибка при начале чтения файла сообщения обмена';uk='Помилка при початку читання файлу повідомлення обміну'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаНачалаЗаписиФайлаСообщенияОбмена()
	
	Возврат НСтр("ru='Ошибка при начале записи файла сообщения обмена';uk='Помилка при качану запису файлу повідомлення обміну'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаЧтенияФайлаСообщенияОбмена()
	
	Возврат НСтр("ru='Ошибка чтения файла сообщения обмена';uk='Помилка читання файлу повідомлення обміну'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаЗаписиФайлаСообщенияОбмена()
	
	Возврат НСтр("ru='Ошибка записи данных в файл сообщения обмена';uk='Помилка запису даних у файл повідомлення обміну'");
	
КонецФункции

Функция ОшибкаВидаОбменаДанными()
	
	Возврат НСтр("ru='Обмен не по правилам конвертации не поддерживается';uk='Обмін не за правилами конвертації не підтримується'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
