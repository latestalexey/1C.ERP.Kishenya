...
//
// Параметры:
//  ПараметрыРасчета - Структура - см. функцию ПараметрыРасчетаПооперационногоРасписания.
...
Процедура РассчитатьРасписание(ПараметрыРасчета, АдресХранилища) Экспорт
	...
	ПослеЗавершенияРасчета(ПараметрыПланирования);
	ПоместитьВоВременноеХранилище(ПараметрыПланирования.РезультатыРасчета, АдресХранилища);
...
//
// Параметры:
//  ПараметрыПланирования - Структура - см. функцию ИнициализироватьПараметрыПланирования.
...
Процедура РассчитатьРасписаниеМоделейПланирования(ПараметрыПланирования)
	...
	Если РасчетВФоновомРежиме Тогда
		...
		НаименованиеЗадания = НСтр("ru='Расчет расписания по модели планирования';uk='Розрахунок розкладу по моделі планування'");
...
Процедура ПрочитатьОперацииДляПланирования(ПараметрыПланирования, Отказ)
	...
	Исключение
		...
		ОтменитьТранзакцию();
 		ДобавитьЗаписьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Отказ = Истина;
 		ВызватьИсключение;
...
Функция ИнтервалПригоденДляРазмещенияОперации(ПараметрыПланирования, ДоступныйИнтервал, ПараметрыПодбора)
	...
	Если ИнтервалЗагруженСинхронно И НЕ ДоступныйИнтервал.Время = ПараметрыПодбора.ВремяПлан Тогда
...
Процедура ПрочитатьДоступностьВспомогательногоРЦ(Знач ПараметрыПланирования, Знач Операция, Результат,
	...
	Если ПрямоеРазмещение Тогда
		ПодобратьДоступноеВремяРЦНаИнтервале(ПараметрыПланирования, РабочийЦентр,
			ИнтервалРазмещения.Начало, ДоступныйИнтервал.Окончание, Интервалы, ДоступныеИнтервалы);
	Иначе
		ПодобратьДоступноеВремяРЦНаИнтервале(ПараметрыПланирования, РабочийЦентр,
			ДоступныйИнтервал.Начало, ИнтервалРазмещения.Окончание, Интервалы, ДоступныеИнтервалы);
	КонецЕсли;
	
	Свободен = ПериодПринадлежитИнтервалам(ПараметрыПланирования, Интервалы,,
		ИнтервалРазмещения.Начало, ИнтервалРазмещения.Окончание);
	...
	Для каждого СтрокаДоступностьРЦ Из ТаблицаДоступность Цикл
		...
		Иначе
			Свободен = ПериодПринадлежитИнтервалам(
				ПараметрыПланирования,, СтрокаДоступностьРЦ.ДоступныеИнтервалы, Начало, Окончание);
			Если Свободен Тогда
				Продолжить;
			Иначе
				Отказ = Ложь;
...
Процедура ПроверитьОбсуживающиеРЦСвободныНаИнтервале(Знач ПараметрыПланирования, ВспомогательныеРЦ, Знач ИнтервалРазмещения)
	...
	Для каждого Строка Из ВспомогательныеРЦ Цикл
		...
		Иначе
			...
			Строка.Свободен = ПериодПринадлежитИнтервалам(ПараметрыПланирования, Интервалы,, Начало, Окончание);
			
			ДоступныеИнтервалы = ИнициализироватьТаблицуДоступныеИнтервалыРЦ();
			ПодобратьДоступноеВремяРЦНаИнтервале(ПараметрыПланирования, Строка.РабочийЦентр,
				ИнтервалРазмещения.Начало, ИнтервалРазмещения.Окончание, Интервалы, ДоступныеИнтервалы);
...
Функция ПериодПринадлежитИнтервалам(Знач ПараметрыПланирования, Знач ДоступноеВремяИнтервалы = Неопределено,
		Знач ДоступныеИнтервалыРЦ = Неопределено, Знач Начало, Знач Окончание)
...
Процедура ПодобратьДоступноеВремяРЦНаИнтервале(Знач ПараметрыПланирования, Знач РабочийЦентр,
		Знач ИнтервалНачало, Знач ИнтервалОкончание, Знач Интервалы = Неопределено, Результат)
...
Функция НовыйОбъектРасписаниеВарианта()
	...
	Результат.Колонки.Добавить("Начало", Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты));
	Результат.Колонки.Добавить("Окончание", Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты));
	Результат.Колонки.Добавить("ОценкаВремя", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	...
	Если НЕ ПроблемаЗафиксирована Тогда
		...
		Если ТипЗнч(РабочийЦентр) = Тип("СправочникСсылка.РабочиеЦентры") Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Недостаточно доступного времени рабочего центра ""%1"" вида ""%2"".';uk='Недостатньо доступного часу робочого центру ""%1"" виду ""%2"".'"),
		...
		Иначе
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Недостаточно доступного времени вида рабочего центра ""%1"".';uk='Недостатньо доступного часу виду робочого центру ""%1"".'"),
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РабочийЦентр, "Представление"));
		КонецЕсли;
		НоваяСтрока.Текст = Текст;
		
		НоваяСтрока.ЧтоДелатьДальше = НСтр("ru='Заполните доступность видов рабочих центров или увеличьте горизонт планирования.';uk='Заповніть доступність видів робочих центрів або збільште горизонт планування.'");
...
Процедура ЗафиксироватьОшибкуРасписаниеЗаблокировано(ПараметрыПланирования)
	...
	Если НЕ ПроблемаЗафиксирована Тогда
		...
		Текст = НСтр("ru='Планирование не может быть выполнено по причине: расписание находится в состоянии расчета';uk='Планування не може бути виконано через: розклад знаходиться в стані розрахунку'");
		...
		Если ЗначениеЗаполнено(Пользователь) Тогда
			...
			Текст = Текст + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='(пользователь: %1 )';uk='(користувач: %1 )'"), ПользовательПредставление);
		КонецЕсли;
		НоваяСтрока.Текст = Текст;
		
		НоваяСтрока.ЧтоДелатьДальше = НСтр("ru='Дождитесь окончания расчета расписания.';uk='Дочекайтеся закінчення розрахунку розкладу.'");
...
Процедура ЗафиксироватьОшибкуНеЗаданоВремяОперации(ПараметрыПланирования, Знач Операция)
	...
	Если НЕ ПроблемаЗафиксирована Тогда
		...
		НоваяСтрока.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='У операции %1 документа %2 не заполнено время выполнения.';uk='У операції %1 документа %2 не заповнено час виконання.'"),
			Операция.НомерОперации,
			Операция.МаршрутныйЛист);
		НоваяСтрока.ЧтоДелатьДальше = НСтр("ru='Заполните время выполнения операции.';uk='Заповніть час виконання операції.'");
...
Процедура ЗафиксироватьОшибкуОтсутствуютДанныеДляПланирования(ПараметрыПланирования)
	...
	Если НЕ ПроблемаЗафиксирована Тогда
		...
		НоваяСтрока.Категория = Категория;
		НоваяСтрока.Тип = 1;
		НоваяСтрока.Текст = НСтр("ru='Планирование не может быть выполнено по причине: отсутствуют операции для планирования.';uk='Планування не може бути виконано через: відсутні операції для планування.'");
		НоваяСтрока.ЧтоДелатьДальше = НСтр("ru='Создайте маршрутные листы либо измените параметры планирования.';uk='Створіть маршрутні листи або змініть параметри планування.'");
		НоваяСтрока.Свойства = Новый Структура("Подразделение", ПараметрыПланирования.Подразделение);
		НоваяСтрока.Ключ = Ключ;
...
Процедура ЗафиксироватьОшибкуНеОпределеныРабочиеЦентры(ПараметрыПланирования, ВидРабочегоЦентра)
	...
	Если НЕ ПроблемаЗафиксирована Тогда
		...
		СтрокаПодстановки = НСтр("ru='Не созданы рабочие центры вида ""%1""';uk='Не створені робочі центри виду ""%1""'");
		...
		НоваяСтрока.ЧтоДелатьДальше = НСтр("ru='Создайте рабочие центры.';uk='Створіть робочі центри.'");
...
Процедура ПослеЗавершенияРасчета(Знач ПараметрыПланирования)
	
	ЗаполнитьРезультатыРасчета(ПараметрыПланирования);
	
	ЗаписатьРезультатыРасчетаВЖурналРегистрации(ПараметрыПланирования);
...
Процедура ЗаполнитьРезультатыРасчета(ПараметрыПланирования)
	...
	РезультатыРасчета.Вставить("РазмещеноОпераций", РазмещеноОпераций);
	РезультатыРасчета.Вставить("РасчетВыполнен", НЕ(РазмещеноОпераций = 0));
	
	СценарииИспользуются = СценарииИспользуются(ПараметрыПланирования);
	РезультатыРасчета.Вставить("СценарииИспользуются", СценарииИспользуются);
	...
	Если НЕ СценарииИспользуются
		...
		РезультатыРасчета.Вставить("ОптимальнаяМодельПланирования", ОптимальнаяМодель);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаписатьРезультатыРасчетаВЖурналРегистрации(Знач ПараметрыПланирования)
	
	ЗаписатьВЖурналРегистрацииВремяПланированияИКоличествоОпераций(ПараметрыПланирования);
	
КонецПроцедуры

Процедура ЗаписатьВЖурналРегистрацииВремяПланированияИКоличествоОпераций(Знач ПараметрыПланирования)
	
	Уровень = УровеньЖурналаРегистрации.Информация;
	Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Расчет расписания. Размещено операций: %1. Время: %2';uk='Розрахунок розкладу. Розміщено операцій: %1. Час: %2'"),
		ПараметрыПланирования.РезультатыРасчета.РазмещеноОпераций,
		ПредставлениеИнтервалаВремени(ПараметрыПланирования.НачалоПланирования, ТекущаяДатаСеанса()));
		
	ДобавитьЗаписьВЖурналРегистрации(Уровень,,, Комментарий);
	
КонецПроцедуры

Процедура ДобавитьЗаписьВЖурналРегистрации(
	Знач Уровень, Знач ОбъектМетаданных=Неопределено, Знач Данные=Неопределено, Знач Комментарий)
	
	ИмяСобытия = НСтр("ru='Пооперационное планирование';uk='Поопераційне планування'");
...
Функция ПредставлениеИнтервалаВремени(ИнтервалНачало, ИнтервалОкончание)
	
	Время = ИнтервалОкончание - ИнтервалНачало;
	Если Время = 0 Тогда
		Время = 1;
	КонецЕсли;
	
	Часы = Цел(Время/3600);
	Время = Время - (Часы*3600);
	Минуты = Цел(Время/60);
	Секунды = Время - (Минуты*60);
	
	Если ЗначениеЗаполнено(Часы) Тогда
		Если ЗначениеЗаполнено(Минуты) Тогда
			Представление = НСтр("ru='%1 ч. %2 мин.';uk='%1 ч. %2 хв.'");
		Иначе
			Представление = НСтр("ru='%1 ч.';uk='%1 год.'");
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(Минуты) Тогда
			Если ЗначениеЗаполнено(Секунды) Тогда
				Представление = НСтр("ru='%2 мин. %3 сек.';uk='%2 хв. %3 сек.'");
			Иначе
				Представление = НСтр("ru='%2 мин.';uk='%2 хв.'");
			КонецЕсли;
		Иначе
			Если НЕ ЗначениеЗаполнено(Секунды) Тогда
				Секунды = 1;
			КонецЕсли;
			Представление = НСтр("ru='%3 сек.';uk='%3 сек.'");
		КонецЕсли;
	КонецЕсли;
