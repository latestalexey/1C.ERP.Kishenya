	...
	Если ЗначениеЗаполнено(МоделиПланирования) Тогда
		...
		Для каждого Строка Из МоделиПланирования Цикл
			...
			Если Строка.ЗадействоватьРезервДоступности Тогда
				ПредставлениеРезерва = НСтр("ru='с резервом';uk='з резервом'");
			Иначе
				ПредставлениеРезерва = НСтр("ru='без резерва';uk='без резерву'");
			КонецЕсли;
...
Процедура ПроверитьУстановленныеОтборы(Отказ)
	...
	Если ОтборРаспоряженияУстановлен И СценарииИспользуются Тогда
		...
		Поле = "Распоряжения";
		ТекстСообщения = НСтр("ru='При расчете с использованием сценариев планирования отбор не может быть установлен.';uk='При розрахунку з використанням сценаріїв планування відбір не може бути встановлений.'");
		ТекстОшибки = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Корректность", Поле,,, ТекстСообщения);
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, Поле, ТекстОшибки,,, ТекстОшибки);
