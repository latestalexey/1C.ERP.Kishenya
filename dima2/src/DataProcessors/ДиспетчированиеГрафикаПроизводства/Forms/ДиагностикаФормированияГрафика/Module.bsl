...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Этапы.Количество() > 1 Тогда
		Элементы.СтраницыДианостика.ТекущаяСтраница = Элементы.СтраницаДиагностикаСВыборомЭтапов;
	Иначе
		Элементы.СтраницыДианостика.ТекущаяСтраница = Элементы.СтраницаДиагностикаБезВыбораЭтапов;
	КонецЕсли;
	...
	НомерДокумента = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Заказ.Номер);
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Диагностика графика производства продукции ""%1"" по заказу %2';uk='Діагностика графіка виробництва продукції ""%1"" по замовленню %2'"),
						Номенклатура,
						ПредставлениеЗаказа);
	...
	ИначеЕсли ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		Если Расшифровка.Свойство("ВидОбласти") Тогда
			...
			Если Расшифровка.ВидОбласти = "ДанныеОЗагрузке" Тогда
				...
				СписокВыбора.Добавить("ИзменениеЗанятостиВидаРабочегоЦентра", НСтр("ru='Занятость вида рабочего центра';uk='Зайнятість виду робочого центру'"));
				Если ЕстьПравоДобавленияДоступностиРЦ Тогда
					СписокВыбора.Добавить("ИзменитьДоступность", НСтр("ru='Изменить доступность вида рабочих центров';uk='Змінити доступність виду робочих центрів'"));
				КонецЕсли; 
...
Процедура ВывестиДанныеДиагностикиГрафика()
	...
	Если Этап.ПланироватьРаботуВидовРабочихЦентров Тогда
		ЗаголовокИсполнители = НСтр("ru='Доступность видов рабочих центров';uk='Доступність видів робочих центрів'");
	Иначе
		ЗаголовокИсполнители = НСтр("ru='Доступность подразделения';uk='Доступність підрозділу'");
	КонецЕсли; 
...
Процедура ВывестиИсполнителей(ДанныеОбИсполнителях, ГраницыЭтапа, ОбластиДляВыводаДоступности, НачалоИнтервала, ОкончаниеИнтервала, ЭтоАльтернативныеИсполнители, ВысотаТаблицыОсновногоВидаРЦ = Неопределено)
	...
	Для каждого ИсполнительЭтапа из Исполнители Цикл
		...
		Если ВыводитьИспользованиеИсполнителя Тогда
			Если НЕ ЭтоАльтернативныеИсполнители Тогда
				ОбластьИсполнитель_Использование.Области.Исполнитель_Использование.Текст = НСтр("ru='показать альтернативы';uk='показати альтернативи'");
			...
			Иначе
				ОбластьИсполнитель_Использование.Области.ИсполнительАльтернативный_Использование.Текст = НСтр("ru='использовать';uk='використовувати'");
...
Процедура ПриИзмененииТекущегоЭтапа()
	...
	Если Элементы.СтраницыДианостика.ТекущаяСтраница = Элементы.СтраницаДиагностикаБезВыбораЭтапов Тогда
		ВывестиШапкуДиагностикиГрафика();
	Иначе
		ПоказатьВыбранныйЭтап();
	КонецЕсли; 
...
Процедура ПоказатьСкрытьАльтернативы(Расшифровка, Форма)
	...
	Если ОбластьАльтернативы.Видимость Тогда
		ОбластьИсполнитель_Использование.Текст = НСтр("ru='скрыть альтернативы';uk='приховати альтернативи'");
	Иначе
		ОбластьИсполнитель_Использование.Текст = НСтр("ru='показать альтернативы';uk='показати альтернативи'");
	КонецЕсли; 
...
Процедура ИспользоватьВидРЦ(Расшифровка)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Изменить используемый вид РЦ на ""%1""?';uk='Змінити використовуваний вид РЦ ""%1""?'"),
						Строка(Расшифровка.ВидРабочегоЦентра));
						
	СписокКнопок = Новый СписокЗначений;
	СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Изменить вид РЦ';uk='Змінити вид РЦ'"));
...
Процедура ИспользоватьВидРЦЗавершение(РезультатВопроса, Расшифровка) Экспорт
	...
	Для Сч = 0 По Расшифровка.КоличествоАльтернатив-1 Цикл
		...
		Если ОбластьИсполнитель_Использование.Расшифровка.ВидРабочегоЦентра = Расшифровка.ВидРабочегоЦентра Тогда
			ОбластьИсполнитель_Использование.Текст = НСтр("ru='будет использоваться';uk='буде використовуватися'");
		...
		Иначе
			ОбластьИсполнитель_Использование.Текст = НСтр("ru='использовать';uk='використовувати'");
