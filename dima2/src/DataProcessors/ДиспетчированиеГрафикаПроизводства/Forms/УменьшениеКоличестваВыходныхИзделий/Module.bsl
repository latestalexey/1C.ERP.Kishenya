...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если МинимальноеКоличествоВыходныхИзделий = 0 Тогда
		Элементы.ИнформационнаяНадпись.Заголовок = НСтр("ru='Расчет на меньшее количество изделий невозможен.';uk='Розрахунок на меншу кількість виробів неможливий.'");
		ЭтаФорма.ТолькоПросмотр = Истина;
	ИначеЕсли МинимальноеКоличествоВыходныхИзделий = ИсходноеКоличествоПоследнихЭтапов Тогда
		Элементы.ИнформационнаяНадпись.Заголовок = НСтр("ru='Расчет на меньшее количество изделий не требуется.';uk='Розрахунок на меншу кількість виробів не потрібний.'");
	Иначе
		ШаблонНадписи = НСтр("ru='С учетом брака на всех этапах возможно продолжение производства %1 из %2 %3 изделий.';uk='З урахуванням браку на всіх етапах можливе продовження виробництва %1 з %2 %3 виробів.'");
...
Процедура ЗавершитьРедактирование(Команда)
	...
	Режим = "Запись";
	ЗавершитьРедактированиеНаСервере(Режим);
...
Процедура ПолучитьДанныеОХодеВыполненияЭтапов()
	...
	ШаблонПредставленияБрака = НСтр("ru='МЛ №%1 испорчено %2 единиц/партий изделий (%3)';uk='МЛ №%1 зіпсовано %2 одиниць/партій виробів (%3)'");
...
&НаСервере
Процедура ЗавершитьРедактированиеНаСервере(Режим, ПараметрыФормы = Неопределено)
	...
	Для Каждого СтрокаЭтап Из ТаблицаЭтапов Цикл
		...
		Если ЭтапСпецификации.Количество = 0 Тогда
			ПланированиеПроизводства.УдалитьЭтапПроизводства(ЗаказОбъект, ЭтапСпецификации.КлючСвязи);
			Продолжить;
		КонецЕсли;
