...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Занятость вида рабочего центра ""%1""';uk='Зайнятість виду робочого центру ""%1""'"),
					ВидРабочегоЦентра);
...
Процедура ЗаполнитьДанные()
	...
	Пока ВыборкаЗанятость.Следующий() Цикл
		...
		НомерДокумента = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаЗанятость.Номер);
		...
		Для каждого ДанныеАльтернативныйВидРЦ Из АльтернативныеВРЦ Цикл
			...
			СтрокаАльтернативныйВРЦ.ТребуетсяВремени = ВыборкаЗанятость.КоличествоЭтапа
														/ ВыборкаЗанятость.ОдновременноПроизводимоеКоличествоЕдиницПартийИзделий
...
Процедура ИспользоватьДругойВидРЦ()
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='В выбранных этапах будет использоваться вид рабочего центра ""%1"".
                                    |Продолжить?'
                                    |;uk='В вибраних етапах буде використовуватися вид робочого центру ""%1"".
                                    |Продовжити?'"),
...
Процедура ИспользоватьДругойВидРЦЗавершение(РезультатВопроса, НовыйВидРабочегоЦентра) Экспорт
	...
	Если ВыполненоЗамен = 0 Тогда
		...
		ПоказатьПредупреждение(,НСтр("ru='Не удалось использовать новый вид рабочего центра.';uk='Не вдалося використовувати новий вид робочого центру.'"));
		Возврат;
		
	ИначеЕсли ВыполненоЗамен = ВсегоЗамен Тогда
		
		// Успешно заменили во всех заказах
		ТекстЗавершеннойОперации = НСтр("ru='Для всех выбранных этапов установлено использование нового вида рабочего центра';uk='Для всіх виділених етапів встановлено використання нового виду робочого центру'");
		ТекстПоясненияЗавершеннойОперации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
														НСтр("ru='Изменено этапов: %1 из %2';uk='Змінено етапів: %1 з %2'"),
	...
	Иначе
		...
		ТекстЗавершеннойОперации = НСтр("ru='Использование нового вида рабочего центра выполнено не для всех выбранных этапов';uk='Використання нового виду робочого центру виконано не для всіх вибраних етапів'");
		ТекстПоясненияЗавершеннойОперации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
														НСтр("ru='Изменено этапов: %1 из %2';uk='Змінено етапів: %1 з %2'"),
														Формат(ВыполненоЗамен, "ЧГ=0"),
														Формат(ВсегоЗамен, "ЧГ=0"));
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru='Использование нового вида рабочего центра выполнено не для всех выбранных этапов (в %1 из %2).';uk='Використання нового виду робочого центру виконано не для всіх вибраних етапів (у %1 з %2).'"), 
										Формат(ВыполненоЗамен, "ЧГ=0"), 
										Формат(ВсегоЗамен, "ЧГ=0"));
