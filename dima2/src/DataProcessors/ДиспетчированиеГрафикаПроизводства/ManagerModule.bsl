...
Функция ОбновитьСостояниеГрафикаПроизводства(Параметры, АдресХранилища) Экспорт
	
	ЗаказыТребуетсяПланировать = Документы.ЗаказНаПроизводство.ТребуетсяПланировать(Параметры.Заказы);
	
	Результат = Новый Структура;
	Результат.Вставить("ЗаказыТребуетсяПланировать", ЗаказыТребуетсяПланировать);
	
	ПоместитьВоВременноеХранилище(Результат, АдресХранилища);
	
КонецФункции

Функция ОбновитьИндикаторы(Параметры, АдресХранилища) Экспорт
	
	ЕстьЗаказыСНедействующимиСпецификациями = Ложь;
	...
	Если ПравоДоступа("Редактирование", Метаданные.Документы.ЗаказНаПроизводство) Тогда
		ЕстьЗаказыСНедействующимиСпецификациями = Документы.ЗаказНаПроизводство.ЕстьЗаказыСНедействующимиСпецификациями(Параметры.Подразделение, Параметры.Ответственный);
	КонецЕсли;
	
	Результат = Новый Структура;
	Результат.Вставить("ИндикаторПрименяютсяНедействующиеСпецификации", ЕстьЗаказыСНедействующимиСпецификациями);
...
Функция РезультатПланированияЗаказа(ЗаказОбъект, КлючСвязиПродукция = Неопределено, Ошибки = Неопределено, Параметры = Неопределено) Экспорт
	...
	|ПОМЕСТИТЬ ВТПолуфабрикатыПроизводимыеПоСпецификации
	|ИЗ
	|	Документ.ЗаказНаПроизводство.МатериалыИУслуги КАК ТМатериалыИУслуги
	|ГДЕ
	|	ТМатериалыИУслуги.Ссылка = &Распоряжение
	|	И ТМатериалыИУслуги.ПроизводитсяВПроцессе
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТЗаказМатериалов.КодСтрокиРаспоряжения КАК КодСтрокиРаспоряжения,
	|	ТЗаказМатериалов.Номенклатура          КАК Номенклатура,
	|	ТЗаказМатериалов.Характеристика        КАК Характеристика,
	|	ТЗаказМатериалов.Склад                 КАК Склад,
	...
	|				И ВариантОбеспечения В (ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ИзЗаказов),
	...
	|						Т.КодСтроки
	|					ИЗ
	|						ВТПолуфабрикатыПроизводимыеПоСпецификации КАК Т)) КАК ТЗаказМатериалов
	|ГДЕ
	|	ТЗаказМатериалов.КоличествоОборот > 0
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|	ТМатериалыИУслуги.ПроизводитсяВПроцессе КАК ПроизводитсяВПроцессе,
	|	ТЗаказМатериалов.Номенклатура           КАК Номенклатура,
	|	ТЗаказМатериалов.Характеристика         КАК Характеристика,
	|	ТЗаказМатериалов.Склад                  КАК Склад,
	|	ТЗаказМатериалов.ДатаПотребности        КАК ДатаПотребности,
	...
	КэшированныеЗначения.Вставить("РеквизитыПодразделений", ПроизводствоСервер.ПараметрыПроизводственныхПодразделений(МассивПодразделений));
...
Функция СоздатьТаблицуЭтапы()
	...
	ТаблицаЭтапы.Колонки.Добавить("НомерЭтапа",                      Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2, 0, ДопустимыйЗнак.Неотрицательный)));
	ТаблицаЭтапы.Колонки.Добавить("НомерСледующегоЭтапа",            Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2, 0, ДопустимыйЗнак.Неотрицательный)));
...
Процедура ЗаполнитьЗагрузку(Загрузка, ПараметрыИсполнителя, НачалоПериода, ОкончаниеПериода, ТекущийЭтап = Неопределено) Экспорт
	...
	|		ПО (ТВидыРЦ.Календарь = ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка))
	|			И (ТВидыРЦ.Подразделение.ГрафикРаботы = ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка))
	...
	|				КОГДА ТВидыРЦ.Подразделение.ГрафикРаботы <> ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
	|					ТОГДА КалендарныеГрафики.Календарь = ТВидыРЦ.Подразделение.ГрафикРаботы
	|				ИНАЧЕ КалендарныеГрафики.Календарь = Константы.ОсновнойКалендарьПредприятия
	|			КОНЕЦ)
	...
	|					ТОГДА &ГрафикРаботыПартнера = ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
	|				ИНАЧЕ ТПодразделение.ГрафикРаботы = ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
	|			КОНЕЦ)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.КалендарныеГрафики КАК КалендарныеГрафики
	|		ПО	(ВЫБОР
	|				КОГДА &ПроизводствоНаСтороне
	...
	|				КОГДА ТПодразделение.ГрафикРаботы <> ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
	|					ТОГДА КалендарныеГрафики.Календарь = ТПодразделение.ГрафикРаботы
	|				ИНАЧЕ КалендарныеГрафики.Календарь = Константы.ОсновнойКалендарьПредприятия
	|			КОНЕЦ)
	|			И (КалендарныеГрафики.ДатаГрафика МЕЖДУ &НачалоПериода И &ОкончаниеПериода)
	|			И (КалендарныеГрафики.ДеньВключенВГрафик)
...
Функция ТаблицаСостоянийЭтапов() Экспорт
	...
	НоваяСтрока.ТекстСостояния = НСтр("ru='Требуется отработать брак';uk='Потрібно відпрацювати брак'");
	...
	НоваяСтрока.ТекстСостояния = НСтр("ru='Не создан маршрутный лист, требуется перепланировать';uk='Не створено маршрутний лист, потрібно перепланувати'");
	НоваяСтрока.Проблемный     = Истина;
	НоваяСтрока.Активный       = Истина;
	НоваяСтрока.Выполненный    = Ложь;
	...
	НоваяСтрока.ТекстСостояния = НСтр("ru='Не создан маршрутный лист';uk='Не створено маршрутний лист'");
	НоваяСтрока.Проблемный     = Истина;
	НоваяСтрока.Активный       = Истина;
	НоваяСтрока.Выполненный    = Ложь;
	...
	НоваяСтрока.ТекстСостояния = НСтр("ru='Ожидается создание маршрутного листа';uk='Очікується створення маршрутного листа'");
	НоваяСтрока.Проблемный     = Ложь;
	НоваяСтрока.Активный       = Истина;
	НоваяСтрока.Выполненный    = Ложь;
	...
	НоваяСтрока.ТекстСостояния = НСтр("ru='Маршрутный лист создан, не выполняется';uk='Маршрутний лист створений, не виконується'");
	НоваяСтрока.Проблемный     = Ложь;
	НоваяСтрока.Активный       = Истина;
	НоваяСтрока.Выполненный    = Ложь;
	...
	НоваяСтрока.ТекстСостояния = НСтр("ru='Ожидается результат выполнения';uk='Очікується результат виконання'");
	НоваяСтрока.Проблемный     = Истина;
	НоваяСтрока.Активный       = Истина;
	НоваяСтрока.Выполненный    = Ложь;
	...
	НоваяСтрока.ТекстСостояния = НСтр("ru='Не создан заказ';uk='Не створено замовлення'");
	НоваяСтрока.Проблемный     = Ложь;
	НоваяСтрока.Активный       = Истина;
	НоваяСтрока.Выполненный    = Ложь;
	...
	НоваяСтрока.ТекстСостояния = НСтр("ru='Заказ создан, не выполняется';uk='Замовлення створене, не виконується'");
	НоваяСтрока.Проблемный     = Ложь;
	НоваяСтрока.Активный       = Истина;
	НоваяСтрока.Выполненный    = Ложь;
