...
Процедура ПриОткрытии(Отказ)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент() Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ru='Извлечение текстов не поддерживается в клиенте под управлением ОС Linux.';uk='Витягування текстів не підтримується у клієнті під управлінням ОС Linux.'");
...
Процедура ИзвлечениеТекстовКлиент(РазмерПорции = Неопределено)
	...
	Состояние(НСтр("ru='Начато извлечение текста';uk='Почато витягування тексту'"));
	...
	Попытка
		...
		Если МассивФайлов.Количество() = 0 Тогда
			Состояние(НСтр("ru='Нет файлов для извлечения текста';uk='Немає файлів для витягування тексту'"));
			Возврат;
		КонецЕсли;
