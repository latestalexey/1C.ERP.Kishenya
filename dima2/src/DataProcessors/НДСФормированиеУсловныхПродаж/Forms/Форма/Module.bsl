...
Процедура ЗаполнитьТаблицуУсловныхПродаж()
	...
	Запрос.Текст = 
	НДСИсходящийСервер.ПолучитьТекстЗапросаАнализаУсловныхПродаж() + Символы.ПС +
	// Показываем только строки по которым действительно необходимо сформировтаь/доформировать документы
	"ГДЕ
	|	ВЫБОР
	...
	|	УсловнаяПродажа.ПредполагаемыйВидУсловнойПродажи,
	...
	Запрос.УстановитьПараметр("ПериодНачало",       НачалоМесяца(ПериодРасчетов));
	Запрос.УстановитьПараметр("ПериодОкончание",    КонецМесяца(ПериодРасчетов));
