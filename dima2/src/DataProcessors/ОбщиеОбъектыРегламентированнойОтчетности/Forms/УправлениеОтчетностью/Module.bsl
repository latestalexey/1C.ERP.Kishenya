	...
	Иначе
		...
		Иначе
			Если ДатаНачалаПериодаОтчета <= ДатаКонцаПериодаОтчета Тогда
				Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ru = 'Отчеты за период %1';uk= 'Звіти за період %1'", ПредставлениеПериода(ДатаНачалаПериодаОтчета, КонецДня(ДатаКонцаПериодаОтчета), "ФП = Истина"));
			Иначе
				Подсказка = "";
			КонецЕсли;
		КонецЕсли;
