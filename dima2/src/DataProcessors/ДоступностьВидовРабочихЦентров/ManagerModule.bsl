...
Функция ЗаполнитьГрафикиРаботыРЦ(ВидРабочегоЦентра, ДанныеПоРЦ, НачалоПериода, ОкончаниеПериода, РасписаниеГрафиковРаботыРЦ) Экспорт
	...
	Для каждого СтрокаРЦ Из ДанныеПоРЦ Цикл
		Если СписокГрафиков.Найти(СтрокаРЦ.ГрафикРаботы) = Неопределено Тогда
			СписокГрафиков.Добавить(СтрокаРЦ.ГрафикРаботы);
		КонецЕсли; 
	КонецЦикла; 
	...
	Для каждого СтрокаРЦ Из ДанныеПоРЦ Цикл
		...
		Пока ДатаПериода <= ОкончаниеПериода Цикл
			...
			СтруктураПоиска = Новый Структура("ГрафикРаботы,ДатаГрафика", СтрокаРЦ.ГрафикРаботы, ДатаПериода);
	  		РасписаниеПоГрафику = РасписанияРаботы.НайтиСтроки(СтруктураПоиска);
			Если РасписаниеПоГрафику.Количество() <> 0 Тогда
...
Функция ТребуетсяВвестиДоступностьДляГрафикаПроизводства() Экспорт
...
Процедура ДелоВводДоступностиДляГрафикаПроизводства(ТекущиеДела, ОбщиеПараметрыЗапросов)
	...
	ДелоРодитель.Представление  = НСтр("ru='Виды рабочих центров, требующие ввода доступности';uk='Види робочих центрів, що потребують введення доступності'");
...
Процедура ДелоВводДоступностиДляРасписанияРЦ(ТекущиеДела, ОбщиеПараметрыЗапросов)
	...
	ДелоРодитель.Представление  = НСтр("ru='Рабочие центры, требующие ввода графика работы';uk='Робочі центри, що потребують введення графіка роботи'");
	ДелоРодитель.Владелец       = Метаданные.Подсистемы.Производство;
