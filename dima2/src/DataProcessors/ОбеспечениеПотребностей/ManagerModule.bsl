...
Функция ТаблицаПотребностейПоЗаказамИОстатков(Отбор, ТипОтбора,
	...
	Иначе
		...
		Запрос.Текст = Запрос.Текст +
			...
			|			ПО &ПодстановкаТоварногоОграничения
			|			 И Настройка.МетодОбеспеченияПотребностей <> ЗНАЧЕНИЕ(Перечисление.МетодыОбеспеченияПотребностей.ЗаказПодЗаказ)
			|			 И Настройка.ОбеспечениеЗаказовПриПоддержанииЗапаса
			...
			|		И Настройка.Номенклатура ЕСТЬ NULL
			|
			|	СГРУППИРОВАТЬ ПО
			...
			|				ГДЕ
			|					Т.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
			|					И Настройка.Номенклатура ЕСТЬ NULL)) КАК Т
			|
			|	ОБЪЕДИНИТЬ ВСЕ
			|
			|	ВЫБРАТЬ
			...
			|				= ЗНАЧЕНИЕ(Перечисление.ОбеспечениеЗаказовПриПоддержанииЗапаса.НезависимоОтЗапасов)
			|	ГДЕ
			|		Настройка.Номенклатура ЕСТЬ NULL) КАК Т
			|
			|СГРУППИРОВАТЬ ПО
...
Процедура УдалитьДокументы(ТаблицаДокументы) Экспорт
	...
	ТекстПомечен       = НСтр("ru='%1 - помечен на удаление.';uk='%1 - позначений на вилучення.'");
	ТекстУдален        = НСтр("ru='%1 - удален.';uk='%1 - вилучено.'");
	
	ИменаОбъектовМетаданных = "Документ.ЗаказПоставщику,Документ.ЗаказНаПеремещение,Документ.ЗаказНаСборку,Документ.ЗаказНаПроизводство";
...
//++ НЕ УТКА
		|	И Т.Распоряжение = ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка)
//-- НЕ УТКА
		|
		|//отборы компоновщика
		|{ГДЕ
...
//++ НЕ УТКА
		...
		|	Т.Период                             КАК ДатаОтгрузки,
		|
		|	СУММА(Т.КоличествоНеобеспечено)      КАК Количество,
		|
		...
		|ОБЪЕДИНИТЬ ВСЕ
		|
		//Необеспеченные строки заказов на производство с вариантом обеспечения "Обособленно", без распоряжений на передачу и заказов переработчику.
		|ВЫБРАТЬ
		|	Т.Номенклатура                       КАК Номенклатура,
		|	Т.Характеристика                     КАК Характеристика,
		|	Т.Склад                              КАК Склад,
		...
		|	Т.Период                             КАК ДатаОтгрузки,
		|
		|	СУММА(Т.КоличествоПодЗаказ)          КАК Количество,
		|
		|	МАКСИМУМ(СпрСпособ.Ссылка)           КАК СпособОбеспечения
		|ИЗ
		|	РегистрНакопления.ГрафикОтгрузкиТоваров КАК Т
...
//++ НЕ УТКА
	ЕстьДоступПроизводство = ПравоДоступа("Изменение", Метаданные.Документы.ЗаказНаПроизводство);
...
//++ НЕ УТ
	ЕстьДоступПереработка = ПравоДоступа("Изменение", Метаданные.Документы.ЗаказПереработчику);
