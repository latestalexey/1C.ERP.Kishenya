...
//++ НЕ УТКА
	...
	Для Каждого СтрокаСписка Из ТаблицаДокументы Цикл
		...
		ИдентификаторСтроки = СтрокаСписка.ПолучитьИдентификатор();
		Если ТипЗнч(СтрокаСписка.Документ) = Тип("ДокументСсылка.ЗаказНаПроизводство")
			И СтрокаСписка.НеПоставленВГрафикПроизводства = 1 Тогда
	...
	УстановитьСтатусНаСервере(МассивСсылок, "КПроизводству");
	ОбновитьНадписьПредупреждениеОРазмещенииЗаказов();
//-- НЕ УТКА
	Возврат;
...
//++ НЕ УТ
	...
	Для Каждого Элемент Из МассивТаблиц Цикл
		...
		Для Каждого Элемент1 Из ТаблицыПоСкладам Цикл
			...
			ДанныеЗаполнения.Вставить("Товары", Элемент1.Таблица);
			Документ = Обработки.ОбеспечениеПотребностей.ЗаполнитьИПровестиДокумент(
				Документы.ЗаказПереработчику, ДанныеЗаполнения, Перечисления.СтатусыЗаказовПереработчикам.КОбеспечению, Сообщения);
			ДокументыОбъекты.Добавить(Документ);
...
//++ НЕ УТКА
	...
	Для Каждого Элемент Из МассивТаблиц Цикл
		...
		ДанныеЗаполнения.Вставить("Комментарий", ТекстКомментарий);
		Документ = Обработки.ОбеспечениеПотребностей.ЗаполнитьИПровестиДокумент(
			Документы.ЗаказНаПроизводство, ДанныеЗаполнения, Перечисления.СтатусыЗаказовНапроизводство.КПроизводству, Сообщения);
		ДокументыОбъекты.Добавить(Документ);
...
Функция ТаблицаЗаказыНаПроизводствоКОформлению()
	...
	ТекстЗапроса =
		...
		|	ВЫБОР КОГДА 
		|		Т.Назначение.Заказ Ссылка Документ.ЗаказДавальца ТОГДА
		|			Т.Назначение.Заказ
		|	ИНАЧЕ
		|		ЗНАЧЕНИЕ(Документ.ЗаказДавальца.ПустаяСсылка)
		|	КОНЕЦ КАК Заказ,
		...
		|СГРУППИРОВАТЬ ПО
		|	Т.Организация, Т.Подразделение, Т.НаправлениеДеятельности,
		|
		|	ВЫБОР КОГДА 
		|		Т.Назначение.Заказ Ссылка Документ.ЗаказДавальца ТОГДА
		|			Т.Назначение.Заказ
...
//++ НЕ УТ
		ИначеЕсли ТаблицаДокументы.Итог("НеПоставленВГрафикПроизводства") > 0 Тогда

			Элементы.СтраницыПредупреждениеОРазмещенииЗаказов.ТекущаяСтраница = Элементы.СтраницаПредупреждениеКПроизводству;
...
//++ НЕ УТ		
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказПереработчику") Тогда
...
//++ НЕ УТКА
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
		...
		ТекстЗапроса = ТекстЗапроса + "
		|	МАКСИМУМ(ЗаказТовары.ДатаПотребности) КАК ДатаПоставки,
		|	МИНИМУМ(ЗаказТовары.ДатаПотребности)  КАК ДатаПоставкиМинимум,
		|	0                                     КАК НеПоставленВГрафик,
		|	МАКСИМУМ(ВЫБОР КОГДА Т.Проведен И Т.Статус В(
		|		ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство.КПроизводству),
		|		ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство.Закрыт)) ТОГДА 0
		|			ИНАЧЕ 1
		|		КОНЕЦ)                               КАК НеПоставленВГрафикПроизводства
		|ИЗ
		|	Документ.ЗаказНаПроизводство КАК Т
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаПроизводство.Продукция КАК ЗаказТовары
		|		ПО ЗаказТовары.Ссылка = &Ссылка";
//-- НЕ УТКА
...
Процедура УстановитьСтраницыПомощника(ИмяШага)
	...
	ИначеЕсли ИмяШага = "ШагЗапасы" Тогда
		...
		Элементы.НадписьШаг.Заголовок = НСтр("ru='Шаг 3 из 5. Поддержание запасов';uk='Крок 3 з 5. Підтримка запасів'");
...
Процедура ОбработатьДоступностьТиповОбеспечения(ПараметрОтбор)
	...
	//++ НЕ УТКА
	ПроизводствоДоступно          = ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство");
	//-- НЕ УТКА
	...
	//++ НЕ УТ
	ПроизводствоНаСторонеДоступно = ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне");
	//-- НЕ УТ
...
//++ НЕ УТКА
	Если Не ПравоДоступа("Добавление", Метаданные.Документы.ЗаказНаПроизводство) Тогда
		ПроизводствоДоступно = Ложь;
	КонецЕсли;
//-- НЕ УТКА
//++ НЕ УТ
...
//++ НЕ УТ
	ИспользуютсяСтатусы = ИспользуютсяСтатусы Или
		ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыЗаказовПереработчикам");
//-- НЕ УТ

//++ НЕ УТКА
	ИспользуютсяСтатусы = ИспользуютсяСтатусы Или
		ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыЗаказовДавальцев");
//-- НЕ УТКА
...
Процедура ОчисткаТаблицыПотребностейВопрос(Операция)
	...
	Кнопки.Добавить(КодВозвратаДиалога.Отмена);
	ТекстВопроса = НСтр("ru='В списке потребностей имеются отмеченные позиции, заказы по которым не сформированы. Список потребностей будет очищен.';uk='У списку потреб є зазначені позиції, замовлення по яких не сформовані. Список потреб буде очищений.'");
...
Процедура СформироватьИнформационнуюСтроку(Строка, ОкругленоИтог, КЗаказуИтог, КЗаказуИтогЗапасы, КЗаказуИтогЗаказы, Запасы)
	...
	Иначе
		...
		ВалютаДляВыбора = ?(Строка.ВалютаСоглашения.Пустая(), ВалютаУправленческогоУчета, Строка.ВалютаСоглашения);
		...
		ИнформационнаяСтрокаМассив.Добавить(Новый ФорматированнаяСтрока(" " + ВалютаДляВыбора, ШрифтЖирный));
