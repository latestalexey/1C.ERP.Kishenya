...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.РежимОткрытия = "Полный" Тогда
		...
		ЗаполнитьЗначенияСвойств(МенеджерЗаписи, Параметры.КлючПотребности);
		МенеджерЗаписи.Прочитать();
		Если Не МенеджерЗаписи.Выбран() Тогда
			ЗаполнитьЗначенияСвойств(МенеджерЗаписи, Параметры.КлючПотребности);
			МенеджерЗаписи.МетодОбеспеченияПотребностей = Перечисления.МетодыОбеспеченияПотребностей.ЗаказПодЗаказ;
		КонецЕсли;
		
		МетодОбеспечения  = МенеджерЗаписи.МетодОбеспеченияПотребностей;
		МинимальныйЗапас  = МенеджерЗаписи.МинимальноеКоличествоЗапаса;
		МаксимальныйЗапас = МенеджерЗаписи.МаксимальноеКоличествоЗапаса;
		СтраховойЗапас    = МенеджерЗаписи.СтраховоеКоличествоЗапаса;
		УпаковкаЗаказа    = МенеджерЗаписи.УпаковкаЗаказа;
		ОбеспечениеЗаказовПриПоддержанииЗапаса = МенеджерЗаписи.ОбеспечениеЗаказовПриПоддержанииЗапаса;
		Если МетодОбеспечения = Перечисления.МетодыОбеспеченияПотребностей.ПоддержаниеЗапасаНаСрокПоНорме Тогда
			НормаПотребления = МенеджерЗаписи.СреднедневноеПотребление;
		КонецЕсли;
...
Функция ВыбранноеЗначение()
	...
	ВыбранноеЗначение.Вставить("СтраховоеКолиествоЗапаса", СтраховойЗапас);
