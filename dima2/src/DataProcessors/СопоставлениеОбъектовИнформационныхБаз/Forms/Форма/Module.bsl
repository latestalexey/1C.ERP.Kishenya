...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НовыйЭлементСписка = СписокВыбора.Добавить("ВсеОбъекты", НСтр("ru='Все данные';uk='Всі дані'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, Новый ФиксированнаяСтруктура);
	
	НовыйЭлементСписка = СписокВыбора.Добавить("СопоставленныеОбъектыНеутвержденные", НСтр("ru='Изменения';uk='Зміни'"));
	...
	НовыйЭлементСписка = СписокВыбора.Добавить("СопоставленныеОбъекты", НСтр("ru='Сопоставленные данные';uk='Зіставлені дані'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставленияДополнительный", 0));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъекты", НСтр("ru='Несопоставленные данные';uk='Незіставлені дані'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставленияДополнительный", 1));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъектыПриемника", НСтр("ru='Несопоставленные данные этой базы';uk='Незіставлені дані цієї бази'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставления",  1));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъектыИсточника", НСтр("ru='Несопоставленные данные второй базы';uk='Незіставлені дані другої бази'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставления", -1));
	...
	КонецЕсли;
	Заголовок = НСтр("ru='Сопоставление данных ""[ПредставлениеДанных]""';uk='Зіставлення даних ""[ПредставлениеДанных]""'");
	...
	НаименованиеЭтойПрограммы = ?(ПустаяСтрока(НаименованиеЭтойПрограммы), НСтр("ru='Эта программа';uk='Дана програма'"), НаименованиеЭтойПрограммы);
	
	НаименованиеВторойПрограммы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.УзелИнформационнойБазы, "Наименование");
	НаименованиеВторойПрограммы = ?(ПустаяСтрока(НаименованиеВторойПрограммы), НСтр("ru='Вторая программа';uk='Друга програма'"), НаименованиеВторойПрограммы);
	...
	Элементы.Пояснение.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
        ...
        |;uk='Для зіставлення даних ""%1""
        |с даними ""%2"" скористайтеся командою ""Зіставити автоматично..."".
        |незіставлені дані, Що залишилися, можна зв''язати один з одним вручну.'"),
...
Процедура ВыполнитьЗагрузкуДанных(Команда)
	НСтрока = НСтр("ru='Получить данные в информационную базу?';uk='Одержати дані в інформаційну базу?'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Нет соответствия, объект будет скопирован';uk='Ні відповідності, об''єкт буде скопійований'"));
...
Процедура ВыполнитьЗагрузкуДанныхПослеВопросаОПодтвержденииПолученияДанных(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если Объект.ДанныеУспешноЗагружены Тогда
		НСтрока = НСтр("ru='Данные уже были получены. Выполнить получение данных повторно?';uk='Дані вже були отримані. Виконати одержання даних повторно?'");
...
Процедура ВыполнитьЗагрузкуДанныхПослеПодтвержденияПолученияДанных()
	...
	Состояние(НСтр("ru='Выполняется получение данных. Пожалуйста, подождите...';uk='Виконується одержання даних. Будь ласка, почекайте...'"));
	...
	Если Отказ Тогда
		НСтрока = НСтр("ru='При получении данных возникли ошибки.
                             ...
                             |;uk='При одержанні даних виникли помилки.
...
Процедура ВыполнитьПроверкуЗаданияПользовательскихПолей(Отказ, ПользовательскиеПоля)
	...
	Если ПользовательскиеПоля.Количество() = 0 Тогда
		...
		НСтрока = НСтр("ru='Следует указать хотя бы одно поле для отображения';uk='Слід указати хоча б одне поле для відображення'");
	...
	ИначеЕсли ПользовательскиеПоля.Количество() > МаксимальноеКоличествоПользовательскихПолей() Тогда
		...
		СтрокаСообщения = НСтр("ru='Уменьшите количество полей (можно выбирать не более %1 полей)';uk='Зменшіть кількість полів (можна вибирати не більш %1 полів)'");
...
Процедура УстановитьСвязьИнтерактивно()
	...
	Если Не (ТекущиеДанные.СтатусСопоставления=-1 Или ТекущиеДанные.СтатусСопоставления=+1) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Объекты уже сопоставлены';uk='Об''єкти вже зіставлені'"), 2);
	...
	Кнопки.Добавить(КодВозвратаДиалога.Да,     НСтр("ru='Установить';uk='Встановити'"));
	...
	ТекстВопроса = НСтр("ru='Установить соответствие между выбранными объектами?';uk='Установити відповідність між обраними об''єктами?'");
...
Функция ПолучитьОбъектДляСопоставления(Данные)
	...
	Если Результат.Количество() = 0 Тогда
		...
		Результат.Добавить(НСтр("ru='<не задан>';uk='<не задано>'"));
...
Процедура ВыполнитьСопоставлениеОбъектов(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			НСтр("ru='Сопоставление объектов';uk='Зіставлення об''єктів'"));
	...
	Исключение
		Отказ = Истина;
		ЗаписьЖурналаРегистрации(НСтр("ru='Помощник сопоставления объектов.Анализ данных';uk='Помічник зіставлення об''єктів.Аналіз даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
