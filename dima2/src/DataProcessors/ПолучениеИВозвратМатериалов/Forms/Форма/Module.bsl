...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		...
		НСтр("ru='Передача в производство';uk='Передача в виробництво'"));
	
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ДокументыПроизводства,
		"ОперацияВозврат",
		НСтр("ru='Возврат из производства';uk='Повернення з виробництва'"));
	
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ДокументыПроизводства,
		"ОперацияПеремещение",
		НСтр("ru='Перемещение в производстве';uk='Переміщення у виробництві'"));
	...
	// ВводНаОсновании
	ВводНаОсновании.ПриСозданииНаСервере(ЭтотОбъект, Элементы.ДокументыПроизводстваСоздатьНаОсновании,, "ДобавитьКомандыСозданияНаОснованииСписокДокументыПроизводства");
	ВводНаОсновании.ПриСозданииНаСервере(ЭтотОбъект, Элементы.ЗаказыНаПередачуМатериаловСоздатьНаОсновании,, "ДобавитьКомандыСозданияНаОснованииСписокЗаказыНаПередачуМатериалов");
...
&НаКлиенте
Процедура СтраницыФомрыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	
	Если Элементы.СтраницыФомры.ТекущаяСтраница = Элементы.СтраницаЗаказыНаПередачу Тогда
		Элементы.ЗаказыНаПередачуМатериалов.Обновить();
	ИначеЕсли Элементы.СтраницыФомры.ТекущаяСтраница = Элементы.СтраницаКОформлениюНакладных Тогда
		Элементы.СписокРаспоряженияНаОформление.Обновить();
	ИначеЕсли Элементы.СтраницыФомры.ТекущаяСтраница = Элементы.СтраницаДокументыПроизводства Тогда
...
Процедура КомандаПометитьНаУдалениеДокументыПроизводства(Команда)
    ...
    Иначе
        Если НЕ ТекущиеДанные.ПометкаУдаления Тогда
            ТекстВопроса = НСтр("ru='Пометить выделенные элементы на удаление?';uk='Відмітити виділені елементи для вилучення?'");
        Иначе
            ТекстВопроса = НСтр("ru='Снять с выделенных элементов пометку на удаление?';uk='Зняти з виділених елементів позначку на вилучення?'");
        КонецЕсли; 
    КонецЕсли; 
...
Процедура КомандаНастройкаПередачиМатериалов(Команда)
	...
	Для каждого Идентификатор Из ВыделенныеСтроки Цикл
		...
		Если ТекущееПодразделение <> Неопределено И ТекущееПодразделение <> ДанныеСтроки.Подразделение Тогда
			ТекстПредупреждения = НСтр("ru='Для настройки передачи материалов нужно выбрать материалы одного подразделения.';uk='Для настройки передачі матеріалів потрібно вибрати матеріали одного підрозділу.'");
...
Процедура УстановитьСтатусОтгружено(Команда)
	...
	УстановитьСтатусДокументовПроизводства("Отгружено", НСтр("ru='Отгружено';uk='Відвантажено'"));
...
Процедура УстановитьСтатусПринято(Команда)
	...
	УстановитьСтатусДокументовПроизводства("Принято", НСтр("ru='Принято';uk='Прийнято'"));
...
Процедура Подключаемый_ВыполнитьКомандуСоздатьНаОсновании(Команда)
	...
	ВводНаОснованииКлиент.ВыполнитьПодключаемуюКомандуСоздатьНаОсновании(Команда, ЭтотОбъект, Элементы.ДокументыПроизводства);
...
&НаКлиенте
Процедура ОбновитьДанные(СохранитьПараметрыОтбора)

	РезультатРасчета = ОбновитьДанныеНаСервере(СохранитьПараметрыОтбора);
...
&НаСервере
Функция ОбновитьДанныеНаСервере(СохранитьПараметрыОтбора)
...
Функция ПолучитьДанныеСПомощьюФоновогоЗадания()
	...
	Если ЗапуститьВыполнениеВФоне Тогда
		...
		НаименованиеЗадания = НСтр("ru='Получение данных о заказе материалов';uk='Отримання даних про замовлення матеріалів'");
...
Процедура ОформитьНакладнуюНаОтрицательныйОстатокВПодразделении()
	...
	Если НЕ ЕстьОтрицательныйОстаток Тогда
		Если СписокСтрок.Количество() = 1 Тогда
			ТекстПредупреждения = НСтр("ru='По выбранному материалу отсутствует отрицательный остаток в подразделении.';uk='По вибраному матеріалу відсутній від''ємний залишок у підрозділі.'");
		Иначе
			ТекстПредупреждения = НСтр("ru='По выбранным материалам отсутствует отрицательный остаток в подразделении.';uk='По вибраних матеріалах відсутній від''ємний залишок у підрозділі.'");
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ВернутьИзлишекНаСклад()
	...
	Для каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		...
		Если ТекущаяОрганизация <> Неопределено 
			...
			ТекстПредупреждения = НСтр("ru='Для оформления возврата необходимо выбрать материалы одной организации,
                                            ...
                                            |;uk='Для оформлення повернення необхідно вибрати матеріали однієї організації,
                                            |які передаються із одного підрозділу.'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
			Возврат;
		КонецЕсли;
	...
	Если ДанныеДляЗаполненияТовары.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Необходимо выбрать материалы, по которым есть излишек.';uk='Необхідно вибрати матеріали, по яких є надлишок.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	...
	Для каждого ДанныеСтроки Из СписокСтрок Цикл
		...
		Если ТекущаяОрганизация <> Неопределено 
			...
			ТекстПредупреждения = НСтр("ru='Для оформления накладной необходимо выбрать материалы одной организации,
                                            ...
                                            |;uk='Для оформлення накладної необхідно вибрати матеріали однієї організації,
                                            |які передаються із одного складу і в один підрозділ.'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
			Возврат;
		КонецЕсли;
	...
	Если СписокСтрок.Количество() = 0 
		...
		Если ВыделенныеСтроки.Количество() = 1 Тогда
			Если Действие = "УстранитьТекущийДефицит" Тогда
				ТекстПредупреждения = НСтр("ru='По выбранному материалу отсутствует дефицит.';uk='По вибраному матеріалу відсутній дефіцит.'");
			Иначе
				ТекстПредупреждения = НСтр("ru='Выбранный материал не требуется резервировать.';uk='Вибраний матеріал не потрібно резервувати.'");
			КонецЕсли; 
		Иначе
			Если Действие = "УстранитьТекущийДефицит" Тогда
				ТекстПредупреждения = НСтр("ru='По выбранным материалам отсутствует дефицит.';uk='По вибраних матеріалах відсутній дефіцит.'");
			Иначе
				ТекстПредупреждения = НСтр("ru='Выбранные материалы не требуется резервировать.';uk='Вибрані матеріали не потрібно резервувати.'");
			КонецЕсли;
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат Новый Массив;
...
Функция ПараметрыИИмяФормыОформленияНакладной(ДокументыПоВидам, ПоОрдерам)
	...
	НастройкиФормы.Заголовок = НСтр("ru='Переоформление передачи материалов по выбранным распоряжениям';uk='Переоформлення передачі матеріалів за вибраними розпорядженнями'");
	...
	Результат = НакладныеСервер.ПроверитьВозможностьВыполненияКомандыОформить(КонтекстВызова, ПараметрыВыполненияКоманды);
	Если Не Результат.ЕстьОшибки Тогда
		Результат = НакладныеСервер.ПараметрыИИмяФормыОформленияНакладной(КонтекстВызова, ПараметрыВыполненияКоманды, Результат.РеквизитыШапки);
	КонецЕсли;
...
Процедура УстановитьТекстЗапросаСписокРаспоряженияНаОформление()
	...
	СписокРаспоряженияНаОформление.ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|					И (Склад = &Склад
	|						ИЛИ &Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка))) КАК ЗаказыМатериалов
	|	ГДЕ
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<несколько складов>';uk='<кілька складів>'"));
	#КонецОбласти
	
	ОбщегоНазначенияУТ.УстановитьУсловноеОформлениеПоляДата(ЭтотОбъект, "ЗаказыНаПередачуМатериалов", "ЗаказыНаПередачуМатериаловДата");
	ОбщегоНазначенияУТ.УстановитьУсловноеОформлениеПоляДата(ЭтотОбъект, "ДокументыПроизводства", "ДокументыПроизводстваДата");
...
Процедура СкорректироватьРеквизитЗаказыПоДату()
	...
	Если ЗаказыПоДату <> '000101010000'
		И ЗаказыПоДату < НачалоДня(ТекущаяДата()) Тогда
		// Дата не может быть в прошлом
		ЗаказыПоДату = КонецДня(ТекущаяДата());
	КонецЕсли;
...
Процедура СохранитьПараметрыОтбора()
	...
	ПараметрыОтбора = Новый Структура("ОтборПодразделение,ОтборОрганизация,ОтборСклад,ЗаказыПоДату,ОтборСостояниеНакладной,ОтборСостояниеОрдера");
...
&НаКлиенте
Процедура УстановитьСтатусДокументовПроизводства(СтатусСтрока, ПредставлениеСтатуса = Неопределено)
	...
	СписокКнопок.Добавить(КодВозвратаДиалога.Да,НСтр("ru='Установить статус';uk='Встановити статус'"));
...
Функция ВыполняетсяОбновлениеЧитаемыхДанных()
	...
	|ИЗ
	|	РегистрНакопления.МатериалыИРаботыВПроизводстве.Изменения КАК ТаблицаИзменений
	|ГДЕ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыКОтгрузке ТаблицаОбъекта
	|		ПО ТаблицаОбъекта.Регистратор = ТаблицаИзменений.Регистратор
	|			И ТаблицаОбъекта.Получатель ССЫЛКА Справочник.СтруктураПредприятия
	|ГДЕ
	|	ТаблицаИзменений.Узел ССЫЛКА ПланОбмена.ОбновлениеИнформационнойБазы
	//++ НЕ УТКА
	|
	...
	|	ТаблицаИзменений.Регистратор
	|ИЗ
	|	РегистрНакопления.ПотребностьВМатериалахВПроизводстве.Изменения КАК ТаблицаИзменений
	|ГДЕ
	|	ТаблицаИзменений.Узел ССЫЛКА ПланОбмена.ОбновлениеИнформационнойБазы
	|
	|ОБЪЕДИНИТЬ ВСЕ
