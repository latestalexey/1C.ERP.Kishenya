...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
			...
			Если Не ПриВходеВПрограмму Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось открыть форму смены пароля по причине:
                               |
                               |%1'
                               |;uk='Не вдалося відкрити форму для зміни пароля з причини:
                               |
                               |%1'"),
	...
	Если ПриВходеВПрограмму Тогда
		...
		Иначе
			...
			Элементы.ПояснениеПриВходе.Заголовок =
				НСтр("ru='Для входа в программу нужно установить пароль.';uk='Для входу в програму потрібно встановити пароль.'")
		КонецЕсли;
	Иначе
		...
		Элементы.ФормаЗакрытьФорму.Заголовок = НСтр("ru='Отмена';uk='Відмінити'");
	КонецЕсли;
	...
	Если Не ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	 Или Не ДополнительныеПараметры.ПарольУстановлен Тогда
		...
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ru='Установка пароля';uk='Установка пароля'");
...
Процедура УстановитьПароль(Команда)
	...
	Если Не ПоказыватьНовыйПароль И НовыйПароль <> Подтверждение Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Подтверждение пароля указано некорректно.';uk='Підтвердження пароля зазначено некоректно.'"));
		Возврат;
	КонецЕсли;
