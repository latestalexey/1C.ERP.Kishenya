...
Процедура СтандартныеУстановитьПометкуУдаления(Команда)
	...
	Если Модифицированность Тогда
		Если ЭтотОбъект.Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ru='Для выполнения действия требуется записать изменения файла.
                      |Записать изменения и снять пометку на удаление с файла
                      |""%1""?'
                      |;uk='Для виконання дії потрібно записати зміни файлу.
                      |Записати зміни і зняти помітку на вилучення з файлу
                      |""%1""?'");
		Иначе
			ТекстВопроса = НСтр("ru='Для выполнения действия требуется записать изменения файла.
                      ...
                      |""%1""?'
                      |;uk='Для виконання дії потрібно записати зміни файлу.
                      |Записати зміни і відмітити для вилучення файл
                      |""%1""?'");
		КонецЕсли;
	Иначе
		Если ЭтотОбъект.Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ru='Снять пометку на удаление с файла
                                      |""%1""?'
                                      |;uk='Зняти помітку вилучення з файлу
                                      |""%1""?'");
		Иначе
			ТекстВопроса = НСтр("ru='Пометить на удаление файл
                                      |""%1""?'
                                      |;uk='Відмітити для вилучення файл
                                      |""%1""?'");
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьЭП(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Удалить выделенные подписи?';uk='Вилучити виділені підписи?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОбновитьЗаголовок()
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Ссылка) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 (Присоединенный файл)';uk='%1 (Приєднаний файл)'"), Строка(ЭтотОбъект.Объект.Ссылка));
	Иначе
		Заголовок = НСтр("ru='Присоединенный файл (Создание)';uk='Приєднаний файл (Створення)'")
	КонецЕсли;
...
Процедура ЗаполнитьСписокШифрования()
	...
	ТекстЗаголовка = НСтр("ru='Разрешено расшифровывать';uk='Дозволено розшифровувати'");
...
Процедура ЗаполнитьСписокПодписей()
	...
	ТекстЗаголовка = НСтр("ru='Электронные подписи';uk='Електронні підписи'");
...
Функция ОбработатьКомандуЗаписиФайла()
	...
	Если ПустаяСтрока(ЭтотОбъект.Объект.Наименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Для продолжения укажите имя файла.';uk='Для продовження вкажіть ім''я файлу.'"), , "Наименование", "Объект");
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗаписатьФайл(Знач ПараметрОбъект = Неопределено)
	...
	Исключение
		Если ТранзакцияАктивна Тогда
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Ошибка записи присоединенного файла';uk='Файли.Помилка запису приєднаного файлу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
