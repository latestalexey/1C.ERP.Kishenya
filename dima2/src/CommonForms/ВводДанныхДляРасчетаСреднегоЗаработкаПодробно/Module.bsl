...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоГодоваяПремия Тогда
		...
		ТекстПодсказки = СтрШаблон(НСтр("ru='Расшифровка годовой премии сотрудника %1 за %2';uk='Розшифровка річної премії співробітника %1 %2'"), Сотрудник, Формат(Параметры.Год, "ЧГ=")); 
	Иначе
		ШаблонПредставления = НСтр("ru='Заработок за %2';uk='Заробіток за %2'");
		Если ЗначениеЗаполнено(СоставнаяЧасть) Тогда
			ШаблонПредставления = НСтр("ru='%1 за %2';uk='%1 за %2'");
		КонецЕсли;
		Заголовок = СтрШаблон(ШаблонПредставления, СоставнаяЧасть, Формат(Месяц, "ДФ='ММММ гггг'"));
		ТекстПодсказки = СтрШаблон(НСтр("ru='Расшифровка среднего заработка сотрудника %1 за %2';uk='Розшифровка середнього заробітку працівника %1 %2'"), Сотрудник, Формат(Месяц, "ДФ='ММММ гггг'")); 
	КонецЕсли;
...
Функция ДанныеРасшифровки()
	...
	Для Каждого СтрокаТаблицы Из СреднийЗаработок Цикл
		...
		Если Не ЭтоГодоваяПремия Тогда
...
Процедура ПроверитьЗаполнениеДанныхРасшифровки(Отказ)
	...
	Пока Индекс < СреднийЗаработок.Количество() Цикл
		Если Не ЗначениеЗаполнено(СреднийЗаработок[Индекс].Сумма) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Сумма не заполнена.';uk='Сума не заповнена.'"), , ИмяПоля, , Отказ);
		КонецЕсли;
		Если ИспользоватьИсточникиФинансирования Тогда
			Если Не ЗначениеЗаполнено(СреднийЗаработок[Индекс].СтатьяФинансирования) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Статья финансирования не заполнена.';uk='Стаття фінансування не заповнена.'"), , ИмяПоля, , Отказ);
			КонецЕсли;
		КонецЕсли;
		Индекс = Индекс + 1;
	КонецЦикла;
