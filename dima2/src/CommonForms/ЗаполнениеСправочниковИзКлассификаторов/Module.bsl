...
Процедура СообщитьОЗагруженностиВсегоКлассификатора()
	...
	ПоказатьПредупреждение(, ТекстПредупреждения, , НСтр("ru='Загрузка классификатора';uk='Завантаження класифікатора'"));
КонецПроцедуры
...
Функция ПолучитьМакетКлассификатора(ПолныйПутьКМакету)
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Макет классификатора ""%1"" не найден. Операция прервана.';uk='Макет класифікатора ""%1"" не знайдено. Операція перервана.'"), ПолныйПутьКМакету);
	КонецЕсли;
