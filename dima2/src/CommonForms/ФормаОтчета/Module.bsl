	...
	Если ТипЗнч(НастройкиОтчета.РазрешеноМенятьВарианты) <> Тип("Булево") Тогда
		...
		Если НастройкиОтчета.РазрешеноМенятьВарианты // Команды не отключены параметрами открытия формы.
			...
			Если Отключенные.Найти(НастройкиОтчета.ПредопределенныйСсылка) <> Неопределено Тогда // Вариант отключен.
				Текст = НСтр("ru='Контекстный вариант ""%1"" отчета ""%2"" открыт без контекста.';uk='Контекстний варіант ""%1"" звіту ""%2"" відкрит без контексту.'");
	...
	Для Каждого ПользовательскаяНастройкаКД Из ПользовательскиеНастройкиКД.Элементы Цикл
		...
		Если Тип = "ЗначениеПараметраНастроек"
			...
			Если ПериодНачало.АвтоОтметкаНезаполненного
				...
				ТекстОшибки = НСтр("ru='Не указан период';uk='Не вказано період'");
				ПутьКДанным = ПериодНачало.ПутьКДанным;
			ИначеЕсли Значение.ДатаНачала > Значение.ДатаОкончания Тогда
				ТекстОшибки = НСтр("ru='Конец периода должен быть больше начала';uk='Кінець періоду повинен бути більше початка'");
...
Процедура Подключаемый_ЗагрузитьВариантОтчета(Команда)
	...
	Если Найденные.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Вариант отчета не найден.';uk='Варіант звіту не знайдено.'"));
		Возврат;
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Иначе
		...
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ru='Отчет не сформирован';uk='Звіт не сформований'"), , Заголовок);
		КонецЕсли;
...
Процедура ПередФормированиемНаКлиенте()
	...
	Если ПеременныеКлиента.ВыполнятьЗамеры Тогда
		...
		Комментарий = НастройкиОтчета.ПрефиксЗамеров + "; " + НСтр("ru='Непосредственно:';uk='Безпосередньо:'") + " " + Строка(ПеременныеКлиента.Непосредственно);
...
Процедура ПоказатьДиалогОтправкиПоЭлектроннойПочте()
	...
	ЗаголовокСохранения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Отправка отчета ""%1"" по почте';uk='Відправлення звіту ""%1"" поштою'"), ОтчетНаименованиеТекущегоВарианта);
...
Процедура ВидимостьДоступностьКорректность(Изменения = "")
	...
	КонецЕсли;
	Если РежимРасшифровки Тогда
		Заголовок = Заголовок + " (" + НСтр("ru='Расшифровка';uk='Розшифровка'") + ")";
	КонецЕсли;
КонецПроцедуры
...
Функция ФоновоеЗаданиеЗапустить(Знач ФормированиеПриОткрытии)
	...
	Иначе
		...
		ПараметрыЗапуска.НаименованиеФоновогоЗадания = НСтр("ru='Варианты отчетов: Формирование отчета';uk='Варіанти звітів: Формування звіту'");
...
Процедура ЗагрузитьВариант(КлючВарианта)
	...
	ОтображениеСостояния.Текст     = НСтр("ru='Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.';uk='Обраний інший варіант звіту. Натисніть кнопку ""Сформувати"" для одержання звіту.'");
КонецПроцедуры
...
Процедура БыстрыеНастройкиЗагрузитьНастройкиВКомпоновщик(ПараметрыЗаполнения)
	...
	Если НастройкиОтчета.События.ПередЗагрузкойНастроекВКомпоновщик Тогда
		НовыеНастройкиКДБезСериализации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкиОтчета, "НовыеНастройкиКДБезСериализации");
		Если ТипЗнч(НовыеНастройкиКДБезСериализации) = Тип("НастройкиКомпоновкиДанных") Тогда
			НовыеНастройкиКД = НовыеНастройкиКДБезСериализации;
			НастройкиОтчета.НовыеНастройкиКДБезСериализации = Неопределено;		
		КонецЕсли;
...
Процедура ПослеИзмененияКлючевыхСостояний(ПараметрыЗаполнения)
	Если ПараметрыЗаполнения.ИмяСобытия <> "ПослеФормирования" Тогда
		...
		ИначеЕсли ПараметрыЗаполнения.ВариантМодифицирован
			...
			ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
			ОтображениеСостояния.Видимость = Истина;
			ОтображениеСостояния.Текст     = НСтр("ru='Изменились настройки. Нажмите ""Сформировать"" для получения отчета.';uk='Змінилися настройки. Натисніть кнопку ""Сформувати"" для одержання звіту.'");
...
Функция СообщениеДанныеЕщеОбновляются()
	Возврат НСтр("ru='Отчет может содержать некорректные данные, так как не завершен переход на новую версию программы. Если отчет долгое время недоступен, необходимо обратиться к администратору.';uk='Звіт може містити некоректні дані, тому що не завершено перехід на нову версію програми. Якщо звіт довгий час недоступний, необхідно звернутися до адміністратора.'");
КонецФункции
