...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Что нового в конфигурации %1';uk='Що нового в конфігурації %1'"), Метаданные.Синоним);
	...
	Если Разделы.Количество() = 0 Тогда
		...
		Если ДокументОписаниеОбновлений <> Неопределено
			...
			Если ВсеРазделы.Количество() <> 0 Тогда
	...
	Если ДокументОписаниеОбновлений.ВысотаТаблицы = 0 Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Конфигурация успешно обновлена на версию %1';uk='Конфігурація успішно оновлена на версію %1'"), Метаданные.Версия);
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		ЗаголовокСообщения = НСтр("ru='Необходимо выполнить дополнительные процедуры обработки данных';uk='Необхідно виконати додаткові процедури обробки даних'");
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		Элементы.ОтложенноеОбновлениеДанных.Заголовок =
			НСтр("ru='Не выполнены дополнительные процедуры обработки данных';uk='Не виконані додаткові процедури обробки даних'");
		Элементы.ОтложенноеОбновлениеДанныхПояснение.Заголовок = 
			НСтр("ru='Работа в программе временно ограничена, так как еще не завершен переход на новую версию.';uk='Робота в програмі тимчасово обмежена, тому що ще не завершений перехід на нову версію.'");
	КонецЕсли;
...
Процедура ОтключеныРегламентныеЗаданияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ru='Перезапустить программу?';uk='Перезапустити програму?'");
