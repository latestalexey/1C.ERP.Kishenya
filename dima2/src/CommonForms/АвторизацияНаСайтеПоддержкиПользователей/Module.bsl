...
Процедура ПерейтиКРегистрацииНаСайтеНажатие(Элемент)
	...
	ПерейтиПоНавигационнойСсылке("http://releases.1c.eu");
...
Процедура СохранитьДанныеАутентификацииИПродолжить()
	...
	Если ПустаяСтрока(Логин) И Не ПустаяСтрока(Пароль) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Введите код пользователя для авторизации на сайте поддержки пользователей.';uk='Введіть код користувача для авторизації на сайті підтримки користувачів.'"),, "Логин");
		Возврат;
	КонецЕсли;
