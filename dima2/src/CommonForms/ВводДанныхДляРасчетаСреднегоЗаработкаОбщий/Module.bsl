...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РучнаяКорректировкаСреднегоЗаработка Тогда
		СреднийЗаработокИтог = Параметры.СреднийЗаработок;
	КонецЕсли;
	ВидУчетаВремениДляСредней = Параметры.ВидУчетаВремениДляСредней;
	...
	Если РучнаяКорректировкаСреднегоЗаработка Тогда
		ОформитьСреднийЗаработокИтогПриИзменении(ЭтаФорма);
	КонецЕсли;
	УстановитьВидимостьКнопкиОтменитьРучнаяКорректировкаСреднегоЗаработка();
	
	УстановитьВидимостьВидаУчетаВремениДляСредней();
...
Процедура ПеречитатьДанныеУчета(Команда)
	...
	ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени("ПересчетДанныхВФормеВводДанныхДляРасчетаСреднегоЗаработкаОбщий");	
...
&НаСервере
Процедура УстановитьВидимостьВидаУчетаВремениДляСредней()
	
	ЭтаФорма.Элементы.ВидУчетаВремениДляСредней.Видимость = ЭтаФорма.ПорядокРасчета = ПредопределенноеЗначение("Перечисление.ПорядокРасчетаСреднегоЗаработкаОбщий.Постановление2010");
...
Функция ДанныеДляРасчетаСреднего()
	...
	СтруктураДанных.Вставить("РучнаяКорректировкаСреднегоЗаработка", РучнаяКорректировкаСреднегоЗаработка);
	СтруктураДанных.Вставить("ВидУчетаВремениДляСредней", ВидУчетаВремениДляСредней);
...
Процедура УстановитьДоступностьКорректировок()
	...
	Элементы.ДополнитьДанныеСреднегоЗаработка.Доступность = СреднийЗаработокИтог = 0 ;
...
Процедура РассчитатьСреднийЗаработок(Форма)
	...
	ДополнительныеПараметры.ВидУчетаВремениДляСредней = Форма.ВидУчетаВремениДляСредней;
	ДополнительныеПараметры.ПорядокРасчета = Форма.ПорядокРасчета;
	
	Если Не Форма.РучнаяКорректировкаСреднегоЗаработка Тогда
...
Процедура ДополнитьПредположительно()
	...
	Если Начало = Неопределено Тогда
		МассивМесяцев.Добавить(НачалоМесяца(ДатаНачалаСобытия));
		Начало = НачалоМесяца(ДатаНачалаСобытия);
		Окончание = КонецМесяца(ДатаНачалаСобытия);
		ОтсутствиеЗаработка = Истина;
	КонецЕсли;
	...
	//НоваяСтрока.Сотрудник = Сотрудник;
	//НоваяСтрока.Организация = Организация;
	//НоваяСтрока.ДатаНачалаПериода = НачалоМесяца(ОкончаниеПериодаРасчета);
	//НоваяСтрока.ДатаОкончанияПериода = КонецМесяца(ОкончаниеПериодаРасчета);
	...
	КалендарныеДниБезУчетаПраздников = КалендарныеГрафики.ПолучитьКалендарныеДниБезПраздниковЗаПериод(ДобавитьМесяц(НачалоМесяца(Окончание),-12), КонецМесяца(ДобавитьМесяц(Окончание,-1)));
	...
	Запрос.Текст = 
		...
		|		И (НастройкаСреднегоЗаработка.ПорядокРасчета = &ПорядокРасчета)
		|
		|СГРУППИРОВАТЬ ПО
		|	НАЧАЛОПЕРИОДА(ПлановыеНачисления.Период, МЕСЯЦ),
		|	ПлановыеНачисления.Сотрудник,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ОтсутствиеЗаработка Тогда
			Если Выборка.Период > ОкончаниеПериодаРасчета Тогда
				ДанныеНачислений.ЗаполнитьЗначения(НачалоМесяца(ОкончаниеПериодаРасчета),"Период");
			КонецЕсли;	
		КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		ОтработаноДнейКалендарных = Окр(КалендарныеДниБезУчетаПраздников/12,1);
		ДанныеВремени.ЗаполнитьЗначения(ОтработаноДнейКалендарных,"СреднемесячноеКоличествоДней");
		Если ПорядокРасчета = ПредопределенноеЗначение("Перечисление.ПорядокРасчетаСреднегоЗаработкаОбщий.Постановление100Отпускные") Тогда
			ДанныеВремени.ЗаполнитьЗначения(ОтработаноДнейКалендарных,"ОтработаноДнейКалендарных");
		ИначеЕсли ОтсутствиеЗаработка Тогда
			Если Выборка.Период > ОкончаниеПериодаРасчета Тогда
				ДанныеВремени.ЗаполнитьЗначения(НачалоМесяца(ОкончаниеПериодаРасчета),"Период");
			КонецЕсли;	
		КонецЕсли;
	КонецЦикла;
	
	Если ПорядокРасчета = ПредопределенноеЗначение("Перечисление.ПорядокРасчетаСреднегоЗаработкаОбщий.Постановление2010") Тогда
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВТГрафикРаботыСотрудников.ГрафикРаботы КАК ГрафикРаботы
		|ИЗ
		|	ВТГрафикРаботыСотрудниковСрезПоследних КАК ВТГрафикРаботыСотрудников
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТГрафикиРаботыСотрудников;
		|УНИЧТОЖИТЬ ВТДанныеКалендарей;";
		
		Выборка = Запрос.Выполнить().Выбрать();
		
		ТаблицаГрафиков = Новый ТаблицаЗначений;
		ТаблицаГрафиков.Колонки.Добавить("ГрафикРаботы", Новый ОписаниеТипов("СправочникСсылка.ГрафикиРаботыСотрудников"));
		ТаблицаГрафиков.Колонки.Добавить("ДатаНачалаПериода", Новый ОписаниеТипов("Дата"));
		ТаблицаГрафиков.Колонки.Добавить("ДатаОкончанияПериода", Новый ОписаниеТипов("Дата"));
		
		Если Выборка.Следующий() Тогда
			СтрокаГрафиков = ТаблицаГрафиков.Добавить();
			СтрокаГрафиков.ДатаНачалаПериода = ДобавитьМесяц(НачалоПериодаРасчета,-2);
			СтрокаГрафиков.ДатаОкончанияПериода = КонецМесяца(ДобавитьМесяц(НачалоПериодаРасчета,-1));
			СтрокаГрафиков.ГрафикРаботы = Выборка.ГрафикРаботы;
		КонецЕсли;
		
		УчетРабочегоВремени.СоздатьВТВремяПоГрафикам(ТаблицаГрафиков, МенеджерВременныхТаблиц);
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВТВремяПоГрафикам.НормаДней КАК НормаДней
		|ИЗ ВТВремяПоГрафикам КАК ВТВремяПоГрафикам";

		Выборка = Запрос.Выполнить().Выбрать();
		Если  Выборка.Следующий() Тогда
			НормаПоГрафикуЗаПредыдущиеМесяцы = Окр(Выборка.НормаДней/2,1);
			ДанныеВремени.ЗаполнитьЗначения(НормаПоГрафикуЗаПредыдущиеМесяцы,"СреднемесячноеКоличествоДней");
		КонецЕсли;	
	КонецЕсли;
...
&НаСервере
Процедура РассчитатьСреднийЗаработокПоОкладу(ДанныеНачислений, ДанныеВремени)
	
	СуммаСреднейПоОкладу = 0;
	СуммаПоОкладуИтог = ДанныеНачислений.Итог("Сумма");
	СреднемесячноеКоличествоДнейИтог = ДанныеВремени.Итог("СреднемесячноеКоличествоДней"); 
	Если НЕ СреднемесячноеКоличествоДнейИтог = 0 Тогда
		СуммаСреднейПоОкладу = Окр(СуммаПоОкладуИтог/СреднемесячноеКоличествоДнейИтог,2);	
	КонецЕсли;
...
Процедура ЗаполнитьОписаниеСтрок()
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Общий заработок';uk='Загальний заробіток'");
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='	в т.ч. неиндексируемый';uk=' в т. ч. неіндексований'");
	НоваяСтрока.ЗаголовокКоманды = НСтр("ru='Общий заработок (неиндексируемый)';uk='Загальний заробіток (неиндексируемый)'");
	НоваяСтрока.СоставнаяЧасть = Перечисления.УчетНачисленийВСреднемЗаработкеОбщий.ОбщийЗаработок;
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Премия, полностью учитываемая';uk='Премія, повністю обліковується'");
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='	в т.ч. неиндексируемая';uk=' в т. ч. неіндексована'");
	НоваяСтрока.ЗаголовокКоманды = НСтр("ru='Премия, полностью учитываемая (неиндексируемая)';uk='Премія, повністю обліковується (неиндексируемая)'");
	НоваяСтрока.СоставнаяЧасть = Перечисления.УчетНачисленийВСреднемЗаработкеОбщий.ПремияПроцентом;
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Премия, учитываемая частично';uk='Премія, яка обліковується частково'");
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='	в т.ч. неиндексируемая';uk=' в т. ч. неіндексована'");
	НоваяСтрока.ЗаголовокКоманды = НСтр("ru='Премия, учитываемая частично (неиндексируемая)';uk='Премія, яка обліковується частково (неиндексируемая)'");
	НоваяСтрока.СоставнаяЧасть = Перечисления.УчетНачисленийВСреднемЗаработкеОбщий.ПремияФиксированнойСуммой;
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Годовая премия, полностью учитываемая';uk='Річна премія, повністю обліковується'");
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='	в т.ч. неиндексируемая';uk=' в т. ч. неіндексована'");
	НоваяСтрока.ЗаголовокКоманды = НСтр("ru='Годовая премия, полностью учитываемая (неиндексируемая)';uk='Річна премія, повністю обліковується (неіндексується)'");
	НоваяСтрока.СоставнаяЧасть = Перечисления.УчетНачисленийВСреднемЗаработкеОбщий.ПремияГодоваяПроцентом;
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Годовая премия, учитываемая частично';uk='Річна премія, яка обліковується частково'");
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='	в т.ч. неиндексируемая';uk=' в т. ч. неіндексована'");
	НоваяСтрока.ЗаголовокКоманды = НСтр("ru='Годовая премия, учитываемая частично (неиндексируемая)';uk='Річна премія, яка обліковується частково (неіндексується)'");
	НоваяСтрока.СоставнаяЧасть = Перечисления.УчетНачисленийВСреднемЗаработкеОбщий.ПремияГодоваяФиксированнойСуммой;
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Отработано дней';uk='Відпрацьовано днів'");
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Отработано часов';uk='Відпрацьовано годин'");
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Отработано дней (календ.)';uk='Відпрацьовано днів (календ.)'");
	НоваяСтрока.ДобавлятьПоУмолчанию = Истина;
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Норма дней (по произв. календарю)';uk='Норма днів (вир. календарем)'");
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Норма часов (по произв. календарю)';uk='Норма годин (вир. календарем)'");
	...
	НоваяСтрока.Точность = 2;
	НоваяСтрока = ОписаниеСтрок.Добавить();
	НоваяСтрока.Идентификатор = "НормаДнейПоГрафикуСотрудника";
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Норма дней (по графику сотр.)';uk='Норма днів (за графіком співр.)'");
	НоваяСтрока.ДобавлятьПоУмолчанию = Истина;
	НоваяСтрока.Точность = 2;
	
	НоваяСтрока = ОписаниеСтрок.Добавить();
	НоваяСтрока.Идентификатор = "НормаЧасовПоГрафикуСотрудника";
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Норма часов (по графику сотр.)';uk='Норма годин (за графіком співр.)'");
	НоваяСтрока.ДобавлятьПоУмолчанию = Истина;
	НоваяСтрока.Точность = 2;

	НоваяСтрока = ОписаниеСтрок.Добавить();
	НоваяСтрока.Идентификатор = "СреднемесячноеКоличествоДней";
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Среднемес. к-во дней';uk='Среднемес. к-ть днів'");
	НоваяСтрока.ДобавлятьПоУмолчанию = Истина;
	НоваяСтрока.Точность = 2;
...
Процедура ЗаполнитьИнформациюОбИндексации()
	...
	Пока ИндексСтроки >= 0 Цикл
		...
		Если СтрокаИндексации.КоэффициентИндексации <> ПредыдущийКоэффициент Тогда
			ТекстИндексации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='%1 года c коэффициентом %2';uk='%1 року з коефіцієнтом %2'"), 
	...
	Элементы.СведенияОбИндексацииГруппа.Заголовок = ?(ЗначениеЗаполнено(ИнфоНадписьКоэффициентыИндексации), 
														НСтр("ru='Индексация заработка сотрудника выполнялась:';uk='Індексація заробітку працівника виконувалася:'"), 
														НСтр("ru='Индексация заработка сотрудника не выполнялась';uk='Індексація заробітку працівника не виконувалась'"));
...
Процедура УстановитьОформлениеГодовыхПремий()
	...
	Элементы.ГодовыеПремииГруппа.Заголовок = СтрШаблон(НСтр("ru='Годовые премии за %1 год';uk='Річні премії за %1 рік'"), Формат(ГодГодовыхПремий, "ЧГ="));
...
Функция ИдентификаторыСтрокДанныхВремени()
	...
	Соответствие.Вставить("НормаДнейПоГрафикуСотрудника", "НормаДнейПоГрафикуСотрудника");
	Соответствие.Вставить("НормаЧасовПоГрафикуСотрудника", "НормаЧасовПоГрафикуСотрудника");
	Соответствие.Вставить("СреднемесячноеКоличествоДней", "СреднемесячноеКоличествоДней");
...
Функция ОписаниеСтрокиДанныхОВремени()
	...
	ПоляОписания.Вставить("НормаЧасовПоГрафикуСотрудника", Новый ОписаниеТипов("Число"));
	ПоляОписания.Вставить("НормаДнейПоГрафикуСотрудника", Новый ОписаниеТипов("Число"));
	ПоляОписания.Вставить("СреднемесячноеКоличествоДней", Новый ОписаниеТипов("Число"));
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОформитьСреднийЗаработокИтогПриИзменении(Форма)
	
	ЖирныйШрифт = Новый Шрифт(Форма.Элементы.СреднийЗаработокИтог.Шрифт, , , Истина);
	Форма.Элементы.СреднийЗаработокИтог.Шрифт = ЖирныйШрифт;
