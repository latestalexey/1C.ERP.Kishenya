...
Процедура ПриОткрытии(Отказ)
	...
	Если ПоказатьОповещениеОПодсказках И ПоказыватьПодсказки Тогда
		ПоказатьОповещениеПользователя(
			НСтр("ru='Новая возможность';uk='Нова можливість'"),
			"e1cib/data/ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ОписаниеНовойВозможностиПоВыводуОписаний",
			НСтр("ru='Вывод описаний в панелях отчетов';uk='Вивід описів в панелях звітів'"),
...
Функция ВведенаСлишкомКороткаяСтрокаПоиска(Текст)
	...
	Если СтрДлина(Текст) < 2 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Введена слишком короткая строка поиска.';uk='Введений занадто короткий рядок пошуку.'"));
		Возврат Истина;
	КонецЕсли;
	...
	Если Не ЕстьНормальноеСлово Тогда
		ПоказатьПредупреждение(, НСтр("ru='Введены слишком короткие слова для поиска.';uk='Введені занадто короткі слова для пошуку.'"));
		Возврат Истина;
	КонецЕсли;
...
Процедура СброситьНастройки(Команда)
	ТекстВопроса = НСтр("ru='Сбросить настройки расположения отчетов?';uk='Скинути настройки розташування звітів?'");
...
Функция НачатьЗамер(Событие, Комментарий = Неопределено)
	...
	Если Событие = "СписокОтчетов.Открытие" Или Событие = "ПанельОтчетов.Открытие" Тогда
		Замер.Имя = Событие;
		Комментарий = Комментарий + "; " + НСтр("ru='Из панели отчетов:';uk='З панелі звітів:'") + " " + ПараметрыКлиента.ПутьКПодсистеме;
	Иначе
		...
		Комментарий = Комментарий + "; " + НСтр("ru='Подсказки:';uk='Підказки:'") + " " + Строка(ПоказыватьПодсказки);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Комментарий = Комментарий
			+ "; " + НСтр("ru='Поиск:';uk='Пошук:'") + " " + Строка(СтрокаПоиска)
			+ "; " + НСтр("ru='Во всех разделах:';uk='У всіх розділах:'") + " " + Строка(ИскатьВоВсехРазделах);
	Иначе
		Комментарий = Комментарий + "; " + НСтр("ru='Без поиска';uk='Без пошуку'");
	КонецЕсли;
	
	Если Событие = "ВыходИзРежимаНастройки" Тогда
		Комментарий = Комментарий + "; " + НСтр("ru='Выход из режима настройки';uk='Вихід з режиму настройки'");
	КонецЕсли;
...
Процедура ОбновитьПанельОтчетовНаСервере(Знач Событие = "")
	...
	// Заголовок.
	СуффиксРежимаНастройки = " (" + НСтр("ru='настройка';uk='настройка'") + ")";
...
Процедура ЗаполнитьИнформациюОПодсистемахИУстановитьЗаголовок()
	...
	Если ТекущийРазделСсылка = Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка() Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Розділ ""%1"" не підключений до підсистеми ""%2"".
            |См. модуль ВариантыОтчетовПереопределяемый, процедуру ОпределитьРазделыСВариантамиОтчетов.'"),
...
Процедура ЗаполнитьПанельОтчетов()
	...
	Иначе
		...
		Если ПараметрыЗаполнения.ДругиеРазделы.Количество() = 0 Тогда
			...
			Надпись.Заголовок = "    " + НСтр("ru='Отчеты в других разделах не найдены.';uk='Звіти в інших розділах не знайдені.'") + Символы.ПС;
			Надпись.Высота = 2;
		КонецЕсли;
		...
		Если ПараметрыЗаполнения.НеВыведено > 0 Тогда // Вывод информационной надписи.
			ЗаголовокНадписи = НСтр("ru='Выведены первые %1 отчетов из других разделов, уточните поисковый запрос.';uk='Виведені перші %1 звітів з інших розділів, уточніть пошуковий запит.'");
...
Процедура ВывестиВариантыРаздела(ПараметрыЗаполнения, РазделСсылка)
	...
	Если ПараметрыЗаполнения.ВариантыКоличество = 0 Тогда
		...
		Если ПараметрыЗаполнения.ВыводятсяВариантыТекущегоРаздела Тогда
			...
			Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
				Если ПараметрыЗаполнения.ТолькоТекущийРаздел Тогда
					Надпись.Заголовок = НСтр("ru='Отчеты не найдены.';uk='Звіти не знайдені.'");
				Иначе
					Надпись.Заголовок = НСтр("ru='Отчеты в текущем разделе не найдены.';uk='Звіти в поточному розділі не знайдені.'");
					Надпись.Высота = 2;
				КонецЕсли;
			Иначе
				Надпись.Заголовок = НСтр("ru='В панели отчетов этого раздела не размещено ни одного отчета.';uk='В панелі звітів цього розділу не розміщено жодного звіту.'");
			КонецЕсли;
...
Функция ДобавитьГруппуПодсистемы(ПараметрыЗаполнения, СтрокаПорядкаВывода, ВГруппу)
	...
	Если ТребуетсяПодсветка Тогда
		Если СтрокаПорядкаВывода.ЭтоПродолжение Тогда
			Суффикс = НСтр("ru='(продолжение)';uk='(продовження)'");
	...
	Иначе
		Если СтрокаПорядкаВывода.ЭтоПродолжение Тогда
			ПодсистемаПредставление = ПодсистемаПредставление + " " + НСтр("ru='(продолжение)';uk='(продовження)'");
		КонецЕсли;
...
Функция ДобавитьЭлементыВариантаОтчета(ПараметрыЗаполнения, Вариант, ВГруппу, УровеньВложенности = 0)
	...
	Если ЗначениеЗаполнено(Вариант.Автор) Тогда
		Надпись.Подсказка = СокрЛ(Надпись.Подсказка + Символы.ПС) + НСтр("ru='Автор:';uk='Автор:'") + " " + СокрЛП(Строка(Вариант.Автор));
	КонецЕсли;
	...
	Если ПараметрыЗаполнения.ИспользоватьПодсветку Тогда
		...
		Если ПараметрыПодсветки <> Неопределено Тогда
			...
			Если ПараметрыПодсветки.ПредставлениеАвтора.КоличествоНайденныхСлов > 0 Тогда
				...
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ru='Автор:';uk='Автор:'") + " ");
			...
			Если ПараметрыПодсветки.НаименованияПользовательскихНастроек.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ru='Сохраненные настройки:';uk='Збережені настройки:'") + " ");
			...
			Если ПараметрыПодсветки.НаименованияПолей.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ru='Поля:';uk='Поля:'") + " ");
			...
			Если ПараметрыПодсветки.НаименованияПараметровИОтборов.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ru='Настройки:';uk='Настройки:'") + " ");
			...
			Если ПараметрыПодсветки.КлючевыеСлова.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("ru='Ключевые слова:';uk='Ключові слова:'") + " ");
