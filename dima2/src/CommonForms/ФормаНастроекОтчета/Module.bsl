...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("ВариантНаименование", ВариантНаименование) Тогда
		ВызватьИсключение НСтр("ru='Не передан служебный параметр ""ВариантНаименование"".';uk='Не переданий службовий параметр ""ВариантНаименование"".'");
	КонецЕсли;
	...
	Если ИдентификаторТекущегоУзлаКД <> Неопределено Тогда
		...
		Если Не Параметры.Свойство("Заголовок", Заголовок) Тогда
			ВызватьИсключение НСтр("ru='Не передан служебный параметр ""Заголовок"".';uk='Не переданий службовий параметр ""Заголовок"".'");
		КонецЕсли;
		Если Не Параметры.Свойство("ТипТекущегоУзлаКД", ТипТекущегоУзлаКД) Тогда
			ВызватьИсключение НСтр("ru='Не передан служебный параметр ""ТипТекущегоУзлаКД"".';uk='Не переданий службовий параметр ""ТипТекущегоУзлаКД"".'");
		КонецЕсли;
	Иначе
		Если Не ЗначениеЗаполнено(ВариантНаименование) Тогда
			ВариантНаименование = НастройкиОтчета.Наименование;
		КонецЕсли;
		Заголовок = НСтр("ru='Настройки отчета';uk='Настройки звіту'") + " """ + ВариантНаименование + """";
	КонецЕсли;
...
Процедура ПередЗагрузкойВариантаНаСервере(НовыеНастройкиКД)
	...
	Если НастройкиОтчета.События.ПередЗагрузкойНастроекВКомпоновщик Тогда
		НастройкиОтчета.Вставить("НовыеНастройкиКДБезСериализации", НовыеНастройкиКД);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ПодсказкаНетПользовательскихНастроекОбработкаНавигационнойСсылки(Элемент, Адрес, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
...
Процедура ПодсказкаЕстьВложенныеОтчетыОбработкаНавигационнойСсылки(Элемент, Адрес, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ПерейтиКНастройкеВложенныхОтчетов();
КонецПроцедуры
...
&НаКлиенте
Процедура Подключаемый_СписокСПодбором_Значение_АвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПоиска, Ожидание, СтандартнаяОбработка)
	...
	Иначе
		Для Каждого ЭлементСпискаДубльВФорме Из СписокЗначенийВФорме Цикл
			Если ЭлементСпискаДубльВФорме.Значение = Значение И ЭлементСпискаДубльВФорме <> ЭлементСпискаВФорме Тогда
				Состояние(НСтр("ru='Обнаружены дублирующиеся записи. Редактирование отменено.';uk='Виявлені записи, що дублюються. Редагування скасовано.'"));
...
Процедура Подключаемый_СписокСПодбором_ОбработкаВыбора(Элемент, РезультатВыбора, СтандартнаяОбработка)
	...
	Если Дополнение.Всего > 0 Тогда
		Если Дополнение.Всего = 1 Тогда
			ЗаголовокОповещения = НСтр("ru='Элемент добавлен в список';uk='Елемент доданий в список'");
		Иначе
			ЗаголовокОповещения = НСтр("ru='Элементы добавлены в список';uk='Елементи додані в список'");
		КонецЕсли;
...
Процедура ВыбранныеПоля_Сортировка_Вправо(Команда)
	...
	Если Не ЗарегистрироватьНачалоПеретаскивания("ВыбранныеПоля", ИсточникПеретаскивания) Тогда
		Возврат;
	КонецЕсли;
	ПеретаскиваниеВыбранныхПолейВСортировки(ИсточникПеретаскивания, Неопределено);
КонецПроцедуры
...
Процедура СтруктураВариантаВыбор(Элемент, СтрокаИдентификатор, Поле, СтандартнаяОбработка)
	...
	Если ИмяКолонки = "СтруктураВариантаПредставление"
		...
		Если СтрокаТаблицы.Тип = "Таблица"
			Или СтрокаТаблицы.Тип = "НастройкиВложенногоОбъекта" Тогда
			ПерейтиКНастройкеВложенныхОтчетов(СтрокаТаблицы.ИдентификаторКД);
			Возврат;
		КонецЕсли;
...
Процедура СтруктураВариантаДобавитьГруппировкуПослеВыбораПоля(ДоступноеПолеКД, ПараметрыВыполнения) Экспорт
	...
	Если ДоступноеПолеКД = "<>" Тогда
		...
		Представление = НСтр("ru='<Детальные записи>';uk='<Детальні записи>'");
	Иначе
	...
	Для Каждого ПеремещаемаяСтрока Из СтрокиПеремещаемыеВНовуюГруппировку Цикл
		Результат = ТаблицыПолейПереместить("СтруктураВарианта", ПеремещаемаяСтрока, СтрокаТаблицы);
	КонецЦикла;
...
Процедура ДобавитьТаблицуИлиДиаграмму(ТипЭлемента)
	...
	СтрокаТаблицы = Результат.СтрокаТаблицы;
	СтрокаТаблицы.Тип = ОтчетыКлиентСервер.ТипНастройкиСтрокой(ТипЭлемента);
	СтрокаТаблицы.Заголовок      = СтрокаТаблицы.Представление;
	СтрокаТаблицы.ФлажокДоступен = Истина;
	СтрокаТаблицы.Использование  = ЭлементКД.Использование;
	
	УзелКД = ТаблицыПолейНайтиУзел(ЭтотОбъект, "СтруктураВарианта");
	ВложенныеСтроки = СтрокаТаблицы.ПолучитьЭлементы();
	...
	Если СтрокаТаблицы.Тип = "Диаграмма" Тогда
		...
		СтрокаТаблицы.Представление = НСтр("ru='Диаграмма';uk='Діаграма'");
		СтрокаТаблицы.ИндексКартинки = 3;
		
		ВложенныйЭлементКД = ЭлементКД.Точки;
		ВложеннаяСтрока = ВложенныеСтроки.Добавить();
		ВложеннаяСтрока.Тип = ОтчетыКлиентСервер.ТипНастройкиСтрокой(ТипЗнч(ВложенныйЭлементКД));
		ВложеннаяСтрока.Подтип = "ДиаграммаТочки";
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ru='Точки';uk='Точки'");
		
		ВложенныйЭлементКД = ЭлементКД.Серии;
		ВложеннаяСтрока = ВложенныеСтроки.Добавить();
		ВложеннаяСтрока.Тип = ОтчетыКлиентСервер.ТипНастройкиСтрокой(ТипЗнч(ВложенныйЭлементКД));
		ВложеннаяСтрока.Подтип = "ДиаграммаСерии";
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ru='Серии';uk='Серії'");
		
	ИначеЕсли СтрокаТаблицы.Тип = "Таблица" Тогда
		
		СтрокаТаблицы.Представление = НСтр("ru='Таблица';uk='Таблиця'");
		СтрокаТаблицы.ИндексКартинки = 2;
		
		ВложенныйЭлементКД = ЭлементКД.Строки;
		ВложеннаяСтрока = ВложенныеСтроки.Добавить();
		ВложеннаяСтрока.Тип = ОтчетыКлиентСервер.ТипНастройкиСтрокой(ТипЗнч(ВложенныйЭлементКД));
		ВложеннаяСтрока.Подтип = "ТаблицаСтроки";
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ru='Строки';uk='Рядки'");
		
		ВложенныйЭлементКД = ЭлементКД.Колонки;
		ВложеннаяСтрока = ВложенныеСтроки.Добавить();
		ВложеннаяСтрока.Тип = ОтчетыКлиентСервер.ТипНастройкиСтрокой(ТипЗнч(ВложенныйЭлементКД));
		ВложеннаяСтрока.Подтип = "ТаблицаКолонки";
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ru='Колонки';uk='Колонки'");
...
Процедура СтруктураВариантаПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, ИдентификаторЦели, Поле)
	...
	Результат = ТаблицыПолейПереместить("СтруктураВарианта", СтрокаТаблицы, НовыйРодитель);
	
	Элементы.СтруктураВарианта.Развернуть(НовыйРодитель.ПолучитьИдентификатор(), Истина);
	Элементы.СтруктураВарианта.ТекущаяСтрока = Результат.СтрокаТаблицы.ПолучитьИдентификатор();
...
Процедура СтруктураВарианта_Переместить(Результат, ПараметрыВыполнения) Экспорт
	...
	Результат1 = ТаблицыПолейПереместить("СтруктураВарианта", СтрокаТаблицыВверх, СтрокаТаблицыВниз.ПолучитьРодителя(), СтрокаТаблицыВниз);
	СтрокаТаблицыВверх = Результат1.СтрокаТаблицы;
	...
	Для Каждого СтрокаТаблицы Из СтрокиВверх Цикл
		Результат2 = ТаблицыПолейПереместить("СтруктураВарианта", СтрокаТаблицы, СтрокаТаблицыВниз);
	КонецЦикла;
	Для Каждого СтрокаТаблицыИдентификатор Из МассивИдентификаторовСтрокВниз Цикл
		...
		Результат3 = ТаблицыПолейПереместить("СтруктураВарианта", СтрокаТаблицы, СтрокаТаблицыВверх);
	КонецЦикла;
	...
	КонецЕсли;
	Результат4 = ТаблицыПолейПереместить("СтруктураВарианта", СтрокаТаблицыВниз, СтрокаТаблицыВверх, ПередЧемВставить);
	СтрокаТаблицыВниз = Результат4.СтрокаТаблицы;
...
Процедура СтруктураВарианта_СохранитьВФайл(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ru='Настройки.xml';uk='Настройки.xml'"), Истина);
КонецПроцедуры
...
Процедура Оформление_ПереместитьВверх(Команда)
	...
	Сценарий.Вставить("Шаги", "ОпределитьВыделенныеСтроки, СдвинутьСтроки");
	Сценарий.Вставить("Действие", "Переместить");
	Сценарий.Вставить("ТаблицаИмя", "Оформление");
	Сценарий.Вставить("Направление", -1);
...
Функция ВыполнитьШаг(ВыборПользователя, Сценарий)
	...
	Иначе
		Сценарий.ПричинаОтказа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка в сценарии: Неизвестный шаг ""%1"".';uk='Помилка в сценарії: Невідомий крок ""%1"".'"), Сценарий.ТекущийШаг);
		Возврат Ложь;
...
Функция ШагОпределитьВыделенныеСтроки(ВыборПользователя, Сценарий)
	...
	Особенности = Новый Структура("МогутБытьРазделы, МогутБытьПараметры, МогутБытьГруппы,
	...
	Особенности.МогутБытьГруппы = (Сценарий.ТаблицаИмя = "Отборы" Или Сценарий.ТаблицаИмя = "ВыбранныеПоля");
	...
	Если Сценарий.СтрокиДерева.Количество() = 0 Тогда
		Сценарий.ПричинаОтказа = НСтр("ru='Выберите элементы.';uk='Оберіть елементи.'");
		Возврат Ложь; // Завершить сценарий.
	КонецЕсли;
	Если Сценарий.ТекущаяСтрока = Неопределено Тогда
		Если Сценарий.Действие = "ИзменитьГруппу" Тогда
			Сценарий.ПричинаОтказа = НСтр("ru='Выберите группу.';uk='Оберіть групу.'");
			Возврат Ложь; // Завершить сценарий.
		КонецЕсли;
	КонецЕсли;
	
	// Исключение из списка удаляемых строк всех подчиненных строк, для которых включены родители.
	Если Сценарий.Действие = "Удалить" И Особенности.МогутБытьГруппы И БылиГруппы Тогда
		Количество = Сценарий.СтрокиДерева.Количество();
		Для Номер = 1 По Количество Цикл
			ОбратныйИндекс = Количество - Номер;
			Родитель = Сценарий.СтрокиДерева[ОбратныйИндекс];
			Пока Родитель <> Неопределено Цикл
				Родитель = Родитель.ПолучитьРодителя();
				Если Сценарий.СтрокиДерева.Найти(Родитель) <> Неопределено Тогда
					Сценарий.СтрокиДерева.Удалить(ОбратныйИндекс);
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
...
Функция ЗарегистрироватьВыделеннуюСтроку(Сценарий, СтрокаДерева, Особенности)
	...
	Если Особенности.МогутБытьПараметры И СтрокаДерева.ЭтоПараметр Тогда
		Если Сценарий.Действие = "Переместить" Тогда
			Сценарий.ПричинаОтказа = НСтр("ru='Параметры не могут быть перемещены.';uk='Параметри не можуть бути переміщені.'");
		ИначеЕсли Сценарий.Действие = "Сгруппировать" Тогда
			Сценарий.ПричинаОтказа = НСтр("ru='Параметры не могут быть участниками групп.';uk='Параметри не можуть бути учасниками груп.'");
		ИначеЕсли Сценарий.Действие = "Удалить" Тогда
			Сценарий.ПричинаОтказа = НСтр("ru='Параметры не могут быть удалены.';uk='Параметри не можуть бути вилучені.'");
		КонецЕсли;
		Возврат Ложь; // Завершить сценарий.
	КонецЕсли;
	Если Особенности.ТребоватьОдногоРодителя Тогда
		...
		ИначеЕсли Сценарий.ТекущийРодитель <> Родитель Тогда
			Если Сценарий.Действие = "Переместить" Тогда
				Сценарий.ПричинаОтказа = НСтр("ru='Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.';uk='Вибрані елементи не можуть бути переміщені, оскільки вони належать різним батькам.'");
			ИначеЕсли Сценарий.Действие = "Сгруппировать" Тогда
				Сценарий.ПричинаОтказа = НСтр("ru='Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.';uk='Вибрані елементи не можуть бути згруповані, оскільки вони належать різним батькам.'");
			КонецЕсли;
			Возврат Ложь; // Завершить сценарий.
		КонецЕсли;
	КонецЕсли;
...
Процедура СтруктураВариантаИзменитьУзел(ТаблицаИмя, СтрокаИдентификатор, СтрокаТаблицы, ИмяСтраницы)
	...
	ШаблонЗаголовка = НСтр("ru='Настройка %1 отчета ""%2""';uk='Настройка %1 звіту ""%2""'");
	Если СтрокаТаблицы.Тип = "Диаграмма" Тогда
		ПредставлениеУзла = НСтр("ru='диаграммы';uk='діаграми'");
	Иначе
		ПредставлениеУзла = НСтр("ru='группировки';uk='групування'");
	КонецЕсли;
	Если ЗначениеЗаполнено(СтрокаТаблицы.Заголовок) Тогда
		ПредставлениеУзла = ПредставлениеУзла + " """ + СтрокаТаблицы.Заголовок + """";
	ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицы.Представление) Тогда
		ПредставлениеУзла = ПредставлениеУзла + " """ + СтрокаТаблицы.Представление + """";
	КонецЕсли;
	
	УзелКД = ТаблицыПолейНайтиУзел(ЭтотОбъект, ТаблицаИмя);
	ЭлементКД = НайтиОбъектКД(УзелКД, СтрокаТаблицы);
	ПолныйПуть = ОтчетыКлиентСервер.ПолныйПутьКЭлементу(Отчет.КомпоновщикНастроек.Настройки, ЭлементКД);
	...
	ПараметрыФормы.Вставить("ПолныйПутьТекущегоУзлаКД",    ПолныйПуть);
...
&НаКлиенте
Процедура ПерейтиКНастройкеВложенныхОтчетов(ИдентификаторТекущегоЭлементаКД = Неопределено)
	ПараметрыФормы = Новый Структура;
	...
	ПараметрыФормы.Вставить("ТекущаяСтрока",                         ИдентификаторТекущегоЭлементаКД);
	
	Обработчик = Новый ОписаниеОповещения("ПерейтиКНастройкеВложенныхОтчетовЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура ПерейтиКНастройкеВложенныхОтчетовЗавершение(Результат, ПараметрыВыполнения) Экспорт
...
Процедура ТаблицыПолейИзменитьУровеньДоступа(ТаблицаИмя, СтрокаИдентификатор, ПоказыватьРежимыВвода, ПоказыватьРежимыФлажков)
	...
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить(2, НСтр("ru='В шапке отчета';uk='В шапці звіту'"), , БиблиотекаКартинок.БыстрыйДоступ);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить(1, НСтр("ru='Только флажок в шапке отчета';uk='Тільки прапорець в шапці звіту'"), , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить(4, НСтр("ru='В настройках отчета';uk='В настройках звіту'"), , БиблиотекаКартинок.Реквизит);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить(3, НСтр("ru='Только флажок в настройках отчета';uk='Тільки прапорець в настройках звіту'"), , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;
	Список.Добавить(5, НСтр("ru='Не показывать';uk='Не показувати'"), , БиблиотекаКартинок.СкрытаяНастройкаОтчета);
...
Процедура ОформлениеИзменитьЭлемент(ТаблицаИмя, СтрокаИдентификатор, СтрокаТаблицы)
	...
	КонецЕсли;
	ПараметрыФормы.Вставить("Заголовок", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Элемент условного оформления отчета ""%1""';uk='Елемент умовного оформлення звіту ""%1""'"), ВариантНаименование));
...
Процедура ТаблицыПолейРазгруппировать(ТаблицаИмя)
	...
	Если ВыделенныеСтроки.Количество() <> 1 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выберите одну группу.';uk='Оберіть одну групу.'"));
		Возврат;
	КонецЕсли;
	...
	Если ГруппаДерева = Неопределено Или Не ГруппаДерева.ЭтоГруппа Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выберите группу.';uk='Оберіть групу.'"));
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтараяСтрокаДерева Из СтрокиРодителя Цикл
		...
		КонецЕсли;
		ВыделенныеСтроки.Добавить(ПеренесенныеСтроки.СтрокаДерева);
	КонецЦикла;
	...
	Для Каждого СтрокаДерева Из ВыделенныеСтроки Цикл
		ТаблицаЭлемент.ВыделенныеСтроки.Добавить(СтрокаДерева.ПолучитьИдентификатор());
	КонецЦикла;
...
Процедура УстановитьФлажкиИспользованиеВоВсемДереве(ТаблицаИмя, Использование, МассивСтрок = Неопределено, УзелКД = Неопределено)
	...
	Для Каждого СтрокаДерева Из МассивСтрок Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтрокаДерева, "ВыводитьФлажок") <> Ложь
			И ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтрокаДерева, "ЭтоРаздел") <> Истина Тогда
			ЭлементКД = УзелКД.ПолучитьОбъектПоИдентификатору(СтрокаДерева.ИдентификаторКД);
			Если ЭлементКД <> Неопределено Тогда
...
&НаКлиенте
Функция ТаблицыПолейПереместить(Знач ТаблицаИмя, Знач СтрокаТаблицы, Знач НовыйРодитель,
	Знач ПередЧемВставить = Неопределено, Знач Индекс = Неопределено, Знач ИндексКД = Неопределено)
	Результат = Новый Структура("СтрокаТаблицы, ЭлементКД, Индекс, ИндексКД");
	...
	Если ПередЧемВставить = Неопределено И ТаблицаИмя <> "СтруктураВарианта" Тогда
		...
		Если ЭтоРаздел <> Истина И ЭтоГруппа <> Истина Тогда // На один уровень с родителем.
	...
	Если ЭлементКД = ПередЧемВставитьКД Тогда
		Результат.ЭлементКД     = ЭлементКД;
		Результат.СтрокаТаблицы = СтрокаТаблицы;
	Иначе
		Если Индекс = Неопределено Или ИндексКД = Неопределено Тогда
			Если ПередЧемВставитьКД = Неопределено Тогда
				Если ДобавлятьВКонец Тогда
					Индекс   = КудаВставить.Количество();
					ИндексКД = КудаВставитьКД.Количество();
				Иначе
					Индекс   = 0;
					ИндексКД = 0;
				КонецЕсли;
			Иначе
				...
				Если СтарыйРодитель = НовыйРодитель Тогда
					СтарыйИндекс = ОткудаПереместить.Индекс(СтрокаТаблицы);
		...
		Результат.ЭлементКД = ОтчетыКлиентСервер.СкопироватьРекурсивно(УзелКД, ЭлементКД, КудаВставитьКД, ИндексКД, ПоискЭлементовКД);
		
		ПоискСтрокТаблицы = Новый Соответствие;
		Результат.СтрокаТаблицы = ОтчетыКлиентСервер.СкопироватьРекурсивно(Неопределено, СтрокаТаблицы, КудаВставить, Индекс, ПоискСтрокТаблицы);
	...
	Результат.Индекс   = КудаВставить.Индекс(Результат.СтрокаТаблицы);
	Результат.ИндексКД = КудаВставитьКД.Индекс(Результат.ЭлементКД);
...
Процедура СтруктураВариантаОбновитьЗаголовокЭлементаВКомпоновщике(СтрокаТаблицы)
	...
	Если ЭлементКД = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если ТипЗнч(ЭлементКД) = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда
...
Функция ТаблицыПолейНайтиУзел(ЭтотОбъект, ТаблицаИмя, СтрокаТаблицы = Неопределено, ИдентификаторУзлаКД = Неопределено)
	Если ЭтотОбъект.РасширенныйРежим = 1 Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Изменение узлов таблицы ""%1"" не поддерживается.';uk='Зміна вузлів таблиці ""%1"" не підтримується.'"), ТаблицаИмя);
		КонецЕсли;
	Иначе
...
Процедура ВидимостьДоступностьКорректность(ИмяСобытия = "")
	Если ИмяСобытия = "ПриСозданииНаСервере" Тогда
		Если РежимИзмененияУзлаВарианта Тогда
			...
			Элементы.ЗакрытьИСформировать.Заголовок = НСтр("ru='Завершить редактирование';uk='Завершити редагування'");
			Элементы.Закрыть.Заголовок = НСтр("ru='Отмена';uk='Відмінити'");
...
Процедура БыстрыеНастройкиЗагрузитьНастройкиВКомпоновщик(ПараметрыЗаполнения)
	...
	Если НастройкиОтчета.События.ПередЗагрузкойНастроекВКомпоновщик Тогда
		НовыеНастройкиКД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкиОтчета, "НовыеНастройкиКДБезСериализации"); 
	...
	Если ОбновитьНастройкиВарианта Тогда
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда
			СтандартныеПодсистемыКлиентСервер.ВывестиИнформациюОбОшибке(
				...
				НСтр("ru='Не удалось получить настройки отчета:';uk='Не вдалося отримати  настройки звіту:'"),
				ИнформацияОбОшибке);
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправленияИЗагрузитьЗначения(ПараметрыЗаполнения, Информация)
	...
	ГруппыВывода.Вставить("БыстрыеОтборы", Новый Структура("Порядок, Размер, ЕстьДлинныеПоля", Новый Массив, 0, Ложь));
	ГруппыВывода.Вставить("ОбычныеОтборы", Новый Структура("Порядок, Размер, ЕстьДлинныеПоля", Новый Массив, 0, Ложь));
	...
	Для Каждого СвойстваНастройки Из ВыводимыеНастройки Цикл
		...
		Иначе
			...
			КонецЕсли;
			Если СтрДлина(СвойстваНастройки.Представление) > 40 Тогда
				ГруппаВывода.ЕстьДлинныеПоля = Истина;
			КонецЕсли;
		КонецЕсли;
	...
	ГруппаВывода = ГруппыВывода.ОбычныеОтборы;
	КоличествоКолонок = 1;
	ОтчетыСервер.ВывестиПоПорядку(ЭтотОбъект, ГруппаВывода, Элементы.ОбычныеОтборы, КоличествоКолонок, Ложь);
...
Процедура РасширенныеНастройкиЗагрузитьЗначения(ПараметрыЗаполнения, Информация)
	...
	Если ВидимостьВыбранныхПолей И ВидимостьСортировок Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("ru='Поля и сортировки';uk='Поля і сортування'");
	ИначеЕсли ВидимостьВыбранныхПолей Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("ru='Поля';uk='Поля'");
	ИначеЕсли ВидимостьСортировок Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("ru='Сортировки';uk='Сортування'");
	КонецЕсли;
	...
	Иначе
		Элементы.СтраницаВыбранныеПоляИСортировки.Видимость    = ВидимостьВыбранныхПолей Или ВидимостьСортировок;
		Элементы.ВыбранныеПоля.Видимость                       = ВидимостьВыбранныхПолей;
		Элементы.Сортировка.Видимость                          = ВидимостьСортировок;
		Элементы.КомандыПолейИСортировок.Видимость             = ВидимостьВыбранныхПолей И ВидимостьСортировок;
	...
	Элементы.КомандыСортировки_ДобавлениеУдаление.Видимость  = (РасширенныйРежим = 1) Или (ВидимостьСортировок И Не ВидимостьВыбранныхПолей);
	Элементы.КомандыСортировки_ДобавлениеУдаление1.Видимость = (РасширенныйРежим = 1) Или (ВидимостьСортировок И Не ВидимостьВыбранныхПолей);
...
Функция ЗарегистрироватьЭлементСтруктурыОтчета(СтрокаДерева, СтрокиРодителя)
	...
	Если СтрокаФормы.Тип = "Настройки" Тогда
		...
		СтрокаФормы.Представление = НСтр("ru='Отчет';uk='Звіт'");
...
Функция ЗарегистрироватьЭлементыСортировки()
	...
	СтрокаРаздел.Представление = НСтр("ru='Сортировки';uk='Сортування'");
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоЭлементПорядка" Тогда
			СтрокаТаблицы.Представление = НСтр("ru='Авто (сортировки родителя)';uk='Авто (сортування батька)'");
...
Процедура ЗарегистрироватьЭлементыСоставаГруппировки(УзелКД = Неопределено, НаборСтрокКД = Неопределено, НаборСтрок = Неопределено)
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоПолеГруппировки" Тогда
			СтрокаТаблицы.Представление  = НСтр("ru='Авто (по всем полям)';uk='Авто (по всіх полях)'");
...
Функция ЗарегистрироватьЭлементыВыбранныхПолей(УзелКД = Неопределено, НаборСтрокКД = Неопределено, НаборСтрок = Неопределено, ДоступныеПоля = Неопределено)
	...
	Если НаборСтрок = Неопределено Тогда
		...
		СтрокаРаздел.ЭтоРаздел = Истина;
		СтрокаРаздел.Представление = НСтр("ru='Поля';uk='Поля'");
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоВыбранноеПоле" Тогда
			Представление = НСтр("ru='Авто (поля родителя)';uk='Авто (поля батька)'");
			ИндексКартинки = 6;
		Иначе
...
Процедура ЗарегистрироватьПараметры(Информация)
	...
	СтрокаРаздел.Представление = НСтр("ru='Параметры';uk='Параметри'");
...
Процедура ЗарегистрироватьЭлементыОтборов(Информация, НаборСтрокВарианта = Неопределено, НаборСтрокФормы = Неопределено)
	...
	Если НаборСтрокФормы = Неопределено Тогда
		...
		СтрокаРаздел.ЭтоРаздел = Истина;
		СтрокаРаздел.Представление = НСтр("ru='Отборы';uk='Відбори'");
	...
	Для Каждого НастройкаВарианта Из НаборСтрокВарианта Цикл
		...
		СтрокаТаблицы.Представление   = НастройкаВарианта.ПредставлениеПоУмолчанию;
		СтрокаТаблицы.Заголовок       = НастройкаВарианта.Представление;
		СтрокаТаблицы.ИдентификаторКД = НастройкаВарианта.ИдентификаторКД;
		...
		Если ЗначениеЗаполнено(ЭлементКД.ИдентификаторПользовательскойНастройки) Тогда
			Если ЭлементКД.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ Тогда
				СтрокаТаблицы.ИндексКартинкиДоступа = ?(НастройкаВарианта.ВыводитьТолькоФлажок, 1, 2);
			ИначеЕсли ЭлементКД.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Обычный Тогда
				СтрокаТаблицы.ИндексКартинкиДоступа = ?(НастройкаВарианта.ВыводитьТолькоФлажок, 3, 4);
			Иначе
				СтрокаТаблицы.ИндексКартинкиДоступа = 5;
			КонецЕсли;
		Иначе
...
Процедура ПеретаскиваниеВПределахТаблицы(ИсточникПеретаскивания, ИдентификаторЦели)
	...
	Для Каждого ПеремещаемаяСтрока Из ПеремещаемыеСтроки Цикл
		...
		// Перемещение.
		Результат = ТаблицыПолейПереместить(ТаблицаИмя, ПеремещаемаяСтрока, СтрокаЦели);
		
		// Бантики (начало).
		Если ЭтоТекущая Тогда
			ТекущаяСтрока = Результат.СтрокаТаблицы;
		КонецЕсли;
		ПеремещенныеСтроки.Добавить(Результат.СтрокаТаблицы);
	КонецЦикла;
	...
	Для Каждого СтрокаТаблицы Из ПеремещенныеСтроки Цикл
		...
		Если ТаблицаЭлемент.ВыделенныеСтроки.Найти(НоваяСтрокаИдентификатор) = Неопределено Тогда
			ТаблицаЭлемент.ВыделенныеСтроки.Добавить(НоваяСтрокаИдентификатор);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПеретаскиваниеВыбранныхПолейВСортировки(ИсточникПеретаскивания, ИдентификаторЦели)
	...
	Если ПередЧемВставить <> Неопределено И ПередЧемВставить.ЭтоРаздел Тогда
		// Цель перетаскивания - раздел, добавляем внутрь раздела.
		ЦельРодитель     = ПередЧемВставить;
		ПередЧемВставить = Неопределено;
		ДобавлятьВКонец  = Ложь;
	Иначе
		// На один уровень с целью перетаскивания.
		ЦельРодитель   = ПолучитьРодителя(ЦельТаблицаИмя, ПередЧемВставить);
		ДобавлятьВКонец = Истина;
	КонецЕсли;
	
	КудаВставить = ПолучитьЭлементы(ЦельТаблица, ЦельРодитель);
	
	Если ПередЧемВставить = Неопределено Тогда
		Количество = КудаВставить.Количество();
		Если Количество > 0 И Не ДобавлятьВКонец Тогда
			ПередЧемВставить = КудаВставить[0];
		КонецЕсли;
	КонецЕсли;
	...
	БыстрыйПоискСтрокиПоПолюКД = Новый Соответствие;
	Для Каждого СтрокаТаблицы Из КудаВставить Цикл
		ЭлементКД = НайтиОбъектКД(ЦельУзелКД, СтрокаТаблицы);
		Если ЭлементКД <> Неопределено Тогда
			БыстрыйПоискСтрокиПоПолюКД.Вставить(ЭлементКД.Поле, СтрокаТаблицы);
		КонецЕсли;
	КонецЦикла;
	...
	Пока Истина Цикл
		...
		Для Каждого ИсточникСтрокаТаблицы Из ТекущаяКоллекция Цикл
			...
			ПолеКД = ИсточникОбъектКД.Поле;
			
			ДоступноеПолеКД = ЦельУзелКД.ДоступныеПоляПорядка.НайтиПоле(ПолеКД);
			Если ДоступноеПолеКД = Неопределено Тогда
				Продолжить; // Не удалось добавить поле.
			КонецЕсли;
			
			НайденнаяСтрока = БыстрыйПоискСтрокиПоПолюКД.Получить(ПолеКД);
			Если НайденнаяСтрока <> Неопределено Тогда
				Результат = ТаблицыПолейПереместить(ЦельТаблицаИмя, НайденнаяСтрока, ЦельРодитель, , Индекс, ИндексКД);
				НоваяСтрока = Результат.СтрокаТаблицы;
				Индекс   = Результат.Индекс + 1;
				ИндексКД = Результат.ИндексКД + 1;
				ПеремещенныеСтроки.Добавить(НоваяСтрока);
				БыстрыйПоискСтрокиПоПолюКД.Вставить(ПолеКД, НоваяСтрока);
				Продолжить; // Строка перемещена, добавлять новую не требуется.
			КонецЕсли;
			
			НовыйЭлементКД = КудаВставитьКД.Вставить(ИндексКД, ТипЭлемента);
			НовыйЭлементКД.Использование     = Истина;
			НовыйЭлементКД.Поле              = ПолеКД;
			...
			НоваяСтрока.ТипНастройки    = "ЭлементПорядка";
