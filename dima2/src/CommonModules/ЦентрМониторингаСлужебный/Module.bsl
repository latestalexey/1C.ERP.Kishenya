	...
	Если (ПараметрыЦентраМониторинга.ВключитьЦентрМониторинга ИЛИ ПараметрыЦентраМониторинга.ЦентрОбработкиИнформацииОПрограмме) И ЭтоГлавныйУзел() Тогда 
		Если ПараметрыЦентраМониторинга.РегистрироватьДампы И ДатаЗапуска >= ПараметрыЦентраМониторинга.РегистрацияДамповСледующееФормирование Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Центр мониторинга - регистрация дампов';uk='Центр моніторингу - реєстрація дампів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		Если ПараметрыЦентраМониторинга.РегистрироватьБизнесСтатистику И ДатаЗапуска >= ПараметрыЦентраМониторинга.БизнесСтатистикаСледующееФормирование Тогда
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Центр мониторинга - регистрация операций статистики';uk='Центр моніторингу - реєстрація операцій статистики'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					Комментарий);
		...
		Если (ПараметрыЦентраМониторинга.РегистрироватьСтатистикуКонфигурации ИЛИ ПараметрыЦентраМониторинга.РегистрироватьНастройкуКонфигурации) И ДатаЗапуска >= ПараметрыЦентраМониторинга.СтатистикаКонфигурацииСледующееФормирование Тогда
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Центр мониторинга - собрать статистику конфигурации';uk='Центр моніторингу - зібрати статистику конфігурації'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					Комментарий);
		...
		Если ДатаЗапуска >= ПараметрыЦентраМониторинга.ОтправкаДанныхСледующееФормирование Тогда
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Центр мониторинга - сформировать пакет для отправки';uk='Центр моніторингу - сформувати пакет для відправлення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					Комментарий);
			...
			Исключение
				Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Центр мониторинга - отправить данные мониторинга';uk='Центр моніторингу - відправити дані моніторингу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					Комментарий);
...
Процедура АгрегироватьЗамерыОперацийСтатистики(ТекущаяДата)
	...
	Если ОбработатьЗаписиДо > ГраницаАгрегирования Тогда
		...
		Исключение
			ОтменитьТранзакцию();
			Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Агрегировать замеры операций статистики';uk='Центр моніторингу.Агрегувати виміри операцій статистики'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
			ВызватьИсключение Ошибка;
		КонецПопытки;
	КонецЕсли;
...
Процедура УдалитьУстаревшиеДанныеОперацийСтатистики()
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Удалить устаревшие данные операций статистики';uk='Центр моніторингу. Вилучити застарілі дані операцій статистики'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
			...
			Пока Выборка.Следующий() Цикл
				...
				Если РегистрыСведений.ОбластиСтатистики.СобиратьСтатистикуКонфигурации(ОбластьДанныхСтрока) Тогда
					...
					Исключение
						...
						ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Статистика конфигурации переопределяемая';uk='Центр моніторингу.Статистика конфігурації переопределяемая'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
						УровеньЖурналаРегистрации.Ошибка,
						,,
						НСтр("ru='Не удалось установить разделение сеанса.Область данных';uk='Не вдалося встановити поділ сеансу.Область даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + " = " + Формат(Выборка.ОбластьДанных, "ЧГ=0")
...
Процедура СформироватьПакетДляОтправки()
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Формирование пакета для отправки';uk='Центр моніторингу.Формування пакету для відправки'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
...
Функция ПолучитьПараметрыПоУмолчанию()
	...
	ПараметрыКонстанты.Вставить("Сервер", "pult.1c.eu");
...
Функция УстановитьПараметрыОтправки(Параметры)
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Установка параметров отправки';uk='Центр моніторингу.Установка параметрів відправлення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
...
Функция УстановитьПараметрыЦентраМониторинга(НовыеПараметры)
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Установка параметров центра мониторинга';uk='Центр моніторингу.Установка параметрів центру моніторингу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
КонецФункции
...
Функция УдалитьПараметрыЦентраМониторинга()
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Удалить параметры центра мониторинга';uk='Центр моніторингу. Вилучити настройки центру моніторингу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
КонецФункции
...
Функция НайтиКаталогДампов(КаталогНастроек)
	...
	Если Файл.Существует() Тогда
		Попытка
			...
			КонецЦикла;
		Исключение
			Сообщение = НСтр("ru='Ошибка чтения файла настроек технологического журнала';uk='Помилка читання файлу настройок технологічного журналу'");
			Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """.";
			Сообщение = Сообщение + НСтр("ru='Скорее всего файл поврежден. Регистрация дампов не возможна. Удалите поврежденный файл или восстановите настройки.';uk='Швидше всього файл пошкоджений. Реєстрація дампів не можлива. Видаліть пошкоджений файл або відновіть настройки.'");
			...
			Если ДокументDOM.ЕстьДочерниеУзлы() Тогда
				...
				Если ВРЕГ(ПервыйДочерний.ИмяУзла) = "CONFIG" Тогда
					...
					Попытка
						...
						ЗаписьXML.Закрыть();
					Исключение
						Сообщение = НСтр("ru='Ошибка записи файла настроек технологического журнала';uk='Помилка запису файла налаштувань технологічного журналу'");
						Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """.";
...
Функция ПолучитьКаталогНастроекТехнологическогоЖурнала()
	...
	Пока ПоискКаталога = Истина Цикл
		...
		Если КаталогиНастроек.Найти(ПутьКаталога) <> Неопределено Тогда
			...
			КаталогНастроек.ОписаниеОшибки = НСтр("ru='Обнаружена циклическая ссылка';uk='Виявлено циклічне посилання'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Иначе
			...
			Иначе
				...
				Иначе
					КаталогНастроек.Путь = "";
					КаталогНастроек.Существует = Ложь;
					КаталогНастроек.ОписаниеОшибки = НСтр("ru='Не найден файл конфигурации настроек в каталоге';uk='Не знайдено файл конфігурації настройок в каталозі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + " " + ПутьКаталога;
...
Функция ПолучитьКаталогИзФайлаКонфигурацииНастроек(ФайлКонфигурацииНастроек)
	...
	Иначе
		...
		КаталогНастроек.Существует = Ложь;
		КаталогНастроек.ОписаниеОшибки = НСтр("ru='Не найдена секция ConfLocation в файле';uk='Не знайдена секція ConfLocation у файлі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + " " + ФайлКонфигурацииНастроек.ПолноеИмя;
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииЦентрМониторингаУдалениеДампа()
	Возврат НСтр("ru='Центр мониторинга.Удаление дампа';uk='Центр моніторингу.Вилучення дампа'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции

Функция СобытиеЖурналаРегистрацииЦентрМониторингаРазобратьБуферОперацийСтатистики()
	Возврат НСтр("ru='Центр мониторинга.Разобрать буфер операций статистики';uk='Центр моніторингу.Розібрати буфер операцій статистики'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
#КонецОбласти
