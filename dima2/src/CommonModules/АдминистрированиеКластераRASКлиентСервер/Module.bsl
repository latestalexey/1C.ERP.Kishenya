...
// - Реализация программного администрирования кластера серверов через сервер
//   администрирования.
//
////////////////////////////////////////////////////////////////////////////////
...
Процедура ПроверитьПараметрыАдминистрирования(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ = Неопределено,
	...
	Если ПроверятьПараметрыАдминистрированияКластера Или ПроверятьПараметрыАдминистрированияИнформационнойБазы Тогда
		...
		ИдентификаторКластера = ПолучитьКластер(ПараметрыАдминистрированияКластера);
		РабочиеПроцессы = ПолучитьРабочиеПроцессы(ИдентификаторКластера, ПараметрыАдминистрированияКластера);
...
Функция СеансыИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	ИдентификаторКластера = ПолучитьКластер(ПараметрыАдминистрированияКластера);
	ИдентификаторИнформационнойБазы = ПолучитьИнформационнуюБазу(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ);
	Возврат ПолучитьСеансы(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИдентификаторИнформационнойБазы, Фильтр);
...
Процедура УдалитьСеансыИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	Для ТекущаяПопытка = 0 По КоличествоПопыток Цикл
		...
		Сеансы = ПолучитьСеансы(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИдентификаторИнформационнойБазы, Фильтр, Ложь);
	...
	Если НЕ ВсеСеансыУдалены Тогда
		...
		ВызватьИсключение НСтр("ru='Не удалось удалить сеансы.';uk='Не вдалося вилучити сеанси.'");
...
Функция СоединенияСИнформационнойБазой(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	ИдентификаторКластера = ПолучитьКластер(ПараметрыАдминистрированияКластера);
	ИдентификаторИнформационнойБазы = ПолучитьИнформационнуюБазу(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ПараметрыАдминистрированияИБ);
	Возврат ПолучитьСоединения(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИдентификаторИнформационнойБазы, ПараметрыАдминистрированияИБ, Фильтр, Истина);
...
Процедура РазорватьСоединенияСИнформационнойБазой(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	Значение.Добавить("1CV8");               // идентификатор приложения платформы в режиме запуска "Толстый клиент"
	Значение.Добавить("1CV8C");              // идентификатор приложения платформы в режиме запуска "Тонкий клиент"
	Значение.Добавить("WebClient");          // идентификатор приложения платформы в режиме запуска "Веб-клиент"
	Значение.Добавить("Designer");           // идентификатор приложения Конфигуратор
	Значение.Добавить("COMConnection");      // идентификатор сессии внешнего соединения платформы через COM
	...
	Для ТекущаяПопытка = 0 По КоличествоПопыток Цикл
		...
		Соединения = ПолучитьСоединения(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИдентификаторИнформационнойБазы, ПараметрыАдминистрированияИБ, Фильтр, Ложь);
	...
	Если НЕ ВсеСоединенияРазорваны Тогда
		...
		ВызватьИсключение НСтр("ru='Не удалось разорвать соединения.';uk='Не вдалося розірвати з''єднання.'");
...
Функция ПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач ИмяПрофиля) Экспорт
	...
	Если ПрофилиБезопасности.Количество() <> 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В кластере серверов %1 не зарегистрирован профиль безопасности %2!';uk='В кластері серверів %1 не зареєстрований профіль безпеки %2!'"), ИдентификаторКластера, ИмяПрофиля);
	КонецЕсли;
...
Процедура СоздатьПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач СвойстваПрофиляБезопасности) Экспорт
	...
	Если ПрофилиБезопасности.Количество() = 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В кластере серверов %1 уже зарегистрирован профиль безопасности %2!';uk='В кластері серверів %1 уже зареєстрований профіль безпеки %2!'"), ИдентификаторКластера, ИмяПрофиля);
	КонецЕсли;
...
//
Функция ПолучитьИнформационнуюБазу(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИнформационнойБазы) Экспорт
	
	Фильтр = Новый Структура("name", ПараметрыАдминистрированияИнформационнойБазы.ИмяВКластере);
	
	ИнформационныеБазы = ПолучитьИнформационныеБазы(ИдентификаторКластера, ПараметрыАдминистрированияКластера, Фильтр);
	
	Если ИнформационныеБазы.Количество() = 1 Тогда
		Возврат ИнформационныеБазы[0].Получить("infobase");
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В кластере серверов %1 не зарегистрирована информационная база %2!';uk='В кластері серверів %1 не зареєстрована інформаційна база %2!'"), ИдентификаторКластера, ПараметрыАдминистрированияИнформационнойБазы.ИмяВКластере);
	КонецЕсли;
...
//
Функция ПолучитьИнформационныеБазы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач Фильтр = Неопределено) Экспорт
...
Функция ПолучитьКластер(Знач ПараметрыАдминистрированияКластера) Экспорт
	...
	Кластеры = ПолучитьКластеры(ПараметрыАдминистрированияКластера, Фильтр);
	
	Если Кластеры.Количество() = 1 Тогда
		Возврат Кластеры[0].Получить("cluster");
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не обнаружен кластер серверов с портом %1';uk='Не виявлений кластер серверів з портом %1'"), ПараметрыАдминистрированияКластера.ПортКластера);
	КонецЕсли;
...
// Возвращаемое значение: Массив(Структура).
//
Функция ПолучитьРабочиеПроцессы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач Фильтр = Неопределено) Экспорт
	...
	РабочиеСерверы = ПолучитьРабочиеСерверы(ИдентификаторКластера, ПараметрыАдминистрированияКластера);
...
// Возвращаемое значение: Массив(Структура).
//
Функция ПолучитьРабочиеСерверы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач Фильтр = Неопределено) Экспорт
...
//
Функция ПолучитьСеансы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ИдентификаторИнформационнойБазы, Знач Фильтр = Неопределено, Знач ИспользоватьСловарь = Истина) Экспорт
...
//
Функция ПолучитьСоединения(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ИдентификаторИнформационнойБазы, Знач ПараметрыАдминистрированияИнформационнойБазы, Знач Фильтр = Неопределено, Знач ИспользоватьСловарь = Ложь) Экспорт
...
//
Функция ПолучитьКлиентСервераАдминистрирования() Экспорт
...
Функция ПолучитьВиртуальныеКаталоги(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ИмяПрофиля, Знач Фильтр = Неопределено)
	...
	Возврат ПолучитьСписокКонтроляДоступа(
...
// Возвращаемое значение: Массив(Структура).
//
Функция ПолучитьСписокКонтроляДоступа(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ИмяПрофиля, Знач ИмяСписка, Знач Словарь, Знач Фильтр = Неопределено)
...
Процедура ОбновитьСвойстваПрофиляБезопасности(Знач ПараметрыАдминистрированияКластера, Знач СвойстваПрофиляБезопасности, Знач ОчиститьСпискиКонтроляДоступа)
	...
	Если ОчиститьСпискиКонтроляДоступа Тогда
		УдаляемыеВиртуальныеКаталоги = ПолучитьСписокКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, ИмяСписка, ТекущийСловарь);
	...
	Если ОчиститьСпискиКонтроляДоступа Тогда
		УдаляемыеCOMКлассы = ПолучитьСписокКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, ИмяСписка, ТекущийСловарь);
	...
	Если ОчиститьСпискиКонтроляДоступа Тогда
		УдаляемыеВнешниеКомпоненты = ПолучитьСписокКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, ИмяСписка, ТекущийСловарь);
	...
	Если ОчиститьСпискиКонтроляДоступа Тогда
		УдаляемыеВнешниеМодули = ПолучитьСписокКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, ИмяСписка, ТекущийСловарь);
	...
	Если ОчиститьСпискиКонтроляДоступа Тогда
		УдаляемыеПриложенияОС = ПолучитьСписокКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, ИмяСписка, ТекущийСловарь);
	...
	Если ОчиститьСпискиКонтроляДоступа Тогда
		УдаляемыеИнтернетРесурсы = ПолучитьСписокКонтроляДоступа(ИдентификаторКластера, ПараметрыАдминистрированияКластера, ИмяПрофиля, ИмяСписка, ТекущийСловарь);
...
Функция ВыполнитьКоманду(Знач Шаблон, Знач ПараметрыАдминистрированияКластера, Знач ЗначенияПараметров = Неопределено)
	...
	#Если Сервер Тогда
		...
		Если БезопасныйРежим() <> Ложь Тогда
			ВызватьИсключение НСтр("ru='Администрирование кластера невозможно в безопасном режиме!';uk='Адміністрування кластера неможливо в безпечному режимі!'");
		КонецЕсли;
		
		Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			ВызватьИсключение НСтр("ru='В модели сервиса недопустимо выполнение прикладной информационной базой функций администрирования кластера!';uk='У моделі сервісу неприпустиме виконання прикладною інформаційною базою функцій адміністрування кластера!'");
		КонецЕсли;
	...
	Клиент = ПолучитьКлиентСервераАдминистрирования();
	...
	Если Не ФайлКлиента.Существует() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Невозможно выполнить операцию администрирования кластера серверов по причине: файл %1 не найден!
                  ...
                  |Для его установки:
                  |- Для компьютеров с ОС Windows требуется переустановить платформу, установив компонент ""Администрирование сервера платформы"";
                  |- Для компьютеров с ОС Linux требуется установить пакет 1c-enterprise83-server*.'
                  |;uk='Неможливо виконати операцію адміністрування кластеру серверів через: файл %1 не знайдений!
                  |
                  |Для адміністрування кластеру через сервер адміністрування (ras) потрібно встановити на даному
                  |комп''ютері клієнт сервера адміністрування (rac).
                  |Для його установки:
                  |- Для комп''ютерів з ОС Windows потрібно перевстановити платформу, встановивши компонент ""Адміністрування сервера платформи"";
                  |- Для комп''ютерів з ОС Linux потрібно встановити пакет 1c-enterprise83-server*.'"),
...
Функция ПарсерВывода(Знач ПотокВывода, Знач Словарь, Знач Фильтр = Неопределено)
	...
	Для Шаг = 1 По РазмерВывода Цикл
		...
		Если ПоложениеРазделителя > 0 Тогда
			...
			Если СвойстваЭкранируемыеКавычками().Найти(ИмяСвойства) <> Неопределено Тогда
...
Функция СвойстваЭкранируемыеКавычками()
	...
	Результат.Добавить("denied-message");
	Результат.Добавить("permission-code");
	Результат.Добавить("denied-parameter");
