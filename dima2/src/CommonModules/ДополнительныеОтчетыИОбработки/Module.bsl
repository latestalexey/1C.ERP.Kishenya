...
Функция ПодключитьВнешнююОбработку(Ссылка) Экспорт
	...
	ЗаписатьПримечание(Ссылка, НСтр("ru='Подключение, БезопасныйРежим = ""%1"".';uk='Підключення, БезопасныйРежим = ""%1"".'"), БезопасныйРежим);
...
Процедура ПечатьПоВнешнемуИсточнику(ДополнительнаяОбработкаСсылка, ПараметрыИсточника, КоллекцияПечатныхФорм,
	...
	Если ВнешняяОбработкаОбъект = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Внешняя обработка ""%1"" (тип ""%2"") не обслуживается подсистемой ""%3""';uk='Зовнішня обробка ""%1"" (тип ""%2"") не обслуговується підсистемою ""%3""'"),
...
//             Запрещены внешние по отношению к платформе действия:
//               COM;
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление = НСтр("ru='Дополнительные отчеты и обработки';uk='Додаткові звіти й обробки'");
...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	ВидДоступа.Представление = НСтр("ru='Дополнительные отчеты и обработки';uk='Додаткові звіти й обробки'");
...
Процедура ПриПодключенииДопОтчета(Ссылка, ПараметрыОтчета, Результат, ПолучатьМетаданные) Экспорт
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		ПараметрыОтчета.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Елемент ""%1"" не підключений, тому що підсистема ""%2"" відключена у настройках програми.
            |Для включення підсистеми зверніться до адміністратора програми.'"),
			"'"+ Строка(Ссылка) +"'",
			ДополнительныеОтчетыИОбработкиКлиентСервер.НаименованиеПодсистемы(Истина));
		Возврат;
	КонецЕсли;
	...
	Если Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет
		...
		Исключение
			ПараметрыОтчета.ТекстОшибки = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='При подключении дополнительного отчета ""%1"" возникла ошибка:';uk='При підключенні додаткового звіту ""%1"" помилка:'"), Строка(Ссылка))
	...
	Иначе
		...
		ПараметрыОтчета.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Элемент %1 не является дополнительным отчетом';uk='Елемент %1 не є додатковим звітом'"),
			"'"+ Строка(Ссылка) +"'");
...
Процедура ПриПодключенииОтчета(Контекст) Экспорт
	...
	Если ТипЗнч(Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Тогда
		Текст    = НСтр("ru='Не указан отчет';uk='Не зазначено звіт'");
		Подробно = НСтр("ru='В процедуру ""ДополнительныеОтчетыИОбработки.ПриПодключенииОтчета"" не передан отчет.';uk='В процедуру ""ДополнительныеОтчетыИОбработки.ПриПодключенииОтчета"" не передано звіт.'");
		СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(Контекст, Текст, Подробно);
		Возврат;
	КонецЕсли;
	
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		Текст    = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Отчет ""%1"" не подключен';uk='Звіт ""%1"" не підключений'"), Строка(Ссылка));
		Подробно = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Подсистема ""%2"" отключена в настройках программы.
            |Для включения подсистемы обратитесь к администратору программы.'
            |;uk='Підсистема ""%2"" відключена в настройках програми.
            |Для включення підсистеми зверніться до адміністратора програми.'"),
			ДополнительныеОтчетыИОбработкиКлиентСервер.НаименованиеПодсистемы(Истина));
		СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(Контекст, Текст, Подробно);
		Возврат;
	...
	Иначе
		...
		СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(Контекст,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Объект ""%1"" не является дополнительным отчетом';uk='Об''єкт ""%1"" не є додатковим звітом'"), Строка(Ссылка)));
...
Процедура ПриПолученииПрочихНастроек(СведенияОПользователе, Настройки) Экспорт
	...
	НазваниеНастройки = НСтр("ru='Настройки быстрого доступа к дополнительным отчетам и обработкам';uk='Настройки швидкого доступу до додаткових звітів й обробок'");
...
Функция ПодключенныеОбъектыМетаданных(Вид) Экспорт
	...
	Для Каждого ТипИлиМетаданные Из МассивТиповИлиМетаданных Цикл
		...
		Если ОбъектМетаданных = ДополнительныеОтчетыИОбработкиКлиентСервер.ИдентификаторРабочегоСтола() Тогда
			...
			СтрокаТаблицы.Представление = НСтр("ru='Рабочий стол';uk='Робочий стіл'");
		Иначе
			...
			Если Не Выборка.ЭтоГруппа И Не ЗначениеЗаполнено(Выборка.РежимСовместимостиРазрешений) Тогда
				...
				Исключение
					...
					ТекстОшибки = """" + Объект.Наименование + """:"
						+ Символы.ПС + НСтр("ru='Не удалось определить режим совместимости разрешений по причине:';uk='Не вдалося визначити режим сумісності дозволів через:'")
						+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
						+ Символы.ПС
						+ Символы.ПС + НСтр("ru='Объект заблокирован в режиме совместимости с версией 2.1.3.';uk='Об''єкт заблокований в режимі сумісності з версією 2.1.3.'");
...
Процедура ВыполнитьОбработкуПоРегламентномуЗаданию(ВнешняяОбработка, ИдентификаторКоманды) Экспорт
	...
	ЗаписатьИнформацию(ВнешняяОбработка, НСтр("ru='Команда %1: Запуск.';uk='Команда %1: Запуск.'"), ИдентификаторКоманды);
	...
	Исключение
		ЗаписатьОшибку(
			ВнешняяОбработка,
			НСтр("ru='Команда %1: Ошибка выполнения:%2';uk='Команда %1: Помилка виконання:%2'"),
			ИдентификаторКоманды,
			Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	
	// Запись журнала регистрации
	ЗаписатьИнформацию(ВнешняяОбработка, НСтр("ru='Команда %1: Завершение.';uk='Команда %1: Завершення.'"), ИдентификаторКоманды);
...
Функция ПредставлениеРаздела(Раздел) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Раздел ""%1""';uk='Розділ ""%1""'"), ПредставлениеРаздела);
КонецФункции
...
Процедура СформироватьПодменюКомандЗаполнения(Форма, Параметры)
	...
	Если Подменю = Неопределено Тогда
		...
		Подменю.Заголовок = НСтр("ru='Заполнить';uk='Заповнити'");
...
Функция ВыполнитьКомандуВнешнегоОбъекта(ВнешнийОбъект, ИдентификаторКоманды, ПараметрыКоманды, АдресРезультата)
	...
	Если ОписаниеКоманды = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Команда %1 не обнаружена!';uk='Команда %1 не виявлена!'"), ИдентификаторКоманды);
...
Функция ЗарегистрироватьОбработку(Знач Объект, Знач ПараметрыРегистрации) Экспорт
	...
	Если РегистрационныеДанные.БезопасныйРежим
		...
		// ничего не делаем
	Иначе
		Результат.ТекстОшибки = НСтр("ru='Для подключения обработки, запускаемой в небезопасном режиме, требуются административные права.';uk='Для підключення обробки, що запускається в небезпечному режимі, потрібні адміністративні права.'");
		Возврат Результат;
	КонецЕсли;
		...
		Результат.ТекстОшибки = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Вид загружаемого объекта (%1) не соответствует текущему (%2).
                    ...
                    |;uk='Вид об''єкта, що завантажується (%1) не відповідає поточному (%2).
                    |Для завантаження нового об''єкта натисніть ""Створити"".'"),
	...
	ИначеЕсли ПараметрыРегистрации.ЭтоОтчет <> (РегистрационныеДанные.Вид = ВидДополнительныйОтчет ИЛИ РегистрационныеДанные.Вид = ВидОтчет) Тогда
		Результат.ТекстОшибки = НСтр("ru='Вид обработки, указанный в сведениях о внешней обработке, не соответствует ее расширению.';uk='Вид обробки, зазначений у відомостях про зовнішню обробку, не відповідає її розширенню.'");
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого КомандаОписание Из РегистрационныеДанные.Команды Цикл
		...
		Если НЕ ЗначениеЗаполнено(КомандаОписание.ВариантЗапуска) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для команды ""%1"" не определен способ запуска.';uk='Для команди ""%1"" не визначений спосіб запуску.'"), КомандаОписание.Представление));
		КонецЕсли;
...
Процедура ПриПолученииРегистрационныхДанных(Объект, РегистрационныеДанные, ПараметрыРегистрации, РезультатРегистрации)
	...
	Если ИнформацияОбОшибке <> Неопределено Тогда
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
            ...
            |;uk='Неможливо підключити додатковий звіт з файлу.
            |Можливо, він не підходить для цієї версії програми.'");
		Иначе
			ТекстОшибки = НСтр("ru='Невозможно подключить дополнительную обработку из файла.
            |Возможно, она не подходит для этой версии программы.'
            |;uk='Неможливо підключити додаткову обробку з файлу.
            |Можливо, вона не підходить для цієї версії програми.'");
		КонецЕсли;
		...
		ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр("ru='Техническая информация:';uk='Технічна інформація:'") + Символы.ПС;
