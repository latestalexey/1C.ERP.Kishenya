...
Функция ПоставляемаяОбработка(ИспользуемаяОбработка) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ВызватьИсключение НСтр("ru='Использование функции ДополнительныеОтчетыИОбработкиВМоделиСервиса.ПоставляемаяОбработка()
                                ...
                                |;uk='Використання функції ДополнительныеОтчетыИОбработкиВМоделисервиса.ПоставляемаяОбработка()
                                |доступне тільки для сеансів з установленим поділом даних!'");
...
Функция ИспользуемаяОбработка(ПоставляемаяОбработка) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ВызватьИсключение НСтр("ru='Использование функции ДополнительныеОтчетыИОбработкиВМоделиСервиса.ИспользуемаяОбработка()
                                |доступно только сеансов с установленным разделением данных!'
                                |;uk='Використання функції ДополнительныеОтчетыИОбработкиВМоделисервиса.ИспользуемаяОбработка()
                                |доступне тільки для сеансів з установленим поділом даних!'");
...
Процедура УстановитьПоставляемуюОбработкуВОбластьДанных(Знач ОписаниеИнсталляции, Знач БыстрыйДоступ, Знач Задания, Знач Разделы, Знач СправочникиИДокументы, Знач НастройкиРасположенияКоманд, Знач ВариантыДополнительногоОтчета, Знач Ответственный) Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Поставляемые дополнительные отчеты и обработки.Инициирована установка поставляемой обработки в область данных';uk='Додаткові звіти та обробки, що поставляються.Ініційовано встановлення обробки, яка поставляється, в область даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Попытка
		...
		Если ОбщегоНазначения.СсылкаСуществует(ПоставляемаяОбработка) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных';uk='Додаткові звіти та обробки, що поставляються.Встановлення в область даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных отложена';uk='Додаткові звіти та обробки, що поставляються.Встановлення в область даних відкладено'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				Строка(ОписаниеИнсталляции.Идентификатор),
				Строка(ОписаниеИнсталляции.Инсталляция));
...
Процедура УдалитьПоставляемуюОбработкуИзОбластиДанных(Знач ПоставляемаяОбработка, Знач ИдентификаторИспользуемойОбработки) Экспорт
	...
	Попытка
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Удаление из области данных';uk='Додаткові звіти та обробки, які поставляються.Вилучення з області даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка,
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Ошибка удаления из область данных';uk='Додаткові звіти та обробки, які поставляються.Помилка вилучення з області даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОбработатьОшибкуУстановкиДополнительнойОбработкиВОбластьДанных(Знач ПоставляемаяОбработка, Знач ИдентификаторИнсталляции, Знач ТекстИсключения) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Ошибка установки в область данных';uk='Додаткові звіти та обробки, що поставляються.Помилка встановлення в область даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,
		,
		ПоставляемаяОбработка,
...
Функция ПараметрыПодключенияИспользуемойОбработки(Знач ИспользуемаяОбработка) Экспорт
	...
	Если ЗначениеЗаполнено(ПоставляемаяОбработка) Тогда
		...
		Свойства = "БезопасныйРежим, ХранилищеОбработки";
...
Процедура ПриДобавленииОбработчиковСлужебныхСобытий(КлиентскиеОбработчики, СерверныеОбработчики) Экспорт
	...
	СерверныеОбработчики["СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки\ПередУдалениемДополнительнойОбработки"].Добавить(
		"ДополнительныеОтчетыИОбработкиВМоделиСервиса");
	
	СерверныеОбработчики["СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки\ПриПолученииРегистрационныхДанных"].Добавить(
		"ДополнительныеОтчетыИОбработкиВМоделиСервиса");
	
	СерверныеОбработчики["СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки\ПриПодключенииВнешнейОбработки"].Добавить(
		"ДополнительныеОтчетыИОбработкиВМоделиСервиса");
	
	СерверныеОбработчики["СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки\ПриСозданииВнешнейОбработки"].Добавить(
		"ДополнительныеОтчетыИОбработкиВМоделиСервиса");
	
	СерверныеОбработчики["СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки\ПриПолученииРазрешенийСессииБезопасногоРежима"].Добавить(
		"ДополнительныеОтчетыИОбработкиВМоделиСервиса");
	...
	СерверныеОбработчики["СтандартныеПодсистемы.РаботаВМоделиСервиса.ОбменСообщениями\ПриОпределенииВерсииИнтерфейсаКорреспондента"].Добавить(
		"ДополнительныеОтчетыИОбработкиВМоделиСервиса");
	...
	Если ЭтоПоставляемаяОбработка(Источник.Ссылка) Тогда
		...
		Для Каждого КонтролируемыйРеквизит Из КонтролируемыеРеквизиты Цикл
			...
			Если ИсходныйРеквизит <> РезультирующийРеквизит Тогда
				...
				ВызватьИсключение СтрШаблон(НСтр("ru='Недопустимая попытка изменения значения реквизита %1 для дополнительной обработки %2, полученной из каталога дополнительных обработок менеджера сервиса!';uk='Неприпустима спроба зміни значення реквізиту %1 для додаткової обробки %2, отриманої з каталогу додаткових обробок менеджера сервісу!'"), 
...
Процедура ПриПодключенииВнешнейОбработки(Знач Ссылка, СтандартнаяОбработка, Результат) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если ТипЗнч(Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Или Ссылка = Справочники.ДополнительныеОтчетыИОбработки.ПустаяСсылка() Тогда
			...
			ВызватьИсключение НСтр("ru='Запрошено подключение несуществующей дополнительной обработки!';uk='Запитано підключення неіснуючої додаткової обробки!'");
		...
		Если ПолучитьФункциональнуюОпцию("ИспользуютсяПрофилиБезопасности") Тогда
			...
			БезопасныйРежим = РаботаВБезопасномРежимеСлужебныйВМоделиСервиса.РежимИсполненияВнешнегоМодуля(Ссылка);
		...
		Если ЭтоОтчет(Ссылка) Тогда
			...
			Результат = ВнешниеОтчеты.Подключить(АдресВоВременномХранилище, , БезопасныйРежим);
			
		Иначе
			
			Результат = ВнешниеОбработки.Подключить(АдресВоВременномХранилище, , БезопасныйРежим);
...
Процедура ПриСозданииВнешнейОбработки(Знач Ссылка, СтандартнаяОбработка, Результат) Экспорт
	...
	Если Не СтандартнаяОбработка Тогда
		...
		Если ИмяОбработки = Неопределено Тогда
			ВызватьИсключение НСтр("ru='Запрошено создание объекта несуществующей дополнительной обработки!';uk='Запитано створення об''єкта неіснуючої додаткової обробки!'");
		КонецЕсли;
...
Процедура ПриПолученииРазрешенийСессииБезопасногоРежима(Знач КлючСессии, ОписанияРазрешений, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если ЭтоПоставляемаяОбработка(ИспользуемаяОбработка) Тогда
		...
		Иначе
			...
			ВызватьИсключение СтрШаблон(НСтр("ru='Не обнаружена поставляемая обработка для ключа запуска %1!';uk='Не виявлена обробка, що поставляється, для ключа запуску %1!'"),
				Строка(КлючСессии));
...
Процедура АктуализацияНастроекИспользуемойОбработки(Знач Ссылка) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Дополнительная обработка с идентификатором %1 не является поставляемой!';uk='Додаткова обробка з ідентифікатором %1 не є такою, що поставляється!'"),
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		ТаблицаСовместимости = ДополнительныеОтчетыИОбработкиВМоделиСервисаСовместимость.ПрочитатьТаблицуСовместимости(ТаблицаСовместимостиXDTO);
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки отменена';uk='Додаткові звіти та обробки, що поставляються.Завантаження обробки, яка поставляється, скасовано'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				,
				НСтр("ru='Поставляемая обработка несовместима с данной конфигурацией';uk='Обробка, що поставляється, несумісна з даною конфігурацією'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемойОбработки.Совместимость);
...
Процедура ЗапроситьОбновлениеДополнительныхОтчетовИОбработок() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='Запрос обновлений дополнительных отчетов и обработок';uk='Запит оновлень додаткових звітів і обробок'"), УровеньЖурналаРегистрации.Предупреждение, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
		ЗафиксироватьТранзакцию();
...
Процедура ОбработатьПоставляемыеДополнительныеОтчетыИОбработки(Дескриптор, ПутьКФайлу)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки';uk='Додаткові звіти та обробки, які поставляються.Завантаження обробки, що поставляється'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
		,
		НСтр("ru='Инициирована загрузка поставляемой обработки';uk='Ініційоване завантаження обробки, що поставляється'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемойОбработки.Манифест);
	...
	Если ПоставляемаяОбработка.ВерсияGUID <> ОписаниеПоставляемойОбработки.Версия Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки завершена';uk='Додаткові звіти та обробки, що поставляються.Завантаження обробки, яка поставляється, завершено'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			ПоставляемаяОбработка.Ссылка,
			НСтр("ru='Завершена загрузка поставляемой обработки';uk='Завершене завантаження обробки, що поставляється'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемойОбработки.Манифест);
		...
		Для Каждого ИнсталляцияОбработки Из ИспользуемыеОбработки Цикл
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые дополнительные отчеты и обработки.Запланирована актуализация настроек поставляемой обработки';uk='Додаткові звіти та обробки, які поставляються.Запланована актуалізація настройок обробки, яка поставляється'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка.Ссылка,
				НСтр("ru='Область данных:';uk='Область даних:'") + ИнсталляцияОбработки.ОбластьДанных);
		...
		Для Каждого ЭлементОчереди Из ОчередьИнсталляций Цикл
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые дополнительные отчеты и обработки.Запланирована отложенная установка поставляемой обработки в область данных';uk='Додаткові звіти та обробки, що поставляються.Заплановано відкладене встановлення обробки, яка поставляється, в область даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемаяОбработка.Ссылка,
				НСтр("ru='Область данных:';uk='Область даних:'") + ЭлементОчереди.ОбластьДанных);
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Иначе
			...
			Если Не Константы.НезависимоеИспользованиеДополнительныхОтчетовИОбработокВМоделиСервиса.Получить() Тогда
				...
				ВызватьИсключение НСтр("ru='Этот дополнительный отчет или обработка не может быть использован в сервисе!';uk='Цей додатковий звіт або обробка не може бути використаний в сервісі!'");
...
Процедура ПроверкаВозможностиВыполненияПоставляемойОбработки(Знач ИспользуемаяОбработка)
	...
	Если ПараметрыПубликацииИспользуемойОбработки.Публикация = Перечисления.ВариантыПубликацииДополнительныхОтчетовИОбработок.Отключена Тогда
		ВызватьИсключение НСтр("ru='Использование дополнительной обработки в вашем приложении запрещено! Обратитесь за помощью к пользователю, обладающего правами администратора в данном приложении.';uk='Використання додаткової обробки у вашому додатку заборонено! Зверніться за допомогою до користувача, який має права адміністратора в даному додатку.'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПоставляемаяОбработка) Тогда
		...
		Если ПараметрыПубликацииИспользуемойОбработки.Версия <> ПараметрыПубликацииПоставляемойОбработки.Версия Тогда
			ВызватьИсключение НСтр("ru='Использование дополнительной обработки временно недоступно по причине выполнения ее обновления. Данный процесс может занять несколько минут. Приносим извинения на доставленные неудобства.';uk='Використання додаткової обробки тимчасово недоступне через виконання її оновлення. Даний процес може зайняти кілька хвилин. Приносимо вибачення за незручності.'");
		КонецЕсли;
...
Процедура ПриСериализацииВладельцаРазрешенийНаИспользованиеВнешнихРесурсов(Знач Владелец, СтандартнаяОбработка, Результат) Экспорт
	...
	Если ТипЗнч(Владелец) = Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Тогда
		
		СтандартнаяОбработка = Ложь;
		
		Результат = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(РаботаВБезопасномРежимеСлужебныйВМоделиСервиса.ПакетАдминистрированиеРазрешений(), "PermissionsOwnerExternalModule"));
