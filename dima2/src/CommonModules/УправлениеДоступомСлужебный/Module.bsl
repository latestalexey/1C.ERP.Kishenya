...
Процедура ПослеЗаписиАдминистратораПриАвторизации(Комментарий) Экспорт
	...
	Комментарий =
		НСтр("ru='Выполнен запуск от имени пользователя с ролью ""Полные права"",
                   ...
                   |Користувач доданий в групу доступу Адміністратори.
                   |
...
Процедура ПриОпределенииТекстаВопросаПередЗаписьюПервогоАдминистратора(ТекстВопроса) Экспорт
	...
	ТекстВопроса = НСтр("ru='В список пользователей программы добавляется первый пользователь,
        ...
        |тому він буде автоматично включений у групу доступу Адміністратори. 
        |Продовжити?'")
...
Процедура ПриСозданииАдминистратора(Администратор, Уточнение) Экспорт
	...
	КомментарийДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Пользователь ""%1"" добавлен в группу доступа Администраторы по причине:
                   |%2'
                   |;uk='Користувач ""%1"" доданий в групу доступу Адміністратори з причини:
                   |%2'"),
		Администратор,
		Уточнение);
	...
	Попытка
		...
		Если Объект.Пользователи.Найти(Администратор, "Пользователь") = Неопределено Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Управление доступом.Автоматическое изменение группы доступа Администраторы';uk='Управління доступом. Автоматична зміна групи доступу Адміністратори'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				...
				Если СтарыеРоли[Строка.Роль] = Неопределено Тогда
					...
					Иначе
						...
						Для каждого Профиль Из Профили Цикл
							ЗаписьЖурналаРегистрации(
								НСтр("ru='Управление доступом.Роль не найдена в метаданных';uk='Управління доступом. Роль не знайдена в метаданих'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
								...
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='При обновлении ролей пользователя ""%1""
                                              ...
                                              |;uk='При оновленні ролей користувача ""%1""
                                              |роль ""%2""
                                              |профілю груп доступу ""%3""
                                              |не знайдена в метаданих.'"),
	...
	Если НЕ НайденДействующийАдминистратор Тогда
		ОписаниеОшибки =
			НСтр("ru='В группе доступа Администраторы
                       ...
                       |;uk='У групі доступу Адміністратори
                       |повинен бути хоча б один користувач,
...
Процедура ЗаполнениеДанныхДляОграниченияДоступа(КоличествоДанных = 0, ТолькоКэшРеквизиты = Ложь, ЕстьИзменения = Неопределено) Экспорт
	...
	Если КоличествоДанных < 10000 Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Управление доступом.Заполнение данных для ограничения доступа';uk='Управління доступом.Заповнення даних для обмеження доступу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			,
			НСтр("ru='Завершено заполнение данных для ограничения доступа.';uk='Завершене заповнення даних для обмеження доступу.'"),
			РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
	...
	Иначе
		ЗаписьЖурналаРегистрации(
			...
			,
			,
			НСтр("ru='Выполнена запись части данных для ограничения доступа.';uk='Виконано запис частини даних для обмеження доступу.'"),
			РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
	КонецЕсли;
	...
	Если Объект.Метаданные().ТабличныеЧасти.Найти("НаборыЗначенийДоступа") = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверные параметры.
                       ...
                       |;uk='Невірні параметри.
                       |У об''єкта типу ""%1""
                       |не знайдена таблична частина ""Набори значень доступу"".'"),
			ТипЗначенияОбъект);
	КонецЕсли;
	...
	Если НЕ НаборыЗаписываются Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверные параметры.
                       ...
                       |;uk='Невірні параметри.
                       |Тип об''єкта ""%1""
                       |не знайдений в підписці на події
                       |""Записати набори значень доступу"".'"),
			ТипЗначенияОбъект);
	КонецЕсли;
	...
	Если Метаданные.РегистрыСведений.НаборыЗначенийДоступа.Измерения.Объект.Тип.Типы().Найти(ТипЗнч(СсылкаНаОбъект)) = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи наборов значений доступа:
                       ...
                       |не задан тип %1'
                       |;uk='Помилка при записі наборів значень доступу:
                       |у регістрі відомостей НаборыЗначенийДоступа у вимірюванні Об''єкт
                       |не заданий тип %1'"),
...
Процедура ПриИзмененииОграниченияДоступаНаУровнеЗаписей(ОграничениеДоступаНаУровнеЗаписейВключено) Экспорт
	...
	Если ОграничениеДоступаНаУровнеЗаписейВключено Тогда
		...
		ЗаписьЖурналаРегистрации(
			...
			,
			,
			НСтр("ru='Начато заполнение данных для ограничения доступа.';uk='Почато заповнення даних для обмеження доступу.'"),
			РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
		...
		Если ИндексРазделителя = 0 Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при выполнении процедуры УправлениеДоступом.УстановитьУсловиеОтбораВЗапросе().
                           ...
                           |""%1"".'
                           |;uk='Помилка при виконанні процедури УправлениеДоступом.УстановитьУсловиеОтбораВЗапросе().
                           |
                           |У параметрі ИмяПараметраУсловияОтбораИмяПоля не знайдений роздільник (двокрапка)
                           |у наступному рядку формату ""<Ім''я параметра умови>:<Ім''я поля>""
                           |""%1"".'"),
				ТекущаяСтрока);
		КонецЕсли;
	...
	Попытка
		Если Данные.НовыеЗаписиСодержатТолькоРазличия Тогда
			...
			Если Данные.ИмяПервогоИзмерения = Неопределено Тогда
				ВызватьИсключение
					НСтр("ru='Некорректные параметры в процедуре ОбновитьНаборыЗаписей.';uk='Некоректні параметри в процедурі ОбновитьНаборыЗаписей.'");
			Иначе
	...
	Если ОбъектМетаданных = Неопределено Тогда
		ВызватьИсключение
			НСтр("ru='Ошибка в функции ПустаяСсылкаОбъектаМетаданных
                       ...
                       |;uk='Помилка у функції ПустаяСсылкаОбъектаМетаданных
                       |загального модуля УправлениеДоступомСлужебный.
                       |
                       |Невірний параметр ОписаниеОбъектаМетаданных.'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка в функции ПустаяСсылкаОбъектаМетаданных
                       ...
                       |загального модуля УправлениеДоступомСлужебный.
                       |
                       |Не вдалося отримати порожнє посилання для об''єкта метаданих
                       |""%1"".'"),
	...
	Если СтрНайти(ТекстЗапросаВыбораНовых, "&ПодстановкаПоляВидИзмененияСтроки") = 0 Тогда
		ВызватьИсключение
			НСтр("ru='Ошибка в значении параметра ТекстЗапросаВыбораСтарых
                       ...
                       |;uk='Помилка в значенні параметра ТекстЗапросаВыбораСтарых
                       |процедури ТекстЗапросаВыбораИзменений модуля УправлениеДоступомСлужебный.
                       |
                       |У тексті запиту не знайдений рядок ""&ПодстановкаПоляВидИзмененияСтроки"".'");
	КонецЕсли;
	...
	Если СтрНайти(ТекстЗапросаВыбораНовых, "&ПодстановкаПоляВидИзмененияСтроки") = 0 Тогда
		ВызватьИсключение
			НСтр("ru='Ошибка в значении параметра ТекстЗапросаВыбораНовых
                       ...
                       |
                       |В тексте запроса не найдена строка ""&ПодстановкаПоляВидИзмененияСтроки"".'
                       |;uk='Помилка в значенні параметра ТекстЗапросаВыбораНовых
                       |процедури ТекстЗапросаВыбораИзменений модуля УправлениеДоступомСлужебный.
                       |
                       |У тексті запиту не знайдений рядок ""&ПодстановкаПоляВидИзмененияСтроки"".'");
	КонецЕсли;
			...
			Исключение
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При обновлении зависимого набора значений доступа объекта
                               ...
                               |
                               |%2'
                               |;uk='При оновленні залежного набору значень доступу об''єкта
                               |""%1"" виникла помилка:
                               |
                               |%2'"),
...
Процедура ЗаполнитьПараметры(ВходныеПараметры, Знач ВсеПараметры, Знач ОбязательныеПараметры = "")
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Некорректный тип набора свойств ""%1"".
                       ...
                       |;uk='Некоректний тип набору властивостей ""%1"".
                       |Припустимі типи: Структура, не Визначено.'"),
	...
	Для каждого КлючИЗначение Из Параметры Цикл
		Если Не ВсеПараметры.Свойство(КлючИЗначение.Ключ) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Указан несуществующий параметр %1';uk='Вказаний неіснуючий параметр %1'"),
				КлючИЗначение.Ключ);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОбязательныеПараметры) Тогда
		...
		Для каждого КлючИЗначение Из ОбязательныеПараметры Цикл
			Если Не Параметры.Свойство(КлючИЗначение.Ключ) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не указан обязательный параметр %1';uk='Не вказано обов''язковий параметр %1'"),
					КлючИЗначение.Ключ);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Если НЕ НаборыЗаписываются Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверные параметры.
                       ...
                       |;uk='Невірні параметри.
                       |Тип об''єкта ""%1""
                       |не знайдений в підписках на події
                       |""Записати набори значень доступу"".'"),
			ТипЗначенияОбъект);
	КонецЕсли;
			...
			Если НЕ ТабличнаяЧастьЗаполняется Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Неверные параметры.
                               ...
                               |Тип об''єкта ""%1""
                               |не знайдений в підписках на події
                               |""Заповнити набори значень доступу табличних частин"".'"),
					ТипЗначенияОбъект);
			КонецЕсли;
	...
	Если НЕ ЭтоВедущийОбъект Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверные параметры.
                       ...
                       |;uk='Невірні параметри.
                       |Тип об''єкта ""%1""
                       |не знайдений у підписці на події
                       |""Записати залежні набори значень доступу"".'"),
			ТипЗначенияОбъект);
	КонецЕсли;
		...
		Если ПарольПользователяСервиса = Неопределено Тогда
			...
			ВызватьИсключение
				НСтр("ru='Для изменения административного доступа
                           ...
                           |;uk='Для зміни адміністративного доступу
                           |потрібен пароль користувача сервісу.
                           |
                           |Операція може бути виконана тільки інтерактивно.'");
		КонецЕсли;
...
Процедура ИсключениеПриОшибкеПоискаЗаписи(Параметры)
		   ...
		   И СтрокаИзменений.ВидИзмененияСтроки <> -1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка в процедуре ОбновитьНаборыЗаписей
                           ...
                           |;uk='Помилка в процедурі ОбновитьНаборыЗаписей
                           |загального модуля УправлениеДоступомСлужебный.
                           |
                           |Невірне значення параметра НовыеЗаписи - колонка
                           |ВидИзмененияСтроки містить неприпустиме значення ""%1"".
                           |
                           |Припустимо тільки 2 значення: ""1"" й ""-1"".'"),
	...
	ВызватьИсключение
		НСтр("ru='Ошибка в процедуре ОбновитьНаборыЗаписей
                   ...
                   |загального модуля УправлениеДоступомСлужебный.
                   |
                   |Не вдалося знайти необхідний рядок
                   |у значенні параметра НовыеЗаписи.'");
...
Процедура ЗаполнитьТаблицуПредставленияВсеРазрешеныВФорме(Форма, ЭтоПрофиль)
	...
	Если ЭтоПрофиль Тогда
		...
		Строка.Представление = НСтр("ru='Все запрещены, исключения назначаются в группах доступа';uk='Всі заборонені, виключення призначаються в групах доступу'");
		
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВначалеВсеРазрешены";
		Строка.Представление = НСтр("ru='Все разрешены, исключения назначаются в группах доступа';uk='Всі дозволені, виключення призначаються в групах доступу'");
		
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВсеЗапрещены";
		Строка.Представление = НСтр("ru='Все запрещены, исключения назначаются в профиле';uk='Всі заборонені, виключення призначаються в профілі'");
		
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВсеРазрешены";
		Строка.Представление = НСтр("ru='Все разрешены, исключения назначаются в профиле';uk='Всі дозволені, виключення призначаються в профілі'");
	Иначе
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВсеЗапрещены";
		Строка.Представление = НСтр("ru='Все запрещены';uk='Всі заборонені'");
		
		Строка = Форма.ПредставленияВсеРазрешены.Добавить();
		Строка.Имя = "ВсеРазрешены";
		Строка.Представление = НСтр("ru='Все разрешены';uk='Всі дозволені'");
	КонецЕсли;
