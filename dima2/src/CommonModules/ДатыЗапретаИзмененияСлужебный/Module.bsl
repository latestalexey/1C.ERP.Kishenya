...
Процедура ПроверитьДатыЗапретаИзмененияЗагрузкиДанных(Источник,
	...
	ЗаписьЖурналаРегистрации(
		?(УзелПроверкиЗапретаЗагрузки <> Неопределено,
			НСтр("ru='Даты запрета изменения.Найдены запреты загрузки';uk='Дати заборони зміни.Знайдені заборони завантаження'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			НСтр("ru='Даты запрета изменения.Найдены запреты изменения';uk='Дати заборони зміни.Знайдені заборони зміни'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
...
Процедура УстановитьНачальнуюДатуЗапретаИзменения() Экспорт
	...
	Запись.Комментарий  = "(" + НСтр("ru='По умолчанию';uk='По умовчанню'") + ")";
	 ...
	 Или ИсточникиДанных.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для проверки запрета изменения не найдены
                       ...
                       |;uk='Для перевірки заборони зміни не знайдені
		...
		Если ОбъектМетаданных = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для проверки запрета изменения 
                           ...
                           |у джерелі даних невірно вказана таблиця
                           |""%1"".'"),
				Строка.Таблица);
		КонецЕсли;
