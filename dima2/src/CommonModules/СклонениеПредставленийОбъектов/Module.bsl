...
Функция СтруктураСклоненияЧерезЗапросКСервису(Представление, ПоказыватьСообщения) 
	...
	Если Результат <> Неопределено Тогда
		...
		Если Не ПустаяСтрока(СообщениеОбОшибке) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Вызов веб-сервиса склонения';uk='Виклик веб-сервісу відмінювання'",КодОсновногоЯзыка), УровеньЖурналаРегистрации.Предупреждение, , ,СообщениеОбОшибке);
		КонецЕсли;	
		
	Иначе
		
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
		ЗаписьЖурналаРегистрации(НСтр("ru='Вызов веб-сервиса склонения';uk='Виклик веб-сервісу відмінювання'",КодОсновногоЯзыка), УровеньЖурналаРегистрации.Ошибка, , ,СообщениеОбОшибке);
		
		Если ПоказыватьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка при вызове сервиса склонения. Обратитесь к администратору.';uk='Помилка при виклику сервісу відмінювання. Зверніться до адміністратора.'") + Символы.ПС + НСтр("ru='Техническая информация:';uk='Технічна інформація:'") + Символы.ПС + СообщениеОбОшибке);
		КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешения.Добавить(РаботаВБезопасномРежиме.РазрешениеНаИспользованиеВнешнейКомпоненты("ОбщийМакет.КомпонентаСклоненияФИО", 
		НСтр("ru='Для использования функций по склонению ФИО.';uk='Для використання функцій по відмінюванню ПІБ.'")));
