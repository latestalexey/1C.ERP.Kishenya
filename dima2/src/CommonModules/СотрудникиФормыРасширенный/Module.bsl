...
Процедура ФизическиеЛицаПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ЗначениеЗаполнено(Форма.ФизическоеЛицо.Фотография) Тогда 
		ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайла(Форма.ФизическоеЛицо.Фотография);
...
Функция НастройкиСпискаФормы(Форма) Экспорт
...
Процедура ПрименитьНастройкиСписка(Форма, НастройкиСписка) Экспорт
	
	Элементы = Форма.Элементы;
	Список = Форма.Список;
	Подразделения = Форма.Подразделения;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"Видимость",
		Не НастройкиСписка.УстановленОтборПоПодразделению И Элементы.ВидСпискаГруппа.Видимость);
	...
	Если Не ЗначениеЗаполнено(НастройкиСписка.ОтборПоФилиалу) И ЗначениеЗаполнено(НастройкиСписка.ОтборПоОрганизации) И ТипЗнч(НастройкиСписка.ОтборПоОрганизации) = Тип("СправочникСсылка.Организации") Тогда
		...
		Если Не ФОИспользоватьНесколькоОрганизаций
			Или Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НастройкиСписка.ОтборПоОрганизации, "ЕстьОбособленныеПодразделения")
	...
	Если ЗначениеЗаполнено(НастройкиСписка.ОтборПоФилиалу) Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			Подразделения, "Владелец", НастройкиСписка.ОтборПоФилиалу, ВидСравненияЭлементаОтбора);
	...
	Если ФОИспользоватьНесколькоОрганизаций Тогда
		...
		Если Не ЗначениеЗаполнено(НастройкиСписка.ОтборПоФилиалу)
			...
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НастройкиСписка.ОтборПоОрганизации, "ЕстьОбособленныеПодразделения")
...
Функция УстановитьЗапросДинамическогоСписка(Форма, ОтборыСписка, ДляВыбораСотрудников = Истина) Экспорт
	
	ДополнительныеПараметры = Неопределено;
	Если Не ОтборыУстанавливались(Форма, ОбщегоНазначенияКлиентСервер.СкопироватьМассив(ОтборыСписка)) Тогда
		
		ЗапросСписка = ЗапросДинамическогоСписка(ОтборыСписка, ДляВыбораСотрудников);
		Если ЗапросСписка.Параметры.Количество() > 0 Тогда
			
			Форма.Список.ТекстЗапроса = ЗапросСписка.Текст;
			ДополнительныеПараметры = ЗапросСписка.Параметры;
			Для каждого ПараметрЗапроса Из ДополнительныеПараметры Цикл
				
				ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
					Форма.Список, ПараметрЗапроса.Ключ, ПараметрЗапроса.Значение);
...
Функция ЗапросДинамическогоСписка(ОтборыСписка, ДляВыбораСотрудников)
