...
#Область ВозвратСырьяОтПереработчика
	Если ОбъектМетаданных = Метаданные.Документы.ВозвратСырьяОтПереработчика Тогда
		
		СтруктураСинонимов.Вставить("ДокументОснование", "ЗаказПереработчику");
		
	КонецЕсли;
#КонецОбласти
#Область ЗаказПереработчику
	Если ОбъектМетаданных = Метаданные.Документы.ЗаказПереработчику Тогда
		
		СтруктураСинонимов.Вставить("БанковскийСчетОрганизации", "БанковскийСчет");
		СтруктураСинонимов.Вставить("НалогообложениеНДС", "ЗакупкаПодДеятельность");
		СтруктураСинонимов.Вставить("ДокументОснование", "Распоряжение");
		
		ЗаполнениеСвойствПоСтатистикеСервер.ДобавитьЗаполняемыеРеквизиты(ДополнительныеРеквизиты,
			"ВернутьМногооборотнуюТару", "Автор");
		
	КонецЕсли;
#КонецОбласти
#Область ОтчетПереработчика
	Если ОбъектМетаданных = Метаданные.Документы.ОтчетПереработчика Тогда
		
		СтруктураСинонимов.Вставить("Соглашение", "СоглашениеНаПередачу");
		СтруктураСинонимов.Вставить("НалогообложениеНДС", "ЗакупкаПодДеятельность");
		СтруктураСинонимов.Вставить("ДокументОснование", "ЗаказПереработчику");
...
#Область ПередачаСырьяПереработчику
	Если ОбъектМетаданных = Метаданные.Документы.ПередачаСырьяПереработчику Тогда
...
#Область ПоступлениеОтПереработчика
	Если ОбъектМетаданных = Метаданные.Документы.ПоступлениеОтПереработчика Тогда
...
#Область ВозвратСырьяДавальцу
	Если ОбъектМетаданных = Метаданные.Документы.ВозвратСырьяДавальцу Тогда
		
		СтруктураСинонимов.Вставить("ДокументОснование", "ЗаказДавальца");
		
	КонецЕсли;
#КонецОбласти
#Область ЗаказДавальца
	Если ОбъектМетаданных = Метаданные.Документы.ЗаказДавальца Тогда
		...
		ЗаполнениеСвойствПоСтатистикеСервер.ДобавитьЗаполняемыеРеквизиты(ДополнительныеРеквизиты,
			"СкладПоступления, ВернутьМногооборотнуюТару", "Автор");
		
	КонецЕсли;
#КонецОбласти
#Область ОтчетДавальцу
	Если ОбъектМетаданных = Метаданные.Документы.ОтчетДавальцу Тогда
...
#Область ПередачаДавальцу
	Если ОбъектМетаданных = Метаданные.Документы.ПередачаДавальцу Тогда
...
#Область ПоступлениеСырьяОтДавальца
	Если ОбъектМетаданных = Метаданные.Документы.ПоступлениеСырьяОтДавальца Тогда
