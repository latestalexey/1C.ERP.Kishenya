...
Процедура ПолучениеИОтправкаЭлектронныхПисем() Экспорт
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), 
		...
		НСтр("ru='Начато регламентное получение и отправка электронной почты';uk='Розпочато регламентне отримання та відправлення електронної пошти'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		...
		Исключение
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Во время подключения к учетной записи %1 произошла ошибка
                    |%2'
                    |;uk='Під час підключення до облікового запису %1 сталася помилка
			...
			Исключение
				...
				ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Во время отправки электронного письма %1 произошла ошибка
                        |%2'
                        |;uk='Під час відправлення електронного листа %1 сталася помилка
                        |%2'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru='Закончено регламентное получение и отправка электронной почты';uk='Закінчено регламентне отримання та відправлення електронної пошти'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура ЗагрузитьПочтуПользователя(ПолученоПисем, ДоступноУчетныхЗаписей, ЕстьОшибки) Экспорт
	...
	Если ДоступноУчетныхЗаписей = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Нет доступных для получения почты учетных записей';uk='Немає доступних для отримання облікових записів пошти'"));
		Возврат;
	КонецЕсли;
			...
			Пока ИдЗагружать.Количество() > (Получено + КоличествоОшибокПриЗаписи) Цикл
				...
				Для Каждого Сообщение Из Сообщения Цикл
					...
					Исключение
                        ...
                        |%4'
                        |;uk='При отриманні листа %1 від %2, що відправлений з адреси %3 сталася помилка
...
Процедура ЗаписатьВложениеЭлектронногоПисьма(Объект, Вложение,МассивПодписей,КоличествоПустыхИменВоВложениях)
	...
	Иначе
		...
		Если ТипЗнч(Вложение.Данные) = Тип("ИнтернетПочтовоеСообщение") Тогда
			...
			НовоеУдаленноеВложение.ИмяВложение = Вложение.Данные.Тема + " " + НСтр("ru='от';uk='від'") + " " +Вложение.Данные.ДатаОтправления;
		КонецЕсли;
		Возврат;
	КонецЕсли;
		...
		Если ЕстьПодписи
		   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
			...
			Для Каждого ПодписьВложения Из МассивПодписей Цикл
				...
				ЭП.Комментарий = НСтр("ru='Вложение электронного письма';uk='Вкладення електронної пошти'");
...
Функция ЗаписатьВложениеЭлектронногоПисьмаИзВременногоХранилища(
	...
	Если ПустаяСтрока(ИмяБезРасширения) Тогда
		...
		ИмяФайлаБезРасширения =
			НСтр("ru='Вложение без имени';uk='Вкладення без імені'") + ?(КоличествоПустыхИменВоВложениях = 0, ""," " + Строка(КоличествоПустыхИменВоВложениях + 1));
...
Процедура СоздатьОтправитьУведомлениеОПрочтении(Выборка,ЭтоРегламентноеЗадание,Соединение = Неопределено)
	...
	ПараметрыПисьма.Вставить("Тема",НСтр("ru='Уведомление о прочтении';uk='Повідомлення про прочитання'") + " / " +"Reading Confirmation");
		...
		Если ЭтоРегламентноеЗадание Тогда
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Во время регламентной отправки уведомления о прочтении электронного письма %1 произошла ошибка
                    |%2'
                    |;uk='Під час регламентного відправлення повідомлення про прочитання електронного листа %1 сталася помилка
                    |%2'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		Иначе
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Во время отправки уведомления о прочтении электронного письма %1 произошла ошибка
                    |%2'
                    |;uk='Під час надсилання повідомлення про прочитання електронного листа %1 сталася помилка
                    |%2'"),
				Выборка.ПредставлениеПисьма,
				КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Взаимодействия';uk='Взаємодії'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура СоздатьПредопределенныеПапкиЭлектронныхПисемДляУчетнойЗаписи(УчетнаяЗапись) Экспорт
	...
	МассивИменПредопределенныхПапок.Добавить(НСтр("ru='Входящие';uk='Вхідні'"));
	МассивИменПредопределенныхПапок.Добавить(НСтр("ru='Исходящие';uk='Вихідні'"));
	МассивИменПредопределенныхПапок.Добавить(НСтр("ru='Нежелательная почта';uk='Небажана пошта'"));
	МассивИменПредопределенныхПапок.Добавить(НСтр("ru='Отправленные';uk='Відправлені'"));
	МассивИменПредопределенныхПапок.Добавить(НСтр("ru='Удаленные';uk='Вилучені'"));
	МассивИменПредопределенныхПапок.Добавить(НСтр("ru='Черновики';uk='Чернетки'"));
