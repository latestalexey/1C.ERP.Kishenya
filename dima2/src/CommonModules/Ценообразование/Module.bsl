...
Процедура СообщитьОбОшибкахКорректностьЗаполненияНоменклатурыПоставщика(Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru='В строке не может быть выбрана ""%НоменклатураПоставщика%"", т.к. в предыдущих строках для номенклатуры поставщика указано другое соответствие номенклатуре';uk='У рядку не може бути вибрана ""%НоменклатураПоставщика%"", т. к. в попередніх рядках для номенклатури постачальника зазначена інша відповідність номенклатурі'");
