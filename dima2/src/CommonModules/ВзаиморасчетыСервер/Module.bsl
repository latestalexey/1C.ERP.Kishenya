...
Функция ПолучитьТекстЗапросаДокументыРасчетовСКлиентами(ВыборОснованияПлатежа, ПараметрыОтбора) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказДавальца) Тогда
		...
		ДополнитьТекстЗапросаДокументыРасчетовСКлиентами(
			ТекстЗапроса, Тип("ДокументСсылка.ЗаказДавальца"), ПараметрыОтбора);
...
Функция ПолучитьТекстЗапросаДокументыРасчетовСПоставщиками(ПараметрыОтбора) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказПереработчику) Тогда
		...
		|	ДанныеДокумента.Дата КАК Дата,
		|	ДанныеДокумента.Номер КАК Номер,
		|	ДанныеДокумента.НомерПоДаннымПартнера КАК НомерПоДаннымКонтрагента,
		|	ДанныеДокумента.ДатаПоДаннымПартнера КАК ДатаПоДаннымКонтрагента,
		|	ДанныеДокумента.Организация,
		|	ДанныеДокумента.Партнер,
		|	ДанныеДокумента.Контрагент,
		|	ДанныеДокумента.Договор,
		...
		ДополнитьТекстЗапросаДокументыРасчетовСПоставщиками(
			ТекстЗапроса, Тип("ДокументСсылка.ЗаказПереработчику"), ПараметрыОтбора);
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ОтчетПереработчика) Тогда
		...
		ДополнитьТекстЗапросаДокументыРасчетовСПоставщиками(
			ТекстЗапроса, Тип("ДокументСсылка.ОтчетПереработчика"), ПараметрыОтбора);
	КонецЕсли;
	
	Если ПравоДоступа("Чтение", Метаданные.Документы.ВыбытиеДенежныхДокументов) Тогда
		ТекстЗапроса = ТекстЗапроса + "ОБЪЕДИНИТЬ ВСЕ" +
		"
		|ВЫБРАТЬ
		|	ДанныеДокумента.Ссылка КАК Ссылка,
		...
		|ГДЕ
		|	ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратДенежныхДокументовПоставщику)
		|	И (ДанныеДокумента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоНакладным)
		|		Или ДанныеДокумента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным))
		|";
		
		ДополнитьТекстЗапросаДокументыРасчетовСПоставщиками(
			ТекстЗапроса, Тип("ДокументСсылка.ВыбытиеДенежныхДокументов"), ПараметрыОтбора);
		
	КонецЕсли;
	
	Если ПравоДоступа("Чтение", Метаданные.Документы.ПоступлениеДенежныхДокументов) Тогда
		ТекстЗапроса = ТекстЗапроса + "ОБЪЕДИНИТЬ ВСЕ" +
		"
		|ВЫБРАТЬ
		|	ДанныеДокумента.Ссылка КАК Ссылка,
		...
		|ГДЕ
		|	ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеДенежныхДокументовОтПоставщика)
		|	И (ДанныеДокумента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоНакладным)
		|		Или ДанныеДокумента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным))
		|";
		
		ДополнитьТекстЗапросаДокументыРасчетовСПоставщиками(
			ТекстЗапроса, Тип("ДокументСсылка.ПоступлениеДенежныхДокументов"), ПараметрыОтбора);
...
Процедура РассчитатьДатыПлатежаКлиента(МенеджерВременныхТаблиц, ДатаОстатков) Экспорт
	...
	Тогда
		...
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентами КАК Периодика
		|		ПО Периодика.Период < &ДатаОстатков
		|		И Периодика.АналитикаУчетаПоПартнерам = Расчеты.АналитикаУчетаПоПартнерам
		|		И Периодика.ЗаказКлиента = Расчеты.ЗаказКлиента
		|		И Периодика.Валюта = Расчеты.Валюта
		|		И НЕ (Периодика.Регистратор ССЫЛКА Документ.ЗаказКлиента
		|			ИЛИ Периодика.Регистратор ССЫЛКА Документ.ЗаявкаНаВозвратТоваровОтКлиента
		//++ НЕ УТКА
		|			ИЛИ Периодика.Регистратор ССЫЛКА Документ.ЗаказДавальца
		//-- НЕ УТКА
		|		)
		|ГДЕ
		|	(Расчеты.СуммаОстаток - Расчеты.КОплатеОстаток) > 0
		|СГРУППИРОВАТЬ ПО
		...
		|	Расчеты.ЗаказКлиента                            КАК ЗаказКлиента,
		|	Расчеты.Валюта                                  КАК Валюта,
		|	Расчеты.КОплатеОстаток                          КАК ДолгОстаток,
		|	Расчеты.КОплатеОстаток                          КАК ДолгОстатокНаКонецПериода,
		|	&ДатаОстатков                                   КАК ДатаПлатежа,
		|	ДОБАВИТЬКДАТЕ(&ДатаОстатков, СЕКУНДА, 1)        КАК Период,
		|	ИСТИНА                                          КАК ПросроченнаяЗадолженность,
		|	ИСТИНА                                          КАК РасчетыПоНакладным,
		...
		|	РегистрНакопления.РасчетыСКлиентами.Остатки(&ДатаОстатков, ) КАК Расчеты
		|ГДЕ
		|	(Расчеты.СуммаОстаток - Расчеты.КОплатеОстаток) <= 0 И Расчеты.КОплатеОстаток <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ЛОЖЬ                                            КАК РасчетыПоНакладным,
		|	Периодика.Регистратор                           КАК РасчетныйДокумент,
		|	НЕОПРЕДЕЛЕНО                                    КАК Регистратор
		|ИЗ
		|	РегистрНакопления.РасчетыСКлиентами.Остатки(&ДатаОстатков, ) КАК Расчеты
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ОстаткиПоЛИФО КАК Периодика
		|		ПО Периодика.АналитикаУчетаПоПартнерам = Расчеты.АналитикаУчетаПоПартнерам
		|		И Периодика.ЗаказКлиента = Расчеты.ЗаказКлиента
		|		И Периодика.Валюта = Расчеты.Валюта
		|ГДЕ
		|	(Расчеты.СуммаОстаток - Расчеты.КОплатеОстаток) > 0
		...
		|	ТаблицаОстатков.РасчетыПоНакладным                 КАК РасчетыПоНакладным
		|ПОМЕСТИТЬ ТаблицаПериодовКлиентов
		|ИЗ
		...
		|			И (ТаблицаОстатков.ДолгОстаток <> 0 ИЛИ ТаблицаОстатков.РасчетныйДокумент = НЕОПРЕДЕЛЕНО)
		|ГДЕ
		|	Расчеты.Активность
		...
		|	ТаблицаОстатков.РасчетыПоНакладным
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	И НЕ ТаблицаОстатков.РасчетыПоНакладным
		|
		|СГРУППИРОВАТЬ ПО
		|	Расчеты.АналитикаУчетаПоПартнерам,
		|	Расчеты.ЗаказКлиента,
		...
		|	ВЫБОР КОГДА Периоды.ЗаказКлиента ССЫЛКА Справочник.ДоговорыКонтрагентов
		...
		|	Периоды.РасчетыПоНакладным                    КАК РасчетыПоНакладным,
		...
		|	КОНЕЦ,
		|	Периоды.ПросроченнаяЗадолженность,
		|	Периоды.РасчетыПоНакладным,
		|	ЕСТЬNULL(Расчеты.Регистратор, НЕОПРЕДЕЛЕНО)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|		И Расчеты.КОплате <> 0
		|ГДЕ
		|	НЕ Периоды.ПросроченнаяЗадолженность
		|	И НЕ Периоды.РасчетыПоНакладным
		|СГРУППИРОВАТЬ ПО
		|	Периоды.АналитикаУчетаПоПартнерам,
		|	Периоды.ЗаказКлиента,
		|	Периоды.Валюта,
		...
		|	Остатки.РасчетыПоНакладным                          КАК РасчетыПоНакладным,
		...
		|ГДЕ
		|	Остатки.РасчетныйДокумент <> НЕОПРЕДЕЛЕНО
		|	И Остатки.Регистратор <> НЕОПРЕДЕЛЕНО
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(Остатки.Регистратор) = ТИП(Документ.ГрафикИсполненияДоговора)
		|		ТОГДА Остатки.ДатаПлатежа
		|		КОГДА Остатки.ДолгОстаток <> 0
		|		ТОГДА ЕСТЬNULL(Отгрузки.ДатаПлатежа, Остатки.ДатаПлатежа)
		|		ИНАЧЕ Остатки.ДатаПлатежа
		|	КОНЕЦ                                               КАК ДатаПлатежа,
		|	Остатки.ПросроченнаяЗадолженность                   КАК ПросроченнаяЗадолженность,
		|	Остатки.РасчетыПоНакладным                          КАК РасчетыПоНакладным,
		|	ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(Отгрузки.Регистратор) В (
		|		ТИП(Документ.ЗаказКлиента),
		|		ТИП(Документ.ЗаявкаНаВозвратТоваровОтКлиента),
		//++ НЕ УТКА
		|		ТИП(Документ.ЗаказДавальца),
		//-- НЕ УТКА
		|		ТИП(Документ.ГрафикИсполненияДоговора))
		|		ТОГДА Остатки.РасчетныйДокумент
		|		ИНАЧЕ ЕСТЬNULL(Отгрузки.Регистратор, Остатки.РасчетныйДокумент)
		|	КОНЕЦ                                               КАК РасчетныйДокумент,
		|	ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(Остатки.Регистратор) = ТИП(Документ.ГрафикИсполненияДоговора)
		|		ТОГДА Остатки.Регистратор
		|		КОГДА Остатки.ЗаказКлиента ССЫЛКА Справочник.ДоговорыКонтрагентов
		|		ТОГДА Отгрузки.Регистратор
		|		КОГДА ЕСТЬNULL(Отгрузки.Долг, 0) = 0 И Остатки.ДолгОстаток <> 0
		|		ТОГДА НЕОПРЕДЕЛЕНО
		|		КОГДА ЕСТЬNULL(Отгрузки.Долг, 0) = 0 И Остатки.ДолгОстаток = 0
		|		ТОГДА Остатки.Регистратор
		|		ИНАЧЕ Отгрузки.Регистратор
		...
		|		И Остатки.РасчетыПоНакладным = Отгрузки.РасчетыПоНакладным
		|ГДЕ
		|	Остатки.ПросроченнаяЗадолженность
		|	И (Остатки.ДолгОстаток <> 0 ИЛИ Остатки.РасчетныйДокумент = НЕОПРЕДЕЛЕНО)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	Остатки.ПросроченнаяЗадолженность                   КАК ПросроченнаяЗадолженность,
		|	Остатки.РасчетыПоНакладным                          КАК РасчетыПоНакладным,
		|	ВЫБОР КОГДА Отгрузки.Регистратор ССЫЛКА Документ.ЗаказКлиента
		|		ИЛИ Отгрузки.Регистратор ССЫЛКА Документ.ЗаявкаНаВозвратТоваровОтКлиента
		|		ИЛИ Отгрузки.Регистратор ССЫЛКА Документ.ГрафикИсполненияДоговора
		//++ НЕ УТКА
		|		ИЛИ Отгрузки.Регистратор ССЫЛКА Документ.ЗаказДавальца
		//-- НЕ УТКА
		|		ТОГДА Остатки.РасчетныйДокумент
		|		ИНАЧЕ ЕСТЬNULL(Отгрузки.Регистратор, Остатки.РасчетныйДокумент)
		|	КОНЕЦ КАК РасчетныйДокумент,
		...
		|		И Остатки.РасчетыПоНакладным = Отгрузки.РасчетыПоНакладным
		|ГДЕ
		|	НЕ Остатки.ПросроченнаяЗадолженность
		|	И НЕ Остатки.РасчетыПоНакладным
		|	И Остатки.ДолгОстаток <> 0
		|;
		|////////////////////////////////////////////////////////////////////////////////
		...
		|		И Расчеты.ЗаказКлиента = ТаблицаОстатков.ЗаказКлиента
		|		И Расчеты.Валюта = ТаблицаОстатков.Валюта
		|		И Расчеты.РасчетныйДокумент = ТаблицаОстатков.РасчетныйДокумент 
		|ГДЕ
...
Процедура РассчитатьДатыПлатежаПоставщика(МенеджерВременныхТаблиц, ДатаОстатков) Экспорт
	...
	Тогда
		...
		|	Расчеты.Валюта                                     КАК Валюта,
		|	МАКСИМУМ(НАЧАЛОПЕРИОДА(Расчеты.ДатаПлатежа, День)) КАК ДатаПлатежа,
		|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.Регистратор = Расчеты.ЗаказПоставщику
		|				ТОГДА НачалоПериода(Расчеты.Период, День)
		...
		|ГДЕ
		|	Остатки.ПросроченнаяЗадолженность
		|	И Остатки.РасчетныйДокумент <> НЕОПРЕДЕЛЕНО
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ЕСТЬNULL(Поступления.КОплате, Остатки.КОплатеОстатокНаКонецПериода) КАК КОплатеОстатокНаКонецПериода,
		|	Поступления.Период                                     КАК Период,
		|	ЕСТЬNULL(Поступления.ДатаПлатежа, Остатки.ДатаПлатежа) КАК ДатаПлатежа,
		|	Остатки.ПросроченнаяЗадолженность                      КАК ПросроченнаяЗадолженность,
		|	ВЫБОР КОГДА Поступления.Регистратор ССЫЛКА Документ.ЗаказПоставщику
		//++ НЕ УТ
		|			ИЛИ Поступления.Регистратор ССЫЛКА Документ.ЗаказПереработчику
		//-- НЕ УТ
		|		ТОГДА Остатки.РасчетныйДокумент
		|		ИНАЧЕ ЕСТЬNULL(Поступления.Регистратор, Остатки.РасчетныйДокумент)
		|	КОНЕЦ                                                  КАК РасчетныйДокумент,
		|	ВЫБОР КОГДА ЕСТЬNULL(Поступления.КОплате, 0) = 0
		|		ТОГДА НЕОПРЕДЕЛЕНО
		|		ИНАЧЕ Поступления.Регистратор
		|	КОНЕЦ                                                  КАК Регистратор
		|ИЗ
		|	ТаблицаОстатковПоставщиков КАК Остатки
		|
		...
		|	ЕСТЬNULL(Поступления.ДатаПлатежа, Остатки.ДатаПлатежа) КАК ДатаПлатежа,
		|	Остатки.ПросроченнаяЗадолженность                      КАК ПросроченнаяЗадолженность,
		|	ЕСТЬNULL(Поступления.Регистратор, Остатки.Регистратор) КАК РасчетныйДокумент,
		|	ЕСТЬNULL(Поступления.Регистратор, Остатки.Регистратор) КАК Регистратор
		|ИЗ
		|	ТаблицаОстатковПоставщиков КАК Остатки
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПоступленийПоставщиков КАК Поступления
...
Процедура ДополнитьТекстЗапросаДокументыРасчетовСПоставщиками(ТекстЗапроса, ТипОбъекта, ПараметрыОтбора)
	...
	//++ НЕ УТ
	Если ПараметрыОтбора.ИсключитьРедактируемыйДокумент
		И ТипОбъекта = Тип("ДокументСсылка.ПоступлениеДенежныхДокументов") Тогда
