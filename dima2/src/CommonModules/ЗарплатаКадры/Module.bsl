...
Функция ЭтоОбъектЗарплатноКадровойБиблиотекиПолной(ПолноеИмяОбъектаМетаданных, 
	...
	ИменаПодсистемБЗК = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(СписокИменПодсистемБЗКВерхнегоУровня);
...
Функция ЗаголовокСвернутогоОтображенияГруппы(Знач ЗаголовокГруппы, ОтображаемыеЗначения, Знач ПредставлениеНезаполненного = Неопределено, Знач ПредставлениеЧастичноЗаполненного = Неопределено)
	...
	Если ПредставлениеНезаполненного = Неопределено Тогда
		ПредставлениеНезаполненного = НСтр("ru='<не указано>';uk='<не вказано>'")
	КонецЕсли;	
...
Функция СтруктураАдресаИзXML(Адрес, ВидАдреса = Неопределено) Экспорт
	...
	СтруктураАдреса = УправлениеКонтактнойИнформацией.ПредыдущаяСтруктураКонтактнойИнформацииXML(Адрес, ВидАдреса);
	
	Если ЗначениеЗаполнено(Адрес) И СтруктураАдреса.Свойство("Улица") И Не ЗначениеЗаполнено(СтруктураАдреса.Улица) Тогда
		
		ДопСведенияОбАдресе = УправлениеКонтактнойИнформацией.СведенияОбАдресе(Адрес);
