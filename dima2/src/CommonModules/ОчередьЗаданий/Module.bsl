...
//    Могут использоваться только методы для которых зарегистрированы псевдонимы через событие.
...
Функция ПолучитьЗадания(Знач Отбор) Экспорт
	...
	РазделительЗаданийОбластейДанных = ОбщегоНазначенияПовтИсп.РазделительВспомогательныхДанных();
	...
	Для Каждого СправочникЗаданий Из СправочникиЗаданий Цикл
		...
		Если Не ПолучатьРазделенные Тогда
			...
			Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() И ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(ИмяСправочника, РазделительЗаданийОбластейДанных) Тогда
				Продолжить;
			КонецЕсли;
		...
		Если Не ПолучатьНеразделенные Тогда
			...
			Если Не ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() ИЛИ Не ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(ИмяСправочника, РазделительЗаданийОбластейДанных) Тогда
				Продолжить;
			КонецЕсли;
		...
		Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() И ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(ИмяСправочника, ОбщегоНазначенияПовтИсп.РазделительВспомогательныхДанных()) Тогда
	...
	Если ПустаяСтрока(ТекстЗапроса) Тогда
		ВызватьИсключение НСтр("ru='Некорректное значение отбора - не обнаружено на одного справочника, задания из которого подходили бы под условия в отборе!';uk='Некоректне значення відбору - не виявлено жодного довідника, завдання з якого б підходили під умови у відборі!'");
	КонецЕсли;
	...
	Иначе
		Результат = ОбщегоНазначения.ВыполнитьЗапросВнеТранзакции(Запрос).Выгрузить();
	КонецЕсли;
...
Функция ДобавитьЗадание(ПараметрыЗадания) Экспорт
	...
	Если НЕ ПараметрыЗадания.Свойство("ИмяМетода") Тогда
		ВызватьИсключение(НСтр("ru='Не задан обязательный параметр задания ИмяМетода';uk='Не заданий обов''язковий параметр завдання ИмяМетода'"));
	КонецЕсли;
	...
	Для каждого ОписаниеПараметра Из ОчередьЗаданийСлужебныйПовтИсп.ПараметрыЗаданийОчереди() Цикл
		Если ПараметрыЗадания.Свойство(ОписаниеПараметра.Имя) Тогда
			Если ОписаниеПараметра.РазделениеДанных Тогда
				Если Не ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() ИЛИ Не ОбщегоНазначения.ЭтоРазделенныйОбъектМетаданных(Задание.Метаданные(), ОбщегоНазначенияПовтИсп.РазделительВспомогательныхДанных()) Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
...
Процедура ИзменитьЗадание(Идентификатор, ПараметрыЗадания) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		И Задание.ОбластьДанных <> ОбщегоНазначения.ЗначениеРазделителяСеанса() Тогда
			...
			Для каждого КлючИЗначение Из ПараметрыЗадания Цикл
				...
				Если Не ОписаниеПараметра.Шаблон Тогда
					ШаблонСообщения = НСтр("ru='Задание очереди с идентификатором %1 создано на основе шаблона.
                        ...
                        |;uk='Завдання черги з ідентифікатором %1 створено на основі шаблону.
                        |Зміна параметра %2 завдань з встановленим шаблоном заборонено.'");
	...
	Попытка
		...
		Если Не ОбщегоНазначения.СсылкаСуществует(Идентификатор) Тогда
			ШаблонСообщения = НСтр("ru='Задание с идентификатором %1 к изменению не найдено. Область данных: %2';uk='Завдання з ідентифікатором %1 до зміни не знайдено. Область даних: %2'");
...
Процедура УдалитьЗадание(Идентификатор) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() И ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(
		...
		Если ЗначениеЗаполнено(Задание.Шаблон) Тогда
			ШаблонСообщения = НСтр("ru='Задание очереди с идентификатором %1 создано на основе шаблона.
                |Удаление заданий с установленным шаблоном запрещено.'
                |;uk='Завдання черги з ідентифікатором %1 створено на основі шаблону.
                |Вилучення завдань з встановленим шаблоном заборонене.'");
	...
	ОбщегоНазначения.УдалитьВспомогательныеДанные(Задание);
...
Функция ШаблонПоИмени(Знач Имя) Экспорт
	...
	Если Результат.Пустой() Тогда
		ШаблонСообщения = НСтр("ru='Не найден шаблон задания с именем %1';uk='Не знайдений шаблон завдання з ім''ям %1'");
...
Функция ПолучитьТекстИсключенияДублированиеЗаданийСОдинаковымКлючом() Экспорт
	...
	Возврат НСтр("ru='Дублирование заданий с одинаковым значения поля ''Ключ'' не допустимо.';uk='Дублювання завдань з однаковим значення поля ''Ключ'' не є допустимим.'");
...
Процедура ПроверитьПараметрыЗадания(Параметры, Режим)
	...
	Если ТипЗнч(Параметры) <> Тип("Структура") Тогда
		ШаблонСообщения = НСтр("ru='Передан недопустимый тип набора параметров задания - %1';uk='Переданий неприпустимий тип набору параметрів завдання - %1'");
	...
	Для каждого КлючИЗначение Из Параметры Цикл
		...
		Если ОписаниеПараметра = Неопределено 
			...
			ШаблонСообщения = НСтр("ru='Передан недопустимый параметр задания - %1';uk='Передано неприпустимий параметр завдання - %1'");
		...
		Если Отбор И ТипЗнч(КлючИЗначение.Значение) = Тип("Массив") Тогда
			...
			Для каждого ОписаниеОтбора Из КлючИЗначение.Значение Цикл
				Если ТипЗнч(ОписаниеОтбора) <> Тип("Структура") Тогда
					ШаблонСообщения = НСтр("ru='Передан недопустимый тип %1 в коллекции описания отбора %2';uk='Переданий неприпустимий тип %1 колекції опису відбору %2'");
				...
				Для каждого ИмяКлюча Из КлючиОписанияОтбора Цикл
					Если НЕ ОписаниеОтбора.Свойство(ИмяКлюча) Тогда
						ШаблонСообщения = НСтр("ru='Передано недопустимое описание отбора в коллекции описания отбора %1.
                            ...
                            |;uk='Передано неправильний опис відбору в колекції опису відбору %1.
                            |Відсутня властивість %2.'");
				...
				Если ВидыСравнения.Получить(ОписаниеОтбора.ВидСравнения) = Неопределено Тогда
					ШаблонСообщения = НСтр("ru='Передан недопустимый вид сравнения в описании отбора в коллекции описания отбора %1';uk='Переданий некоректний вид порівняння в описі відбору в колекції опису відбору %1'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КлючИЗначение.Ключ);
					ВызватьИсключение(ТекстСообщения);
				КонецЕсли;
				...
				Если ОписаниеОтбора.ВидСравнения = ВидСравнения.ВСписке
					...
					Если ТипЗнч(ОписаниеОтбора.Значение) <> Тип("Массив") Тогда
						ШаблонСообщения = НСтр("ru='Передан недопустимый тип %1 в описании отбора в коллекции описания отбора %2.
                            ...
                            |;uk='Переданий неприпустимий тип %1 в описі відбору в колекції опису відбору %2.
                            |Для виду порівняння %3 очікується тип Масив.'");
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		Если Параметры.Свойство("ОбластьДанных") Тогда
			Если Параметры.ОбластьДанных <> ОбщегоНазначения.ЗначениеРазделителяСеанса() Тогда
				ВызватьИсключение(НСтр("ru='В данном сеансе недопустимо обращение к данным из другой области данных!';uk='У даному сеансі неприпустиме звертання до даних з іншої області даних!'"));
			КонецЕсли;
		Иначе
			ОписаниеПараметра = ОписанияПараметров.Найти(ВРег("ОбластьДанных"), "ИмяВРег");
			Если ОписаниеПараметра[Режим] Тогда
				Параметры.Вставить("ОбластьДанных", ОбщегоНазначения.ЗначениеРазделителяСеанса());
			КонецЕсли;
		КонецЕсли;
	...
	Если Параметры.Свойство("ЗапланированныйМоментЗапуска")
		...
		ШаблонСообщения = НСтр("ru='Передано недопустимое значение %1 параметра задания %2';uk='Передано неприпустиме значення %1 параметра завдання %2'");
...
Процедура ПроверитьЗначениеНаСоответствиеОписаниюПараметра(Знач Значение, Знач ОписаниеПараметра)
	...
	Если НЕ ОписаниеПараметра.Тип.СодержитТип(ТипЗнч(Значение)) Тогда
		ШаблонСообщения = НСтр("ru='Передан недопустимый тип %1 параметра задания %2';uk='Переданий неприпустимий тип %1 параметра завдання %2'");
...
Процедура ПроверитьНаличиеРегистрацииОбработчикаЗадания(Знач ИмяМетода)
	...
	Если ОчередьЗаданийСлужебныйПовтИсп.СоответствиеИменМетодовПсевдонимам().Получить(ВРег(ИмяМетода)) = Неопределено Тогда
		ШаблонСообщения = НСтр("ru='Не зарегистрирован псевдоним метода %1 для использования в качестве обработчика задания очереди.';uk='Не зареєстрований псевдонім методу %1 для використання в якості обробника завдання черги.'");
...
Функция ОписаниеЗаданияПоИдентификатору(Знач Идентификатор)
	...
	Если Не ЗначениеЗаполнено(Идентификатор) ИЛИ Не ОбщегоНазначения.СсылкаСуществует(Идентификатор) Тогда
		ШаблонСообщения = НСтр("ru='Передано недопустимое значение %1 параметра задания Идентификатор';uk='Передано неприпустиме значення %1 параметра завдання Ідентифікатор'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Идентификатор);
		ВызватьИсключение(ТекстСообщения);
	КонецЕсли;
	...
	Если Задания.Количество() = 0 Тогда
		ШаблонСообщения = НСтр("ru='Задание очереди с идентификатором %1 не найдено';uk='Завдання черги з ідентифікатором %1 не знайдені'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Идентификатор);
		ВызватьИсключение(ТекстСообщения);
	КонецЕсли;
...
Функция ПолучитьТекстИсключенияПолученияДанныхДругойОбласти()
	...
	Возврат НСтр("ru='В данном сеансе недопустимо обращение к данным из другой области данных!';uk='У даному сеансі неприпустиме звертання до даних з іншої області даних!'");
