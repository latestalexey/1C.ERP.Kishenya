...
Функция Словарь() Экспорт
	...
	ДобавитьПараметр(Параметры, "Host", НСтр("ru='с сервера %1';uk='с сервера %1'"), НСтр("ru='с любого сервера';uk='с будь-якого сервера'"));
	...
	Представление = НСтр("ru='Обращение к веб-сервисам в сети Интернет';uk='Звертання до веб-сервисам у мережі Інтернет'");
	Описание = НСтр("ru='Дополнительному отчету или обработке будет разрешено обращаться к веб-сервисам, расположенным в сети Интернет (при этом возможно как получение дополнительным отчетом или обработкой информации из сети Интернет, так и передача.';uk='Додатковому звіту або обробці буде дозволено звертатися до веб-сервісам, розташованим у мережі Інтернет (при цьому можливо як одержання додатковим звітом або обробкою інформації з мережі Інтернет, так і передача.'");
	Последствия = НСтр("ru='Внимание! Обращение к веб-сервисам потенциально может использоваться дополнительным
                        ...
                        |выданные разрешения.'
                        |;uk='Увага! Звертання до сервісів-веб-сервісів потенційно може використовуватися додатковим
                        |звітом або обробкою для здійснення дій, не передбачуваних адміністратором
                        |інформаційної бази.
                        |
                        |Використовуйте даний додатковий звіт або обробку тільки в тому випадку, якщо Ви довіряєте
	...
	Описание = НСтр("ru='Дополнительному отчету или обработке будет разрешено использовать механизмы внешнего программного обеспечения с помощью создания объекта внешней компоненты, поставляемой в макете конфигурации';uk='Додатковому звіту або обробці буде дозволено використовувати механізми зовнішнього програмного забезпечення за допомогою створення об''єкта зовнішнього компонента, що поставляє в макеті конфігурації'");
	Последствия = НСтр("ru='Внимание! Использование средств стороннего программного обеспечения может использоваться
                        |дополнительным отчетом или обработкой для совершения действий, не предполагаемых администратором
                        |информационной базы, а также для несанкционированного обхода ограничений, накладываемых на дополнительную обработку
                        |в безопасном режиме.
	...
	Параметры = ТаблицаПараметров();
	ДобавитьПараметр(Параметры, "TemplateName", НСтр("ru='из макета %1';uk='з макета %1'"), НСтр("ru='из любого макета';uk='з будь-якого макета'"));
