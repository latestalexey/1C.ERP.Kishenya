	...
	Исключение
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось установить параметр сеанса ТекущийПользователь по причине:
                       ...
                       |;uk='Не вдалося встановити параметр сеансу ТекущийПользователь з причини:
                       |""%1"".
                       |
	...
	Если НЕ ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		Возврат
			НСтр("ru='Недопустимое получение параметра сеанса ТекущийПользователь
                       ...
                       |;uk='Неприпустиме одержання параметра сеансу ТекущийПользователь
                       |у сеансі без вказівки всіх роздільників.'");
	КонецЕсли;
	
	ЗаголовокОшибки = НСтр("ru='Не удалось установить параметр сеанса ТекущийПользователь.';uk='Не вдалося встановити параметр сеансу ТекущийПользователь.'") + Символы.ПС;
		...
		Иначе
			...
			Если НЕ РезультатВнешниеПользователи.Пустой() Тогда
				...
				Если НЕ ВнешниеПользователи.ИспользоватьВнешнихПользователей() Тогда
					Возврат НСтр("ru='Внешние пользователи отключены.';uk='Зовнішні користувачі відключені.'");
				КонецЕсли;
			Иначе
				ТекущийВнешнийПользователь = Справочники.ВнешниеПользователи.ПустаяСсылка();
	...
	Если ТекущийПользователь = Неопределено
	 ИЛИ ТекущийВнешнийПользователь = Неопределено Тогда
		...
		Возврат ЗаголовокОшибки + ТекстСообщенияПользовательНеНайденВСправочнике(ИмяПользователя) + Символы.ПС
			+ НСтр("ru='Возникла внутренняя ошибка при поиске пользователя.';uk='Виникла внутрішня помилка при пошуку користувача.'");
	КонецЕсли;
...
Функция АвторизоватьТекущегоПользователя(ПриНачалеРаботыСистемы = Ложь, РегистрироватьВЖурнале = Ложь) Экспорт
	...
	Если ПользовательПоИдентификаторуСуществует(
		...
		Если ПриНачалеРаботыСистемы И РолиАдминистратораДоступны() Тогда
			ИнтеграцияСтандартныхПодсистем.ПриСозданииАдминистратора(НайденныйПользователь,
				НСтр("ru='При авторизации у пользователя найдены роли администратора.';uk='При авторизації користувача знайдені ролі адміністратора.'"));
		КонецЕсли;
		Возврат РезультатУстановкиПараметровСеанса(РегистрироватьВЖурнале);
	КонецЕсли;
	...
	Если Не РолиАдминистратораДоступны() Тогда
		Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(
			НСтр("ru='Запуск от имени пользователя с правом Администрирование невозможен,
                       ...
                       |режим конфигурирования платформы для этого использовать не следует.'
                       |;uk='Запуск від імені користувача з правом Адміністрування неможливий,
                       |так як він не зареєстрований у списку користувачів.
                       |
                       |Для ведення списку і налаштування прав користувачів призначений список Користувачі
                       |режим конфігурування платформи для цього використовувати не слід.'"),
			, РегистрироватьВЖурнале);
	КонецЕсли;
	...
	Исключение
		...
		Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(ИнформацияОбОшибке,
			НСтр("ru='Не удалось выполнить автоматическую регистрацию администратора в списке по причине:
                       ...
                       |Для ведения списка и настройки прав пользователей предназначен список Пользователи,
                       |режим конфигурирования платформы для этого использовать не следует.'
                       |;uk='Не вдалося виконати автоматичну реєстрацію адміністратора в списку з причини:
                       |""%1"".
                       |
                       |Для ведення списку і налаштування прав користувачів призначений список Користувачі
                       |режим конфігурування платформи для цього використовувати не слід.'"),
	...
	Комментарий =
		НСтр("ru='Выполнен запуск от имени пользователя с ролью ""Полные права"",
                   ...
                   |Для ведения списка и настройки прав пользователей предназначен список Пользователи,
                   |режим конфигурирования платформы для этого использовать не следует.'
                   |;uk='Виконаний запуск від імені користувача з роллю ""Повні права"",
                   |який не зареєстрований у списку користувачів.
                   |Виконана автоматична реєстрація в списку користувачів.
                   |
                   |Для ведення списку і налаштування прав користувачів призначений список Користувачі
                   |режим конфігурування платформи для цього використовувати не слід.'");
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Пользователи.Администратор зарегистрирован в справочнике Пользователи';uk='Користувачі.Адміністратор зареєстрований у довіднику Користувачі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
// Возвращает текст ошибки, если у текущего пользователя роли базовые права или ролей администратора.
...
Функция ОшибкаНедостаточноПравДляВходаВПрограмму(РегистрироватьВЖурнале = Истина) Экспорт
	...
	Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(
		НСтр("ru='Недостаточно прав для входа в программу.
                   |
                   |Обратитесь к администратору.'
                   |;uk='Недостатньо прав для входу в програму.
                   |
                   |Зверніться до адміністратора.'"),
		, РегистрироватьВЖурнале);
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка в процедуре ПользователиСлужебный.ОбработатьИнтерфейсРолей()
                       ...
                       |;uk='Помилка в процедурі ПользователиСлужебный.ОбработатьИнтерфейсРолей()
                       |Невірне значення параметра Дія: ""%1"".'"),
			Действие);
	КонецЕсли;
...
//
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры, Отказ) Экспорт
...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	ВидДоступа.Представление          = НСтр("ru='Пользователи';uk='Користувачі'");
	...
	ВидДоступа.Представление          = НСтр("ru='Внешние пользователи';uk='Зовнішні користувачі'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ru='Некорректные сведения о пользователях';uk='Некоректні відомості про користувачів'");
...
Функция ВозможноИзменитьПароль(Пользователь, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ПроверятьДействительностьПользователя")
	   И РеквизитыПользователя.Недействителен <> Ложь Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пользователь ""%1"" недействителен.';uk='Користувач ""%1"" недійсний.'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ДополнительныеПараметры.Свойство("ПроверятьНаличиеПользователяИБ")
	   И ПользовательИБ = Неопределено Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найдена учетная запись пользователя ""%1"".';uk='Не знайден обліковий запис користувача ""%1"".'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
		...
		Иначе
			ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Установлен запрет изменения пароля пользователя ""%1"".
                           |Обратитесь к администратору.'
                           |;uk='Встановлено заборону зміни пароля користувача ""%1"".
	...
	ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Пароль можно будет сменить только через %1.';uk='Пароль можна буде змінити тільки через %1.'"), ЧислоИПредмет));
...
Функция ОбработатьНовыйПароль(Параметры) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	   ...
	   И (ВызовИзФормыСменаПароля Или СтарыйПароль <> Неопределено) Тогда
		...
		Если Не Параметры.СтарыйПарольСовпадает Тогда
			Возврат НСтр("ru='Старый пароль указан неверно.';uk='Старий пароль вказано невірно.'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1
                       ...
                       |;uk='%1
                       |
                       |Обмеження встановлене в конфігураторі в меню ""Адміністрування"" пункт ""Параметри інформаційної бази ..."".
                       |Слід очистити мінімальну довжину і вимогу складності в конфігураторі і
                       |задати налаштування входу користувачів в налаштуваннях програми.'"),
			ТекстОшибки);
	КонецЕсли;
		...
		Если ПользователиСлужебныйПовтИсп.Настройки().ОбщиеНастройкиВхода
		   И ЗначениеЗаполнено(НастройкиВхода.ЗапретитьПовторениеПароляСредиПоследних) Тогда
			...
			Если ИспользованныеПароли.Найти(СохраняемоеЗначениеПароля) <> Неопределено Тогда
				ОтменитьТранзакцию();
				Возврат НСтр("ru='Новый пароль использовался ранее.';uk='Новий пароль використовувався раніше.'");
			КонецЕсли;
	...
	Исключение
		...
		Если ВызовИзФормыСменаПароля Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Пользователи.Ошибка смены пароля';uk='Користувачі.Помилка зміни паролю'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось сменить пароль пользователя ""%1"" по причине:
                               |%2'
                               |;uk='Не вдалося змінити пароль користувача ""%1"" з причини:
                               |%2'"),
...
Функция ПодсказкаДляНовогоПароля() Экспорт
	...
	Возврат
		НСтр("ru='Надежный пароль:
                   ...
                   |;uk='Надійний пароль:
                   |- має не менше 7 символів;
                   |- містить будь-які 3 з 4-х типів символів: заголовні букви,
                   |малі літери, цифри, спеціальні символи;
                   |- не збігається з ім''ям (входу).'");
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(ИмяПользователя())
			   И ПользователиИнформационнойБазы.ПолучитьПользователей().Количество() = 0 Тогда
				...
				ВременныйАдминистраторИБ.Имя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Временный первый администратор (%1)';uk='Тимчасовий перший адміністратор (%1)'"), Новый УникальныйИдентификатор);
			...
			ВременныйПользовательИБ.Имя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Временный пользователь (%1)';uk='Тимчасовий користувач (%1)'"),
...
Процедура НачатьОбработкуПользователяИБ(ПользовательОбъект,
	...
	ПараметрыОбработки.Вставить("ТекстСообщенияНедостаточноПрав",
		НСтр("ru='Недостаточно прав для изменения пользователя информационной базы.';uk='Недостатньо прав для зміни користувача інформаційної бази.'"));
	...
	Если НЕ ОписаниеПользователяИБ.Свойство("Действие") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |;uk='Помилка при записі користувача ""%1"".
                       |У параметрі ОписаниеПользователяИБ не зазначена властивість Дія.'"),
	   ...
	   И ОписаниеПользователяИБ.Действие <> "Удалить" Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |;uk='Помилка при записі користувача ""%1"".
                       |У параметрі ОписаниеПользователяИБ зазначене
                       |невірне значення ""%2"" властивості Дія.'"),
			ПользовательОбъект.Ссылка,
		...
		Если ПараметрыОбработки.СтарыйПользовательИБСуществует Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при записи пользователя ""%1"".
                           ...
                           |;uk='Помилка при записі користувача ""%1"".
                           |Не можна зіставити користувача ІБ з користувачем у довіднику,
                           |з яким уже зіставлений інший користувач ІБ.'"),
		...
		Если ПользовательПоИдентификаторуСуществует(
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при записи пользователя ""%1"".
                           ...
                           |""%2"".'
                           |;uk='Помилка при записі користувача ""%1"".
                           |Не можна зіставити користувача ІБ із цим користувачем у довіднику,
                           |тому що він уже зіставлений з іншим користувачем у довіднику
                           |""%2"".'"),
...
Процедура ЗавершитьОбработкуПользователяИБ(ПользовательОбъект, ПараметрыОбработки) Экспорт
	...
	Если ПараметрыОбработки.Свойство("СозданиеАдминистратора") Тогда
		УстановитьПривилегированныйРежим(Истина);
		ИнтеграцияСтандартныхПодсистем.ПриСозданииАдминистратора(ПользовательОбъект.Ссылка,
	   ...
	   И ПользователиИнформационнойБазы.ПолучитьПользователей().Количество() = 0 Тогда
			...
			Если ЗапретРедактированияРолей()
				...
				Если ВРег(Пользователи.РольАдминистратораСистемы().Имя) = ВРег("ПолныеПрава") Тогда
					Текст = НСтр("ru='В список пользователей программы добавляется первый пользователь,
                                       ...
                                       |Продолжить?'
                                       |;uk='У список користувачів програми додається перший користувач,
                                       |тому йому автоматично буде призначена роль ""Повні права"".
                                       |Продовжити?'");
				Иначе
					Текст = НСтр("ru='В список пользователей программы добавляется первый пользователь, поэтому ему
                                       ...
                                       |Продолжить?'
                                       |;uk='У список користувачів програми додається перший користувач, тому йому
                                       |автоматично будуть призначені ролі ""Адміністратор системи"" і ""Повні права"".
                                       |Продовжити?'");
				КонецЕсли;
		...
		Иначе
			...
			Текст = НСтр("ru='Первый пользователь информационной базы должен иметь полные права.
                               ...
                               |;uk='Перший користувач інформаційної бази повинен мати повні права.
                               |Зовнішній користувач не може бути повноправним.
                               |Спочатку створіть адміністратора в довіднику Користувачі.'");
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОписаниеПользователяИБ.Свойство("Имя") Тогда
		...
		Если ПустаяСтрока(Имя) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не заполнено Имя (для входа).';uk='Не заповнене Ім''я (для входу).'"),
				,
				"Имя",
				,
				Отказ);
		...
		ИначеЕсли СтрДлина(Имя) > 64 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Имя (для входа) превышает 64 символа.';uk='Ім''я (для входу) перевищує 64 символу.'"),
				,
				"Имя",
				,
				Отказ);
		...
		ИначеЕсли СтрНайти(Имя, ":") > 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Имя (для входа) содержит запрещенный символ "":"".';uk='Ім''я (для входу) містить заборонений символ "":"".'"),
				,
				"Имя",
				,
				Отказ);
			   ...
			   И ПользовательИБ.УникальныйИдентификатор
				...
				Если НайденныйПользователь = Неопределено
				 ИЛИ НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
					...
					ТекстОшибки = НСтр("ru='Имя (для входа) уже занято.';uk='Ім''я (для входу) уже зайнято.'");
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Имя (для входа) уже занято для пользователя ""%1"".';uk='Ім''я (для входу) уже зайнято для користувача ""%1"".'"),
			...
			Исключение
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Пользователь ОС должен быть в формате
                               ...
                               |;uk='Користувач ОС повинен бути у форматі
                               |""\\ИмяДомена\ИмяПользователя"".'"),
					,
					"ПользовательОС",
					,
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При обновлении ролей внешнего пользователя
                          ...
                          |;uk='При оновленні ролей зовнішнього користувача
                          |""%1""
                          |роль ""%2""
                          |групи зовнішніх користувачів ""%3""
                          |не знайдена в метаданних.'"),
				СокрЛП(Выборка.ВнешнийПользователь.Наименование),
				Выборка.Роль,
				Строка(Выборка.ГруппаВнешнихПользователей));
			
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Пользователи.Роль не найдена в метаданных';uk='Користувачі.Роль не знайдена в метаданних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ТекстСообщения,
...
Функция ОбъектАвторизацииИспользуется(Знач СсылкаНаОбъектАвторизации,
	...
	Если Результат Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Уже существует внешний пользователь, связанный с объектом ""%1"".';uk='Вже існує зовнішній користувач, пов''язаний з об''єктом ""%1"".'"),
...
Функция ПеремещениеПользователяВНовуюГруппу(МассивПользователей, ГруппаИсточник,
	...
	Если МассивПеремещенныхПользователей.Количество() = 0 И МассивНеПеремещенныхПользователей.Количество() = 0 Тогда
		Если МассивПользователей.Количество() = 1 Тогда
			ТекстСообщения = НСтр("ru='Пользователь ""%1"" уже включен в группу ""%2"".';uk='Користувач ""%1"" уже включень у групу ""%2"".'");
			ИмяПеремещаемогоПользователя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(МассивПользователей[0], "Наименование");
		Иначе
			ТекстСообщения = НСтр("ru='Все выбранные пользователи уже включены в группу ""%2"".';uk='Усі обрані користувачі вже включені в групу ""%2"".'");
...
Функция ФормированиеСообщенияПользователю(МассивПользователей, ГруппаПриемник,
			...
			Если ТипПользователяСовпадаетСГруппой Тогда
				СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Пользователь ""%1"" не может быть включен в группу ""%2"",
                               ...
                               |;uk='Користувач ""%1"" не може бути включений в групу ""%2"",
                               |т. к. у групи стоїть ознака ""Всі користувачі заданого типу"".'"),
			...
			Иначе
				СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Пользователь ""%1"" не может быть включен в группу ""%2"",
                               ...
                               |;uk='Користувач ""%1"" не може бути включений в групу ""%2"",
                               |т. к. до складу її учасників входять тільки %3.'"),
		...
		Иначе
			...
			СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не все пользователи могут быть включены в группу ""%1"",
                           ...
                           |;uk='Не всі користувачі можуть бути включені в групу ""%1"",
                           |тому що до складу її учасників входять тільки %2
                           |або у групи стоїть ознака ""Всі користувачі заданого типу"".'"),
				НаименованиеГруппы,
	...
	Если КоличествоПользователей = 1 Тогда
		...
		Если ГруппаПриемник = Справочники.ГруппыПользователей.ВсеПользователи
		 Или ГруппаПриемник = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
			...
			СтрокаДействие = НСтр("ru='исключен из группы';uk='виключений із групи'");
			НаименованиеГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаИсточник, "Наименование");
			
		ИначеЕсли Перемещение Тогда
			СтрокаДействие = НСтр("ru='перемещен в группу';uk='переміщений у групу'");
		Иначе
			СтрокаДействие = НСтр("ru='включен в группу';uk='включений у групу'");
		КонецЕсли;
		
		СообщениеПользователю = НСтр("ru='""%1"" %2 ""%3""';uk='""%1"" %2 ""%3""'");
	...
	ИначеЕсли КоличествоПользователей > 1 Тогда
		...
		Если ГруппаПриемник = Справочники.ГруппыПользователей.ВсеПользователи Тогда
			СтрокаДействие = НСтр("ru='исключены из группы';uk='виключені із групи'");
			НаименованиеГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаИсточник, "Наименование");
			
		ИначеЕсли Перемещение Тогда
			СтрокаДействие = НСтр("ru='перемещены в группу';uk='переміщені в групу'");
		Иначе
			СтрокаДействие = НСтр("ru='включены в группу';uk='включені в групу'");
		КонецЕсли;
		СообщениеПользователю = НСтр("ru='%1 %2 ""%3""';uk='%1 %2 ""%3""'");
	КонецЕсли;
...
Процедура ОбновитьПредопределенныеВидыКонтактнойИнформацииПользователей() Экспорт
	...
	ПараметрыВида.Подсказка = НСтр("ru='Адрес электронной почты пользователя';uk='Адреса електронної пошти користувача'");
	...
	ПараметрыВида.Подсказка = НСтр("ru='Контактный телефон пользователя';uk='Контактний телефон користувача'");
	ПараметрыВида.МожноИзменятьСпособРедактирования = Истина;
	ПараметрыВида.РазрешитьВводНесколькихЗначений = Истина;
...
Функция ЗаголовокСообщенияАвторизацияНеВыполненаСПереводомСтроки()
	...
	Возврат НСтр("ru='Авторизация не выполнена. Работа системы будет завершена.';uk='Авторизація не виконана. Робота системи буде завершено.'")
	...
	Если ВнешниеПользователи.ИспользоватьВнешнихПользователей() Тогда
		ШаблонСообщенияОбОшибке =
			НСтр("ru='Пользователь ""%1"" не найден в справочниках
                       ...
                       |
                       |Обратитесь к администратору.'
                       |;uk='Користувач ""%1"" не знайдено у довідниках
                       |""Користувачі"" і ""Зовнішні користувачі"".
                       |
                       |Зверніться до адміністратора.'");
	Иначе
		ШаблонСообщенияОбОшибке
			= НСтр("ru='Пользователь ""%1"" не найден в справочнике ""Пользователи"".
                         |
                         |Обратитесь к администратору.'
                         |;uk='Користувач ""%1"" не знайдений у довіднику ""Користувачі"".
                         |
                         |Зверніться до адміністратора.'");
	КонецЕсли;
...
Функция КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(ИнформацияОбОшибке, ШаблонОшибки = "", РегистрироватьВЖурнале = Истина)
	...
	Если РегистрироватьВЖурнале Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Пользователи.Ошибка входа в программу';uk='Пользователи.Ошибка входу в програму'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПроверитьПраваПользователя(ПользовательИБ, РежимПроверки, ЭтоВнешнийПользователь)
	...
	Для Каждого ОписаниеРоли Из РолиДляПроверки Цикл
		...
		Если НазначениеРолей.ТолькоДляАдминистраторовСистемы.Получить(ИмяРоли) <> Неопределено Тогда
			ТекстШаблона = НСтр("ru='""%1"" (предназначена только для администраторов системы)';uk='""%1"" (призначена тільки для адміністраторів системи)'");
		
		ИначеЕсли РазделениеВключено
		        И НазначениеРолей.ТолькоДляПользователейСистемы.Получить(ИмяРоли) <> Неопределено Тогда
			
			ТекстШаблона = НСтр("ru='""%1"" (предназначена только для пользователей системы)';uk='""%1"" (призначена тільки для користувачів системи)'");
		...
		Иначе
			...
			Если НазначениеРолей.ТолькоДляВнешнихПользователей.Получить(ИмяРоли) <> Неопределено Тогда
				ТекстШаблона = НСтр("ru='""%1"" (предназначена только для внешних пользователей)';uk='""%1"" (призначена тільки для зовнішніх користувачів)'");
				
			Иначе // Это внешний пользователь.
				ТекстШаблона = НСтр("ru='""%1"" (предназначена только для пользователей)';uk='""%1"" (призначена тільки для користувачів)'");
			КонецЕсли;
		КонецЕсли;
	...
	ИмяСобытия = НСтр("ru='Пользователи.Ошибка при установке ролей пользователю ИБ';uk='Користувачі.Помилка при установці ролей користувачеві ІБ'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если РежимПроверки = "ПриЗапуске" Тогда
		Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 Тогда
			ТекстРегистрацииВхода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Попытка входа пользователя %1 с недоступной ролью:
                           |%2.'
                           |;uk='Спроба входу користувача %1 з недоступною роллю:
                           |%2.'"),
		...
		Иначе
			ТекстРегистрацииВхода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Попытка входа пользователя %1 с недоступными ролями:
                           |%2.'
                           |;uk='Спроба входу користувача %1 з недоступними ролями:
                           |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
		КонецЕсли;
		...
		ТекстСообщенияВхода =
			НСтр("ru='Невозможно выполнить вход из-за наличия недоступных ролей.
                       |Обратитесь к администратору.'
                       |;uk='Неможливо виконати вхід через наявність недоступних ролей.
                       |Зверніться до адміністратора.'");
	...
	Если СтрЧислоСтрок(УдаляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(УдаляемыеНедоступныеРоли) Тогда
		ТекстРегистрацииУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Попытка удалить пользователю %1 недоступную роль
                       |""%2"".'
                       |;uk='Спроба вилучити користувача %1 недоступну роль
                       |""%2"".'"),
	...
	ИначеЕсли СтрЧислоСтрок(УдаляемыеНедоступныеРоли) > 1 Тогда
		ТекстРегистрацииУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Попытка удалить пользователю %1 недоступные роли:
                       |%2.'
                       |;uk='Спроба вилучити користувача %1 недоступні ролі:
                       |%2.'"),
			ПользовательИБ.ПолноеИмя, УдаляемыеНедоступныеРоли);
	Иначе
	...
	Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(ДобавляемыеНедоступныеРоли) Тогда
		ТекстРегистрацииДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Попытка назначить пользователю %1 недоступную роль:
                       |%2.'
                       |;uk='Спроба призначити користувача %1 недоступну роль:
                       |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	...
	ИначеЕсли СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) > 1 Тогда
		ТекстРегистрацииДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Попытка назначить пользователю %1 недоступные роли:
                       |%2.'
                       |;uk='Спроба призначити користувача %1 недоступні ролі:
                       |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	Иначе
	...
	Если СтрЧислоСтрок(УдаляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(УдаляемыеНедоступныеРоли) Тогда
		ТекстСообщенияУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пользователю ""%1"" невозможно удалить недоступную роль.';uk='Користувачу ""%1"" неможливо вилучити недоступну роль.'"),
			ПользовательИБ.ПолноеИмя);
	ИначеЕсли СтрЧислоСтрок(УдаляемыеНедоступныеРоли) > 1 Тогда
		ТекстСообщенияУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пользователю ""%1"" невозможно удалить недоступные роли.';uk='Користувачу ""%1"" неможливо вилучити недоступні ролі.'"),
			ПользовательИБ.ПолноеИмя);
	Иначе
	...
	Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(ДобавляемыеНедоступныеРоли) Тогда
		ТекстСообщенияДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пользователю ""%1"" невозможно назначить недоступную роль
                       |%2.'
                       |;uk='Користувачу ""%1"" неможливо призначити недоступну роль
                       |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	...
	ИначеЕсли СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) > 1 Тогда
		ТекстСообщенияДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пользователю ""%1"" невозможно назначить недоступные роли:
                       |%2.'
                       |;uk='Користувачу ""%1"" неможливо призначити недоступні ролі:
                       |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	Иначе
	...
	Исключение
		...
		ШаблонТекстаОшибки =
			НСтр("ru='Не удалось записать сведения о текущем пользователе по причине:
                       ...
                       |
                       |Обратитесь к администратору.'
                       |;uk='Не вдалося записати відомості про поточного користувача з причини:
                       |%1
                       |
                       |Зверніться до адміністратора.'");
		...
		Иначе
			Если РегистрироватьВЖурнале Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Пользователи.Ошибка обновления даты последней активности';uk='Пользователи.Ошибка оновлення дати останньої активності'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Метаданные.НайтиПоТипу(ТипЗнч(ТекущийПользователь)),
					ТекущийПользователь,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекстаОшибки,
...
Функция ОшибкаДлиныИлиСложностиПароля(Пароль, МинимальнаяДлинаПароля, СложныйПароль)
	...
	Если СтрДлина(Пароль) < МинимальнаяДлинаПароля Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Количество символов нового пароля должно быть не менее %1.';uk='Кількість символів нового пароля має бути не менше %1.'"),
	...
	Если СложныйПароль И Не ПарольОтвечаетТребованиямСложности(Пароль) Тогда
		Возврат НСтр("ru='Пароль не отвечает требованиям сложности.';uk='Пароль не відповідає вимогам складності.'")
			+ Символы.ПС + Символы.ПС
...
// Проверяет пароль на требования сложности, реализованными в платформе.
//
// Параметры:
...
Функция ПарольОтвечаетТребованиямСложности(Пароль)
	...
	Попытка
		...
		ВременныйПользовательИБ.Имя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Временный пользователь (%1)';uk='Тимчасовий користувач (%1)'"),
			Новый УникальныйИдентификатор);
	...
	Исключение
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось проверить отвечает ли пароль требованиям сложности по причине:
                       |%1'
                       |;uk='Не вдалося перевірити, чи відповідає пароль вимогам складності з причини:
                       |%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки;
	...
	Если Не ПользовательОбъект.ДополнительныеСвойства.Свойство("СозданиеАдминистратора")
	   И РолиАдминистратораДоступны(ПользовательИБ) Тогда
		...
		ПользовательОбъект.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
			НСтр("ru='Запись пользователя с ролями администратора.';uk='Запис користувача з ролями адміністратора.'"));
	КонецЕсли;
	   ...
	   И БлокируемыеРеквизиты.Служебный <> ПользовательОбъект.Служебный Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |;uk='Помилка при записі користувача ""%1"".
                       |Реквізит Службовий не допускається змінювати в підписках на події.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если БлокируемыеРеквизиты.Подготовлен <> ПользовательОбъект.Подготовлен Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |;uk='Помилка при записі користувача ""%1"".
                       |Реквізит Підготовлений не допускається змінювати в підписках на події.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если АвтоРеквизиты.ИдентификаторПользователяИБ <> ПользовательОбъект.ИдентификаторПользователяИБ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |;uk='Помилка при записі користувача ""%1"".
                       |Реквізит ИдентификаторПользователяИБ не допускається змінювати.
                       |Оновлення реквізиту виконується автоматично.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если Не ОбщегоНазначения.ДанныеСовпадают(АвтоРеквизиты.СвойстваПользователяИБ,
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |Обновление реквизита выполняется автоматически.'
                       |;uk='Помилка при записі користувача ""%1"".
                       |Реквізит СвойстваПользователяИБ не допускається змінювати.
                       |Оновлення реквізиту виконується автоматично.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |;uk='Помилка при записі користувача ""%1"".
                       |Не можна відмічати для вилучення користувача, якому дозволений вхід у програму.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |;uk='Помилка при записі користувача ""%1"".
                       |Не можна позначити недійсним користувача, якому дозволений вхід у програму.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |;uk='Помилка при записі користувача ""%1"".
                       |Не можна позначити підготовленим користувача, якому дозволений вхід у програму.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
			...
			Если Выборка.СрокДействияИстек Тогда
				ШаблонКомментария =
					НСтр("ru='Не удалось снять пользователю ""%1"" признак
                               ...
                               |%2'
                               |;uk='Не вдалося зняти користувачу ""%1"" ознаку
                               |""Вхід в програму дозволено"" у зв''язку із закінченням терміну дії через:
                               |%2'");
			Иначе
				ШаблонКомментария =
					НСтр("ru='Не удалось снять пользователю ""%1"" признак
                               ...
                               |%2'
                               |;uk='Не вдалося зняти користувачу ""%1"" ознаку
                               |""Вхід в програму дозволено"" у зв''язку з відсутністю роботи
                               |у програмі понад встановленого строку через:
                               |%2'");
			КонецЕсли;
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Пользователи.Ошибка автоматического запрещения входа в программу';uk='Пользователи.Ошибка автоматичної заборони входу в програму'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.НайтиПоТипу(ТипЗнч(Пользователь)),
				Пользователь,
