...
Процедура ВыполнитьКоманду(Знач Форма, Знач ИмяКоманды) Экспорт
	...
	ИначеЕсли ТипКонтактнойИнформации = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон") Тогда
		...
		Если ПустаяСтрока(Параметры.НомерТелефона) Тогда
			ПоказатьПредупреждение(,НСтр("ru='Для совершения звонка или отправки SMS требуется ввести номер телефона.';uk='Для здійснення дзвінка або відправки SMS потрібно ввести номер телефону.'"));
			Возврат;
		КонецЕсли;
		
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОтправкаSMS") Тогда
			Список = Новый СписокЗначений;
			Список.Добавить("Позвонить", НСтр("ru='Позвонить';uk='Зателефонувати'"),, БиблиотекаКартинок.Позвонить);
			Список.Добавить("ОтправитьSMS", НСтр("ru='Отправить SMS...';uk='Відправити SMS...'"),, БиблиотекаКартинок.ОтправитьSMS);
	...
	ИначеЕсли ТипКонтактнойИнформации = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Skype") Тогда
		...
		Список = Новый СписокЗначений;
		Список.Добавить("Позвонить", НСтр("ru='Позвонить';uk='Зателефонувати'"));
		Список.Добавить("НачатьЧат", НСтр("ru='Начать чат';uk='Почати чат'"));
...
Процедура СоздатьЭлектронноеПисьмо(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено) Экспорт
	...
	Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Нельзя создать письмо по контактной информацию с типом ""%1""';uk='Не можна створити лист по контактної інформацію з типом ""%1""'"), ТипИнформации);
	КонецЕсли;
	
	Если ЗначенияПолей = "" И ПустаяСтрока(Представление) Тогда
		ПоказатьПредупреждение(,НСтр("ru='Для отправки письма необходимо ввести адрес электронной почты.';uk='Для відправки листа необхідно ввести адресу електронної пошти.'"));
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(АдресПочты) <> Тип("Строка") Тогда
		ВызватьИсключение НСтр("ru='Ошибка получения адреса электронной почты, неверный тип контактной информации';uk='Помилка одержання адреси електронної пошти, невірний тип контактної інформації'");
	КонецЕсли;
	...
	ТекстПредложения = НСтр("ru='Для отправки письма необходимо установить расширение для работы с файлами.';uk='Для відправлення листа необхідно встановити розширення для роботи з файлами.'");
...
Процедура СоздатьSMS(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОтправкаSMS") Тогда
		ВызватьИсключение НСтр("ru='Отправка SMS недоступна.';uk='Відправлення SMS недоступне.'");
	КонецЕсли;
	...
	Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Нельзя отправитьSMS по контактной информацию с типом ""%1""';uk='Неможливо відправити ЅМЅ за контактною інформацією з типом ""%1""'"), ТипИнформации);
	КонецЕсли;
	
	Если ЗначенияПолей = "" И ПустаяСтрока(Представление) Тогда
		ПоказатьПредупреждение(,НСтр("ru='Для отправки SMS необходимо ввести номер телефона.';uk='Для відправки SMS необхідно ввести номер телефону.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПерейтиПоВебСсылке(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено) Экспорт
	...
	Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.ВебСтраница") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Нельзя открыть ссылку по контактной информации с типом ""%1""';uk='Не можна відкрити посилання за контактною інформацією з типом ""%1""'"), ТипИнформации);
	КонецЕсли;
	...
	Если ТипЗнч(АдресСсылки) <> Тип("Строка") Тогда
		ВызватьИсключение НСтр("ru='Ошибка получения ссылки, неверный тип контактной информации';uk='Помилка одержання посилання, невірний тип контактної інформації'");
	КонецЕсли;
...
// Показывает адрес в браузере на картах Google
//
// Параметры:
//  Адрес						 - Строка - Текстовое представление адреса
//  ИмяКартографическогоСервиса	 - Строка - Имя картографического сервиса в котором необходимо показать адрес: GoogleMaps.
//
Процедура ПоказатьАдресНаКарте(Адрес, ИмяКартографическогоСервиса) Экспорт
	...
	Если ИмяКартографическогоСервиса = "GoogleMaps" Тогда
		СтрокаЗапуска = "https://maps.google.com.ua/?q=" + АдресКодированный;
	КонецЕсли;
...
Функция СтраныПоддерживающиеСтруктурированныйВвод() Экспорт
	...
	СписокСтран.Вставить("БІЛОРУСЬ", Истина);
	СписокСтран.Вставить("КАЗАХСТАН", Истина);
	СписокСтран.Вставить("ВІРМЕНІЯ", Истина);
	СписокСтран.Вставить("КИРҐИЗСТАН", Истина);
	СписокСтран.Вставить("РОССИЯ", Истина);
	СписокСтран.Вставить("РОСІЙСЬКА ФЕДЕРАЦІЯ", Истина);
	СписокСтран.Вставить("УКРАИНА", Истина);
	СписокСтран.Вставить("УКРАЇНА", Истина);
	Возврат СписокСтран;
КонецФункции
...
Процедура ПослеВыбораИзМенюSkype(ВыбранныйЭлемент, Параметры) Экспорт
	...
	#Если НЕ ВебКлиент Тогда
		Если ПустаяСтрока(ПрограммаТелефонииУстановлена("skype")) Тогда
			ПоказатьПредупреждение(Неопределено, НСтр("ru='Для совершения звонка по Skype требуется установить программу.';uk='Для здійснення дзвінка по Skype потрібно встановити програму.'"));
			Возврат;
		КонецЕсли;
	#КонецЕсли
	...
	ТекстСообщения = НСтр("ru='Для запуска Skype необходимо установить расширение работы с файлами.';uk='Для запуску Skype необхідно встановити розширення роботи з файлами.'");
...
Процедура ПозвонитьПоТелефону(НомерТелефона)
	...
	#Если НЕ ВебКлиент Тогда
		...
		Если ДоступноеИмяПротокола = Неопределено Тогда
			СтрокаСПредупреждением = Новый ФорматированнаяСтрока(
					НСтр("ru='Для совершения звонка требуется установить программу телефонии, например';uk='Для здійснення дзвінка потрібно встановити програму телефонії, наприклад'"),
	...
	ТекстСообщения = НСтр("ru='Для совершения звонка необходимо установить расширение работы с файлами.';uk='Для здійснення дзвінка необхідно встановити розширення роботи з файлами.'");
	ОбщегоНазначенияКлиент.ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(Оповещение, ТекстСообщения);
...
Процедура ЗаполнитьАдрес(Знач Форма, Знач ИмяРеквизита, Знач НайденнаяСтрока, Знач Результат)
	...
	Если АдресаДляЗаполнения.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Для того чтобы не вводить один и тот же адрес несколько раз, можно скопировать в это поле значение, ранее введенное в соседнем поле.';uk='Для того щоб не вводити одну і ту саму адресу кілька разів можна скопіювати в це поле значення, раніше введене в сусідньому полі.'")
			,, НСтр("ru='Копирование адреса';uk='Копіювання адреси'"));
		Возврат;
	КонецЕсли;
