...
Процедура ЗапланироватьОбновлениеОбластейДанных(Знач БлокироватьОбласти = Истина, Знач СообщениеБлокировки = "") Экспорт
	...
	Если ПустаяСтрока(СообщениеБлокировки) Тогда
		...
		Если ПустаяСтрока(СообщениеБлокировки) Тогда
			СообщениеБлокировки = НСтр("ru='Система заблокирована для выполнения обновления.';uk='Система заблокована для виконання оновлення.'");
		КонецЕсли;
	КонецЕсли;
		...
		Если ИдентификаторБиблиотеки = Метаданные.Имя Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если НЕ Выборка.Обновлена Тогда
					...
					ШаблонКомментария = НСтр("ru='Обновление версии конфигурации было выполнено до обновления версии библиотеки %1';uk='Оновлення версії конфігурації було виконано до оновлення версії бібліотеки %1'");
				...
				Если ОписаниеПланаОбновленияБиблиотеки = Неопределено Тогда
					...
					ШаблонКомментария = НСтр("ru='Не найден план обновления библиотеки %1';uk='Не знайдений план оновлення бібліотеки %1'");
					ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКомментария, Выборка.Имя);
					ЗаписьЖурналаРегистрации(
						ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
						УровеньЖурналаРегистрации.Ошибка,
				...
				Если ОписаниеПланаОбновленияБиблиотеки.ВерсияНа <> Выборка.Версия Тогда
					...
					ШаблонКомментария = НСтр("ru='Обнаружен некорректный план обновления библиотеки %1
                        ...
                        |;uk='Виявлено некоректний план оновлення бібліотеки %1
                        |Потрібен план оновлення на версію %2, знайдений план для оновлення на версію %3'");
	   ...
	   И ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		Если ОбновлениеИнформационнойБазыСлужебный.НеобходимоВыполнитьОбновление(Метаданные.Версия, ВерсияОбщихДанных) Тогда
			Сообщение = НСтр("ru='Не выполнена общая часть обновления информационной базы.
                ...
                |;uk='Не виконана загальна частина оновлення інформаційної бази.
	...
	Если КлючЗаписи <> Неопределено Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации() + "." 
				+ НСтр("ru='Обновление области данных';uk='Оновлення області даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение(НСтр("ru='Ошибка обновления области данных. Запись версий области данных заблокирована.';uk='Помилка оновлення області даних. Запис версій області даних заблокована.'"));
		КонецПопытки;
	КонецЕсли;
	Возврат КлючЗаписи;
	...
	Иначе
		...
		Если Не МонопольныйРежим() Тогда
			...
			Исключение
				...
				СтрокаСообщения = НСтр("ru='Не удалось установить монопольный режим. Описание ошибки: %1';uk='Не вдалося встановити монопольний режим. Опис помилки: %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция МинимальнаяВерсияОбластейДанных() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() И ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("ru='Вызов функции ОбновлениеИнформационнойБазыСлужебныйПовтИсп.МинимальнаяВерсияИБ()
                                     ...
                                     |;uk='Виклик функції ОбновлениеИнформационнойБазыСлужебныйповтисп.МинимальнаяВерсияИБ()
                                     |недоступний із сеансів з установленим значенням роздільників моделі сервісу!'");
	КонецЕсли;
