...
Процедура ИнициализироватьМеханизмФиксацииРеквизитов(Форма, ТекущийОбъект, ПараметрыФиксацииВторичныхДанных = Неопределено) Экспорт
	...
	СоздатьДополнительныеРеквизитыФормы(Форма, ПараметрыФиксацииВторичныхДанных);
	СоздатьРеквизитыФормыФикс(Форма);
	...
	Для каждого ОписаниеФиксацииРеквизита Из ПараметрыФиксацииВторичныхДанных.ОписаниеФиксацииРеквизитов Цикл
		Если ОписаниеФиксацииРеквизита.Значение.РеквизитСтроки Тогда
			...
			Для каждого СтрокаТЧ Из Объект Цикл
				ЗафиксированныйРеквизит = ТекущийОбъект[ПараметрыФиксацииВторичныхДанных.ИмяТабличнойЧастиФиксацияИзменений].НайтиСтроки(Новый Структура("ИмяРеквизита, Путь, ИдентификаторСтроки" , ОписаниеФиксацииРеквизита.Значение.ИмяРеквизита, ОписаниеФиксацииРеквизита.Значение.Путь, СтрокаТЧ.ИдентификаторСтрокиФикс));
				СтрокаТЧ[ПоследнееПоле] = ЗафиксированныйРеквизит.Количество() > 0;
			КонецЦикла;
		Иначе
			ЗафиксированныйРеквизит = ТекущийОбъект[ПараметрыФиксацииВторичныхДанных.ИмяТабличнойЧастиФиксацияИзменений].НайтиСтроки(Новый Структура("ИмяРеквизита, Путь" ,ОписаниеФиксацииРеквизита.Значение.ИмяРеквизита, ОписаниеФиксацииРеквизита.Значение.Путь));
...
// Процедура создает для каждого реквизита формы подключенного к механизму фиксации изменений дублирующий реквизит с
// постфиксом Фикс.
//    	ПараметрыФиксацииВторичныхДанных - структура см.
//    	                                   ФиксацияВторичныхДанныхВДокументахКлиентСервер.ПараметрыФиксацииВторичныхДанных.
//
Процедура СоздатьРеквизитыФормыФикс(Форма)
	...
	МассивПутейДобавляемыеРеквизиты = Новый Массив;
	...
	Для каждого ОписаниеФиксацииРеквизита Из ПараметрыФиксацииВторичныхДанных.ОписаниеФиксацииРеквизитов Цикл
		...
		Если ОписаниеФиксацииРеквизита.Значение.РеквизитСтроки Тогда
			...
			Если МассивПутейДобавляемыеРеквизиты.Найти(Путь) = Неопределено Тогда
				МассивПутейДобавляемыеРеквизиты.Добавить(Путь)
			КонецЕсли;
		КонецЕсли;
	...
	Для каждого Путь Из МассивПутейДобавляемыеРеквизиты Цикл
...
Процедура ДобавитьУсловноеОформлениеНеЖирныйШрифт(Форма, ОписаниеФиксацииРеквизита, ПрефиксПути, МассивЭлементов)
	...
	НеЖирныйШрифт  = Новый Шрифт(ШрифтыСтиля.ОбычныйШрифтТекста);
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Шрифт", НеЖирныйШрифт);
...
//
Процедура СоздатьДополнительныеРеквизитыФормы(Форма, ПараметрыФиксацииВторичныхДанных)
	...
	Если ПредупреждениеОбновленияВторичныхДанных = Неопределено 
		...
		ПредупреждениеОбновленияВторичныхДанных.Заголовок = НСтр("ru='Автозаполняемые данные были обновлены.';uk='Дані, що заповнюються автоматично, були оновлені.'");
		...
		ПредупреждениеОбновленияВторичныхДанных.Подсказка = НСтр("ru='Вторичные данные используемые в документе, такие, как ФИО или паспортные данные сотрудника, были изменены, например, в карточке сотрудника или в первичном документе.';uk='Вторинні дані, що використовуються в документі, такі, як ПІБ або паспортні дані співробітника, були змінені, наприклад, в картці співробітника або в первинному документі.'");		
