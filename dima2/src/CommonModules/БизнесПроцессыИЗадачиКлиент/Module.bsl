...
Процедура Остановить(Знач ПараметрКоманды) Экспорт
	...
	Если ТипЗнч(ПараметрКоманды) = Тип("Массив") Тогда
		...
		Если ПараметрКоманды.Количество() = 0 Тогда
			ПоказатьПредупреждение(,НСтр("ru='Не выбран ни один бизнес-процесс.';uk='Не обраний жоден бізнес-процес.'"));
			Возврат;
		КонецЕсли;	
		...
		Если ПараметрКоманды.Количество() = 1 Тогда
			Если ЧислоЗадач > 0 Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Будет выполнена остановка бизнес-процесса ""%1"" и всех его невыполненных задач (%2). Продолжить?';uk='Буде виконана зупинка бізнес-процесу ""%1"" і всіх його невиконаних задач (%2). Продовжити?'"), 
					Строка(ПараметрКоманды[0]), ЧислоЗадач);
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Будет выполнена остановка бизнес-процесса ""%1"". Продолжить?';uk='Буде виконана зупинка бізнес-процесу ""%1"". Продовжити?'"), 
					Строка(ПараметрКоманды[0]));
			КонецЕсли;		
		Иначе
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Будет выполнена остановка бизнес-процессов (%1) и всех их невыполненных задач (%2). Продолжить?';uk='Буде виконана зупинка бізнес-процесів (%1) і всіх їх невиконаних задач (%2). Продовжити?'"), 
	...
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет, НСтр("ru='Остановка бизнес-процесса';uk='Зупинка бізнес-процесу'"));
...
Процедура ОстановитьБизнесПроцессИзФормыОбъекта(Форма) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Бизнес-процесс остановлен';uk='Бізнес-процес зупинено'"),
...
Процедура СделатьАктивным(Знач ПараметрКоманды) Экспорт
	...
	Если ТипЗнч(ПараметрКоманды) = Тип("Массив") Тогда
		...
		Если ПараметрКоманды.Количество() = 1 Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Бизнес-процесс ""%1"" и все его задачи (%2) будут сделаны активными. Продолжить?';uk='Бізнес-процес ""%1"" і всі його задачі (%2) будуть зроблені активними. Продовжити?'"), 
				Строка(ПараметрКоманды[0]), ЧислоЗадач);
		Иначе		
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Бизнес-процессы (%1) и их задачи (%2) будут сделаны активными. Продолжить?';uk='Бізнес-процеси (%1) та їх задачі (%2) будуть зроблені активними. Продовжити?'"), 
				ПараметрКоманды.Количество(), ЧислоЗадач);
		КонецЕсли;		
...
Процедура ПринятьЗадачиКИсполнению(Знач МассивЗадач) Экспорт
	...
	ИначеЕсли МассивЗадач.Количество() <> 1 Тогда
		Состояние(НСтр("ru='Задачи приняты к исполнению.';uk='Задачі прийняті до виконання.'"));
	Иначе		
		Состояние(НСтр("ru='Задача принята к исполнению.';uk='Задача прийнята до виконання.'"));
	КонецЕсли;	
...
Процедура ОтменитьПринятиеЗадачКИсполнению(Знач МассивЗадач) Экспорт
	...
	Если МассивЗадач.Количество() = 0 Тогда
		...
		Возврат;
	ИначеЕсли МассивЗадач.Количество() <> 1 Тогда
		Состояние(НСтр("ru='Задачи помечены как НЕ принятые к исполнению.';uk='Задачі помічені як НЕ прийняті до виконання.'"));
	Иначе		
		Состояние(НСтр("ru='Задача помечена как НЕ принятая к исполнению.';uk='Задача позначена як НЕ прийнята до виконання.'"));
	КонецЕсли;		
...
Функция ЗаписатьИЗакрытьВыполнить(Форма, ВыполнитьЗадачу = Ложь, ПараметрыОповещения = Неопределено) Экспорт
	...
	Если НЕ Форма.НачальныйПризнакВыполнения И ВыполнитьЗадачу Тогда
		Если НЕ Форма.Записать(Новый Структура("ВыполнитьЗадачу", Истина)) Тогда
			Возврат Ложь;
		КонецЕсли;
		ТекстОповещения = НСтр("ru='Задача выполнена';uk='Задача виконана'");
	Иначе
		Если НЕ Форма.Записать() Тогда
			Возврат Ложь;
		КонецЕсли;
		ТекстОповещения = ?(НовыйОбъект, НСтр("ru='Задача создана';uk='Задача створена'"), НСтр("ru='Задача изменена';uk='Задача змінена'"));
	КонецЕсли;
...
Процедура ПеренаправитьЗадачи(МассивЗадач, ВладелецФорма) Экспорт
	...
	Если НЕ ЗадачиМогутБытьПеренаправлены И МассивЗадач.Количество() = 1 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Невозможно перенаправить уже выполненную задачу или направленную другому исполнителю.';uk='Неможливо передати вже виконану задачу або спрямовану іншому виконавцеві.'"));
		Возврат;
	КонецЕсли;
	...
	ОткрытьФорму("Задача.ЗадачаИсполнителя.Форма.ПеренаправитьЗадачи",
		...
		?(МассивЗадач.Количество() > 1, НСтр("ru='Перенаправить задачи';uk='Перенаправити задачі'"), 
			НСтр("ru='Перенаправить задачу';uk='Перенаправити задачу'"))), 
...
Процедура ОткрытьБизнесПроцесс(Список) Экспорт
	...
	Если Список.ТекущиеДанные.БизнесПроцесс = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ru='У выбранной задачи не указан бизнес-процесс.';uk='У вибраній задачі не зазначений бізнес-процес.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьПредметЗадачи(Список) Экспорт
	...
	Если Список.ТекущиеДанные.Предмет = Неопределено Тогда
		ПоказатьПредупреждение(,НСтр("ru='У выбранной задачи не указан предмет.';uk='У вибраній задачі не зазначений предмет.'"));
		Возврат;
	КонецЕсли;
...
Процедура СписокБизнесПроцессовПометкаУдаления(Список) Экспорт
	...
	ПоказатьВопрос(Оповещение, НСтр("ru='Изменить пометку удаления?';uk='Змінити позначку вилучення?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОстановитьЗавершение(Знач Результат, Знач ПараметрКоманды) Экспорт
	...
	Если ТипЗнч(ПараметрКоманды) = Тип("Массив") Тогда
		Если ПараметрКоманды.Количество() = 1 Тогда
			Состояние(НСтр("ru='Идет остановка бизнес-процесса. Пожалуйста подождите...';uk='Йде зупинка бізнес-процесу. Будь ласка, зачекайте...'"));
		Иначе
			Состояние(НСтр("ru='Идет остановка бизнес-процессов. Пожалуйста подождите...';uk='Йде зупинка бізнес-процесів. Будь ласка, зачекайте...'"));
		КонецЕсли;	
		
		БизнесПроцессыИЗадачиВызовСервера.ОстановитьБизнесПроцессы(ПараметрКоманды);
		
		Если ПараметрКоманды.Количество() = 1 Тогда
			Состояние(НСтр("ru='Бизнес-процесс остановлен.';uk='Бізнес-процес зупинено.'"));
		Иначе	
			Состояние(НСтр("ru='Бизнес-процессы остановлены.';uk='Бізнес-процеси зупинені.'"));
		КонецЕсли;	
	Иначе	
		Состояние(НСтр("ru='Идет остановка бизнес-процесса. Пожалуйста подождите...';uk='Йде зупинка бізнес-процесу. Будь ласка, зачекайте...'"));
...
Процедура СписокБизнесПроцессовПометкаУдаленияЗавершение(Результат, Список) Экспорт
	...
	Список.Обновить();
	ПоказатьОповещениеПользователя(НСтр("ru='Пометка удаления изменена.';uk='Позначка вилучення змінена.'"), 
...
Процедура СделатьАктивнымЗавершение(Знач Результат, Знач ПараметрКоманды) Экспорт
	...
	Если ТипЗнч(ПараметрКоманды) = Тип("Массив") Тогда
		...
		Если ПараметрКоманды.Количество() = 1 Тогда
			Состояние(НСтр("ru='Бизнес-процесс и его задачи делаются активными. Пожалуйста подождите...';uk='Бізнес-процес та його задачі робляться активними. Будь ласка, зачекайте...'"));
		Иначе	
			Состояние(НСтр("ru='Бизнес-процессы и их задачи делаются активными. Пожалуйста подождите...';uk='Бізнес-процеси та їх задачі робляться активними. Будь ласка, зачекайте...'"));
		КонецЕсли;	
		
		БизнесПроцессыИЗадачиВызовСервера.СделатьАктивнымБизнесПроцессы(ПараметрКоманды);
		
		Если ПараметрКоманды.Количество() = 1 Тогда
			Состояние(НСтр("ru='Бизнес-процесс и его задачи сделаны активными.';uk='Бізнес-процес та його задачі зроблені активними.'"));
		Иначе	
			Состояние(НСтр("ru='Бизнес-процессы и их задачи сделаны активными.';uk='Бізнес-процеси та їх задачі зроблені активними.'"));
		КонецЕсли;	
		
	Иначе	
		Состояние(НСтр("ru='Идет отмена остановки бизнес-процесса. Пожалуйста подождите...';uk='Йде скасування зупинки бізнес-процесу. Будь ласка, зачекайте...'"));
...
Процедура ПеренаправитьЗадачиЗавершение(Знач Результат, Знач МассивЗадач) Экспорт
	...
	Если МассивЗадач.Количество() > 10 Тогда
		Состояние(НСтр("ru='Перенаправление задач';uk='Перенаправлення задач'"),, 
			НСтр("ru='Выполняется перенаправление задач...';uk='Виконується перенаправлення задач...'"));
	КонецЕсли;
	...
	Если МассивЗадач.Количество() > 1 Тогда
		Если ЗадачиПеренаправлены Тогда
			Состояние(НСтр("ru='Перенаправление задач';uk='Перенаправлення задач'"),, 
				НСтр("ru='Задачи перенаправлены:';uk='Задачі перенаправлені:'") + " " + МассивЗадач.Количество());
		Иначе
			Состояние(НСтр("ru='Перенаправление задач';uk='Перенаправлення задач'"),,
				НСтр("ru='Не все задачи перенаправлены. Пропущены задачи, отмеченные как выполненные.';uk='Не всі задачі перенаправлені. Пропущені задачі, позначені як виконані.'"));
		КонецЕсли;
	Иначе
		Задача = МассивПеренаправленныхЗадач[0];
		ПоказатьОповещениеПользователя(
			НСтр("ru='Задача перенаправлена';uk='Задача перенаправлена'"),
