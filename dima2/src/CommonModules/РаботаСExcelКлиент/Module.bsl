...
Процедура ВыгрузитьВExcel(Форма, ИмяПроцедуры = "") Экспорт 
	
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ВыгрузитьВExcelРасширениеПодключено",
		ЭтотОбъект,
		Новый Структура("Форма, ИмяПроцедуры", Форма, ИмяПроцедуры));
...
// Параметры:
//  Форма				 - УправляемаяФорма	 - Форма документа
//  ИмяЭлементаВыбора	 - ЭлементФормы	 - Элемент формы в который вернется оповещение о выборе
Процедура ВыбратьПрисоединенныйФайлExcel(Форма, ИмяЭлементаВыбора) Экспорт 
	...
	Если НЕ ЗначениеЗаполнено(Форма.Объект.Ссылка) Тогда
		...
		ПоказатьВопрос(
			Оповещение, 
			НСтр("ru='Перед загрузкой документ необходимо записать. Выполнить запись документа и продолжить?';uk='Перед завантаженням документ необхідно записати. Виконати запис документа і продовжити?'"),
			Кнопки);
		Возврат;
...
// Параметры:
//  Форма					 - УправляемаяФорма	 - Форма документа
//  ПрисоединенныйФайл		 - СправочникСсылка.ПланЗакупокПрисоединенныеФайлы и др. - Ссылка на выбранный присоединенный файл
//  ПараметрыЗагрузкиИзExcel - Структура - Параметры загрузки из Excel
// Возвращаемое значение:
//  Массив - Массив, содержащий загруженные из Excel строки
//
Процедура ЗагрузитьИзExcel(Форма, ПрисоединенныйФайл, ПараметрыЗагрузкиИзExcel = Неопределено) Экспорт
	
	ДанныеФайла = ПрисоединенныеФайлыСлужебныйВызовСервера.ПолучитьДанныеФайла(
		ПрисоединенныйФайл, 
...
Процедура ВыгрузитьВExcelРасширениеПодключено(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если Форма.Модифицированность ИЛИ НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ДополнительныеПараметры = Новый Структура("Форма, ИмяПроцедуры", Форма, ИмяПроцедуры);
	...
	СоздатьИОбработатьПрисоединенныйФайлExcel(Форма, ИмяПроцедуры);
...
Процедура ВыгрузитьВExcelПослеЗаписи(Результат, ДополнительныеПараметры) Экспорт 
	...
	СоздатьИОбработатьПрисоединенныйФайлExcel(Форма, ДополнительныеПараметры.ИмяПроцедуры);
...
Процедура ВыбратьПрисоединенныйФайлExcelРасширениеПодключено(Результат, ДополнительныеПараметры) Экспорт 
	...
	ПрисоединенныеФайлыКлиент.ОткрытьФормуВыбораФайлов(
...
Процедура ЗагрузитьИзExcelПолучениеФайла(Результат, ДополнительныеПараметры) Экспорт
	...
	Исключение
		COMОбъект.Quit();
		COMОбъект = 0;
		СообщениеОбОшибке = НСтр("ru='Не удалось прочитать данные из файла. Подробности:';uk='Не вдалося прочитати дані з файлу. Подробиці:'") + " "
		                    + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение СообщениеОбОшибке;
	КонецПопытки;
	...
	Оповестить("РаботаСExcelКлиент_ЗагрузитьИзExcel", ДанныеЗагрузки);
	
КонецПроцедуры


Процедура СоздатьИОбработатьПрисоединенныйФайлExcel(Форма, ИмяПроцедуры)
	
	Информация = РаботаСExcelВызовСервера.СоздатьПрисоединенныйФайлExcel(Форма.Объект.Ссылка, Форма.УникальныйИдентификатор, ИмяПроцедуры);
	Если Информация = Неопределено Тогда
		СообщениеОбОшибке = НСтр("ru='Не удалось выгрузить документ в Excel.';uk='Не вдалося вивантажити документ в Excel.'");
		ВызватьИсключение СообщениеОбОшибке;
	КонецЕсли;
	...
	ДополнительныеПараметры =
		Новый Структура("Форма, ИмяПроцедуры, Информация", Форма, ИмяПроцедуры, Информация);
	...
	Исключение
		СообщениеОбОшибке =
			НСтр("ru='Не удалось выгрузить документ в Excel.
                ...
                |Подробности:'
                |;uk='Не вдалося вивантажити документ в Excel.
