...
Функция ИспользуемаяВерсияСервисаУправляющегоПриложения(Знач ПарольПользователя = "") Экспорт
	...
	Если ПоддерживаемыеВерсии.Количество() > 0 Тогда
		...
		Для Каждого ПоддерживаемаяВерсия Из ПоддерживаемыеВерсии Цикл
			Если (ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ПоддерживаемаяВерсия, МинимальнаяВерсия) >= 0) И (МаксимальнаяВерсия = Неопределено Или (ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ПоддерживаемаяВерсия, МаксимальнаяВерсия) > 0)) Тогда
				МаксимальнаяВерсия = ПоддерживаемаяВерсия;
			КонецЕсли;
		КонецЦикла;
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Корреспондент %1 не поддерживает интерфейс %2!';uk='Кореспондент %1 не підтримує інтерфейс %2!'"),
...
Процедура ЗавершитьСеансОбластиДанных(Знач НомерСеанса, Знач ПарольПользователя) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='Текущая версия управляющего приложения не поддерживает завершение сеанса из приложений. Необходимо обновить управляющее приложение.';uk='Поточна версія керуючої прикладної програми не підтримує завершення сеансу з прикладної програми. Необхідно оновити керуючу прикладну програму.'");
...
Процедура ОтправитьЗапросыНаИспользованиеВнешнихРесурсов(Знач СериализацияЗапросов) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='Текущая версия управляющего приложения не поддерживает запросы разрешений на использование внешних ресурсов. Необходимо обновить управлящее приложение.';uk='Поточна версія керуючої прикладної програми не підтримує запити дозволів на використання зовнішніх ресурсів. Необхідно оновити керуючу прикладну програму.'");
