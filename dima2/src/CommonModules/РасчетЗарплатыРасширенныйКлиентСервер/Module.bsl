...
Функция ПредставлениеСпособаРасчетаАванса(СпособРасчетаАванса, СжатыйФормат) Экспорт
	...
	Если СпособРасчетаАванса = ПредопределенноеЗначение("Перечисление.СпособыРасчетаАванса.РасчетомЗаПервуюПоловинуМесяца") Тогда
		СтрокаПодстановки = ?(СжатыйФормат, "", НСтр("ru='Ранее получал аванс %1';uk='Раніше отримував аванс %1'"));
	Иначе
...
//
Функция НачислениеВыполняетсяВЦеломЗаМесяц(НачислениеОбъект, ЕстьПоказателиУчетаВремени, ЕстьПоказательРасчетнаяБаза, ЕстьОперативныеПоказатели) Экспорт 
	...
	Если НачислениеОбъект.КатегорияНачисленияИлиНеоплаченногоВремени = ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.Индексация")
		ИЛИ НачислениеОбъект.КатегорияНачисленияИлиНеоплаченногоВремени = ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ДоплатаДоМЗП") Тогда
		//Индексация и доплата до МЗП зависят от времени, но выполнять их надо все равно за месяц
		Возврат Истина;
	КонецЕсли;	
...
Функция ФормаДокументаГотоваДляРасчетаЗарплаты(Форма, ОписаниеДокумента, СообщатьПользователю = Истина) Экспорт
	...
	Для Каждого ОписаниеПоля Из ОписаниеДокумента.ОбязательныеПоля Цикл
		...
		Если Не ЗначениеЗаполнено(ЗначениеРеквизита) Тогда
			...
			Если СообщатьПользователю Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Поле ""%1"" не заполнено';uk='Поле ""%1"" не заповнене'"), ОписаниеПоля.ИмяПоля);
	...
	Для Каждого ОписаниеПериода Из ОписаниеДокумента.ПроверяемыеПериоды Цикл
		...
		Если ЗначениеЗаполнено(ЗначениеРеквизитаНачало) 
			...
			Отказ = Истина;
			Если СообщатьПользователю Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='""%1"" не может быть больше ""%2""';uk='""%1"" не може бути більше ""%2""'"), ОписаниеПериода.ИмяПоляНачало, ОписаниеПериода.ИмяПоляОкончание);
