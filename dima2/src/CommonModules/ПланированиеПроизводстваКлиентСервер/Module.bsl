...
//  ИнтервалПланирования - Перечисление.ТочностьГрафикаПроизводства - интервал планирования
...
//       * ДатаПотребности - Дата - дата потребности
//
Функция СтруктураСпособыПолученияМатериаловПоУмолчанию() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ПолучениеМатериалов", ПредопределенноеЗначение("Перечисление.СпособыПолучениеМатериаловЭтапаПроизводства.ЗаказатьНаСклад"));
...
//    * Назначение - СправочникСсылка.Назначения - назначение продукции заказа на производство
//    * НазначениеЗаказа - СправочникСсылка.Назначения - назначение заказа на производство 
//  РежимОбновления - режим обновления назначений для выходных изделий ("ПоВсем", "ПоПродукции")
//
...
Функция ЕстьОшибкиЗаполненияНазначенияВСтрокеТЧМатериалыИУслуги(Знач ТекущиеДанные) Экспорт
	...
	ТребуетсяОбособление = 
		...
		ИЛИ ТекущиеДанные.ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно");
	
	Если НЕ ТребуетсяОбособление И ЗначениеЗаполнено(ТекущиеДанные.Назначение)
		ИЛИ ТребуетсяОбособление И НЕ ЗначениеЗаполнено(ТекущиеДанные.Назначение) Тогда
...
Функция НеобходимоОбновитьНазначениеВСтрокеТЧМатериалыИУслуги(Знач ТекущиеДанные, Знач СохраненныеЗначения, Знач НоваяСтрока = Ложь) Экспорт
	...
	ОбособленнаяСхема = Новый Массив;
	ОбособленнаяСхема.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.Обособленно"));
	ОбособленнаяСхема.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно"));
	
	Если ЕстьОшибкиЗаполненияНазначенияВСтрокеТЧМатериалыИУслуги(ТекущиеДанные)
			ИЛИ (ОбособленнаяСхема.Найти(ТекущиеДанные.ВариантОбеспечения) <> Неопределено) 
					<> (ОбособленнаяСхема.Найти(СохраненныеЗначения.ВариантОбеспечения) <> Неопределено) Тогда
				...
				Если Ошибка.КлючСвязиЭтап = Этап.КлючСвязи Тогда
					...
					Если ТипЗнч(Ошибка.ВидыРабочихЦентров) = Тип("Массив") Тогда
						...
						Для каждого ВидРабочегоЦентра из Ошибка.ВидыРабочихЦентров Цикл
							...
							ТочкаВидРабочегоЦентра.Текст = ВидРабочегоЦентраНаименование + НСтр("ru='. Доступности вида рабочего центра недостаточно для размещения этапа.';uk='. Доступності виду робочого центру недостатньо для розміщення етапу.'");
					...
					Иначе
						...
						ТочкаПодразделение.Текст = ПодразделениеНаименование + НСтр("ru='. Не установлен график, по которому работает подразделение.';uk='. Не встановлено графік, за яким працює підрозділ.'");
...
Процедура ВывестиИнтервалыЭтапа(ДиаграммаГанта, Этап, ТочкаЭтап, СледующиеЭтапы, СоответствиеКлючейСвязиИнтерваловИИнтерваловДиаграммы, ПараметрыВывода)
	...
	Для каждого Интервал из Этап.Интервалы Цикл
		...
		ИнтервалДиаграммыГанта.Текст       = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru='Производство %1 ед/партий изделий, с %2 по %3';uk='Виробництво %1 од/партій виробів, з %2 по %3'"),
		...
		Если Интервал.ЕстьМаршрутныеЛисты Тогда
			...
			ИнтервалДиаграммыГанта.Текст = ИнтервалДиаграммыГанта.Текст + " (" + НСтр("ru='сформированы маршрутные листы';uk='сформовані маршрутні листи'") + ")";
		...
		Если ОкончаниеЗавершающегоБуфера > ОкончаниеЭтапа Тогда
			...
			ИнтервалДиаграммыГантаЗавершующийБуфер = ЗначенияДиаграммыГанта.Добавить();
			
			ИнтервалДиаграммыГантаЗавершующийБуфер.Начало = ОкончаниеЗавершающегоБуфера;
			ИнтервалДиаграммыГантаЗавершующийБуфер.Конец  = ОкончаниеЗавершающегоБуфера;
			
			СвязьПредварительныйБуфер          = ИнтервалДиаграммыГанта.Добавить(ИнтервалДиаграммыГантаЗавершующийБуфер);
			СвязьПредварительныйБуфер.ТипСвязи = ТипСвязиДиаграммыГанта.КонецНачало;
			СвязьПредварительныйБуфер.Цвет     = WebЦвета.Зеленый;
			
			ИнтервалДобавленияСвязи = ИнтервалДиаграммыГантаЗавершующийБуфер;
...
Процедура ВывестиПолуфабрикаты(ДиаграммаГанта, Этапы, Полуфабрикаты, ТочкаРодитель, Ошибки, НеразвернутыеТочки, ПараметрыВывода)
	...
	Для каждого Полуфабрикат из Полуфабрикаты Цикл
		...
		ТочкаПолуфабрикат.Текст              = НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(Полуфабрикат.Номенклатура, Полуфабрикат.Характеристика);
...
Процедура ВывестиЭтапыРекурсивно(КлючСвязиПолуфабрикат, ТаблицаЭтапы, Ошибки, ИнтервалыДиаграммы, СерияЗанятость, РодительЭтапа, ДиаграммаГанта)
	...
	Для каждого ТекущийЭтап из НайденныеСтроки Цикл
		...
		Для каждого СтрокаМатериал Из ТекущийЭтап.Материалы Цикл
			...
			ТочкаМатериал.Текст = НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(
...
Процедура ВывестиРазмещениеЭтапа(Этап, ЗначениеЭтап, ИнтервалыДиаграммы)
	...
	Для каждого РазмещениеЭтапа из Этап.РазмещениеЭтапа Цикл
		...
		Если РазмещениеЭтапа.ЕстьМаршрутныеЛисты Тогда
			Интервал.Текст = Интервал.Текст + " (" + НСтр("ru='сформированы маршрутные листы';uk='сформовані маршрутні листи'") + ")";
		КонецЕсли;
