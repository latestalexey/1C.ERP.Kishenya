...
// Используется в форме документа "Поступление от переработчика", в процедурах, приводящих к изменению направления деятельности.
// Инициализирует реквизиты формы, используемые при интерактивной работе пользователя. Заполняет назначение, исходя из направления деятельности.
//
// Параметры:
//  Форма - Управляемая форма - форма в которой необходимо инициализировать реквизиты связанные с использованием направлений деятельности
//
Функция ПриИзмененииНаправленияДеятельностиПоступлениеОтПереработчикаСервер(Форма) Экспорт
	...
	Если ЗаполнятьНазначение Тогда
		...
		Для Каждого СтрокаТаблицы Из Форма.Объект.Товары Цикл
			...
			Если Не (ЗначениеЗаполнено(СтрокаТаблицы.ЗаказПереработчику) И СтрокаТаблицы.КодСтроки <> 0) Тогда
				НайденныеСтроки.Добавить(СтрокаТаблицы);
			КонецЕсли;
			
		КонецЦикла;
		
		Для Каждого СтрокаТаблицы Из Форма.Объект.ВозвратныеОтходы Цикл
