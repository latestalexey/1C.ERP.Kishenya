...
Процедура ЗаполнитьДанныеКопируемойСтроки(ДанныеОВремени, ЗаполняемыеСтроки, ИдентификаторКопируемойСтроки)
	...
	СтрокиИсточникиДанных = УчетРабочегоВремениКлиентСервер.ТабельСтрокиПоСотруднику(ДанныеОВремени, КопируемаяСтрока.НомерСтрокиСотрудник);
...
Процедура ТабельВводДанныхОВремениЗаДень(ДанныеОВремени, ДанныеТекущейСтроки, НомерДня)
	...
	ДанныеТекущейСтроки["Время" + НомерДня + "Представление"] = УчетРабочегоВремениКлиентСервер.ТабельПредставлениеВремениПоВиду(ДанныеОВремени.БуквенноеОбозначение, Часы);	
КонецПроцедуры
...
Функция ТабельДанныеОВремениИзПредставления(Форма, Знач ПредставлениеДанныхОВремени, ДанныеТекущейСтроки, НомерДня, Отказ = Ложь)
	ДанныеОВремени = УчетРабочегоВремениКлиентСервер.ТабельРазобратьСтрокуВремени(Форма.ОписаниеВидовВремени, Форма.ОбозначениеВидаВремениПоУмолчанию, ПредставлениеДанныхОВремени, Отказ);
	
	Если Отказ = Истина Тогда
		Возврат Неопределено;
	ИначеЕсли ДанныеОВремени = Неопределено Или ДанныеОВремени.БуквенноеОбозначение = "" И ДанныеОВремени.Часы = 0  Тогда 
		Возврат Новый Структура("ВидВремени, БуквенноеОбозначение, Часы, Целосменное",  ПредопределенноеЗначение("Справочник.ВидыИспользованияРабочегоВремени.ВыходныеДни"), "", 0, Ложь);	
...
Процедура ТабельДанныеОВремениВремяПредставлениеОкончаниеВводаТекста(Форма, Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка) Экспорт
	...
	НомерДня = УчетРабочегоВремениКлиентСервер.ТабельНомерДняПоИмениЭлемента(Элемент.Имя);
	ДанныеОВремени = ТабельДанныеОВремениИзПредставления(Форма, Текст, ДанныеТекущейСтроки, НомерДня, Отказ);       
	Если Отказ Тогда		
		ПоказатьПредупреждение(, НСтр("ru='В поле введены некорректные данные.';uk='У полі введені некоректні дані.'"));
...
Процедура ТабельДанныеОВремениВремяПредставлениеАвтоПодбор(Форма, Элемент, Текст, ДанныеВыбора, Ожидание, СтандартнаяОбработка) Экспорт
	...
	Если Не ПустаяСтрока(Текст) Тогда
		ДанныеОВремени = УчетРабочегоВремениКлиентСервер.ТабельРазобратьСтрокуВремени(
...
Процедура ТабельДанныеОВремениВремяПредставлениеОбработкаВыбора(Форма, Элемент, ВыбранноеЗначение, СтандартнаяОбработка) Экспорт
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда      
		...
		Если ДанныеТекущейСтроки <> Неопределено Тогда
			...
			ДанныеОВремени = УчетРабочегоВремениКлиентСервер.ТабельРазобратьСтрокуВремени(Форма.ОписаниеВидовВремени, Форма.ОбозначениеВидаВремениПоУмолчанию, ДанныеТекущейСтроки[ИмяТекущегоРеквизита]);
...
Процедура ТабельДанныеОВремениПередОкончаниемРедактирования(Форма, Элемент, НоваяСтрока, ОтменаРедактирования, Отказ) Экспорт 
	ДанныеТекущейСтроки = Форма.Элементы.ДанныеОВремени.ТекущиеДанные;
	Если ДанныеТекущейСтроки <> Неопределено Тогда
		ГоловнаяСтрока = УчетРабочегоВремениКлиентСервер.ТабельГоловнаяСтрокаТекущегоСотрудника(Форма.Объект.ДанныеОВремени, ДанныеТекущейСтроки);
		МассивСтрок = УчетРабочегоВремениКлиентСервер.ТабельСтрокиПоСотруднику(Форма.Объект.ДанныеОВремени, ГоловнаяСтрока.НомерСтрокиСотрудник);
		ОбозначенияВидовВремени = УчетРабочегоВремениКлиентСервер.ТабельОбозначенияВидовВремени(Форма.ОписаниеВидовВремени);
		УчетРабочегоВремениКлиентСервер.ТабельЗаполнитьИтогиПоСотруднику(МассивСтрок, ОбозначенияВидовВремени);
...
Процедура ТабельПереместитьСтроку(Форма, Направление)
	...
	ГоловнаяСтрокаПоСотруднику = УчетРабочегоВремениКлиентСервер.ТабельГоловнаяСтрокаТекущегоСотрудника(ДанныеОВремени, ДанныеТекущейСтроки);
...
Процедура ТабельВысотаСтрокиПриИзменении(Форма) Экспорт
	Если Форма.ВысотаСтроки > Форма.УстанавливаемаяВысотаСтроки И
		...
		ПоказатьПредупреждение(, НСтр("ru='Невозможно уменьшить максимальное количество вводимых видов времени за день.
                             ...
                             |;uk='Неможливо зменшити максимальну кількість дозволених видів часу за день.
                             |Можливо, є повністю заповнені дні або ви намагаєтеся встановити цей параметр менше 2-х.'"));
