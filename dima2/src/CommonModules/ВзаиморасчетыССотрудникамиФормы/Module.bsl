...
Процедура ВедомостьПриПолученииДанныхСтрокиСостава(Форма, СтрокаСостава) Экспорт
	...
	Если РабочиеМеста.Количество() > 1 Тогда
		РасшифровкаРабочихМест = НРег(ЧислоПрописью(РабочиеМеста.Количество(),,НСтр("ru='рабочее место, рабочих места, рабочих мест, с, ,,,,0';uk='робоче місце, робочих місця, робочих місць, з, ,,,,0'")));;
	КонецЕсли;
	...
	ИначеЕсли ПериодыВзаиморасчетов.Количество() = 1 Тогда
		...
		РасшифровкаПериодов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='за %1';uk='за %1'"), ПредставлениеПериода);
	Иначе
		...
		РасшифровкаПериодов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='в т.ч. за %1';uk='в т. ч. за %1'"), ПредставлениеПериодов);
...
Процедура ВедомостьУстановитьДоступностьЭлементов(Форма) Экспорт
	...
	Форма.ТолькоПросмотр = 
		ДатыЗапретаИзменения.ИзменениеЗапрещено(Форма.Объект.Ссылка.Метаданные().ПолноеИмя(), Форма.Объект.Ссылка) 
...
Функция СписокОплат(Форма)
	...
	ШаблонПредставления = НСтр("ru='%1 №%2 от %3 - %4';uk='%1 №%2 від %3 - %4'");
...
Функция ПредставлениеОплаты(СписокОплат)  
	...
	Если СписокОплат.Количество() = 0 Тогда
		...
		ПредставлениеОплаты = Новый ФорматированнаяСтрока(НСтр("ru='Выплаты по ведомости не выполнялись';uk='Виплати за відомістю не виконувалися'"));
	...
	ИначеЕсли СписокОплат.Количество() = 1 Тогда	
		...
		ПредставлениеОплаты = Новый ФорматированнаяСтрока(
			НСтр("ru='Выплата по ведомости выполнена документом';uk='Виплата за відомістю виконана документом'"),
			" ",
	...
	Иначе
		...
		ПредставлениеОплаты = Новый ФорматированнаяСтрока(
			НСтр("ru='Выплаты по ведомости:';uk='Виплати по відомості:'"),
			" ",
			Новый ФорматированнаяСтрока(
				НРег(ЧислоПрописью(СписокОплат.Количество(),,НСтр("ru='документ, документа, документов, м, ,,,,0';uk='документ, документа, документів, м, ,,,,0'"))),
				, , , "ссылка"));
