...
Функция ЗагрузитьЧастьФайла(ИдентификаторФайла, НомерЗагружаемойЧастиФайла, ЗагружаемаяЧастьФайла, СообщениеОбОшибке) Экспорт
	...
	Если Не ЗначениеЗаполнено(ИдентификаторФайла) Тогда
		СообщениеОбОшибке = НСтр("ru='Не указан идентификатор загружаемого файла. Дальнейшее выполнение метода невозможно.
                ...
                |;uk='Не вказано ідентифікатор файлу, що завантажується. Подальше виконання методу неможливо.
                |Необхідно файлу, що завантажується, призначити унікальний ідентифікатор.'");
		ВызватьИсключение(СообщениеОбОшибке);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ЗагружаемаяЧастьФайла)
		И ТипЗнч(ЗагружаемаяЧастьФайла) <> Тип("ДвоичныеДанные") Тогда
		СообщениеОбОшибке = НСтр("ru='Метод не может быть выполнен, т.к. переданные данные не соответствуют типу для получения данных.';uk='Метод не може бути виконаний, оскільки передані дані не відповідають типу для отримання даних.'");
		ВызватьИсключение(СообщениеОбОшибке);
	КонецЕсли;
...
Функция ВыгрузитьЧастьФайла(ИдентификаторФайла, НомерВыгружаемойЧастиФайла, СообщениеОбОшибке) Экспорт
	...
	Иначе
		СообщениеОбОшибке = НСтр("ru='Часть файла с указанным номером не найдена.';uk='Частина файлу з вказаним номером не знайдено.'");
	КонецЕсли;
...
Функция ПодготовитьФайлДляЗагрузки(ИдентификаторФайла, СообщениеОбОшибке) Экспорт
	...
	Иначе
		ШаблонСообщения = НСтр("ru='Не найден ни один фрагмент сессии передачи с идентификатором %1.
                ...
                |;uk='Не знайдено жодного фрагменту сесії передачі з ідентифікатором %1.
                |Необхідно переконатися, що в настройках програми задані параметри
                |""Каталог тимчасових файлів для Linux"" і ""Каталог тимчасових файлів для Windows"".'");
	...
	Если Разархиватор.Элементы.Количество() = 0 Тогда
		...
		СообщениеОбОшибке = НСтр("ru='Файл архива не содержит данных.';uk='Файл архіву не містить даних.'");
		ВызватьИсключение(СообщениеОбОшибке);
...
Процедура ЗагрузитьДанныеXDTOВИнформационнуюБазу(ПараметрыWEBСервиса, СообщениеОбОшибке) Экспорт
	...
	Исключение
		...
		СообщениеОбОшибке = НСтр("ru='Ошибка при загрузке данных: %1';uk='Помилка при завантаженні даних: %1'");
...
Процедура ПроверитьБлокировкуИнформационнойБазыДляОбновления() Экспорт
	...
	Если ЗначениеЗаполнено(ОбновлениеИнформационнойБазыСлужебный.ИнформационнаяБазаЗаблокированаДляОбновления()) Тогда
		...
		ВызватьИсключение НСтр("ru='Синхронизация данных временно недоступна в связи с обновлением приложения.';uk='Синхронізація даних тимчасово недоступна у зв''язку з оновленням програми.'");
...
Функция СформироватьКлючСообщенияЖР(НаправлениеОбмена, ПараметрыWEBСервиса)
	...
	Если НаправлениеОбмена = "Получение" Тогда
		ШаблонКлючаСообщения = НСтр("ru='Загрузка данных через Web-сервис %1';uk='Завантаження даних через Web-сервіс %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		ШаблонКлючаСообщения = НСтр("ru='Выгрузка данных через Web-сервис %1';uk='Вивантаження даних через Web-сервіс %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
