...
Процедура ВыполнитьПодключаемуюКомандуПечатиЗавершение(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		ИмяПоказателя = НСтр("ru='Печать';uk='Друк'") + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("/%1/%2/%3/%4/%5/%6/%7",
...
Процедура ПроверитьПроведенностьДокументовДиалогПроведения(Параметры) Экспорт
	...
	Если УправлениеПечатьюВызовСервера.ДоступноПравоПроведения(Параметры.НепроведенныеДокументы) Тогда
		Если Параметры.НепроведенныеДокументы.Количество() = 1 Тогда
			ТекстВопроса = НСтр("ru='Для того чтобы распечатать документ, его необходимо предварительно провести. Выполнить проведение документа и продолжить?';uk='Для того щоб роздрукувати документ, його необхідно попередньо провести. Виконати проведення документа й продовжити?'");
		Иначе
			ТекстВопроса = НСтр("ru='Для того чтобы распечатать документы, их необходимо предварительно провести. Выполнить проведение документов и продолжить?';uk='Для того щоб роздрукувати документи, їх необхідно попередньо провести. Виконати проведення документів і продовжити?'");
		КонецЕсли;
	Иначе
		Если Параметры.НепроведенныеДокументы.Количество() = 1 Тогда
			ТекстПредупреждения = НСтр("ru='Для того чтобы распечатать документ, его необходимо предварительно провести. Недостаточно прав для проведения документа, печать невозможна.';uk='Для того щоб роздрукувати документ, його необхідно попередньо провести. Недостатньо прав для проведення документа, друк неможливий.'");
		Иначе
			ТекстПредупреждения = НСтр("ru='Для того чтобы распечатать документы, их необходимо предварительно провести. Недостаточно прав для проведения документов, печать невозможна.';uk='Для того щоб роздрукувати документи, їх необхідно попередньо провести. Недостатньо прав для проведення документів, друк неможливий.'");
		КонецЕсли;
...
Процедура ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(ОписаниеОповещения)
	#Если ВебКлиент Тогда
		ТекстСообщения = НСтр("ru='Для продолжения печати необходимо установить расширение для веб-клиента платформы.';uk='Для продовження друку необхідно встановити розширення для веб-клієнта платформи.'");
