	...
	Если НЕ Формат9Запятых Тогда
		...
		Если XDTOТип = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
			...
			Если ПустаяСтрока(Состав.Страна) Тогда
				Страна = Справочники.СтраныМира.Украина;
		...
		Если ТипЗнч(АдресРФ) = Тип("Строка") Тогда
			...
			КонецЕсли;
		ИначеЕсли ТипЗнч(АдресРФ) = Тип("Неопределено") Тогда
			// адрес содержит только представление 
			ЭлементыАдреса.Вставить("Страна", НаименованиеСтраны);
			ЭлементыАдреса.Вставить("Индекс", "");
			ЭлементыАдреса.Вставить("Регион", "");
			ЭлементыАдреса.Вставить("Район", "");
...
Функция ПервоеЗначениеКонтактнойИнформацииОбъектаПоТипу(Ссылка, ТипКонтактнойИнформации) Экспорт
	...
	Если СтрНачинаетсяС(ПолноеИмя , НСтр("ru='Справочник';uk='Довідник'")) Тогда
		ИмяГруппыКонтактнойИнформации = НСтр("ru='Справочник';uk='Довідник'") + Ссылка.Метаданные().Имя;
	ИначеЕсли СтрНачинаетсяС(ПолноеИмя , НСтр("ru='Документ';uk='Документ'")) Тогда
		ИмяГруппыКонтактнойИнформации = НСтр("ru='Документ';uk='Документ'") + Ссылка.Метаданные().Имя;
	КонецЕсли;
...
#Область СлужебныеПроцедурыИФункцииКОАТУУ
...
//
Функция УровниКлассификатораКОАТУУ() Экспорт
...
//
Функция АдресаДляИнтерактивногоВыбораКОАТУУ(Родитель, Уровень, ДополнительныеПараметры) Экспорт
	...
	Уровни = УровниКлассификатораКОАТУУ();
	...
	Если Уровень = 1 Тогда
		Результат.Заголовок = НСтр("ru='Выберите область';uk='Оберіть область'");
	ИначеЕсли Уровень = 3 Тогда
		Результат.Заголовок = НСтр("ru='Выберите район';uk='Виберіть район'");
	ИначеЕсли Уровень = 4 Тогда
		Результат.Заголовок = НСтр("ru='Выберите город';uk='Виберіть місто'");
	ИначеЕсли Уровень = 6 Тогда
		Результат.Заголовок = НСтр("ru='Выберите населенный пункт';uk='Оберіть населений пункт'");
	ИначеЕсли Уровень = 7 Тогда
		Результат.Заголовок = НСтр("ru='Выберите улицу';uk='Виберіть вулицю'");
	КонецЕсли;
...
Процедура ЗаполнитьАдресаДляИнтерактивногоВыбораВнутр(Результат, Уровни, Родитель, Уровень, ДополнительныеПараметры)
	...
	Если ФормироватьПорцию Тогда
		...
		Если ДополнительныеПараметры.Свойство("ПерваяЗапись") И ЗначениеЗаполнено(ДополнительныеПараметры.ПерваяЗапись) Тогда
			ПараметрПерваяЗапись = ДополнительныеПараметры.ПерваяЗапись;
		КонецЕсли;
	...
	Если Уровень = 1 Тогда
		...
		ТекстЗапроса = "ВЫБРАТЬ " + ТекстРазмерПорции + "
			|	АдресаДляВыбораУровень1.Идентификатор КАК Идентификатор,
			|	АдресаДляВыбораУровень1.Наименование  КАК Представление
			|
			|ИЗ
			|	РегистрСведений.АдресаДляВыбора КАК АдресаДляВыбораУровень1
			|
			|" + ?(ПараметрПерваяЗапись = Неопределено, "", "
			|ЛЕВОЕ СОЕДИНЕНИЕ 
			|	РегистрСведений.АдресаДляВыбора КАК ТекущийПорядокПорции
			|ПО
			|	ТекущийПорядокПорции.Идентификатор = &ПервыйЭлементПорции
			|") + "
			|
			|ГДЕ
			|	АдресаДляВыбораУровень1.Уровень = 1
			|	
			|" + ?(ПараметрПерваяЗапись = Неопределено, "", "
			|	И АдресаДляВыбораУровень1.Идентификатор <> ТекущийПорядокПорции.Идентификатор
			|	И АдресаДляВыбораУровень1.Наименование " + СравнениеПорядкаПорции + " ТекущийПорядокПорции.Наименование
			|") + "
			|
			|УПОРЯДОЧИТЬ ПО
	...
	Иначе
		// Обычный уровень
		ТекстЗапроса = "ВЫБРАТЬ " + ТекстРазмерПорции + "
			|	ТекущийОбъект.Идентификатор КАК Идентификатор,
			|	ТекущийОбъект.Наименование  КАК Представление,
			|	ЛОЖЬ                        КАК Неактуален
			|ИЗ
			|	РегистрСведений.АдресаДляВыбора КАК РодительскийОбъект
			|
			|" + ?(ПараметрПерваяЗапись = Неопределено, "", "
			|ЛЕВОЕ СОЕДИНЕНИЕ 
			|	РегистрСведений.АдресаДляВыбора КАК ТекущийПорядокПорции
			...
			|
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	РегистрСведений.АдресаДляВыбора КАК ТекущийОбъект
			|ПО	
			|	ТекущийОбъект.Уровень = &Уровень
			|	И ТекущийОбъект.Уровень В (&Уровни)
			|	И ТекущийОбъект.ИдентификаторРодителя = РодительскийОбъект.Идентификатор
			|";
		...
		Иначе
			ВызватьИсключение НСтр("ru='Запрос некорректного уровня адресного объекта';uk='Запит некоректного рівня адресного об''єкта'");
		КонецЕсли;
		
		ТекстЗапроса = ТекстЗапроса + "
			|ГДЕ
			|	РодительскийОбъект.Идентификатор = &Родитель
			|	И РодительскийОбъект.Идентификатор <> ТекущийОбъект.Идентификатор
			|
			|" + ?(ПараметрПерваяЗапись = Неопределено, "", "
			|	И ТекущийОбъект.Идентификатор <> ТекущийПорядокПорции.Идентификатор
			|	И ТекущийОбъект.Наименование " + СравнениеПорядкаПорции + " ТекущийПорядокПорции.Наименование
			|") + "
			|
			|УПОРЯДОЧИТЬ ПО
			|	Представление " + ТекстСортировкиПорции + "
...
Функция ИдентификаторыАдреса(ПоляАдреса, ИскатьПоУлице)
	
	Если ИскатьПоУлице Тогда
		//Нету идентификаторов улицы
		
	Иначе
		
		ВсеИдентификаторыАдреса = Истина;
		НетРайона = Ложь;
		НетГорода = Ложь;
		...
		Если ВсеИдентификаторыАдреса Тогда
			ТекстВыбратьИдентификаторы = "АдресаДляВыбораУровень1.Идентификатор КАК ИдентификаторРегион, ";
		Иначе
			ТекстВыбратьИдентификаторы = "АдресаДляВыбораУровень1.Идентификатор КАК Идентификатор";
		КонецЕсли;
		...
		Иначе
			...
			ТекстСоединения = ТекстСоединения + Символы.ПС + "ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресаДляВыбора КАК АдресаДляВыбораУровень3
			|	ПО АдресаДляВыбораУровень1.Идентификатор = АдресаДляВыбораУровень3.ИдентификаторРодителя
			|	И АдресаДляВыбораУровень3.Наименование = """ + ПоляАдреса["Район"].Наименование + """
			|	И АдресаДляВыбораУровень3.КодСтраны = &КодУкраины
			|	И АдресаДляВыбораУровень3.Уровень = 3";
			Если ВсеИдентификаторыАдреса Тогда
				ТекстВыбратьИдентификаторы = ТекстВыбратьИдентификаторы + "АдресаДляВыбораУровень3.Идентификатор КАК ИдентификаторРайон, ";
			Иначе
				ТекстВыбратьИдентификаторы = "АдресаДляВыбораУровень3.Идентификатор КАК Идентификатор";
			КонецЕсли;
			
		КонецЕсли;
		
		Если НетРайона ИЛИ ПустаяСтрока(ПоляАдреса["Город"].Наименование) Тогда
			НетГорода = Истина;
		Иначе
			
			ТекстСоединения = ТекстСоединения + Символы.ПС + " ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресаДляВыбора КАК АдресаДляВыбораУровень4
			|	ПО АдресаДляВыбораУровень3.Идентификатор = АдресаДляВыбораУровень4.ИдентификаторРодителя
			|	И АдресаДляВыбораУровень4.Наименование = """ + ПоляАдреса["Город"].Наименование + """
			|	И АдресаДляВыбораУровень4.КодСтраны = &КодУкраины
			|	И АдресаДляВыбораУровень4.Уровень = 4";
			Если ВсеИдентификаторыАдреса Тогда
				ТекстВыбратьИдентификаторы = ТекстВыбратьИдентификаторы + "АдресаДляВыбораУровень4.Идентификатор КАК ИдентификаторГород, ";
			Иначе
				ТекстВыбратьИдентификаторы = "АдресаДляВыбораУровень4.Идентификатор КАК Идентификатор";
			КонецЕсли;
		КонецЕсли;
		
		Если НетРайона ИЛИ НетГорода ИЛИ ПустаяСтрока(ПоляАдреса["НаселенныйПункт"].Наименование) Тогда
			
		Иначе
			
			ТекстСоединения = ТекстСоединения + Символы.ПС + " ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресаДляВыбора КАК АдресаДляВыбораУровень6
			|	ПО АдресаДляВыбораУровень4.Идентификатор = АдресаДляВыбораУровень6.ИдентификаторРодителя
			|	И АдресаДляВыбораУровень6.Наименование = """ + ПоляАдреса["НаселенныйПункт"].Наименование + """
			|	И АдресаДляВыбораУровень6.КодСтраны = &КодУкраины
			|	И АдресаДляВыбораУровень6.Уровень = 6";
			Если ВсеИдентификаторыАдреса Тогда
				ТекстВыбратьИдентификаторы = ТекстВыбратьИдентификаторы + "АдресаДляВыбораУровень6.Идентификатор КАК ИдентификаторНаселенныйПункт, ";
			Иначе
				ТекстВыбратьИдентификаторы = "АдресаДляВыбораУровень6.Идентификатор КАК Идентификатор";
			КонецЕсли;
		КонецЕсли;
		...
		|ИЗ
		|	РегистрСведений.АдресаДляВыбора КАК АдресаДляВыбораУровень1 " + ТекстСоединения + "
		|ГДЕ
		|	АдресаДляВыбораУровень1.Наименование = &Регион
		|	И АдресаДляВыбораУровень1.КодСтраны = &КодУкраины
		|	И АдресаДляВыбораУровень1.Уровень = 1";
		
		Запрос.УстановитьПараметр("Регион", ПоляАдреса["Регион"].Наименование);
		Запрос.УстановитьПараметр("КодУкраины", 804);
...
Процедура УстановитьИдентификаторыНаселенногоПункта(ЧастиАдреса, ИдентификаторНаселенногоПункта) Экспорт
	...
	ИдентификаторыАдреса = ИдентификаторыАдреса(ПоляАдреса, Ложь);
...
//
Функция СписокАвтоподбораЧастиАдресаКОАТУУ(Текст, ИмяЧастиАдреса, ЧастиАдреса, ДополнительныеПараметры) Экспорт
	...
	Результат = ВариантыАвтоподбораКОАТУУ(Текст, Родитель, Уровни, ДополнительныеПараметры);
...
Функция ТаблицаДанныхАвтоподбора()
	...
	Колонки.Добавить("Идентификатор",  Новый ОписаниеТипов("Число"));
	Колонки.Добавить("Представление",  Новый ОписаниеТипов("Строка"));
	Колонки.Добавить("Родители",  Новый ОписаниеТипов("Структура"));
...
//             ** РегионЗагружен - Булево - Имеет смысл только для регионов. Истина, если есть записи.
//
Функция ВариантыАвтоподбораКОАТУУ(Текст, Родитель, Уровни, ДополнительныеПараметры) Экспорт 
	
	Результат = Новый Структура("Данные", ТаблицаДанныхАвтоподбора() );
	СтруктураОписанияОшибкиПоставщика(Результат);
	
	Вариант = ДополнительныеПараметры.ФорматАдреса;
	Если Вариант = "КОАТУУ" Тогда
		ОграничениеУровней = УровниКлассификатораКОАТУУ();
...
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаВнутр(Результат, Текст, Родитель, УровниЗапроса, ДополнительныеПараметры)
	...
	Если ЗначениеЗаполнено(Родитель) Тогда
		...
		ТекстЗапросаПодходящиеОбъекты = "ВЫБРАТЬ ПЕРВЫЕ " + ОтбиратьЗаписейДляАнализа + "
			|	ТекущийОбъект.Уровень,
			|	ТекущийОбъект.Идентификатор,
			|	ТекущийОбъект.ИдентификаторРодителя,
			...
			|	РегистрСведений.АдресаДляВыбора КАК ТекущийОбъект
			|ПО
			|	ТекущийОбъект.Уровень В (&Уровни)
			|	И ТекущийОбъект.Идентификатор <> РодительскийОбъект.Идентификатор
			|	И 
			|			(ТекущийОбъект.Наименование ПОДОБНО &НачалоФразы
			|			ИЛИ ТекущийОбъект.Наименование ПОДОБНО &НачалоСлова)
			|
			|ГДЕ 
			|	РодительскийОбъект.Идентификатор = &Родитель
			|	И РодительскийОбъект.Идентификатор = ТекущийОбъект.ИдентификаторРодителя
			|	И НЕ ТекущийОбъект.Идентификатор ЕСТЬ NULL
			|УПОРЯДОЧИТЬ ПО
			|	ТекущийОбъект.Уровень
			|ИНДЕКСИРОВАТЬ ПО
			|	ТекущийОбъект.Уровень,
	...
	Иначе
		...
		ТекстЗапросаПодходящиеОбъекты = "ВЫБРАТЬ ПЕРВЫЕ " + ОтбиратьЗаписейДляАнализа + "
			...
			|	РегистрСведений.АдресаДляВыбора КАК ТекущийОбъект
			|ГДЕ 
			|	(ТекущийОбъект.Наименование ПОДОБНО &НачалоФразы ИЛИ ТекущийОбъект.Наименование ПОДОБНО &НачалоСлова)
			|	И ТекущийОбъект.Уровень В (&Уровни)
			|УПОРЯДОЧИТЬ ПО
			|	ТекущийОбъект.Уровень
			|ИНДЕКСИРОВАТЬ ПО
	...
	ТекстЗапроса =  ТекстЗапросаПодходящиеОбъекты + "
		...
		|	ЕСТЬNULL(ОбъектВышеНа1.Наименование, """") КАК ВышеНа1Наименование,
		|
		|	ВЫБОР 
		|		КОГДА ОбъектВышеНа1.Уровень В (&Уровни) ТОГДА ЕСТЬNULL(ОбъектВышеНа1.Наименование, """")
		|		ИНАЧЕ """"
		|	КОНЕЦ КАК ВышеНа1Представление,
		|
		|	ЕСТЬNULL(ОбъектВышеНа2.Наименование, """") КАК ВышеНа2Наименование,
		|
		|	ВЫБОР 
		|		КОГДА ОбъектВышеНа2.Уровень В (&Уровни) ТОГДА ЕСТЬNULL(ОбъектВышеНа2.Наименование, """")
		|		ИНАЧЕ """"
		|	КОНЕЦ КАК ВышеНа2Представление,
		|	
		|	ЕСТЬNULL(ОбъектВышеНа3.Наименование, """") КАК ВышеНа3Наименование,
		|
		|	ВЫБОР 
		|		КОГДА ОбъектВышеНа3.Уровень В (&Уровни) ТОГДА ЕСТЬNULL(ОбъектВышеНа3.Наименование, """")
		|		ИНАЧЕ """"
		|	КОНЕЦ КАК ВышеНа3Представление,
		|	
		|	ЕСТЬNULL(ОбъектВышеНа4.Наименование, """") КАК ВышеНа4Наименование,
		|
		|	ВЫБОР 
		|		КОГДА ОбъектВышеНа4.Уровень В (&Уровни) ТОГДА ЕСТЬNULL(ОбъектВышеНа4.Наименование, """")
		|		ИНАЧЕ """"
		|	КОНЕЦ КАК ВышеНа4Представление,
		|	
		|	ТекущийОбъект.Идентификатор КАК Идентификатор,
		|	
		|	ТекущийОбъект.Наименование  КАК Наименование,
		|
		|	НЕ ТекущийОбъект.Актуален КАК Неактуален
		|
		|ИЗ
		|	ПодходящиеОбъекты КАК ТекущийОбъект
		|	
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.АдресаДляВыбора КАК ОбъектВышеНа1
		|ПО
		|	ТекущийОбъект.ИдентификаторРодителя <> 0
		|	И ОбъектВышеНа1.Идентификатор = ТекущийОбъект.ИдентификаторРодителя
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.АдресаДляВыбора КАК ОбъектВышеНа2
		|ПО
		|	ОбъектВышеНа1.ИдентификаторРодителя <> 0
		|	И ОбъектВышеНа2.Идентификатор = ОбъектВышеНа1.ИдентификаторРодителя
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.АдресаДляВыбора КАК ОбъектВышеНа3
		|ПО
		|	ОбъектВышеНа2.ИдентификаторРодителя <> 0
		|	И ОбъектВышеНа3.Идентификатор = ОбъектВышеНа2.ИдентификаторРодителя
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.АдресаДляВыбора КАК ОбъектВышеНа4
		|ПО
		|	ОбъектВышеНа3.ИдентификаторРодителя <> 0
		|	И ОбъектВышеНа4.Идентификатор = ОбъектВышеНа3.ИдентификаторРодителя
		|
		|УПОРЯДОЧИТЬ ПО
		|	ВышеНа1Наименование,
		|	ВышеНа2Наименование,
	...
	Для Каждого Строка Из Запрос.Выполнить().Выгрузить() Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаРезультата, Строка, "Идентификатор, Неактуален");
		
		Родители = Новый Структура;
		
		Родители.Вставить("ВышеНа1", Строка.ВышеНа1Наименование);
		Родители.Вставить("ВышеНа2", Строка.ВышеНа2Наименование);
		Родители.Вставить("ВышеНа3", Строка.ВышеНа3Наименование);
		Родители.Вставить("ВышеНа4", Строка.ВышеНа4Наименование);
		
		СтрокаРезультата.Родители = Родители;
		
		Представление = "";
		ДобавитьВПредставлениеЭлементАдреса(Представление, Строка.Наименование, "");
		ДобавитьВПредставлениеЭлементАдреса(Представление, Строка.ВышеНа1Представление, "");
		ДобавитьВПредставлениеЭлементАдреса(Представление, Строка.ВышеНа2Представление, "");
		ДобавитьВПредставлениеЭлементАдреса(Представление, Строка.ВышеНа3Представление, "");
		ДобавитьВПредставлениеЭлементАдреса(Представление, Строка.ВышеНа4Представление, "");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Обновление сведений контактной информации.
        ...
        |;uk='Оновлення відомостей контактної інформації.
        |До завершення обробки деяка контактна інформація може відображатися некоректно.'");
...
#Область ПреобразованиеКонтактнойИнформации

Процедура ПреобразоватьИзФормата31() Экспорт
	...
	МассивСправочников.Добавить("Склады");
	МассивСправочников.Добавить("ФизическиеЛица");
	Для каждого ИмяСправочника Из МассивСправочников Цикл
		ПреобразоватьСправочникИзФормата31(ИмяСправочника);
	КонецЦикла; 
	
КонецПроцедуры

Процедура ПреобразоватьСправочникИзФормата31(ИмяСправочника)
	...
	|	ОрганизацииКонтактнаяИнформация.НомерСтроки
	|ИЗ
	|	Справочник."+ИмяСправочника+".КонтактнаяИнформация КАК ОрганизацииКонтактнаяИнформация
	|ГДЕ
	|	ОрганизацииКонтактнаяИнформация.Тип = &ТипАдрес";
	
	Запрос.УстановитьПараметр("ТипАдрес",ПеречислениеАдрес);
	РезультатЗапроса = Запрос.Выполнить();
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураАдреса = ПустаяСтруктураАдреса();			
		НовоеЗначение = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "КонтактнаяИнформация"));
		НовоеЗначение.Состав = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "Адрес"));
		
		Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(Выборка.ЗначенияПолей) Тогда
			ЧтениеXML = Новый ЧтениеXML;
			ЧтениеXML.УстановитьСтроку(Выборка.ЗначенияПолей);
			Результат = ФабрикаXDTO.ПрочитатьXML(ЧтениеXML, ФабрикаXDTO.Тип(ПространствоИмен, "КонтактнаяИнформация"));
			
			Если Результат.ПолучитьXDTO("Состав") <> Неопределено Тогда 
				СоставКИ = Результат.Состав;
				Если ТипЗнч(СоставКИ) = Тип("ОбъектXDTO")
					И СоставКИ.ПолучитьXDTO("Состав") <> Неопределено Тогда 
					СоставВнутриКИ = СоставКИ.Состав;	 
					Если ТипЗнч(СоставВнутриКИ) = Тип("Строка") Тогда
						//И СтрЧислоВхождений(СоставВнутриКИ, ",") = 9 Тогда
						//Все корректно, это новый формат, преобразование не требуется
						Продолжить;
					ИначеЕсли ТипЗнч(СоставВнутриКИ) = Тип("ОбъектXDTO") Тогда
			...
			Попытка
				НовоеЗначение.Комментарий = Результат.Комментарий;
				НовоеЗначение.Представление = Результат.Представление;
				СтруктураАдреса.Страна = Результат.Состав.Страна;
			Исключение
				
			КонецПопытки; 
			
			Результат2 = ЗначениеИзСтрокиDOM(Выборка.ЗначенияПолей);
			Если ЕстьУзел(Результат2,"КонтактнаяИнформация") Тогда
				КонтактнаяИнформация = ПолучитьУзел(Результат2,"КонтактнаяИнформация");
				СоставКИ = ПолучитьУзел(КонтактнаяИнформация,"Состав");
				Если ЕстьУзел(СоставКИ,"Страна") И ЗначениеЗаполнено(СоставКИ.Страна) Тогда
					СтруктураАдреса.Страна = ПолучитьЗначениеУзла(СоставКИ,"Страна");
				КонецЕсли;
				СоставВложденнаяКИ = ПолучитьУзел(СоставКИ,"Состав");
				Если ЕстьУзел(СоставВложденнаяКИ,"Город") И ЗначениеЗаполнено(ПолучитьЗначениеУзла(СоставВложденнаяКИ,"Город")) Тогда
					СтруктураАдреса.НаселенныйПункт = ПолучитьЗначениеУзла(СоставВложденнаяКИ,"Город");
				КонецЕсли; 
				Если ЕстьУзел(СоставВложденнаяКИ,"НаселПункт") И ЗначениеЗаполнено(ПолучитьЗначениеУзла(СоставВложденнаяКИ,"НаселПункт")) Тогда
					СтруктураАдреса.НаселенныйПункт = ПолучитьЗначениеУзла(СоставВложденнаяКИ,"НаселПункт");
				КонецЕсли; 
				Если ЕстьУзел(СоставВложденнаяКИ,"Улица")  И ЗначениеЗаполнено(ПолучитьЗначениеУзла(СоставВложденнаяКИ,"Улица")) Тогда
					СтруктураАдреса.Улица = ПолучитьЗначениеУзла(СоставВложденнаяКИ,"Улица");
				КонецЕсли; 
				Если ЕстьУзел(СоставВложденнаяКИ,"Индекс")  И ЗначениеЗаполнено(ПолучитьЗначениеУзла(СоставВложденнаяКИ,"Индекс")) Тогда
					СтруктураАдреса.НаселенныйПункт = ПолучитьЗначениеУзла(СоставВложденнаяКИ,"Индекс");
				КонецЕсли; 
				Для каждого Узел Из СоставВложденнаяКИ.ДочерниеУзлы Цикл
					Если Узел.ИмяУзла = "ДопАдрЭл" Тогда
						ВставитьДопЭлАдр(Узел, СтруктураАдреса);
					КонецЕсли; 
				КонецЦикла;
			Иначе
				//Неверный формат
				Сообщить("Неверный формат:"+Выборка.ЗначенияПолей);
				Продолжить;
			КонецЕсли; 
			
		Иначе
			Если СтрЧислоСтрок(Выборка.ЗначенияПолей) > 0 И СтрНайти(Выборка.ЗначенияПолей,"=") > 0 Тогда
				Для Счетчик = 1 По СтрЧислоСтрок(Выборка.ЗначенияПолей) Цикл
					ТекСтрока = СтрПолучитьСтроку(Выборка.ЗначенияПолей, Счетчик);
					МассивАдрес = СтрРазделить(ТекСтрока, "=", Истина);
					Если МассивАдрес.Количество() = 2 Тогда
						СтруктураАдреса.Вставить(МассивАдрес[0],МассивАдрес[1]);
					КонецЕсли;  
				КонецЦикла;
			Иначе
				Продолжить;
		...
		Если НЕ ЗначениеЗаполнено(СтруктураАдреса.Страна) Тогда
		    СтруктураАдреса.Страна = "УКРАИНА";
		КонецЕсли; 
		НовоеЗначение.Состав.Страна = СтруктураАдреса.Страна;
		НовоеЗначение.Состав.Состав = ПредставлениеПоСтруктуреАдреса(СтруктураАдреса);
		ТекстКИ = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияXDTOВXML(НовоеЗначение);
		
		Если Выборка.ЗначенияПолей <> ТекстКИ Тогда
			
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = ИмяСправочника+":"+Выборка.Ссылка+Символы.ПС+"Было:"+Выборка.ЗначенияПолей+Символы.ПС+"Стало:"+ТекстКИ;
			Сообщение.КлючДанных = Выборка.Ссылка;
			Сообщение.УстановитьДанные(Выборка.Ссылка);
			Сообщение.Сообщить();
			
			Если ТекстКИ = Истина Тогда
				Продолжить;
			КонецЕсли; 
			
			Попытка
				ОбъектКИ = Выборка.Ссылка.ПолучитьОбъект();
				СтрокаКИ = ОбъектКИ.КонтактнаяИнформация.Найти(Выборка.НомерСтроки,"НомерСтроки");
				СтрокаКИ.ЗначенияПолей = ТекстКИ;
				ОбъектКИ.ОбменДанными.Загрузка = Истина;
				ОбъектКИ.Записать();
			Исключение
...
Функция ЗначениеИзСтрокиDOM(Знач Текст)
...
Функция ПустаяСтруктураАдреса()
	
	Возврат Новый Структура("Страна,Регион,Индекс,НаселенныйПункт,Город,Улица,ТипДома,Дом,ТипСтроения,Строение,ТипПомещения,Помещение","");
	
КонецФункции // ПустаяСтруктураАдреса()

Функция ПредставлениеПоСтруктуреАдреса(Контекст)
	
	Строка9Запятых = ""+Контекст.Страна + "," + Контекст.Индекс + "," + ?(ЗначениеЗаполнено(Контекст.НаселенныйПункт),Контекст.НаселенныйПункт,Контекст.Город)  + "," + Контекст.Регион + ",,," + Контекст.Улица + ",";
	...
	Возврат Строка9Запятых
КонецФункции // ПустаяСтруктураАдреса()

Функция ЕстьУзел(Узлы, ИмяУзла)
	
	ЕстьУзел = (ПолучитьУзел(Узлы, ИмяУзла) <> Неопределено);
	Возврат ЕстьУзел; 
	
КонецФункции // ПустаяСтруктураАдреса()

Функция ПолучитьЗначениеУзла(Узлы, ИмяУзла)
	
	Узел = ПолучитьУзел(Узлы, ИмяУзла);
	Возврат Узел.ТекстовоеСодержимое; 
	
КонецФункции

Процедура ВставитьДопЭлАдр(Узел, СтруктураАдреса)
	
	Если Узел.ИмяУзла = "ДопАдрЭл" Тогда
		Если Узел.ДочерниеУзлы.Количество() > 0 Тогда
			Для каждого ВложенныйУзел Из Узел.ДочерниеУзлы Цикл
				ВставитьДопЭлАдр(ВложенныйУзел, СтруктураАдреса)
			КонецЦикла;
			Возврат;
		Иначе
			КодОбъекта = Узел.ПолучитьАтрибут("ТипАдрЭл");
		КонецЕсли; 
	ИначеЕсли Узел.ИмяУзла = "Номер" Тогда				
		КодОбъекта = Узел.ПолучитьАтрибут("Тип");
	Иначе
		Возврат;			
	КонецЕсли;				
	Значение = Узел.ПолучитьАтрибут("Значение");
	...
	Если ТипОбъекта.Тип = 1 Тогда
		СтруктураАдреса.ТипДома = ТипОбъекта.Наименование;
		СтруктураАдреса.Дом = Значение;
		
	ИначеЕсли ТипОбъекта.Тип = 2 Тогда
		СтруктураАдреса.ТипСтроения = ТипОбъекта.Наименование;
		СтруктураАдреса.Строение = Значение;
		
	ИначеЕсли ТипОбъекта.Тип = 3 Тогда
		СтруктураАдреса.ТипПомещения = ТипОбъекта.Наименование;
		СтруктураАдреса.Помещение = Значение;
		
	ИначеЕсли ТипОбъекта.Код = "10100000" Тогда
		СтруктураАдреса.Индекс = Значение;
...
Функция ПолучитьУзел(Узлы, ИмяУзла)
	
	Для каждого Узел Из Узлы.ДочерниеУзлы Цикл
...
Процедура ПреобразоватьКИ(Команда)
	ПреобразоватьИзФормата31();
КонецПроцедуры
...
Процедура ЗаполнитьИдентификаторыНаселенногоПункта(НаселенныйПунктДетально, ИдентификаторНаселенногоПункта = Неопределено) Экспорт
	...
	УстановитьИдентификаторыНаселенногоПункта(НаселенныйПунктДетально, ИдентификаторНаселенногоПункта);
...
Функция ПроверитьАдресВXML(АдресВXML, ПараметрыПроверкиАдреса = Неопределено) Экспорт
	...
	Если НЕ ЭтоСтрокаXML(АдресВXML) Тогда
		...
		РезультатПроверки.СписокОшибок.Добавить("ФорматАдреса", НСтр("ru='Некорректный формат адреса';uk='Некоректний формат адреси'"));
...
Функция АдресаДляИнтерактивногоВыбора(Родитель, Уровень, ДополнительныеПараметры) Экспорт
	...
	ФорматАдреса = Неопределено;
	ДополнительныеПараметры.Свойство("ФорматАдреса", ФорматАдреса);
	Если ФорматАдреса <> Неопределено И ФорматАдреса = "КОАТУУ" Тогда
		Возврат АдресаДляИнтерактивногоВыбораКОАТУУ(Родитель, Уровень, ДополнительныеПараметры);
	Конецесли;
...
Функция СписокАвтоподбораЧастиАдреса(Текст, ИмяЧастиАдреса, ЧастиАдреса, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.Свойство("ФорматАдреса", ФорматАдреса);
	Если ФорматАдреса <> Неопределено И ФорматАдреса = "КОАТУУ" Тогда 
		Возврат СписокАвтоподбораЧастиАдресаКОАТУУ(Текст, ИмяЧастиАдреса, ЧастиАдреса, ДополнительныеПараметры);
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Контактная информация';uk='Контактна інформація'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ЗаполнитьСписокАвтоподбораИзТаблицы(Результат, ТабличныеДанные, ЭтоПодборУлицы)
	...
	Для Каждого Строка Из ТабличныеДанные Цикл
		...
		Если Строка.Неактуален Тогда
			Предупреждение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Адрес ""%1"" неактуален.';uk='Адреса ""%1"" неактуальний.'"), Представление);
			Пометка        = Истина;
		...
		ЗначениеЭлемента.Вставить("Родители", Строка.Родители);
...
Функция КонтактнаяИнформацияИзXML(Знач Текст, Знач ОжидаемыйВид = Неопределено, РезультатПреобразования = Неопределено) Экспорт
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(Текст) Тогда
		...
		Исключение
			...
			Если ТипЗнч(ОжидаемыйВид) = Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Некорректный формат XML контактной информации для ""%1"", значения полей были очищены.';uk='Некоректний формат XML контактної інформації для ""%1"", значення полів були очищені.'"),
					Строка(ОжидаемыйВид));
			Иначе
				ТекстОшибки = НСтр("ru='Некорректный формат XML контактной информации, значения полей были очищены.';uk='Некоректний формат XML контактної інформації, значення полів були очищені.'");
			КонецЕсли;
		КонецПопытки;
		...
		Если ТекстОшибки = Неопределено Тогда
			...
			Если ОжидаемыйТип = ПеречислениеАдрес И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается адрес';uk='Помилка десеріалізації контактної інформації, очікується адреса'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеАдресЭлектроннойПочты И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "ЭлектроннаяПочта") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается адрес электронной почты';uk='Помилка десеріалізації контактної інформації, очікується адреса електронної пошти'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеВебСтраница И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "ВебСайт") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается веб-страница';uk='Помилка десеріалізації контактної інформації, очікується веб-стрінка'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеТелефон И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "НомерТелефона") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается телефон';uk='Помилка десеріалізації контактної інформації, очікується телефон'");
			...
			ИначеЕсли ОжидаемыйТип = ПеречислениеSkype И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Skype") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается Skype';uk='Помилка десеріалізації контактної інформації, очікується Skype'");
			ИначеЕсли ОжидаемыйТип = ПеречислениеДругое И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Прочее") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается ""другое""';uk='Помилка десеріалізації контактної інформації, очікується ""інше""'");
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		...
		ТекстОшибки = НСтр("ru='Сведения о виде контактной информации %1 были повреждены или некорректно заполнены,
                                ...
                                |;uk='Відомості про вид контактної інформації %1 були пошкоджені або некоректно заповнені,
                                |т. я. обов''язкове поле тип не заповнено.'");
...
Функция АдресXMLВXDTO(Знач ЗначенияПолей, Знач Представление = "", Знач ОжидаемыйТип = Неопределено) Экспорт
	...
	Результат = СформироватьАдресВСвободнойФормеПоПредставлению(Представление);
	Возврат Результат;
	
КонецФункции

Функция СформироватьАдресВСвободнойФормеПоПредставлению(Представление)
	...
	НаименованиеУкраина = СокрЛП(Справочники.СтраныМира.Украина.Наименование);
	...
	Если СтрокаСтраны = Неопределено Тогда
		Адрес.Страна = НаименованиеУкраина;
	Иначе
	...
	Если ЗначениеЗаполнено(СтрокаСтраны) Тогда
	...
	Иначе
		Адрес.Состав = СокрЛП(Представление);
	КонецЕсли;
...
Функция ДесериализацияПрочейКонтактнойИнформации(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	ИначеЕсли ОжидаемыйТип <> Неопределено Тогда
		ВызватьИсключение НСтр("ru='Ошибка десериализации контактной информации, ожидается другой тип';uk='Помилка десеріалізації контактної інформації, очікується інший тип'");
...
Функция ОшибкиЗаполненияАдресаXDTO(XDTOАдрес, ВидИнформации, РезультатГруппами = Ложь) Экспорт
	...
	Если ТипЗнч(АдресРФ) <> Тип("ОбъектXDTO") Или АдресРФ.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "АдресРФ") Тогда
		...
		Если ФлагиПроверки.АдресТолькоРоссийский Тогда
			ТекстОшибки = НСтр("ru='Адрес должен быть только российским.';uk='Адреса повинна бути тільки російським.'");
	...
	Если Не XDTOКонтактнаяИнформацияЗаполнена(АдресРФ) Тогда
		...
		Если ФлагиПроверки.ОбязательноеЗаполнение Тогда
			...
			ТекстОшибки = НСтр("ru='Адрес не заполнен.';uk='Адреса не заповнена.'");
	...
	Если РезультатГруппами Тогда
		...
		ИначеЕсли КоличествоОшибок > 0 Тогда
			...
			ВсеОшибки.Добавить(ГруппаОшибокАдреса(НаименованиеГруппыОшибки,
				НСтр("ru='Части адреса не соответствуют адресному классификатору:';uk='Частина адреси не відповідає адресному класифікатору:'")));
...
Функция ОбщиеГруппыОшибокЗаполненияАдреса(Знач ДанныеАдреса, Знач ВидИнформации)
	...
	Иначе
		...
		Если XTDOКонтактная.Представление <> Представление Тогда
			Результат.Добавить(ГруппаОшибокАдреса("ПредставлениеНеСоответствуетНаборуПолей",
				НСтр("ru='Адрес не соответствует значениям в наборе полей.';uk='Адреса не відповідає значенням у наборі полів.'")));
			ДобавитьОшибкуЗаполненияАдреса(Результат[0], "",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Представление адреса для вида контактной информации ""%1"" отличается от данных в адресе.';uk='Представлення адреси для виду контактної інформації ""%1"" відрізняється від даних у адресі.'"),
				Строка(ВидИнформации.Наименование)));
		КонецЕсли;
	КонецЕсли;
	
	НеЗаполненыОбязательныеПоля = ГруппаОшибокАдреса("НеЗаполненыОбязательныеПоля",
		НСтр("ru='Не заполнены обязательные поля:';uk='Не заповнені обов''язкові поля:'"));
	Результат.Добавить(НеЗаполненыОбязательныеПоля);
	
	НеУказаныСокращенияПолей = ГруппаОшибокАдреса("НеУказаныСокращенияПолей",
		НСтр("ru='Не указано сокращение для полей:';uk='Не зазначене скорочення для полів:'"));
	Результат.Добавить(НеУказаныСокращенияПолей);
	
	НедопустимыеСимволыПолей = ГруппаОшибокАдреса("НедопустимыеСимволыПолей",
		НСтр("ru='Найдены недопустимые символы в полях:';uk='Знайдено неприпустимі символи в полях:'"));
	Результат.Добавить(НедопустимыеСимволыПолей);
	
	НеСоответствуетДлинаПолей = ГруппаОшибокАдреса("НеСоответствуетДлинаПолей",
		НСтр("ru='Не соответствует установленной длина полей:';uk='Не відповідає встановленій довжина полів:'"));
	...
	Если ПустаяСтрока(Индекс) Тогда
		ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, УправлениеКонтактнойИнформациейКлиентСервер.XPathПочтовогоИндекса(),
			НСтр("ru='Не указан почтовый индекс.';uk='Не зазначений поштовий індекс.'"), "Индекс");
	КонецЕсли;
	...
	Если ПустаяСтрока(Регион) Тогда
		ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "СубъектРФ",
			НСтр("ru='Не указан регион.';uk='Не зазначений регіон.'"), "Регион");
	КонецЕсли;
	...
	Если ИсключатьПроверкуДомаВАдресе(АдресРФ) Тогда
		...
		Если ЗданиеНеУказано Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, 
				...
				НСтр("ru='Не указан дом или корпус';uk='Не зазначений будинок або корпус'"), 
				НСтр("ru='Дом';uk='Будинок'"));
		КонецЕсли;
	...
	Иначе
		...
		Если ДанныеДома = Неопределено Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, 
				УправлениеКонтактнойИнформациейКлиентСервер.XPathНомераДополнительногоОбъектаАдресации("Дом"),
				НСтр("ru='Не указан дом или владение (домовладение).';uk='Не зазначений будинок або володіння (домоволодіння).'"),
				НСтр("ru='Дом';uk='Будинок'"));
		ИначеЕсли ПустаяСтрока(ДанныеДома.Значение) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, ДанныеДома.ПутьXPath,
				НСтр("ru='Не заполнено значение дома или владения (домовладения).';uk='Не заповнене значення будинку або володіння (домоволодіння).'"),
				НСтр("ru='Дом';uk='Будинок'"));
		КонецЕсли;
	...
	Если Не ПустаяСтрока(Регион) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Регион)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, "СубъектРФ",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение в названии региона ""%1"".';uk='Не зазначене скорочення в назві регіону ""%1"".'"), Регион), НСтр("ru='Регион';uk='Регіон'"));
		КонецЕсли;
		Если СтрДлина(Регион) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название региона ""%1"" должно быть короче 50 символов.';uk='Назва регіону ""%1"" повинна бути коротше 50 символів.'"),Регион), НСтр("ru='Регион';uk='Регіон'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Регион, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии региона ""%1"" есть не кириллические символы.';uk='У назві регіону ""%1"" є не кириличні символи.'"), Регион), НСтр("ru='Регион';uk='Регіон'"));
		КонецЕсли
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Район) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Район)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение в названии района ""%1"".';uk='Не вказано скорочення в назві району ""%1"".'"), Район), НСтр("ru='Район';uk='Район'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название района ""%1"" должно быть короче 50 символов.';uk='Назва району ""%1"" повинна бути коротше 50 символів.'"), Район), НСтр("ru='Район';uk='Район'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Район, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии района ""%1"" есть не кириллические символы.';uk='У назві району ""%1"" є не кириличні символи.'"), Район), НСтр("ru='Район';uk='Район'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Город) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Город)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение в названии города ""%1"".';uk='Не зазначене скорочення в назві міста ""%1"".'"), Город), НСтр("ru='Город';uk='Місто'"));
		КонецЕсли;
		Если СтрДлина(Город) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название города ""%1"" должно быть короче 50 символов.';uk='Назва міста ""%1"" повинна бути коротше 50 символів.'"), Город), НСтр("ru='Город';uk='Місто'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Город, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии города ""%1"" есть не кириллические символы.';uk='У назві міста ""%1"" є не кириличні символи.'"), Город), НСтр("ru='Город';uk='Місто'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(НаселенныйПункт) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(НаселенныйПункт)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение в названии населенного пункта ""%1"".';uk='Не зазначене скорочення в назві населеного пункту ""%1"".'"), НаселенныйПункт),
					НСтр("ru='Населенный пункт';uk='Населений пункт'"));
		КонецЕсли;
		Если СтрДлина(НаселенныйПункт) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название населенного пункта ""%1"" должно быть короче 50 символов.';uk='Назва населеного пункту ""%1"" повинна бути коротше 50 символів.'"), НаселенныйПункт),
				НСтр("ru='Населенный пункт';uk='Населений пункт'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(НаселенныйПункт, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии населенного пункта ""%1"" есть не кириллические символы.';uk='У назві населеного пункту ""%1"" є не кириличні символи.'"), НаселенныйПункт),
					НСтр("ru='Населенный пункт';uk='Населений пункт'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Улица) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Улица)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение в названии улицы ""%1"".';uk='Не зазначене скорочення в назві вулиці ""%1"".'"), Улица), НСтр("ru='Улица';uk='Вулиця'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название улицы ""%1"" должно быть короче 50 символов.';uk='Назва вулиці ""%1"" повинна бути коротше 50 символів.'"), Улица), НСтр("ru='Улица';uk='Вулиця'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Улица, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии улицы ""%1"" есть не кириллические символы.';uk='У назві вулиці ""%1"" є не кириличні символи.'"), Улица), НСтр("ru='Улица';uk='Вулиця'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДополнительныйЭлемент) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(ДополнительныйЭлемент)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение у дополнительного элемента ""%1"".';uk='Не вказано скорочення у додаткового елементу ""%1"".'"), ДополнительныйЭлемент),
				НСтр("ru='Улица';uk='Вулиця'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название дополнительного элемента ""%1"" должно быть короче 50 символов.';uk='Назва додаткового елементу ""%1"" повинна бути коротше 50 символів.'"), ДополнительныйЭлемент),
				НСтр("ru='ДополнительныйЭлемент';uk='ДополнительныйЭлемент'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(ДополнительныйЭлемент, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии дополнительного элемента ""%1"" есть не кириллические символы.';uk='У назві додаткового елементу ""%1"" є не кириличні символи.'"), ДополнительныйЭлемент),
				НСтр("ru='ДополнительныйЭлемент';uk='ДополнительныйЭлемент'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Индекс) Тогда
		...
		Если СтрДлина(Индекс) <> 6 Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Индекс) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				НСтр("ru='Почтовый индекс должен состоять из 6 цифр.';uk='Поштовий індекс повинен складатися з 6 цифр.'"),
				НСтр("ru='Индекс';uk='Індекс'"));
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого ДанныеЗдания Из ЗданияПомещения.Здания Цикл
		Если СтрДлина(ДанныеЗдания.Значение) > 10 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, ДанныеЗдания.ПутьXPath,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Значение поля ""%1"" должно быть короче 10 символов.';uk='Значення поля ""%1"" повинне бути коротше 10 символів.'"), ДанныеЗдания.Тип), ДанныеЗдания.Тип);
		КонецЕсли;
	КонецЦикла;
	Для Каждого ДанныеПомещения Из ЗданияПомещения.Помещения Цикл
		Если СтрДлина(ДанныеПомещения.Значение) > 10 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, ДанныеПомещения.ПутьXPath,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Значение поля ""%1"" должно быть короче 10 символов.';uk='Значення поля ""%1"" повинне бути коротше 10 символів.'"), ДанныеПомещения.Тип), ДанныеПомещения.Тип);
		КонецЕсли;
	КонецЦикла;
	...
	Если Не ИсключатьПроверкуУлицыВАдресе(АдресРФ) Тогда
		...
		Если ПустаяСтрока(НаселенныйПункт) И ПустаяСтрока(Улица) И НЕ ЗначениеЗаполнено(ДополнительныйЭлемент) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "Улица",
				НСтр("ru='Город или населенный пункт должен содержать название улицы.';uk='Місто або населений пункт повинен містити назву вулиці.'"), 
				НСтр("ru='Улица';uk='Вулиця'"));
		КонецЕсли;
...
Функция ИсключатьПроверкуДомаВАдресе(Знач АдресРФ)
	...
	Если ВРег(СокрЛП(АдресРФ.СубъектРФ)) = НСтр("ru='МОСКВА Г';uk='МОСКВА Г'") И ВРег(СокрЛП(АдресРФ.Город)) = НСтр("ru='ЗЕЛЕНОГРАД Г';uk='ЗЕЛЕНОГРАД М'") Тогда
		Результат = Истина;
	КонецЕсли;
...
Функция ПроверитьПараметрыВидаКонтактнойИнформации(ВидКонтактнойИнформации) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВидКонтактнойИнформации.Тип) Тогда
		...
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не заполонено обязательное поле тип у вида контактной информации ""%1"".';uk='Не заполонено обов''язкове поле тип у виду контактної інформації ""%1"".'"),
		...
		Если НЕ ВидКонтактнойИнформации.АдресТолькоРоссийский
			...
			ИЛИ ВидКонтактнойИнформации.СкрыватьНеактуальныеАдреса) Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Некорректно заполнены настройки проверки адреса у вида контактной информации %1.
                    ...
                    |;uk='Некоректно заповнені настройки перевірки адреси у виду контактної інформації %1.
                    |Перевірка коректності адреси доступна тільки для російських адрес'"), Строка(ВидКонтактнойИнформации.Наименование));
		...
		Если ВидКонтактнойИнформации.РазрешитьВводНесколькихЗначений
			И ВидКонтактнойИнформации.ХранитьИсториюИзменений Тогда
				Результат.ТекстОшибки = Результат.ТекстОшибки + Разделитель + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Некорректно заполнены настройки адреса у вида контактной информации %1.
                    ...
                    |;uk='Некоректно заповнені настройки адреси у виду контактної інформації %1.
                    |Не допускається можливість введення декількох значень контактної інформації при включеній історії зберігання змін.'"),
						Строка(ВидКонтактнойИнформации.Наименование));
		КонецЕсли;
	КонецЕсли;
...
Функция ДесериализацияАдресаОбщая(Знач ЗначенияПолей, Знач Представление, Знач ОжидаемыйТип = Неопределено)
	...
	Если ОжидаемыйТип <> Неопределено Тогда
		Если ОжидаемыйТип <> Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			ВызватьИсключение НСтр("ru='Ошибка десериализации контактной информации, ожидается адрес';uk='Помилка десеріалізації контактної інформації, очікується адреса'");
		КонецЕсли;
	КонецЕсли;
...
Функция ДесериализацияТелефонаФакса(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Ошибка десериализации контактной информации, ожидается телефон или факс';uk='Помилка десеріалізації контактної інформації, очікується телефон або факс'");
	КонецЕсли;
...
Функция РазличияКонтактнойИнформации(Знач Данные1, Знач Данные2) Экспорт
	...
	Если ТипКонтактнойИнформации <> ДанныеКИ2.ТипКонтактнойИнформации Тогда
		...
		СтрокаРезультата[Колонки.Добавить("Описание").Имя]  = НСтр("ru='Различные типы контактной информации';uk='Різні типи контактної інформації'");
		Возврат Результат;
	КонецЕсли;
...
Функция ПривестиКонтактнуюИнформациюXML(Знач Данные) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Ошибка параметров преобразования, не определен тип контактной информации';uk='Помилка параметрів перетворення, не визначений тип контактної інформації'");
