...
//
// Пример:
// 	РегламентированнаяОтчетность.ПолучитьСсылкуНаРеглОтчет("РегламентированныйОтчетРСВ1", "ФормаОтчета2012Кв1", Организация, '20120331');
//
...
Функция ПроверитьВозможностьОткрытияОтчета(ИсточникОтчета) Экспорт
	...
	ИначеЕсли ПравоДоступаКОтчету = Неопределено Тогда
		Сообщить("В справочнике регл. отчетов не зарегистрирован соответствующий отчет");
		Сообщить("Выгрузка не выполнена!");
		Возврат Ложь;
	КонецЕсли;
	...
	Если ТекОтчет = Неопределено Тогда
		Сообщить("Не удалось открыть соответсвующий регл. отчет");
		Сообщить("Выгрузка не выполнена!");
		Возврат Ложь
	КонецЕсли;
