...
Процедура ОткрытьОтчетИлиФорму(ТекущийЭлемент, Пользователь, ТекущийПользователь, ИмяФормыПерсональныхНастроек) Экспорт
	...
	Если Пользователь <> ТекущийПользователь Тогда
		ТекстПредупреждения =
			НСтр("ru='Для просмотра настроек другого пользователя необходимо
                       ...
                       |;uk='Для перегляду налаштувань користувача необхідно
                       |запустити програму від його імені і відкрити потрібний звіт або форму.'");
	...
	ИначеЕсли ЭлементДереваЗначений.Имя = "ВнешнийВид" Тогда
				...
				Если ЭлементДереваЗначений.ТекущиеДанные.ТипСтроки = "НастройкиРабочегоСтола" Тогда
					ПоказатьПредупреждение(,
						НСтр("ru='Для просмотра настроек рабочего стола перейдите к разделу
                                   ...
                                   |;uk='Для перегляду налаштувань робочого столу перейдіть до розділу
                                   |""Робочий стіл"" у командному інтерфейсі програми.'"));
					Возврат;
				КонецЕсли;
				...
				Если ЭлементДереваЗначений.ТекущиеДанные.ТипСтроки = "НастройкиКомандногоИнтерфейса" Тогда
					ПоказатьПредупреждение(,
						НСтр("ru='Для просмотра настроек командного интерфейса
                                   ...
                                   |;uk='Для перегляду параметрів командного інтерфейсу
                                   |виберіть потрібний розділ командного інтерфейсу програми.'"));
					Возврат;
				КонецЕсли;
				...
				Если РодительЭлемента <> Неопределено Тогда
					ТекстПредупреждения =
						НСтр("ru='Для просмотра данной настройки необходимо открыть ""%1"" 
                                   ...
                                   |;uk='Для перегляду даної настройки необхідно відкрити ""%1"" 
                                   |і потім перейти до форми ""%2"".'");
		...
		ПоказатьПредупреждение(,НСтр("ru='Данную настройку невозможно просмотреть.';uk='Дану настройку неможливо переглянути.'"));
		Возврат;
	...
	ПоказатьПредупреждение(,НСтр("ru='Выберите настройку для просмотра.';uk='Виберіть настройки для перегляду.'"));
...
Функция ФормированиеПоясненияПриКопировании(ПредставлениеНастройки, КоличествоНастроек, ПояснениеКомуСкопированыНастройки) Экспорт
	...
	Если КоличествоНастроек = 1 Тогда
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='""%1"" скопирована %2';uk='""%1"" скопійована %2'"),
	...
	Иначе
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Скопировано %1 %2';uk='Скопійовано %1 %2'"),
...
Функция ПояснениеПользователи(КоличествоПользователей, Пользователь) Экспорт
	...
	Если КоличествоПользователей = 1 Тогда
		ПояснениеКомуСкопированыНастройки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='пользователю ""%1""';uk='користувачеві ""%1""'"), Пользователь);
	Иначе
		ПояснениеКомуСкопированыНастройки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 пользователям';uk='%1 користувачам'"), КоличествоПользователей);
	КонецЕсли;
