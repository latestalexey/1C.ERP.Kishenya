...
Процедура ПередОбновлениемИнформационнойБазы() Экспорт
	...
	Если ПодсистемыКонфигурации.Количество() > 0 Тогда
		...
		Если ПодсистемыКонфигурации.Найти("BASКомплекснаяАвтоматизация") = Неопределено Тогда
			Запись = РегистрыСведений.ВерсииПодсистем.СоздатьМенеджерЗаписи();
			Запись.ИмяПодсистемы = "BASКомплекснаяАвтоматизация";
			Запись.Версия = "2.0.1.1";
			Запись.Записать();
		КонецЕсли;
		Если ПодсистемыКонфигурации.Найти("BASУправлениеТорговлей") = Неопределено Тогда
			Запись = РегистрыСведений.ВерсииПодсистем.СоздатьМенеджерЗаписи();
			Запись.ИмяПодсистемы = "BASУправлениеТорговлей";
			Запись.Версия = "3.2.2.8";
			Запись.Записать();
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриПолученииПоясненияДляРезультатовОбновленияПрограммы(ТекстПодсказки) Экспорт
	...
	ТекстПодсказки = НСтр("ru='Сведения о результатах обновления версии программы можно также открыть из раздела ""НСИ и администрирование"" - ""Интернет-поддержка пользователей"".';uk='Відомості про результати оновлення версії програми можна також відкрити з розділу ""НДІ і адміністрування"" - ""Інтернет-підтримка користувачів"".'");
