...
Функция МенеджерКриптографии(Операция, ПоказатьОшибку = Истина, ОписаниеОшибки = "", Программа = Неопределено) Экспорт
	...
	Если Операция = "Подписание" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось подписать данные на сервере %1 по причине:';uk='Не вдалося підписати дані на сервері %1 з причини:'");
		
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось проверить подпись на сервере %1 по причине:';uk='Не вдалося перевірити підпис на сервері %1 з причини:'");
		
	ИначеЕсли Операция = "Шифрование" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось зашифровать данные на сервере %1 по причине:';uk='Не вдалося зашифрувати дані на сервері %1 з причини:'");
		
	ИначеЕсли Операция = "Расшифровка" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось расшифровать данные на сервере %1 по причине:';uk='Не вдалося розшифрувати дані на сервері %1 з причини:'");
		
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось проверить сертификат на сервере %1 по причине:';uk='Не вдалося перевірити сертифікат на сервері %1 з причини:'");
		
	ИначеЕсли Операция = "ПолучениеСертификатов" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось получить сертификаты на сервере %1 по причине:';uk='Не вдалося отримати сертифікати на сервері %1 з причини:'");
	...
	Иначе
		ЗаголовокОшибки = НСтр("ru='Не удалось выполнить операцию на сервере %1 по причине:';uk='Не вдалося виконати операцію на сервері %1 з причини:'");
	КонецЕсли;
	...
	ИначеЕсли ЗначениеЗаполнено(Программа) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Программа %1 не доступна или не установлена.';uk='Програма %1 не доступна або не встановлена.'"), Программа);
	Иначе
		ТекстОшибки = НСтр("ru='Ни одна из программ не доступна или не установлена.';uk='Жодна з програм не доступна або не встановлена.'");
	КонецЕсли;
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(,, Ложь) Тогда
		ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
			+ НСтр("ru='Обратитесь к администратору.';uk='Зверніться до адміністратора.'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Сертификат не найден на сервере по причине:
                       |%1
                       |'
                       |;uk='Сертифікат не знайдено на сервері через:
                       |%1
                       |'"),
			ПредставлениеОшибки);
	Иначе
		ТекстОшибки = НСтр("ru='Сертификат не найден на сервере.';uk='Сертифікат не знайдено на сервері.'");
	КонецЕсли;
		
	Если Не Пользователи.ЭтоПолноправныйПользователь(,, Ложь) Тогда
		ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("ru='Обратитесь к администратору.';uk='Зверніться до адміністратора.'")
	КонецЕсли;
...
Процедура ПроверитьУникальностьПредставления(Представление, СертификатСсылка, Поле, Отказ) Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		ТекстСообщения = НСтр("ru='Сертификат с таким представлением уже существует.';uk='Сертифікат з таким представленям вже існує.'");
	...
	Исключение
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При получении данных сертификата ""%1""
                       ...
                       |%2'
                       |;uk='При отриманні даних сертифіката ""%1""
                       |з інформаційної бази виникла помилка:
                       |%2'"),
			Сертификат,
