...
//                      уровне платформы (пароль у пользователя не запрашивается - это делает
//                      операционная система, которая не принимает от платформы непустой пароль).
//
// Возвращаемое значение:
...
Процедура УдалитьСтрокуПодписи(ПодписанныйОбъект, ИндексСтроки)
	...
	Если ПодписанныйОбъект.ЭлектронныеПодписи.Количество() < ИндексСтроки + 1 Тогда
		ВызватьИсключение НСтр("ru='Строка с подписью не найдена.';uk='Рядок з підписом не знайдений.'");
	КонецЕсли;
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда 
		Если СтрокаТабличнойЧасти.УстановившийПодпись <> Пользователи.ТекущийПользователь() Тогда
			ВызватьИсключение НСтр("ru='Недостаточно прав на удаление подписи.';uk='Недостатньо прав на вилучення підпису.'");
		КонецЕсли;
	КонецЕсли;
