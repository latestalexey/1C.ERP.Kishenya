...
//  СправочникСсылка.Номенклатура, Неопределено 
//
Функция НоменклатураПоШтрихкоду(ПараметрыОбработки, КэшированныеЗначения) Экспорт
	...
	Штрихкод = ПараметрыОбработки.Штрихкоды.Штрихкод;
	ПрефиксыВесовыхШтрихкодов = ЗначениеНастроекПовтИсп.ПолучитьПрефиксыВесовыхШтрихкодов();
...
Функция ДобавитьИзменитьСтрокуВТЧ(Объект, СтруктураПараметров, ДобавляемыеДанные, РежимОбработки,
	...
	Если ДобавляемыеДанные.НеизвестныйШтрихкод Тогда
		...
		Если ИспользоватьУпаковочныеЛисты
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				ТекстСообщения = НСтр("ru='Номенклатура или упаковочный лист со штрихкодом ""%Штрихкод%"" не найдены';uk='Номенклатура або пакувальний лист зі штрихкодом ""%Штрихкод%"" не знайдені'");
	...
	Если Не СтруктураПараметров.ИзменятьКоличество Тогда
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			Если ЭтоУпаковочныйЛист Тогда
				...
				ТекстСообщения = НСтр("ru='В табличной части не найдена строка, содержащая ""%УпаковочныйЛист%""';uk='У табличній частині не знайдений рядок, що містить ""%УпаковочныйЛист%""'");
			...
			ИначеЕсли ДобавляемыеДанные.НеизвестныйШтрихкод Тогда
				...
				Если СтруктураПараметров.ШтрихКодыВТЧ Тогда
					ТекстСообщения = НСтр("ru='В табличной части не найдена строка, содержащая штрихкод ""%Штрихкод%""';uk='У табличній частині не знайдено рядок, що містить штрих-код ""%Штрихкод%""'");
				ИначеЕсли ИспользоватьУпаковочныеЛисты
					И СтруктураПараметров.УчитыватьУпаковочныеЛисты Тогда
					ТекстСообщения = НСтр("ru='Номенклатура или упаковочный лист со штрихкодом ""%Штрихкод%"" не найдены';uk='Номенклатура або пакувальний лист зі штрихкодом ""%Штрихкод%"" не знайдені'");
				Иначе
					ТекстСообщения = НСтр("ru='Номенклатура со штрихкодом ""%Штрихкод%"" не найдена';uk='Номенклатура зі штрихкодом ""%Штрихкод%"" не знайдена'");
				КонецЕсли;
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Штрихкод%", ДобавляемыеДанные.Штрихкод);
				
			Иначе
				ТекстСообщения = НСтр("ru='В табличной части не найдена строка с товаром ""%НазваниеТовара%""';uk='У табличній частині не знайдено рядок з товаром ""%НазваниеТовара%""'");
				
				НазваниеТовара = НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(ДобавляемыеДанные.Номенклатура,
	...
	Если НайденныеСтроки.Количество() = 0
		...
		Если Не СтруктураПараметров.НеИспользоватьУпаковки //т.к. в этом случае без упаковки уже искали
			И ЗначениеЗаполнено(ДобавляемыеДанные.Упаковка)
			И ДобавляемыеДанные.Коэффициент = 1 Тогда
		...
		//	- если найдем, то увеличим в ней количество
		//	- если нет - добавим новую строку	
		ИначеЕсли ЗначениеЗаполнено(ДобавляемыеДанные.ЕдиничнаяУпаковка) 
	...
	Иначе
		...
		Если ЭтоУпаковочныйЛист
			...
			ТекстСообщения = НСтр("ru='%УпаковочныйЛист% уже есть в табличной части.';uk='%УпаковочныйЛист% вже є в табличній частині.'");
...
Функция СоответствуетУсловиям(ДобавляемыеДанные, СтруктураПараметров)
	...
	Если ЭтоУпаковочныйЛист
		...
		ТекстСообщения = НСтр("ru='Штрихкод ""%Штрихкод%"" соответствует упаковочному листу. В этом документе не поддерживается работа со штрихкодами упаковочных листов.';uk='Штрихкод ""%Штрихкод%"" відповідає пакувальному листу. У цьому документі не підтримується робота зі штрихкодами пакувальних листів.'");
	...
	Если Не ЭтоУпаковочныйЛист
		...
		ТекстСообщения = НСтр("ru='Товар ""%НазваниеТовара%"" является услугой (работой). В данную табличную часть запрещено добавлять услуги (работы).';uk='Товар ""%НазваниеТовара%"" є послугою (роботою). В цю табличну частину заборонено додавати послуги (роботи).'");
	...
	Если СтруктураПараметров.ТолькоУслуги
		...
		ТекстСообщения = НСтр("ru='Номенклатура ""%НазваниеУслуги%"" является товаром (тарой). В данную табличную часть запрещено добавлять товары (тару).';uk='Номенклатура ""%НазваниеУслуги%"" є товаром (тарою). В дану табличну частину заборонено додавати товари (тару).'");
		
		НазваниеУслуги = НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(ДобавляемыеДанные.Номенклатура,
																					 ДобавляемыеДанные.Характеристика,
																					 ДобавляемыеДанные.Упаковка);
	...
	Если СтруктураПараметров.ТолькоНеПодакцизныйТовар
		...
		ТекстСообщения = НСтр("ru='Номенклатура ""%НазваниеНоменклатуры%"" является подакцизным товаром. В данную табличную часть подакцизный товар добавлять нельзя.';uk='Номенклатура ""%НазваниеНоменклатуры%"" є підакцизним товаром. В дану табличну частину підакцизний товар додавати не можна.'");
		
		НазваниеНоменклатуры = НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(ДобавляемыеДанные.Номенклатура,
																							 ДобавляемыеДанные.Характеристика,
																							 ДобавляемыеДанные.Упаковка);
	...
	Если СтруктураПараметров.ТолькоТара
		...
		ТекстСообщения = НСтр("ru='Номенклатура ""%НазваниеТары%"" не является тарой. В данную табличную часть можно добавлять только тару.';uk='Номенклатура ""%НазваниеТары%"" не є тарою. В дану табличну частину можна додавати тільки тару.'");
		
		НазваниеТары = НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(ДобавляемыеДанные.Номенклатура,
																					ДобавляемыеДанные.Характеристика,
																					ДобавляемыеДанные.Упаковка);
