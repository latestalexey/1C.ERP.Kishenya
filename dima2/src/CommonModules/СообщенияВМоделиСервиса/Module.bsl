...
Процедура ОшибкаНеизвестноеИмяКанала(Знач КаналСообщений) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неизвестное имя канала сообщений %1';uk='Невідоме ім''я каналу повідомлень %1'"), КаналСообщений);
...
Процедура ЗаписатьСобытиеНачалоОбработки(Знач Сообщение) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Начало обработки';uk='Повідомлення в моделі сервісу.Початок обробки'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗаписатьСобытиеОкончаниеОбработки(Знач Сообщение) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Окончание обработки';uk='Повідомлення в моделі сервісу.Закінчення обробки'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		,
		,
		ПредставлениеСообщенияДляЖурнала(Сообщение));
...
Процедура ДоставитьБыстрыеСообщения() Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение(НСтр("ru='Доставка быстрых сообщений невозможна в транзакции';uk='Доставка швидких повідомлень неможлива в транзакції'"));
	КонецЕсли;
	...
	Попытка
		ФоновыеЗадания.Выполнить(ИмяМетодаЗадания, , КлючЗадания, НСтр("ru='Доставка быстрых сообщений';uk='Доставка швидких повідомлень'"))
	Исключение
		// Дополнительная обработка исключения не требуется
		// ожидаемое исключение - дублирование задания с таким же ключом.
		ЗаписьЖурналаРегистрации(НСтр("ru='Доставка быстрых сообщений';uk='Доставка швидких повідомлень'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , ,
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		Если СообщенияВМоделиСервисаПовтИсп.ТипТелоОбласти().ЭтоПотомок(Сообщение.Body.Тип()) Тогда
			Если ОбщегоНазначения.ЗначениеРазделителяСеанса() <> Сообщение.Body.Zone Тогда
				ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Отправка сообщения';uk='Повідомлення в моделі сервісу.Відправлення повідомлення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					ПредставлениеСообщенияДляЖурнала(Сообщение));
					
				ШаблонОшибки = НСтр("ru='Ошибка при отправке сообщения. Область данных %1 не совпадает с текущей (%2).';uk='Помилка при відправці повідомлення. Область даних %1 не збігається з поточною (%2).'");
...
Процедура ПриОтправкеСообщения(КаналСообщений, ТелоСообщения, ОбъектСообщения) Экспорт
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Отправка';uk='Повідомлення в моделі сервісу.Відправка'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			,
			ПредставлениеСообщенияДляЖурнала(Сообщение));
...
Процедура ПриПолученииСообщения(КаналСообщений, ТелоСообщения, ОбъектСообщения) Экспорт
	...
	Если ТелоСодержитТипизированноеСообщение(ТелоСообщения, Сообщение) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Сообщения в модели сервиса.Получение';uk='Повідомлення в моделі сервісу.Отримання'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
			,
			,
			ПредставлениеСообщенияДляЖурнала(Сообщение));
...
Функция ПредставлениеСообщенияДляЖурнала(Знач Сообщение)
	...
	Шаблон = НСтр("ru='Канал: %1';uk='Канал: %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если СообщенияВМоделиСервисаПовтИсп.ТипТелоОбласти().ЭтоПотомок(Сообщение.Body.Тип()) Тогда
		Шаблон = НСтр("ru='Область данных: %1';uk='Область даних: %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ТранслироватьСообщениеВВерсиюКорреспондентаПриНеобходимости(Сообщение, Знач ИнформацияПодключения, Знач ПредставлениеПолучателя)
	...
	Если ИнтерфейсСообщения = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Не удалось определить интерфейс отправляемого сообщения: ни для одного из типов, используемых в сообщении, не зарегистрирован обработчик интерфейса!';uk='Не вдалося визначити інтерфейс повідомлення, що відправляється: ні для одного з типів, що використовуються в повідомленні, не зареєстрований обробник інтерфейсу!'");
	КонецЕсли;
	
	Если Не ИнформацияПодключения.Свойство("URL") 
			Или Не ЗначениеЗаполнено(ИнформацияПодключения.URL) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не задан URL сервиса обмена сообщениями с информационной базой %1';uk='Не заданий URL сервісу обміну повідомленнями з інформаційною базою %1'"), ПредставлениеПолучателя);
	КонецЕсли;
	...
	Если ВерсияКорреспондента = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Корреспондент %1 не поддерживает получение версий сообщений интерфейса %2, поддерживаемых текущей информационной базой!';uk='Кореспондент %1 не підтримує отримання версій повідомлень інтерфейсу %2, підтримуваних поточною інформаційною базою!'"),
