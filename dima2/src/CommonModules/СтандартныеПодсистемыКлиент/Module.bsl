...
Процедура УстановитьРасширенныйЗаголовокПриложения(ПриЗапуске = Ложь) Экспорт
	...
	Иначе
		ШаблонЗаголовка = "%1 / %2";
		ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, НСтр("ru='Не установлены разделители';uk='Не встановлені роздільники'"), ПараметрыКлиента.ПодробнаяИнформация);
	КонецЕсли;
	...
	Иначе
		...
		ПоказатьПредупреждение(,
			НСтр("ru='Для того чтобы открыть список активных пользователей, перейдите в меню
                       ...
                       |;uk='Для того щоб відкрити список активних користувачів, перейдіть у меню
                       |Всі функції - Стандартні - Активні користувачі.'"));
...
Процедура ВывестиПредупреждение(Форма, ПараметрыПредупреждения, ОбработчикЗавершения = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Контекст.Подробно) Тогда
		КнопкаПоУмолчанию = 1;
		Кнопки.Добавить(1, НСтр("ru='Подробнее...';uk='Докладніше...'"));
	КонецЕсли;
	Если ТипЗнч(Форма) = Тип("УправляемаяФорма") И ЗначениеЗаполнено(Контекст.ПутьКРеквизитуФормы) Тогда
		Кнопки.Добавить(2, НСтр("ru='Перейти к реквизиту';uk='Перейти до реквізиту'"));
	КонецЕсли;
...
Процедура ПоказатьПодробнуюИнформацию(Обработчик, Текст, Заголовок = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(НастройкиДиалога.Заголовок) Тогда
		НастройкиДиалога.Заголовок = НСтр("ru='Подробнее';uk='Докладніше'");
	КонецЕсли;
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(0, НСтр("ru='Закрыть';uk='Закрити'"));
...
Процедура ИнтерактивнаяОбработкаПроверкиВерсииПлатформыПриЗапуске(Параметры, Неопределен) Экспорт
	...
	Иначе
		Если ПараметрыКлиента.ЕстьДоступДляОбновленияВерсииПлатформы Тогда
			ТекстСообщения = 
				НСтр("ru='Рекомендуется завершить работу программы и обновить версию платформы.
                     |В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.'
                     |;uk='Рекомендується завершити роботу програми і оновити версію платформи.
                     |В іншому випадку деякі можливості програми будуть недоступні або будуть працювати некоректно.'");
		Иначе
			ТекстСообщения = 
				НСтр("ru='Рекомендуется завершить работу программы и обратиться к администратору для обновления версии платформы.
                     |В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.'
                     |;uk='Рекомендується завершити роботу програми і звернутися до адміністратора для оновлення версії платформи.
                     |В іншому випадку деякі можливості програми будуть недоступні або будуть працювати некоректно.'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыКлиента.ВосстановитьСвязьСГлавнымУзлом = Ложь Тогда
		...
		ПоказатьПредупреждение(
			...
			НСтр("ru='Вход в программу временно невозможен до восстановления связи с главным узлом.
                       ...
                       |;uk='Вхід у програму тимчасово неможливий до оновлення зв''язку з головним вузлом.
		...
		Иначе
			ПоказатьПредупреждение(,
				НСтр("ru='Невозможно выбрать группу объектов метаданных.
                           ...
                           |;uk='Неможливо вибрати групу об''єктів метаданих.
                           |Виберіть об''єкт метаданих.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьРазделениеСеанса()
	...
	Если ПараметрыЗапуска.Количество() < 2 Тогда
		ВызватьИсключение
			НСтр("ru='При указании параметра запуска ВойтиВОбластьДанных,
                       ...
                       |;uk='При вказівці параметра запуску ВойтиВОбластьДанных,
                       |додатковим параметром необхідно вказати значення роздільника.'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение
			НСтр("ru='Значением разделителя в параметре ВойтиВОбластьДанных должно быть число.';uk='Значенням роздільника в параметрі ВойтиВОбластьДанных повинне бути число.'");
	КонецПопытки;
...
Процедура ВопросПользователюПередЗавершениемРаботыСистемы(Параметры, ОбработкаОтвета) Экспорт
	...
	ПараметрыВопроса.Заголовок = НСтр("ru='Завершение работы';uk='Завершення роботи'");
	ПараметрыВопроса.БольшеНеЗадаватьЭтотВопрос = Ложь;
	
	ПоказатьВопросПользователю(ОбработкаОтвета, НСтр("ru='Завершить работу с программой?';uk='Завершити роботу із програмою?'"), Кнопки, ПараметрыВопроса);
...
Процедура ОткрытьПрикладнуюФормуПредупреждения(Параметры, ОбработкаОтвета, ПредупреждениеПользователю, ИмяФормы, ПараметрыФормы)
	...
	Если ДействиеГиперссылка.Свойство("ПрикладнаяФормаПредупреждения", Форма) Тогда
		...
		Если ДействиеГиперссылка.Свойство("ПараметрыПрикладнойФормыПредупреждения", ПараметрыФормы) Тогда
			...
			ПараметрыФормы.Вставить("ЗаголовокКнопкиДа",  НСтр("ru='Завершить';uk='Завершити'"));
			ПараметрыФормы.Вставить("ЗаголовокКнопкиНет", НСтр("ru='Отмена';uk='Відмінити'"));
...
Процедура ОбработатьОшибкуПриЗапускеИлиЗавершении(Параметры, ИнформацияОбОшибке, Событие, ПрекратитьРаботу = Ложь)
	...
	ТекстПредупреждения = НачалоОписанияОшибки + Символы.ПС
		+ НСтр("ru='Техническая информация об ошибке записана в журнал регистрации.';uk='Технічна інформація про помилку записана в журнал реєстрації.'")
...
Процедура ПоказатьПомещениеФайлаПриПодключенииРасширенияРаботыСФайлами(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Если ПараметрыДиалога.Свойство("Режим") Тогда
		...
		Если Режим = РежимДиалогаВыбораФайла.ВыборКаталога Тогда
			ВызватьИсключение НСтр("ru='Выбор каталога не поддерживается';uk='Вибір каталогу не підтримується'");
		КонецЕсли;
	Иначе
...
Процедура ОповеститьОНехваткеПамяти() Экспорт
	...
	Заголовок = НСтр("ru='Скорость работы снижена';uk='Швидкість роботи знижена'");
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Рекомендуется увеличить
        | объем памяти до %1 Гб.'
        |;uk='Рекомендується збільшити
        |об''єм пам''яті до %1 Гб.'"), РекомендуемыйОбъем);
...
Функция Обработчик(Сценарий, КоличествоПараметров = 2)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Обработчик оповещения для более чем 2 параметров не поддерживается';uk='Обробник оповіщення для більш ніж 2 параметрів не підтримується'");
	КонецЕсли;
КонецФункции
