...
#КонецЕсли
	...
	Если НЕ ЕстьПраваНаОбновлениеИнформационнойБазы() Тогда
		Сообщение = НСтр("ru='Недостаточно прав для обновления версии программы.';uk='Недостатньо прав для оновлення версії програми.'");
	...
	Если РежимОбновленияДанных = "ПереходСДругойПрограммы" Тогда
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Изменилось имя конфигурации на ""%1"".
                       ...
                       |;uk='Змінилося ім''я конфігурації на ""%1"".
                       |Буде виконаний перехід з іншої програми.'"),
			Метаданные.Имя);
	ИначеЕсли РежимОбновленияДанных = "ОбновлениеВерсии" Тогда
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Изменился номер версии конфигурации: с ""%1"" на ""%2"".
                       ...
                       |;uk='Змінився номер версії конфігурації: з ""%1"" на ""%2"".
                       |Буде виконане оновлення інформаційної бази.'"),
			ВерсияДанных, ВерсияМетаданных);
	Иначе
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Выполняется начальное заполнение данных до версии ""%1"".';uk='Виконується початкове заповнення даних до версії ""%1"".'"),
			ВерсияМетаданных);
	КонецЕсли;
	...
	Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Обновление информационной базы на версию ""%1"" выполнено успешно.';uk='Оновлення інформаційної бази на версію ""%1"" виконано успішно.'"), ВерсияМетаданных);
	ЗаписатьИнформацию(Сообщение);
...
Функция РежимОбновленияДанных() Экспорт
	...
	Если НЕ СтандартнаяОбработка Тогда
		...
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Недопустимое значение параметра %1 в %2. 
            ...
            |;uk='Неприпустиме значення параметра %1 в %2. 
            |Очікувалося: %3; передане значення: %4 (тип %5).'"),
			"РежимОбновленияДанных", "ПриОпределенииРежимаОбновленияДанных", 
			НСтр("ru='НачальноеЗаполнение, ОбновлениеВерсии или ПереходСДругойПрограммы';uk='НачальноеЗаполнение, ОбновлениеВерсии або ПереходСДругойПрограммы'"), 
...
Функция ИнформационнаяБазаЗаблокированаДляОбновления(УчитыватьПривилегированныйРежим = Истина) Экспорт
	...
	СообщениеАдминистраторуСистемы =
		НСтр("ru='Вход в программу временно невозможен в связи с обновлением на новую версию.
                   ...
                   |;uk='Вхід у програму тимчасово неможливий у зв''язку з оновленням на нову версію.
                   |Для завершення оновлення версії програми потрібні адміністративні права
                   |(ролі ""Адміністратор системи"" й ""Повні права"").'");
	...
	Если НеобходимоОбновлениеНеразделенныхДанныхИнформационнойБазы() Тогда
		...
		СообщениеАдминистраторуОбластиДанных =
			НСтр("ru='Вход в приложение временно невозможен в связи с обновлением на новую версию.
                       ...
                       |;uk='Вхід у додаток тимчасово неможливий у зв''язку з оновленням на нову версію.
                       |Зверніться до адміністратора сервісу за подробицями.'");
	...
	Иначе
		...
		Сообщение =
			НСтр("ru='Вход в программу временно невозможен в связи с обновлением на новую версию.
                       ...
                       |;uk='Вхід у програму тимчасово неможливий у зв''язку з оновленням на нову версію.
                       |Зверніться до адміністратора за подробицями.'");
	КонецЕсли;
...
Процедура ЗаписатьСведенияОбОбновленииИнформационнойБазы(Знач СведенияОбОбновлении) Экспорт
	...
	МенеджерКонстанты.ДополнительныеСвойства.Вставить("РегистрироватьНаУзлахПлановОбменаПриОбновленииИБ", Неопределено);
	МенеджерКонстанты.ОбменДанными.Получатели.АвтоЗаполнение = Ложь;
...
Функция ИтерацииОбновления()
	...
	Если ИтерацияОбновлениеОсновнойПодсистемы = Неопределено Тогда
		Если ИмяОсновнойКонфигурации = "БиблиотекаСтандартныхПодсистем" Тогда
			ТекстСообщения = НСтр("ru='Файл поставки Библиотека стандартных подсистем не предназначен для создания
                |информационных баз по шаблону. Перед использованием необходимо
                |ознакомиться с документацией на ИТС (http://its.1c.ua/db/bspdoc)'
                |;uk='Файл поставки Бібліотека стандартних підсистем не призначений для створення
                |інформаційних баз за шаблоном. Перед використанням необхідно
                |ознайомитися з документацією на ІТС (http://its.1c.ua/db/bspdoc)'");
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сведения об основной конфигурации ""%1"" не заданы.
                ...
                |;uk='Відомості про основну конфігурацюї ""%1"" не задані.
                |См. процедуру ПриДобавленииПодсистем загального модуля ПодсистемыКонфигурацииПереопределяемый.'"),
...
Функция ВыполнитьИтерациюОбновления(Знач ИтерацияОбновления, Знач Параметры) Экспорт
	...
	Если ТекущаяВерсияИБ <> "0.0.0.0"
		...
		Если ВыполняемыеОбработчики = Неопределено Тогда
			Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				ШаблонСообщения = НСтр("ru='Не найден план обновления конфигурации %1 с версии %2 на версию %3';uk='Не знайдений план оновлення конфігурації %1 з версії %2 на версію %3'");
			Иначе
				ШаблонСообщения = НСтр("ru='Не найден план обновления библиотеки %1 с версии %2 на версию %3';uk='Не знайдений план оновлення бібліотеки %1 з версії %2 на версію %3'");
			КонецЕсли;
	...
	Для Каждого Версия Из ВыполняемыеОбработчики.Строки Цикл
		...
		Если Версия.Версия = "*" Тогда
			Сообщение = НСтр("ru='Выполняются обязательные процедуры обновления информационной базы.';uk='Виконуються обов''язкові процедури оновлення інформаційної бази.'");
		Иначе
			НоваяВерсияИБ = Версия.Версия;
			Если ТекущаяВерсияИБ = "0.0.0.0" Тогда
				Сообщение = НСтр("ru='Выполняется начальное заполнение данных.';uk='Виконується початкове заповнення даних.'");
			ИначеЕсли ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выполняется обновление информационной базы с версии %1 на версию %2.';uk='Виконується оновлення інформаційної бази з версії %1 на версію %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ);
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выполняется обновление данных библиотеки %3 с версии %1 на версию %2.';uk='Виконується оновлення дані бібліотеки %3 з версії %1 на версію %2.'"), 
		...
		Если Версия.Версия = "*" Тогда
			Сообщение = НСтр("ru='Выполнены обязательные процедуры обновления информационной базы.';uk='Виконано обов''язкові процедури оновлення інформаційної бази.'");
		Иначе
			Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выполнено обновление информационной базы с версии %1 на версию %2.';uk='Виконано оновлення інформаційної бази з версії %1 на версію %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ);
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выполнено обновление данных библиотеки %3 с версии %1 на версию %2.';uk='Виконано оновлення дані бібліотеки %3 з версії %1 на версію %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ, ИдентификаторБиблиотеки);
			КонецЕсли;
		КонецЕсли;
		ЗаписатьИнформацию(Сообщение);
...
Функция ОбновитьИнформационнуюБазуВФоне(УникальныйИдентификаторФормы, БлокировкаИБ) Экспорт
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			НСтр("ru='Фоновое обновление информационной базы';uk='Фонове оновлення інформаційної бази'"));
...
Функция ЗаблокироватьИБ(БлокировкаИБ, ИсключениеПриНевозможностиБлокировкиИБ)
	...
	Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Невозможно выполнить обновление информационной базы:
            ...
            |Подробности ошибки:
            |%1'
            |;uk='Неможливо виконати оновлення інформаційної бази:
            |- Неможливо встановити монопольний режим
            |- Версія конфігурації не передбачає оновлення без установки монопольного режиму
            |
            |Подробиці помилки:
            |%1'"),
...
Процедура ПерейтиСДругойПрограммы()
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ru='При работе в модели сервиса переход с другой программы не предусмотрен.';uk='При роботі в моделі сервісу перехід з іншої програми не передбачений.'");
	КонецЕсли;
		...
		Исключение
			...
			ЗаписатьОшибку(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При вызове обработчика перехода с другой программы
                           ...
                           |произошла ошибка:
                           |""%2"".'
                           |;uk='При виклику обробника переходу з іншої програми
                           |""%1""
                           |відбулася помилка:
                           |""%2"".'"),
				ИмяОбработчика,
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Обновление информационной базы';uk='Оновлення інформаційної бази'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПротокол() Экспорт
	...
	Возврат СобытиеЖурналаРегистрации() + ". " + НСтр("ru='Протокол выполнения';uk='Протокол виконання'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПроверитьСвойстваОтложенногоОбработчика(Знач Обработчик, Знач РежимВыполненияОтложенныхОбработчиков, ТекстОшибок)
	...
	Если РежимВыполненияОтложенныхОбработчиков = "Параллельно"
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не указана процедура заполнения данных
                       ...
                       |""%1"".'
                       |;uk='Не вказана процедура заповнення даних
                       |відкладеної обробки оновлення
                       |""%1"".'"),
	...
	Если Обработчик.МонопольныйРежим = Истина Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='У отложенного обработчика ""%1""
            ...
            |;uk='У відкладеного обробника ""%1""
            |не повинен бути встановлений ознака ""МонопольныйРежим"".'"), 
			Обработчик.Процедура);
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
	...
	Если РежимВыполненияОтложенныхОбработчиков = "Параллельно" И Обработчик.ЗапускатьТолькоВГлавномУзле
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='У отложенного обработчика ""%1""
            ...
            |;uk='У відкладеного обробника ""%1""
            |некоректно заповнені значення властивостей:
            |- ""ЗапускатьТолькоВГлавномУзле""
            |- ""ЗапускатьИВПодчиненномУзлеРИБСФильтрами"".
            |
            |Дані властивості не можуть одночасно приймати значення ""Істина"".'"), 
			Обработчик.Процедура);
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
	...
	Если Обработчик.ОбщиеДанные = Истина Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='У отложенного обработчика ""%1""
            ...
            |;uk='У відкладеного обробника ""%1""
            |зазначено неприпустиме значення властивості ""ОбщиеДанные"".
            |
            |Ця властивість може приймати значення ""Істина"" у відкладеного обробника.'"), 
			Обработчик.Процедура);
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
...
Процедура ПроверитьУникальностьИдентификаторовОтложенныхОбработчиков(ИтерацииОбновления)
	...
	ТекстСообщения = НСтр("ru='У следующих отложенных обработчиков неуникальные идентификаторы:
        |'
        |;uk='У наступних відкладених обробниках неунікальні ідентифікаторів:
        |'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ru='Обновление программы не завершено';uk='Оновлення програми не завершено'");
	...
	Для Каждого ОтключаемыйОбработчик Из ОтключаемыеОбработчики Цикл
		...
		ИначеЕсли ВыполняемыйОбработчик <> Неопределено И ВыполняемыйОбработчик.Версия <> "*"
			...
			ТекстИсключения = НСтр("ru='Обработчик обновления %1 не может быть отключен, 
                                        ...
                                        |;uk='Обробник оновлення %1 не може бути відключений, 
                                        |тому що він виконується тільки при переході на версію %2'");
		...
		ИначеЕсли ВыполняемыйОбработчик = Неопределено Тогда
			ТекстИсключения = НСтр("ru='Отключаемый обработчик обновления %1 не существует';uk='Відключаємий обробник оновлення %1 не існує'");
	...
	Исключение
		...
		ЗаписатьОшибку(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При вызове обработчика обновления:
                       ...
                       |произошла ошибка:
                       |""%2"".'
                       |;uk='При виклику обробника оновлення:
                       |""%1""
                       |відбулася помилка:
                       |""%2"".'"),
			ИмяОбработчика,
...
Процедура ПроверитьВложеннуюТранзакцию(ТранзакцияАктивнаНаНачалоВыполнения, НазваниеОбработчика)
	...
	ИмяСобытия = СобытиеЖурналаРегистрации() + ". " + НСтр("ru='Выполнение обработчиков';uk='Виконання обробників'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Если ТранзакцияАктивнаНаНачалоВыполнения Тогда
		...
		Если ТранзакцияАктивна() Тогда
			...
			Исключение
                ...
                |;uk='Помилка виконання обробника оновлення %1:
                |Обробником оновлення було поглинене виключення при активній зовнішній транзакції.
                |При активних транзакціях, відкритих вище по стеку, виключення також необхідно прокидати вище по стеку.'");
		...
		Иначе
            ...
            |;uk='Помилка виконання обробника оновлення %1:
            |Обробником оновлення була закрита зайва транзакція, відкрита раніше (вище по стеку).'");
			Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКомментария, НазваниеОбработчика);
	...
	Иначе
		Если ТранзакцияАктивна() Тогда
            ...
            |;uk='Помилка виконання обробника оновлення %1:
            |Відкрита усередині обробника оновлення транзакція залишилася активною (не була закрита або скасована).'");
			Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКомментария, НазваниеОбработчика);
	...
	Для каждого Обработчик Из ИтерацияОбновления.Обработчики Цикл
		...
		Если ПустаяСтрока(Обработчик.Версия) Тогда
			...
			Если Обработчик.НачальноеЗаполнение <> Истина Тогда
				ОписаниеОшибки = НСтр("ru='У обработчика не заполнено свойство Версия или свойство НачальноеЗаполнение.';uk='В обробника не заповнена властивість Версія або властивість НачальноеЗаполнение.'");
			КонецЕсли;
			...
			Исключение
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У обработчика не правильно заполнено свойство Версия: ""%1"".
                               ...
                               |;uk='В обробника не правильно заповнена властивість Версія: ""%1"".
                               |Правильний формат, наприклад: ""2.1.3.70"".'"),
					Метаданные.Версия);
			КонецПопытки;
			...
			Если НЕ ЗначениеЗаполнено(ОписаниеОшибки)
			   ...
			   И Обработчик.Приоритет <> 0 Тогда
				...
				ОписаниеОшибки = НСтр("ru='У обработчика не правильно заполнено свойство Приоритет или
                                            ...
                                            |;uk='В обробника не правильно заповнена властивість Приоритет або
                                            |властивість ВыполнятьВГруппеОбязательных.'");
			КонецЕсли;
		КонецЕсли;
		...
		Если Обработчик.РежимВыполнения <> ""
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='У обработчика ""%1"" не правильно заполнено свойство РежимВыполнения.
                           ...
                           |;uk='В обробника ""%1"" не правильно заповнена властивість РежимВыполнения.
                           |Припустиме значення: ""Монопольно"", ""Отложенно"", ""Оперативно"".'"),
				Обработчик.Процедура);
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ОписаниеОшибки)
		   ...
		   И Обработчик.НачальноеЗаполнение = Истина Тогда
			...
			ОписаниеОшибки = НСтр("ru='У обработчика не правильно заполнено свойство Опциональный или
                                        ...
                                        |;uk='В обробника не правильно заповнена властивість Опциональный або
                                        |властивість НачальноеЗаполнение.'");
		КонецЕсли;
		...
		Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда
			ЗаголовокОшибки = НСтр("ru='Ошибка в свойстве обработчика обновления конфигурации';uk='Помилка у властивості обробника оновлення конфігурації'");
		Иначе
			ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка в свойстве обработчика обновления библиотеки %1 версии %2';uk='Помилка у властивості обробника оновлення бібліотеки %1 версії %2'"),
...
Функция КоличествоОбработчиковНаТекущуюВерсию(ИтерацииОбновления, РежимВыполненияОтложенногоОбновления)
	...
	Сообщение = НСтр("ru='Для обновления программы на новую версию будут выполнены обработчики: %1';uk='Для оновлення програми на нову версію будуть виконані обробники: %1'");
...
Процедура ЗаписатьИнформациюОХодеОбновления(Обработчик, ХодВыполненияОбработчиков, ВФоне)
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		Сообщение = НСтр("ru='Выполняется обработчик обновления %1 (%2 из %3).';uk='Виконується обробник оновлення %1 (%2 з %3).'");
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ВыполнитьОтложенныйОбработчикОбновления(СведенияОбОбновлении, ПараметрыОбновления = Неопределено)
	...
	ИначеЕсли (ОбработчикОбновления.Статус = "Выполняется"
		...
		Если КоличествоОбработчиков > 1
			...
			КоллекцияОбработчиков.Удалить(Индекс);
            ИндексПодредакции = Неопределено;
            Для сч = Индекс По КоллекцияОбработчиков.ВГраница() Цикл
                Если СтрНайти(КоллекцияОбработчиков[сч], "НачалоПодредакции") Тогда
                    ИндексПодредакции = сч;       
                    Прервать;
                КонецЕсли; 
            КонецЦикла; 
            Если ИндексПодредакции = Неопределено Тогда
            	КоллекцияОбработчиков.Добавить(ИмяОбработчика);
            Иначе
                КоллекцияОбработчиков.Вставить(ИндексПодредакции, ИмяОбработчика);
            КонецЕсли; 
		КонецЕсли;
	КонецЕсли;
...
Функция НовыеСведенияОбОбработчикахОбновления()
    ...
    ДеревоОбработчиков.Колонки.Добавить("НеЗаполнятьДанныеВОбработчикеПодредакции", Новый ОписаниеТипов("Булево"));
    ДеревоОбработчиков.Колонки.Добавить("ОчередьОбработчикаПодредакции");
...
Процедура ЗапуститьПроцедуруОбработкиДанных(ОбработчикОбновления, ПараметрыОбновления)
	...
	Попытка
		СообщениеОЗапущенномОбработчике = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выполняется процедура обновления ""%1"".';uk='Виконується процедура оновлення ""%1"".'"), ИмяОбработчика);
        ...
        Параметры.Вставить("НеЗаполнятьДанныеВОбработчикеПодредакции", ОбработчикОбновления.НеЗаполнятьДанныеВОбработчикеПодредакции);
        Параметры.Вставить("ОчередьОбработчикаПодредакции", ОбработчикОбновления.ОчередьОбработчикаПодредакции);
        Параметры.Вставить("МожноВыполнять", Истина);
		...
		ИначеЕсли КоличествоЗапусковПроцедурыОбновления > 10000 Тогда // Защита от зацикливания.
			...
			ТекстОшибки = НСтр("ru='Превышено допустимое количество запусков процедуры обновления.
                ...
                |;uk='Перевищена допустима кількість запусків процедури оновлення.
                |Виконання перервано для запобігання зациклення механізму обробки даних.'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		
		// Запуск отложенного обработчика обновления.
		Если ОбработчикОбновления.ИмяБиблиотеки = "BASУправлениеТорговлей" Тогда
			РаботаВБезопасномРежиме.ВыполнитьМетодКонфигурации("ОбновлениеИнформационнойБазыУТ.ПередВыполнениемОбработчикаОтложенногоОбновления", ПараметрыОбработчика);
        КонецЕсли; 
        Если НЕ Параметры.МожноВыполнять Тогда
            ОбработчикОбновления.Статус = "Ожидает";
            Возврат;
        КонецЕсли; 
		...
		Если Параметры.ОбработкаЗавершена = Неопределено Тогда
            ...
            |;uk='Обробник оновлення не ініціалізував параметр ОбработкаЗавершена.
            |Виконання перерване із-за явної помилки в коді обробника.'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		...
		ИначеЕсли ПараметрыОбновления.ПараллельныйРежим Тогда
			...
			Если Не ЕстьОбработанныеОбъекты
				...
				Если ЧислоПопыток >= 2 Тогда
					ТекстИсключения = НСтр("ru='Произошло зацикливание процедуры обработки данных. Выполнение прервано.';uk='Сталося зациклення процедури обробки даних. Виконання перервано.'");
					ВызватьИсключение ТекстИсключения;
				Иначе
	...
	Для Каждого Обработчик Из СтрокаВерсия.Строки Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(БлокируемыеОбъекты) И Не ЗначениеЗаполнено(ПроцедураПроверки) Тогда
			ТекстИсключения = НСтр("ru='У отложенного обработчика обновления ""%1""
                ...
                |;uk='У відкладеного обробника оновлення ""%1""
                |заповнений список заблокованих об''єктів, але не встановлено властивість ""ПроцедураПроверки"".'");
	...
	Для Каждого СтрокаБиблиотека Из СведенияОбОбновлении.ДеревоОбработчиков.Строки Цикл
		...
		Для Каждого СтрокаВерсия Из СтрокаБиблиотека.Строки Цикл
            Для Каждого Обработчик Из СтрокаВерсия.Строки Цикл
                ...
                Если НЕ Обработчик.НеЗаполнятьДанныеВОбработчикеПодредакции Тогда
                    Продолжить;
                КонецЕсли; 
...
Процедура УстановитьПараметрыОтложенногоОбработчикаОбновления(ОбработчикОбновления, Отложенно = Ложь, Параллельно = Ложь)
    ...
    ПараметрыОбработчикаОбновления.НеЗаполнятьДанныеВОбработчикеПодредакции = ОбработчикОбновления.НеЗаполнятьДанныеВОбработчикеПодредакции;
    ПараметрыОбработчикаОбновления.ОчередьОбработчикаПодредакции = ОбработчикОбновления.ОчередьОбработчикаПодредакции;
...
Функция НовыеПараметрыОбработчикаОбновления() Экспорт
    ...
    ПараметрыОбработчикаОбновления.Вставить("НеЗаполнятьДанныеВОбработчикеПодредакции", Ложь);
    ПараметрыОбработчикаОбновления.Вставить("ОчередьОбработчикаПодредакции", 0);
