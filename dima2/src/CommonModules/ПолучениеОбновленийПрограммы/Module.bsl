...
Процедура ЗагрузитьИУстановитьДистрибутивПлатформыВФоновомРежиме(Параметры) Экспорт
	...
	Попытка
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Установка новой версии платформы (%1). %2';uk='Установка нової версії платформи (%1). %2'"),
				Параметры.Версия,
				КомандаЗапуска));
	...
	Если КодВозврата <> 0 Тогда
		...
		Иначе
			...
			СообщениеЖурнала =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Установка платформы завершена с ошибкой.
                        ...
                        |Код возврата: %2;
                        |Команда: %3'
                        |;uk='Установка платформи завершено з помилкою.
                        |Версія %1;
                        |Код повернення: %2;
                        |Команда: %3'"),
					Параметры.Версия,
			...
			ШаблонСообщенияПользователю = ШаблонСообщенияПользователю + Символы.ПС
				+ НСтр("ru='<br /><br />При возникновении проблем напишите в <a href=""mailto:webits-info@1c.ua"">техподдержку</a>.</body>';uk='<br /><br />При виникненні проблем напишіть у <a href=""mailto:webits-info@1c.ua"">техпідтримку</a>.</body>'");
...
Функция НастройкиАвтоматическогоОбновления() Экспорт
	...
	Если ТипЗнч(Настройки) <> Тип("Структура") Тогда
		Настройки = ПолучениеОбновленийПрограммыКлиентСервер.НовыйНастройкиАвтоматическогоОбновления();
	Иначе
		Если НЕ Настройки.Свойство("ДатаПоследнейПроверки") Тогда
			Настройки.Вставить("ДатаПоследнейПроверки", '00010101');
		КонецЕсли;
		Если НЕ Настройки.Свойство("РежимУстановки") Тогда
			Настройки.Вставить("РежимУстановки", 0);
		КонецЕсли;
	КонецЕсли;
