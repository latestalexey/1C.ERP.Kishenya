...
Функция ПараметрыРедактированияСоставаНачисленийИУдержаний() Экспорт
	...
	ПараметрыОткрытия.Вставить("КатегорияЕСВИспользуется", Ложь);
	ПараметрыОткрытия.Вставить("КатегорияЕСВ");
...
Процедура УстановитьВидимостьГруппыПоказателей(Форма, ИмяТаблицы, Видимость) Экспорт 
	Если МаксимальноеКоличествоПоказателей(Форма, ИмяТаблицы) > 0 Тогда
...
Процедура УстановитьКомментарииДействийСНачислением(СтрокаНачисления, МаксимальноеКоличествоПоказателей, РежимРаботы, ОписаниеТаблицыВидовРасчета = Неопределено, ДатаСобытия = Неопределено, ДатаОкончания = Неопределено) Экспорт
			...
			Если ЗначениеЗаполнено(ДатаОкончания) Тогда
				...
				Если ДатаОкончания > СтрокаНачисления.ДатаОкончанияНачисления
					...
					СтрокаНачисления.Комментарий = СтрокаНачисления.Комментарий + " "
						+ НСтр("ru='до';uk='до'") + " " + Формат(ДатаОкончания, "ДЛФ=D");
				...
				ИначеЕсли ДатаОкончания < СтрокаНачисления.ДатаОкончанияНачисления
					...
					СтрокаНачисления.Комментарий = СтрокаНачисления.Комментарий + ", "
						+ НСтр("ru='затем начисляется до';uk='потім нараховується до'") + " " + Формат(СтрокаНачисления.ДатаОкончанияНачисления, "ДЛФ=D");
					
				КонецЕсли;
				
			ИначеЕсли ЗначениеЗаполнено(СтрокаНачисления.ДатаОкончанияНачисления)
				И СтрокаНачисления.ИспользуетсяПоОкончании
				И Не ХарактерНачисления = ПредопределенноеЗначение("Перечисление.ХарактерПлановогоНачисления.Штатное") Тогда
				
				СтрокаНачисления.Комментарий = СтрокаНачисления.Комментарий + " "
					+ НСтр("ru='до';uk='до'") + " " + Формат(СтрокаНачисления.ДатаОкончанияНачисления, "ДЛФ=D");
	...
	ИначеЕсли Не СтрокаНачисления.ДействующийВидРасчета И РасшифровыватьИспользованиеНачисления И ЗначениеЗаполнено(СтрокаНачисления.ДатаОкончанияНачисления) Тогда
		...
		СтрокаНачисления.Комментарий = НСтр("ru='Будет начисляться с';uk='Буде нараховуватися з'") + " " + Формат(ДатаНачалаСостоянияПоДатеОкончанияПредыдущего(СтрокаНачисления.ДатаОкончанияНачисления), "ДЛФ=D");
		...
		Если РасшифровыватьИспользованиеНачисления Тогда
			...
			Если ЗначениеЗаполнено(ДатаОкончания) Тогда
				...
				Если ДатаОкончания < СтрокаНачисления.ДатаОкончанияНачисления
					...
					СтрокаНачисления.Комментарий = СтрокаНачисления.Комментарий + ", "
						+ НСтр("ru='затем начисляется с';uk='потім нараховується з'") + " " + Формат(ДатаНачалаСостоянияПоДатеОкончанияПредыдущего(СтрокаНачисления.ДатаОкончанияНачисления), "ДЛФ=D");
					
				КонецЕсли;
				
			ИначеЕсли ЗначениеЗаполнено(СтрокаНачисления.ДатаОкончанияНачисления)
				И Не СтрокаНачисления.ИспользуетсяПоОкончании
				И Не ХарактерНачисления = ПредопределенноеЗначение("Перечисление.ХарактерПлановогоНачисления.Штатное") Тогда
			...
			Если РежимРаботы = 0 Тогда
				...
				Если СтрокаНачисления.Свойство("ТекущееЗначение") Тогда
					...
					Если Значение > ТекущееЗначение  Тогда
						Комментарий = Комментарий + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Увеличен %1 (+%2);';uk='Збільшено %1 (+%2);'"),
							СтрокаНачисления["Показатель" + Сч],  Значение - ТекущееЗначение) + " ";
					ИначеЕсли  Значение < ТекущееЗначение Тогда
						Комментарий = Комментарий + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Уменьшен %1 (%2);';uk='Зменшено %1 (%2);'"),
							СтрокаНачисления["Показатель" + Сч],  Значение - ТекущееЗначение) + " ";
					КонецЕсли;
				...
				Если СтрокаНачисления.Свойство("ТекущееМинимальноеЗначение") Тогда
					...
					Если Значение > ТекущееЗначение  Тогда
						Комментарий = Комментарий + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Увеличен минимум %1 (+%2);';uk='Збільшений мінімум %1 (+%2);'"), 
							СтрокаНачисления["Показатель" + Сч],  Значение - ТекущееЗначение) + " ";
					ИначеЕсли  Значение < ТекущееЗначение Тогда
						Комментарий = Комментарий + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Уменьшен минимум %1 (%2);';uk='Зменшено мінімум %1 (%2);'"), 
							СтрокаНачисления["Показатель" + Сч],  Значение - ТекущееЗначение) + " ";
					КонецЕсли;
				...
				Если СтрокаНачисления.Свойство("ТекущееМаксимальноеЗначение") Тогда
					...
					Если Значение > ТекущееЗначение  Тогда
						Комментарий = Комментарий + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Увеличен максимум %1 (+%2);';uk='Збільшено максимум %1 (+%2);'"), 
							СтрокаНачисления["Показатель" + Сч],  Значение - ТекущееЗначение) + " ";
					ИначеЕсли  Значение < ТекущееЗначение Тогда
						Комментарий = Комментарий + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Уменьшен максимум %1 (%2);';uk='Зменшено максимум %1 (%2);'"), 
							СтрокаНачисления["Показатель" + Сч],  Значение - ТекущееЗначение) + " ";
					КонецЕсли;
			...
			Иначе
				...
				Если Значение <> ТекущееЗначение Тогда
					...
					Если НачисляетсяВЦеломЗаМесяц И ДатаСобытия <> Неопределено Тогда
						НачалоДействия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru=' с %1';uk=' з %1'"), Формат(НачалоМесяца(ДатаСобытия), "ДФ=""д ММММ"""));
					КонецЕсли;
					...
					Если Значение <> ТекущееЗначение Тогда
						...
						КомментарийТекущегоЗначения = ?(ПустаяСтрока(КомментарийТекущегоЗначения), "", КомментарийТекущегоЗначения + "; ")
							+ НаименованиеПоказателя + " " + Формат(ТекущееЗначение, ФорматПоказателя);
		...
		Если РасшифровыватьИспользованиеНачисления Тогда
			...
			Если ЗначениеЗаполнено(ДатаОкончания) Тогда
				...
				Если ПустаяСтрока(Комментарий) Тогда
					...
					Если ЗначениеЗаполнено(СтрокаНачисления.ДатаОкончанияНачисления)
						...
						Комментарий = НСтр("ru='Начисляется до';uk='Нараховується до'") + " " + Формат(СтрокаНачисления.ДатаОкончанияНачисления, "ДЛФ=D");
						
					КонецЕсли;
					
				Иначе
					
					Если СтрокаНачисления.ДатаОкончанияНачисления > ДатаОкончания Тогда
						
						Комментарий = Комментарий + " " + НСтр("ru='до';uk='до'") + " " + Формат(ДатаОкончания, "ДЛФ=D");
						Если Не ХарактерНачисления = ПредопределенноеЗначение("Перечисление.ХарактерПлановогоНачисления.Штатное")
							Или ДатаОкончания < СтрокаНачисления.ДатаОкончанияНачисления Тогда
							
							Комментарий = Комментарий + ", " + НСтр("ru=' до';uk=' до'") + " " + Формат(СтрокаНачисления.ДатаОкончанияНачисления, "ДЛФ=D") + " " + КомментарийТекущегоЗначения;
							Если Не СтрокаНачисления.ИспользуетсяПоОкончании Тогда
								
								Комментарий = Комментарий + ", " + НСтр("ru='с';uk='з'") + " "
									+ Формат(ДатаНачалаСостоянияПоДатеОкончанияПредыдущего(СтрокаНачисления.ДатаОкончанияНачисления), "ДЛФ=D")
									+ " " + НСтр("ru='отменяется';uk='скасовується'");
								
							КонецЕсли;
							
						КонецЕсли;
						
					ИначеЕсли Не ХарактерНачисления = ПредопределенноеЗначение("Перечисление.ХарактерПлановогоНачисления.Штатное") Тогда
						Комментарий = Комментарий + " " + НСтр("ru='до';uk='до'") + " " + Формат(СтрокаНачисления.ДатаОкончанияНачисления, "ДЛФ=D");
					КонецЕсли;
			...
			ИначеЕсли ЗначениеЗаполнено(СтрокаНачисления.ДатаОкончанияНачисления)
				...
				Иначе
					...
					Комментарий = НСтр("ru='До';uk='До'") + " " + Формат(СтрокаНачисления.ДатаОкончанияНачисления, "ДЛФ=D")
						+ " " + Комментарий;
...
Функция ДатаНачалаСостоянияПоДатеОкончанияПредыдущего(Знач ДатаОкончания)
	
	Если ЗначениеЗаполнено(ДатаОкончания) Тогда
		Возврат КонецДня(ДатаОкончания) + 1;
	Иначе
		Возврат '00010101';
	КонецЕсли;
...
Процедура УстановитьКомментарииДействийСДополнительнымПоказателем(СтрокаПоказателя) Экспорт
	...
	Если СтрокаПоказателя.Действие = ПредопределенноеЗначение("Перечисление.ДействияСНачислениямиИУдержаниями.Утвердить") И (Не СтрокаПоказателя.ДействующийПоказатель) Тогда
		СтрокаПоказателя.Комментарий = НСтр("ru='Новый';uk='Новий'");
	ИначеЕсли СтрокаПоказателя.Действие = ПредопределенноеЗначение("Перечисление.ДействияСНачислениямиИУдержаниями.Отменить") Тогда
		СтрокаПоказателя.Комментарий = НСтр("ru='Отменен';uk='Скасовано'");
	...
	Иначе
		...
		Если Значение > ТекущееЗначение  Тогда
			Комментарий = Комментарий + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Увеличен (+%1);';uk='Збільшено (+%1);'"),
			Значение - ТекущееЗначение) + " ";
		ИначеЕсли  Значение < ТекущееЗначение Тогда
			Комментарий = Комментарий + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Уменьшен (%1);';uk='Зменшено (%1);'"),
			Значение - ТекущееЗначение) + " ";
		КонецЕсли;
...
// Предназначен для подключения типов предметов взаимодействия.
// См. ВзаимодействияКлиентСерверПовтИспПереопределяемый.ПолучитьМассивТиповПредметов.
//
Процедура ЗаполнитьТипыПредметовВзаимодействия(ТипыПредметов) Экспорт
	
	ПодборПерсоналаСуществует = Ложь;
	#Если Клиент Тогда
		ПодборПерсоналаСуществует = ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала");
	#Иначе
		ПодборПерсоналаСуществует = ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала");
	#КонецЕсли
	
	Если ПодборПерсоналаСуществует Тогда
		#Если Клиент Тогда
			Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ПодборПерсоналаКлиентСервер");
		#Иначе
			Модуль = ОбщегоНазначения.ОбщийМодуль("ПодборПерсоналаКлиентСервер");
		#КонецЕсли
		Модуль.ЗаполнитьТипыПредметовВзаимодействия(ТипыПредметов);
	КонецЕсли;
...
Функция ТипыМногофункциональныхДокументов() Экспорт 
	...
	ОписаниеДокумента.СообщениеДокументНеУтвержден = НСтр("ru='%1 - документ не рассчитан.';uk='%1 - документ не розраховано.'");
	...
	ОписаниеДокумента.ВторойОтветственный = "Рассчитал";
	ОписаниеДокумента.ВидУчета = "НачисленнаяЗарплата";
	ОписаниеДокумента.СообщениеДокументНеУтвержден = НСтр("ru='%1 - данные не включены в расчет зарплаты.';uk='%1 - дані не включені в розрахунок зарплати.'");
	...
	ОписаниеДокумента.СообщениеДокументНеУтвержден = НСтр("ru='%1 - ежемесячные начисления не установлены.';uk='%1 - щомісячні нарахування не встановлені.'");
	...
	ОписаниеДокумента.СообщениеДокументНеУтвержден = НСтр("ru='%1 - время не учтено.';uk='%1 - час не враховано.'");
	...
	#Если Клиент Тогда
		ПодработкиСуществуют = ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.Подработки");
	#Иначе
		ПодработкиСуществуют = ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.Подработки");
	#КонецЕсли
	
	Если ПодработкиСуществуют Тогда
		
		#Если Клиент Тогда
			Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("Подработки");
		#Иначе
	...
	#Если Клиент Тогда
		АттестацииСотрудниковСуществуют = ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.АттестацииСотрудников");
	#Иначе
		АттестацииСотрудниковСуществуют = ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.АттестацииСотрудников");
	#КонецЕсли
	
	Если АттестацииСотрудниковСуществуют Тогда
		
		#Если Клиент Тогда
			Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("АттестацииСотрудников");
		#Иначе
...
Функция ПолучитьКомментарийКДействиюСПрименениеПлановыхНачислений(Значение, ТекущееЗначение) Экспорт
	...
	Если Значение <> ТекущееЗначение Тогда
		Если ТекущееЗначение Тогда
			Комментарий = НСтр("ru='(Ранее начисления действовали)';uk='(Раніше нарахування діяли)'");
		Иначе	
			Комментарий = НСтр("ru='(Ранее начисления не действовали)';uk='(Раніше нарахування не діяли)'");
		КонецЕсли;
	КонецЕсли;
	Возврат Комментарий;
КонецФункции 
...
Функция ИменаКолонокТаблицыРаспределенияРезультатовРасчета(ОписаниеТаблицы) Экспорт
	...
	Если ИмяТаблицыРаспределениеРезультатов = "РаспределениеРезультатовНачислений" Тогда
		ИменаКолонок = ИменаКолонок + ",Территория,СпособОтраженияЗарплатыВБухучете,ОблагаетсяЕНВД";
	ИначеЕсли ИмяТаблицыРаспределениеРезультатов = "РаспределениеРезультатовУдержаний" Тогда
		ИменаКолонок = ИменаКолонок + ",ВидУдержания,Сотрудник,Подразделение";
	КонецЕсли;
	...
	Если ПоказательИнфо.ЭтоПоказательЗависящийОтСтажа Тогда
		...
		Если РежимРаботы = 0 Тогда
			...
			Если ПоказательИнфо.МаксимальноеЗначение <> Неопределено Тогда
				...
				СведенияСтрокиНачисления.ОписаниеСтажевыхПоказателей = СведенияСтрокиНачисления.ОписаниеСтажевыхПоказателей + НСтр("ru='макс.';uk='макс.'");
				
				Если ПоказательИнфо.МаксимальноеЗначение = 0 Тогда
					СведенияСтрокиНачисления.ОписаниеСтажевыхПоказателей = СведенияСтрокиНачисления.ОписаниеСтажевыхПоказателей + " " + НСтр("ru='не задан';uk='не заданий'");
				Иначе
...
Процедура ВводНачисленийИнициализироватьСтрокиНачислений(Форма, СтрокиНачислений, ОписаниеТаблицыВидовРасчета, РежимРаботы, Сотрудник = Неопределено, ДатаСведений = Неопределено) Экспорт
...
#КонецЕсли 
			...
			Если РежимРаботы = 2 Тогда
				Если ИмяТаблицы = "Удержания" Тогда
			...
			Если ЗначениеЗаполнено(ОписаниеТаблицыВидовРасчета.ИмяРеквизитаДокументОснование) Тогда 
				СтрокаНачислений["ДоступенВыборОснования"] = ИнфоОВидеРасчета.ПоддерживаетНесколькоПлановыхНачислений 
					И (Не ОписаниеТаблицыВидовРасчета.ЭтоПлановыеНачисленияУдержания Или СтрокаНачислений.ДействующийВидРасчета);
				Если Не ИнфоОВидеРасчета.ПоддерживаетНесколькоПлановыхНачислений Тогда 
					СтрокаНачислений[ОписаниеТаблицыВидовРасчета.ИмяРеквизитаДокументОснование] = Неопределено;
				КонецЕсли;
			КонецЕсли;
			...
			Если МаксимальноеЧислоПоказателей > МаксимальноеКоличествоПоказателейПоОписаниюТаблицы(Форма, ОписаниеТаблицыВидовРасчета,, РежимРаботы) Тогда
...
Функция ТекстПредупрежденияОписаниеНачисленийИзменилось() Экспорт
	...
	ТекстПредупреждения = НСтр("ru='За время вашей работы с документом существенно изменилось описание начислений. 
                                ...
                                |;uk='За час вашої роботи з документом істотно змінилося опис нарахувань. 
                                |Це призвело до того, що вам слід завершити роботу з документом і повторно відкрити його або перечитати дані документа (команда «Перечитати» з меню «%1»).'");	
								
	ЭтоТакси = ЭтоВариантИнтерфейсаКлиентскогоПриложенияТакси();
	
	Возврат СтрШаблон(ТекстПредупреждения, ?(ЭтоТакси, НСтр("ru='Еще';uk='Ще'"), НСтр("ru='Все действия';uk='Всі дії'")));
	...
	Если ИменаЭлементов <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ИмяГруппыОтдельныхПолей) Тогда
			...
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
				...
				"ТолькоПросмотр",
				РедактированиеПоказателей);
			
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
				Форма.Элементы,
				ИмяГруппыОтдельныхПолей + "Размеры",
				"ТолькоПросмотр",
				РедактированиеВкладаВФОТ);
...
Процедура УдалитьДанныеСотрудникаСписочногоДокумента(Объект, СтруктураОписания, Сотрудник) Экспорт
	...
	Если НетСведенийОСтаже Тогда
		...
		Если ВключатьВПредставлениеНаименованиеСтажа Тогда
			ПредставлениеКоманды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Стаж ""%1"" не задан';uk='Стаж ""%1"" не задано'"),
				Строка(ВидСтажа));;
		Иначе
			ПредставлениеКоманды = НСтр("ru='Стаж не задан';uk='Стаж не задано'");
		КонецЕсли;
		...
		Если Лет > 0 Тогда
			...
			Если (Лет < 5 ИЛИ Лет > 20)
				...
				Если ЛетМенееПяти = 1 Тогда
					РасшифровкаЛет = НСтр("ru='год';uk='рік'");
				Иначе
					РасшифровкаЛет = НСтр("ru='года';uk='року'");
				КонецЕсли;
				
			Иначе
				РасшифровкаЛет = НСтр("ru='лет';uk='років'");
			КонецЕсли; 
...
Функция ДатаОтсчетаСтажа(ИсходнаяДата, Лет, Месяцев, Дней) Экспорт 
	
	Возврат ДобавитьМесяц(ИсходнаяДата - 86400 * Дней, -(Лет * 12 + Месяцев));
	
КонецФункции

Функция ПродолжительностьСтажа(ДатаОтсчета, ДатаАктуальности) Экспорт 
	
	ДатаАктуальностиГод = Год(ДатаАктуальности);
	ДатаАктуальностиМесяц = Месяц(ДатаАктуальности);
	ДатаАктуальностиДень = День(ДатаАктуальности);
	
	ДатаОтсчетаГод = Год(ДатаОтсчета);
	ДатаОтсчетаМесяц = Месяц(ДатаОтсчета);
	ДатаОтсчетаДень = День(ДатаОтсчета);
	
	КоличествоЛет = ДатаАктуальностиГод - ДатаОтсчетаГод 
	- ?(ДатаАктуальностиМесяц * 100 + ДатаАктуальностиДень < ДатаОтсчетаМесяц * 100 + ДатаОтсчетаДень, 1, 0);
	...
	Иначе 
		...
		КоличествоМесяцев = ДатаАктуальностиМесяц + 12 - ДатаОтсчетаМесяц - ?(ДатаАктуальностиДень < ДатаОтсчетаДень, 1, 0);
		КоличествоМесяцев = ?(КоличествоМесяцев >= 12, КоличествоМесяцев - 12, КоличествоМесяцев);
		
		ДатаОтсчетаКоличестваДней = ДобавитьМесяц(ДатаОтсчета, КоличествоЛет * 12 + КоличествоМесяцев);
		КоличествоДней = ?(ДатаАктуальностиДень >= ДатаОтсчетаДень, ДатаАктуальностиДень - ДатаОтсчетаДень, 
			Макс(День(КонецМесяца(ДатаОтсчетаКоличестваДней)) - ДатаОтсчетаДень, 0) + ДатаАктуальностиДень);
		
	КонецЕсли;
	
	Возврат Новый Структура("Лет, Месяцев, Дней", КоличествоЛет, КоличествоМесяцев, КоличествоДней);
...
Функция ПредставлениеСтажа(Лет, Месяцев, Дней) Экспорт 
	...
	Если СтрНайти("0,5,6,7,8,9", ПоследняяЦифра) <> 0 Тогда 
		КоличествоЛетШаблон = НСтр("ru='%1 лет';uk='%1 років'");
	Иначе 
		...
		Если ПоследняяЦифра = "1" Тогда 
			КоличествоЛетШаблон = ?(ПоследниеДвеЦифры = "11", НСтр("ru='%1 лет';uk='%1 років'"), НСтр("ru='%1 год';uk='%1 рік'"));
		Иначе
			КоличествоЛетШаблон = ?(Найти("12,13,14", ПоследниеДвеЦифры) = 0, НСтр("ru='%1 года';uk='%1 року'"), НСтр("ru='%1 лет';uk='%1 років'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если СтрНайти("0,5,6,7,8,9", ПоследняяЦифра) <> 0 Тогда 
		КоличествоМесяцевШаблон = НСтр("ru='%1 месяцев';uk='%1 місяців'");
	Иначе 
		ПоследниеДвеЦифры = Прав(Месяцев, 2);
		ПоследниеДвеЦифры = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(ПоследниеДвеЦифры, 2, "0");
		Если ПоследняяЦифра = "1" Тогда 
			КоличествоМесяцевШаблон = ?(ПоследниеДвеЦифры = "11", НСтр("ru='%1 месяцев';uk='%1 місяців'"), НСтр("ru='%1 месяц';uk='%1 місяць'"));
		Иначе
			КоличествоМесяцевШаблон = ?(Найти("12,13,14", ПоследниеДвеЦифры) = 0, НСтр("ru='%1 месяца';uk='%1 місяця'"), НСтр("ru='%1 месяцев';uk='%1 місяців'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если СтрНайти("0,5,6,7,8,9", ПоследняяЦифра) <> 0 Тогда 
		КоличествоДнейШаблон = НСтр("ru='%1 дней';uk='%1 днів'");
	Иначе 
		ПоследниеДвеЦифры = Прав(Дней, 2);
		ПоследниеДвеЦифры = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(ПоследниеДвеЦифры, 2, "0");
		Если ПоследняяЦифра = "1" Тогда 
			КоличествоДнейШаблон = ?(ПоследниеДвеЦифры = "11", НСтр("ru='%1 дней';uk='%1 днів'"), НСтр("ru='%1 день';uk='%1 день'"));
		Иначе
			КоличествоДнейШаблон = ?(Найти("12,13,14", ПоследниеДвеЦифры) = 0, НСтр("ru='%1 дня';uk='%1 дня'"), НСтр("ru='%1 дней';uk='%1 днів'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ПредставлениеСтажа <> "" И Дней <> 0 Тогда 
		ПредставлениеСтажа = ПредставлениеСтажа + " " + НСтр("ru='и';uk='і'") + " " + КоличествоДнейТекст;
	КонецЕсли;
...
Процедура ЗаполнитьЗначенияПоказателейТарифныхСеток(Форма, КоллекцияСтрокНачислений, ОписаниеТаблицыВидовРасчета, РежимРаботы, ТарифнаяСетка, РазрядКатегория, ДатаСведений, ТарифнаяСеткаНадбавки = Неопределено, РазрядКатегорияНадбавки = Неопределено, СчитатьПоказателиПоДолжности = Ложь, ПКУ  = Неопределено) Экспорт
	...
	Для Каждого СтрокаНачислений Из КоллекцияСтрокНачислений Цикл
		...
		Если Действие <> ПредопределенноеЗначение("Перечисление.ДействияСНачислениямиИУдержаниями.Отменить") Тогда 
			ЗаполнитьЗначенияПоказателейТарифныхСетокВСтрокеНачислений(Форма, СтрокаНачислений, ОписаниеТаблицыВидовРасчета, РежимРаботы, ТарифнаяСетка, РазрядКатегория, ДатаСведений, ТарифнаяСеткаНадбавки, РазрядКатегорияНадбавки, СчитатьПоказателиПоДолжности, ПКУ);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаполнитьЗначенияПоказателейТарифныхСетокВСтрокеНачислений(Форма, СтрокаНачислений, ОписаниеТаблицыВидовРасчета, РежимРаботы, ТарифнаяСетка, РазрядКатегория, ДатаСведений, ТарифнаяСеткаНадбавки = Неопределено, РазрядКатегорияНадбавки = Неопределено, СчитатьПоказателиПоДолжности = Ложь, ПКУ  = Неопределено) Экспорт
