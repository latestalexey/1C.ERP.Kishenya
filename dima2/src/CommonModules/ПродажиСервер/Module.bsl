...
Функция ЗаполнитьПоФактическойОтгрузке(ОбъектДокумента, СтруктураДокументаОтгрузки) Экспорт
	...
	Если СтруктураДокументаДляЗаполнения.ЗаказЯвляетсяРаспоряжением Тогда
		...
		//++ НЕ УТКА
		МассивТиповЗаказов.Добавить(Тип("ДокументСсылка.ЗаказДавальца"));
		//-- НЕ УТКА
...
Процедура ПроверитьКорректностьЗаполненияДокументаПродажи(Знач ДокументПродажи, Отказ) Экспорт
	...
	ИначеЕсли ТипДокумента = Тип("ДокументОбъект.ЗаказПереработчику") Тогда
		...
		ВсеСтрокиОтменены = ОбщегоНазначенияУТ.ВсеСтрокиОтменены(ДокументПродажи, "Продукция", "Отменено");
		ВсеСтрокиОтменены = ВсеСтрокиОтменены И ОбщегоНазначенияУТ.ВсеСтрокиОтменены(ДокументПродажи, "Материалы", "Отменено");
	...
	//-- НЕ УТКА
	//++ НЕ УТ
	КонтрольОтгрузкиЗаказаПереработчику			= ПолучитьФункциональнуюОпцию("НеЗакрыватьЗаказыПереработчикамБезПолнойОтработки");
	ИспользоватьСтатусыЗаказовПереработчикам	= ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыЗаказовПереработчикам");
	//-- НЕ УТ
	...
	ИначеЕсли ИмяТаблицы = "Документ.ЗаказПереработчику" Тогда
		...
		Если ДокументПродажи.Статус = Перечисления.СтатусыЗаказовПереработчикам.Закрыт И Не ВсеСтрокиОтменены Тогда
			
			Если КонтрольОтгрузкиЗаказаПереработчику И ИспользоватьСтатусыЗаказовПереработчикам Тогда
				МассивПроверок.Добавить("КонтрольЗавершенияОтгрузки");
			КонецЕсли;
...
Функция ПолучитьТекстЗапросаДокументыРеализации(ВводПоСтроке = Ложь) Экспорт
	...
	//++ НЕ УТКА
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказДавальца)
...
Функция ТекстЗапросаПоФактическойОтгрузке(СтруктураДокументаДляЗаполнения, СтруктураРезультатаЗапроса = Неопределено)
	...
	|	ЗаказТовары.Количество КАК КоличествоВЗаказе,
	|	ЗаказТовары.Цена КАК Цена,
	|	ЗаказТовары.Ссылка.СтавкаНДС КАК СтавкаНДС,
	|	ЗаказТовары.Сумма КАК Сумма,
	|	ЗаказТовары.СуммаНДС КАК СуммаНДС,
	|	ЗаказТовары.СуммаСНДС КАК СуммаСНДС,
...
Процедура СформироватьЗапросВременнаяТаблицаШапкаДокументаПродажи(ТекстЗапроса,
	...
	Если ПоСсылке Тогда
		...
		ИначеЕсли ИмяТаблицы = "Документ.ЗаказКлиента" ИЛИ ИмяТаблицы = "Документ.ЗаданиеТорговомуПредставителю" Тогда
		...
		ИначеЕсли ИмяТаблицы = "Документ.ЗаказДавальца" Тогда
			...
			ВременныйТекст = "
				...
				|	ДокументПродажи.СуммаДокумента        КАК СуммаДокумента,
				|	ДокументПродажи.Склад                 КАК Склад,
				|	ДокументПродажи.СкладПоступления      КАК СкладПоступления,
				|	ДокументПродажи.ФормаОплаты           КАК ФормаОплаты
				|";
	...
	Иначе
		...
		//++ НЕ УТ
		ТаблицаДокумента.Колонки.Добавить("СкладПоступления",                              Новый ОписаниеТипов("СправочникСсылка.Склады"));
		//-- НЕ УТ
...
Процедура СформироватьЗапросВременнаяТаблицаТоварыДокументаПродажи(ТекстЗапроса,
	...
	ИначеЕсли ИмяТаблицы = "Документ.ЗаказДавальца" Тогда
		...
		ТекстЗапроса = ТекстЗапроса + "
			...
			|	Товары.КодСтроки                        КАК КодСтроки
			|
			|ПОМЕСТИТЬ ВТ_ДопТовары
			|
			|ИЗ
			|	%ИмяДопТаблицы% КАК Товары
			|	%УсловиеОтбора%
			|;
			|ВЫБРАТЬ
			|	&ИмяТабличнойЧасти                      КАК ИмяТЧ,
			|	&ПредставлениеТабличнойЧасти            КАК ПредставлениеТЧ,
			|	Товары.НомерСтроки                      КАК НомерСтроки,
			|	Товары.Номенклатура                     КАК Номенклатура,
			|	Товары.Характеристика                   КАК Характеристика,
			|	Товары.Упаковка                         КАК Упаковка,
			...
			|
			|ВЫБРАТЬ
			|	&ИмяДопТабличнойЧасти                   КАК ИмяТЧ,
			|	&ПредставлениеДопТабличнойЧасти         КАК ПредставлениеТЧ,
			|	Товары.НомерСтроки                      КАК НомерСтроки,
			|	Товары.Номенклатура                     КАК Номенклатура,
			|	Товары.Характеристика                   КАК Характеристика,
			|	Товары.Упаковка                         КАК Упаковка,
		...
		ИмяДопТабличнойЧасти = "Материалы";
		
		ПредставлениеТабличнойЧасти    = НСтр("ru='Продукция';uk='Продукція'");
		ПредставлениеДопТабличнойЧасти = НСтр("ru='Материалы';uk='Матеріали'");
		...
		ИменаКолонокДопТабличнойЧасти = "НомерСтроки,
			|Номенклатура,
			|Характеристика,
			|Упаковка,
			|Склад,
	...
	ИначеЕсли ИмяТаблицы = "Документ.ПередачаСырьяПереработчику" Тогда
		...
		ПредставлениеТабличнойЧасти = НСтр("ru='Сырье и материалы';uk='Сировина й матеріали'");
...
// Формирует текст запроса для проверки заполненности склада в Заказе давальца
//
// Параметры:
// ТекстЗапроса - Строка - текстовая строка, к которой необходимо добавить текст запроса
//
Процедура СформироватьЗапросКорректностьЗаполненияСкладаВЗаказеДавальца(ТекстЗапроса)
	
	ТекстЗапроса = ТекстЗапроса +
	"ВЫБРАТЬ
	|	ЕСТЬNULL(МАКСИМУМ(ЕстьОшибкиЗаполненияСклад), ЛОЖЬ) КАК ЕстьОшибкиЗаполненияСклад,
	|	ЕСТЬNULL(МАКСИМУМ(ЕстьОшибкиЗаполненияСкладПоступления), ЛОЖЬ) КАК ЕстьОшибкиЗаполненияСкладПоступления
	|
	|ИЗ
	|
	|(ВЫБРАТЬ ПЕРВЫЕ 1
	|	ИСТИНА КАК ЕстьОшибкиЗаполненияСклад,
	|	ЛОЖЬ КАК ЕстьОшибкиЗаполненияСкладПоступления
	|
	|ИЗ
	|	ВременнаяТаблицаТовары КАК Товары
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВременнаяТаблицаДокументПродажи КАК Продажи
	|	ПО
	|		ИСТИНА
	|
	|ГДЕ
	|	Товары.ИмяТЧ = ""Продукция""
	|	И НЕ Товары.Отменено
	|	И НЕ Товары.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.НеТребуется)
	|	И Продажи.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ЛОЖЬ КАК ЕстьОшибкиЗаполненияСклад,
	|	ИСТИНА КАК ЕстьОшибкиЗаполненияСкладПоступления
	|
	|ИЗ
	|	ВременнаяТаблицаТовары КАК Товары
	|
	...
	|
	|ГДЕ
	|	Товары.ИмяТЧ = ""Материалы""
	|	И Продажи.СкладПоступления = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|
	|) КАК ВложенныйЗапрос
	|;
	|
...
// Формирует текст запроса для проверки заполенности реквизита "Склад" в строках табличных частях Заказа давальца
// Проверка должна выполняться только если:
// 		- в шапке документа указан заполнен реквизит "Склад" для ТЧ "Продукция";
// 		- в шапке документа указан заполнен реквизит "СкладПоступления" для ТЧ "Материалы".
//
Процедура СформироватьЗапросКорректностьЗаполненияСкладовВТабличнойЧастиТоварыЗаказаДавальца(ТекстЗапроса)
	
	ТекстЗапроса = ТекстЗапроса + 
	"ВЫБРАТЬ
	|	Товары.НомерСтроки КАК НомерСтроки,
	|	Товары.ИмяТЧ КАК ИмяТЧ,
	|	Товары.ПредставлениеТЧ КАК ПредставлениеТЧ
	|
	|ИЗ
	|	ВременнаяТаблицаТовары КАК Товары
	|
	...
	|ГДЕ
	|	Товары.ИмяТЧ = ""Материалы""
	|	И Продажи.СкладПоступления.ЭтоГруппа
	|	И Продажи.СкладПоступления <> ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|	И Товары.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|;
	|
...
// Формирует запрос для контроля завершения оформления отчетов по заказу давальца
//
// Параметры:
// ТекстЗапроса - Строка - текстовая строка, к которой необходимо добавить текст запроса
//
Процедура СформироватьЗапросКонтрольОформленияОтчетов(ТекстЗапроса)

	ТекстЗапроса = ТекстЗапроса + "
		|ВЫБРАТЬ
		|	Документ.Валюта КАК Валюта,
		|	УслугиДавальцу.СуммаКонечныйОстаток КАК Сумма,
		|	ВЫБОР КОГДА ЕСТЬNULL(УслугиДавальцу.СуммаКонечныйОстаток, 0) <= 0 ТОГДА
		|		ЛОЖЬ
		|	ИНАЧЕ
		|		ИСТИНА
		|	КОНЕЦ КАК ЕстьНеоформленныеОтчеты
		|ИЗ
		|	РегистрНакопления.УслугиДавальцуКОформлению.ОстаткиИОбороты(, , , , ЗаказДавальца = &Ссылка) КАК УслугиДавальцу
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
		|		ВременнаяТаблицаДокументПродажи КАК Документ
		|	ПО
		|		ИСТИНА
		|;
		|";
...
// Отказ           - Булево - Флаг отказа от проведения документа
//
Процедура СообщитьОбОшибкахКорректностьЗаполненияСкладаЗаказаДавальца(Знач Выборка,
	                                                    Знач ДокументПродажи,
	                                                    Отказ)
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ЕстьОшибкиЗаполненияСкладПоступления Тогда
			
			ТекстОшибки = НСтр("ru='Поле ""Склад поступления"" не заполнено';uk='Поле ""Склад надходження"" не заповнено'");
...
//
Процедура СообщитьОбОшибкахКорректностьЗаполненияСкладаВТабличнойЧастиТоварыЗаказаДавальца(Знач Выборка,
	                                                    Знач ДокументПродажи,
	                                                    Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Выборка.НомерСтроки,
			Выборка.ПредставлениеТЧ);
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ТекстОшибки,
			ДокументПродажи,
			ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(Выборка.ИмяТЧ, Выборка.НомерСтроки, "Склад"),
			,
			Отказ);
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ЕстьОшибкиЗаполненияСклад Тогда
			Если НЕ ВыводитьВОтчет Тогда
				...
				Иначе
					Если Выборка.СкладСоглашениеЕстьГруппа Тогда
						ТекстОшибки = НСтр("ru='Склад должен входить в группу складов соглашения ""%СкладСоглашение%""';uk='Склад повинен входити в групу складів угоди ""%СкладСоглашение%""'");
					Иначе
...
Процедура СообщитьОбОшибкахСоответствиеСуммыПредоплатыГрафикуОплат(Знач Выборка,
	...
	Если Выборка.Следующий() И Выборка.ЕстьОшибкиСоответствиеСуммыПредоплатыГрафикуОплат Тогда
		...
		Если ВыводитьВОтчет Тогда
			Отчеты.ОтклоненияОтУсловийПродаж.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru='Предоплата';uk='Передплата'"), ТекстОшибки, МассивОтклонений);
		Иначе
			СообщитьПользователюОбОшибке(
				ТекстОшибки,
				,
...
Процедура СообщитьОбОшибкахКонтрольЗавершенияРасчетов(Знач Выборка,
	...
	Если Выборка.Следующий() Тогда 
		Если Выборка.ЕстьНезавершенныеРасчеты  Тогда
			Если ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказКлиента") Тогда
                ...
                |Закриття замовлення можливе тільки з повністю сплаченими/скасованими рядками'");
			ИначеЕсли ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказПереработчику") Тогда
				ТекстОшибки = НСтр("ru='Расчеты по заказу не завершены.
                |Для закрытия заказа требуется оплата %СуммаКОплате% %Валюта%.
                |Закрытие заказа возможно только с полностью оплаченными/отмененными строками'
                |;uk='Розрахунки по замовленню не завершені.
                ...
                |Закриття замовлення можливе тільки з повністю сплаченими/скасованими рядками'");
			//++ НЕ УТКА
			ИначеЕсли ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказДавальца") Тогда
				ТекстОшибки = НСтр("ru='Расчеты по заказу не завершены.
                |Для закрытия заказа требуется оплата %СуммаКОплате% %Валюта%.
                |Закрытие заказа возможно только с полностью оплаченными/отмененными строками'
                |;uk='Розрахунки по замовленню не завершені.
			...
			Иначе
                ...
                |Заявка может быть переведена в статус ""Выполнена"" только с полностью оплаченными/отмененными строками'
                |;uk='Розрахунки за заявкою не завершені. Потрібна оплата %СуммаКОплате% %Валюта%.
                |Заявка може бути переведена в статус ""Виконано"" тільки з повністю сплаченими/скасованими рядками'");
			КонецЕсли;
	...
	Если ДокументПродажи.ЭтоНовый() Тогда
			Если ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказКлиента") Тогда
				ТекстОшибки = НСтр("ru='Заказ не оплачен. Закрытие заказа возможно только с полностью оплаченными/отмененными строками';uk='Замовлення не сплачено. Закриття замовлення можливе тільки з повністю сплаченими/скасованими рядками'");
			ИначеЕсли ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказПереработчику") Тогда
				ТекстОшибки = НСтр("ru='Заказ не оплачен. Закрытие заказа возможно только с полностью оплаченными/отмененными строками';uk='Замовлення не сплачено. Закриття замовлення можливе тільки з повністю сплаченими/скасованими рядками'");
			//++ НЕ УТКА
			ИначеЕсли ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказДавальца") Тогда
			...
			//-- НЕ УТКА
			Иначе
				ТекстОшибки = НСтр("ru='Расчеты по заявке не завершены. Заявка может быть переведена в статус ""Выполнена"" только с полностью оплаченными/отмененными строками';uk='Розрахунки за заявкою не завершені. Заявка може бути переведена в статус ""Виконано"" тільки з повністю сплаченими/скасованими рядками'");
			КонецЕсли;
...
Процедура СообщитьОбОшибкахКонтрольЗавершенияОтгрузки (Знач Выборка,
	...
	Если Выборка.Следующий() Тогда 
		Если Выборка.ЕстьНезавершеннаяОтгрузка Тогда
			Если ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказКлиента") Тогда
                ...
                |Закрытие заказа возможно только с полностью отгруженными/отмененными строками'
                |;uk='Замовлення відвантажено не повністю.
                |Закриття замовлення можливе тільки з повністю відвантаженими/скасованими рядками'");
			ИначеЕсли ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказПереработчику") Тогда
				ТекстОшибки = НСтр("ru='Сырье и материалы отгружены не полностью.
                |Закрытие заказа возможно только с полностью отгруженными/отмененными строками'
                |;uk='Сировина і матеріали відвантажені не повністю.
                |Закриття замовлення можливе тільки з повністю відвантаженими/скасованими рядками'");
			//++ НЕ УТКА
			ИначеЕсли ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказДавальца") Тогда
				ТекстОшибки = НСтр("ru='Продукция отгружена не полностью.
                |Закрытие заказа возможно только с полностью отгруженными/отмененными строками'
                |;uk='Продукція відвантажена не повністю.
                |Закриття замовлення можливе тільки з повністю відвантаженими/скасованими рядками'");
			//-- НЕ УТКА
			Иначе
				ТекстОшибки = НСтр("ru='Заменяющие товары отгружены не полностью.
                |Заявка может быть переведена в статус ""Выполнена"" только с полностью оплаченными/отмененными строками'
                |;uk='Замінюючі товари відвантажені не повністю.
                |Заявка може бути переведена в статус ""Виконана"" тільки з повністю сплаченими/скасованими рядками'");
			КонецЕсли;
	...
	Если ДокументПродажи.ЭтоНовый() Тогда
			Если ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказКлиента") Тогда
                ...
                |Закрытие заказа возможно только с полностью отгруженными/отмененными строками'
                |;uk='Замовлення не відвантажено. Закриття замовлення неможливо.
                |Закриття замовлення можливе тільки з повністю відвантаженими/скасованими рядками'");
			ИначеЕсли ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказПереработчику") Тогда
				ТекстОшибки = НСтр("ru='Сырье и материалы не отгружены. Закрытие заказа невозможно.
                |Закрытие заказа возможно только с полностью отгруженными/отмененными строками'
                |;uk='Сировина і матеріали не відвантажені. Закриття замовлення неможливе.
                |Закриття замовлення можливе тільки з повністю відвантаженими/скасованими рядками'");
			//++ НЕ УТКА
			ИначеЕсли ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказДавальца") Тогда
				ТекстОшибки = НСтр("ru='Продукция не отгружена. Закрытие заказа невозможно.
                |Закрытие заказа возможно только с полностью отгруженными/отмененными строками'
                |;uk='Продукція не відвантажена. Закриття замовлення неможливо.
                |Закриття замовлення можливе тільки з повністю відвантаженими/скасованими рядками'");
			//-- НЕ УТКА
			Иначе
				ТекстОшибки = НСтр("ru='Заменяющие товары не отгружены. Заявка не может быть переведена в статус ""Выполнена"".
                |Заявка может быть переведена в статус ""Выполнена"" только с полностью отгруженными/отмененными строками'
                |;uk='Замінюючі товари не відвантажені. Заявка не може бути переведена в статус ""Виконана"".
                |Заявка може бути переведена в статус ""Виконана"" тільки з повністю відвантаженими/скасованими рядками'");
			КонецЕсли;
...
// Выводит сообщения об ошибках оформления отчетов давальцам
//
// Параметры:
// Выборка          - Выборка из результата запроса
// ДокументПродажи  - ДокументОбъект, для которого необходимо вывести сообщения об ошибках
// Отказ            - Булево - Флаг отказа от проведения документа
//
Процедура СообщитьОбОшибкахКонтрольОформленияОтчетов(Знач Выборка,
	                                                          Знач ДокументПродажи,
	                                                          Отказ)
	Если Выборка.Следующий() Тогда 
		
		Если Выборка.ЕстьНеоформленныеОтчеты  Тогда
			
			ТекстОшибки = НСтр("ru='Не все отчеты давальцам сформированы.
            |Для закрытия заказа требуется оформление отчетов на %СуммаКОплате% %Валюта%.
            |Закрытие заказа возможно только с откорректированной стоимостью услуг'
            |;uk='Не всі звіти давальцям сформовані.
            |Для закриття замовлення потрібно оформлення звітів на %СуммаКОплате% %Валюта%.
            |Закриття замовлення можливе тільки з відкоригованою вартістю послуг'");
			
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%СуммаКОплате%",	Строка(Выборка.Сумма));
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Валюта%",			Строка(Выборка.Валюта));
	...
	Если ДокументПродажи.ЭтоНовый() Тогда
		...
		ТекстОшибки = НСтр("ru='Отчеты давальцам не сформированы. 
        |Закрытие заказа возможно только после их оформления'
        |;uk='Звіти давальцям не сформовані. 
        |Закриття замовлення можливе тільки після їх оформлення'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ТекстОшибки,
			,
			,
...
Функция ПолучитьТекстЗапросаВременнойТаблицыЦенПоСоглашению(ИмяТаблицы, ИмяВременнойТаблицыТоваров)
	...
	|				И (РегУчетнаяПолитика.УчетнаяПолитика.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.НалогНаПрибыльИНДС) ИЛИ РегУчетнаяПолитика.УчетнаяПолитика.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.ЕдиныйНалогИНДС)))
