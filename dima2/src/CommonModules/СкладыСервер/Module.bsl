...
Функция ПереоформитьРасходныеОрдера(Параметры) Экспорт
	...
	ТаблицаНоменклатурыДляЗапроса = ОбщегоНазначенияУТ.ДанныеДерева(ДеревоТоварыКОтгрузке);
...
Функция ПолучитьОбъектОрдер(ПараметрыПоискаРасходногоОрдера)
	...
	Запрос.УстановитьПараметр("ОтгрузкаПоЗаданиюНаПеревозку", ЗначениеЗаполнено(ПараметрыПоискаРасходногоОрдера.ЗаданиеНаПеревозку));
	Запрос.УстановитьПараметр("ПустойПолучатель", ПустойПолучательОтправитель());
	...
	Иначе
		...
		ДокументОбъект.ОтгрузкаПоЗаданиюНаПеревозку = ЗначениеЗаполнено(ПараметрыПоискаРасходногоОрдера.ЗаданиеНаПеревозку);
		Если ЗначениеЗаполнено(ПараметрыПоискаРасходногоОрдера.ЗаданиеНаПеревозку) Тогда
...
Функция ЗаполнитьРасходныйОрдерНаТовары(ПараметрыЗаполненияРасходногоОрдера, ДокументОбъект)
	...
	ДокументОбъект.ОтгрузкаПоЗаданиюНаПеревозку = ЗначениеЗаполнено(ПараметрыЗаполненияРасходногоОрдера.ЗаданиеНаПеревозку);
	Если ЗначениеЗаполнено(ПараметрыЗаполненияРасходногоОрдера.ЗаданиеНаПеревозку) Тогда
...
Функция ТекстЗапросаСкладскаяОперацияТипДокумента() Экспорт
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.СкладскиеОперации.ОтгрузкаКлиенту),
	|	ТИП(Документ.ПередачаСырьяПереработчику),
	|	НЕОПРЕДЕЛЕНО
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.СкладскиеОперации.ОтгрузкаНаВнутренниеНужды),
	|	ТИП(Документ.КорректировкаЗаказаМатериаловВПроизводство),
	|	НЕОПРЕДЕЛЕНО
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.СкладскиеОперации.ОтгрузкаПоВозвратуПоставщику),
	|	ТИП(Документ.ВозвратСырьяДавальцу),
	|	НЕОПРЕДЕЛЕНО
	|
	//-- НЕ УТКА
	|;
...
Процедура ЗапускВыполненияФоновогоПереводаСтатусаРасходногоОрдераВОтгружено(ЗаданиеНаПеревозку) Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Перевод статусов расходных ордеров в ""Отгружено"" по отправленному заданию на перевозку - %ЗаданиеНаПеревозку%';uk='Переклад статусів видаткових ордерів у ""Відвантажено"" за надісланим завданням на перевезення - %ЗаданиеНаПеревозку%'");
