...
Процедура ПриЧтенииСозданииФормыСУпаковочнымиЛистами(Форма,
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы,
		ИмяТЧТовары + "НомерСтроки",
		"Видимость",
		Не Форма.ИспользоватьУпаковочныеЛисты);
	
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы,
		ИмяТЧТовары + "ГруппаНоменклатураУпаковочныйЛист",
		"ОтображатьВШапке",
		Форма.ИспользоватьУпаковочныеЛисты);
...
//	СтруктураДействийСТекущейСтрокой - Структура,
...
Процедура НоменклатураУпаковочныйЛистПриИзменении(Форма, ТЧТовары, ИмяПоля, КэшированныеЗначения,
	...
	ТекущаяСтрока = ТЧТовары.НайтиПоИдентификатору(Форма.Элементы[ИмяТЧТовары].ТекущаяСтрока);
	...
	Если ИмяПоля = ИмяТЧТовары + "УпаковочныйЛист" И КэшированныеЗначения.УпаковочныйЛист = ТекущаяСтрока.УпаковочныйЛист
		Или ИмяПоля = ИмяТЧТовары + "Номенклатура" И КэшированныеЗначения.Номенклатура = ТекущаяСтрока.Номенклатура Тогда
		Возврат;
	КонецЕсли;

	Если ИмяПоля = ИмяТЧТовары + "УпаковочныйЛист" Тогда
	...
	Если ИмяПоля = ИмяТЧТовары + "УпаковочныйЛист"
		...
		ТекстОшибки = НСтр("ru='Нельзя выбрать %УпаковочныйЛист%, т.к. он уже включен в состав документа.';uk='Не можна вибрати %УпаковочныйЛист%, т. я. він вже включений до складу документа.'");
...
Процедура РежимПросмотраПриИзменении(Форма, ТЧТовары, ЗаголовокНачальногоУровня, РежимПросмотраПоТоварам = Неопределено) Экспорт
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(
		Форма.Элементы,
		ИмяТЧТовары + "КартинкаУпаковочногоЛиста," + ИмяТЧТовары + "НаУровеньВверх,
		|НадписьНавигация," + ИмяТЧТовары + "Распаковать," + ИмяТЧТовары + "Упаковать,
		|" + ИмяТЧТовары + "ДопИнфоУпаковочныйЛист",
		"Видимость",
		Не РежимПросмотраПоТоварам);
	
	Форма.Элементы[ИмяТЧТовары + "ГруппаНоменклатураУпаковочныйЛист"].ОтображатьВШапке = Не РежимПросмотраПоТоварам;
	
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(
		Форма.Элементы,
		"ДекорацияНавигацияОтступ," + ИмяТЧТовары + "УпаковочныйЛистРодитель",
		"Видимость",
	...
	Форма.Элементы[ИмяТЧТовары + "Номенклатура"].ОтображатьВШапке = РежимПросмотраПоТоварам;
	...
	Иначе
		...
		ИдентификаторСтроки = Форма.Элементы[ИмяТЧТовары].ТекущаяСтрока;
...
Функция УпаковатьСервер(Форма) Экспорт
	...
	Если ЕстьНеотгружаемые Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Неотгружаемые строки не будут добавлены в упаковочный лист.';uk='Невідвантажувані рядки не будуть додані в пакувальний лист.'"));
	КонецЕсли;
	...
	Если ПараметрыУказанияСерий(Форма).ИмяТЧТовары <> ПараметрыУказанияСерий(Форма).ИмяТЧСерии Тогда
		...
		ТаблицаТовары.Свернуть("Номенклатура,Характеристика,Упаковка,УпаковочныйЛист,ЭтоУпаковочныйЛист,УпаковочныйЛистРодитель",
		...
		|			И (Товары.Упаковка = Серии.Упаковка
		|				ИЛИ НЕ &СоединятьПоУпаковкам)
		...
		Если ЕстьНеотгружаемые Тогда
			ТаблицаСерии = Форма.Объект.Серии.Выгрузить(Новый Структура("НеОтгружать", 0));
		Иначе
			ТаблицаСерии = Форма.Объект.Серии.Выгрузить();
		КонецЕсли;
		Запрос.УстановитьПараметр("ТаблицаСерии", ТаблицаСерии);
		
		Запрос.УстановитьПараметр("СоединятьПоУпаковкам",
			ПараметрыУказанияСерий(Форма).ПоляСвязи.Найти("Упаковка") = Неопределено);
	...
	НоваяСтрока = Форма.Объект[ИмяТЧТовары].Вставить(МинимальныйИндекс);
	...
	ЗаполнитьСлужебныеРеквизиты(Форма, Форма.Объект[ИмяТЧТовары], Неопределено, НоваяСтрока, Ложь);
...
Процедура УстановитьУсловноеОформлениеСУчетомУпаковочныхЛистов(Форма, ИмяТЧ = "Товары") Экспорт
	...
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ИмяТЧ + "УпаковочныйЛист");
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ИмяТЧ + "ДопИнфоУпаковочныйЛист");
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ИмяТЧ + "УпаковочныйЛистКод");
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ИмяТЧ + "Номенклатура");
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ИмяТЧ + "ДопИнфоНоменклатура");
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ИмяТЧ + "НоменклатураКод");
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ИмяТЧ + "НоменклатураАртикул");
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<артикул не используется>';uk='<артикул не використовується>'"));
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ИмяТЧ + "Характеристика");
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ИмяТЧ + "НоменклатураЕдиницаИзмерения");
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ИмяТЧ + "Упаковка");
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ИмяТЧ + "КоличествоУпаковок");
...
// Параметры:
//	Объект - ДокументОбъект, ДанныеФормыСтруктура - проверяемый документ,
...
Процедура ПроверитьЗаполнениеТЧСУпаковочнымиЛистами(Объект, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов, Отказ, ИмяТЧ = "Товары") Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ЕстьДубли Тогда
			ТекстОшибки = НСтр("ru='%УпаковочныйЛист% указан в документе несколько раз. Удалите дублирующиеся строки с упаковочными листами, используя режим просмотра ""По товарным местам"".';uk='%УпаковочныйЛист% зазначений у документі кілька разів. Видаліть дублюючі рядки з пакувальними листами, використовуючи режим перегляду ""По товарних місцях"".'");
		...
		Если Выборка.КоличествоЗаполненныхСтрок = 0 Тогда
			ТекстОшибки = НСтр("ru='%УпаковочныйЛист%: состав не заполнен.';uk='%УпаковочныйЛист%: склад не заповнений.'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%УпаковочныйЛист%", Выборка.УпаковочныйЛист);
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого Реквизит Из РеквизитыТЧДляПроверки Цикл
			Если Не ЗначениеЗаполнено(Выборка[Реквизит]) Тогда
				ТекстОшибки = НСтр("ru='Не заполнена колонка ""%Реквизит%"" в строке %НомерСтроки% списка ""%ИмяТЧ%"".';uk='Не заповнена колонка ""%Реквизит%"" в рядку %НомерСтроки% списку ""%ИмяТЧ%"".'");
...
//	возвращает массив ссылок на упаковочные листы, ТЧ которых потребовалось синхронизировать.
// Параметры:
...
Функция СинхронизироватьУпаковочныеЛистыСРасходнымОрдером(ОрдерСсылка, ИспользоватьАдресноеХранение) Экспорт
	...
	Результат = Запрос.ВыполнитьПакет();
	УпаковочныеЛисты = Результат[0].Выгрузить().ВыгрузитьКолонку("УпаковочныйЛист");
...
//	Форма - УправляемаяФорма,
//	ТЧТовары - ДанныеФормыКоллекция - табличная часть, в которой отображаются товары и упаковочные листы,
//	СтрокиДляОбработкиОбъединяемые - Массив из ДанныеФормыЭлементКоллекции - в этот массив собираются строки ТЧ,
//		дополненные из упаковочного листа,
...
Процедура ДополнитьСтрокамиПоУпаковочнымЛистам(Форма,
	...
	Запрос = Новый Запрос;
		...
		|	УпаковочныйЛистТовары.НоменклатураПоставщика,
	...
	ИмяСобытия  = НСтр("ru='Ошибка при дополнении документа строками по упаковочным листам.';uk='Помилка при доповненні документа рядками за пакувальними листами.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ТекстОшибки = НСтр("ru='Нельзя добавить %УпаковочныйЛист%, т.к. он содержит упаковочные листы, которые уже включены в состав документа.';uk='Не можна додати %УпаковочныйЛист%, т. я. він містить пакувальні листи, які вже включені до складу документа.'");
	
	Пока ВыборкаПоОшибкам.Следующий() Цикл
		
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%УпаковочныйЛист%", ВыборкаПоОшибкам.Ссылка);
		НайденныеСтроки = ТЧТовары.НайтиСтроки(Новый Структура("УпаковочныйЛист", ВыборкаПоОшибкам.Ссылка));
		Если НайденныеСтроки.Количество() > 0 Тогда
	...
	Если Не СерииВТЧТовары Тогда
		Для Каждого Стр Из РезультатЗапроса[5].Выгрузить() Цикл
			НоваяСтрока = Форма.Объект[ПараметрыУказанияСерий(Форма).ИмяТЧСерии].Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Стр);
		КонецЦикла;
	КонецЕсли;
...
//	СтрокиОбъединяемые - Массив из ДанныеФормыЭлементКоллекции - в этот массив собираются строки ТЧ,
//		подчиненные упаковочному листу, по умолчанию Неопрделено,
...
//	СтрокиЗаполнения - Массив из ДанныеФормыЭлементКоллекции - строки, в которых нужно заполнить служебные реквизиты,
//		по умолчанию Неопределено,
...
Процедура ЗаполнитьСлужебныеРеквизиты(Форма,
	...
	СтрокаНеЗаполненСостав  = НСтр("ru='<не заполнен состав упаковочного листа>';uk='<не заповнений склад пакувального листа>'");
	СтрокаКоличествоПозиций = НСтр("ru='%КоличествоПозиций% в составе';uk='%КоличествоПозиций% у складі'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.КоличествоЗаполненныхПодчиненныхПозиций > 0 Тогда
			Стр.ДополнительнаяИнформация = СтрЗаменить(СтрокаКоличествоПозиций, "%КоличествоПозиций%",
				СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
					Выборка.КоличествоЗаполненныхПодчиненныхПозиций, "позиция, позиции, позиций"));
		КонецЕсли;
...
//	Форма - УправляемаяФорма,
//	ТЧТовары - ДанныеФормыКоллекция - табличная часть, в которой отображаются товары и упаковочные листы,
//	НачинаяСНомера - номер строки (отображаемый), с которого следует начать нумерацию, по умолчанию 1.
//
...
// Рассчитывает количество мест в ТЧ
// Параметры:
...
Процедура ПриПереходеНаДругойУровень(Форма,
	...
	Если Форма.ИмяФормы <> Метаданные.Обработки.ПроверкаКоличестваТоваровВПриходномОрдере.ОсновнаяФорма.ПолноеИмя() Тогда
		Форма.Элементы[ИмяТЧТовары + "НаУровеньВверх"].Доступность = ЗначениеЗаполнено(УпаковочныйЛистРодитель);
	КонецЕсли;
...
Процедура РекурсивноСобратьМассивСтрокНавигационнойНадписи(УпаковочныеЛисты, УпаковочныйЛист, МассивСтрок, Сжать, ОтображатьГиперссылки)
	...
	ТекущийУпЛист = НайденныеСтроки[0];
	Если ОтображатьГиперссылки Тогда
		НоваяСтрока = Новый ФорматированнаяСтрока(НСтр("ru='Уп. лист';uk='Пак. лист'") + " " + ?(Сжать,"...", ТекущийУпЛист.УпаковочныйЛистКод),,,,
...
Функция ОснованиеУпаковочногоЛистаПоОрдеру(Ордер)
	...
	Основание = НСтр("ru='%Получатель / Ордер %Номер%';uk='%Получатель / Ордер %Номер%'");
	...
	Возврат СтрЗаменить(Основание,
		"%Номер%",
		ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(РеквизитыОрдера.Номер));
