...
Функция КонтрольныеСуммыФайловКомплектаВнешнейКомпоненты(Знач ИмяМакета) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты:
                  ...
                  |;uk='Не вдалося сформувати дозвіл на використання зовнішнього компонента:
                  |некоректне ім''я макета %1!'"), ИмяМакета);
	КонецЕсли;
	...
	Если Макет = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты,
                  ...
                  |;uk='Не вдалося сформувати дозвіл на використання зовнішньої компоненти,
                  |поставляється в макеті %1: макет %1 не виявлений у складі конфігурації!'"), ИмяМакета);
	КонецЕсли;
	...
	Если Метаданные.НайтиПоПолномуИмени(ИмяМакета).ТипМакета <> Метаданные.СвойстваОбъектов.ТипМакета.ДвоичныеДанные Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты:
                  ...
                  |;uk='Не вдалося сформувати дозвіл на використання зовнішнього компонента:
                  |макет %1 не містить двоїчних даних!'"), ИмяМакета);
	КонецЕсли;
	...
	Если ПустаяСтрока(ФайлМанифеста) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты,
                  ...
                  |;uk='Не вдалося сформувати дозвіл на використання зовнішнього компонента,
                  |поставляє в макеті %1: в архіві не виявлений файл MANIFEST.XML!'"), ИмяМакета);
	КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Работа в безопасном режиме.Не удалось удалить временный файл';uk='Робота в безпечному режимі.Не вдалося вилучити тимчасовий файл'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ПриЗаписиСлужебныхДанных(Объект) Экспорт
	...
	Если РаботаВБезопасномРежиме.УстановленБезопасныйРежим() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Запись объекта %1 недоступна: установлен безопасный режим: %2!';uk='Запис об''єкта %1 недоступна: установлений безпечний режим: %2!'"),
