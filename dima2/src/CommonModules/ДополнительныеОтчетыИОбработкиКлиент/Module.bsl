...
Процедура ВыполнитьКомандуВФоне(ИдентификаторКоманды, ПараметрыКоманды, Форма) Экспорт
	...
	Если НекорректныйТип ИЛИ ДополнительнаяОбработкаСсылка = ПредопределенноеЗначение("Справочник.ДополнительныеОтчетыИОбработки.ПустаяСсылка") Тогда
		...
		ТекстОшибки = НСтр("ru='Некорректное значение параметра ""ДополнительнаяОбработкаСсылка"":';uk='Некоректне значення параметра ""ДополнительнаяОбработкаСсылка"":'") + Символы.ПС;
		Если НекорректныйТип Тогда
			ТекстОшибки = ТекстОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Передан тип ""%1"", ожидался ""%2"".';uk='Передано тип ""%1"", очікувався ""%2"".'"),
				Строка(ТипЗнч(ДополнительнаяОбработкаСсылка)),
				Строка(Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки")));
		Иначе
			ТекстОшибки = ТекстОшибки + НСтр("ru='Передана пустая ссылка. Вероятно, обработка была открыта напрямую.';uk='Передано порожнє посилання. Імовірно, обробка була відкрита напряму.'");
		КонецЕсли;
...
Функция ВыполнитьНазначаемуюКомандуНаКлиенте(Форма, ИмяЭлемента) Экспорт
	...
	Если Объект.Ссылка.Пустая() Или Форма.Модифицированность Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для выполнения команды ""%1"" необходимо записать данные.';uk=' Для виконання команди ""%1"" необхідно записати дані.'"),
...
Процедура ПоказатьОповещениеПриВыполненииКоманды(ВыполняемаяКоманда)
	Если ВыполняемаяКоманда.ПоказыватьОповещение Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Команда выполняется...';uk='Команда виконується...'"), , ВыполняемаяКоманда.Представление);
	КонецЕсли;
КонецПроцедуры
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Если ФормаОбработки = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Для звіту або обробки ""%1"" не призначена основна форма,
                |або основна форма не призначена для запуску у звичайному додатку.
                |Команда ""%2"" не може бути виконана.'"),
...
Процедура ВыгрузитьВФайл(ПараметрыВыгрузки) Экспорт
	ТекстСообщения = НСтр("ru='Для выгрузки внешней обработки (отчета) в файл рекомендуется установить расширение для веб-клиента платформы.';uk='Для вивантаження зовнішньої обробки (звіту) в файл рекомендується встановити розширення для веб-клієнта платформи.'");
