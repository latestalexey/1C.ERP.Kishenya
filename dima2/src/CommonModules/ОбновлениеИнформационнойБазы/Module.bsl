...
Процедура ЗаписатьОбъект(Знач Объект, Знач РегистрироватьНаУзлахПлановОбмена = Неопределено, 
	...
	Если ДокументРежимЗаписи <> Неопределено Тогда
		Если ТипЗнч(ДокументРежимЗаписи) <> Тип("РежимЗаписиДокумента") Тогда
			ТекстИсключения = НСтр("ru='Неправильный тип параметра ДокументРежимЗаписи';uk='Неправильний тип параметра ДокументРежимЗаписи'");
...
Функция ОбъектОбработан(Данные) Экспорт
	...
	Если НезавершенныеОбработчики.Количество() > 0 Тогда
		ТекстИсключения = НСтр("ru='Действия с объектом временно запрещены, так как не завершен переход на новую версию программы.
            ...
            |;uk='Дії з об''єктом тимчасово заборонені, тому що не завершено перехід на нову версію програми.
            |Не завершені наступні процедури обробки даних:'");
...
Процедура ОтметитьВыполнениеОбработки(Данные, ДополнительныеПараметры = Неопределено, Очередь = Неопределено) Экспорт
	...
	Если (ТипЗнч(Данные) = Тип("Массив")
		...
		ТекстИсключения = НСтр("ru='В процедуру ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки передан пустой массив. Не возможно отметить выполнение обработки.';uk='В процедуру ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки передано порожній масив. Неможливо відмітити виконання обробки.'");
		ВызватьИсключение ТекстИсключения;
	...
	Иначе
		Если ТипЗнч(Данные) = Тип("ОбъектМетаданных") Тогда
			ТекстИсключения = НСтр("ru='Не поддерживается отметка выполнения обработки обновления целиком объекта метаданных. Нужно отмечать обработку конкретных данных.';uk='Не підтримується відмітка виконання обробки оновлення цілком об''єкта метаданих. Потрібно відзначати обробку даних.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
...
Функция МетаданныеИОтборПоДанным(Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли ТипДанных = Тип("ДанныеФормыСтруктура") Тогда
		...
		Иначе
			ТекстИсключения = НСтр("ru='Процедура ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным не может быть использована для этой формы.';uk='Процедура ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным не може бути використана для цієї форми.'");
		КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистр(МетаданныеОбъекта) Тогда
			...
			КонецЕсли;
		Иначе
			ТекстИсключения = НСтр("ru='Для этого типа метаданных не поддерживается анализ в функции ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным.';uk='Для цього типу метаданих не підтримується аналіз функції ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
...
Процедура ОтметитьКОбработке(ОсновныеПараметры, Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоДвижения Тогда
		...
		Если ДополнительныеПараметры.ОтметитьВсеРегистраторы Тогда
			...
			Иначе
				ТекстИсключения = НСтр("ru='Для регистрации всех регистраторов регистра необходимо в параметре ""Данные"" передать ОбъектМетаданных:Документ или ДокументСсылка.';uk='Для реєстрації всіх реєстраторів регістра необхідно в меню ""Дані"" передати ОбъектМетаданных:Документ або ДокументСсылка.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
	...
	Иначе
		...
		Иначе
			Если ТипЗнч(Данные) = Тип("ОбъектМетаданных") Тогда
				ТекстИсключения = НСтр("ru='Не поддерживается регистрация к обновлению целиком объекта метаданных. Нужно обновлять конкретные данные.';uk='Не підтримується реєстрація до оновлення цілком об''єкта метаданих. Потрібно оновлювати конкретні дані.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
...
Функция ЕстьДанныеДляОбработки(Очередь, ПолноеИмяМетаданныеОбъекта, Отбор = Неопределено) Экспорт
	...
	Иначе
		ТекстИсключения = НСтр("ru='Передан неправильный тип параметра ""ПолноеИмяМетаданныеОбъекта"" в функцию ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки';uk='Передано неправильний тип параметра ""ПолноеИмяМетаданныеОбъекта"" у функцію ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;	
	...
	Для каждого ОбрабатываемыйТип Из ПолныеИменаОбрабатываемыхОбъектов Цикл 
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистрСведений(МетаданныеОбъекта)
			...
			Если ПолныеИменаОбрабатываемыхОбъектов.Количество() > 1 Тогда
				ТекстИсключения = НСтр("ru='В массиве имен в параметре ""ПолноеИмяМетаданныеОбъекта"" в функцию ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки передан независимый регистр сведений.';uk='У масиві імен в параметрі ""ПолноеИмяМетаданныеОбъекта"" функцію ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки переданий незалежний регістр відомостей.		'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;	
		...
		Иначе
			ТекстИсключения = НСтр("ru='Для типа метаданных ""%МетаданныеОбъекта%"" не поддерживается проверка в функции ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки';uk='Для метаданих типу ""%МетаданныеОбъекта%"" не підтримується перевірка функції ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки'");
...
Функция СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок(Очередь, ПолныеИменаОбъектов, МенеджерВременныхТаблиц, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе	
		...
		Для Каждого ОбрабатываемыйТип Из ПолныеИменаОбъектовМассив Цикл
			...
			Иначе
				ТекстИсключения = НСтр("ru='Для типа метаданных ""%МетаданныеОбъекта%"" не поддерживается проверка в функции ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок';uk='Для метаданих типу ""%МетаданныеОбъекта%"" не підтримується перевірка функції ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок'");
				ТекстИсключения = СтрЗаменить(ТекстИсключения, "%МетаданныеОбъекта%", Строка(МетаданныеОбъекта)); 
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
...
// Создает временную таблицу доступных для чтения данных обработчиком указанной очереди
//	Имя таблицы: ВТДоступныДляЧтения<ИмяОбъекта>, например ВТДоступныДляЧтенияНоменклатура 
//  Колонки таблицы
//		для объектов ссылочного типа:
//			* Ссылка
//		для регистров, подчиненных регистратору
//			* Регистратор
//		для регистров с непосредтвенной записью
//			* колонки, соответствующие измерениям регистра
//
// Параметры:
//  Очередь					 - Число - очередь обработки, в которой выполняется текущий обработчик
...
//  МенеджерВременныхТаблиц	 - МенеджерВременныхТаблиц - менеджер, в котором будет создана временная таблица
//  ДополнительныеПараметры	 - Структура - см. ОбновлениеИнформационнойБазы.ДополнительныеПараметрыВыборкиДанныхДляОбработки,
//								параметр ВыбиратьПорциями игнорируется, доступные данные всегда помещаются в таблицу целиком.
// 
// Возвращаемое значение:
//  Структура - результат формирования временной таблицы:
//  * ЕстьЗаписиВоВременнойТаблице - Булево - в создаваемой таблице есть хотя бы одна запись.
...
//
//
Функция СоздатьВременнуюТаблицуДоступныхДляЧтенияДанных(Очередь, ПолноеИмяОбъекта, МенеджерВременныхТаблиц, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если Не ЗначениеЗаполнено(Очередь) Тогда
		ТекстИсключения = НСтр("ru='Не передан параметр """"Очередь""""';uk='Не переданий параметр """"Черга""""'");
		ВызватьИсключение ТекстИсключения;	
	КонецЕсли;
	...
	Если ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(МетаданныеОбъекта) Тогда
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ #ТаблицаИзменения КАК ТаблицаИзменений
		|		ПО (ТаблицаИзменений.Ссылка = ТаблицаОбъекта.Ссылка)
		|			И (ТаблицаИзменений.Узел В (&УзлыМеньшейОчереди))
		|ГДЕ
		|	ТаблицаИзменений.Ссылка ЕСТЬ NULL 
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка";
	...
	ИначеЕсли ОбщегоНазначения.ЭтоРегистрСведений(МетаданныеОбъекта)
		...
		|			И (ТаблицаИзменений.Узел В (&УзлыМеньшейОчереди))
		|ГДЕ
		|	&ТекстУсловияПоОтборуНезаблокированных";
		...
		Для Каждого Измерение из МетаданныеОбъекта.Измерения Цикл
			ТекстВыборкиИзмерений = ТекстВыборкиИзмерений + "
			|	ТаблицаОбъекта." + Измерение.Имя + " КАК " + Измерение.Имя + ",";
			
			ТекстУсловияСоединенияИзмерений = ТекстУсловияСоединенияИзмерений + "
			|	И ТаблицаИзменений." + Измерение.Имя + " = ТаблицаОбъекта." + Измерение.Имя + "
			|		ИЛИ ТаблицаИзменений." + Измерение.Имя + " = &ПустоеЗначениеИзмерения"+ Измерение.Имя + "
			|		ИЛИ ТаблицаОбъекта." + Измерение.Имя + " = &ПустоеЗначениеИзмерения"+ Измерение.Имя;
			Запрос.УстановитьПараметр("ПустоеЗначениеИзмерения"+ Измерение.Имя, Измерение.Тип.ПривестиЗначение()); 
			
			ТекстУсловияОтбораПорции = ТекстУсловияОтбораПорции + "
			|	ТаблицаОбъекта." + Измерение.Имя + ",";
			
			Если ПервоеИзмерение Тогда
				ТекстУсловияПоОтборуНезаблокированных =  "ТаблицаИзменений." + Измерение.Имя + " ЕСТЬ NULL ";
				ПервоеИзмерение = Ложь;
			КонецЕсли;
		КонецЦикла;
	...
	ИначеЕсли ОбщегоНазначения.ЭтоРегистр(МетаданныеОбъекта) Тогда
		...
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ТаблицаОбъекта.Регистратор КАК Регистратор,
		|	МАКСИМУМ(ТаблицаОбъекта.Период) КАК Период
		|ПОМЕСТИТЬ ВТДоступныДляЧтенияПолная
		|ИЗ
		|	#ПолноеИмяОбъекта КАК ТаблицаОбъекта
		|		ЛЕВОЕ СОЕДИНЕНИЕ #ТаблицаИзменения КАК ТаблицаИзменений
		|		ПО (ТаблицаИзменений.Регистратор = ТаблицаОбъекта.Регистратор)
		|			И (ТаблицаИзменений.Узел В (&УзлыМеньшейОчереди))
		|ГДЕ
		|	ТаблицаИзменений.Регистратор ЕСТЬ NULL 
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаОбъекта.Регистратор
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Регистратор
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТДоступныДляЧтенияПолная.Регистратор КАК Регистратор
		|ПОМЕСТИТЬ #ИмяВременнойТаблицы
		|ИЗ
		|	ВТДоступныДляЧтенияПолная КАК ВТДоступныДляЧтенияПолная
		|ГДЕ
		|	ВТДоступныДляЧтенияПолная.Регистратор В
		|			(ВЫБРАТЬ ПЕРВЫЕ 500
		|				ВТДоступныДляЧтенияПолная.Регистратор КАК Регистратор
		|			ИЗ
		|				ВТДоступныДляЧтенияПолная КАК ВТДоступныДляЧтенияПолная
		|			УПОРЯДОЧИТЬ ПО
		|				ВТДоступныДляЧтенияПолная.Период УБЫВ)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТДоступныДляЧтенияПолная";
	
	Иначе
		ТекстИсключения = НСтр("ru='Для этого типа метаданных не поддерживается проверка в функции ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуДоступныхДляЧтенияДанных.';uk='Для цього типу метаданих не підтримується перевірка функції ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуДоступныхДляЧтенияДанных.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Если ПустаяСтрока(ДополнительныеПараметры.ИмяВременнойТаблицы) Тогда
		ИмяВременнойТаблицы =  "ВТДоступныДляЧтения"+ИмяОбъекта;
	Иначе
		ИмяВременнойТаблицы = ДополнительныеПараметры.ИмяВременнойТаблицы;
	КонецЕсли;
	...
	Запрос.Текст = ТекстЗапроса;
	Запрос.УстановитьПараметр("УзлыМеньшейОчереди", УзлыМеньшейОчереди(Очередь));
...
Функция НоваяТаблицаОбработчиковОбновления() Экспорт
    ...
    Обработчики.Колонки.Добавить("НеЗаполнятьДанныеВОбработчикеПодредакции",  Новый ОписаниеТипов("Булево"));
    Обработчики.Колонки.Добавить("ОчередьОбработчикаПодредакции",  Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(4)));
...
Процедура ДобавитьПроверкуБлокировкиДополнительныхИсточников(Очередь, ТекстЗапроса, ПолноеИмяОбъекта, ПолноеИмяРегистра, МенеджерВременныхТаблиц, ЭтоСозданиеВременнойТаблицы, ДополнительныеПараметры)
	...
	Иначе	
		Если ПолноеИмяОбъекта = Неопределено Тогда
			ТекстИсключения = НСтр("ru='Ошибка вызова функции %ИмяФункции%: не передано имя документа, но переданы дополнительные источники данных.';uk='Помилка виклику функції %ИмяФункции%: не передано ім''я документа, але передані додаткові джерела даних.'");
