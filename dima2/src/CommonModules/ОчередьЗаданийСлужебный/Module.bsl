...
Процедура ОбработатьОчередьЗаданий(КлючФоновогоЗадания) Экспорт
	...
	Для Каждого СправочникЗаданий Из СправочникиЗаданий Цикл
		...
		Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() И ОбщегоНазначения.ЭтоРазделенныйОбъектМетаданных(СправочникЗаданий.СоздатьЭлемент().Метаданные().ПолноеИмя(), ОбщегоНазначенияПовтИсп.РазделительВспомогательныхДанных()) Тогда
			...
			|			ИЛИ Блокировки.КонецБлокировки <> ДАТАВРЕМЯ(1, 1, 1) И Блокировки.КонецБлокировки < &ТекущаяУниверсальнаяДата)"
			, СправочникЗаданий.ПустаяСсылка().Метаданные().ПолноеИмя());
	...
	|	ДатаЗавершенияПоследнегоЗапуска,
	|	Идентификатор";
		...
		Пока Выборка.Следующий() Цикл 
			Попытка
				...
				Если ЗначениеЗаполнено(Выборка.Шаблон)
					...
					ШаблонСообщения = НСтр("ru='На найден шаблон задания очереди с идентификатором %1';uk='На знайдений шаблон завдання черги з ідентифікатором %1'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Выборка.Шаблон);
					ЗаписьЖурналаРегистрации(НСтр("ru='Очередь заданий.Выполнение';uk='Черга завдань.Виконання'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ОбработатьОшибку(Знач Задание, Знач ИнформацияОбОшибкеВыполненияЗадания = Неопределено) Экспорт
	...
	Исключение
		...
		ШаблонКомментария = НСтр("ru='Ошибка при выполнении обработчика ошибок
            ...
            |По причине:
            |%3'
            |;uk='Помилка при виконанні обробника помилок
            |Псевдонім методу: %1
            |Метод обробника помилок: %2
            |З причини:
            |%3'");
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Очередь регламентных заданий.Ошибка обработчика ошибок';uk='Черга регламентних завдань.Помилка обробника помилок'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			ТекстКомментария);
...
Процедура СнятьЗаданияОбработкиОшибок(Знач ПараметрыВызова, Знач ИнформацияОбОшибке = Неопределено, Знач СчетчикРекурсии = 1) Экспорт
	...
	Попытка
		...
		Иначе
            ...
            |Псевдоним метода: %1
            |Уровень рекурсии: %2'
            |;uk='Був виконаний обробник зняття завдань.
            |Псевдонім методу: %1
            |Рівень рекурсії: %2'");
			ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКомментария,
				ПараметрыОбработчикаОшибок.ИмяМетодаЗадания,
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Очередь регламентных заданий.Снятие заданий обработки ошибок';uk='Черга регламентних завдань.Зняття завдань обробки помилок'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ВыполнитьМетодКонфигурации(ИмяМетода, Параметры = Неопределено)
	...
	Если ТранзакцияАктивна() Тогда
		...
		ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Перед стартом выполнения обработчика %1 есть активные транзакции!';uk='Перед стартом виконання обробника %1 є активні транзакції!'"),
				ИмяМетода);
			
		ЗаписьЖурналаРегистрации(НСтр("ru='Очередь регламентных заданий.Выполнение';uk='Черга регламентних завдань.Виконання'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
	...
	Попытка
		...
		Если ТранзакцияАктивна() Тогда
			...
			ШаблонСообщения = НСтр("ru='По завершении выполнения обработчика %1 не была закрыта транзакция';uk='По завершенні виконання обробника %1 не була закрита транзакція'");
		...
		Если Не(РазделительУстановлен) И ОбщегоНазначения.ИспользованиеРазделителяСеанса() Тогда
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='По завершении выполнения обработчика %1 не было выключено разделение сеанса!';uk='По завершенні виконання обробника %1 не було вимкнено поділ сеансу!'"),
				ИмяМетода);
		...
		ИначеЕсли РазделительУстановлен И ЗначениеРазделителя <> РаботаВМоделиСервиса.ЗначениеРазделителяСеанса() Тогда
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='По завершении выполнения обработчика %1 было изменено значение разделителя сеанса!';uk='По завершенні виконання обробника %1 було змінено значення роздільника сеансу!'"),
				ИмяМетода);
...
Функция ПолучитьПараметрыОбработчикаОшибок(Знач Задание,Знач ИнформацияОбОшибкеВыполненияЗадания = Неопределено)
	...
	Если Результат.ОбработчикСуществует Тогда
		...
		Если ИнформацияОбОшибкеВыполненияЗадания = Неопределено Тогда
			...
			ИнформацияОбОшибкеВыполненияЗадания = КонструкторИнформацииОбОшибке(НСтр("ru='Задание завершилось с неизвестной ошибкой, возможно вызванной падением рабочего процесса.';uk='Завдання завершилося з невідомої помилкою, можливо викликаної падінням робочого процесу.'"));
...
Процедура ЗаписьКонтроляВыполнения(Знач ИмяСобытия, Знач ЗаписьЗадания, Знач Комментарий = "")
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ОписаниеМетода.Шаблон) Тогда
			...
			ИмяМетода = НСтр("ru='Шаблон:';uk='Шаблон:'") + ОписаниеМетода.Шаблон;
...
Процедура ВыполнитьОбработчикЗадания(Шаблон, Псевдоним, Параметры)
	...
	Если ИмяМетода = Неопределено Тогда
		ШаблонСообщения = НСтр("ru='Метод %1 не разрешен к вызову через очередь заданий.';uk='Метод %1 не дозволений до виклику через чергу завдань.'");
	...
	Попытка
		...
		Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() И ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(Задание.Метаданные().ПолноеИмя(), ОбщегоНазначенияПовтИсп.РазделительВспомогательныхДанных()) Тогда
			...
			Если ЗначениеЗаполнено(Задание.Шаблон)
				...
				ШаблонСообщения = НСтр("ru='На найден шаблон задания очереди  %1';uk='На знайдений шаблон завдання черги %1'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Задание.Шаблон);
				
				ЗаписьЖурналаРегистрации(НСтр("ru='Очередь заданий.Планирование';uk='Черга завдань.Планування'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
					ТекстСообщения);
		...
		ИначеЕсли Задание.СостояниеЗадания = Перечисления.СостоянияЗаданий.Выполняется Тогда
			...
			ЗаписьКонтроляВыполнения(НСтр("ru='Очередь регламентных заданий.Завершено с ошибками';uk='Черга регламентних завдань.Завершено з помилками'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Выборка.Идентификатор, 
				НСтр("ru='Исполняющее задание было принудительно завершено';uk='Завдання, що виконується, було примусово завершене'"));
			...
			Иначе // Нет расписания
				...
				Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() И ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(
					...
					Если ЗначениеЗаполнено(Задание.Шаблон) Тогда // Задание по шаблону без расписания.
						...
						ШаблонСообщения = НСтр("ru='Для шаблон заданий очереди  %1 не найдено расписание';uk='Для шаблон завдань черги %1 не знайдено розклад'");
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Задание.Шаблон);
						ЗаписьЖурналаРегистрации(НСтр("ru='Очередь заданий.Планирование';uk='Черга завдань.Планування'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
							УровеньЖурналаРегистрации.Ошибка,
							,
...
Процедура ВыполнитьЗаданиеОчереди(Знач Ссылка, Знач ИсполняющееФоновоеЗадание, 
	...
	Попытка
		ЗаписьКонтроляВыполнения(НСтр("ru='Очередь регламентных заданий.Старт';uk='Черга регламентних завдань.Старт'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Ссылка);
		...
		ЗаписьКонтроляВыполнения(НСтр("ru='Очередь регламентных заданий.Завершено успешно';uk='Черга регламентних завдань.Завершено успішно'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Ссылка);
		
	Исключение
		
		ИнформацияОбОшибкеВыполненияЗадания = ИнформацияОбОшибке();
		
		ЗаписьКонтроляВыполнения(НСтр("ru='Очередь регламентных заданий.Завершено с ошибками';uk='Черга регламентних завдань.Завершено з помилками'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Ссылка, 
