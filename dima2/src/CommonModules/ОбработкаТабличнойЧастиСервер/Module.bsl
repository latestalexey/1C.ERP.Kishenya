...
Процедура ЗаполнитьСодержаниеУслуги(ТекущаяСтрока, СтруктураДействий) 
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСодержание", СтруктураПараметровДействия) ТОгда
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) И ЗаполнятьДляВсехУслуг <> Неопределено Тогда
			...
			//++ НЕ УТ
			ИначеЕсли СтруктураПараметровДействия.Свойство("УслугиПоПереработкеДавальческогоСырья", УслугиПоПереработкеДавальческогоСырья) И
						УслугиПоПереработкеДавальческогоСырья = Истина Тогда
...
Процедура ЗаполнитьЦенуПередачиПродукцииДавальцуВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если Не СтруктураДействий.Свойство("ЗаполнитьЦенуПередачиПродукцииДавальцу", СтруктураПараметровДействия) Тогда
		Возврат;
	КонецЕсли;
	
	ВидЦеныПередачи = Справочники.ВидыЦен.ВидЦеныПоУмолчанию(, Новый Структура("ИспользоватьПриПередачеПродукцииДавальцу", Истина));
	...
	Если ЗначениеЗаполнено(ВидЦеныПередачи) Тогда
		...
		ЗаполнитьЗначенияСвойств(СтруктураПараметровОтбора, ТекущаяСтрока);
		ЗаполнитьЗначенияСвойств(СтруктураПараметровОтбора, СтруктураПараметровДействия);
		СтруктураПараметровОтбора.ВидЦены = ВидЦеныПередачи;
		...
		ТекущаяСтрока.Цена = ПродажиСервер.ПолучитьЦенуПоОтбору(СтруктураПараметровОтбора);
		ТекущаяСтрока.ВидЦены = ВидЦеныПередачи;
