...
Процедура ФормаЗадачиПриСозданииНаСервере(ФормаЗадачи, ЗадачаОбъект, 
	...
	Если ЗадачаОбъект.Выполнена Тогда
		...
		Элемент.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Задача выполнена %1 пользователем %2.';uk='Задача виконана %1 користувачем %2.'"),
	...
	Если БизнесПроцессыИЗадачиВызовСервера.ЭтоВедущаяЗадача(ЗадачаОбъект.Ссылка) Тогда
		...
		Если Элемент = Неопределено Тогда
			...
			Элемент.Вид = ВидДекорацииФормы.Надпись;
			Элемент.Заголовок = НСтр("ru='Это ведущая задача для вложенных бизнес-процессов. Она будет выполнена автоматически при их завершении.';uk='Це основна задача для вкладених бізнес-процесів. Вона буде виконана автоматично при їх завершенні.'");
			Элемент.Высота = 0; // автовысота
		КонецЕсли;
	КонецЕсли;	
...
Процедура УстановитьОформлениеЗадач(Знач СписокЗадачИлиЕгоУсловноеОформление) Экспорт
	...
	ЭлементЦветаОформления.Значение = НСтр("ru='Срок не указан';uk='Строк не зазначений'");
	ЭлементЦветаОформления.Использование = Истина;
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
			...
			ЭлементЦветаОформления = ЭлементУсловногоОформления.Оформление.Элементы.Найти("Текст");
			ЭлементЦветаОформления.Значение = НСтр("ru='Представитель организации';uk='Представник організації'");
			ЭлементЦветаОформления.Использование = Истина;
	КонецЕсли;
...
Процедура УстановитьОформлениеБизнесПроцессов(Знач УсловноеОформлениеБизнесПроцессов) Экспорт
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Без описания';uk='Без опису'"));
	...
	Если ПредыдущееСостояние <> БизнесПроцессОбъект.Состояние Тогда
		...
		Если Не ЕстьПраваНаОстановкуБизнесПроцесса(БизнесПроцессОбъект) Тогда 
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Недостаточно прав для остановки бизнес-процесса ""%1"".';uk='Недостатньо прав для зупинки бізнес-процесу ""%1"".'"),
		...
		Если ПредыдущееСостояние = Перечисления.СостоянияБизнесПроцессов.Активен Тогда
			...
			Если БизнесПроцессОбъект.Завершен Тогда
				ВызватьИсключение НСтр("ru='Невозможно остановить завершенные бизнес-процессы.';uk='Неможливо зупинити завершені бізнес-процеси.'");
			КонецЕсли;
				
			Если Не БизнесПроцессОбъект.Стартован Тогда
				ВызватьИсключение НСтр("ru='Невозможно остановить не стартовавшие бизнес-процессы.';uk='Неможливо зупинити бізнес-процеси, які ще не стартували.'");
			КонецЕсли;
			
		ИначеЕсли ПредыдущееСостояние = Перечисления.СостоянияБизнесПроцессов.Остановлен Тогда
			
			Если БизнесПроцессОбъект.Завершен Тогда
				ВызватьИсключение НСтр("ru='Невозможно сделать активными завершенные бизнес-процессы.';uk='Неможливо зробити активними завершені бізнес-процеси.'");
			КонецЕсли;
				
			Если Не БизнесПроцессОбъект.Стартован Тогда
				ВызватьИсключение НСтр("ru='Невозможно сделать активными не стартовавшие бизнес-процессы.';uk='Неможливо зробити активними бізнес-процеси, які ще не стартували.'");
			КонецЕсли;
...
Процедура УведомитьИсполнителейОНовыхЗадачах() Экспорт
	...
	ВидСообщения = НСтр("ru='Бизнес-процессы и задачи.Уведомление о новых задачах';uk='Бізнес-процеси та задачі.Повідомлення про нові задачі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ЗаписьЖурналаРегистрации(ВидСообщения,
		УровеньЖурналаРегистрации.Информация, , ,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Начато регламентное уведомление о новых задачах за период %1 - %2';uk='Розпочато регламентне повідомлення про нові задачі за період %1 - %2'"),
	...
	ЗаписьЖурналаРегистрации(ВидСообщения,
		УровеньЖурналаРегистрации.Информация, , ,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Завершено регламентное уведомление о новых задачах (уведомлено исполнителей: %1)';uk='Завершено регламентне повідомлення про нові задачі (повідомлено виконавців: %1)'"),
...
Процедура ЗавершитьОбновлениеПорцииНаборовЗначенийДоступа(Параметры) Экспорт
	...
	Если Параметры.ОбъектовОбработано = 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Процедуре ""%1"" не удалось обновить права доступа для некоторых объектов (пропущены): %1';uk='Процедурою ""%1"" не вдалося оновити всі права доступу для деяких об''єктів (пропущені): %1'"), 
	...
	ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Процедура ""%1"" обновила права доступа для очередной порции объектов: %2';uk='Процедура ""%1"" відновила права доступу для чергової порції об''єктів: %2'"), 
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ru='Мои задачи';uk='Мої задачі'");
		...
		Дело.Представление  = НСтр("ru='просроченные';uk='прострочені'");
		...
		Дело.Представление  = НСтр("ru='сегодня';uk='сьогодні'");
		...
		Дело.Представление  = НСтр("ru='на этой неделе';uk='на цьому тижні'");
		...
		Дело.Представление  = НСтр("ru='на следующей неделе';uk='на наступному тижні'");
...
Процедура ОтправитьУведомлениеОПросроченныхЗадачах(ПисьмоИзНабора)
	...
	Если ПустаяСтрока(ПисьмоИзНабора.ПочтовыйАдрес) Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Уведомление не было отправлено, так как у пользователя %1 не задан адрес электронной почты.';uk='Повідомлення не було відправлено, тому що в користувача %1 не задана адреса електронної пошти.'"), 
			ПисьмоИзНабора.Получатель);
		ЗаписьЖурналаРегистрации(НСтр("ru='Бизнес-процессы и задачи.Уведомление о просроченных задачах';uk='Бізнес-процеси та задачі.Повідомлення про прострочені задачі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если ПисьмоИзНабора.ТипПисьма = "Исполнителю" Тогда
		ТекстТелаПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не выполненные в срок задачи:
            | 
            |%1'
            |;uk='Не виконані в строк задачі:
            |
            |%1'"), ПисьмоИзНабора.ТекстПисьма);
		ПараметрыПисьма.Вставить("Тело", ТекстТелаПисьма);
		
		ТекстТемыПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не выполненные в срок задачи (%1)';uk='Не виконані в строк задачі (%1)'"),
	...
	ИначеЕсли ПисьмоИзНабора.ТипПисьма = "Автору" Тогда
		ТекстТелаПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='По введенным задачам прошел крайний срок:
            | 
            |%1'
            |;uk='По введених задачах минув крайній строк:
            |
            |%1'"), ПисьмоИзНабора.ТекстПисьма);
		ПараметрыПисьма.Вставить("Тело", ТекстТелаПисьма);
		
		ТекстТемыПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='По задачам истек контрольный срок (%1)';uk='За задачами закінчився контрольний строк (%1)'"),
			Строка(ПисьмоИзНабора.КоличествоЗадач));
		ПараметрыПисьма.Вставить("Тема", ТекстТемыПисьма);
	ИначеЕсли ПисьмоИзНабора.ТипПисьма = "Координатору" Тогда
		ТекстТелаПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Прошел крайний срок по задачам:
            | 
            |%1'
            |;uk='Минув крайній строк по задачах:
            |
            |%1'"), ПисьмоИзНабора.ТекстПисьма);
		ПараметрыПисьма.Вставить("Тело", ТекстТелаПисьма);
		
		ТекстТемыПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Истек контрольный срок задач (%1)';uk='Минув контрольний строк задач (%1)'"),
			Строка(ПисьмоИзНабора.КоличествоЗадач));
		ПараметрыПисьма.Вставить("Тема", ТекстТемыПисьма);
	КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при отправке уведомления о просроченных задачах: %1.';uk='Помилка при відправленні повідомлення про прострочені задачі: %1.'"),
			ОписаниеОшибки);
	...
	Если ПустаяСтрока(ТекстСообщения) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Уведомление о просроченных задачах успешно отправлено на адрес %1.';uk='Повідомлення про прострочені задачі успішно відправлено на адресу %1.'"), Кому);
...
Функция СоздатьЗадачуПоНастройкеРолей(ЗадачаСсылка, Ответственные)
	...
	Для Каждого Ответственный Из Ответственные Цикл
		...
		ЗадачаОбъект.Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Задача не может быть исполнена, так как у роли не задано ни одного исполнителя:
            |%1'
            |;uk='Задача не може бути виконана, тому що в ролі не задано жодного виконавця:
            |%1'"), Строка(ЗадачаСсылка));
		ЗадачаОбъект.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Назначить исполнителей: задача не может быть исполнена %1';uk='Призначити виконавців: задача не може бути виконана %1'"), Строка(ЗадачаСсылка));
...
Функция ОтправитьУведомлениеОНовыхЗадачах(Исполнитель, ЗадачиПоИсполнителю)
	...
	Если ПустаяСтрока(ПочтовыйАдресПолучателя) Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Бизнес-процессы и задачи.Уведомление о новых задачах';uk='Бізнес-процеси та задачі.Повідомлення про нові задачі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Информация,,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Уведомление не отправлено, так как не указан почтовый адрес у пользователя %1.';uk='Повідомлення не відправлене, тому що не зазначений поштова адреса в користувача %1.'"), Строка(Исполнитель)));
		Возврат Ложь;
	КонецЕсли;
	...
	КонецЦикла;
	ТемаПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Направлены задачи - %1';uk='Спрямовані задачі - %1'"), Метаданные.КраткаяИнформация);
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Бизнес-процессы и задачи.Уведомление о новых задачах';uk='Бізнес-процеси та задачі.Повідомлення про нові задачі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при отправке уведомления о новых задачах: %1';uk='Помилка при відправці повідомлення про нові задачі: %1'"), 
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Бизнес-процессы и задачи.Уведомление о новых задачах';uk='Бізнес-процеси та задачі.Повідомлення про нові задачі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Уведомления успешно отправлены на адрес %1.';uk='Повідомлення успішно відправлені на адресу %1.'"), ПочтовыйАдресПолучателя));
	Возврат Истина;	
...
Функция СформироватьПредставлениеЗадачи(ЗадачаСтруктура)
	...
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Исполнитель) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Исполнитель: %3';uk='Виконавець: %3'"), ЗадачаСтруктура.Исполнитель) + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.РольИсполнителя) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль: %4';uk='Роль: %4'"), ЗадачаСтруктура.РольИсполнителя) + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.ОсновнойОбъектАдресации) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Основной объект адресации: %5';uk='Основний об''єкт адресації: %5'"), ЗадачаСтруктура.ОсновнойОбъектАдресации) + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.ДополнительныйОбъектАдресации) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Доп. объект адресации: %6';uk='Дод. об''єкт адресації: %6'"), ЗадачаСтруктура.ДополнительныйОбъектАдресации) + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Автор) Тогда
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Автор: %7';uk='Автор: %7'"), ЗадачаСтруктура.Автор) + Символы.ПС;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЗадачаСтруктура.Описание) Тогда
		Результат = Результат + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%8';uk='%8'"), ЗадачаСтруктура.Описание) + Символы.ПС;
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	Возврат НСтр("ru='Бизнес-процессы и задачи';uk='Бізнес-процеси та задачі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
...
Процедура УстановитьПараметрыСпискаМоихЗадач(Список) Экспорт
	...
	Список.Параметры.УстановитьЗначениеПараметра("Просрочено", " " + НСтр("ru='Просрочено';uk='Прострочено'")); // пробел для сортировки
	Список.Параметры.УстановитьЗначениеПараметра("Сегодня", НСтр("ru='В течение сегодня';uk='Протягом сьогодні'"));
	Список.Параметры.УстановитьЗначениеПараметра("ЭтаНеделя", НСтр("ru='До конца недели';uk='До кінця тижня'"));
	Список.Параметры.УстановитьЗначениеПараметра("СледующаяНеделя", НСтр("ru='На следующей неделе';uk='На наступному тижні'"));
	Список.Параметры.УстановитьЗначениеПараметра("Позднее", НСтр("ru='Позднее';uk='Пізніше'"));
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		...
		Обработчик.Комментарий = НСтр("ru='Обновление прав доступа для бизнес-процесса ""Задание"". 
            ...
            |;uk='Оновлення прав доступу для бізнес-процесу ""Завдання"". 
            |До завершення обробки діятимуть колишні права доступу до завдань.'");
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось обновить права доступа для ""%1"" в обработчике ""%2"" по причине:
                    |%3'
                    |;uk='Не вдалося оновити всі права доступу для ""%1"" в обробнику ""%2"" з причини:
                    |%3'"), 
...
Процедура ПроверитьСистемнаяУчетнаяЗаписьНастроена()
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
		ВызватьИсключение НСтр("ru='Отправка почты не предусмотрена в программе.';uk='Відправлення пошти не передбачене в програмі.'");
	КонецЕсли;
	...
	Если Не Результат Тогда
		ВызватьИсключение НСтр("ru='Системная учетная запись электронной почты не настроена для отправки.';uk='Системний обліковий запис електронної пошти не налаштован для відправки.'");
	КонецЕсли;
