...
Процедура ВидыЗапасовПоНалогообложению(Организация, НалоговоеНазначение, КонтролироватьНалогообложение, Дата, МассивСделок, Менеджер, Подразделение, МенеджерВременныхТаблиц) Экспорт	
	...
	|		(ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.ЕдиныйНалог), ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.НалогНаПрибыль), ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Неплательщик))
	|;
	|///////////////////////////////////////////////////////////////////////////////
	|
	|// Собственные виды запасов
...
Процедура ТаблицаОстатковТоваровОрганизаций(ДокументСсылка, Организация, Дата, ДополнительныеСвойства, МенеджерВременныхТаблиц, ПоВсемВидамЗапасов = Ложь) Экспорт
	...
	|	ТоварыОрганизаций.ВидЗапасовВладельца.НалоговоеНазначение КАК НалоговоеНазначение,
...
//
Функция ТаблицаОстатковТоваровПереданныхПереработчику(ДокументСсылка, Материалы, Назначение, Дата, ДополнительныеСвойства) Экспорт
	...
	|		)
	|	И (Товары.ВидЗапасов.Назначение = &Назначение
	|		ИЛИ Товары.ВидЗапасов.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка))
	|
	|СГРУППИРОВАТЬ ПО
	|	Товары.АналитикаУчетаНоменклатуры,
	|	Товары.ВидЗапасов
...
//
Процедура ТаблицаОстатковДляВозвратаПереданныхПереработчикуТоваров(ДокументСсылка, Организация, Партнер, Дата, ДополнительныеСвойства, МенеджерВременныхТаблиц) Экспорт
	...
	|ПОМЕСТИТЬ НаКонецМесяца
	|ИЗ
	|	РегистрНакопления.ТоварыПереданныеПереработчику.Остатки(&Дата,
	|		АналитикаУчетаНоменклатуры В (
	|			ВЫБРАТЬ
	|				Аналитика.КлючАналитики
	|			ИЗ
	...
	|	ТабТовары.ВидЗапасов.НалоговоеНазначение КАК НалоговоеНазначение,	
	|	ЛОЖЬ КАК РеализацияЗапасовДругойОрганизации,
	|	ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка) КАК Назначение,
...
Процедура ТаблицаОстатковКомиссионныхТоваровИнтеркампани(Организация, НачалоПериода, КонецПериода, ВременныеТаблицы, Ссылка, ЭтоОтчетОСписании = Ложь) Экспорт
	...
	|	КОформлению.СуммаВыручки КАК СуммаВыручки
	|ПОМЕСТИТЬ Остатки
	|ИЗ
	|	ТоварыКОформлениюЗаПериод КАК КОформлению
	|ИНДЕКСИРОВАТЬ ПО
	|	АналитикаУчетаНоменклатуры
	|;
	|/////////////////////////////////////////////////////////////////////////////
...
//
Функция ТаблицаОстатковКЗаказу(ДокументСсылка, ПолучатьТовары = Истина, ПолучатьУслуги = Истина, ПолучатьРаботы = Истина) Экспорт
