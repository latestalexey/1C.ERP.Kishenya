...
// Формирует сообщений клиентов по очереди оповещений.
//
Процедура ФормированиеСообщенийПоОповещениямКлиентов() Экспорт
	...
	ТекстЗаписи = НСтр("ru='Окончание формирования сообщений по рассылкам клиентов';uk='Закінчення формування повідомлень щодо розсилок клієнтів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ВыполнитьЗаписьЖурналаРегистрации(ТекстЗаписи ,УровеньЖурналаРегистрации.Информация);
	
КонецПроцедуры

// Формирует сообщений по рассылкам клиентам.
//
Процедура ФормированиеСообщенийПоРассылкамКлиентам() Экспорт
	...
	ТекстЗаписи = НСтр("ru='Начало формирования сообщений по рассылкам клиентов';uk='Початок формування повідомлень по розсилках клієнтів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ВыполнитьЗаписьЖурналаРегистрации(ТекстЗаписи ,УровеньЖурналаРегистрации.Информация);
	...
	ТекстЗаписи = НСтр("ru='Окончание формирования сообщений по рассылкам клиентов. Обработано рассылок %1.';uk='Закінчення формування повідомлень щодо розсилок клієнтів. Опрацьовано розсилок %1.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ВыполнитьЗаписьЖурналаРегистрации(ТекстЗаписи ,УровеньВажности)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Рассылки и оповещения клиентам';uk='Розсилки і оповіщення клієнтам'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		УровеньВажности, , ,
		ТекстЗаписи);
...
Функция ТекстПоясненияКонтактноеЛицоОбъектаОповещения() Экспорт
	...
	Возврат НСтр("ru='Когда это возможно, оповещение будет отправлено контактному лицу, указанному в объекте, являющимся
                       ...
                       |;uk='Коли це можливо, оповіщення буде відправлено контактній особі, вказаній у об''єкті, що є
                       |джерелом оповіщення. Наприклад, оповіщення по зміні стану ""Замовлення клієнта"" буде 
...
Функция ТекстКомуБудутОтправленыСообщения(Данные, Принудительная = Ложь) Экспорт
	...
	Если Данные.ОтправлятьКонтактномуЛицуОбъектаОповещения Тогда
		...
		ТекстАдресаты = ТекстАдресаты + НСтр("ru='контактное лицо объекта оповещения';uk='контактна особа об''єкта оповіщення'");
	КонецЕсли;
...
Функция СообщениеУдовлетворяетУсловиюОтправки(ДанныеВидаОповещения, СтрокаДанныхШаблона, ДанныеПолучателей)
	...
	Попытка
		...
		Если Вычислить(УсловиеОтправки) = Ложь Тогда
			Возврат Ложь;
		Иначе
			Возврат Истина;
		КонецЕсли;
...
Функция КомментарийДляСообщенияПоОповещению(ДанныеВидаОповещения)
	...
	ТекстКомментария =  НСтр("ru='Создано по виду оповещения ""%1""';uk='Створено за видом оповіщення ""%1""'");
