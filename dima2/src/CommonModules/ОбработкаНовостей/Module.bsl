...
Процедура УстановитьСостояниеНовостейДляПользователей(
	...
	ТекстСообщения = НСтр("ru='УстановитьСостояниеНовостейДляПользователей, начало работы.
        ...
        |'
        |;uk='УстановитьСостояниеНовостейДляПользователей, початок роботи.
        |МассивПользователей:
        |	%МассивПользователей%
        |СтруктураОтборов:
        |	%СтруктураОтборов%
        |ЗначенияСостояний:
        |	%ЗначенияСостояний%
        |Функціональна опція Дозволена робота з новинами: %ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями()%
        |'");
	...
	Если (ТипЗнч(МассивПользователей) <> ТипМассив)
		...
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%МассивПользователей%", "По всем");
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%МассивПользователей%", ОбработкаНовостейКлиентСервер.ПолучитьСтроковоеПредставление(МассивПользователей, ", "));
	КонецЕсли;
	Если (ТипЗнч(СтруктураОтборов) = ТипСтруктура) Тогда
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СтруктураОтборов%", ОбработкаНовостейКлиентСервер.ПолучитьСтроковоеПредставление(СтруктураОтборов, " = ", Символы.ПС));
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СтруктураОтборов%", "Передано значения типа, отличного от Структуры");
	КонецЕсли;
	Если (ТипЗнч(ЗначенияСостояний) = ТипСтруктура) Тогда
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЗначенияСостояний%", ОбработкаНовостейКлиентСервер.ПолучитьСтроковоеПредставление(ЗначенияСостояний, " = ", Символы.ПС));
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЗначенияСостояний%", "Передано значения типа, отличного от Структуры");
	КонецЕсли;
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
		НСтр("ru='Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';uk='Новини. Сервіс і регламент. УстановитьСостояниеНовостейДляПользователей'"), // ИдентификаторШага
	...
	Если (ТипЗнч(ЗначенияСостояний) = ТипСтруктура)
		...
		Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			...
			Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
				ТекстСообщения = НСтр("ru='УстановитьСостояниеНовостейДляПользователей, ошибка выполнения: при работе в модели сервиса запускать получение новостей вручную можно только при отключенных разделителях.
                    ...
                    |'
                    |;uk='УстановитьСостояниеНовостейДляПользователей, помилка виконання: при роботі в моделі сервісу запускати отримання новин вручну можна тільки при відключених роздільниках.
                    |Рядок підключення: %СтрокаСоединенияИнформационнойБазы()%
                    |ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%
                    |Це розділена конфігурація: %ЭтоРазделеннаяКонфигурация()%
                    |Поділ включено: %РазделениеВключено()%
                    |Доступно використання розділених даних: %ДоступноИспользованиеРазделенныхДанных()%
                    |Сеанс запущений без роздільників: %СеансЗапущенБезРазделителей()%
                    |Функціональна опція Дозволена робота з новинами: %ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями()%
                    |Функціональна опція Дозволена робота з новинами через інтернет: %ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостямиЧерезИнтернет()%
                    |Поточний користувач ІБ:
                    |	Ім''я: %ТекущийПользователь().Имя%
                    |	УІН: %ТекущийПользователь().УникальныйИдентификатор%
                    |	Поділ даних:
                    |		%ТекущийПользователь().РазделениеДанных%
                    |Користувач: %ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%
                    |'");
				...
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТекущийПользователь().РазделениеДанных%", ОбработкаНовостейКлиентСервер.ПолучитьСтроковоеПредставление(ПользователиИнформационнойБазы.ТекущийПользователь().РазделениеДанных, "=", "; "));
		...
		Для Каждого ТекущийПользователь Из МассивПользователей Цикл
			Если Пользователи.РолиДоступны("ЧтениеНовостей,ПолныеПрава", ТекущийПользователь, Ложь) = Истина Тогда
		...
		Если МассивПользователей.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ru='Ошибка выполнения: нет пользователей, для которых бы следовало установить состояния новостей.
                ...
                |;uk='Помилка виконання: немає користувачів, для яких слід було б встановити стану новин.
                |Стану новин можна встановити тільки для користувачів з роллю ЧтениеНовостей або ПолныеПрава'");
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
				НСтр("ru='Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';uk='Новини. Сервіс і регламент. УстановитьСостояниеНовостейДляПользователей'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
		...
		Если (ТипЗнч(СтруктураОтборов) = ТипСтруктура)
				И (СтруктураОтборов.Свойство("СписокНовостей") = Истина)
		...
		Иначе
			...
			Если (СтруктураОтборов.Свойство("СписокЛентНовостей") = Истина)
			...
			Если (СтруктураОтборов.Свойство("ИнтервалВерсийПродукта") = Истина)
					И (ТипЗнч(СтруктураОтборов.ИнтервалВерсийПродукта) = ТипСтруктура) Тогда
				ПоВсемВерсиямПродукта = Ложь;
				Если СтруктураОтборов.ИнтервалВерсийПродукта.Свойство("ВерсияОТ") = Истина Тогда
					ПродуктВерсияОТ = СтруктураОтборов.ИнтервалВерсийПродукта.ВерсияОТ;
				КонецЕсли;
				Если СтруктураОтборов.ИнтервалВерсийПродукта.Свойство("ВерсияДО") = Истина Тогда
			...
			Если (СтруктураОтборов.Свойство("ИнтервалВерсийПлатформы") = Истина)
					И (ТипЗнч(СтруктураОтборов.ИнтервалВерсийПлатформы) = ТипСтруктура) Тогда
				ПоВсемВерсиямПлатформы = Ложь;
				Если СтруктураОтборов.ИнтервалВерсийПлатформы.Свойство("ВерсияОТ") = Истина Тогда
					ПлатформаВерсияОТ = СтруктураОтборов.ИнтервалВерсийПлатформы.ВерсияОТ;
				КонецЕсли;
				Если СтруктураОтборов.ИнтервалВерсийПлатформы.Свойство("ВерсияДО") = Истина Тогда
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru='УстановитьСостояниеНовостейДляПользователей, завершение.
                ...
                |'
                |;uk='УстановитьСостояниеНовостейДляПользователей, завершення.
                |Опрацьовано користувачів: %1
                |Опрацьовано новин: %2
                |'"),
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='УстановитьСостояниеНовостейДляПользователей, ошибка выполнения: в качестве значений заполнения передано значение типа, отличного от Структура, либо не заполнены настройки заполнения (не указано ни одного ключа структуры).
            ...
            |;uk='УстановитьСостояниеНовостейДляПользователей, помилка виконання: в якості значень заповнення передано значення типу, відмінного від Структура, або не заповнені настройки заповнення (не зазначено жодного ключа структури).
            |Можливі імена ключів збігаються з найменуваннями ресурсів регістра відомостей СостоянияНовостей'");
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';uk='Новини. Сервіс і регламент. УстановитьСостояниеНовостейДляПользователей'") , // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
...
Функция ПолучитьАдресWSDLОпределенияВебСервисаInformation() Экспорт
	...
	Если ТипЗнч(лкПараметрыОкруженияБИП_Новости) = ТипСтруктура Тогда
		Если лкПараметрыОкруженияБИП_Новости.Свойство("АдресWSDLОпределенияВебСервисаInformation") Тогда
			Если НЕ ПустаяСтрока(лкПараметрыОкруженияБИП_Новости.АдресWSDLОпределенияВебСервисаInformation) Тогда
				Результат = лкПараметрыОкруженияБИП_Новости.АдресWSDLОпределенияВебСервисаInformation;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция СоединитьсяССерверомНовостей(ИмяСервера, Протокол, Логин, Пароль, ХттпСоединение) Экспорт
	...
	Пока (СоединениеУстановлено <> Истина) И (ПопыткаСоединения < НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения) Цикл
			...
			Исключение
				...
				ТекстСообщения = СтрШаблон(
					НСтр("ru='Попытка %1 / %2.
                        ...
                        |;uk='Спроба %1 / %2.
                        |Помилка визначення порту з імені сервера %3 (%4).'"),
					ПопыткаСоединения,
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
					НСтр("ru='Новости. Загрузка новостей. Соединение с сервером новостей. Ошибка определения параметров';uk='Новини. Завантаження новин. З''єднання із сервером новин. Помилка визначення параметрів'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
		...
		Попытка
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
				НСтр("ru='Новости. Загрузка новостей. Соединение с сервером новостей';uk='Новини. Завантаження новин. З''єднання з сервером новин'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
		...
		Исключение
			ТекстСообщения = СтрШаблон(
				НСтр("ru='Попытка %1 / %2:
                    ...
                    |%5'
                    |;uk='Спроба %1 / %2:
                    |Помилка підключення до сервера %3 (%4) для отримання новин з причини:
                    |%5'"),
				ПопыткаСоединения,
				НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения,
				ИмяСервера,
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка новостей';uk='БІП:Новини.Завантаження новин'"), // ИмяСобытия
				НСтр("ru='Новости. Загрузка новостей. Соединение с сервером новостей. Ошибка соединения';uk='Новини. Завантаження новин. З''єднання із сервером новин. Помилка з''єднання'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		"news.webits.1c.ru",
		443,
		НСтр("ru='Веб-сервисы получения новостей';uk='Веб-сервіси отримання новин'"));
...
Процедура ПолучитьНовостиСНапоминаниями(ОченьВажныеНовости, ВажныеНовости) Экспорт
	...
	Запрос.Текст = "
		...
		|	ЕСТЬNULL(РегСостояния.ДатаНачалаОповещения, ДАТАВРЕМЯ(1,1,1)) КАК ДатаНачалаОповещения,
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка.Важность = 1 Тогда // Очень важная
				ОченьВажныеНовости.Добавить(
					Новый Структура("Новость,НовостьУникальныйИдентификатор,НовостьНаименование,НовостьПодзаголовок,ОповещениеВключено",
						Выборка.Новость,
			...
			Иначе // Если Выборка.Важность = 2, важная
				ВажныеНовости.Добавить(
					Новый Структура("Новость,НовостьУникальныйИдентификатор,НавигационнаяСсылка,НовостьНаименование,НовостьПодзаголовок,ОповещениеВключено",
						Выборка.Новость,
						Выборка.Новость.УникальныйИдентификатор(),
...
Функция НайтиНовости(СтруктураПараметровПоиска) Экспорт
	...
	Если ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() = РежимПолнотекстовогоПоиска.Разрешить Тогда
		Если НЕ ПустаяСтрока(лкСтрокаПоиска) Тогда
			...
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ТекстСообщения = НСтр("ru='ОбработкаНовостей.НайтиНовости: произошла ошибка поиска новостей, строка поиска [%СтрокаПоиска%] по причине: %ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%';uk='ОбработкаНовостей.НайтиНовости: сталася помилка пошуку новин, рядок пошуку [%СтрокаПоиска%] з причини: %ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'");
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					...
					НСтр("ru='Новости. Разное. Ошибка полнотекстового поиска';uk='Новини. Різне. Помилка повнотекстового пошуку'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
...
Функция ПолучитьТаблицуКонтекстныхНовостей(
	...
	Если Вариант = "Для форм, все поля" Тогда
		...
		Запрос.Текст = "
			...
			|";
		Запрос.УстановитьПараметр("ТекущийПользователь", ПараметрыСеанса.ТекущийПользователь); // В модели сервиса запускается только с включенным разделителем
		Запрос.УстановитьПараметр("ТекущаяУниверсальнаяДата", ТекущаяУниверсальнаяДата());
...
// Массив структур готовится с помощью ПреобразоватьТаблицуЗначенийВМассивСтруктур(ПолучитьТаблицуКонтекстныхНовостей(...)).
//
// Параметры:
...
Процедура ВсеОбновленияНовостей() Экспорт
	...
	ТекстСообщения = НСтр("ru='Начало выполнения регламентного задания
        ...
        |'
        |;uk='Початок виконання завдання регламентного
        |Рядок підключення: %СтрокаСоединенияИнформационнойБазы()%
        |ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%
        |Це поділена конфігурація: %ЭтоРазделеннаяКонфигурация()%
        |Поділ включено: %РазделениеВключено()%
        |Доступно використання поділених даних: %ДоступноИспользованиеРазделенныхДанных()%
        |Сеанс запущений без роздільників: %СеансЗапущенБезРазделителей()%
        |Функціональна опція Дозволена робота з новинами: %ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями()%
        |Функціональна опція Дозволена робота з новинами через інтернет: %ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостямиЧерезИнтернет()%
        |Поточний користувач ІБ:
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
		НСтр("ru='Новости. Все обновления новостей. Начало';uk='Новини. Всі оновлення новин. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
	...
	Если МонопольныйРежим() Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
			НСтр("ru='Новости. Все обновления новостей. Конец';uk='Новини. Всі оновлення новин. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			НСтр("ru='Принудительное завершение выполнения регламентного задания: информационная база работает в монопольном режиме.';uk='Примусове завершення виконання регламентного завдання: інформаційна база працює в монопольному режимі.'") // Комментарий
		);
		ЕстьОшибки = Истина;
	...
	Если ЕстьОшибки = Ложь Тогда
		...
		Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() = Истина Тогда
			...
			Запрос.Текст = "
				...
				|	ЕСТЬNULL(Рег.ДатаПоследнегоОбновления, ДАТАВРЕМЯ(1,1,1)) КАК ДатаПоследнегоОбновления
				|ИЗ
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				Пока Выборка.Следующий() Цикл
					Если Выборка.ЧастотаОбновления = 1 Тогда // Ежедневно
						Если Выборка.ДатаПоследнегоОбновления <= ТекущаяДатаДляРегистров - 1*24*60*60 Тогда
							МассивЛентНовостей.Добавить(Выборка.ЛентаНовостей);
						КонецЕсли;
					ИначеЕсли Выборка.ЧастотаОбновления = 2 Тогда // Ежечасно
						Если Выборка.ДатаПоследнегоОбновления <= ТекущаяДатаДляРегистров - 1*60*60 Тогда
							МассивЛентНовостей.Добавить(Выборка.ЛентаНовостей);
						КонецЕсли;
					ИначеЕсли Выборка.ЧастотаОбновления = 3 Тогда // Каждые 15 минут
						Если Выборка.ДатаПоследнегоОбновления <= ТекущаяДатаДляРегистров - 15*60 Тогда
							МассивЛентНовостей.Добавить(Выборка.ЛентаНовостей);
						КонецЕсли;
					ИначеЕсли Выборка.ЧастотаОбновления = 4 Тогда // Ежеминутно
						Если Выборка.ДатаПоследнегоОбновления <= ТекущаяДатаДляРегистров - 60 Тогда
							МассивЛентНовостей.Добавить(Выборка.ЛентаНовостей);
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
			...
			Если МассивЛентНовостей.Количество() > 0 Тогда
				...
				ТекстСообщения = НСтр("ru='Получение и обработка новостей для лент новостей: %ОбработкаНовостейКлиентСервер.ПолучитьСтроковоеПредставление(МассивЛентНовостей)%
                    |Время начала (мс): %ТНачало%
                    |'
                    |;uk='Отримання і обробка новин для стрічок новин: %ОбработкаНовостейКлиентСервер.ПолучитьСтроковоеПредставление(МассивЛентНовостей)%
                    |Час початку (мс): %ТНачало%
                    |'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОбработкаНовостейКлиентСервер.ПолучитьСтроковоеПредставление(МассивЛентНовостей)%", ОбработкаНовостейКлиентСервер.ПолучитьСтроковоеПредставление(МассивЛентНовостей, ", "));
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т1Начало);
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей. Начало';uk='Новини. Всі оновлення новин. Отримання і обробка новин за стрічками новин. Початок'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
				...
				ТекстСообщения = НСтр("ru='Завершение получения и обработки новостей для лент новостей
                    ...
                    |'
                    |;uk='Завершення отримання і обробки новин для стрічок новин
                    |Час початку (мс): %ТНачало%
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей. Конец';uk='Новини. Всі оновлення новин. Отримання і обробка новин за стрічками новин. Кінець'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			...
			Иначе
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей. Отсутствие лент новостей для обработки';uk='Новини. Всі оновлення новин. Отримання і обробка новин за стрічками новин. Відсутність стрічок новин для обробки'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					НСтр("ru='Нет лент новостей для регламентного обновления';uk='Немає стрічок новин для регламентного оновлення'") // Комментарий
				);
			КонецЕсли;
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ru='1. Получение и обработка новостей из лент новостей (с обязательным заполнением измерения ЛентаНовостей)';uk='1. Отримання і обробка новин зі стрічок новин (з обов''язковим заповненням виміру ЛентаНовостей)'"),
					ТШаг,
			...
			Если РазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
				...
				Если Запись.Выбран() Тогда
					ДатаПоследнегоОбновления = Запись.ДатаПоследнегоОбновления;
				Иначе
					ДатаПоследнегоОбновления = '00010101';
				КонецЕсли;

				// Обновлять классификаторы 1 раз в день.
				Если ДатаПоследнегоОбновления <= ТекущаяДатаДляРегистров - 24*60*60 Тогда
					...
					ТекстСообщения = НСтр("ru='Обновление классификаторов через интернет, начало
                        |Время начала (мс): %ТНачало%
                        |'
                        |;uk='Оновлення класифікаторів через інтернет, початок
                        |Час початку (мс): %ТНачало%
                        |'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т1Начало);
					ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
						НСтр("ru='Новости. Все обновления новостей. Обновление классификаторов. Начало';uk='Новини. Всі оновлення новин. Оновлення класифікаторів. Початок'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
						Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
						, // Данные
						ТекстСообщения // Комментарий
					...
					ТекстСообщения = НСтр("ru='Обновление классификаторов через интернет, завершение
                        ...
                        |Длительность (мс): %ТДлительность%
                        |'
                        |;uk='Оновлення класифікаторів через інтернет, завершення
                        |Час початку (мс): %ТНачало%
                        |Час закінчення (мс): %ТКонец%
                        |Тривалість (мс): %ТДлительность%
                        |'");
					...
					ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
						НСтр("ru='Новости. Все обновления новостей. Обновление классификаторов. Конец';uk='Новини. Всі оновлення новин. Оновлення класифікаторів. Кінець'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
						Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
						Т1Длительность, // Данные
						ТекстСообщения // Комментарий
			...
			Иначе
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Обновление классификаторов. Отключена работа через интернет';uk='Новини. Всі оновлення новин. Оновлення класифікаторів. Відключена робота через інтернет'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					НСтр("ru='Обновление классификаторов через интернет отключено';uk='Оновлення класифікаторів через інтернет вимкнено'") // Комментарий
				);
			КонецЕсли;
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ru='2. Обновление классификаторов';uk='2. Оновлення класифікаторів'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
			...
			Если ДатаПоследнегоОбновления <= ТекущаяДатаДляРегистров - 24*60*60 Тогда
				...
				ТекстСообщения = НСтр("ru='Автоудаление новостей, начало
                    |Время начала (мс): %ТНачало%
                    |'
                    |;uk='Автовилучення новин, початок
                    |Час початку (мс): %ТНачало%
                    |'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т1Начало);
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Автоудаление новостей. Начало';uk='Новини. Всі оновлення новин. Автовидалення новин. Початок'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
				...
				ТекстСообщения = НСтр("ru='Автоудаление новостей, завершение.
                    ...
                    |Длительность (мс): %ТДлительность%
                    |'
                    |;uk='Автовилучення новин, завершення.
                    |Результат виконання:
                    |%РезультатВыполнения%
                    |Час початку (мс): %ТНачало%
                    |Час закінчення (мс): %ТКонец%
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Автоудаление новостей. Конец';uk='Новини. Всі оновлення новин. Автовидалення новин. Кінець'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					Т1Длительность, // Данные
					ТекстСообщения // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ru='3. Автоудаление новостей';uk='3. Автовилучення новин'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
			...
			Если ДатаПоследнегоОбновления <= ТекущаяДатаДляРегистров - 30*60 Тогда
				...
				ТекстСообщения = НСтр("ru='Обновление периодических свойств, начало
                    |Время начала (мс): %ТНачало%
                    |'
                    |;uk='Оновлення періодичних властивостей, початок
                    |Час початку (мс): %ТНачало%
                    |'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т1Начало);
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Обновление периодических свойств. Начало';uk='Новини. Всі оновлення новин. Оновлення періодичних властивостей. Початок'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
				...
				ТекстСообщения = НСтр("ru='Обновление периодических свойств, завершение.
                    ...
                    |Длительность (мс): %ТДлительность%
                    |'
                    |;uk='Оновлення періодичних властивостей, завершення.
                    |Результат виконання:
                    |%РезультатВыполнения%
                    |Час початку (мс): %ТНачало%
                    |Час закінчення (мс): %ТКонец%
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Обновление периодических свойств. Конец';uk='Новини. Всі оновлення новин. Оновлення періодичних властивостей. Кінець'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					Т1Длительность, // Данные
					ТекстСообщения // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ru='4. Обновление периодических свойств (важность, актуальность)';uk='4. Оновлення періодичних властивостей (важливість, актуальність)'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
			...
			Если ДатаПоследнегоОбновления <= ТекущаяДатаДляРегистров - 30*60 Тогда
				...
				ТекстСообщения = НСтр("ru='Обновление привязок к метаданным, начало
                    |Время начала (мс): %ТНачало%
                    |'
                    |;uk='Оновлення прив''язок до метаданих, початок
                    |Час початку (мс): %ТНачало%
                    |'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т1Начало);
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Обновление привязок к метаданным. Начало';uk='Новини. Всі оновлення новин. Оновлення прив''язок до метаданих. Початок'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
				...
				ТекстСообщения = НСтр("ru='Обновление привязок к метаданным, завершение.
                    ...
                    |Длительность (мс): %ТДлительность%
                    |'
                    |;uk='Оновлення прив''язок до метаданих, завершення.
                    |Результат виконання:
                    |%РезультатВыполнения%
                    |Час початку (мс): %ТНачало%
                    |Час закінчення (мс): %ТКонец%
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Обновление привязок к метаданным. Конец';uk='Новини. Всі оновлення новин. Оновлення прив''язок до метаданих. Кінець'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ru='5. Обновление привязок к метаданным (+ важность)';uk='5. Оновлення прив''язок до метаданих (+ важливість)'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
			...
			Если ДатаПоследнегоОбновления <= ТекущаяДатаДляРегистров - 6*60*60 Тогда
				...
				ТекстСообщения = НСтр("ru='Проверка версии платформы, начало
                    |Время начала (мс): %ТНачало%
                    |'
                    |;uk='Перевірка версії платформи, початок
                    |Час початку (мс): %ТНачало%
                    |'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т1Начало);
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Проверка версии платформы. Начало';uk='Новини. Всі оновлення новин. Перевірка версії платформи. Початок'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
				...
				ТекстСообщения = НСтр("ru='Проверка версии платформы, завершение.
                    ...
                    |Длительность (мс): %ТДлительность%
                    |'
                    |;uk='Перевірка версії платформи, завершення.
                    |Результат виконання:
                    |%РезультатВыполнения%
                    |Час початку (мс): %ТНачало%
                    |Час закінчення (мс): %ТКонец%
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Все обновления новостей';uk='БІП:Новини.Всі оновлення новин'"), // ИмяСобытия
					НСтр("ru='Новости. Все обновления новостей. Проверка версии платформы. Конец';uk='Новини. Всі оновлення новин. Перевірка версії платформи. Кінець'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					Т1Длительность, // Данные
					ТекстСообщения // Комментарий
			...
			ЛогШаговВыполнения.Добавить(
				Новый Структура("Шаг, ВремяНачала, ВремяОкончания",
					НСтр("ru='6. Проверка версии платформы';uk='6. Перевірка версії платформи'"),
					ТШаг,
					ТекущаяУниверсальнаяДатаВМиллисекундах()));
	...
	ТекстСообщения = НСтр("ru='Завершение выполнения регламентного задания
        ...
        |%РезультатПоШагам%
        |'
        |;uk='Завершення виконання завдання регламентного
        |Час початку (мс): %ТНачало%
        |Час закінчення (мс): %ТКонец%
        |Тривалість (мс): %ТДлительность%
        |
...
Процедура УдалитьНовости_Ежедневно(РезультатВыполнения = "") Экспорт
	...
	Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() = Истина Тогда
		...
		ТекстСообщения = НСтр("ru='Выполнение регламентного задания
            ...
            |		%ТекущийПользователь().РазделениеДанных%
            |'
            |;uk='Виконання регламентного завдання
            |Рядок підключення: %СтрокаСоединенияИнформационнойБазы()%
            |ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%
            |Це поділена конфігурація: %ЭтоРазделеннаяКонфигурация()%
            |Поділ включено: %РазделениеВключено()%
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. УдалитьНовости_Ежедневно. Начало удаления';uk='Новини. Сервіс і регламент. УдалитьНовости_Ежедневно. Початок видалення'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
		...
		ТекстСообщения = НСтр("ru='Собраны данные для удаления:
            ...
            |;uk='Зібрані дані для вилучення:
            |Новин (для вилучення): %ДанныеДляУдаления.НовостиДляУдаления.Количество()%
            |Новин (для встановлення позначки вилучення): %ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()%
            |КатегорииНовостейПростыеНеобработанные: %ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()%
            |КатегорииНовостейИнтервалыВерсийНеобработанные: %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()%'");
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. УдалитьНовости_Ежедневно. После сбора данных для удаления';uk='Новини. Сервіс і регламент. УдалитьНовости_Ежедневно. Після збору даних для вилучення'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
		...
		ТекстСообщения = НСтр("ru='Результат удаления: %РезультатВыполнения%';uk='Результат видалення: %РезультатВыполнения%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%РезультатВыполнения%", РезультатВыполнения);
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. УдалитьНовости_Ежедневно. Окончание удаления';uk='Новини. Сервіс і регламент. УдалитьНовости_Ежедневно. Закінчення видалення'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
...
Процедура ОбновлениеПериодическихСвойствНовостей(РезультатВыполнения = "") Экспорт
	...
	Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ОбновлениеПериодическихСвойствНовостей. Начало';uk='Новини. Сервіс і регламент. ОбновлениеПериодическихСвойствНовостей. Початок'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
		...
		Если НЕ Результат.Пустой() Тогда
			...
			РезультатВыполнения = НСтр("ru='Найдено %ВсегоСтрок% новостей для обновления периодических свойств';uk='Знайдено %ВсегоСтрок% новин для оновлення періодичних властивостей'");
			...
			КонецЦикла;
		Иначе
			РезультатВыполнения = НСтр("ru='Не найдено данных для обновления';uk='Не знайдено даних для оновлення'");
		КонецЕсли;
		...
		ТекстСообщения = НСтр("ru='Результат обработки периодических реквизитов: %РезультатВыполнения%';uk='Результат обробки періодичних реквізитів: %РезультатВыполнения%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%РезультатВыполнения%", РезультатВыполнения);
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ОбновлениеПериодическихСвойствНовостей. Конец';uk='Новини. Сервіс і регламент. ОбновлениеПериодическихСвойствНовостей. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
...
Процедура ОбновлениеПривязокКМетаданным(РезультатВыполнения = "") Экспорт
	...
	Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ОбновлениеПривязокКМетаданным. Начало';uk='Новини. Сервіс і регламент. ОбновлениеПривязокКМетаданным. Початок'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
		...
		Если НЕ Результат.Пустой() Тогда
			Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
			ВсегоСтрок = Выборка.Количество();
			РезультатВыполнения = НСтр("ru='Найдено %ВсегоСтрок% новостей для обновления привязок к метаданным';uk='Знайдено %ВсегоСтрок% новин для оновлення прив''язок до метаданих'");
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ВсегоСтрок%", ВсегоСтрок);
			Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = НСтр("ru='Результат обработки привязок к метаданным: %РезультатВыполнения%';uk='Результат обробки прив''язок до метаданих: %РезультатВыполнения%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%РезультатВыполнения%", РезультатВыполнения);
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ОбновлениеПривязокКМетаданным. Конец';uk='Новини. Сервіс і регламент. ОбновлениеПривязокКМетаданным. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
...
Процедура ПолучитьИОбработатьНовостиПоЛентамНовостей(МассивЛентНовостей) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
			ТекстСообщения = НСтр("ru='ПолучитьИОбработатьНовостиПоЛентамНовостей, при работе в модели сервиса запускать получение новостей вручную можно только при отключенных разделителях.
                ...
                |Пользователь: %ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%
                |'
                |;uk='ПолучитьИОбработатьНовостиПоЛентамНовостей, при роботі в моделі сервісу запускати отримання новин вручну можна тільки при відключених роздільниках.
                |Рядок підключення: %СтрокаСоединенияИнформационнойБазы()%
                |ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%
                |Це розділена конфігурація: %ЭтоРазделеннаяКонфигурация()%
                |Поділ включено: %РазделениеВключено()%
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
				НСтр("ru='Новости. Сервис и регламент. ПолучитьИОбработатьНовостиПоЛентамНовостей. Ошибка выполнения';uk='Новини. Сервіс і регламент. ПолучитьИОбработатьНовостиПоЛентамновостей. Помилка виконання'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				Метаданные.Справочники.Новости, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
	...
	// Запись в журнал регистрации
	Т0Начало = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ИдентификаторШага = НСтр("ru='Новости. Сервис и регламент. ПолучитьИОбработатьНовостиПоЛентамНовостей. Начало';uk='Новини. Сервіс і регламент. ПолучитьИОбработатьНовостиПоЛентамновостей. Початок'");
	ТекстСообщения = НСтр("ru='Начало загрузки и обработки новостей
        |Время начала (мс): %ТНачало%
        |'
        |;uk='Початок завантаження і обробки новин
        |Час початку (мс): %ТНачало%
        |'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ЗаписатьСообщениеВЖурналРегистрации(
	...
	Если (ПравоДоступа("Изменение", Метаданные.Справочники.Новости) = Истина)
			...
			И (ПравоДоступа("Изменение", Метаданные.РегистрыСведений.КатегорииНовостейИнтервалыВерсийНеобработанные) = Истина)
			Тогда
		...
		Иначе
			ТекстСообщения = НСтр("ru='ПолучитьИОбработатьНовостиПоЛентамНовостей, не удалось определить список лент новостей
                ...
                |'
                |;uk='ПолучитьИОбработатьНовостиПоЛентамновостей, не вдалося визначити список стрічок новин
                |Переданий параметр з кількістю значень: %МассивЛентНовостей.Количество()%
                |Результат виконання запиту: Порожній
                |Можливо, список стрічок новин порожній, або у всіх стрічок новин встановлено ознаку ЛокальнаяЛентаНовостей (що означає, що за нею не треба завантажувати новини), або у всіх стрічок новин встановлена позначка видалення.
                |'");
	...
	Иначе
		ТекстСообщения = НСтр("ru='ПолучитьИОбработатьНовостиПоЛентамНовостей, у пользователя
            ...
            |;uk='ПолучитьИОбработатьНовостиПоЛентамновостей, у користувача
            |%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%
            |недостатньо прав для виконання операції завантаження новин.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%", ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ());
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПолучитьИОбработатьНовостиПоЛентамНовостей. Ошибка выполнения';uk='Новини. Сервіс і регламент. ПолучитьИОбработатьНовостиПоЛентамновостей. Помилка виконання'") , // ИдентификаторШага
	...
	Т0Конец = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Т0Длительность = Т0Конец - Т0Начало;
	ИдентификаторШага = НСтр("ru='Новости. Сервис и регламент. ПолучитьИОбработатьНовостиПоЛентамНовостей. Конец';uk='Новини. Сервіс і регламент. ПолучитьИОбработатьНовостиПоЛентамновостей. Кінець'");
	ТекстСообщения = НСтр("ru='Окончание загрузки и обработки новостей
        ...
        |%РезультатПоШагам%
        |'
        |;uk='Закінчення завантаження і обробки новин
        |Час початку (мс): %ТНачало%
        |Час закінчення (мс): %ТКонец%
        |Тривалість (мс): %ТДлительность%
        |
...
//  Нет.
//
Процедура ОбновитьСтандартныеСпискиДляНовостей_Ежедневно() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
			ТекстСообщения = НСтр("ru='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, при работе в модели сервиса запускать обновление стандартных списков можно только при отключенных разделителях.
                ...
                |Пользователь: %ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%
                |'
                |;uk='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, при роботі в моделі сервісу запускати оновлення стандартних списків можна тільки при відключених роздільниках.
                |Рядок підключення: %СтрокаСоединенияИнформационнойБазы()%
                |ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%
                |Це поділена конфігурація: %ЭтоРазделеннаяКонфигурация()%
                |Поділ включено: %РазделениеВключено()%
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
				НСтр("ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Ошибка выполнения';uk='Новини. Сервіс і регламент. ОбновитьСтандартныеСпискиДляНовостей_ежедневно. Помилка виконання'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				Метаданные.Справочники.Новости, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
	...
	Если (ПравоДоступа("Изменение", Метаданные.Справочники.ЗначенияКатегорийНовостей) = Истина)
			И (ПравоДоступа("Изменение", Метаданные.Справочники.ЛентыНовостей) = Истина)
			И (ПравоДоступа("Изменение", Метаданные.ПланыВидовХарактеристик.КатегорииНовостей) = Истина) Тогда
		...
		Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() = Истина Тогда
			...
			Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостямиЧерезИнтернет() = Истина Тогда
				...
				ТекстСообщения = НСтр("ru='Выполнение регламентного задания (начало)
                    ...
                    |		%ТекущийПользователь().РазделениеДанных%
                    |'
                    |;uk='Виконання регламентного завдання (початок)
                    |Рядок підключення: %СтрокаСоединенияИнформационнойБазы()%
                    |ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%
                    |Це поділена конфігурація: %ЭтоРазделеннаяКонфигурация()%
                    |Поділ включено: %РазделениеВключено()%
                    |Доступно використання поділу даних: %ДоступноИспользованиеРазделенныхДанных()%
                    |Сеанс запущений без роздільників: %СеансЗапущенБезРазделителей()%
                    |Функціональна опція Дозволена робота з новинами: %ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями()%
                    |Функціональна опція Дозволена робота з новинами через інтернет: %ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостямиЧерезИнтернет()%
                    |Поточний користувач ІБ:
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
					НСтр("ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Начало';uk='Новини. Сервіс і регламент. ОбновитьСтандартныеСпискиДляНовостей_ежедневно. Початок'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
				...
				ТекстСообщения = НСтр("ru='Выполнение регламентного задания (конец)
                    ...
                    |	%Результат%
                    |'
                    |;uk='Виконання регламентного завдання (кінець)
                    |Рядок підключення: %СтрокаСоединенияИнформационнойБазы()%
                    |ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%
                    |Це поділена конфігурація: %ЭтоРазделеннаяКонфигурация()%
                    |Поділ включено: %РазделениеВключено()%
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
					НСтр("ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Конец';uk='Новини. Сервіс і регламент. ОбновитьСтандартныеСпискиДляНовостей_ежедневно. Кінець'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
					Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
			...
			Иначе
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
					НСтр("ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Предупреждение выполнения';uk='Новини. Сервіс і регламент. ОбновитьСтандартныеСпискиДляНовостей_ежедневно. Попередження виконання'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					НСтр("ru='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, включен режим ''Работа только с локальными новостями'',
                        ...
                        |;uk='ОбновитьСтандартныеСпискиДляНовостей_ежедневно, включений режим ''Робота тільки з локальними новинами'',
                        |тому ніякі класифікатори не оновлюються з сервера 1С.
                        |Їх можна оновлювати з метаданих (при оновленні конфігурації), або вручну.'") // Комментарий
				);
			КонецЕсли;
		...
		Иначе
			ЗаписатьСообщениеВЖурналРегистрации(
				...
				Метаданные.Справочники.Новости, // ОбъектМетаданных
				, // Данные
				НСтр("ru='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, выключен режим ''Разрешена работа с новостями'',
                    |поэтому все механизмы новостного центра отключены.'
                    |;uk='ОбновитьСтандартныеСпискиДляНовостей_ежедневно, вимкнений режим ''Дозволена робота з новинами'',
                    |тому всі механізми новинного центру відключені.'") // Комментарий
			);
		КонецЕсли;
		...
		Иначе // Неудача
			...
			Запись.ДатаПоследнегоОбновления = Запись.ДатаПоследнегоОбновления + 1*60*60 + СлучайноеЧислоСекунд;
			Запись.ДатаСледующегоОбновления = Запись.ДатаПоследнегоОбновления + 24*60*60;
		КонецЕсли;
		Запись.Записать(Истина);
	...
	Иначе
		ТекстСообщения = НСтр("ru='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, у пользователя
            ...
            |;uk='ОбновитьСтандартныеСпискиДляНовостей_ежедневно, у користувача
            |%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%
            |недостатньо прав для виконання операції завантаження стандартних списків.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%", ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ());
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ОбновитьСтандартныеСпискиДляНовостей_Ежедневно. Ошибка выполнения';uk='Новини. Сервіс і регламент. ОбновитьСтандартныеСпискиДляНовостей_ежедневно. Помилка виконання'"), // ИдентификаторШага
...
Процедура ПересчитатьНеобработанныеКатегорииНовостей() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
			ТекстСообщения = НСтр("ru='ПересчитатьНеобработанныеКатегорииНовостей, при работе в модели сервиса запускать обновление стандартных списков можно только при отключенных разделителях.
                ...
                |Пользователь: %ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%
                |'
                |;uk='ПересчитатьНеобработанныеКатегорииНовостей, при роботі в моделі сервісу запускати оновлення стандартних списків можна тільки при відключених роздільниках.
                |Рядок підключення: %СтрокаСоединенияИнформационнойБазы()%
                |ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%
                |Це поділена конфігурація: %ЭтоРазделеннаяКонфигурация()%
                |Поділ включено: %РазделениеВключено()%
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
				НСтр("ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Ошибка выполнения';uk='Новини. Сервіс і регламент. ПересчитатьНеобработанныеКатегорииновостей. Помилка виконання'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
	...
	Если (ПравоДоступа("Изменение", Метаданные.РегистрыСведений.КатегорииНовостейПростые) = Истина)
			И (ПравоДоступа("Изменение", Метаданные.РегистрыСведений.КатегорииНовостейПростыеНеобработанные) = Истина)
			И (ПравоДоступа("Изменение", Метаданные.РегистрыСведений.КатегорииНовостейИнтервалыВерсий) = Истина)
			И (ПравоДоступа("Изменение", Метаданные.РегистрыСведений.КатегорииНовостейИнтервалыВерсийНеобработанные) = Истина)
			Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение, шаг 1/4';uk='Новини. Сервіс і регламент. ПересчитатьНеобработанныеКатегорииновостей. Виконання, крок 1/4'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ru='ПересчитатьНеобработанныеКатегорииНовостей, начало выполнения обработки перезаполнения категорий новостей.';uk='ПересчитатьНеобработанныеКатегорииновостей, початок виконання обробки перезаполнения категорій новин.'") // Комментарий
		);
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение, шаг 2/4';uk='Новини. Сервіс і регламент. ПересчитатьНеобработанныеКатегорииновостей. Виконання, крок 2/4'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ru='ПересчитатьНеобработанныеКатегорииНовостей, удаление неправильных записей, которые невозможно обработать.';uk='ПересчитатьНеобработанныеКатегорииновостей, видалення неправильних записів, які неможливо обробити.'") // Комментарий
		);
		...
		Для каждого ТекущаяСтрока Из ТаблицаКатегорийИЗначенийПростые Цикл
			Если ТекущаяСтрока.УдалитьЗапись = Истина Тогда
				...
				ТекстСообщения = НСтр("ru='Удалена запись из регистра сведений КатегорииНовостейПростыеНеобработанные с ключами:
                    ...
                    |;uk='Видалена запис з реєстру відомостей КатегорииНовостейПростыеНеобработанные з ключами:
                    |- КатегорияНовостейСтрока: %ТекущаяСтрока.КатегорияНовостейСтрока%
                    |- ЗначениеКатегорииНовостейСтрока: %ТекущаяСтрока.ЗначениеКатегорииНовостейСтрока%
                    |- Умова відбору: %ТекущаяСтрока.УсловиеОтбора%
                    |- ЛентаНовостей: %ТекущаяСтрока.ЛентаНовостей%
                    |- УИННовости: %ТекущаяСтрока.УИННовости%'");
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
					НСтр("ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Удаление неправильных записей простых категорий';uk='Новини. Сервіс і регламент. ПересчитатьНеобработанныеКатегорииновостей. Видалення неправильних записів простих категорій'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
		...
		Для каждого ТекущаяСтрока Из ТаблицаКатегорийИЗначенийИнтервалыВерсий Цикл
			Если ТекущаяСтрока.УдалитьЗапись = Истина Тогда
				...
				ТекстСообщения = НСтр("ru='Удалена запись из регистра сведений КатегорииНовостейИнтервалыВерсийНеобработанные с ключами:
                    ...
                    | - ЛентаНовостей: %ТекущаяСтрока.ЛентаНовостей%
                    | - УИННовости: %ТекущаяСтрока.УИННовости%'
                    |;uk='Видалений запис з реєстру відомостей КатегорииНовостейИнтервалыВерсийНеобработанные з ключами:
                    |- КатегорияНовостейСтрока: %ТекущаяСтрока.КатегорияНовостейСтрока%
                    |- Продукт: %ТекущаяСтрока.Продукт%
                    |- ВерсияОТ: %ТекущаяСтрока.ВерсияОТ%
                    |- ВерсияДО: %ТекущаяСтрока.ВерсияДО%
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
					НСтр("ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Удаление неправильных записей интервалов версий';uk='Новини. Сервіс і регламент. ПересчитатьНеобработанныеКатегорииновостей. Видалення неправильних записів інтервалів версій'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение, шаг 3/4';uk='Новини. Сервіс і регламент. ПересчитатьНеобработанныеКатегорииновостей. Виконання, крок 3/4'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ru='ПересчитатьНеобработанныеКатегорииНовостей, удаление неправильных записей, которые невозможно обработать.';uk='ПересчитатьНеобработанныеКатегорииновостей, видалення неправильних записів, які неможливо обробити.'") // Комментарий
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение, шаг 4/4';uk='Новини. Сервіс і регламент. ПересчитатьНеобработанныеКатегорииновостей. Виконання, крок 4/4'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ru='ПересчитатьНеобработанныеКатегорииНовостей, удаление записей, не привязанных к новостям.';uk='ПересчитатьНеобработанныеКатегорииновостей, видалення записів, не прив''язаних до новин.'") // Комментарий
		);
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Выполнение завершено';uk='Новини. Сервіс і регламент. ПересчитатьНеобработанныеКатегорииновостей. Виконання завершено'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегистрыСведений.КатегорииНовостейПростые, // ОбъектМетаданных
			, // Данные
			НСтр("ru='ПересчитатьНеобработанныеКатегорииНовостей, обработка окончена.';uk='ПересчитатьНеобработанныеКатегорииновостей, обробка закінчена.'") // Комментарий
		);
	...
	Иначе
		ТекстСообщения = НСтр("ru='ПересчитатьНеобработанныеКатегорииНовостей, у пользователя
            ...
            |;uk='ПересчитатьНеобработанныеКатегорииновостей, у користувача
            |%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%
            |недостатньо прав для виконання операції обробка категорій новин.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%", ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ());
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей. Ошибка выполнения';uk='Новини. Сервіс і регламент. ПересчитатьНеобработанныеКатегорииновостей. Помилка виконання'"), // ИдентификаторШага
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
			ТекстСообщения = НСтр("ru='ПересчитатьОтборыПоНовостям_РедкоМеняющиеся, при работе в модели сервиса запускать обновление стандартных списков можно только при отключенных разделителях.
                ...
                |Пользователь: %ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()%
                |'
                |;uk='ПересчитатьОтборыПоНовостям_РедкоМеняющиеся, при роботі в моделі сервісу запускати оновлення стандартних списків можна тільки при відключених роздільниках.
                |Рядок підключення: %СтрокаСоединенияИнформационнойБазы()%
                |ТекущийРежимЗапуска: %ТекущийРежимЗапуска()%
                |Це розділена конфігурація: %ЭтоРазделеннаяКонфигурация()%
                |Поділ включено: %РазделениеВключено()%
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
				НСтр("ru='Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям_РедкоМеняющиеся. Ошибка выполнения';uk='Новини. Сервіс і регламент. ПересчитатьОтборыПоНовостям_Редкоменяющиеся. Помилка виконання'") , // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
	...
	Если ПравоДоступа("Изменение", Метаданные.РегистрыСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся) = Истина Тогда
		...
		Если (КатегорияВерсияПродукта.Пустая())
			...
			ТекстСообщения = НСтр("ru='ПересчитатьОтборыПоНовостям_РедкоМеняющиеся, не заполнены значения плана видов характеристик КатегорииНовостей:
                ...
                |'
                |;uk='ПересчитатьОтборыПоНовостям_Редкоменяющиеся, не заповнені значення плану видів характеристик КатегорииНовостей:
                |КатегорияВерсияПродукта: %КатегорияВерсияПродукта%
                |КатегорияВерсияПлатформы: %КатегорияВерсияПлатформы%
                |Можливо, вони ще не оновилися з сервера при оновленні класифікаторів, а також не були заповнені в макеті СтандартныеЗначения плану видів характеристик КатегорииНовостей
                |'");
		...
		Если НЕ Результат.Пустой() Тогда
			...
			Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
			Пока Выборка.Следующий() Цикл
				// Если явное несовпадение или по платформе или по подукту, то новость не попадает в отбор.
...
Процедура ПересчитатьОтборыПоНовостям_Общие() Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
		...
		Иначе
			...
			КоличествоОтборов = КоличествоНастроенныхОтборов(ПараметрыСеанса.ТекущийПользователь);
...
Процедура ПроверитьИзменениеВерсииПлатформы(РезультатВыполнения = "") Экспорт
	...
	Если МонопольныйРежим() Тогда
		...
		//  то будут ошибки при всех попытках записи данных в базу.
		// Поэтому в монопольном режиме не следует выполнять никаких обновлений в подсистеме новостей.
		РезультатВыполнения = НСтр("ru='В монопольном режиме проверка версии платформы не осуществляется';uk='В монопольному режимі перевірка версії платформи не здійснюється'");
		Возврат;
	КонецЕсли;
	...
	РезультатВыполнения = НСтр("ru='Сохраненная версия платформы: %СохраненнаяВерсияПлатформы%
        ...
        |;uk='Збережена версія платформи: %СохраненнаяВерсияПлатформы%
        |Поточна версія платформи: %ТекущаяВерсияПлатформы%
        |%ТребуетсяОбновлениеРедкоменяющихсяОтборов%
        |
        |Версія конфігурації в цій обробці не перевіряється, а перераховується примусово в модулі оновлення конфігурації, а також після кожного отримання новин.'");
	...
	УстановитьПривилегированныйРежим(Истина);
			...
			Если МенеджерЗаданийАктивен = Истина Тогда
				Если МассивВыполняемыхФоновыхЗаданий.Количество() = 0 Тогда
					...
					МассивФоновыхЗаданий.Добавить(ФоновыеЗадания.Выполнить(
						...
						,
						"", // Ключ
						"ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся в связи с изменением версии платформы с " + лкПоследняяВерсия + " на " + лкТекущаяВерсия));
					// Последняя версия платформы сохранится в "ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся".
				КонецЕсли;
				РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ru='Редко меняющиеся отборы будут пересчитаны (запущено фоновое задание)';uk='Рідко змінюються відбори будуть перераховані (запущено фонове завдання)'"));
			Иначе
				РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ru='Редко меняющиеся отборы будут пересчитаны (явный запуск ПересчитатьОтборыПоНовостям_РедкоМеняющиеся())';uk='Рідко змінюються відбори будуть перераховані (явний запуск ПересчитатьОтборыПоНовостям_РедкоМеняющиеся())'"));
		...
		Иначе
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%СохраненнаяВерсияПлатформы%", лкПоследняяВерсия);
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТекущаяВерсияПлатформы%", лкТекущаяВерсия);
			РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("ru='Редко меняющиеся отборы останутся как есть';uk='Рідко змінюються відбори залишаться як є'"));
		КонецЕсли;
...
Процедура СбросСостоянияНовостиДляВсехПользователей(НовостьСсылка) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
		...
		// При такой записи выдается ошибка "В данной транзакции уже происходили ошибки!".
		// Возможно, из-за того, что при таком чтении (через набор записей) ТекущаяЗапись.Пользователь = Ошибка чтения значения.
		// НаборЗаписей = РегистрыСведений.СостоянияНовостей.СоздатьНаборЗаписей();
		// НаборЗаписей.Отбор["Новость"].Установить(НовостьСсылка, Истина);
		// НаборЗаписей.Прочитать();
		// Для Каждого ТекущаяЗапись Из НаборЗаписей Цикл
		// 	ТекущаяЗапись.Прочтена                = Ложь;
		// 	ТекущаяЗапись.ОповещениеВключено      = Истина;
		// 	ТекущаяЗапись.УдаленаИзСпискаНовостей = Ложь;
		// КонецЦикла;
		// НаборЗаписей.Записать(Истина);
...
//  отключить ленту новостей из автоматического обновления, чтобы не генерировать ненужный траффик или не заставлять
...
Процедура НайтиИОбработатьНеактивныеЛентыНовостей() Экспорт
	...
	ТекстСообщения = НСтр("ru='Начало проверки лент новостей на актуальность
        |Время начала (мс): %ТНачало%
        |'
        |;uk='Початок перевірки стрічок новин на актуальність
        |Час початку (мс): %ТНачало%
        |'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
		НСтр("ru='Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей. Начало';uk='Новини. Сервіс і регламент. НайтиИОбработатьНеактивныеЛентыНовостей. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения); // Комментарий
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Выборка.ЛентаНовостейЧастотаОбновления = 0 Тогда
					лкЧастотаОбновления = "вручную (0)";
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 1 Тогда
					лкЧастотаОбновления = "один раз в день (1)";
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 2 Тогда
					лкЧастотаОбновления = "каждый час (2)";
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 3 Тогда
					лкЧастотаОбновления = "каждые 15 минут (3)";
				ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 4 Тогда
					лкЧастотаОбновления = "каждую минуту (4)";
				Иначе
					лкЧастотаОбновления = "неопределено (" + Строка(Выборка.ЛентаНовостейЧастотаОбновления) + ")";
				КонецЕсли;

				РезультатВыполнения_ЛентаНовостей = НСтр("ru='Лента новостей %ЛентаНовостей%, частота обновления ленты новостей: %ЧастотаОбновления%, дата последней успешной загрузки: %ДатаЗагрузкиПоследнихНовостей%, неудач: %КоличествоНеудачПриЗагрузкеНовостей%, последняя попытка загрузки: %ДатаПоследнейПопыткиЗагрузкиНовостей%. %Действие%';uk='Стрічка новин %ЛентаНовостей%, частота оновлення стрічки новин: %ЧастотаОбновления%, дата останнього успішного завантаження: %ДатаЗагрузкиПоследнихНовостей%, невдач: %КоличествоНеудачПриЗагрузкеНовостей%, остання спроба завантаження: %ДатаПоследнейПопыткиЗагрузкиНовостей%. %Действие%'");
				РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%ЛентаНовостей%", Выборка.ЛентаНовостей);
				РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%ДатаЗагрузкиПоследнихНовостей%", ?(Выборка.ДатаЗагрузкиПоследнихНовостей = '00010101',"(никогда)",Формат(МестноеВремя(Выборка.ДатаЗагрузкиПоследнихНовостей),"ДФ='dd.MM.yyyy HH:mm:ss'")));
				РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%КоличествоНеудачПриЗагрузкеНовостей%", Выборка.КоличествоНеудачПриЗагрузкеНовостей);
				РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%ДатаПоследнейПопыткиЗагрузкиНовостей%", Формат(МестноеВремя(Выборка.ДатаПоследнейПопыткиЗагрузкиНовостей),"ДФ='dd.MM.yyyy HH:mm:ss'"));
				РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%ЧастотаОбновления%", лкЧастотаОбновления);
				...
				Если Выборка.ЛентаНовостейАктивна = Истина Тогда
					...
					Если СтруктураНовости.Ссылка.Пустая() Тогда
						...
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ru='Лента новостей активна, новости о неактивности нет';uk='Стрічка новин активна, новини про неактивність немає'"));
					ИначеЕсли СтруктураНовости.ПометкаУдаления = Истина Тогда
						...
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ru='Лента новостей активна, новость о неактивности уже была помечена на удаление';uk='Стрічка новин активна, новина про неактивність вже була позначена на видалення'"));
					Иначе
						// Пометить найденную новость на удаление.
						НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
						НовостьОбъект.УстановитьПометкуУдаления(Истина);
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ru='Лента новостей активна, новость о неактивности будет помечена на удаление';uk='Стрічка новин активна, новина про неактивність буде помічений на видалення'"));
					КонецЕсли;
				Иначе
					// Найти новость и обновить. Если ее нет - создать и заполнить.
					Если СтруктураНовости.Ссылка.Пустая() Тогда
						НовостьОбъект = Справочники.Новости.СоздатьЭлемент();
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ru='Лента новостей НЕактивна, новость о неактивности будет создана';uk='Стрічка новин НЕактивна, новина про неактивність буде створена'"));
					Иначе
						НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
						РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("ru='Лента новостей НЕактивна, новость о неактивности будет обновлена';uk='Стрічка новин НЕактивна, новина про неактивність буде оновлено'"));
					КонецЕсли;
					...
					ТекстНовости = СтрЗаменить(ТекстНовости,
						...
						Формат(МестноеВремя(Выборка.ДатаЗагрузкиПоследнихНовостей),"ДФ='dd.MM.yyyy HH:mm:ss'"));
					...
					ТекстНовости = СтрЗаменить(ТекстНовости,
						...
						Формат(МестноеВремя(Выборка.ДатаПоследнейПопыткиЗагрузкиНовостей),"ДФ='dd.MM.yyyy HH:mm:ss'"));
					ТекстНовости = СтрЗаменить(ТекстНовости,
						"%URL%",
					...
					НовостьОбъект.Наименование                       = НСтр("ru='Лента новостей неактивна';uk='Стрічка новин неактивна'");
					НовостьОбъект.Подзаголовок                       = НСтр("ru='Лента новостей неактивна. Рекомендуется исключить ее из автообновления';uk='Стрічка новин неактивна. Рекомендується виключити її з автооновлення'");
					...
					Если Отказ <> Истина Тогда
						...
						Исключение
							...
							ТекстСообщения = НСтр("ru='Произошла ошибка при создании / обновлении новости о неактивности ленты новостей %ЛентаНовостей%
                                ...
                                |%ИнформацияОбОшибке%
                                |'
                                |;uk='Помилка створення / оновлення новини про неактивність стрічки новин %ЛентаНовостей%
                                |Докладно:
                                |%ИнформацияОбОшибке%
                                |'");
							...
							ЗаписатьСообщениеВЖурналРегистрации(
								НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
								НСтр("ru='Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей. Ошибка';uk='Новини. Сервіс і регламент. НайтиИОбработатьНеактивныеЛентыНовостей. Помилка'"), // ИдентификаторШага
								УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
								, // ОбъектМетаданных
								, // Данные
								ТекстСообщения); // Комментарий
						КонецПопытки;
					Иначе
						РезультатВыполнения_ЛентаНовостей = РезультатВыполнения_ЛентаНовостей + Символы.ПС + НСтр("ru='Отменено создание / изменение новости про отсутствие активности для ленты новостей';uk='Скасовано створення / зміна новини про відсутність активності для стрічки новин'");
					КонецЕсли;
				КонецЕсли;
	...
	ТекстСообщения = НСтр("ru='Окончание проверки лент новостей на актуальность.
        ...
        |Длительность (мс): %ТДлительность%
        |'
        |;uk='Закінчення перевірки стрічок новин на актуальність.
        |Стрічка новин вважається Неактуальною, якщо по ній було більше 10 невдалих спроб отримання новин і остання вдала спроба була більше доби тому.
        |
        |Результат виконання:
        |%РезультатВыполнения%
        |
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
		НСтр("ru='Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей. Конец';uk='Новини. Сервіс і регламент. НайтиИОбработатьНеактивныеЛентыНовостей. Кінець'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения); // Комментарий
...
// Процедура-обработчик события при записью элемента справочника Пользователи.
// Определяет, что записывается новый пользователь и передает эту информацию дальше, в подписку на событие "ПриЗаписи".
//
// Параметры:
//  ПользовательОбъект - СправочникОбъект.Пользователи - записываемый объект;
//  Отказ              - Булево - установить в Истина, если надо отменить запись.
...
Процедура ПриЗаписиПользователя(ПользовательОбъект, Отказ) Экспорт
	...
	Если ТипЗнч(ПользовательОбъект) = Тип("СправочникОбъект.Пользователи") Тогда
		...
		Если ТипЗнч(ПользовательОбъект.ДополнительныеСвойства) = ТипСтруктура Тогда
			Если ПользовательОбъект.ДополнительныеСвойства.Свойство("ЭтоНовыйОбъект") = Истина
...
Процедура ИзменитьИспользованиеРегламентныхЗаданий(Использовать) Экспорт
	...
	Если ТекущееРегламентноеЗадание <> Неопределено Тогда
		...
		Иначе
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru='ОбработкаНовостей.ИзменитьИспользованиеРегламентныхЗаданий:
                    ...
                    |;uk='ОбработкаНовостей.ИзменитьИспользованиеРегламентныхзаданий:
                    |у параметрі Использовать передане значення неправильного типу: %1'"),
				ТипЗнч(Использовать)
			);
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
				НСтр("ru='Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';uk='Новини. Сервіс і регламент. ИзменениеИспользованияРегламентныхзаданий'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ru='Не найдено предопределенное регламентное задание ВсеОбновленияНовостей.';uk='Не знайдено зумовлене регламентне завдання ВсеОбновленияНовостей.'");
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';uk='Новини. Сервіс і регламент. ИзменениеИспользованияРегламентныхзаданий'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
...
//
// Параметры:
//  ТекущаяДатаДляРасчетов - Дата - текущая дата, относительно которой неообходимо рассчитывать интервал удаляемых данных;
...
Функция УдалениеНовостей_ПолучитьСписокДанныхДляУдаления(ТекущаяДатаДляРасчетов, Отборы)
	...
	Запрос.Текст = "
		...
		|		И Спр.ДатаЗавершения > ДАТАВРЕМЯ(0001,01,01)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|// Актуальные новости.
...
Процедура УдалениеНовостей_ПометитьНаУдалениеДанные(ДанныеДляУдаления, РезультатВыполнения)
	...
	УстановитьПривилегированныйРежим(Истина);
		...
		// Удалить новости.
		Если (ДанныеДляУдаления.Свойство("НовостиДляУдаления") = Истина)
			...
			// ОбластьПоиска = Неопределено; // Новый Массив;
			// УдалитьОбъекты(ДанныеДляУдаления.НовостиДляУдаления, Истина, НайденныеДанные, ОбластьПоиска);
			ТекстСообщения = НСтр("ru='Есть помеченные на удаление новости (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% новостей). Их надо удалить вручную через Все функции / Стандартные / Удаление помеченных объектов.';uk='Є відмічені для вилучення новини (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% новин). Їх треба вилучити вручну через Всі функції / Стандартні / Вилучення відмічених об''єктів.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.НовостиДляУдаления.Количество()%", ДанныеДляУдаления.НовостиДляУдаления.Количество());
		Иначе
			ТекстСообщения = НСтр("ru='Нет помеченных на удаление новостей';uk='Немає позначених на видалення новин'");
		КонецЕсли;
		...
		Если (ДанныеДляУдаления.Свойство("НовостиДляУстановкиПометкиУдаления") = Истина)
			...
			Для Каждого ТекущаяНовость Из ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ТекстСообщения = НСтр("ru='Все устаревшие новости успешно помечены на удаление (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()% новостей).';uk='Всі застарілі новини успішно відмічені для вилучення (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()% новин).'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()%", ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество());
		Иначе
			ТекстСообщения = НСтр("ru='Нет устаревших новостей, которые надо пометить на удаление.';uk='Немає застарілих новин, які треба відмітити на вилучення.'");
		КонецЕсли;
		...
		Если (ДанныеДляУдаления.Свойство("КатегорииНовостейПростыеНеобработанные") = Истина)
			...
			Для Каждого ТекущаяЗапись Из ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные Цикл
				...
				Запись.Удалить();
			КонецЦикла;
			ТекстСообщения = НСтр("ru='Было удалено %ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()% необработанных простых категорий.';uk='Було вилучено %ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()% необроблених простих категорій.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество()%", ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество());
		Иначе
			ТекстСообщения = НСтр("ru='Простых необработанных категорий не было найдено.';uk='Простих необроблених категорій не було знайдено.'");
		КонецЕсли;
		РезультатВыполнения = СтрЗаменить(РезультатВыполнения, "%КатегорииНовостейПростыеНеобработанные%", ТекстСообщения);

		// Записи регистров сведений удалять без учета того, что какие-то новости могли быть НЕ удалены.
		Если (ДанныеДляУдаления.Свойство("КатегорииНовостейИнтервалыВерсийНеобработанные") = Истина)
			...
			Для Каждого ТекущаяЗапись Из ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные Цикл
				...
				Запись.Удалить();
			КонецЦикла;
			ТекстСообщения = НСтр("ru='Было удалено %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()% необработанных категорий интервалов версий.';uk='Було вилучено %ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()% необроблених категорій інтервалів версій.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество()%", ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество());
		Иначе
			ТекстСообщения = НСтр("ru='Необработанных категорий интервалов версий не было найдено.';uk='Необроблених категорій інтервалів версій не було знайдено.'");
		КонецЕсли;
...
Процедура ЗагрузитьВсеСтандартныеЗначения() Экспорт
	...
	ТекстСообщения = НСтр("ru='Начало загрузки всех стандартных значений
        |Время начала (мс): %ТНачало%
        |'
        |;uk='Початок завантаження всіх стандартних значень
        |Час початку (мс): %ТНачало%
        |'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
		НСтр("ru='Новости. Сервис и регламент. Загрузка стандартных значений. Все стандартные значения. Начало';uk='Новини. Сервіс і регламент. Завантаження стандартних значень. Всі стандартні значення. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
	...
	ТекстСообщения = НСтр("ru='Окончание загрузки всех стандартных значений
        ...
        |Длительность (мс): %ТДлительность%
        |'
        |;uk='Закінчення завантаження всіх стандартних значень
        |Час початку (мс): %ТНачало%
        |Час закінчення (мс): %ТКонец%
        |Тривалість (мс): %ТДлительность%
        |'");
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
		НСтр("ru='Новости. Сервис и регламент. Загрузка стандартных значений. Все стандартные значения. Конец';uk='Новини. Сервіс і регламент. Завантаження стандартних значень. Всі стандартні значення. Кінець'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		Т0Длительность, // Данные
		ТекстСообщения // Комментарий
...
// Преобразовывает таблицу значений в массив структур, который читается и на клиенте и на сервере.
//
// Параметры:
//  Таблица - ТаблицаЗначений - Таблица значений, которую необходимо преобразовать.
//
// Возвращаемое значение:
//   Массив - Массив структур, повторяющий все строки со всеми колонками переданной таблицы значений.
//
Функция ПреобразоватьТаблицуЗначенийВМассивСтруктур(Таблица) Экспорт
	...
	Если ТипЗнч(Таблица) = Тип("ТаблицаЗначений") Тогда
		...
		Для каждого ТекущаяСтрока Из Таблица Цикл
			// Структуру для каждой строки надо создавать заново, в противном случае во всех значениямх массива будут одни и те же данные.
...
// Преобразовывавет массив структур (который читается и на клиенте и на сервере) в таблицу значений или в "данные формы коллекция".
//
// Параметры:
//  МассивСтруктур - Массив - массив со структурами, которые необходимо поместить в результирующую таблицу;
//  Таблица        - ТаблицаЗначений, ДанныеФормыКоллекция - Таблица значений или реквизит формы, в которую необходимо поместить данные.
//
Процедура ПреобразоватьМассивСтруктурВТаблицуЗначений(МассивСтруктур, Таблица) Экспорт

	ТипМассив    = Тип("Массив");
	ТипСтруктура = Тип("Структура");

	Если (Тип(МассивСтруктур) = ТипМассив) Тогда
		Если (ТипЗнч(Таблица) = Тип("ТаблицаЗначений"))
				ИЛИ (ТипЗнч(Таблица) = Тип("ДанныеФормыКоллекция")) Тогда
			// Таблицу НЕ очищать - возможно, что ее надо ДОзаполнить.
			// Если надо очищать таблицу, то это надо сделать ДО вызова функции.
			Для каждого ТекущаяСтруктура Из МассивСтруктур Цикл
				Если ТипЗнч(ТекущаяСтруктура) = ТипСтруктура Тогда
					НоваяСтрока = Таблица.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтруктура);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
// Процедура создает реквизиты формы для хранения информации для подсистемы новостей.
// УСТАРЕВШИЙ ФУНКЦИОНАЛ. Программное добавление большого количества реквизитов существенно замедляет открытие формы.
// Теперь добавляется только один реквизит произвольного типа (программно, а лучше - сразу в метаданных).
//
// Параметры:
//  Форма        - Управляемая форма.
//
Процедура СоздатьРеквизитыФормыДляНовостей(Форма) Экспорт

	// 1. Таблица для Списка контекстных новостей.
	...
	ОписаниеТиповНовость   = Новый ОписаниеТипов("СправочникСсылка.Новости");

	ДобавляемыеРеквизиты = Новый Массив;
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Новости",                 Новый ОписаниеТипов("ТаблицаЗначений")));
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Новость",                 Новый ОписаниеТипов("СправочникСсылка.Новости"), "Новости"));
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("НомерСтрокиНовости",      ОписаниеТиповЧисло3,    "Новости"));
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("УИННовости",              ОписаниеТиповСтрока36)); // Контекстные новости могут быть только от 1С, т.е. с гарантированным УИН в 36 символов, не больше
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Важность",                ОписаниеТиповЧисло1,    "Новости"));
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ДатаПубликации",          ОписаниеТиповДата,      "Новости"));
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("НовостьНаименование",     ОписаниеТиповСтрока150, "Новости")); // Длина наименования новостей 1С = 100 символов, новости из других внешних источников могут иметь произвольную длину заголовка
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Форма",                   ОписаниеТиповСтрока50,  "Новости"));
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Событие",                 ОписаниеТиповСтрока50,  "Новости"));
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ПоказыватьВФормеОбъекта", ОписаниеТиповБулево,    "Новости"));
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ЭтоПостояннаяНовость",    ОписаниеТиповБулево,    "Новости"));
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Прочтена",                ОписаниеТиповБулево,    "Новости"));
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ОповещениеВключено",      ОписаниеТиповБулево,    "Новости"));

	// 2. Имя метаданных.
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Новости_Метаданные", ОписаниеТиповСтрока50));
	// 3. Имя формы.
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Новости_Форма", ОписаниеТиповСтрока50));
	// 4. Заголовок формы контекстных новостей.
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Новости_ЗаголовокФормыКонтекстныхНовостей", ОписаниеТиповСтрока50));
	// 5. ТекущаяНовость (если новость открывается в форме объекта).
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Новости_ТекущаяНовость", ОписаниеТиповНовость));
...
Функция ПолучитьКонтекстныеНовости(
	...
	Если ПолучатьКонтекстныеНовости = Истина Тогда
		...
		КонецЕсли;
		МассивСтруктурНовостей = ПреобразоватьТаблицуЗначенийВМассивСтруктур(
...
// Процедура устанавливает все свойства объекта ОбъектХДТО значениями по-умолчанию (из схемы),
//  или инициализирует списки, или заполняет базовые значения по умолчанию (строки, числа, булево, даты).
//
// Параметры:
//  ОбъектХДТО - ОбъектХДТО, в котором надо инициализировать все свойства;
//  Свойства   - СвойстваХДТО, текущая коллекция свойств (может быть не корневого, а нижнего уровня).
//
Процедура ИнициализироватьВсеСвойстваХДТО(ОбъектХДТО, Свойства) Экспорт

	ТипСтрока = ФабрикаXDTO.Тип("http://www.w3.org/2001/XMLSchema", "string");
	ТипБулево = ФабрикаXDTO.Тип("http://www.w3.org/2001/XMLSchema", "boolean");
	ТипЧисло1 = ФабрикаXDTO.Тип("http://www.w3.org/2001/XMLSchema", "long");
	ТипЧисло2 = ФабрикаXDTO.Тип("http://www.w3.org/2001/XMLSchema", "byte");
	ТипЧисло3 = ФабрикаXDTO.Тип("http://www.w3.org/2001/XMLSchema", "decimal");
	ТипДата   = ФабрикаXDTO.Тип("http://www.w3.org/2001/XMLSchema", "dateTime");

	Для каждого ТекущееСвойство Из Свойства Цикл
		Если (ТекущееСвойство.ВерхняяГраница > 1) ИЛИ (ТекущееСвойство.ВерхняяГраница = -1) Тогда // список
			// Список не надо никак инициализировать, получить его значения можно кодом, например Список = ОбъектХДТО.ПолучитьСписок(ТекущееСвойство);
		Иначе // одно значение
			Если НЕ ОбъектХДТО.Установлено(ТекущееСвойство) Тогда
				Если ТипЗнч(ТекущееСвойство.Тип) = Тип("ТипЗначенияXDTO") Тогда // Простое значение
					Если ТекущееСвойство.ЗначениеПоУмолчанию <> Неопределено Тогда
						ОбъектХДТО.Установить(ТекущееСвойство, ТекущееСвойство.ЗначениеПоУмолчанию);
					Иначе
						// Значения по-умолчанию: строка, число, булево, дата
						Если ТекущееСвойство.Тип = ТипСтрока Тогда
							ОбъектХДТО.Установить(ТекущееСвойство, "");
						ИначеЕсли (ТекущееСвойство.Тип = ТипЧисло1)
								ИЛИ (ТекущееСвойство.Тип = ТипЧисло2)
								ИЛИ (ТекущееСвойство.Тип = ТипЧисло3) Тогда
							ОбъектХДТО.Установить(ТекущееСвойство, 0);
						ИначеЕсли ТекущееСвойство.Тип = ТипБулево Тогда
							ОбъектХДТО.Установить(ТекущееСвойство, Ложь);
						ИначеЕсли ТекущееСвойство.Тип = ТипДата Тогда
							ОбъектХДТО.Установить(ТекущееСвойство, '00010101');
						КонецЕсли;
					КонецЕсли;
				ИначеЕсли ТипЗнч(ТекущееСвойство.Тип) = Тип("ТипОбъектаXDTO") Тогда // Сложное значение, возможно, список?
					ОбъектХДТО.Установить(ТекущееСвойство, ФабрикаXDTO.Создать(ТекущееСвойство.Тип));
					ИнициализироватьВсеСвойстваХДТО(ОбъектХДТО.Получить(ТекущееСвойство), ТекущееСвойство.Тип.Свойства);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
// Процедура записывает ошибку в журнал регистрации.
//
// Параметры:
//  ИмяСобытия         - строка;
...
Процедура ЗаписатьСообщениеВЖурналРегистрации(
	...
	Если ВРег(УровеньРегистрации) = ВРег("Информация") Тогда
		лкУровеньЖурналаРегистрации = УровеньЖурналаРегистрации.Информация;
	ИначеЕсли ВРег(УровеньРегистрации) = ВРег("Предупреждение") Тогда
		лкУровеньЖурналаРегистрации = УровеньЖурналаРегистрации.Предупреждение;
	ИначеЕсли ВРег(УровеньРегистрации) = ВРег("Примечание") Тогда
		лкУровеньЖурналаРегистрации = УровеньЖурналаРегистрации.Примечание;
	ИначеЕсли ВРег(УровеньРегистрации) = ВРег("Ошибка") Тогда
		лкУровеньЖурналаРегистрации = УровеньЖурналаРегистрации.Ошибка;
	ИначеЕсли ТипЗнч(УровеньРегистрации) = Тип("УровеньЖурналаРегистрации") Тогда
		лкУровеньЖурналаРегистрации = УровеньРегистрации;
	Иначе
		лкУровеньЖурналаРегистрации = УровеньЖурналаРегистрации.Ошибка;
	КонецЕсли;

	// Сообщения Информация, Примечание писать только если включена настройка "ЖурналРегистрации_ПодробнаяЗапись",
	//  которую можно получить ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации().

	лкЗаписатьСообщениеВЖурналРегистрации = Ложь;
	Если лкУровеньЖурналаРегистрации = УровеньЖурналаРегистрации.Предупреждение Тогда // Всегда писать
		лкЗаписатьСообщениеВЖурналРегистрации = Истина;
	ИначеЕсли лкУровеньЖурналаРегистрации = УровеньЖурналаРегистрации.Ошибка Тогда // Всегда писать
		лкЗаписатьСообщениеВЖурналРегистрации = Истина;
	ИначеЕсли ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации() = Истина Тогда
		лкЗаписатьСообщениеВЖурналРегистрации = Истина;
	КонецЕсли;

	Если лкЗаписатьСообщениеВЖурналРегистрации = Истина Тогда
		лкИмяСобытия = СтрЗаменить(ИмяСобытия, ". ", "."); // Точку с пробелом менять на точку
		лкКомментарий = НСтр("ru='Идентификатор шага: %ИдентификаторШага%
            |%Комментарий%'
            |;uk='Ідентифікатор кроку: %ИдентификаторШага%
            |%Комментарий%'");
		лкКомментарий = СтрЗаменить(лкКомментарий, "%ИдентификаторШага%", ИдентификаторШага);
		лкКомментарий = СтрЗаменить(лкКомментарий, "%Комментарий%", Комментарий);
		ЗаписьЖурналаРегистрации(
			лкИмяСобытия, // ИмяСобытия
			лкУровеньЖурналаРегистрации, // УровеньЖурналаРегистрации.*
			лкОбъектМетаданных, // ОбъектМетаданных
			Данные, // Данные
			лкКомментарий, // Комментарий
...
// Функция для обхода ошибки с ПодробноеПредставлениеОшибки, когда она не выводит исключения, сгенерированные через ВызватьИсключение.
//
// Параметры:
//  ИнформацияОбОшибке - ИнформацияОбОшибке - структура всех сообщений об ошибке.
//
// Возвращаемое значение:
//  Строка - подробное описание ошибки.
//
Функция ПредставлениеОшибки(ИнформацияОбОшибке) Экспорт

	Результат = "";

	ТекущаяИнформацияОбОшибке = ИнформацияОбОшибке;
	Пока (ТипЗнч(ТекущаяИнформацияОбОшибке) = Тип("ИнформацияОбОшибке")) Цикл
		ТекущаяОшибка = ТекущаяИнформацияОбОшибке.Описание + Символы.ПС +
						ТекущаяИнформацияОбОшибке.ИмяМодуля  + "(" + ТекущаяИнформацияОбОшибке.НомерСтроки + ")" + Символы.ПС +
						ТекущаяИнформацияОбОшибке.ИсходнаяСтрока;
		Если ПустаяСтрока(Результат) Тогда
			Результат = ТекущаяОшибка;
		Иначе
			Результат = Результат + Символы.ПС + Символы.ПС + ТекущаяОшибка;
		КонецЕсли;
		ТекущаяИнформацияОбОшибке = ТекущаяИнформацияОбОшибке.Причина;
	КонецЦикла;
...
//
Функция ПолучитьПредставлениеОбъектаХДТО(ОбъектХДТО, ФабрикаХДТО = Неопределено) Экспорт
	...
	Если ТипЗнч(ОбъектХДТО) = Тип("ОбъектXDTO") Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстСообщения = НСтр("ru='Ошибка при получении xml-представления объекта ХДТО:
                |%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'
                |;uk='Помилка при отриманні xml-представлення об''єкта ХДТО:
                |%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Отладка';uk='БІП:Новини.Налагодження'"), // ИмяСобытия
				НСтр("ru='Новости. Отладка. ПолучитьПредставлениеОбъектаХДТО';uk='Новини. Налагодження. ПолучитьПредставлениеОбъектаХДТО'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
...
//  КоличествоСтрок - Число  - какое количество строк необходимо прочитать.
//
// Возвращаемое значение:
//   Строка - первые несколько строк файла.
//
Функция ПолучитьСодержимоеТекстовогоФайла(ПолноеИмяФайла, КоличествоСтрок = 10) Экспорт
	...
	Попытка
		лкЧтениеТекста = Новый ЧтениеТекста;
		лкЧтениеТекста.Открыть(ПолноеИмяФайла, КодировкаТекста.UTF8, , , Ложь); // Открывать НЕ в монопольном режиме
		ДостигнутКонецФайла = Ложь;
		Пока (ЧислоСтрокДляСчитывания > 0) И (ДостигнутКонецФайла = Ложь) Цикл
			СтрокаИзФайла = лкЧтениеТекста.ПрочитатьСтроку();
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ТекстОшибки = НСтр("ru='При чтении файла с именем %ПолноеИмяФайла% произошла ошибка:
            |%ПодробноеПредставлениеОшибки%'
            |;uk='При читанні файлу з ім''ям %ПолноеИмяФайла% сталася помилка:
            |%ПодробноеПредставлениеОшибки%'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ПолноеИмяФайла%", ПолноеИмяФайла);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ПодробноеПредставлениеОшибки%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("ru='Запускается при первом запуске, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).';uk='Запускається при першому запуску, в загальних даних. Встановлення констант, завантаження стандартних значень, скидання регістрів відомостей (загальних даних).'");
	...
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Комментарий         = НСтр("ru='Запускается при первом запуске, для каждой области данных. Установка настроек пользователей (разделенных данных).';uk='Запускається при першому запуску, для кожної області даних. Встановлення настройок користувачів (розподілених даних).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.Комментарий         = НСтр("ru='Запускается при переходе на новую версию, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).';uk='Запускається при переході на нову версію, до загальних даних. Встановлення констант, завантаження стандартних значень, скидання регістрів відомостей (загальних даних).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ru='Запускается при переходе на любую новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';uk='Запускається при переході на будь-яку нову версію, для кожної області даних. Встановлення настройок користувачів (розподілених даних).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ru='Запускается при переходе на новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';uk='Запускається при переході на нову версію, для кожної області даних. Встановлення настройок користувачів (розподілених даних).'");
	Обработчик.РежимВыполнения     = "Оперативно";
...
Процедура ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск() Экспорт
	...
	ТекстСообщения = НСтр("ru='Начало процедуры обновления. Первый запуск (общие данные)
        |Время начала (мс): %ТНачало%
        |'
        |;uk='Початок процедури оновлення. Перший запуск (загальні дані)
        |Час початку (мс): %ТНачало%
        |'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПервыйЗапуск. Начало';uk='Новини. Оновлення ІБ. ОбщиеДанные. ПервыйЗапуск. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если НЕ ПустаяСтрока(Результат) Тогда
				ТекстСообщения = НСтр("ru='Обнаружены ошибки внедрения подсистемы ИнтернетПоддержкаПользователей.Новости.
                    ...
                    |%Результат%'
                    |;uk='Виявлені помилки впровадження підсистеми ИнтернетПоддержкаПользователей.Новости.
                    |Виявлені помилки впровадження:
                    |%Результат%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Результат%", Результат);
				ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
					НСтр("ru='Новости. Обновление ИБ. Проверка внедрения';uk='Новини. Оновлення ІБ. Перевірка впровадження'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения // Комментарий
			...
			ТекстСообщения = НСтр("ru='Начало процедуры обновления. Переход на версию %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()%
                |Время начала (мс): %ТНачало%
                |'
                |;uk='Початок процедури оновлення. Перехід на версію %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()%
                |Час початку (мс): %ТНачало%
                |'");
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПервыйЗапуск. Начало';uk='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПервыйЗапуск. Начало'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
			...
			ТекстСообщения = НСтр("ru='Конец процедуры обновления. Переход на версию %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()%
                ...
                |Длительность (мс): %ТДлительность%
                |'
                |;uk='Кінець процедури оновлення. Перехід на версію %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()%
                |Час початку (мс): %ТНачало%
                |Час закінчення (мс): %ТКонец%
                |Тривалість (мс): %ТДлительность%
                |'");
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПервыйЗапуск. Конец';uk='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПервыйЗапуск. Конец'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения // Комментарий
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Подсистема ИнтернетПоддержкаПользователей.Новости не существует. Обработка первого запуска не выполнена.';uk='Підсистема ИнтернетПоддержкаПользователей.Новости не існує. Обробка першого запуску не виконана.'");
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПервыйЗапуск. Конец';uk='Новини. Оновлення ІБ. ОбщиеДанные. ПервыйЗапуск. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
	...
	ТекстСообщения = НСтр("ru='Конец процедуры обновления. Первый запуск (общие данные)
        ...
        |Длительность (мс): %ТДлительность%
        |'
        |;uk='Кінець процедури оновлення. Перший запуск (загальні дані)
        |Час початку (мс): %ТНачало%
        |Час закінчення (мс): %ТКонец%
        |Тривалість (мс): %ТДлительность%
        |'");
...
Процедура ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск() Экспорт
	...
	ТекстСообщения = НСтр("ru='Начало процедуры обновления. Первый запуск (область данных)
        |Время начала (мс): %ТНачало%
        |'
        |;uk='Початок процедури оновлення. Перший запуск область даних)
        |Час початку (мс): %ТНачало%
        |'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск. Начало';uk='Новини. Оновлення ІБ. ОбластьДанных. ПервыйЗапуск. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() = Истина Тогда

		ТекстСообщения = НСтр("ru='Работа с новостями не поддерживается для внешнего пользователя. Обработчик обновления пропущен.';uk='Робота з новинами не підтримується для зовнішнього користувача. Обробник оновлення пропущено.'");
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск. Конец';uk='Новини. Оновлення ІБ. ОбластьДанных. ПервыйЗапуск. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
	...
	Если ЕстьОшибки = Ложь Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПервыйЗапуск. Начало';uk='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПервыйЗапуск. Начало'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПервыйЗапуск. Конец';uk='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПервыйЗапуск. Конец'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения // Комментарий
	...
	ТекстСообщения = НСтр("ru='Конец процедуры обновления. Первый запуск (область данных)
        ...
        |Длительность (мс): %ТДлительность%
        |'
        |;uk='Кінець процедури оновлення. Перший запуск область даних)
        |Час початку (мс): %ТНачало%
        |Час закінчення (мс): %ТКонец%
        |Тривалість (мс): %ТДлительность%
        |'");
...
Процедура ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию() Экспорт
	...
	ТекстСообщения = НСтр("ru='Начало процедуры обновления. Переход на версию %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (общие данные)
        |Время начала (мс): %ТНачало%
        |'
        |;uk='Початок процедури оновлення. Перехід на версію %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (загальні дані)
        |Час початку (мс): %ТНачало%
        |'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()%", ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки());
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию. Начало';uk='Новини. Оновлення ІБ. ОбщиеДанные. ПерейтиНаВерсию. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию. Начало';uk='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию. Начало'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию. Конец';uk='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию. Конец'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения // Комментарий
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Подсистема ИнтернетПоддержкаПользователей.Новости не существует. Обработка обновления не выполнена.';uk='Підсистема ИнтернетПоддержкаПользователей.Новости не існує. Обробка оновлення не виконана.'");
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию. Конец';uk='Новини. Оновлення ІБ. ОбщиеДанные. ПерейтиНаВерсию. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
	...
	ТекстСообщения = НСтр("ru='Конец процедуры обновления. Переход на версию %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (общие данные)
        ...
        |Длительность (мс): %ТДлительность%
        |'
        |;uk='Кінець процедури оновлення. Перехід на версію %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (загальні дані)
        |Дозволена робота з новинами = %лкРазрешенаРаботаСНовостями%
        |Час початку (мс): %ТНачало%
        |Час закінчення (мс): %ТКонец%
        |Тривалість (мс): %ТДлительность%
        |'");
...
Процедура ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию_2_1_4() Экспорт
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ. Начало';uk='Новини. Оновлення ІБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию_ХХХ. Начало';uk='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию_ХХХ. Начало'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию_ХХХ. Конец';uk='Новости. Обновление ИБ. ОбщиеДанные. Переопределяемый. ПерейтиНаВерсию_ХХХ. Конец'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения // Комментарий
	...
	Иначе
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ. Конец';uk='Новини. Оновлення ІБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
...
Процедура ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию() Экспорт
	...
	ТекстСообщения = НСтр("ru='Начало процедуры обновления. Переход на версию %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (область данных)
        |Время начала (мс): %ТНачало%
        |'
        |;uk='Початок процедури оновлення. Перехід на версію %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (область)
        |Час початку (мс): %ТНачало%
        |'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()%", ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки());
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТНачало%", Т0Начало);
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию. Начало';uk='Новини. Оновлення ІБ. ОбластьДанных. ПерейтиНаВерсию. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию. Конец';uk='Новини. Оновлення ІБ. ОбластьДанных. ПерейтиНаВерсию. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
	...
	Если ЕстьОшибки = Ложь Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию. Начало';uk='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию. Начало'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
			...
			ТекстСообщения = НСтр("ru='Конец процедуры обновления. Переход на версию %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (область данных)
                ...
                |Длительность (мс): %ТДлительность%
                |'
                |;uk='Кінець процедури оновлення. Перехід на версію %ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()% (область)
                |Час початку (мс): %ТНачало%
                |Час закінчення (мс): %ТКонец%
                |Тривалість (мс): %ТДлительность%
                |'");
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию. Конец';uk='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию. Конец'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения // Комментарий
...
Процедура ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию_2_1_4() Экспорт
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
		НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ. Начало';uk='Новини. Оновлення ІБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
			НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ. Конец';uk='Новини. Оновлення ІБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ. Кінець'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
	...
	Если ЕстьОшибки = Ложь Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию_ХХХ. Начало';uk='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию. Конец'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Обновление ИБ';uk='БІП:Новини.Оновлення ІБ'"), // ИмяСобытия
				НСтр("ru='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию_ХХХ. Конец';uk='Новости. Обновление ИБ. ОбластьДанных. Переопределяемый. ПерейтиНаВерсию_ХХХ. Конец'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				Т1Длительность, // Данные
				ТекстСообщения // Комментарий
...
Функция ПроверитьПравильностьВнедренияПодсистемы() Экспорт
		...
		Результат = Результат
			+ СтрШаблон(
				НСтр("ru='Ошибки настройки общего реквизита ОбластьДанныхОсновныеДанные:
                    ...
                    |
                    |'
                    |;uk='Помилки настройки загального реквізиту ОбластьДанныхОсновныеДанные:
                    |%1
                    |
                    |'"),
				РезультатЛокальный
		...
		Результат = Результат
			+ СтрШаблон(
				НСтр("ru='Ошибки настройки общего реквизита ОбластьДанныхВспомогательныеДанные:
                    ...
                    |
                    |'
                    |;uk='Помилки настройки загального реквізиту ОбластьДанныхВспомогательныеДанные:
                    |%1
                    |
                    |'"),
				РезультатЛокальный
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ru='Ошибки настройки прав для роли АдминистраторСистемы:';uk='Помилки настройки прав для ролі АдминистраторСистемы:'")
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ru='Ошибки настройки прав для роли ПолныеПрава:';uk='Помилки настройки прав для ролі ПолныеПрава:'")
			+ Символы.ПС + РезультатЛокальный
			+ Символы.ПС + Символы.ПС;
	КонецЕсли;
	...
	Если ВРег(ИмяКонфигурации) = ВРег("Unknown") Тогда
		РезультатЛокальный = НСтр("ru='Не настроен идентификатор конфигурации.
            ...
            |'
            |;uk='Не налаштований ідентифікатор конфігурації.
            |Для його настройки необхідно внести зміни в загальний модуль
            |ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,
            |для рядка умови ProductName.
            |В іншому випадку новини для цієї конфігурації приходити НЕ будуть.
            |'");
	КонецЕсли;
	...
	Если СписокЗапрещенныхСимволов.Количество() > 0 Тогда
		...
		РезультатЛокальный = РезультатЛокальный  + НСтр("ru='Неправильно настроен идентификатор конфигурации.
            ...
            |'
            |;uk='Неправильно налаштований ідентифікатор конфігурації.
            |Для його установки необхідно внести зміни в загальний модуль
            |ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,
            |для рядка умови ProductName.
            |Ідентифікатор конфігурації має складатися з англійських букв, цифр, знак мінуса і знака підкреслення.
            |'");
	КонецЕсли;

	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ru='Ошибки настройки кода конфигурации:';uk='Помилки настройки коду конфігурації:'")
			+ Символы.ПС + РезультатЛокальный
			+ Символы.ПС + Символы.ПС;
	КонецЕсли;
	...
	Если МассивСлов.Количество() <> 4 Тогда
		РезультатЛокальный = НСтр("ru='Неправильно настроена версия конфигурации (количество чисел отлично от 4).
            ...
            |;uk='Неправильно налаштована версія конфігурації (кількість чисел відмінно від 4).
            |В метаданих, в полі Версія повинні бути дані виду А. Б. В. Г (чотири числа, розділені крапкою).
            |Якщо з якихось причин версія метаданих задана в відмінному від А. Б. В. Г форматі,
            |то необхідно внести зміни у загальний модуль
            |ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,
            |для рядка умови ProductVersion.'")
	Иначе
		НайденныеНеправильныеСимволы = Новый Массив;
		ВсеЦифры = ОбработкаНовостейКлиентСервер.ПолучитьВсеЦифры();
		...
		Если НайденныеНеправильныеСимволы.Количество() > 0 Тогда
			РезультатЛокальный = НСтр("ru='Неправильно настроена версия конфигурации (есть неправильные символы).
                ...
                |;uk='Неправильно налаштована версія конфігурації (є неправильні символи).
                |В метаданих, в полі Версія повинні бути дані виду А. Б. В. Г (чотири числа, розділені крапкою).
                |Якщо з якихось причин версія метаданих задана в відмінному від А. Б. В. Г форматі,
                |то необхідно внести зміни у загальний модуль
                |ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,
                |для рядка умови ProductVersion.
                |Список неправильних символів: %НайденныеНеправильныеСимволы%'");
	...
	Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
		Результат = Результат
			+ НСтр("ru='Ошибки настройки версии конфигурации:';uk='Помилки настройки версії конфігурації:'")
			+ Символы.ПС + РезультатЛокальный
			+ Символы.ПС + Символы.ПС;
	КонецЕсли;
...
Процедура Проверка_ОбщиеРеквизиты(
	...
	Если НайденныйЭлемент <> Неопределено Тогда
		Если (НайденныйЭлемент.Использование = ПравильноеЗначениеИспользования)
				ИЛИ (
					(НайденныйЭлемент.Использование = Свойство_ОбъектАвто)
					И (лкОбщийРеквизит.АвтоИспользование = СоответствующееСвойствоОбщегоРеквизита)
				) Тогда
			// Допустимо, если:
		...
		Иначе
			ТекущийРезультат = НСтр("ru='Неправильно установлено использование для %ПолноеИмяМетаданных% в общем реквизите %ИмяОбщегоРеквизита%. Установлено: %НайденныйЭлемент.Использование%, ожидается: %ПравильноеЗначениеИспользования%.
                ...
                |;uk='Неправильно встановлено використання для %ПолноеИмяМетаданных% у загальному реквізиті %ИмяОбщегоРеквизита%. Встановлено: %НайденныйЭлемент.Использование%, очікується: %ПравильноеЗначениеИспользования%.
                |Також допустимо, що у %ПолноеИмяМетаданных% встановлено використання Авто, але тоді у загального реквізиту %ИмяОбщегоРеквизита% повинно бути встановлено Автовикористання у %ПравильноеЗначениеИспользования%.'");
		...
		КонецЕсли;
	Иначе
		ТекущийРезультат = НСтр("ru='Не найден требуемый объект метаданных: %ПолноеИмяМетаданных%';uk='Не знайшли потрібний об''єкт метаданих: %ПолноеИмяМетаданных%'");
		ТекущийРезультат = СтрЗаменить(
			ТекущийРезультат,
			"%ПолноеИмяМетаданных%",
			ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(лкОбъектМетаданных) + "." + лкОбъектМетаданных.Имя);
...
Процедура Проверка_Права(
	...
	Если ТекущееЗначениеПрава <> ПравильноеЗначениеПрава Тогда
		ТекущийРезультат = НСтр("ru='Неправильно установлено право %Право% в роли %Роль% для %ПолноеИмяМетаданных%. Установлено: %ТекущееЗначениеПрава%, ожидается: %ПравильноеЗначениеПрава%';uk='Неправильно встановлено право %Право% в ролі %Роль% для %ПолноеИмяМетаданных%. Встановлено: %ТекущееЗначениеПрава%, очікується: %ПравильноеЗначениеПрава%'");
		ТекущийРезультат = СтрЗаменить(
			ТекущийРезультат,
			"%ПолноеИмяМетаданных%",
			ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(лкОбъектМетаданных) + "." + лкОбъектМетаданных.Имя);
	...
	Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
		...
		Если (ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя()) Тогда
			...
			Если РольДоступна_ЧтениеНовостей
					...
					ИЛИ (
						РольДоступна_АдминистраторСистемы
						И РольДоступна_ПолныеПрава
					) Тогда
				...
				ТекстСообщения = НСтр("ru='Не рекомендуется давать возможность чтения новостей внешнему пользователю (%ТекущийВнешнийПользователь%)
                    ...
                    |'
                    |;uk='Не рекомендується давати можливість читання новин зовнішньому користувачеві (%ТекущийВнешнийПользователь%)
                    |Підключені наступні ролі:
                    |- ЧтениеНовостей - %ЧтениеНовостей%
                    |- РедактированиеНовостей - %РедактированиеНовостей%
                    |- АдминистраторСистемы - %АдминистраторСистемы%
                    |- ПолныеПрава - %ПолныеПрава%
                    |Це може привести до помилок у роботі з конфігурацією.
                    |'");
...
Процедура УстановкаПараметровСеанса(ИмяПараметра = Неопределено, УстановленныеПараметры = Неопределено) Экспорт
	...
	Результат = Новый Структура("ВерсияПлатформы, ВерсияПродукта, ИмяКонфигурации, АдресWSDLОпределенияВебСервисаInformation",
		"",
		"",
		"",
		"https://news.webits.1c.ru/data/v1/classificators?wsdl");
	...
	лкПараметрыОкруженияБИП_Новости = Новый ХранилищеЗначения(Результат, Новый СжатиеДанных(0));

	УстановитьПривилегированныйРежим(Истина);
		ПараметрыСеанса.ПараметрыОкруженияБИП_Новости = лкПараметрыОкруженияБИП_Новости;
	УстановитьПривилегированныйРежим(Ложь);
...
Процедура ПриНачалеРаботыСистемы() Экспорт
	...
	// 1. Проверять изменение версии платформы необходимо в нескольких случаях:
	...
	Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостямиТекущемуПользователю() = Истина Тогда
		...
		Если ОбщегоНазначенияПовтИсп.ЭтоРазделеннаяКонфигурация() = Истина Тогда
			Если ОбщегоНазначенияПовтИсп.РазделениеВключено() = Истина Тогда
				лкПроверитьИзменениеПлатформы = Ложь;
			Иначе
	...
	// 2. Внешним пользователям не рекомендуется пользоваться подсистемой чтения новостей.
