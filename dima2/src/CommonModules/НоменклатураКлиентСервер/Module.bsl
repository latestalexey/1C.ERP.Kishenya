...
Процедура ПересчитатьСтатусУказанияСерийПриОбработке(ПараметрыУказанияСерий, ТекущийСтатус, СерииУказаныПолностью, КоличествоСерий, ВариантОбеспечения = Неопределено) Экспорт
	...
	//++ НЕ УТ
	ЭтоМатериалыМаршрутногоЛиста = ПараметрыУказанияСерий.СкладскиеОперации.Найти(ПредопределенноеЗначение("Перечисление.СкладскиеОперации.РасходМатериаловПриВыполненииМаршрутныхЛистов")) <> Неопределено;
	//-- НЕ УТ
	
	ПрименятьСтатусМожноУказать = (ПараметрыУказанияСерий.ПодготовкаОрдера
									//++ НЕ УТ
									Или ЭтоМатериалыМаршрутногоЛиста
										И Не ПараметрыУказанияСерий.ФактОтбора
									//-- НЕ УТ
