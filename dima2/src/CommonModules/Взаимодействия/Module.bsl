	...
	Для Каждого Элемент Из МассивЗаменить Цикл
		...
		Исключение
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при замене предмета у %1: %2';uk='Помилка при заміні предмета у %1: %2'"), Элемент, ПредставлениеОшибки), , , , Отказ);
			Отказ = Истина;
			Возврат;
		КонецПопытки;
	КонецЦикла;
...
Процедура ПриПолученииСпискаШаблонов(Шаблоны) Экспорт
	...
	Шаблоны.Добавить("ОбновлениеСтатусовДоставкиSMS");
	Шаблоны.Добавить("ОтправкаSMS");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		ДелоРодитель.Представление  = НСтр("ru='Почта';uk='Пошта'");
...
Функция ПолучитьСписокДоступныхПоисков(ППДВключен, Параметры, ЭлементыФормы, ДляАдреснойКниги) Экспорт
	...
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы, "ПоEmail", НСтр("ru='По email';uk='По email'"), Адрес);
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы, "ПоДомену", НСтр("ru='По доменному имени';uk='За доменним ім''ям'"), ДоменныйАдрес);
	Если Не ДляАдреснойКниги Тогда
		ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы, "ПоТелефону", НСтр("ru='По телефону';uk='По телефону'"), Адрес);
	КонецЕсли;
	...
	Если ППДВключен Тогда
		ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы, "ПоСтроке",
			НСтр("ru='По строке';uk='По рядку'"), ВариантыПоискаПоСтроке);
	КонецЕсли;
	
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы, "НачинаетсяС", НСтр("ru='Начинается с';uk='Починається з'"), Представление);
...
Функция ОсуществитьПоискКонтактовПоСтроке(Форма,ДляАдреснойКниги = Ложь) Экспорт
	...
	Если ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() = РежимПолнотекстовогоПоиска.Запретить
		...
		Возврат НСтр("ru='Использование индекса полнотекстового поиска данных запрещено!';uk='Використання індексу повнотекстового пошуку даних заборонено!'");
	КонецЕсли;
	...
	Возврат ?(КоличествоНайденныхЭлементов < 101, "", НСтр("ru='Уточните параметры поиска. В списке отображены не все найденные контакты!';uk='Уточніть параметри пошуку. У списку відображені не всі знайдені контакти!'"));
...
Процедура СформироватьСписокВыбораТипВзаимодействияТолькоПочта(Элемент)
	...
	Элемент.СписокВыбора.Добавить("ВсеПисьма", НСтр("ru='Все письма';uk='Всі листи'"));
	Элемент.СписокВыбора.Добавить("ВходящиеПисьма", НСтр("ru='Входящие';uk='Вхідні'"));
	Элемент.СписокВыбора.Добавить("ПисьмаЧерновики", НСтр("ru='Черновики';uk='Чернетки'"));
	Элемент.СписокВыбора.Добавить("ИсходящиеПисьма", НСтр("ru='Исходящие';uk='Вихідні'"));
	Элемент.СписокВыбора.Добавить("Отправленные", НСтр("ru='Отправленные';uk='Відправлені'"));
	Элемент.СписокВыбора.Добавить("УдаленныеПисьма", НСтр("ru='Удаленные';uk='Вилучені'"));
...
Функция ПолучитьАдресаЭлектроннойПочтыДляМассиваКонтактов(МассивКонтактов,Группа = "") Экспорт
	...
	|	ТаблицаКонтакт.Наименование КАК Представление
	...
	Для каждого ЭлементМассиваОписания Из МассивОписанияТиповКонтактов Цикл
		...
		Иначе
			...
			|	ТаблицаКонтакт." + ЭлементМассиваОписания.ИмяРеквизитаПредставлениеКонтакта + "
			|ИЗ
	...
	|	КонтактыАдреса.Представление КАК Представление,
	|	&Группа
	|ИЗ
...
Процедура УстановитьЗаголовокФормыЭлектронногоПисьма(Форма) Экспорт
	...
	Если НЕ ОбъектПисьмо.Ссылка.Пустая() Тогда
		...
		ЗаголовокФормы = ?(ПустаяСтрока(ОбъектПисьмо.Тема), НСтр("ru='Письмо без темы';uk='Лист без теми'"), ОбъектПисьмо.Тема);
	...
	Иначе
		Если ТипЗнч(ОбъектПисьмо.Ссылка) = Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") Тогда
			Форма.Автозаголовок = Ложь;
			Форма.Заголовок = НСтр("ru='Исходящее письмо (создание)';uk='Вихідний лист (створення)'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Переносится хранение реквизитов ""Рассмотрено"" и ""РассмотретьПосле"" в регистр сведений ""ПредметыПапкиВзаимодействий"". Без выполнения обработчика значения данных реквизитов будут отображаться некорректно.';uk='Переноситься зберігання реквізитів ""Розглянуто"" і ""РозглянутиПісля"" в регістр відомостей ""ПредметыПапкиВзаимодействий"". Без виконання обробника значення даних реквізитів будуть відображатися некоректно.'");
	...
	Обработчик.Комментарий = НСтр("ru='Заполняется вспомогательный регистр сведений ""КонтактыВзаимодействий"". Без выполнения обработчика не будут корректно работать отборы по контактам взаимодействий.';uk='Заповнюється допоміжний регістр відомостей ""КонтактыВзаимодействий"". Без виконання обробника не будуть коректно працювати відбори за контактами взаємодій.'");
	...
	Обработчик.Комментарий = НСтр("ru='Выполняется первоначальный расчет количества нерассмотренных писем по папкам. Без выполнения обработчика не будет корректно выводится информация о количестве нерассмотренных писем по папкам.';uk='Виконується початковий розрахунок кількості нерозглянутих листів по папках. Без виконання обробника не буде коректно виводиться інформація про кількість нерозглянутих листів по папках.'");
	...
	Обработчик.Комментарий = НСтр("ru='Выполняется первоначальный расчет количества состояний предметов взаимодействий. Без выполнения обработчика не будет корректно выводится информация в панель навигации журнала взаимодействий по предметам.';uk='Виконується початковий розрахунок кількості станів предметів взаємодій. Без виконання обробника не буде коректно виводиться інформація в панель навігації журналу взаємодій з предметів.'");
	...
	Обработчик.Комментарий = НСтр("ru='Выполняется первоначальный расчет количества состояний контактов взаимодействий. Без выполнения обработчика не будет корректно выводится информация в панель навигации журнала взаимодействий по контактам.';uk='Виконується початковий розрахунок кількості станів контактів взаємодій. Без виконання обробника не буде коректно виводиться інформація в панель навігації журналу взаємодій за контактами.'");
...
Процедура ЗаполнитьСписокВыбораДляРассмотретьПосле(СписокВыбора) Экспорт
	...
	СписокВыбора.Добавить(15*60,    НСтр("ru='Через 15 мин.';uk='Через 15 хв.'"));
	СписокВыбора.Добавить(30*60,    НСтр("ru='Через 30 мин.';uk='Через 30 хв.'"));
	СписокВыбора.Добавить(60*60,    НСтр("ru='Через 1 час';uk='Через 1 годину'"));
	СписокВыбора.Добавить(3*60*60,  НСтр("ru='Через 3 часа';uk='Через 3 години'"));
	СписокВыбора.Добавить(24*60*60, НСтр("ru='Завтра';uk='Завтра'"));
...
Процедура ИнициализироватьФормуСпискаВзаимодействий(Форма) Экспорт
	...
	Если Форма.ТолькоПочта Тогда
		Форма.Заголовок = НСтр("ru='Электронная почта';uk='Електронна пошта'");
		...
		Форма.Команды.Предмет.Заголовок = НСтр("ru='Установить предмет переписки';uk='Встановити предмет листування'");
		Форма.Команды.Предмет.Подсказка = НСтр("ru='Установить предмет переписки';uk='Встановити предмет листування'");
		...
		Если Форма.Команды.Найти("ПредметСписок") <> Неопределено Тогда
			Форма.Команды.ПредметСписок.Заголовок = НСтр("ru='Установить предмет переписки';uk='Встановити предмет листування'");
			Форма.Команды.ПредметСписок.Подсказка = НСтр("ru='Установить предмет переписки';uk='Встановити предмет листування'");
		КонецЕсли;
	КонецЕсли;
...
Функция ОтправкаSMSПоДокументу(Документ) Экспорт
	...
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выполнены настройки отправки SMS.';uk='Не виконані настройки відправки SMS.'"),,"Объект");
...
Процедура ОтправкаSMS() Экспорт
	...
	ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), 
		...
		НСтр("ru='Начата регламентная отправка сообщений SMS';uk='Розпочато регламентну відправку повідомлень SMS'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
			ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), 
			УровеньЖурналаРегистрации.Ошибка, , ,
			НСтр("ru='Не выполнены настройки отправки SMS.';uk='Не виконані настройки відправки SMS.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		Возврат;
	КонецЕсли;
	...
	ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru='Закончена регламентная отправка сообщений SMS';uk='Закінчено регламентну відправка повідомлень SMS'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура ОбновлениеСтатусовДоставкиSMS() Экспорт
	...
	ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru='Начато регламентное обновление состояний сообщений SMS';uk='Розпочато регламентне оновлення станів повідомлень SMS'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	...
	ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru='Закончено регламентное обновление состояний сообщений SMS';uk='Закінчено регламентне оновлення станів повідомлень SMS'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура УстановитьРодителяУПапки(Папка, НовыйРодитель, НеЗаписыватьПапку = Ложь) Экспорт
	...
	Запрос.УстановитьПараметр("Удаленные", НСтр("ru='Удаленные';uk='Вилучені'"));
	...
	Если НЕ НовыйРодитель.Пустая()Тогда
		ЗначенияРеквизитовПапки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			...
			Если ЗначенияРеквизитовПапки <> Неопределено 
				...
				И ЗначенияРеквизитовПапки.Наименование = НСтр("ru='Удаленные';uk='Вилучені'") Тогда
...
Процедура ВыполнитьУдалениеПапкиЭлектронныхПисем(Папка, ОписаниеОшибки = "") Экспорт
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ru='При удалении папки произошла ошибка.
                              ...
                              |;uk='При вилученні папки сталася помилка.
                              |Вилучення папки не виконане.
                              |Додатковий опис:
                              |%ДополнительноеОписание%'");
...
Процедура УстановитьПараметрыЗапросаИменаПредопределенныеПапки(Запрос) Экспорт
	...
	Запрос.УстановитьПараметр("Черновики", НСтр("ru='Черновики';uk='Чернетки'"));
	Запрос.УстановитьПараметр("Отправленные", НСтр("ru='Отправленные';uk='Відправлені'"));
	Запрос.УстановитьПараметр("Исходящие", НСтр("ru='Исходящие';uk='Вихідні'"));
	Запрос.УстановитьПараметр("НежелательнаяПочта", НСтр("ru='Нежелательная почта';uk='Небажана пошта'"));
...
Процедура ОбработатьПодменюОтборПоТипуВзаимодействий(Форма) Экспорт
	...
	ШаблонЗаголовка = НСтр("ru='Тип взаимодействий: %1';uk='Тип взаємодій: %1'");
