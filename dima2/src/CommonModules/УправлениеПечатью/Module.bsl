...
Процедура ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, ИмяМакета, СинонимМакета, ТабличныйДокумент,
	...
	Если ОписаниеПечатнойФормы <> Неопределено Тогда
		...
		ОписаниеПечатнойФормы.ЭтоМногоязычнаяПечатнаяФорма = ЭтоМногоязычнаяПечатнаяФорма;
	КонецЕсли;
...
//   ПараметрыВывода    - Структура - структура параметров вывода ПФ
//
Функция МакетыИДанныеОбъектовДляПечати(Знач ИмяМенеджераПечати, Знач ИменаМакетов, Знач СоставДокументов, Знач ПараметрыВывода = Неопределено) Экспорт	
	...
	Если ПараметрыВывода = Неопределено Тогда
	    // Команды печати, не поддерживающие многоязычный вывод имеют сокращенный формат вывода
		МакетыИДанные = МенеджерОбъекта.ПолучитьДанныеПечати(СоставДокументов, МассивИменМакетов);	
	Иначе
		МакетыИДанные = МенеджерОбъекта.ПолучитьДанныеПечати(СоставДокументов, МассивИменМакетов, ПараметрыВывода);
	КонецЕсли;
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление = НСтр("ru='Макеты печатных форм';uk='Макети друкованих форм'");
...
Функция Разрешения()
	...
	Разрешения.Добавить(РаботаВБезопасномРежиме.РазрешениеНаИспользованиеВнешнейКомпоненты(
		"ОбщийМакет.КомпонентаПечатиQRКода", НСтр("ru='Печать QR кодов.';uk='Друк QR кодів.'")));
...
Функция СформироватьПечатныеФормы(Знач ИмяМенеджераПечати, Знач ИменаМакетов, Знач МассивОбъектов, Знач ПараметрыПечати, 
	ПараметрыВывода = Неопределено, ДопустимыеТипыОбъектовПечати = Неопределено) Экспорт
	...
	Для Каждого ИмяМакета Из ИменаМакетов Цикл
		...
		Для Каждого ОписаниеПечатнойФормы Из ВременнаяКоллекцияДляОднойПечатнойФормы Цикл
			ОбщегоНазначенияКлиентСервер.Проверить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не задано количество экземпляров для печатной формы ""%1"".';uk='Не задана кількість екземплярів для друкованої форми ""%1"".'"),
		...
		Если Отказ Тогда
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='При формировании печатной формы ""%1"" возникла ошибка. Обратитесь к администратору.';uk='При формуванні друкованої форми ""%1"" виникла помилка. Зверніться до адміністратора.'"), ИмяМакета);
...
Процедура СообщитьПечатнаяФормаНедоступна(Объект)
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Печать %1 не выполнена: выбранная печатная форма недоступна.';uk='Друк %1 не виконано: обрана друкована форма недоступна.'"), Объект);
