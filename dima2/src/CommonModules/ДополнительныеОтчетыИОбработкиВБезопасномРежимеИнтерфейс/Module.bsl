	...
	Если CLSID = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Разрешение на использование COM-класса ""%1"" не может быть предоставлено дополнительной обработке,
                ...
                |;uk='Дозвіл на використання COM-класу ""%1"" не може бути надано додатковії обробки,
                |працюючої в режимі зворотної сумісності з механізмом дозволів, реалізованих у версії БСП 2.1.3.
                |Для використання COM-класу потрібно переробити додаткову обробку для роботи без режиму сумісності.'"),
			ProgId);
