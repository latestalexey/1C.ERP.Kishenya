...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Обновление сведений о записанных версиях объектов. После выполнения обновления увеличится скорость записи версионируемых объектов.';uk='Оновлення відомостей про записані версії об''єктів. Після виконання оновлення збільшиться швидкість запису об''єктів, що версіонуються.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.2.4.13";
	Обработчик.Процедура = "ВерсионированиеОбъектов.ПеренестиНастройкиВерсионирования";
	Обработчик.Комментарий = НСтр("ru='Обновление настроек версионирования.';uk='Оновлення ностройок версіонування.'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Версионирование';uk='Версіонування'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось обновить сведения о версии №%1 объекта ""%2"" по причине:
                    |%3'
                    |;uk='Не вдалося оновити відомості про версії №%1 об''єкту ""%2"" з причини:
	...
	Если Выборка.Количество() > 0 Тогда
		Если ОбработаноЗаписей = 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Процедуре ОбновитьСведенияОВерсияхОбъектов не удалось обработать некоторые записи регистра сведений ВерсииОбъектов (пропущены): %1';uk='Процедурою ОбновитьСведенияОВерсияхОбъектов не вдалося обробити деякі запису регістра відомостей ВерсииОбъектов (пропущені): %1'"), 
					Выборка.Количество());
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Подсказка = НСтр("ru='Устаревших версий: %1 (%2)';uk='Застарілих версій: %1 (%2)'");
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ru='Устаревшие версии объектов';uk='Застарілі версії об''єктів'");
		...
		Если Не Объект.ЭтоНовый() И (ИзмененаПроведенность И СведенияОВерсииОбъекта.НомерВерсии > 1 Или ВерсияОтличаетсяОтРанееЗаписанной(Объект)) Тогда
			...
			Если СведенияОВерсииОбъекта.НомерВерсии = 1 Тогда
				Если ОбъектВерсионируется(Объект.Ссылка, Ложь) Тогда
					...
					ПараметрыВерсии.Вставить("Комментарий", НСтр("ru='Версия создана по уже имеющемуся объекту';uk='Версія створена за вже наявним об''єктом'"));
...
Процедура ПроверитьНаличиеПравНаИзменениеОбъекта(ОбъектМетаданных)
	...
	Если Не ПривилегированныйРежим() И Не ПравоДоступа("Изменение", ОбъектМетаданных)Тогда
		ТекстСообщения = НСтр("ru='Недостаточно прав на изменение ""%1"".';uk='Недостатньо прав на зміну ""%1"".'");
...
Функция ОтчетПоВерсииОбъекта(СсылкаНаОбъект, Знач ВерсияОбъекта = Неопределено, ПользовательскийНомерВерсии = Неопределено) Экспорт
	...
	Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='№ %1 / (%2) / %3';uk='№ %1 / (%2) / %3'"), ПользовательскийНомерВерсии,
...
Функция ВосстановитьОбъектПоXML(ДанныеОбъекта, ТекстСообщенияОбОшибке = "") Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Версионирование';uk='Версіонування'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		ТекстСообщенияОбОшибке = НСтр("ru='Не удалось перейти на выбранную версию.
                                            ...
                                            |;uk='Не вдалося перейти на обрану версію.
                                            |Можлива причина: версія об''єкта була записана в іншій версії програми.
                                            |Технічна інформація про помилку: %1'");
...
Функция СведенияОВерсииОбъекта(Знач Ссылка, Знач НомерВерсии) Экспорт
	СообщениеНеУдалосьПолучитьВерсию = НСтр("ru='Не удалось получить предыдущую версию объекта.';uk='Не вдалося одержати попередню версію об''єкта.'");
	...
	Если Результат.ВерсияОбъекта = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Выбранная версия объекта отсутствует в программе.';uk='Обрана версія об''єкта відсутня в програмі.'");
	КонецЕсли;
...
Функция РазмерДанныхСтрокой(Знач РазмерДанных)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 %2';uk='%1 %2'"), РазмерДанных, ЕдиницаИзмерения);
...
Процедура ПриПолученииДанныхОтПодчиненного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Отправитель) Экспорт
	...
	Если ТипЗнч(ЭлементДанных) = Тип("РегистрСведенийНаборЗаписей.ВерсииОбъектов") И ЭлементДанных.Количество() > 0 Тогда
		...
		Если НомерВерсииСКоллизией = Неопределено Тогда
			...
			Запись.Комментарий = НСтр("ru='Отклоненная версия (автоматическое разрешение конфликта).';uk='Відхилена версія (автоматичний дозвіл конфлікту).'");
			НаборЗаписей.Записать();
...
Функция ПерейтиНаВерсиюСервер(Ссылка, НомерВерсии, ТекстСообщенияОбОшибке, ОтменаПроведения = Ложь)
	...
	Объект.ДополнительныеСвойства.Вставить("ВерсионированиеОбъектовКомментарийКВерсии",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выполнен переход к версии №%1 от %2';uk='Виконано перехід до версії №%1 від %2'"),
