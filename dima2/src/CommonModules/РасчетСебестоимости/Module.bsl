...
Процедура РассчитатьВсе(Дата, ПредварительныйРасчет, Организация = Неопределено, РегламентноеЗадание = Ложь, ПараметрыОтладки = Неопределено) Экспорт
	...
	МассивДляРасчета = СвязиОрганизацийПоСхемеИнтеркампани(Дата, МассивОрганизаций, Истина);
	...
	Для Каждого ГруппаДляРасчета Из МассивДляРасчета Цикл
...
Процедура ИнициализироватьРегистры(ПараметрыРасчета)
	...
	ПараметрыРасчета.Вставить("РегистрыСРасчетнымиЗаписями",
		Новый Структура("СебестоимостьТоваров, ВыручкаИСебестоимостьПродаж", Истина, Истина));
...
Функция РассчитатьСебестоимостьПоГруппеОрганизаций(Дата, МассивОрганизаций, ДокументыРасчетаСебестоимости, 
	...
	Иначе
		...
		Если НЕ ПараметрыРасчета.ПредварительныйРасчет Тогда
			// Этап 3.9
			// Формирует временную таблицу ВтСтоимостиНезавершенногоПроизводства
		...
		Если НЕ ПараметрыРасчета.ПредварительныйРасчет Тогда
			// Этап 3.13
			// Формирует временную таблицу ВтСтоимостиНезавершенногоПроизводстваРегл
		...
		Если НЕ ПараметрыРасчета.ПредварительныйРасчет Тогда
			...
			// - ПрочиеРасходы
			// - ПрочиеРасходыНезавершенногоПроизводства
			СкорректироватьСтоимостьСписанияНезавершенногоПроизводства(ПараметрыРасчета);
	...
	Если НЕ ПараметрыРасчета.ПредварительныйРасчет Тогда
		...
		ЗарегистрироватьКОтражению(ПараметрыРасчета);
		//++ НЕ УТКА
		// Этап 4.5
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли СтрокаОстатка.Регистратор = Выборка.Регистратор
			...
			Тогда
				...
				СуммыРаспределения.НДСРеглКорректировкаСтоимости = 0;
				СуммыРаспределения.СтоимостьРеглБезНДС = 0;
...
Процедура СкорректироватьСтоимостьСписанияНезавершенногоПроизводстваПоФИФОСкользящая(ПараметрыРасчета)
	...
	ПротоколРасчета_НачалоЭтапаРасчета(ПараметрыРасчета, "СкорректироватьСтоимостьСписанияНезавершенногоПроизводстваПоФИФОСкользящая");
	...
	|		ПО ДД.ВидЗапасов.Назначение = СпрНазначения.Ссылка
	|ГДЕ
	...
	|	НЕОПРЕДЕЛЕНО КАК ХозяйственнаяОперация,
	|	СУММА(ВЫБОР
	|			КОГДА ДД.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) ТОГДА ДД.ДоляСтоимости
	...
	|	НЕОПРЕДЕЛЕНО КАК ПериодИсточника,
	|	НЕОПРЕДЕЛЕНО КАК ХозяйственнаяОперация,
	|	СУММА(Таб.ДоляСтоимости) КАК ДоляСтоимости,
	...
	|	СУММА(Таб.СтоимостьРеглБезНДС) КАК СтоимостьРеглБезНДС,
	|	СУММА(Таб.НДСРегл) КАК НДСРегл,
	...
	|		И ДД.СтатьяРасходовСписания.ВариантРаспределенияРасходов =
	...
	|Организация, СтатьяРасходов, АналитикаРасходов, ИсходноеПодразделение, КорПодразделение, ДокументДвижения, Период,
	|НалоговоеНазначение, 
	|ДоляСтоимости, Стоимость, СтоимостьБезНДС, СуммаДопРасходов, СуммаДопРасходовБезНДС,
	|СтоимостьРеглБезНДС, НДСРегл, 
	|СтоимостьРегл, ПостояннаяРазница, ВременнаяРазница");
	...
	|Стоимость, СтоимостьБезНДС, СуммаДопРасходов, СуммаДопРасходовБезНДС, СтоимостьРегл, СтоимостьРеглБезНДС, НДСРегл, ПостояннаяРазница, ВременнаяРазница");
		...
		Если Выборка.Порядок = 1 Тогда
			...
			Если Выборка.ВидДвижения = ВидДвиженияНакопления.Расход Тогда
				Если СтрокаПрихода.Организация <> Выборка.Организация
				 ИЛИ СтрокаПрихода.НалоговоеНазначение <> Выборка.НалоговоеНазначение 
				 ...
				 ИЛИ СтрокаПрихода.ИсходноеПодразделение <> Выборка.ИсходноеПодразделение Тогда
					...
					СтрокаПрихода.СтоимостьРеглБезНДС 	 = 0;
					СтрокаПрихода.НДСРегл 		         = 0;
			...
			ИначеЕсли Выборка.ВидДвижения = ВидДвиженияНакопления.Приход Тогда
				Если СтрокаПрихода.Организация = Выборка.Организация
				 И СтрокаПрихода.НалоговоеНазначение = Выборка.НалоговоеНазначение 
				 ...
				 И СтрокаПрихода.ИсходноеПодразделение = Выборка.ИсходноеПодразделение Тогда
					...
					СтрокаОстатка.СтоимостьРеглБезНДС 	 = СтрокаОстатка.СтоимостьРеглБезНДС - СтрокаПрихода.СтоимостьРеглБезНДС;
					СтрокаОстатка.НДСРегл 	         	 = СтрокаОстатка.НДСРегл - СтрокаПрихода.НДСРегл;
		...
		ИначеЕсли СтрокаОстатка.Регистратор = Выборка.Регистратор
		 ...
		 И СтрокаОстатка.ИсходноеПодразделение = Выборка.ИсходноеПодразделение Тогда
			...
			Если СтрокаОстатка.ДоляСтоимости <> 0 Тогда
				...
				Суммы.СтоимостьРеглБезНДС 	 = Окр(Выборка.ДоляСтоимости * СтрокаОстатка.СтоимостьРеглБезНДС / СтрокаОстатка.ДоляСтоимости, 2, 1);
				Суммы.НДСРегл 		         = Окр(Выборка.ДоляСтоимости * СтрокаОстатка.НДСРегл / СтрокаОстатка.ДоляСтоимости, 2, 1);
			...
			СтрокаОстатка.СтоимостьРеглБезНДС 	 = СтрокаОстатка.СтоимостьРеглБезНДС - Суммы.СтоимостьРеглБезНДС;
			СтрокаОстатка.НДСРегл 		         = СтрокаОстатка.НДСРегл - Суммы.НДСРегл;
			...
			Если Выборка.ВидДвижения = ВидДвиженияНакопления.Расход Тогда
				...
				СтрокаПрихода.СтоимостьРеглБезНДС 	 = СтрокаПрихода.СтоимостьРеглБезНДС + Суммы.СтоимостьРеглБезНДС;
				СтрокаПрихода.НДСРегл 		         = СтрокаПрихода.НДСРегл + Суммы.НДСРегл;
			...
			ЕстьНенулеваяСумма =
				...
				ИЛИ Суммы.СтоимостьРеглБезНДС <> 0 ИЛИ Суммы.НДСРегл <> 0 
				ИЛИ Суммы.СтоимостьРегл <> 0 ИЛИ Суммы.ПостояннаяРазница <> 0 ИЛИ Суммы.ВременнаяРазница <> 0;
			
			Если ЕстьНенулеваяСумма Тогда
				
				Если Выборка.ВидДвижения = ВидДвиженияНакопления.Расход Тогда
					НалоговоеНазначениеВыпуска = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.ДокументДвижения, "НалоговоеНазначение");
				КонецЕсли; 
				
				СформироватьДвиженияПрочиеРасходыНезавершенногоПроизводства(ПараметрыРасчета, Выборка, Суммы, НалоговоеНазначениеВыпуска);
				Если Выборка.ВидДвижения = ВидДвиженияНакопления.Расход Тогда
				...
				Если Выборка.ВидДвижения = ВидДвиженияНакопления.Расход
				 И ЗначениеЗаполнено(Выборка.РазделУчета) И НЕ Выборка.ЭтоЗаказДавальца	Тогда
					...
					СформироватьДвиженияСебестоимостьТоваровСписаниеНЗП(ПараметрыРасчета, Выборка, Суммы, НалоговоеНазначениеВыпуска);
...
Процедура СкорректироватьСтоимостьПрочегоСписанияНезавершенногоПроизводстваПоФИФОСкользящая(ПараметрыРасчета)
	...
	ПротоколРасчета_НачалоЭтапаРасчета(ПараметрыРасчета, "СкорректироватьСтоимостьПрочегоСписанияНезавершенногоПроизводстваПоФИФОСкользящая");
	...
	|		ДД.СтоимостьБезНДС,
	|		ДД.СтоимостьРегл,
	|		ДД.СтоимостьРегл КАК СтоимостьРеглБезНДС,
...
Процедура ЗарегистрироватьСтоимостьФИФО(ПараметрыРасчета)
	...
	ПротоколРасчета_КэшированиеСформированныхДвижений(
		ПараметрыРасчета,
		"СтоимостьТоваров",
		РазмерВременнойТаблицы(ПараметрыРасчета, "ВТКэшСтоимостьТоваров"));
...
Процедура РаспределитьПрочиеРасходыНезавершенногоПроизводства(ПараметрыРасчета)
	...
	ПротоколРасчета_НачалоЭтапаРасчета(ПараметрыРасчета, "РаспределитьПрочиеРасходыНезавершенногоПроизводства");
	...
	|	Распределение.АналитикаРасходов КАК АналитикаРасходов
	|
	...
	|	Расходы.Регистратор КАК АналитикаУчетаНоменклатуры,
	...
	|		Расходы.Организация,
	|		Расходы.Регистратор,
	|		Расходы.ДоляСтоимости КАК ДоляСтоимостиПриход
	|	ИЗ
	...
	|		Строки.ДоляСтоимости КАК ДоляСтоимостиПриход
	|	ИЗ
	...
	ПротоколРасчета_НачалоФормированиеВременнойТаблицы(ПараметрыРасчета, "ВтНезавершенноеПроизводство");
	Запрос.Выполнить();
	ПротоколРасчета_ОкончаниеФормированиеВременнойТаблицы(ПараметрыРасчета);
...
Процедура СформироватьУзлыКорректировкиСписанияСтоимости(ПараметрыРасчета)
	...
	|
	|ОБЪЕДИНИТЬ ВСЕ
	// Данные по прочим расходам незавершенного производства.
	|	ВЫБРАТЬ
	|		Таблица.Организация                КАК Организация,
	|		Таблица.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	|		Таблица.РазделУчета                КАК РазделУчета,
...
Процедура ЗарегистрироватьСтоимость(ПараметрыРасчета, РегистрацияСтоимостиРегл)
	...
	// Регистратором этого регистра сведений является только документ расчета себестоимости,
	// поэтому с этим кэшем работаем по "индивидуальной" схеме, без использования механизма кэширования движений.
	НачальныйРазмерТаблицы = РазмерВременнойТаблицы(ПараметрыРасчета, "ВТКэшСтоимостьТоваров");
	...
	ПротоколРасчета_КэшированиеСформированныхДвижений(
		ПараметрыРасчета,
		"СтоимостьТоваров",
		РазмерВременнойТаблицы(ПараметрыРасчета, "ВТКэшСтоимостьТоваров") - НачальныйРазмерТаблицы);
...
Процедура СтоимостьНезавершенногоПроизводства(ПараметрыРасчета)
	...
	ПротоколРасчета_НачалоЭтапаРасчета(ПараметрыРасчета, "СтоимостьНезавершенногоПроизводства");
	...
	Если ПараметрыРасчета.КоличествоУзлов <> 0 Тогда
		...
		|	ТаблицаРешений.СтоимостьРеглБезНДС КАК СтоимостьРеглБезНДС,
		|	ТаблицаРешений.НДСРегл КАК НДСРегл,
	...
	ПротоколРасчета_НачалоФормированиеВременнойТаблицы(ПараметрыРасчета, "ВтСтоимостиНезавершенногоПроизводства");
	Запрос.Выполнить();
	ПротоколРасчета_ОкончаниеФормированиеВременнойТаблицы(ПараметрыРасчета);
...
Процедура СтоимостьНезавершенногоПроизводстваРегл(ПараметрыРасчета)
	...
	ПротоколРасчета_НачалоЭтапаРасчета(ПараметрыРасчета, "СтоимостьНезавершенногоПроизводстваРегл");
	...
	ПротоколРасчета_НачалоФормированиеВременнойТаблицы(ПараметрыРасчета, "ВтСтоимостиНезавершенногоПроизводстваРегл");
	Запрос.Выполнить();
	ПротоколРасчета_ОкончаниеФормированиеВременнойТаблицы(ПараметрыРасчета);
...
// Этап 3.15
Процедура СкорректироватьСтоимостьСписанияНезавершенногоПроизводства(ПараметрыРасчета)
	
	ПротоколРасчета_НачалоЭтапаРасчета(ПараметрыРасчета, "СкорректироватьСтоимостьСписанияНезавершенногоПроизводства");
	...
	|	(ВЫБОР
	|		КОГДА Движения.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) ТОГДА Движения.Регистратор
	|		ИНАЧЕ Движения.ДокументИсточник КОНЕЦ) КАК ДокументИсточник,
	...
	|		Документ.РаспределениеПрочихЗатрат КАК Распределение
	|	ПО
	|		Движения.Регистратор = Распределение.Ссылка
	|ГДЕ
	...
	|		ИЛИ Движения.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	...
	|	Движения.АналитикаУчетаПродукции,
	|	Движения.Продукция,
	|	Движения.ХарактеристикаПродукции,
	|	Движения.ГруппаПродукции,
	|	(ВЫБОР
	|		КОГДА Движения.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) ТОГДА Движения.Регистратор
	|		ИНАЧЕ Движения.ДокументИсточник КОНЕЦ),
	...
	|	ВЫРАЗИТЬ(
	|		ТаблицаКорректировки.Количество
	|			* ЕСТЬNULL(Стоимости.ПостояннаяРазница, 0) КАК ЧИСЛО(15,2)) КАК ПостояннаяРазница,
	|
	|	ВЫРАЗИТЬ(
	|		ТаблицаКорректировки.Количество
	|			* ЕСТЬNULL(Стоимости.ВременнаяРазница, 0) КАК ЧИСЛО(15,2)) КАК ВременнаяРазница,
	|
	|
	|	ВЫРАЗИТЬ(
	|		ТаблицаКорректировки.Количество
	|			* ЕСТЬNULL(СтоимостиРегл.СтоимостьРеглБезНДС, 0) КАК ЧИСЛО(15,2)) КАК СтоимостьРеглБезНДС,
	|
	|	ВЫРАЗИТЬ(
	|		ТаблицаКорректировки.Количество
	|			* ЕСТЬNULL(СтоимостиРегл.НДСРегл, 0) КАК ЧИСЛО(15,2)) КАК НДСРегл,
	|
	|	ВЫРАЗИТЬ(
	|		ТаблицаКорректировки.Количество
	...
	|	ПО
	|		ТаблицаКорректировки.ВидЗапасов.Назначение = СпрНазначения.Ссылка
	|ГДЕ
	|	ТаблицаКорректировки.Количество <> 0	
	|
	|УПОРЯДОЧИТЬ ПО
	|	ТаблицаКорректировки.Организация,
	|	ТаблицаКорректировки.НалоговоеНазначение,
	...
	СтрокаОстатка = Новый Структура("Организация, Подразделение, ЗаказНаПроизводство, КодСтрокиПродукция, Этап,
	|СтатьяКалькуляции, СтатьяРасходов, АналитикаРасходов, ГруппаПродукции, ДокументИсточник, Количество,
	|НалоговоеНазначение,
	|Стоимость, СтоимостьБезНДС, СуммаДопРасходов, СуммаДопРасходовБезНДС, СтоимостьРегл, СтоимостьРеглБезНДС, НДСРегл, ПостояннаяРазница, ВременнаяРазница");
	
	Суммы = Новый Структура("Стоимость, СтоимостьБезНДС, СуммаДопРасходов, СуммаДопРасходовБезНДС, СтоимостьРегл, СтоимостьРеглБезНДС, НДСРегл,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтрокаОстатка.Организация <> Выборка.Организация
		 ИЛИ СтрокаОстатка.НалоговоеНазначение <> Выборка.НалоговоеНазначение
		...
		Иначе
			...
			Если Выборка.ВидДвижения = ВидДвиженияНакопления.Приход Тогда
				...
				СтрокаОстатка.Стоимость = СтрокаОстатка.Стоимость + Выборка.Стоимость;
				СтрокаОстатка.СтоимостьБезНДС = СтрокаОстатка.СтоимостьБезНДС + Выборка.СтоимостьБезНДС;
				СтрокаОстатка.СуммаДопРасходов = СтрокаОстатка.СуммаДопРасходов + Выборка.СуммаДопРасходов;
				СтрокаОстатка.СуммаДопРасходовБезНДС = СтрокаОстатка.СуммаДопРасходовБезНДС + Выборка.СуммаДопРасходовБезНДС;
				СтрокаОстатка.СтоимостьРегл = СтрокаОстатка.СтоимостьРегл + Выборка.СтоимостьРегл;
				СтрокаОстатка.СтоимостьРеглБезНДС = СтрокаОстатка.СтоимостьРеглБезНДС + Выборка.СтоимостьРеглБезНДС;
				СтрокаОстатка.НДСРегл = СтрокаОстатка.НДСРегл + Выборка.НДСРегл;
				СтрокаОстатка.ПостояннаяРазница = СтрокаОстатка.ПостояннаяРазница + Выборка.ПостояннаяРазница;
				СтрокаОстатка.ВременнаяРазница = СтрокаОстатка.ВременнаяРазница + Выборка.ВременнаяРазница;
				
				ЗаполнитьЗначенияСвойств(Суммы, Выборка);
				СформироватьДвиженияПрочиеРасходыНезавершенногоПроизводства(ПараметрыРасчета, Выборка, Суммы, Неопределено);
				
				ЗаполнитьЗначенияСвойств(Суммы, СтрокаОстатка);
				
			Иначе
				
				Суммы.Стоимость = ?(СтрокаОстатка.Количество <> 0, Окр(Выборка.Количество * СтрокаОстатка.Стоимость / СтрокаОстатка.Количество, 2, 1), 0);
				Суммы.СтоимостьБезНДС = ?(СтрокаОстатка.Количество <> 0, Окр(Выборка.Количество * СтрокаОстатка.СтоимостьБезНДС / СтрокаОстатка.Количество, 2, 1), 0);
				Суммы.СуммаДопРасходов = ?(СтрокаОстатка.Количество <> 0, Окр(Выборка.Количество * СтрокаОстатка.СуммаДопРасходов / СтрокаОстатка.Количество, 2, 1), 0);
				Суммы.СуммаДопРасходовБезНДС = ?(СтрокаОстатка.Количество <> 0, Окр(Выборка.Количество * СтрокаОстатка.СуммаДопРасходовБезНДС / СтрокаОстатка.Количество, 2, 1), 0);
				Суммы.СтоимостьРегл = ?(СтрокаОстатка.Количество <> 0, Окр(Выборка.Количество * СтрокаОстатка.СтоимостьРегл / СтрокаОстатка.Количество, 2, 1), 0);
				Суммы.СтоимостьРеглБезНДС = ?(СтрокаОстатка.Количество <> 0, Окр(Выборка.Количество * СтрокаОстатка.СтоимостьРеглБезНДС / СтрокаОстатка.Количество, 2, 1), 0);
				Суммы.НДСРегл = ?(СтрокаОстатка.Количество <> 0, Окр(Выборка.Количество * СтрокаОстатка.НДСРегл / СтрокаОстатка.Количество, 2, 1), 0);
				Суммы.ПостояннаяРазница = ?(СтрокаОстатка.Количество <> 0, Окр(Выборка.Количество * СтрокаОстатка.ПостояннаяРазница / СтрокаОстатка.Количество, 2, 1), 0);
				Суммы.ВременнаяРазница = ?(СтрокаОстатка.Количество <> 0, Окр(Выборка.Количество * СтрокаОстатка.ВременнаяРазница / СтрокаОстатка.Количество, 2, 1), 0);
...
Процедура СкорректироватьСтоимостьПрочегоСписанияНезавершенногоПроизводства(ПараметрыРасчета)
	...
	ПротоколРасчета_НачалоЭтапаРасчета(ПараметрыРасчета, "СкорректироватьСтоимостьПрочегоСписанияНезавершенногоПроизводства");
...
Процедура СписатьОшибкиОкругленияРасчетаСебестоимости(ПараметрыРасчета)
	...
	//++ НЕ УТ
	|		И Себестоимость.ПостояннаяРазница МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	|		И Себестоимость.ВременнаяРазница МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	//-- НЕ УТ
	...
	|
	|	ВЫБОР КОГДА ПрочиеРасходы.Сумма МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	|		ТОГДА ПрочиеРасходы.Сумма
	|		ИНАЧЕ 0
	|	КОНЕЦ										КАК Сумма,
	|	ВЫБОР КОГДА ПрочиеРасходы.СуммаБезНДС МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	|		ТОГДА ПрочиеРасходы.СуммаБезНДС
	|		ИНАЧЕ 0
	|	КОНЕЦ										КАК СуммаБезНДС,
	|	ВЫБОР КОГДА ПрочиеРасходы.СуммаРегл МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	|		ТОГДА ПрочиеРасходы.СуммаРегл
	|		ИНАЧЕ 0
	|	КОНЕЦ										КАК СуммаРегл,
	|	ВЫБОР КОГДА ПрочиеРасходы.СуммаРеглБезНДС МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	|		ТОГДА ПрочиеРасходы.СуммаРеглБезНДС
	|		ИНАЧЕ 0
	|	КОНЕЦ										КАК СуммаРеглБезНДС,
	|	ВЫБОР КОГДА ПрочиеРасходы.НДСРегл МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	|		ТОГДА ПрочиеРасходы.НДСРегл
	|		ИНАЧЕ 0
	|	КОНЕЦ										КАК НДСРегл,
	|	ВЫБОР КОГДА ПрочиеРасходы.ПостояннаяРазница МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	|		ТОГДА ПрочиеРасходы.ПостояннаяРазница
	|		ИНАЧЕ 0
	|	КОНЕЦ										КАК ПостояннаяРазница,
	|	ВЫБОР КОГДА ПрочиеРасходы.ВременнаяРазница МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	...
	|	И (ПрочиеРасходы.Сумма МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	|		ИЛИ ПрочиеРасходы.СуммаБезНДС МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	|		ИЛИ ПрочиеРасходы.СуммаРегл МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	|		ИЛИ ПрочиеРасходы.СуммаРеглБезНДС МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	|		ИЛИ ПрочиеРасходы.НДСРегл МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	|		ИЛИ ПрочиеРасходы.ПостояннаяРазница МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности
	|		ИЛИ ПрочиеРасходы.ВременнаяРазница МЕЖДУ -&ЗначениеПогрешности И &ЗначениеПогрешности)
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
	|ВЫБРАТЬ
...
// Этап 4.4
Процедура ЗарегистрироватьКОтражению(ПараметрыРасчета)
	
	ПротоколРасчета_НачалоЭтапаРасчета(ПараметрыРасчета, "ЗарегистрироватьКОтражению");
	...
	|	ГДЕ
	|		НЕ(ИзмененныеДокументы.Регистратор ССЫЛКА Документ.РасчетСебестоимостиТоваров
	|			И ВЫРАЗИТЬ(ИзмененныеДокументы.Регистратор КАК Документ.РасчетСебестоимостиТоваров).Организация = ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация))) КАК ДокументыКОтражению
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Регистратор
	|;
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДокументыКОтражению КАК ВТДокументыКОтражению
	|		ПО ОтражениеДокументовВРеглУчете.Регистратор = ВТДокументыКОтражению.Регистратор
	|			И (ОтражениеДокументовВРеглУчете.Организация В(&МассивОрганизаций))
	|			И (ОтражениеДокументовВРеглУчете.ДатаОтражения МЕЖДУ &НачалоПериода И &КонецПериода)
	|			И (ОтражениеДокументовВРеглУчете.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтраженияДокументовВРеглУчете.ОтраженоВРеглУчете))
	|			И (ОтражениеДокументовВРеглУчете.Активность)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Документ,
	|	Организация
	...
	ПротоколРасчета_НачалоФормированиеВременнойТаблицы(ПараметрыРасчета, "ДокументыКОтражению");
	РезультатЗапроса = Запрос.Выполнить();
	ПротоколРасчета_ОкончаниеФормированиеВременнойТаблицы(ПараметрыРасчета);
	
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Если НЕ ЗначениеЗаполнено(ПараметрыРасчета.РазмерНабораЗаписей) Тогда
	...
	УничтожитьВременныеТаблицы(
		ПараметрыРасчета.МенеджерВременныхТаблиц,
		"ВТВнеоборотныеАктивы, ВТДокументыКОтражению, ДокументыКОтражению");
...
Процедура ЗарегистрироватьКОтражениюВМеждународномУчете(ПараметрыРасчета)
	...
	ПротоколРасчета_НачалоЭтапаРасчета(ПараметрыРасчета, "ЗарегистрироватьКОтражениюВМеждународномУчете");
	...
	|
	|ВЫБРАТЬ
	|	МАКСИМУМ(ИсходнаяТаблица.Период) КАК Период,
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДокументыКОтражению КАК ВТДокументыКОтражению
	|			ПО ДанныеРегистра.Регистратор = ВТДокументыКОтражению.Регистратор
	|				И (ДанныеРегистра.Активность)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ДвиженияНоменклатураДоходыРасходы.Период,
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Иначе
			...
			МеждународныйУчетПроведениеСервер.ЗарегистрироватьДокументыКОтражениюВМеждународномУчете(Запрос);
		КонецЕсли;
	...
	УничтожитьВременныеТаблицы(
		ПараметрыРасчета.МенеджерВременныхТаблиц,
		"ВТДокументыКОтражению, ДокументыКОтражению");
...
Функция ТекстЗапросаДвиженияСтоимости()
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// Распределение постатейных расходов на выпуск продукции
	|	ВЫБРАТЬ
	|		УзлыКорректировкиИсточник.НомерУзла  КАК НомерУзлаИсточник,
	|		УзлыКорректировкиПриемник.НомерУзла  КАК НомерУзлаПриемник,
	|		УзлыКорректировкиПриемник.Количество КАК ВсегоКоличество,
	...
	|			ВтУзлыКорректировки КАК УзлыКорректировкиПриемник
	|		ПО
	|			Расходы.АналитикаУчетаПродукции = УзлыКорректировкиПриемник.АналитикаУчетаНоменклатуры
	|			И Расходы.РазделУчета = УзлыКорректировкиПриемник.РазделУчета
	|			И Расходы.Организация = УзлыКорректировкиПриемник.Организация
	|			И (ВЫБОР КОГДА &УчитыватьСебестоимостьТоваровПоВидамЗапасов 
	|				ТОГДА Расходы.ВидЗапасов = УзлыКорректировкиПриемник.ВидЗапасов
	|				ИНАЧЕ ИСТИНА КОНЕЦ)
	|	ГДЕ
	|		Расходы.Период МЕЖДУ &НачалоПериода И &КонецПериода
...
Функция ТекстЗапросаДвиженияСтоимостиРегл()
	...
	|	УчетСебестоимости.НомерУзлаИсточник 			  КАК НомерУзлаИсточник,
	|	УчетСебестоимости.НомерУзлаПриемник 			  КАК НомерУзлаПриемник,
	|	МАКСИМУМ(УчетСебестоимости.ПередачаВЭксплуатацию) КАК ПередачаВЭксплуатацию,
	|	СУММА(УчетСебестоимости.Количество) 			  КАК Количество
	|
	|ПОМЕСТИТЬ ВтПеремещенияСписания
	|ИЗ (
	...
	|		УзлыКорректировкиПриемник.НомерУзла  КАК НомерУзлаПриемник,
	|		УзлыКорректировкиПриемник.Количество КАК ВсегоКоличество,
	|		ВЫБОР КОГДА Расходы.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА
...
Функция ТекстЗапросаПроверкаКорректностиОстатковВРегистреСебестоимостьТоваровПослеРасчета()
	...
	//++ НЕ УТ
	|		И Т.ПостояннаяРазницаОстаток = 0
	|		И Т.ВременнаяРазницаОстаток = 0
	//-- НЕ УТ
...
Процедура СформироватьКорДвиженияСебестоимостьТоваров(ПараметрыРасчета, ДанныеДвижения)
	...
	Если ДанныеДвижения.НДСРеглКорректировкаСтоимости <> 0 И НЕ ДанныеДвижения.ЭтоПередачаМеждуОрганизациями Тогда	
		...
		КопируемыеПоля = "
		|Период, ХозяйственнаяОперация, ДокументДвижения,
		|ГруппаПродукции,
...
Процедура СформироватьДвиженияСебестоимостьТоваровСписаниеНЗП(ПараметрыРасчета, ДанныеДвижения, Суммы, НалоговоеНазначениеВыпуска)	

	ИмяРегистра = Метаданные.РегистрыНакопления.СебестоимостьТоваров.Имя;
	
	КопируемыеПоля = "
	|Период, Организация, РазделУчета, Подразделение, ГруппаПродукции, ДокументДвижения";
	...
	Запись.НалоговоеНазначение    = НалоговоеНазначениеВыпуска;
	Запись.КорНалоговоеНазначение = ДанныеДвижения.НалоговоеНазначение;
	
	Запись.НДСРегл = Суммы.НДСРегл;
	
	Если  (ЗначениеЗаполнено(Запись.НалоговоеНазначение) И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.НалоговоеНазначение, "ВидДеятельностиНДС") = Перечисления.ВидыДеятельностиНДС.Необлагаемая) 
		ИЛИ ЗначениеЗаполнено(Запись.КорНалоговоеНазначение) И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.КорНалоговоеНазначение, "ВидДеятельностиНДС") = Перечисления.ВидыДеятельностиНДС.Необлагаемая 
		Тогда	
		Запись.СтоимостьРегл = Запись.СтоимостьРеглБезНДС + Запись.НДСРегл;
	КонецЕсли; 
	
	Запись.ВидДвижения 				  = ВидДвиженияНакопления.Приход;
	Запись.АналитикаУчетаНоменклатуры = ДанныеДвижения.АналитикаУчетаПродукции;
...
Процедура СформироватьДвиженияПрочиеРасходы(ПараметрыРасчета, ДанныеДвижения,
	...
	Если ДанныеДвижения.ВариантРаспределенияРасходов = Перечисления.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты Тогда
		Запись.СуммаБезНДС = СуммаБезНДС;
	КонецЕсли;
	
	Если ДанныеДвижения.ВариантРаспределенияРасходов = Перечисления.ВариантыРаспределенияРасходов.НаПрочиеАктивы
		ИЛИ ДанныеДвижения.ВариантРаспределенияРасходов = Перечисления.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы Тогда
		Запись.НалоговоеНазначение = Справочники.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка();
	Иначе	
		Запись.НалоговоеНазначение = ДанныеДвижения.КорНалоговоеНазначение;
	КонецЕсли; 
	...
	Если НЕ ДанныеДвижения.ВариантРаспределенияРасходов = Перечисления.ВариантыРаспределенияРасходов.НаПрочиеАктивы
	 И НЕ ДанныеДвижения.ВариантРаспределенияРасходов = Перечисления.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы Тогда
		...
		Иначе
			...
			Запись.СуммаРеглБезНДС = СуммаРеглБезНДС;
			Запись.НДСРегл = НДСРегл;
			Запись.ПостояннаяРазница = ПостояннаяРазница;
			Запись.ВременнаяРазница  = ВременнаяРазница;
		КонецЕсли;
	КонецЕсли;
	//-- НЕ УТ
...
Процедура СформироватьДвиженияПрочиеРасходыСписаниеНЗП(ПараметрыРасчета, ДанныеДвижения, Суммы)
	...
	КопируемыеПоля = "
	|Период, Организация, НалоговоеНазначение, СтатьяРасходов, АналитикаРасходов, ДокументДвижения, КорПодразделение";
	...
	Запись.СуммаРеглБезНДС       = Суммы.СтоимостьРеглБезНДС;
	Запись.НДСРегл 		         = Суммы.НДСРегл;
	Если (ЗначениеЗаполнено(Запись.НалоговоеНазначение) И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.НалоговоеНазначение, "ВидДеятельностиНДС") = Перечисления.ВидыДеятельностиНДС.Необлагаемая) Тогда
		Запись.СуммаРегл = Запись.СуммаРеглБезНДС + Запись.НДСРегл;
	КонецЕсли; 
...
Процедура СформироватьДвиженияПрочиеРасходыПрочееСписаниеНЗП(ПараметрыРасчета, ДанныеДвижения)
	...
	КопируемыеПоля = "
	|Период, Организация, НалоговоеНазначение, Подразделение, СтатьяРасходов, АналитикаРасходов, ХозяйственнаяОперация, ДокументДвижения";
	...
	Запись.СуммаРеглБезНДС   = ДанныеДвижения.СтоимостьРеглБезНДС;
	Запись.НДСРегл 		     = ДанныеДвижения.НДСРегл;
	Если (ЗначениеЗаполнено(Запись.НалоговоеНазначение) И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.НалоговоеНазначение, "ВидДеятельностиНДС") = Перечисления.ВидыДеятельностиНДС.Необлагаемая) Тогда
		Запись.СуммаРегл = Запись.СуммаРеглБезНДС + Запись.НДСРегл;
	КонецЕсли; 
...
// Служебная, этап 2.3, 3.15
Процедура СформироватьДвиженияПрочиеРасходыНезавершенногоПроизводства(ПараметрыРасчета, ДанныеДвижения, Суммы, НалоговоеНазначениеВыпуска)	
	...
	|Продукция, ХарактеристикаПродукции, АналитикаУчетаПродукции, РазделУчета, ВидЗапасов, ДокументДвижения";
	...
	Запись.СтоимостьРеглБезНДС = Суммы.СтоимостьРеглБезНДС;
	Запись.НДСРегл             = Суммы.НДСРегл;
	Запись.НалоговоеНазначениеВыпуска = НалоговоеНазначениеВыпуска;
...
Процедура ПолучитьСебестоимостьПартийТоваров(ПараметрыРасчета, ДляСтоимостиПродаж = Ложь)
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах КАК ВозвратРозница
	|		ПО ВозвратРозница.Ссылка = ПартииРасходов.Регистратор
	...
	|		СУММА(Таблица.НДСРегл) КАК НДСРегл,
	|		СУММА(Таблица.СтоимостьРеглБезНДС) КАК СтоимостьРеглБезНДС,
	|		СУММА(Таблица.НДСРеглРесурс) КАК НДСРеглРесурс,
	|		СУММА(Таблица.ПостояннаяРазница) КАК ПостояннаяРазница,
	|		СУММА(Таблица.ВременнаяРазница) КАК ВременнаяРазница
	|	ИЗ
	...
	|		0 КАК СуммаДопРасходов,
	|		0 КАК СуммаДопРасходовБезНДС,
    |		ВЫБОР КОГДА Затраты.НалоговоеНазначениеПродукции.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая) 
    |		            И Затраты.НалоговоеНазначениеПартии.ВидДеятельностиНДС <> ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.ПропорциональноОблагаемая) 
	|		ТОГДА
	|			Затраты.СтоимостьРегл + Затраты.НДСРегл
	|		ИНАЧЕ
	|			Затраты.СтоимостьРегл
	...
	|			(
	|				(
	|					(Затраты.НалоговоеНазначениеПродукции.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Облагаемая) 
	|	   					И Затраты.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая))
	|					ИЛИ (Затраты.НалоговоеНазначениеПродукции.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.ПропорциональноОблагаемая) 
	|	   					И Затраты.НалоговоеНазначение = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность))
	|           	)
	|		    	И &ВключатьНДСВСебестоимостьПриУсловнойПродаже
	|			) 
	|			ИЛИ (
	|				Затраты.НалоговоеНазначениеПродукции.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая) 
	|		    	И Затраты.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Облагаемая) 
	|		    	И &ИсключатьНДСИзСебестоимостиПриВосстановленииНДС
	|           )
	|		ТОГДА
	|			Затраты.НДСРегл
	...
	|			0
	|		КОНЕЦ КАК НДСРегл,
	|		Затраты.СтоимостьРегл КАК СтоимостьРеглБезНДС,
	|		Затраты.НДСРегл КАК НДСРеглРесурс,
	...
	|			(
	|				(
	|					(Затраты.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Облагаемая) 
	|	   					И Затраты.НалоговоеНазначениеПродукции.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая))
	|					ИЛИ (Затраты.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.ПропорциональноОблагаемая) 
	|	   					И Затраты.НалоговоеНазначениеПродукции = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность))
	|           	)
	|		    	И &ВключатьНДСВСебестоимостьПриУсловнойПродаже
	|			) 
	|			ИЛИ (
	|				Затраты.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая) 
	|		    	И Затраты.НалоговоеНазначениеПродукции.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Облагаемая) 
	|		    	И &ИсключатьНДСИзСебестоимостиПриВосстановленииНДС
	|           )
	|		ТОГДА
	|			Затраты.НДСРегл
	...
	|		РегистрНакопления.ПартииЗатратНаВыпуск КАК Затраты
	|	ГДЕ
	|		Затраты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И Затраты.Активность
	|		И Затраты.Организация В(&МассивОрганизаций)
	|		И ЕСТЬNULL(Затраты.ВидЗапасов.ТипЗапасов, НЕОПРЕДЕЛЕНО) <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|		И ЕСТЬNULL(Затраты.ВидЗапасов.ТипЗапасов, НЕОПРЕДЕЛЕНО) <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.МатериалДавальца)
	...
	|			КОНЕЦ) КАК СтоимостьРегл,
	|		0 КАК НДСРегл,
	|		СУММА(Затраты.СтоимостьРегл) КАК СтоимостьРеглБезНДС,
	|		СУММА(Затраты.НДСРегл) КАК НДСРеглРесурс,
	|		СУММА(Затраты.ПостояннаяРазница) КАК ПостояннаяРазница,
	|		СУММА(Затраты.ВременнаяРазница) КАК ВременнаяРазница
	|	ИЗ
	|		РегистрНакопления.ПартииНезавершенногоПроизводства КАК Затраты
	|	ГДЕ
	|		Затраты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		И Затраты.Активность
	|		И Затраты.Организация В(&МассивОрганизаций)
	...
	Для Каждого КлючИЗначение Из ПараметрыРасчета.Движения Цикл
		...
		Если КоличествоДокументов > 0 Тогда
			...
			Иначе
				...
				"ВЫБРАТЬ
				|	ИзмененныеДокументы.Регистратор,
				|	ИзмененныеДокументы.МоментВремени,
				|	СУММА(1) КАК КоличествоДвижений
				|ПОМЕСТИТЬ ВТДвиженияДокументов
				|ИЗ
				|	ВТКэшИзмененныеДокументы КАК ИзмененныеДокументы
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКэш%ИмяРегистра КАК Движения
				|		ПО ИзмененныеДокументы.Регистратор = Движения.Регистратор
				|
				|СГРУППИРОВАТЬ ПО
				|	ИзмененныеДокументы.Регистратор,
				|	ИзмененныеДокументы.МоментВремени
				...
				УничтожитьВременныеТаблицы(
					ПараметрыРасчета.МенеджерВременныхТаблиц,
					"ВТДвиженияДокументов, ВТПорцииДвиженийМаленькихДокументов, ВТПорцииИзмененныхДокументов");
...
Процедура ПротоколРасчета_ОкончаниеЭтапаРасчета(ПараметрыРасчета)
	...
	Для Каждого ОписаниеДвижения Из ОписаниеЭтапа.СформированныеДвижения Цикл
		...
		ВсегоДвижений = РазмерВременнойТаблицы(ПараметрыРасчета, "ВТКэш" + ОписаниеДвижения.Ключ);
	...
	Если ЗначениеЗаполнено(ИменаИсходныхТаблиц) Тогда
		...
		Для Каждого КлючИЗначение Из Новый Структура(ИменаИсходныхТаблиц) Цикл
			ТекстИсходныеДанные =
				...
				+ КлючИЗначение.Ключ + " ("
				+ ПротоколРасчета_ПредставлениеЗначения(РазмерВременнойТаблицы(ПараметрыРасчета, КлючИЗначение.Ключ)) + ")";
		КонецЦикла;
...
Процедура ПротоколРасчета_ОкончаниеФормированиеВременнойТаблицы(ПараметрыРасчета)
	...
	КоличествоСтрок = РазмерВременнойТаблицы(ПараметрыРасчета, ИмяВременнойТаблицы);
...
//
Функция РазмерВременнойТаблицы(ПараметрыРасчета, ИмяВременнойТаблицы) Экспорт
...
//
Функция ВыгрузитьВременнуюТаблицу(ПараметрыРасчета, ИмяВременнойТаблицы, КоличествоЗаписей = Неопределено) Экспорт
	...
	Запрос.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Запрос.Текст,
		?(НЕ ЗначениеЗаполнено(КоличествоЗаписей), "", "ПЕРВЫЕ " + Формат(КоличествоЗаписей, "ЧГ=0")),
		ИмяВременнойТаблицы);
...
//
Функция ВыполнитьЗапросСДругимТекстом(Запрос, ПутьКФайлуСТекстом) Экспорт
	
	Результат = Новый Структура;
	
	Файл = Новый ТекстовыйДокумент;
	Файл.Прочитать(ПутьКФайлуСТекстом, КодировкаТекста.UTF8);
	
	СтарыйТекст = Запрос.Текст;
	НовыйТекст = Файл.ПолучитьТекст();
	...
	Результат.Вставить("СтарыйТекст", СтарыйТекст);
	Результат.Вставить("НовыйТекст",  НовыйТекст);
	Результат.Вставить("Время",  	  (ВремяОкончания - ВремяНачала) / 1000);
...
Функция ВходящиеДанныеМеханизма(ТолькоТребующиеПерерасчетаСебестоимости = Ложь) Экспорт
	...
	Если НЕ ТолькоТребующиеПерерасчетаСебестоимости Тогда 
		...
		//++ НЕ УТКА
		ВходящиеДанные.Вставить(Метаданные.Документы.ПринятиеКУчетуНМАМеждународныйУчет, Значение);
		ВходящиеДанные.Вставить(Метаданные.Документы.ПринятиеКУчетуОСМеждународныйУчет, Значение);
		//-- НЕ УТКА
	КонецЕсли;
	...
	Если НЕ ТолькоТребующиеПерерасчетаСебестоимости Тогда 
		...
		//++ НЕ УТ
		ВходящиеДанные.Вставить(Метаданные.РегистрыСведений.ОтражениеДокументовВРеглУчете, Значение);
		//-- НЕ УТ
		
		//++ НЕ УТКА
		ВходящиеДанные.Вставить(Метаданные.РегистрыСведений.ОтражениеДокументовВМеждународномУчете, Значение);
		//-- НЕ УТКА
	КонецЕсли;
