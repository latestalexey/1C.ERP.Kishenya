	...
	Если ФайлВременный.Существует() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Файлы.Удаление файлов в томе при обмене';uk='Файли.Вилучення файлів у томі при обміні'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ДобавитьФайлВТом(ДвоичныеДанныеИлиПуть, ВремяИзмененияУниверсальное, ИмяБезРасширения, Расширение,
			...
			Если СсылкаНаТом.МаксимальныйРазмер <> 0 Тогда
				...
				Если НовыйРазмер > СсылкаНаТом.МаксимальныйРазмер Тогда
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Превышен максимальный размер тома (%1 Мб).';uk='Перевищено максимальний розмір тому (%1 Мб).'"),
		...
		Исключение
			...
			ШаблонОписанияОшибки =
				НСтр("ru='Ошибка при добавлении файла ""%1""
                           |в том ""%2"" (%3):
                           |""%4"".'
                           |;uk='Помилка при додаванні файлу ""%1""
                           |у том ""%2"" (%3):
                           |""%4"".'");
	...
	ШаблонСообщенияОбОшибке = НСтр("ru='Не удалось добавить файл ни в один из томов.
        ...
        |
        |%1'
        |;uk='Не вдалося додати файл ні в один з томів.
        |Список помилок:
        |
        |%1'");
	
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Файлы.Добавление файла';uk='Файли.Додавання файлу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,,,
...
Процедура КриптографияПриСозданииФормыНаСервере(Форма, ЭтоФормаСписка = Истина, ТолькоКартинкаСтрок = Ложь) Экспорт
	...
	Если ЭлектронноеПодписание И Шифрование Тогда
		Заголовок = НСтр("ru='Электронная подпись и шифрование';uk='Електронний підпис і шифрування'");
		Подсказка = НСтр("ru='Наличие электронной подписи или шифрования';uk='Наявність електронного підпису або шифрування'");
		Картинка  = БиблиотекаКартинок["ПодписанЗашифрованЗаголовок"];
	ИначеЕсли ЭлектронноеПодписание Тогда
		Заголовок = НСтр("ru='Электронная подпись';uk='Електронний підпис'");
		Подсказка = НСтр("ru='Наличие электронной подписи';uk='Наявність електронного підпису'");
		Картинка  = БиблиотекаКартинок["ПодписанЭП"];
	Иначе // Шифрование
		Заголовок = НСтр("ru='Шифрование';uk='Шифрування'");
		Подсказка = НСтр("ru='Наличие шифрования';uk='Наявність шифрування'");
...
Функция ПодготовитьДанныеДляСозданияФайловогоНачальногоОбраза(СтруктураПараметров) Экспорт
			...
			Если ЕстьФайлыВТомах И Не ПустаяСтрока(ПутьКАрхивуСФайламиТомов) И (Лев(ПутьКАрхивуСФайламиТомов, 2) <> "\\"
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Путь к архиву с файлами томов должен быть
                               ...
                               |;uk='Шлях до архіву з файлами томів повинен бути
                               |у форматі UNC (\\servername\resource)'"),
					,
			...
			Если Не ПустаяСтрока(ПолноеИмяФайловойБазы) И (Лев(ПолноеИмяФайловойБазы, 2) <> "\\" ИЛИ СтрНайти(ПолноеИмяФайловойБазы, ":") <> 0) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Путь к файловой базе должен быть
                               |в формате UNC (\\servername\resource)'
                               |;uk='Шлях до файлової бази повинен бути
                               |у форматі UNC (\\servername\resource)'"),
					,
	...
	Если ПустаяСтрока(ПолноеИмяФайловойБазы) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Укажите полное имя файловой базы (файл 1cv8.1cd)';uk='Укажіть повне ім''я файлової бази (файл 1cv8.1cd)'"),,
			"ПолноеИмяФайловойБазыWindows");
		Результат.ДанныеПодготовлены = Ложь;
	ИначеЕсли Результат.ДанныеПодготовлены Тогда
		...
		Если ЕстьФайлыВТомах Тогда
			Если ПустаяСтрока(ПутьКАрхивуСФайламиТомов) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Укажите полное имя архива с файлами томов (файл *.zip)';uk='Укажіть повне ім''я архіву з файлами томів (файл *.zip)'"),, 
					"ПутьКАрхивуСФайламиТомовWindows");
				Результат.ДанныеПодготовлены = Ложь;
			Иначе
				...
				Если Файл.Существует() И ФайлБазы.Существует() Тогда
					Результат.ТекстВопроса = СтрШаблон(НСтр("ru='Файлы ""%1"" и ""%2"" уже существуют.
                                       ...
                                       |;uk='Файли ""%1"" та ""%2"" вже існують.
                                       |Замінити існуючі файли?'"), ПутьКАрхивуСФайламиТомов, ПолноеИмяФайловойБазы);
				...
				ИначеЕсли Файл.Существует() Тогда
					Результат.ТекстВопроса = СтрШаблон(НСтр("ru='Файл ""%1"" уже существует.
                                       ...
                                       |;uk='Файл ""%1 вже існує.
                                       |Замінити існуючий файл?'"), ПутьКАрхивуСФайламиТомов);
					Результат.ТребуетсяПодтверждение = Истина;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если Результат.ДанныеПодготовлены Тогда
			Если ФайлБазы.Существует() И НЕ Результат.ТребуетсяПодтверждение Тогда
				Результат.ТекстВопроса = СтрШаблон(НСтр("ru='Файл ""%1"" уже существует.
                                   |Заменить существующий файл?'
                                   |;uk='Файл ""%1 вже існує.
                                   |Замінити існуючий файл?'"), ПолноеИмяФайловойБазы);
				Результат.ТребуетсяПодтверждение = Истина;
			КонецЕсли;
...
Процедура ИзвлечьТекстИзФайлов() Экспорт
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		НСтр("ru='Начато регламентное извлечения текста';uk='Почато регламентне витягування тексту'"));
			...
			ЗаписьЖурналаРегистрации(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Во время регламентного извлечения текста из файла
                               ...
                               |произошла ошибка:
                               |""%2"".'
                               |;uk='Під час регламентного витягування тексту з файлу
                               |""%1""
                               |відбулася помилка:
                               |""%2"".'"),
					ИмяСРасширениемФайла,
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		НСтр("ru='Закончено регламентное извлечение текста';uk='Закінчено регламентне витягування тексту'"));
...
Процедура ПроверитьИсключениеHTTP1С(Ответ, АдресСервера)
	Если (Ответ.КодСостояния >= 400) И (Ответ.КодСостояния <= 599) Тогда
		ШаблонОшибки = НСтр("ru='Ошибка HTTP 1С : %1 в %2';uk='Помилка HTTP 1С : %1 %2'");
...
Процедура ЗагрузитьФайлССервера(ПараметрыФайла, ЭтоФайл = Неопределено)
	...
	ТекстСобытия = НСтр("ru='Загрузка файла с сервера: %1';uk='Завантаження файлу з сервера: %1'");
	...
	Если АдресЗагруженногоФайла <> Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru='Загружен файл с сервера: %1';uk='Завантажений файл з сервера: %1'");
	Иначе
		ТекстСообщения = НСтр("ru='Не удалось загрузить файл с сервера:  %1';uk='Не вдалося завантажити файл з сервера: %1'");
	КонецЕсли;
...
Процедура ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСообщения, ПараметрыСинхронизации)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Синхронизация файлов с облачным сервисом:';uk='Синхронізація файлів з хмарним сервісом:'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Информация,
					,
					ПараметрыСинхронизации.УчетнаяЗапись,
					ТекстСообщения);
			...
			Если НайденныйPropstat<>Неопределено Тогда
				Для каждого ДочернийУзелPropstat Из НайденныйPropstat.ДочерниеУзлы Цикл
					Если ДочернийУзелPropstat.ЛокальноеИмя = "quota-available-bytes" Тогда
						...
						ИнформацияОСвободномМесте = НСтр("ru='Свободное место (байт): %1(%2 Мб)';uk='Вільне місце (байт): %1(%2 Мб)'");
					...
					ИначеЕсли ДочернийУзелPropstat.ЛокальноеИмя = "quota-used-bytes" Тогда
						...
						ИнформацияОЗанятомМесте = НСтр("ru='Занято (байт): %1(%2 Мб)';uk='Зайнято (байт): %1(%2 Мб)'");
	...
	Попытка
		...
		Пока НайденныйResponse <> Неопределено Цикл
			...
			Если НайденныйHref = Неопределено Тогда
				ТекстОшибки = НСтр("ru='Ошибка ответа от сервера: не найден HREF в %1';uk='Помилка відповіді від сервера: не знайдено HREF %1'");
			...
			Если (ТекущаяПапкаФайлов = Неопределено) И (ТаблицаФайлов.Найти(СтрокаДереваФайлов.Href, "Href") = Неопределено) Тогда
				...
				Если ПодсистемаРаботаСФайламиСуществует Тогда
					...
					Если НЕ ПроверитьВозможностьUID1C(СтрокаДереваФайлов.Href,Строка(Новый УникальныйИдентификатор), ПараметрыСинхронизации) Тогда
						ТекстСобытия = НСтр("ru='Невозможно сохранение дополнительных свойств файла, он не будет загружен: %1';uk='Неможливо збереження додаткових властивостей файлу, він не буде завантажений: %1'");
					...
					Попытка
						...
						ТекстСобытия = НСтр("ru='Загружена папка с сервера:  %1';uk='Завантажена папка з сервера: %1'");
...
Процедура СинхронизироватьФайлыСОблачнымСервисом(УчетнаяЗапись)
	...
	ТекстСобытия = НСтр("ru='Начало синхронизации файлов с облачным сервисом.';uk='Початок синхронізації файлів з хмарним сервісом.'");
	...
	Попытка
		...
		Если Отказ = Истина Тогда
			ТекстОшибки = НСтр("ru='При загрузке структуры файлов с сервера произошла ошибка, синхронизация не выполнена.';uk='При завантаженні структури файлів з сервера сталася помилка, синхронізація не виконана.'");
		...
		Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
			...
			Попытка
				...
				Если СозданНовыйВБазе И НЕ УдаленВБазе Тогда
					...
					ТекстСобытия = НСтр("ru='Создан объект в облачном сервисе %1';uk='Створений об''єкт в хмарному сервісі %1'");
				...
				ИначеЕсли УдаленНаСервере И НЕ УдаленВБазе Тогда
					...
					ТекстСобытия = НСтр("ru='Удалено в базе: %1';uk='Вилучено в базі: %1'");
					ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия, СтрокаТаблицы.Наименование), ПараметрыСинхронизации);
				...
				ИначеЕсли (ИзмененВБазе ИЛИ ИзмененНаСервере) И НЕ (УдаленВБазе ИЛИ УдаленНаСервере) Тогда
					...
					Если ИзмененНаСервере И НЕ ИзмененВБазе Тогда
						...
						СтрокаТаблицы.Обработан = Истина;
						ТекстСобытия = НСтр("ru='Обновлен объект в базе: %1';uk='Оновлений об''єкт в базі: %1'");
						ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия, СтрокаТаблицы.Наименование), ПараметрыСинхронизации);
	...
	ТекстСобытия = НСтр("ru='Завершена синхронизация файлов с облачным сервисом';uk='Завершена синхронізація файлів з хмарним сервісом'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации);
...
Процедура ВыполнитьПроверкуПодключения(УчетнаяЗапись, РезультатПроверки) Экспорт 
	...
	ТекстСобытия = НСтр("ru='Начата проверка синхронизации файлов';uk='Розпочата перевірка синхронізації файлів'") + " " + УчетнаяЗапись.Наименование;
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации);
	
	ПрочитатьПараметрыКаталога(РезультатПроверки,АдресСервера, ПараметрыСинхронизации);
	
	ТекстСобытия = НСтр("ru='Завершена проверка синхронизации файлов';uk='Завершена перевірка синхронізації файлів'") + " " + УчетнаяЗапись.Наименование;
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации);
...
Процедура ОсвободитьЗахваченныеФайлыУчетнойЗаписи(УчетнаяЗапись = Неопределено)
	...
	ТекстСобытия = НСтр("ru='Начало освобождения файлов, захваченных облачным сервисом.';uk='Початок визволення файлів, захоплених хмарним сервісом.'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации);
	...
	Попытка
		...
		Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
			...
			Попытка
				...
				ТекстСобытия = НСтр("ru='Удален объект в облачном сервисе %1';uk='Видалений об''єкт в хмарному сервісі %1'");
	...
	ТекстСобытия = НСтр("ru='Завершено освобождения файлов, захваченных облачным сервисом';uk='Завершено визволення файлів, захоплених хмарним сервісом'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации);
