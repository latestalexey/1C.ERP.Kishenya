...
//   - запрещаются все внешние (по отношению к платформе) действия (COM,
...
// Выполняет произвольный алгоритм на встроенном языке платформы, предварительно устанавливая
...
//
// Параметры:
//  Алгоритм - Строка - содержащая произвольный алгоритм на встроенном языке платформы.
//  Параметры - Произвольный - в качестве значения данного параметра может быть передано значение,
...
Процедура ВыполнитьМетодКонфигурации(Знач ИмяМетода, Знач Параметры = Неопределено) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Ошибка при вызове процедуры ВыполнитьМетодКонфигурации общего модуля РаботаВБезопасномРежиме.';uk='Помилка при виклику процедури ВыполнитьМетодКонфигурации загального модуля РаботаВБезопасномРежиме.'")
...
//
// Параметры:
//  Объект - Произвольный - объект встроенного языка платформы, содержащий методы (например, ОбработкаОбъект),
...
Процедура ВыполнитьМетодОбъекта(Знач Объект, Знач ИмяМетода, Знач Параметры = Неопределено) Экспорт
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Некорректное значение параметра ИмяМетода (%1)';uk='Некоректне значення параметра ИмяМетода (%1)'"), ИмяМетода);
	КонецПопытки;
...
// на встроенном языке платформы перед использованием его в операторах Выполнить() и Вычислить()
...
Процедура ПроверитьИмяМетодаКонфигурации(Знач ИмяМетода) Экспорт
	...
	Если ЧастиИмени.Количество() <> 2 И ЧастиИмени.Количество() <> 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неправильный формат параметра ИмяМетода (%1)';uk='Неправильний формат параметра ИмяМетода (%1)'"), ИмяМетода);
	КонецЕсли;
	...
	Если ЧастиИмени.Количество() = 2 И Метаданные.ОбщиеМодули.Найти(ИмяОбъекта) = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неправильный формат параметра ИмяМетода (%1):
                ...
                |;uk='Неправильний формат параметра ИмяМетода (%1):
                |Не знайдений загальний модуль ""%2"".'"),
			ИмяМетода,
			ИмяОбъекта);
	КонецЕсли;
		...
		Если Менеджер = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Неправильный формат параметра ИмяМетода (%1):
                           ...
                           |;uk='Неправильний формат параметра ИмяМетода (%1):
                           |Не знайдений менеджер об''єкта ""%2"".'"),
				ИмяМетода,
				ПолноеИмяОбъекта);
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Безопасное выполнение метода';uk='Безпечне виконання методу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неправильный формат параметра ИмяМетода (%1):
                       |Имя метода ""%2"" не соответствует требованиям образования имен процедур и функций.'
                       |;uk='Неправильний формат параметра ИмяМетода (%1):
                       |Ім''я методу ""%2"" не відповідає вимогам утворення назв процедур і функцій.'"),
			ИмяМетода,
			ИмяМетодаОбъекта);
	КонецПопытки;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользуютсяПрофилиБезопасности") И ЗначениеЗаполнено(ПрофильИнформационнойБазы) Тогда
		...
		Если БезопасныйРежим() <> ПрофильИнформационнойБазы Тогда
			...
			Иначе
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Невозможно выполнение обработчиков установки параметров сеанса по причине: профиль безопасности %1 отсутствует в кластере серверов платформы, или для него запрещено использование в качестве профиля безопасности безопасного режима.
                        |
                        ...
                        |;uk='Неможливе виконання обробників установки параметрів сеансу через: профіль безпеки %1 відсутня в кластері серверів платформи, або для нього заборонено використання в якості профілю безпеки безпечного режиму.
                        |
                        |Для відновлення працездатності програми потрібно відключити використання профілю безпеки через консоль кластера і заново налаштувати профілі безпеки за допомогою інтерфейсу конфігурації (відповідні команди знаходяться в розділі налаштувань програми).'"),
		...
		Если Не ДоступенПривилегированныйРежим Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Невозможно выполнение обработчиков установки параметров сеанса по причине: профиль безопасности %1 не содержит разрешения на установку привилегированного режима. Возможно, он был отредактирован через консоль кластера.
                    |
                    |Для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).'
                    |;uk='Неможливе виконання обробок установки параметрів сеансу через: профіль безпеки %1 не містить дозволу на встановлення привілейованого режиму. Можливо, він був відредагований через консоль кластера.
                    |
                    |Для відновлення працездатності програми потрібно відключити використання профілю безпеки через консоль кластера і заново налаштувати профілі безпеки за допомогою інтерфейсу конфігурації (відповідні команди знаходяться в розділі настройок програми).'"),
				ПрофильИнформационнойБазы);
		...
		Исключение
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Невозможно выполнение обработчиков установки параметров сеанса по причине: %1.
                    ...
                    |;uk='Неможливе виконання обробок установки параметрів сеансу за причиною: %1.
                    |
                    |Можливо, для інформаційної бази через консоль кластера був встановлений профіль безпеки, що не допускає виконання зовнішніх модулів без встановлення безпечного режиму. У цьому випадку для відновлення працездатності програми потрібно відключити використання профілю безпеки через консоль кластера і заново налаштувати профілі безпеки за допомогою інтерфейсу конфігурації (відповідні команди знаходяться в розділі налаштувань програми).При цьому програма буде автоматично коректно налаштована на використання спільно з включеними профілями безпеки.'"),
...
Функция ВозможноВыполнениеОбработчиковУстановкиПараметровСеансаБезУстановкиБезопасногоРежима() Экспорт
	...
	Исключение
		...
		ШаблонЗаписиЖР = НСтр("ru='При установке параметров сеанса произошла ошибка:
            ...
            |;uk='При встановленні параметрів сеансу сталася помилка:
            |
            |--------------------------------------------------------------------------------------------
            |%1
            |--------------------------------------------------------------------------------------------
            |
            |Запуск програми буде неможливий.'");
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Установка параметров сеанса';uk='Установка параметрів сеансу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция МенеджерОбъектаПоИмени(Имя)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось получить менеджер для объекта ""%1""';uk='Не вдалося отримати менеджер для об''єкта ""%1""'"), Имя);
...
//  есть во встроенном языке платформы. Используется для определения сетевого порта в тех
