...
Процедура ФормаНастройкиПередЗакрытием(Отказ, Форма) Экспорт
	...
	Если Форма.Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Данные были изменены. Закрыть форму без сохранения изменений?';uk='Дані були змінені. Закрити форму без збереження змін?'");
...
Функция ЗаголовкиГиперссылокЗагрузкиДанных() Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("Неопределено",               НСтр("ru='Получение данных не выполнялось';uk='Одержання даних не виконувалося'"));
	Структура.Вставить("Ошибка",                     НСтр("ru='Не удалось получить данные';uk='Не вдалося одержати дані'"));
	Структура.Вставить("ВыполненоСПредупреждениями", НСтр("ru='Данные получены с предупреждениями';uk='Дані отримані з попередженнями'"));
	Структура.Вставить("Успех",                      НСтр("ru='Данные успешно получены';uk='Дані успішно отримані'"));
	Структура.Вставить("Выполнение",                 НСтр("ru='Выполняется получение данных...';uk='Виконується отримання даних...'"));
	
	Структура.Вставить("Предупреждение_СообщениеОбменаБылоРанееПринято", НСтр("ru='Нет новых данных для получения';uk='Немає нових даних для одержання'"));
	Структура.Вставить("Ошибка_ТранспортСообщения",                      НСтр("ru='Не удалось получить данные';uk='Не вдалося одержати дані'"));
...
Функция ЗаголовкиГиперссылокВыгрузкиДанных() Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("Неопределено", НСтр("ru='Отправка данных не выполнялась';uk='Відправлення даних не виконувалося'"));
	Структура.Вставить("Ошибка",       НСтр("ru='Не удалось отправить данные';uk='Не вдалося відправити дані'"));
	Структура.Вставить("Успех",        НСтр("ru='Данные успешно отправлены';uk='Дані успішно відправлені'"));
	Структура.Вставить("Выполнение",   НСтр("ru='Выполняется отправка данных...';uk='Виконується відправлення даних...'"));
	
	Структура.Вставить("Предупреждение_СообщениеОбменаБылоРанееПринято", НСтр("ru='Данные отправлены с предупреждениями';uk='Дані відправлені з попередженнями'"));
	Структура.Вставить("ВыполненоСПредупреждениями",                     НСтр("ru='Данные отправлены с предупреждениями';uk='Дані відправлені з попередженнями'"));
	Структура.Вставить("Ошибка_ТранспортСообщения",                      НСтр("ru='Не удалось отправить данные';uk='Не вдалося відправити дані'"));
...
Процедура ОбработчикВыбораФайловогоКаталога(Объект, Знач ИмяСвойства, СтандартнаяОбработка = Ложь, Знач ПараметрыДиалога = Неопределено, ОповещениеЗавершения = Неопределено) Экспорт
	...
	УмолчанияДиалога.Вставить("Заголовок", НСтр("ru='Укажите каталог';uk='Вкажіть каталог'") );
...
Процедура ОбработчикВыбораФайла(Объект, Знач ИмяСвойства, СтандартнаяОбработка = Ложь, Знач ПараметрыДиалога = Неопределено, ОповещениеЗавершения = Неопределено) Экспорт
	...
	УмолчанияДиалога.Вставить("Заголовок",                   НСтр("ru='Выберите файл';uk='Виберіть файл'"));
...
Процедура ПередатьФайлыНаСервер(ОповещениеЗавершения, Знач ИменаФайлов, Знач ИдентификаторФормы = Неопределено, Знач ТекстПредупреждения = Неопределено) Экспорт
	...
	Для Каждого ИмяФайла Из ИменаФайлов Цикл
		...
		Если ПустаяСтрока(ИмяФайла) Тогда
			ЕстьПустые = Истина;
			ОписаниеФайла.ОписаниеОшибки = НСтр("ru='Файл не выбран.';uk='Файл не обраний.'");
		КонецЕсли;
	...
	Если ТекстПредупреждения = Неопределено Тогда
		Если ИменаФайлов.Количество() = 1 Тогда
			ТекстПредупреждения = НСтр("ru='Для передачи файла на сервер необходимо установить расширение для веб-клиента платформы.';uk='Для передачі файлу на сервер необхідно встановити розширення для веб-клієнта платформи.'");
		Иначе
			ТекстПредупреждения = НСтр("ru='Для передачи файлов на сервер необходимо установить расширение для веб-клиента платформы.';uk='Для передачі файлів на сервер необхідно встановити розширення для веб-клієнта платформи.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередатьФайлНаСерверЗавершениеПодключенияРасширенияПослеПроверкиСуществованияФайла(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Не Существует Тогда
		...
		Элемент.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Файл ""%1"" не существует или к нему нет доступа';uk='Файл ""%1"" не існує або до нього немає доступу'"), Элемент.Имя);
...
Процедура ВыбратьИСохранитьФайлНаКлиентеПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	УмолчанияДиалога = Новый Структура;
	УмолчанияДиалога.Вставить("Заголовок",               НСтр("ru='Выберите файл для сохранения';uk='Виберіть файл для збереження'"));
	УмолчанияДиалога.Вставить("МножественныйВыбор",      Ложь);
	УмолчанияДиалога.Вставить("ПредварительныйПросмотр", Ложь);
...
Процедура УдалитьНастройкуСинхронизации(Знач УзелИнформационнойБазы) Экспорт
	...
	Если ОбменДаннымиВызовСервера.УзелЯвляетсяГлавным(УзелИнформационнойБазы) Тогда
		ТекстПредупреждения = НСтр("ru='Отключение информационной базы от главного узла можно выполнить
            ...
            |;uk='Відключення інформаційної бази від головного вузла можна виконати
            |за допомогою параметра запуску конфігуратора /ResetMasterNode.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	Иначе
		ТекстВопроса = НСтр("ru='Удалить настройку синхронизации данных?';uk='Вилучити настройки синхронізації даних?'");
...
Процедура ПриОткрытииИнструкцииКакИзменитьПарольСинхронизацииДанных(Знач АдресДляВосстановленияПароляУчетнойЗаписи) Экспорт
	...
	Если ПустаяСтрока(АдресДляВосстановленияПароляУчетнойЗаписи) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Адрес для восстановления пароля учетной записи не задан.';uk='Адреса для відновлення пароля облікового запису не задана.'"));
...
Функция ОткрытьФормуДополненияВыгрузкиВсеДокументы(Знач ДополнениеВыгрузки, Знач Владелец=Неопределено, Знач Уникальность=Неопределено, Знач Окно=Неопределено) Экспорт
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Добавление документов для отправки';uk='Додавання документів для відправлення'") );
