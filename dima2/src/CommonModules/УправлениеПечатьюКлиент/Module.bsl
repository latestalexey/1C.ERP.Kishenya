...
#КонецЕсли
	...
	Если ПечатныеФормы.Отказ Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Нет прав для вывода печатной формы на принтер, обратитесь к администратору.';uk='Немає прав для виведення друкованої форми на принтер, зверніться до адміністратора.'"));
		Возврат;
	КонецЕсли;
...
Функция ПроверитьДоступностьЯзыкаДляПечати(КодЯзыка) Экспорт
	...
	КодЯзыкаПоУмолчанию = Локализация.ПолучитьЯзыкФормированияПечатныхФорм();
...
Функция ОбластьМакета(Знач СсылкаНаМакет, Знач ОписаниеОбласти) Экспорт
	...
	Если СсылкаНаМакет.Тип = "DOC" Тогда
		...
		ИначеЕсли	ОписаниеОбласти.ТипОбласти = "Список" Тогда
			Область = УправлениеПечатьюMSWordКлиент.ПолучитьОбластьМакетаMSWord(СсылкаНаМакет, ОписаниеОбласти.ИмяОбласти, 1, 0);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Тип области не указан или указан не корректно: %1.';uk='Тип області не зазначений або зазначений не коректно: %1.'"), ОписаниеОбласти.ТипОбласти);
		КонецЕсли;
	...
	ИначеЕсли СсылкаНаМакет.Тип = "ODT" Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Тип области не указан или указан не корректно: %1.';uk='Тип області не зазначений або зазначений не коректно: %1.'"), ОписаниеОбласти.ИмяОбласти);
		КонецЕсли;
...
Процедура ПрисоединитьОбласть(Знач ПечатнаяФорма,
	...
	Попытка
		...
		Если ПечатнаяФорма.Тип = "DOC" Тогда
			...
			ИначеЕсли	ОписаниеОбласти.ТипОбласти = "СтрокаТаблицы" Тогда
				...
				КонецЕсли;
			Иначе
				ВызватьИсключение(НСтр("ru='Тип области не указан или указан не корректно.';uk='Тип області не зазначений або зазначений не коректно.'"));
			КонецЕсли;
	...
	Исключение
		...
		СообщениеОбОшибке = СообщениеОбОшибке + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при попытке вывести область ""%1"" из макета.';uk='Помилка при спробі вивести область ""%1"" з макета.'"),
...
Процедура ПрисоединитьИЗаполнитьКоллекцию(Знач ПечатнаяФорма,
	...
	Если ПечатнаяФорма.Тип = "DOC" Тогда
		...
		Иначе
			ВызватьИсключение(НСтр("ru='Тип области не указан или указан не корректно';uk='Тип області не зазначений або зазначений не коректно'"));
		КонецЕсли;
		ОписаниеОбласти.Вставить("ПереходНаСледующуюСтроку", ПереходНаСледСтроку);
