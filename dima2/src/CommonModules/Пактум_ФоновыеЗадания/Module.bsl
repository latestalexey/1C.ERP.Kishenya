...
Функция ПолучитьКонтрагентJSON(Знач КодПоЕДРПОУ, Знач Токен, КоличествоЦиклов, ОписаниеОшибки)
	...
	WinHttp.Open("Get","https://pactumsys.com/api/v1/cba2911c-01f9-4ca7-833a-46fb0cc079f2/contractors/"+КодПоЕДРПОУ+"?source=1c;skipRegisters=2",0); 
	...
	Если КонтрагентJSON.Свойство("Status") Тогда	//Все ОК
		...
		Иначе
			...
			Если СтатусЕДР = "Done" Тогда
				Возврат КонтрагентJSON;
            Иначе
                ОписаниеОшибки = НСтр("ru='Ошибка при работе с сервисом';uk='Помилка при роботі з сервісом'");
