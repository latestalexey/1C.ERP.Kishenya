...
Функция ПолучитьПреобразованиеXSLИзОбщегоМакета(Знач ИмяОбщегоМакета) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Получение XSL';uk='Одержання XSL'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ТипПлатформыСервера() Экспорт
	...
	ИначеЕсли ТипПлатформыСервераСтрокой = "MacOS_x86" Тогда
		Возврат ТипПлатформы.MacOS_x86;
		
	ИначеЕсли ТипПлатформыСервераСтрокой = "MacOS_x86_64" Тогда
		Возврат ТипПлатформы.MacOS_x86_64;
...
Функция РазделенныеОбъектыМетаданных(Знач Разделитель) Экспорт
	...
	Если МетаданныеОбщегоРеквизита = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Общий реквизит %1 не обнаружен в конфигурации!';uk='Загальний реквізит %1 не виявлений у конфігурації!'"), Разделитель);
	КонецЕсли;
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для общего реквизита %1 не используется разделение данных!';uk='Для загального реквізиту %1 не використовується розподіл даних!'"), Разделитель);
	...
	Для Каждого МетаданныеПоследовательности Из Метаданные.Последовательности Цикл
		...
		Если МетаданныеПоследовательности.Документы.Количество() = 0 Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Получение разделенных объектов метаданных';uk='Одержання розділених об''єктів метаданих'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, 
...
Функция ДанныеКэшаВерсий(Знач Идентификатор, Знач ТипДанных, Знач ПараметрыПолучения, Знач ИспользоватьУстаревшиеДанные = Истина) Экспорт
	...
	Если ТребуетсяВыполнитьОбновление Тогда
		...
		Иначе
			...
			НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Обновление кэша версий. Идентификатор записи %1. Тип данных %2.';uk='Оновлення кеша версій. Ідентифікатор запису %1. Тип даних %2.'"),
				Идентификатор,
				ТипДанных);
		...
		Если ТребуетсяПеречитатьДанные Тогда
			...
			Если Результат.Пустой() Тогда
				ШаблонСообщения = НСтр("ru='Ошибка при обновлении кэша версий. Данные не получены.
                    ...
                    |Тип данных: %2'
                    |;uk='Помилка при оновленні кеша версій. Дані не отримані.
                    |Ідентифікатор запису: %1
                    |Тип даних: %2'");
...
Функция ОпцииИнтерфейса() Экспорт
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru='Настройка интерфейса';uk='Настройка інтерфейсу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='При получении опций интерфейса произошла ошибка:
               |%1'
               |;uk='При одержанні опцій інтерфейсу відбулася помилка:
               |%1'"),
