...
Процедура ПриЗавершенииРаботыСистемы(Предупреждения)
	...
	ПредупреждениеПользователю.ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Открыть список редактируемых файлов (%1)';uk='Відкрити список файлів, що редагуються (%1)'"),
...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
	...
	Если Не ПараметрыВыполнения.Свойство("ПолноеИмяФайла") Тогда
		...
		ДиалогВыбораФайла.Заголовок = НСтр("ru='Выбор файла';uk='Вибір файлу'");
	...
	ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Идет сохранение файла ""%1"" (%2 Мб).
                   ...
                   |;uk='Іде збереження файлу ""%1"" (%2 Мб).
	...
	ИначеЕсли Не ЗначениеЗаполнено(Редактирует) Тогда
		СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Невозможно освободить файл ""%1"",
                       ...
                       |;uk='Неможливо звільнити файл ""%1"",
                       |т.к. він ніким не зайнятий.'"),
			Строка(ОбъектСсылка));
		Возврат Ложь;
	Иначе
		...
		СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Невозможно освободить файл ""%1"",
                       ...
                       |;uk='Неможливо звільнити файл ""%1"",
                       |т.к. він зайнятий користувачем ""%2"".'"),
			Строка(ОбъектСсылка),
			Строка(Редактирует));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОсвободитьФайлБезВопроса(ДанныеФайла, УникальныйИдентификатор = Неопределено)
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Файл освобожден';uk='Файл звільнений'"),
...
Процедура ПеренестиФайлыВПапку(ОбъектыСсылка, Папка) Экспорт
	...
	Для Каждого ДанныеФайла Из ДанныеФайлов Цикл
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru='Перенос файла';uk='Перенос файлу'"),
			ДанныеФайла.НавигационнаяСсылка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Файл ""%1""
                           |перенесен в папку ""%2"".'
                           |;uk='Файл ""%1""
                           |перенесений у папку ""%2"".'"),
...
Процедура ЗакончитьРедактированиеСРасширением(ПараметрыВыполнения)
			...
			Если Не ПустаяСтрока(ПараметрыВыполнения.ПолныйПутьКФайлу) Тогда
				СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось поместить файл
                               ...
                               |
                               |Освободить файл?'
                               |;uk='Не вдалося помістити файл
                               |""%1"" (%2),
                               |у сховище файлів, тому що він не знайдений у робочому каталозі.
                               |
                               |Звільнити файл?'"),
			...
			Иначе
				СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось поместить файл ""%1"",
                               ...
                               |
                               |Освободить файл?'
                               |;uk='Не вдалося помістити файл ""%1"",
                               |у сховище файлів, тому що він не знайдений у робочому каталозі.
                               |
                               |Звільнити файл?'"),
					Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка));
	...
	Исключение
		ТекстОшибки = НСтр("ru='Не удалось поместить файл ""%1"" в хранилище файлов,
            ...
            |;uk='Не вдалося помістити файл ""%1"" у сховище файлів,
	...
	Иначе
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Выполняется передача файла ""%1"" (%2 Мб)...
                       |Пожалуйста, подождите.'
                       |;uk='Виконується передача файлу ""%1"" (%2 Мб)...
                       |Будь ласка, почекайте.'"),
		...
		Исключение
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |
                |Повторить операцию?'
                |;uk='Не вдалось помістити файл в сховище файлів з причини:
                |""%1"".
                |
                |Повторити операцію?'"),
	...
	Если ПараметрыВыполнения.ПоказыватьОповещение Тогда
		Если ПараметрыВыполнения.ВерсияОбновлена Тогда
			ШаблонПояснения = НСтр("ru='Файл ""%1""
                                         ...
                                         |;uk='Файл ""%1""
                                         |оновлено й звільнено.'");
		Иначе
			ШаблонПояснения = НСтр("ru='Файл ""%1""
                                         |не изменился и освобожден.'
                                         |;uk='Файл ""%1""
                                         |не змінився й звільнений.'");
		КонецЕсли;
		
		ПоказатьОповещениеПользователя(
			НСтр("ru='Редактирование закончено';uk='Редагування закінчене'"),
...
Процедура ЗакончитьРедактированиеБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт
	...
	ОписаниеДанных.Вставить("Операция",            НСтр("ru='Шифрование файла';uk='Шифрування файлу'"));
...
Процедура ОбновитьИзФайлаНаДиске(ОбработчикРезультата, ДанныеФайла, ИдентификаторФормы)
		...
		#Если Не ВебКлиент Тогда
			Если СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ЭтоБазоваяВерсияКонфигурации Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Данная команда не поддерживается в базовой версии.';uk='Дана команда не підтримується в базовій версії.'"));
				ВернутьРезультат(ОбработчикРезультата, Ложь);
				Возврат;
			КонецЕсли;
		#КонецЕсли
	...
	Диалог.Заголовок                   = НСтр("ru='Выбор файла';uk='Вибір файлу'");
	...
	Если ЗначениеЗаполнено(РасширениеДляЗашифрованныхФайлов) Тогда
		Фильтр = НСтр("ru='Файл (*.%1)|*.%1|Зашифрованный файл (*.%2)|*.%2|Все файлы (*.*)|*.*';uk='Файл (*.%1)|*.%1|Зашифрований файл (*.%2)|*.%2|Усі файли (*.*)|*.*'");
	...
	Иначе
		Фильтр = НСтр("ru='Файл (*.%1)|*.%1|Все файлы (*.*)|*.*';uk='Файл (*. %1)|*. %1|Все файли (*.*)|*.*'");
...
Процедура ОбновитьИзФайлаНаДискеПередРасшифровкой(ПомещаемыеФайлы, ПараметрыВыполнения) Экспорт
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",              НСтр("ru='Расшифровка файла';uk='Розшифровка файлу'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных",       НСтр("ru='Файл';uk='Файл'"));
	...
	Если ПараметрыВыполнения.ВремяИзмененияВыбранное < ПараметрыВыполнения.ДатаФайлаВБазе Тогда // В хранилище более новый.
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл ""%1""
                       ...
                       |Операция прервана.'
                       |;uk='Файл ""%1""
                       |у сховище файлів має більш пізню дату зміни (%2),
                       |чим обраний файл (%3).
                       |
                       |Операція перервана.'"),
			Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
			...
			Если ПараметрыВыполнения.ВремяИзмененияВыбранное < ВремяИзмененияВКэше Тогда // В рабочем каталоге более новый.
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Файл ""%1""
                               ...
                               |Операция прервана.'
                               |;uk='Файл ""%1""
                               |у робочому каталозі має більше пізню дату зміни (%2),
                               |чим обраний файл (%3).
                               |
                               |Операція перервана.'"),
					Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
				...
				Исключение
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Файл ""%1""
                                   ...
                                   |;uk='Файл ""%1""
                                   |у робочому каталозі відкритий для редагування.
                                   |
                                   |Закінчіть редагування перед виконанням
                                   |оновлення з файлу на диску.'"),
						ПолноеИмяФайла);
...
Процедура ЗанятьФайлПоСсылкеПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Редактирование файла';uk='Редагування файлу'"),
		ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл ""%1""
                       |занят для редактирования.'
                       |;uk='Файл ""%1""
                       |зайнятий для редагування.'"), Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка)),
		БиблиотекаКартинок.Информация32);
...
Процедура ЗанятьФайлыПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Занять файлы';uk='Зайняти файли'"),
		,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файлы (%1 из %2) заняты для редактирования.';uk='Файли (%1 з %2) зайняті для редагування.'"),
			КоличествоЗанятых,
...
Процедура ОсвободитьФайлыПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьВопрос(
		...
		НСтр("ru='Отмена редактирования файлов может
                   ...
                   |
                   |Продолжить?'
                   |;uk='Скасування редагування файлів може
                   |привести до втрати Ваших змін.
                   |
                   |Продовжити?'"),
...
Процедура ОсвободитьФайлыПоСсылкамПослеОтветаНаВопросОтменитьРедактирование(Ответ, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Отменить редактирование файлов';uk='Скасувати редагування файлів'"),,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Отменено редактирование файлов (%1 из %2).';uk='Скасовано редагування файлів (%1 з %2).'"),
	...
	Если ПараметрыВыполнения.НеЗадаватьВопрос = Ложь Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Отмена редактирования файла
                       ...
                       |
                       |Продолжить?'
                       |;uk='Скасування редагування файлу
                       |""%1""
                       |може привести до втрати ваших змін.
                       |
                       |Продовжити?'"),
			Строка(ПараметрыВыполнения.ОбъектСсылка));
...
Процедура ОсвободитьФайлПослеОтветаНаВопросОтменаРедактирования(Ответ, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ПродолжитьРаботу Тогда
		...
		Если Не ПараметрыВыполнения.НеЗадаватьВопрос Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ru='Файл освобожден';uk='Файл звільнений'"),
				ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
		...
		Если Не ПустаяСтрока(ПараметрыВыполнения.ПолныйПутьКФайлу) Тогда
			СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось поместить файл ""%1"" в хранилище файлов:
                           ...
                           |Освободить файл?'
                           |;uk='Не вдалося помістити файл ""%1"" у сховище файлів:
                           |Файл не знайдений у робочому каталозі:
                           | %2.
                           |
                           |Звільнити файл?'"),
				Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
...
Процедура СохранитьИзмененияФайлаСРасширениемПослеПроверкиЗашифрован(Неопределен, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ПоказыватьОповещение Тогда
		Если ВерсияОбновлена Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ru='Новая версия сохранена';uk='Нова версія збережена'"),
				ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
				ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии,
				БиблиотекаКартинок.Информация32);
		Иначе
			ПоказатьОповещениеПользователя(
				НСтр("ru='Новая версия не сохранена';uk='Нова версія не збережена'"),,
				НСтр("ru='Файл не изменился';uk='Файл не змінився'"),
				БиблиотекаКартинок.Информация32);
	...
	Если МассивСертификатов.Количество() = 0 Тогда
		ПоказатьПредупреждение(,
			НСтр("ru='У зашифрованного файла не указаны сертификаты.
                       ...
                       |;uk='У зашифрованого файлу не вказані сертифікати.
                       |Розшифруйте файл і зашифруйте заново.'"));
	...
	Если ПараметрыВыполнения.ПолноеИмяФайла = "" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Ошибка получения файла из хранилища файлов
                       |в рабочий каталог.'
                       |;uk='Помилка одержання файлу зі сховища файлів
                       |у робочий каталог.'"));
...
Процедура КаталогФайла(ОбработчикРезультата, ДанныеФайла) Экспорт
	...
	КнопкиВопроса.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Сохранить и открыть каталог';uk='Зберегти і відкрити каталог'"));
	КнопкиВопроса.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Отмена';uk='Відмінити'"));
	ПоказатьВопрос(Обработчик,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Каталог файла не существует. Возможно, на данном компьютере файл ""%1"" еще не открывался.
            ...
            |;uk='Каталог файлу не існує. Можливо, на даному комп''ютері файл ""%1"" ще не відкривався.
            |Зберегти файл на комп''ютер і відкрити каталог?'"),
			ИмяФайла),
		КнопкиВопроса);
...
Процедура ОсвободитьМестоВРабочемКаталоге(ОбработчикРезультата, РеквизитыВерсии)
	...
	#Если ВебКлиент Тогда
		...
		ВернутьРезультатПослеПоказаПредупреждения(
			ОбработчикРезультата,
			НСтр("ru='Очистка рабочего каталога не поддерживается в веб-клиенте.';uk='Очищення робочого каталогу не підтримується у веб-клиєнті.'"),
			Неопределено);
		Возврат;
	#КонецЕсли
...
Процедура ОчиститьРабочийКаталог(ОбработчикРезультата, РазмерФайловВРабочемКаталоге, РазмерДобавляемогоФайла, ОчищатьВсе) Экспорт
	...
	#Если ВебКлиент Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ОбработчикРезультата, НСтр("ru='Очистка рабочего каталога не поддерживается в веб-клиенте.';uk='Очищення робочого каталогу не підтримується у веб-клиєнті.'"), Неопределено);
		Возврат;
	#КонецЕсли
...
Процедура ОчиститьРабочийКаталогОбработчикЦикла(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.НомерШага = 1 Тогда
		Если Не ПараметрыВыполнения.ДаДляВсех
			...
			Если ПараметрыВыполнения.ОчищатьВсе = Ложь Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Выполняется очистка рабочего каталога при добавлении файла.
                               ...
                               |;uk='Виконується очищення робочого каталогу при додаванні файлу.
                               |
                               |У сховище файлів не знайдено файл
                               |""%1"".
                               |
                               |Вилучити його з робочого каталогу?'"),
			...
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В хранилище файлов не найден файл
                               ...
                               |
                               |Удалить его из рабочего каталога?'
                               |;uk='У сховище файлів не знайдений файл
                               |""%1"".
                               |
                               |Вилучити його з робочого каталогу?'"),
					ПараметрыВыполнения.ИмяКаталога + ПараметрыВыполнения.Элемент.Путь);
			...
			Кнопки.Добавить("Да",         НСтр("ru='Да';uk='Так'"));
			Кнопки.Добавить("ДаДляВсех",  НСтр("ru='Да для всех';uk='Так для всіх'"));
			Кнопки.Добавить("Нет",        НСтр("ru='Нет';uk='Ні'"));
			Кнопки.Добавить("НетДляВсех", НСтр("ru='Нет для всех';uk='Ні для всіх'"));
	...
	Если ПараметрыВыполнения.НомерШага = 2 Тогда
		...
		ФайлНаДиске.УстановитьТолькоЧтение(Ложь);
		Если ПараметрыВыполнения.ОчищатьВсе = Ложь Тогда
			ШапкаВопроса = НСтр("ru='Выполняется очистка рабочего каталога при добавлении файла.';uk='Виконується очищення робочого каталогу при додаванні файлу.'");
		Иначе
			ШапкаВопроса = НСтр("ru='Выполняется очистка рабочего каталога.';uk='Виконується очищення робочого каталогу.'");
		КонецЕсли;
				...
				Если СтрДлина(ПараметрыВыполнения.ИмяФайла) > МаксимальнаяДлинаИмениФайла Тогда
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Длина пути к файлу (рабочий каталог плюс имя файла) превышает %1 символов
                                   |%2'
                                   |;uk='Довжина шляху до файлу (робітник каталог плюс ім''я файлу) перевищує %1 символів
                                   |%2'"),
						ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути,
						ПараметрыВыполнения.ПолноеИмяФайла);
					
					ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК
						+ НСтр("ru='Измените имя файла на более короткое.';uk='Зміните ім''я файлу на більш коротке.'");
...
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловПредложитьВыборКаталога(Ответ, ПараметрыВыполнения)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
        ...
        |;uk='Довжина шляху до файлу перевищує %1 символів:
        | %2
        |
        |Вибрати інший основний робочий каталог?'"),
		ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути,
		ПараметрыВыполнения.ПолноеИмяФайла);
...
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловНачатьВыборКаталога(Ответ, ПараметрыВыполнения) Экспорт
	...
	Заголовок = НСтр("ru='Выберите другой основной рабочий каталог';uk='Виберіть інший основний робочий каталог'");
		...
		Если ПараметрыВыполнения.ДанныеФайла.Владелец = ВРегистреПапка Тогда // Одна и та же папка.
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В рабочем каталоге уже есть файл
                           ...
                           |;uk='У робочому каталозі уже є файл
                           |""%1"",
                           |пов''язаний з іншим файлом в інформаційній базі.
                           |
                           |Код файлу в сховище файлів: %2.
                           |Код файлу в робочому каталозі: %3.
                           |
                           |Перейменуйте один з файлів в інформаційній базі.'"),
				ПараметрыВыполнения.ПолноеИмяФайла,
		...
		Иначе
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В рабочем каталоге уже есть файл
                           ...
                           |;uk='У робочому каталозі уже є файл
                           |""%1"",
                           |пов''язаний з іншим файлом інформаційної бази.
                           |
                           |Код файлу в сховище файлів: %2.
                           |Код файлу в робочому каталозі: %3.
                           |
                           |В інформаційній базі зміните робочий каталог однієї з папок.
                           |(У двох папок не повинне бути однакового робочого каталогу).'"),
				ПараметрыВыполнения.ПолноеИмяФайла,
				ПараметрыВыполнения.ДанныеФайла.КодФайла,
				ВРегистреКодФайла);
		КонецЕсли;
...
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеЗапуск(ПараметрыВыполнения)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Длина полного пути к файлу (рабочий каталог плюс имя файла) превышает %1 символов
                   |""%2"".'
                   |;uk='Довжина повного шляху до файлу (робітник каталог плюс ім''я файлу) перевищує %1 символів
                   |""%2"".'"),
		ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути,
		ПараметрыВыполнения.ПолноеИмяФайла);
	...
	Если СтрДлина(ПараметрыВыполнения.ДанныеФайла.РабочийКаталогВладельца) > ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути - 5 Тогда
		ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК
			+ НСтр("ru='Измените имена папок или перенесите текущую папку в другую папку.';uk='Зміните імена папок або перенесіть поточну папку в іншу папку.'");
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстСообщения, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеПредложитьВыборКаталога(ПараметрыВыполнения)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
        ...
        |
        |Вибрати інший основний робочий каталог?
        |(Вміст робочого каталогу буде перенесено в обраний каталог).'"),
...
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеНачатьВыборКаталога(Ответ, ПараметрыВыполнения) Экспорт
	...
	// Выбираем другой путь к рабочему каталогу.
	Заголовок = НСтр("ru='Выберите другой рабочий каталог';uk='Виберіть інший робочий каталог'");
	...
	Если Результат.ВозниклаОшибка Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось скопировать файл
                       ...
                       |
                       |Повторить операцию?'
                       |;uk='Не вдалося скопіювати файл
                       |""%1"".
                       |Можливо він зайнятий іншою прикладною програмою.
                       |
                       |Повторити операцію?'"),
	...
	Если СтрНайти(НРег(КаталогПриемника), НРег(КаталогИсточника)) <> 0 Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Выбранный рабочий каталог
                       ...
                       |""%2"".'
                       |;uk='Обраний робочий каталог
                       |""%1""
                       |входить у старий робочий каталог
                       |""%2"".'"),
			КаталогПриемника,
			КаталогИсточника);
	...
	Если Результат.ВозниклаОшибка Тогда
		...
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось скопировать обратно содержимое каталога
                       ...
                       |в каталог
                       |""%2"".'
                       |;uk='Не вдалося скопіювати назад уміст каталогу
                       |""%1""
                       |у каталог
                       |""%2"".'"),
			ПараметрыВыполнения.КаталогПриемника,
...
Процедура ИмпортФайловПослеПроверкиРазмеров(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.КоличествоСуммарное = 0 Тогда
		Если ПараметрыВыполнения.РежимЗагрузки Тогда
			ВернутьРезультат(ПараметрыВыполнения.ОбработчикРезультата, Неопределено);
		Иначе
			ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, НСтр("ru='Нет файлов для добавления';uk='Немає файлів для додавання'"), Неопределено);
		КонецЕсли;
		Возврат;
	КонецЕсли;
				...
				Иначе
					ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Папка ""%1"" уже существует.
                                   ...
                                   |;uk='Папка ""%1"" уже існує.
                                   |
                                   |Продовжити імпорт папки?'"),
...
Процедура ИмпортФайловПослеЦиклаПродолжение(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество() > 1 Тогда
		...
		ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Импорт файлов завершен. Импортировано файлов: %1';uk='Імпорт файлів завершено. Імпортовано файлів: %1'"), Строка(ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество()) );
			
		Если ПараметрыВыполнения.РежимЗагрузки Тогда
			ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Загрузка файлов завершена. Загружено файлов: %1';uk='Завантаження файлів завершене. Додано файлів: %1'"), Строка(ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество()) );
		КонецЕсли;
	...
	Если ПараметрыВыполнения.МассивИменФайловСОшибками.Количество() <> 0 Тогда
		...
		Если ПараметрыВыполнения.РежимЗагрузки Тогда
			Параметры.Вставить("Заголовок", НСтр("ru='Отчет о загрузке файлов';uk='Звіт про завантаження файлів'"));
		КонецЕсли;
			...
			Если ДатаФайлаНаДиске > ДатаФайлаВБазе Тогда // В рабочем каталоге более новый (изменен пользователем со стороны).
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Дата изменения файла ""%1""
                               ...
                               |;uk='Дата зміни файлу ""%1""
                               |у робочому каталозі більш пізня (новіша), чим у сховищі файлів.
                               |Можливо, файл був змінений.'"),
					Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка));
...
Процедура СохранитьКакСРасширениемПослеРасшифровки(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Все файлы (*.%1)|*.%1';uk='Всі файли (*. %1)|*. %1'"), Расширение);
	...
	СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Выполняется сохранение файла ""%1"" (%2 Мб)...
                   |Пожалуйста, подождите.'
                   |;uk='Виконується збереження файлу ""%1"" (%2 Мб)...
                   |Будь ласка, почекайте.'"),
		ИмяФайла, 
		ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМб));
	...
	Состояние(НСтр("ru='Файл успешно сохранен';uk='Файл успішно збережений'"), , ПолноеИмяФайла);
...
Процедура ПроверитьПредельныйРазмерФайлов(ОбработчикРезультата, ПараметрыПроверки)
		...
		Если ВыбранКаталог Тогда
			Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Идет сбор информации о каталоге
                           |""%1"".
                           |Пожалуйста, подождите.'
                           |;uk='Іде збір інформації про каталог
                           |""%1"".
                           |Будь ласка, почекайте.'"),
				Путь));
	...
	Если МассивСлишкомБольшихФайлов.Количество() <> 0 Тогда 
		...
		Для Каждого Файл Из МассивСлишкомБольшихФайлов Цикл
			...
			ТекстСтроки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 (%2 МБ)';uk='%1 (%2 МБ)'"), Строка(Файл), Строка(РазмерФайлаВМб));
		...
		Параметры.Вставить("Заголовок", НСтр("ru='Предупреждение при загрузке файлов';uk='Попередження при завантаженні файлів'"));
...
Процедура Зашифровать(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор) Экспорт
	...
	Если ПараметрыВыполнения.ДанныеФайла.Зашифрован Тогда
		ТекстПредупреждения = НСтр("ru='Файл ""%1"" уже зашифрован.';uk='Файл ""%1"" уже зашифрований.'");
	...
	Если ЗначениеЗаполнено(ПараметрыВыполнения.ДанныеФайла.Редактирует) Тогда
		ТекстПредупреждения = НСтр("ru='Нельзя зашифровать занятый файл.';uk='Не можна зашифрувати зайнятий файл.'");
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстПредупреждения, ПараметрыВыполнения);
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыВыполнения.ДанныеФайла.КоличествоВерсий > 1 Тогда
		ПредставлениеФайла = ПредставлениеФайла + " (" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Версий: %1';uk='Версій: %1'"), ПараметрыВыполнения.ДанныеФайла.КоличествоВерсий) + ")";
	КонецЕсли;
	...
	ОписаниеДанных.Вставить("ПредставлениеНабора", НСтр("ru='Файлы (%1)';uk='Файли (%1)'"));
...
Процедура ДобавитьИзФайловойСистемыБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт
	...
	Если ПустаяСтрока(СтруктураПути.Расширение) Тогда
		ТекстВопроса = НСтр("ru='Необходимо указать файл с расширением.';uk='Необхідно вказати файл із розширенням.'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Повторить, НСтр("ru='Выбрать другой файл';uk='Вибрати інший файл'"));
...
Процедура ИнформироватьОШифровании(МассивФайловВРабочемКаталогеДляУдаления,
	...
	МодульЭлектроннаяПодписьКлиент.ИнформироватьОШифрованииОбъекта(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл: %1';uk='Файл: %1'"), ФайлСсылка));
...
Процедура ИмпортФайловРекурсивноБезДиалогов(Знач Владелец, Знач ВыбранныеФайлы, Знач ПараметрыВыполнения, Знач ЗадаватьВопросПапкаУжеСуществует)
	...
	Для Каждого ВыбранныйФайл Из ВыбранныеФайлы Цикл
		...
		Если Не ВыбранныйФайл.Существует() Тогда
			...
			Запись.Вставить("Ошибка", НСтр("ru='Файл отсутствует на диске.';uk='Файл відсутній на диску.'"));
		...
		Попытка
			...
			НадписьПодробнее = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Обрабатывается файл ""%1"" (%2 Мб)...';uk='Обробляється файл ""%1"" (%2 Мб)...'"),
				ВыбранныйФайл.Имя, 
				ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМб));
				
			ТекстСостояния = НСтр("ru='Импорт файлов с диска...';uk='Імпорт файлів з диска...'");
			Если ПараметрыВыполнения.РежимЗагрузки Тогда
				ТекстСостояния = НСтр("ru='Загрузка файлов с диска...';uk='Завантаження файлів з диска...'");
			КонецЕсли;
			...
			Если ПараметрыВыполнения.РежимЗагрузки Тогда
				Если РаботаСФайламиСлужебныйВызовСервера.ЕстьФайлСТакимИменем(ИмяБезРасширения, Владелец) Тогда
					Запись = Новый Структура;
					Запись.Вставить("ИмяФайла", ВыбранныйФайл.ПолноеИмя);
					Запись.Вставить("Ошибка", НСтр("ru='Файл с таким именем уже есть в хранилище файлов.';uk='Файл із таким ім''ям уже є в сховище файлів.'"));
					ПараметрыВыполнения.МассивИменФайловСОшибками.Добавить(Запись);
					Продолжить;
				КонецЕсли;
			КонецЕсли;
	...
	Иначе
		...
		Если ОбщееКоличество > 0 Тогда 
			...
			ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Файлы (%1 шт) перенесены в %2';uk='Файли (%1 шт) перенесені в %2'"),
				ОбщееКоличество,
				ВладелецФайла);
			
			ПоказатьОповещениеПользователя(
				НСтр("ru='Файлы перенесены';uk='Файли перенесені'"),
				,
				ПолноеОписание,
				БиблиотекаКартинок.Информация32);
...
Функция ПроверитьЗанятыеФайлыПриЗавершенииРаботы()
	...
	ПараметрыПрикладнойФормыПредупреждения.Вставить("СообщениеВопрос",      НСтр("ru='Завершить работу с программой?';uk='Завершити роботу із програмою?'"));
	ПараметрыПрикладнойФормыПредупреждения.Вставить("СообщениеЗаголовок",   НСтр("ru='Следующие файлы заняты вами для редактирования:';uk='Наступні файли зайняті вами для редагування:'"));
	ПараметрыПрикладнойФормыПредупреждения.Вставить("Заголовок",            НСтр("ru='Завершение работы';uk='Завершення роботи'"));
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Неверный путь или отсутствуют права на запись в каталог
                           |""%1"".'
                           |;uk='Невірний шлях або відсутні права на запис у каталог
                           |""%1"".'"),
				ИмяКаталога);
			...
			Если МассивФайловВКаталоге.Количество() <> 0 Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В выбранном рабочем каталоге
                               ...
                               |;uk='В обраному робочому каталозі
                               |""%1""
                               |уже є файли.
                               |
