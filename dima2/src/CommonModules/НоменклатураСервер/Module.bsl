...
// Параметры:
//		УпаковкаЕдиницаИзмерения - СправочникСсылка.УпаковкиЕдиницыИзмерения
// Возвращаемое значение:
//		Число
...
Процедура ЗаполнитьСерииПоFEFO(Объект,ПараметрыУказанияСерий, СтатусыУказанияСерийЗаполнены = Истина) Экспорт
	...
	Если ПараметрыУказанияСерий.ИмяТЧТовары = ПараметрыУказанияСерий.ИмяТЧСерии Тогда 
		Если ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.МаршрутныйЛистПроизводства" Тогда
			...
			//++ НЕ УТКА
			ЗаполнитьСерииПоFEFOВМаршрутномЛисте(Объект, ПараметрыУказанияСерий);
			//-- НЕ УТКА
		Иначе
...
Процедура ЗаполнитьСерииПоFEFOВМаршрутномЛисте(Объект, ПараметрыУказанияСерий)
	...
	|	&ТаблицаТоваров КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.СтатусУказанияСерий В (5, 6, 25)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
    ...
    |						&Склад
    |					ИЗ
    |						ТаблицаТоваров)) КАК МатериалыВПроизводствеОстатки
    |
    |	ОБЪЕДИНИТЬ ВСЕ
    |
    |	ВЫБРАТЬ
    ...
    |			ПО МатериалыИРаботыВПроизводстве.Номенклатура = ТаблицаТоваров.Номенклатура
    |				И МатериалыИРаботыВПроизводстве.Характеристика = ТаблицаТоваров.Характеристика
    |				И МатериалыИРаботыВПроизводстве.Назначение = ТаблицаТоваров.Назначение
    |				И МатериалыИРаботыВПроизводстве.Подразделение = &Склад
    |				И МатериалыИРаботыВПроизводстве.Регистратор = &Ссылка) КАК СерииТоваров
	|ГДЕ
	|	СерииТоваров.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	...
	ОстаткиСерий.Индексы.Добавить("Номенклатура,Характеристика,Назначение");
	
	СтатусУказанияСерийСерииНеУказаны = ?(ПараметрыУказанияСерий.ФактОтбора, 5, 25);
	
	КэшированныеЗначения = ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПересчитатьКоличествоУпаковок", Новый Структура("НужноОкруглять", Ложь));
	СтруктураДействий.Вставить("ПересчитатьКоличествоУпаковокСуффикс", Новый Структура("НужноОкруглять,Суффикс", Ложь, "Факт"));
	СтруктураДействий.Вставить("ПересчитатьКоличествоОтклонение", Истина);
	
	СтрокиТоваров = Новый Массив;
	Для каждого СтрокаТоваров Из Объект[ПараметрыУказанияСерий.ИмяТЧТовары] Цикл
		Если (СтрокаТоваров.СтатусУказанияСерий = 5
				ИЛИ СтрокаТоваров.СтатусУказанияСерий = 25)
			И СтрокаТоваров.КоличествоФакт <> 0 Тогда
					
			СтрокаТоваров.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
			СтрокаТоваров.СтатусУказанияСерий = СтатусУказанияСерийСерииНеУказаны;
	...
	Для Каждого СтрокаТоваров из СтрокиТоваров Цикл
		...
		НайденныеОстаткиСерий = ОстаткиСерий.НайтиСтроки(СтруктураПоиска);
		
		ОстатокТовара = СтрокаТоваров.КоличествоФакт;
		ОстатокТовараНорматив = СтрокаТоваров.Количество;
		
		ИзменяемаяСтрока = СтрокаТоваров;
		
		Для Каждого СтрокаОстатокСерии из НайденныеОстаткиСерий Цикл
			
			Если СтрокаОстатокСерии.СвободныйОстаток = 0 Тогда
				Продолжить;
			КонецЕсли;
			
			КоличествоВСтроку = Мин(ОстатокТовара, СтрокаОстатокСерии.СвободныйОстаток);
			
			ИзменяемаяСтрока.КоличествоФакт = КоличествоВСтроку;
			Если КоличествоВСтроку < ОстатокТовара Тогда
				ИзменяемаяСтрока.Количество = Мин(ОстатокТовараНорматив, КоличествоВСтроку);
			КонецЕсли;
			
			ИзменяемаяСтрока.Серия = СтрокаОстатокСерии.Серия;
			
			ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ИзменяемаяСтрока, СтруктураДействий, КэшированныеЗначения);
			
			ОстатокТовара = ОстатокТовара - ИзменяемаяСтрока.КоличествоФакт;
			ОстатокТовараНорматив = ОстатокТовараНорматив - ИзменяемаяСтрока.Количество;
			СтрокаОстатокСерии.СвободныйОстаток = СтрокаОстатокСерии.СвободныйОстаток - ИзменяемаяСтрока.КоличествоФакт;
			
			Если ОстатокТовара > 0 Тогда
				ИзменяемаяСтрока = Объект[ПараметрыУказанияСерий.ИмяТЧТовары].Вставить(ИзменяемаяСтрока.НомерСтроки);
				
				ЗаполнитьЗначенияСвойств(ИзменяемаяСтрока, СтрокаТоваров,,"Серия");
				ИзменяемаяСтрока.КоличествоФакт = ОстатокТовара;	
				ИзменяемаяСтрока.Количество = ОстатокТовараНорматив;	
				ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ИзменяемаяСтрока, СтруктураДействий, КэшированныеЗначения);
			Иначе
				Прервать;
			КонецЕсли;
	...
	// Сообщение что серии не заполнены
	ЕдиницыИзмерения = Неопределено;
	Для каждого СтрокаТоваров Из Объект[ПараметрыУказанияСерий.ИмяТЧТовары] Цикл
		
		Если НЕ(СтрокаТоваров.СтатусУказанияСерий = СтатусУказанияСерийСерииНеУказаны
				И (СтрокаТоваров.СтатусУказанияСерий = 5
					ИЛИ СтрокаТоваров.СтатусУказанияСерий = 25)
				И СтрокаТоваров.КоличествоФакт <> 0) Тогда
			Продолжить;
		КонецЕсли;
		
		Если ЕдиницыИзмерения = Неопределено Тогда
			Запрос = Новый Запрос;
			Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
			Запрос.Текст =
			"ВЫБРАТЬ
		...
		КоличествоНеРаспределено = 0;
		Если СтрокаТоваров.КоличествоФакт = 0 Тогда
			
			ТекстСообщения = НСтр("ru='Номенклатура ""%ПредставлениеТовара%"" - серии не заполнены, т.к. не указано количество.';uk='Номенклатура ""%ПредставлениеТовара%"" - серії не заповнені, тому що не зазначена кількість.'");
			
		Иначе
			
			ТекстСообщения = НСтр("ru='Номенклатура ""%ПредставлениеТовара%"" - серии не заполнены.
                |В подразделении ""%ПредставлениеСклада%"" не хватает ""%Количество%"" ""%ЕдиницаИзмерения%"".'
                |;uk='Номенклатура ""%ПредставлениеТовара%"" - серії не заповнені.
                |У підрозділі ""%ПредставлениеСклада%"" не вистачає ""%Количество%"" ""%ЕдиницаИзмерения%"".'");
				
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ПредставлениеСклада%", Объект[ПараметрыУказанияСерий.ИмяПоляСклад]);

			КоличествоНеРаспределено = СтрокаТоваров.КоличествоФакт;
			
		КонецЕсли;	
		
		ТекстСообщения = СтрЗаменить(ТекстСообщения, 
								"%ПредставлениеТовара%", 
								НоменклатураКлиентСервер.ПредставлениеНоменклатуры(СтрокаТоваров.Номенклатура, СтрокаТоваров.Характеристика));
		
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%Количество%", КоличествоНеРаспределено);
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ЕдиницаИзмерения%", ЕдиницыИзмерения.Найти(СтрокаТоваров.Номенклатура,"Номенклатура").ЕдиницаИзмерения);
...
// Определяет используется ли справочное указание серий в маршрутных листах
//
// Параметры:
//  Спецификация 			- СправочникСсылка.РесурсныеСпецификации - Проверка для указанной спецификации (если не указано, то для всех)
//  ВидСтрокиСпецификации	- ПеречислениеСсылка.ВидыСтрокДереваСпецификаций - Проверка для указанного вида строки спецификации (если не указано, то для всех видов строк)
// 
// Возвращаемое значение:
//   - Булево   - Истина, если серии указываются
//
Функция ИспользуетсяСправочноеУказаниеСерийВМаршуртныхЛистах(Спецификация = Неопределено, ВидСтрокиСпецификации = Неопределено) Экспорт

	УстановитьПривилегированныйРежим(Истина);
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ТребуетсяСправочноеУказаниеСерий.Спецификация
	|ИЗ
	|	РегистрСведений.ТребуетсяСправочноеУказаниеСерий КАК ТребуетсяСправочноеУказаниеСерий
	|ГДЕ
	|	(ТребуетсяСправочноеУказаниеСерий.Спецификация = &Спецификация
	|			ИЛИ &Спецификация = ЗНАЧЕНИЕ(Справочник.РесурсныеСпецификации.ПустаяСсылка))
	|	И (ТребуетсяСправочноеУказаниеСерий.ВидСтрокиСпецификации = &ВидСтрокиСпецификации
	|			ИЛИ &ВидСтрокиСпецификации = ЗНАЧЕНИЕ(Перечисление.ВидыСтрокДереваСпецификаций.ПустаяСсылка))";
	
	Запрос.УстановитьПараметр("Спецификация", Спецификация);
	Запрос.УстановитьПараметр("ВидСтрокиСпецификации", ВидСтрокиСпецификации);
...
Функция ДопустимыеТипыПолитикУчетаСерий(Параметры) Экспорт
	...
	ИначеЕсли  Параметры.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Товар Тогда
		...
		ИначеЕсли Параметры.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваровПоНомеру Тогда	
		...
		ИначеЕсли Параметры.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваровПоНомеруИСрокуГодности
			Или Параметры.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваровПоСрокуГодности Тогда
