...
Функция ТекстЗапросаСостояниеОтраженияДокументов(УчитыватьНДС = Истина) Экспорт
	...
	|ГДЕ
	|	ДанныеРегистра.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтраженияДокументовВРеглУчете.ОтраженоВРеглУчете)
	...
	|		РегистрСведений.ОтражениеДокументовВРеглУчете КАК ДанныеРегистра
	|	ГДЕ
	|		ДанныеРегистра.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтраженияДокументовВРеглУчете.КОтражениюВРеглУчете)
	|		И ДанныеРегистра.ДатаОтражения <= &ДатаОкончания
	|		И (ДанныеРегистра.Организация В (&МассивОрганизаций)
	|			ИЛИ &ПоВсемОрганизациям)
	|	
...
//++ НЕ УТ
Функция СостояниеКорректировкаНалоговогоНазначенияКапитальныхИнвестиций(СписокОрганизаций, Период) Экспорт
	...
	|ГДЕ
	|	НАЧАЛОПЕРИОДА(КорректировкаНалоговогоНазначенияКапитальныхИнвестиций.Дата, МЕСЯЦ) = &Период 
	|	И КорректировкаНалоговогоНазначенияКапитальныхИнвестиций.Проведен
	|	И (КорректировкаНалоговогоНазначенияКапитальныхИнвестиций.Организация В (&СписокОрганизаций)
	|		ИЛИ &ПоВсемОрганизациям)
	|";
	
	Запрос.УстановитьПараметр("Период",             Период);
	Запрос.УстановитьПараметр("Операция",           Перечисления.ОперацииЗакрытияМесяца.КорректировкаНалоговогоНазначенияКапитальныхИнвестиций);
	Запрос.УстановитьПараметр("СписокОрганизаций",  СписокОрганизаций);
	Запрос.УстановитьПараметр("ПоВсемОрганизациям", НЕ ЗначениеЗаполнено(СписокОрганизаций));
	
	Результат = Запрос.ВыполнитьПакет();
	ЕстьЗаданияКРасчету  = НЕ Результат[0].Пустой();
	ЕстьКорректировкаНалоговогоНазначенияКапитальныхИнвестиций = НЕ Результат[1].Пустой();
	
	Если ЕстьЗаданияКРасчету Тогда
		Состояние = Перечисления.СостоянияОперацийЗакрытияМесяца.НеВыполнено;
	ИначеЕсли ЕстьКорректировкаНалоговогоНазначенияКапитальныхИнвестиций Тогда 
...
Процедура РассчитатьЭтап(ИмяЭтапаРасчета, Отказ = Ложь, ПараметрыРасчета)
	...
	ИначеЕсли ИмяЭтапаРасчета = "КорректировкаНалоговогоНазначенияКапитальныхИнвестиций" Тогда
		КорректировкаНалоговогоНазначенияКапитальныхИнвестиций(Отказ, ПараметрыРасчета);
	//-- НЕ УТ
...
//++ НЕ УТ
Процедура КорректировкаНалоговогоНазначенияКапитальныхИнвестиций(Отказ, ПараметрыРасчета)	
	
	НомерЗадания = ТекущийНомерЗадания();
	УвеличитьНомерЗадания(НомерЗадания);
	ОперацияРасчета = Перечисления.ОперацииЗакрытияМесяца.КорректировкаНалоговогоНазначенияКапитальныхИнвестиций;
	...
	Исключение
		...
		Отказ = Истина;
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Закрытие месяца.Корректировка налогового назначения капитальных инвестиций.';uk='Закриття місяця.Коригування податкового призначення капітальних інвестицій.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
...
Процедура РасходыНаПродукцию(Отказ, ПараметрыРасчета)
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьПартионныйУчет") Тогда
		...
		СебестоимостьРассчитана = РасчетСебестоимости.РассчитатьВсеВПопыткеИсключении(
			...
			ПараметрыРасчета.Организация,
			,
			Новый Структура("МестоВызоваРасчета", "ЗакрытиеМесяцаУТВызовСервера.РасходыНаПродукцию"));
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Закрытие месяца.Распределение расходов на продукцию.';uk='Закриття місяця.Розподіл витрат на продукцію.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Метаданные.ОбщиеМодули.ПартионныйУчет,
			,
			ТекстОшибки);
...
Процедура РеглУчет(Отказ, ПараметрыРасчета)
	КоличествоОтраженных = 0;
	НачалоЗамера = ТекущаяУниверсальнаяДатаВМиллисекундах();
	РеглУчетПроведениеСервер.ОтразитьВсе(КонецМесяца(ПараметрыРасчета.Период), ПараметрыРасчета.Организация, КоличествоОтраженных);
	Замер = ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЗамера;
	Комментарий = НСтр("ru='Отражено %КолвоСтрок% док. за %Замер% сек.';uk='Відображено %КолвоСтрок% док. за %Замер% сек.'",Метаданные.ОсновнойЯзык.КодЯзыка);
	Комментарий = СтрЗаменить(Комментарий, "%КолвоСтрок%", КоличествоОтраженных);
	Комментарий = СтрЗаменить(Комментарий, "%Замер%", Формат(Замер/1000., "ЧДЦ=3; ЧН=; ЧГ="));
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Отражение документов в регл. учете';uk='Відображення документів у регл. обліку'",Метаданные.ОсновнойЯзык.КодЯзыка),
		УровеньЖурналаРегистрации.Информация,,, Комментарий);
КонецПроцедуры

Процедура АмортизацияОСиНМА(Отказ, ПараметрыРасчета) Экспорт
	Попытка
		Документы.АмортизацияОС.СоздатьДокументыАмортизацииОС(ПараметрыРасчета.Период, ПараметрыРасчета.Организация, Отказ);
		Документы.АмортизацияНМА.СоздатьДокументыАмортизацииНМА(ПараметрыРасчета.Период, ПараметрыРасчета.Организация, Отказ);
	Исключение
		Отказ = Истина;
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Закрытие месяца. Начисление амортизации ОС и НМА.';uk='Закриття місяця. Нарахування амортизації ОЗ і НМА.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
...
Процедура АмортизацияОСиНМАМеждународный(Отказ, ПараметрыРасчета) Экспорт
	...
	Исключение
		Отказ = Истина;
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Закрытие месяца. Начисление амортизации ОС и НМА (международный учет).';uk='Закриття місяця. Нарахування амортизації ОЗ і НМА (міжнародний облік).'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
...
Процедура НастройкаРаспределенияРасходов(Отказ, ПараметрыРасчета)
	...
	Для Каждого Статья Из СтатьиКРаспределению Цикл
		...
		Исключение
			Отказ = Истина;
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Закрытие месяца. Настройка распределения расходов.';uk='Закриття місяця. Настройка розподілу витрат.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
				УровеньЖурналаРегистрации.Ошибка,
...
Процедура ОтражениеЗарплаты(Отказ, ПараметрыРасчета)
	...
	Исключение
		Отказ = Истина;
		ЗаписьЖурналаРегистрации(
			"ОтражениеЗарплатыПриЗакрытииМесяца", 
			УровеньЖурналаРегистрации.Ошибка,
