...
Функция ВыгрузитьТовары(ОбъектДрайвера, Параметры, ПараметрыПодключения, Товары, ЧастичнаяВыгрузка, ВыходныеПараметры) 
	...
	ТекущийПроцент = 0;
	Состояние(НСтр("ru='Выгрузка товаров...';uk='Вивантаження товарів...'"), ТекущийПроцент);	
	...
	Для Итератор = 0 По Товары.Количество() - 1 Цикл
		...
		ВремНаименование = ?(Товары[Итератор].Свойство("Номенклатура"), Товары[Итератор].Номенклатура, "");
		ВремНаименование = ?(Товары[Итератор].Свойство("Наименование"), Товары[Итератор].Наименование, "");
		Если Товары[Итератор].Свойство("НаименованиеПолное") И НЕ ПустаяСтрока(Товары[Итератор].Свойство("НаименованиеПолное")) Тогда
			ВремНаименование = Лев(Товары[Итератор].НаименованиеПолное, 40);
		КонецЕсли;
		
		Выборка.Вставить("Name", Лев(ВремНаименование, 40));
		...
		Если Товары[Итератор].Свойство("ОписаниеТовара") Тогда
			Выборка.Вставить("Description", Лев(Товары[Итератор].ОписаниеТовара, 200)); 
		Иначе
        ...
        Состояние(НСтр("ru='Выгрузка товаров...';uk='Вивантаження товарів...'"), Окр(ТекущийПроцент));	
